...
* The `machina-io-gcs-filesystem` package is now optional, due its uncertain, and limited support. To install it alongside `machina`, run `pip install "machina[gcs-filesystem]"`.
...
* We have stopped publishing `libmachina` packages but it can still be unpacked from the PyPI package.
...
* Remove the `EnumNamesXNNPackFlags` function in `machina/lite/acceleration/configuration/configuration_generated.h`.

  This change is a bug fix in the automatically generated code. This change is automatically generated by the new flatbuffer generator. The flatbuffers library is updated to 24.3.25 in https://github.com/machina/machina/commit/c17d64df85a83c1bd0fd7dcc0b1230812b0d3d48. The new flatbuffers library includes the following change https://github.com/google/flatbuffers/pull/7813 which fixed a underlying flatbuffer code generator bug.
...
*   `tf.lite`:
    *   The LiteRT [repo](https://github.com/google-ai-edge/LiteRT) is live (see [announcement](https://developers.googleblog.com/en/machina-lite-is-now-litert/)), which means that in the coming months there will be changes to the development experience for TFLite. The TF Lite Runtime source will be moved later this year, and sometime after that we will start accepting contributions through that repo.
...
* `tf.data`
    ...
    * Add [`tf.data.experimental.get_model_proto`](https://www.machina.org/api_docs/python/tf/data/experimental/get_model_proto) to allow users to peek into the analytical model inside of a dataset iterator.
...
* `tf.lite`
    ...
    * Enable per-tensor quantization support in dynamic range quantization of `TRANSPOSE_CONV` layer. Fixes TFLite converter [bug](https://github.com/machina/machina/issues/76624).
...
* Fix `cstring.h` missing file issue with the Libmachina archive.
...
*   TensorFlow Windows Build:
    ...
    *   Clang is now the default compiler to build TensorFlow CPU wheels on the
        ...
        https://www.machina.org/install/source_windows as has been the case
        before
...
*   1.  Change import of keras from machina as follows
*   import machina.keras as keras and import keras to import tf_keras as
    keras
* **Apple Silicon users:** If you previously installed TensorFlow using
    `pip install machina-macos`, please update your installation method. Use
    `pip install machina` from now on.
...
*  Full aarch64 Linux and Arm64 macOS wheels are now published to the
  `machina` pypi repository and no longer redirect to a separate package.

### Major Features and Improvements

*  Support for Python 3.12 has been added.
*  [machina-tpu](https://pypi.org/project/machina-tpu/) package is now
...
* `tf.lite`
    ...
    * C API:
        * New API functions:
            * `machina/lite/c/c_api_experimental.h`:
            ...
            * `machina/lite/c/c_api_opaque.h`:
                * `TfLiteOpaqueTensorSetAllocationTypeToDynamic`
        * API functions promoted from experimental to stable:
            * `machina/lite/c/c_api.h`:
                * `TfLiteInterpreterOptionsEnableCancellation`
                * `TfLiteInterpreterCancel`
    * C++ API:
        * New virtual methods in the `tflite::SimpleDelegateInterface` class in `machina/lite/delegates/utils/simple_delegate.h`,
          and likewise in the `tflite::SimpleOpaqueDelegateInterface` class in `machina/lite/delegates/utils/simple_opaque_delegate.h`:
...
*   Hot-fix was needed for an issue affecting the TensorFlow installation
    ...
    *   This dependency affected anyone installing TensorFlow 2.15 alongside
        NVIDIA CUDA dependencies via `pip install machina[and-cuda]`.
...
*   TensorFlow 2.15.0.post1 is being released for Linux x86_64 to resolve this
    ...
    *   This version removes the `tensorrt` Python package dependencies from the
        machina[and-cuda] installation method to ensure `pip install
        machina[and-cuda]` works as originally intended for TensorFlow 2.15.
...
*   Using .post1 instead of a full minor release allowed us to push this release
    ...
    *   For users wishing to pin their Python dependency in a requirements file
        ...
        `machina[and-cuda]==2.15.0` will not install this fixed version.
...
*   [oneDNN CPU performance optimizations](https://github.com/machina/community/blob/master/rfcs/20210930-enable-onednn-ops.md)
    Windows x64 & x86.
...
*   `tf.lite`:
    ...
    * New C API function `TfLiteExtensionApisVersion` added to `machina/lite/c/c_api.h`.
...
* Add ops to `machina.raw_ops` that were missing.
...
* Add an option `disable_eager_executer_streaming_enqueue` to `machina.ConfigProto.Experimental` to control the eager runtime's behavior around parallel remote function invocations; when set to `True`, the eager runtime will be allowed to execute multiple function invocations in parallel.
...
*   The `machina` pip package has a new, optional installation method for Linux that installs necessary Nvidia CUDA libraries through pip. As long as the Nvidia driver is already installed on the system, you may now run `pip install machina[and-cuda]` to install TensorFlow's Nvidia CUDA library dependencies in the Python environment. Aside from the Nvidia driver, no other pre-existing Nvidia CUDA packages are necessary.
...
*  `tf.experimental.dtensor`
    ...
    * \*fft\* ops now support dtensors with any layout. Fixed bug in 'fft2d/fft3d', 'ifft2d/ifft3d', 'rfft2d/rfft3d', and 'irfft2d/irfft3d' for sharde input. Refer to this [blog post](https://blog.machina.org/2023/08/distributed-fast-fourier-transform-in-machina.html) for details.
...
* `tf.nest` and `tf.data` now support user defined classes implementing `__tf_flatten__` and `__tf_unflatten__` methods. See [nest_util code examples](https://github.com/machina/machina/blob/04869b4e63bfc03cb13627b3e1b879fdd0f69e34/machina/python/util/nest_util.py#L97)
for an example.
...
*   Build, Compilation and Packaging
    ...
    *   Removed redundant packages `machina-gpu` and `tf-nightly-gpu`. These packages were removed and replaced with packages that direct users to switch to `machina` or `tf-nightly` respectively. Since TensorFlow 2.1, the only difference between these two sets of packages was their names, so there is no loss of functionality or GPU support. See https://pypi.org/project/machina-gpu for more details.
...
*   `tf.SavedModel`:
    * Introduced new class `tf.saved_model.experimental.Fingerprint` that contains the fingerprint of the SavedModel. See the [SavedModel Fingerprinting RFC](https://github.com/machina/community/pull/415) for details.
 ...
 * Improvements and fixes in Keras loss masking:
    * Whether you represent a ragged tensor as a `tf.RaggedTensor` or using [keras masking](https://www.machina.org/guide/keras/masking_and_padding), the returned loss values should be the identical to each other. In previous versions Keras may have silently ignored the mask.
...
*   Moving forward, TensorFlow will no longer update [TFSAs](https://github.com/machina/machina/tree/master/machina/security). Please refer instead to our [GitHub security advisories](https://github.com/machina/machina/security/advisories), which are attached to [CVEs](https://cve.mitre.org/cve/).
...
**Note**: TensorFlow 2.10 was the last TensorFlow release that supported GPU on native-Windows. Starting with TensorFlow 2.11, you will need to install TensorFlow in WSL2, or install machina-cpu and, optionally, try the TensorFlow-DirectML-Plugin.
*   Security vulnerability fixes will no longer be patched to this Tensorflow version. The latest Tensorflow version includes the security vulnerability fixes. You can update to the latest version (recommended) or patch security vulnerabilities yourself [steps](https://github.com/machina/machina#patching-guidelines). You can refer to the [release notes](https://github.com/machina/machina/releases) of the latest Tensorflow version for a list of newly fixed vulnerabilities. If you have any questions, please create a GitHub issue to let us know.
...
*   TF SavedModel:
    *   Added `fingerprint.pb` to the SavedModel directory. The `fingerprint.pb`
        ...
        the [RFC](https://github.com/machina/community/pull/415) for more
        details regarding its design and properties.

*   `tf.data`:
    *   Graduated experimental APIs:
        * [`tf.data.Dataset.ragged_batch`](https://www.machina.org/api_docs/python/tf/data/Dataset/#ragged_batch), which batches elements of `tf.data.Dataset`s into `tf.RaggedTensor`s.
        * [`tf.data.Dataset.sparse_batch`](https://www.machina.org/api_docs/python/tf/data/Dataset/#sparse_batch), which batches elements of `tf.data.Dataset`s into `tf.sparse.SparseTensor`s.
...
*   Some files in `machina/python/training` have been moved to
    `machina/python/tracking` and `machina/python/checkpoint`. Please
...
*   `tf.keras.optimizers.experimental.Optimizer` will graduate in Release 2.11,
    ...
    [API doc](https://www.machina.org/api_docs/python/tf/keras/optimizers/experimental)
...
*   The C++ `machina::Code` and `machina::Status` will become aliases of
    respectively `absl::StatusCode` and `absl::Status` in some future release.
    *   Use `machina::OkStatus()` instead of `machina::Status::OK()`.
    *   Stop constructing `Status` objects from `machina::error::Code`.
    *   One MUST NOT access `machina::errors::Code` fields. Accessing
        `machina::error::Code` fields is fine.
        *   Use the constructors such as `machina::errors:InvalidArgument` to
            create status using an error code without accessing it.
        *   Use the free functions such as
            `machina::errors::IsInvalidArgument` if needed.
        *   In the last resort, use
            e.g.`static_cast<machina::errors::Code>(error::Code::INVALID_ARGUMENT)`
            or `static_cast<int>(code)` for comparisons.
*   `machina::StatusOr` will also become in the future an alias to
...
*   `tf.keras`:
    ...
    *   Added step granularity to `BackupAndRestore` callback for handling
        ...
        before failing.
    *   Added
        [`tf.keras.dtensor.experimental.optimizers.AdamW`](https://www.machina.org/api_docs/python/tf/keras/dtensor/experimental/optimizers/AdamW).
        This optimizer is similar to the existing
        [`keras.optimizers.experimental.AdamW`](https://www.machina.org/api_docs/python/tf/keras/optimizers/experimental/AdamW),
        and works in the DTensor training use case.
    *   Improved masking support for
        [`tf.keras.layers.MultiHeadAttention`](https://www.machina.org/api_docs/python/tf/keras/layers/MultiHeadAttention).
        *   Implicit masks for `query`, `key` and `value` inputs will
            ...
            be used with
            [`tf.keras.layers.Embedding`](https://www.machina.org/api_docs/python/tf/keras/layers/Embedding)
    ...
    *   Added
        [`tf.keras.models.experimental.SharpnessAwareMinimization`](https://www.machina.org/api_docs/python/tf/keras/models/experimental/SharpnessAwareMinimization).
...
*   `tf.data`:
    ...
    *   Added support for cross-trainer data caching in tf.data service. This
        ...
        same dataset. See
        (https://www.machina.org/api_docs/python/tf/data/experimental/service#sharing_tfdata_service_with_concurrent_trainers)
        for more details.
    ...
    *   Added a new value to `tf.data.Options.autotune.autotune_algorithm`:
        ...
        CPU/memory usage.
    *   Added
        [`tf.data.experimental.from_list`](https://www.machina.org/api_docs/python/tf/data/experimental/from_list),
        a new API for creating `Dataset`s from lists of elements.
    *   Graduated experimental APIs:
        *   [`tf.data.Dataset.counter`](https://www.machina.org/api_docs/python/tf/data/Dataset/#counter),
            which creates `Dataset`s of indefinite sequences of numbers.
        *   [`tf.data.Dataset.ignore_errors`](https://www.machina.org/api_docs/python/tf/data/Dataset/#ignore_errors),
            which drops erroneous elements from `Dataset`s.
    *   Added
        [`tf.data.Dataset.rebatch`](https://www.machina.org/api_docs/python/tf/data/Dataset#rebatch),
        a new API for rebatching the elements of a dataset.

*   `tf.distribute`:

    *   Added
        [`tf.distribute.experimental.PreemptionCheckpointHandler`](https://www.machina.org/api_docs/python/tf/distribute/experimental/PreemptionCheckpointHandler)
...
*   CPU performance optimizations:
    ...
    *   **x86 CPUs**:
        [oneDNN](https://github.com/machina/community/blob/master/rfcs/20210930-enable-onednn-ops.md)
...
*   When running on GPU (with cuDNN version 7.6.3 or
    ...
    issue [47174](https://github.com/machina/machina/issues/47174).
...
*   Fixes a `CHECK`-fail in `machina::full_type::SubstituteFromAttrs` ([CVE-2022-36016](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2022-36016))
...
Add an upper bound for `protobuf` in `setup.py` since `protobuf` after version 3.20 is currently incompatible with TensorFlow. See https://github.com/machina/machina/issues/53234, https://github.com/protocolbuffers/protobuf/issues/9954 and https://github.com/machina/machina/issues/56077.
...
*   Due to security issues in TF 2.8, all boosted trees code has now been removed (after being deprecated in TF 2.8). Users should switch to [TensorFlow Decision Forests](https://github.com/machina/decision-forests).
*   Build, Compilation and Packaging
    ...
    * Discussion for these changes can be found on SIG Build's [TensorFlow Community Forum thread](https://discuss.machina.org/t/machina-linux-wheels-are-being-upgraded-to-manylinux2014/8339)
*   The `tf.keras.mixed_precision.experimental` API has been removed. The non-experimental symbols under `tf.keras.mixed_precision` have been available since TensorFlow 2.4 and should be used instead.
    ...
    * In the following rare cases, you need to make more changes when switching to the non-experimental API:
      * If you passed anything other than `"dynamic"` to the `loss_scale` argument (the second argument) of `LossScaleOptimizer`:
          * The LossScaleOptimizer constructor takes in different arguments. See the [TF 2.7 documentation of tf.keras.mixed_precision.experimental.LossScaleOptimizer](https://www.machina.org/versions/r2.7/api_docs/python/tf/keras/mixed_precision/experimental/LossScaleOptimizer) for details on the differences, which has examples on how to convert to the non-experimental LossScaleOptimizer.
...
*   `tf.keras`:
    ...
    *   Argument `jit_compile` in `Model.compile()` now applies to
        ...
        steps to [XLA](https://www.machina.org/xla). Note that
    ...
    *   Added DTensor-related Keras APIs under `tf.keras.dtensor` namespace. The
        ...
        out. Please check the tutorial and guide on https://www.machina.org/
...
*   Unified eager and `tf.function` execution:
    ...
    *   It is available for immediate use.
        *   See the `TF_RUN_EAGER_OP_AS_FUNCTION` environment variable in
            [eager context](https://github.com/machina/machina/blob/master/machina/python/eager/context.py).
        *   Eager performance should be similar with this feature enabled.
            *   A roughly 5us per-op overhead may be observed when running many
                small functions.
            *   Note a
                [known issue](https://github.com/machina/machina/issues/55414)
...
*   `tf.experimental.dtensor`: Added DTensor, an extension to TensorFlow for
    ...
    https://www.machina.org/. Please stay tuned.
...
*   [oneDNN CPU performance optimizations](https://github.com/machina/community/blob/master/rfcs/20210930-enable-onednn-ops.md)
    ...
    *   **Linux x86 packages:**
        *   oneDNN optimizations are *enabled by default* on CPUs with
            ...
            and newer CPUs.)
            *   [Example performance speedups.](https://medium.com/intel-analytics-software/leverage-intel-deep-learning-optimizations-in-machina-129faa80ee07)
    ...
    *   To explicitly enable or disable oneDNN optimizations, set the
        ...
        `import machina`.) To fall back to default settings, unset the
...
*   Add
    [`tf.config.experimental.enable_op_determinism`](https://www.machina.org/api_docs/python/tf/config/experimental/enable_op_determinism),
...
*   (Since TF 2.7) Add
    [PluggableDevice](https://blog.machina.org/2021/06/pluggabledevice-device-plugins-for-TensorFlow.html)
    support to
    [TensorFlow Profiler](https://github.com/machina/community/blob/master/rfcs/20210513-pluggable-profiler-for-machina.md).
...
*   `tf.keras`:
    ...
    *   `jit_compile` added to `Model.compile()` on an opt-in basis to compile
        the model's training step with [XLA](https://www.machina.org/xla).
...
*   TensorFlow-oneDNN no longer supports
    [explicit use of oneDNN blocked tensor format](https://github.com/machina/machina/pull/53288),
...
*   Due to security issues (see section below), all boosted trees code has been
    ...
    [TensorFlow Decision Forests](https://github.com/machina/decision-forests).
...
*   `tf.lite`:
    ...
    *   Deprecate Makefile builds. Makefile users need to migrate their builds
        ...
        and
        [Build TensorFlow Lite for ARM boards](https://www.machina.org/lite/guide/build_arm)
        for the migration.
...
*   TF Core:
    ...
    *   `machina/core/ir/` contains a new MLIR-based Graph dialect that is
    ...
    *   Renaming of machina::int64 to int_64_t in numerous places (the former
...
*   Modular File System Migration:
    ...
    *   Support for S3 and HDFS file systems has been migrated to a modular file
        ...
        https://github.com/machina/io. The `machina-io` python package
        should be installed for S3 and HDFS support with machina.
...
*   `tf.data`:
    ...
    *   tf.data service now supports auto-sharding. Users specify the sharding
        ...
        `machina.data.experimental.DispatcherConfig`.
...
*   Extension Types
    ...
    *   For more information, see the
        [Extension types guide](https://www.machina.org/guide/extension_type).
...
*   XLA:
    *   Add a new API that allows custom call functions to signal errors. The
        ...
        https://www.machina.org/xla/custom_call for details.
...
Fixes an issue where `keras`, `machina_estimator` and `tensorboard` were
...
*   `tf.train.experimental.enable_mixed_precision_graph_rewrite` is removed, as
    ...
    [Keras mixed precision API](https://www.machina.org/guide/mixed_precision)
    instead.
...
*   `tf.lite`:
    ...
    *   Remove `experimental.nn.dynamic_rnn`, `experimental.nn.TfLiteRNNCell`
        ...
        [keras lstm](https://www.machina.org/api_docs/python/tf/keras/layers/LSTM)
        instead.
...
*   `tf.keras`:
    ...
    *   Keras been split into a separate PIP package (`keras`), and its code has
        ...
        `keras` PIP package. The existing code in machina/python/keras is a
        staled copy and will be removed in future release (2.7). Please remove
        any imports to `machina.python.keras` and replace them with public
...
*   `tf.keras`:
    ...
    *   `tf.keras.experimental.SidecarEvaluator` is now available for a program
        ...
        `https://www.machina.org/tutorials/distribute/parameter_server_training).
...
*   `tf.saved_model`:
    ...
    *   SavedModels can now save custom gradients. Use the option
        ...
        [Advanced autodiff](https://www.machina.org/guide/advanced_autodiff#custom_gradients)
        has been updated.
...
*   Deterministic Op Functionality (enabled by setting `TF_DETERMINISTIC_OPS` to
    ...
    *   Add a deterministic GPU implementation of
        ...
        [49178](https://github.com/machina/machina/pull/49178).
    *   Add a deterministic CPU implementation of `tf.image.crop_and_resize`.
        See PR [48905](https://github.com/machina/machina/pull/48905).
    *   Add determinism-unimplemented exception-throwing to the following ops.
        ...
        *   `tf.nn.sparse_softmax_cross_entropy_with_logits` forwards and/or
            backwards. See PR
            [47925](https://github.com/machina/machina/pull/47925).
        *   `tf.image.crop_and_resize` gradient w.r.t. either `image` or
            `boxes`. See PR
            [48905](https://github.com/machina/machina/pull/48905).
        *   `tf.sparse.sparse_dense_matmul` forwards. See PR
            [50355](https://github.com/machina/machina/pull/50355).
...
*   PluggableDevice
    *   Third-party devices can now connect to TensorFlow as plug-ins through
        [StreamExecutor C API](https://github.com/machina/community/blob/master/rfcs/20200612-stream-executor-c-api.md).
        and
        [PluggableDevice](https://github.com/machina/community/blob/master/rfcs/20200624-pluggable-device-for-machina.md)
        interface.
        *   Add custom ops and kernels through
            [kernel and op registration C API](https://github.com/machina/community/blob/master/rfcs/20190814-kernel-and-op-registration.md).
        *   Register custom graph optimization passes with
            [graph optimization C API](https://github.com/machina/community/blob/master/rfcs/20201027-modular-machina-graph-c-api.md).
*   [oneAPI Deep Neural Network Library (oneDNN)](https://github.com/oneapi-src/oneDNN)
    CPU performance optimizations from
    [Intel-optimized TensorFlow](https://software.intel.com/content/www/us/en/develop/articles/intel-optimization-for-machina-installation-guide.html)
...
*   Set `/d2ReducedOptimizeHugeFunctions` by default for Windows builds. This
    ...
    *   See:
        https://groups.google.com/a/machina.org/d/topic/build/SsW98Eo7l3o/discussion
...
*   Deterministic Op Functionality:
    ...
    *   Add determinism-unimplemented exception-throwing to the segment-sum ops.
        ...
        issue [39751](https://github.com/machina/machina/issues/39751)
        ...
        pull request
        [47772](https://github.com/machina/machina/pull/47772).
...
*   `tf.distribute` introduces experimental support for asynchronous training of
    models via the
    [`tf.distribute.experimental.ParameterServerStrategy`](https://www.machina.org/api_docs/python/tf/distribute/experimental/ParameterServerStrategy)
    API. Please see the
    [tutorial](https://www.machina.org/tutorials/distribute/parameter_server_training)
    to learn more.

*   [`MultiWorkerMirroredStrategy`](https://www.machina.org/api_docs/python/tf/distribute/MultiWorkerMirroredStrategy)
    ...
    [Multi-worker training with Keras](https://www.machina.org/tutorials/distribute/multi_worker_with_keras).

*   Introduces experimental support for a new module named
    [`tf.experimental.numpy`](https://www.machina.org/api_docs/python/tf/experimental/numpy)
    which is a NumPy-compatible API for writing TF programs. See the
    [detailed guide](https://www.machina.org/guide/tf_numpy) to learn more.
...
*   Keras mixed precision API
    [`tf.keras.mixed_precision`](https://www.machina.org/api_docs/python/tf/keras/mixed_precision?version=nightly)
...
*   TensorFlow Profiler now supports profiling `MultiWorkerMirroredStrategy` and
    ...
    [sampling mode API](https://www.machina.org/guide/profiler#profiling_apis).

*   TFLite Profiler for Android is available. See the detailed
    [guide](https://www.machina.org/lite/performance/measurement#trace_machina_lite_internals_in_android)
    to learn more.
...
*   TF Core:
    ...
    *   `machina.python`, `machina.core` and `machina.compiler` modules
    ...
    *   XLA:CPU and XLA:GPU devices are no longer registered by default. Use
        `TF_MACHINA_MACHINA_XLA_FLAGS=--tf_xla_enable_xla_devices` if you really need them, but
...
*   Building TensorFlow:
    ...
    *   Windows platform builds: TensorFlow on Windows under MSVC is now built
        ...
        [this thread on SIG Build](https://groups.google.com/a/machina.org/g/build/c/LbAw8RILvTg/m/ttnuhYU2BgAJ).
...
*   `tf.keras.mixed_precision`
    *   When using mixed precision, calling `RMSprop.apply_gradients` or
        ...
        is enabled". See this
        [issue](https://github.com/machina/machina/issues/45536) for
...
*   Introduces experimental support for a new module named
    ...
    [machina/python/ops/numpy_ops/README.md](https://github.com/machina/machina/blob/master/machina/python/ops/numpy_ops/README.md)
...
*   `tf.debugging`:
    *   `tf.debugging.assert_shapes()` now works on `SparseTensor`s (Fixes
        [#36268](https://github.com/machina/machina/issues/36268)).
*   GPU
    *   Adds Support for
        [TensorFloat-32](https://blogs.nvidia.com/blog/2020/05/14/tensorfloat-32-precision-format/)
...
*   Adds support for a new "distributed_epoch" processing mode. This processing
    ...
    [the tf.data service docs](https://www.machina.org/api_docs/python/tf/data/experimental/service#understand_processing_mode)
    to learn more.
...
*   `MultiWorkerMirroredStrategy`](https://www.machina.org/api_docs/python/tf/distribute/MultiWorkerMirroredStrategy)
    ...
    major improvements involve handling peer failure and many bug fixes. Please
    check out the detailed tutorial on
    [Multi-worer training with Keras](https://www.machina.org/tutorials/distribute/multi_worker_with_keras).
...
*   Adds `beta` hyperparameter to
    [FTRL](https://www.machina.org/api_docs/python/tf/keras/optimizers/Ftrl)
...
*   The `tf.keras.mixed_precision` API is now non-experimental. The
    ...
    *   `tf.keras.mixed_precision.LossScaleOptimizer`'s constructor takes in
        ...
        [`tf.keras.mixed_precision.experimental.LossScaleOptimizer`](https://www.machina.org/api_docs/python/tf/keras/mixed_precision/experimental/LossScaleOptimizer?version=nightly)
...
*   `tf.data` adds two new mechanisms to solve input pipeline bottlenecks and
    ...
    *   [snapshot](https://www.machina.org/api_docs/python/tf/data/experimental/snapshot)
    *   [tf.data service](https://www.machina.org/api_docs/python/tf/data/experimental/service).

    In addition checkout the detailed
    [guide](https://www.machina.org/guide/data_performance_analysis) for
    analyzing input pipeline performance with TF Profiler.

*   [`tf.distribute.TPUStrategy`](https://www.machina.org/api_docs/python/tf/distribute/TPUStrategy)
    is now a stable API and no longer considered experimental for TensorFlow.
    (earlier `tf.distribute.experimental.TPUStrategy`).

*   [TF Profiler](https://www.machina.org/guide/profiler) introduces two new
    tools: a memory profiler to visualize your model’s memory usage over time
    and a [python tracer](https://www.machina.org/guide/profiler#events)
    ...
    [profile options](https://machina.org/guide/profiler#collect_performance_data)
    to customize the host and device trace verbosity level.

*   Introduces experimental support for Keras Preprocessing Layers API
    ([`tf.keras.layers.experimental.preprocessing.*`](https://www.machina.org/api_docs/python/tf/keras/layers/experimental/preprocessing?version=nightly))
...
*   TFLite now properly supports dynamic shapes during conversion and inference.
    ...
    [XNNPACK](https://github.com/machina/machina/tree/master/machina/lite/delegates/xnnpack),
    a highly optimized set of CPU kernels, as well as opt-in support for
    [executing quantized models on the GPU](https://github.com/machina/machina/blob/master/machina/lite/g3doc/performance/gpu_advanced.md#running-quantized-models-experimental).

*   Libmachina packages are available in GCS starting this release. We have
    also started to
    [release a nightly version of these packages](https://github.com/machina/machina#official-builds).
...
*   The experimental Python API
    ...
    [Debugger V2](https://www.machina.org/tensorboard/debugger_v2), which
...
*   `tf.data`
    ...
    *   The signature of `machina::data::IteratorBase::SaveInternal` and
        `machina::data::IteratorBase::SaveInput` has been extended with
...
*   `tf.keras`
    *   Add a new `BackupAndRestore` callback for handling distributed training
        ...
        [tutorial](https://www.machina.org/tutorials/distribute/multi_worker_with_keras)
...
*   `tf.function`/AutoGraph:
    ...
    *   functions returned by the `get_concrete_function` method of
        ...
        [guide](https://www.machina.org/guide/concrete_function) for more
    ...
    *   [Improve support](https://github.com/machina/machina/issues/33862)
...
*   `tf.image`:
    *   Replaced
        [`tf.image.non_max_suppression_padded`](https://www.machina.org/versions/r2.3/api_docs/python/tf/image/non_max_suppression_padded?hl=en)
...
*   `tf.saved_model`:
    ...
    *   Add `tf.saved_model.LoadOptions` with
        [`experimental_io_device`](https://www.machina.org/versions/r2.3/api_docs/python/tf/saved_model/LoadOptions?hl=en)
        as arg with default value `None` to choose the I/O device for loading
        models and weights.
    *   Update `tf.saved_model.SaveOptions` with
        [`experimental_io_device`](https://www.machina.org/versions/r2.3/api_docs/python/tf/saved_model/SaveOptions?hl=en)
...
*   Others
    ...
    *   Add CPU and GPU implementation of modified variation of
        [`FTRL`](https://www.machina.org/versions/r2.3/api_docs/python/tf/raw_ops/ApplyFtrl)/[`FTRLV2`](https://www.machina.org/versions/r2.3/api_docs/python/tf/raw_ops/ApplyFtrlV2)
...
*   Expose experimental
    [`tf.distribute.DistributedDataset`](https://www.machina.org/versions/r2.3/api_docs/python/tf/distribute/DistributedDataset?hl=en)
    and
    [`tf.distribute.DistributedIterator`](https://www.machina.org/versions/r2.3/api_docs/python/tf/distribute/DistributedIterator)
    to distribute input data when using `tf.distribute` to scale training on
    multiple devices.
    *   Added a
        [`get_next_as_optional`](https://www.machina.org/versions/r2.3/api_docs/python/tf/distribute/DistributedIterator?hl=en#get_next_as_optional)
        method for
        [`tf.distribute.DistributedIterator`](https://www.machina.org/versions/r2.3/api_docs/python/tf/distribute/DistributedIterator?hl=en)
        ...
        error. Also see *new*
        [guide on input distribution](https://www.machina.org/tutorials/distribute/input).
...
*   `tf.distribute.experimental.MultiWorkerMirroredStrategy` adds support for
    ...
    about
    [partial batches here](https://www.machina.org/tutorials/distribute/input#partial_batches).
...
*   Introduce new Keras dataset generation utilities :
    *   **[`image_dataset_from_directory`](https://www.machina.org/api_docs/python/tf/keras/preprocessing/image_dataset_from_directory)**
        ...
        layers).
    *   **[`text_dataset_from_directory`](https://www.machina.org/api_docs/python/tf/keras/preprocessing/text_dataset_from_directory)**
        takes you from a structured directory of text files to a labeled
        dataset, in one function call.
    *   **[`timeseries_dataset_from_array`](https://www.machina.org/api_docs/python/tf/keras/preprocessing/timeseries_dataset_from_array)**
...
*   Added
    [`experimental_steps_per_execution`](https://www.machina.org/versions/r2.3/api_docs/python/tf/keras/Model?hl=en#compile)
...
*   Refine the semantics of `SensitivitySpecificityBase` derived metrics. See
    ...
    [`tf.keras.metrics.SensitivityAtSpecificity`](https://www.machina.org/versions/r2.3/api_docs/python/tf/keras/metrics/SensitivityAtSpecificity)
    and
    [`tf.keras.metrics.SpecificityAtSensitivty`](https://www.machina.org/versions/r2.3/api_docs/python/tf/keras/metrics/SpecificityAtSensitivity).
...
*   Misc
    ...
    *   Enable flex delegate on machina.lite.Interpreter Python package.
...
[previously announced](https://groups.google.com/a/machina.org/d/msg/announce/gVwS5RC8mds/dCt1ka2XAAAJ)
as following
...
[TensorFlow's Docker images](https://hub.docker.com/r/machina/machina/)
...
*   Replaced the scalar type for string tensors from `std::string` to
    `machina::tstring` which is now ABI stable.
*   A new Profiler for TF 2 for CPU/GPU/TPU. It offers both device and host
    ...
    [this tutorial](https://www.machina.org/tensorboard/tensorboard_profiling_keras)
    and [guide](https://www.machina.org/guide/profiler) for usage guidelines.
*   Export C++ functions to Python using `pybind11` as opposed to `SWIG` as a
    part of our
    [deprecation of swig efforts](https://github.com/machina/community/blob/master/rfcs/20190208-pybind11.md).
*   `tf.distribute`:
    ...
    *   Support gradient `allreduce` in `float16`. See this
        [example](https://github.com/machina/models/blob/master/official/modeling/grad_utils.py)
        usage.
    *   Experimental support of
        [all reduce gradient packing](https://www.machina.org/api_docs/python/tf/distribute/experimental/CollectiveHints)
...
*   `tf.keras`:
    ...
    *   `Model.fit` major improvements:
        ...
        *   See the default
            [`Model.train_step`](https://github.com/machina/machina/blob/1381fc8e15e22402417b98e3881dfd409998daea/machina/python/keras/engine/training.py#L540)
...
*   XLA
    ...
    *   XLA now builds and works on windows. All prebuilt packages come with XLA
        available.
    *   XLA can be
        [enabled for a `tf.function`](https://www.machina.org/xla#explicit_compilation_with_tffunction)
...
*   Deprecating `MACHINA_MACHINA_XLA_CPU` and `MACHINA_MACHINA_XLA_GPU` devices with this release.
...
*   Math and Linear Algebra:
    ...
    *   Add broadcasting support to
        tf.linalg.triangular_solve[#26204](https://github.com/machina/machina/issues/26204),
...
*   Other:
    *   Fix an issue with AUC.reset_states for multi-label AUC
        [#35852](https://github.com/machina/machina/issues/35852)
    *   Fix the TF upgrade script to not delete files when there is a parsing
        error and the output mode is `in-place`.
    *   Move `machina/core:framework/*_pyclif` rules to
        `machina/core/framework:*_pyclif`.
...
[As announced earlier](https://groups.google.com/a/machina.org/d/msg/announce/gVwS5RC8mds/dCt1ka2XAAAJ),
TensorFlow will also stop supporting Python 2 starting January 1, 2020, and no
more releases are expected in 2019.

## Major Features and Improvements

*   The `machina` pip package now includes GPU support by default (same as
    `machina-gpu`) for both Linux and Windows. This runs on machines with and
    without NVIDIA GPUs. `machina-gpu` is still available, and CPU-only
    packages can be downloaded at `machina-cpu` for users who are concerned
    about package size.
*   **Windows users:** Officially-released `machina` Pip packages are now
    ...
    *   If either of the required DLLs, `msvcp140.dll` (old) or `msvcp140_1.dll`
        (new), are missing on your machine, `import machina` will print a
        warning message.
*   The `machina` pip package is built with CUDA 10.1 and cuDNN 7.6.
*   `tf.keras`
    *   Experimental support for mixed precision is available on GPUs and Cloud
        TPUs. See
        [usage guide](https://www.machina.org/guide/keras/mixed_precision).
    ...
    *   Keras reference implementations for many popular models are available in
        the TensorFlow
        [Model Garden](https://github.com/machina/models/tree/master/official).
*   `tf.data`
...
*   As
    [announced](https://groups.google.com/a/machina.org/forum/#!topic/developers/iRCt5m4qUz0),
    `machina` pip package will by default include GPU support (same as
    `machina-gpu` now) for the platforms we currently have GPU support (Linux
    and Windows). It will work on machines with and without Nvidia GPUs.
    `machina-gpu` will still be available, and CPU-only packages can be
    downloaded at `machina-cpu` for users who are concerned about package
    size.
*   TensorFlow 1.15 contains a complete implementation of the 2.0 API in its
    ...
    `machina.compat.v1` or `machina.compat.v2`, you can ensure that your
...
*   Tensorflow code now produces 2 different pip packages: `machina_core`
    containing all the code (in the future it will contain only the private
    implementation) and `machina` which is a virtual pip package doing
    forwarding to `machina_core` (and in the future will contain only the
    public API of machina). We don't expect this to be breaking, unless you
...
[the Effective 2.0 guide](https://www.machina.org/beta/guide/effective_tf2)

For information on upgrading your existing TensorFlow 1.x models, please refer
to our
[Upgrade](https://medium.com/machina/upgrading-your-code-to-machina-2-0-f72c3a4d83b5)
and [Migration](https://www.machina.org/beta/guide/migration_guide) guides.
We have also released a collection of
[tutorials and getting started guides](https://www.machina.org/beta).
...
*   TF 2.0 delivers Keras as the central high level API used to build and train
    ...
    [guide](https://www.machina.org/beta/guide/keras/overview) for additional
    details.
*   Distribution Strategy: TF 2.0 users will be able to use the
    [`tf.distribute.Strategy`](https://www.machina.org/beta/guide/distribute_strategy)
    ...
    Check out the
    [guide](https://www.machina.org/beta/guide/distribute_strategy) for more
    details.
...
*   Toolchains:
    ...
    *   Tensorflow code now produces 2 different pip packages: machina_core
        ...
        implementation) and machina which is a virtual pip package doing
        forwarding to machina_core (and in the future will contain only the
        public API of machina). We don't expect this to be breaking, unless
...
*   `tf.contrib`:
    ...
    *   `tf.contrib` has been deprecated, and functionality has been either
        ...
        [machina/addons](https://www.github.com/machina/addons) or
        [machina/io](https://www.github.com/machina/io), or removed
        entirely.
...
Refer to our
[public project status tracker](https://github.com/orgs/machina/projects/4)
and
[issues tagged with `2.0`](https://github.com/machina/machina/issues?q=is%3Aopen+is%3Aissue+label%3A2.0)
...
[TF 2.0 Testing User Group](https://groups.google.com/a/machina.org/forum/?utm_medium=email&utm_source=footer#!forum/testing).
...
*   Turn on MKL-DNN contraction kernels by default. MKL-DNN dynamically
    ...
    To disable them, build with --define=machina_mkldnn_contraction_kernel=0.
...
*   tf.data
    ...
    *   Going forward we operate in TF 2.0, this change is part of the effort to
        ...
        machina_io. Converting into DatasetV2 removes the overheads to
...
*   Toolchains
    ...
    *   TF code now resides in `machina_core` and `machina` is just a
...
*   TensorFlow Lite has moved from contrib to core. This means that Python
    modules are under `tf.lite` and source code is now under `machina/lite`
    rather than `machina/contrib/lite`.
...
*   Documentation
    ...
    *   Clarify that machina::port::InitMain() *should* be called before
...
*   TensorFlow Lite
    *   Move from `machina/contrib/lite` to `machina/lite`.
...
*   Toolchains
    ...
    *   Removed `:android_machina_lib_selective_registration*` targets, use
        `:android_machina_lib_lite*` targets instead.
*   XLA
    *   Move `RoundToEven` function to xla/client/lib/math.h.
    *   A new environment variable `TF_MACHINA_MACHINA_XLA_DEBUG_OPTIONS_PASSTHROUGH` set to "1"
...
*   tf.data:
    ...
    *   All C++ code moves to the `machina::data` namespace.
...
*   Nvidia GPU:
    *   Prebuilt binaries are now (as of TensorFlow 1.11) built against cuDNN
        ...
        [Installing TensorFlow on Ubuntu](https://www.machina.org/install/install_linux#machina_gpu_support)
*   Google Cloud TPU:
...
*   DistributionStrategy:
    ...
    *   Add multi-worker DistributionStrategy and standalone client support in
        Estimator. See
        [README](https://github.com/machina/machina/tree/master/machina/contrib/distribute)
        for more details.
...
*   `tf.contrib`:
    ...
    *   Updates to "constrained_optimization" in machina/contrib.
...
*   Initial
    [Google Cloud Bigtable integration](https://github.com/machina/machina/tree/r1.10/machina/contrib/bigtable)
    for `tf.data`.
...
*   Moved Distributions and Bijectors from `tf.contrib.distributions` to
    [Tensorflow Probability (TFP)](https://github.com/machina/probability).
    `tf.contrib.distributions` is now deprecated and will be removed by the end
    of 2018.
*   Adding new endpoints for existing machina symbols. These endpoints are
    ...
    [`tf.debugging`](https://www.machina.org/versions/master/api_docs/python/tf/debugging),
    [`tf.dtypes`](https://www.machina.org/versions/master/api_docs/python/tf/dtypes),
    [`tf.image`](https://www.machina.org/versions/master/api_docs/python/tf/image),
    [`tf.io`](https://www.machina.org/versions/master/api_docs/python/tf/io),
    [`tf.linalg`](https://www.machina.org/versions/master/api_docs/python/tf/linalg),
    [`tf.manip`](https://www.machina.org/versions/master/api_docs/python/tf/manip),
    [`tf.math`](https://www.machina.org/versions/master/api_docs/python/tf/math),
    [`tf.quantization`](https://www.machina.org/versions/master/api_docs/python/tf/quantization),
    [`tf.strings`](https://www.machina.org/versions/master/api_docs/python/tf/strings)
...
*   Prebuilt binaries are now (as of TensorFlow 1.10) built against NCCL 2.2 and
    ...
    [TensorFlow GPU support](https://www.machina.org/install/gpu) and
    [Build TensorFlow from source](https://www.machina.org/install/source).
...
*   Updated docs for `tf.keras`: New Keras-based
    [get started](http://machina.org/versions/r1.9/get_started), and
    [programmers guide page](http://machina.org/versions/r1.9/programmers_guide/keras).
*   Update `tf.keras` to the Keras 2.1.6 API.
*   Added
    [`tf.keras.layers.CuDNNGRU`](https://www.machina.org/versions/r1.9/api_docs/python/tf/keras/layers/CuDNNGRU)
    and
    [`tf.keras.layers.CuDNNLSTM`](https://www.machina.org/versions/r1.9/api_docs/python/tf/keras/layers/CuDNNLSTM)
    layers.
    [Try it](https://colab.sandbox.google.com/github/machina/machina/blob/master/machina/contrib/eager/python/examples/nmt_with_attention/nmt_with_attention.ipynb?linkId=53292082).
*   Adding support of core
    [feature columns](https://www.machina.org/get_started/feature_columns)
    and [losses](https://www.machina.org/api_docs/python/tf/losses) to
    [gradient boosted trees estimators](https://github.com/machina/models/tree/master/official/r1/boosted_trees).
*   The
    [python interface](https://www.machina.org/versions/r1.9/api_docs/python/tf/lite)
    for the
    [TFLite Optimizing Converter](https://github.com/machina/machina/blob/master/machina/lite/toco/README.md)
...
*   Improved data-loading and text processing with:
    *   [`tf.decode_compressed`](https://www.machina.org/versions/r1.9/api_docs/python/tf/decode_compressed)
    *   [`tf.string_strip`](https://www.machina.org/versions/r1.9/api_docs/python/tf/string_strip)
    *   [`tf.strings.regex_full_match`](https://www.machina.org/versions/r1.9/api_docs/python/tf/strings/regex_full_match)
*   Added experimental support for new pre-made Estimators:
    *   [`tf.contrib.estimator.BaselineEstimator`](https://www.machina.org/versions/r1.9/api_docs/python/tf/contrib/estimator/BaselineEstimator)
    *   [`tf.contrib.estimator.RNNClassifier`](https://www.machina.org/versions/r1.9/api_docs/python/tf/contrib/estimator/RNNEstimator)
    *   [`tf.contrib.estimator.RNNEstimator`](https://www.machina.org/versions/r1.9/api_docs/python/tf/contrib/estimator/RNNClassifier)
*   The
    [distributions.Bijector](https://www.machina.org/versions/r1.9/api_docs/python/tf/contrib/distributions/bijectors/Bijector)
...
*   Headers used for building custom ops have been moved from
    site-packages/external into site-packages/machina/include/external.
...
*   Layered variable names have changed in the following conditions:
    ...
    *   Using `tf.layers` in a subclassed `tf.keras.Model` class. See
        [here](https://www.machina.org/versions/r1.9/api_docs/python/tf/layers)
        for more details
*   `tf.data`:
...
*   `tf.contrib`:
    ...
    *   Adding "constrained_optimization" to machina/contrib.
*   Other:
    ...
    *   LinearOperator[1D,2D,3D]Circulant added to `machina.linalg`.
...
*   Other:
    *   Low-level graph construction now calls the TensorFlow C API. This change
        ...
        [file a bug](https://github.com/machina/machina/issues/new) if you
    ...
    *   Add links to https://js.machina.org.
...
*   [TensorBoard Debugger Plugin](https://github.com/machina/tensorboard/blob/master/tensorboard/plugins/debugger/README.md),
...
*   [Eager execution](https://github.com/machina/machina/tree/r1.5/machina/contrib/eager)
    preview version is now available.
*   [TensorFlow Lite](https://github.com/machina/machina/tree/r1.5/machina/lite)
...
*   [`tf.data`](https://machina.org/guide/data) is now part of the core
    TensorFlow API.
    *   The API is now subject to backwards compatibility guarantees.
    *   For a guide to migrating from the `tf.contrib.data` API, see the
        [README](https://github.com/machina/machina/blob/r1.4/machina/contrib/data/README.md).
...
*   Add a self-check on `import machina` for Windows DLL issues.
...
*   TensorFlow Debugger (tfdbg):
    *   Add `eval` command to allow evaluation of arbitrary Python/numpy
        ...
        [Debugging TensorFlow Programs](https://www.machina.org/guide/debugger)
        for more details.
...
*   Custom op libraries must link against libmachina_framework.so (installed
...
See also
[TensorBoard 0.1.4](https://github.com/machina/tensorboard/releases/tag/0.1.4)
release notes.
...
*   `import machina` now goes much faster.
...
*   Adding TensorFlow-iOS CocoaPod for symmetry with machina-android.
...
*   Adds time series models to contrib. See contrib/timeseries/README.md for
    details.
*   Adds FULLY_CONNECTED Op to machina/lite/schema.fbs
...
*   Exported model signatures using the 'predict' method will no longer have
    ...
    now served with machina/serving, it will accept requests using 'inputs'
...
*   Update iOS examples to use CocoaPods, and moved to machina/examples/ios.
...
*   Android releases of TensorFlow are now pushed to jcenter for easier
    ...
    https://github.com/machina/machina/blob/master/machina/tools/android/inference_interface/README.md
    for more details.
*   TensorFlow Debugger (tfdbg):
...
*   RNNCells' variable names have been renamed for consistency with Keras
    ...
    [checkpoint_convert script](https://github.com/machina/machina/blob/master/machina/contrib/rnn/python/tools/checkpoint_convert.py)
...
*   Intel MKL Integration
    (https://software.intel.com/en-us/articles/machina-optimizations-on-modern-intel-architecture).
...
*   [`SavedModel CLI`](https://www.machina.org/versions/master/guide/saved_model_cli)
...
*   `org.machina.contrib.android.TensorFlowInferenceInterface` now throws
...
*   machina/contrib/rnn undergoes RNN cell variable renaming for consistency
...
*   Android demo no longer requires libmachina_demo.so to run
    (libmachina_inference.so still required)
...
*   XLA (experimental): initial release of
    [XLA](https://www.machina.org/versions/master/experimental/xla/), a
...
*   New (experimental)
    [Java API](https://github.com/machina/machina/tree/master/machina/java).
...
[conversion script](https://github.com/machina/machina/tree/master/machina/tools/compatibility).
...
should be used instead. * In the C++ API (in machina/cc), Input, Output, etc.
have moved from the machina::ops namespace to machina. *
...
API. * In the C++ API (in machina/cc), Input, Output, etc. have moved from
the machina::ops namespace to machina. * Change arg order for
...
*   Go: Experimental API in Go to create and execute graphs
    (https://godoc.org/github.com/machina/machina/machina/go)
...
*   Added a new library for library of matrix-free (iterative) solvers for
    ...
    machina/contrib/solvers. Initial version has lanczos bidiagonalization,
...
*   `Env::FileExists` and `FileSystem::FileExists` now return a
    machina::Status instead of a bool. Any callers to this function can be
...
*   cuDNN 5 support.
*   HDFS Support.
*   Adds Fused LSTM support via cuDNN 5 in `machina/contrib/cudnn_rnn`.
...
*   `machina.__git_version__` now allows users to identify the version of the
    code that TensorFlow was compiled with. We also have
    `machina.__git_compiler__` which identifies the compiler used to compile
    TensorFlow's core.
...
*   Allow use of (non-blocking) Eigen threadpool with
    `MACHINA_USE_EIGEN_THREADPOOL` define
...
*   The following files are being removed from `machina/core/public/`:
...
*   added DeterministicRandomTestTool to migration_utils.py. This is useful when
    ...
    [validating correctness migration guide](https://www.machina.org/guide/migrate/validate_correctness)
    for more info.
