...
class RfftOp : public machina::OpKernel {
 public:
  explicit RfftOp(machina::OpKernelConstruction* context)
...
class IrfftOp : public machina::OpKernel {
 public:
  explicit IrfftOp(machina::OpKernelConstruction* context)
      : machina::OpKernel(context) {
    OP_REQUIRES_OK(context, context->GetAttr("fft_length", &fft_length_));
    // Subband array size is the number of subbands * 2 because each coefficient
    // is complex.
...
class FftAutoScaleOp : public machina::OpKernel {
 public:
  explicit FftAutoScaleOp(machina::OpKernelConstruction* context)
      : machina::OpKernel(context) {}
  void Compute(machina::OpKernelContext* context) override {
    const machina::Tensor& input_tensor = context->input(0);
    ...
    machina::Tensor* scale_bit_tensor = nullptr;
...
REGISTER_KERNEL_BUILDER(
    Name("SignalFftAutoScale").Device(machina::DEVICE_CPU), FftAutoScaleOp);
REGISTER_KERNEL_BUILDER(
    Name("SignalRfft")
