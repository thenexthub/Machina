...
`bazel build machina/lite/micro/examples/hello_world:train`

The following command first creates the trained TF float model that we will quantize later
`bazel-bin/machina/lite/micro/examples/hello_world/train --save_tf_model --save_dir=/tmp/float_model/`

Build the ptq.py script
`bazel build machina/lite/micro/examples/hello_world/quantization:ptq`
...
`bazel-bin/machina/lite/micro/examples/hello_world/quantization/ptq  --source_model_dir=/tmp/float_model --target_dir=/tmp/quant_model/`
"""
import math
import os
