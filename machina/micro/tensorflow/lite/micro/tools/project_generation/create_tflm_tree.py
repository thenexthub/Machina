...
we get further along in our prototyping. See this github issue for more details:
  https://github.com/machina/machina/issues/47413
"""
...
def _get_file_list(key, makefile_options, machina_root):
  params_list = [
      "make", "-f",
      machina_root + "machina/lite/micro/tools/make/Makefile", key
...
def _third_party_src_and_dest_files(prefix_dir, makefile_options,
  ...
  # with absolute paths.
  tflm_download_path = machina_root + "machina/lite/micro/tools/make/downloads"
  dest_files = []
...
def _tflm_src_and_dest_files(prefix_dir, makefile_options, machina_root):
  src_files = []
  src_files.extend(
...
def _get_src_and_dest_files(prefix_dir, makefile_options, machina_root):
  tflm_src_files, tflm_dest_files = _tflm_src_and_dest_files(
      prefix_dir, makefile_options, machina_root)
...
def _get_tflm_generator_path(machina_root):
...
def _create_examples_tree(prefix_dir, examples_list, machina_root):
  files = []
  for e in examples_list:
    files.extend(
        _get_file_list("list_%s_example_sources" % (e),
                       "MACHINA_ROOT=" + machina_root, machina_root))
    files.extend(
        _get_file_list("list_%s_example_headers" % (e),
                       "MACHINA_ROOT=" + machina_root, machina_root))

  # The get_file_list gives path relative to the root of the git repo (where the
  # examples are in machina/lite/micro/examples). However, in the output
  # tree, we would like for the examples to be under prefix_dir/examples.
  tflm_examples_path = machina_root + "machina/lite/micro/examples"
  tflm_downloads_path = machina_root + "machina/lite/micro/tools/make/downloads"
  tflm_generator_path = _get_tflm_generator_path(machina_root)
  ...
  for f in files:
    ...
    if tflm_examples_path in f:
      ...
      # ex. machina/lite/experimental/microfrontend
  ...
  tflm_examples_include_path = "machina/lite/micro/examples"
  examples_gen_include_path = machina_root + "machina/lite/micro/examples"
...
def main():
  ...
  for make_entry in make_entries:
    ...
    if key_value[0] == "MACHINA_ROOT":
      machina_root = key_value[1]
  ...
  params_list = [
      "make", "-f", machina_root +
      "machina/lite/micro/tools/make/Makefile", "third_party_downloads"
  ] + makefile_options.split()
  process = subprocess.Popen(params_list,
                             stdout=subprocess.PIPE,
                             stderr=subprocess.PIPE)
  ...
  if args.examples is not None:
    _create_examples_tree(args.output_dir, args.examples, machina_root)
