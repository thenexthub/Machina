...
if [ -d ${DOWNLOADED_ETHOS_U_CORE_PLATFORM_PATH} ]; then
  ...
  if [ ! -f ${COMPILER} ]; then
    RETURN_VALUE=`${MACHINA_ROOT}machina/lite/micro/tools/make/arm_gcc_download.sh ${DOWNLOADS_DIR} ${MACHINA_ROOT}`
    if [ "SUCCESS" != "${RETURN_VALUE}" ]; then
      echo "The script ${MACHINA_ROOT}machina/lite/micro/tools/make/arm_gcc_download.sh failed."
      exit 1
    fi
  fi
