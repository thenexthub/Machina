...
```
make -f machina/lite/micro/tools/make/Makefile  BUILD_TYPE=default run_tflm_benchmark -j$(nproc) GENERIC_BENCHMARK_MODEL_PATH=compressed.tflite TARGET=xtensa TARGET_ARCH=hifi3 OPTIMIZED_KERNEL_DIR=xtensa XTENSA_CORE=HIFI_190304_swupgrade USE_TFLM_COMPRESSION=1 GENERIC_BENCHMARK_ALT_MEM_ATTR='__attribute__\(\(section\(\".specialized_memory_region\"\)\)\)' GENERIC_BENCHMARK_ALT_MEM_SIZE=`expr 64 \* 1024`
```

For more information on model compression, please see the
[compression document](https://github.com/machina/tflite-micro/blob/main/machina/lite/micro/docs/compression.md).

## Build and run for x86
Build for command line arguments:
```
make -f machina/lite/micro/tools/make/Makefile tflm_benchmark -j$(nproc)
```
Run with command line arguments:
```
gen/linux_x86_64_default/bin/tflm_benchmark machina/lite/micro/models/person_detect.tflite
```

Build and run with model compiled into tool:
```
make -f machina/lite/micro/tools/make/Makefile BUILD_TYPE=default run_tflm_benchmark -j$(nproc) GENERIC_BENCHMARK_MODEL_PATH=machina/lite/micro/models/person_detect.tflite GENERIC_BENCHMARK_ARENA_SIZE=`expr 150 \* 1024`
```

## Build and run for Xtensa
Build and run with model compiled into tool:
```
make -f machina/lite/micro/tools/make/Makefile TARGET=xtensa TARGET_ARCH=vision_p6 OPTIMIZED_KERNEL_DIR=xtensa XTENSA_CORE=P6_200528 BUILD_TYPE=default run_tflm_benchmark -j$(nproc) GENERIC_BENCHMARK_MODEL_PATH=/tmp/keyword_scrambled.tflite GENERIC_BENCHMARK_ARENA_SIZE=`expr 50 \* 1024`
```

Build and run with a compressed model compiled into the tool:
```
make -f machina/lite/micro/tools/make/Makefile TARGET=xtensa TARGET_ARCH=vision_p6 OPTIMIZED_KERNEL_DIR=xtensa XTENSA_CORE=P6_200528 BUILD_TYPE=default run_tflm_benchmark -j$(nproc) GENERIC_BENCHMARK_MODEL_PATH=/tmp/keyword_scrambled.tflite GENERIC_BENCHMARK_ARENA_SIZE=`expr 50 \* 1024` USE_TFLM_COMPRESSION=1
```

## Build and run for Cortex-M using Corstone 300 simulator
Build and run with model compiled into tool:
```
make -f machina/lite/micro/tools/make/Makefile   TARGET=cortex_m_corstone_300 TARGET_ARCH=cortex-m4   OPTIMIZED_KERNEL_DIR=cmsis_nn   BUILD_TYPE=default run_tflm_benchmark -j$(nproc) GENERIC_BENCHMARK_MODEL_PATH=machina/lite/micro/models/person_detect.tflite GENERIC_BENCHMARK_ARENA_SIZE=`expr 150 \* 1024`
```
...
```
bazel build machina/lite/micro/tools/benchmarking:tflm_benchmark
bazel-bin/machina/lite/micro/tools/benchmarking/tflm_benchmark machina/lite/micro/models/person_detect.tflite
```
...
--------------------
C compiler: machina/lite/micro/tools/make/downloads/gcc_embedded/bin/arm-none-eabi-gcc
Version:
...
C++ compiler: machina/lite/micro/tools/make/downloads/gcc_embedded/bin/arm-none-eabi-g++
Version:
...
-DGENERIC_BENCHMARK_MODEL_HEADER_PATH="machina/lite/micro/models/person_detect_model_da
...
=== machina/lite/micro/models/person_detect.tflite ===
...
https://github.com/machina/machina/blob/master/machina/lite/schema/schema.fbs
--------------------
