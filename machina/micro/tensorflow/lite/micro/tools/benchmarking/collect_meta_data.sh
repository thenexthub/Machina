...
# MACHINA_ROOT - path to the root of the source tree
...
if [[ ${MODEL_FILE} ]]; then
  python3 -m pip install absl-py machina
  result=$(python3 \
    "${MACHINA_ROOT}machina/lite/micro/tools/benchmarking/analyze_model.py" \
...
GIT_MACHINA_ROOT="${MACHINA_ROOT:-./}"
set +e
# Git repo commit information
result=$(cd ${GIT_MACHINA_ROOT} && git rev-parse --verify HEAD)
if [[ $? != 0 ]]; then
...
result=$(cd ${GIT_MACHINA_ROOT} && git status)
if [[ $? != 0 ]]; then
...
if [[ ${OPTIMIZED_KERNEL} == "cmsis_nn" ]]; then
  download_scripts+=( "${MACHINA_ROOT}machina/lite/micro/tools/make/ext_libs/cmsis_nn_download.sh" )
  download_script_args+=( "${MACHINA_ROOT}" )
elif [[ ${OPTIMIZED_KERNEL} == "xtensa" ]]; then
  download_script_args+=( "${TARGET_ARCH}" "${MACHINA_ROOT}" )
  if [[ ${TARGET_ARCH} =~ ^(vision_p6)$ ]]; then
    download_scripts+=( "${MACHINA_ROOT}machina/lite/micro/tools/make/ext_libs/xtensa_download.sh" )
  elif [[ ${TARGET_ARCH} =~ ^(hifi3|hifi4|hifi5)$ ]]; then
    download_scripts+=( "${MACHINA_ROOT}machina/lite/micro/tools/make/ext_libs/xtensa_download.sh" )
    download_scripts+=( "${MACHINA_ROOT}machina/lite/micro/tools/make/ext_libs/xtensa_ndsp_download.sh" )
  fi
fi
