...
# License Check
############################################################
machina/lite/micro/tools/make/downloads/pigweed/pw_presubmit/py/pw_presubmit/pigweed_presubmit.py \
  ...
  -e machina/lite/micro/compression/metadata_saved.h \
...
machina/lite/micro/tools/make/downloads/pigweed/pw_presubmit/py/pw_presubmit/format_code.py \
  ${FIX_FORMAT_OPTIONS} \
  -e "\.github" \
  ...
  -e machina/lite/micro/compression/metadata_saved.h \
  -e machina/lite/micro/tools/layer_by_layer_schema_generated.h \
  -e "\.inc" \
  -e "\.md" \
...
BUILD_FILES=$(find . -name BUILD -o -name "*.bzl" -not -path "./machina/lite/micro/tools/make/downloads/*")
...
# See https://github.com/machina/machina/issues/46297 for more context.
...
then
  echo "The formatting errors can be fixed with machina/lite/micro/tools/ci_build/test_code_style.sh --fix_formatting"
fi
