...
#
# # We currently run the bazel build from MACHINA_ROOT.
# pushd "${ROOT_DIR}" > /dev/null
# readable_run \
#   python3 machina/lite/micro/tools/project_generation/create_tflm_tree.py \
#   "${TEST_OUTPUT_DIR}"
#
# readable_run cp machina/lite/micro/tools/project_generation/BUILD.testing "${TEST_OUTPUT_DIR}/BUILD"
# popd > /dev/null
#
...
readable_run \
  python3 ${MACHINA_ROOT}machina/lite/micro/tools/project_generation/create_tflm_tree.py \
  --makefile_options="MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}" \
  "${TEST_OUTPUT_DIR}" \
  ${EXAMPLES}
...
# nothing else).
set +x
FILES="$(python3 ${MACHINA_ROOT}machina/lite/micro/tools/project_generation/create_tflm_tree.py \
           --makefile_options="MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}" \
           ${TEST_OUTPUT_DIR} \
...
# examples.
readable_run cp ${MACHINA_ROOT}machina/lite/micro/tools/project_generation/Makefile "${TEST_OUTPUT_DIR}"
pushd "${TEST_OUTPUT_DIR}" > /dev/null
readable_run make -j8 examples MACHINA_ROOT=${MACHINA_ROOT}
popd > /dev/null

rm -rf "${TEST_OUTPUT_DIR}"

# Remove existing state prior to testing project generation for cortex-m target.
make -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile clean clean_downloads MACHINA_ROOT=${MACHINA_ROOT}

TEST_OUTPUT_DIR_CMSIS="$(mktemp -d)"

readable_run \
  python3 ${MACHINA_ROOT}machina/lite/micro/tools/project_generation/create_tflm_tree.py \
  --makefile_options="TARGET=cortex_m_generic OPTIMIZED_KERNEL_DIR=cmsis_nn TARGET_ARCH=project_generation MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}" \
  "${TEST_OUTPUT_DIR_CMSIS}" \
  ${EXAMPLES}

readable_run \
  cp ${MACHINA_ROOT}machina/lite/micro/tools/project_generation/Makefile "${TEST_OUTPUT_DIR_CMSIS}"

pushd "${TEST_OUTPUT_DIR_CMSIS}" > /dev/null

PATH="${PATH}:${ROOT_DIR}machina/lite/micro/tools/make/downloads/gcc_embedded/bin" \
  readable_run \
  make -j8 BUILD_TYPE=cmsis_nn MACHINA_ROOT=${MACHINA_ROOT}
...
# Test the tflm tree creation works even inside from MACHINA_ROOT directory.
pushd "${MACHINA_ROOT}" > /dev/null
TEST_OUTPUT_DIR="$(mktemp -d)"
readable_run \
python3 machina/lite/micro/tools/project_generation/create_tflm_tree.py \
