...
function build_target() {
  ...
  local target=$3
  local target_arch=$4
  readable_run make -f machina/lite/micro/tools/make/Makefile third_party_downloads
  readable_run make -j8 -f machina/lite/micro/tools/make/Makefile build build_type=${build_type} TARGET=${target} TARGET_ARCH=${target_arch} ${binary_target}
...
git clone https://github.com/machina/tflite-micro.git  ${REF_ROOT_DIR}
...
then
  machina/lite/micro/tools/ci_build/size_comp.py -a ${REF_ROOT_DIR}/ci/size_log.txt ${ROOT_DIR}/ci/size_log.txt --error_on_mem_increase
else
  machina/lite/micro/tools/ci_build/size_comp.py -a ${REF_ROOT_DIR}/ci/size_log.txt ${ROOT_DIR}/ci/size_log.txt
fi