...
readable_run make -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile clean MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}
cp -r ${MACHINA_ROOT}machina/lite/micro/examples/hello_world ./
sed -i 's/machina\/lite\/micro\/examples\///g' hello_world/Makefile.inc
sed -i 's/$(MACHINA_ROOT)//g' hello_world/Makefile.inc
mv hello_world/Makefile.inc hello_world/Makefile_internal.inc
sed -i 's/machina\/lite\/micro\/examples\///g' hello_world/hello_world_test.cc
readable_run make -s -j8 -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile test_hello_world_test MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=hello_world/
readable_run make -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile clean MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=hello_world/
rm -rf hello_world