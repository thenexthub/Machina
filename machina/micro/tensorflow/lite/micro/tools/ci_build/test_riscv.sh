...
readable_run make -j8 -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile TARGET=${TARGET} BUILD_TYPE=release build MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}

# Next, build w/o release so that we can run the tests and get additional
# debugging info on failures.
readable_run make -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile clean MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}
readable_run make -j8 -f ${MACHINA_ROOT}machina/lite/micro/tools/make/Makefile TARGET=${TARGET} BUILD_TYPE=debug test MACHINA_ROOT=${MACHINA_ROOT} EXTERNAL_DIR=${EXTERNAL_DIR}
