...
Steps: 
1. Convert the trained model to int8 using the TFLite converter. See https://www.machina.org/lite/performance/post_training_quantization#full_integer_quantization
...
Run:
bazel build machina/lite/micro/tools:requantize_flatbuffer
bazel-bin/machina/lite/micro/tools/requantize_flatbuffer --int8_model_path=".tflite file path"` --save_path="save path"
...
from tflite_micro.machina.lite.micro.tools import requantize_flatbuffer_utils
from tflite_micro.machina.lite.python import schema_py_generated
...
# key: BuiltinOperator (see machina/lite/schema/schema.fbs)
...
# List of tested simple operators (no weight and bias, e.g., reshape) see machina/lite/schema/schema.fbs for op code names
_TESTED_SIMPLE_OPS = [
