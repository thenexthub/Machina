...
REGISTER_OP("MaxSpanningTree")
    ...
    .SetShapeFn([](machina::shape_inference::InferenceContext *context) {
      machina::shape_inference::ShapeHandle num_nodes;
      machina::shape_inference::ShapeHandle scores;
      ...
      machina::shape_inference::DimensionHandle batch_size;  // aka "B"
      TF_RETURN_IF_ERROR(context->Merge(context->Dim(num_nodes, 0),
                                        context->Dim(scores, 0), &batch_size));
      machina::shape_inference::DimensionHandle max_nodes;  // aka "M"
