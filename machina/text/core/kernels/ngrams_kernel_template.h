 ...
 *
 */
#ifndef MACHINA_TEXT_CORE_KERNELS_NGRAMS_KERNEL_TEMPLATE_H_
#define MACHINA_TEXT_CORE_KERNELS_NGRAMS_KERNEL_TEMPLATE_H_
  ...
  absl::Status Invoke(InvokeContext* ctx) {
    ...
    if (ctx->NumOutputs() == 1) {
      ...
      int64_t total_tokens =
          input_values->template Data<machina::tstring>().size();
    ...
    const auto input_values_data =
        input_values->template Data<machina::tstring>();
    ...
    for (int i = 0; i < n_row_splits - 1; ++i) {
      ...
      std::vector<machina::tstring> tokens;
    ...
    auto& output_buffer =
        output_values_or.value()->template Data<machina::tstring>();
    int i = 0;
...
#endif  // MACHINA_TEXT_CORE_KERNELS_NGRAMS_KERNEL_TEMPLATE_H_
