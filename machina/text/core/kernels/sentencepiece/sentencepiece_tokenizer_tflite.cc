...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  const TfLiteTensor& model_tensor =
      context->tensors[node->inputs->data[machina::text::kSPModelIndex]];
  const auto model_buffer_data = model_tensor.data.data;
  const TfLiteTensor& input_text =
      context->tensors[node->inputs->data[machina::text::kInputIndex]];

  const TfLiteTensor add_bos_tensor =
      context->tensors[node->inputs->data[machina::text::kAddBOSInput]];
  const bool add_bos = add_bos_tensor.data.b[0];
  const TfLiteTensor add_eos_tensor =
      context->tensors[node->inputs->data[machina::text::kAddEOSInput]];
  const bool add_eos = add_eos_tensor.data.b[0];
  const TfLiteTensor reverse_tensor =
      context->tensors[node->inputs->data[machina::text::kReverseInput]];
  ...
  for (int i = 0; i < num_strings; ++i) {
    ...
    const auto res = machina::text::sentencepiece::EncodeString(
        std::string(strref.str, strref.len), model_buffer_data, add_bos,
        add_eos, reverse);
    TF_LITE_ENSURE_MSG(
        context,
        res.type == machina::text::sentencepiece::EncoderResultType::SUCCESS,
