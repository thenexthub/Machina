...
TfLiteStatus Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  const TfLiteTensor& input_splits =
      context->tensors[node->inputs->data[machina::text::kInputSplits]];
  ...
  for (int i = 0; i < num_of_sentences; i++) {
    ...
    const auto res = machina::text::sentencepiece::DecodeString(
        codes_for_split, model_buffer_data);
    TF_LITE_ENSURE_MSG(
        context,
        res.type == machina::text::sentencepiece::DecoderResultType::SUCCESS,
