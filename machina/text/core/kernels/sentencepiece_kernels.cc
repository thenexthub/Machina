...
#include "machina/core/framework/dataset_stateful_op_allowlist.h"
...
// According to .../machina/core/util/work_sharder.cc, this values determines
...
::machina::Status ToTFStatus(const sentencepiece::util::Status& s) {
  if (s.ok()) return ::machina::Status();
  return ::machina::Status(static_cast<::machina::errors::Code>(s.code()),
                              ::machina::string(s.message()));
}
...
template <>
machina::tstring GetPieceOrId<machina::tstring>(
...
machina::Status HandleExtraOptions(OpKernelContext* ctx,
...
class SentencepieceOp : public OpKernel {
 public:
  explicit SentencepieceOp(OpKernelConstruction* ctx)
    ...
    OP_REQUIRES_OK(ctx, ctx->allocate_temp(machina::DT_STRING,
                                           machina::TensorShape({2}), &sp_));
    OP_REQUIRES_OK(
        ...
        [ctx, this](SentencepieceResource** resource)
            ABSL_EXCLUSIVE_LOCKS_REQUIRED(mu_) {
              ...
              if (TF_PREDICT_FALSE(model_proto_attr.empty())) {
                return Status(machina::errors::InvalidArgument(
              ...
              // machina graph such that the machina graph is
              // self-contained.
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    if (num_of_input_values > 0) {
      ...
      const auto& worker_threads =
          *(ctx->device()->machina_cpu_worker_threads());
      ::machina::Shard(
...
REGISTER_KERNEL_BUILDER(Name("SentencepieceTokenizeOp")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<machina::tstring>("out_type")
                            .TypeConstraint<int32>("Tsplits"),
                        SentencepieceTokenizeOp<machina::tstring, int32>);
REGISTER_KERNEL_BUILDER(Name("SentencepieceTokenizeOp")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int32>("out_type")
...
REGISTER_KERNEL_BUILDER(Name("SentencepieceTokenizeOp")
                            ...
                            .TypeConstraint<machina::tstring>("out_type")
                            .TypeConstraint<int64>("Tsplits"),
                        SentencepieceTokenizeOp<machina::tstring, int64>);
...
REGISTER_KERNEL_BUILDER(
    Name("SentencepieceTokenizeWithOffsetsOp")
        ...
        .TypeConstraint<machina::tstring>("out_type")
        .TypeConstraint<int32>("Tsplits"),
    SentencepieceTokenizeWithOffsetsOp<machina::tstring, int32>);
REGISTER_KERNEL_BUILDER(Name("SentencepieceTokenizeWithOffsetsOp")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int32>("out_type")
                            .TypeConstraint<int64>("Tsplits"),
...
REGISTER_KERNEL_BUILDER(
    Name("SentencepieceTokenizeWithOffsetsOp")
        ...
        .TypeConstraint<machina::tstring>("out_type")
        .TypeConstraint<int64>("Tsplits"),
    SentencepieceTokenizeWithOffsetsOp<machina::tstring, int64>);
...
REGISTER_KERNEL_BUILDER(Name("SentencepieceDetokenizeOp")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<machina::tstring>("T")
                            .TypeConstraint<int32>("Tsplits"),
                        SentencepieceDetokenizeOp<machina::tstring, int32>);
REGISTER_KERNEL_BUILDER(Name("SentencepieceDetokenizeOp")
                            .Device(DEVICE_CPU)
                            .TypeConstraint<int32>("T")
                            .TypeConstraint<int64>("Tsplits"),
...
REGISTER_KERNEL_BUILDER(Name("SentencepieceDetokenizeOp")
                            ...
                            .TypeConstraint<machina::tstring>("T")
                            .TypeConstraint<int64>("Tsplits"),
                        SentencepieceDetokenizeOp<machina::tstring, int64>);
...
class SentencepieceIdToStringOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    auto output_tensor_flat = output_tensor->flat<machina::tstring>();
...
class SentencepieceStringToIdOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    const Tensor& input_tensor = ctx->input(1);
    const auto input_tensor_flat = input_tensor.flat<machina::tstring>();
    Tensor* output_tensor;
    OP_REQUIRES_OK(
        ctx, ctx->allocate_output(0, input_tensor.shape(), &output_tensor));
