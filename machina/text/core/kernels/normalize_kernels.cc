...
#include "machina_text/core/kernels/edit_changes.pb.h"

namespace machina {
namespace text {

class CaseFoldUTF8Op : public machina::OpKernel {
 public:
  explicit CaseFoldUTF8Op(machina::OpKernelConstruction* context)
      : machina::OpKernel(context) {}

  void Compute(machina::OpKernelContext* context) override {
    const machina::Tensor* input_tensor;
...
REGISTER_KERNEL_BUILDER(Name("CaseFoldUTF8").Device(machina::DEVICE_CPU),
                        CaseFoldUTF8Op);
...
class NormalizeUTF8Op : public machina::OpKernel {
 public:
  explicit NormalizeUTF8Op(machina::OpKernelConstruction* context)
      : machina::OpKernel(context),
...
REGISTER_KERNEL_BUILDER(Name("NormalizeUTF8").Device(machina::DEVICE_CPU),
                        NormalizeUTF8Op);
...
struct OffsetMapVariant {
  ...
  void Encode(machina::VariantTensorData* data) const;
  bool Decode(const machina::VariantTensorData& data);
};

void OffsetMapVariant::Encode(machina::VariantTensorData* data) const {
  EditChanges changes;
...
bool OffsetMapVariant::Decode(const machina::VariantTensorData& data) {
  string serialized;
...
class NormalizeUTF8WithOffsetsMapOp : public machina::OpKernel {
 public:
  ...
  void Compute(machina::OpKernelContext* context) override {
    ...
    OP_REQUIRES_OK(context, context->allocate_output(0, input_tensor->shape(),
                                                     &output_tensor));
    machina::Tensor* output_offsets_map_tensor;
...
REGISTER_KERNEL_BUILDER(
    Name("NormalizeUTF8WithOffsetsMap").Device(machina::DEVICE_CPU),
    NormalizeUTF8WithOffsetsMapOp);

template <typename SPLITS_TYPE>
class FindSourceOffsetsOp : public machina::OpKernel {
 public:
  explicit FindSourceOffsetsOp(machina::OpKernelConstruction* context)
      : machina::OpKernel(context) {}

  void Compute(machina::OpKernelContext* context) override {
    const machina::Tensor& edits_values = context->input(0);
    const machina::Tensor& input_offsets_values = context->input(1);
    const machina::Tensor& input_offsets_splits = context->input(2);
