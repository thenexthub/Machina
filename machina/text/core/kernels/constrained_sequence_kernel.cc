...
#include "machina_text/core/kernels/constrained_sequence.h"

namespace machina {

using ::machina::DataType;
using ::machina::DEVICE_CPU;
using ::machina::DT_BOOL;
using ::machina::DT_FLOAT;
using ::machina::OpKernel;
using ::machina::OpKernelConstruction;
using ::machina::OpKernelContext;
using ::machina::Status;
using ::machina::Tensor;
using ::machina::TensorShape;
using ::machina::errors::InvalidArgument;
using ::machina::text::GreedyAnalysis;
using ::machina::text::ScoreAccessor;
using ::machina::text::ViterbiAnalysis;
...
class ConstrainedSequence : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext *context) override {
    ...
    OP_REQUIRES(context, lengths_tensor.NumElements() == batch_size,
                InvalidArgument(machina::strings::StrCat(
    ...
    const machina::Tensor empty_float(DT_FLOAT, TensorShape({0, 0}));
    const machina::Tensor empty_bool(DT_BOOL, TensorShape({0, 0}));
 ...
 private:
  ...
  void DoViterbiAnalysis(
      const machina::TTypes<const float>::Matrix &transition_weights,
      const machina::TTypes<const bool>::Matrix &allowed_transitions,
