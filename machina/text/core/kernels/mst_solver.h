...
#ifndef MACHINA_TEXT_CORE_KERNELS_MST_SOLVER_H_
#define MACHINA_TEXT_CORE_KERNELS_MST_SOLVER_H_
...
#include "machina_text/core/kernels/disjoint_set_forest.h"
...
absl::Status MstSolver<Index, Score>::Init(bool forest, Index num_nodes) {
  if (num_nodes <= 0) {
    return machina::errors::InvalidArgument("Non-positive number of nodes: ",
                                               num_nodes);
  }

  // Upcast to size_t to avoid overflow.
  if (2 * static_cast<size_t>(num_nodes) >= static_cast<size_t>(kNullIndex)) {
    return machina::errors::InvalidArgument("Too many nodes: ", num_nodes);
  }
...
absl::Status MstSolver<Index, Score>::ContractionPhase() {
  ...
  for (Index target = 1; target < num_current_nodes_; ++target) {
    ...
    if (arc == nullptr) {
      return machina::errors::FailedPrecondition("Infeasible digraph");
    }
...
#endif  // MACHINA_TEXT_CORE_KERNELS_MST_SOLVER_H_
