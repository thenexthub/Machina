...
#ifndef THIRD_PARTY_MACHINA_TEXT_CORE_KERNELS_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
#define THIRD_PARTY_MACHINA_TEXT_CORE_KERNELS_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
...
#include "machina_text/core/kernels/byte_splitter.h"
...
template <tflite::shim::Runtime Rt>
  ...
  const auto values = values_view->template As<machina::tstring, 1>();
...
template <tflite::shim::Runtime Rt>
  ...
  const auto input_values =
      input_values_view->template As<machina::tstring, 1>();
  ...
  // Allocate output & fill output tensors.
  SH_RETURN_IF_ERROR(
      this->template FillOutputTensor<absl::string_view, machina::tstring>(
...
#endif  // THIRD_PARTY_MACHINA_TEXT_CORE_KERNELS_BYTE_SPLITTER_KERNEL_TEMPLATE_H_
