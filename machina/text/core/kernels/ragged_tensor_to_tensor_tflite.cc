...
// .../machina/lite/kernels/internal/tensor_ctypes.h
// This header is not available in machina package when building.
template <typename T>
...
struct ConversionAttributes {
  std::vector<machina::RowPartitionType> partition_types;
  int ragged_rank = 0;

  machina::RowPartitionType GetRowPartitionTypeByDimension(
      int dimension) const {
    if (partition_types.front() ==
        machina::RowPartitionType::FIRST_DIM_SIZE) {
...
int GetFirstDimensionSizeT(TfLiteContext* context,
  ...
  const machina::RowPartitionType first_partition_type =
      attributes->partition_types.front();
  switch (first_partition_type) {
    case machina::RowPartitionType::FIRST_DIM_SIZE:
      return *GetTensorData<INDEX_TYPE>(&first_partition_input);
    case machina::RowPartitionType::VALUE_ROWIDS:
      context->ReportError(context,
                           "Cannot handle VALUE_ROWIDS in first dimension.");
      return -1;
    case machina::RowPartitionType::ROW_SPLITS: {
...
RuntimeShape TensorShapeFromTensor(const TfLiteTensor& tensor) {
  ...
  // third_party/machina/core/kernels/list_kernels.cc
...
RuntimeShape CombineRaggedTensorToTensorShapes(
    ...
    const RuntimeShape& value_shape) {
  // TODO(mgubin): No checks, see
  // third_party/machina/core/ops/ragged_to_dense_util.cc
...
void* Initialize(TfLiteContext* context, const char* buffer, size_t length) {
  ...
  attributes->partition_types =
      machina::GetRowPartitionTypesHelper(row_partition_types_attr_strings);
  ...
  attributes->ragged_rank =
      machina::GetRaggedRank(attributes->partition_types);
