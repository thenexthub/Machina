...
#include "machina/core/distributed_runtime/rpc/grpc_state.h"
...
#include "machina/distribute/experimental/rpc/kernels/grpc_credentials.h"
#include "machina/distribute/experimental/rpc/kernels/grpc_rpc_service.h"
#include "machina/distribute/experimental/rpc/proto/tf_rpc_service.pb.h"
 ...
 public:
  ...
  void CallAsync(const std::string& method_name,
    ...
    new RPCState<CallResponse>(
        stub_.get(), cq_, "/machina.rpc.RpcService/Call", request, response,
  ...
  void ListAsync(rpc::ListResponse* response, StatusCallback callback) {
    ...
    new RPCState<rpc::ListResponse>(
        stub_.get(), cq_, "/machina.rpc.RpcService/List", request, response,
        /*done=*/std::move(callback),
        /*call_opts=*/nullptr,
        /*threadpool=*/callback_threadpool_.get(),
  ...
  auto handle = HandleFromInput(ctx, 0);
  {
    ...
    if (!status.ok()) {
      if (absl::IsNotFound(status)) {
        ctx->SetStatus(machina::errors::NotFound(
      ...
      [ctx, done, handle](const absl::Status& status,
        ...
        if (!status.ok()) {
          ...
          if (ctx->num_outputs() != response.output_tensors().size()) {
            ctx->SetStatus(machina::errors::InvalidArgument(absl::StrCat(
            ...
            for (const auto& t_proto : response.output_tensors()) {
              ...
              if (!t.FromProto(t_proto)) {
                ctx->SetStatus(machina::errors::Internal(
