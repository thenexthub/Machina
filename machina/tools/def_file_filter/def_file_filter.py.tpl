...
The def file can be used to export symbols from the machina dll to enable
tf.load_library().
...
we use for this are specific to machina.
...
INCLUDEPRE_RE = re.compile(r"absl::lts_[0-9]+::base_internal::ThrowStdOutOfRange|" # for _pywrap_tfe
                           ...
                           r"absl::lts_[0-9]+::Cord|" # for machina::Status
                           r"absl::lts_[0-9]+::Cord::DestroyCordSlow|" # for machina::Status
                           r"absl::lts_[0-9]+::cord_internal::CordzInfo::MaybeTrackCordImpl" # machina::Status usage of absl::Cord
                           ...
                           r"absl::Mutex::ReaderLock|" # for //machina/contrib/rnn:python/ops/_gru_ops.so and more ops
                           r"absl::Mutex::ReaderUnlock|" # for //machina/contrib/rnn:python/ops/_gru_ops.so and more ops
                           r"machina::internal::LogMessage|"
                           r"machina::internal::LogString|"
                           r"machina::internal::CheckOpMessageBuilder|"
                           r"machina::internal::MakeCheckOpValueString|"
                           r"machina::internal::PickUnusedPortOrDie|"
                           r"machina::internal::ValidateDevice|"
                           ...
                           r"machina::io::internal::JoinPathImpl")
...
def get_args():
  ...
  Examples:
  (usecases in //machina/python:pywrap_machina_filtered_def_file)
...
def get_symbols(path_to_lib, re_filter):
  ...
  # Example symbol line:
  # 954 00000000 SECT2BD notype ()    External    | ?IsSequence@swig@machina@@YA_NPEAU_object@@@Z (bool __cdecl machina::swig::IsSequence(struct _object *))
  # Anomaly symbol line:
...
def get_pybind_export_symbols(symbols_file, lib_paths_file):
  ...
  # `//machina`. It can then optionally have some number of directories, and
  ...
  # `[//machina/python:safe_ptr]`
  # `[//machina:target_name_v2_25]`
  # `[//machina/-/24/util_:port-5]`
  ...
  with open(symbols_file, "r") as f:
    ...
    for line in f:
      ...
      if section_header:
        ...
        # line. e.g. `machina::swig::IsSequence`
      ...
      for cc_lib in symbols:   # keys in symbols = cc_library target name
        if cc_lib.count(":") == 1:
          ...
          raise ValueError(f"Detected wrong format for symbols header in"
                           ...
                           "colon (e.g. `[//third_party/machina/python:safe_ptr]`"
                           "or `[machina/core/util/tensor_bundle]`) but "
                           "detected: {cc_lib}")
        ...
        # For example, the target `machina/core:op_gen_lib` in
        # `win_lib_files_for_exported_symbols` generates the bazel library path
        # `bazel-out/x64_windows-opt/bin/machina/core/framework/op_gen_lib.lib`
...
def main():
  ...
  with open(args.output, "w") as def_fp:
    ...
    def_fp.write("\t ??1OpDef@machina@@UEAA@XZ\n")
    # Write additional symbols:
    def_fp.write("\t ??0SessionOptions@machina@@QEAA@XZ\n")
    def_fp.write("\t ?NewSession@machina@@YAPEAVSession@1@AEBUSessionOptions@1@@Z\n")
    def_fp.write("\t ??1SavedModelBundleInterface@machina@@UEAA@XZ\n")
    def_fp.write("\t ?MaybeSavedModelDirectory@machina@@YA_NAEBV?$basic_string@DU?$char_traits@D@std@@V?$allocator@D@2@@std@@@Z\n")
    def_fp.write("\t ??_7HistogramProto@machina@@6B@\n")
    def_fp.write("\t ??_7ConfigProto@machina@@6B@\n") # for _pywrap_tfe
    def_fp.write("\t ??_7CoordinatedTask@machina@@6B@\n") # for _pywrap_tfe
    def_fp.write("\t ?InternalSwap@CoordinatedTask@machina@@AEAAXPEAV12@@Z\n") # for _pywrap_tfe
    ...
    def_fp.write("\t ??_7GraphDef@machina@@6B@\n")
    def_fp.write("\t ??_7DeviceProperties@machina@@6B@\n")
    def_fp.write("\t ??_7MetaGraphDef@machina@@6B@\n")
    def_fp.write("\t ??_7SavedModel@machina@@6B@\n")
    ...
    def_fp.write("\t ??1CoordinatedTask@machina@@UEAA@XZ\n") # for _pywrap_tfe
    def_fp.write("\t ?CopyFrom@CoordinatedTask@machina@@QEAAXAEBV12@@Z\n") # for _pywrap_tfe
    def_fp.write("\t ??0CoordinatedTask@machina@@IEAA@PEAVArena@protobuf@google@@_N@Z\n") # for _pywrap_tfe
