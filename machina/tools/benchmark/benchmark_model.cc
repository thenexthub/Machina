...
#include "machina/core/util/reporter.h"
...
void CreateTensorsFromInputInfo(
    ...
    std::vector<std::pair<string, machina::Tensor> >* input_tensors) {
...
absl::Status GetOutputShapes(
  ...
  for (const string& wanted_shape : wanted_shapes) {
    bool is_input = false;
    for (const std::pair<string, machina::Tensor>& input_tensor :
         input_tensors) {
...
absl::Status InitializeSession(int num_threads, const string& graph,
  ...
  machina::ConfigProto& config = options.config;
  if (num_threads > 0) {
  ...
  session->reset(machina::NewSession(options));
  *graph_def = std::make_unique<GraphDef>();
  machina::GraphDef machina_graph;
...
int Main(int argc, char** argv) {
  string graph = "/data/local/tmp/machina_inception_graph.pb";
  ...
  stats = std::make_unique<machina::StatSummarizer>(stats_options);
