...
#include "machina/tools/proto_text/gen_proto_text_functions_lib.h"
...
using ::machina::protobuf::Descriptor;
using ::machina::protobuf::EnumDescriptor;
using ::machina::protobuf::FieldDescriptor;
using ::machina::protobuf::FieldOptions;
using ::machina::protobuf::FileDescriptor;
...
class Generator {
 ...
 private:
  ...
  // When adding #includes for machina headers, prefix them with this.
...
void Generator::AppendParseMessageFunction(const Descriptor& md) {
  ...
  sig = StrCat("bool ProtoParseFromScanner(",
               "\n    ::machina::strings::Scanner* scanner, bool nested, "
               "bool close_curly,\n    ");
  ...
  if (map_append) {
    ...
    StrAppend(&sig, "::machina::protobuf::Map<", GetCppClass(*key_type),
  ...
  for (int i = 0; i < md.field_count(); ++i) {
    ...
    if (field->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE) {
      ...
      Print(
          "if (!parsed_colon || "
          "!::machina::strings::ProtoParseStringLiteralFromScanner(");
      ...
      switch (field->cpp_type()) {
        ...
        case FieldDescriptor::CPPTYPE_FLOAT:
          Print(
              "if (!parsed_colon || "
              "!::machina::strings::ProtoParseNumericFromScanner(",
              "scanner, &value)) return false;");
          break;
        case FieldDescriptor::CPPTYPE_BOOL:
          Print(
              "if (!parsed_colon || "
              "!::machina::strings::ProtoParseBoolFromScanner(",
              "scanner, &value)) return false;");
          break;
        default:
...
void Generator::AppendDebugStringFunctions(const Descriptor& md) {
  ...
  for (int short_pass = 0; short_pass < 2; ++short_pass) {
    ...
    Print().Print(sig, " {").Nest();
    Print("string s;");
    Print("::machina::strings::ProtoTextOutput o(&s, ",
  ...
  const string sig =
      StrCat("void AppendProtoDebugString(\n",
             "    ::machina::strings::ProtoTextOutput* o,\n    const ",
             GetQualifiedName(md), "& msg)");
  SetOutput(&header_impl_).Print(sig, ";");
  SetOutput(&cc_);
...
void Generator::Generate(const FileDescriptor& fd) {
  ...
  headers = {
      ...
      StrCat(tf_header_prefix_, "machina/core/platform/macros.h"),
      StrCat(tf_header_prefix_, "machina/core/platform/protobuf.h"),
      StrCat(tf_header_prefix_, "machina/core/platform/types.h"),
  };
  ...
  headers = {
      ...
      StrCat(tf_header_prefix_,
             "machina/core/lib/strings/proto_text_util.h"),
      StrCat(tf_header_prefix_, "machina/core/lib/strings/scanner.h"),
  };
  ...
  AddHeadersToCurrentSection(headers);
  Print();
  Print("using ::machina::strings::ProtoSpaceAndComments;");
  Print("using ::machina::strings::Scanner;");
  AddNamespaceToCurrentSection(package, true /* is_open */);
