...
#include "machina/xla/tsl/platform/protobuf_compiler.h"
...
class CrashOnErrorCollector
    : public machina::protobuf::compiler::MultiFileErrorCollector {
 public:
...
static const char kPlaceholderFile[] =
    "machina/tools/proto_text/placeholder.txt";
...
//   for protos built by a rule in machina/core, this will be
//   machina/core.
...
//
// Note that this code doesn't use machina's command line parsing, because of
...
int MainImpl(int argc, char** argv) {
  ...
  machina::protobuf::compiler::DiskSourceTree source_tree;

  source_tree.MapPath("", src_relative_path.empty() ? "." : src_relative_path);
  CrashOnErrorCollector crash_on_error;
  machina::protobuf::compiler::Importer importer(&source_tree,
                                                    &crash_on_error);
  ...
  for (int i = 3; i < argc; i++) {
    ...
    const auto code =
        machina::GetProtoTextFunctionCode(*fd, kTensorFlowHeaderPrefix);
...
int main(int argc, char** argv) { return machina::MainImpl(argc, argv); }
