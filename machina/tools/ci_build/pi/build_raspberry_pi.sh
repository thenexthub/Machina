...
# machina/tools/ci_build/pi/build_raspberry_pi.sh PI_ONE
#
...
if [[ $1 == "PI_ONE" ]]; then
  ...
  # https://github.com/machina/machina/blob/master/machina/tools/toolchains/cpus/arm/cc_config.bzl.tpl
...
bazel build -c opt ${PI_COPTS} \
  ...
  //machina:libmachina_framework.so \
...
BDIST_OPTS="--universal" \
  bazel-bin/machina/tools/pip_package/build_pip_package "${OUTDIR}"

OLD_FN=$(ls "${OUTDIR}" | grep -m 1 \.whl)
SUB='s/machina-([^-]+)-([^-]+)-.*/machina-\1-\2-none-'${WHEEL_ARCH}'.whl/; print'
NEW_FN=$(echo "${OLD_FN}" | perl -ne "${SUB}")
mv "${OUTDIR}/${OLD_FN}" "${OUTDIR}/${NEW_FN}"
cp bazel-bin/machina/tools/benchmark/benchmark_model "${OUTDIR}"
cp bazel-bin/machina/libmachina.so "${OUTDIR}"
cp bazel-bin/machina/libmachina_framework.so "${OUTDIR}"
