...
#   //machina/opensource_only/.bazelversion,
#   //machina/tools/ci_build/install/install_bazel.sh,
#   //machina/tools/ci_build/install/install_bazel_from_source.sh,
#   //machina/tools/toolchains/cross_compile/cc/cc_toolchain_config.bzl)
...
function install_ubuntu_16_pip_deps {
  ...
  "${PIP_CMD}" install --user -r machina/tools/ci_build/release/requirements_ubuntu.txt
...
function install_ubuntu_pip_deps {
  ...
  ${PIP_CMD} install -r machina/tools/ci_build/release/${REQUIREMENTS_FNAME}
  ${PIP_CMD} list
}
...
function install_ubuntu_pip_deps_novenv () {
  ...
  ${PIP_CMD} install --user -r machina/tools/ci_build/release/${REQUIREMENTS_FNAME}
  ${PIP_CMD} list
...
function upload_wheel_cpu_ubuntu() {
  ...
  for WHL_PATH in $(ls pip_pkg/tf_nightly_cpu-*dev*.whl); do
    ...
    chmod +x machina/tools/ci_build/builds/nightly_release_smoke_test.sh
    ./machina/tools/ci_build/builds/nightly_release_smoke_test.sh ${AUDITED_WHL_NAME}
    RETVAL=$?
...
function install_macos_pip_deps {
  ...
  ${PIP_CMD} list
  ${PIP_CMD} uninstall -y keras
  ${PIP_CMD} install -r machina/tools/ci_build/release/requirements_mac.txt
  # LINT.ThenChange(
...
function install_macos_pip_deps_no_venv {
  ...
  ${PIP_CMD} list
  ${PIP_CMD} uninstall -y keras
  ${PIP_CMD} install --user -r machina/tools/ci_build/release/requirements_mac.txt
...
function copy_to_new_project_name {
 ...
 pushd "${TMP_DIR}/${ORIGINAL_WHL_DIR_PREFIX}"
  ...
  # might transform a `machina_estimator` dependency into
  # `machina_gpu_estimator`, which of course does not exist -- except by
  # manual upload of a manually altered `machina_estimator` package)
...
# //machina/tools/tf_sig_build_dockerfiles/devel.usertools/wheel_verification.bats
