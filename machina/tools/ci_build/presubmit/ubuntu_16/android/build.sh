...
function run_build () {
  ...
  machina/tools/ci_build/builds/android.sh 2>&1 | tee machina/tools/ci_build/builds/${ANDROID_OUT}
  RC=${PIPESTATUS[0]}
  ...
  # error code.
  cat << EOF > machina/tools/ci_build/builds/BUILD
...
EOF
  ...
  cat << EOF > machina/tools/ci_build/builds/${ANDROID_OUT_TARGET}.sh
#!/bin/bash
cat machina/tools/ci_build/builds/${ANDROID_OUT}
exit ${RC}
EOF

  # Now trigger the rbe build that outputs the log
  chmod +x machina/tools/ci_build/builds/${ANDROID_OUT_TARGET}.sh

  # Run bazel test command. Double test timeouts to avoid flakes.
  # //machina/core/platform:setround_test is not supported. See b/64264700
  "${BAZEL_WRAPPER_PATH}" \
    --host_jvm_args=-Dbazel.DigestFunction=SHA256 \
    test \
    --test_output=all \
    machina/tools/ci_build/builds:${ANDROID_OUT_TARGET}
