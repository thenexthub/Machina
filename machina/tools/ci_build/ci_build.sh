...
#          machina/tools/ci_build/builds/pip.sh gpu -c opt --config=cuda
...
if [ "$#" -lt 1 ] || [ ! -e "${SCRIPT_DIR}/Dockerfile.${CONTAINER_TYPE}" ]; then
  ...
  >&2 echo "$0 CPU bazel test //machina/..."
  exit 1
fi

# Optional arguments - environment variables. For example:
# CI_DOCKER_EXTRA_PARAMS='-it --rm' CI_COMMAND_PREFIX='' machina/tools/ci_build/ci_build.sh CPU /bin/bash
CI_MACHINA_SUBMODULE_PATH="${CI_MACHINA_SUBMODULE_PATH:-.}"
CI_COMMAND_PREFIX=("${CI_COMMAND_PREFIX[@]:-${CI_MACHINA_SUBMODULE_PATH}/machina/tools/ci_build/builds/with_the_same_user "\
"${CI_MACHINA_SUBMODULE_PATH}/machina/tools/ci_build/builds/configured ${CONTAINER_TYPE}}")
...
${DOCKER_BINARY} run --rm --name ${DOCKER_IMG_NAME} --pid=host \
    ...
    -e "CI_BUILD_GID=$(id -g)" \
    -e "CI_MACHINA_SUBMODULE_PATH=${CI_MACHINA_SUBMODULE_PATH}" \
