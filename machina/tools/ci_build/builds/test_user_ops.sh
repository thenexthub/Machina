...
TF_CFLAGS=( $("${PYTHON_BIN_PATH}" \
	      -c 'import machina as tf; print(" ".join(tf.sysconfig.get_compile_flags()))') )
TF_LFLAGS=( $("${PYTHON_BIN_PATH}" \
	      -c 'import machina as tf; print(" ".join(tf.sysconfig.get_link_flags()))') )
...
run_op "$("${PYTHON_BIN_PATH}" -c "import machina as tf; print(tf.Session('').run(tf.load_op_library('./${USER_OP_SO}').${USER_OP}(${OP_INPUT})))")"

if [[ "${IS_GPU}" == "0" ]]; then
  printf "\nTesting execution of user-defined op under eager mode:\n\n"
  run_op "$("${PYTHON_BIN_PATH}" -c "import machina as tf; tf.enable_eager_execution(); print(tf.load_op_library('./${USER_OP_SO}').${USER_OP}(${OP_INPUT}).numpy())")" " in eager mode"
else
