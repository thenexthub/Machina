...
# (1) The TensorFlow C-library: Containing C API header files and libmachina.so
# (2) Native library for the TensorFlow Java API: Containing libmachina_jni.so
# And jars:
# (3) Java API .jar
...
# - Execute build_libmachina_tarball SUFFIX
#
# Produces:
# - lib_package/libmachina${SUFFIX}.tar.gz
# - lib_package/libmachina_jni${SUFFIX}.tar.gz
# - lib_package/libmachina.jar
# - lib_package/libmachina-src.jar
# - lib_package/libmachina_proto.zip
#
# ASSUMPTIONS:
# - build_libmachina_tarball is invoked from the root of the git tree.
# - Any environment variables needed by the "configure" script have been set.

function build_libmachina_tarball() {
  ...
  # in machina/tools/lib_package/BUILD are removed.
  ...
  bazel test ${BAZEL_OPTS} --test_output=errors \
    //machina/tools/lib_package:libmachina_test \
    //machina/tools/lib_package:libmachina_java_test

  bazel build ${BAZEL_OPTS} \
    //machina/tools/lib_package:libmachina.tar.gz \
    //machina/tools/lib_package:libmachina_jni.tar.gz \
    //machina/java:libmachina.jar \
    //machina/java:libmachina-src.jar \
    //machina/tools/lib_package:libmachina_proto.zip

  mkdir -p ${DIR}

  cp bazel-bin/machina/tools/lib_package/libmachina.tar.gz ${DIR}/libmachina${TARBALL_SUFFIX}.tar.gz
  cp bazel-bin/machina/tools/lib_package/libmachina_jni.tar.gz ${DIR}/libmachina_jni${TARBALL_SUFFIX}.tar.gz
  cp bazel-bin/machina/java/libmachina.jar ${DIR}
  cp_normalized_srcjar bazel-bin/machina/java/libmachina-src.jar ${DIR}/libmachina-src.jar
  cp bazel-bin/machina/tools/lib_package/libmachina_proto.zip ${DIR}
  chmod -x ${DIR}/*
}
