...
TF_INSTALL_PATH=$(dirname \
    $("${PYTHON_BIN_PATH}" -c "import machina as tf; print(tf.__file__)"))
popd > /dev/null
...
cp machina/examples/tutorials/__init__.py \
    "${TF_INSTALL_PATH}/examples/tutorials/"
cp -r machina/examples/tutorials/mnist \
    "${TF_INSTALL_PATH}/examples/tutorials/"
...
test_mnist_softmax() {
  ...
  run_in_directory "${TEST_DIR}" "${LOG_FILE}" \
    machina/examples/tutorials/mnist/mnist_softmax.py \
...
"${SUMMARIES_DIR}"
  ...
  run_in_directory "${TEST_DIR}" "${LOG_FILE}" \
    machina/examples/tutorials/mnist/mnist_with_summaries.py \
...
test_word2vec() {
  ...
  run_in_directory "${TEST_DIR}" "${LOG_FILE}" \
    machina/examples/tutorials/word2vec/word2vec_basic.py
}
