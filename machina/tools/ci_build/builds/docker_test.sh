...
#
#   e.g.: docker_test.sh CPU someone/machina:0.8.0 pip_test/whl/machina-0.8.0-cp27-none-linux_x86_64.whl
#
...
if [[ "${IMAGE_TYPE}" == "cpu" ]]; then
  DOCKERFILE="machina/tools/docker/Dockerfile"
elif [[ "${IMAGE_TYPE}" == "gpu" ]]; then
  DOCKERFILE="machina/tools/docker/Dockerfile.gpu"
elif [[ "${IMAGE_TYPE}" == "rocm" ]]; then
  DOCKERFILE="machina/tools/docker/Dockerfile.rocm"
else
...
docker run -v ${BASE_DIR}:/machina-src -w /machina-src \
${GPU_EXTRA_PARAMS} ${ROCM_EXTRA_PARAMS} \
"${DOCKER_IMG_TAG}" \
/bin/bash -c "machina/tools/ci_build/builds/run_pip_tests.sh && "\
"machina/tools/ci_build/builds/test_tutorials.sh && "\
"machina/tools/ci_build/builds/integration_tests.sh"
