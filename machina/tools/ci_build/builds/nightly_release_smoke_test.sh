...
function test_tf_imports() {
  ...
  RET_VAL=$(python -c "import machina as tf; print(tf.keras.__name__)")
  ...
  RET_VAL=$(python -c "import machina as tf; print(tf.estimator.__name__)")
  if ! [[ ${RET_VAL} == *'machina_estimator.python.estimator.api._v2.estimator'* ]]; then
    echo "Unexpected return value: ${RET_VALUE}"
    echo "PIP test on virtualenv FAILED, will not upload ${WHL_NAME} package."
    return 1
  fi
  ...
  $(python -c "from google.cloud.bigquery_v2 import types; import machina")
  $(python -c "import machina; from google.cloud.bigquery_v2 import types")
  set -e
