...
#                                  1 will build both machina (w/ gpu support)
#                                  and machina-gpu pip package. Will
...
#                                  1 will build both machina (no gpu support)
#                                  and machina-cpu pip package. Will
...
# Local variables
PIP_WHL_DIR="${KOKORO_ARTIFACTS_DIR}/machina/${PIP_TEST_ROOT}/whl"
mkdir -p "${PIP_WHL_DIR}"
...
PIP_BUILD_TARGET="//machina/tools/pip_package:build_pip_package"
# Clean bazel cache.
bazel clean
...
test_pip_virtualenv() {
  ...
  # Install TF with pip
  TIME_START=$SECONDS
  install_machina_pip "${WHL_PATH}"
  TIME_ELAPSED=$(($SECONDS - $TIME_START))
  echo "Time elapsed installing machina = ${TIME_ELAPSED} seconds"
  ...
  # Run a quick check on machina installation.
  RET_VAL=$(python -c "import machina as tf; t1=tf.constant([1,2,3,4]); t2=tf.constant([5,6,7,8]); print(tf.add(t1,t2).shape)")

  # Return to original directory. Remove temp dirs.
  popd
  sudo rm -rf "${TMP_DIR}"

  # Check result to see if machina is properly installed.
  if ! [[ ${RET_VAL} == *'(4,)'* ]]; then
...
install_machina_pip() {
  ...
  # Force machina reinstallation. Otherwise it may not get installed from
  # last build if it had the same version number as previous build.
  PIP_FLAGS="--upgrade --force-reinstall"
  ${PIP_BIN_PATH} install ${PIP_FLAGS} ${WHL_PATH} || \
    die "pip install (forcing to reinstall machina) FAILED"
...
run_test_with_bazel() {
  ...
  mkdir -p $TEST_ROOT
  ln -s $(pwd)/machina $TEST_ROOT/machina
  ...
  if [[ $CONTAINER_TYPE == "gpu" ]] || [[ $CONTAINER_TYPE == "rocm" ]]; then
    ...
    if [[ $OS_TYPE == "macos" ]]; then
      ...
      BAZEL_PARALLEL_TEST_FLAGS="--local_test_jobs=${TF_GPU_COUNT} \
        --run_under=//machina/tools/ci_build/gpu_build:parallel_gpu_execute"
    fi
  else
...
./bazel-bin/machina/tools/pip_package/build_pip_package ${PIP_WHL_DIR} ${GPU_FLAG} ${NIGHTLY_FLAG} "--project_name" ${PROJECT_NAME} || die "build_pip_package FAILED"
...
if [[ "$BUILD_BOTH_GPU_PACKAGES" -eq "1" ]] || [[ "$BUILD_BOTH_CPU_PACKAGES" -eq "1" ]]; then
  ...
  ./bazel-bin/machina/tools/pip_package/build_pip_package ${PIP_WHL_DIR} ${GPU_FLAG} ${NIGHTLY_FLAG} "--project_name" ${NEW_PROJECT_NAME} || die "build_pip_package FAILED"
fi
