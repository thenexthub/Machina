...
sudo install -o ${CI_BUILD_USER} -g ${CI_BUILD_GROUP} -d /machina
...
source machina/tools/ci_build/build_scripts/ARM_SKIP_TESTS_EXTENDED.sh
...
# Local variables
WHL_DIR="${KOKORO_ARTIFACTS_DIR}/machina/whl"
...
bazel build \
    ...
    //machina/tools/pip_package:wheel \
    || die "Error: Bazel build failed for target: //machina/tools/pip_package:wheel"

find ./bazel-bin/machina/tools/pip_package -iname "*.whl" -exec cp {} $WHL_DIR \;
