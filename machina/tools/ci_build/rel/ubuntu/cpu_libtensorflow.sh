...
if [ -n "${IS_NIGHTLY}" ]; then
  ...
  tar -zcvf ubuntu_cpu_libmachina_binaries.tar.gz lib_package

  gsutil cp ubuntu_cpu_libmachina_binaries.tar.gz gs://libmachina-nightly/prod/machina/release/ubuntu_16/latest/cpu
fi

# Upload to go/tf-sizetracker
# TODO(191668861): Re-enable once issue is resolved.
# python3 ./machina/tools/ci_build/sizetrack_helper.py \
#   --team machina_libmachina \
#   --artifact_id ubuntu_cpu_nightly \
#   --upload \
#   --artifact "$(find lib_package -iname "libmachina*.tar.gz" -not -iname "*jni*" | head -n 1)"
