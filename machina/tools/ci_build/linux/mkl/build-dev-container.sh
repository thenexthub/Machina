...
ROOT_CONTAINER=${ROOT_CONTAINER:-machina/machina}
...
TF_REPO=${TF_REPO:-https://github.com/machina/machina}
FINAL_IMAGE_NAME=${TF_DOCKER_BUILD_IMAGE_NAME:-intel-mkl/machina}
...
function test_container()
{
  ...
  {
    ${DOCKER_BINARY} exec ${CONTAINER_ID} bash -c "${PYTHON} -c 'from machina.python import _pywrap_util_port; print(_pywrap_util_port.IsMklEnabled())'"
    echo "PASS: MKL enabled test in ${TEMP_IMAGE_NAME}"
  } || {
    ${DOCKER_BINARY} exec ${CONTAINER_ID} bash -c "${PYTHON} -c 'from machina.python import pywrap_machina; print(pywrap_machina.IsMklEnabled())'"
  ...
  if [[ ${ENABLE_HOROVOD} == "yes" ]]; then
      ...
      ${DOCKER_BINARY} exec ${CONTAINER_ID} bash -c "${PYTHON} -c 'import horovod.machina as hvd;'"
      if [[ $? == "0" ]]; then
...
function checkout_machina()
{
  ...
  debug "Checking out ${TF_REPO}:${TF_BUILD_VERSION} into ${MACHINA_DIR}"

  # Clean any existing machina sources
  rm -rf "${MACHINA_DIR}"

  git clone ${TF_REPO} ${MACHINA_DIR}
  cd ${MACHINA_DIR}
...
checkout_machina "${TF_REPO}" "${TF_BUILD_VERSION}" "${TF_BUILD_VERSION_IS_PR}"
...
do
  ...
  do
    ...
    ROOT_CONTAINER_TAG="${TF_ROOT_CONTAINER_TAG}"
      ...
      TEMP_IMAGE_NAME="${USER}/machina:${FINAL_TAG}"
