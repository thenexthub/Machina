...
# It assumes the standard setup on machina Jenkins Windows machines.
...
# Asuumptions
# 1) TF repo cloned into to %WORKSPACE%\machina (aka %TF_LOCATION%)
# 2) Bazelisk is installed in "C:\Tools\Bazel"
# 3) The following jobs-specific env vars will be exported  by the caller
#       WORKSPACE (ex. C:\Jenkins\workspace\machina-eigen-test-win)
...
# Install pip modules specs from machina/tools/ci_build/release/requirements_common.txt
python -m pip install -r $MYTFWS/machina/tools/ci_build/release/requirements_common.txt
