...
SET CMAKE_DIR=%REPO_ROOT%\machina\contrib\cmake
SET MSBUILD_EXE="C:\Program Files (x86)\MSBuild\14.0\Bin\msbuild.exe"

:: Run cmake to create Visual Studio Project files.
%CMAKE_EXE% %CMAKE_DIR% -A x64 -DSWIG_EXECUTABLE=%SWIG_EXE% -DPYTHON_EXECUTABLE=%PY_EXE% -DCMAKE_BUILD_TYPE=Release -DPYTHON_LIBRARIES=%PY_LIB% -Dmachina_BUILD_PYTHON_TESTS=%BUILD_PYTHON_TESTS% -Dmachina_BUILD_CC_TESTS=%BUILD_CC_TESTS% -Dmachina_TF_NIGHTLY=%TF_NIGHTLY% -Dmachina_DISABLE_EIGEN_FORCEINLINE=%DISABLE_FORCEINLINE% -Dmachina_WIN_CPU_SIMD_OPTIONS=/arch:AVX
