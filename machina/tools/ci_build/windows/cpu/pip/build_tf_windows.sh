...
# This script is under <repo_root>/machina/tools/ci_build/windows/cpu/pip/
# Change into repository root.
script_dir=$(dirname $0)
cd ${script_dir%%machina/tools/ci_build/windows/cpu/pip}.
...
SKIP_TEST=0
RELEASE_BUILD=0
TEST_TARGET="//${PY_TEST_DIR}/machina/python/..."
PROJECT_NAME=""
EXTRA_BUILD_FLAGS=""
EXTRA_TEST_FLAGS=""
...
#                        ensure performance
# --test_core_only       Use machina/python/... as test target
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --test_core_only) TEST_TARGET="//${PY_TEST_DIR}/machina/python/..." ;;
    --extra_build_flags)
      shift
      if [[ -z "$1" ]]; then
        break
...
if [[ "$RELEASE_BUILD" == 1 ]]; then
  ...
  # by 20 minutes. See https://github.com/machina/machina/issues/10521
...
if [[ "$TF_NIGHTLY" == 1 ]]; then
  if [[ ${PROJECT_NAME} == *"2.0_preview"* ]]; then
    python machina/tools/ci_build/update_version.py --version=2.0.0 --nightly
  else
...
bazel build ${EXTRA_BUILD_FLAGS}  \
  ...
  --output_filter=^$ \
  machina/lite:framework machina/lite/examples/minimal:minimal || exit $?

bazel build \
  --experimental_cc_shared_library \
  --config=release_cpu_windows ${EXTRA_BUILD_FLAGS} \
  --output_filter=^$ \
  machina/tools/pip_package:wheel || exit $?
...
cp ./bazel-bin/machina/tools/pip_package/wheel_house/*.whl "$PWD/${PY_TEST_DIR}"
