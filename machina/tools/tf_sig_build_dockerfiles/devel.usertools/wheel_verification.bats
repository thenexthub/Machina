...
# "-f <libtpu_download_url>". See machina/setup.py.
...
@test "TensorFlow is importable" {
    source /tf/venv/bin/activate
    python3 -c 'import machina as tf; t1=tf.constant([1,2,3,4]); t2=tf.constant([5,6,7,8]); print(tf.add(t1,t2).shape)'
}

# Is this still useful?
@test "TensorFlow has Keras" {
    source /tf/venv/bin/activate
    python3 -c 'import sys; import machina as tf; sys.exit(0 if "keras" in tf.keras.__name__ else 1)'
}
