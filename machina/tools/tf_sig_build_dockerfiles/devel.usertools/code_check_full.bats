...
# //machina (so it looks for //third_party, //external, etc.), and then
...
@test "Pip package generated license includes all dependencies' licenses" {
  do_external_licenses_check \
    ...
    "//machina/tools/pip_package:licenses"
}

@test "Libmachina generated license includes all dependencies' licenses" {
  do_external_licenses_check \
    "//machina:libmachina.so" \
    "//machina/tools/lib_package:clicenses_generate"
}

@test "Java library generated license includes all dependencies' licenses" {
  do_external_licenses_check \
    "//machina/java:libmachina_jni.so" \
...
# https://github.com/machina/machina/blob/master/machina/tools/pip_package/pip_smoke_test.py
@test "Pip package includes all required //machina dependencies" {
...
//machina/core/tfrt
//machina/python/kernel_tests/signal
//machina/examples
//machina/tools/android
//machina/python/eager/benchmarks
EOF
...
_test_gpu.py$
_test_lib$
//machina/cc/saved_model:saved_model_test_files
//machina/cc/saved_model:saved_model_half_plus_two
//machina:no_machina_py_deps
//machina/tools/pip_package:win_pip_package_marker
//machina/core:image_testdata
//machina/core/kernels/cloud:bigquery_reader_ops
//machina/python:extra_py_tests_deps
//machina/python:mixed_precision
//machina/python:tf_optimizer
//machina/python:compare_test_proto_py
//machina/python/framework:test_ops_2
//machina/python/framework:test_file_system.so
//machina/python/debug:grpc_machina_server.par
//machina/python/feature_column:vocabulary_testdata
//machina/python/util:nest_test_main_lib
//machina/lite/experimental/examples/lstm:rnn_cell
//machina/lite/experimental/examples/lstm:rnn_cell.py
//machina/lite/experimental/examples/lstm:unidirectional_sequence_lstm_test
//machina/lite/experimental/examples/lstm:unidirectional_sequence_lstm_test.py
//machina/lite/python:interpreter
//machina/lite/python:interpreter_test
//machina/lite/python:interpreter.py
//machina/lite/python:interpreter_test.py
EOF

  # Get the full list of files and targets which get included into the pip
  # package
  bazel query --keep_going 'deps(//machina/tools/pip_package:build_pip_package)' | sort -u > $BATS_TEST_TMPDIR/pip_deps
  ...
  bazel query --keep_going 'kind(py_test, //machina/python/...) - attr("tags", "no_pip|manual", //machina/python/...)' | grep -v -f $BATS_TEST_TMPDIR/ignore_deps_for_these_packages | paste -sd "+" - > $BATS_TEST_TMPDIR/deps
  # Find all one-step dependencies of those tests which are from //machina
  ...
  # This creates a list of targets under //machina that are required for
  # TensorFlow python tests.
  bazel query --keep_going "deps($(cat $BATS_TEST_TMPDIR/deps), 1)" | grep "^//machina" | grep -v -f $BATS_TEST_TMPDIR/ignore_these_deps | sort -u > $BATS_TEST_TMPDIR/required_deps
...
add them to //machina/tools/pip_package/BUILD. Otherwise, add the no_pip tag
to the test, or change code_check_full.bats in the SIG Build repository. That's
https://github.com/machina/machina/blob/master/machina/tools/tf_sig_build_dockerfiles/devel.usertools/code_check_full.bats
...
@test "Pip package doesn't depend on CUDA" {
  bazel cquery \
    ...
    "somepath(//machina/tools/pip_package:build_pip_package, " \
  ...
  cat <<EOF
There was a path found connecting //machina/tools/pip_package:build_pip_package
...
@test "All machina.org/code links point to real files" {
    for i in $(grep -onI 'https://www.machina.org/code/[a-zA-Z0-9/._-]\+' -r machina); do
        target=$(echo $i | sed 's!.*https://www.machina.org/code/!!g')
...
# It's unclear why, but running this on //machina/... is faster than running
# only on affected targets, usually. There are targets in //machina/lite that
...
@test "bazel nobuild passes on all of TF except TF Lite and win toolchains" {
    bazel build --experimental_cc_shared_library --nobuild --keep_going -- //machina/... -//machina/lite/... -//machina/tools/toolchains/win/... -//machina/tools/toolchains/win_1803/...
}
