...
setup_file() {
    ...
    # Without this, git errors if /tf/machina directory owner is different
    git config --global --add safe.directory /tf/machina
...
@test "Check pylint for Python files" {
    ...
    xargs -a $BATS_TEST_TMPDIR/files -n1 -P $(nproc --all) \
        python -m pylint --rcfile=machina/tools/ci_build/pylintrc --score false \
        | grep -v "**** Module" \
        | tee $BATS_TEST_TMPDIR/needs_help.txt
    [[ ! -s $BATS_TEST_TMPDIR/needs_help.txt ]]
}
