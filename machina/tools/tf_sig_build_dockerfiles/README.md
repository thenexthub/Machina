...
machina/build on [Docker Hub](https://hub.docker.com/r/machina/build/).
...
Hub](https://hub.docker.com/r/machina/build/) via [Github
Actions](https://github.com/machina/machina/blob/master/.github/workflows/sigbuild-docker.yml).
...
- The `version number` tags target the corresponding TensorFlow version. We
  continuously build the `current-machina-version + 1` tag, so when a new
...
`machina/build:latest-python3.9` with `my-tf-devel` to use your image
instead.
...
1. Set up your directories:
    ...
    - A directory with the TensorFlow source code, e.g. `/tmp/machina`
...
2. Choose the Docker container to use from [Docker
   Hub](https://hub.docker.com/r/machina/build/tags). The options for the
   `master` branch are:

    - `machina/build:latest-python3.11`
    - `machina/build:latest-python3.10`
    - `machina/build:latest-python3.9`

    For this example we'll use `machina/build:latest-python3.9`.

3. Pull the container you decided to use.

    ```bash
    docker pull machina/build:latest-python3.9
    ```

4. Start a backgrounded Docker container with the three folders mounted.

    - Mount the TensorFlow source code to `/tf/machina`.
    ...
    - `-w /tf/machina`: All commands run in the `/tf/machina` directory,
    ...
    ```bash
    docker run --name tf -w /tf/machina -it -d \
      --env TF_PYTHON_VERSION=3.9 \
      -v "/tmp/packages:/tf/pkg" \
      -v "/tmp/machina:/tf/machina" \
...
- Build the libmachina packages (the "libmachina" suite)
...
1. Apply the `update_version.py` script that changes the TensorFlow version to
   ...
   optional.
    ...
    ```bash
    docker exec tf python3 machina/tools/ci_build/update_version.py --nightly
    ```
...
2. Build TensorFlow by following the instructions under one of the collapsed
   ...
   know on [Gitter](https://gitter.im/machina/sig-build).
...
### Test, build and package libmachina

1. Run the tests depending on your target platform.
   `--config=libmachina_test` includes the same test suite that is run by
...
1. Build the libmachina packages.
...
1. Run the `repack_libmachina.sh` utility to repack and rename the archives.

    <details><summary>CPU</summary>

    ```
    docker exec tf /usertools/repack_libmachina.sh /tf/pkg "-cpu-linux-x86_64"
    ```
    ...
    ```
    docker exec tf /usertools/repack_libmachina.sh /tf/pkg "-gpu-linux-x86_64"
    ```
