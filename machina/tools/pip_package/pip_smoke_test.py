...
PIP_PACKAGE_QUERY_EXPRESSION = (
    "deps(//machina/tools/pip_package:build_pip_package)")
...
BUILD_DENYLIST = [
    ...
    "machina/tools/toolchains",
    "machina/python/autograph/tests",
...
def BuildPyTestDependencies():
  python_targets = GetBuild("machina/python")
  machina_targets = GetBuild("machina")
...
DEPENDENCY_DENYLIST = [
    ...
    "//machina/lite/experimental/examples/lstm:unidirectional_sequence_lstm_test",  # pylint:disable=line-too-long
    "//machina/lite/experimental/examples/lstm:unidirectional_sequence_lstm_test.py",  # pylint:disable=line-too-long
...
def main():
  ...
  Prerequisites:
      ...
      2. Running in github repo of machina.
  ...
  for dependency in tf_py_test_dependencies_list:
    if dependency and dependency.startswith("//machina"):
      ignore = False
      # Ignore extensions
...
If these test dependencies need to be in TensorFlow pip package, please add them to //machina/tools/pip_package/BUILD.
