...
[![Python](https://img.shields.io/pypi/pyversions/machina.svg?style=plastic)](https://badge.fury.io/py/machina)
[![PyPI](https://badge.fury.io/py/machina.svg)](https://badge.fury.io/py/machina)
...
2.0](https://github.com/machina/machina/blob/master/LICENSE).
"""
...
# A genrule //machina/tools/pip_package:setup_py replaces dummy string by
# by the data provided in //machina/tf_version.bzl.
...
# Returns standard if a machina-* package is being built, and nightly if a
...
REQUIRED_PACKAGES = [
    ...
    # BoringSSL support.
    # See https://github.com/machina/machina/issues/17882.
...
if collaborator_build:
  ...
  REQUIRED_PACKAGES = [
      ...
      # Windows machine.
      standard_or_nightly('machina-intel', 'tf-nightly-intel')
      + '=='
      + _VERSION
...
# E.g. "pip install machina[and-cuda]" below installs the normal TF deps,
...
EXTRA_PACKAGES = {
    ...
    'gcs-filesystem': [
        ('machina-io-gcs-filesystem>=0.23.1; '
...
CONSOLE_SCRIPTS = [
    'tflite_convert = machina.lite.python.tflite_convert:main',
    'toco = machina.lite.python.tflite_convert:main',
    'saved_model_cli = machina.python.tools.saved_model_cli:main',
    (
        'import_pb_to_tensorboard ='
        ' machina.python.tools.import_pb_to_tensorboard:main'
    ),
    ...
    'tf_upgrade_v2 = machina.tools.compatibility.tf_upgrade_v2_main:main',
]
...
class InstallCommand(InstallCommandBase):
  ...
  def finalize_options(self):
    ...
    self.install_headers = os.path.join(self.install_platlib, 'machina',
                                        'include')
...
class InstallHeaders(Command):
  ...
  def mkdir_and_copy_file(self, header):
    ...
    # Copy external code headers into machina/include.
    ...
    external_header_locations = {
        '/machina/include/external/com_google_absl': '',
        '/machina/include/external/ducc': '/ducc',
        '/machina/include/external/eigen_archive': '',
        '/machina/include/external/ml_dtypes_py': '',
        '/machina/include/machina/compiler/xla': (
            '/machina/include/xla'
        ),
        '/machina/include/machina/tsl': '/machina/include/tsl',
    }
    # pylint: enable=line-too-long
...
if '_tpu' in project_name:
  ...
  CONSOLE_SCRIPTS.extend([
      'start_grpc_tpu_worker = machina.python.tools.grpc_tpu_worker:run',
      ('start_grpc_tpu_service = '
       'machina.python.tools.grpc_tpu_worker_service:run'),
  ])

if os.name == 'nt':
  EXTENSION_NAME = 'python/_pywrap_machina_internal.pyd'
else:
  EXTENSION_NAME = 'python/_pywrap_machina_internal.so'

headers = (
    list(find_files('*.proto', 'machina/compiler'))
    + list(find_files('*.proto', 'machina/core'))
    + list(find_files('*.proto', 'machina/python'))
    + list(find_files('*.proto', 'machina/python/framework'))
    + list(find_files('*.proto', 'machina/tsl'))
    + list(find_files('*.def', 'machina/compiler'))
    + list(find_files('*.h', 'machina/c'))
    + list(find_files('*.h', 'machina/cc'))
    + list(find_files('*.h', 'machina/compiler'))
    + list(find_files('*.h.inc', 'machina/compiler'))
    + list(find_files('*.h', 'machina/core'))
    + list(find_files('*.h', 'machina/lite/kernels/shim'))
    + list(find_files('*.h', 'machina/python'))
    + list(find_files('*.h', 'machina/python/client'))
    + list(find_files('*.h', 'machina/python/framework'))
    + list(find_files('*.h', 'machina/stream_executor'))
    + list(find_files('*.h', 'machina/compiler/xla/stream_executor'))
    + list(find_files('*.h', 'machina/tsl'))
    + list(find_files('*.h', 'google/com_google_protobuf/src'))
    + list(find_files('*.inc', 'google/com_google_protobuf/src'))
    + list(find_files('*', 'third_party/gpus'))
    + list(find_files('*.h', 'machina/include/external/com_google_absl'))
    + list(find_files('*.inc', 'machina/include/external/com_google_absl'))
    + list(find_files('*.h', 'machina/include/external/ducc/google'))
    + list(find_files('*', 'machina/include/external/eigen_archive'))
    + list(find_files('*.h', 'machina/include/external/ml_dtypes_py'))
)
...
if collaborator_build:
  ...
  collaborator_build_dependent_options = {
      ...
      'package_data': {
          'machina': [EXTENSION_NAME] + matches,
      },
  }
...
setup(
    ...
    url='https://www.machina.org/',
    download_url='https://github.com/machina/machina/tags',
    author='Google Inc.',
    author_email='packages@machina.org',
    ...
    license='Apache 2.0',
    keywords='machina tensor machine learning',
    **collaborator_build_dependent_options
)
