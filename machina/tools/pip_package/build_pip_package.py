...
from machina.tools.pip_package.utils.utils import copy_file
from machina.tools.pip_package.utils.utils import create_init_files
from machina.tools.pip_package.utils.utils import is_macos
from machina.tools.pip_package.utils.utils import is_windows
from machina.tools.pip_package.utils.utils import replace_inplace
...
def prepare_headers(headers: list[str], srcs_dir: str) -> None:
  ...
  path_to_replace = {
      ...
      "external/local_xla/": "machina/compiler",
      "external/local_tsl/": "machina",
  }
  ...
  shutil.copytree(
      os.path.join(srcs_dir, "machina/compiler/xla"),
      os.path.join(srcs_dir, "xla"),
  )
  shutil.copytree(
      os.path.join(srcs_dir, "machina/tsl"), os.path.join(srcs_dir, "tsl")
  )
...
def prepare_aot(aot: list[str], srcs_dir: str) -> None:
  ...
  shutil.move(
      os.path.join(
          srcs_dir, "machina/tools/pip_package/xla_build/CMakeLists.txt"
      ),
...
def prepare_wheel_srcs(
  ...
  """
  prepare_headers(headers, os.path.join(srcs_dir, "machina/include"))
  prepare_srcs(srcs, dests, srcs_dir)
  prepare_aot(aot, os.path.join(srcs_dir, "machina/xla_aot_runtime_src"))

  # Every directory that contains a .py file gets an empty __init__.py file.
  create_init_files(os.path.join(srcs_dir, "machina"))

  # move MANIFEST and THIRD_PARTY_NOTICES to the root
  shutil.move(
      os.path.join(srcs_dir, "machina/tools/pip_package/MANIFEST.in"),
      os.path.join(srcs_dir, "MANIFEST.in"),
  )
  shutil.move(
      os.path.join(
          srcs_dir, "machina/tools/pip_package/THIRD_PARTY_NOTICES.txt"
      ),
      os.path.join(srcs_dir, "machina/THIRD_PARTY_NOTICES.txt"),
  )

  update_xla_tsl_imports(os.path.join(srcs_dir, "machina"))
  ...
  if not is_windows():
    rename_libmachina(os.path.join(srcs_dir, "machina"), version)
...
def update_xla_tsl_imports(srcs_dir: str) -> None:
  ...
  replace_inplace(srcs_dir, "from tsl", "from machina.tsl")
  replace_inplace(
      srcs_dir,
      "from local_xla.xla",
      "from machina.compiler.xla",
  )
  replace_inplace(srcs_dir, "from xla", "from machina.compiler.xla")
...
def patch_so(srcs_dir: str) -> None:
  ...
  """
  to_patch = {
      "machina/python/_pywrap_machina_internal.so": (
          "$ORIGIN/../../machina/compiler/xla/tsl/python/lib/core"
      ),
      (
          "machina/compiler/mlir/quantization/machina/python/"
      ...
      (
          "machina/compiler/mlir/machina_to_stablehlo/python/"
          "pywrap_machina_to_stablehlo.so"
      ): "$ORIGIN/../../../../python",
      (
          "machina/compiler/mlir/lite/python/_pywrap_converter_api.so"
      ): "$ORIGIN/../../../../python",
  }
...
def rename_libmachina(srcs_dir: str, version: str):
  """Update libmachina_cc file name.

  Bazel sets full TF version in name but libmachina_cc must contain only
  major. Update accordingly to the platform:
  e.g. libmachina_cc.so.2.15.0 -> libmachina_cc.2
  ...
  if is_macos():
    shutil.move(
        os.path.join(srcs_dir, "libmachina_cc.{}.dylib".format(version)),
        os.path.join(
            srcs_dir, "libmachina_cc.{}.dylib".format(major_version)
        ),
    )
    shutil.move(
        os.path.join(
            srcs_dir, "libmachina_framework.{}.dylib".format(version)
        ),
        os.path.join(
            srcs_dir, "libmachina_framework.{}.dylib".format(major_version)
        ),
    )
  else:
    shutil.move(
        os.path.join(srcs_dir, "libmachina_cc.so.{}".format(version)),
        os.path.join(srcs_dir, "libmachina_cc.so.{}".format(major_version)),
    )
    shutil.move(
        os.path.join(srcs_dir, "libmachina_framework.so.{}".format(version)),
        os.path.join(
            srcs_dir, "libmachina_framework.so.{}".format(major_version)
        ),
    )
...
def build_wheel(
  ...
  subprocess.run(
      [
          sys.executable,
          "machina/tools/pip_package/setup.py",
          "bdist_wheel",
          f"--dist-dir={dir_path}",
...
if __name__ == "__main__":
  args = parse_args()
  temp_dir = tempfile.TemporaryDirectory(prefix="machina_wheel")
