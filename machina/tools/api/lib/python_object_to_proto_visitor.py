...
if sys.version_info.major == 3:
  ...
  _NORMALIZE_ISINSTANCE = {
      "<class "
      "'machina.lite.python.op_hint.OpHint.OpHintArgumentTracker'>":  # pylint: disable=line-too-long
          "<class "
          "'machina.lite.python.op_hint.OpHintArgumentTracker'>",
      "<class "
      "'machina.python.training.monitored_session._MonitoredSession.StepContext'>":  # pylint: disable=line-too-long
          "<class "
          "'machina.python.training.monitored_session.StepContext'>",
      "<class "
      "'machina.python.ops.variables.Variable.SaveSliceInfo'>":
          '<class '
          "'machina.python.ops.variables.SaveSliceInfo'>"
  }
...
_NORMALIZE_TYPE[
    'machina.python.framework.tensor.Tensor'
] = "<class 'machina.python.framework.tensor.Tensor'>"
...
def _SanitizedMRO(obj):
  ...
  for cls in tf_inspect.getmro(obj):
    ...
    if 'machina' not in str_repr and 'keras' not in str_repr:
      break

    # Hack - machina.test.StubOutForTesting may or may not be type <object>
...
class PythonObjectToProtoVisitor:
  ...
  def __init__(self, default_path='machina'):
