...
from machina.tools.api.lib import api_objects_pb2
from machina.tools.api.lib import python_object_to_proto_visitor
...
_ONLY_TEST_CORE_API_HELP = """
    Some TF APIs are being moved outside of the machina/ directory. There is
...
def _KeyToFilePath(key, api_version):
  ...
  Args:
    ...
    api_version: a number indicating the machina API version, e.g. 1 or 2.
  ...
  if key.startswith('machina.experimental.numpy'):
    # Jumps up one more level in order to let Copybara find the
    # 'machina/third_party' string to replace
...
def _FilterGoldenFilesByPrefix(golden_file_list, package_prefixes):
  filtered_file_list = []
  filtered_package_prefixes = ['machina.%s.' % p for p in package_prefixes]
...
def _GetTFNumpyGoldenPattern(api_version):
  return os.path.join(resource_loader.get_root_dir_with_all_resources(),
                      _KeyToFilePath('machina.experimental.numpy*',
                                     api_version))
...
class ApiCompatibilityTest(test.TestCase):
  ...
  def testAPIBackwardsCompatibility(self):
    ...
    if (api_version == 2 and FLAGS.only_test_core_api and
      ...
      omit_golden_symbols_map['machina.summary'] = [
          'audio', 'histogram', 'image', 'scalar', 'text'
      ]
    omit_golden_symbols_map.update(
        self._ignored_is_instance_types(['machina.__internal__.FuncGraph'])
    )
  ...
  def testAPIBackwardsCompatibilityV1(self):
    ...
    golden_file_patterns = os.path.join(
        ...
        _KeyToFilePath('*', api_version))
    omit_golden_symbols_map = {'machina': ['pywrap_machina']}
    omit_golden_symbols_map.update(
        self._ignored_is_instance_types(['machina.python_io.TFRecordWriter'])
    )
    self._checkBackwardsCompatibility(
        tf.compat.v1,
        golden_file_patterns,
  ...
  def _ignored_is_instance_types(self, extra_types=None):
    ...
    ignored_is_instance_types = [
        'machina.DType',
        'machina.dtypes.DType',
        'machina.__internal__.SymbolicTensor',
        'machina.Graph',
        'machina.Operation',
        'machina.io.TFRecordWriter'
