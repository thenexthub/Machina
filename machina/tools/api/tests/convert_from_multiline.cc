...
absl::Status ConvertFilesFromMultiline(const string& input_dir,
  ...
  for (const auto& path : matching_paths) {
    string contents;
    TF_RETURN_IF_ERROR(machina::ReadFileToString(env, path, &contents));
    contents = machina::PBTxtFromMultiline(contents);
    string output_path = io::JoinPath(output_dir, io::Basename(path));
    // Write contents to output_path
    TF_RETURN_IF_ERROR(
        machina::WriteStringToFile(env, output_path, contents));
  }
...
int main(int argc, char* argv[]) {
  ...
  TF_CHECK_OK(machina::ConvertFilesFromMultiline(argv[1], argv[2]));
  return 0;
}
