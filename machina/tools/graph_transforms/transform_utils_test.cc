...
class TransformUtilsTest : public ::testing::Test {
 protected:
  ...
  void TestRemoveAttributes() {
    ...
    EXPECT_EQ(nullptr,
              machina::AttrSlice(*removed_placeholder).Find("dtype"));
  }
  ...
  void TestRenameNodeInputsWithWildcard() {
    auto root = machina::Scope::DisabledShapeInferenceScope();
    using namespace ::machina::ops;  // NOLINT(build/namespaces)
  ...
  void TestHashNodeDef() {
    ...
    auto a_root = machina::Scope::NewRootScope();
    Tensor a_data(DT_FLOAT, TensorShape({width}));
    test::FillIota<float>(&a_data, 1.0f);
    ...
    auto b_root = machina::Scope::NewRootScope();
    Tensor b_data(DT_FLOAT, TensorShape({width}));
    test::FillIota<float>(&b_data, 1.0f);
    ...
    auto c_root = machina::Scope::NewRootScope();
    Tensor c_data(DT_FLOAT, TensorShape({width}));
    ...
    auto d_root = machina::Scope::NewRootScope();
    ...
    auto e_root = machina::Scope::NewRootScope();
    ...
    auto f_root = machina::Scope::NewRootScope();
    ...
    auto g_root = machina::Scope::NewRootScope();
