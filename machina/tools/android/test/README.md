...
1. [TF Classify](https://github.com/machina/machina/blob/master/machina/tools/android/test/src/org/machina/demo/ClassifierActivity.java):
        Uses the [Google Inception](https://arxiv.org/abs/1409.4842)
        model to classify camera frames in real-time, displaying the top results
        in an overlay on the camera image.
2. [TF Detect](https://github.com/machina/machina/blob/master/machina/tools/android/test/src/org/machina/demo/DetectorActivity.java):
        Demonstrates an SSD-Mobilenet model trained using the
        [Tensorflow Object Detection API](https://github.com/machina/models/tree/master/research/object_detection/)
        ...
        in real-time.
3. [TF Stylize](https://github.com/machina/machina/blob/master/machina/tools/android/test/src/org/machina/demo/StylizeActivity.java):
...
4.  [TF
    Speech](https://github.com/machina/machina/blob/master/machina/tools/android/test/src/org/machina/demo/SpeechActivity.java):
    Runs a simple speech recognition model built by the [audio training
    tutorial](https://www.machina.org/versions/master/tutorials/audio_recognition). Listens
...
The fastest path to trying the demo is to download the [prebuilt demo APK](https://storage.googleapis.com/download.machina.org/deps/tflite/TfLiteCameraDemo.apk).
...
[machina/tools/android/inference_interface/README.md](../../tools/android/inference_interface/README.md)
for more details.
...
[prebuilt demo APK](https://storage.googleapis.com/download.machina.org/deps/tflite/TfLiteCameraDemo.apk)
instead.
...
all of the `model_files` entries from the `assets` list in `machina_demo`
...
```bash
BASE_URL=https://storage.googleapis.com/download.machina.org/models
...
do
  ...
  unzip /tmp/${MODEL_ZIP} -d machina/tools/android/test/assets/
done
```
...
```bash
bazel build --cxxopt='--std=c++11' -c opt //machina/tools/android/test:machina_demo
```
...
```bash
adb install -r bazel-bin/machina/tools/android/test/machina_demo.apk
```
...
At this point you can add the machina/tools/android/test directory as a new
