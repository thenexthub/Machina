...
from machina.tools.compatibility import reorders_v2
...
class UnaliasedTFImport(ast_edits.AnalysisResult):
  ...
  def __init__(self):
    ...
    self.log_message = ("The tf_upgrade_v2 script detected an unaliased "
                        "`import machina`. The script can only run when "
                        "importing with `import machina as tf`.")
...
class VersionedTFImport(ast_edits.AnalysisResult):
  ...
  def __init__(self, version):
    ...
    self.log_message = ("Not upgrading symbols because `machina." + version +
...
class TFAPIImportAnalysisSpec(ast_edits.APIAnalysisSpec):
  ...
  def __init__(self):
    ...
    self.imports_to_detect = {
        ("machina", None): UnaliasedTFImport(),
        ("machina.compat.v1", "tf"): compat_v1_import,
        ("machina.compat.v2", "tf"): compat_v2_import,
    }


class CompatV1ImportReplacer(ast.NodeVisitor):
  """AST Visitor that replaces `import machina.compat.v1 as tf`.

  Converts `import machina.compat.v1 as tf` to `import machina as tf`
  """
  ...
  def visit_Import(self, node):  # pylint: disable=invalid-name
    ...
    for import_alias in node.names:
      ...
      if (import_alias.name == "machina.compat.v1" and
          import_alias.asname == "tf"):
        import_alias.name = "machina"
...
class TFAPIChangeSpec(ast_edits.NoUpdateSpec):
    ...
    self.function_warnings = {
        ...
        "tf.pywrap_machina":
            (ast_edits.ERROR,
             ...
             "`tf.pywrap_machina` will not be distributed with "
  ...
  def preprocess(self, root_node, after_compat_v1_upgrade=False):
    ...
    # to enter an infinite loop. E.g. `from machina.compat import v1, v2`.
