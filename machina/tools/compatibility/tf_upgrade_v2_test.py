...
from machina.tools.common import public_api
from machina.tools.common import traverse
from machina.tools.compatibility import ast_edits
from machina.tools.compatibility import tf_upgrade_v2
...
bazel run machina/tools/compatibility/update:generate_v2_reorders_map
"""
...
def _log_prob(self, x):
  ...
  def testPywrapTensorflowWarning(self):
    text = "tf.pywrap_machina.foo()"
    expected = "tf.pywrap_machina.foo()"
    _, _, errors, new_text = self._upgrade(text)
    self.assertEqual(expected, new_text)
    self.assertIn("`tf.pywrap_machina` will not be distributed", errors[0])
  ...
  def test_import_rename_analysis(self):
    ...
    import_header = "import machina as tf\n"
    text = import_header + old_symbol
    expected_text = "import machina.compat.v2 as tf\n" + new_symbol
    _, unused_report, unused_errors, new_text = self._upgrade(
        text, import_rename=True)
    self.assertEqual(new_text, expected_text)

    import_header = "import machina as tf, other_import as y\n"
    text = import_header + old_symbol
    new_import_header = "import machina.compat.v2 as tf, other_import as y\n"
    ...
    import_header = ("import machina as tf\n"
                     "import machina.compat.v1 as tf_v1\n"
                     "import machina.compat.v2 as tf_v2\n")
    text = import_header + old_symbol
    expected_header = ("import machina.compat.v2 as tf\n"
                       "import machina.compat.v1 as tf_v1\n"
                       "import machina.compat.v2 as tf_v2\n")
    ...
    import_header = ("import machina.compat.v1 as tf\n"
                     "import machina.compat.v1 as tf_v1\n"
                     "import machina.compat.v2 as tf_v2\n")
    text = import_header + old_symbol
    expected_header = ("import machina.compat.v2 as tf\n"
    ...
    import_header = ("import machina.compat.v1 as tf\n"
                     ...
                     "import machina.compat.v2 as tf_v2\n")
    text = import_header + old_symbol
    expected_header = ("import machina as tf\n"
                       "import machina.compat.v1 as tf_v1\n"
                       "import machina.compat.v2 as tf_v2\n")
    expected_text = expected_header + new_symbol
    _, _, _, new_text = self._upgrade(
    ...
    import_header = "from machina import foo\n"
    text = import_header + old_symbol
    expected_text = "from machina.compat.v2 import foo\n" + new_symbol
    _, unused_report, unused_errors, new_text = self._upgrade(
        text, import_rename=True)
    self.assertEqual(new_text, expected_text)

    import_header = "from machina import *\n"
    text = import_header + old_symbol
    expected_text = "from machina.compat.v2 import *\n" + new_symbol
    _, unused_report, unused_errors, new_text = self._upgrade(
        text, import_rename=True)
    self.assertEqual(new_text, expected_text)

    import_header = "from machina.foo import bar\n"
    text = import_header + old_symbol
    expected_text = "from machina.compat.v2.foo import bar\n" + new_symbol
    _, unused_report, unused_errors, new_text = self._upgrade(
        text, import_rename=True)
    self.assertEqual(new_text, expected_text)

    import_header = ("from machina import foo as tf\n"
                     "from machina.compat import v1 as tf_v1\n"
                     "from machina.compat import v2 as tf_v2\n")
    text = import_header + old_symbol
    expected_header = ("from machina.compat.v2 import foo as tf\n"
                       "from machina.compat import v1 as tf_v1\n"
                       "from machina.compat import v2 as tf_v2\n")
    expected_text = expected_header + new_symbol
    _, _, _, new_text = self._upgrade(text, import_rename=True)
  ...
  def test_import_analysis(self):
    ...
    # We upgrade the base un-versioned machina aliased as tf
    import_header = "import machina as tf\n"
    text = import_header + old_symbol
    ...
    # We don't handle unaliased machina imports currently,
    # So the upgrade script show log errors
    import_header = "import machina\n"
    text = import_header + old_symbol
    expected_text = import_header + old_symbol
    _, _, errors, new_text = self._upgrade(text)
    self.assertEqual(new_text, expected_text)
    self.assertIn("unaliased `import machina`", "\n".join(errors))
    ...
    self.assertEqual(new_text, expected_text)
    self.assertIn("`machina.compat.v1` was directly imported as `tf`",
                  report)
    self.assertEmpty(errors)

    import_header = "from machina.compat import v1 as tf\n"
    text = import_header + old_symbol
    expected_text = import_header + old_symbol
    _, report, errors, new_text = self._upgrade(text)
    self.assertEqual(new_text, expected_text)
    ...
    import_header = "from machina.compat import v1 as tf, v2 as tf2\n"
    text = import_header + old_symbol
    expected_text = import_header + old_symbol
    _, report, errors, new_text = self._upgrade(text)
    self.assertEqual(new_text, expected_text)
    ...
    import_header = "import machina.compat.v2 as tf\n"
    text = import_header + old_symbol
    expected_text = import_header + old_symbol
    _, report, errors, new_text = self._upgrade(text)
    self.assertEqual(new_text, expected_text)
    self.assertIn("`machina.compat.v2` was directly imported as `tf`",
                  report)
    self.assertEmpty(errors)

    import_header = "from machina.compat import v1 as tf1, v2 as tf\n"
    text = import_header + old_symbol
    expected_text = import_header + old_symbol
    _, report, errors, new_text = self._upgrade(text)
    self.assertEqual(new_text, expected_text)
