...
def jni_utils_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/java/src/main/native:jni_utils."""
    return ["//machina/lite:__subpackages__"]
...
def op_resolver_internal_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/core/api:op_resolver_internal.

    This is a no-op outside of Google."""
    return []

def c_api_opaque_internal_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/c:c_api_opaque_internal.

    This is a no-op outside of Google."""
    return []

def nnapi_plugin_impl_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/acceleration/configuration:nnapi_plugin_impl.

    This is a no-op outside of Google."""
    return []

def nnapi_sl_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/nnapi/sl:nnapi_support_library_headers.

    This is a no-op outside of Google."""
    return []

def nnapi_native_srcs_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/delegates/nnapi/java/src/main/native:native_srcs

    This is a no-op outside of Google."""
    return []

def verifier_internal_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/tools:verifier_internal.

    This is a no-op outside of Google."""
    return []

def gpu_compatibility_without_gl_deps_internal_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/experimental/acceleration/compatibility:gpu_compatibility_without_gl_deps.

    This is a no-op outside of Google."""
    return []

def xnnpack_plugin_impl_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite/core/acceleration/configuration:xnnpack_plugin.

    This is a no-op outside of Google."""
    return []

def tflite_internal_cc_3p_api_deps_src_all_visibility_allowlist():
    """Returns a list of packages that can depend on machina/lite:tflite_internal_cc_3p_api_deps_src_all.
...
def if_nnapi(supported, not_supported = [], supported_android = None):
    ...
    return select({
        clean_dep("//machina:emscripten"): not_supported,
        clean_dep("//machina:ios"): not_supported,
        clean_dep("//machina:macos"): not_supported,
        clean_dep("//machina:windows"): not_supported,
        clean_dep("//machina:android"): supported_android,
...
def tflite_hexagon_nn_skel_libraries():
    ...
    https://www.machina.org/lite/performance/hexagon_delegate#step_2_add_hexagon_libraries_to_your_android_app.
...
def flex_portable_machina_deps():
    """Returns dependencies for building portable machina in Flex delegate."""
