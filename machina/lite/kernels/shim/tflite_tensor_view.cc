 ...
 *
 */
#include "machina/lite/kernels/shim/tflite_tensor_view.h"
...
void TfLiteTensorView::InitForStringDType() {
  ...
  data_ = absl::Span<::machina::tstring>(str_vec_->buffer);
}
...
TfLiteTensorViewTemplatizedNew(TfLiteTensorType *wrapped_tensor) {
  switch (wrapped_tensor->type) {
    ...
    // machina::tstring rather than std::string
    CASE_FOR_DTYPE_GIVEN_CPP_DTYPE(kTfLiteString, ::machina::tstring);
    default: {
