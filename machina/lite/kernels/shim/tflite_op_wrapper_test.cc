...
TEST_F(SetVariantOpTest, TestSetVariantOp_3x3) {
  auto op_wrapper = OpWrapperFriend<
      ...
      Attr<AttrName<kAttrName1>, bool, int64_t, ::machina::tstring>,
      Attr<AttrName<kAttrName2>, float, int32_t, uint32_t>>();

  auto map = CreateAttrMap<::machina::tstring, int32_t>();
  auto context = FakeInitContext(&map);
  EXPECT_OK(op_wrapper.Init(&context));

  bool b;
  b = std::holds_alternative<
      TmplOp<Runtime::kTfLite, ::machina::tstring, int32_t>>(
      *op_wrapper.GetOp());
  EXPECT_TRUE(b);
}
...
TEST_F(SetVariantOpTest, TestSetVariantOp_2x2x2) {
  auto op_wrapper = OpWrapperFriend<
      ...
      Attr<AttrName<kAttrName3>, ::machina::tstring, int64_t>>();

  auto map = CreateAttrMap<int32_t, uint32_t, ::machina::tstring>();
  auto context = FakeInitContext(&map);
  EXPECT_OK(op_wrapper.Init(&context));

  bool b = std::holds_alternative<
      TmplOp<Runtime::kTfLite, int32_t, uint32_t, ::machina::tstring>>(
      *op_wrapper.GetOp());
  EXPECT_TRUE(b);
}
...
TEST_F(SetVariantOpTest, TestSetVariantOp_2x1x3) {
  auto op_wrapper = OpWrapperFriend<
      Runtime::kTfLite, TmplOp, Attr<AttrName<kAttrName1>, bool, int32_t>,
      Attr<AttrName<kAttrName2>, float>,
      Attr<AttrName<kAttrName3>, ::machina::tstring, int64_t, uint32_t>>();

  auto map = CreateAttrMap<int32_t, float, ::machina::tstring>();
  auto context = FakeInitContext(&map);
  EXPECT_OK(op_wrapper.Init(&context));

  bool b = std::holds_alternative<
      TmplOp<Runtime::kTfLite, int32_t, float, ::machina::tstring>>(
      *op_wrapper.GetOp());
  EXPECT_TRUE(b);
}
