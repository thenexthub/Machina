 ...
 *
 */
#ifndef MACHINA_LITE_KERNELS_SHIM_TF_TENSOR_VIEW_H_
#define MACHINA_LITE_KERNELS_SHIM_TF_TENSOR_VIEW_H_
...
class TfTensorView : public TensorView {
 ...
 protected:
  ...
  TfTensorView(const ::machina::Tensor *wrapped_tensor, const DType &dtype);
...
// Map ::machina::Tensor -> TfTensorView
template <>
struct TensorViewSubType<::machina::Tensor> {
  using Type = TfTensorView;
};

// Map const ::machina::Tensor -> const TfTensorView
template <>
struct TensorViewSubType<const ::machina::Tensor> {
  using Type = const TfTensorView;
};

// Specialization of New() factory
template <>
absl::StatusOr<TfTensorView> TensorView::New<::machina::Tensor>(
    ::machina::Tensor *wrapped_tensor);

// Specialization of New() factory
template <>
absl::StatusOr<const TfTensorView> TensorView::New<const ::machina::Tensor>(
    const ::machina::Tensor *wrapped_tensor);
...
// Templated ctor
template <typename DType>
TfTensorView::TfTensorView(const ::machina::Tensor *wrapped_tensor,
                           const DType &dtype)
...
#endif  // MACHINA_LITE_KERNELS_SHIM_TF_TENSOR_VIEW_H_
