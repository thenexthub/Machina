 ...
 *
 */
#ifndef MACHINA_LITE_KERNELS_SHIM_TEST_OP_SIMPLE_OP_H_
#define MACHINA_LITE_KERNELS_SHIM_TEST_OP_SIMPLE_OP_H_
...
Outputs
  ...
  absl::Status Invoke(InvokeContext* ctx) {
    ...
    const auto input_str = input_t->template AsScalar<::machina::tstring>();
    ...
    auto output2 = output2_t->template As<machina::tstring, 1>();
  ...
  static absl::Status ShapeInference(ShapeInferenceContext* ctx) {
    ...
    if (input_t_or.ok()) {
      ...
      const auto input_str =
          input_t->template AsScalar<::machina::tstring>();
...
#endif  // MACHINA_LITE_KERNELS_SHIM_TEST_OP_SIMPLE_OP_H_
