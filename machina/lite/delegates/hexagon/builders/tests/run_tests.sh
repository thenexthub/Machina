...
bazel --bazelrc=/dev/null build -c opt --config=android_arm64 //machina/lite/delegates/hexagon/builders/tests:all
bazel --bazelrc=/dev/null build -c opt --config=android_arm64 //machina/lite/delegates/hexagon/hexagon_nn:libhexagon_interface.so

adb push bazel-bin/machina/lite/delegates/hexagon/hexagon_nn/libhexagon_interface.so "${on_device_dir}"
adb push ${hexagon_libs_path} "${on_device_dir}"

for test_binary in bazel-bin/machina/lite/delegates/hexagon/builders/tests/hexagon_*_test; do
