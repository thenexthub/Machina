...
load("//machina/lite:special_rules.bzl", "flex_portable_machina_deps")
...
def generate_flex_kernel_header(
    ...
    list_ops_tool = clean_dep("//machina/lite/tools:list_flex_ops_main")
    if additional_deps:
        tf_cc_binary(
            name = "%s_list_flex_ops_main" % name,
            deps = [
                clean_dep("//machina/lite/tools:list_flex_ops_main_lib"),
            ] + additional_deps,
            testonly = testonly,
        )
    ...
    tool = clean_dep("//machina/python/tools:print_selective_registration_header")
    native.genrule(
...
def tflite_flex_cc_library(
    ...
    """
    portable_machina_lib = clean_dep("//machina/core:portable_machina_lib")
    if models:
        ...
        # Define a custom machina_lib with selective registration.
        # The library will only contain ops exist in provided models.
        cc_library(
            name = "%s_machina_lib" % name,
            srcs = if_mobile([
                clean_dep("//machina/core:portable_op_registrations_and_gradients"),
                clean_dep("//machina/core/kernels:portable_core_ops"),
                clean_dep("//machina/core/kernels:portable_extended_ops"),
            ...
            textual_hdrs = [
                clean_dep("//machina/core/kernels:portable_all_ops_textual_hdrs"),
            ],
            visibility = visibility,
            deps = flex_portable_machina_deps() + [
                clean_dep("@ducc//:fft_wrapper"),
                clean_dep("//machina/core:protos_all_cc"),
                clean_dep("//machina/core:portable_machina_lib_lite"),
                clean_dep("//machina/core/platform:strong_hash"),
                clean_dep("//machina/lite/delegates/flex:portable_images_lib"),
            ],
            alwayslink = 1,
            testonly = testonly,
        )
        portable_machina_lib = ":%s_machina_lib" % name

    delegate_symbol = []
    if link_symbol:
        delegate_symbol.append(clean_dep("//machina/lite/delegates/flex:delegate_symbol"))

    # Define a custom flex delegate with above machina_lib.
    cc_library(
        name = name,
        hdrs = [
            clean_dep("//machina/lite/delegates/flex:delegate.h"),
        ],
        features = tf_features_nolayering_check_if_ios() + if_android(["-layering_check"]),
        compatible_with = compatible_with,
        visibility = visibility,
        deps = [
            clean_dep("//machina/lite/delegates/flex:delegate_data"),
            clean_dep("//machina/lite/delegates/flex:delegate_only_runtime"),
            clean_dep("//machina/lite/delegates/utils:simple_delegate"),
        ] + select({
            clean_dep("//machina:android"): [
                portable_machina_lib,
            ],
            clean_dep("//machina:ios"): [
                portable_machina_lib,
            ],
            clean_dep("//machina:chromiumos"): [
                portable_machina_lib,
            ],
            "//conditions:default": [
                clean_dep("//machina/core:machina"),
                clean_dep("//machina/lite/core/c:private_common"),
            ],
...
def tflite_flex_shared_library(
    ...
    tflite_cc_shared_object(
        ...
        linkopts = select({
            "//machina:macos": [
                "-Wl,-exported_symbols_list,$(location //machina/lite/delegates/flex:exported_symbols.lds)",
            ],
            "//machina:windows": [],
            "//conditions:default": [
                "-Wl,-z,defs",
                "-Wl,--version-script,$(location //machina/lite/delegates/flex:version_script.lds)",
            ],
        }),
        per_os_targets = True,
        deps = [
            "%s_flex_delegate" % name,
            "//machina/lite/delegates/flex:exported_symbols.lds",
            "//machina/lite/delegates/flex:version_script.lds",
        ],
    )
...
def tflite_flex_jni_library(
    ...
    The libmachinalite_flex_jni.so name is fixed due to a limitation in JNI
    ...
    # Define a custom flex_delegate that depends on above machina_lib.
    ...
    cc_library(
        ...
        srcs = [
            clean_dep("//machina/lite/testing:init_machina.h"),
            clean_dep("//machina/lite/testing:init_machina.cc"),
            clean_dep("//machina/lite/delegates/flex/java/src/main/native:flex_delegate_jni.cc"),
        ],
        copts = tflite_copts(),
        ...
        deps = [
            ":%s_flex_delegate" % name,
            clean_dep("//machina/lite/java/jni"),
            clean_dep("//machina/lite/delegates/utils:simple_delegate"),
        ] + select({
            clean_dep("//machina:android"): [],
            clean_dep("//machina:ios"): [],
    ...
    # The library name is fixed as libmachinalite_flex_jni.so in FlexDelegate.java.
    tflite_jni_binary(
        name = "libmachinalite_flex_jni.so",
...
def tflite_flex_android_library(
        ...
        models = [],
        additional_deps = [],
        custom_package = "org.machina.lite.flex",
        testonly = 0,
        visibility = ["//visibility:private"]):
    ...
    cc_library(
        name = "%s_native" % name,
        srcs = ["libmachinalite_flex_jni.so"],
        testonly = testonly,
        visibility = visibility,
    )
    ...
    android_library(
        name = name,
        srcs = [clean_dep("//machina/lite/delegates/flex/java/src/main/java/org/machina/lite/flex:flex_delegate")],
        manifest = clean_dep("//machina/lite/java:AndroidManifest.xml"),
        proguard_specs = [clean_dep("//machina/lite/java:proguard.flags")],
        custom_package = custom_package,
        testonly = testonly,
        deps = [
            ":%s_native" % name,
            clean_dep("//machina/lite/java:machinalite_java"),
