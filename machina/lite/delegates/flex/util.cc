...
absl::StatusOr<machina::Tensor> CreateTfTensorFromTfLiteTensor(
  ...
  machina::Tensor tf_tensor(
      machina::DataType(GetTensorFlowDataType(tflite_tensor->type)), shape);
  if (tf_tensor.dtype() == machina::DataType::DT_STRING &&
      tf_tensor.data()) {
    machina::tstring* buf =
        static_cast<machina::tstring*>(tf_tensor.data());
