...
TEST(UtilTest, ConvertStatus) {
  ...
  EXPECT_EQ(ConvertStatus(&context, machina::errors::Internal("Some Error")),
            kTfLiteError);
...
TEST(UtilTest, CopyShapeAndType) {
  ...
  EXPECT_EQ(
      CopyShapeAndType(&context, Tensor(machina::DT_HALF, {1, 2}), &dst),
      kTfLiteOk);
  EXPECT_THAT(context.new_size, ElementsAre(1, 2));
...
TEST(UtilTest, CreateTfTensorFromTfLiteTensorFloat) {
  ...
  machina::Tensor tf_tensor = tf_tensor_or.value();
...
TEST(UtilTest, CreateTfTensorFromTfLiteTensorString) {
  ...
  auto* tf_data = static_cast<machina::tstring*>(tf_tensor.data());
