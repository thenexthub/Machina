...
#include "machina/lite/delegates/flex/test_util.h"
...
class KernelTest : public testing::FlexModelTest {
 public:
  ...
  TfLiteStatus ApplyFlexDelegate(
    ...
    CHECK_OK(delegate_data_->Prepare(machina::SessionOptions{}));
...
machina::OpDef MakeOpDef(int num_inputs, int num_outputs) {
  machina::OpRegistrationData op_reg_data;
  machina::OpDefBuilder b("dummy");
  for (int i = 0; i < num_inputs; ++i) {
    b.Input(machina::strings::StrCat("i", i, ": float"));
  }
  for (int i = 0; i < num_outputs; ++i) {
    b.Output(machina::strings::StrCat("o", i, ": float"));
  }
  CHECK_OK(b.Attr("foo:string").Finalize(&op_reg_data));
  return op_reg_data.op_def;
}

machina::PartialTensorShape S(std::initializer_list<int64_t> dims) {
  return machina::PartialTensorShape(dims);
}

TEST(ValidateOutputTensorShapeConsistencyTest, ShapeHandleDebugString) {
  // Setup test to contain an input tensor list of size 3.
  machina::OpDef op_def = MakeOpDef(4, 1);
  machina::NodeDef def;
  machina::shape_inference::InferenceContext c(
      0, def, op_def, {S({1}), S({2, 3}), S({4, 5, 6}), {}}, {}, {}, {});
  c.SetInput(3, c.UnknownShape());

  std::vector<machina::shape_inference::ShapeHandle> shapes;
