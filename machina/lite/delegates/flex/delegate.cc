...
TfLiteStatus FlexDelegate::Initialize(TfLiteContext* context) {
  ...
  if (!cancellation_manager_) {
    cancellation_manager_ = std::make_unique<machina::CancellationManager>();
...
TfLiteStatus FlexDelegate::CopyFromBufferHandle(
  ...
  machina::Tensor t = buffer_map->GetTensor(buffer_handle);

  if (output->type == kTfLiteString) {
    if (t.dtype() != machina::DT_STRING) {
    ...
    auto tf_data = t.flat<machina::tstring>();
