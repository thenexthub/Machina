 ...
 *
 */
#include "machina/lite/delegates/flex/kernel.h"
...
#include "machina/lite/delegates/flex/delegate.h"
...
class OpInputs {
 ...
 private:
  ...
  // in this list while the corresponding machina::OpKernel is executed.
  std::vector<int> forwardable_;
};

// A list of outputs of a given node of the TensorFlow graph, along with
// the actual outputs of the machina::OpKernel.
class OpOutputs {
 public:
  ...
  const machina::Tensor& GetTensor(int i) const { return vector_[i]; }
  machina::Tensor ReleaseTensor(int i) { return std::move(vector_[i]); }

  int Size() const { return outputs_.size(); }

  int TfLiteIndex(int i) const { return outputs_[i]; }

  absl::InlinedVector<machina::Tensor, 2UL>* GetTensors() {
    return &vector_;
  }
 ...
 private:
  ...
  absl::InlinedVector<machina::Tensor, 2UL> vector_;
};
...
class OpNode {
 public:
  ...
  const machina::NodeDef& nodedef() const { return nodedef_; }
  const machina::OpRegistrationData* op_reg_data() const {
    return op_reg_data_;
  }
  ...
  const machina::tfrt_stub::OpKernelRunner& op_kernel_runner() const {
  ...
  absl::Status InitializeNodeDef(const void* custom_initial_data,
    ...
    TF_RETURN_IF_ERROR(
        machina::OpRegistry::Global()->LookUp(nodedef_.op(), &op_reg_data_));
    ...
    //
    // It should be ok to remove this when/if the machina::Executor::Run
  ...
  absl::Status BuildOpKernelRunner(machina::EagerContext* eager_context) {
    // Create machina::OpKernel on host CPU.
    TF_ASSIGN_OR_RETURN(op_kernel_runner_,
                        machina::tfrt_stub::OpKernelRunner::Create(
                            name_, inputs_.Size(), /*attr_builder=*/
                            [this](machina::AttrValueMap* attr_value_map) {
  ...
  absl::Status MaybePersistTensorflowOutputs(TfLiteContext* context,
    ...
    for (int i = 0; i < outputs_.Size(); ++i) {
      if (outputs_.IsSubgraphOutput(i)) {
        machina::Tensor& tf_tensor = tensors->at(i);
...
absl::Status DelegateKernel::ExecuteOpKernelRunner(
    machina::tfrt_stub::OpKernelRunState* run_state, TfLiteContext* context,
    OpNode* node_data) {
  const auto& op_kernel_runner = node_data->op_kernel_runner();

  if (op_kernel_runner.op_kernel()->num_outputs() != node_data->NumOutputs()) {
    return machina::errors::Internal(
        "Unexpected number of outputs from machina::OpKernel");
  }
  ...
  machina::OpKernelContext tf_context(&run_state->params,
...
TfLiteStatus DelegateKernel::Init(TfLiteContext* context,
  ...
  // tensors that are not produced by machina::Opkernels.
...
TfLiteStatus DelegateKernel::Prepare(TfLiteContext* context, TfLiteNode* node) {
  TF_LITE_ENSURE_MSG(
      ...
      "machina/core:core_cpu_impl were not linked into the binary.");
...
TfLiteStatus DelegateKernel::ValidateOutputTensorShapeConsistency(
  ...
  for (const auto& node_data : op_data_->nodes) {
    ...
    std::vector<const machina::Tensor*> input_tensors_vector(num_inputs,
                                                                nullptr);
    InferenceContext c(
...
static machina::CancellationManager* GetDefaultCancellationManager() {
  static auto* const cancellation_manager = new machina::CancellationManager;
...
TfLiteStatus DelegateKernel::Eval(TfLiteContext* context, TfLiteNode* node) {
  ...
  for (auto tensor_index : op_data_->subgraph_outputs) {
    ...
    TfLiteTensor* tensor = &context->tensors[tensor_index];
    const machina::Tensor& tf_tensor = buffer_map->GetTensor(tensor_index);
    if (tensor->allocation_type == kTfLiteDynamic) {
