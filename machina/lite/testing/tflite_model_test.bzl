...
tflite_model_test(
    ...
    machina_model_file = "//machina/lite:testdata/multi_add.pb",
    input_layer = "a,b,c,d",
...
def tflite_model_test(
    ...
    Args:
      ...
      machina_model_file: The binary GraphDef proto to run the benchmark on.
    ...
    diff_args = [
        ...
        # "--machina_model=%s" % machina_model_file.replace("//", "").replace(":", "/"),
        # "--tflite_model=%s" % tflite_model_file.replace("//", "").replace(":", "/"),
        "--machina_model=$(location %s)" % machina_model_file,
    ...
    tf_cc_test(
        name = name,
        size = size,
        srcs = ["//machina/lite/testing:tflite_diff_example_test.cc"],
        args = diff_args,
        data = [
            machina_model_file,
            tflite_model_file,
        ...
        tags = tags,
        deps = [
            "//machina/lite/testing:init_machina",
            "//machina/lite/testing:tflite_diff_flags",
            "//machina/lite/testing:tflite_diff_util",
        ],
    )
...
def make_tflite_files(
    ...
    tool = "//machina/lite/python:tflite_convert"
    cmd = ("$(location %s) " +
    ...
    native.genrule(
        ...
        tools = [tool],
        visibility = ["//machina/lite/testing:__subpackages__"],
    )
