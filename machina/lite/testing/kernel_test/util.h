 ...
 *
 */
#ifndef MACHINA_LITE_TESTING_KERNEL_TEST_UTIL_H_
#define MACHINA_LITE_TESTING_KERNEL_TEST_UTIL_H_

#include <fstream>

#include "machina/core/util/command_line_flags.h"
#include "machina/lite/core/c/common.h"
#include "machina/lite/testing/kernel_test/input_generator.h"
...
inline TestOptions ParseTfliteKernelTestFlags(int* argc, char** argv) {
  ...
  std::vector<machina::Flag> flags = {
      machina::Flag("tflite_model", &options.tflite_model,
                       "Path of machina lite model."),
      machina::Flag("read_input_from_file", &options.read_input_from_file,
                       "File to read input data from. If empty, generates "
                       "input at runtime."),
      machina::Flag("dump_input_to_file", &options.dump_input_to_file,
                       "File to dump randomly generated input."),
      machina::Flag("dump_output_to_file", &options.dump_output_to_file,
                       "File to dump output."),
      machina::Flag("input_distribution", &options.input_distribution,
                       "Input distribution. Default: Gaussian."),
      machina::Flag("kernel_type", &options.kernel_type, "Kernel type."),
  };

  machina::Flags::Parse(argc, argv, flags);
...
#endif  // MACHINA_LITE_TESTING_KERNEL_TEST_UTIL_H_
