...
machina::Tensor CreateTensor(const machina::DataType type,
                                const std::vector<int64_t>& dim) {
  machina::TensorShape shape{absl::Span<const int64_t>{
      reinterpret_cast<const int64_t*>(dim.data()), dim.size()}};
  return {type, shape};
}

template <typename T>
int FillTensorWithData(machina::Tensor* tensor,
...
// TensorFlow tensor.
int FillTensorWithTfLiteHexString(machina::Tensor* tensor,
                                  const string& values_as_string) {
  ...
  if (num_strings == tensor->NumElements()) {
    auto data = tensor->flat<machina::tstring>();
...
template <typename T>
void FillTensorWithZeros(machina::Tensor* tensor) {
  auto data = tensor->flat<T>();
...
template <typename T>
string TensorDataToCsvString(const machina::Tensor& tensor) {
  const auto& data = tensor.flat<T>();
  return Join(data.data(), data.size(), ",");
}

string TensorDataToTfLiteHexString(const machina::Tensor& tensor) {
...
void TfDriver::LoadModel(const string& bin_file_path) {
  ...
  if (!graphdef.ParseFromIstream(&model)) {
    Invalidate("Failed to parse machina graphdef");
    return;
  }

  machina::SessionOptions options;
  session_.reset(machina::NewSession(options));
  ...
  switch (input_types_[id]) {
    ...
    default:
      Invalidate(absl::StrCat("Unsupported tensor type ", input_types_[id],
                              machina::DataType_Name(input_types_[id]),
                              " in ResetInput"));
      return;
  }
}
  ...
  switch (tensor->dtype()) {
    ...
    default:
      Invalidate(absl::StrCat("Unsupported tensor type ",
                              machina::DataType_Name(tensor->dtype()),
                              " in SetInput"));
      return;
  }
...
string TfDriver::ReadOutput(const machina::Tensor& tensor) {
  switch (tensor.dtype()) {
    case machina::DT_FLOAT:
