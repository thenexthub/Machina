...
#include "machina/core/util/stats_calculator.h"
#include "machina/lite/profiling/proto/profiling_info.pb.h"
...
std::string ProfileSummaryDefaultFormatter::GetOutputString(
    const std::map<uint32_t, std::unique_ptr<machina::StatsCalculator>>&
        stats_calculator_map,
    const machina::StatsCalculator& delegate_stats_calculator,
...
machina::StatSummarizerOptions
ProfileSummaryDefaultFormatter::GetStatSummarizerOptions() const {
  auto options = machina::StatSummarizerOptions();
...
std::vector<machina::StatsCalculator::Detail>
ProfileSummaryProtoFormatter::GetDetailsSortedByRunOrder(
    const machina::StatsCalculator* stats_calculator) const {
  std::vector<machina::StatsCalculator::Detail> details;
  std::map<std::string, machina::StatsCalculator::Detail> unsorted_details =
      stats_calculator->GetDetails();

  std::priority_queue<
      std::pair<std::string, const machina::StatsCalculator::Detail*>>
      sorted_list;
  const int num_nodes = unsorted_details.size();
  for (const auto& det : unsorted_details) {
    const machina::StatsCalculator::Detail* detail = &(det.second);
...
void ProfileSummaryProtoFormatter::GenerateOpProfileDataFromDetail(
    const machina::StatsCalculator::Detail* detail,
    const machina::StatsCalculator* stats_calculator,
...
void ProfileSummaryProtoFormatter::GenerateSubGraphProfilingData(
    const machina::StatsCalculator* stats_calculator, int subgraph_index,
  ...
  for (machina::StatsCalculator::Detail& detail :
...
void ProfileSummaryProtoFormatter::GenerateDelegateProfilingData(
  ...
  for (const machina::StatsCalculator::Detail& detail :
       GetDetailsSortedByRunOrder(stats_calculator)) {
    OpProfileData* const op_profile_data =
...
std::string ProfileSummaryProtoFormatter::GetOutputString(
  ...
  for (const auto& stats_calc : stats_calculator_map) {
    auto subgraph_index = stats_calc.first;
    machina::StatsCalculator* subgraph_stats = stats_calc.second.get();
