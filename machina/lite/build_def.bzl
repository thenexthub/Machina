...
load("//machina:machina.bzl", "if_oss", "tf_binary_additional_srcs", "tf_cc_shared_object")
load("//machina/lite:special_rules.bzl", "tflite_copts_extra")
...
def tflite_copts():
    ...
    copts = [
        ...
        clean_dep("//machina:android_arm"): [
            "-mfpu=neon",
        ],
        # copybara:uncomment_begin(google-only)
        # clean_dep("//machina:chromiumos_x86_64"): [],
        # copybara:uncomment_end
        clean_dep("//machina:ios_x86_64"): [
            "-msse4.1",
        ],
        clean_dep("//machina:linux_x86_64"): [
            "-msse4.2",
        ],
        clean_dep("//machina:linux_x86_64_no_sse"): [],
        ...
        clean_dep("//machina:optimized"): ["-O3"],
        "//conditions:default": [],
    }) + select({
        ...
        clean_dep("//machina/lite:tflite_with_xnnpack_explicit_false"): ["-DTFLITE_WITHOUT_XNNPACK"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/lite:machina_profiler_config"): ["-DTF_LITE_MACHINA_PROFILER"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/lite/delegates:tflite_debug_delegate"): ["-DTFLITE_DEBUG_DELEGATE"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/lite:tflite_mmap_disabled"): ["-DTFLITE_MMAP_DISABLED"],
        "//conditions:default": [],
    })
...
EXPORTED_SYMBOLS = clean_dep("//machina/lite/java/src/main/native:exported_symbols.lds")
LINKER_SCRIPT = clean_dep("//machina/lite/java/src/main/native:version_script.lds")
...
def tflite_symbol_opts():
    ...
    return select({
        clean_dep("//machina:debug"): [],
        clean_dep("//machina/lite:tflite_keep_symbols"): [],
        "//conditions:default": [
...
def tf_to_tflite(name, src, options, out):
    """Convert a frozen machina graphdef to TF Lite's flatbuffer.
    ...
    toco_cmdline = " ".join([
        "$(location //machina/lite/python:tflite_convert)",
        "--enable_v1_converter",
    ...
    native.genrule(
        ...
        outs = [out],
        cmd = toco_cmdline,
        tools = ["//machina/lite/python:tflite_convert"] + tf_binary_additional_srcs(),
    )

def DEPRECATED_tf_to_tflite(name, src, options, out):
    """DEPRECATED Convert a frozen machina graphdef to TF Lite's flatbuffer, using toco.
    ...
    toco_cmdline = " ".join([
        "$(location //machina/lite/toco:toco)",
        "--input_format=MACHINA_GRAPHDEF",
    ...
    native.genrule(
        ...
        outs = [out],
        cmd = toco_cmdline,
        tools = ["//machina/lite/toco:toco"] + tf_binary_additional_srcs(),
    )
...
def tflite_to_json(name, src, out):
    ...
    schema = "//machina/lite/schema:schema.fbs"
    native.genrule(
        name = name,
        srcs = [schema, src],
        outs = [out],
...
def json_to_tflite(name, src, out):
    ...
    flatc = "@flatbuffers//:flatc"
    schema = "//machina/lite/schema:schema_fbs"
    native.genrule(
        name = name,
        srcs = [schema, src],
        outs = [out],
    ...
    attrs = {
        ...
        "_generate_op_registrations": attr.label(
            ...
            default = Label(clean_dep(
                "//machina/lite/tools:generate_op_registrations",
            )),
            cfg = "exec",
        ),
    },
...
def flex_dep(target_op_sets):
    if "SELECT_TF_OPS" in target_op_sets:
        return ["//machina/lite/delegates/flex:delegate"]
    else:
        return []
...
def gen_model_coverage_test(
    ...
    for target_op_sets in ["TFLITE_BUILTINS", "TFLITE_BUILTINS,SELECT_TF_OPS", "SELECT_TF_OPS"]:
        ...
        full_deps = [
            ...
            "//machina/lite/testing/model_coverage:model_coverage_lib",
...
def tflite_custom_cc_library(
    ...
    if models:
        ...
        real_srcs.append("//machina/lite:create_op_resolver_with_selected_ops.cc")
    else:
        # Support all operators if `models` not specified.
        real_deps.append("//machina/lite:create_op_resolver_with_builtin_ops")

    if experimental:
        framework = "//machina/lite:framework_experimental"
    else:
        framework = "//machina/lite:framework_stable"
    native.cc_library(
        name = name,
        srcs = real_srcs,
        hdrs = [
            "//machina/lite:create_op_resolver.h",
        ],
        copts = tflite_copts(),
        linkopts = select({
            "//machina:windows": [],
        ...
        deps = depset([
            ...
            "//machina/lite/core:private_create_op_resolver_header",
        ] + real_deps),
        visibility = visibility,
        **kwargs
    )
...
def tflite_custom_android_library(
        ...
        srcs = [],
        deps = [],
        custom_package = "org.machina.lite",
        visibility = ["//visibility:private"],
    ...
    has to be named as machinalite_jni.so so please make sure that there is no naming conflict.
    ...
    if include_nnapi_delegate:
        delegate_deps.append("//machina/lite/delegates/nnapi/java/src/main/native")
    if include_xnnpack_delegate:
        delegate_deps.append("//machina/lite/delegates/xnnpack:xnnpack_delegate")

    if experimental:
        native_framework_only = "//machina/lite/java/src/main/native:native_experimental_framework_only"
    else:
        native_framework_only = "//machina/lite/java/src/main/native:native_stable_framework_only"

    # JNI wrapper expects a binary file called `libmachinalite_jni.so` in java path.
    tflite_jni_binary(
    ...
    native.cc_library(
        name = "%s_jni" % name,
        srcs = ["libmachinalite_jni.so"],
        visibility = visibility,
    )

    if experimental:
        java_srcs = "//machina/lite/java:java_srcs"
    else:
        java_srcs = "//machina/lite/java:java_stable_srcs"

    android_library(
        name = name,
        manifest = "//machina/lite/java:AndroidManifest.xml",
        srcs = [java_srcs],
        deps = [
            ":%s_jni" % name,
...
def tflite_custom_c_library(
    ...
    """
    op_resolver_deps = "//machina/lite:create_op_resolver_with_builtin_ops"
    if models:
        gen_selected_ops(
            name = "%s_registration" % name,
            model = models,
        ...
        native.cc_library(
            ...
            hdrs = ["//machina/lite:create_op_resolver.h"],
            copts = tflite_copts(),
            deps = [
                "//machina/lite/core:private_create_op_resolver_header",
                "//machina/lite:create_op_resolver_with_selected_ops",
    ...
    if experimental:
        ...
        deps = [
            "//machina/lite/c:c_api_experimental_without_op_resolver_without_alwayslink",
            "//machina/lite/core/c:private_c_api_experimental_without_op_resolver_without_alwayslink",
            "//machina/lite/c:c_api_opaque_without_op_resolver_without_alwayslink",
            "//machina/lite/core/c:private_c_api_opaque_without_op_resolver_without_alwayslink",
        ]
    else:
        hdrs = [
            "//machina/lite/c:c_api.h",
    ...
    native.cc_library(
        ...
        deps = [
            ...
            "//machina/lite/c:c_api_without_op_resolver_without_alwayslink",
            # TODO(bekzhan): Remove this dependency after we move c_api_opaque.h to tflite/core/.
            "//machina/lite/core/c:private_c_api_types",
            "//machina/lite/core/c:private_c_api_without_op_resolver_without_alwayslink",
            "//machina/lite/core/c:private_common",
...
def _label(target):
    ...
    """
    if target[0:2] == "//" or "@org_machina//" in target:
        return Label(target)
    if target[0] == ":":
