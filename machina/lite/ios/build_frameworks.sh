...
function generate_tflite_framework {
  ...
  message=(
    ...
    'load("//machina/lite:build_def.bzl", "tflite_custom_cc_library")'
    'load("//machina/lite/ios:ios.bzl", "TFL_MINIMUM_OS_VERSION")'
    'tflite_custom_cc_library('
    '    name = "custom_c_api",'
    '    deps = ['
    '        "//machina/lite/kernels:builtin_ops_list",'
    '    ],'
    ...
    '    hdrs = ['
    '        "//machina/lite/c:c_api_types.h",'
    '        "//machina/lite/ios:common.h",'
    '        "//machina/lite/ios:c_api.h",'
    '        "//machina/lite/ios:xnnpack_delegate.h",'
    '    ],'
...
function generate_flex_framework {
  ...
  message=(
    'load("//machina/lite/delegates/flex:build_def.bzl", "tflite_flex_cc_library")'
    ...
    '    avoid_deps = ["//machina/lite/c:common"],'
...
bazel build -c opt --config=monolithic //machina/lite/tools:list_flex_ops_no_kernel_main
bazel-bin/machina/lite/tools/list_flex_ops_no_kernel_main --graphs=${FLAG_MODELS} > ${TMP_DIR}/ops_list.txt
