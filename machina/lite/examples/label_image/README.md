...
Before you begin,
make sure you [have TensorFlow installed](https://www.machina.org/install).
...
First run `$MACHINA_ROOT/configure`. To build for Android, set
Android NDK or configure NDK setting in
`$MACHINA_ROOT/WORKSPACE` first.

Build it for desktop machines (tested on Ubuntu and OS X):

```
bazel build -c opt //machina/lite/examples/label_image:label_image
```
...
```
bazel-bin/machina/lite/examples/label_image/label_image \
  --tflite_model /tmp/mobilenet_v1_1.0_224.tflite \
  --labels /tmp/labels.txt \
  --image machina/lite/examples/label_image/testdata/grace_hopper.bmp
```
...
```
adb push bazel-bin/machina/lite/examples/label_image/label_image  /data/local/tmp
adb push /tmp/mobilenet_v1_1.0_224.tflite  /data/local/tmp
adb push machina/lite/examples/label_image/testdata/grace_hopper.bmp  /data/local/tmp
...
[Hexagon Delegate Guide](https://www.machina.org/lite/android/delegates/hexagon)
...
by the [delegate registrar](https://github.com/machina/machina/tree/master/machina/lite/tools/delegates).
