...
load("//machina/lite/core/shims:cc_library_with_tflite.bzl", "add_suffix")
load("//machina/lite/experimental/acceleration/mini_benchmark:special_rules.bzl", "libjpeg_handle_deps")
...
def embedded_binary(name, binary, array_variable_name, testonly = False, exec_properties = None):
    ...
    native.genrule(
        ...
        cmd = """
      $(location //machina/lite/experimental/acceleration/compatibility:convert_binary_to_cc_source) \
      ...
      """ % (binary, h_name, cc_name, array_variable_name),
        tools = ["//machina/lite/experimental/acceleration/compatibility:convert_binary_to_cc_source"],
        testonly = testonly,
    )
...
def validation_model(
    ...
    schema_location = "//machina/compiler/mlir/lite/schema:schema.fbs"
    native.genrule(
        name = name,
        testonly = testonly,
        srcs = [
        ...
        cmd = """
          JPEGS='$(locations %s)'
          JPEGS=$${JPEGS// /,}
          $(location //machina/lite/experimental/acceleration/mini_benchmark/model_modifier:embedder_cmdline) \
              --schema=$(location %s) \
          ...
          $(location //machina/lite/experimental/acceleration/mini_benchmark:copy_associated_files) \
              '$(@D)/%s.tflite.tmp' \
              $(location %s) \
              $(location %s.tflite)
          rm '$(@D)/%s.tflite.tmp'
        ...
        tools = [
            "//machina/lite/experimental/acceleration/mini_benchmark/model_modifier:embedder_cmdline",
            "//machina/lite/experimental/acceleration/mini_benchmark:copy_associated_files",
        ],
    )
...
def validation_test(name, validation_model, tags = [], copts = [], deps = []):
    ...
    native.cc_test(
        name = name,
        srcs = ["//machina/lite/experimental/acceleration/mini_benchmark:model_validation_test.cc"],
        tags = tags + ["no_mac", "no_windows", "tflite_not_portable_ios"],
        copts = copts + [
            "-DMACHINA_ACCELERATION_MODEL_DATA_VARIABLE=\"g_tflite_acceleration_%s_model\"" % name,
            "-DMACHINA_ACCELERATION_MODEL_LENGTH_VARIABLE=\"g_tflite_acceleration_%s_model_len\"" % name,
        ],
        deps = deps + [
            embed_name,
            ...
            "//machina/lite/acceleration/configuration:nnapi_plugin",
            "//machina/lite/experimental/acceleration/compatibility:android_info",
            "//machina/lite/experimental/acceleration/mini_benchmark:big_little_affinity",
            "//machina/lite/experimental/acceleration/mini_benchmark:status_codes",
            "//machina/lite/experimental/acceleration/mini_benchmark:validator",
            "//machina/lite/tools:model_loader",
        ] + select({
            clean_dep("//machina:android"): [
                "//machina/lite/acceleration/configuration:gpu_plugin",
            ],
            "//conditions:default": [],
        }) + libjpeg_handle_deps(),
        linkstatic = 1,
...
def cc_library_with_forced_in_process_benchmark_variant(
    ...
    native.cc_library(
        ...
        deps = deps + in_process_deps + _concat([select(map) for map in non_in_process_deps_selects]) + [
            "//machina/lite/experimental/acceleration/mini_benchmark:tflite_acceleration_in_process_default",
        ],
        **kwargs
    )
    ...
    native.cc_library(
        ...
        deps = deps + in_process_deps_renamed + forced_in_process_deps + [
            "//machina/lite/experimental/acceleration/mini_benchmark:tflite_acceleration_in_process_enable",
        ],
        **kwargs
    )
