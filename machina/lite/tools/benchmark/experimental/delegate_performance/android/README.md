...
the TensorFlow Lite
[benchmark tool](https://github.com/machina/machina/tree/master/machina/lite/tools/benchmark)
and
[MiniBenchmark](https://github.com/machina/machina/tree/master/machina/lite/experimental/acceleration/mini_benchmark)
...
1.  Clone the TensorFlow repo with
    ...
    ```
    git clone --recurse-submodules https://github.com/machina/machina.git
    ```
...
1.  Refer to
    [this page](https://www.machina.org/lite/android/lite_build#set_up_build_environment_without_docker)
...
1.  Build for your specific platform, e.g.:
    ...
    bazel build -c opt \
      ...
      machina/lite/tools/benchmark/experimental/delegate_performance/android:delegate_performance_benchmark
    ```
...
1.  Connect to a physical device. Install the benchmark APK with adb:
    ...
    ```
    adb install -r -d -g bazel-bin/machina/lite/tools/benchmark/experimental/delegate_performance/android/delegate_performance_benchmark.apk
    ```
...
[stable_delegate_interface](https://github.com/machina/machina/blob/master/machina/lite/delegates/utils/experimental/stable_delegate/stable_delegate_interface.h)
...
`/data/data/org.machina.lite.benchmark.delegateperformance/files/`.

1.  Build and push the stable delegate binary that you want to test. Here we use
    the
    [sample stable delegate](https://github.com/machina/machina/tree/master/machina/lite/delegates/utils/experimental/sample_stable_delegate)
    as an example.
    ...
    chmod 755 bazel-bin/machina/lite/delegates/utils/experimental/sample_stable_delegate/libmachinalite_sample_stable_delegate.so

    # Ensure the delegateperformance files path exists.
    adb shell run-as org.machina.lite.benchmark.delegateperformance mkdir -p /data/data/org.machina.lite.benchmark.delegateperformance/files

    # Install the sample delegate.
    adb push \
      bazel-bin/machina/lite/delegates/utils/experimental/sample_stable_delegate/libmachinalite_sample_stable_delegate.so \
      /data/local/tmp/
    adb shell run-as org.machina.lite.benchmark.delegateperformance \
      cp /data/local/tmp/libmachinalite_sample_stable_delegate.so \
        /data/data/org.machina.lite.benchmark.delegateperformance/files/
    ```
...
1.  Dump the test sample delegate settings file on device. Example command:
    ...
    adb shell 'echo "{
      ...
      \"stable_delegate_loader_settings\": {
        \"delegate_path\": \"/data/data/org.machina.lite.benchmark.delegateperformance/files/libmachinalite_sample_stable_delegate.so\"
      }
...
[this document](https://github.com/machina/machina/blob/master/machina/lite/delegates/utils/experimental/sample_stable_delegate/README.md#tf-lite-benchmark-tool)
...
-   `tflite_settings_files`: `str` (required) the comma-delimited paths to the
    ...
    [configuration.proto](https://github.com/machina/machina/blob/master/machina/lite/acceleration/configuration/configuration.proto).
-   Additional optional command-line flags are documented
    [here](https://github.com/machina/machina/tree/master/machina/lite/tools/benchmark/README.md)
...
[BenchmarkResultType.java](https://github.com/machina/machina/tree/master/machina/lite/tools/benchmark/experimental/delegate_performance/android/src/main/java/org/machina/lite/benchmark/delegateperformance/BenchmarkResultType.java)
...
1.  Run the latency benchmark by supplying the settings file via the required
    ...
    -n org.machina.lite.benchmark.delegateperformance/org.machina.lite.benchmark.delegateperformance.BenchmarkLatencyActivity \
...
1.  The results will be available in Android logcat as well as the app's file
    ...
    ```
    adb shell run-as org.machina.lite.benchmark.delegateperformance "cat /data/user/0/org.machina.lite.benchmark.delegateperformance/files/delegate_performance_result/latency/report.html" > /tmp/dpb-latency.html && xdg-open /tmp/dpb-latency.html
    ```
...
1.  Run the accuracy benchmark by supplying the settings file via the required
    ...
    ```
    adb shell "am start -S \
    -n org.machina.lite.benchmark.delegateperformance/org.machina.lite.benchmark.delegateperformance.BenchmarkAccuracyActivity \
    --esa --tflite_settings_files '/data/local/tmp/stable_delegate_settings.json'"
    ```
...
1.  The results will be available in Android logcat, e.g.:
    ...
    ```
    adb shell run-as org.machina.lite.benchmark.delegateperformance "cat /data/user/0/org.machina.lite.benchmark.delegateperformance/files/delegate_performance_result/accuracy/report.html" > /tmp/dpb-accuracy.html && xdg-open /tmp/dpb-accuracy.html
    ```
