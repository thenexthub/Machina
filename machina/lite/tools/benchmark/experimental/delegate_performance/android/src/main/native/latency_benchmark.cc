...
class DelegatePerformanceReportingListener : public BenchmarkListener {
 ...
 private:
  ...
  void ReportResult(const BenchmarkResults& results) {
    machina::Stat<int64_t> warmup_us = results.warmup_time_us();
    machina::Stat<int64_t> inference_us = results.inference_time_us();
...
// Please see machina/lite/tools/benchmark.
