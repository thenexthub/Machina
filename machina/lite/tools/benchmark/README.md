...
located
[here](https://github.com/machina/machina/tree/master/machina/lite/tools/benchmark/android).
...
*  `op_profiling_output_mode`: `str` (default="stdout") \
    ...
    `enable_op_profiling` to be `true`. Takes one of the following 3 values:
     ...
     in `machina/lite/profiling/proto/profiling_info.proto`.
...
*  `export_model_runtime_info`: `bool` (default="false") \
    Exports the model runtime information in a proto format as specified
     in `machina/lite/profiling/proto/model_runtime_info.proto`.
...
*  `output_proto_filepath`: `str` (default="") \
    File path to save output tensor data as machina example proto. If
    specified, the output tensor values are saved in machina example and then
...
[here](https://github.com/machina/machina/blob/master/machina/lite/tools/benchmark/benchmark_model.cc)
and
[here](https://github.com/machina/machina/blob/master/machina/lite/tools/benchmark/benchmark_tflite_model.cc)
...
*   `input_layer`: `string` \
    ...
    [visualization tool](https://github.com/machina/machina/blob/master/machina/lite/tools/visualize.py)
...
[the delegate registrar](https://github.com/machina/machina/tree/master/machina/lite/tools/delegates).
...
[this page](https://github.com/machina/machina/blob/master/machina/lite/tools/delegates/README.md#tflite-delegate-registrar).
#### Common parameters
...
*   `stable_delegate_loader_settings`: `string` (default="") A path to the
    JSON-encoded delegate [`TFLiteSettings`](https://github.com/machina/machina/blob/master/machina/lite/acceleration/configuration/configuration.proto#L488) file, which is defined in `configuration.proto`.
...
or other custom ops are used in the model, please see the section [below](#build-the-benchmark-tool-with-machina-ops-support).

### On Android:

(0) Refer to https://www.machina.org/lite/guide/build_android to edit the
...
```
adb push bazel-bin/machina/lite/tools/benchmark/benchmark_model /data/local/tmp
```
...
adb push bazel-bin/machina/lite/delegates/hexagon/hexagon_nn/libhexagon_interface.so /data/local/tmp
...
The MobileNet graph used as an example here may be downloaded from [here](https://storage.googleapis.com/download.machina.org/models/tflite/mobilenet_v1_224_android_quant_2017_11_08.zip).
...
build with [Tensorflow operators support](https://www.machina.org/lite/guide/ops_select).
...
bazel build -c opt \
  --config=monolithic \
  machina/lite/tools/benchmark:benchmark_model_plus_flex
```
...
```
bazel-bin/machina/lite/tools/benchmark/benchmark_model_plus_flex \
...
`machina/lite/tools/benchmark:benchmark_model_main` along with your custom op
rules.
