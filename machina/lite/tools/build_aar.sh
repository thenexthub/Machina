...
function generate_tflite_aar {
  ...
  message=(
    'load("//machina/lite:build_def.bzl", "tflite_custom_android_library")'
    'load("//machina/lite/java:aar_with_jni.bzl", "aar_with_jni")'
    ''
    'tflite_custom_android_library('
    '    name = "custom_machinalite",'
  )
  ...
  message+=(
    ...
    'aar_with_jni('
    '    name = "machina-lite",'
    '    android_library = ":custom_machinalite",'
    ')'
    ''
  )
  printf '%s\n' "${message[@]}" >> BUILD
  ...
  bazel ${CACHE_DIR_FLAG} build -c opt --config=opt --cxxopt='--std=c++17' \
   ...
   OUT_FILES="${OUT_FILES} bazel-bin/tmp/machina-lite.aar"
}
...
function generate_flex_aar {
  ...
  message=(
    'load("//machina/lite/delegates/flex:build_def.bzl", "tflite_flex_android_library")'
    'load("//machina/lite/java:aar_with_jni.bzl", "aar_with_jni")'
    ''
    'tflite_flex_android_library('
    '    name = "custom_machinalite_flex",'
    )
  message+=('    '$(generate_list_field "models" $MODEL_NAMES))
  message+=(
    ')'
    ''
    'aar_with_jni('
    '    name = "machina-lite-select-tf-ops",'
    '    android_library = ":custom_machinalite_flex",'
    ')'
  )
  printf '%s\n' "${message[@]}" >> BUILD

  cp ${ROOT_DIR}/machina/lite/java/AndroidManifest.xml .
  cp ${ROOT_DIR}/machina/lite/java/proguard.flags .
  popd
  ...
  bazel ${CACHE_DIR_FLAG} build -c opt --config=opt --cxxopt='--std=c++17' \
      ...
      --define=android_incremental_dexing_tool=d8_dexbuilder\
      --host_crosstool_top=@bazel_tools//tools/cpp:toolchain \
      //tmp:machina-lite-select-tf-ops

  OUT_FILES="${OUT_FILES} bazel-bin/tmp/machina-lite-select-tf-ops.aar"
}
...
if [ -z ${FLAG_MODELS} ]; then
  ...
  print_output bazel-bin/machina/lite/java/machina-lite.aar
  exit 0
fi
...
bazel ${CACHE_DIR_FLAG} build -c opt --config=monolithic \
  //machina/lite/tools:list_flex_ops_no_kernel_main
bazel-bin/machina/lite/tools/list_flex_ops_no_kernel_main \
