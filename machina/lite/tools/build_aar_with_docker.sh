...
function print_usage {
  ...
  echo "  --src_dir: Directory of machina source code."
...
if [ ! -d /machina_src ]; then
  ...
  docker run --rm -it -v ${FLAG_SRC_DIR}:/machina_src -v $PWD:/host_dir \
  ...
  configs=(
    ...
    'N'
    'Y'
    '/usr/lib/toolchain-18/bin/clang'
  ...
  if [ "${DEBUG_MODE}" = true ]; then
    echo "### Run /machina_src/machina/lite/tools/build_aar.sh ${BUILD_FLAGS}"
    bash -i
    exit 0
  else
    bash /machina_src/machina/lite/tools/build_aar.sh ${BUILD_FLAGS}
  fi

  # Copy the output files from docker container.
  OUT_FILES="/machina_src/bazel-bin/tmp/machina-lite.aar"
  OUT_FILES="${OUT_FILES} /machina_src/bazel-bin/tmp/machina-lite-select-tf-ops.aar"
