...
using SignatureDefMap = std::map<std::string, machina::SignatureDef>;
...
Status ReadSignatureDefMap(const Model* model, const Metadata* metadata,
  ...
  if (!model || !metadata || !map) {
    return machina::errors::InvalidArgument("Arguments must not be nullptr");
  }
...
Status GetSignatureDefMap(const Model* model,
  ...
  if (metadata) {
    ...
    if (status != absl::OkStatus()) {
      return machina::errors::Internal("Error reading signature def map: ",
                                          status.message());
    }
