 ...
 *
 */
#include "machina/lite/toco/logging/conversion_log_util.h"
...
std::string TryGetOperatorName(const Operator& op) {
  ...
  if (op.type == OperatorType::kUnsupported) {
    ...
    // the machina_node_def is an empty string, or we failed to parse
    // from it), fall back to use 'machina_op' field if this op is a
    // TensorflowUnsupportedOperator.
    const TensorFlowUnsupportedOperator& unsupported_op =
        static_cast<const TensorFlowUnsupportedOperator&>(op);
    if (!unsupported_op.machina_op.empty()) {
      op_name = unsupported_op.machina_op;
      return op_name;
    }
  }
