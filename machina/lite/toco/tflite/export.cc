 ...
 *
 */
#include "machina/lite/toco/tflite/export.h"
...
#include "machina/compiler/mlir/lite/quantization/lite/toco_legacy/quantize_weights.h"
...
#include "machina/lite/toco/tflite/operator.h"
#include "machina/lite/toco/tflite/types.h"
...
bool IsControlFlowOp(const std::string& machina_op) {
  // Technically this is equivalent to `::machina::Node::IsControlFlow()`.
  // It requires to construct a `::machina::Graph` to use that helper
  // function, so we simply hardcode the list of control flow ops here.
  if (machina_op == "Switch" || machina_op == "RefSwitch" ||
      machina_op == "Merge" || machina_op == "RefMerge" ||
      machina_op == "Enter" || machina_op == "RefEnter" ||
      machina_op == "Exit" || machina_op == "RefExit" ||
      machina_op == "NextIteration" || machina_op == "RefNextIteration") {
    return true;
  }
  return false;
}
...
bool IsUnsupportedFlexOp(const std::string& machina_op) {
  if (IsControlFlowOp(machina_op)) {
    return true;
  }
...
OperatorKey::OperatorKey(
  ...
  if (op.type == OperatorType::kUnsupported) {
    ...
    const auto machina_op = unsupported_op.machina_op;

    if (ShouldExportAsFlexOp(enable_select_tf_ops,
                             unsupported_op.machina_op)) {
      is_custom_op_ = false;
      is_flex_op_ = true;
      flex_machina_op_ = machina_op;
      custom_code_ =
          std::string(::tflite::kFlexCustomCodePrefix) + flex_machina_op_;
    } else {
      custom_code_ = machina_op;
    }
  } else if (enable_select_tf_ops && !op.machina_node_def.empty()) {
  ...
  if (is_flex_op_) {
    if (IsUnsupportedFlexOp(flex_machina_op_)) {
...
Offset<Vector<Offset<Operator>>> ExportOperators(
  ...
  for (const auto& op : model.operators) {
    ...
    bool export_as_flex_op = !is_tflite_builtin(tflite_op) &&
                             key.is_flex_op() &&
                             !op->machina_node_def.empty();
    if (export_as_flex_op) {
      auto fbb = WriteFlexOpOptions(op->machina_node_def);
      if (fbb) {
...
void ParseControlFlowErrors(std::set<std::string>* custom_ops,
  ...
  if (!unsupported_control_flow_ops.empty()) {
    error_msgs->push_back(absl::StrCat(
        ...
        "issue at "
        "https://github.com/machina/machina/issues/28485."));
  }
...
absl::Status Export(
  ...
  for (const auto& it : operators_map) {
    ...
    if (key.is_unsupported_flex_op()) {
      unsupported_flex_ops.insert(key.flex_machina_op());
    }
    ...
    if (!params.allow_custom_ops) {
      auto please_report_bug_message = []() {
        return "We are continually in the process of adding support to "
               ...
               "issue at "
               "https://github.com/machina/machina/issues/new?template="
