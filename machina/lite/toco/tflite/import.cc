...
#include "machina/lite/core/tools/verifier.h"
...
void ImportOperators(
  ...
  for (const auto* input_op : *ops) {
    ...
    if (ops_by_name.count(opname) == 0) {
      std::string effective_opname = "MACHINA_UNSUPPORTED";
      if (ops_by_name.count(effective_opname) == 0) {
        LOG(FATAL) << "Internal logic error: MACHINA_UNSUPPORTED not found.";
      }
      ...
      if (new_op->type == OperatorType::kUnsupported) {
        ...
        unsupported_op->machina_op = opname;
