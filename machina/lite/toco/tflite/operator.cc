...
#include "machina/lite/toco/graph_transformations/lstm_utils.h"
#include "machina/lite/toco/model.h"
#include "machina/lite/toco/runtime/types.h"
#include "machina/lite/toco/tflite/builtin_operator.h"
#include "machina/lite/toco/tflite/custom_operator.h"
#include "machina/lite/toco/tflite/simple_operator.h"
...
std::unique_ptr<flexbuffers::Builder> WriteFlexOpOptions(
    const std::string& machina_node_def) {
  auto fbb = std::make_unique<flexbuffers::Builder>();

  ::machina::NodeDef node_def;
  if (!node_def.ParseFromString(machina_node_def)) {
 ...
 public:
  ...
  std::unique_ptr<flexbuffers::Builder> WriteOptions(
    ...
    if (enable_select_tf_ops_) {
      return WriteFlexOpOptions(op.machina_node_def);
    }
    auto fbb = std::make_unique<flexbuffers::Builder>();

    ::machina::NodeDef node_def;
    if (!node_def.ParseFromString(op.machina_node_def)) {
      LOG(ERROR) << "Failed to parse TensorFlow NodeDef";
    ...
    if (ShouldExportAsFlexOp(enable_select_tf_ops_, node_def.op())) {
      fbb->Vector([&]() {
        fbb->String(node_def.op());
        fbb->String(op.machina_node_def);
      });
      fbb->Finish();
      LOG(INFO) << "Writing flex op: " << node_def.op();
      return std::unique_ptr<flexbuffers::Builder>(fbb.release());
  ...
  void ReadOptions(const flexbuffers::Map& m,
    ...
    node_def.SerializeToString(&op->machina_node_def);
  }
...
std::vector<std::unique_ptr<BaseOperator>> BuildOperatorList(
  ...
  ops.push_back(std::make_unique<TensorFlowUnsupported>(
      "MACHINA_UNSUPPORTED", OperatorType::kUnsupported,
...
// LINT.ThenChange(//machina/compiler/mlir/lite/tools/versioning/op_version.cc)
...
bool ShouldExportAsFlexOp(bool enable_select_tf_ops,
                          const std::string& machina_op_name) {
  ...
  if (!machina::OpRegistry::Global()
           ->LookUpOpDef(machina_op_name, &op_def)
           .ok()) {
    return false;
  }

  if (!::tflite::flex::IsAllowlistedFlexOp(machina_op_name)) {
    LOG(WARNING) << "Op " << machina_op_name
