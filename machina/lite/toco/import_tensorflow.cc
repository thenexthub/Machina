 ...
 *
 */
#include "machina/lite/toco/import_machina.h"
...
#include "machina/lite/toco/machina_graph_matching/resolve_cluster.h"
#include "machina/lite/toco/machina_util.h"
#include "machina/lite/toco/tooling_util.h"

using machina::AttrValue;
...
using machina::DT_QUINT8;
using machina::DT_STRING;
using machina::DT_UINT16;
...
machina::DataType GetDataTypeAttr(const NodeDef& node,
                                     const std::string& attr_name) {
  CHECK(HasAttr(node, attr_name));
  const auto& attr = node.attr().at(attr_name);
...
absl::Status CheckOptionalAttr(const NodeDef& node,
                               const std::string& attr_name,
                               const machina::DataType& expected_value) {
  if (HasAttr(node, attr_name)) {
    const machina::DataType& value = GetDataTypeAttr(node, attr_name);
    if (value != expected_value) {
      return machina::errors::InvalidArgument(
          "Unexpected value for attribute '" + attr_name + "'. Expected '" +
          machina::DataType_Name(expected_value) + "'");
    }
  }
  return absl::OkStatus();
}
...
ArrayDataType ConvertDataType(machina::DataType dtype) {
  if (dtype == DT_UINT8)
...
absl::Status ImportShape(const TFLITE_PROTO_NS::RepeatedPtrField<
                             machina::TensorShapeProto_Dim>& input_dims,
  ...
  for (auto& d : input_dims) {
    ...
    if (d.size() > std::numeric_limits<int>::max()) {
      return machina::errors::InvalidArgument("Shape element overflows");
    }
    if (d.size() == 0) {
  ...
  // elements valued -1 or 0, which are perfectly legal in machina. However
...
// TODO(b/80208043): simply use machina::Tensor::FromProto() instead.
template <typename T>
struct TensorTraits;
...
absl::Status ImportTensorData(const TensorProto& input_tensor,
  ...
  if (num_elements_in_tensor == input_flat_size) {
    ...
    // TODO(b/80208043): use machina::Tensor::FromProto() which is the
...
absl::Status CheckInputsCount(const NodeDef& node,
  ...
  if (GetInputsCount(node, tf_import_flags) != expected_input_count) {
    return machina::errors::FailedPrecondition(
...
void RetainTensorFlowNodeDef(const NodeDef& node, Operator* op) {
  node.SerializeToString(&op->machina_node_def);
}
...
void GetOutputTypesFromNodeDef(const NodeDef& node,
  ...
  auto add_type = [&node, op](machina::DataType type) {
    if (type == machina::DT_INVALID) {
  ...
  auto get_type = [&node](const machina::OpDef::ArgDef& a) {
    if (a.type() != machina::DT_INVALID) {
      return a.type();
...
absl::Status ConvertUnsupportedOperator(
  ...
  if (machina::OpRegistry::Global()->LookUpOpDef(node.op(), &op_def).ok()) {
...
absl::Status ConvertConvOperator(const NodeDef& node,
  ...
  if (!HasAttr(node, "strides")) {
    return machina::errors::InvalidArgument("Missing attribute 'strides'");
  }
...
absl::Status ConvertDepthToSpaceOperator(
  ...
  machina::DataType dtype = GetDataTypeAttr(node, "T");
  if (dtype != DT_FLOAT && dtype != DT_UINT8 && dtype != DT_INT32 &&
      dtype != DT_INT64) {
    const auto* enum_descriptor = machina::DataType_descriptor();
...
absl::Status ConvertBatchMatMulOperator(
  ...
  // https://www.machina.org/versions/r0.12/api_docs/python/math_ops/matrix_math_functions
...
absl::Status ConvertBatchNormWithGlobalNormalizationOperator(
  ...
  // TODO(ahentz): to really match machina we need to add variance_epsilon
...
bool InlineAllFunctions(GraphDef* graphdef) {
  ...
  for (auto& function :
    ...
    if (attributes->count(machina::kNoInlineAttr) != 0) {
      (*attributes)[machina::kNoInlineAttr].set_b(false);
    }
  }
  ...
  TF_CHECK_OK(machina::DeviceFactory::AddDevices(
      options, "/job:localhost/replica:0/task:0", &devices));

  machina::FunctionLibraryDefinition fld(machina::OpRegistry::Global(),
                                            graphdef_copy.library());
  machina::StaticDeviceMgr device_mgr(std::move(devices));
  machina::ProcessFunctionLibraryRuntime pflr(
      &device_mgr, machina::Env::Default(), &options.config,
      TF_GRAPH_DEF_VERSION, &fld,
      options.config.graph_options().optimizer_options(), nullptr);
  machina::FunctionLibraryRuntime* flr;
  flr = pflr.GetFLR("/job:localhost/replica:0/task:0/cpu:0");

  machina::Graph graph(fld);
  machina::ImportGraphDefOptions gc_opts;
  gc_opts.validate_shape = false;
  const auto& tf_convert_status = machina::ImportGraphDef(
      gc_opts, graphdef_copy, &graph, nullptr, nullptr);
  if (!tf_convert_status.ok()) {
    LOG(ERROR) << "machina::ImportGraphDef failed with status: "
  ...
  while (machina::ExpandInlineFunctions(flr, &graph)) {
    graph_modified = true;
  }
...
absl::Status ConvertUnidirectionalSequenceRnn(
  ...
  const auto& indices = GetListAttr(node, "_tflite_input_indices");
  if (indices.i_size() != node.input().size()) {
    return machina::errors::InvalidArgument("Input size does not match.");
  }
...
absl::Status ImportTensorFlowNode(const machina::NodeDef& node,
                                  const TensorFlowImportFlags& tf_import_flags,
                                  const ModelFlags& model_flags, Model* model,
