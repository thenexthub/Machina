...
machina::Status Writable(const string& filename) {
  FILE* f = fopen(filename.c_str(), "w");
  if (f) {
    fclose(f);
    return machina::OkStatus();
  }
  return machina::errors::NotFound("not writable");
}

machina::Status Readable(const string& filename,
                            const file::Options& options) {
  ...
  return machina::errors::NotFound("not readable");
}

machina::Status Exists(const string& filename,
                          const file::Options& options) {
  struct stat statbuf;
  int ret = stat(filename.c_str(), &statbuf);
  if (ret == -1) {
    return machina::errors::NotFound("file doesn't exist");
  }
  return machina::OkStatus();
}

machina::Status GetContents(const string& path, string* output,
                               const file::Options& options) {
  output->clear();

  int fd = open(path.c_str(), kFileReadFlags);
  if (fd == -1) {
    return machina::errors::NotFound("can't open() for read");
  }
  ...
  while (true) {
    ...
    if (size == 0) {
      ...
      // Error.
      close(fd);
      return machina::errors::Internal("error during read()");
    } else {
  ...
  CHECK(0);
  return machina::errors::Internal("internal error");
}

machina::Status SetContents(const string& filename, const string& contents,
                               const file::Options& options) {
  int fd = open(filename.c_str(), kFileWriteFlags, kFileCreateMode);
  if (fd == -1) {
    return machina::errors::Internal("can't open() for write");
  }
  ...
  while (i < contents.size()) {
    ...
    if (written == -1) {
      close(fd);
      return machina::errors::Internal("write() error");
    }
    i += written;
  }
  close(fd);
