...
#include "machina/lite/toco/machina_util.h"
#include "machina/lite/toco/tooling_util.h"

using machina::DT_BOOL;
using machina::DT_COMPLEX64;
using machina::DT_FLOAT;
using machina::DT_INT16;
using machina::DT_INT32;
using machina::DT_INT64;
using machina::DT_UINT32;
using machina::DT_UINT8;
using machina::GraphDef;
using machina::TensorProto;

namespace toco {
namespace {

machina::DataType GetTensorFlowDataType(ArrayDataType data_type,
...
machina::DataType GetTensorFlowDataTypeForOp(ArrayDataType data_type,
                                                const std::string& op_name) {
  return GetTensorFlowDataType(data_type, "op '" + op_name + "'");
}

machina::DataType GetTensorFlowDataType(const Model& model,
...
bool HasAlreadyExportedConst(const std::string& name,
                             const GraphDef& machina_graph) {
  for (const auto& node : machina_graph.node()) {
...
void ConvertFloatTensorConst(const std::string& name, const Shape& input_shape,
                             ...
                             GraphDef* machina_graph,
                             LegacyScalarPolicy legacy_scalar_policy) {
  if (HasAlreadyExportedConst(name, *machina_graph)) {
    return;
  }
  machina::NodeDef* const_op = machina_graph->add_node();
...
void ConvertFloatTensorConst(const std::string& name, const Shape& input_shape,
  ...
  ConvertFloatTensorConst(name, input_shape, input_data, input_axes_order,
                          output_axes_order, machina_graph,
...
void ConvertConvOperator(const Model& model, const ConvOperator& src_op,
  ...
  machina::NodeDef* conv2d_op = machina_graph->add_node();
  ...
  *conv2d_op->add_input() = src_op.inputs[1];
  ...
  if (has_bias) {
    machina::NodeDef* biasadd_op = machina_graph->add_node();
...
void ConvertDepthwiseConvOperator(const Model& model,
  ...
  machina::NodeDef* dc2d_op = machina_graph->add_node();
  ...
  *dc2d_op->add_input() = src_op.inputs[1];
  ...
  ConvertFloatTensorConst(src_weights_name, dst_weights_shape, src_weights_data,
                          AxesOrder::kHWIM, AxesOrder::kHWIM, machina_graph);
...
void ConvertDepthToSpaceOperator(const Model& model,
                                 ...
                                 GraphDef* machina_graph) {
  machina::NodeDef* op = machina_graph->add_node();
...
void ConvertFullyConnectedOperator(const Model& model,
  ...
  CreateMatrixShapeTensorConst(reshape_shape, fc_weights_shape.dims(1), -1,
                               machina_graph);
  machina::NodeDef* reshape_op = machina_graph->add_node();
  ...
  CreateIntTensorConst(transpose_perm, {1, 0}, {2}, machina_graph);
  machina::NodeDef* transpose_op = machina_graph->add_node();
  ...
  *transpose_op->add_input() = transpose_perm;
  ...
  machina::NodeDef* matmul_op = machina_graph->add_node();
  ...
  *matmul_op->add_input() = transpose_op->name();
  ...
  if (has_bias) {
    ...
    ConvertFloatTensorConst(WalkUpToConstantArray(model, src_op.inputs[2]),
                            ...
                            AxesOrder::kOneAxis, machina_graph,
                            LegacyScalarPolicy::kDoCreateLegacyScalars);
  }
}
...
void ConvertAddOperator(const Model& model, const AddOperator& src_op,
                        GraphDef* machina_graph) {
  machina::NodeDef* add_op = machina_graph->add_node();
  add_op->set_op("Add");
...
void ConvertMulOperator(const Model& model, const MulOperator& src_op,
                        GraphDef* machina_graph) {
  machina::NodeDef* mul_op = machina_graph->add_node();
  mul_op->set_op("Mul");
...
void ConvertDivOperator(const Model& model, const DivOperator& src_op,
                        GraphDef* machina_graph) {
  machina::NodeDef* div_op = machina_graph->add_node();
  div_op->set_op("Div");
...
void ConvertReluOperator(const Model& model, const ReluOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* relu_op = machina_graph->add_node();
  relu_op->set_op("Relu");
...
void ConvertRelu1Operator(const Relu1Operator& src_op,
  ...
  machina::NodeDef* max_bounds_const_op = machina_graph->add_node();
  ...
  machina::NodeDef* min_bounds_const_op = machina_graph->add_node();
  ...
  machina::NodeDef* max_op = machina_graph->add_node();
  ...
  *max_op->add_input() = max_bounds;
  ...
  machina::NodeDef* min_op = machina_graph->add_node();
...
void ConvertLogOperator(const LogOperator& src_op, GraphDef* machina_graph) {
  machina::NodeDef* op = machina_graph->add_node();
  op->set_op("Log");
  op->set_name(src_op.outputs[0]);
...
void ConvertTanhOperator(const TanhOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* tanh_op = machina_graph->add_node();
  tanh_op->set_op("Tanh");
...
void ConvertSoftmaxOperator(const Model& model, const SoftmaxOperator& src_op,
  ...
  if (providing_op != nullptr && providing_op->type == OperatorType::kReshape) {
    ...
    *reshape_op->add_input() = softmax_size;
    ...
    CreateReshapeShapeTensorConst(softmax_size, shape_data, machina_graph);
  }

  machina::NodeDef* softmax_op = machina_graph->add_node();
...
void ConvertLogSoftmaxOperator(const Model& model,
  ...
  machina::NodeDef* log_softmax_op = machina_graph->add_node();
...
void ConvertL2NormalizationOperator(const L2NormalizationOperator& src_op,
  ...
  machina::NodeDef* sum_reduction_indices_op = machina_graph->add_node();
  ...
  machina::NodeDef* square_op = machina_graph->add_node();
  ...
  *square_op->add_input() = src_op.inputs[0];
  ...
  machina::NodeDef* sum_op = machina_graph->add_node();
  sum_op->set_op("Sum");
  ...
  *sum_op->add_input() = sum_reduction_indices;
  ...
  machina::NodeDef* rsqrt_op = machina_graph->add_node();
...
void ConvertLocalResponseNormalizationOperator(
    ...
    GraphDef* machina_graph) {
  machina::NodeDef* lrn_op = machina_graph->add_node();
  lrn_op->set_op("LRN");
...
void ConvertFakeQuantOperator(const FakeQuantOperator& src_op,
                              GraphDef* machina_graph) {
  machina::NodeDef* fakequant_op = machina_graph->add_node();
...
void ConvertMaxPoolOperator(const MaxPoolOperator& src_op,
                            GraphDef* machina_graph) {
  machina::NodeDef* maxpool_op = machina_graph->add_node();
...
void ConvertAveragePoolOperator(const AveragePoolOperator& src_op,
                                GraphDef* machina_graph) {
  machina::NodeDef* avgpool_op = machina_graph->add_node();
...
void ConvertConcatenationOperator(const Model& model,
                                  ...
                                  GraphDef* machina_graph) {
  machina::NodeDef* dc_op = machina_graph->add_node();
  dc_op->set_op("ConcatV2");
  dc_op->set_name(src_op.outputs[0]);
  const std::string dummy_axis = src_op.outputs[0] + "/axis";
  CreateDummyConcatDimTensorConst(dummy_axis, src_op.axis, machina_graph);
  for (const auto& input : src_op.inputs) {
...
void ConvertTensorFlowReshapeOperator(const Model& model,
  ...
  *reshape_op->add_input() = src_op.inputs[1];
  ...
  CreateReshapeShapeTensorConst(src_op.inputs[1], shape_data, machina_graph);
}
...
void ConvertL2PoolOperator(const L2PoolOperator& src_op,
  ...
  *avgpool_op->add_input() = square_output;
  ...
  machina::NodeDef* sqrt_op = machina_graph->add_node();
  sqrt_op->set_op("Sqrt");
...
void ConvertRsqrtOperator(const Model& model,
  ...
  *rsqrt_op->add_input() = src_op.inputs[0];
  const machina::DataType data_type =
      GetTensorFlowDataType(model, src_op.inputs[0]);
...
void ConvertSplitOperator(const Model& model,
                          ...
                          GraphDef* machina_graph) {
  machina::NodeDef* split_op = machina_graph->add_node();
...
void ConvertSplitVOperator(const Model& model,
                           ...
                           GraphDef* machina_graph) {
  machina::NodeDef* split_v_op = machina_graph->add_node();
  ...
  ConvertIntTensorConst(model, src_op.inputs[1], machina_graph);
}

void ConvertCastOperator(const Model& model, const CastOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* cast_op = machina_graph->add_node();
  cast_op->set_op("Cast");
...
void ConvertFloorOperator(const Model& model, const FloorOperator& src_op,
                          GraphDef* machina_graph) {
  machina::NodeDef* floor_op = machina_graph->add_node();
...
void ConvertCeilOperator(const Model& model, const CeilOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* ceil_op = machina_graph->add_node();
  ceil_op->set_op("Ceil");
...
void ConvertRoundOperator(const Model& model, const RoundOperator& src_op,
                          GraphDef* machina_graph) {
  machina::NodeDef* round_op = machina_graph->add_node();
...
void ConvertGatherOperator(const Model& model, const GatherOperator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* gather_op = machina_graph->add_node();
  ...
  *gather_op->add_input() = src_op.inputs[1];
  ...
  const machina::DataType params_type =
      GetTensorFlowDataType(model, src_op.inputs[0]);
...
void ConvertArgMaxOperator(const Model& model, const ArgMaxOperator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* argmax_op = machina_graph->add_node();
...
void ConvertArgMinOperator(const Model& model, const ArgMinOperator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* argmin_op = machina_graph->add_node();
...
void ConvertTensorFlowShapeOperator(const Model& model,
                                    ...
                                    GraphDef* machina_graph) {
  machina::NodeDef* shape_op = machina_graph->add_node();
...
void ConvertRankOperator(const Model& model,
                         ...
                         GraphDef* machina_graph) {
  machina::NodeDef* rank_op = machina_graph->add_node();
  rank_op->set_op("Rank");
...
void ConvertRangeOperator(const Model& model, const RangeOperator& src_op,
                          GraphDef* machina_graph) {
  machina::NodeDef* range_op = machina_graph->add_node();
...
void ConvertPackOperator(const Model& model, const PackOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* pack_op = machina_graph->add_node();
  pack_op->set_op("Pack");
...
void ConvertFillOperator(const Model& model, const FillOperator& src_op,
                         GraphDef* machina_graph) {
  machina::NodeDef* fill_op = machina_graph->add_node();
  fill_op->set_op("Fill");
...
void ConvertFloorDivOperator(const Model& model, const FloorDivOperator& src_op,
                             GraphDef* machina_graph) {
  machina::NodeDef* floor_div_op = machina_graph->add_node();
...
void ConvertFloorModOperator(const Model& model, const FloorModOperator& src_op,
                             GraphDef* machina_graph) {
  machina::NodeDef* floor_mod_op = machina_graph->add_node();
...
void ConvertExpandDimsOperator(const Model& model,
                               ...
                               GraphDef* machina_graph) {
  machina::NodeDef* expand_dims_op = machina_graph->add_node();
...
void ConvertResizeBilinearOperator(const Model& model,
                                   ...
                                   GraphDef* machina_graph) {
  machina::NodeDef* resize_op = machina_graph->add_node();
...
void ConvertOneHotOperator(const Model& model, const OneHotOperator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* onehot_op = machina_graph->add_node();
...
void ConvertLstmCellOperator(const Model& model, const LstmCellOperator& src_op,
  ...
  CreateDummyConcatDimTensorConst(axis_output, axis, machina_graph);
  machina::NodeDef* concat_op = machina_graph->add_node();
  ...
  *concat_op->add_input() = axis_output;
  ...
  ConvertFloatTensorConst(weights_output, weights_shape, weights_data,
                          AxesOrder::kCR, AxesOrder::kRC, machina_graph);
  ...
  *matmul_op->add_input() = weights_output;
  ...
  CreateDummyConcatDimTensorConst(split_dim_output, axis, machina_graph);
  ...
  *split_op->add_input() = biasadd_output;
  ...
  machina::NodeDef* tanh_0_op = machina_graph->add_node();
  ...
  *tanh_0_op->add_input() = split_output + ":1";
  ...
  machina::NodeDef* logistic_1_op = machina_graph->add_node();
  ...
  *logistic_1_op->add_input() = split_output;
  ...
  machina::NodeDef* mul_1_op = machina_graph->add_node();
  mul_1_op->set_op("Mul");
  ...
  *mul_1_op->add_input() = tanh_0_output;
  ...
  machina::NodeDef* logistic_2_op = machina_graph->add_node();
  ...
  *logistic_2_op->add_input() = split_output + ":2";
  ...
  machina::NodeDef* logistic_3_op = machina_graph->add_node();
  ...
  *logistic_3_op->add_input() = split_output + ":3";
  ...
  machina::NodeDef* mul_0_op = machina_graph->add_node();
  mul_0_op->set_op("Mul");
  ...
  *mul_0_op->add_input() = sigmoid_0_output;
  ...
  machina::NodeDef* add_1_op = machina_graph->add_node();
  add_1_op->set_op("Add");
  ...
  *add_1_op->add_input() = mul_1_output;
  ...
  machina::NodeDef* tanh_1_op = machina_graph->add_node();
  ...
  *tanh_1_op->add_input() = add_1_output;
  ...
  const std::string mul_2_output =
      src_op.outputs[LstmCellOperator::ACTIV_OUTPUT];
  machina::NodeDef* mul_2_op = machina_graph->add_node();
  mul_2_op->set_op("Mul");
...
void ConvertSpaceToBatchNDOperator(const Model& model,
                                   ...
                                   GraphDef* machina_graph) {
  machina::NodeDef* new_op = machina_graph->add_node();
...
void ConvertPadOperator(const Model& model, const PadOperator& src_op,
  ...
  *new_op->add_input() = src_op.inputs[1];
  ...
  machina::NodeDef* params_op = machina_graph->add_node();
...
void ConvertStridedSliceOperator(const Model& model,
  ...
  *new_op->add_input() = src_op.inputs[3];
  ...
  CreateSliceInput(src_op.inputs[1], src_op.start_indices, machina_graph);
  CreateSliceInput(src_op.inputs[2], src_op.stop_indices, machina_graph);
  CreateSliceInput(src_op.inputs[3], src_op.strides, machina_graph);
}
...
void ConvertSliceOperator(const Model& model, const SliceOperator& src_op,
  ...
  *new_op->add_input() = src_op.inputs[2];
  ...
  CreateSliceInput(src_op.inputs[1], src_op.begin, machina_graph);
  CreateSliceInput(src_op.inputs[2], src_op.size, machina_graph);
}
...
void ConvertReduceOperator(const Model& model, const T& src_op,
  ...
  *new_op->add_input() = src_op.inputs[1];
  ...
  const machina::DataType indices_type =
      GetTensorFlowDataType(model, src_op.inputs[1]);
...
void ConvertSubOperator(const Model& model, const SubOperator& src_op,
                        GraphDef* machina_graph) {
  machina::NodeDef* sub_op = machina_graph->add_node();
  sub_op->set_op("Sub");
...
void ConvertSelectOperator(const Model& model, const SelectOperator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* select_op = machina_graph->add_node();
...
void ConvertTileOperator(const Model& model,
                         ...
                         GraphDef* machina_graph) {
  machina::NodeDef* tile_op = machina_graph->add_node();
  tile_op->set_op("Tile");
  ...
  *tile_op->add_input() = src_op.inputs[1];
  ...
  const machina::DataType multiples_data_type =
      GetTensorFlowDataType(model, src_op.inputs[1]);
...
void ConvertTopKV2Operator(const Model& model, const TopKV2Operator& src_op,
                           GraphDef* machina_graph) {
  machina::NodeDef* topk_op = machina_graph->add_node();
...
void ConvertRandomUniformOperator(const Model& model,
                                  ...
                                  GraphDef* machina_graph) {
  CHECK(machina_graph != nullptr);
  machina::NodeDef* new_op = machina_graph->add_node();
  new_op->set_op("RandomUniform");
  CHECK_EQ(src_op.inputs.size(), 1);
  new_op->set_name(src_op.outputs[0]);
  *new_op->add_input() = src_op.inputs[0];
  const machina::DataType shape_type =
      GetTensorFlowDataType(model, src_op.inputs[0]);
...
void ConvertComparisonOperator(const Model& model, const Operator& src_op,
                               const char* op_name,
                               GraphDef* machina_graph) {
  machina::NodeDef* comparison_op = machina_graph->add_node();
...
void ConvertSparseToDenseOperator(const Model& model,
                                  ...
                                  const char* op_name,
                                  GraphDef* machina_graph) {
  machina::NodeDef* sparse_to_dense_op = machina_graph->add_node();
  ...
  const machina::DataType index_type =
      GetTensorFlowDataType(model, src_op.inputs[0]);
...
void ConvertPowOperator(const Model& model, const PowOperator& src_op,
                        const char* op_name, GraphDef* machina_graph) {
  machina::NodeDef* pow_op = machina_graph->add_node();
...
void ConvertLogicalAndOperator(const Model& model,
                               ...
                               GraphDef* machina_graph) {
  machina::NodeDef* logical_op = machina_graph->add_node();
...
void ConvertLogicalOrOperator(const Model& model,
                              ...
                              const char* op_name, GraphDef* machina_graph) {
  machina::NodeDef* logical_or_op = machina_graph->add_node();
...
void ConvertCTCBeamSearchDecoderOperator(
    ...
    const char* op_name, GraphDef* machina_graph) {
  auto* op = machina_graph->add_node();
  op->set_op(op_name);
  op->set_name(src_op.outputs[0]);
  CHECK_EQ(src_op.inputs.size(), 2);
  for (int i = 0; i < 2; ++i) {
...
void ConvertUnpackOperator(const Model& model, const UnpackOperator& src_op,
                           const char* op_name, GraphDef* machina_graph) {
  machina::NodeDef* unpack_op = machina_graph->add_node();
...
void ConvertZerosLikeOperator(const Model& model,
                              ...
                              const char* op_name, GraphDef* machina_graph) {
  machina::NodeDef* zeros_like_op = machina_graph->add_node();
...
void ConvertReverseV2Operator(const Model& model,
                              ...
                              const char* op_name, GraphDef* machina_graph) {
  machina::NodeDef* reverse_v2_op = machina_graph->add_node();
...
void ConvertReverseSequenceOperator(const Model& model,
                                    ...
                                    GraphDef* machina_graph) {
  machina::NodeDef* reverse_seq_op = machina_graph->add_node();
...
void ConvertOperator(const Model& model, const Operator& src_op,
  ...
  if (src_op.type == OperatorType::kConv) {
    ...
    ConvertL2NormalizationOperator(
        static_cast<const L2NormalizationOperator&>(src_op), machina_graph);
    ...
    ConvertTransposeOperator(
        model, static_cast<const TransposeOperator&>(src_op), machina_graph);
    ...
    ConvertComparisonOperator(model, src_op, "Equal", machina_graph);
  } else if (src_op.type == OperatorType::kNotEqual) {
    ConvertComparisonOperator(model, src_op, "NotEqual", machina_graph);
  } else if (src_op.type == OperatorType::kGreater) {
    ConvertComparisonOperator(model, src_op, "Greater", machina_graph);
  } else if (src_op.type == OperatorType::kGreaterEqual) {
    ConvertComparisonOperator(model, src_op, "GreaterEqual", machina_graph);
  } else if (src_op.type == OperatorType::kLess) {
    ConvertComparisonOperator(model, src_op, "Less", machina_graph);
  } else if (src_op.type == OperatorType::kLessEqual) {
    ConvertComparisonOperator(model, src_op, "LessEqual", machina_graph);
...
void AddPlaceholder(const std::string& name, ArrayDataType type,
                    GraphDef* machina_graph) {
  machina::NodeDef* placeholder = machina_graph->add_node();
...
void AddPlaceholderForRNNState(const Model& model, const std::string& name,
                               int size, GraphDef* machina_graph) {
  machina::NodeDef* placeholder = machina_graph->add_node();
  placeholder->set_op("Placeholder");
  placeholder->set_name(name);
  (*placeholder->mutable_attr())["dtype"].set_type(DT_FLOAT);
...
void ExportTensorFlowGraphDefImplementation(const Model& model,
  ...
  for (const auto& op : model.operators) {
    ConvertOperator(model, *op, machina_graph);
  }
  ...
  for (const auto& array_pair : model.GetArrayMap()) {
    ...
    if (array.buffer) {
      switch (array.data_type) {
        case ArrayDataType::kBool:
          ConvertBoolTensorConst(model, array_name, machina_graph);
          break;
        case ArrayDataType::kFloat:
          ConvertFloatTensorConst(model, array_name, machina_graph);
          break;
        case ArrayDataType::kInt32:
          ConvertIntTensorConst(model, array_name, machina_graph);
          break;
        case ArrayDataType::kComplex64:
          ConvertComplex64TensorConst(model, array_name, machina_graph);
          break;
        default:
          break;
      }
...
void ExportTensorFlowGraphDef(const Model& model,
  ...
  GraphDef machina_graph;
  ExportTensorFlowGraphDefImplementation(model, &machina_graph);
  LogDumpGraphDef(kLogLevelModelChanged, "AT EXPORT", machina_graph);
  CHECK(machina_graph.SerializeToString(output_file_contents));
}
}  // namespace toco
