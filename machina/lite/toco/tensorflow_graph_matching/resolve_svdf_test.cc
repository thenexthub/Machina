 ...
 *
 */
#include "machina/lite/toco/machina_graph_matching/resolve_svdf.h"
...
class ResolveSvdfTest : public ::testing::Test {
 ...
 protected:
  ...
  void AddNewNode(const std::string& name, const std::string& op,
    ...
    (*node->mutable_attr())["dtype"].set_type(machina::DT_FLOAT);
    machina::TensorProto* allocated_tensor = new machina::TensorProto;
    machina::TensorShapeProto* allocated_tensor_shape =
        new machina::TensorShapeProto;
  ...
  void AddShapeNode(const std::string& name, const std::vector<int>& values) {
    ...
    node->set_device("");
    // Add the float vector as an attribute to the node.
    (*node->mutable_attr())["dtype"].set_type(machina::DT_INT32);
    machina::TensorProto* allocated_tensor = new machina::TensorProto;
    machina::TensorShapeProto* allocated_tensor_shape =
        new machina::TensorShapeProto;
    auto tensor_shape_dim0 = allocated_tensor_shape->add_dim();
...
TEST_F(ResolveSvdfTest, TestResolveSvdfFlow) {
  ...
  for (const std::unique_ptr<Cluster>& cluster : clusters_) {
    ...
    for (const std::unique_ptr<machina::NodeDef>& node :
         cluster->GetNewNodes()) {
      new_node_names.push_back(node->name());
      if (node->op() == "Const") {
        CHECK_EQ(node->attr().at("dtype").type(), machina::DT_FLOAT);
