  ...
  for (const NodeDef* node : cluster_nodes) {
    ...
    if (StrContains(node->name(), const_pattern) && node->op() == "Const") {
      if (StrContains(node_name_to_upper, "/PART_")) {
        ...
        const machina::TensorProto& tensor = value_attr.tensor();
...
void SvdfCluster::CreateNodes() {
  ...
  std::unique_ptr<machina::NodeDef> svdf_node(new NodeDef);
  ...
  for (const std::unique_ptr<machina::NodeDef>& node : new_nodes_) {
...
void SvdfCluster::CreateConstNode(const std::string& const_pattern) {
  ...
  std::unique_ptr<machina::NodeDef> merged_node(new NodeDef);
...
void SvdfCluster::MaybeMergeConstNodes(
  ...
  int dim0_size = 0;
  int dim1_size = 1;
  machina::TensorProto* allocated_tensor =
  ...
  for (size_t i = 0; i < const_node_parts.size(); i++) {
    ...
    const machina::TensorShapeProto shape = tensor.tensor_shape();
