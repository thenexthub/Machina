...
BEGIN DUMP OF MACHINA GRAPHDEF (%s)
There are %d nodes.
...
BEGIN NODE: name = %s
    ...
    for (const auto& attr : node.attr()) {
      ...
      END EMBEDDED STRING
        ...
        toco::port::AppendF(&dump, "    type: %s\n",
                            machina::DataType_Name(attr.second.type()));
        ...
        if (tensor.dtype() == machina::DT_INT32) {
        ...
        if (tensor.dtype() == machina::DT_FLOAT) {
  ...
  toco::port::AppendF(&dump, "END DUMP OF MACHINA GRAPHDEF (%s)\n", message);
