 ...
 *
 */
#include "machina/lite/toco/toco_tooling.h"
...
#include "machina/lite/toco/allocate_transient_arrays.h"
#include "machina/lite/toco/dump_graphviz.h"
#include "machina/lite/toco/export_machina.h"
...
#include "machina/lite/toco/tflite/import.h"
...
bool SupportsLstmCell(FileFormat format) {
  return (format == MACHINA_GRAPHDEF || format == GRAPHVIZ_DOT ||
          format == TFLITE);
}
    ...
    case MACHINA_GRAPHDEF: {
      ...
      tf_import_flags.drop_control_dependency =
          toco_flags.has_drop_control_dependency()
              ...
              : (toco_flags.output_format() != MACHINA_GRAPHDEF);
...
absl::Status TransformWithStatus(const TocoFlags& toco_flags, Model* model) {
  ...
  if (output_format == MACHINA_GRAPHDEF) {
