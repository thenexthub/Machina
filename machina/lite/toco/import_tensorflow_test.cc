...
NodeDef BuildNode(
  ...
  for (const auto& output_shape : output_shapes) {
    machina::TensorShapeProto* shape = shapes->add_shape();
...
void BuildConstNode(std::initializer_list<int64_t> shape,
                    machina::DataType dtype, int64_t num_elements,
                    NodeDef* node) {
  node->set_op("Const");
  node->set_name("Node1");
...
class ShapeImportTest : public ::testing::TestWithParam<machina::DataType> {
};
...
std::vector<machina::DataType> TestTypes() {
...
std::vector<std::pair<machina::DataType, ArrayDataType>> UnaryTestTypes() {
...
class TypeImportTest : public ::testing::TestWithParam<
                           std::pair<machina::DataType, ArrayDataType>> {
 protected:
  TypeImportTest() {}

  void BuildUnaryNode(const std::string& op_name, machina::DataType dtype,
                      NodeDef* node) {
    node->set_op(op_name);
    node->set_name("Node1");
...
TEST(ImportTest, UnsupportedOpWithMultipleOutputs) {
  ...
  {
    AttrValue value_attr;
    std::vector<machina::DataType> types;
    types.push_back(machina::DT_FLOAT);
    types.push_back(machina::DT_STRING);
  ...
  // the 'Tdense' attribute.
  {
    ...
    types.push_back(machina::DT_STRING);
    types.push_back(machina::DT_FLOAT);
    types.push_back(machina::DT_INT64);
    SetAttrValue(types, &value_attr);
