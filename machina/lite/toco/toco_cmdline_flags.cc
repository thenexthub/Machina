...
#include "machina/lite/toco/toco_cmdline_flags.h"
...
bool ParseTocoFlagsFromCommandLineFlags(
  ...
  std::vector<machina::Flag> flags = {
      ...
      Flag("input_format", parsed_flags.input_format.bind(),
           ...
           "Input file format. One of: MACHINA_GRAPHDEF, TFLITE."),
      Flag("output_format", parsed_flags.output_format.bind(),
           parsed_flags.output_format.default_value(),
           "Output file format. "
           "One of MACHINA_GRAPHDEF, TFLITE, GRAPHVIZ_DOT."),
  ...
  if (asked_for_help) {
    *msg += machina::Flags::Usage(argv[0], flags);
    return false;
  } else {
    return machina::Flags::Parse(argc, argv, flags);
  }
}
