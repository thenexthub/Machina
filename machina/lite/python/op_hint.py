...
Once you have built your whole machina graph, you can run it and train it
...
from machina.core.framework import attr_value_pb2 as _attr_value_pb2
from machina.core.framework import graph_pb2 as _graph_pb2
from machina.core.framework import node_def_pb2 as _node_def_pb2
from machina.python.framework import dtypes as _dtypes
from machina.python.framework import ops as _ops
from machina.python.framework import tensor_util as _tensor_util
from machina.python.framework.graph_util_impl import _bfs_for_reachable_nodes
from machina.python.framework.graph_util_impl import _extract_graph_summary
...
@_deprecation.deprecated(
    ...
    "https://www.machina.org/lite/convert/operation_fusion for operation"
    "fusion in tflite."
)
class OpHint:
...
class _LiteSingleOperand(_LiteOperand):
  ...
  def aggregate_and_return_name_for_output(self, fused_op_name, index,
    ...
    output_node.input.append(_machina_output_name(fused_op_name, index))
  ...
  def aggregate_and_return_name_for_output(self, fused_op_name, output_index,
    ...
    if len(flattened) == 1 and self.aggregation != OpHint.AGGREGATE_STACK:
      ...
      stack_node.input.append(
          _machina_output_name(fused_op_name, output_index))
      out_graphdef.node.extend([stack_node])

      for idx, discrete in enumerate(flattened):
        output_node = _copy.deepcopy(discrete)
        del output_node.input[:]
        output_node.input.append(_machina_output_name(stack_node.name, idx))
        out_graphdef.node.extend([output_node])
...
def _machina_output_name(tensor_name, output_index):
...
def _convert_single_op_hint_to_stub(call,
  ...
  Note: after this process, the graph_def can no longer be loaded into
      the machina runtime, so all future manipulations are done in graph_def
      level.
  """
