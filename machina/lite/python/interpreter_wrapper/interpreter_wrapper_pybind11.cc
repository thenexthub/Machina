...
#include "machina/lite/python/interpreter_wrapper/interpreter_wrapper.h"
#include "machina/python/lib/core/pybind11_lib.h"

namespace py = pybind11;
using tflite::interpreter_wrapper::InterpreterWrapper;

PYBIND11_MODULE(_pywrap_machina_interpreter_wrapper, m) {
  m.doc() = R"pbdoc(
    _pywrap_machina_interpreter_wrapper
    -----
  )pbdoc";
  ...
  py::class_<InterpreterWrapper>(m, "InterpreterWrapper", py::module_local())
      .def(
          ...
          [](InterpreterWrapper& self, int subgraph_index) {
            return machina::PyoOrThrow(self.AllocateTensors(subgraph_index));
          },
          // LINT.IfChange
          py::arg("subgraph_index") = -1)
      // LINT.ThenChange(//machina/lite/python/interpreter_wrapper/interpreter_wrapper.cc)
      .def(
          "Invoke",
          [](InterpreterWrapper& self, int subgraph_index) {
            return machina::PyoOrThrow(self.Invoke(subgraph_index));
          },
          py::arg("subgraph_index") = 0)
      .def("InputIndices",
           [](const InterpreterWrapper& self) {
             return machina::PyoOrThrow(self.InputIndices());
           })
      .def("OutputIndices",
           [](InterpreterWrapper& self) {
             return machina::PyoOrThrow(self.OutputIndices());
           })
      .def(
          "ResizeInputTensor",
      ...
      .def("TensorQuantizationParameters",
           [](InterpreterWrapper& self, int tensor_index, int subgraph_index) {
             return machina::PyoOrThrow(self.TensorQuantizationParameters(
      ...
      .def("GetSignatureDefs",
           [](InterpreterWrapper& self) {
             return machina::PyoOrThrow(self.GetSignatureDefs());
           })
      .def("ResetVariableTensors",
           [](InterpreterWrapper& self) {
             return machina::PyoOrThrow(self.ResetVariableTensors());
           })
      .def("NumNodes", &InterpreterWrapper::NumNodes)
      .def("NodeName", &InterpreterWrapper::NodeName)
      .def("NodeInputs",
           [](const InterpreterWrapper& self, int i) {
             return machina::PyoOrThrow(self.NodeInputs(i));
           })
      .def("NodeOutputs",
           [](const InterpreterWrapper& self, int i) {
             return machina::PyoOrThrow(self.NodeOutputs(i));
           })
      .def(
          "tensor",
      ...
      .def(
          ...
          [](InterpreterWrapper& self, uintptr_t delegate_ptr) {
            return machina::PyoOrThrow(self.ModifyGraphWithDelegate(
      ...
      .def(
          ...
          [](InterpreterWrapper& self, int num_threads) {
            return machina::PyoOrThrow(self.SetNumThreads(num_threads));
          },
          R"pbdoc(
