...
from machina_docs.api_generator import gen_java
...
#   /machina (github.com/machina/machina - home of this script)
#   /machina_lite_support (github.com/machina/tflite-support)
...
SOURCE_PATH_CORE = pathlib.Path('machina/lite/java/src/main/java')
SOURCE_PATH_GPU = pathlib.Path(
    'machina/lite/delegates/gpu/java/src/main/java')
SOURCE_PATH_SUPPORT = pathlib.Path('machina_lite_support/java/src/java')
SOURCE_PATH_METADATA = pathlib.Path(
    'machina_lite_support/metadata/java/src/java')
SOURCE_PATH_ODML = pathlib.Path('machina_lite_support/odml/java/image/src')
SOURCE_PATH_ANDROID_SDK = pathlib.Path('android/sdk/api/26.txt')

# This (key) ordering is preserved in the TOC output.
SECTION_LABELS = {
    'org.machina.lite': 'Core',
    'org.machina.lite.support': 'Support Library',
    'org.machina.lite.gpu': 'Delegates',
...
def main(unused_argv):
  ...
  with tempfile.TemporaryDirectory() as merge_tmp_dir:
    ...
    if gms_path := os.getenv('GMS_PATH'):
      ...
      overlay(gms_path, merged_temp_dir / 'org/machina/lite/task/gms')

    gen_java.gen_java_docs(
        package=['org.machina.lite', 'com.google.android.odml'],
