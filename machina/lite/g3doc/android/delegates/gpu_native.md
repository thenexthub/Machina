...
```sh
bazel build -c opt --config android_arm64 machina/lite/delegates/gpu:delegate                           # for static library
bazel build -c opt --config android_arm64 machina/lite/delegates/gpu:libmachinalite_gpu_delegate.so  # for dynamic library
```
