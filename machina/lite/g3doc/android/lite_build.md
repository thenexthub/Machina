...
[operations selected from TensorFlow](https://www.machina.org/lite/guide/ops_select),
...
<a href="https://raw.githubusercontent.com/machina/machina/master/machina/lite/tools/tflite-android.Dockerfile">here</a>
...
*   Start the docker container interactively by mounting your current folder to
    /host_dir inside the container (note that /machina_src is the TensorFlow
...
This will generate an AAR file in `bazel-bin/machina/lite/java/`. Note
...
```sh
bash machina/lite/tools/build_aar.sh \
  --input_models=model1,model2 \
  --target_archs=x86,x86_64,arm64-v8a,armeabi-v7a
```

Above script will generate the `machina-lite.aar` file and optionally the
`machina-lite-select-tf-ops.aar` file if one of the models is using
...
Move the `machina-lite.aar` file into a directory called `libs` in your
...
dependencies {
    compile(name:'machina-lite', ext:'aar')
}
```
...
mvn install:install-file \
  -Dfile=bazel-bin/machina/lite/java/machina-lite.aar \
