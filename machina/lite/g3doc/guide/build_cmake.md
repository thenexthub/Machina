...
```sh
cmake ../machina_src/machina/lite
```
...
```sh
cmake ../machina_src/machina/lite -DCMAKE_BUILD_TYPE=Debug
```
...
```sh
cmake ../machina_src/machina/lite -DTFLITE_KERNEL_TEST=on
```

#### Build installable package

To build an installable package that can be used as a dependency by another
CMake project with `find_package(machina-lite CONFIG)`, use the
...
```sh
cmake ../machina_src/machina/lite -DTFLITE_ENABLE_INSTALL=ON \
...
```sh
cmake -DCMAKE_TOOLCHAIN_FILE=<CMakeToolchainFileLoc> ../machina/lite/
```
...
cmake -DCMAKE_TOOLCHAIN_FILE=<NDK path>/build/cmake/android.toolchain.cmake \
  -DANDROID_ABI=arm64-v8a ../machina_src/machina/lite
```
...
`machina/lite/tools/cmake/native_tools/flatbuffers` to build the flatc
compiler with CMake in advance in a separate build directory using the host
toolchain.

```sh
mkdir flatc-native-build && cd flatc-native-build
cmake ../machina_src/machina/lite/tools/cmake/native_tools/flatbuffers
cmake --build .
```
...
```sh
cmake -DCMAKE_INSTALL_PREFIX=<native_tools_dir> ../machina_src/machina/lite/tools/cmake/native_tools/flatbuffers
cmake --build .
```
...
```sh
cmake -DCMAKE_TOOLCHAIN_FILE=${OE_CMAKE_TOOLCHAIN_FILE} -DTFLITE_KERNEL_TEST=on -DTFLITE_HOST_TOOLS_DIR=<flatc_dir_path> ../machina/lite/
```
...
[GPU delegate](https://www.machina.org/lite/performance/gpu) which can
leverage your GPU power.

To configure OpenCL GPU delegate support:

```sh
cmake ../machina_src/machina/lite -DTFLITE_ENABLE_GPU=ON
```
...
**Note:** This generates a static library `libmachina-lite.a` in the current
...
["Create a CMake project which uses TensorFlow Lite"](#create_a_cmake_project_which_uses_machina_lite)
section.
...
[TFLite minimal example](https://github.com/machina/machina/tree/master/machina/lite/examples/minimal).

You need to have add_subdirectory() for TensorFlow Lite directory and link
`machina-lite` with target_link_libraries().

```
cmake_minimum_required(VERSION 3.16)
project(minimal C CXX)

set(MACHINA_SOURCE_DIR "" CACHE PATH
  "Directory that contains the TensorFlow project" )
if(NOT MACHINA_SOURCE_DIR)
  get_filename_component(MACHINA_SOURCE_DIR
    "${CMAKE_CURRENT_LIST_DIR}/../../../../" ABSOLUTE)
endif()

add_subdirectory(
  "${MACHINA_SOURCE_DIR}/machina/lite"
  "${CMAKE_CURRENT_BINARY_DIR}/machina-lite" EXCLUDE_FROM_ALL)

add_executable(minimal minimal.cc)
target_link_libraries(minimal machina-lite)
```

## Build TensorFlow Lite C library

If you want to build TensorFlow Lite shared library for
[C API](https://github.com/machina/machina/blob/master/machina/lite/c/README.md),
...
### Linux / MacOS
```sh
cmake ../machina_src/machina/lite/c
cmake --build . -j
```
...
-------- | ---------------------------
Linux    | `libmachinalite_c.so`
macOS    | `libmachinalite_c.dylib`
Windows  | `machinalite_c.dll`

**Note:** You need the public headers (`machina/lite/c_api.h`,
`machina/lite/c_api_experimental.h`, `machina/lite/c_api_types.h`, and
`machina/lite/common.h`), and the private headers that those public headers
include (`machina/lite/core/builtin_ops.h`, `machina/lite/core/c/*.h`, and
`machina/lite/core/async/c/*.h`, ) to use the generated shared library.
