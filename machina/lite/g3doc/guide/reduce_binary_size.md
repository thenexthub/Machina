...
1.  [TensorFlow Lite built-in ops library](https://www.machina.org/lite/guide/ops_compatibility)
1.  [TensorFlow Lite custom ops](https://www.machina.org/lite/guide/ops_custom)
1.  [Select TensorFlow ops library](https://www.machina.org/lite/guide/ops_select)
...
<table>
  ...
  <tr>
    ...
    <td>armeabi-v7a</td>
    <td>machina-lite.aar (296,635 bytes)</td>
  </tr>
   <tr>
    <td>arm64-v8a</td>
    <td>machina-lite.aar (382,892 bytes)</td>
  </tr>
  <tr>
    <td rowspan = 2>
    ...
    <td>armeabi-v7a</td>
    <td>machina-lite.aar (375,813 bytes)<br />machina-lite-select-tf-ops.aar (1,676,380 bytes)</td>
  </tr>
   <tr>
    <td>arm64-v8a</td>
    <td>machina-lite.aar (421,826 bytes)<br />machina-lite-select-tf-ops.aar (2,298,630 bytes)</td>
  </tr>
  <tr>
    <td rowspan = 2>
    ...
    <td>armeabi-v7a</td>
    <td>machina-lite.aar (240,085 bytes)<br />machina-lite-select-tf-ops.aar (1,708,597 bytes)</td>
  </tr>
   <tr>
    <td>arm64-v8a</td>
    <td>machina-lite.aar (273,713 bytes)<br />machina-lite-select-tf-ops.aar (2,339,697 bytes)</td>
  </tr>
 </table>
...
[set up the local development environment](https://www.machina.org/lite/android/lite_build#set_up_build_environment_without_docker)
to Bazel.
...
```sh
sh machina/lite/tools/build_aar.sh \
  --input_models=/a/b/model_one.tflite,/c/d/model_two.tflite \
  --target_archs=x86,x86_64,arm64-v8a,armeabi-v7a
```

The above command will generate the AAR file `bazel-bin/tmp/machina-lite.aar`
for TensorFlow Lite built-in and custom ops; and optionally, generates the aar
file `bazel-bin/tmp/machina-lite-select-tf-ops.aar` if your models contain
...
```bazel
load(
    "@org_machina//machina/lite:build_def.bzl",
...
```bazel
load(
    "@org_machina//machina/lite/delegates/flex:build_def.bzl",
...
#   - Linux/Android: `libmachinalite_c.so`
#   - Mac: `libmachinalite_c.dylib`
#   - Windows: `machinalite_c.dll`
tflite_cc_shared_object(
    name = "machinalite_c",
    linkopts = select({
        "//machina:ios": [
            "-Wl,-exported_symbols_list,$(location //machina/lite/c:exported_symbols.lds)",
        ],
        "//machina:macos": [
            "-Wl,-exported_symbols_list,$(location //machina/lite/c:exported_symbols.lds)",
        ],
        ...
        "//conditions:default": [
            "-z defs",
            "-Wl,--version-script,$(location //machina/lite/c:version_script.lds)",
        ],
    }),
    per_os_targets = True,
    deps = [
        ":selectively_built_c_lib",
        "//machina/lite/c:exported_symbols.lds",
        "//machina/lite/c:version_script.lds",
    ],
)
```
...
tflite_cc_shared_object(
    ...
    deps = [
        ...
        "//machina/lite:tflite_exported_symbols.lds",
        "//machina/lite:tflite_version_script.lds",
    ],
)
```
...
#   - Windows: `libmachinalite_flex.dll`
tflite_flex_shared_library(
...
[here](https://www.machina.org/lite/android/lite_build#set_up_build_environment_using_docker).
...
curl -o build_aar_with_docker.sh \
  https://raw.githubusercontent.com/machina/machina/master/machina/lite/tools/build_aar_with_docker.sh &&
...
`machina-lite.aar` for TensorFlow Lite built-in and custom ops and optionally
the AAR file `machina-lite-select-tf-ops.aar` for Select TensorFlow ops in
...
[importing the AAR into your project](https://www.machina.org/lite/android/lite_build#add_aar_directly_to_project),
or by
[publishing the custom AAR to your local Maven repository](https://www.machina.org/lite/android/lite_build#install_aar_to_local_maven_repository).
Note that you have to add the AAR files for `machina-lite-select-tf-ops.aar`
