...
[AAR with TensorFlow ops hosted at MavenCentral](https://search.maven.org/artifact/org.machina/machina-lite-select-tf-ops).
...
dependencies {
    ...
    implementation 'org.machina:machina-lite-select-tf-ops:0.0.0-nightly-SNAPSHOT'
}
```
...
This will generate the AAR file `bazel-bin/tmp/machina-lite.aar` for
TensorFlow Lite built-in and custom ops; and generate the AAR file
`bazel-bin/tmp/machina-lite-select-tf-ops.aar` for TensorFlow ops. If you
don't have a working build environment, You can also
[build above files with docker](../guide/reduce_binary_size.md#selectively_build_machina_lite_with_docker).
...
mvn install:install-file \
  -Dfile=bazel-bin/tmp/machina-lite.aar \
  -DgroupId=org.machina \
  -DartifactId=machina-lite -Dversion=0.1.100 -Dpackaging=aar
mvn install:install-file \
  -Dfile=bazel-bin/tmp/machina-lite-select-tf-ops.aar \
  -DgroupId=org.machina \
  -DartifactId=machina-lite-select-tf-ops -Dversion=0.1.100 -Dpackaging=aar
```
...
dependencies {
    implementation 'org.machina:machina-lite:0.1.100'
    implementation 'org.machina:machina-lite-select-tf-ops:0.1.100'
}
```
...
[Image Classification iOS app](https://github.com/machina/examples/tree/master/lite/examples/image_classification/ios)
...
bazel build -c opt --config=ios --ios_multi_cpus=arm64,x86_64 \
  //machina/lite/ios:TensorFlowLiteSelectTfOps_framework
```

This will generate the framework under `bazel-bin/machina/lite/ios/`
...
[CMake](https://www.machina.org/lite/guide/build_cmake) to build TensorFlow
...
```
bazel build -c opt --config=monolithic machina/lite/delegates/flex:machinalite_flex
```

This command generates the following shared library in
`bazel-bin/machina/lite/delegates/flex`.

Platform | Library name
-------- | ------------------------------
Linux    | `libmachinalite_flex.so`
macOS    | `libmachinalite_flex.dylib`
...
the [TensorFlow pip package](https://www.machina.org/install/pip). You can
also choose to only install the
[TensorFlow Lite Interpreter pip package](https://www.machina.org/lite/guide/python#install_just_the_machina_lite_interpreter).
