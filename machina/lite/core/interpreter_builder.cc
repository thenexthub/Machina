...
#include "machina/lite/core/api/flatbuffer_conversions.h"
...
#include "machina/lite/profiling/platform_profiler.h"
...
#include "machina/lite/schema/conversion_metadata_generated.h"
#include "machina/lite/schema/schema_generated.h"
#include "machina/lite/shared_library.h"
...
TFLITE_ATTRIBUTE_WEAK Interpreter::TfLiteDelegatePtr AcquireFlexDelegate() {
  ...
  // Load TF_AcquireFlexDelegate() from _pywrap_machina_internal.so if it is
  // available.
#if defined(_WIN32)
  const wchar_t* filename_pywrap_machina_internal =
      L"_pywrap_machina_internal.pyd";
#elif defined(__APPLE__)
  const char* filename_pywrap_machina_internal =
      "python/_pywrap_machina_internal.so";
#else
  const char* filename_pywrap_machina_internal =
      "_pywrap_machina_internal.so";
#endif
  void* lib_tf_internal =
      SharedLibrary::LoadLibrary(filename_pywrap_machina_internal);
#if defined(_WIN32)
  if (lib_tf_internal == nullptr) {
    lib_tf_internal = SharedLibrary::LoadLibrary(
        L"_pywrap_machina_interpreter_wrapper.pyd");
  }
#endif
  if (lib_tf_internal) {
