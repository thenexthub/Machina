...
def android_library_with_tflite(
    ...
    library targets from //machina/lite/core/shims:*.
...
def cc_library_with_tflite(
    ...
    Args:
      ...
      generate_opaque_delegate_target: (bool) If set, generates an additional
        ...
        //machina/lite/core/shims:tflite_use_opaque_delegate
    ...
    if generate_opaque_delegate_target:
        ...
        native.cc_library(
            ...
            deps = deps + tflite_deps_renamed + _concat([select(map) for map in tflite_deps_selects_renamed]) + [
                clean_dep("//machina/lite/core/shims:tflite_use_opaque_delegate"),
            ],
            **kwargs
        )
...
def custom_c_library_with_tflite(
    ...
    if experimental:
        hdrs = [
            clean_dep("//machina/lite/core/shims:c/c_api.h"),
            clean_dep("//machina/lite/core/shims:c/c_api_experimental.h"),
            clean_dep("//machina/lite/core/shims:c/c_api_opaque.h"),
        ]
    else:
        hdrs = [
            clean_dep("//machina/lite/core/shims:c/c_api.h"),
    ...
    cc_library_with_tflite(
        ...
        deps = [
            ":%s_c_api" % name,
            "//machina/lite/core/c:private_c_api_without_op_resolver",
        ],
        **kwargs
    )
