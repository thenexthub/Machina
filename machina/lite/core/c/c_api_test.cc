...
#include "machina/lite/c/c_api_internal.h"
#include "machina/lite/core/c/builtin_op_data.h"
#include "machina/lite/core/c/c_api_opaque.h"
#include "machina/lite/core/c/c_api_types.h"
#include "machina/lite/core/c/common.h"
#include "machina/lite/core/subgraph.h"
#include "machina/lite/delegates/delegate_test_util.h"
...
TEST(CApiSimple, Smoke) {
  TfLiteModel* model =
      TfLiteModelCreateFromFile("machina/lite/testdata/add.bin");
...
TEST(CApiSimple, QuantizationParams) {
  TfLiteModel* model = TfLiteModelCreateFromFile(
      ...
      "machina/lite/testdata/add_quantized.bin");
  ASSERT_NE(model, nullptr);
...
TEST(CApiSimple, TfLiteInterpreterGetTensor) {
  ...
  // The 'machina/lite/testdata/add.bin' model uses model tensor
  ...
  // The 'third_party/machina/testdata/add.bin' model uses model tensor
...
TEST(CApiSimple, OpaqueDelegate_ReplaceNodeSubsetsWithDelegateKernels) {
  ...
  // We know that "machina/lite/testdata/add.bin" contains two
...
TEST(CApiSimple, ModelCreateWithErrorReporter) {
  ...
  std::ifstream model_file("machina/lite/testdata/add.bin");
...
TEST(CApiSimple, ModelCreateFromFileWithErrorReporter) {
  ...
  model = TfLiteModelCreateFromFileWithErrorReporter(
      "machina/lite/testdata/add.bin", error_reporter,
      &reporter);
  ASSERT_NE(model, nullptr);
  EXPECT_EQ(reporter.error_messages(), "");
  TfLiteModelDelete(model);
...
void* FlexSinhInit(void* user_data, TfLiteOpaqueContext* context,
  ...
  // (third_party/machina/compiler/mlir/lite/schema/schema.fbs) `Operator`
...
TEST(CApiSimple, CustomOpSupport) {
  TfLiteModel* model = TfLiteModelCreateFromFile(
      "machina/lite/testdata/custom_sinh.bin");
  ASSERT_NE(model, nullptr);
