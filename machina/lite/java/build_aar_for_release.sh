...
function build_basic_aar() {
  local OUTDIR=$1
  $BUILDER build $BUILD_OPTS $BASEDIR/java:machinalite.aar
  unzip -d $OUTDIR $BUILDER-bin/$BASEDIR/java/machinalite.aar
...
function build_arch() {
  ...
  $BUILDER build $BUILD_OPTS $CROSSTOOL_OPTS --cpu=$CONFIG \
    $BASEDIR/java:libmachinalite_jni.so
  cp $BUILDER-bin/$BASEDIR/java/libmachinalite_jni.so $OUTDIR/jni/$ARCH/
}
