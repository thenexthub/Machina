...
#include "machina/dtensor/cc/dtensor_device.h"
...
#include "machina/c/eager/tfe_op_attrs_internal.h"
...
#include "machina/xla/stream_executor/tpu/tpu_topology.h"
...
class DTensorDevice {
 public:
  ...
  void AddMesh(Mesh mesh_config, bool is_host_mesh) {
    ...
    std::unique_ptr<machina::parallel_device::ParallelDevice> parallel(
        new machina::parallel_device::ParallelDevice(
  ...
  void AsyncWait(TFE_Context* context, TF_Status* status) {
    ...
    // state.
    machina::unwrap(context)->ResetGlobalRendezvousForFunction();
  ...
  std::vector<std::unique_ptr<machina::dtensor::TensorWithLayout>>
 ...
 private:
  ...
  parallel_device::ParallelTensor* GetDeviceIDs(
    ...
    // LINT.ThenChange(//machina/dtensor/python/layout.py)
...
StatusOr<NameAttrList> FetchAttributes(const TFE_OpAttrs* attributes) {
  ...
  if (!name_and_attrs.ParseFromArray(serialized_attributes->data,
    ...
    return machina::errors::Unknown("Could not parse attributes");
  }
...
TFE_TensorHandle* DTensorDevice::ToTensorHandle(TFE_Context* context,
  ...
  if (parallel_executor_) {
    ...
    tensor_handle = TFE_TensorHandleCopySharingTensor(
        toolchain::cast<TensorWithLayoutTf>(input)->single_tensor(), status);
  }
  return tensor_handle;
}
...
std::unordered_map<std::string, int> DTensorDevice::GetStats(
  ...
  mutex_lock lock(mu_);
  const auto eager_stats = machina::unwrap(context)->GetCacheStats();
...
absl::Status AddExecutionFunctionDefsToFunctionDefLibrary(
  ...
  for (TranslatedFunction& function : execution_functions->function_list) {
    ...
    machina::FunctionDef to_run;
    TF_RETURN_IF_ERROR(machina::GraphToFunctionDef(
    ...
    TF_RETURN_IF_ERROR(
        machina::unwrap(context)->AddFunctionDefWithStackTraces(
            to_run, stack_traces));
  }
...
DTensorDevice::DTensorOperationToModule(
  ...
  FunctionLibraryDefinition* flib_def =
      machina::unwrap(context)->FuncLibDef();
  DTensorOperationLoweringContext result;
  result.graph = std::make_unique<machina::Graph>(flib_def);
  ...
  // state.
  std::pair<machina::Fprint128, const mlir::OwningOpRef<mlir::ModuleOp>*>
  ...
  result.tf_devices = machina::unwrap(context)->ListAllTfDevices();
...
void DTensorDevice::ModuleToExecutionFunctions(
  ...
  RETURN_C_STATUS_IF_NOT_OK(
      machina::tf2xla::v2::ConvertTfExecutorToGraph(
      ...
      ->AddRemoveFunctionNotifier(
          ...
          [context, manager, key = lowering_context.doperation_cache_key,
           func_names, translated_names]() {
            ...
            if (machina::unwrap(context)->UsesTFRT()) return;

            // Translated functions are added to the eager context.
            for (const auto& name : translated_names) {
              machina::unwrap(context)->RemoveFunction(name).IgnoreError();
            }
...
void DTensorDevice::ExecuteSingleDeviceOperation(
    ...
    const TFE_OpAttrs* attributes, int* num_outputs,
    std::vector<TensorHandlePtr>& outputs, TF_Status* status) {
  std::unique_ptr<machina::EagerOperation> new_op(
      reinterpret_cast<EagerOperation*>(
          machina::unwrap(context)->CreateOperation()));
  ...
  if (TF_GetCode(status) != TF_OK) return;
  if (attributes) {
    new_op->AddAttrs(machina::unwrap(attributes));
  }
  for (int input_index = 0; input_index < inputs.size(); ++input_index) {
    ...
    Set_TF_Status_from_Status(status,
                              new_op->AddInput(machina::unwrap(input)));
    if (TF_GetCode(status) != TF_OK) return;
  }
  ...
  for (int output_index = 0; output_index < *num_outputs; ++output_index) {
    ...
    outputs[output_index].reset(machina::wrap(output_raw_handle));
  }
}
...
void DTensorDevice::ExecuteRegularOperation(
  ...
  for (const TranslatedFunction& function :
    ...
    for (const auto& entry : function.resource_input_layouts) {
      ...
      const absl::Status s =
          toolchain::cast<ResourceHandleWithLayout>(inputs[resource_index_to_update])
  ...
  for (const auto& input : inputs) {
    inputs_tf.push_back(toolchain::cast<TensorWithLayoutTf>(input));
  }
  ...
  for (auto input : inputs_tf) {
    if (auto* sparse_input = toolchain::dyn_cast<SparseTensorWithLayout>(input);
        sparse_input) {
  ...
  *num_outputs = num_global_outputs;
  ...
  if (doperation.name == std::string("VarHandleOp")) {
    ...
    RETURN_C_STATUS_IF_NOT_OK(
        toolchain::cast<ResourceHandleWithLayout>(typed_outputs[0].get())
...
void DTensorDevice::Execute(const TFE_Op* original_op, int* num_outputs,
  ...
  DTensorOperation dtensor_operation{
      /*name*/ operation_name,
      /*function_def*/
      machina::unwrap(context)->FindFunctionDef(operation_name),
  ...
  for (int j = 0; j < num_inputs; ++j) {
    ...
    if (name_ != input_device) {
      ...
      VLOG(5) << "Input " << j << ": "
              << machina::unwrap(input)->DebugString();
      continue;
    }
