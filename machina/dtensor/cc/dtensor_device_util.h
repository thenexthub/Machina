...
#ifndef MACHINA_DTENSOR_CC_DTENSOR_DEVICE_UTIL_H_
#define MACHINA_DTENSOR_CC_DTENSOR_DEVICE_UTIL_H_
...
using TensorHandlePtr = machina::Safe_TFE_TensorHandlePtr;
...
class TensorWithLayoutTf
    : public toolchain::RTTIExtends<TensorWithLayoutTf, TensorWithLayout> {
 public:
  ...
  void EncodeAttributes(machina::NodeDefBuilder& builder) const override {}

  machina::Fprint128 CacheKey() const override;
...
class ResourceHandleWithLayout
    : public toolchain::RTTIExtends<ResourceHandleWithLayout, TensorWithLayoutTf> {
 public:
 ...
 private:
  ResourceHandleWithLayout(std::vector<TensorHandlePtr>&& tensors,
                           const Layout& layout,
                           const std::vector<int64_t>& local_shape)
      : toolchain::RTTIExtends<ResourceHandleWithLayout, TensorWithLayoutTf>(
...
class SparseTensorWithLayout
    : public toolchain::RTTIExtends<SparseTensorWithLayout, TensorWithLayoutTf> {
 public:
 ...
 private:
  SparseTensorWithLayout(
      ...
      std::optional<TF_DataType> dtype = std::nullopt,
      std::optional<NodeDef> const_value = std::nullopt)
      : toolchain::RTTIExtends<SparseTensorWithLayout, TensorWithLayoutTf>(
...
std::string ShapeToDebugString(const std::vector<T> shape_vector) {
  std::vector<machina::int64> cast_shape(shape_vector.begin(),
                                            shape_vector.end());
  machina::PartialTensorShape shape;
  if (!machina::PartialTensorShape::MakePartialShape(
...
class ExecutableManagerImpl {
 ...
 public:
  ...
  // folding.
  machina::Fprint128 CacheKeyForDTensorOperation(
...
class ExecutableManager : public tsl::core::WeakRefCounted {
 public:
  ...
  const T* AddCachedExecutable(machina::Fprint128 cache_key, T executable);

  // Removes the executable.
  void Remove(machina::Fprint128 cache_key);
  ...
  StatusOr<std::pair<machina::Fprint128, const T*>> GetCachedExecutable(
  ...
  const T* GetCachedExecutableSimple(machina::Fprint128 cache_key);
 ...
 private:
  ...
  // inputs, and outputs.
  StatusOr<machina::Fprint128> CacheKeyForGraph(
      const DTensorOperation& doperation, const NameAttrList& attributes,
      const std::vector<TensorWithLayout*>& inputs,
      const std::vector<const Layout*>& output_layouts);
  ...
  absl::flat_hash_map<machina::Fprint128, T, machina::Fprint128Hasher>
  ...
  // folding validation.
  absl::flat_hash_map<machina::Fprint128, absl::flat_hash_map<int, NodeDef>,
                      machina::Fprint128Hasher>
...
template <typename T>
StatusOr<machina::Fprint128> ExecutableManager<T>::CacheKeyForGraph(
    const DTensorOperation& doperation, const NameAttrList& attributes,
    const std::vector<TensorWithLayout*>& inputs,
    const std::vector<const Layout*>& output_layouts) {
  machina::Fprint128 cache_key = machina::Fingerprint128(doperation.name);
  std::string serialized;
  SerializeToStringDeterministic(attributes, &serialized);
  cache_key =
      FingerprintCat128(cache_key, machina::Fingerprint128(serialized));
  cache_key = FingerprintCat128(
      cache_key,
      machina::Fingerprint128(doperation.default_mesh.ToString()));
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    ...
    cache_key = FingerprintCat128(
        cache_key, machina::Fingerprint128(absl::StrFormat("%x", i)));
  ...
  for (int output_index = 0; output_index < output_layouts.size();
    ...
    if (output_layouts[output_index]) {
      ...
      cache_key = FingerprintCat128(
          cache_key,
          machina::Fingerprint128(output_layouts[output_index]->ToString()));
    }
  }
  return cache_key;
}
...
// Thread-safe method.
template <typename T>
StatusOr<std::pair<machina::Fprint128, const T*>>
ExecutableManager<T>::GetCachedExecutable(
    const DTensorOperation& doperation, const NameAttrList& attributes,
    const std::vector<TensorWithLayout*>& inputs,
    const std::vector<const Layout*>& output_layouts) {
  TF_ASSIGN_OR_RETURN(
      machina::Fprint128 cache_key,
...
bool ExecutableManager<T>::UpdateDTensorOpAndSmallInputsCache(
    const DTensorOperation& doperation,
    const std::vector<TensorWithLayout*>& inputs) {
  const machina::Fprint128 doperation_hash =
...
const T* ExecutableManager<T>::AddCachedExecutable(
    machina::Fprint128 cache_key, T executable) {
  mutex_lock lock(mu_);
...
template <typename T>
void ExecutableManager<T>::Remove(machina::Fprint128 cache_key) {
  mutex_lock lock(mu_);
  auto iter = function_cache_.find(cache_key);
  if (iter != function_cache_.end()) {
...
#endif  // MACHINA_DTENSOR_CC_DTENSOR_DEVICE_UTIL_H_
