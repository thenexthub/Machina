...
#include "machina/dtensor/cc/small_constant_optimization.h"
...
std::unique_ptr<TensorWithLayoutTf> BroadcastResourceTensor(
  ...
  if (!r->dtypes_and_shapes().empty()) {
    ...
    const absl::Status s =
        toolchain::cast<ResourceHandleWithLayout>((*result).get())
...
absl::Status ParseAttrMap(const Node& node, absl::string_view indices_attr,
  ...
  for (int i = 0; i < indices->int_val_size(); ++i) {
    ...
    indices_layout_map->emplace(
        arg_index, machina::dtensor::Layout::FromString(arg_layout).value());
  }
  return absl::OkStatus();
}
...
StatusOr<std::vector<int64_t>> GetTensorShapeAsVector(
    const machina::PartialTensorShape& shape) {
...
StatusOr<std::vector<int64_t>> GetTensorShapeAsVector(
  ...
  const absl::Status status = machina::unwrap(tensor)->Shape(&shape);
  if (status.ok()) {
...
machina::Fprint128 TensorWithLayoutTf::CacheKey() const {
  machina::Fprint128 f = machina::Fingerprint128(layout_.ToString());
  ...
  if (const_value_node_->const_value().has_value()) {
    ...
    SerializeToStringDeterministic(const_value_node_->const_value().value(),
                                   &serialized);
    f = FingerprintCat128(f, machina::Fingerprint128(serialized));
  }
  return f;
}
...
StatusOr<std::string> SummarizeValues(
  ...
  for (int component_index = 0; component_index < tensors_.size();
    ...
    ImmediateExecutionTensorHandle* component =
        machina::unwrap(tensors_[component_index].get());
...
std::string TensorWithLayoutTf::SummarizeValue() const {
  ...
  if (layout_.IsSingleDevice() || layout_.IsFullyReplicated()) {
    status =
        machina::unwrap(tensors_[0].get())->SummarizeValue(value_summary);
  } else {
...
void ResourceHandleWithLayout::EncodeAttributes(
    machina::NodeDefBuilder& builder) const {
...
machina::Fprint128 ResourceHandleWithLayout::CacheKey() const {
  machina::Fprint128 f = machina::Fingerprint128(layout().ToString());
  if (dereferenced_shape().has_value()) {
    std::string serialized;
...
std::string SparseTensorWithLayout::SummarizeValue() const {
  ...
  if (layout().IsFullyReplicated()) {
    indices_status = machina::unwrap(indices_->tensor(0))
                         ->SummarizeValue(indices_summary);
    values_status =
        machina::unwrap(values_->tensor(0))->SummarizeValue(values_summary);
    dense_shapes_status = machina::unwrap(dense_shapes_->tensor(0))
...
absl::Status InferOutputLayouts(const DTensorOperation& doperation,
  ...
  machina::Node* op_node = graph->AddNode(op_node_def, &status);
...
absl::Status PrepareGraphForMlir(
    ...
    const DTensorOperation& doperation,
    const machina::FunctionLibraryDefinition& flib_def,
    const NameAttrList& attributes,
    const std::vector<const Layout*>& output_layouts, machina::Graph* graph,
  ...
  absl::Status status;
  {
    ...
    machina::NodeDefBuilder builder("device_id", "_Arg");
    machina::PartialTensorShape partial_shape;
    TF_RETURN_IF_ERROR(machina::PartialTensorShape::MakePartialShape(
        static_cast<int*>(nullptr), 0, &partial_shape));
    machina::NodeDef arg_node_def;
    TF_RETURN_IF_ERROR(builder.Attr("shape", partial_shape)
                           .Attr("T", machina::DT_INT32)
                           .Attr("index", 0)
                           .Finalize(&arg_node_def, /*consume=*/true));
    machina::Node* arg_node = graph->AddNode(arg_node_def, &status);
    TF_RETURN_IF_ERROR(status);
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    ...
    std::vector<machina::int64> cast_shape(shape.begin(), shape.end());
    machina::PartialTensorShape partial_shape;
    ...
    machina::NodeDef arg_node_def;
    auto dtype = static_cast<machina::DataType>(input->dtype());
    machina::NodeDefBuilder builder(absl::StrCat("op_input_", i), "_Arg");
    ...
    if (!should_fold_input) {
      ...
      graph_op_inputs.push_back(FunctionArgument{
          const_value_n, machina::NodeDefBuilder::NodeOut{
  ...
  if (function_def) {
    ...
    std::vector<machina::NodeDefBuilder::NodeOut> func_inputs;
    std::vector<machina::DataType> inputs_types;
    for (const auto& in : graph_op_inputs) {
      func_inputs.emplace_back(in.output);
      inputs_types.emplace_back(in.output.data_type);
    }

    std::vector<machina::DataType> output_types;
  ...
  for (int output_index = 0; output_index < op_node->num_outputs();
       ++output_index) {
    machina::NodeDefBuilder builder(absl::StrCat("op_output_", output_index),
                                       "_Retval");
    machina::NodeDef ret_node_def;
    machina::DataType output_type = op_node->output_type(output_index);
    ...
    machina::Node* ret_node = graph->AddNode(ret_node_def, &status);
    TF_RETURN_IF_ERROR(status);
...
absl::Status MaybeInsertIdentityNodes(const FunctionDef* function_def,
  ...
  for (Node* n : graph->nodes()) {
    ...
    machina::NodeDefBuilder identity_builder(
        absl::StrCat("op_output_identity_", ret_index), "Identity");
    machina::NodeDef ret_identity_node_def;
    machina::DataType output_type = n->input_type(0);
    TF_RETURN_IF_ERROR(
...
machina::Fprint128 ExecutableManagerImpl::CacheKeyForDTensorOperation(
    const DTensorOperation& doperation) const {
  return machina::Fingerprint128(doperation.name);
}
