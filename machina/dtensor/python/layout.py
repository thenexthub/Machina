...
from machina.dtensor.proto import layout_pb2
from machina.python import _pywrap_dtensor_device
...
UNSHARDED = 'unsharded'
MATCH = 'match'
USE_MACHINA_MACHINA_XLA_SPMD = False
...
class Mesh(_pywrap_dtensor_device.Mesh):
  ...
  Refer to [DTensor Concepts](https://www.machina.org/guide/dtensor_overview)
  ...
  def __init__(
      ...
      use_xla_spmd: bool = USE_MACHINA_MACHINA_XLA_SPMD,
  ):
    """Builds a Mesh.
    ...
    if any(
      ...
      raise ValueError('global_device_ids must sequentially increase: %s' %
                       global_device_ids)
    # LINT.ThenChange(//machina/dtensor/cc/dtensor_device.cc)
