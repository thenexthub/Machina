...
mlir::BlockArgument InsertArgumentForDevice(mlir::OpBuilder& builder,
  ...
  std::vector<mlir::NamedAttribute> named_attrs = {builder.getNamedAttr(
      kFuncDeviceAttr, builder.getStringAttr(toolchain::StringRef(device)))};

  toolchain::ArrayRef<mlir::NamedAttribute> named_array_ref(named_attrs);
  ...
  for (mlir::Operation* op : ops) {
    for (mlir::Operation* user : op->getUsers()) {
      ...
      if constexpr (std::is_same_v<UserType, mlir::Operation*>) {
        ...
        typed = toolchain::dyn_cast_or_null<UserType>(user);
      }
      if (typed) {
        if (*result == nullptr) {
          *result = typed;
...
StatusOr<std::optional<Layout>> FindResourceLayout(mlir::BlockArgument arg) {
  ...
  for (mlir::Operation* user : arg.getUsers()) {
    ...
    for (auto [i, index] : toolchain::enumerate(resource_indices)) {
...
mlir::LogicalResult RewriteTPUFunction(mlir::func::FuncOp func,
  ...
  toolchain::iterator_range<mlir::Region::OpIterator> op_range =
  ...
  for (mlir::Operation* op : ops) {
    ...
    if (toolchain::isa<mlir::tf_device::LaunchOp>(op)) {
    ...
    } else if (toolchain::isa<mlir::func::ReturnOp>(op)) {
...
mlir::LogicalResult ExpandTPUOperation(
  ...
  func.walk([&](mlir::tf_device::LaunchOp op) {
    (op.getBody()).walk([&](mlir::Operation* child) {
      if (toolchain::isa<mlir::TF::TPUExecuteOp>(child) ||
          toolchain::isa<mlir::TF::TPUExecuteAndUpdateVariablesOp>(child)) {
        launch_op = op;
  ...
  if (return_op) {
    ...
    for (const auto [result_index, operand] : toolchain::enumerate(operands)) {
      ...
      if (op == operand.getDefiningOp()) {
        ...
        const mlir::Operation::result_range::iterator search =
            toolchain::find(results, operand);
...
mlir::LogicalResult ExpandOperation(
  ...
  toolchain::SmallVector<Operation> replications;
  ...
  if (return_op) {
    mlir::Operation::operand_range operands = return_op->getOperands();
    for (const auto [i, operand] : toolchain::enumerate(operands)) {
      if (op == operand.getDefiningOp()) {
        const mlir::Operation::result_range results = op->getResults();
        const mlir::Operation::result_range::iterator search =
            toolchain::find(results, operand);
...
mlir::NamedAttribute GetNamesAttr(mlir::OpBuilder& builder, const char* name,
                                  toolchain::ArrayRef<mlir::Type> types,
                                  const Format& fmt) {
  toolchain::SmallVector<string, 8> names;
...
void UpdateEntryFuncAttr(mlir::OpBuilder& builder, mlir::func::FuncOp func) {
  ...
  toolchain::SmallVector<mlir::NamedAttribute, 2> named_attrs = {inputs, outputs};
  toolchain::ArrayRef<mlir::NamedAttribute> named_array_ref(named_attrs);
  mlir::DictionaryAttr dic_attr = builder.getDictionaryAttr(named_array_ref);
...
mlir::LogicalResult GetInferredResourceAttributes(
  ...
  toolchain::SmallVector<mlir::Attribute, 8> resource_layouts;
  toolchain::SmallVector<int32_t, 8> resource_indices;
  for (mlir::Operation* call_op : call_ops) {
    ...
    if (resource_indices_attr && resource_layouts_attr) {
      for (auto [index, layout] :
           toolchain::zip(resource_indices_attr, resource_layouts_attr)) {
...
mlir::LogicalResult BuildOuterMainFunc(
  ...
  for (auto [arg_index, arg] :
       toolchain::enumerate(translated_func.getArguments())) {
...
absl::Status ExtractResultLayouts(
  ...
  mlir::Operation::operand_range operands = return_op.getOperands();
  for (auto [layout_index, result] : toolchain::enumerate(op->getResults())) {
...
struct DTensorMultiDeviceExpansion
  ...
  void runOnOperation() override {
    ...
    std::string translated_func_name =
        toolchain::formatv("_multi_device_func_{0}_{1}", OpHash(module),
                      OpHash(main_func))
            .str();
    ...
    toolchain::SmallVector<mlir::TF::StatefulPartitionedCallOp, 8> stateful_call_ops;
    main_func.walk([&](mlir::Operation* op) {
      if (const auto stateful_call_op =
              toolchain::dyn_cast_or_null<mlir::TF::StatefulPartitionedCallOp>(op)) {
    ...
    std::vector<mlir::Value> results;
    toolchain::SmallVector<mlir::Attribute, 8> num_local_outputs;
    if (return_op) {
