...
mlir::Value GetIdentitySkippedInputs(mlir::Value val) {
  ...
  while (auto identity = toolchain::dyn_cast_or_null<mlir::TF::IdentityOp>(
             input.getDefiningOp())) {
...
bool IsZeroConstant(mlir::Value val) {
  auto const_input = toolchain::dyn_cast_or_null<mlir::TF::ConstOp>(
...
mlir::LogicalResult CheckReduceAndSumOptimizationCriteria(
    ...
    toolchain::SmallVectorImpl<mlir::Value>* reduction_inputs,
    toolchain::SmallVectorImpl<mlir::TF::DTensorAllReduceOp>* reduction_ops,
  ...
  for (mlir::Value operand : add_op->getOperands()) {
    ...
    auto reduction_op = toolchain::dyn_cast_or_null<mlir::TF::DTensorAllReduceOp>(
  ...
  toolchain::SmallDenseSet<mlir::Attribute> reduction_group_assignments;
...
mlir::LogicalResult OptimizeAllReduceAndSum(mlir::Operation* op,
                                            bool* changed) {
  bool can_be_reordered;
  toolchain::SmallVector<mlir::TF::DTensorAllReduceOp, 4> reduction_ops;
  toolchain::SmallVector<mlir::Value, 4> reduction_op_inputs;
  ...
  if (!layout_or_status.ok())
    return first_reduction_op->emitOpError(toolchain::formatv(
  ...
  op->getResult(0).replaceAllUsesExcept(
      ...
      toolchain::SmallPtrSet<mlir::Operation*, 1>{all_reduce.getOperation()});
...
mlir::Value SkipIdentityLikeOpsOutputs(mlir::Value val) {
  while (val.hasOneUse() &&
         toolchain::isa<mlir::TF::CastOp, mlir::TF::ReshapeOp, mlir::TF::IdentityOp>(
             *val.user_begin())) {
...
bool MayRemoveAllReduce(mlir::Operation* op) {
  ...
  if (value_after_identity_like_ops.hasOneUse() &&
      toolchain::isa<mlir::TF::AddNOp, mlir::TF::AddV2Op, mlir::TF::AddOp>(
...
void OptimizeIdentityLikeOps(mlir::Operation* op, bool* changed) {
  auto dtensor_all_reduce =
      toolchain::dyn_cast_or_null<mlir::TF::DTensorAllReduceOp>(
  ...
  toolchain::SmallPtrSet<mlir::Operation*, 4> exceptions{dtensor_all_reduce};
...
bool CheckWhileLoopOptimizationCriteria(
  ...
  if (!output_defining_op ||
      !toolchain::isa<mlir::TF::AddV2Op, mlir::TF::AddOp>(output_defining_op)) {
    return false;
  }
  ...
  if (all_reduce) {
    ...
    *add_input = &second_operand;
    ...
    all_reduce = toolchain::dyn_cast_or_null<mlir::TF::DTensorAllReduceOp>(
  ...
  if (!group_assignment || !toolchain::isa<mlir::TF::ConstOp>(group_assignment))
    return false;
...
mlir::LogicalResult ExtractAllReduceFromWhileOp(
  ...
  if (!layout_or_status.ok())
    return all_reduce->emitOpError(toolchain::formatv(
        "Malformed layout specification for DTensorAllReduce op found: {0}",
        layout_or_status.status().message()));
  ...
  toolchain::SmallPtrSet<mlir::Operation*, 4> exceptions;
...
mlir::LogicalResult OptimizeWhileLoopLazyAllReduce(
  ...
  for (const auto& data :
       toolchain::enumerate(while_body_terminator->getOpOperands())) {
...
mlir::LogicalResult ApplyOptimization(
    ...
    const toolchain::SmallVectorImpl<mlir::Operation*>& identity_like_ops,
    const toolchain::SmallVectorImpl<mlir::TF::WhileRegionOp>& while_ops,
    const toolchain::SmallVectorImpl<mlir::Operation*>& add_ops, bool* changed) {
...
void CollectOptimizationCandidates(
    ...
    toolchain::SmallVectorImpl<mlir::Operation*>* identity_like_ops,
    toolchain::SmallVectorImpl<mlir::Operation*>* add_ops,
    toolchain::SmallVectorImpl<mlir::TF::WhileRegionOp>* while_ops) {
  func.walk([&](mlir::Operation* op) {
    if (toolchain::isa<mlir::TF::IdentityOp, mlir::TF::CastOp, mlir::TF::ReshapeOp>(
            op))
    ...
    if (toolchain::isa<mlir::TF::AddOp, mlir::TF::AddV2Op, mlir::TF::AddNOp>(op))
...
struct DTensorAllReduceSumOptimization
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::Operation*, 4> identity_like_ops;
    toolchain::SmallVector<mlir::Operation*, 4> add_ops;
