...
#include "machina/dtensor/cc/dtensor_utils.h"
...
bool AllOpResultsHaveLayouts(
    ...
    const toolchain::DenseMap<mlir::Value, Layout>& layouts) {
...
void UpdateLayoutForSkippedOps(
    ...
    toolchain::DenseMap<mlir::Value, Layout>& layouts) {
  toolchain::SmallVector<mlir::Value, 4> skipped_values;
...
StatusOr<Layout> MergeLayouts(
  ...
  toolchain::DenseSet<toolchain::StringRef> producer_dims;
  for (int j = 0; j < producer->rank(); ++j) {
    toolchain::StringRef spec = producer->sharding_spec(j);
...
mlir::LogicalResult InsertLayoutsForDTensorLayout(
    ...
    toolchain::DenseMap<mlir::Value, std::optional<Layout>>& producer_request,
    toolchain::DenseSet<mlir::Value>& is_updated,
    toolchain::DenseSet<mlir::Value>& is_locked) {
  return mlir::failure(
      module
...
mlir::LogicalResult InsertInitialLayoutsFromComputeLayout(
    mlir::ModuleOp module,
    const toolchain::DenseMap<mlir::Value, std::vector<mlir::OpOperand*>>& consumers,
    const toolchain::DenseMap<mlir::OpOperand*, std::vector<mlir::Value>>& producers,
    toolchain::DenseMap<mlir::Value, std::optional<Layout>>& producer_request,
    toolchain::DenseMap<mlir::Value, mlir::DenseMap<mlir::OpOperand*, Layout>>&
        consumer_requests,
    toolchain::DenseSet<mlir::Value>& is_updated) {
  auto walk_result = module.walk([&](mlir::Operation* op) {
    ...
    StatusOr<toolchain::DenseMap<int, Layout>> forward_result =
        expander->ComputeLayoutForward(
            op, /*input_layouts=*/toolchain::DenseMap<int, Layout>(),
            /*output_layouts=*/toolchain::DenseMap<int, Layout>());
    if (!forward_result.ok()) {
      op->emitOpError() << "ComputeLayoutForward error: "
                        << forward_result.status().message();
      return mlir::WalkResult::interrupt();
    }
    StatusOr<toolchain::DenseMap<int, Layout>> backward_result =
...
mlir::LogicalResult MergeAndGetUpdatedLayouts(
    const toolchain::DenseSet<mlir::Value>& is_locked,
    toolchain::DenseSet<mlir::Value>& is_updated,
    toolchain::DenseMap<mlir::Value, std::optional<Layout>>& producer_request,
    toolchain::DenseMap<mlir::Value, mlir::DenseMap<mlir::OpOperand*, Layout>>&
        consumer_requests,
    toolchain::DenseMap<mlir::Value, Layout>& merged_layouts) {
  toolchain::DenseSet<mlir::Value> updated_merge;
...
mlir::LogicalResult GetMostShardedLayoutHelper(toolchain::ArrayRef<Layout> layouts,
  ...
  if (!new_layout.ok()) {
    return mlir::emitError(
        location, toolchain::formatv("error in layout propagation while merging "
                                "producer layouts. {0}",
...
mlir::LogicalResult MergeProducerLayouts(
    const toolchain::DenseMap<mlir::Value, Layout>& merged_layouts,
  ...
  toolchain::SmallVector<Layout, 4> candidate_layouts;
    ...
    for (mlir::Value value : producer_values->getSecond()) {
      ...
      if (input_layout != new_input_layouts.end() &&
        ...
        const bool exempt_restore_unknown_rank =
            ...
            toolchain::isa<mlir::TF::RestoreV2Op>(value.getDefiningOp());
        bool producer_is_resource =
            value.getDefiningOp() &&
            toolchain::isa<mlir::TF::VarHandleOp>(value.getDefiningOp());
...
mlir::LogicalResult UpdateDTensorSendRecvOps(mlir::ModuleOp module,
                                             mlir::OpBuilder& builder) {
  toolchain::SmallVector<mlir::TF::DTensorSend, 4> send_ops;
  toolchain::SmallVector<mlir::TF::DTensorRecv, 4> recv_ops;
  module.walk([&](mlir::Operation* op) {
    if (auto send_op = toolchain::dyn_cast<mlir::TF::DTensorSend>(op))
      send_ops.emplace_back(send_op);
    if (auto recv_op = toolchain::dyn_cast<mlir::TF::DTensorRecv>(op))
...
void GetOperationsNeedingUpdate(
    const toolchain::DenseSet<mlir::Value>& is_updated,
    const toolchain::DenseMap<mlir::Value, std::vector<mlir::OpOperand*>>& consumers,
    toolchain::DenseSet<mlir::Operation*>& operations) {
  for (auto& value : is_updated) {
...
class LayoutPrinter : public mlir::OpAsmPrinter {
 public:
  ...
  toolchain::raw_ostream& getStream() const override { return os_; }

  void printRegionArgument(mlir::BlockArgument arg,
                           toolchain::ArrayRef<mlir::NamedAttribute> argAttrs,
  ...
  void printOperand(mlir::Value value, toolchain::raw_ostream& os) override {
  ...
  void printLayout(const Layout& layout, toolchain::raw_ostream& os) {
  ...
  void printSymbolName(toolchain::StringRef symbolRef) override {
    os_ << symbolRef;
  };
  ...
  void printOptionalAttrDict(
      ...
      toolchain::ArrayRef<toolchain::StringRef> elidedAttrs) override {}
 ...
 private:
  int indent_level_;
  toolchain::raw_ostream& os_;
  toolchain::DenseMap<mlir::Operation*, int> location_;
  int current_location_;
  toolchain::DenseMap<mlir::BlockArgument, int> arguments_;
  int next_argument_;
  const toolchain::DenseMap<mlir::Value, Layout>& merged_layouts_;
};
...
void LogLayoutsAndOps(const int stage, const int steps,
  ...
  if (machina::CreateFileForDumping(prefix, &os, &filepath).ok()) {
...
mlir::LogicalResult InsertDTensorLayoutForIfRegionOp(
    const toolchain::SmallVectorImpl<mlir::TF::IfRegionOp>& if_ops,
  ...
  for (mlir::TF::IfRegionOp if_op : if_ops) {
    for (mlir::OpResult if_result : if_op.getResults()) {
      ...
      auto if_result_layout =
          toolchain::dyn_cast<mlir::TF::DTensorLayout>(*if_result.user_begin());
      auto then_result_layout = toolchain::dyn_cast<mlir::TF::DTensorLayout>(
          *then_branch_result.getDefiningOp());
      auto else_result_layout = toolchain::dyn_cast<mlir::TF::DTensorLayout>(
          *else_branch_result.getDefiningOp());
      toolchain::SmallVector<Layout, 4> layouts{if_result_layout.getLayout(),
...
mlir::LogicalResult InsertRelayoutForWhileLoops(
    const toolchain::SmallVectorImpl<mlir::TF::WhileRegionOp>& while_ops,
    mlir::OpBuilder& builder) {
  for (mlir::TF::WhileRegionOp op : while_ops) {
    ...
    for (int i = 0; i < op.getBody().getNumArguments(); ++i) {
      ...
      op->getResult(i).replaceAllUsesExcept(
          second_relayout.getOutput(), toolchain::SmallPtrSet<mlir::Operation*, 1>{
...
void FindRoot(
    ...
    toolchain::DenseMap<mlir::OpOperand*, std::vector<mlir::Value>>& producers,
    toolchain::DenseSet<mlir::Value>* roots) {
  // Standard BFS to find root values of current_value.
  std::deque<mlir::Value> to_process;
  to_process.push_back(current_value);

  toolchain::DenseSet<mlir::Value> visited;
...
void FindRootsAndEmitError(
  ...
  for (auto& root : roots) {
    for (mlir::OpOperand& operand : root.getUses()) {
      ...
      toolchain::SmallVector<mlir::OpOperand*, 4> consuming_operands =
          TraceUseToNextTFOp(&operand, func_to_caller, &skipped_values);
...
absl::Status RunOneIteration(
    ...
    toolchain::DenseMap<mlir::Value, Layout>& merged_layouts, mlir::ModuleOp& module,
    int stage, int* steps) {
  ...
  toolchain::DenseSet<mlir::Operation*> operations_needing_update;
...
absl::Status CompareMergedLayouts(
    const toolchain::DenseMap<mlir::Value, Layout>& merged_a,
    const toolchain::DenseMap<mlir::Value, Layout>& merged_b,
    toolchain::DenseSet<mlir::Value>& changed) {
...
struct DLayoutPropagationPassV2
  ...
  void runOnOperation() override {
    ...
    while (!is_updated.empty() && stage < kLayoutPropagationMaxStages) {
      ...
      // changing layouts.
      toolchain::DenseMap<mlir::Value, Layout> merged_layouts_at_max_steps =
          merged_layouts;
      toolchain::DenseSet<mlir::Value> changed;
    ...
    toolchain::SmallVector<mlir::TF::IfRegionOp, 4> if_ops;
    toolchain::SmallVector<mlir::TF::WhileRegionOp, 4> while_ops;
    module.walk([&](mlir::Operation* op) {
      if (auto if_op = toolchain::dyn_cast<mlir::TF::IfRegionOp>(op))
        if_ops.emplace_back(if_op);
      else if (auto while_op = toolchain::dyn_cast<mlir::TF::WhileRegionOp>(op))
