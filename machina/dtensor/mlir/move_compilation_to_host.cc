...
mlir::LogicalResult CreateSendRecvOpsToTransferProgramKey(
  ...
  toolchain::SmallDenseMap<int, std::string> device_key_map;
  ...
  // computation.
  toolchain::SmallVector<mlir::func::FuncOp, 4> compilation_key_functions;
  ...
  for (int i = 0; i < num_tpu_devices; ++i) {
    auto func_type = mlir::FunctionType::get(
        builder.getContext(), toolchain::ArrayRef<mlir::Type>{},
        toolchain::ArrayRef<mlir::Type>{compilation_key.getType()});

    mlir::func::FuncOp recv_select_fn = mlir::func::FuncOp::create(
        compile_op.getLoc(),
        toolchain::formatv("recv_compile_key_{0}_{1}", i, *num_send_recv).str(),
  ...
  auto program_key = builder.create<mlir::TF::CaseOp>(
      compile_op.getLoc(),
      /*output=*/toolchain::SmallVector<mlir::Type, 4>{compilation_key.getType()},
      /*branch_index=*/*device_id,
      /*input=*/toolchain::ArrayRef<mlir::Value>{},
...
mlir::LogicalResult SendRecvCompilationKey(
    ...
    toolchain::SmallVectorImpl<CompilationKeyRecvInfo>* recv_info) {
...
mlir::LogicalResult HandleCompilationOps(
    const toolchain::SmallVectorImpl<
  ...
  const auto xla_mesh = toolchain::find_if(
  ...
  for (auto compilation_key : compilation_key_ops) {
    ...
    if (!host_function) {
      ...
      auto mesh_it = toolchain::find_if(computation_map, [&](auto& it) {
  ...
  if (host_mesh.local_device_ids().size() > 1) {
    ...
    if (!device_ordinal_host.ok())
      return compile_op.emitOpError(
          toolchain::formatv("error while creating TPU compilation logic. {0}",
    ...
    auto if_host = builder.create<mlir::TF::IfRegionOp>(
        compile_op.getLoc(), toolchain::SmallVector<mlir::Type, 4>{}, predicate_host,
        /*is_stateless=*/builder.getBoolAttr(false),
  ...
  toolchain::SmallVector<CompilationKeyRecvInfo, 4> compilation_key_recv_info;
  ...
  tpu_function.walk([&](mlir::Operation* op) {
    if (toolchain::isa<mlir::TF::TPUExecuteOp,
...
struct DTensorMoveCompilationToHost
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::TF::_XlaCompileMlirPlaceholderProgramKeyOp, 4>
        compilation_key_ops;
