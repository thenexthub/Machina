...
bool IsStringType(mlir::Type type) {
  ...
  bool has_string =
      toolchain::any_of(sub_type.GetSubtypes(), [](mlir::TensorType type) {
...
StatusOr<mlir::Value> GetDeviceOrdinal(const Mesh& mesh,
  ...
  toolchain::SmallVector<int32, 4> device_id_to_ordinal(mesh.num_devices(), 0);
...
StatusOr<mlir::Operation*> LowerDTensorSendToXlaOp(
  ...
  mlir::Operation* lowered_send_op;
  if (send_from_cpu) {
    toolchain::SmallVector<mlir::Value, 4> value_to_send{send_input};
...
StatusOr<mlir::Operation*> LowerDTensorRecvToXlaOp(
  ...
  if (recv_at_cpu) {
    ...
    toolchain::SmallVector<mlir::Type, 4> output_types{output_type};
    auto recv_cluster =
...
StatusOr<mlir::Operation*> LowerDTensorRecvFromCPUToTFOp(
  ...
  mlir::OpBuilder builder(&recv_cluster.GetBody().front());
  toolchain::SmallVector<mlir::Type, 4> output_types{dtensor_recv.getType()};
  builder.setInsertionPoint(dtensor_recv);
...
toolchain::SmallVector<mlir::Attribute, 4> GenerateBranches(
    mlir::Operation* op, mlir::SymbolTable& symbol_table,
    toolchain::ArrayRef<mlir::Type> result_types, const char* fmt, const It& values,
    const Fn& fn) {
  toolchain::SmallVector<mlir::Attribute, 4> branches;
  for (const auto& it : toolchain::enumerate(values)) {
    ...
    mlir::func::FuncOp func_op = mlir::func::FuncOp::create(
        location, toolchain::formatv(fmt, OpName(op), OpHash(op), it.index()).str(),
...
StatusOr<mlir::Operation*> LowerOneToOneDTensorSendToTFHostSend(
  ...
  auto device_pairs =
      toolchain::zip(send_mesh.local_devices(), recv_mesh.local_devices());
  mlir::OpBuilder builder(dtensor_send);
  ...
  auto branches = GenerateBranches(
      dtensor_send, symbol_table, toolchain::ArrayRef<mlir::Type>{},
...
StatusOr<mlir::Operation*> LowerOneToOneDTensorRecvToTFHostRecv(
  ...
  auto branches = GenerateBranches(
      dtensor_recv, symbol_table, toolchain::ArrayRef<mlir::Type>{local_output_type},
  ...
  mlir::Operation* case_op = builder.create<mlir::TF::CaseOp>(
      dtensor_recv.getLoc(),
      /*output=*/toolchain::ArrayRef<mlir::Type>{local_output_type},
      /*branch_index=*/device_ordinal,
...
StatusOr<mlir::Operation*> LowerDTensorSend(mlir::Operation* send_op,
  ...
  auto dtensor_send = toolchain::cast<mlir::TF::DTensorSend>(send_op);
  ...
  if (IsTpuToHostMeshTransfer(input_mesh, target_mesh) && one_to_one) {
    ...
    auto send_if = builder.create<mlir::TF::IfRegionOp>(
        loc, toolchain::SmallVector<mlir::Type, 4>{}, predicate,
    ...
    auto yield = builder.create<mlir::TF::YieldOp>(
        loc, /*operands=*/toolchain::ArrayRef<mlir::Value>{});
...
StatusOr<mlir::Operation*> LowerDTensorRecv(mlir::Operation* send_op,
                                            mlir::Operation* recv_op) {
  auto dtensor_recv = toolchain::cast<mlir::TF::DTensorRecv>(recv_op);
  ...
  if (IsGpuToHostMeshTransfer(send_mesh, recv_mesh) &&
    ...
    auto recv_if = builder.create<mlir::TF::IfRegionOp>(
        loc, toolchain::SmallVector<mlir::Type, 4>{output_type}, predicate,
        /*is_stateless=*/builder.getBoolAttr(true),
    ...
    if (output_element_type.isIntOrIndex()) {
      if (output_element_type.isInteger(64)) {
        const_attr = mlir::DenseIntElementsAttr::get(
            output_type, toolchain::SmallVector<int64_t>{0});
      } else {
        const_attr = mlir::DenseIntElementsAttr::get(
            output_type, toolchain::SmallVector<int32_t>{0});
      }
      ...
      const_attr = mlir::DenseElementsAttr::get(
          output_type, toolchain::SmallVector<mlir::Attribute>{zero});
    } else if (output_element_type.isF16() || output_element_type.isF32()) {
      const_attr = mlir::DenseFPElementsAttr::get(
          output_type, toolchain::SmallVector<float>{0.0});
    } else if (output_element_type.isF64()) {
      const_attr = mlir::DenseFPElementsAttr::get(
          output_type, toolchain::SmallVector<double>{0.0});
    } else {
    ...
    builder.create<mlir::TF::YieldOp>(
        loc, /*operands=*/toolchain::ArrayRef<mlir::Value>{zeros});
    ...
    builder.create<mlir::TF::YieldOp>(
        loc,
        /*operands=*/toolchain::ArrayRef<mlir::Value>{xla_recv->getResult(0)});
