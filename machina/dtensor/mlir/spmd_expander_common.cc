...
StatusOr<mlir::TensorType> LocalTypeFromGlobalType(
  ...
  auto shape = toolchain::to_vector<4>(original_type.getShape());
...
StatusOr<std::optional<Layout>> GetMergedOperandLayout(
  ...
  toolchain::SmallVector<std::pair<const Layout&, toolchain::ArrayRef<int64_t>>, 4>
  ...
  for (const auto& index_and_layout : operand_layouts) {
    TF_ASSIGN_OR_RETURN(
        toolchain::ArrayRef<int64_t> shape_to_merge,
  ...
  toolchain::ArrayRef<int64_t> merged_shape =
...
mlir::Value GetForwardedDTensorLayoutInput(mlir::Value value) {
  auto layout_op =
      toolchain::dyn_cast_or_null<mlir::TF::DTensorLayout>(value.getDefiningOp());
...
toolchain::SmallVector<mlir::OpOperand*, 4> TraceUseToNextTFOp(
  ...
  toolchain::SmallVector<mlir::Value, 4> unused_values;
  ...
  toolchain::SmallVector<mlir::OpOperand*, 4> ret;
...
void RemoveUnusedClusterResults(mlir::tf_device::ClusterOp cluster) {
  toolchain::SmallVector<mlir::OpResult, 4> new_result_values;
  toolchain::SmallVector<mlir::Value, 4> result_producing_values;
  ...
  toolchain::SmallVector<mlir::Type, 4> new_result_types;
  toolchain::transform(new_result_values, std::back_inserter(new_result_types),
  ...
  for (auto it : toolchain::zip(new_result_values, new_cluster.getResults())) {
...
absl::Status ExtractConstStringVectorFromValue(
  ...
  if (!matchPattern(value, m_Constant(&attr))) {
    return errors::Internal(
        toolchain::formatv("failed to extract constant string vector from : {0}",
                      value)
            .str());
  }
