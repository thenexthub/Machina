...
mlir::LogicalResult ExtractMeshFromOpOutput(mlir::Value value,
  ...
  auto operand_cluster =
      toolchain::dyn_cast<mlir::tf_device::ClusterOp>(value.getDefiningOp());
  if (!operand_cluster) {
  ...
  if (!mesh_or_status.ok())
    return operand_cluster.emitOpError(
        toolchain::formatv("Failed during mesh propagation. {0}",
...
mlir::LogicalResult InferMeshFromInputs(
  ...
  toolchain::SmallVector<mlir::OpOperand*, 8> inputs_with_inferred_mesh;
  ...
  mlir::visitUsedValuesDefinedAbove(
      cluster.getBody(), cluster.getBody(), [&](mlir::OpOperand* operand) {
        ...
        if (auto layout_op =
                toolchain::dyn_cast<mlir::TF::DTensorLayout>(operand->getOwner())) {
        ...
        if (mesh->has_value() && extracted_config != mesh->value()) {
          toolchain::SmallVector<std::string, 8> input_debug_strings;
          int index = 0;
          for (const auto& input : inputs_with_inferred_mesh) {
            input_debug_strings.push_back(
                toolchain::formatv("Input Cluster {0}: {1}", index, input->get()));
            ++index;
          }
          result = cluster.emitOpError(
              toolchain::formatv("failed during mesh propagation. All inputs to "
...
StatusOr<std::optional<Mesh>> ExtractMeshFromFuctionOutput(
  ...
  auto terminator = toolchain::cast<mlir::func::ReturnOp>(
...
mlir::LogicalResult InferMeshFromConsumers(
    mlir::tf_device::ClusterOp cluster, std::optional<Mesh>* mesh,
    toolchain::SmallVector<mlir::OpOperand*, 8>* consumers_with_mesh) {
  for (auto& use_value : cluster.getOperation()->getUses()) {
    ...
    if (toolchain::isa<mlir::TF::CopyToMeshOp>(consumer)) continue;
    if (toolchain::isa<mlir::TF::RelayoutOp>(consumer)) continue;
    if (toolchain::isa<mlir::TF::CopyToMeshGradOp>(&cluster.GetBody().front()))
      continue;
    if (toolchain::isa<mlir::TF::RelayoutLikeOp>(&cluster.GetBody().front()))
      continue;
    ...
    if (auto return_op = toolchain::dyn_cast<mlir::func::ReturnOp>(consumer)) {
...
mlir::LogicalResult InferFunctionDefaultMesh(
  ...
  for (auto& result_value : terminator->getOpOperands()) {
    ...
    auto result_cluster =
        toolchain::cast<mlir::tf_device::ClusterOp>(result_defining_op);
    auto result_mesh =
...
struct DTensorMeshPropagation
  ...
  mlir::LogicalResult PropagateMesh(
      const toolchain::DenseMap<mlir::OpOperand*, std::vector<mlir::Value>>&
          producers,
...
DTensorMeshPropagation::PropagateDefaultMeshToUnAssignedClusters(
  ...
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 4> clusters_without_mesh;
...
mlir::LogicalResult DTensorMeshPropagation::PropagateMeshFromInputs(
  ...
  // `tf.CopyToMesh`.
  if (toolchain::isa<mlir::TF::CopyToMeshOp, mlir::TF::RelayoutOp,
...
mlir::LogicalResult DTensorMeshPropagation::PropagateMeshFromConsumers(
  ...
  toolchain::SmallVector<mlir::OpOperand*, 8> consumers_with_mesh_information;
...
mlir::LogicalResult DTensorMeshPropagation::PropagateMesh(
  ...
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 8> cluster_ops;
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 8> while_cluster_ops;
  auto walk_result = function.walk([&](mlir::tf_device::ClusterOp cluster)
                                       -> mlir::WalkResult {
    if (toolchain::isa<mlir::TF::WhileRegionOp>(cluster.GetBody().front())) {
  ...
  // consumers.
  for (auto cluster : toolchain::reverse(cluster_ops)) {
