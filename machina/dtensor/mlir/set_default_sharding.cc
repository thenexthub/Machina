...
void SetDefaultSharding(mlir::tf_device::ClusterFuncOp cluster,
  ...
  toolchain::SmallVector<toolchain::StringRef, 4> input_sharding(cluster.getNumOperands(),
                                                       sharding);
  toolchain::SmallVector<toolchain::StringRef, 4> output_sharding(cluster.getNumResults(),
                                                        sharding);
