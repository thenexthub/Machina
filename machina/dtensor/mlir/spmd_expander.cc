...
absl::Status AdjustPartedLayout(
    const toolchain::DenseMap<int, Layout>& input_layouts,
    toolchain::DenseMap<int, Layout>* computed_layouts) {
...
bool SkipExpansionForPartedLayout(mlir::Operation* op) {
  ...
  if (toolchain::isa<mlir::func::ReturnOp, mlir::tf_device::ReturnOp>(op)) {
    return false;
  }
...
absl::Status SPMDExpanderBase::ExpandOpAndSetLayout(mlir::Operation* op,
  ...
  toolchain::SmallVector<toolchain::SmallVector<int64_t, 4>, 4> global_output_shapes;
  ...
  for (auto output_value : op->getResults()) {
    ...
    if (toolchain::isa<mlir::TF::RestoreV2Op, mlir::TF::DTensorRestoreV2Op>(op) &&
        (!maybe_ranked || !maybe_ranked.hasStaticShape()))
      continue;
    TF_ASSIGN_OR_RETURN(auto global_shape,
                        ExtractGlobalOutputShape(output_value));
    global_output_shapes.emplace_back(toolchain::SmallVector<int64_t, 4>{
  ...
  for (const auto& output_layout_and_index :
       toolchain::enumerate(toolchain::zip((*output)->getResults(), computed_layout))) {
    ...
    for (const auto& expanded_and_true_global_shape :
         toolchain::zip(global_output_shapes[index], expected_global_shape)) {
...
StatusOr<toolchain::DenseMap<int, Layout>> SPMDExpanderBase::ComputeLayoutForward(
...
StatusOr<toolchain::DenseMap<int, Layout>> SPMDExpanderBase::ComputeLayoutForward(
  ...
  if (mesh.IsSingleDevice()) {
    ...
    auto layouts = toolchain::DenseMap<int, Layout>{};
...
StatusOr<toolchain::DenseMap<int, Layout>> SPMDExpanderBase::ComputeLayoutBackward(
