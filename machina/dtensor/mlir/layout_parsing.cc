...
bool OpUsesV2LayoutAnnotation(mlir::Operation* op) {
  return !op->getUsers().empty() &&
         toolchain::all_of(op->getUsers(), [](mlir::Operation* user_op) {
           return toolchain::isa<mlir::TF::DTensorLayout>(user_op);
         });
}
...
StatusOr<std::optional<Layout>> ExtractSingleLayoutFromOp(
  ...
  if (OpUsesV2LayoutAnnotation(op)) {
    ...
    out.emplace(
        toolchain::cast<mlir::TF::DTensorLayout>(*users.begin()).getLayout());
  } else {
...
StatusOr<std::vector<std::optional<Layout>>> ExtractLayoutFromOp(
  ...
  if (OpUsesV2LayoutAnnotation(op)) {
    for (auto op_result : op->getOpResults()) {
      outs.emplace_back(
          toolchain::cast<mlir::TF::DTensorLayout>(*op_result.getUsers().begin())
              .getLayout());
    }
  } else {
...
void SetLayoutOnOp(mlir::Operation* op, mlir::OpBuilder builder,
  ...
  toolchain::SmallVector<std::string, 8> serialized_layouts;
  for (auto const& layout : layouts) {
    serialized_layouts.emplace_back(layout.has_value() ? layout->ToString()
                                                       : "");
  }
  op->setAttr(kLayoutAttr,
              builder.getStrArrayAttr(toolchain::SmallVector<toolchain::StringRef, 8>(
...
StatusOr<std::optional<Layout>> ExtractLayoutFromFunctionReturnAttr(
  ...
  if (!result_layout_or_status.ok())
    return errors::InvalidArgument(
        toolchain::formatv("Malformed default return layout received. {0} Received "
                      "layout : {1}",
...
StatusOr<toolchain::SmallVector<Layout, 4>> ExtractElementLayoutsFromOperand(
  ...
  if (!enclosing_function)
    return errors::InvalidArgument(
        toolchain::formatv("Could not find iterator at {0}-th input to op: {1}",
  ...
  toolchain::SmallVector<Layout, 4> layouts(array_attr.size());
