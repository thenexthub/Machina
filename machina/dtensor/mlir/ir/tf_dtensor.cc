...
mlir::LogicalResult DTensorLayout::verify() {
  ...
  for (const auto& dim_and_index :
       toolchain::enumerate(toolchain::zip(type.getShape(), num_shards))) {
...
mlir::LogicalResult DTensorAllGatherOp::verify() {
  ...
  const machina::dtensor::Layout input_layout = op.getInputLayout();
  const machina::dtensor::Layout output_layout = op.getOutputLayout();
  ...
  for (int32_t i = 0; i < input_layout.rank(); ++i) {
    if (input_layout.sharding_spec(i) != output_layout.sharding_spec(i) &&
        machina::dtensor::Layout::IsShardedDimension(
...
mlir::LogicalResult DTensorAllToAllOp::verify() {
  ...
  for (int32_t i = 0; i < input_layout.rank(); ++i) {
    if (input_layout.sharding_spec(i) == output_layout.sharding_spec(i))
      continue;
    if (machina::dtensor::Layout::IsUnshardedDimension(
            input_layout.sharding_spec(i)) &&
        machina::dtensor::Layout::IsShardedDimension(
            output_layout.sharding_spec(i))) {
      num_split_dims++;
      split_spec = output_layout.sharding_spec(i);
    } else if (machina::dtensor::Layout::IsShardedDimension(
                   input_layout.sharding_spec(i)) &&
               machina::dtensor::Layout::IsUnshardedDimension(
                   output_layout.sharding_spec(i))) {
      num_concat_dims++;
...
#define GET_OP_LIST
#include "machina/dtensor/mlir/ir/tf_dtensor.cc.inc"
      >();
}
