...
mlir::Operation* GetConstOp(mlir::Operation* op) {
  if (toolchain::isa<mlir::TF::ConstOp>(op)) return op;

  if (auto layout = toolchain::dyn_cast<mlir::TF::DTensorLayout>(op)) {
    mlir::Operation* input_op = layout.getInput().getDefiningOp();
    if (input_op && toolchain::isa<mlir::TF::ConstOp>(input_op)) return input_op;
  }
  return nullptr;
}
...
mlir::LogicalResult CloneOpToCluster(mlir::Operation* const_op,
  ...
  auto copy_to_mesh =
      toolchain::dyn_cast<mlir::TF::CopyToMeshOp>(operand->getOwner());
  assert(copy_to_mesh);
  ...
  if (!mesh.ok())
    return copy_to_mesh.emitOpError(
        toolchain::formatv(kInvalidMeshMsg, mesh_attr, mesh.status().message()));
...
mlir::LogicalResult GetInputProducingValue(mlir::OpOperand& operand,
  ...
  auto input_cluster =
      toolchain::dyn_cast<mlir::tf_device::ClusterOp>(input_value.getOwner());
  if (input_cluster) {
...
mlir::LogicalResult CloneConstantsAcrossMesh(
  ...
  mlir::visitUsedValuesDefinedAbove(
      body_region, body_region, [&](mlir::OpOperand* operand) {
        ...
        if (!toolchain::isa<mlir::TF::CopyToMeshOp>(operand->getOwner())) {
          result =
...
mlir::LogicalResult LowerToSendRecv(mlir::TF::CopyToMeshOp copy_to_mesh,
  ...
  auto src_cluster =
      toolchain::cast<mlir::tf_device::ClusterOp>(copied_value.getDefiningOp());
  ...
  const std::string op_key =
      toolchain::formatv("communication_key_{0}_{1}", copy_to_mesh.getMesh(),
                    *send_recv_counter)
          .str();
  const std::string mesh_attr = copy_to_mesh.getMesh().str();
  auto mesh_or_status = Mesh::FromString(mesh_attr);
  if (!mesh_or_status.ok())
    return copy_to_mesh.emitOpError(toolchain::formatv(
  ...
  mlir::visitUsedValuesDefinedAbove(
      cluster_region, cluster_region, [&](mlir::OpOperand* operand) {
        ...
        mlir::TF::CopyToMeshOp copy_to_mesh =
            toolchain::dyn_cast<mlir::TF::CopyToMeshOp>(input_op);
        if (!copy_to_mesh) {
          result =
              operand->getOwner()->emitOpError(kInvalidTensorTransferErrorMsg);
          return;
...
mlir::LogicalResult InsertCopyToMesh(mlir::tf_device::ClusterOp cluster) {
  ...
  toolchain::SmallVector<mlir::Operation*, 4> relayout_ops;
