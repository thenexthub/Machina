...
bool DependsOn(mlir::Operation* successor, mlir::Operation* predecessor,
  ...
  toolchain::SmallVector<mlir::Operation*, 4> to_visit;
  toolchain::SmallPtrSet<mlir::Operation*, 4> visited;
...
mlir::LogicalResult MergeAllReduceGroup(
  ...
  std::vector<toolchain::ArrayRef<int64_t>> all_reduce_shapes;
...
createSubgroupsByElemType(
  ...
  for (const auto& all_reduce_group : all_reduce_groups) {
    toolchain::DenseMap<mlir::Type, std::vector<mlir::TF::DTensorAllReduceOp>>
...
createSubgroupsByReductionAttr(
  ...
  for (const auto& all_reduce_group : all_reduce_groups) {
    toolchain::DenseMap<toolchain::StringRef, std::vector<mlir::TF::DTensorAllReduceOp>>
        all_reduces_by_attr_reduce_op;
    for (mlir::TF::DTensorAllReduceOp all_reduce : all_reduce_group) {
      toolchain::StringRef attr_reduce_op = all_reduce.getReduceOp();
...
createSubgroupsByGroupAssignment(
  ...
  for (const auto& all_reduce_group : all_reduce_groups) {
    ...
    toolchain::DenseMap<mlir::Value, std::vector<mlir::TF::DTensorAllReduceOp>>
...
createSubgroupsByTopoDist(
    std::vector<std::vector<mlir::TF::DTensorAllReduceOp>> all_reduce_groups,
    toolchain::DenseMap<mlir::TF::DTensorAllReduceOp, int> all_reduce_topo,
    int topo_dist) {
...
toolchain::DenseMap<mlir::TF::DTensorAllReduceOp, int> computeAllReduceTopoLevel(
    mlir::tf_device::ClusterOp cluster) {
  toolchain::DenseMap<mlir::Operation*, int> op_topo_level;
  toolchain::DenseMap<mlir::TF::DTensorAllReduceOp, int> all_reduce_topo;
  ...
  cluster.getBody().walk([&](mlir::Operation* op) {
    ...
    mlir::TF::DTensorAllReduceOp all_reduce =
        toolchain::dyn_cast<mlir::TF::DTensorAllReduceOp>(op);
...
struct DTensorAllReduceCombineOptimization
  ...
  void runOnOperation() override {
    ...
    function.walk([&](mlir::tf_device::ClusterOp cluster) {
      ...
      toolchain::DenseSet<mlir::Block*> blocks;
