...
StatusOr<mlir::Value> EmitRelayout(
  ...
  toolchain::SmallPtrSet<mlir::Operation*, 4> local_newly_created_ops;
...
StatusOr<mlir::Operation*> EmitAllReduce(
  ...
  input->getOpResult(0).replaceAllUsesExcept(
      ...
      toolchain::SmallPtrSet<mlir::Operation*, 1>{all_reduce});
...
StatusOr<mlir::Value> CreateConstSrcTargetPair(const Mesh& mesh,
  ...
  toolchain::SmallVector<int, 4> src_target_pair_flat;
  ...
  for (const int local_device_id : mesh.local_device_ids()) {
    ...
    toolchain::SmallVector<int, 4> mesh_coordinate_for_device_id;
    ...
    for (const auto& data : toolchain::enumerate(mesh_dimensions)) {
...
StatusOr<mlir::Value> EmitHaloExchange(mlir::OpBuilder& builder, int halo_size,
  ...
  toolchain::ArrayRef<int64_t> input_tensor_shape = input_tensor_type.getShape();
  const std::vector<std::string> sharding_specs = layout.sharding_spec_strs();
  const int split_dim_index = std::distance(
      sharding_specs.begin(), toolchain::find(sharding_specs, mesh_dim));
  ...
  toolchain::SmallVector<int64_t, 4> halo_exchange_tensor_shape;
  for (const auto& size_and_index : toolchain::enumerate(input_tensor_shape)) {
  ...
  if (halo_type.getElementType().isIntOrIndex()) {
    const_attr =
        mlir::DenseIntElementsAttr::get(halo_type, toolchain::SmallVector<int>{0});
  } else {
    const_attr =
        mlir::DenseFPElementsAttr::get(halo_type, toolchain::SmallVector<float>{0});
  }
  ...
  toolchain::SmallVector<int64_t, 4> begin_left(layout.rank(), 0);
  ...
  toolchain::SmallVector<int64_t, 4> size(input_tensor_shape.begin(),
  ...
  toolchain::SmallVector<int64_t, 4> begin_right(layout.rank(), 0);
  ...
  toolchain::SmallVector<int64_t, 4> final_shape(input_tensor_shape.begin(),
                                            input_tensor_shape.end());
  ...
  mlir::Value final_value = builder.create<mlir::TF::ConcatV2Op>(
      location, final_type,
      toolchain::SmallVector<mlir::Value, 4>{left_concat_value, tensor,
                                        right_concat_value},
      concat_axis);
