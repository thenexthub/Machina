...
struct FunctionToChangeInfo {
  ...
  toolchain::SmallVector<mlir::Operation*, 4> callsite_ops;
};

// Finds all functions in graph that is not a public functions and retrieves
// their callsite operations.
toolchain::SmallVector<FunctionToChangeInfo, 4> FindFunctionsToRewrite(
    mlir::ModuleOp module) {
  toolchain::SmallVector<FunctionToChangeInfo, 4> functions_to_change;
  module.walk([&](mlir::Operation* op) {
    if (!toolchain::isa<mlir::TF::StatefulPartitionedCallOp,
    ...
    if (auto call_op =
            toolchain::dyn_cast<mlir::TF::StatefulPartitionedCallOp>(op)) {
      symbol = call_op.getF();
    } else {
      auto symbol_ref = toolchain::dyn_cast<mlir::TF::PartitionedCallOp>(op).getF();
    ...
    toolchain::SmallVector<mlir::Operation*, 4> function_use_ops;
...
void PrependArgumentToFunction(mlir::func::FuncOp function, mlir::Type type,
  ...
  auto new_argument_types =
      toolchain::to_vector<4>(function_body.getArgumentTypes());
  function.setType(
...
mlir::LogicalResult PrependDeviceIdToCallsites(mlir::OpBuilder* builder,
  ...
  if (auto stateful_partitioned_call =
    ...
    auto partitioned_call = toolchain::cast<mlir::TF::PartitionedCallOp>(op);
  ...
  for (auto results : toolchain::zip(op->getResults(), new_call->getResults()))
