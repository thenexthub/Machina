...
#include "machina/dtensor/mlir/collectives_common.h"
...
bool UseNcclCommunicationOnGpu() {
  ...
  // machina/dtensor/python/config.py.
...
toolchain::SmallVector<int32_t, 4> GetGroupKeyOffsets(
  ...
  *group_size = shape[1];
  ...
  toolchain::SmallVector<int32, 4> device_id_to_group_key(num_devices);
  ...
  for (const auto& it :
       toolchain::enumerate(group_assignment.getValues<toolchain::APInt>())) {
...
int32_t GetCollectiveKeyBase(
  ...
  static machina::mutex* mtx = new machina::mutex();
  static auto* mesh_to_key_base =
      new std::map<std::tuple<std::string, toolchain::SmallVector<int32, 4>>,
                   int32_t>();
  int32_t group_size;
  const toolchain::SmallVector<int32, 4> group_key_offsets =
      GetGroupKeyOffsets(group_assignment, &group_size);

  const auto iter =
      mesh_to_key_base->find({mesh.ToString(), group_key_offsets});
  machina::mutex_lock lock(*mtx);
...
void CreateGroupAndInstanceKey(
  ...
  int32_t group_size;
  toolchain::SmallVector<int32, 4> device_id_to_group_key =
      GetGroupKeyOffsets(group_assignment, &group_size);
  // 21 bits + 11 bits allow roughly 2M all-reduces in one program and up to a
  // full DF pod.
...
mlir::Value SelectElementsBasedOnId(
    ...
    const toolchain::SmallVectorImpl<int64>& candidates_flat, int64 num_devices,
...
mlir::LogicalResult LowerAllGatherOp(mlir::TF::DTensorAllGatherOp all_gather) {
  ...
  toolchain::SmallVector<int64, 4> concat_dims;
  ...
  toolchain::SmallVector<int64, 4> device_id_to_begin_flat;
  toolchain::SmallVector<int64, 4> device_id_to_end_flat;
  toolchain::SmallVector<int64, 4> device_id_to_strides_flat;
  for (int64 device_id = 0; device_id < num_devices; ++device_id) {
    for (int64 i = 0; i < output_shape_size; ++i) {
      if (toolchain::find(concat_dims, i) == std::end(concat_dims)) {
...
mlir::LogicalResult LowerAllScatterOp(
  ...
  toolchain::ArrayRef<int64_t> output_shape = output_type.getShape();
  ...
  toolchain::SmallVector<int32_t> output_shape_int32(output_shape.begin(),
                                                output_shape.end());
...
struct DTensorAllReduceLowering
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::TF::DTensorAllReduceOp, 4> all_reduces;
...
struct DTensorReduceScatterLowering
  ...
  void runOnOperation() override {
    ...
    // Find all DTensorAllReduce ops.
    toolchain::SmallVector<mlir::TF::DTensorReduceScatterOp, 4> all_reduces;
...
struct DTensorAllGatherLowering
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::TF::DTensorAllGatherOp, 4> all_gathers;
...
struct DTensorAllScatterLowering
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::TF::DTensorAllScatterOp, 4> all_scatters;
...
struct DTensorAllToAllLowering
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::TF::DTensorAllToAllOp, 4> all_to_alls;
