...
absl::Status ExtractEquationRelations(
  ...
  for (const auto& char_and_index : toolchain::enumerate(parts.second)) {
  ...
  for (int i = 0; i < inputs.size(); ++i) {
    for (const auto& char_and_index : toolchain::enumerate(inputs[i])) {
...
StatusOr<toolchain::DenseMap<int, Layout>> EinsumSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  if (input_layouts.empty()) return toolchain::DenseMap<int, Layout>();
...
StatusOr<toolchain::DenseMap<int, Layout>> EinsumSPMDExpander::ComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> input_layouts(num_inputs);
