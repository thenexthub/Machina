...
StatusOr<toolchain::SmallVector<int64_t, 4>> GetShape(mlir::Value value) {
  TF_ASSIGN_OR_RETURN(const auto shape, GetShapeOfValue(value));
  return toolchain::SmallVector<int64_t, 4>{shape.begin(), shape.end()};
}
...
ElementwiseSPMDExpander::ComputeLayoutBackward(
  ...
  for (const auto& operand_and_index : toolchain::enumerate(op->getOperands())) {
    ...
    for (const auto& dim_shape_and_index : toolchain::enumerate(operand_shape)) {
