...
StatusOr<mlir::Operation*> TensorScatterOpExpand(mlir::Operation* op) {
  auto scatter_op = toolchain::cast<OpType>(op);
  ...
  // operations.
  TF_ASSIGN_OR_RETURN(
      toolchain::ArrayRef<int64_t> tensor_shape,
      GetGlobalShapeOfValueFromDTensorLayout(scatter_op.getTensor()));
  TF_ASSIGN_OR_RETURN(
      toolchain::ArrayRef<int64_t> indices_shape,
      GetGlobalShapeOfValueFromDTensorLayout(scatter_op.getIndices()));
  TF_ASSIGN_OR_RETURN(
      toolchain::ArrayRef<int64_t> updates_shape,
...
template <typename OpType>
StatusOr<toolchain::DenseMap<int, Layout>> TensorScatterOpComputeLayoutForward(
...
template <typename OpType>
StatusOr<toolchain::DenseMap<int, Layout>> TensorScatterOpComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> input_layouts(scatter_op.getNumOperands());
...
StatusOr<mlir::Operation*> TensorScatterOpSPMDExpander::ExpandOp(
    mlir::Operation* op) {
  if (toolchain::isa<mlir::TF::TensorScatterUpdateOp>(op)) {
    return TensorScatterOpExpand<mlir::TF::TensorScatterUpdateOp>(op);
  }
  if (toolchain::isa<mlir::TF::TensorScatterAddOp>(op)) {
...
StatusOr<mlir::Operation*> ScatterNdOpSPMDExpander::ExpandOp(
  ...
  TF_ASSIGN_OR_RETURN(Mesh mesh, ExtractDeviceMeshEnclosingCluster(op));
  auto scatter_op = toolchain::cast<mlir::TF::ScatterNdOp>(op);
