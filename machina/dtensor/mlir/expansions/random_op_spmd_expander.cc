...
absl::Status ValidateShapeAndGetNewShape(
    const toolchain::SmallVector<int64_t, 4>& op_shape, const Layout& layout,
    toolchain::SmallVectorImpl<int64_t>& new_random_shape) {
...
StatusOr<mlir::Value> GetDeviceSeed(const Layout& layout, mlir::Operation* op) {
  ...
  toolchain::SmallVector<int32_t, 4> layout_dims;
  toolchain::SmallSet<int32_t, 4> layout_dims_set;
  ...
  toolchain::SmallVector<uint32_t, 4> multipliers(layout.mesh().rank(), 0);
...
StatusOr<toolchain::SmallVector<int64_t, 4>> GetNewLocalShape(mlir::Operation* op,
                                                         const Layout& layout) {
  auto random_op = toolchain::cast<RandomOp>(op);
  toolchain::SmallVector<int64_t, 4> op_shape;
  TF_RETURN_IF_ERROR(
      ExtractConstVectorFromValue(random_op.getShape(), &op_shape));

  // Validate that sharding of random op is compatible with it's user defined
  // shape and calculate new shape of local random op.
  toolchain::SmallVector<int64_t, 4> new_random_shape;
  TF_RETURN_IF_ERROR(
