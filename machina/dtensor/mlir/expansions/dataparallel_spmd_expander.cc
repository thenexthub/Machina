...
bool AllBatchParallel(const std::vector<Layout>& layouts,
                      const toolchain::DenseMap<int, int>& batchable_indices) {
...
bool AnyLayoutExist(const toolchain::DenseMap<int, Layout>& layouts,
                    const toolchain::DenseMap<int, int>& indices) {
...
StatusOr<Layout> MergeBatchLayouts(
    const toolchain::DenseMap<int, Layout>& layouts,
    const toolchain::DenseMap<int, int>& batchable_args, const Mesh& mesh) {
  // Get the batch rank
  int layout_idx = -1;
...
StatusOr<Layout> IntermediateBatchLayout(
    ...
    const toolchain::DenseMap<int, int>& batchable_operands,
    const std::vector<Layout>& output_layouts,
    const toolchain::DenseMap<int, int>& batchable_outputs, const Mesh& mesh) {
  if (batchable_operands.empty()) {
    return errors::Unimplemented(
        toolchain::formatv("There must be at least one batchable operand").str());
  }
...
StatusOr<mlir::Operation*> DataparallelSPMDExpander::ExpandOp(
  ...
  if (!AllBatchParallel(operand_layouts, batchable_operands_) ||
    ...
    return errors::Unimplemented(
        toolchain::formatv("All operands and outputs must be batch parallel.")
            .str());
  }
  ...
  if (!SameBatchRank(operand_layouts, batchable_operands_) ||
    ...
    return errors::Unimplemented(
        toolchain::formatv("All operands and outputs with batch dimensions must "
...
DataparallelSPMDExpander::ComputeLayoutForward(
  ...
  if (AnyLayoutExist(input_layouts, batchable_operands_)) {
    ...
    for (const auto& output_and_index : toolchain::enumerate(op->getOpResults())) {
