...
absl::Status VerifyConcatLayout(mlir::Value concat_dim_operand,
  ...
  for (const auto& shard_and_dimension :
       toolchain::enumerate(concat_layout.num_shards())) {
...
StatusOr<mlir::Operation*> ConcatSPMDExpander::ExpandOp(mlir::Operation* op) {
  if (!toolchain::isa<mlir::TF::ConcatOp, mlir::TF::ConcatV2Op>(op))
    return errors::InvalidArgument(
  ...
  // different position.
  bool is_concat_v1 = toolchain::isa<mlir::TF::ConcatOp>(op);
...
StatusOr<toolchain::DenseMap<int, Layout>> ConcatSPMDExpander::ComputeLayoutForward(
  ...
  // operand layouts.
  const bool is_concat_v1 = toolchain::isa<mlir::TF::ConcatOp>(op);
  auto begin_idx = is_concat_v1 ? 1 : 0;
  auto end_idx =
      is_concat_v1 ? op->getNumOperands() - 1 : op->getNumOperands() - 2;

  toolchain::DenseMap<int, Layout> concat_operands_layouts;
  ...
  if (concat_operand_layout) {
    ...
    return toolchain::DenseMap<int, Layout>({{0, reduced_concat_layout}});
  } else {
    return toolchain::DenseMap<int, Layout>();
  }
}

StatusOr<toolchain::DenseMap<int, Layout>> ConcatSPMDExpander::ComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> operand_layouts;
