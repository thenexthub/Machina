...
#include "machina/dtensor/mlir/expansions/save_restore_spmd_expander.h"
...
#include "machina/dtensor/cc/save_restore_util.h"
  ...
  mlir::Value new_prefix =
      mlir::TF::AddOp::create(
          ...
          StringConst(builder, prefix.getLoc(),
                      toolchain::SmallVector<toolchain::StringRef>(
  ...
  for (int64_t device_id = 1; device_id < mesh.num_devices(); ++device_id) {
    ...
    new_prefix =
        mlir::TF::ConcatOp::create(builder, prefix.getLoc(),
                                   ...
                                   /*concat_dim=*/
                                   zero_const,
                                   toolchain::SmallVector<mlir::Value, 4>{
...
mlir::Value DeviceIdToLocalBranchIndex(
    ...
    const toolchain::ArrayRef<int64_t>& local_device_ids, mlir::Value device_id,
    mlir::OpBuilder& builder) {
  mlir::Value local_device_id_tensors =
      IntConst(builder, location,
               toolchain::SmallVector<int32_t>(local_device_ids.begin(),
...
StatusOr<mlir::TF::CaseOp> ConditionalSave(
  ...
  toolchain::SmallVector<mlir::func::FuncOp, 8> branch_funs;
  ...
  // empty.
  toolchain::SmallVector<std::string, 4> original_shape_and_slices;
  ...
  for (int device_id : mesh.local_device_ids()) {
    ...
    if (it == saving_specs.end()) {
      ...
      mlir::func::FuncOp no_op = mlir::func::FuncOp::create(
          location,
          toolchain::formatv("{0}_no_op_on_device_{1}_{2}", OpName(original_save),
      ...
      mlir::func::FuncOp new_save = mlir::func::FuncOp::create(
          location,
          toolchain::formatv("{0}_save_op_on_device_{1}_{2}", OpName(original_save),
                        device_id, OpHash(original_save))
              .str(),
          func_type, toolchain::ArrayRef<mlir::NamedAttribute>{});
      // Set function visibility to private to indicate that it is only used in
      ...
      TF_ASSIGN_OR_RETURN(toolchain::ArrayRef<int64_t> tensor_names_shape,
                          GetGlobalShapeOfValueFromDTensorLayout(
                              original_save.getTensorNames()));
      if (tensor_names_shape.size() != 1)
        return absl::InternalError(
            toolchain::formatv("SaveV2 op got `tensor_names` with rank {0}) but "
      ...
      for (int save_op_index = 0; save_op_index < new_tensor_indices.size();
           ++save_op_index) {
        toolchain::SmallVector<mlir::Value, 4> new_tensor_names;
        toolchain::SmallVector<std::string, 4> new_shape_and_slices;
        toolchain::SmallVector<mlir::Value, 4> new_tensors;
        ...
        mlir::TF::SaveV2Op::create(
            ...
            StringConst(
                fn_builder, location,
                toolchain::SmallVector<toolchain::StringRef>(new_shape_and_slices.begin(),
  ...
  toolchain::SmallVector<mlir::Attribute, 4> symbols;
  ...
  toolchain::SmallVector<int64_t> local_device_ids(mesh.local_device_ids().begin(),
  ...
  auto case_op =
      mlir::TF::CaseOp::create(builder, location,
                               ...
                               /*output=*/toolchain::ArrayRef<mlir::Type>{},
...
StatusOr<mlir::Operation*> ExpandSaveV2Op(mlir::Operation* op) {
  if (!toolchain::isa<mlir::TF::SaveV2Op>(op)) {
    return absl::InvalidArgumentError(
        toolchain::formatv("Expecting SaveV2Op but got {0}", OpName(op)).str());
  }
  ...
  for (const auto& it : toolchain::enumerate(save_v2.getTensors())) {
    ...
    TF_ASSIGN_OR_RETURN(toolchain::ArrayRef<int64_t> tensor_shape,
...
StatusOr<mlir::Operation*> ExpandMergeV2Op(mlir::Operation* op) {
  ...
  if (!merge_v2) {
    return absl::InvalidArgumentError(
        toolchain::formatv("Expecting MergeV2CheckpointsOp but got {0}", OpName(op))
            .str());
  }
  ...
  toolchain::SmallVector<mlir::Type> input_types(merge_v2.getOperandTypes());
  ...
  mlir::func::FuncOp then_func = mlir::func::FuncOp::create(
      location,
      toolchain::formatv("{0}_then_func_{1}", OpName(merge_v2), OpHash(merge_v2))
          .str(),
      func_type, toolchain::ArrayRef<mlir::NamedAttribute>{});
  // Set function visibility to private to indicate that it is only used in
  // this module.
  ...
  mlir::func::FuncOp else_func = mlir::func::FuncOp::create(
      location,
      toolchain::formatv("{0}_else_func_{1}", OpName(merge_v2), OpHash(merge_v2))
          .str(),
      func_type, toolchain::ArrayRef<mlir::NamedAttribute>{});
  // Set function visibility to private to indicate that it is only used in
  // this module.
  ...
  auto new_operands = toolchain::to_vector<4>(merge_v2.getOperands());
...
StatusOr<mlir::Operation*> ExpandRestoreV2OpHelper(
  ...
  toolchain::SmallVector<mlir::func::FuncOp> branch_funcs(
  ...
  for (int local_device_idx = 0;
    ...
    toolchain::SmallVector<std::string> new_shapes_and_slices(op->getNumResults());

    // For each tensor, build its restore shape_and_slice.
    for (const auto& it : toolchain::enumerate(toolchain::zip(global_shapes, layouts))) {
      ...
      new_shapes_and_slices[it.index()] =
          toolchain::formatv("{0} {1}", absl::StrJoin(global_shape, " "),
    ...
    shapes_and_slices_mutable.assign(StringConst(
        builder, op->getLoc(),
        toolchain::SmallVector<toolchain::StringRef>(new_shapes_and_slices.begin(),
                                           new_shapes_and_slices.end())));
    mlir::func::FuncOp device_restore_fn = mlir::func::FuncOp::create(
        location,
        toolchain::formatv("{0}_on_device_{1}_{2}", OpName(op), device_id,
                      OpHash(op))
            .str(),
        func_type, toolchain::ArrayRef<mlir::NamedAttribute>{});
    // Set function visibility to private to indicate that it is only used in
...
StatusOr<mlir::Operation*> ExpandDTensorRestoreV2Op(mlir::Operation* op) {
  ...
  if (!restore_v2) {
    return absl::InvalidArgumentError(
        toolchain::formatv("Expecting DTensorRestoreV2Op but got {0}", OpName(op))
            .str());
  }
  ...
  for (const auto& shape : input_shapes_attr) {
    ...
    if (!shape_attr.hasStaticShape()) {
      return absl::InvalidArgumentError(
          toolchain::formatv("DTensorRestoreV2Op requires statically known input "
...
StatusOr<mlir::Operation*> ExpandRestoreV2Op(mlir::Operation* op) {
  ...
  for (const auto& it :
       toolchain::zip(op->getResultTypes(), global_shapes, layouts)) {
...
StatusOr<mlir::Operation*> SaveRestoreSPMDExpander::ExpandOp(
  ...
  if (toolchain::isa<mlir::TF::MergeV2CheckpointsOp>(op)) {
    return ExpandMergeV2Op(op);
  }
  if (toolchain::isa<mlir::TF::DTensorRestoreV2Op>(op)) {
    return ExpandDTensorRestoreV2Op(op);
  }
  if (toolchain::isa<mlir::TF::RestoreV2Op>(op)) {
...
StatusOr<toolchain::SmallVector<Layout>> GetLayoutsFromAssignVariableOps(
    mlir::ModuleOp module, mlir::TF::RestoreV2Op* restore_op) {
  toolchain::SmallVector<Layout> layouts(restore_op->getNumResults());
  ...
  for (auto result : restore_op->getResults()) {
    ...
    for (auto consuming_op : result.getUsers()) {
      ...
      // these ops first.
      while (toolchain::isa<mlir::TF::CastOp, mlir::TF::IdentityOp,
      ...
      if (auto assign_op = toolchain::dyn_cast_or_null<mlir::TF::AssignVariableOp>(
              consuming_op)) {
...
SaveRestoreSPMDExpander::ComputeLayoutForward(
  ...
  if (toolchain::isa<mlir::TF::SaveV2Op, mlir::TF::MergeV2CheckpointsOp>(op)) {
  ...
  if (toolchain::isa<mlir::TF::RestoreV2Op>(op)) {
    ...
    if (layouts.size() != restore_v2.getNumResults()) {
      return absl::InternalError(
          toolchain::formatv("Failed to get {0} output layouts "
                        "for RestoreV2Op. Got {1} layouts.",
                        restore_v2.getNumResults(), layouts.size())
              .str());
    }
    toolchain::DenseMap<int, Layout> output_layouts(restore_v2.getNumResults());
  ...
  if (toolchain::isa<mlir::TF::DTensorRestoreV2Op>(op)) {
    ...
    for (const auto& it : toolchain::enumerate(restore_v2.getInputLayouts())) {
      TF_ASSIGN_OR_RETURN(
          Layout layout,
          Layout::FromString(
  ...
  return absl::UnimplementedError(
      toolchain::formatv("Layout propagation for op : {0} is not implemented",
                    OpName(op))
          .str());
}
