...
#include "machina/dtensor/cc/slice_util.h"
...
StatusOr<std::vector<slice_util::Token>> TokenizeOp(T strided_slice) {
  ...
  toolchain::SmallVector<int64_t, 4> spec_begin;
  toolchain::SmallVector<int64_t, 4> spec_end;
  toolchain::SmallVector<int64_t, 4> spec_strides;

  TF_ASSIGN_OR_RETURN(toolchain::ArrayRef<int64_t> strides_shape,
...
absl::Status UpdateOpFromTokens(T strided_slice,
  ...
  toolchain::SmallVector<int64_t, 4> end;
...
StatusOr<mlir::Operation*> StridedSliceSPMDExpander::ExpandOp(
  ...
  TF_ASSIGN_OR_RETURN(
      const toolchain::ArrayRef<int64_t> global_input_shape,
...
StridedSliceSPMDExpander::ComputeLayoutForward(
  ...
  TF_ASSIGN_OR_RETURN(const toolchain::ArrayRef<int64_t> global_input_shape,
  ...
  return toolchain::DenseMap<int, Layout>({{0, forward.expander_value_layout()}});
}
...
StridedSliceSPMDExpander::ComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> input_layouts(strided_slice_op.getNumOperands());
...
StatusOr<mlir::Operation*> TensorStridedSliceUpdateSPMDExpander::ExpandOp(
  ...
  mlir::TF::TensorStridedSliceUpdateOp strided_slice_op =
      toolchain::cast<mlir::TF::TensorStridedSliceUpdateOp>(op);
  TF_ASSIGN_OR_RETURN(
...
StatusOr<mlir::Operation*> StridedSliceGradSPMDExpander::ExpandOp(
    mlir::Operation* op) {
  auto strided_slice_grad_op = toolchain::cast<mlir::TF::StridedSliceGradOp>(op);
  TF_ASSIGN_OR_RETURN(
      const Layout input_layout,
  ...
  TF_ASSIGN_OR_RETURN(const toolchain::ArrayRef<int64_t> global_output_shape,
...
StridedSliceGradSPMDExpander::ComputeLayoutForward(
  ...
  return toolchain::DenseMap<int, Layout>({{0, backward.expander_input_layout()}});
}
