...
#include "machina/dtensor/mlir/expansions/meta_spmd_expander.h"
...
StatusOr<toolchain::DenseMap<int, Layout>> LayoutsFromPackedTensor(
  ...
  toolchain::DenseMap<int, Layout> layouts(num_unpacked_tensors);
...
StatusOr<toolchain::DenseMap<int, Layout>> LayoutFromUnpackedTensors(
    int axis, const toolchain::DenseMap<int, Layout>& unpacked_layouts) {
  if (unpacked_layouts.empty()) return toolchain::DenseMap<int, Layout>();
  ...
  return toolchain::DenseMap<int, Layout>({{0, inferred_packed_layout}});
}

}  // namespace

StatusOr<mlir::Operation*> PackSPMDExpander::ExpandOp(mlir::Operation* op) {
  auto pack = toolchain::cast<mlir::TF::PackOp>(op);
...
StatusOr<toolchain::DenseMap<int, Layout>> PackSPMDExpander::ComputeLayoutForward(
...
StatusOr<toolchain::DenseMap<int, Layout>> PackSPMDExpander::ComputeLayoutBackward(
...
StatusOr<mlir::Operation*> UnpackSPMDExpander::ExpandOp(mlir::Operation* op) {
  auto unpack = toolchain::cast<mlir::TF::UnpackOp>(op);
...
StatusOr<toolchain::DenseMap<int, Layout>> UnpackSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
...
StatusOr<toolchain::DenseMap<int, Layout>> UnpackSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  auto unpack = toolchain::cast<mlir::TF::UnpackOp>(op);
  const int axis = unpack.getAxis();
...
StatusOr<toolchain::DenseMap<int, Layout>> PadSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  if (input_layouts.find(0) == input_layouts.end())
    return toolchain::DenseMap<int, Layout>();
  ...
  if (auto pad_v2 = toolchain::dyn_cast<mlir::TF::PadV2Op>(op)) {
    pad_output = pad_v2.getOutput();
    pad_input = pad_v2.getInput();
  } else {
    auto pad_op = toolchain::cast<mlir::TF::PadOp>(op);
...
StatusOr<toolchain::DenseMap<int, Layout>> PadSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
...
absl::Status VerifyTileOperandLayout(const Layout& operand_layout,
                                     toolchain::ArrayRef<int64_t> static_multiples) {
  for (const auto& tensor_dim_and_multiple :
       toolchain::enumerate(static_multiples)) {
...
StatusOr<mlir::Operation*> TileSPMDExpander::ExpandOp(mlir::Operation* op) {
  auto tile_op = toolchain::cast<mlir::TF::TileOp>(op);
  ...
  toolchain::SmallVector<int64_t, 4> static_multiples;
  auto status =
  ...
  toolchain::SmallVector<int, 4> local_tile_multiples;
...
StatusOr<toolchain::DenseMap<int, Layout>> TileSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  if (input_layouts.find(0) == input_layouts.end())
    return toolchain::DenseMap<int, Layout>();
  ...
  toolchain::SmallVector<int64_t, 4> static_multiple;
  auto status =
  ...
  for (const auto& multiple_and_dim_sharding :
       toolchain::zip(static_multiple, input_layout.sharding_spec_strs())) {
...
StatusOr<toolchain::DenseMap<int, Layout>> TileSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  TF_ASSIGN_OR_RETURN(auto mesh, ExtractDeviceMeshEnclosingCluster(op));
  auto tile_op = toolchain::cast<mlir::TF::TileOp>(op);
  ...
  if (output_layouts.find(0) != output_layouts.end()) {
    ...
    for (const auto& multiple_and_dim_sharding :
         toolchain::zip(static_multiple, output_layout.sharding_spec_strs())) {
      const int multiple = std::get<0>(multiple_and_dim_sharding);
      const auto& tensor_dimension = std::get<1>(multiple_and_dim_sharding);
...
void ComputeReshapeSegments(
    toolchain::ArrayRef<int64_t> input_shape, toolchain::ArrayRef<int64_t> output_shape,
    toolchain::SmallVectorImpl<int64_t>& input_segment_start,
    toolchain::SmallVectorImpl<int64_t>& input_segment_end,
    toolchain::SmallVectorImpl<int64_t>& output_segment_start,
    toolchain::SmallVectorImpl<int64_t>& output_segment_end) {
  int input_offset = 0;
  int output_offset = 0;
...
StatusOr<Layout> MakeLayoutForReshape(
    const Layout& input_layout, const toolchain::ArrayRef<int64_t> output_shape,
    toolchain::SmallVectorImpl<int64_t>& input_segment_start,
    toolchain::SmallVectorImpl<int64_t>& output_segment_start) {
...
StatusOr<mlir::Operation*> ReshapeSPMDExpander::ExpandOp(mlir::Operation* op) {
  ...
  toolchain::SmallVector<int64_t, 4> input_segment_start;
  toolchain::SmallVector<int64_t, 4> input_segment_end;
  toolchain::SmallVector<int64_t, 4> output_segment_start;
  toolchain::SmallVector<int64_t, 4> output_segment_end;

  toolchain::SmallVector<int64_t, 4> local_reshape_const;
...
StatusOr<toolchain::DenseMap<int, Layout>> ReshapeSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  if (input_layouts.find(0) == input_layouts.end())
    return toolchain::DenseMap<int, Layout>();
...
StatusOr<mlir::Operation*> TransposeSPMDExpander::ExpandOp(
  ...
  // cost as low as possible. Before that, add a check with good error message.
  {
    ...
    toolchain::SmallVector<int64, 4> perm;
    TF_RETURN_IF_ERROR(ExtractConstVectorFromValue(transpose.getPerm(), &perm));

    for (const auto& p : toolchain::enumerate(perm)) {
...
TransposeSPMDExpander::ComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> input_layouts(transpose->getNumOperands());
  input_layouts[1] = Layout::ReplicatedOnMesh(mesh, /*rank=*/1);

  if (output_layouts.find(0) != output_layouts.end()) {
    const Layout output_layout = output_layouts.lookup(0);

    toolchain::SmallVector<int64, 4> inverse_perm(perm.size());
    for (const auto& p : toolchain::enumerate(perm)) {
...
StatusOr<mlir::Operation*> OneHotSPMDExpander::ExpandOp(mlir::Operation* op) {
  auto one_hot_op = toolchain::cast<mlir::TF::OneHotOp>(op);
  ...
  if (mesh_dim_name != Layout::kUnshardedDim) {
    ...
    toolchain::SmallVector<int32_t, 4> multiplier(num_mesh_dimensions);
...
StatusOr<toolchain::DenseMap<int, Layout>> OneHotSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  if (input_layouts.find(0) == input_layouts.end())
    return toolchain::DenseMap<int, Layout>();
...
StatusOr<toolchain::DenseMap<int, Layout>> OneHotSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  auto one_hot = mlir::dyn_cast<mlir::TF::OneHotOp>(op);
  int axis = one_hot.getAxis();
  if (axis == -1) axis = ValueRank(one_hot.getIndices());
  ...
  toolchain::DenseMap<int, Layout> input_layouts(one_hot->getNumOperands());
...
StatusOr<mlir::Operation*> ShapeSPMDExpander::ExpandOp(mlir::Operation* op) {
  ...
  toolchain::SmallVector<std::string, 4> input_layouts;
  ...
  toolchain::SmallVector<mlir::TF::MulOp, 4> output_ops;
  for (int i = 0; i < op->getNumOperands(); ++i) {
    ...
    op->getResult(i).replaceAllUsesExcept(
        ...
        toolchain::SmallPtrSet<mlir::Operation*, 1>{global_shape});
  ...
  if (!input_layouts.empty()) {
    ...
    enclosing_cluster->setAttr(
        kShapeOpInputLayout,
        builder.getStrArrayAttr(toolchain::SmallVector<toolchain::StringRef, 4>(
...
StatusOr<toolchain::DenseMap<int, Layout>> ShapeSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
...
StatusOr<toolchain::DenseMap<int, Layout>> ShapeSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  return toolchain::DenseMap<int, Layout>();
}
