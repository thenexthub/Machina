...
absl::Status MatMulSPMDExpander::MaybeRelayoutInputs(
  ...
  //   output.
  toolchain::SmallSet<std::string, 4> used_mesh_dimensions;
...
StatusOr<toolchain::DenseMap<int, Layout>> MatMulSPMDExpander::ComputeLayoutForward(
...
StatusOr<toolchain::DenseMap<int, Layout>> MatMulSPMDExpander::ComputeLayoutBackward(
  ...
  auto specs_matmul_operands = [](const toolchain::ArrayRef<int64>& tensor_shape,
                                  const Layout& layout,
  ...
  return toolchain::DenseMap<int, Layout>({{0, left}, {1, right}});
}
