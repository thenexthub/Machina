...
StatusOr<int64_t> GatherV2SPMDExpander::GetAxis(mlir::Operation* op) {
  return ExtractConstIntFromValue(
      toolchain::cast<mlir::TF::GatherV2Op>(op).getAxis());
}

StatusOr<uint64_t> GatherV2SPMDExpander::GetBatchDim(mlir::Operation* op) {
  return toolchain::cast<mlir::TF::GatherV2Op>(op).getBatchDims();
}
...
StatusOr<uint64_t> ResourceGatherSPMDExpander::GetBatchDim(
    mlir::Operation* op) {
  return toolchain::cast<mlir::TF::ResourceGatherOp>(op).getBatchDims();
}
...
GatherCommonSPMDExpander::ComputeLayoutForward(
  ...
  toolchain::DenseSet<toolchain::StringRef> params_mesh_dims;
  if (params_layout) {
...
GatherCommonSPMDExpander::ComputeLayoutBackward(
  ...
  if (toolchain::isa<mlir::TF::GatherV2Op>(op)) {
...
StatusOr<mlir::Operation*> GatherNdSPMDExpander::ExpandOp(mlir::Operation* op) {
  auto gather_op = toolchain::cast<mlir::TF::GatherNdOp>(op);
  ...
  // Step 2)
  toolchain::DenseSet<toolchain::StringRef> used_dimensions;
...
GatherNdSPMDExpander::ComputeLayoutForward(
  ...
  TF_ASSIGN_OR_RETURN(toolchain::ArrayRef<int64_t> indices_shape,
