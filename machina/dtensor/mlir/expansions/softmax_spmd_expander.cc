...
StatusOr<Layout> GetBroadcastedLayout(toolchain::ArrayRef<int64_t> global_shape,
...
StatusOr<Layout> SoftmaxLossOpSPMDExpander::MaybeRelayoutInputs(
  ...
  TF_ASSIGN_OR_RETURN(
      toolchain::ArrayRef<int64_t> features_global_shape,
  ...
  TF_ASSIGN_OR_RETURN(
      toolchain::ArrayRef<int64_t> labels_global_shape,
...
StatusOr<mlir::Operation*> SoftmaxLossOpSPMDExpander::MaybeRelayoutOutputs(
  ...
  toolchain::SmallVector<mlir::Type, 4> types = {new_loss.getType(),
                                            new_backprop.getType()};
  toolchain::SmallVector<mlir::Value, 4> values = {new_loss, new_backprop};
...
SoftmaxLossOpSPMDExpander::ComputeLayoutForward(
  ...
  return toolchain::DenseMap<int, Layout>({{0, loss_layout}, {1, backprop_layout}});
}
