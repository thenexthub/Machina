...
StatusOr<Layout> MergeLayoutsForSplitOutput(
    int64_t split_dim, const toolchain::DenseMap<int, Layout>& layouts) {
...
StatusOr<toolchain::DenseMap<int, Layout>> SplitSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  auto split_op = mlir::cast<mlir::TF::SplitOp>(op);
  toolchain::DenseMap<int, Layout> output_layouts(split_op.getNumResults());
...
StatusOr<toolchain::DenseMap<int, Layout>> SplitSPMDExpander::ComputeLayoutBackward(
  ...
  toolchain::DenseMap<int, Layout> input_layouts(split_op.getNumOperands());
  // axis
...
StatusOr<toolchain::DenseMap<int, Layout>> SplitVSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  auto split_v_op = mlir::cast<mlir::TF::SplitVOp>(op);
  toolchain::DenseMap<int, Layout> output_layouts(split_v_op.getNumResults());
...
StatusOr<toolchain::DenseMap<int, Layout>> SplitVSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  auto split_v_op = mlir::cast<mlir::TF::SplitVOp>(op);
  TF_ASSIGN_OR_RETURN(Mesh mesh, ExtractDeviceMeshEnclosingCluster(op));

  toolchain::DenseMap<int, Layout> input_layouts(split_v_op.getNumOperands());
  // size_splits
