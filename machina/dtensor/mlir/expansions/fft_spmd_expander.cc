...
absl::Status IsProperFFTLength(
    mlir::Operation* op, const toolchain::SmallVector<int64_t, 4>& fft_length_vec) {
...
StatusOr<toolchain::SmallVector<int64_t, 4>> ExtractFFTLengthFromOp(
    mlir::Operation* op) {
  mlir::Value fft_length = op->getOperand(1);
  toolchain::SmallVector<int64_t, 4> fft_length_vec;
...
StatusOr<mlir::Operation*> ExpandFFTN(mlir::Operation* fft_op,
  ...
  if (IsComplexFFT(input)) {
    ...
    toolchain::ArrayRef<int64_t> rfft_shape =
...
StatusOr<mlir::Operation*> FFTSPMDExpander::ExpandOp(mlir::Operation* op) {
  ...
  return toolchain::TypeSwitch<mlir::Operation*, StatusOr<mlir::Operation*>>(op)
      // Forward prop ops.
...
StatusOr<toolchain::DenseMap<int, Layout>> FFTSPMDExpander::ComputeLayoutForward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& input_layouts) {
  auto iter = input_layouts.find(0);
  if (iter == input_layouts.end()) return toolchain::DenseMap<int, Layout>();
  ...
  toolchain::TypeSwitch<mlir::Operation*>(op)
      .Case<mlir::TF::FFTOp, mlir::TF::RFFTOp>(
...
StatusOr<toolchain::DenseMap<int, Layout>> FFTSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  auto iter = output_layouts.find(0);
  if (iter == output_layouts.end()) return toolchain::DenseMap<int, Layout>();
