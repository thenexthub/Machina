...
StatusOr<mlir::Operation*> ExpandVarHandleOp(mlir::Operation* op) {
  ...
  // auto var_op = toolchain::cast<mlir::TF::VarHandleOp>(op);
...
absl::Status ValidateAndAssignResourceInputLayout(
      ...
      [&](std::vector<mlir::StringRef> new_resource_layouts,
        ...
        op->setAttr(kNewResourceArgLayouts,
                    builder->getStrArrayAttr(
                        toolchain::ArrayRef<mlir::StringRef>(new_resource_layouts)));
  ...
  std::vector<toolchain::StringRef> mutable_input_layouts;
  std::vector<int> mutable_input_indices;
  for (auto layout_index_pair :
       toolchain::zip(resource_input_indices_attrs, resource_input_layouts_attrs)) {
...
StatusOr<mlir::Operation*> ResourceSPMDExpander::ExpandOp(mlir::Operation* op) {
  ...
  if (toolchain::isa<mlir::TF::DestroyResourceOp, mlir::TF::VarIsInitializedOp>(op))
    return InferSPMDExpandedLocalShape(op);

  if (toolchain::isa<mlir::TF::VarHandleOp>(op)) {
    return ExpandVarHandleOp(op);
  }
  if (toolchain::isa<mlir::TF::SummaryWriterOp>(op)) {
  ...
  // specially.
  if (toolchain::isa<mlir::TF::ReadVariableOp>(op)) {
    builder.setInsertionPointAfter(op);
  ...
  if (!toolchain::isa<mlir::TF::AssignVariableOp, mlir::TF::AssignAddVariableOp,
...
ResourceSPMDExpander::ComputeLayoutForward(
  ...
  if (toolchain::isa<mlir::TF::VarHandleOp, mlir::TF::VarIsInitializedOp>(op)) {
    return toolchain::DenseMap<int, Layout>({{0, Layout::Empty()}});
  }
  ...
  if (toolchain::isa<mlir::TF::DestroyResourceOp>(op))
    return toolchain::DenseMap<int, Layout>();
    ...
    if (input_layouts.find(0) != input_layouts.end()) {
      ...
      if (resource_layout.IsEmpty()) {
        ...
        if (mesh.ok()) {
          ...
          // for the resource.
          return toolchain::DenseMap<int, Layout>({{0, layout}, {1, layout}});
        }
      }
      return toolchain::DenseMap<int, Layout>({{1, input_layouts.lookup(0)}});
    }
    return toolchain::DenseMap<int, Layout>();
  }
  // Handling of these ops are no-ops.
  if (toolchain::isa<mlir::TF::DestroyResourceOp, mlir::TF::VarHandleOp,
