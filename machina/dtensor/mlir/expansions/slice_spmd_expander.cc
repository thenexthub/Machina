...
absl::Status GetSliceOpArguments(mlir::TF::SliceOp slice_op,
                                 toolchain::SmallVector<int64_t, 4>& begins,
                                 bool& dynamic_begins,
...
StatusOr<Layout> VerifySliceLayout(
    ...
    toolchain::ArrayRef<int64_t>* global_shape = nullptr) {
  if (layout.IsFullyReplicated()) return layout;

  TF_ASSIGN_OR_RETURN(toolchain::ArrayRef<int64_t> shape,
  ...
  toolchain::SmallVector<int64_t, 4> begins, sizes;
  bool dynamic_begins = false;
  begins.reserve(rank);
  sizes.reserve(rank);

  TF_RETURN_IF_ERROR(GetSliceOpArguments(
      toolchain::cast<mlir::TF::SliceOp>(slice_op), begins, dynamic_begins, sizes))
...
StatusOr<mlir::Operation*> SliceSPMDExpander::ExpandOp(mlir::Operation* op) {
  ...
  if (type.getElementType().isInteger(32))
    new_size = IntConst(
        builder, loc, toolchain::SmallVector<int32, 4>(sizes.begin(), sizes.end()));
  else
...
StatusOr<toolchain::DenseMap<int, Layout>> SliceSPMDExpander::ComputeLayoutForward(
  ...
  return toolchain::DenseMap<int, Layout>({{0, proposed_layout}});
}

StatusOr<toolchain::DenseMap<int, Layout>> SliceSPMDExpander::ComputeLayoutBackward(
    mlir::Operation* op, const toolchain::DenseMap<int, Layout>& output_layouts) {
  auto slice_op = mlir::cast<mlir::TF::SliceOp>(op);
  TF_ASSIGN_OR_RETURN(const Mesh mesh, ExtractDeviceMeshEnclosingCluster(op));

  toolchain::DenseMap<int, Layout> input_layouts(slice_op.getNumOperands());
