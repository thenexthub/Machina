...
toolchain::SmallVector<mlir::tf_device::ClusterOp, 4> FindAllDeviceClusters(
    mlir::Operation* op) {
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 4> nested_clusters;
...
mlir::LogicalResult MergeAttributes(
    ...
    mlir::ArrayAttr layout_attr2, toolchain::SmallVector<int, 4>* merged_indices,
    toolchain::SmallVector<mlir::Attribute, 4>* merged_layout) {
  toolchain::SmallDenseMap<toolchain::APInt, mlir::Attribute> attr_map;
  attr_map.reserve(indices_attr.size() + indices_attr2.size());
  for (const auto& data : toolchain::zip(indices_attr, layout_attr))
    attr_map.try_emplace(std::get<0>(data), std::get<1>(data));

  for (const auto& data : toolchain::zip(indices_attr2, layout_attr2)) {
...
mlir::LogicalResult MergeClusterMetadata(
  ...
  if (src_inferred_resource_handle_layouts_metadata &&
    ...
    toolchain::SmallVector<int, 4> merged_resource_indices;
    toolchain::SmallVector<mlir::Attribute, 4> merged_resource_layouts;
    ...
    target_cluster->setAttr(
        ...
        builder.getArrayAttr(
            toolchain::ArrayRef<mlir::Attribute>(merged_resource_layouts)));

    target_cluster->setAttr(
        kNewResourceLayoutIndices,
        builder.getI32VectorAttr(toolchain::ArrayRef<int>(merged_resource_indices)));
  }
  ...
  if (src_shape_layouts && !target_shape_layouts) {
    ...
    toolchain::SmallVector<int, 4> merged_shape_indices;
    toolchain::SmallVector<mlir::Attribute, 4> merged_shape_layouts;
    if (mlir::failed(MergeAttributes(
    ...
    target_cluster->setAttr(
        kShapeOpInputLayout,
        builder.getArrayAttr(
            toolchain::ArrayRef<mlir::Attribute>(merged_shape_layouts)));

    target_cluster->setAttr(
        kShapeOpInputLayoutIndices,
        builder.getI32VectorAttr(toolchain::ArrayRef<int>(merged_shape_indices)));
  }
...
mlir::LogicalResult InlineNestedDeviceClusters(mlir::Operation* op) {
  ...
  for (mlir::tf_device::ClusterOp cluster : clusters) {
    ...
    // cluster instead.
    for (auto it : toolchain::zip(cluster.GetBody().getTerminator()->getOperands(),
                             cluster.getResults())) {
    ...
    for (mlir::Operation& op :
         toolchain::make_early_inc_range(cluster.GetBody().without_terminator())) {
...
void CloneEmptyIfWithPredicate(mlir::TF::IfRegionOp if_region, const Mesh& mesh,
  ...
  *num_send_recvs += 1;
  ...
  auto new_cluster = builder.create<mlir::tf_device::ClusterOp>(
      if_region.getLoc(), toolchain::SmallVector<mlir::Type, 4>{});
  ...
  auto return_op = builder.create<mlir::tf_device::ReturnOp>(
      if_region.getLoc(), toolchain::SmallVector<mlir::Value, 4>{});
  ...
  *cloned_if_region_op = host_side_if;
  ...
  builder.create<mlir::TF::YieldOp>(if_region.getLoc(),
                                    /*operands=*/toolchain::ArrayRef<mlir::Value>{});
...
mlir::LogicalResult MergeClusters(mlir::ModuleOp module) {
  ...
  std::map<Mesh, toolchain::SmallVector<mlir::tf_device::ClusterOp, 4>> cluster_map;
  ...
  for (mlir::tf_device::ClusterOp cluster : clusters) {
    ...
    if (cluster_map.find(mesh) != cluster_map.end()) {
      ...
      cluster_map[mesh] =
          toolchain::SmallVector<mlir::tf_device::ClusterOp, 4>{cluster};
  ...
  for (const Mesh& mesh : meshes) {
    ...
    toolchain::SmallVector<mlir::Value, 4> merged_cluster_outputs;
    toolchain::SmallVector<mlir::Value, 4> merged_return_values;
    toolchain::SmallVector<mlir::Type, 4> merged_return_types;
    ...
    for (mlir::tf_device::ClusterOp cluster : mesh_cluster_list) {
      ...
      for (mlir::Operation& op_to_move :
           toolchain::make_early_inc_range(cluster_body.without_terminator())) {
        for (mlir::OpOperand& use : op_to_move.getUses()) {
          auto return_op =
              toolchain::dyn_cast<mlir::tf_device::ReturnOp>(use.getOwner());
    ...
    for (auto it :
         toolchain::zip(merged_cluster_outputs, new_cluster.getResults())) {
