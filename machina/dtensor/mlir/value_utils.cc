...
int ValueRank(mlir::Value operand_value) {
  ...
  const auto operand_type = toolchain::cast<mlir::TensorType>(type);
...
StatusOr<toolchain::SmallVector<int64_t>> GetTFShapeFromType(mlir::Type type) {
  auto ranked_type = toolchain::dyn_cast<mlir::RankedTensorType>(type);
  if (!ranked_type) {
    return errors::InvalidArgument(
        toolchain::formatv("Type {0} is not a RankedTensorType.", type).str());
  }
...
mlir::Value IntConstWithMatchingType(mlir::OpBuilder& builder,
                                     ...
                                     toolchain::ArrayRef<int64_t> values,
                                     mlir::Type type) {
  if (toolchain::cast<mlir::RankedTensorType>(type).getElementType().isInteger(64)) {
    return Int64Const(builder, loc, values);
  } else {
    toolchain::SmallVector<int32, 4> values32(values.begin(), values.end());
...
StatusOr<int64_t> ExtractConstIntFromValue(mlir::Value value) {
  ...
  auto a = *attr.value_begin<toolchain::APInt>();
...
StatusOr<mlir::Value> SelectScalarValueFromArray(mlir::OpBuilder& builder,
                                                 ...
                                                 mlir::Value array) {
  mlir::TensorType arrayType = toolchain::cast<mlir::TensorType>(array.getType());
