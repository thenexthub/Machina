...
toolchain::SmallVector<std::pair<mlir::Value, mlir::Value>, 8>
GetMergedMeshClusterResults(mlir::tf_device::ClusterOp current_cluster,
  ...
  for (auto result : toolchain::zip(current_cluster_return_op->getOpOperands(),
    ...
    for (auto& use : toolchain::make_early_inc_range(outer_op_result.getUses())) {
  ...
  for (auto result : toolchain::zip(merging_cluster_return_op->getOpOperands(),
...
void ReplaceOperandUsagesWithMergedClusterOutputs(
  ...
  for (auto result :
       toolchain::zip(values_to_replace, merged_cluster.getResults())) {
...
mlir::LogicalResult CreateMergedMeshCluster(
  ...
  toolchain::SmallVector<mlir::Type, 8> merged_cluster_output_types;
  toolchain::SmallVector<mlir::Value, 8> merged_cluster_output_values;
  toolchain::SmallVector<mlir::Value, 8> output_values_to_replace;
...
mlir::LogicalResult ClusterDeviceClusterOpsInBlock(mlir::OpBuilder* builder,
                                                   mlir::Block* block) {
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 4> block_ops;
  block->walk([&](mlir::Operation* op) {
    if (auto cluster = toolchain::dyn_cast<mlir::tf_device::ClusterOp>(op))
      block_ops.emplace_back(cluster);
  });

  std::optional<mlir::tf_device::ClusterOp> current_cluster;
  for (mlir::tf_device::ClusterOp cluster :
       toolchain::make_early_inc_range(block_ops)) {
