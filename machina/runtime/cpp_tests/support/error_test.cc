...
toolchain::Expected<int> ValueExpected() { return 42; }
toolchain::Expected<int> FailExpected() { return FailError(); }
...
TEST(Test, Macros) {
  ...
  // No viable conversion from 'const toolchain::Error' to 'bool':
...
static ::testing::AssertionResult Contains(toolchain::StringRef string,
                                           toolchain::StringRef substr) {
  if (string.contains(substr)) {
    return ::testing::AssertionSuccess()
           << toolchain::formatv("'{0}' contains '{1}'", string, substr).str();
  }
  return ::testing::AssertionFailure()
         << toolchain::formatv("'{1}' not found in '{0}'", string, substr).str();
}
...
TEST(Test, StackTrace) {
  ...
  std::string buffer;
  toolchain::raw_string_ostream(buffer) << stack_trace;
  ...
  if (toolchain::StringRef(buffer).contains(" @ ")) {
    EXPECT_TRUE(Contains(buffer, __FILE__));
  }
}

TEST(Test, TypedError) {
  Error e0 = toolchain::make_error<RpcUnavailableErrorInfo>(
  ...
  Error e1 = toolchain::make_error<RpcCancelledErrorInfo>(
...
TEST(Test, ErrorCollection) {
  Error e0 = toolchain::make_error<RpcDeadlineExceededErrorInfo>(
  ...
  Error e2 = toolchain::make_error<RpcCancelledErrorInfo>(
