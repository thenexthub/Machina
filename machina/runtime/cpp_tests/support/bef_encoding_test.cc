  ...
  for (unsigned prefix_size = 1; prefix_size <= kTestMaxAlignment;
    ...
    for (unsigned alignment = 1; alignment <= kTestMaxAlignment;
      ...
      for (size_t offset = 0; offset <= kTestMaxAlignment; ++offset) {
        const unsigned padding = toolchain::offsetToAlignment(
            offset + prefix_size, toolchain::Align(alignment));
      ...
      for (unsigned alignment = 1; alignment <= kTestMaxAlignment;
        ...
        for (size_t offset = 0; offset <= kTestMaxAlignment; ++offset) {
          const unsigned padding =
              (alignment >= prefix2)
                  ? toolchain::offsetToAlignment(offset + prefix1 + prefix2,
                                            toolchain::Align(alignment))
                  : toolchain::offsetToAlignment(offset + prefix1,
                                            toolchain::Align(prefix2));
          auto test_offset = offset + padding;
