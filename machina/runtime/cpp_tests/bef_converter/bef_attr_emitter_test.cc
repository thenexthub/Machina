...
TEST_F(BefAttrEmitterTest, EmitShapeAttribute) {
  ...
  EXPECT_THAT(attr.GetShape(),
              ::testing::ContainerEq(toolchain::ArrayRef(kTestShape, 3)));
}
...
TEST_F(BefAttrEmitterTest, EmitI32ArrayAttribute) {
  toolchain::SmallVector<mlir::Attribute, kTestI32ArraySize> elements;
  ...
  EXPECT_THAT(attr.data(), ::testing::ContainerEq(toolchain::ArrayRef(
...
TEST_F(BefAttrEmitterTest, EmitF64ArrayAttribute) {
  mlir::Builder builder(&context_);
  toolchain::SmallVector<mlir::Attribute, kTestF64ArraySize> elements;
...
TEST_F(BefAttrEmitterTest, EncodeDenseAttribute) {
  ...
  toolchain::SmallVector<mlir::Attribute, 8> elements;
...
TEST_F(BefAttrEmitterTest, EmitAggregateAttribute) {
  ...
  toolchain::SmallVector<mlir::Attribute, 4> elements;
