...
void LogIfErrorImpl(Error error, Severity severity) {
  toolchain::handleAllErrors(std::move(error), [&](const toolchain::ErrorInfoBase& info) {
...
void ErrorCollection::AddError(Error error) {
  if (error) {
    DieIfError(toolchain::handleErrors(
        std::move(error),
...
void ErrorCollection::log(raw_ostream& OS) const {
  if (errors_.empty()) {
    OS << toolchain::toString(Error::success());
    return;
  }
  if (errors_.size() == 1) {
    errors_[0]->log(OS);
  ...
  toolchain::SmallVector<std::string, 4> msg;
