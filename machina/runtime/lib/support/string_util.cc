...
//
// Based on machina/core/platform/numbers.cc
...
std::string HumanReadableNum(int64_t value) {
  ...
  if (value < 1000) {
    ...
    ss << toolchain::format("%.2f%c", value / 1000.0, *unit);
  }
  return s;
}
...
std::string HumanReadableElapsedTime(double seconds) {
  ...
  toolchain::raw_string_ostream ss(human_readable);
  ...
  if (microseconds < 999.5) {
    ss << toolchain::format("%0.3g us", microseconds);
    return ss.str();
  }
  ...
  if (milliseconds < 999.5) {
    ss << toolchain::format("%0.3g ms", milliseconds);
    return ss.str();
  }
  if (seconds < 60.0) {
    ss << toolchain::format("%0.3g s", seconds);
    return ss.str();
  }
  seconds /= 60.0;
  if (seconds < 60.0) {
    ss << toolchain::format("%0.3g min", seconds);
    return ss.str();
  }
  seconds /= 60.0;
  if (seconds < 24.0) {
    ss << toolchain::format("%0.3g h", seconds);
    return ss.str();
  }
  seconds /= 24.0;
  if (seconds < 30.0) {
    ss << toolchain::format("%0.3g days", seconds);
    return ss.str();
  }
  if (seconds < 365.2425) {
    ss << toolchain::format("%0.3g months", seconds / 30.436875);
    return ss.str();
  }
  seconds /= 365.2425;
  ss << toolchain::format("%0.3g years", seconds);
  return ss.str();
}
