...
void RunWhenReady(ArrayRef<AsyncValue*> values,
                  toolchain::unique_function<void()> callee) {
  ...
  toolchain::SmallVector<AsyncValue*, 4> unavailable_values;
  for (auto i : values) {
...
void RunWhenReady(ArrayRef<RCReference<AsyncValue>> values,
                  toolchain::unique_function<void()> callee) {
  auto mapped = toolchain::map_range(
      values, [](const RCReference<AsyncValue>& ref) -> AsyncValue* {
        return ref.get();
      });
  toolchain::SmallVector<AsyncValue*, 8> values_ptr(mapped.begin(), mapped.end());
