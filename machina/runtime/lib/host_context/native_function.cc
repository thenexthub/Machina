...
void NativeFunction::Execute(
  ...
  toolchain::SmallVector<AsyncValue*, 4> unavailable_args;
  ...
  // async execution.
  toolchain::SmallVector<RCReference<AsyncValue>, 4> args;
  args.reserve(arguments.size());
  for (auto* av : arguments) args.push_back(FormRef(av));

  toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4> indirect_results;
