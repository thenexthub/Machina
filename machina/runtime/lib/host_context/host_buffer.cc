...
RCReference<HostBuffer> HostBuffer::CreateUninitialized(
  ...
  assert(toolchain::isPowerOf2_32(alignment) &&
...
toolchain::raw_ostream &operator<<(toolchain::raw_ostream &os, const HostBuffer &buffer) {
  os << "HostBuffer<pointer="
     << toolchain::format_hex(reinterpret_cast<uintptr_t>(buffer.data()), 1)
     << ", size=" << buffer.size() << ">";
  return os;
}
