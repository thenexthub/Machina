...
static void RunBefFunction(
    ...
    const std::function<toolchain::Expected<ExecutionContext>(
...
int RunBefExecutor(const RunBefConfig& run_config) {
  return RunBefExecutor(
      ...
      [](HostContext* host, ResourceContext* resource_context)
          -> toolchain::Expected<ExecutionContext> {
...
int RunBefExecutor(
  ...
  toolchain::ArrayRef<uint8_t> buffer_arr;
  ...
  if (reinterpret_cast<uint64_t>(buffer.data()) % GetRequiredBefAlignment()) {
    ...
    buffer_arr = toolchain::ArrayRef<uint8_t>(
  ...
  if (work_queue == nullptr) {
    toolchain::errs() << run_config.program_name
  ...
  auto async_value_guard = toolchain::make_scope_exit([]() {
    assert(AsyncValue::GetNumAsyncValueInstances() == 0 &&
  ...
  auto core_rt =
      CoreRuntime::Create(decoded_diagnostic_handler, std::move(host_allocator),
                          std::move(work_queue));
  if (!core_rt) {
    toolchain::errs() << core_rt.takeError();
    return 1;
  }
  ...
  for (const auto& lib_name : run_config.shared_libs) {
    std::string err;
    auto dyn_lib =
        toolchain::sys::DynamicLibrary::getPermanentLibrary(lib_name.c_str(), &err);
    if (!dyn_lib.isValid()) {
      toolchain::errs() << run_config.program_name << ": couldn't load library "
                   << err << "\n";
      return 1;
    }
  ...
  toolchain::SmallVector<const Function*, 8> function_list;
  ...
  if (run_config.functions.empty()) {
    ...
    for (auto& fn_name : run_config.functions) {
      ...
      if (!fn) {
        toolchain::errs() << run_config.program_name << ": couldn't find function "
                     << fn_name << "\n";
        return 1;
      }
...
static void RunSyncBefFunctionHelper(const ExecutionContext& exec_ctx,
  ...
  toolchain::SmallVector<Value, 4> results;
  results.resize(function.result_types().size());

  toolchain::SmallVector<Value*, 4> result_ptrs;
...
static void RunBefFunction(
...

  {
    ...
    if (!exec_ctx) {
      toolchain::errs() << "Failed to create execution context.\n";
      abort();
    }
  ...
  if (AsyncValue::AsyncValueAllocationTrackingEnabled()) {
    ...
    if (before_num_values != after_num_values) {
      toolchain::errs() << "Evaluation of function '" << function.name()
