...
static toolchain::Expected<int32_t> TestFail() {
...
static std::string TestAsyncValueGet(const ExecutionContext& exec_ctx) {
  ...
  toolchain::raw_string_ostream sstr(return_value);
...
// different locations.
static toolchain::Expected<Chain> TestUniqueLoc(StringAttribute name,
                                           const ExecutionContext& exec_ctx) {
  class LocMap {
   public:
   ...
   private:
    mutex m_;
    toolchain::DenseMap<intptr_t, string_view> locs_;
  };
