...
class BenchmarkStats {
 public:
  ...
  void Summarize() {
    ...
    std::string prefix;
    toolchain::raw_string_ostream(prefix) << "BM:" << name_ << ':';
...
class AsyncBenchmarkRunner {
 public:
  ...
  void Start(toolchain::unique_function<void()> clean_up) {
 ...
 private:
  ...
  toolchain::SmallVector<AsyncValue*, 4> args_;
  ExecutionContext exec_ctx_;
  // Clean up function to run after the end of the benchmark.
  toolchain::unique_function<void()> clean_up_;
};
}  // namespace
...
static Error TestSyncBenchmark(RemainingSyncArguments args,
  ...
  toolchain::SmallVector<Value*, 16> func_args;
