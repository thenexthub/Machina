...
struct BEFFunction {
  ...
  toolchain::SmallVector<mlir::Type, 4> argument_types;
...
struct BEFFile {
  ...
  toolchain::DenseMap<size_t, string_view> strings;

  toolchain::DenseMap<size_t, mlir::Attribute> attributes;
...
struct BEFFunctionContext {
  ...
  toolchain::DenseMap<mlir::Operation*, ArrayRef<uint32_t>> region_references;
};
...
class BEFToMLIRConverter {
 ...
 private:
  ...
  mlir::LogicalResult ReadNullTerminatedStrings(
      ...
      toolchain::function_ref<void(size_t, string_view)> action);
  ...
  mlir::LogicalResult ReadStringOffsetSection(
      ArrayRef<uint8_t> section_data,
      toolchain::function_ref<void(string_view)> action);
...
class BEFFunctionReader {
 ...
 private:
  ...
  toolchain::DenseMap<mlir::Operation*, ArrayRef<uint32_t>>& region_references_;
  mlir::MLIRContext& context_;

  mlir::Location location_;
  std::vector<RegisterInfo> register_table_;
  std::vector<KernelTableEntry> kernel_table_;
  toolchain::SmallVector<int, 2> result_regs_;
};
...
mlir::LogicalResult BEFToMLIRConverter::ReadFunctionIndex(
  ...
  for (int i = 0; i < function_count; ++i) {
    ...
    auto read_types =
        [this, &function_index_reader](toolchain::SmallVector<mlir::Type, 4>* out) {
...
mlir::func::FuncOp BEFToMLIRConverter::CreateBEFFuncOp(
  ...
  toolchain::SmallVector<mlir::Type, 4> result_types(return_op.operand_type_begin(),
...
mlir::LogicalResult BEFToMLIRConverter::AddCompilationUnits(
  ...
  for (auto& compilation_unit : compilation_units_) {
    ...
    source_mgr.AddNewSourceBuffer(
        toolchain::MemoryBuffer::getMemBuffer(blob, "<unknown>"), toolchain::SMLoc());
...
BEFFunctionReader::ReadFunction(BefLocationReader* location_reader,
  ...
  block->addArguments(arg_types, toolchain::SmallVector<mlir::Location, 2>(
...
mlir::Operation* BEFFunctionReader::ReadKernel(
  ...
  for (int i = 0; i < attributes.size(); ++i) {
    ...
    // one.
    std::string attr_name = std::string("attr") + toolchain::utostr(i);
