...
mlir::OwningOpRef<mlir::ModuleOp> BEFToMLIRTranslate(
  ...
  source_mgr.setDiagHandler([](const toolchain::SMDiagnostic &diag, void *) {
    toolchain::SMDiagnostic bef_diag(diag.getFilename(), diag.getKind(),
                                diag.getMessage());
    bef_diag.print(nullptr, toolchain::errs());
  });

  auto *buffer_start = input->getBufferStart();
  auto buffer_size = input->getBufferSize();
  toolchain::ArrayRef<uint8_t> bef_file;
  ...
  if (reinterpret_cast<uint64_t>(buffer_start) % GetRequiredBefAlignment()) {
    ...
    bef_file = toolchain::ArrayRef<uint8_t>(
