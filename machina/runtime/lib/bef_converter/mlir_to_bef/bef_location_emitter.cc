...
bool BefLocationEmitter::IsSupportedLocation(const mlir::Location& loc) {
  if (toolchain::isa<mlir::UnknownLoc>(loc)) return true;
  if (toolchain::isa<mlir::NameLoc>(loc)) return true;
  if (toolchain::isa<mlir::FileLineColLoc>(loc)) return true;
  if (auto callsite_loc = toolchain::dyn_cast<mlir::CallSiteLoc>(loc)) {
    return IsSupportedLocation(callsite_loc.getCallee()) &&
           IsSupportedLocation(callsite_loc.getCaller());
  }
  if (auto fused_loc = toolchain::dyn_cast<mlir::FusedLoc>(loc)) {
...
ArrayRef<uint8_t> BefLocationEmitter::GetStringsSection() const {
  ...
  return toolchain::ArrayRef(result.data(), result.size());
}
...
size_t BefLocationEmitter::EmitLocation(const mlir::Location& loc) {
  ...
  if (auto filelinecol_loc = toolchain::dyn_cast<mlir::UnknownLoc>(loc)) {
    // Encoding format: `0x00`
    EmitByte(static_cast<uint8_t>(BefLocationType::kUnknown));
    return offset;
  }

  if (auto filelinecol_loc = toolchain::dyn_cast<mlir::FileLineColLoc>(loc)) {
  ...
  if (auto name_loc = toolchain::dyn_cast<mlir::NameLoc>(loc)) {
