...
static mlir::FunctionType GetRegionFunctionType(mlir::Region* region) {
  ...
  // Arguments.
  toolchain::SmallVector<mlir::Type, 4> inputs;
  ...
  toolchain::SmallVector<mlir::Type, 4> results;
...
struct EntityTable {
  ...
  toolchain::SetVector<mlir::Attribute> attributes;

  // Uniquing set of the kernels that we need to emit.
  std::vector<string_view> kernels;
  toolchain::StringMap<unsigned> kernel_ids;
  ...
  toolchain::DenseMap<mlir::Region*, unsigned> region_function_ids;
  toolchain::StringMap<unsigned> named_function_ids;

  // Types we've seen so far.
  std::vector<mlir::Type> types;
  toolchain::DenseMap<mlir::Type, unsigned> type_ids;
  ...
  toolchain::StringMap<uint8_t> strings;

  // This is all of the filenames referred to by locations in the file.
  toolchain::SmallVector<string_view, 4> location_filenames;
  toolchain::StringMap<uint32_t> location_filenames_index;
...
void EntityTable::AddType(mlir::Type type) {
  ...
  toolchain::SmallVector<char, 64> result_str;
  toolchain::raw_svector_ostream os(result_str);
  type.print(os);
  AddString(os.str());
}
...
LogicalResult EntityTable::Collect(mlir::ModuleOp module,
  ...
  module.walk([&](mlir::Operation* op) {
    ...
    if (auto fn = toolchain::dyn_cast<mlir::func::FuncOp>(op)) {
      if (IsNativeFunc(fn)) {
        ...
        if (IsSyncFunc(fn)) {
          toolchain::SmallSetVector<mlir::Value, 4> return_operands;
          for (const auto& iter : toolchain::enumerate(last_op.getOperands())) {
...
class EntityIndex {
 public:
  ...
  toolchain::ArrayRef<FunctionIndexEntry> GetFunctionIndex() const {
    return function_index_;
  }

 private:
  toolchain::StringMap<unsigned> strings_;
  toolchain::DenseMap<mlir::Attribute, unsigned> attribute_offsets_;
  ...
  toolchain::DenseMap<mlir::Operation*, size_t> location_position_offsets_;
};
...
class BEFFileEmitter : public BefEmitter {
 public:
  ...
  void EmitSection(BEFSectionID section_id,
                   toolchain::ArrayRef<uint8_t> section_data,
...
class BEFFunctionEmitter : public BEFFileEmitter {
 ...
 private:
  ...
  toolchain::DenseMap<mlir::Value, unsigned> register_number_;
  toolchain::DenseMap<mlir::Operation*, unsigned> kernel_index_;
...
void BEFFunctionEmitter::EmitFunction(mlir::Region* region,
  ...
  assert(toolchain::hasSingleElement(*region) && "should have a single block");
  auto& block = region->front();
...
void BEFFunctionEmitter::EmitArgumentsPseudoKernel(
  ...
  // result.
  toolchain::SmallVector<mlir::Operation*, 4> ready_kernels;
  for (auto& op : *block) {
