...
class LoggingOpHandler : public OpHandler {
 public:
  static toolchain::Expected<std::unique_ptr<LoggingOpHandler>> Create(
      CoreRuntime *runtime, OpHandler *fallback, bool sync_log_results) {
    std::unique_ptr<toolchain::raw_fd_ostream> metadata_ostream;
    if (auto metadata_dump_prefix =
      ...
      metadata_ostream = std::make_unique<toolchain::raw_fd_ostream>(
          metadata_dump_filename, error_code, toolchain::sys::fs::OF_Text);
      if (error_code)
      ...
      metadata_ostream =
          std::make_unique<toolchain::raw_fd_ostream>(/*fd=*/STDERR_FILENO,
                                                 /*shouldClose=*/false,
                                                 /*unbuffered=*/true);
    }
 ...
 private:
  ...
  toolchain::SmallVector<RCReference<AsyncValue>, 4> CollectAsyncHostTensors(
    ...
    toolchain::SmallVector<RCReference<AsyncValue>, 4> async_hts;
    for (auto &th : tensor_handles) {
  ...
  void PrintTensorToFile(const HostTensor &tensor, int log_counter,
    ...
    std::error_code error_code;
    toolchain::raw_fd_ostream fostream(tensor_dump_filename, error_code,
    ...
    } else if (auto *sht = toolchain::dyn_cast<StringHostTensor>(&tensor)) {
  ...
  mutable mutex mu_;
  std::unique_ptr<toolchain::raw_fd_ostream> metadata_ostream_ TFRT_GUARDED_BY(mu_);
};
}  // namespace
      ...
      [this, op_name = op_name.str(),
       fallback_handle =
...

        {
          ...
          toolchain::SmallVector<RCReference<AsyncValue>, 4> async_host_tensors =
...
toolchain::Expected<tfrt::OpHandler *> CreateLoggingOpHandler(
