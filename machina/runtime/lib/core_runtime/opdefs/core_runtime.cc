...
ShapeAttr ParseShapeAttr(MLIRContext *context, toolchain::StringRef spec,
                         Location loc) {
...
void PrintShapeAttr(ShapeAttr attr, mlir::DialectAsmPrinter &os) {  // NOLINT
  ...
  if (attr.hasRank()) {
    ...
    toolchain::interleave(attr.getShape(), os, print_dim, "x");
  } else {
    os << "*";
  }
  os << ">";
...
void ExecuteOp::getOpAttrs(
    toolchain::SmallVectorImpl<std::pair<StringRef, Attribute>> *op_attrs) {
  assert(op_attrs);
  op_attrs->clear();
