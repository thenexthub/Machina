...
#include "tfrt/core_runtime/op_attr_type.def"
  ...
  return toolchain::offsetToAlignment(offset + prefix_size,
                                 toolchain::Align(peak_alignment));
}
...
static void GetSortedAttrs(
    ...
    toolchain::SmallVectorImpl<const OpAttrsRawEntry *> *result) {
...
class OpAttrs::OutOfLineRepresentation {
 ...
 public:
  toolchain::StringMap<OpAttrsRawEntry> entries_;
  toolchain::BumpPtrAllocatorImpl<toolchain::MallocAllocator, 256, 256> allocator_;
};
...
void OpAttrs::Dump() const { Print(toolchain::errs()); }
...
RCReference<ImmutableOpAttrs> ImmutableOpAttrs::create(const OpAttrs &attrs) {
  // Sort the elements by attribute name.
  toolchain::SmallVector<const OpAttrsRawEntry *, 16> sorted_attrs;
  ...
  switch (type) {
    ...
    case OpAttrType::SHAPE: {
      ...
      if (shape_attr.HasRank())
        toolchain::interleave(shape_attr.GetShape(), os, "x");
      else
        os << "*";
      os << ">";
      break;
...
void OpAttrsRef::Dump() const { Print(toolchain::errs()); }
