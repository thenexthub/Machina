...
MDFunctionExecResult ExecuteMetadataFunction(
  ...
  // chain is available.
  toolchain::SmallVector<TensorMetadata, 4> argument_mds;
...
void ExecuteWhenMetadataIsReady(const OpInvocation& invocation,
  ...
  toolchain::SmallVector<AsyncValue*, 8> async_mds;
  async_mds.reserve(arguments.size());
  toolchain::SmallVector<TensorHandle, 4> arguments_copy;
  ...
  toolchain::SmallVector<RCReference<AsyncValue>, 8> result_th_avs;
  ...
  RunWhenReady(async_mds, [metadata_fn, callback = std::move(callback),
    ...
    toolchain::SmallVector<TensorMetadata, 4> result_mds(num_results);
    if (auto error =
...
std::string GetOpDebugString(string_view op_name,
  ...
  std::string message;
  toolchain::raw_string_ostream trace_message(message);
