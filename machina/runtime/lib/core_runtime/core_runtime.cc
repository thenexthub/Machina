...
class OpHandlerRegistry {
 ...
 private:
  ...
  toolchain::StringMap<OpHandler*> op_handers_by_name_;
...
toolchain::Expected<std::unique_ptr<CoreRuntime>> CoreRuntime::Create(
...
Expected<CoreRuntimeOp> CoreRuntime::MakeCompositeOp(const Function* fn) {
  for (const auto& iter : toolchain::enumerate(fn->argument_types().drop_front())) {
    size_t i = iter.index();
  ...
  for (const auto& iter : toolchain::enumerate(fn->result_types().drop_front())) {
    size_t i = iter.index();
    auto& type = iter.value();
    if (type.GetName() != kTensorHandleType) {
  ...
  auto execute_fn = [fn = fn](const OpInvocation& invocation) {
    ...
    toolchain::SmallVector<AsyncValue*, 4> arguments;
    toolchain::SmallVector<RCReference<AsyncValue>, 4> arguments_ref;
    ...
    for (const auto& iter : toolchain::enumerate(toolchain::drop_begin(results, 1))) {
      size_t i = iter.index();
