...
static Chain PrintTensorHandle(const TensorHandle &arg) {
  toolchain::SmallString<256> message;
  toolchain::raw_svector_ostream(message) << arg << "\n";
...
static toolchain::Expected<TensorHandle> ConstStringTensor(
...
static toolchain::Expected<TensorHandle> ConstDenseTensor(
...
static toolchain::Expected<TensorHandle> CreateDenseTensor(
...
static toolchain::Expected<CoreRuntimeOp> GetCoreRuntimeOp(
...
static void CoreRtConditional(RemainingArguments args, RemainingResults results,
  ...
  toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4> result_refs;
  ...
  auto if_impl =
      [](const HostTensor &ht, const Function *true_fn,
         ...
         const ExecutionContext &exec_ctx) {
        toolchain::Expected<bool> predicate = GetTensorPredicateValue(ht);
        if (!predicate) {
        ...
        toolchain::SmallVector<RCReference<AsyncValue>, 8> results;
...
static void CoreRtWhileLoopIteration(
    ...
    toolchain::SmallVector<RCReference<AsyncValue>, 4> arg_refs,
    toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4> result_refs);
...
static void CoreRtWhileLoopIterationImpl(
  ...
  toolchain::Expected<bool> predicate = GetTensorPredicateValue(condition);
  if (!predicate) {
  ...
  toolchain::SmallVector<AsyncValue *, 4> args;
  for (RCReference<AsyncValue> &arg : arg_refs) args.push_back(arg.get());
  toolchain::SmallVector<RCReference<AsyncValue>, 4> passed_args;
...
static void CoreRtWhileLoopIteration(
  ...
  toolchain::SmallVector<AsyncValue *, 4> args;
  for (RCReference<AsyncValue> &arg : arg_refs) args.push_back(arg.get());
  toolchain::SmallVector<RCReference<AsyncValue>, 2> condition;
  condition.resize(2);
