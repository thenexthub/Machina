...
static void TFRTCase(RemainingArguments args, RemainingResults results,
  ...
  auto case_impl = [exec_ctx](
                       toolchain::SmallVector<const Function*, 4> branches,
    ...
    if (branch_index < 0 || branch_index >= branches.size()) {
      ...
      // is to be consistent with machina's case op.
  ...
  if (branch_index_av->IsAvailable()) {
    ...
    toolchain::SmallVector<const Function*, 4> branch_vector;
  ...
  // closure runs.
  toolchain::SmallVector<RCReference<const Function>, 4> branch_refs;
...
static void TFRTOnce(RemainingArguments args, RemainingResults results,
  ...
  struct TFRTOnceResource {
    ...
    toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4> results;
  ...
  if (!resource->executed.exchange(true)) {
    toolchain::SmallVector<RCReference<AsyncValue>, 4> values(
        function->num_results());
    function->Execute(exec_ctx, args.values(), values);
    for (auto pair : toolchain::zip_first(resource->results, values))
      std::get<0>(pair)->ForwardTo(std::get<1>(pair));
  }

  toolchain::copy(resource->results, results.values().begin());
}
...
static void TFRTRepeatI32Block(
  ...
  toolchain::SmallVector<AsyncValue*, 8> passed_args(args.values().begin(),
                                                args.values().end());

  toolchain::SmallVector<RCReference<AsyncValue>, 4> results;
  results.resize(result_refs.size());
  ...
  if (end >= count_value) {
    ...
    passed_args[0]->AndThen(
        [end, block_size, count_value, exec_ctx,
         ...
         arg_refs = RCArray<AsyncValue>(toolchain::ArrayRef(passed_args)),
...
static void TFRTRepeatI32(RemainingArguments args, RemainingResults results,
  ...
  auto while_impl = [exec_ctx](
                        ...
                        toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4>
                            result_refs) mutable {
