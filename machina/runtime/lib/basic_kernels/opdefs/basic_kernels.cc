...
void createArgs(ArrayRef<OpAsmParser::UnresolvedOperand> operands,
  ...
  for (auto argAndType : toolchain::zip(operands, types)) {
...
ParseResult IfOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  FunctionType types;
  toolchain::SMLoc type_loc = parser.getCurrentLocation();
  ...
  auto body_operands = toolchain::ArrayRef(operands).drop_front();
...
void IfOp::print(OpAsmPrinter &p) {
  ...
  p << " : (";
  interleaveComma(toolchain::drop_begin(getOperandTypes(), 1), p);
  p << ") -> (";
  ...
  // the region.
  auto arg_name_values = toolchain::drop_begin(getOperands(), 1);
...
ParseResult RepeatI32Op::parse(OpAsmParser &parser, OperationState &result) {
  ...
  auto loop_operands = toolchain::ArrayRef(operands).drop_front();
...
void RepeatI32Op::print(OpAsmPrinter &p) {
  ...
  // Reuse the argument names provided to the op for the bbarg names within
  // the region.
  SmallVector<Value, 4> arg_name_values(toolchain::drop_begin(getOperands(), 1));
...
void ParallelForI32Op::print(OpAsmPrinter &p) {
  ...
  if (getNumOperands() > 3) {
    p << ", ";
    p.printOperands(toolchain::drop_begin(getOperands(), 3));
    p << " : ";
    interleaveComma(toolchain::drop_begin(getOperandTypes(), 3), p);
  }
...
struct RemoveUnnecessaryMergeChainsOp : public OpRewritePattern<MergeChainsOp> {
  ...
  LogicalResult matchAndRewrite(MergeChainsOp op,
    ...
    toolchain::SmallVector<mlir::Value> one_use;
