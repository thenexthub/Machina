...
struct TFRTInlinerInterface : public mlir::DialectInlinerInterface {
  ...
  bool isLegalToInline(mlir::Operation *call, mlir::Operation *callable,
    ...
    return toolchain::isa<CallOp>(call);
  }
  ...
  void handleTerminator(mlir::Operation *op,
    ...
    auto return_op = toolchain::dyn_cast<ReturnOp>(op);
    if (!return_op) return;

    for (auto iter : toolchain::zip(values_to_replace, return_op.getOperands())) {
...
mlir::Type TFRTDialect::parseType(mlir::DialectAsmParser &parser) const {
  toolchain::StringRef spec = parser.getFullSymbolSpec();
