...
static toolchain::StringMap<CompilerPass*>& GetStaticCompilerPasses() {
  static toolchain::StringMap<CompilerPass*>* compiler_passes =
      new toolchain::StringMap<CompilerPass*>();
  return *compiler_passes;
}

void RegisterCompilerPass(const std::string& name, CompilerPass* pass) {
  toolchain::StringMap<CompilerPass*>& compiler_passes = GetStaticCompilerPasses();
