...
class PrintStreamPass
 ...
 public:
  ...
  toolchain::StringRef getArgument() const final { return "tfrt-print-stream"; }
  ...
  void runOnOperation() override {
    ...
    auto emit_stream = [&](mlir::Operation* op, mlir::Location loc) {
      ...
      if (!child_streams.empty()) {
        toolchain::SmallVector<std::string> child_stream_ids;
        ...
        diag << ", child streams: [" << toolchain::join(child_stream_ids, ", ")
             << "]";
      }
    };
