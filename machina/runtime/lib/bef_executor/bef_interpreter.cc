...
class BEFInterpreterImpl final {
 ...
 private:
  ...
  toolchain::SmallVector<Value*, 16> registers_;

  // Store local Values used in the computation.
  toolchain::SmallVector<Value, 16> local_values_;

  // All kernel entries in the function.
  toolchain::SmallVector<KernelEntry, 16> kernel_entries_;

  // Registers that are retired at each kernel.
  toolchain::SmallVector<Value*, 16> retired_register_pool_;
  // Attributes used in all kernels.
  toolchain::SmallVector<const void*, 16> attribute_pool_;
};
...
void BEFInterpreterImpl::SetupKernelEntries() {
  toolchain::SmallVector<int, 16> user_counts;
  ...
  for (auto kernel_offset : func_.kernel_offsets()) {
    ...
    kernel_entry.retired_regs =
        toolchain::ArrayRef(retired_register_pool_.begin() + retired_reg_start,
...
Error BEFInterpreterImpl::Execute(const ExecutionContext& exec_ctx,
  ...
  for (auto& kernel_entry : kernel_entries_) {
    ...
    kernel_frame.SetAttributes(
        toolchain::ArrayRef(attribute_pool_.data() + kernel_entry.attribute_start,
