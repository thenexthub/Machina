...
class SyncBEFFunction final : public BEFFunction {
 ...
 private:
  ...
  // their register number.
  toolchain::SmallVector<RegisterInfo, 16> register_infos_;
  ...
  toolchain::SmallVector<uint32_t, 8> kernel_offsets_;

  // This is an array of register index for the result registers.
  toolchain::SmallVector<uint32_t, 4> result_regs_;
};
...
class BEFFileImpl : public BEFFile {
 public:
  ...
  toolchain::SmallVector<KernelImplementation, 8> kernels_;
  toolchain::SmallVector<TypeName, 8> type_names_;
  toolchain::StringMap<size_t> function_symbol_table_;
  toolchain::SmallVector<std::unique_ptr<Function>, 8> functions_;
