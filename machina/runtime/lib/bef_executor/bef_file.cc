...
struct FunctionIndex {
  ...
  size_t name_offset;
  toolchain::SmallVector<TypeName, 4> arguments;
  toolchain::SmallVector<TypeName, 4> results;
};
...
class BEFFileReader : public BEFReader {
 ...
 private:
  bool ReadFunctionIndexSectionInternal(
      toolchain::SmallVectorImpl<FunctionIndex>* function_indices);
...
bool BEFFileReader::DiagnoseUnknownKernel(size_t kernel_idx,
  ...
  // below.
  toolchain::SmallVector<FunctionIndex, 8> function_indices;
  ...
  toolchain::SmallVector<size_t, 4> result_regs;
...
bool BEFFileImpl::ReadFunction(size_t function_offset,
                               ...
                               toolchain::SmallVectorImpl<size_t>* result_regs,
                               HostAllocator* host_allocator) {
  auto format_error = [&]() -> bool {
  ...
  function_info->kernels =
      toolchain::ArrayRef(reinterpret_cast<const uint32_t*>(reader.file().begin()),
...
void BEFFile::GetFunctionList(
    toolchain::SmallVectorImpl<const Function*>* results) const {
