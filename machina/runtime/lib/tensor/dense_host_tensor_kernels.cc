...
static void MakeTensor(Argument<RCReference<HostBuffer>> buffer,
  ...
  if (buffer.get()->size() !=
    ...
    toolchain::raw_string_ostream ss(error_msg);
...
template <typename T>
static toolchain::Expected<Chain> SetDenseTensorWithValues(
...
static toolchain::Expected<RCReference<HostBuffer>> AllocateBuffer(
...
static toolchain::Expected<RCReference<HostBuffer>> SyncAllocateBuffer(
...
static toolchain::Expected<RCReference<HostBuffer>> GetBufferSlice(
