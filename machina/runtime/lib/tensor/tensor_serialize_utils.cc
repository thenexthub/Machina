...
size_t SerializeDenseHostTensorToDenseAttr(const DenseHostTensor& dht,
  ...
  const size_t offset = encoder->EncodeDenseAttr(
      element_type, toolchain::ArrayRef(shape),
      toolchain::ArrayRef(static_cast<const uint8_t*>(dht.data()),
                     dht.DataSizeInBytes()));
  return offset;
}

toolchain::Expected<DenseHostTensor> DeserializeDenseHostTensorFromDenseAttr(
...
void SerializeTensorMetadataInternal(const TensorMetadata& md, char* pos) {
  ...
  toolchain::SmallVector<Index, 4> dimensions;
...
toolchain::Expected<TensorMetadata> DeserializeTensorMetadataInternal(
...
toolchain::Expected<TensorMetadata> DeserializeTensorMetadata(
...
toolchain::Expected<toolchain::SmallVector<RCReference<HostBuffer>, 4>>
SerializeDenseHostTensor(const DenseHostTensor& dht, HostContext* host) {
  toolchain::SmallVector<RCReference<HostBuffer>, 4> buffers;
...
toolchain::Expected<DenseHostTensor> DeserializeDenseHostTensor(
    const toolchain::SmallVector<RCReference<HostBuffer>, 4>& serialized,
    HostContext* host) {
  TensorMetadata md =
