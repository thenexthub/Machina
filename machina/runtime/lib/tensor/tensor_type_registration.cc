...
class TensorTypeRegistry {
 ...
 private:
  ...
  toolchain::SmallVector<std::string, 10> name_list_ TFRT_GUARDED_BY(mu_);
  toolchain::StringMap<int8_t> name_to_id_map_ TFRT_GUARDED_BY(mu_);
};
...
TensorType TensorTypeRegistry::GetRegisteredTensorType(
  ...
  mutex_lock l(mu_);
  toolchain::StringMap<int8_t>::const_iterator itr =
