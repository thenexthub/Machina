...
toolchain::SmallVector<Index, 4> ComputeInnerDims(
    const toolchain::SmallVector<Index, 4>& orig_dims, int64_t num_out_dims) {
  toolchain::SmallVector<Index, 4> out_dims(num_out_dims, 0);
...
raw_ostream& operator<<(raw_ostream& os, const TensorShape& value) {
  os << '[';
  toolchain::SmallVector<Index, 8> dims;
...
// process.
void TensorShape::GetDimensions(toolchain::SmallVectorImpl<Index>* result) const {
  result->resize(GetRank());
  GetDimensions(*result);
}

// Return strides of this TensorShape in a way that is easy to process.
void TensorShape::GetStrides(toolchain::SmallVectorImpl<Index>* result) const {
  result->resize(GetRank());
  GetStrides(*result);
}
...
raw_ostream& operator<<(raw_ostream& os, const PartialTensorShape& value) {
  ...
  if (!value.GetShape()->empty()) {
    toolchain::interleaveComma(value.GetShape().value(), os);
  }
  return os << ']';
}
...
PartialTensorShape::PartialTensorShape(std::optional<ArrayRef<Index>> dims) {
  if (dims.has_value()) {
    toolchain::SmallVector<Index, 4> dims_vec{dims.value().begin(),
                                         dims.value().end()};
...
std::optional<ArrayRef<Index>> PartialTensorShape::GetShape() const {
  ...
  return toolchain::ArrayRef(dims_.value());
}
...
Expected<TensorShape> PartialTensorShape::ToTensorShape() const {
  ...
  toolchain::SmallVector<Index, 4> unknown_dims;
  ...
  os << "[";
  toolchain::interleaveComma(unknown_dims, os);
  os << "]";
...
TensorShape GetFlattenedInnerDimsShape(const TensorShape& shape,
  ...
  toolchain::SmallVector<Index, 4> orig_dims(shape.GetRank());
