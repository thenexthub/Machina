...
toolchain::SmallVector<Index, 4> GetDimensions(const TensorShape& shape);

void RotateRight(toolchain::MutableArrayRef<Index> array, size_t k = 1);
...
struct WindowedOutputData {
  toolchain::SmallVector<Index, 4> output_dims;  // NCHW
  toolchain::SmallVector<Index, 2> strides;
  toolchain::SmallVector<Index, 2> dilations;
  toolchain::SmallVector<Index, 2> paddings_before;
  toolchain::SmallVector<Index, 2> paddings_after;
};

// Get the output shape, strides, dilations and padding of conv-like ops.
toolchain::Expected<WindowedOutputData> GetTfWindowedOutputData(
    toolchain::ArrayRef<Index> input_dims,   // NCHW
    toolchain::ArrayRef<Index> filter_dims,  // OIHW
...
toolchain::SmallVector<Index, 4> MaybeExpandFilterSizes(toolchain::ArrayRef<Index> sizes,
                                                   int rank,
                                                   ChannelOrder channel_order);
...
Expected<TensorMetadata> TfPadOutputShape(
  ...
  toolchain::SmallVector<Index, 8> output_shape;
