...
#define TFRT_RETURN_IF_ERROR(KERNEL_ERROR, ERR)               \
  do {                                                        \
    if (auto err = ERR) {                                     \
      toolchain::Error unknown = toolchain::handleErrors(               \
          std::move(err), [&](const toolchain::StringError& err) { \
...
toolchain::Error UnaryEigenKernel(
...
template <typename Tin, typename Tout, typename Fn>
toolchain::Error BinaryEigenKernel(
...
template <typename Tin, typename Tout, typename Fn>
toolchain::Error BinaryEigenKernelBroadcast(
