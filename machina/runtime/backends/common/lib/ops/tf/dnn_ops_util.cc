...
toolchain::SmallVector<Index, 4> GetDimensions(const TensorShape& shape) {
  toolchain::SmallVector<Index, 4> dimensions(shape.GetRank());
  shape.GetDimensions(&dimensions);
  return dimensions;
}

void RotateRight(toolchain::MutableArrayRef<Index> array, size_t k) {
...
toolchain::SmallVector<Index, 4> MaybeExpandFilterSizes(toolchain::ArrayRef<Index> sizes,
                                                   int rank,
                                                   ChannelOrder channel_order) {
  toolchain::SmallVector<Index, 4> result(sizes.begin(), sizes.end());
  ...
  if (result.size() == rank - 2) {
    ...
    // NHWC to NCHW.
    RotateRight(toolchain::MutableArrayRef<Index>(result).drop_front());
  }
  return result;
}
