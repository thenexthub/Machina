...
Expected<TensorShape> GetBroadcastedShape(const TensorShape& arg0_shape,
  ...
  toolchain::SmallVector<Index, 8> ret;
  ret.reserve(rank);
...
Expected<ArgumentBCast> GetArgumentBCast(const TensorShape& argument_shape,
  ...
  toolchain::SmallVector<Index, 8> arg_dims(rank, 1);
  toolchain::SmallVector<Index, 8> res_dims(rank, 1);
  ...
  toolchain::SmallVector<Index, 8> broadcast;
