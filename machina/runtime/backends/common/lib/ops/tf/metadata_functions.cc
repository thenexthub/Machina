...
static Expected<TensorMetadata> TfConvOpMd(const TensorMetadata& input,
  ...
  if (channel_order == ChannelOrder::ChannelLast)
    RotateRight(toolchain::MutableArrayRef<Index>(input_dims_nchw).drop_front());
  ...
  if (channel_order == ChannelOrder::ChannelLast) {
    RotateRight(toolchain::MutableArrayRef<Index>(output_dims_nchw).drop_front(),
...
static Expected<TensorMetadata> TfPadOpMd(
  ...
  toolchain::SmallVector<int32_t, 8> default_paddings(8, 0);
...
static Expected<TensorMetadata> TfTransposeOpMdImpl(const TensorMetadata& input,
  ...
  toolchain::SmallVector<Index, 4> output_dims;
...
static Expected<TensorMetadata> TfTransposeOpFoldedMd(
  ...
  toolchain::SmallVector<Index, 4> perm;
...
static Expected<TensorMetadata> TfMeanOpMdImpl(
  ...
  toolchain::SmallVector<bool, 4> reduced_dim(input.shape.GetRank(), false);
...
toolchain::ArrayRef<std::pair<toolchain::StringRef, OpMetadataFn>>
GetAllTFMetadataFunctions() {
  static auto* md_functions = [] {
    auto* result = new std::vector<std::pair<toolchain::StringRef, OpMetadataFn>>;
