...
inline toolchain::Error CheckBias(const Conv2DParams& params,
...
inline toolchain::Error CheckBatchNormArgs(const Conv2DParams& params,
...
AsyncValueRef<Chain> Conv2DBatchNorm(
    ...
    const ExecutionContext& exec_ctx) {
  using OutputKernel = toolchain::Expected<BatchNormOutputKernel<T, Activation>>;
...
AsyncValueRef<Chain> Conv2DBias(const DenseHostTensor& input,
                                ...
                                const ExecutionContext& exec_ctx) {
  using OutputKernel = toolchain::Expected<BiasAddOutputKernel<T, Activation>>;
