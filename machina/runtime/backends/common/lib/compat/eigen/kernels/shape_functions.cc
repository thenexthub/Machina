...
toolchain::Expected<PaddingType> ParsePaddingType(string_view padding_type) {
  ...
  return toolchain::createStringError(toolchain::errc::invalid_argument,
                                 "Unknown padding type %s",
                                 padding_type.data());
}

toolchain::Expected<WindowedOutputDimension> ComputeWindowedOutputDimension(
...
toolchain::Error CheckShapeMatch(string_view lhs_name, const TensorShape& lhs_shape,
                            string_view rhs_name,
                            const TensorShape& rhs_shape) {
  if (lhs_shape != rhs_shape) {
    toolchain::Twine lhs = lhs_name + " is " + StrCat(lhs_shape);
    toolchain::Twine rhs = rhs_name + " is " + StrCat(rhs_shape);
...
toolchain::Error CheckDimensionMatch(string_view lhs_dim_name, Index lhs_dim,
