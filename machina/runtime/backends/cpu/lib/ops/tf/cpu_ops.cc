...
struct MeanHelper {
  ...
  // numbers.
  toolchain::SmallVector<int32_t, 4> positive_reduction_indices;
};
...
static Expected<MeanHelper> TfMeanOutputMd(
  ...
  toolchain::SmallVector<Index, 4> final_output_dims;
...
void RegisterTfCpuOps(CpuOpRegistry* op_registry) {
  for (const std::pair<toolchain::StringRef, OpMetadataFn>& md_function :
