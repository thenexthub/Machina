...
static AsyncValueRef<DenseHostTensor> TfShapeOp(
  ...
  toolchain::SmallVector<Index, 4> dims(input.shape().GetRank());
...
static AsyncValueRef<Tensor> TfExpandDimsOp(const Tensor& input,
  ...
  toolchain::SmallVector<Index, 4> output_dims(input_rank + 1);
  ...
  if (const auto* dht = toolchain::dyn_cast<const DenseHostTensor>(&input)) {
    return MakeAvailableAsyncValueRef<DenseHostTensor>(output_md,
                                                       dht->buffer());
  }

  if (const auto* sht = toolchain::dyn_cast<const StringHostTensor>(&input)) {
