...
//
// Based on machina/core/lib/jpeg/jpeg_mem.cc
...
// See also http://toolchain.org/docs/LibFuzzer.html#fuzzer-friendly-build-mode
...
uint8_t* UncompressLow(const void* srcdata, FewerArgsForCompiler* argball) {
  ...
  switch (components) {
    ...
    case 3:
      ...
      break;
    default:
      toolchain::errs() << " Invalid components value " << components << "\n";
  ...
  if (cinfo.output_width <= 0 || cinfo.output_height <= 0) {
    toolchain::errs() << "Invalid image size: " << cinfo.output_width << " x "
                 << cinfo.output_height;
    jpeg_destroy_decompress(&cinfo);
    return nullptr;
  }
  if (total_size >= (1LL << 29)) {
    toolchain::errs() << "Image too large: " << total_size;
    jpeg_destroy_decompress(&cinfo);
    return nullptr;
  }
  ...
  if (stride == 0) {
    ...
    toolchain::errs() << "Incompatible stride: " << stride << " < " << min_stride;
    jpeg_destroy_decompress(&cinfo);
    return nullptr;
  }
  ...
  while (cinfo.output_scanline < max_scanlines_to_read) {
    ...
    // Handle error cases
    if (num_lines_read == 0) {
      toolchain::errs() << "Premature end of JPEG data. Stopped at line "
  ...
  switch (error) {
    ...
    case JPEGERRORS_BAD_PARAM:
      ...
      break;
    default:
      toolchain::errs() << "Unhandled case " << error;
      break;
  }
