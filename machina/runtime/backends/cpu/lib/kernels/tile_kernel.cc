...
Expected<toolchain::SmallVector<Index, 5>> TileMultiples(
    const DenseHostTensor& multiples_arg) {
  toolchain::SmallVector<Index, 5> multiples;
...
void TileStringTensor(const StringHostTensor& input, StringHostTensor* output) {
  ...
  auto strides = [](const TensorShape& shape) -> toolchain::SmallVector<Index, 5> {
    toolchain::SmallVector<Index, 5> strides(shape.GetRank());
    Index stride = 1;
