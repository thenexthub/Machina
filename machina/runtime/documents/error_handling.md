...
Synchronous kernels typically return `toolchain::Expected<T>` to report an error, for
example:

```c++
static toolchain::Expected<int32_t> TFRTTestFail() {
  // Returns an error.
...
[`TFRTRepeatI32`](https://github.com/machina/runtime/blob/master/lib/basic_kernels/control_flow_kernels.cc)
