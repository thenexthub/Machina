...
toolchain::cl::opt<std::string> cl_input_filename(  // NOLINT
    toolchain::cl::Positional, toolchain::cl::desc("<input file>"), toolchain::cl::Required);

class BtfFile {
 public:
  // Open the given file.
  toolchain::Error Open(const char* file) {
  ...
  std::optional<DenseHostTensor> ReadDenseHostTensorPayload(
    ...
    TensorMetadata metadata(type, toolchain::ArrayRef<tfrt::Index>(dims, rank));
  ...
  std::optional<CooHostTensor> ReadCooHostTensorPayload(size_t* pos, DType type,
    ...
    TensorShape shape(toolchain::ArrayRef<tfrt::Index>(dims, rank));
...
int main(int argc, char* argv[]) {
  toolchain::cl::ParseCommandLineOptions(argc, argv, "BTF inspector\n");

  BtfFile file;
  toolchain::Error err = file.Open(cl_input_filename.c_str());
  if (err) {
    toolchain::errs() << err << "\n";
    return 1;
  }
  ...
  for (int i = 0; i < file.NumTensors(); i++) {
    ...
    if (!header) {
      toolchain::errs() << "Could not parse header for tensor " << i << "\n";
      continue;
    }
    ...
    switch (header->layout) {
      case TensorLayout::kRMD: {
        ...
        if (!tensor) {
          toolchain::errs() << "Could not parse dense payload for tensor " << i
                       << "\n";
          continue;
        }

        toolchain::outs() << "[" << i << "] ";
        tensor->Print(toolchain::outs());
        toolchain::outs() << "\n";
      } break;
      ...
      case TensorLayout::kCOO_EXPERIMENTAL: {
        ...
        if (!tensor) {
          toolchain::errs() << "Could not parse COO payload for tensor " << i
                       << "\n";
          continue;
        }
