...
class RandomAccessFile {
 public:
  ...
  // call.
  // On error, toolchain::Error is returned.
  virtual toolchain::Expected<size_t> Read(char* buf, size_t max_count,
...
class FileSystem {
 public:
  ...
  // toolchain::Error::success(). Otherwise, stores NULL in `file` and returns the
  // error.
  virtual toolchain::Error NewRandomAccessFile(
...
class FileSystemRegistry {
 ...
 private:
  mutex mu_;
  toolchain::StringMap<std::unique_ptr<FileSystem>> file_systems_
      TFRT_GUARDED_BY(mu_);
};
