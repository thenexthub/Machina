...
class Stream {
 public:
  ...
  toolchain::ArrayRef<mlir::Operation*> ops() const { return ops_; }

  // Returns the child streams for the `op` in this stream.
  const toolchain::SmallSetVector<const Stream*, 4>& GetChildStreams(
      mlir::Operation* op) const {
    auto iter = child_streams_.find(op);
    if (iter != child_streams_.end()) return iter->second;

    static const auto* empty_set = new toolchain::SmallSetVector<const Stream*, 4>();
    return *empty_set;
  }

  // Returns the child streams for the root op if this is a root stream.
  const toolchain::SmallSetVector<const Stream*, 4>& GetChildStreamsForRootOp()
      const {
    assert(parent_id_ == -1);
 ...
 private:
  ...
  // The operations in this stream in the original topological order.
  toolchain::SmallVector<mlir::Operation*> ops_;
  toolchain::DenseMap<mlir::Operation*, toolchain::SmallSetVector<const Stream*, 4>>
      child_streams_;
...
class StreamAnalysis {
 ...
 private:
  ...
  void MergeInterDependentStreams(int64_t cost_from_root,
                                  toolchain::SmallVector<int, 4>& child_stream_ids);
  ...
  struct BuildInfo {
    struct StreamInfo {
      ...
      toolchain::SmallSetVector<int, 4> side_deps;
  ...
  // for users to query.
  toolchain::SmallVector<std::unique_ptr<Stream>, 4> streams_;

  // `stream_map_` contains the finalized op-to-stream mapping.
  toolchain::DenseMap<mlir::Operation*, Stream*> stream_map_;
