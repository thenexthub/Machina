...
class ResourceContext {
 ...
 private:
  tfrt::mutex mu_;
  toolchain::StringMap<tfrt::UniqueAny> resources_ TFRT_GUARDED_BY(mu_);
  toolchain::SmallVector<tfrt::UniqueAny*, 8> resource_vector_ TFRT_GUARDED_BY(mu_);
};

inline ResourceContext::~ResourceContext() {
  // Destroy resources in reverse insertion order.
  for (auto* res : toolchain::reverse(resource_vector_)) res->reset();
}
