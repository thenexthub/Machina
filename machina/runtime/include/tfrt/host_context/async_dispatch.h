...
// std::vector<Foo> foos;
// AwaitRange(toolchain::map_range(foos, [](auto& foo) { return foo.async_value; }));
...
void EnqueueWork(const ExecutionContext& exec_ctx,
                 toolchain::unique_function<void()> work);

// An overload set that automatically converts toolchain::Expected values to the
...
// from toolchain::Expected to absl::StatusOr and will be removed.
...
void EnqueueWork(HostContext* host, toolchain::unique_function<void()> work);
