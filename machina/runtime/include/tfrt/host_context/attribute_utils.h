...
class ArrayAttribute {
 public:
  explicit ArrayAttribute(const void* data) {
    ...
    data_ = toolchain::ArrayRef(reinterpret_cast<const T*>(ptr), element_count);
  }
...
class CompilationUnitAttribute {
 public:
  explicit CompilationUnitAttribute(const void* value)
    ...
    toolchain::SmallVector<size_t, 4> nested_symbols_len(num_nested_symbols);
 ...
 private:
  ...
  toolchain::SmallVector<string_view> nested_symbols_;
...
// toolchain style cast (isa, cast, dyn_cast, etc) for efficient down-casting to
// subclasses.
class TypedAttrBase {
 public:
...
class ArrayAttr : public TypedAttrBase {
 public:
  ...
  ArrayRef<T> GetValue() const {
    ...
    return toolchain::ArrayRef(static_cast<const T*>(GetElements()),
                          GetNumElements());
  }
...
class ShapeAttr
 ...
 public:
  ...
  ArrayRef<AttrShapeT> GetShape() const {
    return toolchain::ArrayRef<AttrShapeT>(header_->dims, header_->rank);
  }
};
...
class DenseAttr
 ...
 public:
  ...
  toolchain::ArrayRef<int64_t> shape() const {
    return toolchain::ArrayRef(header_->dims, header_->rank);
  }
  ...
  ArrayRef<char> GetRawData() const {
    return toolchain::ArrayRef<char>(
