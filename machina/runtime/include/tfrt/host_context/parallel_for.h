 ...
 public:
  ...
  class BlockSizes {
   ...
   private:
    ...
    explicit BlockSizes(toolchain::unique_function<size_t(size_t)> impl)
        : impl_(std::move(impl)) {}
    ...
    mutable toolchain::unique_function<size_t(size_t)> impl_;
  };
  ...
  void Execute(size_t total_size, const BlockSizes& block_sizes,
               toolchain::unique_function<void(size_t, size_t)> compute,
               toolchain::unique_function<void()> on_done) const;
  ...
  AsyncValueRef<Chain> Execute(
      ...
      toolchain::unique_function<void(size_t, size_t)> compute) const;
  ...
  AsyncValueRef<R> Execute(
      size_t total_size, const BlockSizes& block_sizes,
      toolchain::unique_function<AsyncValueRef<T>(size_t, size_t)> compute,
      toolchain::unique_function<R(ArrayRef<AsyncValueRef<T>>)> on_done) const;
...
AsyncValueRef<R> ParallelFor::Execute(
  ...
  using ComputeFn = toolchain::unique_function<AsyncValueRef<T>(size_t, size_t)>;
  using DoneFn = toolchain::unique_function<R(ArrayRef<AsyncValueRef<T>>)>;
  ...
  struct ExecuteContext {
    ...
    toolchain::SmallVector<AsyncValue*, 32> BlockResults() const {
      auto mapped = toolchain::map_range(block_results, [](auto& async_value) {
    ...
    toolchain::SmallVector<AsyncValueRef<T>, 32> block_results;
  };
  ...
  Execute(
      ...
      [ctx = std::move(ctx)]() mutable -> void {
        toolchain::SmallVector<AsyncValue*, 32> block_results = ctx->BlockResults();
