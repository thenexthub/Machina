...
class DeviceManager {
 public:
  ...
  template <typename T,
            std::enable_if_t<std::is_base_of<Device, T>::value, int> = 0>
  toolchain::SmallVector<RCReference<T>, 4> ListDevices() const {
    toolchain::SmallVector<RCReference<T>, 4> devices;
 ...
 private:
  mutable mutex mu_;
  toolchain::StringMap<RCReference<Device>> device_map_ TFRT_GUARDED_BY(mu_);
};
...
class DeviceTypeRegistry {
 ...
 private:
  ...
  toolchain::SmallVector<std::unique_ptr<DeviceType>, 4> types_;
};
