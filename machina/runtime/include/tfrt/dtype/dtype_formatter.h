...
raw_ostream &operator<<(raw_ostream &os, const DTypeFormatter<T> &f) {
  if (f.full_precision) {
    os << toolchain::format("%.*g", std::numeric_limits<T>::max_digits10, f.value);
  } else {
    os << f.value;
  }
  return os;
...
// float f = 2.0;
// double d = 2.0;
// toolchain::outs() << "int32 value is " << FormatDType(i);
// toolchain::outs() << "f32 value is " << FormatDType(f);
// // Format with full precision.
// toolchain::outs() << "f64 value is " << FormatDType(d, true);
//
// // Format with explicitly DType value.
// toolchain::outs() << "f64 value is " << FormatDType(DType::F64, &d, true);
