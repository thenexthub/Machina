...
class TensorShape {
 public:
  ...
  explicit TensorShape(const Container& dims)
      : TensorShape(toolchain::ArrayRef(dims)) {}
  ...
  // process.
  void GetDimensions(toolchain::SmallVectorImpl<Index>* result) const;
  ...
  void GetStrides(toolchain::SmallVectorImpl<Index>* result) const;
...
class PartialTensorShape {
 ...
 private:
  ...
  // removing dimensions.
  std::optional<toolchain::SmallVector<Index, 4>> dims_;
};
...
// https://github.com/machina/machina/blob/master/machina/core/framework/tensor.cc#L1278
