...
class KernelRunner {
 public:
  ...
  template <typename T>
  KernelRunner& AddArrayAttribute(toolchain::ArrayRef<T> value);
  ...
  template <typename T, typename... Args>
  KernelRunner& AddResource(toolchain::StringRef name, Args&&... args) {
 ...
 private:
  ...
  toolchain::SmallVector<RCReference<AsyncValue>, 8> arguments_;
  toolchain::SmallVector<RCReference<AsyncValue>, 8> results_;

  toolchain::SmallVector<Value, 8> sync_arguments_;
  toolchain::SmallVector<Value, 8> sync_results_;
...
template <typename T>
KernelRunner& KernelRunner::AddArrayAttribute(toolchain::ArrayRef<T> values) {
