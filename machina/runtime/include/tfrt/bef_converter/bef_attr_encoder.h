 ...
 public:
  ...
  size_t EncodeStringAttr(string_view sv) {
    return EncodeArrayAttr(
        toolchain::ArrayRef(reinterpret_cast<const char*>(sv.data()), sv.size()));
  }
...
size_t BefAttrEncoder::EncodeAttr(T attr) {
  ...
  EmitBytes(
      toolchain::ArrayRef(reinterpret_cast<const uint8_t*>(&attr), entry_size));
  return offset;
}
...
size_t BefAttrEncoder::EncodeArrayAttr(ArrayRef<T> array) {
  ...
  EmitBytes(toolchain::ArrayRef(reinterpret_cast<const uint8_t*>(array.data()),
