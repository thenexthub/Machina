...
// Helper macro to get value from toolchain::Expected.
//
// The result of 'expr' should be a toolchain::Expected<T>. If it has a value, it
...
// Holds a stack trace that can be written to a toolchain::raw_ostream.
using StackTrace =
...
namespace internal {
// TMP to prevent elements of temporary reference types (i.e. toolchain::ArrayRef,
// toolchain::StringRef) because the underlying data is likely to go away before
// the Error is printed.
template <typename T>
struct IsTempRef : public std::integral_constant<bool, false> {};
template <typename T>
struct IsTempRef<toolchain::ArrayRef<T>>
    : public std::integral_constant<bool, true> {};
template <>
struct IsTempRef<toolchain::StringRef> : public std::integral_constant<bool, true> {
};
}  // namespace internal

// ErrorInfo with a pack of elements that are logged to toolchain::raw_ostream.
template <typename... Args>
class TupleErrorInfo : public toolchain::ErrorInfo<TupleErrorInfo<Args...>> {
 ...
 private:
  ...
  void log(toolchain::raw_ostream& os, std::index_sequence<Is...>) const {
...
// Create error from args, which are written to an toolchain::raw_ostream when the
// error is logged.
template <typename... Args>
toolchain::Error MakeTupleError(Args&&... args) {
  return toolchain::make_error<TupleErrorInfo<Args...>>(std::forward<Args>(args)...);
}

// Create error from args by writing them to an toolchain::raw_ostream immediately.
template <typename... Args>
toolchain::Error MakeStringError(Args&&... args) {
...
class BaseTypedErrorInfo : public toolchain::ErrorInfo<BaseTypedErrorInfo> {
 public:
  // Required field for all ErrorInfo derivatives.
...
class TypedErrorInfo
    : public toolchain::ErrorInfo<TypedErrorInfo<ErrorTrait>, BaseTypedErrorInfo> {
 public:
  static char ID;  // NOLINT
  using toolchain::ErrorInfo<TypedErrorInfo<ErrorTrait>,
...
class ErrorCollection : public toolchain::ErrorInfo<ErrorCollection> {
 public:
  // Required field for all ErrorInfo derivatives.
  static char ID;  // NOLINT
  ...
  const toolchain::SmallVector<std::unique_ptr<BaseTypedErrorInfo>, 4>&
  GetAllErrors() const;
 ...
 private:
  toolchain::SmallVector<std::unique_ptr<BaseTypedErrorInfo>, 4> errors_;
};
