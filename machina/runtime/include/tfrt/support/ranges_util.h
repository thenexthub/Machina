...
toolchain::SmallVector<typename RangeT::value_type, n> AsSmallVector(
    const RangeT& range) {
  return {range.begin(), range.end()};
}

template <size_t n, typename RangeT>
toolchain::SmallVector<typename RangeT::value_type, n> CopyRefToSmallVector(
    const RangeT& range) {
  return detail::CopyRefToVector<
      toolchain::SmallVector<typename RangeT::value_type, n>>(range);
}
