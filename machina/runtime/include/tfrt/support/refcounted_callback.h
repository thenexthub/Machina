...
namespace tfrt {
// Wrapper of a callback function that takes an toolchain::Error argument. The
...
class RefCountedCallback : public ReferenceCounted<RefCountedCallback> {
 public:
  explicit RefCountedCallback(toolchain::unique_function<void(Error)> done)
      : done_(std::move(done)) {}
 ...
 private:
  toolchain::unique_function<void(Error)> done_;
  mutex mu_;
