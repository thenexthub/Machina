...
class RCArray {
 public:
  explicit RCArray(toolchain::ArrayRef<T*> values) {
  ...
  explicit RCArray(toolchain::ArrayRef<RCReference<T>> references) {
  ...
  toolchain::ArrayRef<T*> values() const { return values_; }
 ...
 private:
  toolchain::SmallVector<T*, 4> values_;
};
