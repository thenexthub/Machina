 ...
 public:
  BEFKernel(const uint32_t* kernel_start)
      : header_(reinterpret_cast<const BEFKernelHeader*>(kernel_start)),
        result_table_(kernel_start + toolchain::alignTo(sizeof(BEFKernelHeader),
  ...
  ArrayRef<uint32_t> GetKernelEntries(int offset, int num_entries) const {
    return toolchain::ArrayRef(body_start_ + offset, num_entries);
  }

  ArrayRef<uint32_t> GetArguments() const {
    return toolchain::ArrayRef(body_start_, num_arguments());
  }

  ArrayRef<uint32_t> GetAttributes() const {
    return toolchain::ArrayRef(body_start_ + num_arguments(), num_attributes());
  }
