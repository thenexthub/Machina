...
class CoreRuntimeOp {
 public:
  ...
  CoreRuntimeOp(toolchain::unique_function<void(const OpInvocation&) const>&& fn,
                bool is_fallback);
  ...
  explicit CoreRuntimeOp(
      toolchain::unique_function<void(const CompositeOpInvocation&) const>&&
          native_fn);
 ...
 private:
  ...
  // toolchain::unique_function::operator() is non-const for some reason.
  toolchain::unique_function<void(const OpInvocation&) const> fn_;
  toolchain::unique_function<void(const CompositeOpInvocation&) const> native_fn_;
  bool is_fallback_;
