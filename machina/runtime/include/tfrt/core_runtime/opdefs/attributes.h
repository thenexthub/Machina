...
struct ShapeAttrStorage : public mlir::AttributeStorage {
  using KeyTy = std::pair<toolchain::ArrayRef<int64_t>, unsigned>;

  explicit ShapeAttrStorage(toolchain::ArrayRef<int64_t> shape,
                            unsigned unranked = 0)
...
class ShapeAttr : public mlir::Attribute::AttrBase<ShapeAttr, mlir::Attribute,
 ...
 public:
  ...
  static ShapeAttr get(mlir::MLIRContext* context) {
    return Base::get(context, toolchain::ArrayRef<int64_t>(), /*unranked=*/1);
  }
  ...
  toolchain::ArrayRef<int64_t> getShape() const {
    assert(hasRank());
