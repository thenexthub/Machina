...
MDFunctionExecResult ExecuteMetadataFunction(
    ...
    toolchain::SmallVectorImpl<TensorMetadata>& result_mds);

using MetadataIsReadyCallback = toolchain::unique_function<void(
    const ExecutionContext& exec_ctx, MutableArrayRef<TensorHandle> arguments,
    const OpAttrsRef& attrs, size_t num_results,
    const toolchain::SmallVector<TensorMetadata, 4>& result_mds,
    toolchain::SmallVectorImpl<AsyncValueRef<Tensor>>* result_tensor_avs,
    AsyncValueRef<Chain>* chain)>;
...
class AsyncOpDispatcher {
 public:
  ...
  explicit AsyncOpDispatcher(
      ...
      toolchain::SmallVectorImpl<RCReference<AsyncValue>>&& arguments,
      AsyncValueRef<Chain> chain,
      const toolchain::SmallVectorImpl<TensorMetadata>& result_mds,
      typename OpHandlerTraits::OpEntryTy op_entry,
      typename OpHandlerTraits::OpHandlerInfoTy op_handler_info)
      : exec_ctx_(exec_ctx),
  ...
  const toolchain::SmallVectorImpl<TensorMetadata>& result_mds() const {
    return result_mds_;
  }
  ...
  toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4>& result_ind_avs_ref() {
    return result_ind_avs_;
  }

  toolchain::SmallVector<AsyncValueRef<TensorMetadata>, 4>&
  ...
  static void RunDispatchFunctionSync(
      ...
      toolchain::SmallVectorImpl<RCReference<AsyncValue>>* results,
 ...
 private:
  ...
  toolchain::SmallVector<TensorMetadata, 4> result_mds_;

  AsyncValueRef<Chain> chain_;
  toolchain::SmallVector<RCReference<AsyncValue>, 4> arguments_;

  // These are the IndirectAsyncValue's for the results, that we need to
  // fulfill.
  toolchain::SmallVector<RCReference<IndirectAsyncValue>, 4> result_ind_avs_;

  // If we had no metadata function, then these will be the AsyncValues that
  // need to be fulfilled with a TensorMetadata.
  toolchain::SmallVector<AsyncValueRef<TensorMetadata>, 4> result_missing_md_avs_;
...
void AsyncOpDispatcher<OpHandlerTraits>::RunDispatchFunction() {
  ...
  toolchain::SmallVector<RCReference<AsyncValue>, 4> result_tensors;
...
/*static*/ void AsyncOpDispatcher<OpHandlerTraits>::RunDispatchFunctionSync(
    ...
    toolchain::SmallVectorImpl<RCReference<AsyncValue>>* results,
    AsyncValueRef<Chain>* chain, const ExecutionContext& exec_ctx) {
  toolchain::SmallVector<InputTensorTy*, 4> arg_tensors;
...
void ExecuteWithResultMetadataResolved(
    ...
    const OpAttrsRef& attrs, size_t num_results,
    const toolchain::SmallVector<TensorMetadata, 4>& result_mds,
    toolchain::SmallVectorImpl<AsyncValueRef<TensorMetadata>>* result_md_avs,
    toolchain::SmallVectorImpl<AsyncValueRef<Tensor>>* result_tensor_avs,
  ...
  toolchain::SmallVector<AsyncValue*, 4> async_args;
  async_args.reserve(arguments.size() + 1);
  toolchain::SmallVector<RCReference<AsyncValue>, 4> arg_tensors;
  ...
  if (async_args.empty()) {
    ...
    toolchain::SmallVector<RCReference<AsyncValue>, 4> result_tensors;
    toolchain::SmallVector<AsyncValueRef<TensorMetadata>, 0> empty_md_avs;
...
bool ExecuteOnOpHandlerImpl(
  ...
  toolchain::SmallVector<TensorMetadata, 4> result_mds;
  ...
  toolchain::SmallVector<AsyncValueRef<TensorMetadata>, 8> result_md_avs;
  toolchain::SmallVector<AsyncValueRef<Tensor>, 8> result_tensor_avs;
