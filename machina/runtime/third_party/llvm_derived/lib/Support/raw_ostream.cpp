...
// The implementation is derived from toolchain::raw_fd_ostream. It has mostly
...
static int getFD(toolchain::StringRef Filename, std::error_code &EC,
                 toolchain::sys::fs::CreationDisposition Disp,
                 toolchain::sys::fs::FileAccess Access,
                 toolchain::sys::fs::OpenFlags Flags) {
  assert((Access & toolchain::sys::fs::FA_Write) &&
  ...
  if (Filename == "-") {
    ...
    // possible.
    if (!(Flags & toolchain::sys::fs::OF_Text)) toolchain::sys::ChangeStdoutToBinary();
    return STDOUT_FILENO;
  }

  int FD;
  if (Access & toolchain::sys::fs::FA_Read)
    EC = toolchain::sys::fs::openFileForReadWrite(Filename, FD, Disp, Flags);
  else
    EC = toolchain::sys::fs::openFileForWrite(Filename, FD, Disp, Flags);
  if (EC) return -1;

  return FD;
}

raw_fd_ostream::raw_fd_ostream(toolchain::StringRef Filename, std::error_code &EC,
                               toolchain::sys::fs::OpenFlags Flags)
    : raw_fd_ostream(Filename, EC, toolchain::sys::fs::CD_CreateAlways,
                     toolchain::sys::fs::FA_Write, Flags) {}
...
raw_fd_ostream::~raw_fd_ostream() {
  if (FD >= 0) {
    flush();
    if (ShouldClose) {
      if (auto EC = toolchain::sys::Process::SafelyCloseFileDescriptor(FD))
        error_detected(EC);
    }
  }
}
...
#else
  return toolchain::raw_ostream::preferred_buffer_size();
#endif
}
...
toolchain::raw_ostream &outs() {
  // Set buffer settings to model stdout behavior.
  std::error_code EC;
  static raw_fd_ostream S("-", EC, toolchain::sys::fs::OF_None);  // NOLINT
  assert(!EC);
  return S;
}
