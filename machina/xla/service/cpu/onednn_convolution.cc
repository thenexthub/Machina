...
ABSL_ATTRIBUTE_NO_SANITIZE_MEMORY void __xla_cpu_runtime_OneDnnConvolution(
  ...
  MACHINA_MACHINA_XLA_LIGHTWEIGHT_CHECK(run_options != nullptr);
  MACHINA_MACHINA_XLA_LIGHTWEIGHT_CHECK(run_options->intra_op_thread_pool() != nullptr);
  ...
  MACHINA_MACHINA_XLA_LIGHTWEIGHT_CHECK(num_args == arg_indx);
  ...
  if (conv_config.optimization_config().user_scratchpad()) {
    MACHINA_MACHINA_XLA_LIGHTWEIGHT_CHECK(scratch != nullptr);
    MemrefInfo scratch_minfo(scratch);
    memory::desc scratchpad_md = conv_pd->scratchpad_desc();
    MACHINA_MACHINA_XLA_LIGHTWEIGHT_CHECK(scratchpad_md.get_size() <=
