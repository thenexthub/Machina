...
#include "machina/xla/hlo/analysis/hlo_reachability.h"
...
absl::Status DefaultSchedulerCore::ScheduleAnnotation(
  ...
  while (!sched_state->annotation_ready.empty()) {
    ...
    MACHINA_MACHINA_XLA_VLOG_LINES(2, [&]() {
      struct LogFormatter {
...
absl::Status DefaultSchedulerCore::SchedulingStep(
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(5, node->ToString());
  return absl::OkStatus();
}
...
DefaultSchedulerCore::ScheduleComputation(
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(5, sched_state->sched_graph.ToString());
  ...
  while (!sched_state->ready_set.empty() || !sched_state->nop_set.empty()) {
    ...
    MACHINA_MACHINA_XLA_VLOG_LINES(2, [&sched_state]() {
      struct LogFormatter {
        void operator()(std::string* out, const HloGraphNode* n) const {
...
absl::StatusOr<bool> LatencyHidingScheduler::Run(
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(5, module->ToString());
  ...
  if (VLOG_IS_ON(1)) {
    ...
    for (HloComputation* computation : computations_to_schedule_) {
      ...
      MACHINA_MACHINA_XLA_VLOG_LINES(1, LatencyHidingStatistics(
  ...
  if (VLOG_IS_ON(1)) {
    ...
    for (HloComputation* computation : computations_to_schedule_) {
      ...
      MACHINA_MACHINA_XLA_VLOG_LINES(1,
                     LatencyHidingStatistics(computation, scheduling_context_,
