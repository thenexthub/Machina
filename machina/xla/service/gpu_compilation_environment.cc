...
#include "machina/xla/service/gpu_compilation_environment.h"
...
absl::StatusOr<GpuCompilationEnvironment> CreateGpuCompEnvFromEnvVar() {
  ...
  ParseFlagsFromEnvAndIgnoreUnknown("MACHINA_MACHINA_XLA_FLAGS", flag_objects);
  return env;
}
  ...
  for (int j = 0; j < descriptor->field_count(); ++j) {
    ...
    if (reflection->HasField(env, field) &&
      ...
      return InvalidArgument(
          "Flag %s is set in both MACHINA_MACHINA_XLA_FLAGS env var and "
