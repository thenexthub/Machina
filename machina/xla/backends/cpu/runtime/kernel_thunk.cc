...
static absl::Status CheckBufferAlignment(
    ...
    absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> kernel_args) {
...
KernelThunk<num_arguments, num_results>::KernelThunk(
  ...
  for (size_t i = 0; i < arguments_buffers.size(); ++i) {
    kernel_args_[i] = MACHINA_MACHINA_XLA_CPU_KernelArg{
        nullptr, static_cast<size_t>(arguments_buffers_[i].size())};
  }
  for (size_t i = 0; i < results_buffers.size(); ++i) {
    kernel_args_[arguments_buffers_.size() + i] = MACHINA_MACHINA_XLA_CPU_KernelArg{
...
KernelThunk<num_arguments, num_results>::ExecuteInternal(
  ...
  MACHINA_MACHINA_XLA_CPU_KernelArg* kernel_args_ptr = kernel_args.data();
...
static bool Contains(absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> container,
                     const MACHINA_MACHINA_XLA_CPU_KernelArg& memory) {
  return absl::c_any_of(container, [&memory](const MACHINA_MACHINA_XLA_CPU_KernelArg& element) {
...
KernelThunk<num_arguments, num_results>::CheckInvariantBuffersMemory(
  ...
  auto arguments = absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg>(
      kernel_args.data(), arguments_buffers_.size());
  auto results = absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg>(
  ...
  for (int64_t i = 0; i < arguments.size(); ++i) {
    const MACHINA_MACHINA_XLA_CPU_KernelArg& argument = arguments[i];
