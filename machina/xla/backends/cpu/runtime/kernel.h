...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_H_
...
class Kernel {
 public:
  ...
  class KernelFunction {
   public:
    ...
    virtual MACHINA_MACHINA_XLA_CPU_Kernel* kernel() const = 0;
  };

  // A wrapper around function pointer that implements MACHINA_MACHINA_XLA_CPU_Kernel API.
  class KernelFunctionPtr final : public KernelFunction {
   public:
    explicit KernelFunctionPtr(MACHINA_MACHINA_XLA_CPU_Kernel* ptr) : ptr_(ptr) {}
    MACHINA_MACHINA_XLA_CPU_Kernel* kernel() const final { return ptr_; }

   private:
    MACHINA_MACHINA_XLA_CPU_Kernel* ptr_;  // not owned
  };

  // TODO(tsilytskyi): make this implementation detail private
  Kernel(unsigned arity, MACHINA_MACHINA_XLA_CPU_Kernel* kernel);
  ...
  absl::Status CallOnce(absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> args) const;
 ...
 private:
  ...
  MACHINA_MACHINA_XLA_CPU_Kernel* kernel_;  // pointer to the kernel owned by `function_`
...
inline ABSL_ATTRIBUTE_ALWAYS_INLINE absl::Status Kernel::CallOnce(
    absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> args) const {
  constexpr MACHINA_MACHINA_XLA_CPU_NumWorkGroups num_workgroups = {1, 1, 1};
  constexpr MACHINA_MACHINA_XLA_CPU_WorkGroupId workgroup_id = {0, 0, 0};

  MACHINA_MACHINA_XLA_CPU_KernelCallFrame call_frame = {&num_workgroups, &workgroup_id,
                                        args.size(), args.data()};
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_H_
