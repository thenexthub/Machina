...
#include "machina/xla/hlo/evaluator/hlo_evaluator_typed_visitor.h"
...
#include "machina/xla/index_util.h"
...
#include "machina/xla/service/gather_scatter_utils.h"
    ...
    if (to_sort.size() >= kMinElementsForMergesort) {
      ...
      if (to_sort.size() >= work_per_thread) {
        std::unique_ptr<tsl::Thread> thread = absl::WrapUnique(env->StartThread(
            tsl::ThreadOptions(), "MACHINA_MACHINA_XLA_mergesort",
