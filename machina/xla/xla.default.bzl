...
load(
    ...
    "LEGACY_MACHINA_MACHINA_XLA_USERS",
)
load(
    "//xla/tsl:tsl.bzl",
    "tsl_copts",
...
visibility(DEFAULT_LOAD_VISIBILITY + LEGACY_MACHINA_MACHINA_XLA_USERS)
...
_MACHINA_MACHINA_XLA_SHARED_OBJECT_SENSITIVE_DEPS = [
...
def xla_cc_binary(deps = [], copts = tsl_copts(), **kwargs):
    cc_binary(deps = deps + _MACHINA_MACHINA_XLA_SHARED_OBJECT_SENSITIVE_DEPS, copts = copts, **kwargs)
...
def xla_cc_test(
    ...
    strict_cc_test(
        name = name,
        deps = deps + _MACHINA_MACHINA_XLA_SHARED_OBJECT_SENSITIVE_DEPS,
