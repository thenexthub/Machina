...
MACHINA_MACHINA_XLA_FFI_REGISTER_STRUCT_ATTR_DECODING(::xla::ffi::PairOfI32AndF32,
...
TEST(FfiTest, StaticHandlerRegistration) {
  ...
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp0, noop, Ffi::Bind(),
                         {Traits::kCmdBufferCompatible});

  // Automatically infer binding specification from function signature.
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp1, noop);

  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "no-op-0", "Host", NoOp0);
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "no-op-1", "Host", NoOp1);
  ...
  ASSERT_EQ(handler0->traits, MACHINA_MACHINA_XLA_FFI_HANDLER_TRAITS_COMMAND_BUFFER_COMPATIBLE);
...
TEST(FfiTest, RegistrationTraitsBackwardsCompatibility) {
  static constexpr auto* noop = +[] { return absl::OkStatus(); };
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp, noop, Ffi::Bind());
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "traits-bwd-compat", "Host", NoOp,
                           MACHINA_MACHINA_XLA_FFI_HANDLER_TRAITS_COMMAND_BUFFER_COMPATIBLE);
  auto handler = FindHandler("traits-bwd-compat", "Host");
  TF_ASSERT_OK(handler.status());
  ASSERT_EQ(handler->traits, MACHINA_MACHINA_XLA_FFI_HANDLER_TRAITS_COMMAND_BUFFER_COMPATIBLE);
}

// Declare XLA FFI handler as a function (extern "C" declaration).
MACHINA_MACHINA_XLA_FFI_DECLARE_HANDLER_SYMBOL(NoOpHandler);

// Define XLA FFI handler as a function forwarded to `NoOp` implementation.
static absl::Status NoOp() { return absl::OkStatus(); }
MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER_SYMBOL(NoOpHandler, NoOp, Ffi::Bind());

TEST(FfiTest, StaticHandlerSymbolRegistration) {
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "no-op-sym-0", "Host", NoOpHandler);
...
TEST(FfiTest, DuplicateHandlerTraits) {
  static constexpr auto* noop = +[] { return absl::OkStatus(); };
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp1, noop, Ffi::Bind());
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp2, noop, Ffi::Bind(),
                         {Traits::kCmdBufferCompatible});
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "duplicate-traits", "Host", NoOp1);
...
TEST(FfiTest, DuplicateHandlerAddress) {
  ...
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp1, noop1, Ffi::Bind());
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp2, noop2, Ffi::Bind());
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "duplicate-address", "Host", NoOp1);
  auto status = TakeStatus(Ffi::RegisterStaticHandler(
...
TEST(FfiTest, AllowRegisterDuplicateWhenEqual) {
  static constexpr auto* noop = +[] { return absl::OkStatus(); };
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(NoOp, noop, Ffi::Bind());
  MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(GetXlaFfiApi(), "duplicate-when-equal", "Host",
                           NoOp);
  auto status = TakeStatus(Ffi::RegisterStaticHandler(
...
TEST(FfiTest, Metadata) {
  static constexpr auto* noop = +[] { return absl::OkStatus(); };
  MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(handler, noop, Ffi::Bind());
  ...
  EXPECT_EQ(metadata.api_version.major_version, MACHINA_MACHINA_XLA_FFI_API_MAJOR);
  EXPECT_EQ(metadata.api_version.minor_version, MACHINA_MACHINA_XLA_FFI_API_MINOR);
}
...
TEST(FfiTest, MetadataTraits) {
  ...
  EXPECT_TRUE(maybe_metadata.ok());
  auto metadata = maybe_metadata.value();
  EXPECT_EQ(metadata.traits, MACHINA_MACHINA_XLA_FFI_HANDLER_TRAITS_COMMAND_BUFFER_COMPATIBLE);
  EXPECT_EQ(metadata.api_version.major_version, MACHINA_MACHINA_XLA_FFI_API_MAJOR);
  EXPECT_EQ(metadata.api_version.minor_version, MACHINA_MACHINA_XLA_FFI_API_MINOR);
}
