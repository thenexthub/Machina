...
#include "machina/xla/shape_layout.h"
...
#include "machina/xla/stream_executor/tpu/c_api_defn.h"  // IWYU pragma: keep
#include "machina/xla/stream_executor/tpu/proto_helper.h"
#include "machina/xla/stream_executor/tpu/tpu_api.h"  // IWYU pragma: keep
#include "machina/xla/stream_executor/tpu/tpu_executor_api.h"
...
static void CreateVector(const absl::Span<const xla::Tile> src, TileList* dst) {
  dst->size = src.size();
  MACHINA_MACHINA_XLA_Tile* c_tiles;
  if (dst->size > TPU_C_API_MAX_INLINED) {
    dst->heap = new MACHINA_MACHINA_XLA_Tile[dst->size];
    c_tiles = dst->heap;
  } else {
    c_tiles = dst->inlined;
  }
...
xla::ShapedBuffer FromC(MACHINA_MACHINA_XLA_ShapedBuffer* c_buffer) {
...
void ToC(const xla::Shape& xla_shape, MACHINA_MACHINA_XLA_Shape* c_shape) {
  ...
  if (c_shape->ntuple_shapes > 0) {
    c_shape->tuple_shapes = new MACHINA_MACHINA_XLA_Shape[c_shape->ntuple_shapes];
...
xla::Shape FromC(const MACHINA_MACHINA_XLA_Shape* c_shape) {
...
void Destroy(MACHINA_MACHINA_XLA_Shape* c_shape) {
...
void ToC(const xla::Layout& layout, MACHINA_MACHINA_XLA_Layout* c_layout) {
...
xla::Layout FromC(const MACHINA_MACHINA_XLA_Layout* c_layout) {
  absl::Span<const int64_t> minor_to_major = MakeSpan(c_layout->minor_to_major);
  absl::InlinedVector<xla::Tile, 1> tiles;
  const MACHINA_MACHINA_XLA_Tile* c_tiles = c_layout->tiles.size > TPU_C_API_MAX_INLINED
                                ? c_layout->tiles.heap
...
void Destroy(MACHINA_MACHINA_XLA_Layout* c_layout) {
...
void ToC(const xla::Tile& tile, MACHINA_MACHINA_XLA_Tile* c_tile) {
  CreateVector(tile.dimensions(), &c_tile->dimensions);
}

xla::Tile FromC(const MACHINA_MACHINA_XLA_Tile* c_tile) {
  absl::Span<const int64_t> dims = MakeSpan(c_tile->dimensions);
  return xla::Tile(dims);
}

void Destroy(MACHINA_MACHINA_XLA_Tile* c_tile) {
  if (c_tile->dimensions.size > TPU_C_API_MAX_INLINED) {
    delete[] c_tile->dimensions.heap;
  }
}

MACHINA_MACHINA_XLA_ShapeIndex ToC(const xla::ShapeIndex& xla_shape) {
  MACHINA_MACHINA_XLA_ShapeIndex c_shape;
...
xla::ShapeIndex FromC(MACHINA_MACHINA_XLA_ShapeIndex* c_shape) {
  return xla::ShapeIndex(c_shape->indices, c_shape->indices + c_shape->count);
}

void ToC(const xla::LiteralSlice& literal, MACHINA_MACHINA_XLA_Literal* c_literal) {
...
xla::MutableBorrowingLiteral FromC(MACHINA_MACHINA_XLA_Literal* c_literal) {
...
void ToC(const xla::ShapedBuffer& buffer, MACHINA_MACHINA_XLA_ShapedBuffer* c_device_buffer) {
...
void Destroy(MACHINA_MACHINA_XLA_ShapeIndex* shape_index) { delete[] shape_index; }
void Destroy(SE_DeviceMemoryBase*) {}

void Destroy(MACHINA_MACHINA_XLA_Literal* c_literal) {
  delete[] c_literal->buffers;
  delete[] c_literal->sizes;
  ApiConverter::Destroy(&c_literal->shape);
}

void Destroy(MACHINA_MACHINA_XLA_ShapedBuffer* c_buffer) {
  ApiConverter::Destroy(&c_buffer->on_device_shape);
  delete[] c_buffer->bases;
}

MACHINA_MACHINA_XLA_HloModule ToC(const xla::HloModule& module) {
  MACHINA_MACHINA_XLA_HloModule c_module;
...
absl::StatusOr<std::unique_ptr<xla::HloModule>> FromC(
    const MACHINA_MACHINA_XLA_HloModule& c_module) {
...
void Destroy(MACHINA_MACHINA_XLA_HloModule* c_module) {
...
static xla::HloModuleConfig ConfigWithLayout(
    const MACHINA_MACHINA_XLA_HloModuleConfig& se_config) {
...
MACHINA_MACHINA_XLA_HloModuleConfig ToC(const xla::HloModuleConfig& config) {
  MACHINA_MACHINA_XLA_HloModuleConfig hlo_config;
  ...
  if (config.has_entry_computation_layout()) {
    ...
    hlo_config.entry_computation_layout.parameter_layouts =
        new MACHINA_MACHINA_XLA_Shape[layout.parameter_count()];
...
xla::HloModuleConfig FromC(const MACHINA_MACHINA_XLA_HloModuleConfig& c_config) {
...
void Destroy(MACHINA_MACHINA_XLA_HloModuleConfig* c_config) {
