...
#include "machina/xla/backends/cpu/runtime/dot_lib.h"
...
ThunkProtoExecutionDeserializer::GetCopyThunkRunImpl(
    const xla::cpu::ThunkProto& thunk) {
  // IMPORTANT(basioli): tfcompiled models should always emit toolchain kernels for
...
ThunkProtoExecutionDeserializer::GetKernelThunkRunImpl(
  ...
  auto get_args_initializer_as_string =
    ...
    for (const auto& buffer_proto : kernel_thunk.arguments_buffers()) {
      args_initializer.push_back(absl::StrCat(
          "MACHINA_MACHINA_XLA_CPU_KernelArg{", GetBufferAllocationString(buffer_proto), ", ",
          buffer_proto.size(), "}"));
    }
  ...
  absl::string_view kernel_invocation_format = R"(
     // Kernel Thunk
     {
       std::array<MACHINA_MACHINA_XLA_CPU_KernelArg, {{NUM_ARGS}}> args = {{ARGS_INITIALIZER}};
       MACHINA_MACHINA_XLA_CPU_NumWorkGroups kernel_thread_dims = {
           {{THREAD_DIM_X}},
           {{THREAD_DIM_Y}},
           {{THREAD_DIM_Z}},
       };
       ...
       for (uint64_t z = 0; z < {{THREAD_DIM_Z}}; ++z) {
         for (uint64_t y = 0; y < {{THREAD_DIM_Y}}; ++y) {
           for (uint64_t x = 0; x < {{THREAD_DIM_X}}; ++x) {
             MACHINA_MACHINA_XLA_CPU_WorkGroupId kernel_thread = {x, y, z};

             MACHINA_MACHINA_XLA_CPU_KernelCallFrame call_frame = {
                 &kernel_thread_dims, &kernel_thread, args.size(), args.data()};

             MACHINA_MACHINA_XLA_CPU_KernelError* error = (*{{KERNEL_NAME}})(&call_frame);
