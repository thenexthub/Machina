...
static void CompareWithGoldenFile(
    const string& machina_relative_golden_file_name,
  ...
  //   "third_party/machina/compiler/aot:codegen_test"
  const bool update_golden = false;
  string golden_file_name =
      GetDataDependencyFilepath(machina_relative_golden_file_name);
...
TEST(CodegenTest, Golden) {
  ...
  CompareWithGoldenFile("machina/compiler/aot/codegen_test_o.golden",
  ...
  CompareWithGoldenFile("machina/compiler/aot/codegen_test_h.golden", header,
                        true);
}
#endif
}  // namespace
