...
#include "machina/compiler/aot/codegen.h"
...
#include "machina/compiler/aot/thunk_proto_execution_deserializer.h"
...
absl::StatusOr<std::string> GenFunctionDeclaration(
  ...
  switch (symbol.function_type_id()) {
    ...
    case xla::cpu::SymbolProto::KERNEL:
      absl::StrAppend(&function_declaration, " MACHINA_MACHINA_XLA_CPU_KernelError* ",
                      symbol.name(),
                      "(const MACHINA_MACHINA_XLA_CPU_KernelCallFrame* call_frame);");
      break;
    default:
...
#include "machina/xla/types.h")"};
  ...
  if (HasThunkKind(aot_thunks->proto().thunk_sequence(),
    ...
    if (xla_cpu_multi_thread_eigen) {
      runtime_specific_includes.push_back(
          R"(#include "machina/xla/service/cpu/runtime_matmul.h")");
    }
    runtime_specific_includes.push_back(
        R"(#include "machina/xla/service/cpu/runtime_single_threaded_matmul.h")");
  }
  ...
  if (HasThunkKind(aot_thunks->proto().thunk_sequence(),
    ...
    if (xla_cpu_multi_thread_eigen) {
      runtime_specific_includes.push_back(
          R"(#include "machina/xla/service/cpu/runtime_conv2d.h")");
    }

    runtime_specific_includes.push_back(
        R"(#include "machina/xla/service/cpu/runtime_single_threaded_conv2d.h")");
  }

  if (HasThunkKind(aot_thunks->proto().thunk_sequence(),
                   xla::cpu::ThunkProto::kSortThunk)) {
    runtime_specific_includes.push_back(
        R"(#include "machina/xla/service/cpu/runtime_key_value_sort.h")");
  }

  if (HasThunkKind(aot_thunks->proto().thunk_sequence(),
                   xla::cpu::ThunkProto::kTopKThunk)) {
    runtime_specific_includes.push_back(
        R"(#include "machina/xla/service/cpu/runtime_topk.h")");
  }
  ...
  if (codegen_opts.use_xla_nanort_runtime) {
    ...
    runtime_specific_includes.push_back(
        R"(#include "machina/compiler/tf2xla/xla_compiled_cpu_function_thunks.h")");
...
absl::Status ExtendRewrites(
  ...
  const string include_hlo_profile_printer_data_proto =
      opts.gen_hlo_profile_printer_data
          ? R"(#include "machina/xla/service/hlo_profile_printer_data.pb.h")"
          : "";
...
absl::Status GenerateHeader(
  ...
  const string include_xla_data_proto =
      opts.gen_program_shape
          ? R"(#include "machina/xla/xla_data.pb.h")"
          : "";
...
{{INCLUDE_MACHINA_MACHINA_XLA_DATA_PROTO}}
{{RUNTIME_SPECIFIC_INCLUDES}}
...
class {{CLASS}} final : public machina::{{COMPUTATION_CLASS_BASE}} {
 public:
  ...
  static const ::int64_t ArgSize(::machina::int32 index) {
    return BufferInfos()[ArgIndexToBufferIndex()[index]].size();
  }

  // Returns static data used to create an XlaCompiledCpuFunction.
  static const machina::XlaCompiledCpuFunction::StaticData& StaticData() {
...
{{BUFFER_INFOS_AS_STRING}}
  ...
  static const ::machina::int32* ResultIndexToBufferIndex() {
    static constexpr ::machina::int32 kResultIndexToBufferIndex[kNumResults] = {
{{RESULT_INDEX_TABLE}}
    };
    return kResultIndexToBufferIndex;
  }

  static const ::machina::int32* ArgIndexToBufferIndex() {
    static constexpr ::machina::int32 kArgIndexToBufferIndex[kNumArgs] = {
{{ARG_INDEX_TABLE}}
    };
...
// clang-format on
  ...
  std::vector<std::pair<string, string>> rewrites = {
      ...
      {"{{INCLUDE_MACHINA_MACHINA_XLA_DATA_PROTO}}", include_xla_data_proto},
      {"{{METHODS_ARG}}\n", methods_arg},
      {"{{METHODS_RESULT}}\n", methods_result},
      {"{{METHODS_VARIABLE}}\n", methods_variable},
      {"{{NS_END}}\n", ns_end},
