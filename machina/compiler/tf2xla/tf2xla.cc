...
#include "machina/compiler/aot/aot_only_var_handle_op.h"
...
absl::Status ConvertGraphToXla(std::unique_ptr<Graph> graph,
  ...
  for (Node* node : graph->nodes()) {
    node->set_assigned_device_name(
        absl::StrCat("/device:", DEVICE_CPU_MACHINA_MACHINA_XLA_JIT));
  }
  ...
  compiler_options.device_type = DeviceType(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT);
