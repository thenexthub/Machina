...
static const char* const SUPPORTED_DEVICES[SUPPORTED_DEVICES_NUM] = {
    DEVICE_GPU_MACHINA_MACHINA_XLA_JIT, DEVICE_CPU_MACHINA_MACHINA_XLA_JIT};
...
absl::Status RegisterBackends(absl::string_view device_name) {
  ...
  if (!XlaOpRegistry::IsBackendRegistered(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT)) {
    static auto gpu_backend =
        XlaBackendRegistrar(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT, kGpuAllTypes, op_filter);
  }
  if (!XlaOpRegistry::IsBackendRegistered(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT)) {
    static auto cpu_backend =
        XlaBackendRegistrar(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT, kCpuAllTypes, op_filter);
  }
