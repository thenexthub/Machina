...
class SvdOp : public XlaOpKernel {
 public:
  ...
  void Compile(XlaOpKernelContext* ctx) override {
    ...
    int max_iter = 2 * machina::Log2Ceiling(std::max(m, n));
...
REGISTER_MACHINA_MACHINA_XLA_OP(Name("XlaSvd").TypeConstraint("T", kFloatTypes), XlaSvdOp);
REGISTER_MACHINA_MACHINA_XLA_OP(Name("Svd").TypeConstraint("T", kFloatTypes), SvdOp);
