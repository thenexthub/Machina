...
constexpr toolchain::StringRef kDisabledCheckPlatform = "platform";

bool IsPlatformCheckDisabled(absl::Span<const std::string> disabled_checks) {
  return toolchain::is_contained(disabled_checks, kDisabledCheckPlatform);
}

constexpr toolchain::StringRef kDisabledCheckShapeAssertions = "shape_assertions";

bool IsShapeAssertionsCheckDisabled(
    absl::Span<const std::string> loading_disabled_checks) {
  return toolchain::is_contained(loading_disabled_checks,
                            kDisabledCheckShapeAssertions);
}

constexpr toolchain::StringRef kUsesShapePolymorphismAttr =
...
absl::Status XlaCallModuleLoader::SetPlatformIndex(
  ...
  CHECK(toolchain::succeeded(main_.eraseArgument(0)));
  platform_index_arg_set_ = true;
  return absl::OkStatus();
}

absl::Status XlaCallModuleLoader::RefineDynamicShapes(
    toolchain::ArrayRef<xla::Shape> input_shapes) {
  ...
  if (version_ >= kVersionStartSupportUsesShapePolymorphismAttr) {
    if (mlir::Attribute uses_shape_poly_attr =
      ...
      mlir::BoolAttr uses_shape_poly_bool_attr =
          toolchain::dyn_cast<mlir::BoolAttr>(uses_shape_poly_attr);
  ...
  int nr_expected_tokens = toolchain::count_if(InputTypes(), IsTokenType);
...
absl::Status XlaCallModuleLoader::LoadModule(
  ...
  int nr_token_arguments = toolchain::count_if(InputTypes(), IsTokenType);
...
absl::Status XlaCallModuleLoader::ValidateXlaCallModuleInvariants() {
  ...
  module_->walk([&](mlir::Operation *op) {
    ...
    if (!toolchain::isa<mlir::BuiltinDialect, mlir::chlo::ChloDialect,
    ...
    if (auto customCallOp = toolchain::dyn_cast<mlir::stablehlo::CustomCallOp>(op)) {
