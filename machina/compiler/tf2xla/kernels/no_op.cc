...
// MACHINA_XLA* devices also register a "real" NoOp operator so we suppress the
// dummy operator using CompilationOnly().
REGISTER_MACHINA_XLAOP(Name("NoOp").CompilationOnly(), NoOp);

// We register ControlTrigger as a no-op. This is correct since nodes seen
// by the XLA compiler are never dead.
REGISTER_MACHINA_XLAOP(Name("ControlTrigger").CompilationOnly(), NoOp);
