...
class XlaCallModuleOp : public XlaOpKernel {
 public:
  explicit XlaCallModuleOp(OpKernelConstruction *ctx) : XlaOpKernel(ctx) {
    ...
    if (compilation_device_type == DEVICE_CPU_MACHINA_MACHINA_XLA_JIT) {
      compilation_platform_ = "CPU";
    } else if (compilation_device_type == DEVICE_GPU_MACHINA_MACHINA_XLA_JIT) {
#if GOOGLE_CUDA
...
#endif
    } else if (compilation_device_type == DEVICE_TPU_MACHINA_MACHINA_XLA_JIT) {
  ...
  void Compile(XlaOpKernelContext *ctx) override {
    ...
    if (VLOG_IS_ON(3)) {
      ...
      MACHINA_MACHINA_XLA_VLOG_LINES(3, absl::StrCat("XlaCallModule converted to HLO module ",
    ...
    for (auto it : toolchain::enumerate(output_types)) {
      int i = it.index();
 ...
 private:
  ...
  absl::Status LowerTfFunctionCalls(XlaOpKernelContext *ctx) {
    ...
    toolchain::SmallDenseSet<mlir::func::FuncOp> updated_funcs;
    ...
    auto lower =
      ...
      toolchain::SmallVector<mlir::Value> input_args;
      {
        toolchain::SmallVector<mlir::Value> args(custom_call->getOperands());
        if (custom_call_has_token_input_output) {
      ...
      {
        toolchain::SmallVector<mlir::Value> results(custom_call->getResults());
        if (custom_call_has_token_input_output) {
          // Adjust the indexes since custom calls with `has_token_input_output`
    ...
    for (auto func : updated_funcs) {
      auto ret = toolchain::cast<mlir::func::ReturnOp>(
...
REGISTER_MACHINA_MACHINA_XLA_OP(Name("XlaCallModule"), XlaCallModuleOp);
