...
absl::Status MlirXlaOpKernel::ContextToXlaArgs(
  ...
  TF_RETURN_IF_ERROR(machina::XlaOpRegistry::CompileTimeConstantInputs(
      *this, &registered_consts_vec));
  toolchain::SmallDenseSet<int, 4> registered_consts;
...
absl::Status MlirXlaOpKernel::ConstructXlaOp(XlaOpKernelContext* ctx) {
  ...
  // Create input XlaOps.
  toolchain::SmallVector<xla::XlaOp, 4> xla_params(ctx->num_inputs());
