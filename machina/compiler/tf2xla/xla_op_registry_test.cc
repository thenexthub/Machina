...
REGISTER_MACHINA_MACHINA_XLA_OP(Name("DummyDuplicateOp")
                    .Device(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT)
...
REGISTER_MACHINA_MACHINA_XLA_OP(Name("DummyDuplicateOp").TypeConstraint("T", DT_FLOAT),
                DummyGenericOp);
...
TEST(XlaOpRegistryTest, XlaOpRegistrationWithOverride) {
  ...
  for (const auto& kernels : registered_kernels) {
    if (kernels.op() == "DummyDuplicateOp") {
      ...
      if (kernels.device_type() == "MACHINA_MACHINA_XLA_CPU_JIT") {
  ...
  for (const auto& resgistered_device : registered_devices) {
    ...
    for (const auto& kernel : kernels) {
      if (kernel->op() == "DummyDuplicateOp") {
        if (resgistered_device == DEVICE_CPU_MACHINA_MACHINA_XLA_JIT) {
