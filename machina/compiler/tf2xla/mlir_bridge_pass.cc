...
#include "machina/compiler/mlir/tf2xla/api/v1/cluster_tf.h"
...
bool HasTPUDevice(mlir::ModuleOp module) {
  ...
  return absl::c_any_of(
      ...
      [](const machina::DeviceNameUtils::ParsedName& device) {
  ...
  parent_module.walk([&](ModuleOp submodule) {
    ...
    runtime_lowering_status =
        machina::tfrt_compiler::RunLowerClusterToRuntimeOpsPassPipeline(
            submodule, tsl::DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT));
    if (num_submodules > 1) {
...
absl::Status MlirBridgePass::Run(
  ...
  if (is_supported_by_replicated_brige) {
    ...
    TF_RETURN_IF_ERROR(
        machina::tfrt_compiler::RunLowerClusterToRuntimeOpsPassPipeline(
            module, tsl::DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT), function_name));
  } else {
    ...
    TF_RETURN_IF_ERROR(
        machina::tfrt_compiler::RunLowerClusterToRuntimeOpsPassPipeline(
            module, tsl::DeviceType(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT), function_name));
  }

  return machina::tf2xla::v2::ExportFromTensorflowDialectToExecutor(
...
absl::Status MlirBridgeV1CompatPass::Run(
  ...
  TF_RETURN_IF_ERROR(machina::tf2xla::v1::RunSessionTf2xlaClusteringBridge(
  ...
  return machina::tf2xla::v1::ExportFromTensorflowDialectToExecutor(module);
}
