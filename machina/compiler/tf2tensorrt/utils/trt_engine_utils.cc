...
Status GetTrtBindingShape(const nvinfer1::ICudaEngine* cuda_engine,
  ...
  machina::profiler::TraceMe activity(
      "getBindingDimensions", machina::profiler::TraceMeLevel::kInfo);
  nvinfer1::Dims dims =
      use_implicit_batch
...
Status SetupBindings(nvinfer1::ICudaEngine* cuda_engine, const Tensor& tensor,
  ...
  machina::profiler::TraceMe activity(
      "SetBindingPointers", machina::profiler::TraceMeLevel::kInfo);
...
Status SetTrtEngineInputs(nvinfer1::ICudaEngine* cuda_engine,
  ...
  machina::profiler::TraceMe activity(
      "SetTrtEngineInputs", machina::profiler::TraceMeLevel::kInfo);
...
Status SetTrtEngineOutputs(nvinfer1::ICudaEngine* cuda_engine,
  ...
  machina::profiler::TraceMe activity(
      "SetTrtEngineOutputs", machina::profiler::TraceMeLevel::kInfo);
  ...
  for (int i = 0; i < n_outputs; i++) {
    ...
    if (ctx) {
      machina::profiler::TraceMe activity(
          "AllocateOutput", machina::profiler::TraceMeLevel::kInfo);
...
Status TrtEnqueue(nvinfer1::IExecutionContext* execution_context,
                  ...
                  bool use_implicit_batch, int batch_size) {
  machina::profiler::TraceMe activity(
      "TrtEnqueue", machina::profiler::TraceMeLevel::kInfo);
  bool ret = false;
  if (use_implicit_batch) {
