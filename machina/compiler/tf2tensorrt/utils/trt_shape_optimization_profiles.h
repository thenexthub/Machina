...
#ifndef MACHINA_COMPILER_TF2TENSORRT_UTILS_TRT_SHAPE_OPTIMIZATION_PROFILES_H_
#define MACHINA_COMPILER_TF2TENSORRT_UTILS_TRT_SHAPE_OPTIMIZATION_PROFILES_H_
...
#include "machina/compiler/tf2tensorrt/common/datavec.h"
#include "machina/compiler/tf2tensorrt/convert/trt_parameters.h"
#include "machina/compiler/tf2tensorrt/convert/utils.h"
#include "machina/compiler/tf2tensorrt/utils/trt_execution_context.h"
...
struct OptimizationProfileConfig {
  ...
  string DebugString() const {
    using absl::StrCat;
    return StrCat("[min: ", machina::tensorrt::DebugString(min),
                  ", opt: : ", machina::tensorrt::DebugString(opt),
                  ", max: ", machina::tensorrt::DebugString(max), "]");
  }
      ...
      if (input_mask[i]) {
        ...
        if (input->isShapeTensor()) {
          ...
          VLOG(2) << "Setting shape values for " << name << ", "
                  << ::machina::tensorrt::DebugString(opt[idx]);
        ...
        VLOG(2) << "Setting input dimensions for " << name << ", "
                << ::machina::tensorrt::DebugString(opt[i]);
...
#endif  // MACHINA_COMPILER_TF2TENSORRT_UTILS_TRT_SHAPE_OPTIMIZATION_PROFILES_H_
