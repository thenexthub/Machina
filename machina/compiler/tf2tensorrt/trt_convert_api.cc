...
#include "machina/compiler/tf2tensorrt/trt_convert_api.h"
...
#include "machina/cc/tools/freeze_saved_model.h"
...
Status GetTrtRewriterConfig(const TfTrtConversionParams& params,
  ...
  opt_config->set_meta_optimizer_iterations(machina::RewriterConfig::ONE);
...
Status RunSession(Session* session, const std::vector<std::string>& input_names,
  ...
  std::vector<std::pair<std::string, machina::Tensor>> input_pairs;
  ...
  std::vector<machina::Tensor> output_tensors;
  for (int i = 0; i < output_names.size(); i++) {
...
Status Build(GraphDef& segmented_graph_def,
             const std::vector<std::string>& input_names,
             const std::vector<std::string>& output_names,
             const std::vector<std::vector<machina::Tensor>>& inputs,
...
machina::SessionOptions GetSessionConfg() {
  ...
  *meta_graph.mutable_graph_def() = frozen_graph_def;
  ...
  if (!inputs.empty() && conv_params.convert_to_static_engine) {
    ...
    std::unique_ptr<machina::Session> session(
        machina::NewSession(GetSessionConfg()));
    if (!session) {
...
std::vector<std::string> GetNodeNames(
    const google::protobuf::Map<std::string, machina::TensorInfo>& signature) {
...
StatusOr<GraphDef> ConvertAndBuild(
  ...
  *bundle->meta_graph_def.mutable_graph_def() = inlined_graph_def;
  ...
  const machina::SignatureDef& signature = signature_map[signature_key];
