...
#include "machina/compiler/tf2tensorrt/utils/trt_allocator.h"
#include "machina/compiler/tf2tensorrt/utils/trt_engine_utils.h"
#include "machina/compiler/tf2tensorrt/utils/trt_logger.h"
  ...
  Status AllocateDeviceMemory(nvinfer1::IExecutionContext* execution_context,
    ...
    }
    {
      machina::profiler::TraceMe activity(
          "setDeviceMemory", machina::profiler::TraceMeLevel::kInfo);
...
class TRTEngineOp : public AsyncOpKernel {
 ...
 private:
  ...
  machina::grappler::Cluster* cluster_;
...
TRTEngineOp::TRTEngineOp(OpKernelConstruction* context)
  ...
  machina::profiler::TraceMe activity(
      "TRTEngineOp::TRTEngineOp", machina::profiler::TraceMeLevel::kInfo);
  ...
  if (status.code() == machina::error::NOT_FOUND) {
...
void TRTEngineOp::ComputeAsync(OpKernelContext* ctx,
  ...
  machina::profiler::TraceMe activity(
      "TRTEngineOp::ComputeAsync", machina::profiler::TraceMeLevel::kInfo);
...
StatusOr<TrtUniquePtrType<nvinfer1::ICudaEngine>> TRTEngineOp::BuildEngine(
    ...
    TRTEngineCacheResource* cache_resource, OpKernelContext* ctx) {
  machina::profiler::TraceMe activity(
      "TRTEngineOp::BuildEngine", machina::profiler::TraceMeLevel::kInfo);
  ...
  std::unordered_map<string, machina::DeviceProperties> device_map;
...
StatusOr<std::pair<EngineContext*, int>> TRTEngineOp::GetEngine(
    ...
    TRTEngineCacheResource* cache_res) {
  machina::profiler::TraceMe activity(
      "TRTEngineOp::GetEngine", machina::profiler::TraceMeLevel::kInfo);
...
Status TRTEngineOp::AllocateCalibrationResources(
  ...
  const int platform_device_id =
      ctx->device()->machina_accelerator_device_info()->gpu_id;
