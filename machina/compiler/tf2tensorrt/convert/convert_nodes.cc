...
#include "machina/compiler/tf2tensorrt/common/utils.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/algorithm_selector.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/op_converter_registry.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/ops/layer_utils.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/ops/quantization_ops.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/ops/slice_ops.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/timing_cache.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/convert/utils.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/utils/trt_experimental_features.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/utils/trt_logger.h"  // IWYU pragma: keep
#include "machina/compiler/tf2tensorrt/utils/trt_shape_optimization_profiles.h"  // IWYU pragma: keep
#include "machina/core/common_runtime/graph_constructor.h"  // IWYU pragma: keep
#include "machina/core/framework/node_def.pb.h"  // IWYU pragma: keep
#include "machina/core/framework/node_def_builder.h"  // IWYU pragma: keep
#include "machina/core/framework/tensor.pb.h"  // IWYU pragma: keep
#include "machina/core/framework/tensor_shape.h"  // IWYU pragma: keep
#include "machina/core/framework/tensor_shape.pb.h"  // IWYU pragma: keep
#include "machina/core/framework/tensor_util.h"  // IWYU pragma: keep
#include "machina/core/framework/types.h"  // IWYU pragma: keep
#include "machina/core/graph/algorithm.h"  // IWYU pragma: keep
#include "machina/core/graph/graph.h"      // IWYU pragma: keep
#include "machina/core/grappler/grappler_item.h"  // IWYU pragma: keep
#include "machina/core/grappler/op_types.h"  // IWYU pragma: keep
#include "machina/core/grappler/optimizers/constant_folding.h"  // IWYU pragma: keep
#include "machina/core/grappler/optimizers/generic_layout_optimizer.h"  // IWYU pragma: keep
#include "machina/core/lib/core/errors.h"  // IWYU pragma: keep
#include "machina/core/lib/core/status.h"  // IWYU pragma: keep
#include "machina/core/lib/strings/numbers.h"  // IWYU pragma: keep
#include "machina/core/lib/strings/str_util.h"  // IWYU pragma: keep
#include "machina/core/lib/strings/strcat.h"  // IWYU pragma: keep
#include "machina/core/platform/logging.h"   // IWYU pragma: keep
#include "machina/core/platform/mutex.h"     // IWYU pragma: keep
#include "machina/core/platform/protobuf.h"  // IWYU pragma: keep
#include "machina/core/platform/tensor_coding.h"  // IWYU pragma: keep
#include "machina/core/platform/tensor_float_32_utils.h"  // IWYU pragma: keep
#include "machina/core/platform/types.h"  // IWYU pragma: keep
#include "machina/core/profiler/lib/annotated_traceme.h"  // IWYU pragma: keep
#include "machina/core/profiler/lib/traceme.h"  // IWYU pragma: keep
#include "machina/core/public/release_version.h"  // IWYU pragma: keep
#include "machina/core/util/env_var.h"  // IWYU pragma: keep
#include "machina/core/util/strided_slice_op.h"  // IWYU pragma: keep
...
Status Converter::BuildCudaEngine(
  ...
  machina::profiler::AnnotatedTraceMe activity(
      [&]() {
        return machina::profiler::TraceMeOpOverride("TRTEngineOp",
                                                       "BuildEngine");
      },
      machina::profiler::TraceMeLevel::kInfo);
  ...
  static SparseComputeMode sparse_compute_mode = []() {
    ...
    TF_CHECK_OK(machina::ReadInt64FromEnvVar("TF_TRT_SPARSE_MODE",
                                                /*default_val=*/1,
                                                &_sparse_mode));
  ...
  if (machina::tensor_float_32_execution_enabled()) {
...
Status ConvertGraphDefToEngine(
    ...
    bool use_explicit_precision, machina::grappler::Cluster* cluster,
    const string& device) {
  engine->reset();
  ...
  if (cluster != nullptr) {
    ...
    if (apply_layout_optim) {
      machina::grappler::GrapplerItem grappler_item;
      ...
      machina::grappler::GenericLayoutOptimizer layout_optimizer("NCHW");
      TF_RETURN_IF_ERROR(
          layout_optimizer.Optimize(cluster, grappler_item, &graph));

      grappler_item.graph = graph;

      machina::grappler::ConstantFolding const_optimizer(
          nullptr,
  ...
  for (const auto& node_def : graph.node()) {
    ...
    if (IsEngineInput(node_name)) {
      ...
      if (node_def.op() == "Placeholder") {
        ...
        type_key = "dtype";
      } else if (machina::grappler::IsArg(node_def)) {
      ...
      } else if (machina::grappler::IsRetval(node_def)) {
        slot_number = node_def.attr().at("index").i();
      } else {
        return errors::InvalidArgument(
...
Status ConvertSegmentToGraphDef(
  ...
  machina::profiler::TraceMe activity(
      "ConvertSegmentToGraphDef", machina::profiler::TraceMeLevel::kInfo);
