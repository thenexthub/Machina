...
#include "machina/compiler/tf2tensorrt/convert/convert_graph.h"
...
#include "machina/compiler/tf2tensorrt/convert/logger_registry.h"
#include "machina/compiler/tf2tensorrt/convert/ops/quantization_ops.h"
#include "machina/compiler/tf2tensorrt/convert/utils.h"
#include "machina/compiler/tf2tensorrt/segment/segment.h"
...
#include "machina/core/protobuf/config.pb.h"  // NOLINT
#include "machina/core/protobuf/device_properties.pb.h"  // NOLINT
#include "machina/core/protobuf/rewriter_config.pb.h"  // NOLINT
...
using absl::StrCat;
using ::machina::tensorrt::segment::ClusterProperty;
using ::machina::tensorrt::segment::NodePtrCompare;
using ::machina::tensorrt::segment::Segment;
...
machina::TensorShapeProto ComputeTRTNodeIOShape(
    std::vector<PartialTensorShape>& partial_tensorshape_vect,
    std::vector<machina::TensorShapeProto>& shape_proto_vect,
    const PartialTensorShape& conn_shape, int port_number) {
  machina::TensorShapeProto tmp_shape_proto;
...
Status CreateTRTNode(const TRTOptimizationPass::ConversionParams& params,
  ...
  std::vector<machina::TensorShapeProto> input_shape_protos;
  std::vector<machina::TensorShapeProto> output_shape_protos;
  ...
  for (const auto& conn : info.connections) {
    ...
    if (conn.is_control_edge()) {
      ...
      if (!conn.is_input_edge) {
        ...
        machina::TensorShapeProto out_shape = ComputeTRTNodeIOShape(
        ...
        machina::TensorShapeProto in_shape = ComputeTRTNodeIOShape(
...
std::pair<int, Allocator*> GetDeviceAndAllocator(
  ...
  if (!devices.empty()) {
    ...
    cuda_device_id = devices[0]->machina_accelerator_device_info()->gpu_id;
