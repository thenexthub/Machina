...
class ZetaTest(xla_test.XLATestCase, parameterized.TestCase):
  ...
  def testLargeXSmallQ(self, dtype, rtol, atol):
    rtol, atol = self.adjust_tolerance_for_tpu(dtype, rtol, atol)
    if self.device not in ['MACHINA_MACHINA_XLA_GPU', 'MACHINA_MACHINA_XLA_CPU'] and dtype == np.float64:
...
if __name__ == '__main__':
  os.environ['MACHINA_MACHINA_XLA_FLAGS'] = '--xla_cpu_enable_fast_math=false'
  test.main()
