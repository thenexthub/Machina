...
TEST_F(OpTest, AvgPool3D) {
  if (machina::tf_xla_test_use_mlir) GTEST_SKIP() << "b/201095155";
  if (!machina::tf_xla_test_use_mlir) GTEST_SKIP() << "b/197140886";
  Repeatedly([this]() {
    std::uniform_int_distribution<int> random_int(1, 5);
...
int main(int argc, char** argv) {
  machina::tf_xla_test_device_ptr = new machina::string("GPU:0");
  machina::tf_xla_reference_device_ptr = new machina::string("CPU:0");
  std::vector<machina::Flag> flag_list = {
      machina::Flag(
          "tf_xla_random_seed", &machina::tf_xla_random_seed,
      ...
      machina::Flag("tf_xla_test_repetitions",
                       &machina::tf_xla_test_repetitions,
                       "Number of repetitions for each test."),
      machina::Flag("tf_xla_max_tensor_size",
                       &machina::tf_xla_max_tensor_size,
                       "Maximum number of elements for random input tensors."),
      machina::Flag("tf_xla_test_device", machina::tf_xla_test_device_ptr,
                       "Tensorflow device type to use for test"),
      machina::Flag("tf_xla_reference_device",
                       machina::tf_xla_reference_device_ptr,
                       "Tensorflow device type to use for reference"),
      machina::Flag("tf_xla_test_use_jit", &machina::tf_xla_test_use_jit,
                       "Use JIT compilation for the operator under test"),
      machina::Flag(
          "tf_xla_test_use_mlir", &machina::tf_xla_test_use_mlir,
  ...
  TF_CHECK_OK(machina::DeviceFactory::AddDevices(
      machina::SessionOptions(), "", &devices));
  machina::StaticDeviceMgr device_mgr(std::move(devices));

  machina::Device* ignored;
  TF_QCHECK_OK(
      device_mgr.LookupDevice(*machina::tf_xla_test_device_ptr, &ignored))
      << "Unknown test device (" << *machina::tf_xla_test_device_ptr
      << "). Did you build in the right configuration (e.g., is CUDA enabled)?";

  if (machina::tf_xla_test_use_mlir)
