...
flags.DEFINE_string('tf_xla_flags', None,
                    'Value to set the TF_MACHINA_MACHINA_XLA_FLAGS environment variable to')
...
class TPURewriteSession(session.Session):
  ...
  def run(self, fetches, feed_dict=None, options=None, run_metadata=None):
    from machina.python.tpu import tpu  # pylint: disable=g-import-not-at-top
...
class XLATestCase(test.TestCase):
  ...
  def __init__(self, method_name='runTest'):
    ...
    self.has_custom_call = (self.device == 'MACHINA_MACHINA_XLA_CPU')
    ...
    if FLAGS.tf_xla_flags is not None:
      os.environ['TF_MACHINA_MACHINA_XLA_FLAGS'] = FLAGS.tf_xla_flags
...
def Benchmark(tf_bench,
  ...
  Args:
    ...
    device: The machina device to run on, e.g. "cpu", "gpu".
