...
load("//machina:machina.bzl", "py_test")
load("//machina/compiler/tests:plugin.bzl", "plugins")
load(
...
def tf_xla_py_test(
    ...
    Args:
      ...
      disabled_backends: A list of backends that should not be tested. Supported
        values include "cpu" and "gpu". If not specified, defaults to None.
      use_xla_device: If true then the --test_device argument is set to MACHINA_MACHINA_XLA_CPU
        and MACHINA_MACHINA_XLA_GPU for the CPU and GPU tests.  Otherwise it is set to CPU and
        GPU.
    ...
    if use_xla_device:
        cpu_xla_device = "MACHINA_MACHINA_XLA_CPU"
        gpu_xla_device = "MACHINA_MACHINA_XLA_GPU"
    else:
        cpu_xla_device = "CPU"
        gpu_xla_device = "GPU"
    ...
    for backend in backends:
        ...
        for mlir_option in enable_mlir_bridge_options:
            ...
            mlir_bridge_dep = "//machina/python/framework:is_mlir_bridge_test_true"
