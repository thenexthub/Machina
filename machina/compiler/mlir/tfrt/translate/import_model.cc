...
#include "machina/compiler/mlir/tf2xla/api/v2/cluster_tf.h"
#include "machina/compiler/mlir/tf2xla/api/v2/tf_dialect_to_executor.h"
...
absl::Status ConvertTfMlirToRuntimeExecutable(
  ...
  if (options.backend_compiler != nullptr) {
    if (VLOG_IS_ON(1)) {
      machina::DumpMlirOpToFile("tf_dialect_before_backend_compile", module);
    }
    TF_RETURN_IF_ERROR(
    ...
    if (VLOG_IS_ON(1)) {
      machina::DumpMlirOpToFile("tpu_bct_conversion_before", module);
    }
    ...
    auto backward_compat_result =
        machina::RunTPUBackwardCompatConversion(module, tpu_compile_options);
    ...
    if (VLOG_IS_ON(1)) {
      machina::DumpMlirOpToFile("tpu_bct_conversion_after", module);
    }

    TF_RETURN_IF_ERROR(
        machina::tf2xla::v2::RunFunctionTf2xlaClusteringBridge(
            module, /*is_supported_by_replicated_brige*/ true,
            /*is_in_fallback_enabled_mode=*/VLOG_IS_ON(1)));

    TF_RETURN_IF_ERROR(
        machina::tfrt_compiler::RunLowerClusterToRuntimeOpsPassPipeline(
            module, tsl::DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT)));

    TF_RETURN_IF_ERROR(
        machina::tf2xla::v2::ExportFromTensorflowDialectToExecutor(module));
  } else if (options.device_target == TfrtDeviceInfraTarget::kTfFallback) {
    auto tpu_partitioned_call_fallback_compat_result =
        machina::RunTPUPartitionedCallFallbackCompatConversion(module);
    ...
    TF_RETURN_IF_ERROR(
        machina::tfrt_compiler::RunLowerClusterToRuntimeOpsPassPipeline(
            module, tsl::DeviceType(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT)));
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("tf_dialect", module);
  }
  ...
  TF_RETURN_IF_ERROR(
      machina::CreateTFExecutorToTFPreInvariantOptimizationPipeline(
          pm, *pipeline_options));

  auto status = emit_executable(pm, module, *pipeline_options);

  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("tfrt_dialect", module);
  }
...
absl::Status ConvertTfMlirToBef(
  ...
  return ConvertTfMlirToRuntimeExecutable(
      ...
      [bef_buffer](mlir::PassManager& pm, mlir::ModuleOp module,
                   const machina::TfrtPipelineOptions& options) {
        mlir::StatusScopedDiagnosticHandler diag_handler(module.getContext());
        machina::CreateTFExecutorToTFInvariantOptimizationPipelineHelper(
            pm, options);
        machina::CreateTfToTfrtPipeline(pm, options);

        if (mlir::failed(pm.run(module))) {
          if (VLOG_IS_ON(1)) {
            machina::DumpMlirOpToFile("tf_to_corert_failure", module);
          }
          return diag_handler.Combine(absl::InternalError(
...
std::unique_ptr<machina::TfrtPipelineOptions> GetTfrtPipelineOptions(
  ...
  auto pipeline_options = std::make_unique<machina::TfrtPipelineOptions>();
  ...
  pipeline_options->allowed_batch_sizes = toolchain::ArrayRef<int64_t>(
