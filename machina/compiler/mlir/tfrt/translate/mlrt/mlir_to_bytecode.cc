 ...
 *
 */
#include "machina/compiler/mlir/tfrt/translate/mlrt/mlir_to_bytecode.h"
...
std::optional<std::string> EncodeListOfInteger(mlir::ArrayAttr array) {
  ...
  for (int i = 0; i < array.size(); ++i) {
    if (auto integer_attr = mlir::dyn_cast<mlir::IntegerAttr>(array[i])) {
      ...
      toolchain::APInt value = integer_attr.getValue();
...
template <typename T>
std::optional<std::string> EncodeDenseArray(toolchain::ArrayRef<T> array) {
  bc::Buffer buffer;
  bc::Allocator allocator(&buffer);
  auto ctor = bc::New<bc::Vector<T>>(&allocator, array.size());
...
std::optional<std::string> EncodeDenseBoolArray(toolchain::ArrayRef<bool> array) {
  bc::Buffer buffer;
  bc::Allocator allocator(&buffer);
...
struct FunctionEmitterContext {
  ...
  int next_reg_id = 0;
  toolchain::DenseMap<mlir::Value, RegInfo> register_table;
...
void EmitKernel(FunctionEmitterContext& function_context,
  ...
  } else if (toolchain::isa<mlir::func::CallOp>(&op)) {
    constructor.set_code(function_context.module_context.GetKernelId("call"));
  } else {
    toolchain::StringRef op_name = op.getName().getStringRef();
...
void EmitFunction(const ModuleEmitterContext& module_context,
                  bc::Function::Constructor& constructor, toolchain::StringRef name,
                  mlir::Region& region) {
  FunctionEmitterContext function_context(&module_context);

  constructor.construct_name(name.str());

  DCHECK(toolchain::hasSingleElement(region)) << "should have a single block";
  ...
  for (const auto& iter : toolchain::enumerate(block.getOperations())) {
    int i = iter.index();
...
std::optional<std::string> EncodeSimpleAttribute(
  ...
  return toolchain::TypeSwitch<mlir::Attribute, std::optional<std::string>>(attr)
      ...
      .Case<mlir::IntegerAttr>(
          [](const auto& integer_attr) -> std::optional<std::string> {
            switch (toolchain::APInt value = integer_attr.getValue();
                    value.getBitWidth()) {
              case 1:
      ...
      .Case<mlir::FloatAttr>(
          [](const auto& float_attr) -> std::optional<std::string> {
            toolchain::APFloat value = float_attr.getValue();
...
absl::StatusOr<std::string> ModuleEmitterContext::DefaultEncodeAttribute(
  ...
  std ::string attr_str;
  toolchain::raw_string_ostream os(attr_str);
  attr.print(os);
