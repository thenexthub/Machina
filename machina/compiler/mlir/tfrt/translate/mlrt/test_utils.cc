 ...
 *
 */
#include "machina/compiler/mlir/tfrt/translate/mlrt/test_utils.h"
...
absl::StatusOr<std::string> EncodeAttribute(const machina::AttrValue& attr) {
  ...
  if (attr.has_tensor()) {
    ...
    auto tensor_attr_ctor = mlrt::bc::New<machina::tf_mlrt::TensorAttr>(
...
bool CanBeInlined(const machina::AttrValue& attr) {
...
absl::Status EncodeAttributes(AttributeTable& attributes,
  ...
  std::vector<std::pair<std::string, machina::AttrValue>> attrs(
      attr_map.begin(), attr_map.end());
  std::sort(attrs.begin(), attrs.end(),
            [](const auto& x, const auto& y) { return x.first < y.first; });

  for (int i = 0; i < attrs.size(); ++i) {
    const machina::AttrValue& attr = attrs[i].second;
