...
toolchain::StringRef ProcessIndexPath(mlir::ArrayAttr index_path) {
...
absl::StatusOr<std::pair<machina::DataType, machina::PartialTensorShape>>
ProcessTensorSpec(mlir::TensorType type) {
  ...
  if (!type.hasRank())
    return std::make_pair(dtype, machina::PartialTensorShape());

  auto shape = type.getShape();
  toolchain::SmallVector<int64_t, 4> dims;
  dims.assign(shape.begin(), shape.end());
  return std::make_pair(dtype, machina::PartialTensorShape(dims));
}
...
absl::Status MapFunctionSignaturesFromTFSavedModelMLIR(
  ...
  module.walk([&symbol_table, map_fn, &status](mlir::func::FuncOp func) {
    ...
    toolchain::SmallVector<toolchain::StringRef, 4> input_names;
    toolchain::SmallVector<
        std::pair<machina::DataType, machina::PartialTensorShape>, 4>
        input_specs;
    toolchain::SmallVector<toolchain::StringRef, 4> input_devices;
    toolchain::SmallVector<mlir::Operation*, 4> bound_inputs;
    ...
    toolchain::SmallVector<toolchain::StringRef, 4> output_names;
    toolchain::SmallVector<
        std::pair<machina::DataType, machina::PartialTensorShape>, 4>
        output_specs;
