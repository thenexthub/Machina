...
void RecursivelyMoveOp(mlir::TF::_TPUCompileMlirOp compile_op,
                       ...
                       toolchain::SmallDenseSet<mlir::Operation *> *ops_to_move) {
...
mlir::Value FindFullShapedOperand(mlir::TF::SplitOp split_op) {
  ...
  if (auto parent_split_op = toolchain::dyn_cast<mlir::TF::SplitOp>(defining_op))
...
bool MaybeFindUsedExecuteOp(
    const toolchain::SmallVector<mlir::TF::TPUExecuteOp, 4> &exec_op_in_group,
...
void FuseCompileAndExecuteOps(
    ...
    toolchain::SmallDenseMap<
        ...
        toolchain::SmallDenseMap<int, mlir::TF::SetStaticDimensionBoundsOp>>
  ...
  toolchain::SmallVector<int> static_shaped_operand_indices_attr;
  toolchain::SmallVector<mlir::Value> static_shape_tensors;
  toolchain::SmallVector<mlir::Value> exec_op_args;
  ...
  for (int i = 0; i < used_exec_op.getArgs().size(); ++i) {
    ...
    if (iter != static_shaped_operands.end()) {
      ...
      auto split_op = ::toolchain::dyn_cast_or_null<mlir::TF::SplitOp>(
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    func.walk([&](mlir::Operation *op) {
      if (toolchain::isa<mlir::TF::TPUCompileSucceededAssertOp>(op)) {
        op->erase();
      }
    });
    ...
    toolchain::SmallVector<mlir::TF::TPUExecuteOp, 4> tpu_execute_ops;
    func.walk([&](mlir::Operation *op) {
      if (auto exec_op = toolchain::dyn_cast<mlir::TF::TPUExecuteOp>(op)) {
        ...
        for (const auto &operand : toolchain::enumerate(exec_op.getOperands())) {
          if (auto defining_op =
                  operand.value()
    ...
    toolchain::SmallDenseMap<mlir::TF::_TPUCompileMlirOp,
                        toolchain::SmallVector<mlir::TF::TPUExecuteOp, 4>>
        compile_and_execute_groups;

    for (auto exec_op : tpu_execute_ops) {
      auto compile_cache_entry = exec_op.getKey();
      auto compile_op = ::toolchain::dyn_cast<mlir::TF::_TPUCompileMlirOp>(
