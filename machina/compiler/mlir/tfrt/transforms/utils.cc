...
bool IsResourceArgument(mlir::Value value) {
  ...
  auto func = toolchain::cast<mlir::func::FuncOp>(arg.getOwner()->getParentOp());
...
bool IsResultVariable(const mlir::Value &original_operand,
  ...
  if (mlir::isa<mlir::OpResult>(original_operand)) {
    ...
    if (toolchain::isa<mlir::TF::ReadVariableOp>(defining_op) &&
        defining_op->getNumOperands() == 1) {
      return true;
    } else if (toolchain::isa<mlir::TF::_TfrtGetResourceOp>(defining_op)) {
      return true;
    }
    return false;
  }
