 ...
 *
 */
#include "machina/compiler/mlir/tfrt/transforms/mlrt/tf_to_mlrt.h"
...
#include "mlir/Dialect/Func/Transforms/FuncConversions.h"  // part of Codira Toolchain
...
#include "machina/compiler/mlir/tfrt/ir/mlrt/mlrt_ops.h"
...
#include "machina/compiler/mlir/tfrt/transforms/mlrt/tpu_conversion_patterns.h"
...
mlir::Value CreateCustomDevice(mlir::Location loc, toolchain::StringRef device_name,
...
class FuncOpSignatureConversion final
 ...
 public:
  explicit FuncOpSignatureConversion(
      ...
      const toolchain::DenseMap<toolchain::StringRef, toolchain::SmallVector<mlir::Type>>
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    const toolchain::SmallVector<mlir::Type> &call_site_input_types = it->second;
    ...
    for (const auto &[index, value] : toolchain::enumerate(call_site_input_types)) {
...
class TFPromiseOpConversion final
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
      ...
      mlir::ConversionPatternRewriter &rewriter) const override {
    if (toolchain::isa<::mlrt::compiler::FutureType>(
...
class ExecuteOpConversion final : public mlir::ConversionPattern {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
      mlir::Operation *op, toolchain::ArrayRef<mlir::Value> operands,
      mlir::ConversionPatternRewriter &rewriter) const override {
    // TODO(b/173017701): Avoid fallback for ops within XLA GPU clusters.
    if (!UseFallback(op)) return mlir::failure();

    if (auto const_op = toolchain::dyn_cast<mlir::TF::ConstOp>(op)) {
    ...
    if (!op->hasAttr(machina::tfrt_compiler::kOpKeyAttrName))
    ...
    uint32_t execute_key = op->getAttrOfType<mlir::IntegerAttr>(
                                 machina::tfrt_compiler::kOpKeyAttrName)
                               .getInt();

    absl::StrAppend(&node_name, "_", execute_key);

    auto statusor_node_def = machina::ConvertTFDialectOpToNodeDef(
...
class WhileOpConversion : public mlir::OpConversionPattern<mlir::TF::WhileOp> {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<mlir::Type, 4> while_result_types(
...
void CreateFallbackInitializationFunction(
  ...
  for (const auto &[op_index, op] :
       toolchain::enumerate(execute_op_registry.GetExecuteOps())) {
    if (op) {
...
void MoveAwaitOpToFirstUse(mlir::Block &block) {
  toolchain::SmallVector<tf_mlrt::AwaitOp> await_ops;
  for (auto &op : block) {
    if (auto await_op = toolchain::dyn_cast<tf_mlrt::AwaitOp>(&op)) {
 ...
 private:
  ...
  mlir::LogicalResult runOnFunction(mlir::func::FuncOp func) {
    ...
    target.addLegalDialect<mlrt::compiler::MlrtDialect,
                           machina::tf_mlrt::TensorflowMlrtDialect,
  ...
  void maySetTpuHostAllocatorForBatch(mlir::TF::BatchFunctionOp batch_op,
                                      ...
                                      mlir::SymbolTable &symbol_table) {
    mlir::func::FuncOp batched = toolchain::dyn_cast<mlir::func::FuncOp>(
    ...
    for (auto arg : batched.getArguments()) {
      ...
      for (auto user : arg.getUsers()) {
        if (toolchain::isa<mlir::TF::TPUCompileMlirAndExecuteOp>(user)) {
          used_by_tpu++;
          break;
        }
      }
...
class TfToMlrtConversionPass
 ...
 private:
  ...
  toolchain::StringRef getArgument() const final { return "tf-to-mlrt"; }
  toolchain::StringRef getDescription() const final { return "tf-to-mlrt"; }
  ...
  void runOnOperation() override {
    ...
    // Use toolchain::make_early_inc_range instead of the stock range from
  ...
  void AddAwaitOpToUnusedFutures(mlir::ModuleOp module) {
    for (auto func : module.getOps<mlir::func::FuncOp>()) {
      toolchain::SmallVector<mlir::Value> unused_futures;

      auto is_unused_future = [](mlir::Value result) {
        return toolchain::isa<::mlrt::compiler::FutureType>(result.getType()) &&
               result.use_empty();
      };
    ...
    target.addDynamicallyLegalOp<mlir::func::FuncOp>(
        [this](mlir::func::FuncOp func) {
          ...
          for (auto [expected_input_type, call_site_type] :
               toolchain::zip(func.getFunctionType().getInputs(),
  ...
  void CollectFunctionCallSiteInputTypes(mlir::func::FuncOp func) {
    func.walk([&function_call_site_input_types =
      ...
      if (auto async_op = toolchain::dyn_cast<mlrt::compiler::AsyncOp>(op)) {
        function_call_site_input_types[async_op.getCallee()
                                           .getLeafReference()] =
            toolchain::SmallVector<mlir::Type>(async_op.getOperandTypes().begin(),
  ...
  toolchain::DenseMap<toolchain::StringRef, toolchain::SmallVector<mlir::Type>>
      function_call_site_input_types_;
};
