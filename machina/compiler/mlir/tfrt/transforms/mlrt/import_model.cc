  ...
  TF_RETURN_IF_ERROR(ConvertTfMlirToRuntimeExecutable(
      ...
      [&bytecode_buffer, &fallback_state, &model_context,
       ...
       module_with_op_keys](mlir::PassManager& pm, mlir::ModuleOp module,
        ...
        if (VLOG_IS_ON(1)) {
          machina::DumpMlirOpToFile("tf_dialect_after_assign_op_key",
                                       module);
        }
        // Save the module.
        ...
        registry.Register("tf_mlrt",
                          &machina::tf_mlrt::EncodeTensorflowAttribute);
...
absl::StatusOr<mlrt::bc::Buffer> ConvertTfMlirWithOpKeysToBytecode(
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("tf_dialect_with_op_keys",
                                 module_with_op_keys);
  }
  ...
  // Generate bytecode.
  mlrt::AttributeEncoderRegistry registry;
  registry.Register("tf_mlrt", &machina::tf_mlrt::EncodeTensorflowAttribute);
  auto statusor = mlrt::EmitExecutable(registry, module_with_op_keys);
  if (!statusor.ok()) return statusor.status();
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("tfrt_dialect_from_tf_dialect_with_op_keys",
                                 module_with_op_keys);
  }
