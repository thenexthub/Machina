 ...
 *
 */
#include "machina/compiler/mlir/tfrt/transforms/mlrt/fuse_mlrt_ops.h"
...
class FuseMlrtOpPass
 ...
 private:
  toolchain::StringRef getArgument() const final { return "tf-mlrt-fuse"; }
...
void FuseGetResourceOps(mlir::OpBuilder& builder, mlir::Block& block) {
  toolchain::SmallVector<tf_mlrt::GetResourceOp> get_resource_ops;
  for (auto& op : toolchain::make_early_inc_range(block)) {
    if (auto get_resource_op = toolchain::dyn_cast<tf_mlrt::GetResourceOp>(&op)) {
  ...
  toolchain::SmallVector<mlir::Attribute> indices;
  toolchain::SmallVector<mlir::Type> result_types;
  toolchain::SmallVector<mlir::Value> old_values;
  ...
  for (auto [old_value, new_value] :
       toolchain::zip(old_values, new_op.getResults())) {
...
void FuseAwaitOps(mlir::OpBuilder& builder, mlir::Block& block) {
  toolchain::SmallVector<AwaitOpType> await_ops;
  for (auto& op : toolchain::make_early_inc_range(block)) {
    if (auto await_op = toolchain::dyn_cast<AwaitOpType>(&op)) {
    ...
    if (await_ops.size() > 1) {
      ...
      toolchain::SmallVector<mlir::Value> futures;
      ...
      if constexpr (!std::is_same_v<ValueType, void>) {
        for (auto [await_op, new_value] :
             toolchain::zip(await_ops, await_all.getResults())) {
...
void FusePromiseReturn(mlir::OpBuilder& builder, mlir::Block& block) {
  ...
  auto return_op = toolchain::dyn_cast<mlir::func::ReturnOp>(terminator);
  if (!return_op || return_op->getNumOperands() > 0) return;

  auto promise_op =
      toolchain::dyn_cast_or_null<tf_mlrt::PromiseOp>(return_op->getPrevNode());
