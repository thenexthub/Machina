 ...
 *
 */
#include "machina/compiler/mlir/tfrt/transforms/mlrt/parallelization.h"
...
using machina::tfrt_compiler::CostAnalysis;
...
bool IsConstant(mlir::Operation* op) {
  return op && toolchain::isa<mlir::TF::ConstOp, mlir::TF::_TfrtGetResourceOp>(op);
}
...
toolchain::DenseMap<const Stream*, StreamInfo> PreprocessStreamInfo(
    mlir::Block& block,
    const toolchain::DenseMap<mlir::Operation*,
                         toolchain::SmallSetVector<mlir::Operation*, 4>>&
        control_predecessors,
    const StreamAnalysis& stream_analysis) {
  toolchain::DenseMap<const Stream*, StreamInfo> stream_map;

  // All values that will be promises in the block.
  toolchain::DenseSet<mlir::Value> promises;

  // All operations that will be control promises in the block.
  toolchain::DenseSet<mlir::Operation*> control_promises;

  // Keep track of all available values and controls as we traverse the stream
  // tree in depth-first order.
  toolchain::DenseSet<mlir::Value> available_values;
  toolchain::DenseSet<mlir::Operation*> available_controls;
  ...
  for (auto& op : toolchain::reverse(block)) {
...
struct Mapping {
  ...
  toolchain::DenseMap<mlir::Operation*, mlir::Value> future_control_mapping;
  toolchain::DenseMap<mlir::Operation*, mlir::Value> promise_control_mapping;
};

mlrt::compiler::AsyncOp CreateAsyncOp(
    mlir::OpBuilder& builder, absl::string_view function_name,
    const toolchain::DenseMap<const Stream*, StreamInfo>& stream_map,
  ...
  toolchain::SmallVector<mlir::Value> async_operands;
...
mlir::func::FuncOp CreateStreamFunction(
  ...
  toolchain::SmallVector<mlir::Type> arg_types;
...
void CreateAllocateFuturesOp(mlir::OpBuilder& builder, Mapping& mapping,
  ...
  toolchain::SmallVector<mlir::Type> promise_types(
      stream_info.promises.size(),
      builder.getType<mlrt::compiler::PromiseType>());
  toolchain::SmallVector<mlir::Type> future_types(
...
bool SkipControlDep(mlir::Operation* op) {
  ...
  return toolchain::isa<mlir::TF::TPUCompileMlirAndExecuteOp, mlir::TF::AssertOp>(
      op);
}
...
void ParallelizeBlock(
  ...
  toolchain::DenseMap<mlir::Operation*, toolchain::SmallSetVector<mlir::Operation*, 4>>
      control_predecessors;
  for (auto& op : block) {
  ...
  CostAnalysis cost_analysis(
      toolchain::cast<mlir::func::FuncOp>(block.getParentOp()), cost_recorder);
  ...
  toolchain::SmallVector<mlir::Operation*> to_remove;
  ...
  while (!stack.empty()) {
    ...
    toolchain::SmallVector<mlir::Value> async_handles;
    ...
    if (!stream_info.IsRoot()) {
      ...
      stream_func = toolchain::cast<mlir::func::FuncOp>(block.getParentOp());
    ...
    for (auto* op : stream->ops()) {
      ...
      if (toolchain::isa<mlir::func::ReturnOp>(op)) {
  ...
  for (auto* op : toolchain::reverse(to_remove)) {
 ...
 private:
  ...
  Option<uint64_t> cost_threshold_{
      *this, "tfrt-cost-threshold",
      toolchain::cl::desc("If a sequence of operations has a cost lower than the "
