 ...
 *
 */
#include "machina/compiler/mlir/tfrt/transforms/mlrt/async_while.h"
...
void MoveTFAwaitOpToFirstUse(mlir::Block &block) {
  toolchain::SmallVector<tf_mlrt::TFAwaitOp> await_ops;
  for (auto &op : block) {
    if (auto await_op = toolchain::dyn_cast<tf_mlrt::TFAwaitOp>(&op)) {
...
void MoveTFPromiseOpRightAfterAvailable(mlir::Block &block) {
  toolchain::SmallVector<tf_mlrt::TFPromiseOp> promise_ops;
  for (auto &op : block) {
    if (auto promise_op = toolchain::dyn_cast<tf_mlrt::TFPromiseOp>(&op)) {
...
class AsyncWhilePass
 ...
 private:
  ...
  toolchain::StringRef getArgument() const final { return "tf-mlrt-async-while"; }
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallDenseMap<toolchain::StringRef, mlir::func::FuncOp>
        processed_functions;
  ...
  void MayConvertWhileToAsyncWhile(
    ...
    for (mlir::Operation &op : toolchain::make_early_inc_range(op.front())) {
      ...
      if (ShouldConvertWhileToAsyncWhile(body_fn, predicate_fn)) {
        ...
        toolchain::SmallVector<mlir::Value> predicate_input;
        ...
        toolchain::SmallVector<mlir::Value> async_while_input;
        ...
        toolchain::SmallVector<mlir::Type> async_while_output_types;
  ...
  mlir::func::FuncOp CreatePredicate(
    ...
    toolchain::SmallVector<mlir::Type> remapped_input_types;
    for (int i : predicate_determinat) {
  ...
  mlir::func::FuncOp CreateBody(
    ...
    toolchain::SmallVector<mlir::Value> predicate_inputs;
    for (int i : predicate_determinat) {
    ...
    toolchain::SmallVector<mlir::Type> predicate_returned_types;
    auto empty_string_attr = builder.getStringAttr("");
  ...
  bool ShouldConvertWhileToAsyncWhile(mlir::func::FuncOp body_fn,
    ...
    for (auto &op : predicate_fn.getBody().front()) {
      if (!toolchain::isa<mlir::TF::ConstOp, mlir::TF::LessOp, mlir::TF::IdentityOp,
    ...
    toolchain::SmallSetVector<mlir::Operation *, 4> predicate_determinant_ops;
    toolchain::SmallSetVector<mlir::Operation *, 4> worker_ops;
    for (int i : argument_groups.mutables) {
  ...
  void SortBodyFnByFirstReadyFirstRun(
    ...
    toolchain::DenseMap<mlir::Operation *, int> dependency_cnt;
    ...
    toolchain::DenseSet<mlir::Operation *> final_result_producing_ops;
    ...
    toolchain::SmallDenseSet<mlir::Operation *> predicate_determined_ops;
    for (int i : predicate_determinat) {
    ...
    // don't get resorted.
    toolchain::SmallDenseSet<mlir::Operation *> pre_scheduled_ops;
  ...
  void MoveNewReadyOps(
      std::deque<mlir::Operation *> &ready_ops,
      std::list<mlir::Operation *> new_ready_ops,
      const toolchain::DenseSet<mlir::Operation *> &final_result_producing_ops) {
