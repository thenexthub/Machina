 ...
 *
 */
#include "machina/compiler/mlir/tfrt/transforms/mlrt/while_to_map_fn.h"
...
void RemoveIdentityOp(mlir::func::FuncOp func) {
  ...
  toolchain::SmallVector<mlir::TF::IdentityOp> identity_ops;
  for (auto &op : block) {
    if (auto identity_op = toolchain::dyn_cast<mlir::TF::IdentityOp>(&op)) {
      identity_ops.push_back(identity_op);
    }
  }

  for (auto op : toolchain::reverse(identity_ops)) {
...
// tf.map_fn (https://www.machina.org/api_docs/python/tf/map_fn) is converted
...
class WhileToMapFnPass
 ...
 private:
  ...
  void MayConvertWhileToMapFn(mlir::func::FuncOp op,
    ...
    for (mlir::Operation &op : toolchain::make_early_inc_range(op.front())) {
      auto while_op = toolchain::dyn_cast<mlir::TF::WhileOp>(&op);
      ...
      if (mlir::succeeded(MatchPredicate(while_op.getCondAttr(), symbol_table,
        ...
        toolchain::SmallVector<mlir::Type> result_types;
        toolchain::SmallVector<mlir::Value> tensor_list_operands;
  ...
  mlir::LogicalResult MatchInputSource(mlir::TF::WhileOp while_op,
    ...
    for (auto tensor_list_index : loop_info.tensor_list_or_flow_in) {
      ...
      if (auto tensor_list_reserve =
              toolchain::dyn_cast<mlir::TF::TensorListReserveOp>(
      ...
      } else if (auto tensor_array = toolchain::dyn_cast<mlir::TF::TensorArrayV3Op>(
                     tensor_list_or_flow_in_defining_op)) {
        ...
        if (!toolchain::is_contained(while_op.getOperands(),
  ...
  mlir::LogicalResult MatchOutputUse(mlir::TF::WhileOp &while_op,
    ...
    for (auto result_index : loop_info.tensor_list_or_flow_in) {
      ...
      if (toolchain::isa<mlir::TF::StopGradientOp>(use_op)) {
        use_op = *use_op->getUsers().begin();
      }

      if (!toolchain::isa<mlir::TF::TensorListStackOp,
  ...
  mlir::LogicalResult MatchPredicate(mlir::FlatSymbolRefAttr predicate_fn,
    ...
    for (auto &body_op : predicate_fn_op.getBody().front()) {
      switch (expecting_op) {
        case PredicateBodyExpectingOp::kExpectFirstLess:
          if (toolchain::isa<mlir::TF::LessOp>(body_op)) {
            expecting_op = PredicateBodyExpectingOp::kExpectSecondLess;
            less_ops.push_back(&body_op);
          } else if (!toolchain::isa<mlir::TF::ConstOp, mlir::TF::IdentityOp>(
                         body_op)) {
            return mlir::failure();
          }
          break;
        ...
        case PredicateBodyExpectingOp::kExpectLogicalAnd:
          if (toolchain::isa<mlir::TF::LogicalAndOp>(body_op)) {
            expecting_op = PredicateBodyExpectingOp::kExpectTerminator;
          } else if (!toolchain::isa<mlir::TF::IdentityOp>(body_op)) {
            return mlir::failure();
          }
          break;
        case PredicateBodyExpectingOp::kExpectTerminator:
          if (!toolchain::isa<mlir::TF::ToBoolOp, mlir::func::ReturnOp,
    ...
    int counter_index = -1;
    auto counter_iter =
        toolchain::find(predicate_fn_op.getArguments(), less_ops[0]->getOperand(0));
    ...
    if (auto max_iter_iter = toolchain::find(predicate_fn_op.getArguments(),
    ...
    auto element_index_iter =
        toolchain::find(predicate_fn_op.getArguments(), less_ops[1]->getOperand(0));
  ...
  mlir::LogicalResult MatchLoopCounterElementIndexInBody(
    ...
    toolchain::SmallVector<mlir::Operation *> loop_counter_users =
        GetUsersIgnoringIdentityOp(loop_counter);
    if (loop_counter_users.size() != 1 ||
        !toolchain::isa<mlir::TF::AddOp, mlir::TF::AddV2Op>(
    ...
    for (auto *element_index_use : GetUsersIgnoringIdentityOp(element_index)) {
      if (toolchain::isa<mlir::TF::AddOp, mlir::TF::AddV2Op>(element_index_use)) {
      ...
      } else if (toolchain::isa<mlir::TF::TensorListSetItemOp>(element_index_use)) {
        if (auto tensor_list_index = MayGetArgumentIndexIgnoringIdentityOp(
                while_body_func,
                toolchain::dyn_cast<mlir::TF::TensorListSetItemOp>(element_index_use)
                    .getInputHandle());
      ...
      } else if (toolchain::isa<mlir::TF::TensorArrayWriteV3Op>(element_index_use)) {
        if (auto flow_in_index = MayGetArgumentIndexIgnoringIdentityOp(
                while_body_func, toolchain::dyn_cast<mlir::TF::TensorArrayWriteV3Op>(
                                     element_index_use)
                                     .getFlowIn());
      ...
      } else if (!toolchain::isa<mlir::TF::GatherOp, mlir::TF::GatherV2Op,
  ...
  mlir::func::FuncOp CreateMapFnBodyFunction(mlir::OpBuilder &builder,
    ...
    variant_arguments.insert(loop_info.tensor_list_or_flow_in.begin(),
                             loop_info.tensor_list_or_flow_in.end());
    toolchain::SmallVector<mlir::Type> remapped_input_type;
    ...
    if (while_body_func.getArgAttrs().has_value()) {
      toolchain::SmallVector<mlir::Attribute> remapped_input_attributes;
  ...
  std::optional<int> MayGetArgumentIndexIgnoringIdentityOp(
    ...
    while (value.getDefiningOp()) {
      if (!toolchain::isa<mlir::TF::IdentityOp>(value.getDefiningOp())) {
        return std::nullopt;
      }
    ...
    auto argument_iter = toolchain::find(func.getArguments(), value);
  ...
  toolchain::SmallVector<mlir::Operation *> GetUsersIgnoringIdentityOp(
      mlir::Value value) {
    ...
    while (!users_stack.empty()) {
      ...
      if (!toolchain::isa<mlir::TF::IdentityOp>(descendent_user)) {
  ...
  mlir::Operation *GetDefiningOpIgnoringIdentityOp(mlir::Value value) {
    ...
    while (toolchain::isa<mlir::TF::IdentityOp>(source_op)) {
  ...
  mlir::Value GetReturnedOperand(const mlir::func::FuncOp func,
    ...
    auto return_op = toolchain::dyn_cast<mlir::func::ReturnOp>(
  ...
  bool IsScalarI32Tensor(mlir::Value value) const {
    if (auto value_type = toolchain::dyn_cast<mlir::TensorType>(value.getType())) {
