...
struct HoistInfo {
  ...
  toolchain::SmallVector<mlir::Operation *, 4> hoists_in_topological_order;
  ...
  // passes.
  toolchain::SmallVector<std::pair<mlir::Value, mlir::TF::ResourceHandle>, 4>
      hoisted_values;
};

bool OnlyHasReadOrNoEffect(mlir::Operation *op) {
  auto interface = toolchain::dyn_cast<mlir::MemoryEffectOpInterface>(op);
  if (!interface) return false;
  return interface.onlyHasEffect<mlir::MemoryEffects::Read>() ||
         interface.hasNoEffect();
}

bool CanHoist(const toolchain::DenseSet<mlir::TF::ResourceHandle> &read_only_vars,
              mlir::Operation *op) {
  ...
  if (toolchain::isa<mlir::TF::BatchFunctionOp>(op)) return false;

  // Non-side-effecting ops can be hoisted.
  if (mlir::isMemoryEffectFree(op)) return true;

  // ResourceHandle ops can be hoisted.
  if (toolchain::isa<mlir::TF::VarHandleOp, mlir::TF::HashTableV2Op>(op))
    return true;

  // If it is ReadVariableOp and the variable is readonly, it can be hoisted.
  if (auto read_var_op = toolchain::dyn_cast<mlir::TF::ReadVariableOp>(op)) {
    if (auto var_handle_op = toolchain::dyn_cast_or_null<mlir::TF::VarHandleOp>(
  ...
  if (auto lookup_table_size_op =
          toolchain::dyn_cast<mlir::TF::LookupTableSizeV2Op>(op)) {
    if (auto hash_table_op = toolchain::dyn_cast_or_null<mlir::TF::HashTableV2Op>(
...
void HoistInvariantOpsInFunction(
    ...
    const toolchain::DenseSet<mlir::TF::ResourceHandle> &read_only_vars,
  ...
  toolchain::DenseSet<mlir::Operation *> hoists;
  ...
  func.walk([&work_list](mlir::Operation *op) {
    if (toolchain::isa<mlir::TF::VarHandleOp, mlir::TF::HashTableV2Op,
                  mlir::TF::ConstOp>(op))
...
void FindCalleesRecursive(const mlir::SymbolTable &symbol_table,
                          mlir::func::FuncOp func, toolchain::StringSet<> &callees) {
  assert(func);
  func.walk([&](mlir::Operation *op) {
    FindCalleesRecursiveForOp(symbol_table, op, callees);
  });
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    module.walk([resource_id, bound_id, path_id,
      ...
      if (auto func_op = toolchain::dyn_cast<mlir::func::FuncOp>(op)) {
    ...
    module.walk([](mlir::Operation *op) {
      if (toolchain::isa<mlir::tf_saved_model::TensorFlowSavedModelDialect>(
              op->getDialect())) {
...
void LowerTFSavedModelPass::HoistInvariantOps(mlir::ModuleOp module) {
  ...
  toolchain::DenseMap<mlir::TF::ResourceHandle,
                 toolchain::SmallVector<mlir::Operation *, 4>>
      resources;

  // Find all callees referenced in the initialization functions.
  toolchain::StringSet<> init_callees;
  ...
  toolchain::StringSet<> xla_launch_callees;
  ...
  module.walk([&](mlir::Operation *op) {
    ...
    } else if (auto func = toolchain::dyn_cast<mlir::func::FuncOp>(op)) {
      if (!IsSessionInitializer(func)) return;
      FindCalleesRecursive(symbol_table, func, init_callees);
    } else if (toolchain::isa<mlir::TF::XlaLaunchOp>(op)) {
  ...
  toolchain::DenseSet<mlir::TF::ResourceHandle> read_only_vars;
  ...
  for (auto iter : toolchain::enumerate(module_hoist_info.hoisted_values)) {
    ...
    toolchain::StringRef device = kCpuDeviceName;
  ...
  for (auto *op :
       toolchain::reverse(module_hoist_info.hoists_in_topological_order)) {
    assert(op->use_empty());
    op->erase();
  }
}
...
void LowerTFSavedModelPass::ReplaceHoistedValues(
  ...
  struct HoistedValueInfo {
    toolchain::SmallVector<mlir::Value, 4> hoisted_values;
    toolchain::SmallVector<int64_t, 4> indices;
    toolchain::SmallVector<toolchain::StringRef, 4> shared_names;
    toolchain::SmallVector<toolchain::StringRef, 4> containers;
  };
  // Rearrange the hoisted values by each function and each device.
  toolchain::DenseMap<mlir::Block *, toolchain::StringMap<HoistedValueInfo>>
  ...
  for (auto iter : toolchain::enumerate(hoisted_values)) {
  ...
  for (const auto &block_iter : hoisted_values_by_block_device) {
    ...
    for (const auto &device_iter : device_map) {
      toolchain::StringRef device = device_iter.getKey();
      ...
      toolchain::SmallVector<mlir::Value> new_values;
      ...
      for (auto iter : toolchain::zip(old_values, new_values)) {
...
static toolchain::SmallVector<unsigned, 4> CompareTypes(mlir::TypeRange x,
                                                   mlir::TypeRange y) {
  toolchain::SmallVector<unsigned, 4> results;
...
//    http://cs?q=machina/core/common_runtime/executor.cc:932%20at_cl:356873227
...
mlir::LogicalResult ConvertReferenceVariableToResourceVariable(
  ...
  toolchain::SmallVector<mlir::TF::AssignOp, 4> assign_ops;
  toolchain::SmallVector<std::pair<mlir::Operation *, unsigned>, 4>
      side_effect_free_ops;

  for (mlir::OpOperand &use : var_op.getRef().getUses()) {
    mlir::Operation *user = use.getOwner();

    if (auto identity = toolchain::dyn_cast<mlir::TF::IdentityOp>(user)) {
      identity_ops.push_back(identity);
      continue;
    } else if (auto assign = toolchain::dyn_cast<mlir::TF::AssignOp>(user)) {
  ...
  auto var_handle_op = builder.create<mlir::TF::VarHandleOp>(
      ...
      mlir::RankedTensorType::get(
          {}, mlir::TF::ResourceType::get(
                  toolchain::ArrayRef<mlir::TensorType>{tensor_type},
                  builder.getContext())),
...
void ConvertReferenceVariableToResourceVariablePass::runOnOperation() {
  ...
  // identify a variable.
  toolchain::DenseMap<std::tuple<toolchain::StringRef, toolchain::StringRef, toolchain::StringRef>,
                 toolchain::SmallVector<mlir::TF::VariableV2Op, 4>>
      ref_vars;
