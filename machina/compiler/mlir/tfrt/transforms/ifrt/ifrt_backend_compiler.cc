...
absl::Status IfrtBackendCompiler::CompileTensorflow(
  ...
  mlir::StatusScopedDiagnosticHandler diag_handler(module->getContext());
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("ifrt_tpu_bct_conversion_before", module);
  }
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("ifrt_tpu_bct_conversion_after", module);
  }

  // Use bridge for cluster formation.
  TF_RETURN_IF_ERROR(machina::tf2xla::v2::RunFunctionTf2xlaClusteringBridge(
      module, /*is_supported_by_replicated_brige*/ true,
      /*is_in_fallback_enabled_mode=*/false));

  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("before_ifrt_outlining", module);
  }
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("after_ifrt_outlining", module);
  }

  // IFRT program is no longer needed.
  toolchain::SmallVector<mlir::func::FuncOp> to_erase;
  for (auto func : module.getOps<mlir::func::FuncOp>()) {
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("after_ifrt_program_removal", module);
  }
