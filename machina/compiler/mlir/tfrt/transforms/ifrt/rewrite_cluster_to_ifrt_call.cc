...
class RewriteClusterToIfrtCallPass
 ...
 public:
  void runOnOperation() override {
    ...
    if (failed(machina::GetDevicesFromOp(module.getOperation(), &devices)))
      return signalPassFailure();

    // key: original callee function in tf_device.cluster_func. value: ifrt
    // program.
    toolchain::DenseMap<mlir::func::FuncOp, mlir::func::FuncOp>
    ...
    toolchain::SmallVector<mlir::TF::TPUCompilationResultOp> compilation_result_ops;
 ...
 private:
  ...
  static int64_t NewProgramId() {
    const uint64_t id = static_cast<int64_t>(machina::random::New64());
  ...
  mlir::LogicalResult GetTpuCompileMetadata(
    ...
    if (!num_cores_per_replica_attr)
      return cluster_func.emitOpError()
             << "Attribute" << machina::kNumCoresPerReplicaAttr
             << " is missing";
