 ...
 private:
  ...
  mlir::LogicalResult ValidateThenUpdateUser(
    ...
    for (;;) {
      ...
      if (auto cast_op = toolchain::dyn_cast<mlir::TF::CastOp>(user)) {
        ...
        user = *cast_op.getResult().getUsers().begin();
                     ...
                     toolchain::dyn_cast<mlir::TF::AssignVariableOp>(user)) {
        // The chain must end with an `AssignVariableOp`.
        if (auto var_handle_op = toolchain::dyn_cast<mlir::TF::VarHandleOp>(
  ...
  mlir::LogicalResult RewriteRestore(mlir::TF::RestoreV2Op restore_op) {
    ...
    toolchain::SmallVector<bool, 4> truncate_in_cast;
