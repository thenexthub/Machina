...
void GetExecuteOpAttrsCommon(
  ...
  for (auto iter : op_attr_array) {
    ...
    toolchain::StringRef key = mlir::cast<mlir::StringAttr>(key_value[0]).getValue();
...
mlir::ParseResult ParseExecuteOpCommon(mlir::OpAsmParser &parser,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> in_chains;
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 4> operands;
  ...
  toolchain::SmallVector<mlir::Type, 4> operand_types;
  ...
  toolchain::SmallVector<mlir::Attribute, 4> op_attr_array;
  ...
  if (options.has_func_attr) {
    toolchain::SmallVector<mlir::Attribute, 4> op_func_attr_array;
