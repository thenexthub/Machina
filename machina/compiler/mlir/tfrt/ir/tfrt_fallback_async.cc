...
FallbackAsyncDialect::FallbackAsyncDialect(MLIRContext *context)
  ...
  addOperations<
#define GET_OP_LIST
#include "machina/compiler/mlir/tfrt/ir/tfrt_fallback_async.cpp.inc"
      >();
}
...
ParseResult ExecuteOpWithAllocator::parse(OpAsmParser &parser,
  ...
  auto &builder = parser.getBuilder();
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 1> allocator;
...
ParseResult ExecuteOpSeqWithAllocator::parse(OpAsmParser &parser,
                                             OperationState &result) {
  auto &builder = parser.getBuilder();
  toolchain::SmallVector<mlir::OpAsmParser::UnresolvedOperand, 2>
      chain_and_allocator;
