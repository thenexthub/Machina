...
#ifndef MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_IR_UNIFORMSUPPORT_H_
#define MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_IR_UNIFORMSUPPORT_H_
...
class UniformQuantizedValueConverter {
 public:
  ...
  virtual APInt quantizeFloatToInt(APFloat expressed_value) const {
    ...
    if (&expressed_value.getSemantics() == &APFloat::IEEEsingle() &&
        ...
        round_mode_ == toolchain::APFloatBase::rmNearestTiesToAway) {
    ...
    APFloat fixed_point = toolchain::minimum(scaled, clamp_max_);
    fixed_point = toolchain::maximum(fixed_point, clamp_min_);

    toolchain::APSInt result(storage_bit_width_, !is_signed_);
 ...
 private:
  ...
  virtual APInt quantizeF32ToInt8(const APFloat& expressed_value) const {
    ...
    assert(round_mode_ == toolchain::APFloatBase::rmNearestTiesToAway);
  ...
  const toolchain::APFloat::roundingMode round_mode_;
};
...
#endif  // MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_IR_UNIFORMSUPPORT_H_
