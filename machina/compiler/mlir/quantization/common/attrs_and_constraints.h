 ...
 *
 */
#ifndef MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_ATTRS_AND_CONSTRAINTS_H_
#define MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_ATTRS_AND_CONSTRAINTS_H_
...
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_utils.h"  // IWYU pragma: keep
...
FailureOr<T> TryCast(Operation* op, const StringRef name) {
  ...
  if (cast_op) {
    ...
    DEBUG_WITH_TYPE("mlir-quant-attrs-and-constraints",
                    toolchain::dbgs() << "Failed to match " << name << " ("
                                 << T::getOperationName() << ").\n");
    return failure();
  }
}
...
#endif  // MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_ATTRS_AND_CONSTRAINTS_H_
