...
TEST_F(AttrsAndConstraintsTest, I64ValueInI32RangeAreCastedCorrectly) {
  EXPECT_TRUE(succeeded(CastI64ToI32(toolchain::minIntN(32))));
  EXPECT_TRUE(succeeded(CastI64ToI32(toolchain::maxIntN(32))));
}

TEST_F(AttrsAndConstraintsTest, CastingFailsForI64ValueOutOfI32Range) {
  EXPECT_TRUE(failed(CastI64ToI32(toolchain::minIntN(32) - 10)));
  EXPECT_TRUE(failed(CastI64ToI32(toolchain::maxIntN(32) + 10)));
}

TEST_F(AttrsAndConstraintsTest, I64ArrayInI32RangeAreCastedCorrectly) {
  const SmallVector<int64_t> array_i64 = {toolchain::minIntN(32), -2, -1, 0, 1, 2,
                                          toolchain::maxIntN(32)};

  FailureOr<SmallVector<int32_t>> array_i32 = CastI64ArrayToI32(array_i64);
  EXPECT_TRUE(succeeded(array_i32));
  EXPECT_THAT(
      *array_i32,
      ElementsAreArray({static_cast<int32_t>(toolchain::minIntN(32)), -2, -1, 0, 1,
                        2, static_cast<int32_t>(toolchain::maxIntN(32))}));
}
...
TEST_F(AttrsAndConstraintsTest, CastingFailsForI64ArrayUnderI32Range) {
  ...
  EXPECT_EQ(under_min_i32, toolchain::minIntN(32) - 10);
...
TEST_F(AttrsAndConstraintsTest, CastingFailsForI64ArrayAboveI32Range) {
  ...
  EXPECT_EQ(below_max_i32, toolchain::maxIntN(32) + 10);
  EXPECT_TRUE(failed(CastI64ArrayToI32(array_i64)));
}
