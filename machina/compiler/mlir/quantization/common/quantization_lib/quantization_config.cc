...
static bool IsQuantizationType(machina::DataType dtype) {
  switch (dtype) {
...
bool ParseInputNodeQuantSpecs(const absl::string_view node_names,
  ...
  if (!min_values.empty()) {
    ...
    for (const std::string& node_mins_str : node_mins_str) {
      ...
      if (!absl::SimpleAtod(node_mins_str, &value)) {
        toolchain::errs() << "Unexpected mins: " << node_mins_str << "\n";
        return true;
      }
  ...
  if (!max_values.empty()) {
    ...
    for (const std::string& node_maxs_str : node_maxs_str) {
      ...
      if (!absl::SimpleAtod(node_maxs_str, &value)) {
        toolchain::errs() << "Unexpected mins: " << node_maxs_str << "\n";
        return true;
      }
  ...
  machina::DataType final_type = machina::DT_FLOAT;
...
bool GetInputNodeQuantSpecs(const std::vector<std::string>& node_names,
                            ...
                            const machina::DataType inference_type,
                            QuantizationSpecs* quant_specs) {
  ...
  if (!node_mins.empty()) {
    toolchain::dbgs() << "Ignored input_min_values.";
  }
  if (!node_maxs.empty()) {
    toolchain::dbgs() << "Ignored input_max_values.";
  }
  return false;
}
