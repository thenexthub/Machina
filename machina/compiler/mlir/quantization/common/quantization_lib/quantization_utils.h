...
#ifndef MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
#define MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
...
#include "machina/compiler/mlir/quantization/common/ir/FakeQuantSupport.h"
#include "machina/compiler/mlir/quantization/common/ir/QuantOps.h"
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_config.h"
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_traits.h"
...
using QuantParamsForResults = toolchain::SmallVector<QuantizedType, 4>;
...
struct OpQuantSpec {
  ...
  // outputs.
  toolchain::DenseMap<SignedInteger, QuantParamsForResults> restricted_output_params;
  ...
  toolchain::DenseMap<int, int> coeff_op_quant_dim;
...
inline std::string GetTensorNameFromLoc(Location loc) {
  if (auto name_loc = toolchain::dyn_cast<NameLoc>(loc)) {
...
struct ConvertStatsToQDQs
  ...
  LogicalResult matchAndRewrite(mlir::quant::ir::StatisticsOp op,
    ...
    Type expressed = toolchain::cast<ShapedType>(op.getType()).getElementType();
    ...
    if (op.getAxisStats().has_value()) {
      ...
      auto stats =
          toolchain::dyn_cast<DenseFPElementsAttr>(op.getAxisStats().value());
      ...
      if (legacy_float_scale) {
        quant_type = DownCastScale(quant_type, mins, maxs, op->getLoc());
                   ...
                   toolchain::dyn_cast<DenseFPElementsAttr>(op.getLayerStats())) {
...
bool UsedBy(Operation* op) {
  for (Operation* user : op->getUsers()) {
    if (toolchain::isa_and_nonnull<VerifierT>(user)) return true;
  }
  return false;
}
...
class QuantizationPattern : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation* op,
                                PatternRewriter& rewriter) const override {
    toolchain::SmallVector<Operation*, 4> quantizing_ops;
    ...
    machina::DataType inference_type =
    ...
    for (Operation* quantizing_op : quantizing_ops) {
      ...
      if (toolchain::isa<QuantizeOpT, DequantizeOpT>(quantizing_op)) {
        return failure();
      }
      ...
      if (!nodes_blocklist.empty()) {
        if (auto name_loc = toolchain::dyn_cast<NameLoc>(quantizing_op->getLoc())) {
      ...
      for (auto operand : quantizing_op->getOperands()) {
        ...
        auto ele_type =
            toolchain::cast<TensorType>(operand.getType()).getElementType();
        if (static_cast<const ConcreteT*>(this)
          ...
          if (dq_op && inference_type == machina::DT_QINT8 &&
      ...
      if (QuantizableOpSupportsFloatOutputType(quantizing_op)) {
        ...
        for (const auto& indexed_regions :
             toolchain::enumerate(quantizing_op->getRegions())) {
        ...
        toolchain::SmallDenseMap<Value, int> outputs_replaced;
        SmallVector<Type, 4> output_types;
        output_types.reserve(quantizing_op->getNumResults());
        for (const auto& enumerated_result :
             toolchain::enumerate(quantizing_op->getResults())) {
          ...
          Type result_ele_type =
              toolchain::cast<TensorType>(result.getType()).getElementType();
          // If the user is the QuantizeOp, it must be the only user.
          if (result.hasOneUse() &&
              toolchain::isa<QuantizeOpT>(*result.user_begin())) {
            auto user = toolchain::cast<QuantizeOpT>(*result.user_begin());
        ...
        if (inference_type == machina::DT_HALF &&
      ...
      if (enable_verify && !std::is_same_v<VerifierT, void>) {
        ...
        for (int i = 0, e = quantized_op->getNumOperands(); i < e; ++i) {
          ...
          if (auto q = toolchain::dyn_cast_or_null<QuantizeOpT>(def)) {
            DenseFPElementsAttr attr;
 ...
 private:
  ...
  void RewireFloatModelBackbone(Operation* quantized_op,
    ...
    for (int i = 0, e = quantized_op->getNumResults(); i < e; ++i) {
      if (!toolchain::cast<ShapedType>(float_op->getResult(i).getType())
               .getElementType()
               .isF32()) {
        continue;
      }
      ...
      for (auto user : result.getUsers()) {
        ...
        if (toolchain::isa<QuantizeOpT>(user)) {
          user = *user->getResult(0).getUsers().begin();
        }
        if (auto dequantize = toolchain::dyn_cast<DequantizeOpT>(user)) {
...
struct ConvertUnsignedToSigned : public OpRewritePattern<QuantizeOpT> {
  ...
  LogicalResult matchAndRewrite(QuantizeOpT op,
    ...
    QType new_qtype;
    if (auto uqtype = toolchain::dyn_cast<quant::UniformQuantizedType>(qtype)) {
      new_qtype = quant::UniformQuantizedType::getChecked(
          ...
          uqtype.getStorageTypeMax() - offset);
                   ...
                   toolchain::dyn_cast<quant::UniformQuantizedPerAxisType>(qtype)) {
      auto zero_points = aqtype.getZeroPoints();
      toolchain::SmallVector<int64_t, 4> new_zero_points(zero_points.begin(),
                                                    zero_points.end());
...
struct FoldTrivalRequantizeOp : public OpRewritePattern<RequantizeOpT> {
  ...
  LogicalResult matchAndRewrite(RequantizeOpT op,
    ...
    if (toolchain::isa<FixedOutputRangeInterface, SameScalesOpInterface>(def) ||
    ...
    toolchain::SmallVector<Type, 4> new_output_types;
...
#endif  // MACHINA_COMPILER_MLIR_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
