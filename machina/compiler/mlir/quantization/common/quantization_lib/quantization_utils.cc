...
#include "machina/compiler/mlir/quantization/common/ir/QuantizeUtils.h"
#include "machina/compiler/mlir/quantization/common/ir/UniformSupport.h"
#include "machina/compiler/mlir/quantization/common/quantization_lib/portable_tensor_utils.h"
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_traits.h"
#include "machina/compiler/mlir/tools/optimize/quantization_utils.h"
...
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_interface.cc.inc"
...
void ExpandVerySmallRange(const ArrayRef<double> mins,
  ...
  for (const auto [min, max] : toolchain::zip(mins, maxs)) {
...
bool QuantizableOpSupportsFloatOutputType(Operation* op) {
  ...
  for (const auto [attr_element, result_type] :
       toolchain::zip_equal(output_types_attr, op->getResultTypes())) {
...
void ExtractMinMaxFromAttr(const DenseFPElementsAttr values, const int dim_size,
  ...
  if (values.isSplat()) {
    const double single_value =
        FloatAttr::getValueAsDouble(values.getSplatValue<toolchain::APFloat>());
...
quant::QuantizedType GetUniformQuantizedTypeForBias(
  ...
  for (const auto op_type : op_types) {
    if (const auto type =
            dyn_cast<quant::UniformQuantizedPerAxisType>(op_type)) {
      for (const auto& index_scale : toolchain::enumerate(type.getScales())) {
...
bool RemoveRedundantStatsOps(
  ...
  toolchain::DenseSet<Operation*> redundant_stats_ops;
  ...
  for (int i = 1; i < collected_quant_params.size(); ++i) {
    ...
    if (!same_scale_op.RequiredSameQuantizedAxes()) {
      ...
      if (expected_per_axis_qtype && compared_per_axis_qtype &&
          toolchain::equal(expected_per_axis_qtype.getScales(),
                      compared_per_axis_qtype.getScales()) &&
          toolchain::equal(expected_per_axis_qtype.getZeroPoints(),
