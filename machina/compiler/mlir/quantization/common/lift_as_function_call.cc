...
LogicalResult SetAttributeMap(MLIRContext& context,
  ...
  toolchain::SmallDenseMap<NamedAttribute, Operation*> attr_to_op_map;
  ...
  for (int idx : toolchain::seq<int>(0, attributes.size())) {
...
SmallVector<Value, 4> LiftAsFunctionCall(
  ...
  IRMapping mapping;
  for (int32_t i : toolchain::seq<int32_t>(0, arguments.size())) {
