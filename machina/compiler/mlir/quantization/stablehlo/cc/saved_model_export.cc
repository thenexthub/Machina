...
using ::machina::FunctionDefLibrary;
...
using ::machina::SaverDef;
using ::machina::quantization::ExportedModel;
using ::machina::quantization::RunPasses;
using ::machina::quantization::UnfreezeConstantsAndSaveVariables;
...
absl::StatusOr<ExportedModel> ConvertMlirModuleToExportedModel(
    ...
    const absl::flat_hash_map<FunctionName, FunctionAlias>& function_aliases,
    const std::vector<AssetFileDef>& asset_file_defs) {
  const machina::GraphExportConfig config{};
  FunctionLibraryDefinition flib_def{OpRegistry::Global(),
                                     FunctionDefLibrary()};
  std::unique_ptr<Graph> graph;
  absl::flat_hash_set<Node*> control_ret_nodes{};
  TF_RETURN_IF_ERROR(machina::tf2xla::v2::ConvertTfExecutorToGraph(
