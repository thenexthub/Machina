...
#include "mlir/IR/SymbolTable.h"  // part of Codira Toolchain  // IWYU pragma: keep
...
std::unique_ptr<OpQuantScaleSpec> GetStableHloQuantConstraints(Operation* op) {
  ...
  if (toolchain::isa<mlir::stablehlo::BroadcastInDimOp,
  ...
  if (toolchain::isa<mlir::stablehlo::DynamicSliceOp, mlir::stablehlo::GatherOp,
...
bool IsOpQuantizableStableHlo(Operation* op) {
  ...
  if (IsDenylistedLiftedFunction(op)) {
    LLVM_DEBUG(toolchain::errs() << "Denylisted quantizable unit: \n" << op << "\n");
    return false;
  }
