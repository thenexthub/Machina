...
class ConvertTfQuantToMhloIntTest : public Test {
 protected:
  ...
  absl::StatusOr<OwningOpRef<ModuleOp>> ReplaceFuncArgsByConstant(
    ...
    CHECK(module_op);
    auto func_op = toolchain::dyn_cast<func::FuncOp>(
    ...
    for (int i = 0; i < arguments.size(); ++i) {
      ...
      TF_ASSIGN_OR_RETURN(auto data_type,
                          machina::EncodePrimitiveTypeAsDataType(
                              xla_literal->shape().element_type()));
      TF_RETURN_IF_ERROR(
          machina::XLAShapeToTensorShape(xla_literal->shape(), &shape));
      machina::Tensor tensor(data_type, shape);
  ...
  absl::StatusOr<std::shared_ptr<xla::Literal>> EvaluateTfFunction(
    ...
    auto& return_op = toolchain::dyn_cast<func::FuncOp>(
                          *module_op->getBodyRegion().getOps().begin())
                          .getFunctionBody()
                          .getBlocks()
                          .back()
                          .back();
    if (!toolchain::isa<func::ReturnOp>(return_op) ||
    ...
    if (fold_results.size() != 1 ||
        !toolchain::isa<TF::ConstOp>(fold_results[0].getDefiningOp())) {
      return absl::InternalError("Failed to evaluate TF ops");
    }

    // Convert output tensor back to xla::Literal.
    machina::Tensor tensor;
    TF_RETURN_IF_ERROR(machina::ConvertToTensor(
        toolchain::dyn_cast<TF::ConstOp>(fold_results[0].getDefiningOp()).getValue(),
        &tensor));
    xla::Shape xla_shape;
    TF_RETURN_IF_ERROR(machina::TensorShapeToXLAShape(
