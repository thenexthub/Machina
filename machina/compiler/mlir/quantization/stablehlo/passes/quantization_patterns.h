 ...
 *
 */
#ifndef MACHINA_COMPILER_MLIR_QUANTIZATION_STABLEHLO_PASSES_QUANTIZATION_PATTERNS_H_
#define MACHINA_COMPILER_MLIR_QUANTIZATION_STABLEHLO_PASSES_QUANTIZATION_PATTERNS_H_
...
#include "machina/compiler/mlir/quantization/stablehlo/ops/stablehlo_op_quant_spec.h"
  ...
  LogicalResult matchAndRewrite(RootOpT op,
    ...
    for (Operation* candidate_op : *candidate_ops) {
      ...
      for (const auto& enumerated_result :
           toolchain::enumerate(candidate_op->getResults())) {
      ...
      if (candidate_op->getNumRegions() != 0) {
        for (const auto& indexed_regions :
             toolchain::enumerate(candidate_op->getRegions())) {
...
#endif  // MACHINA_COMPILER_MLIR_QUANTIZATION_STABLEHLO_PASSES_QUANTIZATION_PATTERNS_H_
