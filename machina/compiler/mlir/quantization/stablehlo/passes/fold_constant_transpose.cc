...
#include "machina/compiler/mlir/quantization/stablehlo/cc/permutation.h"
...
int64_t GetContiguousOffset(const ArrayRef<int64_t> indices,
  ...
  for (auto [i, dimension] : toolchain::reverse(toolchain::zip_equal(indices, shape))) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(mlir::stablehlo::TransposeOp op,
    ...
    const SmallVector<float> original_values =
        toolchain::to_vector(value_attr.getValues<float>());
