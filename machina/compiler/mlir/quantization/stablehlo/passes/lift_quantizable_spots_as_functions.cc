...
#include "machina/compiler/mlir/quantization/stablehlo/passes/lift_quantizable_spots_as_functions_simple.inc"
}

namespace fusion_patterns {
#include "machina/compiler/mlir/quantization/stablehlo/passes/lift_quantizable_spots_as_functions_fusion.inc"
}
...
FailureOr<std::string> QuantizationMethodToTextProto(const Method& method) {
  ...
  if (!printer.PrintToString(method, &method_txtpb)) {
    LLVM_DEBUG(toolchain::dbgs() << "Failed to convert Method to textproto\n.");
    return failure();
  }
