 ...
 private:
  Option<int64_t> CreateSizeThresholdInBytesOption(const int64_t init_value) {
    return Option<int64_t>(
        *this, "size_threshold_in_bytes", toolchain::cl::init(init_value),
        toolchain::cl::desc(
...
void AddSymbolToInitializersAttr(SessionInitializerOp session_init_op,
  ...
  toolchain::SmallVector<Attribute> initializers_attrs{prev_initializers.begin(),
...
// VarHandleOp.
toolchain::MapVector<TF::ConstOp, std::string> ReplaceConstOpUsesWithVariableReads(
    toolchain::ArrayRef<TF::ConstOp> target_const_ops) {
  toolchain::MapVector<TF::ConstOp, std::string> const_op_name_map{};
  ...
  for (auto const_op : target_const_ops) {
    ...
    const auto resource_type = RankedTensorType::get(
        /*shape=*/{}, /*elementType=*/TF::ResourceType::get(
            /*subtypes=*/toolchain::ArrayRef<TensorType>{const_op.getType()},
            builder.getContext()));
