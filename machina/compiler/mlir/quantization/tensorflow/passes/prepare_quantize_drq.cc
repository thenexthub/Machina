...
// //third_party/machina/compiler/mlir/lite/transforms/prepare_quantize_dynamic_range.cc
...
#include "machina/compiler/mlir/quantization/common/attrs_and_constraints.h"  // IWYU pragma: keep
 ...
 private:
  ...
  bool insertQDQ(PatternRewriter& rewriter, arith::ConstantOp op,
    ...
    for (auto connected_op : op->getUsers()) {
      auto q_op =
          toolchain::dyn_cast_or_null<mlir::quant::ir::QuantizeCastOp>(connected_op);
      if (q_op && q_op.getType() == cast_type) {
        auto dq_op = toolchain::cast<mlir::quant::ir::DequantizeCastOp>(
