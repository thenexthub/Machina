...
struct TFQuantizationBase
  ...
  static bool AllowDynamicRangeQuantizedOperand(
    ...
    StringRef function_name =
        toolchain::cast<FlatSymbolRefAttr>(call_op.getFAttr()).getValue();
 ...
 public:
  ...
  LogicalResult matchAndRewrite(DequantizeCastOp op,
    ...
    for (Operation* quantizing_op : quantizing_ops) {
      ...
      if (toolchain::isa<QuantizeCastOp, DequantizeCastOp>(quantizing_op)) {
        return failure();
      }
      ...
      for (const auto& operand : quantizing_op->getOperands()) {
        ...
        Type elem_type = toolchain::cast<TensorType>(operand_type).getElementType();
        if (auto dq_op =
                dyn_cast_or_null<DequantizeCastOp>(operand.getDefiningOp())) {
          auto dq_arg_type = toolchain::cast<TensorType>(dq_op.getArg().getType());
          auto qtype = toolchain::cast<QuantizedType>(dq_arg_type.getElementType());
      ...
      for (const auto& enumerated_result :
        ...
        auto result_tensor_type = toolchain::cast<TensorType>(result_type);
        // If the user is the Quantize op, it must be the only user.
        if (result.hasOneUse() &&
            toolchain::isa<QuantizeCastOp>(*result.user_begin())) {
          auto user = toolchain::cast<QuantizeCastOp>(*result.user_begin());
          outputs_replaced.insert(
              {user.getResult(), enumerated_result.index()});
          auto qtype = toolchain::cast<QuantizedType>(
              toolchain::cast<TensorType>(user.getType()).getElementType());
 ...
 private:
  ...
  bool IsConnectedWithCompsiteFunction(Operation* same_scale_op) const {
    for (const auto& operand : same_scale_op->getOperands()) {
      ...
      if (toolchain::isa<TF::PartitionedCallOp>(preceding_op)) {
        auto call_op = toolchain::cast<TF::PartitionedCallOp>(preceding_op);
        if (!IsCompositeFunction(call_op)) continue;
        return true;
      }

      // Check if the preceding op is a quantized same-scale op.
      if (toolchain::isa<StorageCastOp>(preceding_op)) {
        auto sc_op = toolchain::cast<StorageCastOp>(preceding_op);
        auto sc_arg_type = toolchain::dyn_cast<TensorType>(sc_op.getArg().getType());
    ...
    for (const auto& result : same_scale_op->getResults()) {
      ...
      auto q_op = toolchain::cast<QuantizeCastOp>(*result.user_begin());
      for (auto following_op : q_op->getUsers()) {
        // Check whether the preceding op is a quantized composite function.
        if (toolchain::isa<TF::PartitionedCallOp>(following_op)) {
          auto call_op = toolchain::cast<TF::PartitionedCallOp>(following_op);
          if (!IsCompositeFunction(call_op)) continue;
          return true;
        }

        // Check if the preceding op is a quantized same-scale op.
        if (toolchain::isa<StorageCastOp>(following_op)) {
          auto sc_op = toolchain::cast<StorageCastOp>(following_op);
          auto sc_arg_type =
              toolchain::dyn_cast<TensorType>(sc_op.getResult().getType());
          if (sc_arg_type.getElementType().isInteger(8)) {
            return true;
          }
        }
  ...
  bool IsCompositeFunction(TF::PartitionedCallOp call_op) const {
    ...
    const auto f_attr = toolchain::dyn_cast<FlatSymbolRefAttr>(call_op.getFAttr());
    ...
    for (Value input : call_op.getArgs()) {
      if (auto type = toolchain::dyn_cast<TensorType>(input.getType())) {
    ...
    for (Value output : call_op.getOutput()) {
      if (auto type = toolchain::dyn_cast<TensorType>(output.getType())) {
        if (isa<FloatType>(type.getElementType())) {
          return false;
        }
        if (isa<QuantizedType>(type.getElementType())) {
...
struct QuantizeAvgPoolOpPattern : public OpRewritePattern<StorageCastOp> {
  ...
  LogicalResult matchAndRewrite(StorageCastOp sc_op,
    ...
    auto dq_arg_type =
        toolchain::cast<TensorType>(preceding_sc_op.getArg().getType());
    auto qtype = toolchain::cast<QuantizedType>(dq_arg_type.getElementType());
    auto q_result_type = toolchain::cast<TensorType>(sc_op.getType());
    auto out_qtype = toolchain::cast<QuantizedType>(q_result_type.getElementType());
    if (qtype != out_qtype) {
...
class QuantizePass
 ...
 public:
  ...
  explicit QuantizePass() {
    quant_specs_.inference_type = machina::DT_QINT8;
  }
 ...
 private:
  ...
  Option<bool> weight_quantization_{
      *this, "weight-quantization", toolchain::cl::init(false),
      toolchain::cl::desc("Whether to enable weight quantization.")};
