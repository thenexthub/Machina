...
// //third_party/machina/compiler/mlir/lite/transforms/prepare_quantize.cc
...
#include "machina/compiler/mlir/quantization/common/quantization_lib/quantization_driver.h"
...
class PrepareQuantizePass
 ...
 public:
  ...
  explicit PrepareQuantizePass(const QuantizationSpecs& quant_specs,
    ...
    enable_post_training_quantize_ =
        (quantization_method == machina::quantization::QuantizationMethod::
 ...
 private:
  ...
  GetMinMaxValuesForArgument(toolchain::StringRef func_name, int index) {
  ...
  Option<bool> enable_post_training_quantize_{
      *this, "post-training-quantize", toolchain::cl::init(false),
      toolchain::cl::desc("Enable post training quantization. Only used in tests.")};
...
bool PrepareQuantizePass::SetInputNodesQuantizationParams(func::FuncOp func) {
  ...
  auto has_quantize_op = [&](const Value arg) {
    return (arg.hasOneUse() && toolchain::isa<QuantizeCastOp>(*arg.user_begin()));
  };
...
static Value Quantized(Operation* user) {
  if (auto q = toolchain::dyn_cast_or_null<QuantizeCastOp>(user)) {
    if (auto dq = toolchain::dyn_cast_or_null<DequantizeCastOp>(
...
void PrepareQuantizePass::SanityCheckAndAdjustment(func::FuncOp func) {
  ...
  func.walk([&](func::ReturnOp ret) {
    ...
    for (Value returned : ret.getOperands()) {
      toolchain::SmallVector<Value, 4> quantized;
...
bool PrepareQuantizePass::ContainsQuantizeOps(func::FuncOp func) {
  for (const auto& op : func.getOps()) {
    if (toolchain::isa<DequantizeCastOp>(op)) return true;
  }
  return false;
}
...
#include "machina/compiler/mlir/quantization/machina/passes/prepare_quantize.inc"
