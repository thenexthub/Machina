...
#include "machina/compiler/mlir/quantization/common/lift_as_function_call.h"  // IWYU pragma: keep
...
using ::machina::quantization::QuantizationComponentSpec;
using ::machina::quantization::QuantizationMethod;
using ::machina::quantization::QuantizationOptions;
using ::machina::quantization::UnitWiseQuantizationSpec;
 ...
 private:
  ...
  absl::Status checkQuantizableOpsForXla(TF::PartitionedCallOp call_op,
    ...
    for (auto iter : spec->coeff_op_quant_dim) {
      ...
      if (auto dq_op =
              toolchain::dyn_cast_or_null<mlir::quant::ir::DequantizeCastOp>(
                  preceding_op)) {
        if (auto q_op = toolchain::dyn_cast_or_null<mlir::quant::ir::QuantizeCastOp>(
  ...
  void removeAttrMapAttribute(TF::PartitionedCallOp call_op,
    ...
    composite_func.walk([&](Operation* op) {
      if (op->hasAttr(kAttrMapAttribute)) {
        ...
        toolchain::raw_string_ostream log_stream(log_message);
...
#include "machina/compiler/mlir/quantization/machina/passes/lift_quantizable_spots_as_functions.inc"
