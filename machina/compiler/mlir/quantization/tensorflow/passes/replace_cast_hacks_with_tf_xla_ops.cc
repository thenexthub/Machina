...
void PrepareXlaConvParams(OpBuilder &builder, Location loc, ArrayAttr strides,
  ...
  for (int64_t i : toolchain::seq<int64_t>(1, num_dims - 1)) {
...
Value CreateZeroPointPartialOffset(OpBuilder &builder, Location loc,
  ...
  for (int64_t i : toolchain::seq<int64_t>(0, shape.getRank())) {
...
Value MergeZeroPointOffset(OpBuilder &builder, Location loc, Value weight,
  ...
  for (auto i : toolchain::seq<int64_t>(0, weight_shape.getRank())) {
...
Value CreateXlaDotV2OpFromTfBatchMatMulOp(OpBuilder &builder, Location loc,
  ...
  xla::DotDimensionNumbers dnums;
  for (int i : toolchain::seq<int32_t>(0, num_batch_dim)) {
...
#include "machina/compiler/mlir/quantization/machina/passes/replace_cast_hacks_with_tf_xla_ops.inc"
