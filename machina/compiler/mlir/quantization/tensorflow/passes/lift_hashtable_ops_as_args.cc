...
bool IsHashTableOp(Operation* op) {
  return toolchain::isa<TF::HashTableOp, TF::HashTableV2Op,
...
bool IsMainOrInitializerFunction(ModuleOp module, func::FuncOp func) {
  if (func.getSymName() ==
          toolchain::StringRef(machina::kImportModelDefaultGraphFuncName) ||
...
bool UsedBySupportedOps(ModuleOp module, func::FuncOp func) {
  ...
  for (auto& function_use : function_uses.value()) {
    if (!toolchain::isa<TF::PartitionedCallOp, TF::StatefulPartitionedCallOp>(
...
LogicalResult LiftHashTableOpsToArguments(ModuleOp module_op,
  ...
  if (!toolchain::hasSingleElement(target_func)) return success();
  ...
  toolchain::StringMap<int> shared_name_to_arg_idx;
  toolchain::SmallVector<std::pair<Operation*, int>> lifted_op_and_arg_idx;
