...
bool IsOutputCompileTimeConstantOrBounded(Operation* op) {
  return toolchain::isa_and_nonnull<TF::ShapeOp, TF::ShapeNOp, TF::RankOp,
...
void RecursivelyDuplicateConstantsForOperands(
    toolchain::ArrayRef<OpOperand*> op_operands) {
  // Target operands to duplicate if it is a ConstOp.
  toolchain::SmallVector<OpOperand*, 4> duplication_targets{op_operands.begin(),
                                                       op_operands.end()};
  ...
  while (target_idx < duplication_targets.size()) {
    ...
    if (toolchain::isa_and_nonnull<TF::ConstOp>(defining_op)) {
...
toolchain::SmallVector<OpOperand*> GetOperands(Operation& op,
                                          toolchain::ArrayRef<int> operand_indices) {
  toolchain::SmallVector<OpOperand*> operands{};
