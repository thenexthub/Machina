...
class QuantizeWeightsPass
 ...
 public:
  ...
  explicit QuantizeWeightsPass(
      const machina::quantization::QuantizationOptions& quant_options)
 ...
 private:
  ...
  bool test_mode_;
  machina::quantization::QuantizationOptions quant_options_;

  // Initialize for tests.
  void initializeForTest() {
    if (!test_mode_) return;

    machina::quantization::QuantizationComponentSpec quant_spec;
    ...
    quant_spec.set_tensor_type(
        machina::quantization::QuantizationComponentSpec::TENSORTYPE_INT_8);
...
class QuantizeConstWeights : public OpRewritePattern<TF::ConstOp> {
 ...
 private:
  ...
  bool hasUsageFromQuantizableOp(TF::ConstOp op) const {
    toolchain::SmallVector<mlir::Value> uses_at_current_level{op};
    while (!uses_at_current_level.empty()) {
      toolchain::SmallVector<mlir::Value> next_values_to_visit;
      for (auto cur_op : uses_at_current_level) {
        for (auto& cur_op_use : cur_op.getUses()) {
          ...
          if (auto call_op = toolchain::dyn_cast<mlir::CallOpInterface>(next_op)) {
            ...
            next_values_to_visit.push_back(
                func.getArgument(next_op_operand_num));
                         ...
                         toolchain::dyn_cast_or_null<TF::WhileOp>(next_op)) {
