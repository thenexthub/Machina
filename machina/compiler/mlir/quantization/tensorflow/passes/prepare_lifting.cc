...
#include "machina/compiler/mlir/quantization/machina/cc/constant_fold.h"
...
LogicalResult MatchSupportedAffineOp(Operation* op, Value& binding_output,
  ...
  if (toolchain::isa<TF::Conv2DOp, TF::Conv3DOp, TF::DepthwiseConv2dNativeOp>(op)) {
...
Value MakeOneDimValueBroadcastable(OpBuilder& builder, Location loc,
  ...
  SmallVector<int64_t> new_shape;
  for (auto idx : toolchain::reverse(toolchain::seq<int32_t>(0, rhs_shape.getRank()))) {
...
bool CanBeSymmetricallyQuantized(Value weight) {
  ...
  if (auto uniform_type = toolchain::dyn_cast_or_null<UniformQuantizedType>(qtype)) {
    return uniform_type.getZeroPoint() == 0;
  } else if (auto per_axis_type =
                 toolchain::dyn_cast_or_null<UniformQuantizedPerAxisType>(qtype)) {
...
SmallVector<T> MultiplyTwoArrays(ArrayRef<T> a, ArrayRef<T> b) {
  ...
  for (size_t i : toolchain::seq<size_t>(0, max_size)) {
...
Value MultiplyFakeQuantValue(OpBuilder& builder, Location loc, Value value,
  ...
  if (auto uniform_type = toolchain::dyn_cast<UniformQuantizedType>(element_type)) {
    if (multiplier_attr.isSplat()) {
      ...
      new_qtype = UniformQuantizedPerAxisType::get(
          ...
          uniform_type.getStorageTypeMax());
                 ...
                 toolchain::dyn_cast_or_null<UniformQuantizedPerAxisType>(
                     element_type)) {
    auto new_scales =
...
#include "machina/compiler/mlir/quantization/machina/passes/prepare_lifting.inc"
