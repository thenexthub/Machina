...
class QuantizeCompositeFunctionsPass
 ...
 private:
  ...
  Option<OpSet> target_opset_{
      *this, "target-opset", toolchain::cl::init(OpSet::TF),
...
LogicalResult CreateUniformQuantizedPerAxisTypeParams(
  ...
  toolchain::SmallVector<float, 4> float_scales;
  toolchain::SmallVector<int32_t, 4> int32_zero_points;
...
LogicalResult TransferTFAttributesToTFUniformAttributes(
  ...
  for (Operation& inner_op : float_func.getBody().front().getOperations()) {
    ...
    for (absl::string_view element_str : absl::StrSplit(attr_map_str, ',')) {
      ...
      identifier_to_attr.insert(
          {toolchain::StringRef(std::string(key_and_value_pair[1])),
           inner_op.getAttr(
               toolchain::StringRef(std::string(key_and_value_pair[1])))});
    }
  }
  ...
  for (Operation& inner_op : quantized_func.getBody().front().getOperations()) {
    if (auto uniform_op =
            toolchain::dyn_cast<TF::UniformQuantizedConvolutionHybridOp>(inner_op);
        uniform_op != nullptr) {
      if (failed(FillAttributesForUniformQuantizedConvolutionOp(
        ...
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformQuantizedConvolutionOp>(inner_op);
               uniform_op != nullptr) {
      if (failed(FillAttributesForUniformQuantizedConvolutionOp(
              rewriter, uniform_op, identifier_to_attr, quantization_method,
              enable_per_channel_quantization)))
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformQuantizedDotHybridOp>(inner_op);
               uniform_op != nullptr) {
      if (failed(FillAttributesForUniformQuantizedDotOp(
        ...
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformQuantizedAddOp>(inner_op);
               uniform_op != nullptr) {
      if (failed(FillAttributesForUniformQuantizedAddOp(
        ...
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformQuantizedClipByValueOp>(inner_op);
               uniform_op != nullptr) {
      if (failed(FillAttributesForUniformQuantizedClipByValueOp(
        ...
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformRequantizeOp>(inner_op);
               uniform_op != nullptr) {
      if (failed(FillAttributesForUniformRequantizeOp(
        ...
        return failure();
                   ...
                   toolchain::dyn_cast<TF::UniformQuantizeOp>(inner_op);
               uniform_op != nullptr) {
...
LogicalResult TransferAttributes(func::FuncOp float_func,
  ...
  for (Operation& inner_op : float_func.getBody().front().getOperations()) {
    ...
    for (absl::string_view element_str : absl::StrSplit(attr_map_str, ',')) {
      ...
      identifier_to_attr.insert(
          {toolchain::StringRef(std::string(key_and_value_pair[0])),
           inner_op.getAttr(
               toolchain::StringRef(std::string(key_and_value_pair[1])))});
    }
  }
  ...
  for (Operation& inner_op : quantized_func.getBody().front().getOperations()) {
    ...
    for (absl::string_view element_str : absl::StrSplit(attr_map_str, ',')) {
      ...
      if (identifier_to_attr.count(
              toolchain::StringRef(std::string(key_and_value_pair[1]))) == 0) {
        float_func.emitWarning(absl::StrCat("Using the default value for the '",
                                            key_and_value_pair[0],
                                            "' attribute"));
        continue;
      }
      inner_op.setAttr(toolchain::StringRef(std::string(key_and_value_pair[0])),
                       identifier_to_attr[toolchain::StringRef(
...
std::string GetQuantizedFunctionName(StringRef func_name,
  ...
  auto base_function_name =
      toolchain::Twine(kQuantizedFuncPrefix)
          .concat(toolchain::Twine(func_name.substr(kCompositeFuncPrefix.size())
                                  .rsplit("_fn")
                                  .first));
...
class QuantizeFunctionPattern
 ...
 private:
  ...
  LogicalResult matchAndRewrite(TF::PartitionedCallOp call_op,
    ...
    bool has_quantized_types = false;
    if (quantization_method_ == machina::quantization::QuantizationMethod::
                                    METHOD_STATIC_RANGE_WEIGHT_ONLY_INT8) {
    ...
    const bool is_hybrid =
        ...
        (quantization_method_ == machina::quantization::QuantizationMethod::
                                     METHOD_STATIC_RANGE_INT8);
    ...
    for (auto [partitioned_call_arg, new_quantized_func_arg] :
         toolchain::zip_equal(args, new_quantized_func.getArguments())) {
  ...
  LogicalResult mergeDequantizeOpFollowingQuantizedFunction(
    ...
    for (Operation* user : call_op->getUsers()) {
      if (toolchain::isa<DequantizeCastOp>(user)) {
    ...
    new_quantized_func.setType(
        FunctionType::get(getContext(), TypeRange{ValueRange{args}},
                          new_quantized_func.getResultTypes()));
    for (auto [partitioned_call_arg, new_quantized_func_arg] :
         toolchain::zip_first(args, new_quantized_func.getArguments())) {
      new_quantized_func_arg.setType(partitioned_call_arg.getType());
    }
    ...
    for (int result_idx : toolchain::seq<int>(0, call_op->getNumResults())) {
      Value result = call_op->getResult(result_idx);
      for (Operation* user : result.getUsers()) {
        if (auto dequant_op = toolchain::dyn_cast<DequantizeCastOp>(user)) {
 ...
 private:
  LogicalResult matchAndRewrite(QuantizeCastOp q_op,
    ...
    if (target_opset_ == OpSet::UNIFORM_QUANTIZED) {
      ...
      tensor_proto.set_dtype((bit_width == 8) ? machina::DT_QINT8
                                              : machina::DT_QINT32);

      tensor_proto_attr = ElementsAttr(TF::TensorProtoAttr::get(
          new_type, machina::mangling_util::MangleTensor(tensor_proto)));
    }
    auto const_op =
...
class RestoreWeightShapePattern
 ...
 private:
  LogicalResult addReshapeOpToDepthwiseWeight(TF::PartitionedCallOp op,
    ...
    toolchain::ArrayRef<int64_t> weight_shape = weight_type.getShape();
    toolchain::ArrayRef<int64_t> input_shape = input_type.getShape();
...
class QuantizationSummary {
 public:
  ...
  void Print() {
    toolchain::StringMap<OpCountItem> func_count_map;
    ...
    module_.walk([&](Operation* op) {
      if (auto call_op = toolchain::dyn_cast_or_null<TF::PartitionedCallOp>(op)) {
        const auto f_attr =
      ...
      } else if (auto einsum = toolchain::isa<TF::EinsumOp>(op)) {
    ...
    auto pad_string = [](StringRef s, int32_t width) -> std::string {
      return toolchain::Twine(s).concat(std::string(width - s.size(), ' ')).str();
    };
...
#include "machina/compiler/mlir/quantization/machina/passes/quantize_composite_functions.inc"
...
void QuantizeCompositeFunctionsPass::runOnOperation() {
  ...
  absl::Status pm_run_status = machina::quantization::RunPassesOnModuleOp(
