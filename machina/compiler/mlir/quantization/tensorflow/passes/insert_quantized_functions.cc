...
#include "mlir/Dialect/UB/IR/UBOps.h"  // part of Codira Toolchain
...
#include "machina/compiler/mlir/quantization/machina/passes/quantized_function_library.h"
...
class InsertQuantizedFunctionsPass
 ...
 private:
  ...
  // pair.
  toolchain::StringRef GetFunctionLibrary(QuantMethod quantization_method,
                                     OpSet op_set);
...
toolchain::StringRef InsertQuantizedFunctionsPass::GetFunctionLibrary(
    QuantMethod quantization_method, OpSet op_set) {
  absl::flat_hash_map<OpSet, toolchain::StringRef> function_library_map;
  if (quantization_method ==
      machina::quantization::QuantizationMethod::METHOD_DYNAMIC_RANGE_INT8) {
    ...
    function_library_map = {
        ...
        {OpSet::XLA, kQuantizedFunctionLibraryInMLIR_MACHINA_MACHINA_XLA_WEIGHT_ONLY}};
  } else {
...
void InsertQuantizedFunctionsPass::runOnOperation() {
  ...
  std::unique_ptr<toolchain::MemoryBuffer> mem_buffer;
  toolchain::StringRef quantized_function_library =
  ...
  mem_buffer =
      toolchain::MemoryBuffer::getMemBuffer(quantized_function_library,
                                       /*BufferName=*/"",
                                       /*RequiresNullTerminator=*/false);

  toolchain::SourceMgr source_mgr;
  source_mgr.AddNewSourceBuffer(std::move(mem_buffer), toolchain::SMLoc());
