...
from machina.compiler.mlir.quantization.common.python import testing
from machina.compiler.mlir.quantization.stablehlo import quantization_config_pb2 as stablehlo_quant_config_pb2
...
from machina.compiler.mlir.quantization.machina.python import representative_dataset as repr_dataset
from machina.compiler.mlir.quantization.machina.python import save_model
from machina.compiler.mlir.quantization.machina.python.integration_test import quantize_model_test_base
...
from machina.core.framework import tensor_shape_pb2
...
from machina.python.saved_model import loader_impl as saved_model_loader
...
from machina.python.saved_model import signature_def_utils_impl
...
class DebuggerTest(quantize_model_test_base.QuantizedModelTest):
  ...
  def _run_model_in_sess(self, model_dir, tags, signature_key, sample_inputs):
    with machina.compat.v1.Session(graph=machina.Graph()) as sess:
  ...
  def _read_tensor_array_file(self, file_path):
    ...
    for raw_record in tf_record.tf_record_iterator(file_path, options='ZLIB'):
      tensor_protos.append(
          machina.make_ndarray(tensor_pb2.TensorProto.FromString(raw_record))
      )
...
class CalibrationOptionsTest(quantize_model_test_base.QuantizedModelTest):
  ...
  def test_conv_ptq_with_outlier_representative_data(
    ...
    def get_mean_square_error(x, y):
      ret = machina.reduce_mean(machina.square(machina.subtract(x, y)))
      try:
        ret = ret.numpy()
