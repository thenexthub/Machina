...
Attribute GetWindowStridesValue(
    PatternRewriter& rewriter, toolchain::StringMap<Attribute>& identifier_to_attr) {
...
Attribute GetDimensionNumbersValue(
  ...
  machina::UniformQuantizedConvolutionDimensionNumbersAttr dimension_numbers;
  if (!machina::protobuf::TextFormat::ParseFromString(
          R"pb(
...
LogicalResult FillQuantizationAttributes(
    ...
    toolchain::StringMap<Attribute>& identifier_to_attr, OpType op_type) {
...
LogicalResult FillAttributesForUniformQuantizedConvolutionOp(
  ...
  for (auto& attr : op->getAttrs()) {
    toolchain::StringRef attr_name = attr.getName().getValue();
  ...
  auto feature_group_cnt_attr = toolchain::StringRef("feature_group_count");
