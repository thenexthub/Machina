...
class WritableFileWrapper : public toolchain::raw_ostream {
 public:
...
absl::StatusOr<std::unique_ptr<toolchain::raw_ostream>> CreateMlirDumpFile(
  ...
  TF_ASSIGN_OR_RETURN(std::unique_ptr<toolchain::raw_ostream> file,
...
class PrinterConfig : public mlir::PassManager::IRPrinterConfig {
 ...
 private:
  ...
  toolchain::DenseMap<mlir::Pass*, std::unique_ptr<toolchain::raw_ostream>>
      pass_to_dump_file_before_map_;
  toolchain::DenseMap<mlir::Pass*, std::unique_ptr<toolchain::raw_ostream>>
      pass_to_dump_file_after_map_;
  toolchain::DenseMap<mlir::Pass*, int64_t> pass_to_number_map_;
  ...
  void Dump(mlir::Pass* pass, PrintCallbackFn print_callback, bool is_before) {
    ...
    if (!pass_to_dump_file_map.contains(pass)) {
      std::string filename = toolchain::formatv(
