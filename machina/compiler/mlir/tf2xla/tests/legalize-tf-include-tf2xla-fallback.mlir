// RUN: tf-opt "-xla-legalize-tf=use-tf2xla-fallback=true device-type=MACHINA_MACHINA_XLA_CPU_JIT" -verify-diagnostics %s | FileCheck --check-prefix SUPPORTED_FALLBACK_DEVICE %s
