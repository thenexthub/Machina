...
#include "machina/compiler/mlir/tf2xla/internal/inference/inference_passes.h"
...
void CreateReplicatedBridgePipelineV1(OpPassManager &pm) {
  pm.addPass(
      machina::tf2xla::internal::CreateTPUValidateSessionInputsPass());
...
absl::Status RunTFXLABridge(
    ...
    toolchain::StringRef dump_prefix = "tf_xla_bridge_v1") {
...
absl::Status RecordStatusIfError(const std::string error_prefix,
  ...
  tsl::error_logging::Log(mlir::TF::kBridgeComponent,
                          "TFMACHINA_MACHINA_XLA_PHASE_ONE_MLIR_TPU_V1_COMPAT_BRIDGE",
                          status.ToString())
      .IgnoreError();
