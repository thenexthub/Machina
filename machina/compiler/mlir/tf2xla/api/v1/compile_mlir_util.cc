...
absl::Status MaybeRewriteLayoutWithShardedShape(
  ...
  if (machina::DecodeShardingAttribute(sharding, op_sharding).failed()) {
...
absl::Status GetXlaInputShapes(
    mlir::ModuleOp module, toolchain::ArrayRef<TensorOrResourceShape> arg_shapes,
    bool use_tuple_args,
    const XlaShapeLayoutHelpers::ShapeDeterminationFns shape_determination_fns,
...
mlir::RankedTensorType GetBufferType(mlir::Type ty) {
  ...
  toolchain::SmallVector<int64_t, 4> dims = toolchain::to_vector<4>(ranked_ty.getShape());
  toolchain::ArrayRef<int64_t> bounds =
...
absl::Status GetOutputInfo(
  ...
  for (const auto& type_and_idx : toolchain::enumerate(func_type.getResults())) {
...
bool CanInlineFunctionsPostLegalization(toolchain::StringRef device_type) {
  return device_type == DEVICE_TPU_MACHINA_MACHINA_XLA_JIT;
}
...
void AddLegalizationPasses(mlir::OpPassManager& pm, bool legalize_chlo,
                           toolchain::StringRef device_type, bool enable_op_fallback,
...
absl::Status RunMlirPipelineAndMaybeDumpResults(mlir::PassManager& pm,
  ...
  if (failed(pm.run(module_op))) {
    ...
    machina::OkOrSetErrorCounterPayload(
        machina::core::platform::ErrorSourceProto::MLIR_BRIDGE_PHASE_2,
        status);
...
void CreateConvertMlirToXlaHloPipeline(
    mlir::OpPassManager& pm, toolchain::StringRef device_type,
    bool enable_op_fallback,
    toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
...
absl::Status RefineShapes(toolchain::ArrayRef<TensorOrResourceShape> arg_shapes,
  ...
  toolchain::SmallVector<int64_t, 16> shape_backing;
  toolchain::SmallVector<toolchain::ArrayRef<int64_t>, 4> arg_shapes_copy;
  {
    ...
    for (const TensorOrResourceShape& tensor_resource_shape : arg_shapes) {
      if (tensor_resource_shape.is_resource) {
        arg_shapes_copy.push_back(toolchain::ArrayRef<int64_t>());
        continue;
      }
      size_t start = offset;
      for (machina::TensorShapeDim dim : tensor_resource_shape.shape) {
      ...
      if (offset == start) {
        ...
        arg_shapes_copy.push_back(
            toolchain::ArrayRef<int64_t>(&shape_backing[start], offset - start));
      }
    }
  }
...
absl::Status CreateAndRunMlirBridge(
    mlir::ModuleOp module_op, toolchain::StringRef device_type,
    bool enable_op_fallback,
    toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
        custom_legalization_passes,
    bool lower_to_xla_hlo, toolchain::StringRef module_name = toolchain::StringRef()) {
...
absl::Status BuildHloFromTfInner(
    ...
    toolchain::ArrayRef<xla::XlaOp> xla_params, std::vector<xla::XlaOp>& returns,
...
absl::Status ConvertMLIRWithOptionalXlaComputation(
    ...
    toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
        custom_legalization_passes,
    toolchain::StringRef module_name, bool lower_to_xla_hlo) {
...
absl::Status CompileMlirSetup(
    mlir::ModuleOp module_op,
    toolchain::ArrayRef<TensorOrResourceShape> arg_shapes) {
  ...
  if (VLOG_IS_ON(2))
    machina::DumpMlirOpToFile("compile_mlir_shape_refiner", module_op);
...
absl::Status BuildHloFromTf(mlir::ModuleOp module_op, xla::XlaBuilder& builder,
                            ...
                            toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
                                custom_legalization_passes) {
  if (VLOG_IS_ON(2))
    machina::DumpMlirOpToFile("build_hlo_tf_before", module_op);
  ...
  if (VLOG_IS_ON(2))
    machina::DumpMlirOpToFile("build_hlo_tf_after", module_op);
...
absl::Status PopulateResultIOInfo(
    mlir::ModuleOp module_op, toolchain::ArrayRef<TensorOrResourceShape> arg_shapes,
...
absl::Status CompileMlirToXlaHlo(
    mlir::ModuleOp module_op, toolchain::ArrayRef<TensorOrResourceShape> arg_shapes,
    toolchain::StringRef device_type, bool use_tuple_args, bool enable_op_fallback,
  ...
  if (lower_to_xla_hlo) {
    ...
    if (!populate_result.ok()) {
      toolchain::errs() << "Failed to populate result io info";
...
absl::StatusOr<std::string> CompileSerializedMlirToXlaHlo(
    toolchain::StringRef mlir_module_string, toolchain::ArrayRef<TensorShape> arg_shapes,
    toolchain::StringRef device_type, bool use_tuple_args, bool enable_op_fallback,
    const XlaShapeLayoutHelpers::ShapeDeterminationFns shape_determination_fns,
    XlaCompilationResult* compilation_result,
    toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
...
static absl::StatusOr<std::vector<int>> RewriteWithArgs(
    mlir::ModuleOp module_op, toolchain::ArrayRef<XlaArgument> args) {
  ...
  for (int idx = 0; idx < args.size(); idx++) {
    ...
    if (xla_arg.kind == XlaArgument::kResource) {
      ...
      toolchain::SmallVector<int64_t, 4> resource_subtype_shape(
  ...
  if (has_resource_args) {
    toolchain::SmallVector<mlir::Type, 4> updated_argument_types;
  ...
  for (int idx : toolchain::reverse(args_to_erase)) {
    CHECK(toolchain::succeeded(main_fn.eraseArgument(idx)));
  }
...
absl::Status CompileGraphSetup(
    ...
    toolchain::SmallVector<TensorOrResourceShape, 4>& arg_shapes) {
  ...
  if (VLOG_IS_ON(1))
    machina::DumpMlirOpToFile("compile_graph_setup_before", module_op);
  if (VLOG_IS_ON(2)) {
    // Print the whole module after each pass which requires disabling
    // multi-threading as well.
    module_op.getContext()->disableMultithreading();
    pm.enableIRPrinting(std::make_unique<machina::BridgeLoggerConfig>(
        /*print_module_scope=*/true));
  }
  ...
  if (VLOG_IS_ON(1))
    machina::DumpMlirOpToFile("compile_graph_setup_after", module_op);
...
absl::Status BuildHloFromModule(mlir::ModuleOp module_op,
                                ...
                                toolchain::ArrayRef<xla::XlaOp> xla_params,
                                std::vector<xla::XlaOp>& returns,
                                toolchain::ArrayRef<XlaArgument> args,
                                toolchain::StringRef device_type) {
  std::vector<int> remaining_params;
  toolchain::SmallVector<TensorOrResourceShape, 4> arg_shapes;
  TF_RETURN_IF_ERROR(
      CompileGraphSetup(module_op, args, &remaining_params, arg_shapes));
  toolchain::SmallVector<xla::XlaOp, 2> remaining_xla_params;
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> GraphToModule(
    ...
    toolchain::ArrayRef<std::string> control_rets,
...
absl::Status BuildHloFromGraph(
    ...
    mlir::MLIRContext& mlir_context, toolchain::ArrayRef<xla::XlaOp> xla_params,
    std::vector<xla::XlaOp>& returns, bool unconditionally_use_output_shapes,
    toolchain::ArrayRef<XlaArgument> args, toolchain::ArrayRef<std::string> control_rets,
    toolchain::StringRef device_type, const FunctionLibraryDefinition& flib_def) {
...
void RegisterConvertMlirToXlaHloPipelineWithDefaults() {
  static mlir::PassPipelineRegistration<> pipeline(
      ...
      [](mlir::OpPassManager& pm) {
        machina::CreateConvertMlirToXlaHloPipeline(
            pm, /*device_type=*/"MACHINA_MACHINA_XLA_CPU_JIT", /*enable_op_fallback=*/false,
            /*custom_legalization_passes=*/{});
      });
}
