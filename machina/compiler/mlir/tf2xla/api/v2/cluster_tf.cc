...
#include "machina/compiler/mlir/tf2xla/api/v2/device_type.pb.h"
...
absl::Status RunTFXLABridge(
    ModuleOp module,
    toolchain::function_ref<void(OpPassManager &pm)> pipeline_builder,
    toolchain::StringRef module_name = toolchain::StringRef(),
    toolchain::StringRef dump_prefix = "tf_xla_bridge_v2") {
  ...
  ::machina::applyTensorflowAndCLOptions(bridge);
  ...
  if (VLOG_IS_ON(1) ||
    ...
    ::machina::DumpMlirOpToFile(
        ...
        module, toolchain::StringRef(), &bridge);
  }

  if (VLOG_IS_ON(2) ||
      DEBUG_DATA_DUMPER()->ShouldDump(module_name.str(),
                                      kDebugGroupBridgePhase1Clustering)) {
    ::machina::tf2xla::internal::EnablePassIRPrinting(
...
absl::Status RecordIfErrorStatus(const std::string error_prefix,
  ...
  machina::metrics::UpdateTfMlirBridgeFirstPhaseCounter(
      /*bridge_type*/ bridge_type, /*bridge_version=*/"v2", device_type,
      /*fallback_enabled=*/fallback_enabled,
      /*result=*/"failure");

  tsl::OkOrSetErrorCounterPayload(
      machina::core::platform::ErrorSourceProto::MLIR_BRIDGE_PHASE_1,
      status);

  std::string bridge_subcomponent = "TFMACHINA_MACHINA_XLA_PHASE_ONE_MLIR_TPU_BRIDGE";
  if (device_type != "tpu") {
    bridge_subcomponent = "TFMACHINA_MACHINA_XLA_PHASE_ONE_MLIR_CPU/GPU_BRIDGE";
  }
...
void CreateReplicatedClusteringPipeline(OpPassManager &pm,
  ...
  pm.addPass(machina::tf2xla::internal::CreateTPUValidateInputsPass());
  pm.addNestedPass<FuncOp>(
      mlir::TF::CreateCanonicalizeCompileAndReplicateAttributesPass());
  machina::tf2xla::internal::AddReplicatedBridgeClusteringPipelinePasses(
      pm, module_name);
}
...
absl::Status RunFunctionTf2xlaClusteringBridge(
    ...
    bool is_in_fallback_enabled_mode, toolchain::StringRef module_name) {
