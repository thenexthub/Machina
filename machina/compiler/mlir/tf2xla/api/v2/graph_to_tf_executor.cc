...
#include "machina/compiler/mlir/tf2xla/internal/node_order.h"
#include "machina/compiler/tf2xla/functionalize_control_flow.h"
...
#include "machina/core/graph/graph_debug_info_builder.h"
#include "machina/core/graph/graph_node_util.h"
...
#include "machina/core/protobuf/saver.pb.h"
...
void LoadImporterDialects(mlir::MLIRContext& context) {
  ...
  for (toolchain::StringRef name : registry.getDialectNames())
...
class NameUniquifier : public OpOrArgNameMapper {
 ...
 private:
  bool IsUnique(toolchain::StringRef name) override {
...
class ImporterBase {
 protected:
  explicit ImporterBase(
      ...
      toolchain::StringRef function_name_for_debug_info = "")
  ...
  // of the function.
  absl::Status Convert(toolchain::StringRef func_name, mlir::FunctionType func_type,
  ...
  // interface.
  absl::Status ConvertLibFunction(toolchain::StringRef func_name);
  ...
  // nodes.
  toolchain::ArrayRef<Node*> GetOrderedNodes() const { return ordered_nodes_; }
 ...
 private:
  ...
  using ElementSubtypes = toolchain::SmallVector<TensorType, 1>;
  ...
  absl::StatusOr<mlir::ElementsAttr> ConvertTensorProto(
    ...
    return machina::ConvertTensorProto(value, &builder_);
  }
  ...
  mlir::Operation* CreateOperation(
      const Node& node, toolchain::StringRef node_type_name,
      const mlir::OperationState& result,
      const toolchain::SmallVectorImpl<mlir::Value>& control_operands);
  ...
  absl::Status ConvertFunctionArgAndRets(
      ...
      toolchain::ArrayRef<mlir::Type> arg_types,
      const absl::InlinedVector<OutputTensor, 4>& arg_nodes,
      const absl::InlinedVector<OutputTensor, 4>& ret_nodes,
      const absl::InlinedVector<Node*, 4>& control_ret_nodes);
  ...
  toolchain::StringRef function_name_for_debug_info_;
  ...
  toolchain::DenseSet<mlir::StringAttr> unmodelled_op_names_;
...
ImporterBase::CreatePlaceholderNodeForFeed(
  ...
  for (const machina::Edge* edge : node->out_edges()) {
...
absl::Status ImporterBase::AddNodesToShapeRefiner(
  ...
  for (Node* node : ordered_nodes_) {
    ...
    auto set_shape_from_list_attr = [&](const AttrValue* attr) {
      ...
      for (const auto& shape : toolchain::enumerate(list.shape())) {
...
absl::StatusOr<TensorType> ImporterBase::ConvertDataTypeAndShape(
  ...
  if (dtype == DT_VARIANT)
    ...
    TF_RETURN_IF_ERROR(
        ::machina::ConvertDataType(dtype, builder, &element_type));
...
absl::StatusOr<TensorType> ImporterBase::ConvertElementTypeAndShape(
  ...
  return GetTypeFromTFTensorShape(
      toolchain::ArrayRef(dimensions.begin(), dimensions.end()), element_type);
}
...
absl::StatusOr<ImporterBase::ElementSubtypes> ImporterBase::ConvertSubtypes(
  ...
  for (const auto& subtype : *handle_subtypes) {
    mlir::Type element_type;
    TF_RETURN_IF_ERROR(
        ::machina::ConvertDataType(subtype.dtype, builder, &element_type));
    ...
    case AttrValue::kList: {
      if (!value.list().func().empty()) {
        ...
        return builder_.getArrayAttr(
            toolchain::ArrayRef(attrs.begin(), attrs.end()));
      }
...
absl::Status ImporterBase::ConvertLibFunction(toolchain::StringRef func_name) {
...
absl::Status ImporterBase::PrepareConvert(const Graph& graph,
  ...
  TF_RETURN_IF_ERROR(::machina::ConvertGraphDefToGraph(
...
absl::Status ImporterBase::Convert(
    toolchain::StringRef func_name, mlir::FunctionType func_type,
    const absl::InlinedVector<OutputTensor, 4>& arg_nodes,
    const absl::InlinedVector<OutputTensor, 4>& ret_nodes,
    const absl::InlinedVector<Node*, 4>& control_ret_nodes,
    toolchain::ArrayRef<mlir::NamedAttribute> attrs) {
  ...
  if (!specs_.enable_shape_inference) {
    ...
    bool all_equal = true;
    for (auto it :
         toolchain::zip_first(graph.getResults(), fetch.getOperandTypes())) {
...
absl::Status ImporterBase::ConvertFunctionArgAndRets(
  ...
  // construction.
  toolchain::SmallVector<mlir::NamedAttrList, 4> arg_attrs;
  arg_attrs.resize(func.getNumArguments());
  toolchain::SmallVector<mlir::NamedAttrList, 4> ret_attrs;
  ...
  auto* bb = &func.front();
  toolchain::SmallDenseMap<std::pair<Node*, int>, mlir::Value, 4>
      arg_nodes_to_values;
  for (int i = 0, e = arg_types.size(); i < e; ++i) {
    ...
    for (auto& control_use : toolchain::make_early_inc_range(control_uses))
  ...
  toolchain::SmallVector<mlir::Value, 8> inst_to_return;
  for (const auto& ret_and_idx : toolchain::enumerate(ret_nodes)) {
    ...
    if (ret.node->IsRetval()) {
      ...
      auto island_op = toolchain::cast<mlir::tf_executor::IslandOp>(inst);
  ...
  func.setAllArgAttrs(
      toolchain::to_vector<4>(toolchain::map_range(arg_attrs, [&](NamedAttrList& list) {
        return list.getDictionary(context_);
      })));
  func.setAllResultAttrs(
      toolchain::to_vector<4>(toolchain::map_range(ret_attrs, [&](NamedAttrList& list) {
        return list.getDictionary(context_);
      })));
...
mlir::Location ImporterBase::GetLocation(const Node& node) {
  ...
  auto create_location = [&](toolchain::StringRef name,
                             toolchain::StringRef function_name) -> mlir::Location {
    ...
    toolchain::SmallVector<mlir::Location, 4> locations;
    ...
    // return the name loc.
    auto callsite_locs = toolchain::ArrayRef(locations).drop_front();
  ...
  if (original_nodes.empty()) {
    ...
    // of the node_def.
    toolchain::SmallVector<mlir::Location, 4> node_locations;
...
absl::StatusOr<mlir::FunctionType> ImporterBase::InferLibFunctionType(
  ...
  toolchain::SmallVector<mlir::Type, 4> arg_types;
  if (specs_.inputs.empty()) {
    ...
    arg_types.reserve(fbody.arg_types.size());
    for (const auto& it : toolchain::enumerate(specs_.inputs)) {
      mlir::Type element_type;
  ...
  toolchain::SmallVector<mlir::Type, 4> ret_types;
...
class GraphDefImporter : public ImporterBase {
 ...
 private:
  ...
  absl::Status GetControlRetsFromGraph(
      toolchain::ArrayRef<std::string> control_outputs,
      absl::InlinedVector<Node*, 4>* control_ret_nodes);
};
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> GraphDefImporter::Convert(
  ...
  auto scope_exit = [&]() {
    ...
    return toolchain::make_scope_exit(std::move(cleanup));
  }();

  VLOG(2) << "Importing: "
          << ::machina::DumpGraphToFile("tf_mlir_importer_base", graph,
                                           &flib_def);
  ...
  if (specs.graph_as_function) {
    ...
    toolchain::interleave(arg_nodes, ss, node_name, ",");
    auto inputs = b.getNamedAttr("inputs", b.getStringAttr(ss.str()));
    s.clear();
    toolchain::interleave(ret_nodes, ss, node_name, ",");
    auto outputs = b.getNamedAttr("outputs", b.getStringAttr(ss.str()));
    s.clear();
    toolchain::interleave(specs.control_outputs, ss, ",");
    ...
    if (!specs.inputs.empty() || !specs.outputs.empty() ||
      ...
      auto inputs = b.getNamedAttr("inputs", b.getStringAttr(ss.str()));
      s.clear();
      toolchain::interleave(specs.outputs, ss, ",");
      auto outputs = b.getNamedAttr("outputs", b.getStringAttr(ss.str()));
      s.clear();
      toolchain::interleave(specs.control_outputs, ss, ",");
      auto control_outputs =
  ...
  const toolchain::StringRef& graph_func_name =
  ...
  VLOG(2) << "Imported: "
          << machina::DumpMlirOpToFile("tf_mlir_imported_base",
                                          module.get());
  return module;
}
...
absl::StatusOr<mlir::FunctionType> GraphDefImporter::InferMainFunctionType(
  ...
  for (const auto& input_and_idx : toolchain::enumerate(specs.inputs)) {
...
GraphDefImporter::GetArgsRetsAndTypesFromFunctionGraph(
  ...
  for (const auto& arg_node_and_idx : toolchain::enumerate(*arg_nodes)) {
  ...
  for (const auto& ret_node_and_idx : toolchain::enumerate(*ret_nodes)) {
...
absl::Status GraphDefImporter::GetControlRetsFromGraph(
  ...
  toolchain::SmallDenseMap<toolchain::StringRef, int32_t> controls_to_idx;
  for (const auto& control_and_idx : toolchain::enumerate(control_outputs))
  ...
  for (auto node_and_name : toolchain::zip(*control_ret_nodes, control_outputs))
...
bool IsCompiledNode(const Node* n) {
  return n->attrs().Find(machina::kTpuReplicateAttr) ||
         n->attrs().Find(machina::kCompileDeviceTypeAttr);
}
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> ConvertGraphToTfExecutor(
  ...
  if (specs.upgrade_legacy) {
    ...
    TF_RETURN_WITH_CONTEXT_IF_ERROR(
        ...
        machina::kFunctionalizeControlFlowFailureMessage);
  }
