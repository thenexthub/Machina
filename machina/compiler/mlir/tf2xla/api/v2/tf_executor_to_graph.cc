...
class Exporter {
 public:
  ...
  static absl::Status ConvertLibFunction(
      ...
      FunctionLibraryDefinition* flib_def,
      toolchain::SmallDenseSet<FuncOp>& visited_functions);
 ...
 private:
  ...
  absl::Status AddFetchNode(FuncOp function, mlir::tf_executor::FetchOp fetch,
                            toolchain::ArrayRef<toolchain::StringRef> names);
  ...
  absl::StatusOr<std::unique_ptr<NodeDef>> GetArgumentNode(
      BlockArgument arg, unsigned index, toolchain::StringRef name);
  ...
  toolchain::DenseMap<BlockArgument, Node*> args_;
...
absl::StatusOr<std::unique_ptr<NodeDef>> Exporter::GetArgumentNode(
  ...
  if (auto resource_type =
    ...
    toolchain::ArrayRef<mlir::TensorType> subtypes = resource_type.getSubtypes();
    if (!subtypes.empty()) {
  ...
  toolchain::ArrayRef<mlir::NamedAttribute> func_arg_i_attrs =
...
absl::StatusOr<std::unique_ptr<NodeDef>> Exporter::GetReturnNode(
    FuncOp function, Value operand, unsigned index, toolchain::StringRef name) {
  auto node_def = std::make_unique<NodeDef>();
  if (!name.empty())
    node_def->set_name(ParseTensorName(name.str()).node());
  else
  ...
  toolchain::ArrayRef<mlir::NamedAttribute> func_res_i_attrs =
...
absl::Status Exporter::AddEdgeBetweenNodes(Value src, Node* dst_node,
  ...
  if (auto input_result = mlir::dyn_cast<mlir::OpResult>(src)) {
    ...
    if (auto next_iter_source =
            toolchain::dyn_cast<mlir::tf_executor::NextIterationSourceOp>(
                input_inst))
...
absl::Status Exporter::AddEdge(Operation* inst) {
  ...
  // later.
  if (auto fetch = toolchain::dyn_cast<mlir::tf_executor::FetchOp>(inst)) {
    for (auto operand_and_idx : toolchain::enumerate(fetch.getOperands())) {
  ...
  if (auto next_iter_sink =
          toolchain::dyn_cast<mlir::tf_executor::NextIterationSinkOp>(inst)) {
    auto* dst_node = nodes_[inst];
    TF_RETURN_IF_ERROR(
        AddEdgeBetweenNodes(next_iter_sink.getInput(), dst_node, 0));
    for (auto control_and_idx :
         toolchain::enumerate(next_iter_sink.getControlInputs()))
  ...
  if (toolchain::isa<mlir::tf_executor::NextIterationSourceOp>(inst)) {
  ...
  // edges.
  if (auto island = toolchain::dyn_cast<mlir::tf_executor::IslandOp>(inst)) {
    for (auto operand_and_idx : toolchain::enumerate(op->getOperands()))
  ...
  for (auto operand_and_idx : toolchain::enumerate(inst->getOperands()))
    TF_RETURN_IF_ERROR(
...
absl::StatusOr<std::unique_ptr<Graph>> Exporter::Convert(
  ...
  toolchain::SmallVector<toolchain::StringRef, 2> unique_output_names;
  auto dict_attr =
  ...
  auto graph_op = toolchain::cast<mlir::tf_executor::GraphOp>(block.front());
  ...
  if (!output_names.empty()) {
    ...
    toolchain::DenseMap<Operation*, toolchain::StringRef> output_op_to_name;
    toolchain::StringMap<Operation*> name_to_op;
    for (const auto& it : toolchain::enumerate(graph_op.GetFetch().getOperands())) {
      // Skip control rets.
  ...
  if (!input_names.empty()) {
    ...
    for (const auto& it : toolchain::enumerate(function.getArguments())) {
  ...
  for (auto it : toolchain::enumerate(block.getArguments())) {
    int index = it.index();
    auto arg = it.value();
  ...
  auto convert_called_function = [&](toolchain::StringRef name) {
  ...
  for (Operation& inst : graph_op.GetBody()) {
    ...
    if (toolchain::isa<mlir::tf_executor::NextIterationSourceOp>(inst)) {
      ...
      continue;
    } else if (auto fetch = toolchain::dyn_cast<mlir::tf_executor::FetchOp>(inst)) {
      TF_RETURN_IF_ERROR(
          exporter.AddFetchNode(function, fetch, unique_output_names));
    } else if (auto island =
                   toolchain::dyn_cast<mlir::tf_executor::IslandOp>(inst)) {
      Operation& inner_op = island.GetBody().front();
      auto op_name = GetTensorFlowOpName(inner_op.getName().getStringRef());
      if (toolchain::isa<FuncOp>(inner_op) && op_name.ok()) {
...
absl::Status Exporter::ConvertLibFunction(
  ...
  toolchain::SmallVector<mlir::NamedAttribute, 8> funcAttrs(
