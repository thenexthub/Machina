...
void AddReplicatedBridgeClusteringPipelinePasses(OpPassManager& pm,
  ...
  const toolchain::SmallVector<std::string, 4> ops_to_preserve = {
      "tf.TPUReplicateMetadata", "tf.TPUCompilationResult",
      "tf.TPUReplicatedOutput"};
  bool strict_clusters =
      machina::GetMlirCommonFlags()->tf_mlir_enable_strict_clusters;
  ...
  pm.addPass(machina::tf2xla::internal::CreateTPUClusterFormationPass(
      strict_clusters));
  ...
  pm.addPass(
      machina::tf2xla::internal::CreateMarkOpsForOutsideCompilationPass());
  pm.addPass(machina::tf2xla::internal::
                 CreateExtractHeadTailOutsideCompilationPass());
  pm.addPass(
      machina::tf2xla::internal::CreateExtractOutsideCompilationPass());
  pm.addNestedPass<FuncOp>(
  ...
  pm.addNestedPass<FuncOp>(
      machina::tf2xla::internal::CreateHoistBroadcastReadPass());
  pm.addNestedPass<FuncOp>(
      machina::tf2xla::internal::CreateXlaBroadcastPass());
  ...
  pm.addPass(
      machina::tf2xla::internal::CreateTPUShardingIdentificationPass());
  pm.addNestedPass<FuncOp>(
  ...
  pm.addNestedPass<FuncOp>(
      machina::tf2xla::internal::CreateVerifyClusteringPass());
}
...
void AddNonReplicatedBridgeClusteringPipelinePasses(OpPassManager& pm) {
  ...
  pm.addPass(machina::tf2xla::internal::CreateXlaClusterFormationPass());
  // Running canonicalizer before decomposing resource ops in cluster helps the
  // latter pass to converge faster as it does not have to spend time folding
  // away dead ops.
  pm.addNestedPass<FuncOp>(mlir::createCanonicalizerPass());
