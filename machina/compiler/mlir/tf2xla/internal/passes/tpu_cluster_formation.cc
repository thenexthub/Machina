...
constexpr toolchain::StringRef kDeviceAttr = "device";
constexpr toolchain::StringRef kNameAttr = "name";
constexpr toolchain::StringRef kNumCoresPerReplicaAttr = "num_cores_per_replica";
constexpr toolchain::StringRef kNumReplicasAttr = "num_replicas";
constexpr toolchain::StringRef kMirroredVariableIndicesAttr =
    "_mirrored_variable_indices";

constexpr toolchain::StringRef kBadReplicateInfoAttrMsg =
    "requires '_replication_info' string attribute";

// Mapping for `_replication_info` attribute to TPUReplicateMetadata attributes.
using MetadataMap = toolchain::SmallDenseMap<toolchain::StringRef, NamedAttrList, 8>;
...
using OpSetVector = toolchain::SmallSetVector<Operation*, 8>;

// Mapping for `_replication_info` attribute to ops of a cluster.
using ClusterMap = toolchain::SmallDenseMap<toolchain::StringRef, OpSetVector, 8>;
...
LogicalResult CollectMetadata(Block* block, MetadataMap* metadata_map) {
  ...
  for (Operation& op : toolchain::make_early_inc_range(*block)) {
    auto metadata_op = toolchain::dyn_cast<mlir::TF::TPUReplicateMetadataOp>(op);
...
LogicalResult HasValidDeviceTypeAttribute(Block* block) {
  ...
  std::set<toolchain::StringRef> device_types;
  ...
  if (device_types.size() > 1) {
    return block->getParentOp()->emitError()
           ...
           << "' attribute values (" << toolchain::join(device_types, ",")
...
Operation* getOpClusterDataDependency(
  ...
  op->walk([&](Operation* inner_op) {
    ...
    toolchain::SmallVector<Operation*, 4> candidates;
...
// dependencies.
mlir::FailureOr<toolchain::SmallSetVector<Operation*, 8>> CollectClusterSuccessorOps(
  ...
  for (Operation& op : toolchain::make_range(rback, rfront)) {
  ...
  for (Operation& op : toolchain::make_range(front, back)) {
    if (cluster_ops.contains(&op)) continue;
...
toolchain::SmallVector<Value, 8> CollectClusterResults(
    Block* block, const OpSetVector& cluster_ops) {
  toolchain::SmallVector<Value, 8> results;
...
mlir::tf_device::ClusterOp CreateClusterOp(
    Block* block, const OpSetVector& cluster_ops, toolchain::ArrayRef<Value> results,
    toolchain::ArrayRef<Operation*> cluster_successor_ops) {
  ...
  for (Operation* cluster_op : cluster_ops) {
    ...
    cluster_op->walk([&](Operation* inner_op) {
      ...
      if (auto attr = inner_op->getAttrOfType<StringAttr>(kDeviceAttr)) {
        ...
        // machina/python/tpu/tpu_feed.py
        if (!machina::IsTPUReplicatedCore(attr.getValue())) {
  ...
  for (auto ret_vals : toolchain::zip(results, cluster.getResults())) {
    Value old_ret = std::get<0>(ret_vals);
    Value new_ret = std::get<1>(ret_vals);
    for (auto& use : toolchain::make_early_inc_range(old_ret.getUses())) {
...
template <typename T>
std::tuple<T, toolchain::SmallVector<mlir::TF::IdentityOp, 4>> GetSingleUserOfType(
    OpResult result) {
  toolchain::SmallVector<mlir::TF::IdentityOp, 4> identity_ops;

  do {
    Operation* user = result.hasOneUse() ? *result.getUsers().begin() : nullptr;
    if (auto t = toolchain::dyn_cast_or_null<T>(user)) {
      return std::make_tuple(t, identity_ops);
    } else if (auto identity =
                   toolchain::dyn_cast_or_null<mlir::TF::IdentityOp>(user)) {
...
using PartitionedClusterOutputMap = absl::flat_hash_map<
    uint64_t, toolchain::SmallVector<mlir::TF::TPUPartitionedOutputV2Op, 8>>;
...
Operation* BuildPartitionedOutputs(
    ...
    PartitionedClusterOutputMap& partitioned_outputs,
    toolchain::SmallVector<mlir::TF::IdentityOp, 8>& erase_list,
    toolchain::SmallVector<Type, 8>& result_types, int num_replicas) {
  Operation* result_op;
  toolchain::SmallVector<Value, 8> results;
  ...
  for (uint64_t result_id = 0; result_id < num_results; ++result_id) {
    ...
    toolchain::SmallVector<mlir::TF::TPUPartitionedOutputV2Op, 8>& ops =
        search->second;
    for (auto [replica_id, partitioned_output] : toolchain::enumerate(ops)) {
      for (auto [core_id, result] :
           toolchain::enumerate(partitioned_output->getResults())) {
...
toolchain::SmallVector<Type, 8> GetClusterResultTypes(
  ...
  if (partitioned_outputs.empty()) {
    ...
    for (auto [output_id, result_type] :
         toolchain::enumerate(cluster_result_types)) {
...
LogicalResult ReplicateCluster(mlir::tf_device::ClusterOp cluster,
  ...
  toolchain::SmallVector<mlir::TF::TPUReplicatedInputOp, 8> replicated_input_ops;
  toolchain::SmallSet<mlir::TF::TPUReplicatedInputOp, 8> seen_ops;
  mlir::visitUsedValuesDefinedAbove(
      cluster.getBody(), cluster.getBody(), [&](mlir::OpOperand* operand) {
        Operation* def = operand->get().getDefiningOp();
        if (auto ri =
                toolchain::dyn_cast_or_null<mlir::TF::TPUReplicatedInputOp>(def)) {
        ...
        if (auto pi = toolchain::dyn_cast_or_null<mlir::TF::TPUPartitionedInputV2Op>(
                def)) {
          ...
          for (auto operand : pi.getInputs()) {
            if (auto ri =
                    toolchain::dyn_cast_or_null<mlir::TF::TPUReplicatedInputOp>(
  ...
  toolchain::SmallVector<int64_t, 8> mirrored_variable_indices;
  ...
  toolchain::SmallVector<std::pair<ValueRange, Type>, 8> replicated_inputs;
  toolchain::SmallVector<Value, 8> packed_inputs;
  toolchain::SmallVector<mlir::TF::TPUReplicatedInputOp, 8> replicated_ops;
  toolchain::SmallVector<mlir::TF::TPUReplicatedInputOp, 8> packed_ops;
  for (const auto& pos_and_input : toolchain::enumerate(replicated_input_ops)) {
  ...
  // list.
  toolchain::SmallVector<mlir::TF::TPUReplicatedInputOp, 8>
  ...
  for (const auto& pos_and_input :
       toolchain::enumerate(ordered_tpu_replicate_inputs)) {
  ...
  auto replicate_op = mlir::tf_device::ReplicateOp::create(
      ...
      toolchain::SmallDenseMap<toolchain::StringRef,
  ...
  uint64_t offset = 0;
  for (auto [idx, result] : toolchain::enumerate(cluster.getResults())) {
    ...
    auto replicate_outputs = toolchain::make_range(
        std::next(replicate_op.result_begin(), offset),
        std::next(replicate_op.result_begin(), offset + num_replicas));
    for (auto& use : toolchain::make_early_inc_range(result.getUses())) {
      Operation* def = use.getOwner();
      if (!toolchain::isa<mlir::TF::TPUReplicatedOutputOp>(def)) {
  ...
  toolchain::SmallSet<Operation*, 4> partitioned_inputs;
  for (auto input_and_block_arg :
       toolchain::zip(ordered_tpu_replicate_inputs,
    ...
    for (auto& use : input->getUses()) {
      auto pi =
          toolchain::dyn_cast<mlir::TF::TPUPartitionedInputV2Op>(use.getOwner());
      if (pi) {
...
LogicalResult FormClustersInFunction(
    ...
    const mlir::TF::SideEffectAnalysis::Info& side_effect_analysis,
    bool strict_clusters) {
  if (!toolchain::hasSingleElement(func))
  ...
  auto remove_result = func.walk([&](Operation* op) {
    if (!toolchain::isa<mlir::TF::TPUReplicatedInputOp,
