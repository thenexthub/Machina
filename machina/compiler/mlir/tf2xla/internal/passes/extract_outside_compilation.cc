...
// as inputs.
FuncOp BuildFunction(toolchain::ArrayRef<Operation*> ops,
                     toolchain::ArrayRef<Value> inputs,
                     toolchain::ArrayRef<Value> outputs, OpBuilder* builder) {
  ...
  toolchain::SmallVector<Value, 4> results_after_mapping;
...
Operation* CreateSendFromHostOp(OpBuilder& builder, Location loc,
                                ...
                                toolchain::StringRef communication_key) {
  if (device_ordinal)
...
mlir::TF::IfRegionOp CloneEmptyIfWithPredicate(mlir::TF::IfRegionOp if_region,
  ...
  auto host_side_if = builder.create<mlir::TF::IfRegionOp>(
      if_region.getLoc(), toolchain::SmallVector<Type, 4>{}, if_region.getCond(),
...
CreateCompilationKeyPlaceholder(Location loc, OpBuilder& builder) {
  ...
  return builder.create<mlir::TF::_XlaCompileMlirPlaceholderProgramKeyOp>(
      loc, /*program=*/result_type, toolchain::ArrayRef<Value>{});
}

// Creates a `tf_device.launch` to wrap cluster ops.
mlir::tf_device::LaunchOp CreateLaunchOpForOutsideCluster(
    OpBuilder& builder, Operation* loc_op, toolchain::StringRef host_device,
    toolchain::SmallVector<Value, 4>& return_value_from_host) {
  toolchain::SmallVector<Type, 4> host_result_types;
...
bool HasDynamicOutputs(const toolchain::SmallSetVector<Operation*, 4>& cluster_ops) {
...
// static shape.
toolchain::SmallSetVector<Value, 4> GetStaticExternalOperands(
    mlir::tf_device::ClusterOp device_cluster,
    const toolchain::SmallSetVector<Operation*, 4>& cluster_ops) {
  toolchain::SmallSetVector<Value, 4> external_values;
  for (Operation* op : cluster_ops) {
    op->walk([&](Operation* walked_op) {
      if (toolchain::isa<mlir::TF::_XlaRecvAtHostOp, mlir::TF::_XlaRecvAtHostV2Op,
                    mlir::TF::_XlaSendFromHostOp,
                    mlir::TF::_XlaSendFromHostV2Op>(walked_op))
        return WalkResult::advance();
      for (Value v : walked_op->getOperands()) {
        if (!machina::TypeValidForXLA(v.getType())) continue;
        if (auto* defining_op = v.getDefiningOp()) {
          if (!op->isAncestor(defining_op) &&
              ...
              // created cluster.
              !toolchain::isa<mlir::TF::_XlaRecvAtHostOp,
...
// `cluster_ops`.
toolchain::SmallSetVector<Value, 4> GetAllExternalOperands(
    const toolchain::SmallSetVector<Operation*, 4>& cluster_ops) {
  toolchain::SmallSetVector<Value, 4> external_values;
  for (Operation* op : cluster_ops) {
    op->walk([&](Operation* walked_op) {
...
toolchain::SmallSetVector<Value, 4> GetExternalOperands(
    mlir::tf_device::ClusterOp device_cluster,
    const toolchain::SmallSetVector<Operation*, 4>& cluster_ops) {
...
void GetExternalOutputs(const toolchain::SmallSetVector<Operation*, 4>& cluster_ops,
                        toolchain::SmallSetVector<Value, 4>& external_outputs,
                        toolchain::SmallVector<Value, 4>& host_outputs) {
  bool has_dynamic_outputs = HasDynamicOutputs(cluster_ops);
  toolchain::SmallVector<Value, 4> tmp_host_outputs;
  for (Operation* op : cluster_ops) {
    toolchain::SmallDenseSet<Operation*, 4> user_set;
    for (Operation* user : op->getUsers()) {
      ...
      if (!HasOutsideCompilationAncestor(user) || has_dynamic_outputs ||
        ...
        for (Value v : user->getOperands()) {
          if (machina::TypeValidForXLA(v.getType()) &&
              v.getDefiningOp() == op &&
              !toolchain::isa<mlir::tf_device::ReturnOp>(user))
...
LogicalResult GetShardShapedType(Operation* context_op,
  ...
  toolchain::SmallVector<int64_t, 4> shape;
...
LogicalResult CreateHostComputeMap(
  ...
  // Get output types.
  toolchain::SmallVector<Type, 4> shard_output_types;
  toolchain::SmallVector<Type, 4> full_output_types;
  ...
  // and outputs.
  toolchain::SmallVector<Value, 4> manual_inputs;
  ...
  for (auto [full_type, out] :
       toolchain::zip(full_output_types, host_compute.getResults())) {
...
void CreateHostComputeNotMap(OpBuilder& builder, Location loc,
                             ...
                             SmallVector<Operation*, 4>& host_compute_out_ops) {
  toolchain::SmallVector<Type, 4> device_output_types;
  for (const auto& output : outputs)
...
bool ShouldCloseCluster(toolchain::ArrayRef<Value> outputs) {
  ...
  for (Value v : outputs) {
    ...
    if (!machina::TypeValidForXLA(v.getType()))
      for (const Operation* user : v.getUsers())
        if (!toolchain::isa<mlir::tf_device::ReturnOp>(user))
...
void ReplaceExternalOperandUsage(ArrayRef<Value> external_operands,
  ...
  for (auto result : toolchain::zip(external_operands, recv_at_host->getResults())) {
...
bool HasDynamicOutputs(toolchain::ArrayRef<Value> outputs) {
  for (Value v : outputs) {
    if (mlir::TF::CanBeRefined(v.getType())) {
      return true;
    }
...
void ReplaceExternalOutputUsage(ArrayRef<Value> external_outputs,
  ...
  for (auto result : toolchain::zip(external_outputs, host_compute_outputs)) {
...
std::pair<std::string, std::string> MakeCommunicationKeys(
  ...
  std::string args_communication_key =
      toolchain::formatv("host_compute_channel_{0}_args", (communication_key_index))
          .str();
  std::string retvals_communication_key =
      toolchain::formatv("host_compute_channel_{0}_retvals",
  ...
  if (clustered_ops.size() == 1 && toolchain::isa<mlir::TF::IfRegionOp>(op) &&
      external_operands.size() == 1) {
    args_communication_key =
        toolchain::formatv("if_predicate_channel_{0}", (communication_key_index))
            .str();
  }
...
Operation* CreateHostOps(ArrayRef<Operation*> clustered_ops,
  ...
  toolchain::SmallVector<Type, 4> host_operand_types;
...
void CloneFirstHost(toolchain::SmallVector<IRMapping>& core_to_mapping,
    ...
    for (Operation* op : host0_ops) {
      ...
      if (auto recv_at_host =
        ...
        recv_at_host.setOperand(1, core_to_device_ordinal[core]);
                     ...
                     toolchain::dyn_cast<mlir::TF::_XlaSendFromHostV2Op>(clone)) {
...
LogicalResult MoveToHostSingleCluster(
    ArrayRef<Operation*> clustered_ops, ArrayRef<Value> external_operands,
    ArrayRef<Value> external_outputs,
    toolchain::SmallVector<IRMapping>& core_to_mapping,
    ArrayRef<Operation*> core_to_host_insertion_point,
    ArrayRef<Value> core_to_compilation_key,
...
LogicalResult MoveToHostMultiCluster(
    ...
    toolchain::SmallVector<Value, 4>* return_value_from_host = nullptr) {
  ...
  // non-static.
  toolchain::SmallSetVector<Operation*, 4> clustered_ops;
  auto device_type_attr = device_cluster->getAttrOfType<StringAttr>(
      mlir::TF::kCompileDeviceTypeAttr);

  for (Operation& op : toolchain::make_early_inc_range(*src)) {
    ...
    if (!HasDynamicExternalValues(&op) && !clustered_ops.empty()) {
      toolchain::SmallSetVector<Value, 4> external_operands =
...
void GetReturnValueFromDevice(
    mlir::tf_device::ClusterOp device_cluster,
    const toolchain::SmallVector<Value, 4>& return_value_from_host,
    toolchain::SmallVector<Value, 4>& return_value_from_device) {
  if (auto return_op = toolchain::dyn_cast_or_null<mlir::tf_device::ReturnOp>(
...
LogicalResult DecomposeControlFlow(mlir::tf_device::ClusterOp device_cluster,
  ...
  auto result = device_cluster.GetBody().walk([&](Operation* op) {
    if (auto if_op = toolchain::dyn_cast<mlir::TF::IfRegionOp>(op)) {
    ...
    if (auto while_op = toolchain::dyn_cast<mlir::TF::WhileRegionOp>(op)) {
      if (!HasOutsideCompilationNested(op)) return WalkResult::advance();
      ...
      const auto condition_send_recv_key =
          toolchain::formatv("while_condition_channel_{0}",
                        communication_key_index++)
              .str();
...
toolchain::SmallVector<Type, 4> GetParallelExecuteResultsTypes(
    ArrayRef<Value> return_value_from_host,
    ArrayRef<Value> return_value_from_device) {
  toolchain::SmallVector<Type, 4> parallel_execute_result_types;
...
toolchain::SmallVector<Value, 4> GetRemappedTpuClusterResults(
    mlir::tf_device::ClusterOp device_cluster,
    const toolchain::SmallVector<Value, 4>& return_value_from_host,
    const toolchain::SmallVector<Value, 4>& return_value_from_device) {
  toolchain::SmallVector<Value, 4> remapped_device_cluster_results;

  std::vector<int> order_from_new_output_to_previous_output;
  order_from_new_output_to_previous_output.reserve(
      return_value_from_host.size() + return_value_from_device.size());

  toolchain::SmallDenseMap<Value, std::deque<int>> return_operand_map;
  auto return_op = toolchain::dyn_cast<mlir::tf_device::ReturnOp>(
      device_cluster.GetBody().getTerminator());
...
void RemapDeviceClusterResultsWithParallelExecuteResults(
  ...
  for (auto result : toolchain::zip(remapped_device_cluster_results,
                               parallel_execute_op.getResults())) {
    Value device_cluster_result = std::get<0>(result);
    Value parallel_execute_result = std::get<1>(result);
    for (auto& use :
         toolchain::make_early_inc_range(device_cluster_result.getUses()))
...
toolchain::SmallVector<Value, 4> GetNewDeviceResults(
    ArrayRef<Value> return_value_from_device) {
  toolchain::SmallVector<Value, 4> device_results;
...
toolchain::SmallVector<Type, 4> GetNewDeviceTypes(
    ArrayRef<Value> return_value_from_device) {
  toolchain::SmallVector<Type, 4> device_result_types;
...
void MoveOldTpuClusterToNewTpuCluster(mlir::tf_device::ClusterOp device_cluster,
  ...
  for (Operation& op : toolchain::make_early_inc_range(device_cluster.GetBody())) {
...
void MoveTmpLaunchOpToNewLaunchOp(mlir::tf_device::LaunchOp tmp_host_launch_op,
  ...
  for (Operation& op :
       toolchain::make_early_inc_range(tmp_host_launch_op.GetBody())) {
...
mlir::tf_device::ParallelExecuteOp CreateFinalParallelExecuteOp(
  ...
  for (int core = 0; core < core_to_tmp_host_launch.size(); ++core) {
    ...
    toolchain::SmallVector<Value, 4> host_results;
  ...
  auto new_device_cluster = builder.create<mlir::tf_device::ClusterOp>(
      ...
      /*operands=*/toolchain::ArrayRef<Value>{}, device_cluster->getAttrs());
...
LogicalResult CreateParallelExecuteForOutsideCompilation(
    mlir::tf_device::ClusterOp device_cluster,
    toolchain::SmallVector<mlir::tf_device::ParallelExecuteOp, 4>& ops,
    std::optional<bool>& is_map_oc, ArrayRef<std::string> core_to_host,
    bool has_tpu_device) {
  OpBuilder builder(device_cluster);
  toolchain::SmallVector<Value, 4> returns_from_host;
  ...
  auto tmp_parallel_execute_op =
      builder.create<mlir::tf_device::ParallelExecuteOp>(
          device_cluster.getLoc(), num_regions, toolchain::ArrayRef<Type>{});
  ...
  for (int core = 0; core < num_host_regions; ++core) {
    ...
    toolchain::SmallVector<Value, 4> tmp_host_results;
    ...
    compilation_key =
        toolchain::dyn_cast<mlir::TF::_XlaCompileMlirPlaceholderProgramKeyOp>(
            compilation_key_op)
            .getProgram();
    if (has_tpu_device) {
      device_ordinal_op =
  ...
  toolchain::SmallVector<Value, 4> returns_from_device;
...
LogicalResult CheckClusterResults(mlir::tf_device::ClusterOp cluster) {
  for (OpResult result : cluster.getResults()) {
    if (!machina::TypeValidForXLA(result.getType())) {
...
void ExtractOutsideCompilation::runOnOperation() {
  ...
  if (failed(machina::GetDevicesFromOp(module, &devices)))
    return signalPassFailure();

  toolchain::SmallVector<mlir::tf_device::ParallelExecuteOp, 4>
