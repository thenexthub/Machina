...
using OpShardingVariant = std::variant<mlir::Operation*, toolchain::StringRef>;
using OpShardingVector = toolchain::SmallVector<OpShardingVariant, 8>;
using OptionalOpShardingVector =
    toolchain::SmallVector<std::optional<OpShardingVariant>, 8>;
...
std::optional<StringRef> GetXlaShardingFromOperator(mlir::Operation* op) {
  if (auto partitioned_output =
          toolchain::dyn_cast<mlir::TF::TPUPartitionedOutputV2Op>(op)) {
    ...
    return partitioned_output.get_XlaSharding();
                 ...
                 toolchain::dyn_cast<mlir::TF::TPUPartitionedInputV2Op>(op)) {
...
std::optional<xla::OpSharding> GetShardingFromVariant(
  ...
  if (machina::DecodeShardingAttribute(sharding_string->str(), sharding,
                                          false)
          .failed()) {
    return std::nullopt;
  }
...
mlir::ArrayAttr GetStrArrayAttr(Builder* builder,
  ...
  toolchain::SmallVector<mlir::Attribute, 8> strings;
...
LogicalResult VerifyShardings(mlir::func::FuncOp func,
  ...
  for (auto sharding_and_arg :
       toolchain::zip(sharding_for_args, function_block.getArguments())) {
  ...
  for (auto sharding_and_retval :
       toolchain::zip(sharding_for_rets, terminator->getOpOperands())) {
...
// logical device `n`.
std::optional<toolchain::StringRef> AssignLogicalDeviceFromTPUReplicatedCoreAttr(
    Operation* op, const toolchain::SmallVector<std::string>& logical_device_vec) {
  if (auto device = op->getAttrOfType<StringAttr>("device")) {
    if (!device.getValue().empty() && !device.getValue().str().empty()) {
      machina::DeviceNameUtils::ParsedName name;
      if (machina::DeviceNameUtils::ParseFullName(device.str(), &name)) {
        if (name.type == "TPU_REPLICATED_CORE") {
          // TODO(hanxiongwang): Add check for out of bound of name.id
          return toolchain::StringRef(logical_device_vec[name.id]);
        }
      }
    }
  }
...
absl::StatusOr<std::optional<toolchain::StringRef>> GetXlaShardingFromShardingOp(
    mlir::TF::XlaShardingOp sharding) {
  TF_ASSIGN_OR_RETURN(auto attr, GetXlaShardingAttrFromShardingOp(sharding));
  return attr ? ::std::optional<::toolchain::StringRef>(attr.getValue())
              : (::std::nullopt);
}
...
// inputs.
absl::StatusOr<std::optional<toolchain::StringRef>> GetXlaShardingFromArg(
    Value value, const toolchain::SmallVector<std::string>& logical_device_vec) {
  toolchain::SmallPtrSet<Value, 4> visited_values;
  toolchain::SmallVector<Value, 4> values_to_visit{value};
  while (!values_to_visit.empty()) {
    toolchain::SmallVector<Value, 4> next_values_to_visit;
    for (Value value_to_visit : values_to_visit) {
      ...
      for (auto& use : value_to_visit.getUses()) {
        ...
        if (auto sharding = toolchain::dyn_cast<mlir::TF::XlaShardingOp>(owner))
        ...
        if (auto while_op = toolchain::dyn_cast<mlir::TF::WhileRegionOp>(owner)) {
        ...
        if (toolchain::isa<mlir::TF::CastOp, mlir::TF::XlaAllReduceOp,
                      mlir::TF::ReadVariableOp>(owner)) {
          next_values_to_visit.push_back(use.getOwner()->getResult(0));
          continue;
        }

        if (auto call_op = toolchain::dyn_cast<mlir::CallOpInterface>(owner)) {
          mlir::func::FuncOp func =
              toolchain::dyn_cast<mlir::func::FuncOp>(call_op.resolveCallable());
          if (!func) continue;
          next_values_to_visit.push_back(
...
absl::Status IdentifyXlaShardingForComputationInputs(
    const toolchain::SmallVector<std::string>& logical_device_vec,
  ...
  for (auto operand_and_arg :
       toolchain::zip(cluster_func.getOperands(), function_block.getArguments())) {
...
mlir::Operation* GetXlaShardingFromResult(Value value) {
  ...
  if (auto partitioned_output =
          toolchain::dyn_cast<mlir::TF::TPUPartitionedOutputV2Op>(user))
    return NullUnlessSharded(partitioned_output);

  if (auto assign_var = toolchain::dyn_cast<mlir::TF::AssignVariableOp>(user))
    if (auto partitioned_input =
...
absl::StatusOr<std::optional<StringRef>> GetXlaShardingFromRetval(
    Value value, const toolchain::SmallVector<std::string>& logical_device_vec) {
  toolchain::SmallPtrSet<Value, 4> visited_values;
  toolchain::SmallVector<Value, 4> values_to_visit;
  ...
  while (!values_to_visit.empty()) {
    ...
    if (auto sharding = toolchain::dyn_cast_or_null<mlir::TF::XlaShardingOp>(def))
      return GetXlaShardingFromShardingOp(sharding);
    ...
    if (auto call_op = toolchain::dyn_cast_or_null<mlir::CallOpInterface>(def)) {
      mlir::func::FuncOp func =
          toolchain::dyn_cast<mlir::func::FuncOp>(call_op.resolveCallable());
      if (!func) continue;
    ...
    if (auto while_op = toolchain::dyn_cast<mlir::TF::WhileRegionOp>(def)) {
      if (auto op_result = mlir::cast<OpResult>(value_to_visit)) {
        int result_idx = op_result.getResultNumber();
        if (auto yield_op = toolchain::dyn_cast<mlir::TF::YieldOp>(
...
absl::Status IdentifyXlaShardingForComputationOutputs(
  ...
  for (auto result_and_retval :
       toolchain::zip(cluster_func.getResults(), terminator->getOpOperands())) {
...
void SetReplicatedOrMaximalShardingIfNoShardingFound(
    const toolchain::SmallVector<std::string>& logical_device_vec, bool use_spmd,
...
LogicalResult IdentifyXlaShardingForTPUComputation(
  ...
  toolchain::SmallVector<std::string> logical_device_vec(num_cores_per_replica);
  ...
  cluster_func->setAttr(machina::kInputShardingAttr,
                        GetStrArrayAttr(builder, sharding_for_args));
  cluster_func->setAttr(machina::kOutputShardingAttr,
