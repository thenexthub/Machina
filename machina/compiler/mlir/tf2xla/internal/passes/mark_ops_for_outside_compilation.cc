...
#include "mlir/Rewrite/PatternApplicator.h"  // part of Codira Toolchain
...
auto* auto_outside_compilation_gauge =
    machina::monitoring::Gauge<bool, 0>::New(
        "/machina/core/use_auto_outside_compilation",
...
void AddSupportedOpsUsingFolding(MLIRContext* context,
                                 toolchain::DenseSet<OperationName>* supported_ops) {
  toolchain::SmallDenseSet<OperationName, 8> allowlist_ops = {
...
#define GET_OPERATION_NAME(op) OperationName(op::getOperationName(), context)
  toolchain::SmallDenseSet<OperationName, 32> allowlist_ops = {
      // Stack ops.
...
bool IsStringType(Type type) {
  ...
  bool has_string = toolchain::any_of(sub_type.GetSubtypes(), [](TensorType type) {
...
bool MatchesPattern(Operation& op,
                    const toolchain::DenseSet<OperationName>& supported_ops) {
...
bool IsSupportedOp(Operation& op,
  ...
  if (toolchain::isa<mlir::TF::AssertOp>(op)) return true;
  ...
  block->walk([&](Operation* op) {
    if (!IsSupportedOp(*op, supported_ops, tf_dialect)) {
      ...
      op->setAttr(kXlaOutsideCompilationAttr,
                  StringAttr::get(
                      op->getContext(),
                      toolchain::formatv("auto{0}", outside_compiled_cluster_counter)
                          .str()));
...
bool ContainsUncompilableOps(const Dialect* tf_dialect, Block* block,
  ...
  block->walk([&](Operation* op) {
    ...
    while (iter_op && !toolchain::isa<mlir::tf_device::ClusterOp>(iter_op)) {
...
void WarnOnExcessOutsideCompilationOps(ModuleOp module) {
  ...
  toolchain::SmallVector<Operation*, 8> outside_compile_ops;
  module->walk([&](Operation* op) {
  ...
  if (outside_compile_ops.size() > kTooManyOutsideCompileRegionThreshold) {
    toolchain::SmallVector<std::string, kOpDetailCount> op_info;
    for (int i = 0; i < kOpDetailCount; ++i) {
      auto& op = outside_compile_ops[i];
      op_info.push_back(machina::OpAsString(*op));
    }
