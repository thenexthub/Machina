...
mlir::LogicalResult EncapsulateFirstXlaCompilablePartitionedCalls(
  ...
  toolchain::SmallVector<SymbolUserOpInterface> noinline_pcall_ops,
      outermost_pcall_ops;
  if (mlir::failed(
  ...
  for (auto &pcall_op : outermost_pcall_ops) {
    auto call = toolchain::cast<CallOpInterface>(pcall_op.getOperation());
  ...
  for (auto &pcall_op : noinline_pcall_ops) {
    auto call = toolchain::cast<CallOpInterface>(pcall_op.getOperation());
    auto callee = toolchain::cast<FuncOp>(
...
void XlaClusterFormationPass::runOnOperation() {
  ...
  toolchain::SmallVector<FuncOp> entry_funcs = GetEntryFunctions(module);
