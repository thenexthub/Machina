...
mlir::tf_device::LaunchOp CreateLaunchForBlock(OpBuilder* builder,
                                               ...
                                               Block* launch_block,
                                               toolchain::StringRef host_device) {
  // Find results and result types of ops in block that needs to returned.
  toolchain::SmallVector<Value, 4> launch_results;
  toolchain::SmallVector<Type, 4> launch_result_types;
...
bool IsEmbeddingOp(Operation* op) {
  return toolchain::isa<mlir::TF::EnqueueTPUEmbeddingRaggedTensorBatchOp,
...
toolchain::SmallVector<Operation*, 4> FindOutsideCompiledOpsAtHead(
  ...
  toolchain::SmallSetVector<Operation*, 4> head_outside_compiled_ops;
  ...
  for (Operation& cluster_op : cluster_ops) {
    ...
    if (!predecessors.empty() && !IsEmbeddingOp(&cluster_op)) {
      bool skip = false;
      for (Operation* predecessor : toolchain::reverse(predecessors)) {
...
void CreateHeadComputation(OpBuilder* builder,
                           mlir::tf_device::ClusterOp cluster,
                           toolchain::ArrayRef<Operation*> head_outside_compiled_ops,
                           toolchain::StringRef host_device) {
  ...
  for (auto result : toolchain::zip(launch.GetBody().getTerminator()->getOperands(),
                               launch.getResults()))
...
mlir::LogicalResult LiftHeadOutsideCompiledOps(
  ...
  toolchain::SmallVector<Operation*, 4> head_outside_compiled_ops =
      FindOutsideCompiledOpsAtHead(side_effect_analysis, cluster);
  if (head_outside_compiled_ops.empty()) return mlir::success();
  if (failed(machina::GetHostDeviceOutsideComputation(devices, cluster,
                                                         host_device)))
    return mlir::failure();
...
void FindOutsideCompiledOpsAtTailAndClusterResults(
    const mlir::TF::SideEffectAnalysis& side_effect_analysis,
    mlir::tf_device::ClusterOp cluster,
    toolchain::SmallVectorImpl<Operation*>* tail_outside_compiled_ops,
    toolchain::SmallVectorImpl<Value>* cluster_results) {
  const auto& analysis = side_effect_analysis.GetAnalysisForFunc(
      cluster->getParentOfType<FuncOp>());
  Region* cluster_region = &cluster.getBody();
  toolchain::SmallSetVector<Operation*, 4> tail_outside_compiled_ops_set;
  Operation* terminator = cluster.GetBody().getTerminator();
  toolchain::SmallSetVector<Value, 4> cluster_results_set;
  cluster_results_set.insert(terminator->getOperands().begin(),
                             terminator->getOperands().end());

  auto cluster_ops = toolchain::reverse(cluster.GetBody().without_terminator());
  for (Operation& cluster_op : cluster_ops) {
    if (!HasOutsideCompilationAttribute(&cluster_op)) continue;
    ...
    toolchain::SmallVector<int, 4> results_to_forward;
    bool can_be_extracted =
        toolchain::all_of(cluster_op.getUsers(), [&](Operation* op) {
...
void CreateTailComputation(OpBuilder* builder,
                           mlir::tf_device::ClusterOp cluster,
                           toolchain::ArrayRef<Operation*> tail_outside_compiled_ops,
                           toolchain::StringRef host_device) {
  Block* launch_block = new Block;
...
mlir::tf_device::ClusterOp UpdateClusterResults(
    ...
    toolchain::ArrayRef<Value> new_cluster_results) {
  ...
  toolchain::SmallVector<Type, 4> new_cluster_result_types;
  ...
  auto new_cluster = builder->create<mlir::tf_device::ClusterOp>(
      ...
      /*operands=*/toolchain::ArrayRef<Value>{}, cluster->getAttrs());
  ...
  for (auto result :
       toolchain::zip(new_cluster.GetBody().getTerminator()->getOperands(),
...
mlir::LogicalResult LiftTailOutsideCompiledOps(
  ...
  toolchain::SmallVector<Operation*, 4> tail_outside_compiled_ops;
  toolchain::SmallVector<Value, 4> cluster_results;
...
void RemoveClusterAliasedOutputs(OpBuilder* builder,
                                 mlir::tf_device::ClusterOp cluster) {
  toolchain::SmallVector<Value, 4> used_old_cluster_results;
  toolchain::SmallVector<Value, 4> new_cluster_results;
  toolchain::SmallVector<Type, 4> new_cluster_result_types;
  Operation* cluster_terminator = cluster.GetBody().getTerminator();
  for (auto result :
       toolchain::zip(cluster_terminator->getOperands(), cluster.getResults())) {
  ...
  for (auto result :
       toolchain::zip(used_old_cluster_results, new_cluster.getResults()))
...
void ExtractHeadTailOutsideCompilationPass::runOnOperation() {
  ...
  toolchain::SmallVector<mlir::tf_device::ClusterOp, 4> clusters;
  module.walk(
