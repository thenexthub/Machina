...
void SplitIntoIslandPerOpPass::runOnOperation() {
  ...
  if (toolchain::hasSingleElement(func.front().without_terminator())) {
  ...
  if (!(toolchain::hasSingleElement(graph_op.GetBody().without_terminator()) &&
        toolchain::isa<tf_executor::IslandOp>(graph_op.GetBody().front()))) {
    graph_op.emitError(
...
void PopulateEmptyIsland(tf_executor::IslandOp island) {
  ...
  if (yield.getNumOperands() == 0) {
    ...
    for (const auto& it : toolchain::enumerate(identity_n.getResults()))
...
void SplitIsland(mlir::tf_executor::IslandOp island_op,
  ...
  for (const auto& fetch : toolchain::enumerate(graph_op.GetFetch().getFetches())) {
