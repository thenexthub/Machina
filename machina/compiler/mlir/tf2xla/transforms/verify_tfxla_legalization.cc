...
auto* mlir_non_static_op_count = machina::monitoring::Counter<1>::New(
    "/machina/core/tf2xla/"
    "mlir_second_phase_non_static_op_count",
    "Counts which ops do not have static results", "op_name");

auto* mlir_non_static_op_skip_count = machina::monitoring::Counter<1>::New(
    "/machina/core/tf2xla/"
...
static void IncrementCounterFor(machina::monitoring::Counter<1>* counter,
                                Operation* op) {
...
bool IsMhloAndStatic(Operation* op) {
  if (!toolchain::isa<mlir::mhlo::MhloDialect>(op->getDialect())) {
