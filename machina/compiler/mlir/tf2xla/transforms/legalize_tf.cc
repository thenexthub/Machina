...
static DenseI64ArrayAttr ToDenseI64ArrayAttr(DenseIntElementsAttr attr,
  ...
  if (attr.getElementType().isInteger(64)) {
    return GetI64ArrayAttr(toolchain::to_vector(attr.getValues<int64_t>()), builder);
  }
...
static DenseI64ArrayAttr GetI64ArrayAttrForValue(int size, int64_t val,
                                                 Builder *builder) {
  toolchain::SmallVector<int64_t, 4> vals(size, val);
  return builder->getDenseI64ArrayAttr(vals);
}
 ...
 public:
  ...
  LogicalResult matchAndRewriteDynamicConv(OpT op,
    ...
    rewriter.replaceOpWithNewOp<stablehlo::DynamicConvOp>(
        op, op.getType(), operands, toolchain::ArrayRef(attrs));
    return success();
  }
...
/// Converts the RangeOp machina op to a stablehlo.iota op with a scaling and
...
/// Converts the LinSpace machina op to a stablehlo.iota op with a scaling
...
/// Converts a generic OpTy machina op to a stablehlo.reduce op over
/// ReductionOp.
...
class ConvertArgMinMaxOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto reduction = rewriter.create<stablehlo::ReduceOp>(
        ...
        toolchain::ArrayRef<Value>(init_values), GetI64ArrayAttr({axis}, &rewriter),
...
class ConvertXlaShardingOp : public OpRewritePattern<TF::XlaShardingOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::XlaShardingOp op,
    ...
    auto sharding = machina::GetXlaShardingAttrFromShardingOp(op);
    if (!sharding.ok()) {
      return failure();
    }
...
#include "machina/compiler/mlir/tf2xla/transforms/generated_legalize_tf.inc"
