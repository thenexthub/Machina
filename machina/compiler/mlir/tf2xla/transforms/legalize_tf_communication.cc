...
#include "machina/xla/side_effect_util.h"
...
#include "machina/compiler/mlir/tf2xla/transforms/tf_xla_passes.h.inc"
...
LogicalResult GetControlFlowAncestors(
    Operation* op, toolchain::SmallPtrSetImpl<Operation*>& control_flow_ops,
    toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks) {
  ...
  while (block && parent && !isa<func::FuncOp>(parent)) {
    ...
    if (!toolchain::hasSingleElement(block->getParent()->getBlocks()))
...
LogicalResult FindCommunicationOps(
    func::FuncOp func, toolchain::SmallPtrSetImpl<Operation*>& control_flow_ops,
    toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks,
...
struct FuncToRewrite {
  ...
  toolchain::SmallPtrSet<Operation*, 4> control_flow_ops;
  toolchain::SmallPtrSet<Block*, 4> control_flow_blocks;
  func::FuncOp clone;
};
...
LogicalResult GetFunctionsToRewrite(
    ModuleOp module,
    toolchain::SmallDenseMap<StringRef, FuncToRewrite>& funcs_to_rewrite) {
...
void SetFrontendAttributes(Operation* op, int32_t index, StringRef key,
  ...
  std::string formatted_key =
      device_to_host ? toolchain::formatv("{0}_dtoh_{1}", key, index).str()
                     : toolchain::formatv("{0}_htod_{1}", key, index).str();
...
Value CreateSinkToken(OpBuilder& builder, Location loc, ArrayRef<Value> tokens,
  ...
  } else if (toolchain::hasSingleElement(tokens)) {
    return tokens[0];
  } else {
...
Value RewriteHostComputeOp(OpBuilder& builder,
  ...
  for (auto operand : toolchain::enumerate(host_compute.getInputs())) {
  ...
  for (auto result : toolchain::enumerate(host_compute.getOutputs())) {
...
Value RewriteCallOp(OpBuilder& builder, func::CallOp call,
  ...
  auto new_operands = toolchain::to_vector(call.getArgOperands());
  new_operands.push_back(token);
  auto new_result_types = toolchain::to_vector(call.getResultTypes());
  ...
  for (auto results : toolchain::zip(call.getResults(), new_call.getResults()))
...
SmallVector<Value> GetValueWithToken(
    ...
    toolchain::SmallDenseMap<Value, Value>& rewritten_values, bool flatten_tuple) {
  if (flatten_tuple) {
    auto operands = toolchain::to_vector(values);
  ...
  if (auto tuple_op = value.getDefiningOp<TupleOp>()) {
    auto tuple_operands = toolchain::to_vector(tuple_op.getOperands());
  ...
  for (auto idx : toolchain::seq<int32_t>(0, tuple_type.getTypes().size()))
...
SmallVector<Type> GetTypeWithToken(OpBuilder& builder, ArrayRef<Type> types,
  ...
  if (flatten_tuple) {
    auto result_types = toolchain::to_vector(types);
  ...
  if (auto tuple_type = mlir::dyn_cast<TupleType>(type)) {
    auto result_types = toolchain::to_vector(tuple_type.getTypes());
    result_types.push_back(token_type);
...
Value CreateSubTuple(OpBuilder& builder, Value value, size_t end) {
  SmallVector<Value, 4> tuple_operands;
  for (auto idx : toolchain::seq<int32_t>(0, end))
    tuple_operands.push_back(
        builder.create<GetTupleElementOp>(value.getLoc(), value, idx)
            .getResult());
...
void ReplaceWithTupleResult(OpBuilder& builder, ValueRange values,
  ...
  Value sub_tuple;
  for (auto& use : toolchain::make_early_inc_range(value.getUses())) {
...
void RewriteControlFlowTerminator(OpBuilder& builder, Operation* terminator,
  ...
  toolchain::SmallDenseMap<Value, Value> rewritten_operands;
  auto new_results =
      GetValueWithToken(builder, toolchain::to_vector(terminator->getOperands()),
...
void RewriteRegionIfOp(OpBuilder& builder, IfOp region_if,
  ...
  auto new_result_types =
      GetTypeWithToken(builder, toolchain::to_vector(region_if.getResultTypes()),
                       /*flatten_tuple=*/true);
...
void RewriteControlFlowOpRegion(
    ...
    SmallVectorImpl<OpVisitorState>& ops_to_visit,
    const toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks, Value token) {
  ops_to_visit.push_back({region_idx + 1, token, region_op});

  Region& region = region_op->getRegion(region_idx);
  assert(toolchain::hasSingleElement(region));
...
bool ProcessRegionIfOp(OpBuilder& builder, IfOp region_if,
                       std::optional<unsigned> region_idx,
                       SmallVectorImpl<OpVisitorState>& ops_to_visit,
                       const toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks,
                       Value token) {
...
void RewriteRegionWhileOp(OpBuilder& builder, WhileOp region_while,
  ...
  auto new_val_operands =
      GetValueWithToken(builder, toolchain::to_vector(region_while.getOperands()),
                        token, rewritten_operands,
                        /*flatten_tuple=*/true);

  auto new_result_types =
      GetTypeWithToken(builder, toolchain::to_vector(region_while.getResultTypes()),
                       /*flatten_tuple*/ true);
...
void UpdateFunctionType(OpBuilder& builder, func::FuncOp func,
                        Block& func_body) {
  auto new_argument_types = toolchain::to_vector(func_body.getArgumentTypes());
  auto new_result_types =
      toolchain::to_vector(func_body.getTerminator()->getOperandTypes());
...
void RewriteFunctionTerminator(OpBuilder& builder,
  ...
  auto new_results = toolchain::to_vector(terminator.getOperands());
...
LogicalResult RewriteFunction(
    ...
    const toolchain::SmallDenseMap<StringRef, FuncToRewrite>& funcs,
    const toolchain::SmallPtrSetImpl<Operation*>& control_flow_ops,
    const toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks, bool is_clone) {
  MLIRContext* context = module.getContext();
  if (!toolchain::hasSingleElement(func.getBody()))
...
bool IsFunctionCallWithCommunication(
    Operation* op,
    const toolchain::SmallDenseMap<StringRef, FuncToRewrite>& funcs_to_rewrite) {
...
void GetCommunicationControlFlowOps(
    func::FuncOp func,
    const toolchain::SmallDenseMap<StringRef, FuncToRewrite>& funcs_to_rewrite,
    toolchain::SmallPtrSetImpl<Operation*>& control_flow_ops,
    toolchain::SmallPtrSetImpl<Block*>& control_flow_blocks) {
...
void LegalizeTFCommunication::runOnOperation() {
  ...
  for (const auto& func_and_name : funcs_to_rewrite) {
    ...
    if (!clone) continue;
    toolchain::SmallPtrSet<Operation*, 4> clone_control_flow_ops;
    toolchain::SmallPtrSet<Block*, 4> clone_control_flow_blocks;
