...
class OpOrArgLocNameMapperWithoutInvalidCharacters
    : public machina::OpOrArgLocNameMapper {
 public:
  OpOrArgLocNameMapperWithoutInvalidCharacters() = default;
  ~OpOrArgLocNameMapperWithoutInvalidCharacters() override = default;

 protected:
  std::string GetName(machina::OpOrVal op_or_val) override {
    std::string name = OpOrArgLocNameMapper::GetName(op_or_val);
    return absl::StrReplaceAll(name, {{";", "."}});
  }
};

static std::unique_ptr<machina::StaticDeviceMgr> CreateDeviceMgr(
    const std::string& device_type) {
  // Register compilation kernels for all registered XLA backends.
  machina::XlaOpRegistry::RegisterCompilationKernels();

  auto device = std::make_unique<machina::XlaCompilationDevice>(
      machina::SessionOptions(), machina::DeviceType(device_type));
  return std::make_unique<machina::StaticDeviceMgr>(std::move(device));
}
...
absl::StatusOr<stablehlo::TupleOp> Tf2XlaRewriter::ImportXlaComputation(
  ...
  toolchain::SmallVector<mlir::Value> arguments;
...
LogicalResult Tf2XlaRewriter::PrepareParams() {
  ...
  context_ = new machina::XlaContext(/*compiler=*/nullptr, &xla_builder_,
                                        /*graph=*/nullptr);
  context_->Ref();
  ...
  step_container_ = std::make_unique<machina::ScopedStepContainer>(
      /*step_id=*/0, cleanup);
  absl::Status status = step_container_->Create(
      device_->resource_manager(),
      machina::XlaContext::kXlaContextResourceName, context_);
  if (!status.ok()) {
  ...
  absl::StatusOr<int64_t> version_or = machina::GetTfGraphProducerVersion(
      op_->getParentOfType<mlir::ModuleOp>());
  if (!version_or.ok()) {
    return emitError(op_->getLoc()) << version_or.status().ToString();
  }

  flib_def_ = std::make_unique<machina::FunctionLibraryDefinition>(
      machina::OpRegistry::Global(), machina::FunctionDefLibrary());
  pflr_ = std::make_unique<machina::ProcessFunctionLibraryRuntime>(
      device_mgr_.get(), machina::Env::Default(), /*config=*/nullptr,
      version_or.value(), flib_def_.get(), machina::OptimizerOptions());
...
LogicalResult Tf2XlaRewriter::PrepareKernelInputs(
  ...
  for (auto it : toolchain::enumerate(op_->getOperands())) {
    Value operand = it.value();
    size_t idx = it.index();

    machina::XlaExpression expr = GetExprForOperand(operand, op_, idx);
    machina::XlaExpression::Kind kind = expr.kind();
    if (kind == machina::XlaExpression::Kind::kInvalid) return failure();
    expressions.push_back(expr);

    if (!machina::DataTypeCanUseMemcpy(expr.dtype())) {
    ...
    tensors.emplace_back(
        device_->GetAllocator(machina::AllocatorAttributes()), expr.dtype(),
        shape_or.value());

    machina::Tensor& tensor = tensors.back();
    machina::XlaExpression::AssignExpressionToTensor(expr, &tensor);
...
LogicalResult Tf2XlaRewriter::LegalizeOp() {
  ...
  auto nodedef_or = machina::ConvertTFDialectOpToNodeDef(
  ...
  std::shared_ptr<const machina::NodeProperties> props;
  absl::Status status = machina::NodeProperties::CreateFromNodeDef(
      *nodedef_or.value(),
  ...
  machina::OpKernel* op_kernel_raw;
  ...
  status = machina::XlaOpRegistry::CompileTimeConstantInputs(
  ...
  toolchain::SmallDenseSet<int> required_consts;
  ...
  toolchain::SmallVector<machina::AllocatorAttributes, 4> output_attr(
      op_->getNumResults());
  params_.output_attr_array = output_attr.data();

  machina::OpKernelContext op_context(&params_, op_->getNumResults());
  ...
  toolchain::SmallVector<Value> output_values;
...
absl::StatusOr<stablehlo::TupleOp> Tf2XlaRewriter::CompileWithHloImporter(
  ...
  for (int i = 0, e = op_->getNumResults(); i < e; i++) {
    machina::Tensor* output = op_context.mutable_output(i);
    const machina::XlaExpression* expr =
        machina::XlaExpression::CastExpressionFromTensor(*output);
...
mlir::LogicalResult Tf2XlaRewriter::VerifyOpResults(
  ...
  for (int i = 0, e = op_->getNumResults(); i < e; i++) {
    ...
    if (expr->kind() != machina::XlaExpression::Kind::kXlaOp &&
        expr->kind() != machina::XlaExpression::Kind::kConstant) {
...
mlir::LogicalResult Tf2XlaRewriter::UnpackTupleResults(
    stablehlo::TupleOp tuple_result, toolchain::SmallVector<Value>& outputs) {
...
mlir::LogicalResult Tf2XlaRewriter::GetKernelOutputs(
    machina::OpKernelContext& op_context, stablehlo::TupleOp tuple_results,
...
machina::XlaExpression Tf2XlaRewriter::GetExprForOperand(
  ...
  if (defining_op && matchPattern(defining_op, m_Constant(&const_attr))) {
    ...
    auto status = machina::ConvertToTensor(const_attr, &tensor);
    if (!status.ok()) {
      op->emitRemark() << "skipping legalization due to failed const conversion"
                       << status.ToString();
      return machina::XlaExpression::Invalid();
    }

    return machina::XlaExpression::Constant(tensor);
  }

  machina::DataType dtype;
  auto status = machina::ConvertToDataType(operand.getType(), &dtype);
  if (!status.ok()) {
    op->emitRemark() << "skipping legalization due to " << status.ToString();
    return machina::XlaExpression::Invalid();
  }
  return machina::XlaExpression::XlaOp(xla_op, dtype);
}
