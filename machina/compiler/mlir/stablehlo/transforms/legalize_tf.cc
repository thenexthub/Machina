...
#include "machina/xla/hlo/builder/lib/conv_grad_size_util.h"
#include "machina/xla/hlo/builder/padding.h"
#include "machina/xla/hlo/builder/sharding_builder.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/attribute_importer.h"
...
#include "machina/core/framework/kernel_shape_util.h"
#include "machina/core/framework/rng_alg.h"
#include "machina/core/kernels/conv_grad_shape_utils.h"
...
static size_t GetFeatureDimension(machina::TensorFormat format,
...
static DenseIntElementsAttr GetI32ElementsAttr(ArrayRef<int32_t> values,
                                               Builder *builder) {
  RankedTensorType ty = machina::GetTypeFromTFTensorShape(
...
static DenseIntElementsAttr GetI64ElementsAttrForSeq(int start, int end,
  ...
  TensorType ty =
      machina::GetTypeFromTFTensorShape({size}, builder->getIntegerType(64));
...
static ArrayAttr GetPrecisionConfig(Builder *builder) {
  ...
  toolchain::SmallVector<mlir::Attribute, 2> attr_vec;
...
// corresponding to the machina axis. In particular, the machina axis can
...
static TF::UnpackOp UnpackTensorAlongZeroDim(Location loc, Value value,
  ...
  SmallVector<Type, 2> unpacked_indices_type(
      num_outputs,
      machina::GetTypeFromTFTensorShape({}, indices_type.getElementType()));
...
machina::TensorShape ToTensorShape(toolchain::ArrayRef<T> sizes) {
  return machina::TensorShape(
      toolchain::SmallVector<int64_t, num_dims>(sizes.begin(), sizes.end()));
}

template <typename T, int num_dims>
machina::TensorShape ToTensorShape(
    toolchain::iterator_range<DenseElementsAttr::ElementIterator<T>> sizes) {
  return machina::TensorShape(
      toolchain::SmallVector<int64_t, num_dims>(sizes.begin(), sizes.end()));
}
...
static RankedTensorType GetStaticBroadcastType(
  ...
  toolchain::SmallVector<int64_t, 4> out_shape(shape_large.begin(),
                                          shape_large.end());

  // Update according to the broadcast dimensions.
  for (const auto &index_pair : toolchain::enumerate(broadcast_dimensions)) {
...
static RankedTensorType GetExtentsTensorTypeFor(TensorType value_type) {
  ...
  return machina::GetTypeFromTFTensorShape({dim}, b.getIndexType());
}
...
using WhileBodyFnType = toolchain::function_ref<void(
...
static DenseIntElementsAttr SliceDenseIntElementsAttrColumn2D(
  ...
  toolchain::SmallVector<int64_t, 4> values;
  values.reserve(shaped_type.getNumElements() / shape[1]);

  for (const auto &it : toolchain::enumerate(int_attr.getValues<APInt>())) {
  ...
  return DenseIntElementsAttr::get(
      machina::GetTypeFromTFTensorShape({shape[0]}, element_type), values);
}
...
static DenseIntElementsAttr GetInteriorPadding(ElementsAttr tf_padding) {
  ...
  return DenseIntElementsAttr::get<int64_t>(
      machina::GetTypeFromTFTensorShape({length}, element_type), 0);
}
...
static Type ChangeTensorElementType(Builder *b, Type tensor_type,
  ...
  if (ranked_type) {
    return machina::GetTypeFromTFTensorShape(ranked_type.getShape(),
                                                element_type);
  }
...
static DenseElementsAttr GetEpsilonValue(Type ty) {
  ...
  auto scalar_ty = machina::GetTypeFromTFTensorShape({}, element_ty);
  if (element_ty.isF16()) {
...
static void BuildArgMinMaxReductionBody(Type input_element_type,
  ...
  Type input_type =
      machina::GetTypeFromTFTensorShape(/*shape=*/{}, input_element_type);
  Type index_type =
      machina::GetTypeFromTFTensorShape(/*shape=*/{}, index_element_type);
  Block *block = builder->createBlock(body);
...
static bool ArgTypesMatchCallee(mlir::Operation *op, OperandRange args,
  ...
  for (auto arg_in : toolchain::zip(args, function_ty.getInputs())) {
...
class ConvertConvDynamic : public OpRewritePattern<OpT> {
 public:
  ...
  bool GetPaddingValues(OpT &op, PatternRewriter &rewriter, Value input_size,
                        ...
                        int64_t stride, machina::Padding padding_type,
  ...
  LogicalResult matchAndRewriteDynamicConv(OpT op,
    ...
    if (padding == machina::Padding::EXPLICIT) {
    ...
    for (auto i : toolchain::seq<int>(0, num_spatial_dims)) {
    ...
    Value paddings_op = rewriter.create<tensor::FromElementsOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape(2 * num_spatial_dims,
                                             rewriter.getI32Type()),
        paddings);
    ...
    if (depthwise_conv) {
      ...
      toolchain::SmallVector<int64_t, num_dims> new_shape(
      ...
      operands[1] = rewriter.create<mhlo::ReshapeOp>(
          op.getLoc(),
          machina::GetTypeFromTFTensorShape(new_shape,
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    for (auto i : toolchain::seq<int>(0, num_spatial_dims)) {
      ...
      if (padding == machina::Padding::EXPLICIT) {
        ...
        absl::Status status = machina::GetWindowedOutputSizeVerbose(
    ...
    RankedTensorType paddings_ty = machina::GetTypeFromTFTensorShape(
...
class ConvertPadOpDynamic : public OpRewritePattern<TF::PadV2Op> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::PadV2Op op,
    ...
    toolchain::SmallVector<int64_t, 2> transposed_shape = {2, input_rank};
    ...
    Value reshaped_paddings = rewriter.create<mhlo::ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({input_rank * 2},
                                             paddings_elem_ty),
        transposed_paddings);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::BroadcastToOp op,
    ...
    if (input_type.getRank() > 0) {
      ...
      // broadcasting.
      broadcast_dimensions = toolchain::to_vector<4>(
          toolchain::seq<int64_t>(rank_diff, ranked_output_type.getRank()));
    }
...
class ConvertDiagPartOp : public OpRewritePattern<TF::DiagPartOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::DiagPartOp op,
    ...
    int64_t new_size = 1;
    toolchain::SmallVector<int64_t, 4> new_dims;
    ...
    Value reshaped_input = rewriter.create<mhlo::ReshapeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape({new_size, new_size},
                                             input_type.getElementType()),
        op.getInput());
    auto iota_type = machina::GetTypeFromTFTensorShape(
    ...
    rewriter.replaceOpWithNewOp<ReshapeOp>(
        op,
        machina::GetTypeFromTFTensorShape(new_dims,
                                             input_type.getElementType()),
        reduce.getResult(0));
    return success();
  }
...
class ConvertMatrixDiagPartV3Op
  ...
  using Shape = toolchain::SmallVector<int64_t, 4>;
  ...
  BroadcastOp BroadcastConstant(Location loc, Shape shape, int32_t constant,
    ...
    return rewriter.create<BroadcastOp>(
        loc,
        machina::GetTypeFromTFTensorShape(shape,
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::MatrixDiagPartV3Op op,
    ...
    RankedTensorType iota_type = machina::GetTypeFromTFTensorShape(
    ...
    Value in_bounds = rewriter.create<ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape(Shape({num_diags, max_diag_len}),
    ...
    Value start_indices = rewriter.create<ConcatenateOp>(
        loc,
        machina::GetTypeFromTFTensorShape(concat_shape,
    ...
    auto dims_attr = GatherDimensionNumbersAttr::get(
        rewriter.getContext(),
        /*offsetDims=*/toolchain::to_vector<4>(toolchain::seq<int64_t>(0, num_dims - 2)),
    ...
    Value b_in_bounds = rewriter.create<BroadcastOp>(
        loc,
        machina::GetTypeFromTFTensorShape(output_shape,
                                             rewriter.getIntegerType(1)),
...
class ConvertEinsumOp : public OpRewritePattern<TF::EinsumOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::EinsumOp op,
    ...
    toolchain::SmallVector<Value, 2> inputs;
...
class ConvertFFTOp : public OpRewritePattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto expected_shape = toolchain::to_vector<4>(input_shape.drop_back());
    ...
    if (input_shape.back() > expected_dim) {
      reshaped = rewriter.create<SliceOp>(
          op.getLoc(),
          machina::GetTypeFromTFTensorShape(expected_shape,
 ...
 public:
  ...
  LogicalResult matchAndRewrite(FusedBatchNormGradOpT op,
    ...
    if (op.getIsTraining()) {  // training
      ...
      Type feature_type = machina::GetTypeFromTFTensorShape(
      ...
      RankedTensorType scalar_float =
          machina::GetTypeFromTFTensorShape({}, kernel_type);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(FusedBatchNormOpT op,
    ...
    if (op.getIsTraining()) {
      ...
      Type mean_var_type = machina::GetTypeFromTFTensorShape(
      ...
      if (std::is_same<FusedBatchNormOpT, TF::FusedBatchNormV2Op>::value) {
        ...
        int num_elements = reserve_space_3_type.hasStaticShape()
                               ...
                               : 0;
        auto const_attr_type = machina::GetTypeFromTFTensorShape(
...
static PaddingArray GetReduceWindowPaddingAsArray(
    toolchain::ArrayRef<int64_t> input_dims, ArrayAttr window_dims,
    ArrayAttr window_strides, StringRef padding, Builder *builder) {
  if (padding == "VALID") {
    return PaddingArray(num_dims, std::make_pair(0, 0));
  }
  assert(padding == "SAME");
  toolchain::SmallVector<int64_t, num_dims> input_shape, window_shape, strides;
...
static DenseIntElementsAttr GetReduceWindowPaddingAsAttr(
  ...
  toolchain::SmallVector<int64_t, num_dims * 2> flatten_paddings(rank * 2);
  for (int i = 0; i < rank; i++) {
    flatten_paddings[2 * i] = paddings[i].first;
    flatten_paddings[2 * i + 1] = paddings[i].second;
  }
  return DenseIntElementsAttr::get(machina::GetTypeFromTFTensorShape(
...
Operation *AvgPoolDivideByCount(
  ...
  RankedTensorType orig_input_type =
      machina::GetTypeFromTFTensorShape(input_shape, element_type);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    if (auto ranked_type = mlir::dyn_cast<RankedTensorType>(op.getType()))
      result_type = machina::GetTypeFromTFTensorShape(ranked_type.getShape(),
                                                         sum_element_type);
    else
    ...
    SmallVector<int64_t, num_dims> input_shape(
        toolchain::to_vector<num_dims>(input_type.getShape()));
...
// is a special case see `machina/core/kernels/conv_grad_ops.h`.
// `machina/compiler/mlir/xla/tests/legalize-tf.mlir` has more
 ...
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    DimVector out_grad_shape(
        toolchain::to_vector<num_dims>(out_grad_type.getShape()));
    ...
    for (int i = 0; i < num_spatial_dims; ++i) {
      int dim = machina::GetTensorSpatialDimIndex(num_dims, data_format, i);
    ...
    Value reduce_window_input = rewriter.create<PadOp>(
        loc, machina::GetTypeFromTFTensorShape(out_grad_shape, element_type),
    ...
    auto reduce_window_op = rewriter.create<ReduceWindowOp>(
        loc,
        machina::GetTypeFromTFTensorShape(orig_input_shape,
                                             sum_element_type),
  ...
  LogicalResult matchAndRewrite(TF::SelectOp op,
    ...
    if (needs_broadcast) {
      ...
      cond = b.create<mhlo::DynamicBroadcastInDimOp>(
          machina::GetTypeFromTFTensorShape(result_type.getShape(),
                                               b.getI1Type()),
          cond, result_extents,
...
class ConvertBatchMatMulV2Op : public OpRewritePattern<TF::BatchMatMulV2Op> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::BatchMatMulV2Op op,
    ...
    auto batch_dimensions = toolchain::to_vector<4>(toolchain::seq<int64_t>(0, rank - 2));
    auto lhs_contracting_dimensions = toolchain::to_vector<4>(
        toolchain::ArrayRef({op.getAdjX() ? rank - 2 : rank - 1}));
    auto rhs_contracting_dimensions = toolchain::to_vector<4>(
        toolchain::ArrayRef({op.getAdjY() ? rank - 1 : rank - 2}));
...
class ConvertSplitOp : public OpRewritePattern<TF::SplitOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::SplitOp op,
    ...
    auto slice_shape = toolchain::to_vector<4>(input_type.getShape());
    slice_shape[dim_index] = slice_size;
    Type slice_type = machina::GetTypeFromTFTensorShape(
    ...
    auto end_indices = toolchain::to_vector<4>(input_type.getShape());
...
class ConvertStridedSliceOp : public OpRewritePattern<TF::StridedSliceOp> {
 public:
  ...
  LogicalResult rewriteWithUnknownBegin(TF::StridedSliceOp op,
    ...
    // Scalar tensor type.
    TensorType type =
        machina::GetTypeFromTFTensorShape(/*shape=*/{}, begin_element_ty);
    Location loc = op.getLoc();
    ...
    auto sliced_type = machina::GetTypeFromTFTensorShape(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::StridedSliceGradOp op,
    ...
    grad = rewriter.create<mhlo::ReshapeOp>(
        op.getLoc(), machina::GetTypeFromTFTensorShape(shape, element_type),
        grad);
...
/// Converts the RangeOp machina op to a mhlo.iota op with a scaling and
  ...
  LogicalResult matchAndRewrite(TF::RangeOp op,
    ...
    auto compute_element_type =
        mlir::isa<FloatType>(getElementTypeOrSelf(start.getType()))
            ...
            : rewriter.getF64Type();
    auto compute_type = machina::GetTypeFromTFTensorShape(
    ...
    auto steps = rewriter.create<mhlo::ConvertOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape({}, rewriter.getI64Type()), ceil);
    auto reshape = rewriter.create<mhlo::ReshapeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape({1}, rewriter.getI64Type()),
        steps);

    // Using the resulting length compute the correct range value:
    //
    // %range = %start + %delta * iota(%size)
    auto out_scalar_type = machina::GetTypeFromTFTensorShape(
...
/// Converts the LinSpace machina op to a mhlo.iota op with a scaling
...
/// Converts a generic OpTy machina op to a mhlo.reduce op over
/// ReductionOp.
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    if (std::is_same<OpTy, TF::MeanOp>::value) {
      ...
      Value divisor_tensor = rewriter.create<tensor::FromElementsOp>(
          loc, machina::GetTypeFromTFTensorShape({}, rewriter.getI64Type()),
          divisor_casted);
      Value divisor = rewriter.create<ConvertOp>(
          loc, machina::GetTypeFromTFTensorShape({}, reduce_element_type),
          divisor_tensor);
    ...
    if (op.getKeepDims()) {
      for (const auto &dim_is_reduced :
           toolchain::enumerate(reduced_dimensions_bitmap)) {
...
// Converts machina ArgMin or ArgMax op to mhlo operations that perform
...
class ConvertArgMinMaxOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    RankedTensorType index_type = machina::GetTypeFromTFTensorShape(
    ...
    auto reduction = rewriter.create<ReduceOp>(
        loc, toolchain::ArrayRef<Value>(operands),
        toolchain::ArrayRef<Value>(init_values), reduction_dimensions,
...
// Converts machina ArgMax op to mhlo operations. The actual
...
// Converts machina ArgMin op to mhlo operations. The actual
// implementation is in class ConvertArgMinMaxOp:
//
//   %init_index = arith.constant dense<...> : tensor<T>
//   %init = arith.constant dense<...> : tensor<T>
 ...
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    if (updates_ty.getRank() == 0 &&
      ...
      toolchain::SmallVector<int64_t> expected_update_shape;
      ...
      auto const_type = machina::GetTypeFromTFTensorShape(
      ...
      auto broadcast_to_type = machina::GetTypeFromTFTensorShape(
          toolchain::ArrayRef<int64_t>(expected_update_shape),
    ...
    auto dims_attr = ScatterDimensionNumbersAttr::get(
        rewriter.getContext(),
        toolchain::to_vector<4>(
            toolchain::seq<int64_t>(updates_rank - window_dims, updates_rank)),
        toolchain::to_vector<4>(toolchain::seq<int64_t>(0, num_index_dims)),
        /*inputBatchingDims=*/{},
...
class ConvertTensorScatterUpdateOp
 ...
 public:
  ...
  static void BuildScatterBody(Type element_type, Region *region, Location loc,
    ...
    Type type =
        machina::GetTypeFromTFTensorShape(/*shape=*/{}, element_type);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::TileOp op,
    ...
    for (auto multiple_and_input :
         toolchain::zip(multiples.getValues<APInt>(), input_shape)) {
    ...
    Type broadcasted_type =
        machina::GetTypeFromTFTensorShape(broadcasted_shape, element_type);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::TileOp op,
    ...
    RankedTensorType broadcast_type =
        machina::GetTypeFromTFTensorShape(broadcast_shape, element_type);
    ...
    Value shape = rewriter.create<tensor::FromElementsOp>(
        loc, machina::GetTypeFromTFTensorShape({input_rank}, index_ty),
        shape_values);
...
class ConvertConvBackpropInputOp : public OpRewritePattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    std::vector<machina::int32> strides{
    ...
    for (int i : toolchain::seq<int>(0, num_spatial_dims)) {
    ...
    const int feature_dim =
        machina::GetTensorFeatureDimIndex(num_dims, data_format);
    ...
    if (feature_group_count != 1) {
      ...
      auto new_shape = toolchain::to_vector<6>(filter_shape);
      ...
      auto ty =
          machina::GetTypeFromTFTensorShape(new_shape, filter_element_ty);
      filter = rewriter.create<ReshapeOp>(op.getLoc(), ty, filter);

      // 2. Transpose to [H, W, ..., G, filter_in_depth, out_depth / G].
      toolchain::SmallVector<int64_t, 6> perm(num_dims + 1);
      ...
      ty = machina::GetTypeFromTFTensorShape(new_shape, filter_element_ty);
...
class ConvertConvBackpropFilterOp : public OpRewritePattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    machina::ConvBackpropDimensions dims;
    if (!machina::ConvBackpropComputeDimensionsV2(
    ...
    for (int i : toolchain::seq<int>(0, num_spatial_dims)) {
      const int64_t dim =
          machina::GetTensorSpatialDimIndex(num_dims, data_format, i);
      ...
      const int64_t pad_before = padding == machina::Padding::EXPLICIT
                                     ? explicit_paddings[2 * dim]
                                 : padding == machina::Padding::SAME
    ...
    const int batch_dim =
        machina::GetTensorBatchDimIndex(num_dims, data_format);
...
class ConvertOneHotOp : public OpRewritePattern<TF::OneHotOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::OneHotOp op,
    ...
    toolchain::SmallVector<int64_t, 4> broadcast_dims(indices_shape.size());
    std::iota(broadcast_dims.begin(), broadcast_dims.begin() + axis, 0);
    std::iota(broadcast_dims.begin() + axis, broadcast_dims.end(), axis + 1);

    toolchain::SmallVector<int64_t, 4> output_dims =
        toolchain::to_vector<4>(indices_shape);
    ...
    auto index_type =
        machina::GetTypeFromTFTensorShape(output_dims, element_type);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::InfeedDequeueTupleOp op,
    ...
    if (op.get_XlaSharding().has_value()) {
      ...
      if (machina::DecodeShardingAttribute(
    ...
    for (const auto &idx_and_type : toolchain::enumerate(result_types)) {
...
class ConvertUnpackOp : public OpRewritePattern<TF::UnpackOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::UnpackOp op,
    ...
    auto end_indices = toolchain::to_vector<4>(value_type.getShape());
  ...
  LogicalResult matchAndRewrite(TF::UnpackOp op,
    ...
    for (int64_t i = 0; i < op.getNumResults(); ++i) {
      ...
      Value slice_op = rewriter.create<RealDynamicSliceOp>(
          loc,
          machina::GetTypeFromTFTensorShape(slice_shape,
  ...
  LogicalResult matchAndRewrite(TF::SigmoidGradOp op,
    ...
    Value one = rewriter.create<mhlo::ConstantOp>(
        loc, DenseElementsAttr::get(
                 machina::GetTypeFromTFTensorShape({}, elem_tp), attr));
...
class GenericConvertUnsortedSegmentReductionOp : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto output_type = machina::GetTypeFromTFTensorShape(
    ...
    auto dims_attr = ScatterDimensionNumbersAttr::get(
        rewriter.getContext(),
        toolchain::to_vector<4>(toolchain::seq<int64_t>(segment_ids_rank, data_rank)),
...
class ConvertRandomShuffleOp : public OpRewritePattern<TF::RandomShuffleOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::RandomShuffleOp op,
    ...
    auto indices_type = machina::GetTypeFromTFTensorShape(
    ...
    auto swap_body_fn = [&](Location loc, Value i, ArrayRef<Value> old_values,
      ...
      auto one_cross_i64_type = machina::GetTypeFromTFTensorShape(
      ...
      indices = builder->create<mhlo::DynamicUpdateSliceOp>(
          loc, indices.getType(), indices, target_index, toolchain::ArrayRef(i));
      // indices[swaps[i]] <- indices[i]
      indices = builder->create<mhlo::DynamicUpdateSliceOp>(
          loc, indices.getType(), indices, source_index,
          toolchain::ArrayRef(swap_index));
    ...
    auto slice_sizes = machina::ConvertMlirShapeToTF(input_type.getShape());
    slice_sizes[0] = 1;
    auto dims_attr = GatherDimensionNumbersAttr::get(
        rewriter.getContext(),
        /*offsetDims=*/toolchain::to_vector<4>(toolchain::seq<int64_t>(1, input_rank)),
    ...
    for (auto i = 0; i < slice_sizes.size(); ++i) {
      if (slice_sizes[i] == machina::kTFDynamicSize) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::InplaceUpdateOp op,
    ...
    SmallVector<Type, 4> unpacked_indices_type(
        indices_type.getDimSize(0), machina::GetTypeFromTFTensorShape(
    ...
    split_updates_type.resize(
        ...
        machina::GetTypeFromTFTensorShape(split_updates_shape,
    ...
    for (auto pair :
         toolchain::zip(unpacked_indices.getOutput(), split_updates.getOutput())) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::ClipByValueOp op,
    ...
    auto shape = rewriter.create<TF::ShapeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape({input_ty.getRank()},
                                             rewriter.getI32Type()),
        input);
...
class ConvertCumOp : public OpRewritePattern<OpT> {
  ...
  LogicalResult matchAndRewrite(OpT op,
    ...
    if (op.getReverse()) {
      toolchain::SmallVector<int64_t, 4> dims_to_reverse({axis});
      input = rewriter.create<ReverseOp>(
    ...
    auto paddings_attr =
        DenseIntElementsAttr::get(machina::GetTypeFromTFTensorShape(
    ...
    if (op.getExclusive()) {
      ...
      toolchain::SmallVector<int64_t, 4> low_padding(rank, 0);
      toolchain::SmallVector<int64_t, 4> high_padding(rank, 0);
      toolchain::SmallVector<int64_t, 4> interior_padding(rank, 0);
      low_padding[axis] = 1;
      high_padding[axis] = -1;
...
class ConvertShapeOp : public OpRewritePattern<TF::ShapeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::ShapeOp op,
    ...
    auto index_tensor = machina::GetTypeFromTFTensorShape(
  ...
  LogicalResult matchAndRewrite(TF::ExpandDimsOp op,
    ...
    auto shape = rewriter.create<shape::ShapeOfOp>(
        ...
        machina::GetTypeFromTFTensorShape({input_ty.getRank()},
                                             rewriter.getIndexType()),
        input);
    auto expand_dims = toolchain::to_vector<6>(expand_dims_attr.getValues<APInt>());

    toolchain::SmallVector<Value, 4> dims;
...
class ConvertDynamicSqueezeOp : public OpRewritePattern<TF::SqueezeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::SqueezeOp op,
    ...
    toolchain::SmallVector<Value> dims;
    for (int64_t i = 0; i != input_rank; ++i) {
      if (toolchain::is_contained(squeeze_dims, i)) continue;
...
inline std::optional<xla::RandomAlgorithm> TensorFlowRngAlgToXla(
    machina::Algorithm alg) {
  if (alg == machina::RNG_ALG_PHILOX) {
    return xla::RandomAlgorithm::RNG_PHILOX;
  } else if (alg == machina::RNG_ALG_THREEFRY) {
    return xla::RandomAlgorithm::RNG_THREE_FRY;
  } else if (alg == machina::RNG_ALG_AUTO_SELECT) {
...
class ConvertXlaRngBitGeneratorOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::XlaRngBitGeneratorOp op,
    ...
    auto alg = static_cast<machina::Algorithm>(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::XlaVariadicReduceV2Op op,
    ...
    auto func_op = cast<mlir::func::FuncOp>(SymbolTable::lookupSymbolIn(
        op->getParentOfType<mlir::ModuleOp>(), func));
    auto func_ty = func_op.getFunctionType();
    SmallVector<Type> elementTypes{toolchain::map_range(
        func_ty.getResults(),
...
class LowerControlFlowOp : public OpConversionPattern<SrcOpT> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<Type, 4> element_types;
    ...
    if constexpr (std::is_same<DstOpT, mhlo::CaseOp>::value) {
      ...
      toolchain::SmallVector<Type, 4> while_result_types;
    ...
    for (int64_t idx = 0; idx < num_regions; ++idx) {
      ...
      if constexpr (std::is_same<DstOpT, mhlo::WhileOp>::value) {
        ...
        for (const auto &[block_idx, original_ty] :
             toolchain::enumerate(block.getArgumentTypes())) {
...
// version for now.
#include "machina/compiler/mlir/stablehlo/transforms/generated_legalize_tf.inc"
