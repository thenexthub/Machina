 ...
 *
 */
#include "machina/compiler/mlir/stablehlo/transforms/fold_broadcast_pass.h"
...
// match result shape.
int64_t GetElementIndex(toolchain::SmallVectorImpl<int64_t> &shape,
                        toolchain::SmallVectorImpl<int64_t> &current_index) {
  int64_t ind = 0;
  int64_t mul = 1;
...
Attribute ConstFoldBroadcastInDim(ShapedType result_type,
  ...
  auto dimensions = toolchain::to_vector(bcast_dims.getValues<int64_t>());
  const auto result_shape = result_type.getShape();
  // Index for the broadcasted matrix.
  toolchain::SmallVector<int64_t, 16> current_index(result_type.getRank(), 0);
  // Computes the new operand shape using the original shape and the broadcast
  // dimensions to match result shape.
  toolchain::SmallVector<int64_t, 16> operand_new_shape(result_type.getRank(), 1);
  for (int i = 0; i < dimensions.size(); ++i) {
    operand_new_shape[dimensions[i]] = operand.getType().getDimSize(i);
  }

  toolchain::SmallVector<ElementValueT, 16> new_values;
...
static Attribute BinaryFolder(Op *op) {
  ...
  for (const auto zip : toolchain::zip(lhs.template getValues<ValType>(),
