...
toolchain::hash_code hash_value(ValueOrConst value) {
  return value.isConstant() ? static_cast<toolchain::hash_code>(value.constant())
...
struct ShapeValue {
  ...
  ArrayRef<ValueOrConst> scalars() const {
    assert(!is_vector);
    return toolchain::ArrayRef(shape);
  }
...
toolchain::hash_code hash_value(ShapeValue shape) {
...
class ShapeEqualityKnowledge {
 ...
 private:
  ...
  void registerAssociation(ShapeValue shape, Value value) {
    LLVM_DEBUG({ toolchain::dbgs() << "Processing " << value << "\n"; });
    auto insert_symbolic = symbolic_shapes_.insert({shape, value});
    if (insert_symbolic.second) {
      LLVM_DEBUG({ toolchain::dbgs() << "New symbolic shape " << shape << "\n"; });
      ...
      LLVM_DEBUG({ toolchain::dbgs() << "Aliasing with rep " << rep << "\n"; });
  ...
  void tryEvaluateShapeToRoot(ShapeValue shape, Value value) {
    ...
    if (!shape.isVector()) {
      ...
      Value candidate;
      bool all_are_dimops =
          toolchain::all_of(toolchain::enumerate(shape.scalars()), [&candidate](auto p) {
  ...
  toolchain::EquivalenceClasses<void *> equal_shapes_;
  // A map from a value that encodes a shape to a value that has this shape.
  toolchain::DenseMap<ShapeValue, Value> symbolic_shapes_;
};
...
/// That way, toolchain can CSE index computations on same-shaped inputs.
struct PropagateShapeKnowledgeToKernels
  ...
  void runOnOperation() override {
    ...
    getOperation().walk([&](gpu::LaunchFuncOp launch) {
      ...
      toolchain::SmallVector<std::pair<Value, int>, 4> seen_memrefs;
      ...
      for (auto operand : launch.getKernelOperands()) {
        ...
        for (auto previous : seen_memrefs) {
          ...
          auto current_args = kernel.getArguments()
                                  ...
                                  .take_front(args_to_replace);
          for (auto pair : toolchain::zip(previous_args, current_args)) {
