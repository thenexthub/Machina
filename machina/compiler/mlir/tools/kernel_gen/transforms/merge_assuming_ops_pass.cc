...
struct ShapeReificationPattern : public OpRewritePattern<shape::ShapeOfOp> {
  ...
  LogicalResult matchAndRewrite(shape::ShapeOfOp op,
    ...
    toolchain::SmallVector<Value, 1> reifications;
...
LogicalResult moveUpIntoAssumingOpMatchAndRewrite(Operation *op,
  ...
  while (prev != nullptr && !toolchain::isa<shape::AssumingOp>(prev))
    prev = prev->getPrevNode();
  auto assumingOp = toolchain::dyn_cast_or_null<shape::AssumingOp>(prev);
  ...
  if (!toolchain::all_of(op->getOperands(), isAvailable)) return failure();

  Block *body = assumingOp.getBody();
  auto yieldOp = toolchain::cast<shape::AssumingYieldOp>(body->getTerminator());
  ...
  SmallVector<Value, 8> newOperandsUnmapped =
      toolchain::to_vector<8>(toolchain::map_range(op->getOperands(), [&](Value v) {
        for (const auto &result : toolchain::enumerate(assumingOp->getResults())) {
  ...
  auto newAssumingOp = rewriter.create<shape::AssumingOp>(
      ...
      [&](OpBuilder &b, Location) {
        ...
        for (auto it : toolchain::zip(op->getOperands(), newOperandsUnmapped)) {
        ...
        auto mappedResults = toolchain::to_vector<8>(toolchain::map_range(
            yieldOp.getOperands(),
...
struct MoveUpOutOfAssumingOpPattern : public OpRewritePattern<OpTy> {
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto isNewOpResult = [newOp](Value v) {
      return toolchain::is_contained(newOp->getResults(), v);
    };
    auto yieldOp = cast<shape::AssumingYieldOp>(body->getTerminator());
    if (toolchain::none_of(yieldOp.getOperands(), isNewOpResult)) return success();
...
struct MergeAssumingOpsPattern : public OpRewritePattern<shape::AssumingOp> {
  ...
  LogicalResult matchAndRewrite(shape::AssumingOp op,
    ...
    // available.
    auto precedingOp =
        toolchain::dyn_cast_or_null<shape::AssumingOp>(op->getPrevNode());
    ...
    auto newAssumingOp = rewriter.create<shape::AssumingOp>(
        precedingOp.getLoc(), newWitness, [&](OpBuilder &b, Location) {
          ...
          auto yieldOpA =
              toolchain::dyn_cast<shape::AssumingYieldOp>(body_a->getTerminator());
          for (auto pair :
               toolchain::zip(precedingOp->getResults(), yieldOpA.getOperands())) {
          ...
          auto yieldOpB =
              toolchain::dyn_cast<shape::AssumingYieldOp>(body_b->getTerminator());
...
struct EliminateDuplicateCstrBroadcastableOps
  ...
  LogicalResult matchAndRewrite(shape::CstrBroadcastableOp op,
    ...
    while (it != nullptr) {
      if (auto candidate = toolchain::dyn_cast<shape::CstrBroadcastableOp>(it)) {
