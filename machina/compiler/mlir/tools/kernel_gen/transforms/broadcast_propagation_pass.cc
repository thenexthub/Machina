...
bool allowsForBroadcastPropagation(Operation *op) {
  return toolchain::isa_and_nonnull<DynamicBroadcastInDimOp>(op) ||
...
DenseIntElementsAttr composeBroadcastDimensionsAttr(OpBuilder &builder,
  ...
  SmallVector<int64_t> bVec =
      toolchain::to_vector(toolchain::map_range(b, [](const APInt &it) {
        return static_cast<int64_t>(it.getLimitedValue());
      }));
  SmallVector<int64_t> composedVec = toolchain::to_vector(toolchain::map_range(
...
void findBroadcastIntents(
  ...
  for (int64_t i = 0; i < static_cast<int64_t>(bcastIntents.size()); ++i) {
    ...
    if (auto producerBcastOp =
            toolchain::dyn_cast<DynamicBroadcastInDimOp>(producerOp)) {
...
DenseMap<BroadcastIntent, Value> realizeBroadcastIntents(
  ...
  for (auto it : sortedBcastIntents) {
    ...
    if (auto producerBcastOp =
            toolchain::dyn_cast_or_null<DynamicBroadcastInDimOp>(producerOp)) {
    ...
    auto bcastedOperands = toolchain::to_vector(
        toolchain::map_range(deps->second, [&](BroadcastIntent operandIntent) {
...
void transitivelyEraseUnusedSideEffectFreeOps(Operation *root,
  ...
  while (!worklist.empty()) {
    ...
    if (!isMemoryEffectFree(op) ||
        !toolchain::all_of(op->getUsers(), [opsToEraseSet](Operation *user) {
