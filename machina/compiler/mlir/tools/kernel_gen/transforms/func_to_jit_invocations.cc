...
LogicalResult RewriteToFullJit(func::FuncOp op) {
  ...
  toolchain::SmallVector<Location> locs(old_body->getNumArguments(), loc);
...
void PackJITCompileOp(tf_framework::JITCompileOp op,
                      ...
                      toolchain::ArrayRef<int64_t> unroll_factors, bool enable_ftz,
                      bool index_64bit, bool cpu_codegen) {
  IRRewriter rewriter(op.getContext());
  Block *body = op.SingleBlock::getBody();
  auto yield_op =
      toolchain::cast<tf_framework::JITCompileYieldOp>(body->getTerminator());
  ...
  std::string code;
  toolchain::raw_string_ostream ss(code);
...
struct FuncToJITInvocationPass
  ...
  explicit FuncToJITInvocationPass(toolchain::ArrayRef<int64_t> tile_sizes,
