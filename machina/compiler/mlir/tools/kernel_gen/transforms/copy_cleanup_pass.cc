...
void RemoveCopyIfTargetOnlyRead(func::FuncOp func) {
  toolchain::SmallVector<memref::AllocOp, 8> allocs_to_remove;
  toolchain::SmallVector<memref::CopyOp, 8> copies_to_remove;
  ...
  func->walk([&](memref::AllocOp op) {
    ...
    for (auto user : op->getUsers()) {
      ...
      if (auto effect_interface = dyn_cast<MemoryEffectOpInterface>(user)) {
        ...
        if (toolchain::any_of(effects, [](MemoryEffects::EffectInstance it) {
  ...
  toolchain::for_each(allocs_to_remove, [](Operation *op) { op->erase(); });
  toolchain::for_each(copies_to_remove, [](Operation *op) { op->erase(); });
}
...
void RemoveCopyIfTargetIsFunctionArg(func::FuncOp func) {
  ...
  toolchain::SmallVector<memref::AllocOp> allocs_to_remove;
  toolchain::SmallVector<memref::CopyOp> copies_to_remove;

  Block &body = func.getBody().front();
  for (auto &op : toolchain::reverse(body.without_terminator())) {
