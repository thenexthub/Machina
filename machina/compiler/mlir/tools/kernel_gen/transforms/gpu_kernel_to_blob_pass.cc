...
#include "machina/xla/service/gpu/gpu_asm_opts_util.h"
...
#include "machina/xla/stream_executor/cuda/cuda_asm_compiler.h"
#elif MACHINA_USE_ROCM
#include "machina/xla/stream_executor/gpu/asm_compiler.h"
...
class GpuKernelToBlobPass
 ...
 public:
  GpuKernelToBlobPass(StringRef blob_annotation,
                      toolchain::ArrayRef<std::string> architectures, bool print_ptx,
  ...
  absl::StatusOr<std::vector<uint8_t>> GetGpuBinaryBlob(
    ...
    using AmdGpuHsaco = std::vector<machina::uint8>;
    std::vector<machina::se::HsacoImage> images;
    ...
    for (const std::string& arch_str : architectures_) {
      ...
      auto llvm_module_copy = toolchain::CloneModule(*llvmModule);
      auto hsaco_or = xla::gpu::amdgpu::CompileToHsaco(
          llvm_module_copy.get(),
          machina::se::RocmComputeCapability{arch_str}, options,
          options.DebugString());
      if (!hsaco_or.ok()) {
        return machina::errors::Internal("Failure when generating HSACO");
      }
    ...
    return machina::se::BundleGpuAsm(images, machina::RocmRoot());
    ...
    llvmModule->setTargetTriple(toolchain::Triple(xla::gpu::nvptx::TargetTriple()));

    // Compile and collect requested cubin and PTX images.
    std::vector<machina::se::CubinOrPTXImage> images;
    ...
    for (const std::string& arch_str : architectures_) {
      ...
      machina::se::CudaComputeCapability cc{cc_major, cc_minor};

      // Generate PTX code.
      // Module may be changed by CompileToPtx.
      auto llvm_module_copy = toolchain::CloneModule(*llvmModule);
      auto enable_fusion = [](toolchain::TargetMachine* target) {
        target->Options.AllowFPOpFusion =
            toolchain::FPOpFusion::FPOpFusionMode::Fast;
      };
      TF_ASSIGN_OR_RETURN(std::string ptx, xla::gpu::nvptx::CompileToPtx(
                                               llvm_module_copy.get(), cc,
                                               options, enable_fusion));
      if (print_ptx_) {
        toolchain::dbgs() << "Generated PTX code for module '"
      ...
      if (!is_compute_profile) {
        auto gpu_asm = machina::se::CompileGpuAsm(cc, ptx, gpu_asm_opts);
        if (gpu_asm.ok()) {
          images.push_back(
    ...
    if (images.size() == 1) return images.front().bytes;
    return machina::se::BundleGpuAsm(images, gpu_asm_opts);
