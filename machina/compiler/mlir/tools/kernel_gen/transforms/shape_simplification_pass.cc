...
struct BroadcastRemoveSubsumedOperandsPattern
  ...
  LogicalResult matchAndRewrite(BroadcastOp op,
    ...
    if (!toolchain::is_contained(knownExtents, ShapedType::kDynamic)) {
    ...
    for (auto tuple : toolchain::zip(op.getShapes(), operandExtents)) {
      ...
      if (toolchain::count_if(operandExtents, [&](ArrayRef<int64_t> op) {
      ...
      for (size_t i = 0, e = extents.size(); i != e; ++i) {
        ...
        if (knownExtent != ShapedType::kDynamic && extent == knownExtent &&
            toolchain::count_if(operandExtents, [&](ArrayRef<int64_t> operandShape) {
