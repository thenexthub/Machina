...
class ConvertToLLVMCallOpPattern : public ConvertOpToLLVMPattern<OpTy> {
 ...
 protected:
  ...
  std::pair<Value, Value> ConvertArrayAttrToStackAllocatedArray(
    ...
    for (const auto &e : toolchain::enumerate(array_attr)) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    getMemRefDescriptorSizes(loc, memref_type,
                             toolchain::to_vector<4>(adaptor.getDynSizes()),
    ...
    Value allocated_byte_ptr =
        rewriter
            .create<LLVM::CallOp>(
                ...
                toolchain::ArrayRef({adaptor.getCtx(), num_elements, element_size,
 ...
 protected:
  ...
  Type GetFuncType() const override {
    ...
    return LLVM::LLVMFunctionType::get(
        llvm_ptr_type,
        toolchain::ArrayRef({/*void* op_kernel_ctx*/ llvm_ptr_type,
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<LLVM::CallOp>(
        ...
        toolchain::ArrayRef({adaptor.getCtx(), allocated_bytes_ptr}));
    return success();
  }
...
class JITCompileFromStrOpConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<LLVM::CallOp>(
        ...
        toolchain::ArrayRef({adaptor.getCtx(), jit_module_code, tile_sizes.first,
  ...
  LogicalResult matchAndRewrite(
    ...
    Value args_ptr =
        rewriter.create<LLVM::AllocaOp>(loc, ptr_ty, args_elem_ty, num_args,
                                        /*alignment=*/0);
    for (const auto &it : toolchain::enumerate(adaptor.getInputs())) {
      Value index = rewriter.create<LLVM::ConstantOp>(
    ...
    toolchain::SmallVector<Value, 1> final_result = {
...
class ReportErrorOpConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<LLVM::CallOp>(
        op, std::nullopt, tf_func_ref,
        toolchain::ArrayRef({adaptor.getCtx(), error_code, message_constant}));
    return success();
  }
 ...
 private:
  ...
  Value GenerateErrorMessageConstant(Location loc, Operation *module,
    ...
    std::string err_str;
    toolchain::raw_string_ostream err_stream(err_str);
    err_stream << message;
