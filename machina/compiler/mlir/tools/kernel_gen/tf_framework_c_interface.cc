...
#include "machina/compiler/mlir/tools/kernel_gen/tf_framework_c_interface.h"
...
#include "mlir/ExecutionEngine/CRunnerUtils.h"  // part of Codira Toolchain
...
#include "machina/compiler/mlir/tools/kernel_gen/compile_cache_item.pb.h"
...
using machina::Allocator;
using machina::AllocatorAttributes;

Allocator* GetAllocator(void* op_kernel_ctx) {
  auto* ctx = static_cast<machina::OpKernelContext*>(op_kernel_ctx);
...
extern "C" void* _mlir_ciface_tf_alloc(void* op_kernel_ctx, size_t num_elements,
  ...
  if (output_index != kAmbiguousOutputIndex) {
    ...
    machina::TensorShape output_shape;
...
std::string GetFileCachePath(const std::string cache_dir,
  ...
  size_t hash = toolchain::hash_value(code);
  return machina::io::JoinPath(cache_dir, std::to_string(hash));
}

// A callback to register all externally defined symbols needed by the kernel.
toolchain::orc::SymbolMap TFFrameworkSymbolMap(toolchain::orc::MangleAndInterner mangle) {
  toolchain::orc::SymbolMap symbol_map;
  auto bind = [&](toolchain::StringRef name, auto symbol_ptr) {
    symbol_map[mangle(name)] = {toolchain::orc::ExecutorAddr::fromPtr(symbol_ptr),
                                toolchain::JITSymbolFlags()};
  };
...
toolchain::Expected<std::unique_ptr<ExecutionEngine>> Compile(
    const std::string code, toolchain::SmallVectorImpl<std::string>& architectures,
    toolchain::SmallVectorImpl<int64_t>& tile_sizes,
    toolchain::SmallVectorImpl<int64_t>& unroll_factors, bool enable_ftz,
    bool index_64bit) {
  ...
  auto tenv = machina::Env::Default();
  if (!cache_dir.empty() && tenv->RecursivelyCreateDir(cache_dir).ok()) {
    std::string data;
    if (machina::ReadFileToString(tenv, GetFileCachePath(cache_dir, code),
                                     &data)
            .ok()) {
  ...
  if (item.result_module().empty()) {
    ...
    absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> status_or_module =
        machina::kernel_gen::GenerateKernelForHloCode(
    ...
    if (!cache_dir.empty() && tenv->RecursivelyCreateDir(cache_dir).ok()) {
      ...
      toolchain::raw_string_ostream stream(*item.mutable_result_module());
      module.get().print(stream);
      stream.flush();

      machina::WriteStringToFile(tenv, GetFileCachePath(cache_dir, code),
                                    item.SerializeAsString())
          .IgnoreError();
    }
  } else {
    module = machina::kernel_gen::SetupContextAndParseModule(
  ...
  toolchain::Expected<std::unique_ptr<ExecutionEngine>> engine =
...
toolchain::SmallVector<T, 8> SmallVectorFromCArray(int64_t num_elements,
                                              U* elements_ptr) {
  toolchain::SmallVector<T, 8> result;
...
extern "C" void* _mlir_ciface_tf_jit_compile(
  ...
  auto* ctx = static_cast<machina::OpKernelContext*>(op_kernel_ctx);
  machina::ResourceMgr* rm = ctx->resource_manager();
  if (!rm) {
  ...
  machina::core::ScopedUnref jit_cache_ref(jit_cache);
  if (!status.ok()) {
  ...
  toolchain::SmallVector<int64_t, 8> tile_sizes =
      SmallVectorFromCArray<int64_t>(num_tile_sizes, tile_sizes_ptr);
  toolchain::SmallVector<int64_t, 8> unroll_factors =
...
extern "C" void _mlir_ciface_tf_jit_execute(void* op_kernel_ctx, void* callable,
  ...
  toolchain::SmallVector<void*, 8> args_array = {&op_kernel_ctx};
  ...
  toolchain::Error invocation_result =
