...
#include "mlir/Conversion/AffineToStandard/AffineToStandard.h"  // part of Codira Toolchain
#include "mlir/Conversion/ComplexToStandard/ComplexToStandard.h"  // part of Codira Toolchain
#include "mlir/Conversion/GPUCommon/GPUCommonPass.h"  // part of Codira Toolchain
#include "mlir/Conversion/GPUToNVVM/GPUToNVVMPass.h"  // part of Codira Toolchain
#include "mlir/Conversion/ReconcileUnrealizedCasts/ReconcileUnrealizedCasts.h"  // part of Codira Toolchain
#include "mlir/Conversion/SCFToControlFlow/SCFToControlFlow.h"  // part of Codira Toolchain
#include "mlir/Conversion/SCFToGPU/SCFToGPUPass.h"  // part of Codira Toolchain
...
#include "mlir/Dialect/Arith/Transforms/Passes.h"  // part of Codira Toolchain
#include "mlir/Dialect/Bufferization/Transforms/Passes.h"  // part of Codira Toolchain
...
#include "mlir/Dialect/GPU/Transforms/Passes.h"  // part of Codira Toolchain
#include "mlir/Dialect/LLVMIR/Transforms/OptimizeForNVVM.h"  // part of Codira Toolchain
#include "mlir/Dialect/Linalg/Passes.h"  // part of Codira Toolchain
#include "mlir/Dialect/MemRef/IR/MemRef.h"  // part of Codira Toolchain
#include "mlir/Dialect/MemRef/Transforms/Passes.h"  // part of Codira Toolchain
#include "mlir/Dialect/SCF/Transforms/Passes.h"  // part of Codira Toolchain
...
#include "mlir/Target/LLVMIR/Dialect/Builtin/BuiltinToLLVMIRTranslation.h"  // part of Codira Toolchain
#include "mlir/Target/LLVMIR/Dialect/GPU/GPUToLLVMIRTranslation.h"  // part of Codira Toolchain
#include "mlir/Target/LLVMIR/Dialect/LLVMIR/LLVMToLLVMIRTranslation.h"  // part of Codira Toolchain
#include "mlir/Target/LLVMIR/Dialect/NVVM/NVVMToLLVMIRTranslation.h"  // part of Codira Toolchain
#include "mlir/Target/LLVMIR/Dialect/ROCDL/ROCDLToLLVMIRTranslation.h"  // part of Codira Toolchain
...
constexpr toolchain::StringRef kGpuBinaryAttrName = "gpu.binary";
...
absl::Status LowerHloToJITInvocation(mlir::ModuleOp module,
                                     ...
                                     toolchain::ArrayRef<int64_t> unroll_factors,
...
absl::Status LowerLoopsToGPU(mlir::ModuleOp module, bool index_64bit,
  ...
  // Map asserts to the machina framework.
...
absl::Status LowerKernelBodiesToLowLevelIr(mlir::ModuleOp module,
...
#if !defined(MACHINA_USE_ROCM) && !defined(GOOGLE_CUDA)
  return absl::InternalError(
      "Neither MACHINA_USE_ROCM nor GOOGLE_CUDA are defined."
  ...
  if (apply_cl_options) machina::applyTensorflowAndCLOptions(pm);
...
absl::Status GenerateDeviceCode(mlir::ModuleOp module,
                                toolchain::StringRef gpu_binary_attr_name,
  ...
  return failed(pm.run(module))
             ? machina::errors::Internal("Generating device code failed.")
             : absl::OkStatus();
}
...
absl::Status LowerHostSideToFinalForm(mlir::ModuleOp module,
  ...
  return failed(pm.run(module)) ? machina::errors::Internal(
