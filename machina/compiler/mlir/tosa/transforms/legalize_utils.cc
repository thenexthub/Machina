...
#include "machina/compiler/mlir/lite/kernels/internal/common.h"
#include "machina/compiler/mlir/lite/kernels/internal/quantization_util.h"
...
mlir::TypeAttr getConvAccTypeAttr(PatternRewriter& rewriter,
  ...
  if (auto qtype =
          toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(input_etype))
    input_etype = qtype.getStorageType();

  if (auto qtype =
          toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(output_etype))
...
std::optional<Value> convertTFConv2DCommon(
  ...
  auto a1_filter_transpose_op = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a1_transpose_dims,
  ...
  }
  {
    ...
    if (tf_pad == machina::Padding::EXPLICIT) {
...
LogicalResult getDynamicDims(PatternRewriter& rewriter, Operation* op,
                             Value value, toolchain::SmallVector<Value>& dims) {
  ...
  RankedTensorType dim_ty =
      machina::GetTypeFromTFTensorShape({}, rewriter.getI32Type());
...
std::optional<Value> buildReshapeWithDynamicDims(PatternRewriter& rewriter,
  ...
  toolchain::SmallVector<int64_t> static_dims;

  if (output_type.hasRank()) {
    static_dims = machina::ConvertMlirShapeToTF(output_type.getShape());
  } else {
    static_dims.resize(dims.size(), machina::kTFDynamicSize);
  }
  ...
  for (int i = 0, s = dims.size(); i < s; ++i) {
    ...
    if (matchPattern(dim, m_Constant(&dim_attr))) {
      ...
      if (size != machina::kTFDynamicSize &&
          static_dims[i] != machina::kTFDynamicSize &&
          size != static_dims[i]) {
        (void)rewriter.notifyMatchFailure(
      ...
      static_dims[i] =
          size == machina::kTFDynamicSize ? static_dims[i] : size;
    }

    if (static_dims[i] == machina::kTFDynamicSize) dyn_count++;
  }
...
Value buildRescaleOpConvOutput(PatternRewriter& rewriter, Operation* op,
  ...
  if (auto weight_per_tensor_qtype =
    ...
    for (double weight_scale : weight_scale_arr) {
      ...
      // Reference: machina/lite/kernels/internal/common.cc
      if (!scale32) {
...
Value getTosaConstHardSwish8bitTable(PatternRewriter& rewriter, Operation* op,
  ...
  // machina/lite/kernels/internal/reference/hardswish.h
  ...
  auto const_type = machina::GetTypeFromTFTensorShape({256}, element_qtype);
  auto storage_type = machina::GetTypeFromTFTensorShape(
      {256}, element_qtype.getStorageType());
  auto const_attr = DenseElementsAttr::get(storage_type, toolchain::ArrayRef(table));
...
// Follow LUTPopulateInt8() machina/lite/kernels/internal/common.h
...
// Follow LUTPopulateInt16() machina/lite/kernels/internal/common.h
...
Value getTosaConst16bitTable(PatternRewriter& rewriter, Operation* op,
  ...
  auto const_type =
      machina::GetTypeFromTFTensorShape({513}, rewriter.getIntegerType(16));
  auto const_attr = DenseElementsAttr::get(const_type, toolchain::ArrayRef(table));
...
void getTosaConst32bitSoftmaxExpTable(PatternRewriter& rewriter, Operation* op,
  ...
  auto const_type = machina::GetTypeFromTFTensorShape({513}, element_qtype);
  auto storage_type = machina::GetTypeFromTFTensorShape(
      {513}, element_qtype.getStorageType());

  auto first_const_attr =
      DenseElementsAttr::get(storage_type, toolchain::ArrayRef(first_table));
  auto second_const_attr =
      DenseElementsAttr::get(storage_type, toolchain::ArrayRef(second_table));
  auto third_const_attr =
      DenseElementsAttr::get(storage_type, toolchain::ArrayRef(third_table));
  auto fourth_const_attr =
      DenseElementsAttr::get(storage_type, toolchain::ArrayRef(fourth_table));
...
Value getTosaConstTensorSingleF32(PatternRewriter& rewriter, Operation* op,
  ...
  if (rank == 0) {
    const_type = machina::GetTypeFromTFTensorShape({}, element_type);
    const_attr = DenseElementsAttr::get(const_type, val);
  } else {
    std::vector<int64_t> shape(rank, 1);
    const_type = machina::GetTypeFromTFTensorShape(toolchain::ArrayRef(shape),
                                                      element_type);
    std::vector<float> values(1, val);
    const_attr = DenseElementsAttr::get(const_type, toolchain::ArrayRef(values));
  }
...
// SAME/VALID.
//
// This could pass machina::FilterTensorFormat and do
...
bool getPaddingValuesFromPadType(machina::Padding tf_pad,
                                 machina::TensorFormat data_format_tf,
  ...
  assert(tf_pad != machina::Padding::EXPLICIT);
  if (!input_type.hasRank() || !filter_type.getRank()) return false;
  // Only support NHWC for now.
  if (data_format_tf != machina::FORMAT_NHWC) return false;
...
DenseI64ArrayAttr getPaddingValuesFromExplicitPadAttr(
    ArrayAttr explicit_pad, machina::TensorFormat data_format_tf,
    PatternRewriter& rewriter) {
  SmallVector<int64_t> computed_paddings;
...
bool getTransposeConv2dPaddingValues(
    machina::Padding tf_pad, machina::TensorFormat data_format_tf,
...
std::optional<Value> getConstTensor<float>(PatternRewriter& rewriter,
  ...
  auto const_type =
      machina::GetTypeFromTFTensorShape(shape, rewriter.getF32Type());
  auto const_attr = DenseElementsAttr::get(const_type, vec);
...
toolchain::SmallVector<int64_t> getOutputSpatialSizeRemainder(
    machina::TensorFormat data_format_tf, ShapedType input_type,
    DenseI64ArrayAttr kernel_size, DenseI64ArrayAttr pads,
    DenseI64ArrayAttr strides, DenseI64ArrayAttr dilations) {
  toolchain::SmallVector<int64_t> output_size_remainder;
...
Value getInputSlicedToItsUsedSize(PatternRewriter& rewriter, Operation* op,
  ...
  const toolchain::SmallVector<int64_t> output_size_remainder =
      getOutputSpatialSizeRemainder(data_format_tf, input_type, kernel_size,
                                    pads, strides, dilations);

  const bool need_slicing =
      toolchain::any_of(output_size_remainder, [](int64_t v) { return v > 0; });
  const bool zero_pads =
      toolchain::all_of(pads.asArrayRef(), [](int64_t v) { return v == 0; });
  if (need_slicing && zero_pads) {
    toolchain::SmallVector<int64_t> start(input_type.getRank(), 0);
    toolchain::SmallVector<int64_t> size =
        machina::ConvertMlirShapeToTF(input_type.getShape());
    for (int spatial_dim = 0; spatial_dim < nb_spatial_dims; spatial_dim++) {
...
LogicalResult ApplyPatternsWithShapeResolution(
  ...
  toolchain::SmallVector<Type> result_tys(returnOp.getOperandTypes());
...
Value reshapeScalarTo1D(PatternRewriter& rewriter, Location loc, Value value) {
  ...
  auto output_type = machina::GetTypeFromTFTensorShape({1}, element_type);

  // for rank-0 constant value, return a rank-1 constant value
  mlir::DenseElementsAttr attr;
  if (matchPattern(value, m_Constant(&attr))) {
    auto storage_type = machina::GetTypeFromTFTensorShape({1}, element_type);
    auto element_qtype = dyn_cast<quant::QuantizedType>(element_type);
    if (element_qtype) {
      storage_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult broadcastLowRankTensor(PatternRewriter& rewriter, Operation* op,
  ...
  auto input1_ty = toolchain::dyn_cast<RankedTensorType>(input1.getType());
  auto input2_ty = toolchain::dyn_cast<RankedTensorType>(input2.getType());
  ...
  ArrayRef<int64_t> high_rank_shape =
      toolchain::cast<RankedTensorType>(high_rank_tensor.getType()).getShape();
  ArrayRef<int64_t> low_rank_shape =
      toolchain::cast<RankedTensorType>(low_rank_tensor.getType()).getShape();
  ...
  auto broadcast_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertMlirShapeToTF(broadcast_shape));
...
bool checkUniqueConstantScatterIndices(ShapedType indices_type,
  ...
  toolchain::ArrayRef<int64_t> const indices_shape = indices_type.getShape();
