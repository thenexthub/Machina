...
#include "machina/compiler/mlir/tosa/transforms/tf_legalize_patterns.inc"
...
LogicalResult ConvertTFAvgPoolOp::matchAndRewrite(
  ...
  }
  {
    ...
    RankedTensorType filter_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult ConvertTFShapeOp::matchAndRewrite(
  ...
  RankedTensorType shape_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult ConvertTFFillOp::matchAndRewrite(
  ...
  RankedTensorType fill_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult ConvertTFConv2DOp::matchAndRewrite(
  ...
  RankedTensorType bias_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult ConvertTFConv2DBackpropInputOp::matchAndRewrite(
  ...
  auto a1_filter_transpose_op = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(ArrayRef<int64_t>(a1_transpose_dims),
...
LogicalResult ConvertTFFusedBatchNormOp::matchAndRewrite(
  ...
  RankedTensorType epsilon_type =
      machina::GetTypeFromTFTensorShape({1}, variance_type.getElementType());
  auto epsilon_attr =
...
LogicalResult ConvertTFFusedBatchNormV3Op::matchAndRewrite(
  ...
  toolchain::SmallVector<Value> replacements = {
...
LogicalResult ConvertTFMatMulOp::matchAndRewrite(
  ...
  RankedTensorType batch_a_type = machina::GetTypeFromTFTensorShape(
      batch_a_shape, a_type.getElementType());
  RankedTensorType batch_b_type = machina::GetTypeFromTFTensorShape(
      batch_b_shape, b_type.getElementType());
  RankedTensorType batch_output_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult ConvertTFBatchMatMulV2Op::matchAndRewrite(
  ...
  if (x_type.getRank() == 3) {
    ...
    RankedTensorType rank3_x_type = machina::GetTypeFromTFTensorShape(
        rank3_x_shape, x_type.getElementType());
    RankedTensorType rank3_y_type = machina::GetTypeFromTFTensorShape(
        rank3_y_shape, y_type.getElementType());
    RankedTensorType rank3_output_type = machina::GetTypeFromTFTensorShape(
