...
// toolchain::Optional value after each call.
...
#include "mlir/Dialect/Arith/Utils/Utils.h"  // part of Codira Toolchain
...
#include "mlir/Dialect/Tosa/Utils/ConversionUtils.h"  // part of Codira Toolchain
#include "mlir/Dialect/Tosa/Utils/QuantUtils.h"  // part of Codira Toolchain
#include "mlir/Dialect/Tosa/Utils/ConversionUtils.h"  // part of Codira Toolchain
#include "mlir/Dialect/Utils/StaticValueUtils.h"  // part of Codira Toolchain
...
static int64_t multiply_dims(toolchain::ArrayRef<int64_t> dims, int64_t res = 1) {
  for (auto dim : dims) {
...
static Value buildReshape(ImplicitLocOpBuilder& builder, Value value,
  ...
  auto staticShape = toolchain::map_to_vector(shape, extractStaticDimension);
  auto elementType = getElementTypeOrSelf(value.getType());

  if (toolchain::count_if(staticShape, ShapedType::isDynamic) > 1) {
    ...
    return builder.create<tensor::ReshapeOp>(
        machina::GetTypeFromTFTensorShape(staticShape, elementType), value,
        runtimeShape);
  }
  ...
  auto staticShapeAttr =
      builder.getIndexTensorAttr(machina::ConvertMlirShapeToTF(staticShape));
  ...
  return CreateOpAndInfer<tosa::ReshapeOp>(
      builder, machina::GetTypeFromTFTensorShape(staticShape, elementType),
...
// machina/core/kernels/fake_quant_ops_functor.h.
// Suggested approach to avoid significant TensorFlow
// build dependency.
void machina_nudge(const float min, const float max, const int quant_min,
...
std::optional<Value> convertPackOp(PatternRewriter& rewriter, Operation* op,
  ...
  toolchain::SmallVector<int64_t> input_shape;
  ...
  if (output_type) {
    ...
    for (auto [i, dim] : toolchain::enumerate(out_tensor_shape)) {
    ...
    for (int i = 0; i < inputs.size(); i++) {
      ...
      if (current_input_type) {
        ...
        for (auto [i, dim] : toolchain::enumerate(current_input_shape)) {
    ...
    toolchain::SmallVector<int64_t> output_shape;
    ...
    if (current_input_type) {
      ...
      auto const_shape =
          getTosaConstShape(rewriter, op->getLoc(),
                            machina::ConvertMlirShapeToTF(input_shape));
  ...
  if (input_tensor_rank == 0) {
    ...
    RankedTensorType reshape_rank1_size1_type =
        machina::GetTypeFromTFTensorShape(reshape_rank1_size1_shape,
                                             element_type);
    auto shape_rank1_size1_value = getTosaConstShape(
        rewriter, op->getLoc(),
        machina::ConvertMlirShapeToTF(reshape_rank1_size1_shape));
    for (int i = 0; i < inputs.size(); i++) {
  ...
  RankedTensorType concat_type = machina::GetTypeFromTFTensorShape(
  ...
  RankedTensorType reshape_output_type =
      machina::GetTypeFromTFTensorShape(reshape_output_shape, element_type);
  auto shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertMlirShapeToTF(reshape_output_shape));
...
std::optional<SmallVector<Value>> convertUnpackOp(PatternRewriter& rewriter,
  ...
  if (axis != 0) {
    ...
    auto a1_transpose_op = CreateOpAndInfer<tosa::TransposeOp>(
        rewriter, op->getLoc(),
        machina::GetTypeFromTFTensorShape(a1_transpose_shape,
  ...
  for (int i = 0; i < transposed_input_shape[0]; i++) {
    ...
    auto shape_vals_value = getTosaConstShape(
        rewriter, op->getLoc(), machina::ConvertMlirShapeToTF(shape_vals));
...
std::optional<Value> convertZerosLikeOp(PatternRewriter& rewriter,
  ...
  ShapedType zero_type = machina::GetTypeFromTFTensorShape(
...
std::optional<Value> convertConcatV2Op(PatternRewriter& rewriter, Operation* op,
  ...
  for (auto v : values) {
    ...
    // https://github.com/machina/machina/issues/39658
    ...
    if (result_quant_type && operand_quant_type) {
      ...
      if (operand_scale != result_scale) {
        RankedTensorType rescale_type = machina::GetTypeFromTFTensorShape(
...
std::optional<Value> convertSpaceToBatchNDOp(PatternRewriter& rewriter,
  ...
  // machina/compiler/tf2xla/kernels/spacetobatch_op.cc
  ...
  auto a1_pad_input_op = CreateOpAndInfer<tosa::PadOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(padded_shape,
                                           result_type.getElementType()),
  ...
  auto a2_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
      machina::ConvertMlirShapeToTF(a2_shape));
  ...
  auto a3_transpose_a2_op = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a3_transpose_shape,
                                           result_type.getElementType()),
  ...
  auto a4_reshape_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(a4_reshape_shape));
  return CreateOpAndInfer<tosa::ReshapeOp>(
...
std::optional<Value> convertBatchToSpaceNDOp(PatternRewriter& rewriter,
  ...
  auto a1_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(a1_shape));
  auto a1_reshape_input_op = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a1_shape,
                                           result_type.getElementType()),
      input_value,
      a1_shape_value);
  ...
  auto a2_transpose_a1_op = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a2_transpose_shape,
                                           result_type.getElementType()),
  ...
  auto a4_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(a4_shape));
  auto a3_reshape_a2 = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a4_shape,
                                           result_type.getElementType()),
  ...
  return CreateOpAndInfer<tosa::SliceOp>(
             rewriter, op->getLoc(),
             machina::GetTypeFromTFTensorShape(a4_size_vals,
                                                  result_type.getElementType()),
...
std::optional<Value> convertExpandDimsOp(PatternRewriter& rewriter,
  ...
  auto shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(reshape_dims));
...
std::optional<Value> convertEluOp(PatternRewriter& rewriter, Operation* op,
  ...
  auto a3_ge_in_zero_op = CreateOpAndInfer<tosa::GreaterEqualOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(output_type.getShape(),
...
std::optional<Value> convertSoftmaxOp(PatternRewriter& rewriter, Operation* op,
  ...
  // For more details: https://github.com/machina/machina/issues/60435
  if (beta != 1.0) {
    (void)rewriter.notifyMatchFailure(
  ...
  if (mlir::isa<mlir::quant::QuantizedType>(input_type.getElementType()) &&
    ...
    RankedTensorType int16_logits_type =
        machina::GetTypeFromTFTensorShape(logits_shape, int16_element_qtype);
    RankedTensorType int32_logits_type = machina::GetTypeFromTFTensorShape(
        logits_shape, rewriter.getIntegerType(32));
    RankedTensorType int16_rsum_type =
        machina::GetTypeFromTFTensorShape(rsum_shape, int16_element_qtype);
    RankedTensorType int32_rsum_type = machina::GetTypeFromTFTensorShape(
    ...
    if (in_quant_type.getStorageTypeIntegralWidth() == 8) {
      ...
      // Follow TFLite reference: machina/lite/kernels/activations.cc
    ...
    // op5 = reciprocal(op4)
    // op6 = mul(op3, op5)
    RankedTensorType rsum_type = machina::GetTypeFromTFTensorShape(
        rsum_shape, output_type.getElementType());
    RankedTensorType logits_type = machina::GetTypeFromTFTensorShape(
...
std::optional<Value> convertSpaceToDepthOp(PatternRewriter& rewriter,
  ...
  RankedTensorType a_reshape_output_type = machina::GetTypeFromTFTensorShape(
      a_reshape_dims, output_type.getElementType());

  auto a_reshape_dims_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(a_reshape_dims));
  ...
  auto a3_transpose_a2_op = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a2_transpose_dims,
                                           output_type.getElementType()),
  ...
  RankedTensorType a3_reshape_output_type =
      machina::GetTypeFromTFTensorShape(a3_reshape_dims,
                                           output_type.getElementType());

  auto a3_reshape_dims_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(a3_reshape_dims));
...
std::optional<SmallVector<Value>> convertSplitOp(
  ...
  if (is_dyn_split) {
    ...
    auto new_shape_value =
        getTosaConstShape(rewriter, op->getLoc(),
                      machina::ConvertMlirShapeToTF(new_shape));
    slice_value = CreateOpAndInfer<tosa::ReshapeOp>(
        rewriter, op->getLoc(),
        machina::GetTypeFromTFTensorShape(new_shape, etype), input_value,
        new_shape_value);
  }
  ...
  for (int j = 0, s = slice_type.getRank(); j < s; j++) {
    ...
    size_vals.push_back(ShapedType::isDynamic(dim) ? machina::kTFDynamicSize
                                                   : dim);
  }
  ...
  for (int i = 0; i < num_split; i++) {
    ...
    Value result = CreateOpAndInfer<tosa::SliceOp>(
        rewriter, op->getLoc(),
        machina::GetTypeFromTFTensorShape(size_vals,
                                             result_type.getElementType()),
    ...
    if (is_dyn_split) {
      ...
      auto out_reshape_shape_value =
          getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertMlirShapeToTF(out_reshape_shape));
      result =
          CreateOpAndInfer<tosa::ReshapeOp>(
              rewriter, op->getLoc(),
              machina::GetTypeFromTFTensorShape(out_reshape_shape, etype),
              result,
...
static Value reverseNegativeStride(PatternRewriter& rewriter, Operation* op,
  ...
  for (const auto& it : toolchain::enumerate(strides)) {
    auto axis = it.index();
...
std::optional<Value> convertStridedSliceOp(
  ...
  if (toolchain::any_of(strides, [](auto i) { return i < -1; })) {
    (void)rewriter.notifyMatchFailure(op, "stride < -1 unsupported");
    return std::nullopt;
  }

  // Set begin mask values if possible.
  for (const auto& val : toolchain::enumerate(begin))
  ...
  auto a1_slice_op = CreateOpAndInfer<tosa::SliceOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a1_size, element_type), input_value,
  ...
  auto a2_reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a2_shape, element_type),
  ...
  auto a3_slice_op = CreateOpAndInfer<tosa::SliceOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(a3_size, element_type),
...
Value floorIntDiv(PatternRewriter& rewriter, Operation* op,
  ...
  auto output_shape_value = getTosaConstShape(
      rewriter, op->getLoc(),
      machina::ConvertMlirShapeToTF(output_type.getShape()));
...
std::optional<Value> convertReduceOpCommon(
  ...
  toolchain::SmallVector<int64_t> axes;
  ...
  for (auto axis_val : axes) {
    ...
    RankedTensorType reduce_type =
        machina::GetTypeFromTFTensorShape(shape_vec, reduce_element_type);
  ...
  const auto squeeze_axes = [](toolchain::ArrayRef<int64_t> in, toolchain::ArrayRef<int64_t> axes) {
    toolchain::SmallVector<int64_t> sorted_axes{axes};
    std::sort(sorted_axes.begin(), sorted_axes.end());
    auto current_axis = sorted_axes.begin();

    toolchain::SmallVector<int64_t> out;
    out.reserve(in.size() - axes.size());
    for (const auto& [i, dim] : toolchain::enumerate(in)) {
  ...
  auto output_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(output_shape));
  auto reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
...
std::optional<Value> convertReduceMeanOp(PatternRewriter& rewriter,
  ...
  if (input_is_qtype) {
    ...
    // machina/lite/kernels/internal/reference/reduce.h taking into account
...
std::optional<Value> convertResizeOp(PatternRewriter& rewriter, Operation* op,
  ...
  if (output_height % input_height == 0) {
    ...
    if (toolchain::isPowerOf2_64(quotient)) {
  ...
  if (input_is_qtype) {
    ...
    if (is_bilinear) {
      ...
      if (input_element_qtype.getStorageTypeIntegralWidth() == 16) {
        is_scale32 = false;
        output_acc_type = machina::GetTypeFromTFTensorShape(
...
std::optional<Value> convertFakeQuantOp(PatternRewriter& rewriter,
  ...
  machina_nudge(min, max, qmin, qmax, &nudged_min, &nudged_max,
                   &nudged_scale);
  auto rank = input_type.getRank();
...
std::optional<Value> convertGatherOp(PatternRewriter& rewriter, Operation* op,
  ...
  if (axis < 0 || axis >= params_rank) {
    (void)rewriter.notifyMatchFailure(
        op, toolchain::formatv("axis {} must be within range of params rank", axis));
    return std::nullopt;
  }
  ...
  // Calculate N, K, W, C
  auto paramsLow = toolchain::ArrayRef(params_shape).take_front(batch_dims);

  auto paramsMid =
      toolchain::ArrayRef(params_shape).slice(batch_dims, axis - batch_dims);

  auto paramsHigh =
      toolchain::ArrayRef(params_shape).slice(axis + 1, params_rank - axis - 1);

  auto indicesMid = toolchain::ArrayRef(indices_shape)
  ...
  auto params_transpose_op = CreateOpAndInfer<tosa::TransposeOp>(
      builder,
      machina::GetTypeFromTFTensorShape(params_transpose_shape,
  ...
  auto tosa_gather_static_shape =
      toolchain::map_to_vector(tosa_gather_result_shape, extractStaticDimension);

  auto tosa_gather_op = CreateOpAndInfer<tosa::GatherOp>(
      builder,
      machina::GetTypeFromTFTensorShape(tosa_gather_static_shape,
                                           result_type.getElementType()),
...

{
  ...
  auto tosa_values_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(tosa_values_shape));
  auto tosa_values_reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(tosa_values_shape,
                                           params_type.getElementType()),
      params_value,
  ...
  auto indices_matrix_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(indices_matrix_shape));
  auto indices_matrix_reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(indices_matrix_shape,
  ...
  RankedTensorType mul_type = machina::GetTypeFromTFTensorShape(
  ...
  auto flattened_indices_reduce_op = CreateOpAndInfer<tosa::ReduceSumOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(tosa_indices_shape,
                                           indices_type.getElementType()),
  ...
  auto tosa_indices_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(tosa_indices_shape));
  ...
  auto tosa_gather_op = CreateOpAndInfer<tosa::GatherOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(tosa_gather_result_shape,
                                           result_type.getElementType()),
  ...
  auto result_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(result_type.getShape()));
...
std::optional<Value> convertScatterNdOp(PatternRewriter& rewriter,
  ...
  if (!indices_op || !toolchain::isa<tosa::ConstOp>(indices_op)) {
  ...
  auto input_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF(tosa_input_shape));
  auto tosa_input_reshape_op = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(tosa_input_shape,
                                           result_type.getElementType()),
...
std::optional<Value> convertOneHotOp(PatternRewriter& rewriter, Operation* op,
  ...
  auto shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF({ 1, 1, 1 }));
  auto op1_reshape_on_value = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({1, 1, 1},
  ...
  auto op2_tile_op1 = CreateOpAndInfer<tosa::TileOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({N, W, C},
                                           on_value_type.getElementType()),
  ...
  auto op4_tile_op3 = CreateOpAndInfer<tosa::TileOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({N, K, C},
                                           on_value_type.getElementType()),
  ...
  shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF({ N, W }));
  auto op5_reshape_indices = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({N, W},
                                           indices_type.getElementType()),
      indices_value,
      shape_value);
  ...
  shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                    machina::ConvertMlirShapeToTF({left_dim, right_dim, K}));
  auto op7_reshape_op6 = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({left_dim, right_dim, K},
                                           result_type.getElementType()),
  ...
  auto op8_transpose_op7 = CreateOpAndInfer<tosa::TransposeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape({left_dim, K, right_dim},
                                           result_type.getElementType()),
...
std::optional<Value> convertBroadcastToOp(PatternRewriter& rewriter,
  ...
  if (input_rank < shape_rank) {
    ...
    input_type =
        machina::GetTypeFromTFTensorShape(reshaped_shape, element_type);

    auto reshaped_shape_value =
        getTosaConstShape(rewriter, op->getLoc(),
                      machina::ConvertMlirShapeToTF(reshaped_shape));
  ...
  RankedTensorType output_type =
      machina::GetTypeFromTFTensorShape(new_shape, element_type);
