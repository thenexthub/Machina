...
#include "machina/lite/kernels/internal/reference/gelu.h"
...
#include "machina/compiler/mlir/tosa/transforms/tfl_legalize_patterns.inc"
...
LogicalResult ConvertTFLGeluOp::matchAndRewrite(
  ...
  //   machina/lite/kernels/internal/reference/gelu.h
  //
...
static LogicalResult prepareMatchAndRewriteComparison(
    ...
    toolchain::SmallVectorImpl<Value>& newOperands) {
  Value x = operands[0];
  Value y = operands[1];
...
LogicalResult ConvertTFLEqualOp::matchAndRewrite(
    Operation* op, PatternRewriter& rewriter) const {
  toolchain::SmallVector<Value, 2> newOperands;
...
static LogicalResult matchAndRewriteAddSub(Operation* op,
  ...
  if (output_is_qtype && input_lhs_is_qtype && input_rhs_is_qtype) {
    ...
    // Following quantization described in machina/lite/kernels/add.cc
    // In details it does:
...
bool isInt16InputAndVariableBias(mlir::ShapedType input_type, Value orig_bias) {
  auto input_qtype = toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(
      input_type.getElementType());
...
LogicalResult ConvertTFLConv2DOp::matchAndRewrite(
  ...
  {
    // TFLite filter format is machina::FORMAT_OHWI
...
LogicalResult ConvertTFLConv3DOp::matchAndRewrite(
  ...
  }
  {
    ...
    if (!getPaddingValuesFromPadType(
            tf_pad,
            machina::FORMAT_NHWC,  // TFLite only supports NDHWC format,
                                      // machina::FORMAT_NHWC is used for
  ...
  // TFLite only supports NDHWC format, machina::FORMAT_NHWC is used for both
...
LogicalResult ConvertTFLDepthwiseConv2DOp::matchAndRewrite(
  ...
  {
    // DepthwiseConv2D TFLite filter format is machina::FORMAT_IHWO
    int64_t kernel_h = filter_type.getDimSize(1);
    int64_t kernel_w = filter_type.getDimSize(2);
    kernel_size = rewriter.getDenseI64ArrayAttr({kernel_h, kernel_w});
  }
...
LogicalResult ConvertTFLBatchMatMulOp::matchAndRewrite(
  ...
  if (batch_dims.size() != 1) {
    ...
    toolchain::SmallVector<int64_t> new_lhs_shape{N};
    toolchain::SmallVector<int64_t> new_rhs_shape{N};
  ...
  if (batch_dims.size() != 1) {
    toolchain::SmallVector<int64_t> new_shape{};
    for (auto d : batch_dims) {
...
LogicalResult ConvertTFLFullyConnectedOp::matchAndRewrite(
  ...
  auto new_input_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertTFShapeToMlir(new_input_shape));
  input_val = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(new_input_shape,
                                           input_type.getElementType()),
  ...
  auto new_filter_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertTFShapeToMlir(new_filter_shape));
  filter_val = CreateOpAndInfer<tosa::ReshapeOp>(
      rewriter, op->getLoc(),
      machina::GetTypeFromTFTensorShape(new_filter_shape,
                                           filter_type.getElementType()),
  ...
  if (tfl_fc_op.getKeepNumDims()) {
    const toolchain::ArrayRef<int64_t> orig_input_shape = tfl_fc_op.getInput().getType().getShape();
...
LogicalResult ConvertTFLShapeOp::matchAndRewrite(
  ...
  auto shape_attr =
      DenseI32ArrayAttr::get(rewriter.getContext(), toolchain::ArrayRef(shape_arr));
...
LogicalResult ConvertTFLFillOp::matchAndRewrite(
  ...
  if (mlir::isa<FloatType>(value_elem.getShapedType().getElementType())) {
    ...
    fill_attr =
        DenseF32ArrayAttr::get(rewriter.getContext(), toolchain::ArrayRef(fill_arr));
  } else {
    SmallVector<int32_t> fill_arr(
        total_size, value_elem.getValues<APInt>()[0].getLimitedValue());
    fill_attr =
        DenseI32ArrayAttr::get(rewriter.getContext(), toolchain::ArrayRef(fill_arr));
  }
...
LogicalResult ConvertTFLRsqrtOp::matchAndRewrite(
  ...
  if (input_qtype && output_qtype) {
    ...
    // https://github.com/machina/machina/pull/58406
...
LogicalResult ConvertTFLReverseV2Op::matchAndRewrite(
  ...
  toolchain::SmallVector<int64_t> axis_vals;
...
LogicalResult ConvertTFLSparseToDenseOp::matchAndRewrite(
  ...
  toolchain::SmallVector<int32_t> multiply_constant_ints;
  ...
  auto multiply_constant_attr = DenseElementsAttr::get(
      multiply_constant_type, toolchain::ArrayRef(multiply_constant_ints));
  ...
  auto shape_value = getTosaConstShape(
      rewriter, op->getLoc(),
      machina::ConvertMlirShapeToTF({1, values_ty.getDimSize(0), 1}));
  ...
  shape_value = getTosaConstShape(
      rewriter, op->getLoc(),
      machina::ConvertMlirShapeToTF({1, indices_ty.getDimSize(0)}));
  ...
  auto result_shape_value =
      getTosaConstShape(rewriter, op->getLoc(),
                        machina::ConvertMlirShapeToTF(result_ty.getShape()));
...
LogicalResult ConvertTFLRealOp::matchAndRewrite(
  ...
  // this discrepancy.
  toolchain::SmallVector<int64_t> cast_size = to_vector(input_ty.getShape());
  ...
  toolchain::SmallVector<int64_t> start(rank + 1, 0);
  toolchain::SmallVector<int64_t> size = to_vector(input_ty.getShape());
  size.push_back(1);
...
LogicalResult ConvertTFLImagOp::matchAndRewrite(
  ...
  toolchain::SmallVector<int64_t> start(input_ty.getRank(), 0);
  start.push_back(1);
  toolchain::SmallVector<int64_t> size = to_vector(input_ty.getShape());
  size.push_back(1);
...
LogicalResult ConvertTFLRFFT2dOp::matchAndRewrite(
  ...
  toolchain::SmallVector<int32_t> fft_length;
  ...
  if (fft_length[0] < input_shape[rank - 2] ||
    ...
    toolchain::SmallVector<int64_t> slice_begin(rank, 0);
    toolchain::SmallVector<int64_t> slice_size;
  ...
  toolchain::SmallVector<int64_t> new_shape{output_shape};
  ...
  toolchain::SmallVector<Value, 2> values = {reshape_1, reshape_2};
