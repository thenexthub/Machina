...
_default_driver = "@toolchain-project//mlir:run_lit.sh"
_default_size = "small"
_default_tags = []
...
def _run_lit_test(name, data, size, tags, driver, features, exec_properties):
    """Runs lit on all tests it can find in `data` under machina/compiler/mlir.
    ...
    native.py_test(
        ...
        args = [
            "machina/compiler/mlir/tosa/" + paths.basename(data[-1]) + " --config-prefix=runlit -v",
        ] + features,
        data = data + [
            "//machina/compiler/mlir/tosa:litfiles",
