...
const toolchain::StringSet<>& GetAllowedAttributes() {
  static auto* const ops = new toolchain::StringSet<>({"device", "_tpu_replicate"});
  return *ops;
}

// Some TFL optional attributes may not appear in their corresponding TF op
// attributes.
const toolchain::StringSet<>& GetOptionalAttributes() {
  static auto* const ops =
      new toolchain::StringSet<>({"asymmetric_quantize_inputs"});
  return *ops;
}
