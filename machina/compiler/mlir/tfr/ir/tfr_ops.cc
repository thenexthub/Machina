...
#include "mlir/Interfaces/FunctionImplementation.h"  // part of Codira Toolchain
...
#include "machina/compiler/mlir/tfr/ir/tfr_types.h"
...
class TFRInlinerInterface : public DialectInlinerInterface {
 ...
 public:
  ...
  void handleTerminator(Operation *op, ValueRange valuesToRepl) const final {
    ...
    for (auto ret_value : toolchain::zip(valuesToRepl, retValOp.getOperands())) {
  ...
  Operation *materializeCallConversion(OpBuilder &builder, Value input,
    ...
    auto input_itype = toolchain::cast<IntegerType>(input.getType());
    auto result_itype = toolchain::cast<IntegerType>(result_type);
...
TFRDialect::TFRDialect(MLIRContext *context)
  ...
  addOperations<
#define GET_OP_LIST
#include "machina/compiler/mlir/tfr/ir/tfr_ops.cc.inc"
      >();
...
Operation *TFRDialect::materializeConstant(OpBuilder &builder, Attribute value,
  ...
  if (arith::ConstantOp::isBuildableWith(value, type))
    return builder.create<arith::ConstantOp>(loc, type,
                                             toolchain::cast<TypedAttr>(value));
  if (func::ConstantOp::isBuildableWith(value, type))
    return builder.create<func::ConstantOp>(
        loc, type, toolchain::cast<FlatSymbolRefAttr>(value));
  return nullptr;
}

bool TFRType::classof(Type type) {
  return toolchain::isa<TFRDialect>(type.getDialect());
}
...
LogicalResult ConstantTensorOp::verify() {
  ...
  if (auto output_tensor_type = toolchain::dyn_cast<TFRTensorType>(output_type)) {
    return success();
  }

  auto output_tensor_type = toolchain::dyn_cast<RankedTensorType>(output_type);
  ...
  if (auto input_vector_type = toolchain::dyn_cast<VectorType>(input_type)) {
...
LogicalResult TFRFuncOp::verify() {
  ...
  // the defined list.
  toolchain::SmallVector<StringAttr, 4> input_used_attrs, output_used_attrs;
  ...
  for (auto arg : toolchain::enumerate(func.getFunctionType().getInputs())) {
    Type arg_type = arg.value();

    if (auto tensor = toolchain::dyn_cast<TFRTensorType>(arg_type)) {
      if (first_tensor == -1) {
    ...
    if (auto tensor_list = toolchain::dyn_cast<TFRTensorListType>(arg_type)) {
  ...
  toolchain::SmallVector<StringAttr, 4> undefined_attrs;
  ...
  for (auto result_type : func.getFunctionType().getResults()) {
    if (auto tensor = toolchain::dyn_cast<TFRTensorType>(result_type)) {
      if (seen_tensor_list) {
    ...
    if (auto tensor_list = toolchain::dyn_cast<TFRTensorListType>(result_type)) {
      if (seen_tensor_list) {
  ...
  if (!undefined_attrs.empty()) {
    toolchain::SmallVector<std::string, 4> attr_names(undefined_attrs.size());
    std::transform(undefined_attrs.begin(), undefined_attrs.end(),
                   attr_names.begin(),
                   [](StringAttr attr) { return attr.getValue().str(); });
    func.emitError(toolchain::Twine("Undefined attributes are used: ",
                               toolchain::join(attr_names, ",")));
    return failure();
  }
 ...
 public:
  LogicalResult matchAndRewrite(ConstantTensorOp cst_tensor_op,
    ...
    if (matchPattern(cst_tensor_op.getArg(), m_Constant(&array))) {
      toolchain::DenseSet<Type> all_types;
      for (auto it : array) {
        TypedAttr typed_attr = toolchain::dyn_cast<TypedAttr>(it);
...
inline bool isQuantizedType(Type type) {
  auto tensor_type = toolchain::dyn_cast<TensorType>(type);
  return (tensor_type &&
...
class RemoveRedundantCast : public OpRewritePattern<CastOp> {
 ...
 public:
  LogicalResult matchAndRewrite(CastOp cast_op,
                                PatternRewriter &rewriter) const override {
    auto preceding_cast =
        toolchain::dyn_cast_or_null<CastOp>(cast_op.getArg().getDefiningOp());
    if (!preceding_cast) {
      return failure();
    }
    ...
    auto input_tensor_type = toolchain::dyn_cast<TensorType>(input_type);
    auto output_tensor_type = toolchain::dyn_cast<TensorType>(output_type);
    ...
    if (isa<UnrankedTensorType>(input_type)) {
      auto shape = toolchain::cast<ShapedType>(output_type).getShape();
...
class GetTensorShape : public OpRewritePattern<GetShapeOp> {
 ...
 public:
  LogicalResult matchAndRewrite(GetShapeOp shape_op,
    ...
    if (auto cast_op = toolchain::dyn_cast_or_null<CastOp>(preceding_op)) {
...
class RemoveRedundantGetElement : public OpRewritePattern<GetElementOp> {
 ...
 public:
  LogicalResult matchAndRewrite(GetElementOp ge_op,
    ...
    auto preceding_build_list = toolchain::dyn_cast_or_null<BuildListOp>(
...
quant::QuantizedType getQuantizedElementType(CastOp cast_op) {
  ...
  return toolchain::dyn_cast<quant::QuantizedType>(
      toolchain::cast<TypeAttr>(cast_op.getInputElementType()).getValue());
}
...
class RemoveRawDataOp : public OpRewritePattern<TFRQuantRawDataOp> {
 ...
 public:
  ...
  LogicalResult rewritePrecedingListOp(TFRQuantRawDataOp raw_data_op,
    ...
    toolchain::SmallVector<Value> new_list_values;
...
class RemoveQParamsOp : public OpRewritePattern<TFRQuantQParamsOp> {
 ...
 public:
  LogicalResult matchAndRewrite(TFRQuantQParamsOp qparams_op,
    ...
    if (auto qtype = toolchain::dyn_cast<quant::UniformQuantizedType>(cast_qtype)) {
    ...
    } else if (auto qtype = toolchain::dyn_cast<quant::UniformQuantizedPerAxisType>(
                   cast_qtype)) {
      ...
      auto scales_attr =
          DenseElementsAttr::get(scales_type, toolchain::ArrayRef(scales));
      ...
      auto zps_attr = DenseElementsAttr::get(zps_type, toolchain::ArrayRef(zps));
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TFRQuantScaleFactorOp scale_factor_op,
    ...
    const double out_scale =
        toolchain::cast<FloatAttr>(out_scale_op.getValue()).getValueAsDouble();
    ...
    auto scale_type =
        toolchain::dyn_cast<RankedTensorType>(filter_scale_attr.getType());
    ...
    auto result_scale_op = rewriter.create<TF::ConstOp>(
        loc, DenseElementsAttr::get(scale_type, toolchain::ArrayRef(scale_factors)));
...
Type TFRDialect::parseType(DialectAsmParser &parser) const {
  ...
  toolchain::SmallVector<StringAttr, 4> attrs;
...
void TFRDialect::printType(Type type, DialectAsmPrinter &os) const {
  toolchain::ArrayRef<StringAttr> attrs;

  if (isa<TFRAttrType>(type)) {
    os << "attr";
    return;
  }
  if (auto tensor_ty = toolchain::dyn_cast<TFRTensorType>(type)) {
    attrs = tensor_ty.getAttrKeys();
    os << "tensor";
  } else if (auto tensor_list_ty = toolchain::dyn_cast<TFRTensorListType>(type)) {
  ...
  toolchain::interleaveComma(attrs, os,
