...
auto* tf_core_op_expansion_op_counter =
    monitoring::Counter<1>::New("/machina/core/op_expansion/op_counter",
...
#include "machina/compiler/mlir/tfr/passes/generated_decompose.inc"
...
LogicalResult DecomposeTFOpsPass::RewriteUnregisteredTFOps() {
  ...
  func.walk([&table, &builder, &changed](Operation* op) {
    ...
    machina::IncreaseOpExpansionExecuteCounterByOne(
    ...
    for (auto arg : toolchain::enumerate(compose_func_type.getInputs())) {
      if (auto tensor_type = mlir::dyn_cast<TFRTensorType>(arg.value())) {
        ...
        toolchain::SmallVector<Value, 4> variadic_operands;
    ...
    for (auto res : toolchain::enumerate(compose_func_type.getResults())) {
    ...
    for (auto res : toolchain::zip(op->getResults(), new_results)) {
