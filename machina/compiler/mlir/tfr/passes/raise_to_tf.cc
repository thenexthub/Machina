...
#include "machina/compiler/mlir/tfr/utils/utils.h"
...
class RewriteTFRCallOp : public OpRewritePattern<CallOp> {
 ...
 private:
  ...
  LogicalResult AddDerivedAttrs(
      ...
      toolchain::StringMap<Attribute>* derived_attrs) const;
  ...
  LogicalResult DeriveOutputTypes(Location loc, FunctionType signature,
                                  const toolchain::StringMap<Attribute>& attrs,
  ...
  LogicalResult CreateAndReplaceOp(
      ...
      const toolchain::StringMap<Attribute>& derived_attrs) const;
  ...
  void CastValuesToSameType(PatternRewriter& rewriter, Location loc,
                            const toolchain::SmallVectorImpl<Attribute>& input_types,
                            toolchain::SmallVectorImpl<Value>& input_values) const {
  ...
  const toolchain::SmallDenseSet<StringRef, 4> fixed_elt_type_attrs_{"i32_", "i64_",
                                                                "f32_", "i1_"};
};
...
LogicalResult RewriteTFRCallOp::CollectInputsAndAttributes(
  ...
  for (const auto& operand :
       toolchain::enumerate(signature.getFunctionType().getInputs())) {
    ...
    if (auto list_op = dyn_cast_or_null<BuildListOp>(input_op)) {
      ...
      toolchain::SmallVector<Attribute, 4> list_input_types;
      toolchain::SmallVector<Value, 4> list_inputs;
...
LogicalResult RewriteTFRCallOp::DeriveOutputTypes(
    ...
    const toolchain::StringMap<Attribute>& attrs,
    SmallVectorImpl<Type>* output_types) const {
  for (auto res : toolchain::enumerate(signature.getResults())) {
...
LogicalResult RewriteTFRCallOp::CreateAndReplaceOp(
  ...
  for (auto res : toolchain::enumerate(call_op.getResultTypes())) {
...
LogicalResult RewriteTFRCallOp::matchAndRewrite(
  ...
  toolchain::StringMap<Attribute> derived_attrs;
