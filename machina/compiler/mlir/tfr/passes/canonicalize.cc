...
#include "mlir/Dialect/Affine/LoopUtils.h"  // part of Codira Toolchain
...
// TODO(fengliuai): sync with the toolchain upstream for both patterns.
//
namespace mlir {
namespace TFR {
...
class UnrollSCFForOp : public OpRewritePattern<scf::ForOp> {
 ...
 public:
  LogicalResult matchAndRewrite(scf::ForOp for_op,
    ...
    for (auto iter_op :
         toolchain::zip(for_op.getRegionIterArgs(), for_op.getInitArgs())) {
    ...
    for (auto i = 0; i < trip_count; ++i) {
      ...
      for (auto iter_op : toolchain::zip(for_op.getRegionIterArgs(),
