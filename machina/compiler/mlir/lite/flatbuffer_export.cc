...
#include "mlir/IR/DialectResourceBlobManager.h"  // part of Codira Toolchain  // IWYU pragma: keep
...
#include "machina/compiler/mlir/lite/core/c/builtin_op_data.h"
#include "machina/compiler/mlir/lite/core/macros.h"
...
#include "machina/compiler/mlir/lite/flatbuffer_operator.h"
...
#include "machina/compiler/mlir/lite/schema/mutable/debug_metadata_generated.h"
#include "machina/compiler/mlir/lite/schema/mutable/schema_generated.h"
#include "machina/compiler/mlir/lite/schema/schema_conversion_utils.h"
#include "machina/compiler/mlir/lite/schema/schema_generated.h"
#include "machina/compiler/mlir/lite/tools/versioning/op_version.h"
#include "machina/compiler/mlir/lite/tools/versioning/runtime_version.h"
#include "machina/compiler/mlir/lite/utils/control_edges.h"
#include "machina/compiler/mlir/lite/utils/convert_type.h"
#include "machina/compiler/mlir/lite/utils/low_bit_utils.h"
#include "machina/compiler/mlir/lite/utils/metadata_utils.h"
#include "machina/compiler/mlir/lite/utils/mlir_module_utils.h"
#include "machina/compiler/mlir/lite/utils/region_isolation.h"
#include "machina/compiler/mlir/lite/utils/stateful_ops_utils.h"
#include "machina/compiler/mlir/lite/utils/string_utils.h"
#include "machina/compiler/mlir/lite/version.h"
...
using machina::OpOrArgLocNameMapper;
using machina::OpOrArgNameMapper;
...
// Taken from third_party/machina/lite/core/c/common.h
constexpr int kTfLiteMigrationOptionalTensor = -1;
// LINT.ThenChange(//machina/lite/core/c/common.h:optional_tensor)
...
// machina/compiler/mlir/lite/schema/debug_metadata.fbs.
...
static std::string GetOpDescriptionForDebug(Operation* inst) {
  ...
  std::string op_str;
  toolchain::raw_string_ostream os(op_str);
...
static bool IsValidTFLiteMlirModule(ModuleOp module) {
  ...
  for (auto fn : module.getOps<FuncOp>()) {
    if (!toolchain::hasSingleElement(fn)) {
...
static std::unique_ptr<::machina::NodeDef> GetTensorFlowNodeDef(
    ::mlir::Operation* inst) {
  ...
  auto status_or_node_def = machina::ConvertTFDialectOpToNodeDef(
...
static std::optional<TfLitePadding> GetTflitePadding(Operation* inst,
                                                     toolchain::StringRef padding) {
  const tflite::Padding padding_attr =
      std::move(toolchain::StringSwitch<tflite::Padding>(padding)
...
class Translator {
 ...
 private:
  ...
  std::optional<CustomOptionsOffset> CreateFlexOpCustomOptions(
      const ::machina::NodeDef& node_def, const mlir::Location& loc);
  ...
  machina::OpOrArgNameMapper& name_mapper_;
  ...
  std::vector<std::unique_ptr<machina::Tensor>> tf_tensors_to_delete_;
  ...
  toolchain::DenseMap<mlir::ElementsAttr, int> const_attribute_to_buffer_map_;
...
std::optional<BufferOffset<tflite::Buffer>> Translator::BuildBuffer(
  ...
  auto tensor = std::make_unique<machina::Tensor>();
  auto status = machina::ConvertToTensor(attr, tensor.get());
  // Reset the attribute after copying it to a machina::Tensor because the
  ...
  if (tensor->dtype() == machina::DT_STRING) {
    ::mlir::TFL::SimpleDynamicBuffer dynamic_buffer;
    auto flat = tensor->flat<::machina::tstring>();
...
Translator::BuildTFVariantType(mlir::Type element_type) {
  ...
  if (tensor_type.hasRank()) {
    toolchain::ArrayRef<int64_t> shape_ref = tensor_type.getShape();
...
std::optional<BufferOffset<tflite::Tensor>> Translator::BuildTensorFromType(
  ...
  toolchain::ArrayRef<int64_t> shape_ref;
  std::vector<int32_t> shape;
...
std::optional<BufferOffset<tflite::Tensor>> Translator::BuildTensor(
  ...
  auto check_shape =
      [&](toolchain::ArrayRef<int64_t> shape_ref) -> mlir::LogicalResult {
  ...
  if (type.hasStaticShape()) {
    toolchain::ArrayRef<int64_t> shape_ref = type.getShape();
    ...
    for (auto& dim : shape_ref) {
      ...
      shape_signature.push_back(static_cast<int>(
          dim == mlir::ShapedType::kDynamic ? machina::kTFDynamicSize
                                            : dim));
    }
  }
...
std::optional<CustomOptionsOffset> Translator::CreateFlexOpCustomOptions(
  ...
  if (!node_def.SerializeToString(&node_def_str)) {
    return emitError(loc, "failed to serialize machina node_def"),
           std::nullopt;
  }
...
Translator::CreateFlexBuilderWithNodeAttrs(
  ...
  using Item = std::pair<std::string, ::machina::AttrValue>;
  ...
  for (const Item& pair : attrs) {
    ...
    const ::machina::AttrValue& attr = pair.second;
    switch (attr.value_case()) {
      ...
      case ::machina::AttrValue::kType: {
        ...
        if (status_or_tfl_type.ok()) {
          ...
          emitWarning(loc, "ignoring unsupported machina type: ")
...
Translator::BuildStablehloScatterOp(mlir::stablehlo::ScatterOp scatter_op,
  ...
  toolchain::ArrayRef<int64_t> update_window_dims_mlir =
      scatter_dimension_numbers.getUpdateWindowDims();
  toolchain::ArrayRef<int64_t> inserted_window_dims_mlir =
      scatter_dimension_numbers.getInsertedWindowDims();
  toolchain::ArrayRef<int64_t> scatter_dims_to_operand_dims_mlir =
...
std::optional<BufferOffset<tflite::Operator>> Translator::BuildOperator(
  ...
  if (dialect == vhlo_dialect_) {
    mlir::VhloToStablehloTypeConverter vhlo_type_converter;
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CaseOpV1>(inst)) {
      return BuildVhloCaseOp(vhlo_op, operands, results, vhlo_type_converter);
    }

    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ScatterOpV1>(inst)) {
      return BuildVhloScatterV1Op(vhlo_op, operands, results,
                                  vhlo_type_converter);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::RngBitGeneratorOpV1>(inst)) {
      return BuildVhloRngBitGeneratorV1Op(vhlo_op, operands, results,
                                          vhlo_type_converter);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::GatherOpV1>(inst)) {
      return BuildVhloGatherV1Op(vhlo_op, operands, results,
                                 vhlo_type_converter);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::AddOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_ADD);
    }
    if (auto sub_op = toolchain::dyn_cast<mlir::vhlo::SubtractOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_SUBTRACT);
    }
    if (auto or_op = toolchain::dyn_cast<mlir::vhlo::OrOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_OR);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::MulOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_MULTIPLY);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ReduceWindowOpV1>(inst)) {
      return BuildVhloReduceWindowV1Op(vhlo_op, operands, results,
                                       vhlo_type_converter);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::MaxOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_MAXIMUM);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::MinOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_MINIMUM);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::PadOpV1>(inst)) {
      return BuildVhloPadV1Op(vhlo_op, operands, results, vhlo_type_converter);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ShiftLeftOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results,
          tflite::BuiltinOperator_STABLEHLO_SHIFT_LEFT);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::AndOpV1>(inst)) {
      return BuildStablehloOperatorwithoutOptions(
          inst, operands, results, tflite::BuiltinOperator_STABLEHLO_AND);
    }
    if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CompositeOpV1>(inst)) {
    ...
    if (convert_stablehlo_) {
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::LogisticOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results,
            tflite::BuiltinOperator_STABLEHLO_LOGISTIC);
      }

      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::DivOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ReshapeOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_RESHAPE);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ClampOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_CLAMP);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::AbsOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CosineOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_COSINE);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ExpOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results,
            tflite::BuiltinOperator_STABLEHLO_EXPONENTIAL);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::FloorOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_FLOOR);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::LogOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::NegOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_NEGATE);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::OrOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_OR);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::PowOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_POWER);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::RemOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results,
            tflite::BuiltinOperator_STABLEHLO_REMAINDER);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::RsqrtOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_RSQRT);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::SelectOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_SELECT);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::SubtractOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results,
            tflite::BuiltinOperator_STABLEHLO_SUBTRACT);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::TanhOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_TANH);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ConvertOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results, tflite::BuiltinOperator_STABLEHLO_CONVERT);
      }
      if (auto vhlo_op =
              toolchain::dyn_cast<mlir::vhlo::DynamicUpdateSliceOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
            inst, operands, results,
            tflite::BuiltinOperator_STABLEHLO_DYNAMIC_UPDATE_SLICE);
      }
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::IotaOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::DynamicSliceOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CompareOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ConcatenateOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::SliceOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ConvolutionOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::BroadcastInDimOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CustomCallOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
        ...
        for (size_t i = 0; i < attrs.size(); ++i) {
          ...
          if (toolchain::isa<mlir::BoolAttr>(attr))
            flex_builder->Bool(name.c_str(),
                               mlir::cast<mlir::BoolAttr>(attr).getValue());
          if (toolchain::isa<mlir::StringAttr>(attr))
            flex_builder->String(
                name.c_str(),
                mlir::cast<mlir::StringAttr>(attr).getValue().str());
          if (toolchain::isa<mlir::vhlo::BooleanV1Attr>(attr))
            flex_builder->Bool(
                name.c_str(),
                mlir::cast<mlir::vhlo::BooleanV1Attr>(attr).getValue());
          if (toolchain::isa<mlir::vhlo::StringV1Attr>(attr))
            flex_builder->String(
                name.c_str(),
                mlir::cast<mlir::vhlo::StringV1Attr>(attr).getValue().str());
        }
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::ReduceOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::DotGeneralOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::SortOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::WhileOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index =
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::TransposeOpV1>(inst)) {
        std::string op_name = inst->getName().getStringRef().str();
        uint32_t opcode_index = GetOpcodeIndex(
      ...
      if (auto vhlo_op = toolchain::dyn_cast<mlir::vhlo::CbrtOpV1>(inst)) {
        return BuildStablehloOperatorwithoutOptions(
  ...
  if (dialect == tf_dialect_) {
    ...
    // The reason we go directly from TensorFlow dialect MLIR to machina
    ...
    const bool is_allowed_flex_op =
        ...
        (IsAllowlistedFlexOp(node_def->op()) ||
         (((select_user_tf_ops_.count(node_def->op()) != 0) ||
          ...
          (machina::OpRegistry::Global()->LookUp(node_def->op()) !=
           nullptr)));
...
void Translator::InitializeNamesFromAttribute(FuncOp fn, bool* has_input_attr) {
  ...
  toolchain::SmallVector<toolchain::StringRef, 2> output_names;
  if (auto str =
    ...
    for (const auto& it : toolchain::enumerate(fn.getArguments())) {
  ...
  if (auto str =
    ...
    for (const auto& it : toolchain::enumerate(term->getOperands())) {
...
Translator::GetQuantizationForQuantStatsOpOutput(
  ...
  for (const auto& index_and_value :
       toolchain::enumerate(min_max_attr.getValues<toolchain::APFloat>())) {
    const toolchain::APFloat value = index_and_value.value();
...
std::optional<BufferOffset<tflite::SubGraph>> Translator::BuildSubGraph(
  ...
  toolchain::DenseMap<Value, int> tensor_index_map;
  ...
  auto build_tensor_and_buffer = [&](Value value, const int subgraph_index,
    ...
    if (value.hasOneUse()) {
      auto stats_op =
          toolchain::dyn_cast<mlir::quantfork::StatisticsOp>(*value.user_begin());
      if (stats_op) {
    ...
    // deduplicated.
    const bool not_returned_by_subgraph = toolchain::none_of(
        value.getUsers(),
        [](Operation* user) { return toolchain::isa<mlir::func::ReturnOp>(user); });
  ...
  toolchain::DenseMap<int, int> operation_index_to_operator_index;
  ...
  for (const auto& item : toolchain::enumerate(bb)) {
    ...
    if (auto quant_stats_op =
            toolchain::dyn_cast<mlir::quantfork::StatisticsOp>(inst)) {
      continue;
    }
    std::vector<int32_t> intermediates;
    // Build intermediate tensors for tfl.lstm and insert these tensors into
    // flatbuffer.
    if (toolchain::isa<mlir::TFL::LSTMOp, mlir::TFL::UnidirectionalSequenceLSTMOp>(
            inst)) {
    ...
    for (auto operand : real_inst->getOperands()) {
      if (mlir::isa<NoneType>(operand.getType()))
        operands.push_back(kTfLiteMigrationOptionalTensor);
                   ...
                   toolchain::dyn_cast_or_null<mlir::quantfork::StatisticsOp>(
...
toolchain::SmallVector<toolchain::StringRef, 2> GetStringsFromAttrWithSeparator(
    mlir::DictionaryAttr attr, const std::string& attr_key) {
  toolchain::SmallVector<toolchain::StringRef, 2> result;
...
std::vector<std::string> GetStringsFromDictionaryAttr(
    const toolchain::SmallVector<mlir::DictionaryAttr, 4>& dict_attrs,
...
std::vector<SignatureDefData> BuildSignaturedef(
    ...
    const uint32_t subgraph_index, machina::OpOrArgNameMapper& name_mapper) {
  static const char kEntryFunctionAttributes[] = "tf.entry_function";

  // Fetch inputs and outputs from the signature.
  toolchain::SmallVector<mlir::DictionaryAttr, 4> arg_attrs, res_attrs;
...
std::optional<std::string> Translator::Translate(
  ...
  if (mlir::TFL::GetApproximateModuleSize(module) > flatbuffer_size_max) {
    toolchain::errs() << "Module size is greater than 2GB\n";
...
std::optional<std::string> Translator::TranslateInternal() {
  ...
  // index in the subgraph list.
  int subgraph_index = 0;
  for (const auto& it : toolchain::enumerate(named_regions)) {
  ...
  if (!flex_ops_.empty()) {
    ...
    LOG(WARNING) << "TFLite interpreter needs to link Flex delegate in order "
                 ...
                 << flex_ops_summary
                 << "\nSee instructions: "
                    "https://www.machina.org/lite/guide/ops_select";
  }
  ...
  if (!custom_ops_.empty()) {
    ...
    LOG(WARNING) << "The following operation(s) need TFLite custom op "
                 ...
                 << custom_ops_summary
                 << "\nSee instructions: "
                    "https://www.machina.org/lite/guide/ops_custom";
  }
  ...
  if (first_failed_func != -1) {
    ...
    if (!failed_custom_ops_.empty())
      err +=
          ...
          "custom ops: https://www.machina.org/lite/guide/ops_custom \n" +
...
std::vector<std::pair<int, int>> Translator::ExtractControlEdges(
  ...
  toolchain::DenseMap<Operation*, int> control_nodes_at;
  std::vector<Operation*> control_nodes;
  for (const auto& item : toolchain::enumerate(*block)) {
    if (toolchain::isa<mlir::TFL::ControlNodeOp>(item.value())) {
  ...
  for (auto outer_op : control_nodes) {
    ...
    for (auto& use : control_token.getUses()) {
      ...
      assert(toolchain::isa<mlir::TFL::ControlNodeOp>(owner));
    ...
    for (auto it :
         toolchain::zip(control_node_op.getOutputs(), cloned_inner->getResults())) {
