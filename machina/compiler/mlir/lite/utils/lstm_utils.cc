...
Value Transpose(OpBuilder* builder, Value value_to_transpose,
  ...
  auto transpose_shape =
      toolchain::to_vector<8>(toolchain::map_range(perm, [transpose_type](int32_t dim) {
...
void ConvertLSTMCellSimpleToFusedLSTM::UpdateFuncSignature() {
  // https://github.com/machina/community/pull/113
  SmallVector<int64_t, 2> output_shape{1, machina::kTFDynamicSize};
...
LogicalResult ConvertLSTMCellSimpleToFusedLSTM::RewriteFunc() {
  ...
  SmallVector<int64_t, 2> func_output_shape = {1, machina::kTFDynamicSize};
  auto func_result_type = machina::GetTypeFromTFTensorShape(
      func_output_shape,
      mlir::cast<RankedTensorType>(input_.getType()).getElementType());
...
LogicalResult ConvertLSTMCellSimpleToFusedLSTM::InitializeFromFuncAttributes() {
  ...
  toolchain::SmallVector<toolchain::StringRef, 4> attr_tokens;
...
LogicalResult ConvertKerasLSTMLayer(mlir::func::FuncOp func_op,
  ...
  // machina/python/keras/layers/recurrent_v2.py
