...
#ifndef MACHINA_COMPILER_MLIR_LITE_UTILS_UTILS_H_
#define MACHINA_COMPILER_MLIR_LITE_UTILS_UTILS_H_
...
inline DenseElementsAttr RemapPermutation(Value permutation1,
  ...
  return mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef(new_permutation));
}
...
inline bool IsTransposeNoop(Value permutation) {
  ...
  for (const auto& [idx, perm_value] :
       toolchain::enumerate(perm_values_attr.getValues<APInt>())) {
...
inline bool IsTransposeTrivial(toolchain::ArrayRef<int64_t> input_shape,
                               Value perm) {
  DenseElementsAttr perm_values_attr;
...
inline DenseElementsAttr GetPermutationFromTrivialReshape(
  ...
  return mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef(permutation));
}
...
inline mlir::ShapedType GetTransposedType(
    Value input, toolchain::ArrayRef<int64_t> permutation_array) {
  auto input_type = mlir::cast<ShapedType>(input.getType());
  if (permutation_array.size() != input_type.getRank()) {
    return nullptr;
  }
  toolchain::SmallVector<int64_t> transposed_shape(permutation_array.size());
...
inline DenseElementsAttr GetExpandedShapeAttr(Value input_val, int n) {
  ...
  return mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef(expanded_shape));
}
...
inline bool AreBroadcastAndReductionAxesIndependent(
  ...
  // Use toolchain::dyn_cast for safer casting.
  auto ranked_input_type =
      toolchain::dyn_cast<mlir::RankedTensorType>(input_val.getType());
  if (!ranked_input_type) {
    // Consider logging or error emission if builder context is
    // available/needed.
    return false;  // Expect ranked type.
  }
  toolchain::ArrayRef<int64_t> input_shape = ranked_input_type.getShape();
  const int64_t input_rank = ranked_input_type.getRank();

  // 2. Validate and extract reduction axes.
  // Use toolchain::dyn_cast for safer casting.
  auto indices = toolchain::dyn_cast<mlir::DenseElementsAttr>(indices_attr);
  ...
  auto target_shape_value_attr =
      toolchain::dyn_cast<mlir::DenseElementsAttr>(target_shape_attr);
  if (!target_shape_value_attr ||
      !target_shape_value_attr.getElementType().isIntOrIndex()) {
    return false;  // Invalid target shape attribute.
  }

  // Use toolchain::SmallVector for efficient shape storage.
  toolchain::SmallVector<int64_t, 4> target_shape_vec;
  ...
  // Use toolchain::ArrayRef for safe, non-owning view of the shape vector.
  toolchain::ArrayRef<int64_t> target_shape = target_shape_vec;
...
#endif  // MACHINA_COMPILER_MLIR_LITE_UTILS_UTILS_H_
