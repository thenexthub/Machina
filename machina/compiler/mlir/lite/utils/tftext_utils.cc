...
#include "machina/compiler/mlir/lite/utils/tftext_utils.h"
...
LogicalResult ConvertWhitespaceTokenizer(func::FuncOp func, toolchain::StringRef api,
                                         FuncAttr attr) {
  func.eraseBody();
  func.addEntryBlock();
...
LogicalResult ConvertNgrams(func::FuncOp func, toolchain::StringRef api,
                            FuncAttr attr) {
  func.eraseBody();
  func.addEntryBlock();
  func->setAttr(kTFImplements, attr);
...
LogicalResult ConvertSgnnProjection(func::FuncOp func, toolchain::StringRef api,
                                    FuncAttr attr) {
  // See more details in machina_models/sequence_projection/sgnn/sgnn.py
  func.eraseBody();
  func.addEntryBlock();
  func->setAttr(kTFImplements, attr);
  OpBuilder builder(func.getBody());
...
LogicalResult ConvertTFTextAPI(func::FuncOp func, toolchain::StringRef api,
                               FuncAttr attr) {
...
bool IsTFTextRegistered(const machina::OpRegistry* op_registery) {
