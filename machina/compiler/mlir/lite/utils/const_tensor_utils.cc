...
template <typename T>
toolchain::SmallVector<mlir::APInt> ReadAsHostEndian(ArrayRef<uint8_t> bytes) {
  toolchain::SmallVector<mlir::APInt> ret;
  ...
  for (int i = 0; i < elem_count; i++) {
    T val = toolchain::support::endian::readNext<T, toolchain::endianness::native,
                                            toolchain::support::unaligned>(data_ptr);
...
void truncateLimitedIntegerAPInt(toolchain::SmallVector<mlir::APInt>& values) {
  mlir::APInt min = values[0];
  mlir::APInt max = values[0];
  for (mlir::APInt& val : values) {
    min = toolchain::APIntOps::smin(val, min);
    max = toolchain::APIntOps::smax(val, max);
  }
...
StatusOr<QuantizedType> GetQuantizedType(const TensorT& tensor, Builder builder,
  ...
  toolchain::SmallVector<double> scales;
...
StatusOr<mlir::TensorType> GetTensorType(const TensorT& tensor, Builder builder,
  ...
  if (tensor.type == tflite::TensorType_VARIANT) {
    toolchain::SmallVector<mlir::TensorType> tensor_types;
    ...
    for (const auto& nested_tensor : tensor.variant_tensors) {
      ...
      if (nested_tensor->has_rank) {
        toolchain::SmallVector<int64_t> shape(nested_tensor->shape.begin(),
                                         nested_tensor->shape.end());
        tensor_types.push_back(
            machina::GetTypeFromTFTensorShape(shape, nested_elem_type));
      } else {
  ...
  if (!tensor.shape_signature.empty()) {
    toolchain::SmallVector<int64_t, 4> shape(tensor.shape_signature.begin(),
                                        tensor.shape_signature.end());
    return machina::GetTypeFromTFTensorShape(shape, elem_type);
  }

  if (!tensor.shape.empty()) {
    toolchain::SmallVector<int64_t, 4> shape(tensor.shape.begin(),
                                        tensor.shape.end());
    return machina::GetTypeFromTFTensorShape(shape, elem_type);
  }
...
mlir::ElementsAttr GetSplat(RankedTensorType type, int unique_index,
  ...
  if (auto qtype = mlir::dyn_cast<QuantizedType>(element_ty)) {
    mlir::RankedTensorType new_type = machina::GetTypeFromTFTensorShape(
...
StatusOr<mlir::ElementsAttr> ConvertIntBuffer(
  ...
  if (auto itype = mlir::dyn_cast<mlir::IntegerType>(elem_type)) {
    ...
    shaped_type = machina::GetTypeFromTFTensorShape(shaped_type.getShape(),
                                                       qtype.getStorageType());
  } else {
    return absl::InvalidArgumentError("unsupported integer constant type");
  }

  toolchain::SmallVector<mlir::APInt> values;
  switch (bit_width) {
    case 1: {
      // vector<bool> doesn't convert to an ArrayRef
      toolchain::SmallVector<bool, 8> boolValues;
  ...
  if (truncate) {
    ...
    shaped_type =
        machina::GetTypeFromTFTensorShape(shaped_type.getShape(), ety);
  }
  ...
  switch (elem_type.getIntOrFloatBitWidth()) {
    case 16: {
      ...
      if (elem_type.isF16()) {
        ...
        for (int i = 0; i < elem_count; i++) {
          uint16_t bit_repr = toolchain::support::endian::readNext<
              uint16_t, toolchain::endianness::native, toolchain::support::unaligned>(
              data);
    ...
    case 32: {
      ...
      for (int i = 0; i < elem_count; i++) {
        uint32_t bit_repr =
            toolchain::support::endian::readNext<uint32_t, toolchain::endianness::native,
    ...
    case 64: {
      ...
      for (int i = 0; i < elem_count; i++) {
        uint64_t bit_repr =
            toolchain::support::endian::readNext<uint64_t, toolchain::endianness::native,
                                            toolchain::support::unaligned>(data);
...
machina::TensorProto ConvertTfliteConstTensor(
    const TensorT& tensor, const std::vector<uint8_t>& buffer) {
  machina::TensorProto ret;
  ret.set_dtype(TflTypeToTfType(tensor.type));

  machina::TensorShapeProto* shape = ret.mutable_tensor_shape();
