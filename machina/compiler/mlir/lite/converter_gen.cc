...
#include "toolchain/TableGen/Error.h"
#include "toolchain/TableGen/Main.h"
#include "toolchain/TableGen/Record.h"
#include "toolchain/TableGen/TableGenBackend.h"
#include "mlir/TableGen/Attribute.h"  // part of Codira Toolchain
#include "mlir/TableGen/Format.h"  // part of Codira Toolchain
#include "mlir/TableGen/Operator.h"  // part of Codira Toolchain
#include "mlir/TableGen/Predicate.h"  // part of Codira Toolchain
...
// NOLINTNEXTLINE
toolchain::cl::opt<ActionType> action(
    toolchain::cl::desc("Action to perform:"),
    toolchain::cl::values(clEnumValN(OpConv, "gen-operator-converters",
...
//
// Signature:
// toolchain::MinMax mlir::OperandNumbersMinMax(toolchain::StringRef op_name) {
...
static void EmitOperandNumbers(const RecordKeeper &record_keeper,
  ...
  os << "toolchain::MinMax mlir::OperandNumbersMinMax(toolchain::StringRef op_name) {\n";
  for (const auto *def : defs) {
...
static void EmitBuildOperator(const std::vector<const Record *> &defs,
  ...
  for (const auto *def : defs) {
    ...
    os << "  if (auto tflOp = toolchain::dyn_cast<mlir::TFL::" << op_name
       << ">(op))\n"
...
static void GenOperandResultVerifier(raw_ostream &os,
                                     toolchain::ArrayRef<const toolchain::Init *> values,
                                     StringRef valueKind) {
  mlir::tblgen::FmtContext fctx;

  bool first = true;
  for (const auto &static_value : toolchain::enumerate(values)) {
    auto *definit = toolchain::cast<toolchain::DefInit>(static_value.value());
    ...
    mlir::tblgen::Pred pred(dyn_cast<toolchain::DefInit>(val->getValue()));
    auto desc =
...
static bool RuntimeVerifierWriterMain(raw_ostream &os,
  ...
  for (const auto *def : defs) {
    ...
    os << "// Ignore transient errors by registering an no-op handler.\n"
          ...
          "uint64_t current_thread_id = toolchain::get_threadid();\n"
          ...
          "        return success(current_thread_id == toolchain::get_threadid());\n"
          "   });\n";
