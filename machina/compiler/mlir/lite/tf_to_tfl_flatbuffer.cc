...
#include "machina/compiler/mlir/lite/tools/optimize/reduced_precision_metadata.h"
...
#include "machina/compiler/mlir/quantization/stablehlo/quantize_passes.h"
  ...
  if (result.wasInterrupted()) {
    mlir::TFL::AttachErrorCode(
        module.emitError(
            ...
            "instead. See https://www.machina.org/api_docs/python/tf/compat/"
...
mlir::LogicalResult GraphContainsStatefulPartitionedOp(mlir::ModuleOp module) {
  auto result = module.walk([&](Operation* op) {
    return toolchain::isa_and_nonnull<mlir::TF::StatefulPartitionedCallOp>(op)
