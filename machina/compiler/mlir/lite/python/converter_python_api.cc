 ...
 *
 */
#include "machina/compiler/mlir/lite/python/converter_python_api.h"
...
#include "machina/compiler/mlir/lite/python/flatbuffer_to_mlir.h"
#include "machina/compiler/mlir/lite/python/graphdef_to_tfl_flatbuffer.h"
#include "machina/compiler/mlir/lite/python/interpreter_wrapper/python_error_reporter.h"
#include "machina/compiler/mlir/lite/python/interpreter_wrapper/python_utils.h"
#include "machina/compiler/mlir/lite/python/jax_to_tfl_flatbuffer.h"
#include "machina/compiler/mlir/lite/python/saved_model_to_tfl_flatbuffer.h"
...
#include "machina/compiler/mlir/lite/sparsity/sparsify_model.h"
...
PyObject* Convert(PyObject* model_flags_proto_txt_raw,
                  ...
                  const machina::quantization::PyFunctionLibrary*
  ...
  if (model_flags.use_hlo_import()) {
    ...
    status = machina::ConvertGraphDefToTFLiteFlatBuffer(
...
PyObject* MlirQuantizeModel(PyObject* data, bool disable_per_channel,
  ...
  std::optional<machina::converter::DebugOptions> debug_options =
      machina::converter::DebugOptions();
...
PyObject* RegisterCustomOpdefs(PyObject* list) {
  ...
  for (int i = 0; i < size; ++i) {
    ...
    machina::OpDef opdef;
    if (!machina::protobuf::TextFormat::ParseFromString(tf_opdefs, &opdef)) {
      PyErr_Format(
          PyExc_ValueError,
    ...
    machina::OpRegistry::Global()->Register(
        [opdef](machina::OpRegistrationData* op_reg_data) -> absl::Status {
          *op_reg_data = machina::OpRegistrationData(opdef);
...
std::string FlatBufferFileToMlir(const std::string& model,
  ...
  return ::machina::FlatBufferFileToMlir(model, input_is_filepath);
}
