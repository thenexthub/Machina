...
#include "toolchain/ADT/TypeSwitch.h"  // IWYU pragma: keep
...
#include "machina/compiler/mlir/lite/utils/arithmetic_count_util.h"
#include "machina/compiler/mlir/lite/utils/shape_and_size_utils.h"
...
Operation* getDefiningBroadcastArgsOp(Value operand) {
  ...
  if (!toolchain::dyn_cast_or_null<TF::BroadcastToOp>(defining_op) &&
      !toolchain::dyn_cast_or_null<TFL::BroadcastToOp>(defining_op)) {
    return nullptr;
  }
  ...
  if (!toolchain::dyn_cast_or_null<TF::BroadcastArgsOp>(parent_of_defining_op) &&
      !toolchain::dyn_cast_or_null<TFL::BroadcastArgsOp>(parent_of_defining_op)) {
    return nullptr;
  }
...
DenseElementsAttr GetSqueezedShape(Value value_tensor) {
  auto value_shape_type = toolchain::dyn_cast<ShapedType>(value_tensor.getType());
  ...
  return mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef(return_squeeze_shape));
}
...
DenseElementsAttr GetSqueezedPermutation(Value input_value,
  ...
  return mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef(squeezed_permutation));
}
...
Type InferReductionOpType(Value input, Value reduction_indices,
  ...
  toolchain::SmallVector<bool, 4> is_reduce_dim(rank, false);
...
#include "machina/compiler/mlir/lite/ir/tfl_canonicalize.inc"
...
class FlatIndHelper {
 public:
  ...
  toolchain::SmallVector<int64_t> GetShapedInd(int64_t flat_ind) const {
    toolchain::SmallVector<int64_t> result;
  ...
  int64_t GetFlatInd(toolchain::ArrayRef<int64_t> shaped_ind) const {
    int64_t result = 0;

    auto num_els = type_.getNumElements();
    for (auto [max_dim, ind_dim] : toolchain::zip(type_.getShape(), shaped_ind)) {
      num_els /= max_dim;
  ...
  static void AddOffset(toolchain::SmallVector<int64_t>& target,
                        const toolchain::SmallVector<int64_t>& offset) {
    for (auto [ind, val] : toolchain::enumerate(offset)) {
      target[ind] += val;
    }
  }
...
void TFLDialect::initialize() {
  addOperations<
#define GET_OP_LIST
#include "machina/compiler/mlir/lite/ir/tfl_ops.cc.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "machina/compiler/mlir/lite/ir/tfl_ops_attrdefs.cc.inc"
      >();
...
template <class DenseElementsT,
          ...
          class CalculationT = toolchain::function_ref<ResultT(ElementT, ElementT)>>
Attribute ConstFoldBinaryOpDenseDense(ShapedType result_type,
                                      DenseElementsAttr lhs,
                                      DenseElementsAttr rhs,
                                      const CalculationT& calculate) {
  auto type = toolchain::dyn_cast_or_null<ShapedType>(
...
Attribute ConstFoldBinaryOp(
    ...
    toolchain::function_ref<APFloat(APFloat, APFloat)> float_calculate,
    toolchain::function_ref<APInt(APInt, APInt)> int_calculate) {
...
Attribute ConstFoldUnaryOp(Type result_type, Attribute operand,
                           toolchain::function_ref<APFloat(APFloat)> calculate) {
...
void buildComparisonBinOp(Builder* builder, OperationState& result, Value lhs,
  ...
  if (auto shaped_type = mlir::dyn_cast<RankedTensorType>(result_type)) {
    ...
    result.types.push_back(machina::GetTypeFromTFTensorShape(
...
OpFoldResult AddOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::isa<quant::QuantizedType>(getType().getElementType())) {
...
OpFoldResult LogicalNotOp::fold(FoldAdaptor adaptor) {
  ...
  auto data = toolchain::dyn_cast_or_null<DenseIntElementsAttr>(adaptor.getLhs());
  if (!data) {
    return {};
  }
  ...
  return DenseIntElementsAttr::get(
      data.getType(),
      toolchain::to_vector(toolchain::map_range(data.getValues<bool>(), compute)));
}
...
LogicalResult VerifyConcatenationOpTypes(Operation* op,
  ...
  auto FormatLoc = [&result_dim_sizes_loc](int64_t dim) {
    ...
    return toolchain::formatv("operand #{0}", loc).str();
  };

  for (const auto& operand : toolchain::enumerate(operand_types)) {
...
bool IsConcatenationOpConstFoldable(ConcatenationOp op,
  ...
  return toolchain::all_of(operands, [](Attribute operand) {
...
LogicalResult CustomTfOp::inferReturnTypes(
  ...
  if (toolchain::isa<TF::FakeQuantWithMinMaxArgsOp, TF::FakeQuantWithMinMaxVarsOp,
...
OpFoldResult GatherOp::fold(GatherOp::FoldAdaptor adaptor) {
  ...
  toolchain::SmallVector<int64_t> result_shape;
  // batch dims:
  ...
  // machina/lite/kernels/internal/reference/gather.h
...
LogicalResult BatchMatMulOp::verify() {
  ...
  toolchain::SmallVector<int64_t, 4> result_batch_shape;
  toolchain::ArrayRef<int64_t> x_batches = x_shape.drop_back(2);
  toolchain::ArrayRef<int64_t> y_batches = y_shape.drop_back(2);
  ...
  // https://www.machina.org/api_docs/cc/class/machina/ops/batch-mat-mul
...
LogicalResult FullyConnectedOp::fold(FoldAdaptor adaptor,
  ...
  auto is_foldable = [](toolchain::ArrayRef<int64_t> shape) {
...
static LogicalResult ComputeConvWindowedOutputSize(
    ...
    int64_t stride, machina::Padding padding, int64_t* output_size) {
  int64_t pad_low;
  int64_t pad_high;
...
static void BuildGatherOp(OpBuilder* builder, OperationState& result,
  ...
  TFL::GatherOp::build(
      *builder, result,
      machina::GetTypeFromTFTensorShape(shape, params_type.getElementType()),
      params, indices, axis, batch_dims);
}
...
mlir::LogicalResult ScatterNdOp::verify() {
  ...
  if (matchPattern(shape, m_Constant(&shape_value))) {
    ...
    for (const auto& shape_it : toolchain::enumerate(shape_value)) {
...
OpFoldResult MulOp::fold(FoldAdaptor adaptor) {
  ...
  const bool is_quantized =
      toolchain::isa<quant::QuantizedType>(getType().getElementType());

  auto lhs = toolchain::dyn_cast_or_null<DenseElementsAttr>(adaptor.getLhs());
  auto rhs = toolchain::dyn_cast_or_null<DenseElementsAttr>(adaptor.getRhs());
  ...
  if (input_type.hasStaticShape() && alpha_type.hasStaticShape()) {
    ...
    for (int i = 0; i < alpha_type.getRank(); i++) {
      if (alpha_type.getDimSize(i) != input_type.getDimSize(i + 1) &&
        ...
        return op.emitOpError(
            toolchain::formatv("'alpha' is not broadcastable at dimension {0}.", i));
      }
    }
  }
...
struct ConvertShapeTo1D : public OpRewritePattern<ReshapeOp> {
  ...
  LogicalResult matchAndRewrite(ReshapeOp reshape,
    ...
    auto new_shape = shape.reshape(machina::GetTypeFromTFTensorShape(
...
OpFoldResult ReshapeOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto dense_elements =
    ...
    if (!result_type.hasStaticShape()) {
      ...
      result_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult GetReshapeOutputType(Value input, Value shape,
  ...
  if (unknown_index != -1) {
    ...
    if (!input_ty_zero_dim && shape_ty_size * missing_dim != input_ty_size)
      return error_handler(
          toolchain::formatv("requires 'input' number of elements be a multiple of "
                        "{0}, but got {1}",
...
LogicalResult ReshapeOp::inferReturnTypes(
  ...
  auto error_handler = [&](const toolchain::Twine& message) -> LogicalResult {
...
struct RemoveRedundantUnpackPack : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation* op,
    ...
    for (auto input_output :
         toolchain::zip(pack_op.getOperands(), input_unpack_op.getResults())) {
...
toolchain::SmallVector<int64_t> UnpackIndexOperand(Attribute operand) {
  ...
  if (data.isSplat()) {
    ...
    return toolchain::SmallVector<int64_t>(data.getShapedType().getNumElements(),
                                      splat_val);
  }

  if (!is_i32) {
    return toolchain::to_vector(data.getValues<int64_t>());
  }

  return toolchain::to_vector(toolchain::map_range(data.getValues<int32_t>(), [](auto v) {
...
OpFoldResult SliceOp::fold(FoldAdaptor adaptor) {
  ...
  for (auto [begin, size, dim] :
       toolchain::zip(begin_vals, size_vals, input_type.getShape())) {
    if (size < 0) {
...
mlir::LogicalResult SliceOp::verify() {
  ...
  if (matchPattern(op.getBegin(), m_Constant(&begin))) {
    int axis = 0;
    for (const auto& begin_i : toolchain::enumerate(begin)) {
      if (begin_i.value().getSExtValue() < 0) {
        return op.emitError(
            toolchain::formatv("begin[{0}] cannot be negative", axis));
      }
      axis++;
    }
  }
  ...
  if (matchPattern(op.getSize(), m_Constant(&size))) {
    int axis = 0;
    for (const auto& size_i : toolchain::enumerate(size)) {
      if (size_i.value().getSExtValue() < -1) {
        return op.emitError(
            toolchain::formatv("size[{0}] cannot be negative other than -1", axis));
      }
      axis++;
    }
  }
...
TFL::ConstOp NarrowDownInt64InputValuesForOp(Operation* input_op,
  ...
  auto value_shape_type = machina::GetTypeFromTFTensorShape(
...
static void BuildTopKOp(OpBuilder* builder, OperationState& result, Value input,
  ...
  TFL::TopKV2Op::build(
      *builder, result,
      machina::GetTypeFromTFTensorShape(shape, val_type.getElementType()),
      machina::GetTypeFromTFTensorShape(shape, k.getType()), input, k);
}
...
LogicalResult UnpackOp::inferReturnTypes(
  ...
  auto output_shape = toolchain::to_vector<4>(input_type.getShape());
...
bool UnpackOp::isCompatibleReturnTypes(TypeRange lhs, TypeRange rhs) {
  ...
  for (auto pair : toolchain::zip(lhs, rhs)) {
    if (failed(
...
static RankedTensorType SubstituteRankedTensorTypeDimSize(
  ...
  shape[dim] = dim_size;
  return machina::GetTypeFromTFTensorShape(shape,
...
mlir::LogicalResult SplitVOp::verify() {
  ...
  if (size_splits_attr.getNumElements() != num_splits) {
    ...
    RankedTensorType expected_size_splits_type =
        machina::GetTypeFromTFTensorShape({num_splits},
  ...
  toolchain::SmallVector<int64_t, 4> size_splits;
...
OpFoldResult AbsOp::fold(FoldAdaptor adaptor) {
  ...
  auto compute = [](APFloat value) -> APFloat { return toolchain::abs(value); };
  return ConstFoldUnaryOp(result_type, operands[0], compute);
}
...
OpFoldResult NegOp::fold(FoldAdaptor adaptor) {
  ...
  auto compute = [](APFloat value) -> APFloat { return toolchain::neg(value); };
  return ConstFoldUnaryOp(result_type, operands[0], compute);
}
...
OpFoldResult RsqrtOp::fold(FoldAdaptor adaptor) {
  ...
  auto compute = [](APFloat value) -> APFloat {
    bool loseInfo;
    const toolchain::fltSemantics& original_float_semantics = value.getSemantics();
...
OpFoldResult ReluOp::fold(FoldAdaptor adaptor) {
  ...
  if (getType().getElementType().isSignlessInteger(32)) {
    return DenseIntElementsAttr::get(
        data.getType(),
        toolchain::map_to_vector(data.getValues<int32_t>(), ComputeRelu<int32_t>));
  }
  if (getType().getElementType().isF32()) {
    return DenseFPElementsAttr::get(
        data.getType(),
        toolchain::map_to_vector(data.getValues<float>(), ComputeRelu<float>));
  }
...
OpFoldResult MaximumOp::fold(FoldAdaptor adaptor) {
  ...
  if (lhs && rhs) {
    return ConstFoldBinaryOp(
        ...
        [](APFloat a, APFloat b) { return toolchain::maximum(a, b); },
...
OpFoldResult MinimumOp::fold(FoldAdaptor adaptor) {
  ...
  if (lhs && rhs) {
    return ConstFoldBinaryOp(
        getType(), adaptor.getOperands(),
        [](APFloat a, APFloat b) { return toolchain::minimum(a, b); },
...
OpFoldResult SelectOp::fold(FoldAdaptor adaptor) {
  ...
  auto condition_vals =
      toolchain::dyn_cast_or_null<DenseIntElementsAttr>(adaptor.getCondition());
  ...
  auto lhs_vals = toolchain::dyn_cast_or_null<DenseElementsAttr>(adaptor.getX());
  auto rhs_vals = toolchain::dyn_cast_or_null<DenseElementsAttr>(adaptor.getY());
...
OpFoldResult SumOp::fold(FoldAdaptor adaptor) {
  ...
  auto in_data = toolchain::cast<DenseFPElementsAttr>(input);
...
OpFoldResult CastFloatToInt(DenseFPElementsAttr data, FloatType in_type,
  ...
  auto cast = [&](APFloat value) -> APInt {
    ...
    bool is_exact;
    value.convertToInteger(result, toolchain::RoundingMode::TowardZero, &is_exact);
    return result;
  };
...
toolchain::SmallVector<OutType> MapStaticCast(DenseElementsAttr data) {
  return toolchain::map_to_vector(data.getValues<InType>(),
...
OpFoldResult CastOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto int_in_type = toolchain::dyn_cast_or_null<IntegerType>(in_type)) {
    auto in_data = toolchain::dyn_cast_or_null<DenseIntElementsAttr>(input);
    if (!in_data) {
      return {};
    }
    if (auto float_out_type = toolchain::dyn_cast_or_null<FloatType>(out_type)) {
      return CastIntToFloat(in_data, int_in_type, float_out_type);
    }
    if (auto int_out_type = toolchain::dyn_cast_or_null<IntegerType>(out_type)) {
      return CastIntToInt(in_data, int_in_type, int_out_type);
    }
  }

  if (auto float_in_type = toolchain::dyn_cast_or_null<FloatType>(in_type)) {
    auto in_data = toolchain::dyn_cast_or_null<DenseFPElementsAttr>(input);
    if (!in_data) {
      return {};
    }
    if (auto float_out_type = toolchain::dyn_cast_or_null<FloatType>(out_type)) {
...
static void BuildSelectV2Op(Builder* builder, OperationState& result,
  ...
  if (has_static_cond_shape && has_static_operand_shape) {
    result.types.push_back(
        machina::GetTypeFromTFTensorShape(broadcastedShape, elementType));
  } else {
...
int64_t TransposeConvOp::GetArithmeticCount(Operation* op) {
  int64_t count = -1;
  auto transpose_conv = toolchain::dyn_cast<TransposeConvOp>(op);
...
OpFoldResult TransposeOp::fold(FoldAdaptor adaptor) {
  ...
  // permutation.
  toolchain::OwningArrayRef<uint64_t> current_input_index(
      input_tensor.getType().getRank());

  // Allocate raw data and retrieve address of the first char in its raw
  // buffer.
  toolchain::OwningArrayRef<char> raw_output_arr(input_tensor.getRawData());
...
mlir::LogicalResult TransposeOp::verify() {
  ...
  for (const auto& axis_int : perm.getValues<APInt>()) {
    ...
    if (axis < 0 || (input_type.hasRank() && axis >= input_type.getRank())) {
      return op.emitOpError(
          toolchain::formatv("perm[{0}] must be in [-rank, rank)", index));
    }
    if (std::count(axes.begin(), axes.end(), axis) > 0) {
      return op.emitOpError(
          toolchain::formatv("perm[{0}] cannot have duplicated axis", index));
    }
    axes.push_back(axis);
    index++;
  }
  ...
  if (input_type.hasStaticShape() && output_type.hasStaticShape()) {
    toolchain::SmallVector<int64_t, 4> transposed_shape;
...
static void BuildTransposeOp(OpBuilder* builder, OperationState& result,
  ...
  TFL::TransposeOp::build(
      *builder, result,
      machina::GetTypeFromTFTensorShape(output_shape, element_type), input,
      perm);
}
...
LogicalResult WhileOp::verify() {
  ...
  for (auto it : toolchain::zip(op.getCond().front().getArgumentTypes(),
...
struct WhileResultOperandsMatchAndImplicitCapture
  ...
  LogicalResult matchAndRewrite(WhileOp while_op,
    ...
    for (auto ba : body_block.getArguments()) {
      ...
      if (mlir::isa<TF::ResourceType>(getElementTypeOrSelf(ba.getType()))) {
        ...
        for (auto user : ba.getUsers()) {
          ...
          if (!toolchain::isa<mlir::TF::ReadVariableOp>(user)) {
    ...
    toolchain::SmallVector<Type, 4> types;
...
int64_t AveragePool2DOp::GetArithmeticCount(Operation* op) {
  int64_t count;
  if (ArithmeticCountUtilHelper::GetFirstOutputCount(op, &count)) {
    auto avg_pool = toolchain::dyn_cast<AveragePool2DOp>(op);
...
int64_t MaxPool2DOp::GetArithmeticCount(Operation* op) {
  int64_t count;
  if (ArithmeticCountUtilHelper::GetFirstOutputCount(op, &count)) {
    auto max_pool = toolchain::dyn_cast<MaxPool2DOp>(op);
...
OpFoldResult ReverseV2Op::fold(FoldAdaptor adaptor) {
  ...
  auto axis_val = toolchain::cast<DenseIntElementsAttr>(axis);
  toolchain::SetVector<int32_t> axis_set(axis_val.value_begin<int32_t>(),
  ...
  auto input_data = toolchain::cast<DenseElementsAttr>(input);

  for (auto [i, val] : toolchain::enumerate(input_data.getValues<Attribute>())) {
...
YieldOp ControlNodeOp::GetYield() {
  return toolchain::cast<YieldOp>(GetBody().back());
}
...
mlir::LogicalResult ControlNodeOp::verify() {
  ...
  for (const int operand_idx : toolchain::seq<int>(0, yield.getNumOperands())) {
...
void ConstBytesAttr::print(mlir::AsmPrinter& printer) const {
  ...
  if (limit && limit.value() < bytes_str.size()) {
    ...
    printer << "0x" << toolchain::toHex(bytes_str);
  }
  printer << "\"";
}
...
#include "machina/compiler/mlir/lite/ir/tfl_ops_interface.cc.inc"
...
#include "machina/compiler/mlir/lite/ir/tfl_ops_dialect.cc.inc"
#include "machina/compiler/mlir/lite/ir/tfl_ops_enums.cc.inc"
...
#include "machina/compiler/mlir/lite/runtime_verifiers.inc"
