...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/conv.h"  // IWYU pragma: keep
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/custom_call.h"
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/dot_general.h"  // IWYU pragma: keep
...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/iota.h"
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/pad.h"
...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/sort.h"
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/util.h"  // IWYU pragma: keep
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/while.h"
...
arith::ConstantOp ExpandedShape(OpBuilder& b, Value input,
  ...
  toolchain::SmallVector<Attribute> expanded_shape(
      toolchain::cast<ShapedType>(output.getType()).getRank(),
      /*Value=*/b.getI32IntegerAttr(1));

  // Set dimension sizes specified by broadcast_dimensions.
  auto input_shape = toolchain::cast<ShapedType>(input.getType()).getShape();
...
Value ExpandedDynamicShape(OpBuilder& b, Value input,
  ...
  for (int64_t i : expanded_dimensions) {
    ...
    auto cur_type = toolchain::cast<ShapedType>(expanded_input.getType());
...
bool IsAbsOpLegal(mhlo::AbsOp op) {
  return !toolchain::cast<ShapedType>(op.getOperand().getType())
              .getElementType()
              .isIntOrFloat() &&
         !toolchain::isa<mlir::ComplexType>(
...
bool IsAddOpLegal(mhlo::AddOp op) {
  return toolchain::cast<ShapedType>(op.getOperand(0).getType())
      .getElementType()
...
void SetBinaryBitwiseLegal(ConversionTarget& target) {
  auto is_logical = [](Operation* op) {
    return toolchain::cast<ShapedType>(op->getResultTypes()[0])
        .getElementType()
        .isInteger(1);
  };
...
#include "machina/compiler/mlir/lite/stablehlo/transforms/generated_tflite_legalize_hlo.inc"
