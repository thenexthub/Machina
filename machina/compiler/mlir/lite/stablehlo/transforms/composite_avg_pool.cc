...
#include "machina/compiler/mlir/lite/stablehlo/transforms/composite_avg_pool.h"
...
#include "machina/compiler/mlir/lite/stablehlo/transforms/composite_utils.h"
...
DenseFPElementsAttr GetCorrectionMatrix(Builder& builder, CompositeOp op) {
  ...
  toolchain::SmallVector<int64_t, 4> nhwc_out_shape(4);
...
DenseIntElementsAttr GetPadOpAttr(Builder& builder, CompositeOp op) {
  ...
  toolchain::SmallVector<int32_t> padding_vec(8, 0);  // NHWC
  for (auto [ind, val] : toolchain::enumerate(values)) {
...
ShapedType GetPadOpType(CompositeOp op) {
  ...
  toolchain::SmallVector<int64_t> shape = {pool.n, h, w, pool.c};
