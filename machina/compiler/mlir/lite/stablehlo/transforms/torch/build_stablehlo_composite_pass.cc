...
#include "mlir/Analysis/TopologicalSortUtils.h"  // part of Codira Toolchain
...
static bool IsMarkTensorOp(mlir::Operation* op) {
  ...
  if (!toolchain::isa<mlir::stablehlo::CustomCallOp>(op)) {
    return false;
  }
  auto target_name =
...
struct BoundaryMetadata {
  ...
  static std::unique_ptr<BoundaryMetadata> Parse(toolchain::StringRef str_ref) {
    Json::Value root;
    Json::Reader reader;
 ...
 private:
  template <typename T>
  static bool CopyJsonValue(const Json::Value& json, toolchain::StringRef key,
...
class BuildStableHLOCompositePass
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<mlir::func::FuncOp> func_ops(
        module_op.getOps<mlir::func::FuncOp>());
    for (mlir::func::FuncOp& func_op : func_ops) {
      toolchain::DenseMap<const mlir::Operation*, size_t> op_order_map =
          BuildOpOrderMap(func_op);
      toolchain::SmallVector<toolchain::SmallVector<mlir::Operation*>>
 ...
 private:
  ...
  void EraseCompositeOp(mlir::stablehlo::CompositeOp composite_op) {
    ...
    auto dummy_op = builder.create<mlir::stablehlo::CustomCallOp>(
        ...
        toolchain::SmallVector<NamedAttribute>{
    ...
    for (auto [old_result, new_result] :
         toolchain::zip(composite_op.getResults(), dummy_op.getResults())) {
      old_result.replaceAllUsesWith(new_result);
    }
    composite_op.erase();
  }

  toolchain::DenseMap<const mlir::Operation*, size_t> BuildOpOrderMap(
      mlir::func::FuncOp func_op) const {
    toolchain::DenseMap<const mlir::Operation*, size_t> op_order_map;
    for (const auto& op : toolchain::enumerate(func_op.getOps())) {
  ...
  BuildBoundaryOutputOpsMap(mlir::func::FuncOp func_op) {
    std::unordered_map<std::string, toolchain::SmallVector<mlir::Operation*>>
        boundary_output_ops;
  ...
  mlir::FailureOr<mlir::Attribute> BuildAttrFromJson(
    ...
    switch (json_value.type()) {
      ...
      case Json::ValueType::arrayValue: {
        ...
        bool is_homogeneous = toolchain::all_of(json_value, [&](auto& el) {
        ...
        switch (head_type) {
          case Json::intValue: {
            toolchain::SmallVector<int64_t> int_values;
          ...
          case Json::realValue: {
            toolchain::SmallVector<float> float_values;
            for (const auto& json_value : json_value) {
          ...
          case Json::booleanValue: {
            toolchain::SmallVector<bool> bool_values;
            for (const auto& json_value : json_value) {
  ...
  mlir::FailureOr<mlir::DictionaryAttr> BuildDictionaryAttrFromJsonMap(
    ...
    toolchain::SmallVector<mlir::NamedAttribute> named_attrs;
  ...
  mlir::LogicalResult BuildStableHLOComposite(
      const toolchain::SmallVector<mlir::Operation*>& output_ops,
      toolchain::DenseMap<const mlir::Operation*, size_t>& op_order_map) {
    if (output_ops.empty()) {
      return mlir::success();
    }
  ...
  mlir::FailureOr<std::pair<toolchain::SmallVector<mlir::Value>,
                            toolchain::SmallVector<mlir::Operation*>>>
  GetBoundaryArgsAndOps(
      const toolchain::SmallVector<mlir::Operation*> boundary_output_ops,
      const BoundaryMetadata& metadata,
      const toolchain::DenseMap<const mlir::Operation*, size_t>& op_order_map) {
    toolchain::SetVector<mlir::Operation*> impl_ops_setvec;
    toolchain::SetVector<std::pair<mlir::Value, int64_t>> arg_pos_setvec;
    toolchain::SmallVector<mlir::Operation*> processing(boundary_output_ops.begin(),
    ...
    // order.
    toolchain::SmallVector<mlir::Operation*> impl_ops = impl_ops_setvec.takeVector();
  ...
  mlir::func::FuncOp BuildStableHLOCompositeImplFunc(
      const toolchain::SmallVector<mlir::Operation*> boundary_output_ops,
      toolchain::StringRef func_name, const toolchain::SmallVector<mlir::Value>& args,
      const toolchain::SmallVector<mlir::Operation*>& impl_ops) {
    mlir::ModuleOp module_op = getOperation();
    ...
    toolchain::SmallVector<mlir::Location> arg_locs;
    ...
    builder.createBlock(&impl_func.getBody(), impl_func.begin(), arg_types,
                        arg_locs);
    for (const auto& arg : toolchain::enumerate(args)) {
  ...
  mlir::FailureOr<mlir::Operation*> BuildStableHLOCompositeOp(
      ...
      const toolchain::SmallVector<mlir::Value>& args,
      const BoundaryMetadata& metadata) {
    mlir::MLIRContext* context = &getContext();
    mlir::OpBuilder builder(context);
