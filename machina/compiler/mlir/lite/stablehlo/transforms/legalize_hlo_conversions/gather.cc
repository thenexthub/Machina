 ...
 *
 */
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/gather.h"
...
LogicalResult LegalizeGatherToSlice::matchAndRewrite(
  ...
  for (int64_t i = 0; i < batch_size; ++i) {
    auto zero = BuildIntArrayConstOp<arith::ConstantOp>(
        builder, rewriter, toolchain::SmallVector<int64_t>({i, 0}),
        rewriter.getI32Type());
    auto two = BuildIntArrayConstOp<arith::ConstantOp>(
        builder, rewriter, toolchain::SmallVector<int64_t>({1, 2}),
        rewriter.getI32Type());
    ...
    auto squeeze_op = TFL::SqueezeOp::create(
        rewriter, gather_op.getLoc(),
        RankedTensorType::get({rank_two}, start_indices_type.getElementType()),
        begin, rewriter.getI64ArrayAttr(toolchain::ArrayRef<int64_t>({batch_dim})));
