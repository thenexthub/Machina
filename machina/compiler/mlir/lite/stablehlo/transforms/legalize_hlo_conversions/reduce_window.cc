...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/reduce_window_util.h"
...
bool AreDilationsSupported(const ReduceWindowView& op) {
  ...
  return toolchain::all_of(op.BaseDilations(), is_one) &&
         toolchain::all_of(op.WindowDilations(), is_one);
}
...
Layout TFLNativePoolingLayout(int64_t rank) {
  return Layout(0, rank - 1, toolchain::to_vector(toolchain::seq<int64_t>(1, rank - 1)));
}
...
toolchain::SmallVector<int64_t> Permute(toolchain::ArrayRef<int64_t> data,
                                   toolchain::ArrayRef<int64_t> perm) {
  toolchain::SmallVector<int64_t> res(data.size());
...
Value TransposeTensor(OpBuilder& b, Value tensor,
                      toolchain::SmallVector<int64_t> perm) {
...
std::optional<std::tuple<Value, Value>> GetInputAndInitIfValid(
  ...
  if (toolchain::dyn_cast<ShapedType>(init_val.getType()).getNumElements() != 1) {
    return std::nullopt;
  }
...
LogicalResult RelayoutReduceWindow::matchAndRewrite(
  ...
  toolchain::SmallVector<int64_t> perm_for_inputs =
  ...
  toolchain::SmallVector<int64_t> new_paddings(paddings.size() * 2);
  ...
  // permute window dims
  toolchain::SmallVector<int64_t> new_window_dims =
  ...
  toolchain::SmallVector<int64_t> new_window_strides =
  ...
  toolchain::SmallVector<int64_t> perm_for_outputs =
      target_layout.GetPermForReLayout(layout);
  auto cur_out_type = toolchain::dyn_cast<ShapedType>(op.getResult(0).getType());
  toolchain::SmallVector<int64_t> new_rw_out_shape =
...
LogicalResult LegalizeCumSum::matchAndRewrite(
  ...
  const bool trivial_window_dilate =
      toolchain::all_of(view.WindowDilations(), trivial);
  const bool trivial_base_dilate = toolchain::all_of(view.BaseDilations(), trivial);
  const bool trivial_stride = toolchain::all_of(view.WindowStrides(), trivial);
  ...
  for (const auto& [ind, dim_pad] : toolchain::enumerate(view.Paddings())) {
    if (dim_pad.Hi() != 0) {
...
TFL::PadV2Op LegalizeMaxPool::BuildExplicitPadOp(
  ...
  toolchain::SmallVector<int64_t, 8> padding_values;
  ...
  auto padding_dense_attr = mlir::DenseElementsAttr::get(
      ...
      toolchain::ArrayRef<int64_t>(padding_values));

  auto padding_values_op =
      rewriter.create<arith::ConstantOp>(op.getLoc(), padding_dense_attr);

  toolchain::SmallVector<int64_t, 4> pad_output_shape_vector;
...
LogicalResult LegalizeMaxPool::matchAndRewrite(
  ...
  auto input_type = toolchain::dyn_cast<ShapedType>(input.getType());
...
void ReplaceWithAvgPool(mhlo::DivOp op, Value rw_lhs_input,
                        ...
                        toolchain::StringRef padding, PatternRewriter& rewriter,
...
LogicalResult LegalizeAvgPool::matchAndRewrite(
  ...
  if (auto div_lhs_op = div_lhs.getDefiningOp()) {
    opt_final_tpose = toolchain::dyn_cast_or_null<mhlo::TransposeOp>(div_lhs_op);
  }

  auto rw_lhs_val = RecursivelyWalkUp<mhlo::TransposeOp>(div_lhs);
  auto rw_lhs =
      toolchain::dyn_cast_or_null<mhlo::ReduceWindowOp>(rw_lhs_val.getDefiningOp());
  if (!rw_lhs) {
    return rewriter.notifyMatchFailure(
  ...
  auto rw_lhs_input_type = toolchain::dyn_cast<ShapedType>(rw_lhs_input.getType());
