...
PermutationAndShape GetPermutationAndTransposedShape(
    toolchain::ArrayRef<int64_t> permutation_array, ShapedType input_type,
...
toolchain::SmallVector<int64_t> GetInversePermutationArray(
    toolchain::ArrayRef<int64_t> permutation_array) {
  toolchain::SmallVector<int64_t> inverse_permutation_array(
...
LogicalResult NormalizeIndexVector(Operation* parent_op, Value& indices,
  ...
  if (index_vector_dim == indices_type.getRank()) {
    toolchain::SmallVector<int64_t, 4> new_start_indices_shape(
...
std::pair<RankedTensorType, DenseIntElementsAttr> GetReformatTypeAndPermutation(
  ...
  toolchain::SmallVector<int64_t, 4> permutation_array(num_spatial_dims + 2);
  permutation_array[default_batch_dim] = batch_dim;
  permutation_array[default_feature_dim] = feature_dim;
  toolchain::SmallVector<int64_t, 4> transposed_shape(num_spatial_dims + 2);
