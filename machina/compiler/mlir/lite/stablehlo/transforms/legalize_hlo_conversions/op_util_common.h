 ...
 *
 */
#ifndef MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_OP_UTIL_COMMON_H_
#define MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_OP_UTIL_COMMON_H_
...
class Layout {
 public:
  toolchain::ArrayRef<int64_t> Spatials() const { return spatials_; }
  ...
  int64_t SpecialDim1(toolchain::ArrayRef<int64_t> shape) const {
  ...
  // special dimension from a shape.
  int64_t SpecialDim2(toolchain::ArrayRef<int64_t> shape) const {
  ...
  toolchain::SmallVector<int64_t, 4> GetPermForReLayout(
      const Layout& to_layout) const;

  // Permutes given shape based on the permutaion implied to take this Layout to
  // the given one.
  toolchain::SmallVector<int64_t, 4> PermuteShape(const Layout& to_layout,
 ...
 private:
  ...
  toolchain::SmallVector<int64_t> spatials_;
};
...
inline toolchain::SmallVector<int64_t> UnrollI64Splat(DenseElementsAttr data) {
  if (!data.isSplat()) {
    return toolchain::SmallVector<int64_t>(data.getValues<int64_t>());
  }
  return toolchain::SmallVector<int64_t>(data.getType().getNumElements(),
...
#endif  // MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_OP_UTIL_COMMON_H_
