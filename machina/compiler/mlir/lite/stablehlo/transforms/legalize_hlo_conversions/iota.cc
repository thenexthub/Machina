...
bool IsIotaLegal(mhlo::IotaOp op) {
  auto e_type = toolchain::cast<ShapedType>(op.getType()).getElementType();
...
LogicalResult LegalizeIota::matchAndRewrite(
  ...
  auto type = toolchain::cast<ShapedType>(op.getType());
  ...
  toolchain::SmallVector<int64_t> reshape_shape(type.getRank(), 1);
  ...
  reshape_shape_cst =
      TFL::CastOp::create(rewriter, op->getLoc(),
                          toolchain::cast<ShapedType>(reshape_shape_cst.getType())
