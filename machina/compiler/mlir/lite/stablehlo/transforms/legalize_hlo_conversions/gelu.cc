...
Operation* GetInputOpWithOneUse(Operation* op, int opr_num) {
  ...
  if (toolchain::isa<BlockArgument>(opr)) return nullptr;
...
bool HasSplatArg(Operation* op, float val, int opr_num) {
  ...
  auto cst_op = toolchain::dyn_cast_or_null<stablehlo::ConstantOp>(cst_input);
...
bool MatchERF(Operation* op) {
  if (auto custom_call = toolchain::dyn_cast_or_null<stablehlo::CustomCallOp>(op)) {
    return custom_call.getCallTargetName() == "mhlo.erf";
  }
  return toolchain::isa<chlo::ErfOp>(op);
}
...
LogicalResult LowerGELU::matchAndRewrite(Operation* op,
  ...
  auto rhs_mul = toolchain::dyn_cast_or_null<stablehlo::MulOp>(erf_input);
  if (!rhs_mul) return failure();

  auto lhs_mul = toolchain::dyn_cast_or_null<stablehlo::MulOp>(erf_user_user_input);
  if (!lhs_mul) return failure();

  auto output_mul = toolchain::dyn_cast_or_null<stablehlo::MulOp>(erf_user_user);
  if (!output_mul) return failure();

  auto rhs_add = toolchain::dyn_cast_or_null<stablehlo::AddOp>(erf_user);
