...
bool IsSupportedRfftOp(mhlo::FftOp fft_op) {
  const auto fft_type = toolchain::StringSwitch<std::optional<mhlo::FftType>>(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(mhlo::FftOp fft_op,
    ...
    const auto fft_lengths =
        toolchain::to_vector(fft_op.getFftLength().getValues<int64_t>());
    ...
    if (input_shape[input_shape.size() - 2] != 1) {
      ...
      auto expanded_input_type = machina::GetTypeFromTFTensorShape(
...
class LegalizeRfftOp : public OpConversionPattern<mhlo::FftOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<int32_t, 2> fft_lengths_i32{fft_lengths.begin(),
                                                  fft_lengths.end()};
