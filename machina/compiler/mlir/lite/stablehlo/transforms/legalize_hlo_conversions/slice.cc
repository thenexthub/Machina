...
Value PackScalarIndices(mlir::ValueRange indices, OpBuilder& b) {
  auto e_type =
      toolchain::cast<ShapedType>(indices.front().getType()).getElementType();
...
Value BuildTFLCastOp(OpBuilder& b, Value value) {
  return b.create<TFL::CastOp>(
      value.getLoc(),
      RankedTensorType::get(toolchain::cast<ShapedType>(value.getType()).getShape(),
                            b.getI32Type()),
      value);
}
...
LogicalResult CastSliceIndicesToSignless::matchAndRewrite(
  ...
  auto start_type =
      toolchain::cast<ShapedType>(op.getStartIndices().front().getType());
  ...
  toolchain::SmallVector<Value> casted_start_inds;
...
bool IsDynamicSliceLegal(mhlo::DynamicSliceOp op) {
  return !toolchain::cast<ShapedType>(op.getOperand().getType()).hasStaticShape();
}
...
LogicalResult LegalizeDynamicSliceOp::matchAndRewrite(
  ...
  auto input_type = toolchain::cast<ShapedType>(op.getOperand().getType());
  ...
  toolchain::SmallVector<Value> new_start_indices;
  const auto stride_sizes = UnrollI64Splat(op.getSliceSizes());

  for (auto [dim_size, start_ind_opr, stride_size] :
       toolchain::zip(input_type.getShape(), op.getStartIndices(), stride_sizes)) {
