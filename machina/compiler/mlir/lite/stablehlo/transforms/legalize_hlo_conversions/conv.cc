...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/conv_util.h"
...
bool IsShapeFullyStatic(ArrayRef<int64_t> shape) {
  return toolchain::all_of(shape, [](int64_t d) { return d >= 0; });
}
...
bool IsPaddingSupported(const ConvView& data) {
  return toolchain::all_of(data.Padding(), [](const DimPadding& p) {
    return p.Hi() == 0 && p.Lo() == 0;
  });
}

bool IsInputDilationSupported(const ConvView& data) {
  return toolchain::all_of(data.InputDilations(), [](int64_t v) { return v == 1; });
}
...
bool IsWindowReversalSupported(const ConvView& data) {
  return toolchain::all_of(data.WindowReversal(), [](bool b) { return !b; });
}
...
LogicalResult SliceDepthwiseTransposedConvolution::matchAndRewrite(
  ...
  auto create_slice = [&](mlir::Value tensor, int64_t depth_idx,
    ...
    toolchain::SmallVector<int64_t> start_indices(tensor_shape.size(), 0);
    auto limit_indices = tensor_shape;
    const toolchain::SmallVector<int64_t> strides(tensor_shape.size(), 1);
  ...
  toolchain::SmallVector<mlir::Value> conv_results;
...
arith::ConstantOp ShapeToConst(PatternRewriter& rewriter,
  ...
  auto casted_shape = toolchain::map_range(shape, [](auto i64) -> int32_t {
    return (i64 < 0) ? -1 : static_cast<int32_t>(i64);
  });
  auto attr =
      DenseIntElementsAttr::get(attr_type, toolchain::to_vector(casted_shape));
  return arith::ConstantOp::create(rewriter, loc, attr_type, attr);
}

std::tuple<toolchain::SmallVector<int64_t>, int64_t, Layout> InsertTrivialSpatialDim(
  ...
  toolchain::SmallVector<int64_t> new_spatials(layout.Spatials());
  const int64_t new_last_spatial = new_spatials.back() + 1;
  new_spatials.push_back(new_last_spatial);

  // Get new shape.
  toolchain::SmallVector<int64_t, 4> new_shape(shape.size() + 1, 1);
...
LogicalResult Conv1DToConv2D::matchAndRewrite(mhlo::ConvolutionOp op,
  ...
  // Window Strides
  toolchain::SmallVector<int64_t, 2> strides_2d;
