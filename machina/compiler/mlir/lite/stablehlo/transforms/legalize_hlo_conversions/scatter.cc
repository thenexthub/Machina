...
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/scatter.h"
...
LogicalResult CanonicalizeScatterUpdates(
    Operation* scatter_op, toolchain::ArrayRef<int64_t> update_window_dims,
  ...
  auto canonical_update_window_dims = toolchain::to_vector(
      toolchain::seq<int64_t>(indices_type.getRank() - 1, updates_type.getRank()));
...
LogicalResult ConvertScatterOp<BinaryOp, TfOp>::matchAndRewrite(
  ...
  for (int64_t i = 0; i < operand_type.getRank(); ++i) {
    if (!toolchain::is_contained(scatter_dims_to_operand_dims, i)) {
  ...
  if (std::is_same<TfOp, TF::TensorScatterUpdateOp>::value &&
    ...
    auto indices_shape = BuildIntArrayConstOp(
        builder, rewriter,
        toolchain::SmallVector<int64_t>({num_updates, index_depth}),
        rewriter.getI32Type());
    ...
    auto updates_shape = BuildIntArrayConstOp(
        builder, rewriter,
        toolchain::SmallVector<int64_t>({num_updates, updates_type.getDimSize(0)}),
        rewriter.getI32Type());
