...
bool MatchTopKComparator(Region& comparator) {
  ...
  auto compare_op =
      toolchain::dyn_cast_or_null<mhlo::CompareOp>(&operations.front());
  auto return_op = toolchain::dyn_cast_or_null<ReturnOpType>(&operations.back());
...
bool IsSortOpNotTopK(mhlo::SortOp op) {
  ...
  auto keys_type = toolchain::cast<ShapedType>(keys_opr.getType());
  ...
  auto indices_type = toolchain::cast<ShapedType>(indices_opr.getType());
...
LogicalResult LegalizeSortOp::matchAndRewrite(
  ...
  auto indices_type = toolchain::cast<ShapedType>(indices.getType());
