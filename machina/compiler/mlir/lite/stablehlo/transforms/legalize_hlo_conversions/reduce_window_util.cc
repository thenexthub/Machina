...
std::optional<Layout> ReduceWindowView::GuessLayout() const {
  auto zip_dims_strides = toolchain::zip(WindowDims(), WindowStrides());
  auto simple_window_dims =
      toolchain::to_vector(toolchain::map_range(zip_dims_strides, [](auto it) {
        return std::get<0>(it) == 1 && std::get<1>(it) == 1;
      }));

  if (toolchain::count(simple_window_dims, 1) < 2) {
    return std::nullopt;
  }
  ...
  if (is_channel_last) {
    return Layout(0, Rank() - 1,
                  toolchain::to_vector(toolchain::seq<int64_t>(1, Rank() - 1)));
  }

  const bool is_channel_first = simple_window_dims[0] && simple_window_dims[1];
  if (is_channel_first) {
    return Layout(0, 1, toolchain::to_vector(toolchain::seq<int64_t>(2, Rank())));
  }
