 ...
 *
 */
#ifndef MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_CONV_UTIL_H_
#define MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_CONV_UTIL_H_
...
class ConvView {
 public:
  ...
  toolchain::ArrayRef<int64_t> Strides() const { return strides_; }

  // 2d array for each spatial dim. Default 0.
  toolchain::ArrayRef<DimPadding> Padding() const { return padding_; }
  ...
  // int for each spatial dim. Default 1.
  toolchain::ArrayRef<int64_t> InputDilations() const { return input_dilations_; }

  // int for each spatial dim. Default 1.
  toolchain::ArrayRef<int64_t> KernelDilations() const { return kernel_dilations_; }

  // bool for each spatial dim. Default false.
  toolchain::ArrayRef<bool> WindowReversal() const { return window_reversal_; }

  toolchain::ArrayRef<int64_t> InputShape() const { return input_shape_; }

  const Layout& InputLayout() const { return input_layout_; }

  toolchain::ArrayRef<int64_t> KernelShape() const { return kernel_shape_; }

  const Layout& KernelLayout() const { return kernel_layout_; }

  toolchain::ArrayRef<int64_t> OutputShape() const { return output_shape_; }
 ...
 private:
  toolchain::SmallVector<int64_t, 2> strides_;

  toolchain::SmallVector<DimPadding, 2> padding_;

  toolchain::SmallVector<int64_t, 2> input_dilations_;
  toolchain::SmallVector<int64_t, 2> kernel_dilations_;

  toolchain::SmallVector<bool, 2> window_reversal_;

  Layout input_layout_;
  Layout kernel_layout_;
  Layout output_layout_;

  toolchain::SmallVector<int64_t, 4> input_shape_;
  toolchain::SmallVector<int64_t, 4> kernel_shape_;
  toolchain::SmallVector<int64_t, 4> output_shape_;
...
inline bool IsTrivialConv(const ConvView& data) {
  return toolchain::all_of(data.InputDilations(), [](auto d) { return d == 1; });
}
...
inline bool IsSupportedNonTrivialConv(const ConvView& data) {
  ...
  bool has_nagative_padding = toolchain::all_of(
      data.Padding(),
...
inline Layout GetTFLNativeInputOrOutputLayout(int64_t rank) {
  auto spatials = toolchain::to_vector(toolchain::seq<int64_t>(1, rank - 1));
...
inline Layout GetTFLNativeStandardConvKernelLayout(int64_t rank) {
  ...
  auto spatials = toolchain::to_vector(toolchain::seq(rank - 2));
...
#endif  // MACHINA_COMPILER_MLIR_LITE_STABLEHLO_TRANSFORMS_LEGALIZE_HLO_CONVERSIONS_CONV_UTIL_H_
