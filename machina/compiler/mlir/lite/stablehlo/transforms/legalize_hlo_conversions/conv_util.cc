...
toolchain::SmallVector<bool, 2> ResolveWindowReversal(
    const int64_t num_spatials,
    std::optional<mlir::DenseElementsAttr> opt_reversals) {
  if (!opt_reversals.has_value()) {
    return toolchain::SmallVector<bool, 2>(num_spatials, false);
  }
  auto reversals = opt_reversals.value();
  if (reversals.isSplat()) {
    return toolchain::SmallVector<bool, 2>(num_spatials,
                                      reversals.getSplatValue<bool>());
  }
  return toolchain::SmallVector<bool, 2>(reversals.getValues<bool>());
}
...
ConvView::ConvView(mhlo::ConvolutionOp op)
    : input_layout_(
      ...
      input_shape_(
          toolchain::SmallVector<int64_t, 4>(op.getLhs().getType().getShape())),
      kernel_shape_(
          toolchain::SmallVector<int64_t, 4>(op.getRhs().getType().getShape())),
      output_shape_(
          toolchain::SmallVector<int64_t, 4>(op.getResult().getType().getShape())),
...
Value CreatePadOpFromConvPadding(OpBuilder& b, mhlo::ConvolutionOp op) {
  ...
  toolchain::SmallVector<int64_t, 4> hi_padding(rank, 0);
  toolchain::SmallVector<int64_t, 4> lo_padding(rank, 0);

  for (const auto& [ind, dim_padding] : toolchain::enumerate(data.Padding())) {
  ...
  const toolchain::SmallVector<int64_t, 4> interior_padding(rank, 0);
...
bool MatchWithResizeBilinearOp(const ConvView& data, bool& align_corners) {
  ...
  if (data.InputDilations().size() != 2 ||
      !(toolchain::all_of(data.KernelDilations(), [](auto d) { return d == 1; })) ||
