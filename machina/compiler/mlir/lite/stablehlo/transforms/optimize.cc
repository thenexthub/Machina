...
LogicalResult RemoveReshapeAroundDotGeneral(mhlo::ReshapeOp reshape_after,
  ...
  if (reshape_after.getType().getShape() !=
      ArrayRef<int64_t>(toolchain::to_vector(
          toolchain::concat<const int64_t>(shape_b, shape_y1, shape_z)))) {
    return failure();
  }
...
LogicalResult LiftDotConcatLHS(mhlo::ConcatenateOp concat,
  ...
  const auto is_lhs_batch_or_contracting_dim = [&](uint64_t dim) {
    ...
    return toolchain::is_contained(dim_nums.getLhsBatchingDimensions(), dim) ||
           toolchain::is_contained(dim_nums.getLhsContractingDimensions(), dim);
  };
...
LogicalResult FuseSliceConcat(mhlo::ConcatenateOp concat,
  ...
  toolchain::append_range(new_concat_values, concat.getVal().drop_front(2));
...
LogicalResult ConvertReshapeDotRhsToBatchedDot(mhlo::DotGeneralOp dot,
  ...
  for (int i = 0; i < dot.getLhs().getType().getRank(); ++i) {
    if (!toolchain::is_contained(dim_nums.getLhsContractingDimensions(), i)) {
...
class SimplifyBroadcastInDimsReshape
  ...
  LogicalResult matchAndRewrite(mhlo::BroadcastInDimOp op,
    ...
    // same order.
    toolchain::SmallVector<int64_t> broadcast_dim_to_reshape_dim(
        broadcast_type.getRank());
    int64_t reshape_dim_idx = -1;
    for (auto [idx, dim] : toolchain::enumerate(broadcast_type.getShape())) {
      if (dim == 1) {
        continue;
      }
    ...
    toolchain::SmallVector<int64_t> current_broadcast_dims(
        op.getBroadcastDimensions().getValues<int64_t>());
    toolchain::SmallVector<int64_t> new_broadcast_dims;
    toolchain::SmallVector<int64_t> new_broadcast_input_shape;

    for (auto [idx, dim] : toolchain::enumerate(broadcast_input_type.getShape())) {
      if (dim == 1) {
        continue;
      }
