...
#include "mlir/Dialect/Quant/IR/Quant.h"  // part of Codira Toolchain  // NOLINT: Required to register quantization dialect.
...
TFL::QConstOp CreateTransposedTflConstOpForFilter(
  ...
  const auto filter_values =
      toolchain::cast<DenseIntElementsAttr>(filter_constant_op.getValue())
          .getValues<int8_t>();

  ArrayRef<int64_t> filter_shape =
      toolchain::cast<TensorType>(filter_constant_op.getType()).getShape();
  ...
  if (is_per_channel) {
    auto filter_quantized_type = toolchain::cast<quant::UniformQuantizedPerAxisType>(
    ...
    auto filter_quantized_type = toolchain::cast<quant::UniformQuantizedType>(
        GetElementType(filter_constant_op.getResult()));
...
TFL::QConstOp CreateTflConstOpForDummyBias(
  ...
  if (is_per_channel) {
    const auto filter_quantized_element_type =
        toolchain::cast<quant::UniformQuantizedPerAxisType>(
    ...
    const auto filter_quantized_element_type =
        toolchain::cast<quant::UniformQuantizedType>(
            GetElementType(filter_const_op.getResult()));
...
Type GetQuantizedOutputType(Operation* op, PatternRewriter& rewriter,
  ...
  auto result_quantized_type = toolchain::cast<quant::UniformQuantizedType>(
  ...
  rewriter.replaceAllUsesWith(uniform_quantize_op->getResult(0),
                              op->getResult(0));
  return toolchain::cast<TensorType>(op->getResult(0).getType())
...
LogicalResult MatchConvolutionFormat(stablehlo::ConvolutionOp op) {
  ...
  if (kernel_input_feature_dim != 2) {
    LLVM_DEBUG(toolchain::dbgs() << "Expected kernel input feature == 2. Got: "
  ...
  if (kernel_output_feature_dim != 3) {
    LLVM_DEBUG(toolchain::dbgs() << "Expected kernel output feature == 3. Got: "
                            << kernel_output_feature_dim << ".\n");
    return failure();
  }

  const auto input_type = toolchain::cast<TensorType>(op.getLhs().getType());
  if (input_type.getRank() != 4) {
    LLVM_DEBUG(toolchain::dbgs() << "Only 2D convolution op is supported. "
                               "Expected input rank of 4. Got: "
                            << input_type.getRank() << ".\n");
    return failure();
  }

  const auto filter_type = toolchain::cast<TensorType>(op.getRhs().getType());
  ...
  if (Operation* filter_op = op.getRhs().getDefiningOp();
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Filter should be a constant.\n");
    return failure();
  }
...
class RewriteUniformQuantizeOp
  ...
  // (https://github.com/machina/machina/blob/8f145d579aa0ee7f4187af32dbbf4e12fdabbffe/machina/lite/kernels/quantize.cc#L105).
  LogicalResult matchAndRewrite(stablehlo::UniformQuantizeOp op,
    ...
    if (!(toolchain::isa<FloatType>(input_element_type) ||
          IsI32F32UniformQuantizedType(input_element_type) ||
          IsI32F32UniformQuantizedPerAxisType(input_element_type))) {
      LLVM_DEBUG(toolchain::dbgs() << "Uniform quantize op's input should be a "
    ...
    const auto output_storage_type = toolchain::cast<IntegerType>(
        toolchain::cast<quant::QuantizedType>(GetElementType(op.getResult()))
            .getStorageType());
...
class RewriteUniformDequantizeOp
  ...
  // detailed limitations
  // (https://github.com/machina/machina/blob/8f145d579aa0ee7f4187af32dbbf4e12fdabbffe/machina/lite/kernels/dequantize.cc#L52).
  LogicalResult matchAndRewrite(stablehlo::UniformDequantizeOp op,
                                PatternRewriter& rewriter) const override {
    const auto input_storage_type = toolchain::cast<IntegerType>(
        toolchain::cast<quant::QuantizedType>(GetElementType(op.getOperand()))
            .getStorageType());
    ...
    const auto output_element_type =
        toolchain::cast<FloatType>(GetElementType(op.getResult()));
    if (!toolchain::isa<Float32Type>(output_element_type)) {
      LLVM_DEBUG(toolchain::dbgs() << "Uniform dequantize op's output element type "
                                 "should be f32. Got: "
                              << output_element_type << ".\n");
      return failure();
    }
 ...
 private:
  ...
  static LogicalResult MatchDotGeneralToTflBatchMatmulOp(
    ...
    if (num_lhs_batching_dims > 3) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match batching dimension for "
    ...
    if (num_lhs_contracting_dims != 1) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match contract dimension for "
                                 "quantized dot_general.\n");
      return failure();
    }
    const auto input_type = toolchain::cast<TensorType>(op.getLhs().getType());
    ...
    const int rhs_rank = toolchain::cast<TensorType>(filter_type).getRank();
  ...
  static LogicalResult MatchDotGeneralToTflFullyConnectedOp(
    ...
    if (!(input_type.getRank() == 2 || input_type.getRank() == 3)) {
      LLVM_DEBUG(toolchain::dbgs() << "Input expected to have rank of 2 or 3. Got: "
                              << input_type << ".\n");
      return failure();
    }

    const Value filter = op.getRhs();
    const auto filter_type = toolchain::cast<TensorType>(filter.getType());
    ...
    if (!IsI8F32UniformQuantizedPerAxisType(filter_type.getElementType())) {
      LLVM_DEBUG(toolchain::dbgs() << "Expected a per-channel uniform quantized "
    ...
    if (auto add_op = FindUserOfType<stablehlo::AddOp>(op);
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Expected a `stablehlo.constant` as the "
  ...
  static LogicalResult MatchInputDotGeneralCommonPattern(const Value input) {
    const auto input_type = toolchain::cast<TensorType>(input.getType());
    ...
    if (!input_type.hasRank()) {
      LLVM_DEBUG(toolchain::dbgs() << "Expected input_type to have rank.\n");
      return failure();
    }
    return success();
  }
  ...
  static LogicalResult MatchFilterCommonPattern(const Value filter) {
    auto filter_type = toolchain::cast<TensorType>(filter.getType());
    if (!filter_type.hasRank()) {
      LLVM_DEBUG(toolchain::dbgs() << "Expected rhs of dot_general has rank. Got: "
                              << filter.getType() << "\n");
      return failure();
    }
  ...
  static LogicalResult MatchOutput(const Value output,
    ...
    if (has_i32_output) {
      if (is_batch_matmul &&
        ...
        LLVM_DEBUG(toolchain::dbgs() << "Expected a per-tensor "
        ...
        LLVM_DEBUG(toolchain::dbgs() << "Expected a per-channel "
                                   "uniform quantized (i32->f32) type "
  ...
  static void RewriteDotGeneralToTflBatchMatmulOp(
    ...
    const int lhs_rank = toolchain::cast<TensorType>(lhs_value.getType()).getRank();
    const BoolAttr adj_x =
        (lhs_contracting_dims[0] == lhs_rank - 2 ? rewriter.getBoolAttr(true)
                                                 : rewriter.getBoolAttr(false));
    const int rhs_rank = toolchain::cast<TensorType>(rhs_value.getType()).getRank();
    ...
    if (filter_constant_op != nullptr) {
      const auto rhs_uniform_quantized_type =
          toolchain::cast<ShapedType>(rhs_value.getType());
  ...
  static void RewriteDotGeneralToTflFullyConnectedOp(
    ...
    const double input_scale =
        toolchain::cast<quant::UniformQuantizedType>(GetElementType(lhs_value))
            .getScale();
    ...
    if (has_i32_output) {
      ...
      if (fuse_bias_constant) {
        ...
        const SmallVector<double> bias_scales = GetBiasScales(
            /*input_scale=*/toolchain::cast<quant::UniformQuantizedType>(
                GetElementType(op->getOperand(0)))
                .getScale(),
            /*filter_scales=*/filter_quantized_type.getScales());
        const ArrayRef<int64_t> output_shape =
            toolchain::cast<TensorType>(op->getResult(0).getType()).getShape();
      ...
      output_type = toolchain::cast<TensorType>(op->getResult(0).getType())
                        .clone(new_result_quantized_type);
 ...
 private:
  LogicalResult match(stablehlo::ConvolutionOp op) const {
    ...
    if (fuse_bias_constant) {
      Operation* add_op = FindUserOfType<stablehlo::AddOp>(op);
      if (add_op == nullptr) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to find AddOp for bias fusion.\n");
        return failure();
      }
      Operation* bias_const_op = GetBiasConstOp(add_op);
      if (bias_const_op == nullptr) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to find bias constant.\n");
        return failure();
      }
    }
  ...
  static LogicalResult MatchInput(Value input) {
    auto input_type = toolchain::cast<TensorType>(input.getType());
    if (const auto input_element_type = input_type.getElementType();
        !IsI8F32UniformQuantizedType(input_element_type)) {
      LLVM_DEBUG(toolchain::dbgs()
                 << "Expected an i8->f32 uniform quantized type. Got: "
  ...
  static LogicalResult MatchFilter(Value filter) {
    ...
    if (toolchain::cast<quant::UniformQuantizedPerAxisType>(filter_element_type)
            .getQuantizedDimension() != 3) {
      LLVM_DEBUG(toolchain::dbgs() << "Quantized dimension should be 3. Got: "
                              << filter_element_type << "\n");
      return failure();
    }
    return success();
  ...
  TFL::PadOp CreateTflPadOp(Location loc,
    ...
    const auto input_tensor_type =
        toolchain::cast<RankedTensorType>(input_value.getType());
  ...
  bool IsSamePadding(
    ...
    // referenced from
    // https://www.machina.org/api_docs/python/tf/nn/convolution. The
  ...
  // (https://www.machina.org/api_docs/python/tf/nn).
  ...
  std::tuple<int64_t, int64_t, int64_t, int64_t> GetInOutDimensions(
    ...
    const auto [input_height, input_width] = GetDimSize(
        toolchain::cast<ShapedType>(op->getOperand(0).getType()).getShape(),
        dimension_numbers.getInputSpatialDimensions());
    const auto [output_height, output_width] = GetDimSize(
        toolchain::cast<ShapedType>(op->getResult(0).getType()).getShape(),
  ...
  TFL::QConstOp GetBiasOpAndUpdateQuantizedFilterConstant(
    ...
    // (https://github.com/machina/machina/blob/5430e5e238f868ce977df96ba89c9c1d31fbe8fa/machina/compiler/mlir/lite/ir/tfl_ops.td#L933).
...
class RewriteQuantizedTransposeOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::TransposeOp op,
    ...
    auto operand_type = toolchain::cast<TensorType>(op.getOperand().getType());
...
class RewriteQuantizedPadOp : public OpRewritePattern<stablehlo::PadOp> {
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::PadOp op,
    ...
    if (toolchain::any_of(op.getInteriorPadding(),
                     [](int64_t pad) { return pad != 0; })) {
      input = InsertDilateOp(op, rewriter);
    }

    TensorType operand_type = toolchain::cast<TensorType>(input.getType());
    const int64_t rank = operand_type.getRank();
    ...
    TensorType output_type = toolchain::cast<TensorType>(op.getResult().getType());
...
class RewriteQuantizedSliceOp : public OpRewritePattern<stablehlo::SliceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::SliceOp op,
    ...
    if (toolchain::all_of(strides, [](int64_t stride) { return stride == 1; })) {
...
class RewriteQuantizedBroadcastInDimOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::BroadcastInDimOp op,
    ...
    auto operand_type = toolchain::cast<TensorType>(op.getOperand().getType());
    auto output_type = toolchain::cast<TensorType>(op.getResult().getType());
    Value input = op.getOperand();

    // If broadcast_dimensions is not in ascending order, transpose first.
    if (!toolchain::is_sorted(op.getBroadcastDimensions())) {
  ...
  Value InsertTransposeOp(stablehlo::BroadcastInDimOp op,
    ...
    SmallVector<int64_t> sorted_dims =
        toolchain::to_vector(op.getBroadcastDimensions());
    toolchain::sort(sorted_dims);
    auto broadcast_dims = op.getBroadcastDimensions();
    SmallVector<int32_t> permutation(
        toolchain::map_range(broadcast_dims, [sorted_dims](int64_t dim) {
          return static_cast<int32_t>(toolchain::find(sorted_dims, dim) -
                                      sorted_dims.begin());
        }));
    auto operand_type = toolchain::cast<TensorType>(op.getOperand().getType());
  ...
  Value InsertExpandDimsOp(stablehlo::BroadcastInDimOp op,
    ...
    while (input_dims.size() < output_rank) {
      ...
      for (int32_t i = 0; i < output_rank; ++i) {
        if (!toolchain::is_contained(input_dims, i)) {
          dim_to_expand = i;
          break;
        }
      }
...
class RewriteQuantizedGatherOp : public OpRewritePattern<stablehlo::GatherOp> {
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::GatherOp op,
    ...
    auto output_tensor_type = toolchain::cast<TensorType>(output_type);
    ...
    for (const auto& [index, offset_dim] : toolchain::enumerate(offset_dims)) {
    ...
    for (int64_t i = 0; i < slice_sizes.size(); ++i) {
      ...
      if (!toolchain::is_contained(collapsed_slice_dims, i)) {
    ...
    const auto input_shape =
        toolchain::cast<TensorType>(op.getOperand().getType()).getShape();
    SmallVector<int64_t> input_offset_shape;
    for (int64_t i = 0; i < input_shape.size(); ++i) {
      if (!toolchain::is_contained(start_index_map, i)) {
    ...
    for (auto [slice_size, input_offset_size] :
         toolchain::zip_equal(slice_shape, input_offset_shape)) {
...
class RewriteQuantizedDynamicSliceOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::DynamicSliceOp op,
    ...
    for (auto [i, start_index] : toolchain::enumerate(op.getStartIndices())) {
    ...
    auto GetBroadcastedConstOp = [&](Value operand) -> TFL::QConstOp {
      if (auto broadcast_op = dyn_cast_or_null<stablehlo::BroadcastInDimOp>(
        ...
        auto const_uniform_quantized_type =
            toolchain::cast<ShapedType>(stablehlo_const_op.getResult().getType());
        return rewriter.create<TFL::QConstOp>(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::DotGeneralOp op,
    ...
    Type lhs_element_type =
        toolchain::cast<TensorType>(op.getLhs().getType()).getElementType();
...
class RewriteHybridQuantizedConvolutionOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::ConvolutionOp op,
    ...
    Type lhs_element_type =
        toolchain::cast<TensorType>(op.getOperand(0).getType()).getElementType();
 ...
 private:
  ...
  Type GetNewWeightQuantizedType(MLIRContext* context, Location location,
    ...
    auto tensor_type = toolchain::cast<TensorType>(filter_type);
    auto element_type = tensor_type.getElementType();
    RankedTensorType new_filter_result_type;
    if (toolchain::isa<UniformQuantizedPerAxisType>(element_type)) {
      auto per_axis_type =
          toolchain::cast<quant::UniformQuantizedPerAxisType>(element_type);
      int64_t kernel_output_feature_dim =
          GetConvolutionKernelOutputFeatureDimension(is_depthwise);
      auto new_filter_quantized_type = CreateI8F32UniformQuantizedPerAxisType(
      ...
      new_filter_result_type =
          RankedTensorType::getChecked(location,
                                       /*shape=*/new_shape,
                                       /*type=*/new_filter_quantized_type);
    } else if (toolchain::isa<UniformQuantizedType>(element_type)) {
      auto per_tensor_type =
          toolchain::cast<quant::UniformQuantizedType>(element_type);
      new_filter_result_type =
          RankedTensorType::getChecked(location,
                                       /*shape=*/new_shape,
