...
LogicalResult MatchZeroPointsOperand(Value zero_points) {
  if (!zero_points) {
    LLVM_DEBUG(toolchain::dbgs() << "Zero point value is empty.\n");
    return failure();
  }
  ...
  if (!zero_points_type) {
    LLVM_DEBUG(toolchain::dbgs() << "Zero point value should be a tensor type. Got: "
                            << zero_points_type << ".\n");
    return failure();
  }
  ...
  if (Type zero_points_element_type = zero_points_type.getElementType();
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Zero point should be an integer type. Got: "
...
LogicalResult MatchInverseScalesOperand(Value inverse_scales) {
  if (!inverse_scales) {
    LLVM_DEBUG(toolchain::dbgs() << "Inverse scales value is empty.\n");
    return failure();
  }
  ...
  if (!inverse_scales_type) {
    LLVM_DEBUG(toolchain::dbgs() << "Inverse scales should be a tensor type. Got: "
                            << inverse_scales_type << ".\n");
    return failure();
  }
 ...
 public:
  ...
  static FailureOr<UniformQuantizeFunctionCallPattern> Match(
    ...
    if (failed(MatchZeroPointsOperand(call_op.getOperand(2)))) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match the zero point operand of "
                                 "the @uniform_quantize call pattern.\n");
      return failure();
    }
 ...
 public:
  ...
  static FailureOr<UniformDequantizeFunctionCallPattern> Match(
    ...
    if (!call_op.getCallee().contains(
      ...
      toolchain::dbgs() << "Failed to match uniformDequantizeCallOp - name doesn't "
    ...
    Value input_value = call_op.getOperand(0);
    if (!input_value) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match @uniform_dequantize call "
 ...
 private:
  LogicalResult match(stablehlo::ConvolutionOp op) const {
    ...
    if (!input_i8_to_f32_convert_op) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match. Input is not defined by a "
                                 "stablehlo::ConvertOp.\n");
      return failure();
    }
    ...
    if (!uniform_quantize_call_op) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match: Input is not quantized by a "
    ...
    if (!subtract_op) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match subtract_op.\n");
      return failure();
    }
    if (!subtract_op->hasOneUse()) {
      toolchain::dbgs() << "Failed to match op - doesn't have a single use.\n";
      return failure();
    }
    ...
    if (!other_conv_op) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match other_conv_op.\n");
      return failure();
    }
    ...
    if (!isa<stablehlo::ConstantOp>(
        ...
        !isa<stablehlo::ConvertOp>(
            other_conv_op.getOperand(1).getDefiningOp())) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match filter of other_conv_op.\n");
      return failure();
    }
    ...
    if (!other_zp_reshape_op) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match other_zp_reshape_op.\n");
      return failure();
    }
    ...
    if (!isa<stablehlo::ConstantOp>(filter_op) &&
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match filter_constant_op.\n");
      return failure();
    }
...
class ComposeUniformQuantizedDotGeneralOp
 ...
 private:
  LogicalResult match(stablehlo::DotGeneralOp op) const {
    ...
    if (!IsI8ToF32Cast(*input_i8_to_f32_convert_op)) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match input_i8_to_f32_convert_op. "
    ...
    if (failed(output_uniform_quantize_call_pattern)) {
      toolchain::dbgs() << "Failed match uniform quantize call pattern.\n";
      return failure();
    }
    ...
    if (failed(output_uniform_dequantize_call_pattern)) {
      toolchain::dbgs() << "Failed to match output uniform quantize call pattern.\n";
      return failure();
    }
  ...
  int64_t GetFilterQuantizationDimension(
    ...
    auto seq_range = toolchain::seq(int64_t{0}, filter_rank);
 ...
 private:
  ...
  LogicalResult MatchFilter(Value filter_value) const {
    ...
    if (!IsI8ToF32Cast(*filter_i8_to_f32_convert_op)) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to match filter_i8_to_f32_convert_op. "
                                 "It should be a i8->f32 cast.\n");
      return failure();
    }
