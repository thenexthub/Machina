 ...
 *
 */
#include "machina/compiler/mlir/lite/stablehlo/transforms/tfl_stablehlo_pass.h"
...
static bool isDenseI64Array(toolchain::StringRef op_name,
                            toolchain::StringRef field_name) {
...
class TflToStablehloPass
 ...
 private:
  ...
  toolchain::SmallVector<mlir::NamedAttribute, 4> ReadAttr(const flexbuffers::Map& m,
                                                      Builder* builder,
                                                      std::string op_name) {
    ...
    for (size_t i = 0; i < keys.size(); ++i) {
      ...
      switch (value.GetType()) {
        ...
        case flexbuffers::FBT_VECTOR_BOOL: {
          toolchain::SmallVector<bool> vec;
          ...
          if (std::string{key} == "dimension_numbers") {
            ...
            auto conv_dimension_numbers_attr =
                mlir::stablehlo::ConvDimensionNumbersAttr::get(
                    ...
                    value_vec[1].AsInt64(), toolchain::ArrayRef<int64_t>(vec1),
                    value_vec[3].AsInt64(), value_vec[4].AsInt64(),
                    toolchain::ArrayRef<int64_t>(vec2), value_vec[6].AsInt64(),
                    value_vec[7].AsInt64(), toolchain::ArrayRef<int64_t>(vec3));
            auto named_attr =
...
void TflToStablehloPass::runOnOperation() {
  ...
  fn.walk([&](TFL::CustomOp custom_op) {
    ...
    toolchain::SmallVector<mlir::Type, 4> output_tys;
