...
#include "machina/compiler/mlir/lite/stablehlo/transforms/hlo_matchers.h"
#include "machina/compiler/mlir/lite/stablehlo/transforms/legalize_hlo_conversions/reduce.h"
...
#include "machina/core/lib/math/math_util.h"
...
void ReplaceReturnOp(Region& region, PatternRewriter& rewriter) {
  ...
  for (auto& block : region.getBlocks()) {
    ...
    auto return_op = toolchain::dyn_cast_or_null<mhlo::ReturnOp>(terminator);
...
class ConvertNdConvOp : public OpConversionPattern<mhlo::ConvolutionOp> {
 ...
 private:
  bool IsSamePadding(mhlo::ConvolutionOp conv_op, ArrayRef<int64_t> strides,
    ...
    for (auto i : toolchain::seq<int>(0, num_spatial_dims)) {
      ...
      absl::Status status = machina::GetWindowedOutputSizeVerbose(
          ...
          dilation[dim], strides[dim], machina::Padding::SAME, &output_size,
  ...
  Value SliceNegativePadding(Value value, ArrayRef<int64_t> explicit_padding,
    ...
    if (toolchain::all_of(explicit_padding, [](int64_t pad) { return pad >= 0; })) {
      return value;
    }
    ...
    toolchain::SmallVector<int64_t, 4> start;
    toolchain::SmallVector<int64_t, 4> size;
  ...
  void CreateConvOp(mhlo::ConvolutionOp conv_op, ArrayRef<int64_t> strides,
    ...
    auto new_padding =
        toolchain::to_vector<(num_spatial_dims * 2) + 4>(toolchain::map_range(
    ...
    if (is_depthwise_conv && num_spatial_dims == 2) {
      ...
      toolchain::ArrayRef<int64_t> hlo_filter_shape = filter_type.getShape();
      toolchain::SmallVector<int64_t, 4> tf_filter_shape(hlo_filter_shape.begin(),
...
LogicalResult IsSupportedNonTrivialConvOp(mhlo::ConvolutionOp conv_op,
  ...
  if (toolchain::any_of(conv_op.getPadding().value().getValues<int64_t>(),
...
class ConvertToResizeBilinearOpOrDepthwiseTransposedConvOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    bool align_corners;
    toolchain::SmallVector<int, 2> output_sizes;
 ...
 private:
  void CreateResizeBilinearOp(mhlo::ConvolutionOp conv_op,
                              toolchain::ArrayRef<int32_t> output_sizes,
                              bool align_corners,
                              ConversionPatternRewriter& rewriter) const {
  ...
  LogicalResult MatchToResizeBilinearOp(
    ...
    auto lhs_type = mlir::cast<ShapedType>(conv_op.getLhs().getType());
    toolchain::SmallVector<int> input_sizes = {
    ...
    auto can_convert_to_bilinear = [](bool align_corners, int64_t dilation,
      ...
      int64_t gcd =
          machina::MathUtil::GCD(static_cast<uint64_t>(input_spatial_size),
...
class ConvertDynamicSliceOp : public OpConversionPattern<mhlo::DynamicSliceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<Value, 4> start_indices_vector;
...
void Append(toolchain::SmallVectorImpl<ValueT>& values, Range&& range) {
...
toolchain::SmallVector<ValueT, 4> Concat(RangeTs&&... ranges) {
  toolchain::SmallVector<int64_t, 4> results;
...
class ConvertDynamicUpdateSliceOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<Value> start_indices_vector;
...
bool MatchTopKComparator(Region& comparator) {
  ...
  using OpListType = toolchain::iplist<Operation>;
 ...
 public:
  DotDimensionsInfo(ShapedType type, ArrayRef<int64_t> batch_dimensions,
    ...
    for (int64_t dim = 0; dim < rank; ++dim) {
      if (toolchain::count(contracting_dimensions_.axes, dim) > 0 ||
          toolchain::count(batch_dimensions_.axes, dim) > 0) {
        continue;
      }
...
Value BuildDotOperandFlattenedShapeOp(Value operand,
  ...
  toolchain::SmallVector<int32_t, 4> flattened_out_segids =
      toolchain::SmallVector<int32_t, 4>(operand_rank, static_cast<int32_t>(-1));
  for (int64_t i : dot_dimensions_info.out_dimensions().AxesArray()) {
    flattened_out_segids[i] = 0;
  }
  toolchain::SmallVector<int32_t, 4> flattened_contracting_segids =
      toolchain::SmallVector<int32_t, 4>(operand_rank, static_cast<int32_t>(-1));
  ...
  auto flattened_contracting_dims = builder.create<TF::UnsortedSegmentProdOp>(
      ...
      num_segids_tensor);
  toolchain::SmallVector<Value, 3> flattend_shape_values;
  ...
  if (!dot_dimensions_info.batch_dimensions().AxesArray().empty()) {
    if (ShapedType::isDynamicShape(
      ...
      toolchain::SmallVector<int32_t> batch_i32_vec;
      for (int64_t element :
           dot_dimensions_info.batch_dimensions().SizesArray()) {
...
Value ConvertDot(PatternRewriter& rewriter, Value lhs, Value rhs,
  ...
  toolchain::SmallVector<int64_t, 4> lhs_permutation = Concat<int64_t>(
  ...
  toolchain::SmallVector<int64_t, 4> lhs_transposed_shape = Concat<int64_t>(
  ...
  toolchain::SmallVector<int64_t, 4> rhs_permutation = Concat<int64_t>(
  ...
  toolchain::SmallVector<int64_t, 4> rhs_transposed_shape = Concat<int64_t>(
  ...
  toolchain::SmallVector<int64_t, 4> lhs_flattened_shape = Concat<int64_t>(
      lhs_dot_dimensions_info.batch_dimensions().SizesArray(),
      toolchain::ArrayRef<int64_t>{
  ...
  toolchain::SmallVector<int64_t, 4> rhs_flattened_shape = Concat<int64_t>(
      rhs_dot_dimensions_info.batch_dimensions().SizesArray(),
      toolchain::ArrayRef<int64_t>{
  ...
  toolchain::SmallVector<int64_t, 4> matmul_shape =
  ...
  toolchain::SmallVector<int64_t, 4> lhs_batch_and_out =
...
// Converts a mhlo.reduce op with a mlho binary operation into a machina
...
bool IsSpatialPoolingWithoutDilation(
    mhlo::ReduceWindowOp rw, toolchain::SmallVectorImpl<int64_t>* window_strides,
  ...
  toolchain::SmallVector<int64_t, 10> padding;
  ...
  if (toolchain::all_of(padding, [](int64_t i) { return i == 0; })) {
    *padding_mode = "VALID";
    return true;
  }
  ...
  for (uint64_t i = 1; i < rank - 1; ++i) {
    ...
    if (padding[2 * i] != machina::MathUtil::FloorOfRatio(
                              padding_size, static_cast<int64_t>(2)) ||
        padding[2 * i + 1] != machina::MathUtil::CeilOfRatio(
                                  padding_size, static_cast<int64_t>(2)))
      return false;
  }
...
class ConvertLoweredCumOp : public OpConversionPattern<mhlo::ReduceWindowOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto const_op = toolchain::dyn_cast_or_null<mhlo::ConstantOp>(
...
class ConvertAvgPoolOp : public OpConversionPattern<mhlo::DivOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<int64_t, 5> window_strides;
    ...
    if (matchPattern(div_rhs, m_Constant(&divisor))) {
      ...
      return replaceWithAvgPool(
          ...
          toolchain::to_vector<4>(rw.getWindowDimensions().getValues<int64_t>()),
 ...
 private:
  ...
  LogicalResult replaceWithAvgPool(mhlo::DivOp op, Value input,
                                   toolchain::ArrayRef<int64_t> ksizes,
                                   toolchain::ArrayRef<int64_t> kstrides,
                                   toolchain::StringRef padding,
                                   toolchain::StringRef data_format,
                                   ConversionPatternRewriter& rewriter) const {
    if (ksizes.size() == 4) {
  ...
  Value recursivelyWalkUp(Value op) const {
    while (toolchain::isa_and_nonnull<Tys...>(op.getDefiningOp())) {
...
bool TensorIsSign(PatternRewriter& rewriter, ElementsAttr float_or_int,
  ...
  auto sgn_splat = toolchain::dyn_cast<SplatElementsAttr>(sgn_cst);
  if (!sgn_splat) return false;

  auto splat = dyn_cast<SplatElementsAttr>(float_or_int);
  if (auto float_spl = toolchain::dyn_cast_if_present<FloatAttr>(splat),
      sgn_cst_spl = toolchain::dyn_cast_if_present<FloatAttr>(sgn_splat);
      float_spl && sgn_cst_spl) {
    return IsSign(float_spl.getValue(), sgn_cst_spl.getValue());
  }
  if (auto int_spl = toolchain::dyn_cast_if_present<IntegerAttr>(splat),
      sgn_cst_spl = toolchain::dyn_cast_if_present<IntegerAttr>(sgn_splat);
      int_spl && sgn_cst_spl) {
  ...
  if (mlir::isa<DenseFPElementsAttr>(float_or_int)) {
    ...
    return toolchain::all_of(float_or_int.getValues<APFloat>(), [&](APFloat value) {
  ...
  if (mlir::isa<DenseIntElementsAttr>(float_or_int)) {
    ...
    return toolchain::all_of(float_or_int.getValues<APInt>(), [&](APInt value) {
      return IsSign(value, sgn_splat_value);
    });
  }
  return false;
...
class ConvertGatherOp : public OpConversionPattern<mhlo::GatherOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::ArrayRef<int64_t> operand_batching_dims =
        gather_op.getDimensionNumbers().getOperandBatchingDims();
    toolchain::ArrayRef<int64_t> start_indices_batching_dims =
        gather_op.getDimensionNumbers().getStartIndicesBatchingDims();
    toolchain::ArrayRef<int64_t> start_index_map =
        gather_op.getDimensionNumbers().getStartIndexMap();
    toolchain::ArrayRef<int64_t> collapsed_slice_dims =
    ...
    if (!start_indices_type.hasStaticShape() || !result_type.hasStaticShape()) {
      ...
      for (auto c : collapsed_slice_dims) {
        if (toolchain::count(start_index_map, c) == 0) {
          return rewriter.notifyMatchFailure(
              gather_op,
    ...
    toolchain::SmallVector<int64_t> slice_sizes_vector;
    ...
    for (int i = 0; i < slice_sizes_vector.size(); ++i) {
      ...
      if (toolchain::count(operand_batching_dims, i)) {
        if (s != 1) {
          return rewriter.notifyMatchFailure(gather_op,
                                             "unsupported slice sizes");
        }
      } else if (toolchain::count(start_index_map, i) == 0) {
    ...
    toolchain::SmallVector<int64_t> canonical_result_shape;
  ...
  LogicalResult ConvertGatherOpToSlice(
    ...
    auto offset_dims = gather_op.getDimensionNumbers().getOffsetDims();
    auto slice_sizes = gather_op.getSliceSizes();
    toolchain::SmallVector<int64_t, 2> slice_sizes_vector;
    slice_sizes_vector.reserve(slice_sizes.size());
    for (int64_t s : slice_sizes.getValues<int64_t>()) {
      slice_sizes_vector.push_back(s);
    }

    toolchain::SmallVector<int64_t, 1> batch_dims;
    ...
    auto min_start_indices = BuildIntArrayConstOp(
        builder, rewriter, toolchain::SmallVector<int64_t>({0, 0}),
        start_indices_type.getElementType());
    ...
    if (batch_size == 1) {
      auto squeeze_op = rewriter.create<TF::SqueezeOp>(
          ...
          rewriter.getI64ArrayAttr(toolchain::ArrayRef<int64_t>({batch_dim})));
      auto slice_op =
 ...
 private:
  ...
  Value UncanonicalizeResult(mhlo::GatherOp gather_op, Value canonical_result,
    ...
    toolchain::SmallVector<int64_t> permutation_to_canonical;
    toolchain::SmallVector<int64_t> unflattened_shape;
    ...
    for (int64_t i = 0; i < batch_dims.size(); ++i) {
      if (toolchain::count(start_indices_batching_dims, i) == 0) {
        int64_t dim = batch_dims[i];
        permutation_to_canonical.push_back(dim);
        unflattened_shape.push_back(original_result_type.getDimSize(dim));
      }
    ...
    toolchain::SmallVector<int64_t> operand_dim_to_offset_dim_map(slice_sizes.size(),
                                                             -1);
    int offset_dim_index = 0;
    toolchain::SmallVector<int64_t> remaining_operand_dims;
    for (int64_t operand_dim = 0; operand_dim < slice_sizes.size();
         ++operand_dim) {
      if (toolchain::count(collapsed_slice_dims, operand_dim) ||
          toolchain::count(operand_batching_dims, operand_dim)) {
        continue;
      } else {
        if (toolchain::count(start_index_map, operand_dim) == 0) {
    ...
    for (int64_t s : start_index_map) {
      if (toolchain::count(collapsed_slice_dims, s) == 0) {
  ...
  Value CanonicalizeOperand(mhlo::GatherOp gather_op, Value operand,
                            ...
                            ConversionPatternRewriter& rewriter) const {
    int batch_size = 1;
    toolchain::SmallVector<int64_t> permutation;
    toolchain::SmallVector<int64_t> transposed_shape;
    toolchain::SmallVector<int64_t> flattened_shape;
    ...
    for (int64_t i = 0; i < operand_type.getRank(); i++) {
      if (toolchain::count(operand_batching_dims, i) == 0 &&
          toolchain::count(start_index_map, i) == 0) {
  ...
  Value CanonicalizeStartIndices(mhlo::GatherOp gather_op, Value start_indices,
    ...
    toolchain::SmallVector<int64_t> permutation;
    toolchain::SmallVector<int64_t> transposed_shape;
    toolchain::SmallVector<int64_t> reshaped_shape;
    ...
    for (int64_t dim = 0; dim < start_indices_type.getRank() - 1; dim++) {
      if (toolchain::count(start_indices_batching_dims, dim) == 0) {
        permutation.push_back(dim);
    ...
    // dimension 1.
    toolchain::SmallVector<int64_t> start_indices_shape = reshaped_shape;
    ...
    for (int i = 0; i < start_index_map.size(); ++i) {
      ...
      toolchain::SmallVector<int64_t> offsets_shape(start_indices_shape.size(), 1);
      ...
      toolchain::SmallVector<int64_t> padding_low(rank, 0);
      toolchain::SmallVector<int64_t> padding_high(rank, 0);
      toolchain::SmallVector<int64_t> padding_interior(rank, 0);
      padding_low.back() = i;
    ...
    if (!start_indices_batching_dims.empty()) {
      ...
      // operand.
      toolchain::SmallVector<int64_t> offsets_shape = start_indices_shape;
...
Value ConvertPadOp(PatternRewriter& rewriter, Operation* old_op) {
  ...
  toolchain::SmallVector<int64_t, 8> padding;
  toolchain::SmallVector<int64_t, 8> pad_output_shape;

  bool has_negative_padding_amount = false;
  toolchain::SmallVector<int64_t, 8> slice_begins;
  toolchain::SmallVector<int64_t, 8> slice_sizes;

  for (auto p : toolchain::zip(pad_op.getOperand().getType().getShape(),
...
class ConvertGetDimensionSizeOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    Value size =
        BuildIntArrayConstOp(builder, rewriter, toolchain::SmallVector<int64_t>({1}),
                             rewriter.getI32Type());
    ...
    Value squeeze_op = rewriter.create<TF::SqueezeOp>(
        ...
        rewriter.getI64ArrayAttr(toolchain::ArrayRef<int64_t>({0})));
...
bool IsWithinOpRegion(mlir::OpResult op_result) {
  ...
  if (toolchain::dyn_cast<ParentType>(parent_op)) {
    return true;
  }
  return false;
}
...
arith::ConstantOp ExpandedShape(PatternRewriter& rewriter, Value input,
  ...
  for (auto x : toolchain::enumerate(broadcast_dimensions)) {
...
Value ExpandedDynamicShape(PatternRewriter& rewriter, Value input,
  ...
  int64_t output_rank = mlir::cast<ShapedType>(output.getType()).getRank();
  toolchain::SmallVector<int64_t, 4> expanded_dimensions;
  toolchain::SmallSet<int64_t, 4> broadcast_dimensions_values;
  for (auto x : toolchain::enumerate(broadcast_dimensions)) {
...
#include "machina/compiler/mlir/lite/stablehlo/transforms/generated_legalize_hlo.inc"
