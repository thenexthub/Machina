...
class PrintOpStatsPass : public PassWrapper<PrintOpStatsPass, OperationPass<>> {
 public:
  ...
  explicit PrintOpStatsPass(raw_ostream *os = &toolchain::errs(),
  ...
  void CountOp(DenseMap<StringRef, toolchain::StringMap<int64_t>> &op_count_map,
               StringRef op_name, StringRef dtype);

 private:
  toolchain::StringMap<int64_t> op_with_dialect_count_;
  DenseMap<StringRef, toolchain::StringMap<int64_t>> op_dtype_count_;
  toolchain::StringMap<int64_t> dialect_count_;
  toolchain::StringMap<StringRef> dialect_name_of_;
  toolchain::StringMap<StringRef> op_name_of_;
...
void PrintOpStatsPass::CountOp(
    DenseMap<StringRef, toolchain::StringMap<int64_t>> &op_count_map,
    StringRef op_name, StringRef dtype) {
...
void PrintOpStatsPass::PrintSummary() {
  ...
  *os_ << "---------------------------------\n";
  ...
  toolchain::sort(sorted_op);
  toolchain::sort(sorted_dialect);
...
std::unique_ptr<mlir::Pass> mlir::odml::createPrintOpStatsPass(
    std::vector<std::string> accepted_dialects) {
  return std::make_unique<mlir::odml::PrintOpStatsPass>(&toolchain::errs(),
                                                        accepted_dialects);
}
