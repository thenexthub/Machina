...
#include "toolchain/Analysis/AssumeBundleQueries.h"
...
#include "machina/compiler/mlir/lite/offset_buffer.h"
...
#include "machina/compiler/mlir/lite/schema/schema_utils.h"
...
Location OpLoc(const OperatorT& op, Builder builder,
  ...
  if (debug_metadata.operator_location_map.contains(
    ...
    toolchain::SmallVector<Location, 4> locations;
    if (!op.outputs.empty()) {
...
mlir::Operation* ConvertMinMaxToStatsOp(const TensorT& tensor, OpBuilder b,
  ...
  if (!toolchain::isa<mlir::FloatType>(
          toolchain::cast<mlir::ShapedType>(res.getType()).getElementType())) {
    return nullptr;
  }
  ...
  toolchain::SmallVector<toolchain::APFloat, 4> min_maxs;
  min_maxs.reserve(mins.size() * 2);
  for (int i = 0, end = mins.size(); i < end; ++i) {
    toolchain::APFloat min(mins[i]);
    toolchain::APFloat max(maxs[i]);
  ...
  mlir::ElementsAttr layer_stats = mlir::DenseFPElementsAttr::get(
      machina::GetTypeFromTFTensorShape({2}, b.getF32Type()),
      {min_maxs[0], min_maxs[1]});
  mlir::ElementsAttr axis_stats;
  mlir::IntegerAttr axis;
  if (mins.size() > 1) {
    toolchain::SmallVector<int64_t, 4> axis_stats_shape{
        static_cast<int64_t>(mins.size()), 2};
    axis_stats = mlir::DenseFPElementsAttr::get(
        machina::GetTypeFromTFTensorShape(axis_stats_shape, b.getF32Type()),
        min_maxs);
...
StatusOr<Operation*> BuildExternalConstOp(const tflite::TensorT& tensor,
  ...
  TF_ASSIGN_OR_RETURN(mlir::TensorType type,
                      tfl::GetTensorType(tensor, builder,
                                         /*is_constant=*/true));
  auto shaped_type = toolchain::dyn_cast<mlir::RankedTensorType>(type);
  if (!shaped_type) {
...
static StatusOr<Operation*> BuildSparseConstOp(
  ...
  machina::TensorProto repr = tfl::ConvertTfliteConstTensor(tensor, buffer);
  repr.clear_tensor_shape();
  if (tfl::IsQuantized(tensor)) {
    repr.mutable_tensor_shape()->add_dim()->set_size(buffer.size());
    repr.set_dtype(machina::DT_INT8);
  } else {
  ...
  TF_ASSIGN_OR_RETURN(mlir::ElementsAttr compressed_data,
                      machina::ConvertTensorProto(repr, &builder));
...
StatusOr<Operation*> BuildConstOp(const tflite::TensorT& tensor,
  ...
  if (auto float_type = toolchain::dyn_cast<mlir::FloatType>(elem_type)) {
    TF_ASSIGN_OR_RETURN(value, tfl::ConvertFloatBuffer(shaped_type, buffer));
  } else if (toolchain::isa<mlir::IntegerType>(elem_type)) {
    TF_ASSIGN_OR_RETURN(value, tfl::ConvertIntBuffer(shaped_type, buffer));
  } else if (toolchain::isa<mlir::TF::StringType>(elem_type)) {
    machina::TensorProto repr =
        tfl::ConvertTfliteConstTensor(tensor, buffer);
    std::vector<toolchain::StringRef> refs;
  ...
  } else if (toolchain::isa<mlir::ComplexType, mlir::TF::TensorFlowType>(
                 elem_type)) {
    machina::TensorProto repr =
        tfl::ConvertTfliteConstTensor(tensor, buffer);
    std::string mangled = machina::mangling_util::MangleTensor(repr);
  ...
  if (use_stablehlo_constant) {
    ...
    toolchain::ArrayRef<char> val_ref(reinterpret_cast<const char*>(buffer.data()),
                                 buffer.size());
...
StatusOr<toolchain::SmallVector<mlir::NamedAttribute, 4>>
ConvertSubgraphIdxsToFunctionAttrs(tflite::BuiltinOptionsUnion options,
  ...
  if (auto* opts = options.AsCallOnceOptions()) {
    ...
    return toolchain::SmallVector<mlir::NamedAttribute, 4>{
...
Status AddOpIntermediatesForLstm(
  ...
  if (!op.intermediates.empty()) {
    ...
    const toolchain::SmallVector<toolchain::StringRef, 5> kIntermediateNames = {
    ...
    for (auto type_and_name :
         toolchain::zip(intermediate_types, kIntermediateNames)) {
...
StatusOr<Operation*> ConvertOp(
  ...
  toolchain::SmallVector<Value, 4> operands;
  toolchain::SmallVector<mlir::Type, 2> outputTypes;
  ...
  for (auto output_num : op.outputs) {
    ...
    if (op_name == "tfl.quantize") {
      ...
      auto shape_type = machina::GetTypeFromTFTensorShape(
  ...
  toolchain::MinMax input_min_max = mlir::OperandNumbersMinMax(op_name);
  ...
  if (input_max_num != 0 && input_max_num > op_input_num) {
    ...
    // with `none` value,
    toolchain::SmallVector<Value, 4> none_operands(
  ...
  if (op_name == "tfl.reshape") {
    ...
    if (matchPattern(op_state.operands[1], m_Constant(&shape_attr))) {
      auto shape_ty = toolchain::dyn_cast<mlir::RankedTensorType>(
          op_state.operands[1].getType());
      if (shape_ty != nullptr && shape_ty.hasRank() && shape_ty.getRank() > 1) {
        toolchain::SmallVector<mlir::Attribute, 4> shape;
        int32_t dim_size = 0;
        for (const auto& dim :
             toolchain::enumerate(shape_attr.getValues<toolchain::APInt>())) {
  ...
  toolchain::SmallVector<mlir::NamedAttribute, 2> attrs;
...
StatusOr<std::vector<int>> GetTensorIndices(
  ...
  for (const auto& index_and_tensor : toolchain::enumerate(subgraph.tensors)) {
...
mlir::NamedAttribute BuildTFEntryFunctionAttribute(
  ...
  auto tensor_names = toolchain::map_range(
      indices, [&](int i) { return subgraph.tensors.at(i)->name; });
  return builder->getNamedAttr(
      name, builder->getStringAttr(toolchain::join(tensor_names, ",")));
}
...
StatusOr<absl::flat_hash_set<const tflite::OperatorT*>> PruneSubgraph(
  ...
  for (const auto& op : subgraph.operators) {
    for (int32_t output : op->outputs) {
      if (!toolchain::is_contained(input_indices, output)) {
...
static StatusOr<FuncOp> PostProcessFuncOp(FuncOp func) {
  ...
  func.walk([&](tfl::QConstOp cst) {
    ...
    if (!qtype || qtype.getStorageTypeIntegralWidth() != 8 ||
        !(toolchain::isa<mlir::quant::UniformQuantizedType>(qtype) ||
          toolchain::isa<mlir::quant::UniformQuantizedPerAxisType>(qtype))) {
      return;
    }
    for (auto& use : value.getUses()) {
      ...
      auto affine_user =
          toolchain::dyn_cast<mlir::TFL::AffineQuantizedOpInterface>(user);
      if (affine_user &&
      ...
      if (full_range_const == value) {
        ...
        if (auto per_axis =
                toolchain::dyn_cast<mlir::quant::UniformQuantizedPerAxisType>(
                    qtype)) {
          new_qtype = mlir::quant::UniformQuantizedPerAxisType::get(
              ...
              per_axis.getStorageTypeMin() - 1, per_axis.getStorageTypeMax());
                       ...
                       toolchain::dyn_cast<mlir::quant::UniformQuantizedType>(
                           qtype)) {
...
int GetTensorIndex(const std::string& tensor_name,
                   toolchain::SmallVector<toolchain::StringRef, 2> tensors,
                   const std::set<int>& exclude_indices = {}) {
  for (const auto& tensor_index_pair : toolchain::enumerate(tensors)) {
...
toolchain::SmallVector<toolchain::StringRef, 2> GetStringsFromAttrWithSeparator(
  ...
  if (auto str =
          toolchain::dyn_cast_if_present<mlir::StringAttr>(attr.get(attr_key))) {
...
void SetSignature(
  ...
  for (const auto& input_pair : toolchain::enumerate(signature->inputs)) {
  ...
  for (const auto& output_pair : toolchain::enumerate(signature->outputs)) {
    const int arg_index =
...
using ControlNodes = toolchain::DenseMap<int, ControlNodeDesc>;
...
mlir::ResultRange MaybeWrapInControlNode(mlir::Operation* op,
  ...
  // populated.
  toolchain::SmallVector<Value, 2> control_tokens;
...
StatusOr<FuncOp> ConvertSubgraph(
    const tflite::SubGraphT& subgraph, toolchain::StringRef name,
    const std::vector<std::unique_ptr<tflite::OperatorCodeT>>& op_codes,
    const std::vector<std::string>& func_names,
  ...
  toolchain::SmallVector<mlir::Type, 2> ret_types;
  toolchain::SmallVector<mlir::Type, 4> input_types;
  ...
  toolchain::SmallVector<bool, 16> is_op_output(subgraph.tensors.size(), false);
  ...
  if (is_entry_point) {
    toolchain::SmallVector<mlir::NamedAttribute, 2> attributes;
  ...
  for (const auto& it : toolchain::enumerate(subgraph.operators)) {
    auto& op = it.value();
    ...
    for (const auto& pair : toolchain::enumerate(MaybeWrapInControlNode(
  ...
  toolchain::SmallVector<Value, 4> return_operands;
...
std::string SubgraphName(bool set_implicit_main_func, unsigned index,
  ...
  if (subgraph.name.empty()) {
    return toolchain::formatv("fn_{0}", index).str();
  }
...
void AddRegionsForTflWhileOp(mlir::ModuleOp module) {
  ...
  module.walk([&](mlir::TFL::WhileOp while_op) {
    auto cond = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(while_op->getAttr("cond"))
            .getValue());
    AddCallOpInWhileOpRegion(while_op.getCond(), cond);
    while_op->removeAttr("cond");
    auto body = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(while_op->getAttr("body"))
            .getValue());
...
void AddRegionsForStableHLOOp(mlir::ModuleOp module) {
  ...
  module.walk([&](mlir::vhlo::ReduceOpV1 reduce_op) {
    auto body = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(reduce_op->getAttr("body"))
            .getValue());
  ...
  module.walk([&](mlir::vhlo::ReduceWindowOpV1 reduce_window_op) {
    auto body = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(reduce_window_op->getAttr("body"))
            .getValue());
  ...
  module.walk([&](mlir::vhlo::ScatterOpV1 scatter_op) {
    auto update_computation = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(
  ...
  module.walk([&](mlir::vhlo::SortOpV1 sort_op) {
    auto comparator = symbol_table.lookup<mlir::func::FuncOp>(
        toolchain::cast<mlir::FlatSymbolRefAttr>(sort_op->getAttr("comparator"))
            .getValue());
...
OwningOpRef<mlir::ModuleOp> tflite::FlatBufferToMlir(
  ...
  toolchain::SmallVector<mlir::NamedAttribute> metadata_attrs;
  ...
  for (const auto& metadata : model->metadata) {
    ...
    metadata_attrs.emplace_back(
        ...
        builder.getStringAttr(toolchain::StringRef(
  ...
  for (const auto& e : toolchain::enumerate(model->subgraphs)) {
