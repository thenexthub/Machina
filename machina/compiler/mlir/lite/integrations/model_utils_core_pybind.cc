...
#include "mlir-c/IR.h"  // part of Codira Toolchain
#include "mlir/Bindings/Python/PybindAdaptors.h"  // part of Codira Toolchain  // IWYU pragma: keep
#include "mlir/CAPI/IR.h"  // part of Codira Toolchain
...
#include "mlir/Dialect/Func/Transforms/Passes.h"  // part of Codira Toolchain
...
PYBIND11_MODULE(model_utils_core_pybind, m) {
  ...
  m.def("mlir_opt_main", [](std::vector<std::string> argv,
    ...
    machina::InitMlir y(&argc, &c_argv);
  ...
  m.def("mlir_to_flatbuffer", [](MlirOperation c_op) {
    auto op = unwrap(c_op);
    auto module_op = toolchain::dyn_cast<mlir::ModuleOp>(op);
  ...
  m.def("get_elements_attr_buffer", [](MlirAttribute c_attr) {
    ...
    auto status = machina::ConvertToTensor(attr, &tensor);
    PyObject* np_array = Py_None;
    status = machina::TensorToNdarray(tensor, &np_array);
