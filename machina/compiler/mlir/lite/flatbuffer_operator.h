...
#ifndef MACHINA_COMPILER_MLIR_LITE_FLATBUFFER_OPERATOR_H_
#define MACHINA_COMPILER_MLIR_LITE_FLATBUFFER_OPERATOR_H_
...
toolchain::MinMax OperandNumbersMinMax(toolchain::StringRef op_name);
...
absl::Status CustomOptionsToAttributes(
    ...
    // NOLINTNEXTLINE
    Location loc, toolchain::SmallVectorImpl<mlir::NamedAttribute> *attributes);
...
inline std::vector<bool> GetVector(DenseElementsAttr elements) {
  ...
  if (elemType.isSignlessInteger(1)) {
    auto vec = toolchain::to_vector(
        toolchain::map_range(elements.getValues<bool>(),
...
inline std::vector<int8_t> GetVector(DenseElementsAttr elements) {
  ...
  if (elemType.isSignlessInteger(8)) {
    auto vec = toolchain::to_vector(toolchain::map_range(
...
#endif  // MACHINA_COMPILER_MLIR_LITE_FLATBUFFER_OPERATOR_H_
