...
class FallbackToFlexOps
 ...
 private:
  ...
  Option<std::string> mode_{*this, "mode",
                            toolchain::cl::desc("The mode of TF Quantization."),
                            toolchain::cl::init("")};
...
bool FallbackToFlexOps::ConvertToFlexOp(Operation *op) {
  ...
  if (!(*node_def)->SerializeToString(&node_def_str)) {
    op->emitError("Failed to serialize machina NodeDef");
    return false;
  }
...
#include "machina/compiler/mlir/lite/quantization/machina/fallback_to_flex_patterns.inc"
