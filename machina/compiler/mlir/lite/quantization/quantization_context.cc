...
#include "machina/compiler/mlir/lite/quantization/quantization_context.h"
...
QuantizeContext::QuantizeContext(func::FuncOp func, const DeviceTarget &spec)
  ...
  toolchain::DenseMap<Value, int> value_to_state;
...
LogicalResult QuantizeContext::Handle(
    quantfork::QuantizeRegionOp op,
    toolchain::SmallVectorImpl<Operation *> *new_items, bool *changed) {
...
LogicalResult QuantizeContext::Finalize() {
  ...
  func_.walk([&](quantfork::QuantizeRegionOp op) {
    toolchain::SmallVector<Attribute, 4> input_specs;
    ...
    toolchain::SmallVector<Attribute, 4> output_specs;
...
void QuantizeContext::DumpStates(quantfork::QuantizeRegionOp current_op) {
  ...
  func_.walk([&](quantfork::QuantizeRegionOp op) {
    if (current_op == op) toolchain::errs() << "===>>>";
    toolchain::errs() << op.getLogicalKernel() << " : (";
    for (auto i = 0; i < op.getNumOperands(); ++i) {
      if (auto params = GetOperandParams(op, i))
        params.print(toolchain::errs());
      else
        toolchain::errs() << "_";
      toolchain::errs() << ",";
    }
...
LogicalResult QuantizeContext::PropagateQuantParams(
  ...
  for (int i = 0, e = op->getNumOperands(); i != e; ++i) {
    auto ele =
        toolchain::cast<ShapedType>(op->getOperand(i).getType()).getElementType();
  ...
  for (int res = 0, e = op->getNumResults(); res != e; ++res) {
    auto ele =
        toolchain::cast<ShapedType>(op->getResult(res).getType()).getElementType();
...
void QuantizeContext::StatesManager::InitializeOperandState(
    ...
    toolchain::DenseMap<Value, int> *cache) {
