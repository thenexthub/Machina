...
#include "machina/compiler/mlir/lite/quantization/lite/test_util.h"
...
// Note: branched from machina/lite/tools/optimize/quantize_weights_test.cc

namespace {
machina::string* g_test_model_dir = nullptr;
}  // namespace
...
std::unique_ptr<FlatBufferModelAbslError> ReadTestModel() {
  auto model_path = machina::io::JoinPath(
...
int main(int argc, char** argv) {
  machina::string model_file;
  const std::vector<machina::Flag> flag_list = {
      machina::Flag("test_model_file", &model_file,
  ...
  g_test_model_dir =
      new machina::string(machina::io::Dirname(model_file));
