...
void TfLiteBuiltinOpToMlir(const BuiltinOperatorSet& tflite_builtin_codes,
  ...
  for (const auto& entry : tflite_builtin_codes) {
    ...
    std::string mlir_name = toolchain::Twine("tfl.", tflite_op_name.lower()).str();
...
absl::Status QuantizeWeights(
  ...
  if (quant_specs.inference_type == machina::DT_INT8)
    quant_specs.inference_type = machina::DT_QINT8;

  if (!(quant_specs.inference_type == machina::DT_HALF ||
        quant_specs.inference_type == machina::DT_QINT8)) {
  ...
  toolchain::dbgs() << "weight_quantization: " << true
  ...
  machina::AddDynamicRangeQuantizationPasses(
