...
using mlir::lite::toco_legacy::
    CustomOpMap;  // Use this instead of mlir::lite::CustomOpMap because that
                  ...
                  // machina/compiler/mlir/lite/quantization/lite/quantize_weights.h,
                  // and we need mlir::lite::toco_legacy::CustomOpInfo, in
                  // machina/compiler/mlir/lite/quantization/lite/optimize/quantize_weights.h
...
// Redefined from machina/lite/core/c/common.h as local const int instead of
...
std::vector<int32_t> GetWeightInputIndices(const OperatorCodeT* op_code,
  ...
  if (builtin_op_code == BuiltinOperator_CUSTOM) {
    ...
    // machina/lite/kernels/svdf.cc
    return {1, 2};
  } else if (builtin_op_code == BuiltinOperator_LSTM ||
             builtin_op_code == BuiltinOperator_UNIDIRECTIONAL_SEQUENCE_LSTM) {
    // machina/lite/kernels/lstm.cc
    // machina/lite/kernels/unidirectional_sequence_lstm.cc
    ...
    // machina/lite/kernels/basic_rnn.cc
    // machina/lite/kernels/unidirectional_sequence_rnn.cc
    return {1, 2};
  } else if (builtin_op_code == BuiltinOperator_BIDIRECTIONAL_SEQUENCE_LSTM) {
    // machina/lite/kernels/bidirectional_sequence_lstm.cc
    ...
    // machina/lite/kernels/bidirectional_sequence_rnn.cc
    return {1, 2, 4, 5, 6, 8, 9, 10, 11};
  } else if (builtin_op_code == BuiltinOperator_GATHER) {
    // machina/lite/kernels/gather.cc
    return {0};
  }
  return {};
}
