...
QuantizedType CalculateNewQuantizedType(
    toolchain::ArrayRef<UniformQuantizedType> qtypes) {
  if (qtypes.size() == 1) {
    return qtypes[0];
  }
...
bool QuantizationDriver::PropagateParamsAndReturnIfChanged() {
  ...
  while (!work_list_.empty()) {
    ...
    if (qdq_conversion_mode_ != QDQConversionMode::kQDQStrict &&
      ...
      auto concat = mlir::dyn_cast_or_null<TFL::ConcatenationOp>(op);
      toolchain::DenseMap<int, UniformQuantizedType> operand_qtypes;
      ...
      toolchain::DenseMap<int, UniformQuantizedType> result_qtypes;
      toolchain::SmallVector<Operation*> users(op->user_begin(), op->user_end());
      ...
      // point.
      toolchain::SmallVector<UniformQuantizedType> qtypes;
