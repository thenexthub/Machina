...
#ifndef MACHINA_COMPILER_MLIR_LITE_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
#define MACHINA_COMPILER_MLIR_LITE_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
...
struct ConvertStatsToQDQs : public OpRewritePattern<quantfork::StatisticsOp> {
  ...
  LogicalResult matchAndRewrite(quantfork::StatisticsOp op,
    ...
    if (op.getAxisStats().has_value()) {
      ...
      auto stats = toolchain::dyn_cast<DenseFPElementsAttr>(*op.getAxisStats());
...
class QuantizationPattern : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(mlir::Operation* op,
                                PatternRewriter& rewriter) const override {
    toolchain::SmallVector<mlir::Operation*, 4> quantizing_ops;
...
#endif  // MACHINA_COMPILER_MLIR_LITE_QUANTIZATION_COMMON_QUANTIZATION_LIB_QUANTIZATION_UTILS_H_
