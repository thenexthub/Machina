...
bool CheckTypeConstraints(const toolchain::Init *input_value,
                          std::list<std::string> required_types,
                          bool per_axis) {
  auto *def_init = toolchain::cast<toolchain::DefInit>(input_value);
  ...
  toolchain::StringRef supported_types =
...
void GenerateStaticQuantOp(std::vector<const Record *> &defs,
  ...
  for (const auto *def : defs) {
    ...
    const toolchain::DagInit *args_in_dag = def->getValueAsDag("arguments");
