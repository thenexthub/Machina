...
#include "machina/compiler/mlir/lite/quantization/device_target.h"
...
void DeviceTarget::AppendToSignature(Type spec,
  ...
  if (auto quant = toolchain::dyn_cast_or_null<UniformQuantizedType>(spec)) {
  ...
  } else if (auto any = toolchain::dyn_cast_or_null<AnyQuantizedType>(spec)) {
...
LogicalResult DeviceTarget::RegisterKernel(
    toolchain::StringRef kernel, const KernelSpecs::Signature& signature,
...
LogicalResult DeviceTarget::DecomposeMultiplyAccumulateScale(
  ...
  auto rop = toolchain::dyn_cast<quantfork::QuantizeRegionOp>(op);
  if (!rop) return failure();

  toolchain::SmallVector<Type, 4> input_specs, out_specs;
  for (auto spec : rop.getInputSpecs()) {
    input_specs.push_back(toolchain::cast<TypeAttr>(spec).getValue());
  }
  for (auto spec : rop.getOutputSpecs()) {
    out_specs.push_back(toolchain::cast<TypeAttr>(spec).getValue());
  }

  auto in_spec = toolchain::dyn_cast<UniformQuantizedType>(input_specs[0]);
  // TODO(fengliuai): handles the PerAxis QuantizedType.
  auto w_spec = toolchain::dyn_cast<UniformQuantizedType>(input_specs[1]);
  auto b_spec = toolchain::dyn_cast<UniformQuantizedType>(input_specs[2]);
  auto o_spec = toolchain::dyn_cast<UniformQuantizedType>(out_specs[0]);
...
LogicalResult DeviceTarget::DecomposeSameScale(
  ...
  auto o_spec = toolchain::dyn_cast<UniformQuantizedType>(
      toolchain::cast<TypeAttr>(rop.getOutputSpecs()[0]).getValue());
