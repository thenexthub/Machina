...
toolchain::SmallVector<Value> AccumulateOperandsDefinedAbove(
  ...
  toolchain::SetVector<Value> been_defined;
  toolchain::SetVector<Value> results;
...
toolchain::SmallVector<Value> AccumulateResultsDefinedWithin(
    const toolchain::SetVector<Operation*>& partition_ops) {
  toolchain::SmallVector<Value> values_for_results;
  for (Operation* op : partition_ops) {
    ...
    for (Value output : op->getResults()) {
      ...
      for (Operation* consumer : output.getUsers()) {
        if (toolchain::all_of(partition_ops, [&](Operation* op) {
...
// Operation partition.
toolchain::SmallVector<Type> TypesFromValues(
    const toolchain::SmallVector<Value>& values) {
...
func::FuncOp BuildFuncOp(const Subgraph& subgraph, OpBuilder& builder,
  ...
  toolchain::SmallVector<Type> input_types =
