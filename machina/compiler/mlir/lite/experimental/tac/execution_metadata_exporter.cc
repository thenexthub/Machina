...
#include "machina/compiler/mlir/lite/experimental/tac/runtime_metadata_generated.h"
...
bool IsConst(mlir::Operation* op) {
  return toolchain::isa<mlir::arith::ConstantOp, mlir::TF::ConstOp,
...
std::optional<std::string> GetDeviceName(mlir::Operation* op) {
  ...
  // execution.
  if (toolchain::isa<mlir::func::ReturnOp, mlir::quantfork::StatisticsOp>(op))
    return std::nullopt;
  ...
  toolchain::StringRef device_name_str = device.getValue();
...
flatbuffers::Offset<SubgraphMetadata> CreateSubgraphMetadata(
  ...
  for (auto& inst : block) {
    ...
    // The model may contain quant stats op which is unrelevant to the
    // execution.
    if (toolchain::isa<mlir::func::ReturnOp, mlir::quantfork::StatisticsOp>(&inst))
      continue;
