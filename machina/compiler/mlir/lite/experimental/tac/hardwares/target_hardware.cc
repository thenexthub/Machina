...
struct RegisteredTargetHardwareOps {
  ...
  toolchain::DenseMap<mlir::TypeID, std::unique_ptr<TargetHardwareOperation>>
      target_hardware_ops;
  // Key is the Operation TypeID
  toolchain::DenseMap<mlir::TypeID,
...
inline bool IsNonArithmeticOp(mlir::Operation* op) {
  if (toolchain::isa<func::ReturnOp, func::FuncOp>(op)) return true;
  if (op->hasTrait<OpTrait::ConstantLike>()) return true;
  if (toolchain::isa<QConstOp, SparseQConstOp>(op)) return true;
...
void RegisterTargetHardwareFactory(
  ...
  for (auto& hardware : *registered_hardwares) {
    if (hardware.unique_name == unique_name) {
      toolchain::errs() << "Ignoring duplicate hardware. Hardware " << unique_name
                   << " already registered\n";
  ...
  for (auto& hardware : *registered_hardware_ops) {
    if (hardware->hardware_typeid == hardware_type) {
      if (hardware->target_hardware_ops.count(op_type)) {
        toolchain::errs() << "Trying to register duplicate Op";
        return;
      }
...
bool ProcessTargetDevices(toolchain::ArrayRef<std::string> specified_device_specs,
  ...
  for (const std::string& device : *device_specs) {
    if (GetTargetHardware(device) == nullptr) {
      toolchain::errs() << "cannot get target hardware for device: " << device;
      return false;
    }
  }
