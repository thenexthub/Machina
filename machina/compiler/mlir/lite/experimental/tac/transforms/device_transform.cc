...
namespace tac {
namespace {
#include "machina/compiler/mlir/lite/experimental/tac/transforms/generated_transform_patterns.inc"
}  // namespace
...
void ConvertQuantizedOpToFloat(mlir::func::FuncOp func, OpBuilder* builder) {
  func.walk([&](Operation* op) {
    ...
    if (op->hasTrait<OpTrait::IsTerminator>() ||
        toolchain::isa<TFL::QConstOp, TFL::ConstOp>(op) ||
        toolchain::isa<TFL::QConstOp, TFL::ConstOp, TF::ConstOp, ConstOp>(op))
      return;
...
struct FoldQuantizedI32ToFloat : public OpRewritePattern<TFL::DequantizeOp> {
  ...
  LogicalResult matchAndRewrite(TFL::DequantizeOp dequant_op,
    ...
    auto input_dequant = toolchain::dyn_cast_or_null<TFL::QConstOp>(input);
    ...
    // is actually float.
    using DequantizeFuncType = toolchain::APInt(const toolchain::APInt&);
    ...
    auto dequant_values =
        mlir::cast<DenseIntOrFPElementsAttr>(input_values)
            .mapValues(Float32Type::get(rewriter.getContext()),
                       toolchain::function_ref<DequantizeFuncType>(dequantize_func));
