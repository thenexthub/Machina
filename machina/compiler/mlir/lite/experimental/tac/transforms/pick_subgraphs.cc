...
#include "machina/compiler/mlir/lite/experimental/tac/common/subgraph.h"
...
inline func::CallOp GetProducerCallOpOrNull(Value output) {
  ...
  if (output_op != nullptr && toolchain::isa<func::CallOp>(output_op)) {
    return toolchain::cast<func::CallOp>(output_op);
  }
  return nullptr;
}
...
class PickSubgraphsPass
 ...
 private:
  toolchain::StringRef getArgument() const final { return "tfl-pick-subgraphs"; }
  ...
  void BuildSubgraphs(
      ...
      const std::unordered_map<std::string, std::vector<func::FuncOp>>&
          func_impls,
      toolchain::SetVector<Operation*>* unprocessed_subgraphs,
  ...
  bool PickSubgraphs(
      toolchain::SetVector<Operation*>* all_subgraphs,
  ...
  toolchain::DenseMap<Operation*, Subgraph> subgraphs_;

  toolchain::DenseMap<Operation*, InferenceDeviceType> decisions_;
};
...
void PickSubgraphsPass::BuildSubgraphs(
    ...
    toolchain::SetVector<Operation*>* unprocessed_subgraphs,
    SmallVector<func::CallOp, 4>* output_subgraphs) {
  toolchain::DenseSet<Operation*> returned_call_op_set;
...
bool PickSubgraphsPass::MakeDecisions(ArrayRef<func::CallOp> output_subgraphs) {
  ...
  while (!processing_queue.empty()) {
    ...
    for (const auto& input_with_plans : current->input_subgraph_plans) {
      func::CallOp input = toolchain::cast<func::CallOp>(input_with_plans.first);
...
void PickSubgraphsPass::RewireSubgraphs(
  ...
  for (auto& kv : decisions_) {
    func::CallOp call = toolchain::cast<func::CallOp>(kv.first);
...
bool PickSubgraphsPass::PickSubgraphs(
  ...
  while (!all_subgraphs->empty()) {
    func::CallOp current_subgraph =
        toolchain::cast<func::CallOp>(all_subgraphs->front());
