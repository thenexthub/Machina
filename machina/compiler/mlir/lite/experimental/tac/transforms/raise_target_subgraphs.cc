...
#include "machina/compiler/mlir/lite/experimental/common/outline_operations.h"
 ...
 private:
  Option<bool> skip_raise_cpu_ops_{
      *this, "skip-raise-cpu-ops",
      toolchain::cl::desc("Whether to cluster and raise CPU ops."),
      toolchain::cl::init(false)};

  Option<bool> ignore_inference_type_{
      *this, "ignore-inference-type",
      toolchain::cl::desc("Whether to ignore the inference type in clustering."),
      toolchain::cl::init(false)};
...
bool RaiseTargetSubgraphsPass::RaiseTargetSubgraphsForBlock(
    Block& block, OpBuilder& builder, ModuleOp module, bool skip_cpu,
    int& func_count, const TF::SideEffectAnalysis::Info& side_effect_info) {
  toolchain::SetVector<Operation*> partition_ops;

  auto device_is = [&](InferenceDeviceType t, toolchain::StringRef device) -> bool {
  ...
  auto op_device_is = [&](Operation& op, toolchain::StringRef device) -> bool {
  ...
  auto extract = [&](const toolchain::SetVector<Operation*>& partition_ops) -> bool {
  ...
  const toolchain::StringMap<SmallVector<TF::Cluster>>& all_clusters =
