...
#include "machina/compiler/mlir/lite/experimental/tac/tac_filter.pb.h"
...
using ::third_party::machina::compiler::mlir::lite::experimental::tac::
    FunctionFilter;
using ::third_party::machina::compiler::mlir::lite::experimental::tac::
    OpFilter;
using ::third_party::machina::compiler::mlir::lite::experimental::tac::
...
class TacFilterPass
 ...
 private:
  ...
  toolchain::StringRef getArgument() const final { return "tfl-tac-filter"; }
...
void ApplyTacFilter(
  ...
  toolchain::Regex op_regex(tac_filter.op_filter().op_name_pattern());
...
std::string TacFilterToString(const TacFilter& tac_filter) {
  ...
  if (tac_filter.has_function_filter()) {
    tac_filter_type_str = (toolchain::Twine("function filter ") +
  ...
  return (toolchain::Twine("filter type: ") + tac_filter_type_str +
...
void PrintTacFilterResult(Location module_loc, const TacFilter& tac_filter,
  ...
  emitRemark(module_loc) << toolchain::formatv("Tac filter ({0}): {1}", count,
                                          TacFilterToString(tac_filter));
  if (filtered_ops.empty()) {
    emitRemark(module_loc) << toolchain::formatv(
  ...
  if (tac_filter.has_function_filter()) {
    for (Operation* op : filtered_ops) {
      ...
      func.emitRemark() << toolchain::formatv("filtered by tac filter ({0})", count);
    }
    return;
  }
  ...
  for (auto& [func, ops] : func_to_filtered_ops_map) {
    ...
    toolchain::raw_string_ostream os(interleaved_op_name);
    toolchain::interleaveComma(
        ops, os, [&](Operation* op) { os << "\"" << op->getName() << "\""; });
    os.flush();
    func.emitRemark() << toolchain::formatv(
...
void TacFilterPass::runOnOperation() {
  ...
  for (const auto& tac_filter : toolchain::enumerate(tac_filters_->tac_filters())) {
