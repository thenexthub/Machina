 ...
 private:
  ...
  Option<bool> fold_all_constants_flag_{
      *this, "fold-all-constants",
      toolchain::cl::desc("Whether to fold all constants or just i32."),
...
void CopyConstantIntoFunc(int argument_index, Operation* const_op,
                          func::FuncOp func) {
  assert(
      (toolchain::isa<TFL::ConstOp, TFL::QConstOp, arith::ConstantOp>(const_op)) &&
...
bool IsConstOrQConstInt(Operation* op) {
  if (!toolchain::isa<TFL::ConstOp, TFL::QConstOp, arith::ConstantOp>(op))
    return false;
  ...
  for (auto fn : module.getOps<func::FuncOp>()) {
    fn.walk([&](Operation* op) {
      ...
      for (auto consumer : op->getResult(0).getUsers()) {
        auto consumer_call = toolchain::dyn_cast_or_null<func::CallOp>(consumer);
