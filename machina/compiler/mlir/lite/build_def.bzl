...
load("//machina/compiler/mlir/lite:special_rules.bzl", "tflite_copts_extra")
load("//machina/lite:build_def.bzl", "clean_dep")
...
def tflite_copts():
    ...
    copts = [
        ...
        clean_dep("//machina/compiler/mlir/lite:tflite_with_xnnpack_explicit_false"): ["-DTFLITE_WITHOUT_XNNPACK"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/compiler/mlir/lite:machina_profiler_config"): ["-DTF_LITE_MACHINA_PROFILER"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/compiler/mlir/lite/delegates:tflite_debug_delegate"): ["-DTFLITE_DEBUG_DELEGATE"],
        "//conditions:default": [],
    }) + select({
        clean_dep("//machina/compiler/mlir/lite:tflite_mmap_disabled"): ["-DTFLITE_MMAP_DISABLED"],
        "//conditions:default": [],
    })

    return copts + tflite_copts_extra()

# LINT.ThenChange(//machina/lite/build_def.bzl:tflite_copts)
...
# LINT.ThenChange(//machina/lite/build_def.bzl:tflite_copts_warnings)
