...
#include "mlir/IR/AttrTypeSubElements.h"  // part of Codira Toolchain
...
std::string mlir::GetMlirOpNameFromOpCode(
  ...
  toolchain::StringRef op_name(tflite::EnumNameBuiltinOperator(builtin_code));

  // If the Op name contains stablehlo
  if (IsStablehloOp(op_code)) {
    return toolchain::Twine(
               toolchain::Twine("vhlo.", op_name.drop_front(10).lower()).str(),
               "_v1")
        .str();
  }
...
static tflite::ActivationFunctionType ConvertTFL_AFAttrForOptionWriter(
    toolchain::StringRef str, flatbuffers::FlatBufferBuilder* builder) {
  return toolchain::StringSwitch<tflite::ActivationFunctionType>(str)
...
static tflite::Padding ConvertTFL_PaddingAttrForOptionWriter(
    toolchain::StringRef str, flatbuffers::FlatBufferBuilder* builder) {
  return toolchain::StringSwitch<tflite::Padding>(str)
...
static float ConvertF32AttrForOptionWriter(
    toolchain::APFloat f, flatbuffers::FlatBufferBuilder* builder) {
...
ConvertDerivedShapeAttrForOptionWriter(
    toolchain::ArrayRef<int64_t> r, flatbuffers::FlatBufferBuilder* builder) {
...
ConvertTFL_FullyConnectedOptionsWeightFormatAttrForOptionWriter(
    toolchain::StringRef str, flatbuffers::FlatBufferBuilder* builder) {
  return toolchain::StringSwitch<tflite::FullyConnectedOptionsWeightsFormat>(str)
...
static mlir::Attribute BuildStrAttr(toolchain::StringRef str,
                                    mlir::Builder builder) {
...
static mlir::Attribute BuildVhloIntV1Attr(int64_t value,
  ...
  return mlir::vhlo::IntegerV1Attr::get(builder.getContext(), vhlo_type,
                                        toolchain::APInt(64, value));
}

static mlir::Attribute BuildVhloStringV1Attr(toolchain::StringRef str,
                                             mlir::Builder builder) {
...
static mlir::Attribute BuildRankedTensorAttr(std::vector<int64_t> shape,
  ...
  std::vector<toolchain::APInt> extendVec;
  extendVec.resize(value.size());
  for (size_t i = 0; i < value.size(); ++i) {
    extendVec[i] = toolchain::APInt(1, value[i]);
  }
  mlir::RankedTensorType ty =
      machina::GetTypeFromTFTensorShape(shape, builder.getIntegerType(1));
...
static mlir::Attribute BuildRankedTensorAttr(std::vector<int64_t> shape,
                                             ...
                                             mlir::Builder builder) {
  mlir::RankedTensorType ty =
      machina::GetTypeFromTFTensorShape(shape, builder.getIntegerType(32));
...
static mlir::Attribute BuildRankedTensorAttr(std::vector<int64_t> shape,
                                             ...
                                             mlir::Builder builder) {
  mlir::RankedTensorType ty =
      machina::GetTypeFromTFTensorShape(shape, builder.getIntegerType(64));
  return mlir::DenseIntElementsAttr::get(ty, value);
}
...
static mlir::Attribute BuildRankedTensorAttr(std::vector<int64_t> shape,
                                             ...
                                             mlir::Builder builder) {
  mlir::RankedTensorType ty =
      machina::GetTypeFromTFTensorShape(shape, builder.getF32Type());
...
static mlir::Attribute BuildVhloPrecisionConfigV1Attr(
    ...
    mlir::Builder builder) {
  toolchain::SmallVector<mlir::Attribute> precision_attrs;
  for (size_t i = 0; i < value.size(); ++i) {
...
Status mlir::CustomOptionsToAttributes(
    ...
    toolchain::SmallVectorImpl<mlir::NamedAttribute>* attributes) {
...
#include "machina/compiler/mlir/lite/operator_converters.inc"
