...
// thenexthub/Codira/mlir/lib/Transforms/Utils/TopologicalSortUtils.cpp.
//
...
bool PartitionedTopologicalSort(
  ...
  toolchain::iterator_range<Block::iterator> ops = block->without_terminator();
  if (ops.empty()) {
    return true;
  }
  ...
  toolchain::DenseMap<Operation *, int> unscheduled_ops;
  ...
  while (!unscheduled_ops.empty()) {
    ...
    for (Operation &unscheduled_op : toolchain::make_early_inc_range(
             toolchain::make_range(next_unscheduled_op, end))) {
