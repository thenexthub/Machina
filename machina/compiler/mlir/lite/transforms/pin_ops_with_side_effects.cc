...
bool OpHasSideEffects(Operation *op) {
  ...
  // be future-proof.
  if (toolchain::isa<TF::IfOp, TFL::IfOp, TFL::CallOnceOp, TFL::WhileOp>(op))
    return true;
...
// machina/lite/core/subgraph). That runtime logic will now be a no-op for
...
void PinOpsWithSideEffectsPass::runOnOperation() {
  ...
  toolchain::SmallVector<Operation *, 4> ops_with_side_effects;
  ...
  // 1.
  toolchain::SmallVector<Value, 1> control_tokens;
