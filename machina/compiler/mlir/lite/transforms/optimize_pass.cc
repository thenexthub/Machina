...
#include "machina/compiler/mlir/lite/transforms/optimize_pass.h"
...
#include "machina/compiler/mlir/lite/utils/attribute_utils.h"
#include "machina/compiler/mlir/lite/utils/constant_utils.h"
#include "machina/compiler/mlir/lite/utils/convert_type.h"
#include "machina/compiler/mlir/lite/utils/utils.h"
#include "machina/compiler/mlir/lite/utils/validators.h"
...
bool IsBalancedPaddingArray(int spatials_start, int spatials_end,
  ...
  if (data.getElementType().isInteger(64)) {
    return IsBalancedPaddingArray<int64_t>(
        ...
        toolchain::SmallVector<int64_t>(data.value_begin<int64_t>(),
                                   data.value_end<int64_t>()));
  }
  if (data.getElementType().isInteger(32)) {
    return IsBalancedPaddingArray<int32_t>(
        spatials_start, spatials_end,
        toolchain::SmallVector<int32_t>(data.value_begin<int32_t>(),
...
bool HasSameStridedShape(TFL::Conv2DOp op, ArrayRef<int64_t> pre_pad_shape) {
  auto conv_in_shape =
      toolchain::dyn_cast<ShapedType>(op.getInput().getType()).getShape();
  auto kernel_shape =
      toolchain::dyn_cast<ShapedType>(op.getFilter().getType()).getShape();
...
Value GetBiasMultiplier(OpBuilder &builder, Value binary_op,
  ...
  float multiplier =
      (toolchain::isa<mlir::TFL::AddOp>(binary_op.getDefiningOp()) ? 1.0 : -1.0);
...
static bool ShapeMatchesReduceWithKeepAxes(Value input,
  ...
  toolchain::SmallSet<uint64_t, 4> axes_set;
...
static bool FloatValueEquals(const Attribute &attr, double value) {
  ...
  return toolchain::all_of(fp_attr.getValues<APFloat>(), [value](const APFloat &f) {
...
bool IsConstantValueOf(mlir::TypedAttr value, T raw_value) {
  ...
  if (mlir::isa<FloatType>(element_type)) {
    ...
    return toolchain::all_of(int_attr.getValues<APInt>(),
...
TypedAttr GetNumElementsOrOne(Type type) {
  ...
  return DenseIntElementsAttr::get(
      ...
      {toolchain::APInt(32, num_elements, true)});
}
...
bool HasOneUseOrUsedByOnlyBinaryOps(Value out_value) {
  ...
  for (auto &use : out_value.getUses()) {
    ...
    if (!toolchain::isa<mlir::TFL::AddOp>(owner) &&
        !toolchain::isa<mlir::TFL::SubOp>(owner) &&
        !toolchain::isa<mlir::TFL::DivOp>(owner) &&
        !toolchain::isa<mlir::TFL::MulOp>(owner)) {
      return false;
    }
  }
...
bool IsOneHotIndexAttribute(Attribute attr) {
  ...
  if (index_type.getRank() < 1 ||
      toolchain::any_of(index_type.getShape().drop_back(),
...
#include "machina/compiler/mlir/lite/transforms/generated_optimize.inc"
...
struct SqueezeReshapesAroundBroadcastOp
  ...
  LogicalResult matchAndRewrite(TFL::BroadcastToOp tfl_broadcast_to_op,
    ...
    //     broadcast_shape),
    //   outer_shape))) pattern.
    if (!toolchain::dyn_cast_or_null<TFL::ReshapeOp>(
        ...
        !toolchain::dyn_cast_or_null<TFL::ReshapeOp>(
    ...
    auto inner_reshape_op = toolchain::dyn_cast_or_null<TFL::ReshapeOp>(
        tfl_broadcast_to_op.getInput().getDefiningOp());
    auto inner_reshape_input = inner_reshape_op.getInput();
    auto outer_reshape_op = toolchain::dyn_cast_or_null<TFL::ReshapeOp>(
        *tfl_broadcast_to_op.getOutput().getUsers().begin());
    ...
    toolchain::SmallVector<int32_t, 6> new_reshape_shape_i32{
    ...
    toolchain::SmallVector<int64_t, 6> new_broadcast_shape{
...
struct Convert2DUpscalingToResizeNearestNeighor
  ...
  LogicalResult matchAndRewrite(TFL::GatherNdOp gather_nd_first,
    ...
    auto reshape_op = rewriter.create<TFL::ReshapeOp>(
        gather_nd_first->getLoc(),
        machina::GetTypeFromTFTensorShape(reshape_shape_in_int64,
...
struct FuseFullyConnectedAndMul : public OpRewritePattern<TFL::MulOp> {
  ...
  LogicalResult matchAndRewrite(TFL::MulOp mul_op,
    ...
    if (filter_type.hasStaticShape()) {
      ...
      // machina/compiler/mlir/machina/transforms/constant_fold.cc
  ...
  LogicalResult matchAndRewrite(TFL::MulOp mul_op,
    ...
    if (auto transpose_op =
      ...
      SmallVector<int32_t> permutation_vec =
          toolchain::to_vector(permutation_cst.getValues<int32_t>());
...
struct FuseBinaryOpToFollowingAffineOp : public OpRewritePattern<AffineOpType> {
  ...
  LogicalResult matchAndRewrite(AffineOpType fc_op,
    ...
    if (!matchPattern(filter, m_Constant(&filter_cst))) {
      ...
      auto dq = toolchain::dyn_cast_or_null<DequantizeOp>(filter.getDefiningOp());
      if (!dq) return failure();
      auto q =
          toolchain::dyn_cast_or_null<QuantizeOp>(dq.getInput().getDefiningOp());
    ...
    if (toolchain::isa<AddOp, SubOp>(binary_op)) {
      ...
      if (toolchain::isa<SubOp>(binary_op)) cst_value.changeSign();
...
struct RemoveReshapeAfterFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::ReshapeOp reshape_op,
                                PatternRewriter &rewriter) const override {
    auto fully_connected_op = toolchain::dyn_cast_or_null<TFL::FullyConnectedOp>(
    ...
    toolchain::SmallVector<Type, 1> output_type{reshape_op.getType()};
...
struct FuseUnpackAndConcatToReshape
  ...
  LogicalResult matchAndRewrite(TFL::ConcatenationOp concat_op,
    ...
    for (const auto &index_and_value : toolchain::enumerate(concat_op.getValues())) {
...
struct OptimizeTopK : public OpRewritePattern<TFL::TopKV2Op> {
  ...
  std::optional<int32_t> ComputeSliceK(Value value) const {
    ...
    auto slice_op =
        toolchain::dyn_cast_or_null<TFL::SliceOp>(value.getUses().begin().getUser());
  ...
  LogicalResult matchAndRewrite(TFL::TopKV2Op op,
    ...
    if (!values.use_empty()) {
      auto values_slice_op = toolchain::dyn_cast_or_null<TFL::SliceOp>(
    ...
    if (!indices.use_empty()) {
      auto indices_slice_op = toolchain::dyn_cast_or_null<TFL::SliceOp>(
...
struct FuseReshapeAndTransposeAroundBatchMatmul
  ...
  LogicalResult matchAndRewrite(TFL::TransposeOp op,
    ...
    for (const auto &[perm_index, match_perm_index] :
         toolchain::zip(transpose_perm.getValues<APInt>(), match_perm)) {
...
struct MoveReshapeAfterFullyConnected
  ...
  LogicalResult matchAndRewrite(TFL::ReshapeOp reshape,
                                PatternRewriter &rewriter) const override {
    auto fc = toolchain::dyn_cast_or_null<TFL::FullyConnectedOp>(
    ...
    toolchain::SmallVector<int32_t> new_input_shape(reshape_ty.getShape());
...
struct EnableFullyConnectedKeepNumDimsBeforeReshape
  ...
  LogicalResult matchAndRewrite(TFL::ReshapeOp reshape,
    ...
    toolchain::SmallVector<int64_t> new_fc_shape(input_ty.getShape());
...
struct PushTransposeThroughSqueeze : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(mlir::Operation *op,
    ...
    auto transpose = toolchain::dyn_cast_or_null<TFL::TransposeOp>(
    ...
    toolchain::SmallVector<int32_t, 4> squeeze_dims;
    ...
    toolchain::SmallVector<int32_t, 4> perm;
    ...
    toolchain::sort(squeeze_dims);
    toolchain::SmallVector<int32_t, 4> new_squeeze_dims;
    for (int32_t dim : squeeze_dims) {
      new_squeeze_dims.push_back(perm[dim]);
    }
    toolchain::sort(new_squeeze_dims);

    // Filter the original transpose permutation to keep only non-squeezed
    // positions.
    toolchain::SmallVector<int32_t> filtered_perm_original_indices;
    for (int i = 0; i < input_rank; ++i) {
      if (!toolchain::is_contained(squeeze_dims, i)) {
    ...
    // squeeze.
    toolchain::SmallVector<int32_t> original_remaining_dims;
    for (int i = 0; i < input_rank; ++i) {
      if (!toolchain::is_contained(new_squeeze_dims, i)) {
        original_remaining_dims.push_back(i);
      }
    }

    toolchain::SmallVector<int32_t> original_to_new_index_map(input_rank, -1);
    for (int i = 0; i < original_remaining_dims.size(); ++i) {
      original_to_new_index_map[original_remaining_dims[i]] = i;
    }

    toolchain::SmallVector<int32_t> new_perm;
    for (const auto &original_dim : filtered_perm_original_indices) {
      new_perm.push_back(original_to_new_index_map[original_dim]);
    }

    toolchain::SmallVector<int64_t> new_squeeze_shape;
    for (int i = 0; i < input_rank; ++i) {
      if (!toolchain::is_contained(new_squeeze_dims, i)) {
...
inline DenseIntElementsAttr GetI32ElementsAttr(ArrayRef<int64_t> values,
                                               Builder *builder) {
  toolchain::SmallVector<int32_t> new_values;
  for (auto el : values) {
...
struct ReorderTransposeReshapeTranspose
  ...
  LogicalResult matchAndRewrite(TFL::TransposeOp outer_tpose,
    ...
    toolchain::SmallVector<int32_t, 4> reshape_shape;
    {
    ...
    toolchain::SmallVector<int64_t, 4> new_reshape_shape(reshape_shape.size());
    toolchain::SmallVector<int32_t, 4> new_inner_perm(reshape_shape.size());
