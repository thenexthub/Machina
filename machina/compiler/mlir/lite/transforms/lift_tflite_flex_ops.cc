 ...
 *
 */
#include "machina/compiler/mlir/lite/transforms/lift_tflite_flex_ops.h"
...
class LiftFlexCustomOp : public OpRewritePattern<TFL::CustomOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TFL::CustomOp op,
    ...
    toolchain::StringRef tf_op_name =
        op.getCustomCode().substr(kFlexOpNamePrefix.size());
    const std::string tf_op_full_name = toolchain::Twine("tf.", tf_op_name).str();
    ...
    if (auto tensor_array_v3_op = dyn_cast<TF::TensorArrayV3Op>(tf_op)) {
      ...
      // https://www.machina.org/api_docs/python/tf/raw_ops/TensorArrayWriteV3
    ...
    // Referenced from machina::ImporterBase::CreateOperation

    const auto set_segment_sizes_attr =
        [&](const machina::NameRangeMap& arg_ranges,
            const machina::protobuf::RepeatedPtrField<
                machina::OpDef::ArgDef>& args,
    ...
    if (tf_op->hasTrait<mlir::OpTrait::AttrSizedOperandSegments>() ||
      ...
      machina::NameRangeMap input_ranges, output_ranges;
 ...
 private:
  ...
  static LogicalResult ParseCustomOption(
    ...
    for (const auto& name_and_value : node_def.attr()) {
      ...
      const machina::AttrValue& attr_value = name_and_value.second;
      absl::StatusOr<Attribute> mlir_attr =
          machina::ConvertAttributeValue(attr_value, &builder);
      if (!mlir_attr.ok()) {
