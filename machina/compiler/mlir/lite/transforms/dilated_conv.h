...
#ifndef MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_DILATED_CONV_H_
#define MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_DILATED_CONV_H_
...
LogicalResult ConvertTFDilatedConvOp<Conv2dOpTy>::matchAndRewrite(
  ...
  // Expand + Squeeze op.
  if (toolchain::isa<TF::ExpandDimsOp>(producer_op)) {
    if (!toolchain::isa<TF::SqueezeOp>(consumer_op)) {
      // Expand/Squeeze op must come in pair.
      return rewriter.notifyMatchFailure(
          op, "ExpandDimsOp and SqueezeOp should come in pair");
    }
    expand_op = toolchain::cast<TF::ExpandDimsOp>(producer_op);
    squeeze_op = toolchain::cast<TF::SqueezeOp>(consumer_op);
    ...
    if (auto const_op =
            toolchain::dyn_cast<TF::ConstOp>(expand_op.getDim().getDefiningOp())) {
  ...
  // SpaceToBatchND op.
  if (!toolchain::isa<TF::SpaceToBatchNDOp>(producer_op)) {
  ...
  TF::SpaceToBatchNDOp stb_op = toolchain::cast<TF::SpaceToBatchNDOp>(producer_op);
  ...
  if (toolchain::isa<TF::PadOp>(consumer_op)) {
    pad_op = toolchain::cast<TF::PadOp>(consumer_op);
  ...
  if (toolchain::isa<TF::BiasAddOp>(consumer_op)) {
    // Must be BiasAdd + BatchToSpaceND.
    biasadd_op = toolchain::cast<TF::BiasAddOp>(consumer_op);
    ...
    if (!toolchain::isa<TF::BatchToSpaceNDOp>(maybeConsumer.second)) {
      return rewriter.notifyMatchFailure(
          consumer_op, "op's next node isn't BatchToSpaceND op");
    }
    consumer_op = maybeConsumer.second;
    bts_op = toolchain::cast<TF::BatchToSpaceNDOp>(consumer_op);
  } else if (toolchain::isa<TF::BatchToSpaceNDOp>(consumer_op)) {
    // BatchToSpaceND + (optional) BiasAdd.
    bts_op = toolchain::cast<TF::BatchToSpaceNDOp>(consumer_op);
    std::pair<LogicalResult, Operation*> maybeConsumer =
        tryGetDirectConsumerOp(consumer_op);
    Operation* tmp = maybeConsumer.second;
    if (tmp && toolchain::isa<TF::BiasAddOp>(tmp)) {
      consumer_op = tmp;
      biasadd_op = toolchain::cast<TF::BiasAddOp>(consumer_op);
...
#endif  // MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_DILATED_CONV_H_
