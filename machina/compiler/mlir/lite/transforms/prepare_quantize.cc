...
#include "machina/compiler/mlir/lite/quantization/common/quantization_lib/tfl_quantization_driver.h"
...
auto* tflite_quantizer_usage_stats = machina::monitoring::Counter<1>::New(
    "/machina/lite/quantization/transforms/stats",
...
bool PrepareQuantizePass::SetInputNodesQuantizationParams(func::FuncOp func) {
  ...
  if (target_func != func_name &&
      !toolchain::is_contained(quantize_allowlist_, func_name)) {
    return false;
  }
  auto has_quantize_op = [&](const Value arg) {
    return (arg.hasOneUse() &&
            toolchain::isa<quantfork::QuantizeCastOp>(*arg.user_begin()));
  };
...
static Value Quantized(Operation* user) {
  if (auto q = toolchain::dyn_cast_or_null<quantfork::QuantizeCastOp>(user)) {
    if (auto dq = toolchain::dyn_cast_or_null<quantfork::DequantizeCastOp>(
...
bool PrepareQuantizePass::ContainsQuantizeOps(func::FuncOp func) {
  for (const auto& op : func.getOps()) {
    if (toolchain::isa<quantfork::DequantizeCastOp>(op)) return true;
  }
  return false;
}
...
void PrepareQuantizePass::runOnOperation() {
  ...
  if (use_quantization_flags_) {
    ...
    if (quant_specs_.inference_type == machina::DT_INVALID) {
