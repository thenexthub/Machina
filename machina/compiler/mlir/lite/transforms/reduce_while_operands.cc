...
void FindConsumedOp(Operation *start_op,
                    toolchain::DenseSet<Operation *> &consumed_ops) {
...
bool ReduceWhileOperands(TFL::WhileOp while_op) {
  ...
  toolchain::BitVector erase_indices(n);
  ...
  for (Operation &op :
       toolchain::make_early_inc_range(reverse(body.getOperations())))
  ...
  toolchain::SmallVector<Type> new_result_types;
