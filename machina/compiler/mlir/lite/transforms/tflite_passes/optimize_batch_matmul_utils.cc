...
bool HasFlattenedContractingDims(
  ...
  if (!toolchain::all_of(
          toolchain::zip(batch_dimensions, reshape_input_shape_batch_dims),
  ...
  toolchain::ArrayRef<int32_t> reshape_input_shape_out_dims;
  ...
  if (!toolchain::all_of(
          toolchain::zip(out_dimensions, reshape_input_shape_out_dims),
          [](auto dims) { return std::get<0>(dims) == std::get<1>(dims); })) {
    return false;
  }
  ...
  toolchain::ArrayRef<int32_t> reshape_input_shape_contracting_dims;
...
bool HasFlattenedOutDims(toolchain::ArrayRef<int32_t> reshape_input_shape,
  ...
  if (!toolchain::all_of(
          toolchain::zip(contracting_dimensions,
                    reshape_input_shape_contracting_dims),
          [](auto dims) { return std::get<0>(dims) == std::get<1>(dims); })) {
    return false;
  }
