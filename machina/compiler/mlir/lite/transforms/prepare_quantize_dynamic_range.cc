...
using QuantizationUnits = toolchain::SetVector<std::pair<Operation*, int>>;
...
class PrepareDynamicRangeQuantizePass
 ...
 private:
  ...
  toolchain::SetVector<Operation*> visited_nonquantizable_ops_;
  QuantizationSpecs quant_specs_;
};

#include "machina/compiler/mlir/lite/utils/generated_op_quant_spec_getters.inc"
...
class PrepareDynamicRangeQuantizableOp
 ...
 public:
  explicit PrepareDynamicRangeQuantizableOp(
      ...
      toolchain::SetVector<Operation*>* const visited_nonquantizable_ops)
  ...
  LogicalResult matchAndRewrite(arith::ConstantOp op,
    ...
    if (quant_specs_.inference_type == machina::DT_QINT8 &&
        (setAsymmetricQuantizeInputAttr(rewriter, quantizable_ops))) {
      return failure();
    }
    if (quant_specs_.inference_type == machina::DT_HALF &&
 ...
 private:
  toolchain::SetVector<Operation*>* const visited_nonquantizable_ops_;
  ...
  bool hasInt8QuantizableOperandAt(Operation* op, int operand_index) const {
    ...
    if (auto custom_op = toolchain::dyn_cast_or_null<CustomOp>(op)) {
      ...
      if (custom_map_iter != quant_specs_.custom_map.end())
        return isQuantizableIndex(
            operand_index, custom_map_iter->second.quantizable_input_indices);
                   ...
                   toolchain::dyn_cast<DynamicRangeQuantizedOpInterface>(op)) {
      ...
      if (!toolchain::dyn_cast<UnidirectionalSequenceLSTMOp>(op)) {
        return true;
      }

      for (int qi : quantizable_indices) {
        auto const_op = toolchain::dyn_cast_or_null<arith::ConstantOp>(
  ...
  void quantizeOpAsFloat16(PatternRewriter& rewriter, arith::ConstantOp op,
    ...
    if (toolchain::dyn_cast_or_null<func::ReturnOp>(quantize_op)) {
      return;
    }
    ...
    for (auto& connected_op : op.getResult().getUses()) {
      auto cast_op = toolchain::dyn_cast_or_null<CastOp>(connected_op.getOwner());
  ...
  bool quantizeOpAsInt8(PatternRewriter& rewriter, arith::ConstantOp op,
    ...
    if (!toolchain::dyn_cast_or_null<CustomOp>(quantize_op)) {
  ...
  bool insertQDQ(PatternRewriter& rewriter, arith::ConstantOp op,
    ...
    for (auto connected_op : op->getUsers()) {
      auto q_op = toolchain::dyn_cast_or_null<Q>(connected_op);
      if (q_op && q_op.getType() == cast_type) {
        auto dq_op = toolchain::cast<DQ>(q_op.getResult().use_begin()->getOwner());
  ...
  bool getQuantizableOps(arith::ConstantOp op,
    ...
    for (auto& use : value.getUses()) {
      ...
      } else if (quant_specs_.inference_type == machina::DT_HALF) {
        quantizable_ops.insert({user, operand_num});
      }
    }
  ...
  // details, see machina/compiler/mlir/lite/quantization/quantization.td
  bool setAsymmetricQuantizeInputAttr(
    ...
    for (auto& quant_op : quantizable_ops) {
      ...
      if (dynamic_range_quantized_user &&
        ...
        // machina/lite/kernels/fully_connected.cc. The kernels will handle
...
void PrepareDynamicRangeQuantizePass::runOnOperation() {
  ...
  if (enable_float16_quantization_) {
    quant_specs_.inference_type = machina::DT_HALF;
  }
