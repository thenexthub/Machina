...
#include "machina/compiler/mlir/lite/transforms/push_transpose_through_ewise_pass.h"
...
toolchain::SmallVector<int32_t> InvertPermutation(toolchain::SmallVector<int32_t> arr) {
  toolchain::SmallVector<int32_t> inverse_arr(arr.size());
...
toolchain::SmallVector<int64_t> PermuteShape(toolchain::ArrayRef<int64_t> shape,
                                        toolchain::ArrayRef<int32_t> perm) {
  toolchain::SmallVector<int64_t> new_shape(shape.size());
...
bool IsElementwise(Operation *op) {
  if (!(toolchain::isa<TFL::AddOp, TFL::MulOp, TFL::DivOp, TFL::SubOp,
                  TFL::MaximumOp, TFL::MinimumOp>(op))) {
    return false;
  }

  auto opr1_type =
      toolchain::dyn_cast_or_null<RankedTensorType>(op->getOperand(0).getType());
  auto opr2_type =
      toolchain::dyn_cast_or_null<RankedTensorType>(op->getOperand(1).getType());
  auto res_type =
      toolchain::dyn_cast_or_null<RankedTensorType>(op->getResult(0).getType());
...
class CommuteBothInputsTransposedWithEwiseOps : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    const bool any_blargs = toolchain::any_of(
        op->getOperands(),
        [](Value opr) -> bool { return toolchain::isa<BlockArgument>(opr); });
    if (any_blargs) {
      return failure();
    }

    auto tpose_arg1 = toolchain::dyn_cast_or_null<TFL::TransposeOp>(
        op->getOperand(0).getDefiningOp());
    auto tpose_arg2 = toolchain::dyn_cast_or_null<TFL::TransposeOp>(
    ...
    auto tpose_arg1_type =
        toolchain::dyn_cast<RankedTensorType>(tpose_arg1->getResultTypes()[0]);
    auto tpose_arg2_type =
        toolchain::dyn_cast<RankedTensorType>(tpose_arg2->getResultTypes()[0]);
    if (tpose_arg1_type.getRank() != tpose_arg2_type.getRank()) {
      return failure();
    }

    if (toolchain::isa<BlockArgument>(tpose_arg1.getPerm()) ||
        toolchain::isa<BlockArgument>(tpose_arg2.getPerm())) {
      return failure();
    }
    auto perm1 = toolchain::dyn_cast_or_null<arith::ConstantOp>(
        tpose_arg1.getPerm().getDefiningOp());
    auto perm2 = toolchain::dyn_cast_or_null<arith::ConstantOp>(
        tpose_arg2.getPerm().getDefiningOp());
    if (!perm1 || !perm2) {
      return failure();
    }

    auto perm1_value = toolchain::dyn_cast<DenseElementsAttr>(perm1.getValue())
                           .getValues<int32_t>();
    auto perm2_value = toolchain::dyn_cast<DenseElementsAttr>(perm2.getValue())
                           .getValues<int32_t>();

    toolchain::SmallVector<int32_t> perm1_arr(perm1_value.begin(),
                                         perm1_value.end());
    toolchain::SmallVector<int32_t> perm2_arr(perm2_value.begin(),
                                         perm2_value.end());
    ...
    toolchain::SmallVector<int32_t> inverse_perm_arr = InvertPermutation(perm1_arr);
    ...
    auto current_out_type =
        toolchain::dyn_cast<RankedTensorType>(op->getResult(0).getType());
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (Value opr : op->getOperands()) {
      ...
      if (toolchain::isa<TFL::TransposeOp>(defining_op) && !tpose_arg) {
        tpose_arg = toolchain::dyn_cast<TFL::TransposeOp>(defining_op);
        continue;
      }
    ...
    if (toolchain::isa<BlockArgument>(tpose_arg.getPerm())) {
      return failure();
    }
    ...
    auto tpose_arg_type =
        toolchain::dyn_cast<RankedTensorType>(tpose_arg->getResultTypes()[0]);
    auto cst_arg_type =
        toolchain::dyn_cast<RankedTensorType>(cst_arg->getResultTypes()[0]);
    ...
    auto perm = toolchain::dyn_cast_or_null<arith::ConstantOp>(
        tpose_arg.getPerm().getDefiningOp());
    if (!perm) {
      return failure();
    }
    auto perm_value =
        toolchain::dyn_cast<DenseElementsAttr>(perm.getValue()).getValues<int32_t>();
    toolchain::SmallVector<int32_t> perm_arr(perm_value.begin(), perm_value.end());

    // Compute inverse of input transpose.
    toolchain::SmallVector<int32_t> inverse_perm = InvertPermutation(perm_arr);
    ...
    if (other_input_type.getNumElements() == 1) {
      ...
      auto in_rtt =
          toolchain::dyn_cast<RankedTensorType>(cst_arg->getResult(0).getType());
