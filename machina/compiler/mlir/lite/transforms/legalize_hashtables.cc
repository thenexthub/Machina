...
class LegalizeHashTableOpPattern : public OpRewritePattern<TF::HashTableV2Op> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::HashTableV2Op hashtable_op,
    ...
    auto table_id =
        static_cast<int32_t>(::toolchain::hash_value(hashtable_op.getSharedName()));
...
class LegalizeHashTableFindOpPattern
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::LookupTableFindV2Op find_op,
    ...
    auto hashtable_op = toolchain::dyn_cast<TFL::HashtableOp>(handle_op);
...
bool checkWhetherGraphHasValidStaticLookupTables(ModuleOp module) {
  ...
  for (auto hashtable : hashtables) {
    ...
    for (auto& use : hashtable->getUses()) {
      ...
      // hash table kernels.
      if (auto find_op = toolchain::dyn_cast<TF::LookupTableFindV2Op>(user))
        continue;
      if (auto import_op = toolchain::dyn_cast<TF::LookupTableImportV2Op>(user))
        continue;
      if (auto size_op = toolchain::dyn_cast<TF::LookupTableSizeV2Op>(user))
        continue;
