...
#include "machina/compiler/mlir/lite/transforms/lower_quant_annotations_helper.h"
...
static LogicalResult IsDrqTensor(Value value, Value& fq_input) {
  ...
  auto v = value;
  if (auto reshape_op = toolchain::dyn_cast_or_null<ReshapeOp>(v.getDefiningOp())) {
    v = reshape_op.getOperand(0);
  }
  if (auto composite_op =
          toolchain::dyn_cast_or_null<stablehlo::CompositeOp>(v.getDefiningOp())) {
...
static LogicalResult HasDQParent(Value value, Value& dq_input) {
  if (auto dq_op =
          toolchain::dyn_cast_or_null<DequantizeOp>(value.getDefiningOp())) {
...
static OpQuantizationType GetOpQuantizationType(Operation* op) {
  ...
  for (auto result : op->getResults()) {
    for (auto user : result.getUsers()) {
      auto q_op = toolchain::dyn_cast_or_null<QuantizeOp>(user);
      if (!q_op) {
...
class StrictQuantizationPattern : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation* op,
    ...
    for (Operation* quantizing_op : quantizing_ops) {
      ...
      if (toolchain::isa<QuantizeOp, DequantizeOp>(quantizing_op)) {
        return failure();
      }
      ...
      if (QuantizableOpSupportsFloatOutputType(quantizing_op)) {
        ...
        for (const auto& enumerated_result :
          ...
          if (result.hasOneUse() &&
              toolchain::isa<QuantizeOp>(*result.user_begin()) &&
              op_quant_type == OpQuantizationType::kSRQ) {
            auto user = toolchain::cast<QuantizeOp>(*result.user_begin());
      ...
      if (enable_verify && !std::is_same_v<NumericVerifyOp, void>) {
        ...
        for (int i = 0, e = quantized_op->getNumOperands(); i < e; ++i) {
          ...
          if (auto q = toolchain::dyn_cast_or_null<QuantizeOp>(def)) {
 ...
 private:
  bool IsQuantizableCustomOp(Operation* op,
    ...
    auto custom_op = toolchain::dyn_cast_or_null<CustomOp>(op);
  ...
  void RewireFloatModelBackbone(Operation* quantized_op,
    ...
    for (int i = 0, e = quantized_op->getNumResults(); i < e; ++i) {
      ...
      for (auto user : result.getUsers()) {
        ...
        if (toolchain::isa<QuantizeOp>(user)) {
          user = *user->getResult(0).getUsers().begin();
        }
        if (auto dequantize = toolchain::dyn_cast<DequantizeOp>(user)) {
...
struct QuantizePass : public impl::QuantizePassBase<QuantizePass> {
 public:
  ...
  explicit QuantizePass() { quant_specs.inference_type = machina::DT_QINT8; }
...
#include "machina/compiler/mlir/lite/transforms/generated_quantize.inc"

namespace quantize_by_converter_patterns {
#include "machina/compiler/mlir/lite/transforms/generated_quantize_by_converter.inc"
}
