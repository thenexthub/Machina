...
void RaiseCustomOpsPass::runOnOperation() {
  ...
  toolchain::SmallVector<Operation *, 4> custom_ops;
  fn.walk([&](Operation *op) {
    // Skips already imported ops that are imported as CustomTfOp.
    if (op->getParentOfType<CustomTfOp>()) return;
    if (toolchain::isa<TFL::CustomTfOp>(op) || toolchain::isa<TFL::CustomOp>(op)) return;
  ...
  for (auto *op : custom_ops) {
    ...
    for (const auto &idx_args : toolchain::enumerate(new_block->getArguments())) {
