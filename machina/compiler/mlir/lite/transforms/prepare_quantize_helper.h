...
#ifndef MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_PREPARE_QUANTIZE_HELPER_H_
#define MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_PREPARE_QUANTIZE_HELPER_H_
...
#include "machina/compiler/mlir/lite/tools/optimize/operator_property.h"
...
// Same with the ordering of //machina/compiler/mlir/lite/ir/tfl_ops.td
...
LogicalResult GetLstmProperty(LstmOp op,
  ...
  if (toolchain::isa<TFL::LSTMOp>(op.getOperation())) {
    lstm_variant->op_code = tflite::BuiltinOperator_LSTM;
  } else if (toolchain::isa<TFL::UnidirectionalSequenceLSTMOp>(op.getOperation())) {
...
class PrepareLstmOutputScale : public OpRewritePattern<SourceOp> {
 ...
 private:
  ...
  LogicalResult processRestrictScale(SourceOp op, int input_index,
    ...
    toolchain::SmallVector<quantfork::StatisticsOp, 2> stats_ops = {
        toolchain::dyn_cast_or_null<quantfork::StatisticsOp>(
    ...
    toolchain::SmallVector<toolchain::APFloat, 4> min_max_values;

    for (auto& stats_op : stats_ops) {
      auto values =
          mlir::dyn_cast<DenseFPElementsAttr>(stats_op.getLayerStats())
              .getValues<toolchain::APFloat>();
    ...
    mlir::ElementsAttr layer_stats = mlir::DenseFPElementsAttr::get(
        ...
        {toolchain::minimum(min_max_values[0], min_max_values[2]),
         toolchain::maximum(min_max_values[1], min_max_values[3])});
 ...
 protected:
  ...
  LogicalResult processInputs(
    ...
    for (auto& enumerated_inputs : op_property.inputs) {
      ...
      if (toolchain::isa<func::ConstantOp, arith::ConstantOp, TFL::ConstOp>(
              input.getDefiningOp())) {
        ...
        if (auto stats_op = toolchain::dyn_cast<quantfork::StatisticsOp>(
                input.getDefiningOp())) {
          if (failed(replaceStatsOp(op, stats_op, index, tensor_property,
                                    rewriter))) {
            return failure();
          }
        } else if (!toolchain::isa<DQ>(input.getDefiningOp()) &&
                   !toolchain::isa<SameScalesOpInterface, FixedOutputRangeInterface>(
                       input.getDefiningOp())) {
  ...
  LogicalResult processConstantOp(
    ...
    if (tensor_property.number_of_bits == 10) {
      ...
      double scale = maxs[0] / -toolchain::minIntN(tensor_property.number_of_bits);
      quant_type = UniformQuantizedType::getChecked(
          ...
          /*zeroPoint=*/0, toolchain::minIntN(10), -toolchain::minIntN(10));
    } else {
  ...
  LogicalResult replaceStatsOp(
    ...
    if (tensor_property.extend_to_power_of_two) {
      ...
      quant_type = UniformQuantizedType::getChecked(
          ...
          /*scale=*/bound / -toolchain::minIntN(tensor_property.number_of_bits),
          /*zeroPoint=*/0, toolchain::minIntN(tensor_property.number_of_bits),
          toolchain::maxIntN(tensor_property.number_of_bits));
    } else {
...
#endif  // MACHINA_COMPILER_MLIR_LITE_TRANSFORMS_PREPARE_QUANTIZE_HELPER_H_
