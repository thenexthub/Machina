...
class TrimFunctionsPass
 ...
 public:
  ...
  explicit TrimFunctionsPass(toolchain::ArrayRef<std::string> trim_funcs_allowlist) {
...
bool TrimFunctionsPass::TrimModule() {
  ...
  toolchain::SmallVector<func::FuncOp, 4> funcs_to_trim;
  for (auto func : getOperation().getOps<func::FuncOp>()) {
    if (toolchain::is_contained(trim_funcs_allowlist_, func.getName())) {
      ...
      // machina function in MLIR TF import using an entry_point attr.
      if (!toolchain::is_contained(trim_funcs_allowlist_, "main") &&
...
void TrimFunctionsPass::Verify() {
  ...
  toolchain::SetVector<func::FuncOp> reachable_funcs;
  for (auto func : getOperation().getOps<func::FuncOp>()) {
...
std::unique_ptr<OperationPass<ModuleOp>> CreateTrimFunctionsPass(
    toolchain::ArrayRef<std::string> trim_funcs_allowlist) {
