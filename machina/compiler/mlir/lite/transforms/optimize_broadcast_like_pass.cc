...
#include "machina/compiler/mlir/lite/transforms/optimize_broadcast_like_pass.h"
...
using BroadcastedShapeFunction =
    std::function<bool(toolchain::ArrayRef<int64_t>, toolchain::ArrayRef<int64_t>,
...
Value PrepareBroadcastLikeOpInput(Operation* op, PatternRewriter& rewriter) {
  ...
  if (auto broadcast_to_op = toolchain::dyn_cast<TFL::BroadcastToOp>(op)) {
    return broadcast_to_op.getInput();
  }
  if (auto fill_op = toolchain::dyn_cast<TFL::FillOp>(op)) {
...
LogicalResult ConvertResultsBroadcastableShapeOp::RewriteOp(
  ...
  auto result_type = toolchain::cast<ShapedType>(op->getResultTypes().front());
  ...
  for (size_t i = 0, e = op->getNumOperands(); i < e; ++i) {
    ...
    // PReluOp.
    if (toolchain::isa<TFL::PReluOp>(op)) {
      continue;
    }
    ...
    auto broadcast_arg_type =
        toolchain::cast<ShapedType>(broadcast_like_op_input.getType());
    ...
    if (toolchain::isa<TFL::AddOp, TFL::SubOp, TFL::MulOp, TFL::DivOp>(op) &&
    ...
    auto other_arg_type = toolchain::cast<ShapedType>(other_arg.getType());
...
LogicalResult ConvertResultsBroadcastableBatchMatMulShapeOp::RewriteOp(
    Operation* op, PatternRewriter& rewriter) const {
  auto matmul_op = toolchain::dyn_cast<TFL::BatchMatMulOp>(op);
  ...
  BroadcastedShapeFunction get_broadcasted_shape =
      [&](toolchain::ArrayRef<int64_t> shape_x, toolchain::ArrayRef<int64_t> shape_y,
...
LogicalResult ReorderBroadcastToCast::matchAndRewrite(
    Operation* op, PatternRewriter& rewriter) const {
  auto cast_op = toolchain::dyn_cast<TFL::CastOp>(op);
  if (!cast_op) return rewriter.notifyMatchFailure(op, "Not a CastOp");

  auto broadcast_to_op = toolchain::dyn_cast_or_null<TFL::BroadcastToOp>(
...
#include "machina/compiler/mlir/lite/transforms/generated_optimize_broadcast_like.inc"
}  // namespace
