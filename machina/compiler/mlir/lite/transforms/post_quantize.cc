...
void RemoveQuantizationAdaptorOps(func::FuncOp func) {
  ...
  toolchain::SmallVector<Type, 4> input_types;
  ...
  for (int i = 0; i != num_return_operands; ++i) {
    ...
    if (returned_op && returned_op->hasOneUse() &&
        toolchain::isa<DequantizeOp>(returned_op)) {
...
std::optional<mlir::Value> GetConstTensor(PatternRewriter& rewriter,
                                          Location loc, toolchain::ArrayRef<T> vec,
...
std::optional<mlir::Value> ConvertDequantizeOp(
    ...
    toolchain::ArrayRef<double> scale, toolchain::ArrayRef<int64_t> zeropoint,
    int64_t dim) {
  ...
  if (zeropoint.size() == 1) {
    auto const_type =
        machina::GetTypeFromTFTensorShape({}, rewriter.getF32Type());
    auto const_attr =
...
struct RemoveVolatileOps : public OpRewritePattern<DequantizeOp> {
  ...
  LogicalResult matchAndRewrite(DequantizeOp op,
    ...
    if (auto q = toolchain::dyn_cast_or_null<QuantizeOp>(input_op)) {
      ...
      return success();
    } else if (auto qconst_op = toolchain::dyn_cast_or_null<QConstOp>(input_op)) {
...
struct FoldTransposeOp : public OpRewritePattern<TransposeOp> {
  ...
  void ComputePermutation(ElementsAttr input_tensor, ArrayRef<int32_t> perm,
    ...
    // machina/lite/kernels/internal/reference/reference_ops.h
  ...
  LogicalResult matchAndRewrite(TransposeOp op,
    ...
    auto qconst_op = toolchain::dyn_cast_or_null<QConstOp>(def_op);
...
struct PruneUnusedOpsWithSideEffect : public OpRewritePattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(OpTy op,
    ...
    auto custom_op = toolchain::isa<CustomOp>(op);
    if (custom_op) {
      auto q = toolchain::cast<CustomOp>(op);
...
#include "machina/compiler/mlir/lite/transforms/generated_post_quantize.inc"
