...
void UpdateFuncType(func::FuncOp func) {
  ...
  if (toolchain::ArrayRef(return_types) == func_type.getResults()) return;
...
class FoldIfOp : public OpRewritePattern<TF::IfOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::IfOp op,
    ...
    if (!toolchain::hasSingleElement(parent_op)) return failure();
    ...
    if (!toolchain::hasSingleElement(func)) return failure();

    IRMapping mapper;
    for (int i = 0, e = func.getNumArguments(); i != e; ++i)
      mapper.map(func.getArgument(i), op.getOperand(i + 1));

    toolchain::SmallVector<Value, 4> updated_results;
