...
#include "mlir/Dialect/Affine/Analysis/LoopAnalysis.h"  // part of Codira Toolchain
...
#include "machina/core/kernels/tensor_list.h"
...
Value CreateI32SplatConst(Location loc, PatternRewriter *rewriter,
  ...
  RankedTensorType type =
      machina::GetTypeFromTFTensorShape(shape, rewriter->getIntegerType(32));
...
Value CreateI64SplatConst(Location loc, PatternRewriter *rewriter,
  ...
  RankedTensorType type =
      machina::GetTypeFromTFTensorShape(shape, rewriter->getIntegerType(64));
  DenseElementsAttr attr =
...
Value CreateI32SplatTensor(Location loc, PatternRewriter *rewriter,
  ...
  return rewriter->create<TF::FillOp>(
      loc,
      machina::GetTypeFromTFTensorShape({-1}, rewriter->getIntegerType(32)),
...
Type PrependLeadingDimIfRanked(int64_t dim, Type type,
  ...
  if (RankedTensorType ty = toolchain::dyn_cast<RankedTensorType>(type)) {
    toolchain::SmallVector<int64_t, 4> shape = {dim};
    shape.append(ty.getShape().begin(), ty.getShape().end());
    return machina::GetTypeFromTFTensorShape(shape, dtype);
  }
  return type;
}
...
// function.
toolchain::SmallSet<int, 4> GetResizedTensorListIndexes(
    func::FuncOp func, const toolchain::SmallSet<int, 4> &tensor_list_args) {
  // `indexes` stores the argument index of tensorlists which size may get
  // updated in the function.
  toolchain::SmallSet<int, 4> indexes;
  for (BlockArgument &arg : func.getArguments()) {
    if (tensor_list_args.contains(arg.getArgNumber())) {
      for (const mlir::OpOperand &use : arg.getUses()) {
        ...
        if (toolchain::isa<TF::TensorListPushBackOp>(op) ||
            toolchain::isa<TF::TensorListResizeOp>(op)) {
...
TF::SliceOp CreateSliceOpForTensorList(Location loc, Value input_list,
  ...
  RankedTensorType position_type =
      machina::GetTypeFromTFTensorShape({-1}, shape_dtype);
...
struct ConvertConst : public OpConversionPattern<TF::ConstOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    auto proto_attr = toolchain::dyn_cast<tf_type::TensorProtoAttr>(op.getValue());
    if (!proto_attr) return failure();
    machina::Tensor tensor;
    if (!machina::ConvertToTensor(proto_attr, &tensor).ok())
      return failure();
    if (tensor.dtype() != machina::DT_VARIANT) return failure();
    if (!machina::TensorShapeUtils::IsScalar(tensor.shape()))
      return failure();

    const machina::TensorList *list =
        tensor.scalar<machina::Variant>()().get<machina::TensorList>();
    if (!list) return failure();

    // Verify output type is variant and contains exactly one ranked subtypes.
    auto variant_ty = toolchain::dyn_cast<tf_type::VariantType>(
    ...
    RankedTensorType list_element_ty =
        toolchain::dyn_cast<RankedTensorType>(subtypes.front());
    ...
    const std::vector<machina::Tensor> &tensors = list->tensors();
    toolchain::SmallVector<int64_t, 4> result_shape = {
        static_cast<int64_t>(tensors.size())};
    result_shape.append(list_element_ty.getShape().begin(),
                        list_element_ty.getShape().end());
    auto result_ty = machina::GetTypeFromTFTensorShape(
    ...
    if (tensors.empty()) {
      machina::Tensor tensor(list->element_dtype,
                                machina::TensorShape(result_shape));
      auto attr_or = machina::ConvertTensor(tensor, &rewriter);
    ...
    toolchain::SmallVector<Value, 4> values;
    values.reserve(tensors.size());
    for (const machina::Tensor &tensor : tensors) {
      auto attr_or = machina::ConvertTensor(tensor, &rewriter);
      if (!attr_or.ok()) return failure();
...
struct ConvertTensorListSetItem
  ...
  LogicalResult matchAndRewriteImplWithSliceAndConcat(
    ...
    auto item_rank = rewriter.create<TF::RankOp>(
        loc, machina::GetTypeFromTFTensorShape({}, shape_dtype), item);
    Value scalar_zero = CreateI32SplatConst(loc, &rewriter, {}, 0);
    ...
    auto item_position_shape = rewriter.create<TF::ExpandDimsOp>(
        loc, machina::GetTypeFromTFTensorShape({1}, shape_dtype), item_rank,
        scalar_zero);
    // Create two slice ops.
    Type element_type =
        toolchain::cast<TensorType>(input.getType()).getElementType();
...
struct ConvertTensorListInitOp : public TensorListOpConverterBase<OpT> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (auto shaped_type =
            toolchain::dyn_cast<ShapedType>(element_shape.getType())) {
      if (shaped_type.hasRank() && shaped_type.getRank() == 0) {
        bool element_shape_acquired = false;
        auto uses = op.getResult().getUses();
        for (auto &use : toolchain::make_early_inc_range(uses)) {
          if (TF::TensorListSetItemOp set_op =
                  toolchain::dyn_cast<TF::TensorListSetItemOp>(use.getOwner())) {
            ...
            element_shape_acquired = true;
                         ...
                         toolchain::dyn_cast<TF::WhileOp>(use.getOwner())) {
            ...
            auto inside_uses = while_op.body_function()
                                   ...
                                   .getUses();
            for (auto &inside_use : toolchain::make_early_inc_range(inside_uses)) {
              if (TF::TensorListSetItemOp set_op =
                      toolchain::dyn_cast<TF::TensorListSetItemOp>(
                          inside_use.getOwner())) {
                if (auto shaped_type = toolchain::dyn_cast<ShapedType>(
    ...
    if (matchPattern(element_shape, m_Constant(&dense_elem_attr))) {
      ...
      auto attr = DenseIntElementsAttr::get(
          toolchain::cast<ShapedType>(element_shape.getType()),
    ...
    if (auto element_type =
            toolchain::dyn_cast<RankedTensorType>(op.element_type())) {
      ...
      result_type =
          machina::GetTypeFromTFTensorShape(result_shape, element_dtype);
    }
    ...
    Type shape_type =
        machina::GetTypeFromTFTensorShape({result_rank}, shape_dtype);
    ...
    RankedTensorType zero_type =
        machina::GetTypeFromTFTensorShape({}, element_dtype);
  ...
  Value GetNumElements(TF::TensorListReserveOp op, ValueRange operands,
    ...
    if (auto const_op = num_elements.getDefiningOp<TF::ConstOp>()) {
      return CreateI32SplatConst(
          ...
          (*toolchain::cast<DenseElementsAttr>(const_op.getValue())
                .getValues<APInt>()
                .begin())
              .getSExtValue());
    }
    return rewriter->create<TF::ExpandDimsOp>(
        op.getLoc(), machina::GetTypeFromTFTensorShape({1}, shape_dtype),
...
struct ConvertTensorListPushBack
  ...
  LogicalResult matchAndRewrite(
    ...
    auto handle_dtype = toolchain::cast<tf_type::VariantType>(
...
struct ConvertTensorListResize
  ...
  LogicalResult matchAndRewrite(
    ...
    auto input_size = rewriter.create<TF::TensorListLengthOp>(
        loc, machina::GetTypeFromTFTensorShape({}, shape_dtype),
        op.getOperand(0));
 ...
 private:
  ...
  void CreateCondFalseBranch(Location loc, Type shape_dtype, Type result_type,
    ...
    auto input_rank = rewriter->create<TF::RankOp>(
        loc, machina::GetTypeFromTFTensorShape({}, shape_dtype), input);
    auto partial_position_shape = rewriter->create<TF::SubOp>(
        loc, machina::GetTypeFromTFTensorShape({1}, shape_dtype), input_rank,
        vector_one);
    auto slice_op =
        CreateSliceOpForTensorList(loc, /*input_list=*/input,
...
struct ConvertTensorListStack
  ...
  LogicalResult matchAndRewrite(
    ...
    auto ranked_type =
        toolchain::dyn_cast<RankedTensorType>(element_shape.getType());
    DenseIntElementsAttr dense_elem_attr;
    ...
    RankedTensorType result_type = machina::GetTypeFromTFTensorShape(
...
struct ConvertTensorListConcatV2
  ...
  LogicalResult matchAndRewrite(
    ...
    if (auto type = toolchain::dyn_cast<RankedTensorType>(input.getType())) {
    ...
    toolchain::SmallVector<Type, 1> unpack_output_type;
    unpack_output_type.insert(
        unpack_output_type.begin(), num_unpacked,
        machina::GetTypeFromTFTensorShape(output_shape, input_element_type));
...
Type VariantToUnrankedTensorType(Type type, Value value) {
  TF::VariantType variant_ty =
      toolchain::dyn_cast<tf_type::VariantType>(getElementTypeOrSelf(type));
  if (!variant_ty) {
    return type;
  }
  ...
  Type element_type;
  variant_ty = toolchain::dyn_cast<tf_type::VariantType>(value_type);
...
bool IsTensorListType(Type type, std::optional<Value> value) {
  ...
  for (const mlir::OpOperand &use : value.value().getUses()) {
    mlir::Operation *op = use.getOwner();
    if (toolchain::isa<TF::TensorListGetItemOp>(op) ||
        toolchain::isa<TF::TensorListLengthOp>(op) ||
        toolchain::isa<TF::TensorListPushBackOp>(op) ||
        toolchain::isa<TF::TensorListReserveOp>(op) ||
        toolchain::isa<TF::TensorListSetItemOp>(op) ||
        toolchain::isa<TF::TensorListStackOp>(op) ||
        toolchain::isa<TF::TensorListResizeOp>(op)) {
      return true;
    }
  }
...
// the function.
toolchain::SmallSet<int, 4> GetTensorListArgumentsIndex(func::FuncOp func) {
  toolchain::SmallSet<int, 4> set;
  for (const auto &arg_and_idx : toolchain::enumerate(func.getArguments())) {
...
// function.
toolchain::SmallSet<int, 4> GetTensorListResultsIndex(func::FuncOp func) {
  toolchain::SmallSet<int, 4> set;

  for (const auto &result_and_idx :
       toolchain::enumerate(func.getFunctionType().getResults())) {
...
void UpdateTensorListTypes(
    const toolchain::SmallSet<int, 4> &tensor_list_index,
    const toolchain::SmallSet<int, 4> &resized_tensor_list_index,
    ArrayRef<Type> types, R &&range, ValueRange operands,
    toolchain::SmallVectorImpl<Type> *updated_types) {
  int i = 0;
  for (const auto it : toolchain::zip(types, range, operands)) {
...
void ChangeVariantToUnrankedTensorType(
    const toolchain::SmallSet<int, 4> &tensor_list_index, ArrayRef<Type> types,
    R &&range, toolchain::SmallVectorImpl<Type> *updated_types) {
  int i = 0;
  for (const auto it : toolchain::zip(types, range)) {
    if (tensor_list_index.count(i)) {
      updated_types->push_back(
          VariantToUnrankedTensorType(std::get<0>(it), std::get<1>(it)));
    } else {
...
void UpdateFunctionAndRegionType(ConversionPatternRewriter &rewriter,
                                 func::FuncOp func,
                                 toolchain::ArrayRef<Type> updated_argument_types,
                                 toolchain::ArrayRef<Type> updated_result_types) {
...
LogicalResult UpdateFunctionTypesForWhileOp(
    ...
    const toolchain::SmallSet<int, 4> &tensor_list_args,
    const toolchain::SmallSet<int, 4> &resized_tensor_lists) {
  int func_index = 0;
...
// Returns a `toolchain::DenseMap` which maps from the index of tensorlist in the
...
toolchain::DenseMap<int, int> MapTensorListResultToArgument(func::FuncOp func) {
  ...
  toolchain::SmallVector<Value, 4> returns;
  for (auto res : func.getBody().back().getTerminator()->getOperands()) {
    returns.push_back(res);
  }
  toolchain::DenseMap<int, int> result;
  for (const auto &result_and_idx : toolchain::enumerate(returns)) {
...
void UpdateTensorListResultTypesForIf(
    const toolchain::SmallSet<int, 4> &tensor_list_index,
    const toolchain::SmallSet<int, 4> &resized_tensor_list_index,
    const toolchain::DenseMap<int, int> &tensor_list_map, ArrayRef<Type> types,
...
struct ConvertIf : public OpConversionPattern<TF::IfOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallSet<int, 4> resized_tensor_lists =
        GetResizedTensorListIndexes(op.else_function(), tensor_list_args);

    toolchain::SmallVector<Type, 8> result_types;
    result_types.reserve(op.getNumResults());
    toolchain::SmallVector<Type, 4> op_result_types;
...
struct ConvertWhileRegion : public OpConversionPattern<TF::WhileRegionOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    result_types.reserve(op.getNumOperands());
    // Change all DT_VARIANT result types to unranked tensor type.
    for (auto it : toolchain::zip(op.getResultTypes(), adaptor.getOperands()))
    ...
    for (auto it : toolchain::zip(op.getRegions(), converted.getRegions())) {
      ...
      for (auto it : toolchain::zip(entry.getArguments(), adaptor.getOperands())) {
...
#include "machina/compiler/mlir/lite/transforms/generated_lower_static_tensor_list.inc"
...
void LowerStaticTensorListPass::runOnOperation() {
  ...
  auto is_legal = [](Operation *op) {
    ...
    return toolchain::all_of(op->getOperandTypes(), is_not_variant) &&
           toolchain::all_of(op->getResultTypes(), is_not_variant);
  };

  auto is_set_item_legal = [](Operation *op) {
    return op->hasAttr("resize_if_index_out_of_bounds") &&
           toolchain::cast<BoolAttr>(op->getAttr("resize_if_index_out_of_bounds"))
               .getValue();
  };
