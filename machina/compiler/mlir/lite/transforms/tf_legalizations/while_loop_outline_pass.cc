...
func::FuncOp CreateOutlineFunc(StringRef name, Region& region,
                               ...
                               const toolchain::SetVector<Value>& extern_values,
  ...
  toolchain::SmallVector<Value, 4> new_args;
  ...
  toolchain::SmallVector<Value, 4> args;
  ...
  if (passthru_extra_args) {
    ...
    for (auto it : toolchain::zip_first(loop_carried_yield_operands, types)) {
...
void ReplaceRegionWithCall(StringRef name, Region& region,
  ...
  for (Type t : toolchain::ArrayRef(types).drop_back(extern_values.size()))
...
void WhileOutlinePass::OutlineWhile(WhileOp while_op) {
  ...
  toolchain::SmallVector<Region*, 2> regions{&while_op.getCond(),
                                        &while_op.getBody()};
  for (const auto& it : toolchain::enumerate(regions)) {
    ...
    for (auto extern_value : region_extern_values) {
      if (!matchPattern(extern_value, m_Constant()) &&
          !toolchain::dyn_cast_or_null<TFL::QConstOp>(
  ...
  toolchain::SetVector<Value> extra_operands;
