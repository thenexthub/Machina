...
#include "machina/compiler/mlir/lite/transforms/prepare_quantize_helper.h"  // IWYU pragma: keep
...
void DefaultQuantParamsPass::AddToWorkListIfUnquantized(
  ...
  if (value.hasOneUse() &&
      toolchain::isa<TFL::QuantizeOp>(*value.getUsers().begin()))
    return;
...
QuantParams DefaultQuantParamsPass::GetQuantParamsForBias(
  ...
  for (int non_bias : non_biases) {
    ...
    if (auto dequant = toolchain::dyn_cast<TFL::DequantizeOp>(non_bias_define)) {
