...
#include "machina/compiler/mlir/lite/utils/fake_quant_utils.h"
...
#include "machina/compiler/mlir/machina/transforms/einsum.h"
#include "machina/compiler/mlir/machina/transforms/passes.h"
#include "machina/compiler/mlir/machina/transforms/unroll_batch_matmul.h"
#include "machina/compiler/mlir/machina/utils/dynamic_shape_utils.h"
#include "machina/compiler/mlir/machina/utils/verification_utils.h"
...
class ConvertTFConvOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto bias_type =
        machina::GetTypeFromTFTensorShape({bias_dim}, elem_type);
    ...
    if (op->getAttrOfType<StringAttr>("padding").getValue() == "EXPLICIT") {
      ...
      RankedTensorType padding_attr_type = machina::GetTypeFromTFTensorShape(
...
class ConvertTFConv2D : public ConvertTFConvOp<ConvertTFConv2D, TF::Conv2DOp> {
 ...
 private:
  ...
  Value legalizeFilter(PatternRewriter &rewriter, Location loc,
    ...
    auto perm_type = machina::GetTypeFromTFTensorShape(
        {static_cast<int>(perm.size())}, rewriter.getIntegerType(32));
    auto perm_attr =
        DenseElementsAttr::get(perm_type, toolchain::ArrayRef<int>(perm));
    ...
    auto result_shape =
        toolchain::to_vector<4>(toolchain::map_range(perm, [filter_type](int64_t dim) {
          return filter_type.getDimSize(dim);
        }));
    auto elem_type = filter_type.getElementType();
    auto result_type =
        machina::GetTypeFromTFTensorShape(result_shape, elem_type);
...
class ConvertTFDepthwiseConv2dNative
 ...
 private:
  ...
  Value legalizeFilter(PatternRewriter &rewriter, Location loc,
    ...
    auto shape_type =
        machina::GetTypeFromTFTensorShape({4}, rewriter.getIntegerType(32));
...
struct ConvertTFStridedSlice : public RewritePattern {
  ...
  LogicalResult RewriteNewAxisMask(Operation *op,
    ...
    TF::StridedSliceOp strided_slice_op = toolchain::cast<TF::StridedSliceOp>(op);
    ...
    auto revised_output_type = machina::GetTypeFromTFTensorShape(
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto begin_end_type = machina::GetTypeFromTFTensorShape(
    ...
    auto strides_type = machina::GetTypeFromTFTensorShape(
...
struct FusedBatchNormV3Pat : public ::mlir::RewritePattern {
  ...
  ::mlir::LogicalResult matchAndRewrite(
    ...
    if (is_training.getValue()) {
      ...
      {
        auto reduce_dim_type = machina::GetTypeFromTFTensorShape(
      ...
      auto new_mean_type = machina::GetTypeFromTFTensorShape(
    ...
    ::toolchain::SmallVector<::mlir::Value, 4> replace_values;
    ...
    for (auto v :
         ::toolchain::SmallVector<::mlir::Value, 4>{add_op_2.getODSResults(0)}) {
      replace_values.push_back(v);
    }
    for (auto v : ::toolchain::SmallVector<::mlir::Value, 4>{x}) {
      replace_values.push_back(v);
    }
    for (auto v : ::toolchain::SmallVector<::mlir::Value, 4>{x}) {
      replace_values.push_back(v);
...
static LogicalResult static_dag_matcher(
    ...
    ::toolchain::SmallVector<Operation *, 4> &target_ops,
  ...
  auto fakequant_op = ::toolchain::dyn_cast<TF::FakeQuantWithMinMaxVarsOp>(op);
  if (!(fakequant_op)) {
...
struct ReorderFakeQuantPattern : public RewritePattern {
  ...
  LogicalResult findTargetOps(
      ...
      Operation::operand_range &input, IntegerAttr &num_bits,
      BoolAttr &narrow_range,
      ::toolchain::SmallVector<Operation *, 4> &target_ops) const {
  ...
  LogicalResult createFakeQuantOp(
    ...
    ::toolchain::SmallVector<Value, 4> target_values;
    ::toolchain::SmallVector<NamedAttribute, 4> target_attrs;
    ...
    ::toolchain::SmallVector<Type, 4> target_types;
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    ReorderOp old_reorder_op = ::toolchain::dyn_cast<ReorderOp>(op);
    ...
    ::toolchain::SmallVector<Value, 4> target_repl_values;
    ...
    for (auto v :
         ::toolchain::SmallVector<Value, 4>{new_fakequant_op.getODSResults(0)}) {
...
#include "machina/compiler/mlir/lite/transforms/generated_prepare_tf.inc"
...
LogicalResult ConvertTf2XlaOps(func::FuncOp func, MLIRContext *context) {
  ...
  mhlo::PopulateLegalizeTfWithTf2XlaPatterns("MACHINA_MACHINA_XLA_CPU_JIT", patterns, context,
                                             converter);
...
struct ConvertRfftToRfft2d : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto expaned_input_type = machina::GetTypeFromTFTensorShape(
    ...
    auto expanded_fft_len_type = machina::GetTypeFromTFTensorShape(
    ...
    // Insert the rfft_2d.
    auto rfft2d_out_type = machina::GetTypeFromTFTensorShape(
...
toolchain::FailureOr<TF::FakeQuantWithMinMaxVarsOp> TryGetAncestorFakeQuantOp(
    Operation *operand) {
  if (auto fq =
...
class QuantizeConcatResult : public OpRewritePattern<TF::ConcatV2Op> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::ConcatV2Op concat,
    ...
    // fake quants.
    toolchain::SmallVector<TF::FakeQuantWithMinMaxVarsOp> fake_quant_ops;
    ...
    toolchain::SmallVector<OpOperand *> uses;
    for (OpOperand &use : concat_result.getUses()) {
      uses.push_back(&use);
    }

    toolchain::SmallVector<Value, 4> inputs{concat_result, min_v, max_v};
...
class QuantizeMeanResult : public OpRewritePattern<TF::MeanOp> {
 public:
  ...
  LogicalResult matchAndRewrite(TF::MeanOp mean,
    ...
    toolchain::SmallVector<Value, 4> inputs{mean_result, fq.getMin(), fq.getMax()};
