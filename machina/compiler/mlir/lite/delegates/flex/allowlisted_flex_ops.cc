...
const std::set<std::string>& GetFlexAllowlist() {
  ...
  // LINT.ThenChange(//machina/lite/g3doc/guide/op_select_allowlist.md)
...
bool IsAllowedTFTextOpForFlex(const std::string& op_name) {
  ...
  return machina::OpRegistry::Global()->LookUp(op_name) != nullptr;
}
...
bool IsAllowlistedFlexOp(const std::string& machina_op_name) {
  if (GetFlexAllowlist().count(machina_op_name) != 0) return true;

  // Check if the op is an allowlisted tf.text or sentencepiece op.
  return IsAllowedTFTextOpForFlex(machina_op_name) ||
         IsAllowedSentencePieceOpForFlex(machina_op_name);
}
