...
auto* mlir_function_pass_fallback_count = monitoring::Counter<1>::New(
    /* metric name */ "/machina/core/mlir_function_pass_fallback_count",
    /* metric description */
...
auto* mlir_graph_optimization_pass_fallback_count = monitoring::Counter<1>::New(
    /* metric name */
    "/machina/core/mlir_graph_optimization_pass_fallback_count",
    /* metric description */
...
auto* mlir_function_pass_graph_conversion_count = monitoring::Counter<1>::New(
    /* metric name */
    "/machina/core/mlir_function_pass_graph_conversion_count",
    /* metric description */
...
auto* mlir_v1_compat_graph_conversion_count = monitoring::Counter<1>::New(
    /* metric name */
    "/machina/core/mlir_v1_compat_graph_conversion_count",
    /* metric description */
...
static void DumpModule(mlir::ModuleOp module, std::string file_prefix) {
  ...
  prefix += "/" + file_prefix;
  if (!machina::Env::Default()->CreateUniqueFileName(&prefix, ".mlir")) {
...
absl::Status MlirFunctionOptimizationPass::Run(
  ...
  auto module_ref_status = machina::tf2xla::v2::ConvertGraphToTfExecutor(
  ...
  for (auto& pass_registration : registry_->passes()) {
    toolchain::StringRef name = pass_registration.pass->name();
    ...
    if (DEBUG_DATA_DUMPER()->ShouldDump(function_name, kDebugGroupMain) ||
      ...
      ::machina::DumpMlirOpToFile(
          DEBUG_DATA_DUMPER()->GetDumpFilename(
              ...
              toolchain::formatv("mlir_{0}_before", name)),
          *module_ref, toolchain::StringRef(), nullptr);
    }
    ...
    if (DEBUG_DATA_DUMPER()->ShouldDump(function_name, kDebugGroupMain) ||
        VLOG_IS_ON(1)) {
      ::machina::DumpMlirOpToFile(DEBUG_DATA_DUMPER()->GetDumpFilename(
                                         function_name, kDebugGroupMain,
                                         toolchain::formatv("mlir_{0}_after", name)),
                                     *module_ref, toolchain::StringRef(), nullptr);
    }
  }
  ...
  // resulted graph.
  absl::Status status = machina::tf2xla::v2::ConvertTfExecutorToGraph(
...
absl::Status MlirV1CompatGraphOptimizationPass::Run(
  ...
  auto module_ref_status = machina::tf2xla::v2::ConvertGraphToTfExecutor(
      **options.graph, debug_info, *options.flib_def, import_config, &context,
      ...
      machina::TF2XLABridgeVersion::kV1Compat);
  ...
  auto module_ref_clone = module_ref->clone();
  toolchain::StringRef name = pass->name();
  VLOG(2) << "Run MLIR V1 graph optimization pass: " << StringRefToView(name);

  if (VLOG_IS_ON(1)) {
    DumpModule(*module_ref, toolchain::formatv("mlir_{0}_before_", name));
  }
  ...
  if (VLOG_IS_ON(1)) {
    DumpModule(*module_ref, toolchain::formatv("mlir_{0}_after_", name));
  }
