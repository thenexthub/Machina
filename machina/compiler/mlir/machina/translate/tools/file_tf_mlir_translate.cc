...
static absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> GraphdefToMlirImport(
  ...
  GraphDef graphdef;
  TF_RETURN_IF_ERROR(
      machina::LoadProtoFromBuffer({input.data(), input.size()}, &graphdef));
  ...
  if (specs.prune_unused_nodes) {
    ...
    TF_RETURN_IF_ERROR(machina::grappler::SetTransitiveFaninGraph(
      ...
      for (auto& inst : bb) {
        ...
        if (auto attr = inst.getAttrOfType<mlir::ElementsAttr>(attr_id)) {
          ...
          auto new_attr = mlir::DenseElementsAttr::get(
              toolchain::cast<mlir::ShapedType>(attr.getType()), rand_val);
