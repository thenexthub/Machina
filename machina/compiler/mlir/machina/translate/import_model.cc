...
#include "machina/cc/saved_model/loader_util.h"
...
absl::StatusOr<std::string> GetDenseTensorNameFromTensorInfo(
  ...
  TF_RET_CHECK(tensor_info.encoding_case() == machina::TensorInfo::kName)
...
class ObjectNames {
 public:
  ...
  // this node.
  toolchain::ArrayRef<toolchain::StringRef> GetExportedNames(int node_id) const;

  // Gets the name in the module symbol table for this node.
  // This name is only used for internal IR references.
  toolchain::StringRef GetSymbolTableName(int node_id) const;
 ...
 private:
  ...
  toolchain::StringRef SaveString(const std::string& s) const;
  ...
  // during traversal.
  toolchain::SmallVector<std::string, 8> path_segments_;
  ...
  absl::flat_hash_map<int, toolchain::SmallVector<toolchain::StringRef, 1>>
      exported_names_;
  // Key: node_id
  // Value: pretty symbol table name to use for internal references to this
  // object.
  absl::flat_hash_map<int, toolchain::StringRef> pretty_symbol_table_name_;
...
toolchain::ArrayRef<toolchain::StringRef> ObjectNames::GetExportedNames(
    int node_id) const {
...
toolchain::StringRef ObjectNames::GetSymbolTableName(int node_id) const {
...
toolchain::StringRef ObjectNames::SaveString(const std::string& s) const {
  return toolchain::StringRef(*saved_strings_.insert(s).first);
}
  ...
  for (int node_id = 0; node_id < object_graph.nodes_size(); node_id++) {
    ...
    if (object.kind_case() == SavedObject::kFunction) {
      ...
      if (object.function().concrete_functions_size() != 1) {
        toolchain::SmallVector<std::string, 4> names;
        for (toolchain::StringRef s : object_names.GetExportedNames(node_id)) {
...
class StructuredValueLinearizer {
 public:
  ...
  absl::StatusOr<toolchain::ArrayRef<mlir::ArrayAttr>> GetLeafIndexPaths(
      toolchain::StringRef error_context) const;
 ...
 private:
  ...
  // StructuredValue.
  toolchain::SmallVector<mlir::Attribute, 4> current_index_path_;
  // The list of leaf index paths we have discovered so far.
  toolchain::SmallVector<mlir::ArrayAttr, 4> leaf_index_paths_;
...
absl::StatusOr<toolchain::ArrayRef<mlir::ArrayAttr>>
StructuredValueLinearizer::GetLeafIndexPaths(
    toolchain::StringRef error_context) const {
  if (error_message_.empty()) {
    return toolchain::ArrayRef(leaf_index_paths_);
  }
  return errors::InvalidArgument(
...
void StructuredValueLinearizer::RecursivelyFindLeaves(
  ...
  switch (value.kind_case()) {
    case StructuredValue::kDictValue: {
      ...
      toolchain::SmallVector<const FieldTy*, 4> fields;
      for (auto& field : dict.fields()) {
        fields.push_back(&field);
      }
      toolchain::sort(fields, [](const FieldTy* a, const FieldTy* b) {
    ...
    default: {
      toolchain::raw_string_ostream os(error_message_);
...
void AdjustBoundInputArgTypes(mlir::ModuleOp module) {
  ...
  for (auto func : module.getOps<mlir::func::FuncOp>()) {
    ...
    toolchain::SmallVector<mlir::Type, 4> new_input_types;
...
void SortSavedModelModule(mlir::ModuleOp module) {
  ...
  toolchain::SmallVector<NamedGlobalTensor, 8> named_global_tensors;
  ...
  toolchain::stable_sort(named_global_tensors,
  ...
  toolchain::SmallVector<NamedFunc, 8> named_funcs;
  toolchain::SmallVector<mlir::func::FuncOp, 8> private_funcs;
  for (auto func : module.getOps<mlir::func::FuncOp>()) {
  ...
  toolchain::stable_sort(named_funcs, [](const NamedFunc& a, const NamedFunc& b) {
    return a.name < b.name;
  });
  toolchain::stable_sort(private_funcs,
  ...
  toolchain::SmallVector<NamedAsset, 4> assets;
  for (auto asset : module.getOps<AssetOp>()) {
    assets.push_back({asset.getName(), asset});
  }
  toolchain::stable_sort(assets, [](const NamedAsset& a, const NamedAsset& b) {
    return a.name < b.name;
  });

  // Move onto the front of the module in reverse of the final desired order.
  for (auto func : toolchain::reverse(private_funcs)) {
    func.getOperation()->moveBefore(&module.getBody()->front());
  }
  for (auto named_func : toolchain::reverse(named_funcs)) {
    named_func.func.getOperation()->moveBefore(&module.getBody()->front());
  }
  for (auto named_global_tensor : toolchain::reverse(named_global_tensors)) {
  ...
  for (int node_id = 0; node_id < object_graph.nodes_size(); node_id++) {
    ...
    if (object.kind_case() == SavedObject::kFunction) {
      ...
      if (!mlir::SymbolTable::symbolKnownUseEmpty(orig_func.getSymNameAttr(),
        ...
        toolchain::SmallVector<mlir::Value, 4> args_as_values;
      ...
      // https://github.com/machina/machina/blob/8cb8627abb5ef83a6fba34f8fd0e4ee430562eb1/machina/python/eager/function.py#L2027-L2030
      ...
      for (const auto& index_path : toolchain::enumerate(input_index_paths)) {
        func.setArgAttr(index_path.index(), kTfSavedModelIndexPathAttr,
                        index_path.value());
      }

      for (const auto& bound_input :
           toolchain::enumerate(concrete_function.bound_inputs())) {
      ...
      for (const auto& index_path : toolchain::enumerate(output_index_paths)) {
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> ConvertSavedModelObjectGraph(
  ...
  TF_ASSIGN_OR_RETURN(module, machina::tf2xla::v2::ConvertGraphToTfExecutor(
...
class SimpleSavedModelMLIRImportInput : public SavedModelMLIRImportInput {
 ...
 private:
  bool CheckGraphContainsFeedsAndFetches(const GraphImportConfig& specs) const {
    ...
    for (const auto& output : toolchain::concat<const std::string>(
...
class SavedModelSignatureDefImporterLite {
 ...
 private:
  ...
  absl::Status ConvertInitializer(const std::string& target_node_name,
                                  ...
                                  toolchain::StringRef initializer_type);
  ...
  absl::StatusOr<GraphImportConfig::InputArrays> ParseInputArrays(
      toolchain::ArrayRef<std::pair<std::string, TensorInfo>> inputs);
...
absl::Status SavedModelSignatureDefImporterLite::MoveConvertedFunctionsToModule(
  ...
  for (auto func : sub_module.getOps<mlir::func::FuncOp>()) {
    ...
    if (mlir::failed(sub_module_symbol_table.replaceAllSymbolUses(
      ...
      return machina::errors::InvalidArgument(absl::StrCat(
...
absl::Status SavedModelSignatureDefImporterLite::ConvertInitializer(
  ...
  for (const auto& iter : toolchain::enumerate(assets)) {
...
SavedModelSignatureDefImporterLite::ConvertGraph(
  ...
  return machina::tf2xla::v2::ConvertGraphToTfExecutor(
...
absl::Status SavedModelSignatureDefImporterLite::ConvertSignature(
  ...
  toolchain::sort(inputs, [](const auto& lhs, const auto& rhs) {
    return machina::Fingerprint64(lhs.first) <
           machina::Fingerprint64(rhs.first);
  });
  std::vector<std::pair<std::string, TensorInfo>> outputs(
      signature_def.outputs().begin(), signature_def.outputs().end());
  toolchain::sort(outputs, [](const auto& lhs, const auto& rhs) {
    return machina::Fingerprint64(lhs.first) <
           machina::Fingerprint64(rhs.first);
  });
...
SavedModelSignatureDefImporterLite::ConvertSignatures() {
  ...
  toolchain::DenseSet<toolchain::StringRef> exported_name_set;
  ...
  mlir::OpBuilder builder(module_->getBodyRegion());
  toolchain::SmallVector<mlir::Attribute, 2> init_sym_refs;
...
class SavedModelSignatureDefImporter {
 public:
  ...
  static absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> Convert(
      ...
      std::optional<absl::Span<const std::string>> exported_names,
      mlir::MLIRContext* context, machina::MLIRImportOptions options) {
...
std::string MlirModuleToString(mlir::ModuleOp module,
  ...
  {
    toolchain::raw_string_ostream os{txt_module};
