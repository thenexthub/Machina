...
SavedModelObjectGraphToMlirImport(absl::string_view saved_model_dir,
  ...
  auto load_status = machina::SavedModelV2Bundle::Load(
...
SavedModelSignatureDefsToMlirImport(
  ...
  std::unique_ptr<machina::SavedModelBundle> bundle;
  if (saved_model_bundle == nullptr) {
    bundle = std::make_unique<machina::SavedModelBundle>();
  } else if (*saved_model_bundle == nullptr) {
    *saved_model_bundle = std::make_unique<machina::SavedModelBundle>();
  }
  ...
  auto load_status = machina::LoadSavedModel(
