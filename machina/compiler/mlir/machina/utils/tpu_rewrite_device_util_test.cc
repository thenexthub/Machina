...
bool DeviceNamesToParsedNames(toolchain::ArrayRef<std::string> device_names,
                              toolchain::SmallVectorImpl<Device>* parsed_devices) {
...
using DeviceNames = toolchain::SmallVector<std::string, 8>;
...
TEST_P(ParameterizedDeviceSetTest, BadDeviceSet) {
  toolchain::SmallVector<Device, 8> devices;
...
TEST_P(ParameterizedMetadataTest, BadMetadata) {
  ...
  toolchain::SmallVector<toolchain::SmallVector<std::string, 8>, 8> execution_devices;
...
std::string TopologyWithMeshShape(toolchain::ArrayRef<int> mesh_shape) {
...
std::string TopologyWithMeshShapeAndTasks(toolchain::ArrayRef<int> mesh_shape,
                                          int num_tasks,
...
std::string TopologyWithDeviceCoordinates(
    toolchain::ArrayRef<int> device_coordinates) {
  tpu::TopologyProto topology_proto;
...
std::vector<std::string> MakeDeviceSet(int num_tasks,
  ...
  for (int task = 0; task < num_tasks; ++task) {
    devices.push_back(
        toolchain::formatv("/job:worker/replica:0/task:{0}/device:CPU:0", task)
            .str());
    devices.push_back(
        toolchain::formatv("/job:worker/replica:0/task:{0}/device:TPU_SYSTEM:0",
                      task)
            .str());
    for (int device = 0; device < num_devices_per_task; ++device)
      devices.push_back(
          toolchain::formatv("/job:worker/replica:0/task:{0}/device:TPU:{1}", task,
                        device)
              .str());
  }
...
TEST(TPURewriteDeviceUtilTest, TestGetHostDeviceFailBadDeviceAssignment) {
  ...
  cluster->setAttr(kDeviceAssignmentAttr,
                   builder.getStrArrayAttr(toolchain::ArrayRef<toolchain::StringRef>(
  ...
  (*module_ref)
      ->setAttr("tf.devices",
                builder.getStrArrayAttr(
                    toolchain::ArrayRef<toolchain::StringRef>({"bad_device_name"})));
...
TEST(TPURewriteDeviceUtilTest, TestGetHostDeviceTPUReplicate) {
  ...
  toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallVector<toolchain::StringRef, 4>>
      devices;
  auto replicate = builder.create<mlir::tf_device::ReplicateOp>(
      mlir::UnknownLoc::get(&context), /*num_replicas=*/2, devices,
      toolchain::ArrayRef<std::pair<mlir::ValueRange, mlir::Type>>{},
