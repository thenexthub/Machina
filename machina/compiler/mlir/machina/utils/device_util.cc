...
mlir::Attribute ParseGpuDeviceMetadata(const Device& device,
  ...
  static auto* r = new toolchain::Regex("compute capability: ([0-9]+)\\.([0-9]+)");

  toolchain::SmallVector<toolchain::StringRef, 3> cc;
...
mlir::LogicalResult GetDevicesFromOp(mlir::Operation* op,
  ...
  for (const auto& kv : toolchain::enumerate(array_attr)) {
    ...
    if (DeviceNameUtils::ParseFullName(string_attr.getValue().str(), &device)) {
      ...
      return op->emitOpError(
          toolchain::formatv("bad '{0}' attribute, '{1}', not a valid device",
...
void AddDevicesToOp(mlir::Operation* op, const DeviceSet* device_set) {
  ...
  toolchain::SmallVector<mlir::NamedAttribute, 8> devices;
...
mlir::LogicalResult GetDevicesFromOp(mlir::Operation* op,
  ...
  return op->emitOpError(
      toolchain::formatv("unsupported '{0}' attribute", kDevicesAttr));
}
