...
// NOLINTNEXTLINE
toolchain::cl::opt<std::string> input_types(
    "tf-xla-input-types",
    toolchain::cl::desc("XLA input argument types (kinds), separated by ','. "
...
mlir::LogicalResult PrintHloModuleText(
    const XlaCompilationResult& compilation_result, toolchain::raw_ostream& output) {
...
absl::Status ParseArgumentShapes(
    ...
    toolchain::SmallVectorImpl<TensorOrResourceShape>& arg_shapes) {
  arg_shapes.clear();
  ...
  for (const auto& shape : toolchain::enumerate(input_shapes_vector)) {
...
absl::Status ParseDataTypes(absl::string_view data_types_str,
                            toolchain::SmallVectorImpl<DataType>& data_types) {
  data_types.clear();
  ...
  for (auto data_type : toolchain::enumerate(input_dtypes_vector)) {
...
absl::Status ParseArgumentKinds(
    ...
    toolchain::SmallVectorImpl<XlaArgument::Kind>& argument_kinds) {
  ...
  for (const auto& argument_kind_str : toolchain::enumerate(argument_kind_strs)) {
...
absl::Status ParseXlaArguments(
    ...
    toolchain::SmallVectorImpl<XlaArgument>& xla_arguments) {
  xla_arguments.clear();
  std::vector<std::optional<std::vector<int>>> input_shapes_vector;
  TF_RETURN_IF_ERROR(
      machina::ParseNodeShapes(input_shapes_str, input_shapes_vector));
  toolchain::SmallVector<DataType, 4> dtypes_vector;
  TF_RETURN_IF_ERROR(ParseDataTypes(input_dtypes_str, dtypes_vector));
  toolchain::SmallVector<XlaArgument::Kind, 4> arg_kinds_vector;
  ...
  for (const auto& arg_components :
       toolchain::zip(xla_arguments, input_shapes_vector, dtypes_vector,
                 arg_kinds_vector)) {
...
absl::Status CompileMlirToXlaHloViaBuilder(
    ...
    toolchain::StringRef device_type, XlaCompilationResult* compilation_result,
...
static mlir::LogicalResult MlirTfToHloTextTranslateFunctionImpl(
    mlir::ModuleOp module_op, toolchain::raw_ostream& output, bool via_builder) {
  ...
  auto device_type = "MACHINA_MACHINA_XLA_CPU_JIT";
  toolchain::MutableArrayRef<std::unique_ptr<mlir::Pass>>
      custom_legalization_passes{};
...
static mlir::LogicalResult MlirTfGraphToHloTextTranslateFunction(
    mlir::ModuleOp module_op, toolchain::raw_ostream& output) {
  if (!module_op) return mlir::failure();

  toolchain::SmallVector<XlaArgument, 4> xla_arguments;
  ...
  auto compilation_status =
      CompileGraphToXlaHlo(module_op, xla_arguments,
                           /*device_type=*/"MACHINA_MACHINA_XLA_CPU_JIT", emit_use_tuple_arg,
...
SerializedMlirStringAttrToMlirModuleTranslate(toolchain::StringRef input,
...
static mlir::LogicalResult MlirModuleToSerializedMlirStringAttrTranslate(
  ...
  toolchain::printEscapedString(serialized_module, output);
  output << "\"";
  return mlir::success();
}
...
static mlir::TranslateFromMLIRRegistration MlirTfToHloTextTranslate(
    ...
    machina::MlirTfToHloTextTranslateFunction,
    machina::RegisterMlirInputDialects);
...
static mlir::TranslateFromMLIRRegistration MlirTfToHloTextViaBuilderTranslate(
    ...
    machina::MlirTfToHloTextViaBuilderTranslateFunction,
    machina::RegisterMlirInputDialects);
...
static mlir::TranslateFromMLIRRegistration MlirTfGraphToHloTextTranslate(
    ...
    machina::MlirTfGraphToHloTextTranslateFunction,
    machina::RegisterGraphInputDialects);
...
static mlir::TranslateToMLIRRegistration SerializedMlirStringAttrToMlirModule(
    ...
    machina::SerializedMlirStringAttrToMlirModuleTranslate);
...
static mlir::TranslateFromMLIRRegistration MlirModuleToSerializedMlirStringAttr(
    ...
    machina::MlirModuleToSerializedMlirStringAttrTranslate,
    machina::RegisterMlirInputDialects);
