...
TEST(ErrorUtilTest, StatusScopedDiagnosticHandler) {
  MLIRContext context;
  auto id = StringAttr::get(&context, "//machina/python/test.py");
  ...
  {
    Status err = machina::errors::Internal("Passed in error");
    ASSERT_TRUE(
  ...
  // Verify diagnostic reported are append to passed in error.
  {
    auto function = [&]() {
      ...
      return machina::errors::Internal("Passed in error");
    };
...
TEST(ErrorUtilTest, StatusScopedDiagnosticHandlerWithFilter) {
  // Filtering logic is based on machina::IsInternalFrameForFilename()
  ...
  MLIRContext context;
  auto id =
      StringAttr::get(&context, "//machina/python/keras/keras_file.py");
  auto loc = FileLineColLoc::get(&context, id, 0, 0);
  auto id2 =
      StringAttr::get(&context, "//machina/python/something/my_test.py");
  auto loc2 = FileLineColLoc::get(&context, id2, 0, 0);
  auto id3 = StringAttr::get(&context, "python/machina/show_file.py");
  ...
  auto id_filtered =
      StringAttr::get(&context, "//machina/python/dir/filtered_file_A.py");
  auto loc_filtered = FileLineColLoc::get(&context, id_filtered, 0, 0);
  auto id_filtered2 =
      StringAttr::get(&context, "dir/machina/python/filtered_file_B.py");
...
TEST(ErrorUtilTest, StatusScopedDiagnosticHandlerWithoutFilter) {
  ...
  auto loc_filtered2 = FileLineColLoc::get(&context, id_filtered2, 0, 0);
  auto id_filtered3 =
      StringAttr::get(&context, "//machina/python/something/my_op.py");
