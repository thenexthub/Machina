...
#include "machina/xla/hlo/ir/tile_assignment.h"
...
mlir::TF::PadOp CreatePadOp(mlir::OpBuilder* builder,
  ...
  toolchain::SmallVector<int64_t, 4> padding_values;
...
mlir::LogicalResult CreateSplitOp(
  ...
  if (input_type.hasRank()) {
    if (input_type.getShape()[split_dimension] == mlir::ShapedType::kDynamic) {
      ...
      auto shape = toolchain::to_vector<4>(input_type.getShape());
  ...
  toolchain::SmallVector<mlir::Type, 4> output_types(num_split, output_type);
...
mlir::LogicalResult CreateXlaSplitNDOp(const mlir::Location& location,
  ...
  auto output_slice_shape = toolchain::to_vector<4>(input_type.getShape());
  int num_tiles = 1;
  ...
  toolchain::SmallVector<mlir::Type, 4> output_types(num_tiles, output_type);
...
mlir::LogicalResult HandleTileShardedInputsUsingXlaSplitOps(
    ...
    toolchain::SmallVectorImpl<mlir::Value>* tiled_inputs,
    bool is_ici_weight_dist_spmd) {
  ...
  auto shape = toolchain::to_vector<4>(
...
mlir::LogicalResult HandleTileShardedInputsUsingTfSplitOps(
    ...
    toolchain::SmallVectorImpl<mlir::Value>* tiled_inputs,
    bool is_ici_weight_dist_spmd) {
  toolchain::SmallVector<mlir::TF::SplitOp, 4> split_ops_for_tiled_input;
  ...
  for (const auto& dimension_and_num_splits : *dimension_to_splits_map) {
    ...
    toolchain::SmallVector<mlir::TF::SplitOp, 4> new_split_ops;
...
mlir::LogicalResult DecodeShardingAttribute(const std::string& shard_str,
  ...
  if (report_error)
    toolchain::errs() << std::string(sharding_hlo.status().message()) << "\n";
  return mlir::failure();
}
...
void EncodeSharding(mlir::Operation* op, toolchain::StringRef shard_str) {
...
mlir::LogicalResult ExtractInputsForLogicalDevices(
    ...
    toolchain::SmallVectorImpl<toolchain::SmallVector<mlir::Value, 4>>* input_list) {
...
mlir::LogicalResult ExtractInputsForLogicalDevices(
  ...
  for (int i = 0; i < num_cores_per_replica; ++i)
    input_list->emplace_back(toolchain::SmallVector<mlir::Value, 4>());

  toolchain::SmallVector<mlir::Value, 4> cluster_func_inputs(
  ...
  // the input.
  for (const auto& sharding_attr_and_index : toolchain::enumerate(sharding_attrs)) {
    ...
    auto tiled_sharding_mismatched = [&](int tiled_input_size) {
      return cluster_func.emitError(
          toolchain::formatv("incorrect {0}-th tiled input sharding received. "
    ...
    if (auto partitioned_input =
            toolchain::dyn_cast_or_null<mlir::TF::TPUPartitionedInputV2Op>(
      ...
      if (input_sharding_type == xla::OpSharding::REPLICATED) {
        for (const auto& index_and_inputs : toolchain::enumerate(*input_list)) {
    ...
    if (IsSplitSharding(sharding)) {
      ...
      toolchain::SmallVector<mlir::Value, 4> tiled_inputs;
...
mlir::LogicalResult ParseAndValidateOutputSharding(
  ...
  for (const auto& output_sharding_and_index :
       toolchain::enumerate(output_sharding_attrs)) {
    ...
    if (!mlir::isa<mlir::StringAttr>(output_sharding))
      return cluster_func.emitError(toolchain::formatv(
...
mlir::LogicalResult LookupClusterToCoreIndex(
  ...
  *core_output_index =
  ...
  if (*core_output_index == -1) {
    mlir::emitError(
        location,
        toolchain::formatv("Attempted to map cluster_func output index {0} to "
...
mlir::LogicalResult GetTileShardedOutputsToMerge(
    ...
    toolchain::SmallVector<mlir::Value, 4>* outputs_to_merge) {
  ...
  for (const auto& core_id_and_index :
       toolchain::enumerate(sharding.tile_assignment_devices())) {
...
mlir::LogicalResult HandleTileShardedOutputsUsingXlaConcatOps(
  ...
  if (output_type.hasRank()) {
    auto shape = toolchain::to_vector<4>(output_type.getShape());
    for (int dim = 0; dim < rank; ++dim) {
...
mlir::LogicalResult HandleTileShardedOutputsUsingTfConcatOps(
  ...
  for (auto it = dimension_to_splits_map->rbegin();
    ...
    toolchain::SmallVector<mlir::Value, 4> new_outputs;
...
mlir::LogicalResult ValidateAndGetTiledExecuteOutputShape(
  ...
  auto new_output_shape = toolchain::to_vector<4>(output_shape);
  auto dimension_to_splits_map =
...
bool AreInputOutputShapesStaticallyKnownForSplitSharding(
  ...
  if (sharding_attrs) {
    ...
    for (const auto& sharding_attr_and_index :
         toolchain::enumerate(sharding_attrs)) {
      const auto& sharding_attr = sharding_attr_and_index.value();
      const auto input_index = sharding_attr_and_index.index();
      const auto& input_value = cluster_func_inputs[input_index];
      const auto input_type =
  ...
  for (const auto& result_and_index :
       toolchain::enumerate(cluster_func.getResults())) {
...
mlir::LogicalResult GetOutputTypesForLogicalDeviceComputation(
    ...
    mlir::tf_device::ClusterFuncOp cluster_func,
    toolchain::SmallVectorImpl<mlir::Type>* output_types,
    toolchain::SmallVectorImpl<int>* cluster_to_core_index) {
...
mlir::LogicalResult GetOutputTypesForLogicalDeviceComputation(
    const int core_id, toolchain::ArrayRef<xla::OpSharding> output_sharding_config,
    mlir::tf_device::ClusterFuncOp cluster_func,
    toolchain::SmallVectorImpl<mlir::Type>* output_types, bool use_xla_nd_ops,
    toolchain::SmallVectorImpl<int>* cluster_to_core_index) {
...
mlir::LogicalResult RemapOutputsFromLogicalDevices(
  ...
  for (auto [output_index, old_parallel_execute_output] :
       toolchain::enumerate(old_parallel_execute.getResults())) {
    if (output_index < num_results_pre_cluster) {
      ...
      for (auto& use : toolchain::make_early_inc_range(
    ...
    for (auto user : old_parallel_execute_output.getUsers()) {
      if (auto partitioned_output_user =
              toolchain::dyn_cast_or_null<mlir::TF::TPUPartitionedOutputV2Op>(
                  user)) {
    ...
    if (partitioned_output) {
      ...
      if (output_sharding_type == xla::OpSharding::REPLICATED) {
        for (const auto& index_and_output :
             toolchain::enumerate(partitioned_output.getOutput())) {
        ...
        toolchain::SmallVector<mlir::Value, 4> tile_sharded_outputs;
        if (failed(GetTileShardedOutputsToMerge(
          ...
          return mlir::failure();
        for (auto result :
             toolchain::zip(partitioned_output.getOutput(), tile_sharded_outputs))
...
toolchain::SmallVector<toolchain::SmallVector<int64_t, 4>, 4> GetMetadataArgumentMapping(
    const tpu::TPUCompileMetadataProto& metadata) {
  toolchain::SmallVector<toolchain::SmallVector<int64_t, 4>, 4> input_mappings(
      metadata.num_cores_per_replica(), toolchain::SmallVector<int64_t, 4>());
  ...
  for (const auto& arg_and_idx : toolchain::enumerate(metadata.args())) {
...
mlir::LogicalResult VerifyShardingEquivalent(mlir::Attribute sharding_attr1,
  ...
  if (machina::DecodeShardingAttribute(sharding_attr1, sharding_proto1)
          .failed()) {
    return mlir::failure();
  }
  xla::OpSharding sharding_proto2;
  if (machina::DecodeShardingAttribute(sharding_attr2, sharding_proto2)
          .failed()) {
    return mlir::failure();
  }

  return machina::VerifyShardingEquivalent(sharding_proto1, sharding_proto2);
}
}  // namespace
