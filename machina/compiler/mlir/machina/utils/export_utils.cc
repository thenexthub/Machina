...
absl::Status ConvertLocation(mlir::Location inst_loc, toolchain::StringRef node_name,
  ...
  if (auto call_site = mlir::dyn_cast<mlir::CallSiteLoc>(unwrapped_inst_loc)) {
    if (auto name_loc = mlir::dyn_cast<mlir::NameLoc>(
      ...
      toolchain::StringRef original_node_name, original_func_name;
...
static bool IsRefTypeControlOp(mlir::Operation* op) {
  if (auto next_iter_sink =
          toolchain::dyn_cast<mlir::tf_executor::NextIterationSinkOp>(op))
...
absl::StatusOr<toolchain::StringRef> GetTensorFlowOpName(toolchain::StringRef op_name) {
...
absl::StatusOr<std::unique_ptr<NodeDef>> GetOperationNodeDef(
  ...
  toolchain::SmallString<64> op_name;
...
absl::Status ConvertAttributes(
    const toolchain::ArrayRef<mlir::NamedAttribute> attrs,
  ...
  for (const mlir::NamedAttribute& named_attr : attrs) {
    ...
    TF_RETURN_IF_ERROR(
        toolchain::TypeSwitch<mlir::Attribute, absl::Status>(attr)
...
bool IsLegacyCallInstruction(mlir::Operation* inst) {
  return toolchain::dyn_cast<mlir::TF::LegacyCallOp>(inst);
}
