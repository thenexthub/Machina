...
func.func @while_body_func(%arg0: tensor<i32>) -> (tensor<i32>) {
  ...
  toolchain::SmallVector<mlir::func::FuncOp> callees;
...
func.func @func(%arg0: tensor<i32>) -> tensor<i32> {
  ...
  auto has_compile_device_type = [](mlir::SymbolUserOpInterface op) {
    return op->hasAttr(machina::kCompileDeviceTypeAttr);
  };
  mlir::MLIRContext context;
  context.loadDialect<mlir::func::FuncDialect, mlir::TF::TensorFlowDialect>();
  mlir::OwningOpRef<mlir::ModuleOp> module =
  ...
  ASSERT_TRUE(module);
  mlir::SymbolTable symtab(*module);
  toolchain::SmallVector<mlir::func::FuncOp> entry_funcs =
      GetEntryFunctions(*module);
  EXPECT_EQ(entry_funcs.size(), 1);
  EXPECT_EQ(entry_funcs[0].getSymName(), "entry_func");
  toolchain::SmallVector<mlir::SymbolUserOpInterface> outermost_pcall_ops;
  auto result =
  ...
  auto func =
      toolchain::dyn_cast<mlir::func::FuncOp>(outermost_pcall_ops[0]->getParentOp());
  ASSERT_TRUE(func);
...
func.func @func(%arg0: tensor<i32>) -> tensor<i32> {
  ...
  EXPECT_EQ(entry_funcs.size(), 1);
  EXPECT_EQ(entry_funcs[0].getSymName(), "entry_func");
  toolchain::SmallVector<mlir::SymbolUserOpInterface> noinline_pcall_ops,
      outermost_pcall_ops;
  auto result =
  ...
  auto func =
      toolchain::dyn_cast<mlir::func::FuncOp>(noinline_pcall_ops[0]->getParentOp());
  ASSERT_TRUE(func);
  EXPECT_EQ(func.getSymName(), "while_body_func");
  func =
      toolchain::dyn_cast<mlir::func::FuncOp>(noinline_pcall_ops[1]->getParentOp());
  ASSERT_TRUE(func);
  EXPECT_EQ(func.getSymName(), "outer_stateful_pcall_func");
