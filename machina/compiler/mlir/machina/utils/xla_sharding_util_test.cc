...
TEST(XLAShardingUtilTest, TestShapesCheckForSplitSharding) {
  ...
  auto result = machina::ParseAndValidateOutputSharding(2, cluster_func_op,
                                                           &output_shardings);
  ASSERT_TRUE(succeeded(result));
  ASSERT_TRUE(machina::AreInputOutputShapesStaticallyKnownForSplitSharding(
...
TEST(XLAShardingUtilTest, TestShapesCheckForSplitShardingWithUnknownShapes) {
  ...
  auto result = machina::ParseAndValidateOutputSharding(2, cluster_func_op,
                                                           &output_shardings);
  ASSERT_TRUE(succeeded(result));
  ASSERT_FALSE(machina::AreInputOutputShapesStaticallyKnownForSplitSharding(
      output_shardings, cluster_func_op));
}
...
TEST(XLAShardingUtilTest, NotDivisibleShardingSplitOpTest) {
  ...
  result = machina::ExtractInputsForLogicalDevices(
  ...
  toolchain::SmallVector<mlir::Value, 4> split_inputs(split_op->getOperands());
  ...
  toolchain::SmallVector<mlir::Value, 4> pad_inputs(pad_op->getOperands());
