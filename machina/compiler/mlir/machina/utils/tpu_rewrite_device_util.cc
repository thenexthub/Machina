...
using ParsedDevices = toolchain::ArrayRef<DeviceNameUtils::ParsedName>;

namespace {
// Find matching devices in `devices` based on pattern `spec`.
toolchain::SmallVector<ParsedDevice, 8> FindMatchingDevices(
    ParsedDevices devices, const ParsedDevice& spec) {
  toolchain::SmallVector<ParsedDevice, 8> matching_devices;
...
absl::StatusOr<toolchain::SmallVector<ParsedDevice, 8>> GetTPUSystemDevices(
  ...
  toolchain::SmallVector<ParsedDevice, 8> system_devices =
  ...
  for (const auto& device : toolchain::make_range(std::next(system_devices.begin()),
                                             system_devices.end())) {
    if (device.job != job)
...
absl::StatusOr<toolchain::SmallVector<toolchain::SmallVector<ParsedDevice, 8>, 8>>
GetTPUDevices(ParsedDevices devices,
              toolchain::ArrayRef<ParsedDevice> system_devices) {
  toolchain::SmallVector<toolchain::SmallVector<ParsedDevice, 8>, 8> tpu_devices;
  ...
  auto lookup = [&devices](ParsedDevice device_spec) {
    ...
    toolchain::SmallVector<ParsedDevice, 8> host_tpu_devices =
  ...
  for (const auto& device_spec : toolchain::make_range(
...
std::string GetTPUCompilationDevice(ParsedDevice system_device) {
  ...
  system_device.type = machina::DEVICE_CPU;
...
absl::StatusOr<TPUDevicesAndHosts> GetFullMeshTPUExecutionDeviceAssignment(
    ...
    toolchain::ArrayRef<toolchain::SmallVector<ParsedDevice, 8>> tpu_devices,
    ParsedDevices devices) {
  ...
  for (int i = 0; i < num_replicas; ++i) {
    ...
    devices_and_hosts.push_back({TPUDeviceAndHost(
        /*device=*/machina::DeviceNameUtils::ParsedNameToString(tpu_device),
...
absl::StatusOr<xla::Array4D<TaskAndDevice>> ParseTopologyAttr(
    toolchain::StringRef topology_attr, int num_tasks, int num_tpus_per_task) {
  ...
  for (auto mesh_shape_dim : toolchain::enumerate(topology_proto.mesh_shape()))
...
GetGeneralTPUExecutionDeviceAssignment(
    int num_replicas, int num_cores_per_replica,
    toolchain::ArrayRef<toolchain::SmallVector<ParsedDevice, 8>> tpu_devices,
    ParsedDevices devices, toolchain::StringRef topology_attr,
    toolchain::ArrayRef<int64_t> device_assignment_attr) {
  const int num_tasks = tpu_devices.size();
  const int num_tpus_per_task = tpu_devices[0].size();
  ...
  std::vector<bool> used_device_ids(bound_x * bound_y * bound_z * bound_core,
                                    false);
  TPUDevicesAndHosts devices_and_hosts(
      num_replicas, toolchain::SmallVector<TPUDeviceAndHost, 8>(
...
mlir::LogicalResult GetTopology(mlir::tf_device::ClusterOp cluster,
  ...
  mlir::StringAttr topology_attr =
      cluster->getAttrOfType<mlir::StringAttr>(machina::kTopologyAttr);
  if (topology_attr) {
    topology = topology_attr.getValue();
    return mlir::success();
  } else {
    return cluster.emitOpError(
        toolchain::formatv("requires attribute '{0}'", machina::kTopologyAttr)
            .str());
  }
}
...
mlir::LogicalResult GetDeviceAssignmentCoordinates(
    ...
    toolchain::SmallVector<int64_t, 8>& device_coordinates) {
  ...
  if (!device_assignment_attr)
    return cluster.emitOpError(toolchain::formatv("requires attribute '{0}'",
                                             machina::kDeviceAssignmentAttr)
                                   .str());
  if (absl::StatusOr<toolchain::SmallVector<int64_t, 8>> fetched_device_coordinates =
...
mlir::LogicalResult GetTPUDevicesAndHostsNotReplicated(
    ...
    machina::TPUDevicesAndHosts& devices_and_hosts) {
  std::string topology;
...
// second host.
toolchain::SmallVector<std::string, 8> GetTPUToHostMapReplicated(
    mlir::tf_device::ClusterOp cluster) {
  int num_cores_per_replica = GetNumCoresPerReplica(cluster);
  toolchain::SmallVector<std::string, 8> core_to_host;
...
absl::StatusOr<xla::DeviceAssignmentProto> GetXlaDeviceAssignmentProto(
    toolchain::StringRef topology_attr, int num_replicas, int num_cores_per_replica,
    toolchain::ArrayRef<int64_t> device_assignment_attr) {
  tpu::TopologyProto topology_proto;
  if (!topology_proto.ParseFromString(topology_attr.str()))
    return absl::InvalidArgumentError(absl::StrCat(
...
absl::StatusOr<TPUDeviceAssignment> GetTPUCompilationAndExecutionDevices(
    ...
    toolchain::StringRef topology_attr,
    toolchain::ArrayRef<int64_t> device_assignment_attr) {
...
std::string GetDeviceAliasForLogicalCore(const int core_index) {
  return toolchain::formatv("{0}_{1}", kTPUReplicatedCore, core_index).str();
}

std::string GetDeviceAliasForHostOfLogicalCore(const int core_index) {
  return toolchain::formatv("{0}_{1}", kTPUReplicatedHost, core_index).str();
}
  ...
  for (const auto& device : devices.device_names()) {
    if (device.has_type && device.type == "CPU" && device.id == 0) {
      if (!host_device->empty()) {
        ...
        if (device.job == "chief")
          *host_device =
              machina::DeviceNameUtils::ParsedNameToString(device);
        continue;
      }
      *host_device = machina::DeviceNameUtils::ParsedNameToString(device);
    }
  }
  if (host_device->empty()) {
...
bool IsTPUDevice(toolchain::StringRef device) {
...
bool IsTPUReplicatedCore(toolchain::StringRef device) {
  ParsedDevice parsed_device;
  if (!DeviceNameUtils::ParseFullName(mlir::StringRefToView(device),
                                      &parsed_device))
    return false;
...
mlir::LogicalResult GetDeviceToHostMap(
  ...
  if (failed(machina::GetDevicesFromOp(
          cluster->getParentOfType<mlir::ModuleOp>(), &devices))) {
    return mlir::failure();
  }

  if (machina::HasTPUDevice(devices) ||
      cluster->getParentOfType<mlir::tf_device::ReplicateOp>()) {
    return GetTPUToHostMap(devices, cluster, core_to_host);
  }

  std::string host_device;
  if (failed(machina::GetHostDeviceOutsideCompilationInGenericPipeline(
          devices, &host_device))) {
    return mlir::failure();
  } else {
...
mlir::LogicalResult GetNonReplicatedTPU0(mlir::Operation* op,
  ...
  mlir::TF::RuntimeDevices devices;
  if (failed(machina::GetDevicesFromOp(moduleOp, &devices)))
    return moduleOp.emitOpError() << "No available devices.";
  toolchain::ArrayRef<machina::DeviceNameUtils::ParsedName> device_names =
...
mlir::LogicalResult GetNonReplicatedCPU0(mlir::Operation* op,
  ...
  std::string tpu0_device;
  if (failed(machina::GetNonReplicatedTPU0(op, &tpu0_device)))
    return mlir::failure();
  auto status = machina::DeviceNameUtils::DeviceNameToCpuDeviceName(
