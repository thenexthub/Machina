...
using machina::AbstractTensorInterface;
...
using machina::errors::Unimplemented;
...
Status ConvertDataTypeToTensor(machina::DataType dtype, Builder builder,
                               Type* type) {
  Status s = machina::ConvertDataType(dtype, builder, type);
...
class MlirTensor : public TracingTensorHandle {
 public:
  ...
  machina::DataType DataType() const override {
    machina::DataType type;
  ...
  absl::Status Shape(machina::PartialTensorShape* shape) const override {
  ...
  machina::FullTypeDef FullType() const override {
    return machina::FullTypeDef();
  }
...
class MlirAbstractOp : public TracingOperation {
 public:
  ...
  Status SetAttrType(const char* attr_name,
                     machina::DataType dtype) override;
 ...
 private:
  ...
  toolchain::StringMap<Attribute> attrs_;
  ...
  const machina::OpDef* op_def_ = nullptr;
...
class MlirFunction : public AbstractFunction {
 public:
  ...
  Status GetFunctionDef(const machina::FunctionDef** f) override;

  absl::StatusOr<machina::core::RefCountPtr<machina::FunctionRecord>>
 ...
 private:
  ...
  func::FuncOp func_;
  machina::core::RefCountPtr<machina::FunctionRecord> func_record_;
};
...
class MlirFunctionContext : public TracingContext {
 public:
  ...
  Status AddParameter(machina::DataType dtype,
...
Status MlirAbstractOp::Reset(const char* op, const char* device_name) {
  ...
  TF_RETURN_IF_ERROR(
      machina::OpRegistry::Global()->LookUpOpDef(op, &op_def_));
  assert(op_def_);
...
Status MlirAbstractOp::Create(ArrayRef<Value> operands,
  ...
  state_->operands = toolchain::to_vector<4>(operands);
  Builder builder(context_);
  ...
  // attributes.
  for (const machina::OpDef::ArgDef& output_arg : op_def_->output_arg()) {
    ...
    if (!output_arg.number_attr().empty()) {
      ...
      } else if (output_arg.type() != machina::DT_INVALID) {
        for (int i = 0; i < repeats; ++i) {
          Type type;
          TF_RETURN_IF_ERROR(
    ...
    if (output_arg.is_ref()) {
      ...
      for (Type& type : toolchain::make_range(&state_->types[original_size],
                                         state_->types.end())) {
        Type output_type;
...
Status MlirFunction::GetFunctionDef(const machina::FunctionDef** f) {
  if (!func_record_) {
...
MlirFunction::GetFunctionRecord() {
  ...
  // the MLIRContext into a machina::Status.
  ...
  TF_RETURN_IF_ERROR(
      machina::tf2xla::v2::ConvertMlirFunctionToFunctionLibraryDef(
          func_, configs, &fdef));
  func_record_ = machina::core::RefCountPtr<machina::FunctionRecord>(
      new machina::FunctionRecord(std::move(fdef), {}, true));
...
Status MlirFunctionContext::AddParameter(
    machina::DataType dtype, const machina::PartialTensorShape& shape,
...
Status MlirAbstractOp::AddInput(AbstractTensorHandle* input) {
  ...
  // to this input.
  const machina::OpDef::ArgDef& arg_def =
      op_def_->input_arg(current_ods_input_++);
  Type expected_type;
  if (arg_def.type() != machina::DT_INVALID) {
    Builder builder(context_);
    TF_RETURN_IF_ERROR(
        machina::ConvertDataType(arg_def.type(), builder, &expected_type));
    if (arg_def.is_ref()) {
      Type output_type;
