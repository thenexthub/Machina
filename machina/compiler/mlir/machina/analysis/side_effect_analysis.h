...
#ifndef MACHINA_COMPILER_MLIR_MACHINA_ANALYSIS_SIDE_EFFECT_ANALYSIS_H_
#define MACHINA_COMPILER_MLIR_MACHINA_ANALYSIS_SIDE_EFFECT_ANALYSIS_H_
...
class SideEffectAnalysisInfo {
 public:
  ...
  const toolchain::SmallVector<Operation*, 4>& DirectControlPredecessors(
      Operation* op) const;
  toolchain::SmallVector<Operation*, 4> DirectControlPredecessors(
      Operation* op, toolchain::function_ref<bool(Operation*)> filter) const;

  // pass the filter (returning true) will be included.
  const toolchain::SmallVector<Operation*, 4>& DirectControlSuccessors(
      Operation* op) const;
  toolchain::SmallVector<Operation*, 4> DirectControlSuccessors(
      Operation* op, toolchain::function_ref<bool(Operation*)> filter) const;

  // Returns a vector of ops that are control sinks (i.e. side-effecting ops
  // with no control successors).
  toolchain::ArrayRef<Operation*> ControlSinks() const {
  ...
  const toolchain::SmallVector<std::pair<ResourceId, bool>>& GetResourceIds(
      Operation* op) const;
 ...
 private:
  ...
  toolchain::SmallSet<ResourceId, 8> GetDependentIds(ResourceId resource_id,
  ...
  toolchain::SmallDenseMap<Operation*, toolchain::SmallPtrSet<Operation*, 4>, 8>
      control_predecessors_;
  // Maps from an op to its control predecessors sorted in program order.
  toolchain::SmallDenseMap<Operation*, toolchain::SmallVector<Operation*, 4>, 8>
  ...
  toolchain::SmallVector<Operation*, 4> sorted_control_sinks_;

  // Maps from an op to its resource IDs along with a bool indicating if the
  // resource is accessed `read-only`.
  toolchain::SmallDenseMap<Operation*,
                      toolchain::SmallVector<std::pair<ResourceId, bool>>>
      op_to_resource_ids_;
  toolchain::SmallVector<std::pair<ResourceId, bool>> empty_resource_ids_;

  // For predecessor / successor queries on ops we don't track.
  toolchain::SmallVector<Operation*, 4> empty_operation_set_;
  ...
  struct PerResourceAccessInfo {
    ...
    toolchain::SmallVector<Operation*, 8> reads_since_last_write;
  ...
  toolchain::SmallDenseMap<ResourceId, PerResourceAccessInfo, 8>
...
#endif  // MACHINA_COMPILER_MLIR_MACHINA_ANALYSIS_SIDE_EFFECT_ANALYSIS_H_
