...
#include "mlir/Analysis/CallGraph.h"  // part of Codira Toolchain
...
class BacktrackAnalysisInfo {
 ...
 private:
  ...
  toolchain::SmallVector<Value, 4> backtracked_values_;
};
...
class BacktrackAnalysis {
 ...
 private:
  toolchain::SmallDenseMap<Region*, InfoT> info_map_;
...
BacktrackAnalysis::BacktrackAnalysis(
  ...
  auto scc_begin = toolchain::scc_begin(&call_graph);
  auto scc_end = toolchain::scc_end(&call_graph);
...
BacktrackAnalysisInfo::BacktrackAnalysisInfo(
  ...
  assert(toolchain::hasSingleElement(region.getBlocks()));
...
ResourceAliasAnalysisInfo::ResourceAliasAnalysisInfo(
  ...
  const bool has_arg_unique_id_attrs =
      toolchain::any_of(func_op.getArguments(), [&](const BlockArgument& arg) {
  ...
  if (has_arg_unique_id_attrs) {
    ...
    toolchain::SmallDenseMap<int64_t, int64_t> attr_id_to_internal_id;
  ...
  func_op.walk([&](Operation* op) {
    if (auto resource_alloc = dyn_cast<ResourceHandleAllocatorInterface>(op)) {
      toolchain::SmallVector<ResourceHandleValueAndId, 4> resources =
    ...
    } else if (toolchain::isa<TPUReplicatedInputOp>(op)) {
    ...
    } else if (toolchain::isa<IdentityNOp, IdentityOp, CastOp>(op)) {
      for (auto result : filter_resources(op->getResults()))
      ...
      toolchain::SmallVector<func::FuncOp, 4> functions;
      ...
      AnalyzeFunctionalCaseOrIfOp(
          ...
          backtrack_analysis);
    } else if (toolchain::isa<CaseRegionOp, IfRegionOp>(op)) {
...
bool ResourceAliasAnalysisInfo::PropagateInputToOutput(const Value& operand,
  ...
  const toolchain::SmallSet<int64_t, 8> id_set = operand_it->second;
...
void ResourceAliasAnalysisInfo::AnalyzeWhileLoop(
  ...
  toolchain::SmallVector<std::optional<int>, 4> passthrough_args(
...
void ResourceAliasAnalysisInfo::AnalyzeFunctionalCaseOrIfOp(
    CaseOrIfOp case_or_if_op, toolchain::ArrayRef<func::FuncOp> functions,
    const BacktrackAnalysis& backtrack_analysis) {
  toolchain::SmallVector<const BacktrackAnalysisInfo*, 2> infos;
  ...
  for (auto result : filter_resources(case_or_if_op.getResults())) {
    toolchain::SmallVector<std::optional<int>, 2> passthrough_args;
    passthrough_args.reserve(functions.size());
    for (const auto* info : infos)
      passthrough_args.emplace_back(info->GetArg(result.getResultNumber()));

    const bool all_passthrough_args_known = toolchain::all_of(
...
const toolchain::SmallSet<int64_t, 8>&
...
const toolchain::SmallSetVector<Value, 8>&
...
toolchain::SmallSetVector<Value, 8> ResourceAliasAnalysisInfo::GetResourceAliases(
  ...
  for (int64_t id : GetResourceUniqueIds(resource)) {
    const toolchain::SmallSetVector<Value, 8>& resources_aliasing_id =
