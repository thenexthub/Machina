...
using ResourceIdSet = toolchain::SmallSet<ResourceId, 8>;

// Note that we cannot simply define a `static const toolchain::SmallSet` here
// because of missing `initializer_list` support for `toolchain::SmallSet`.
...
class OpSideEffectCollector {
 ...
 private:
  ...
  void CollectOpSideEffects(Operation* op) {
    ...
    // some special cases.
    if (auto func = toolchain::dyn_cast<func::FuncOp>(op)) {
    ...
    } else if (auto if_op = toolchain::dyn_cast<IfOp>(op)) {
      ...
      toolchain::SmallVector<func::FuncOp, 4> branch_funcs;
      ...
      for (const auto& effect : effects) {
        ...
        if (toolchain::isa<ResourceEffects::MustExecute>(effect.getResource()))
...
bool OpSideEffectCollector::IsPureFunction(func::FuncOp func_op) const {
  ...
  if (it == is_pure_function_.end()) {
    ...
    func_op->walk([&](Operation* op) {
      ...
      if (toolchain::isa<AssertOp>(op)) {
        return WalkResult::advance();
      }
      if (auto if_op = toolchain::dyn_cast<IfOp>(op)) {
...
SideEffectsByResourceId CollectSideEffectsByResourceId(
  ...
  toolchain::SmallDenseSet<Value, 8> processed_values;
  for (const auto& effect : effects) {
    Value value = effect.getValue();
...
void SideEffectAnalysisInfo::AddPredecessorsForAccess(ResourceId resource_id,
  ...
  toolchain::SmallPtrSet<Operation*, 4> new_control_predecessors;
...
void SideEffectAnalysisInfo::AnalyzeFunction(func::FuncOp func_op) {
  ...
  for (auto& entry : sorted_control_predecessors_) {
    toolchain::sort(entry.getSecond(), [](Operation* a, Operation* b) {
  ...
  toolchain::sort(sorted_control_sinks_, [](Operation* a, Operation* b) {
    return a->isBeforeInBlock(b);
  });
}
...
const toolchain::SmallVector<Operation*, 4>&
...
toolchain::SmallVector<Operation*, 4>
SideEffectAnalysisInfo::DirectControlPredecessors(
    Operation* op, toolchain::function_ref<bool(Operation*)> filter) const {
  toolchain::SmallVector<Operation*, 4> result;
  auto it = sorted_control_predecessors_.find(op);
...
const toolchain::SmallVector<std::pair<ResourceId, bool>>&
