...
struct TensorFlowExecutorInlinerInterface : public DialectInlinerInterface {
  ...
  bool isLegalToInline(Region *dest, Region *src, bool wouldBeCloned,
    ...
    // block.
    return toolchain::isa<tf_executor::IslandOp>(dest->getParentOp()) &&
...
TensorFlowExecutorDialect::TensorFlowExecutorDialect(MLIRContext *context)
  ...
  addOperations<
#define GET_OP_LIST
#include "machina/compiler/mlir/machina/ir/tf_executor.cc.inc"
      >();
...
LogicalResult VerifyControlOperandsAfterAllData(Operation *op) {
  ...
  for (int operand_idx : toolchain::seq<int>(0, op->getNumOperands())) {
...
FetchOp GraphOp::GetFetch() { return toolchain::cast<FetchOp>(GetBody().back()); }
...
LogicalResult GraphOp::verify() {
  ...
  for (int i : toolchain::seq<int>(0, fetch.getNumOperands())) {
...
YieldOp IslandOp::GetYield() { return toolchain::cast<YieldOp>(GetBody().back()); }
...
mlir::LogicalResult IslandOp::verify() {
  ...
  for (int operand_idx : toolchain::seq<int>(0, yield.getNumOperands())) {
  ...
  for (int operand_idx : toolchain::seq<int>(0, island.getNumResults() - 1)) {
...
ParseResult LoopCondOp::parse(OpAsmParser &parser, OperationState &result) {
  ...
  Type control_type = ControlType::get(parser.getBuilder().getContext());
  if (FunctionType type = mlir::dyn_cast<FunctionType>(types.front())) {
    if (toolchain::count_if(type.getInputs(),
...
bool HasSingleOpInBlock(Block *block) {
  ...
  if (!toolchain::isa<InnerOpT>(block->front())) return false;
...
struct DropEmptyIslandNoOperandNoDataResult
  ...
  LogicalResult matchAndRewrite(IslandOp op,
    ...
    for (auto &use : toolchain::make_early_inc_range(op.getControl().getUses()))
...
LogicalResult IslandOp::fold(FoldAdaptor,
                             toolchain::SmallVectorImpl<OpFoldResult> &results) {
