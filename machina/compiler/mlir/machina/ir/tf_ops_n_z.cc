...
#include "machina/compiler/mlir/machina/ir/tf_arith_ops_folder.h"
#include "machina/compiler/mlir/machina/ir/tf_op_interfaces.h"
#include "machina/compiler/mlir/machina/ir/tf_ops.h"
#include "machina/compiler/mlir/machina/ir/tf_ops_canonicalization_helper.h"
#include "machina/compiler/mlir/machina/ir/tf_ops_device_helper.h"
#include "machina/compiler/mlir/machina/ir/tf_ops_layout_helper.h"
#include "machina/compiler/mlir/machina/ir/tf_ops_tensor_helper.h"
...
LogicalResult OneHotOp::verify() {
  ...
  auto indices_ty = toolchain::dyn_cast<RankedTensorType>(op.getIndices().getType());
  if (indices_ty &&
...
static TensorType InferOneHotOpType(Value indices, Value depth, Value on_value,
  ...
  Type element_ty = toolchain::cast<TensorType>(on_value.getType()).getElementType();
  auto unranked_ty = UnrankedTensorType::get(element_ty);
  if (axis_val < -1) return unranked_ty;

  auto indices_ty = toolchain::dyn_cast<RankedTensorType>(indices.getType());
  if (!indices_ty) return unranked_ty;

  auto shape = toolchain::to_vector<2>(indices_ty.getShape());
  ...
  return machina::GetTypeFromTFTensorShape(shape, element_ty);
}
...
LogicalResult PackOp::verify() {
  ...
  for (Value value : values) {
    if (auto ty = toolchain::dyn_cast<RankedTensorType>(value.getType())) {
...
OpFoldResult PackOp::fold(FoldAdaptor) {
  ...
  auto get_const_int = [](Value value,
    ...
    auto value_attr = toolchain::dyn_cast<DenseIntElementsAttr>(const_op.getValue());
  ...
  for (Value operand : toolchain::drop_begin(getValues(), 1)) {
  ...
  auto arg_ty = toolchain::dyn_cast<ShapedType>(tensor.getType());
  ...
  auto arg_dims = toolchain::drop_begin(arg_ty.getShape(), 1);
...
struct ConvertPackToReshape : public OpRewritePattern<PackOp> {
  ...
  LogicalResult matchAndRewrite(PackOp pack_op,
    ...
    auto input_ty = toolchain::cast<ShapedType>(pack_op.getOperand(0).getType());
    auto output_ty = toolchain::cast<ShapedType>(pack_op.getOutput().getType());
    if (!input_ty.hasStaticShape() || !output_ty.hasStaticShape()) {
      return failure();
    }

    auto output_int_type = machina::GetTypeFromTFTensorShape(
    ...
    if (elements_all_in_int32_range) {
      ...
      output_int_type = machina::GetTypeFromTFTensorShape(
...
LogicalResult PadOp::FoldOperandsPermutation(ArrayRef<int64_t> permutation) {
  ...
  auto paddings_value =
      toolchain::dyn_cast<DenseElementsAttr>(paddings_op.getValue());
  if (!paddings_value ||
  ...
  for (const auto &index_pair :
       toolchain::enumerate(paddings_value.getValues<APInt>())) {
  ...
  auto type = machina::GetTypeFromTFTensorShape(
  ...
  getResult().setType(toolchain::cast<TensorType>(ShuffleRankedTensorType(
...
static LogicalResult VerifyPartitionedCall(CallOpClass op,
  ...
  SymbolRefAttr func = toolchain::cast<SymbolRefAttr>(op->getAttr("f"));
...
OpFoldResult PowOp::fold(FoldAdaptor adaptor) {
  ...
  auto constant_y = toolchain::dyn_cast_if_present<DenseFPElementsAttr>(operands[1]);
  if (constant_y && constant_y.isSplat()) {
    APFloat y_value = constant_y.getSplatValue<APFloat>();
    auto output_type = toolchain::cast<ShapedType>(getType());
...
LogicalResult QrOp::verify() {
  QrOp op = *this;
  auto ttype = toolchain::cast<TensorType>(op.getInput().getType());
...
DenseElementsAttr BuildConstRangeTensor(Type result_elem_type, int num_elements,
  ...
  auto new_result_type =
      machina::GetTypeFromTFTensorShape({num_elements}, result_elem_type);
...
void RangeOp::build(OpBuilder &builder, OperationState &result, Value start,
  ...
  if (matchPattern(start, m_Constant(&start_val)) &&
    ...
    auto size = toolchain::APIntOps::RoundingSDiv(
        *limit_val.begin() - *start_val.begin(), *delta_val.begin(),
        toolchain::APInt::Rounding::DOWN);
    return RangeOp::build(
        builder, result,
        machina::GetTypeFromTFTensorShape(
            size.getSExtValue(),
            toolchain::cast<TensorType>(start.getType()).getElementType()),
        start, limit, delta);
  }
  return RangeOp::build(
      builder, result,
      machina::GetTypeFromTFTensorShape(
          {-1}, toolchain::cast<TensorType>(start.getType()).getElementType()),
      start, limit, delta);
}
...
OpFoldResult RangeOp::fold(FoldAdaptor adaptor) {
  ...
  auto start_tensor = toolchain::dyn_cast_if_present<ElementsAttr>(operands[0]);
  auto limit_tensor = toolchain::dyn_cast_if_present<ElementsAttr>(operands[1]);
  auto delta_tensor = toolchain::dyn_cast_if_present<ElementsAttr>(operands[2]);
  ...
  Type elem_type = toolchain::cast<ShapedType>(getType()).getElementType();
...
void RankOp::build(OpBuilder &builder, OperationState &result, Value input) {
  return RankOp::build(
      builder, result,
      machina::GetTypeFromTFTensorShape({}, builder.getIntegerType(32)),
      input);
}
...
OpFoldResult RankOp::fold(FoldAdaptor) {
  ...
  auto ranked_type = toolchain::dyn_cast<RankedTensorType>(type);
  if (!ranked_type) return {};

  // DenseIntElementsAttr::get requires the output type be ranked with static
  // shape.
  auto output_type = toolchain::dyn_cast<RankedTensorType>(getType());
...
LogicalResult GetReshapeOutputType(Value tensor, Value shape,
  ...
  if (!matchPattern(shape, m_Constant(&shape_attr))) {
    ...
    if (shape_ty.hasStaticShape()) {
      toolchain::SmallVector<int64_t, 8> dynamic_shape(shape_ty.getDimSize(0),
                                                  ShapedType::kDynamic);
      output_ty =
          machina::GetTypeFromTFTensorShape(dynamic_shape, element_ty);
    }
    return success();
  }
  ...
  toolchain::SmallVector<int64_t, 8> output_ty_shape;
  output_ty_shape.reserve(shape_attr.getNumElements());
  for (const auto &dim : toolchain::enumerate(shape_attr.getValues<APInt>())) {
    const int64_t size = dim.value().getSExtValue();
    if (size == machina::kTFDynamicSize ||  // NOLINT
      ...
      return error_handler(
          toolchain::formatv("requires 'shape' to have dimensions greater than -1, "
  ...
  if (!tensor_ty.hasStaticShape()) {
    output_ty =
        machina::GetTypeFromTFTensorShape(output_ty_shape, element_ty);
    return success();
  }
  ...
  if (unknown_index != -1) {
    ...
    if (!tensor_ty_zero_dim && shape_ty_size * missing_dim != tensor_ty_size)
      return error_handler(
          toolchain::formatv("requires 'tensor' number of elements be a multiple of "
                        "{0}, but got {1}",
  ...
  output_ty = machina::GetTypeFromTFTensorShape(output_ty_shape, element_ty);
...
LogicalResult ReshapeOp::verify() {
  ReshapeOp op = *this;
  auto error_handler = [&op](const toolchain::Twine &message) -> LogicalResult {
  ...
  auto output_ty = toolchain::dyn_cast<RankedTensorType>(op.getType());
  if (!output_ty) return success();
  auto tensor_ty = toolchain::cast<TensorType>(op.getTensor().getType());
...
void ReshapeOp::build(OpBuilder &builder, OperationState &result, Value tensor,
                      Value shape) {
  auto error_handler = [&result](const toolchain::Twine &message) {
...
OpFoldResult ReshapeOp::fold(FoldAdaptor) {
  ...
  auto result_ty = toolchain::dyn_cast<ShapedType>(getType());
...
LogicalResult SelectOp::verify() {
  SelectOp op = *this;
  auto then_tensor = toolchain::cast<TensorType>(op.getThenValue().getType());
  auto else_tensor = toolchain::cast<TensorType>(op.getElseValue().getType());
  // Check (1).
  ...
  auto cond_tensor =
      toolchain::dyn_cast<RankedTensorType>(op.getCondition().getType());
...
static Type InferSelectV2OpType(Value condition, Value e, Value t) {
  Type element_ty = toolchain::cast<TensorType>(e.getType()).getElementType();
  auto unranked_ty = UnrankedTensorType::get(element_ty);
  ...
  auto cond_ranked_ty = toolchain::dyn_cast<RankedTensorType>(condition.getType());
  auto broadcasted_ranked_ty = toolchain::dyn_cast<RankedTensorType>(broadcasted_ty);
  ...
  if (!OpTrait::util::getBroadcastedShape(cond_ranked_ty.getShape(),
                                          ...
                                          result_shape))
    return unranked_ty;
  return machina::GetTypeFromTFTensorShape(result_shape, element_ty);
}
...
LogicalResult VerifyShapeOperandAndResult(Operation *op, Type operand_type,
  ...
  std::string variadic_idx_str =
      variadic_idx < 0 ? "" : toolchain::formatv(" #{0}", variadic_idx).str();

  auto result_ranked_type = toolchain::dyn_cast<RankedTensorType>(result_type);
  ...
  auto operand_ranked_type =
      toolchain::dyn_cast_or_null<RankedTensorType>(operand_type);
...
static Attribute ConvertShapeToAttr(Type input_ty, int out_width) {
  auto ranked_ty = toolchain::dyn_cast<RankedTensorType>(input_ty);
  ...
  auto result_type = machina::GetTypeFromTFTensorShape(
...
OpFoldResult ShapeOp::fold(FoldAdaptor) {
  int width = toolchain::cast<ShapedType>(getType())
                  .getElementType()
...
void ShapeOp::build(OpBuilder &builder, OperationState &result, Value input,
                    BoolAttr use32Bit) {
  auto rankedTensorType = toolchain::dyn_cast<RankedTensorType>(input.getType());
  ...
  return ShapeOp::build(builder, result,
                        machina::GetTypeFromTFTensorShape({rank}, out_type),
                        input);
}
...
LogicalResult ShapeNOp::verify() {
  ...
  for (auto i : toolchain::seq<uint64_t>(0, num_tensors)) {
...
class ShapeNPartialStaticInputShape : public OpRewritePattern<ShapeNOp> {
  ...
  LogicalResult matchAndRewrite(ShapeNOp op,
    ...
    for (const auto &e : toolchain::enumerate(op.getOperands())) {
    ...
    if (!dynamic_inputs.empty()) {
      ...
      for (auto index_result :
           toolchain::zip(dynamic_indices, dynamic_shape_n.getResults())) {
...
OpFoldResult SizeOp::fold(FoldAdaptor) {
  ShapedType output_type = toolchain::cast<ShapedType>(getType());
  if (!output_type.hasRank()) return {};
  ShapedType input_type = toolchain::cast<ShapedType>(getOperand().getType());
...
LogicalResult SliceOp::verify() {
  ...
  auto input_ty = toolchain::dyn_cast<RankedTensorType>(op.getInput().getType());
  ...
  auto output_ty = toolchain::dyn_cast<RankedTensorType>(op.getOutput().getType());
...
LogicalResult SoftmaxCrossEntropyWithLogitsOp::verify() {
  ...
  auto broadcasted_ty =
      toolchain::dyn_cast_or_null<ShapedType>(OpTrait::util::getBroadcastedType(
...
LogicalResult SpaceToBatchNDOp::verify() {
  ...
  const auto input_type = toolchain::cast<TensorType>(op.getInput().getType());
  const auto block_shape_type =
      toolchain::cast<TensorType>(op.getBlockShape().getType());
  const auto paddings_type = toolchain::cast<TensorType>(op.getPaddings().getType());
...
LogicalResult SparseSoftmaxCrossEntropyWithLogitsOp::verify() {
  ...
  auto features_ty =
      toolchain::dyn_cast<RankedTensorType>(op.getFeatures().getType());
  auto labels_ty = toolchain::dyn_cast<RankedTensorType>(op.getLabels().getType());
...
LogicalResult VerifySplitInputAndSplitDim(Op op,
  ...
  *dim_index = std::nullopt;
  ...
  if (auto split_dim_type =
          toolchain::dyn_cast<RankedTensorType>(split_dim.getType()))
  ...
  auto input_type = toolchain::dyn_cast<RankedTensorType>(op.getValue().getType());
...
LogicalResult SplitOp::verify() {
  ...
  int64_t input_dim_size = toolchain::cast<RankedTensorType>(op.getValue().getType())
...
LogicalResult SplitVOp::verify() {
  ...
  auto split_sizes_type =
      toolchain::dyn_cast<RankedTensorType>(op.getSizeSplits().getType());
  ...
  split_sizes.reserve(
      toolchain::cast<ShapedType>(split_sizes_attr.getType()).getNumElements());

  for (const auto &dim : toolchain::enumerate(split_sizes_attr)) {
    int64_t dim_val = dim.value().getSExtValue();
    split_sizes.push_back(dim_val);
    if (dim_val == machina::kTFDynamicSize) {
...
LogicalResult SqueezeOp::verify() {
  SqueezeOp op = *this;
  auto input_type = toolchain::dyn_cast<RankedTensorType>(op.getInput().getType());
...
OpFoldResult SumOp::fold(FoldAdaptor) {
  auto input_ty = toolchain::dyn_cast<RankedTensorType>(getInput().getType());
  if (!input_ty) return {};
  auto result_ty = toolchain::dyn_cast<RankedTensorType>(getType());
  if (!result_ty) return {};
...
static LogicalResult VerifyStridedSliceBase(OpTy op) {
  ...
  for (Value val : {op.getBegin(), op.getEnd(), op.getStrides()}) {
    auto operand_ty = toolchain::dyn_cast<ShapedType>(val.getType());
  ...
  if (matchPattern(op.getStrides(), m_Constant(&strides))) {
    if (toolchain::is_contained(strides.getValues<APInt>(), 0))
  ...
  if (ellipsis_mask != 0 && !toolchain::isPowerOf2_32(ellipsis_mask))
...
bool StridedSliceOp::GetSlicedBoundRanges(
  ...
  auto input_ty = toolchain::dyn_cast<RankedTensorType>(this->getInput().getType());
  if (!input_ty || !input_ty.hasStaticShape()) return false;
  auto input_shape = toolchain::to_vector<4>(input_ty.getShape());
...
OpFoldResult StridedSliceOp::fold(FoldAdaptor) {
  ...
  auto tensor_ty =
      toolchain::dyn_cast<RankedTensorType>(shape_op.getInput().getType());
  ...
  auto output_ty = toolchain::dyn_cast<RankedTensorType>(getOutput().getType());
  if (!output_ty || !output_ty.hasStaticShape()) {
    if (getShrinkAxisMask() == 1) {
      output_ty = machina::GetTypeFromTFTensorShape({}, output_elt_ty);
    } else {
      output_ty = machina::GetTypeFromTFTensorShape(
...
LogicalResult StridedSliceGradOp::verify() {
  ...
  auto shape_type = toolchain::dyn_cast<RankedTensorType>(op.getShape().getType());
...
LogicalResult TPUExecuteAndUpdateVariablesOp::verify() {
  ...
  auto check_attr = [&](ArrayAttr indices, toolchain::StringRef name,
    ...
    for (const auto &entry : toolchain::enumerate(indices.getValue())) {
      auto int_attr = toolchain::cast<IntegerAttr>(entry.value());
...
OpFoldResult TensorListElementShapeOp::fold(FoldAdaptor) {
  int width = toolchain::cast<ShapedType>(getType())
                  .getElementType()
                  .getIntOrFloatBitWidth();
  auto variant_type = toolchain::cast<tf_type::VariantType>(
...
LogicalResult TensorScatterUpdateOp::verify() {
  ...
  auto tensor_ty = toolchain::dyn_cast<RankedTensorType>(op.getTensor().getType());
  auto indices_ty = toolchain::dyn_cast<RankedTensorType>(op.getIndices().getType());
...
LogicalResult TileOp::verify() {
  ...
  auto input_type = toolchain::dyn_cast<RankedTensorType>(op.getInput().getType());
  auto multiples_type =
      toolchain::dyn_cast<RankedTensorType>(op.getMultiples().getType());
  auto output_type = toolchain::dyn_cast<RankedTensorType>(op.getOutput().getType());
...
class FuseWithBroadcastCompatibleOp
 ...
 public:
  ...
  LogicalResult matchAndRewrite(mlir::Operation *op,
    ...
    std::vector<toolchain::SmallVector<int64_t, 6>> input_shapes;
    for (mlir::Value operand : op->getOperands()) {
      if (auto shape = mlir::dyn_cast<RankedTensorType>(operand.getType())) {
        input_types.push_back(operand.getType());
        input_shapes.push_back(toolchain::SmallVector<int64_t, 6>(shape.getShape()));
        continue;
      }
      return failure();
    }
    ...
    for (mlir::OpOperand &operand : op->getOpOperands()) {
      ...
      auto shape = toolchain::dyn_cast<RankedTensorType>(tile.getInput().getType());
      if (!shape) {
        continue;
      }
      toolchain::ArrayRef<int64_t> input_shape = shape.getShape();

      toolchain::SmallVector<int64_t, 5> multiples;
      ...
      toolchain::SmallVector<int64_t, 6> old_output_shape = input_shapes[pos];
      mlir::Type old_input_type = input_types[pos];
      input_shapes[pos] = toolchain::SmallVector<int64_t, 6>(input_shape);
      input_types[pos] = shape;
      if (!OpTrait::util::staticallyKnownBroadcastable(
              toolchain::ArrayRef(input_shapes)) ||
...
class ToBoolOfRankedTensor : public OpRewritePattern<ToBoolOp> {
  ...
  LogicalResult matchAndRewrite(ToBoolOp op,
                                PatternRewriter &rewriter) const override {
    auto type = toolchain::dyn_cast<RankedTensorType>(op.getOperand().getType());
    ...
    auto result_type = toolchain::cast<RankedTensorType>(op.getResult().getType());
    ...
    if (type.getRank() == 0) {
      ...
      // zero.
      bool any_zero =
          toolchain::any_of(type.getShape(), [](int64_t dim) { return dim == 0; });
...
LogicalResult ToBoolOp::inferReturnTypes(
  ...
  inferredReturnTypes.push_back(
      machina::GetTypeFromTFTensorShape({}, IntegerType::get(context, 1)));
  return success();
}
...
LogicalResult TPUPartitionedInputV2Op::verify() {
  ...
  for (const mlir::Attribute &dim : partition_dims) {
    num_partitions *= toolchain::cast<IntegerAttr>(dim).getInt();
  }
...
LogicalResult TransposeOp::verify() {
  TransposeOp op = *this;
  auto perm_type = toolchain::dyn_cast<RankedTensorType>(op.getPerm().getType());
  auto x_type = toolchain::dyn_cast<RankedTensorType>(op.getX().getType());
  auto y_type = toolchain::dyn_cast<RankedTensorType>(op.getY().getType());
  ...
  if (matchPattern(op.getPerm(), m_Constant(&attr_perm))) {
    ...
    // for i = [0, 1, ..., rank(x) - 1]
    for (const auto &e : toolchain::enumerate(attr_perm)) {
      ...
      if (x_idx < -x_rank || x_idx >= x_rank) {
        return op.emitOpError(
            toolchain::formatv("perm[{0}]={1} must be in range [-{2}, {2})", y_idx,
                          x_idx, x_rank));
      }
...
void TransposeOp::build(OpBuilder &builder, OperationState &result, Value x,
                        Value perm) {
  auto x_type = toolchain::cast<TensorType>(x.getType());
  ...
  auto etype = toolchain::cast<ShapedType>(x_type).getElementType();
  DenseIntElementsAttr attr_shape;
  if (matchPattern(perm, m_Constant(&attr_shape))) {
    toolchain::SmallVector<int64_t, 4> const_shape;
    ...
    return TransposeOp::build(
        builder, result,
        machina::GetTypeFromTFTensorShape(const_shape, etype), x, perm);
  }
...
OpFoldResult FoldIdentityTranspose(TransposeOp op) {
  ...
  for (const auto &it : toolchain::enumerate(elements)) {
...
OpFoldResult FoldCancellableTranspose(TransposeOp op) {
  ...
  if (transpose->getBlock() != op->getBlock()) {
    ...
    auto status = machina::ConvertToDataType(
        toolchain::cast<TensorType>(op.getX().getType()).getElementType(), &dtype);
    if (status.ok()) {
      // We can only leave the transpose op on host if its dtype is supported on
      // host.
      if (dtype == machina::DT_UINT64 || dtype == machina::DT_INT64 ||
          dtype == machina::DT_UINT32 || dtype == machina::DT_INT32 ||
          dtype == machina::DT_UINT16 || dtype == machina::DT_INT16 ||
          dtype == machina::DT_UINT8 || dtype == machina::DT_INT8 ||
          dtype == machina::DT_HALF || dtype == machina::DT_BFLOAT16 ||
          dtype == machina::DT_FLOAT || dtype == machina::DT_DOUBLE ||
          dtype == machina::DT_COMPLEX64 ||
          dtype == machina::DT_COMPLEX128 || dtype == machina::DT_BOOL) {
        return {};
      }
    }
  }
...
class NMSV3ToNMSV4Op : public OpRewritePattern<NonMaxSuppressionV3Op> {
  ...
  LogicalResult matchAndRewrite(NonMaxSuppressionV3Op nms_op,
    ...
    auto input_ty = toolchain::cast<ShapedType>(nms_op.getType());
    // corresponds to the second result type of nmsv4
    RankedTensorType valid_output_type =
        machina::GetTypeFromTFTensorShape({}, input_ty.getElementType());
...
class ConvertFusedBatchNorm : public OpRewritePattern<TF::FusedBatchNormOp> {
  ...
  LogicalResult matchAndRewrite(TF::FusedBatchNormOp tf_fused_batch_norm_op,
    ...
    auto new_result_types =
        toolchain::to_vector<6>(tf_fused_batch_norm_op.getResultTypes());
    // reserve_space_3
...
LogicalResult XlaCallModuleOp::verifySymbolUses(
  ...
  for (auto f : getFunctionList()) {
    auto func = symbolTable.lookupNearestSymbolFrom<func::FuncOp>(
        getOperation(), toolchain::cast<SymbolRefAttr>(f));
    if (!func) {
...
LogicalResult UnpackOp::verify() {
  UnpackOp op = *this;
  auto value_type = toolchain::dyn_cast<RankedTensorType>(op.getValue().getType());
...
LogicalResult HoistCwiseUnaryOutOfUnpack::matchAndRewrite(
  ...
  bool users_same_op = toolchain::all_of(op->getUsers(), [&](Operation *user) {
  ...
  // results instead.
  for (auto pair : toolchain::zip(op.getResults(), unpack_unary_op.getResults())) {
    ...
    for (Operation *user : toolchain::make_early_inc_range(old_result.getUsers()))
...
static LogicalResult VerifyUnsortedSegmentReduction(Op op) {
  ...
  auto data_type = toolchain::dyn_cast<RankedTensorType>(op.getData().getType());
  auto segment_ids_type =
      toolchain::dyn_cast<RankedTensorType>(op.getSegmentIds().getType());
  if (data_type && segment_ids_type) {
    ...
    int index = 0;
    for (auto shape_pair :
         toolchain::zip_first(segment_ids_type.getShape(), data_type.getShape())) {
...
LogicalResult VariableShapeOp::verify() {
  ...
  auto input_type = toolchain::cast<TensorType>(op.getInput().getType());
  if (input_type.hasStaticShape() && input_type.getNumElements() != 1)
    return op.emitOpError("requires input to have one resource");

  auto resource_type =
      toolchain::cast<tf_type::ResourceType>(input_type.getElementType());
...
OpFoldResult VariableShapeOp::fold(FoldAdaptor) {
  int width = toolchain::cast<ShapedType>(getType())
                  .getElementType()
                  .getIntOrFloatBitWidth();
  auto resource_type = toolchain::cast<tf_type::ResourceType>(
      getElementTypeOrSelf(getOperand().getType()));
...
LogicalResult WhileRegionOp::verify() {
  ...
  auto cond_type =
      toolchain::dyn_cast<RankedTensorType>(cond_yield->getOperand(0).getType());
  ...
  if (cond_yield->getNumOperands() > 1) {
    ...
    for (auto [arg, yield] :
         toolchain::zip(op.getCond().getArguments(), forwarded_operands)) {
      if (arg != yield) {
        return op.emitOpError()
...
struct WhileRegionExplicitCast : public OpRewritePattern<WhileRegionOp> {
  ...
  LogicalResult matchAndRewrite(WhileRegionOp while_op,
    ...
    bool changed = false;
    for (int op_idx : toolchain::seq<int>(0, while_op.getNumOperands())) {
...
struct WhileRegionEliminatePassThrough
  ...
  LogicalResult matchAndRewrite(WhileRegionOp while_op,
    ...
    toolchain::BitVector removed_operand(old_num_operands);

    for (int op_idx : toolchain::seq<int>(0, old_num_operands)) {
      auto body_arg = body_block.getArgument(op_idx);
      auto cond_arg = cond_block.getArgument(op_idx);
...
LogicalResult XlaBroadcastHelperOp::inferReturnTypeComponents(
  ...
  RankedTensorType lhs_ty = toolchain::dyn_cast<RankedTensorType>(lhs.getType());
  RankedTensorType rhs_ty = toolchain::dyn_cast<RankedTensorType>(rhs.getType());
  ...
  if (dims.empty()) {
    ...
    inferredReturnShapes.emplace_back(toolchain::cast<ShapedType>(lhs_ty));
    inferredReturnShapes.emplace_back(toolchain::cast<ShapedType>(rhs_ty));
    return success();
  }
  ...
  toolchain::SmallVector<int64_t, 4> broadcast_shape(output_rank, 1LL);
  toolchain::SmallVector<bool, 4> is_broadcasted(output_rank, false);
  for (const auto &item : toolchain::enumerate(dims)) {
...
LogicalResult XlaSetDynamicDimensionSizeOp::inferReturnTypeComponents(
  ...
  TensorType operand_ty = toolchain::cast<TensorType>(op.getInput().getType());
  Type element_ty = operand_ty.getElementType();

  TensorType result_ty;
  if (operand_ty.hasRank()) {
    auto shape = toolchain::to_vector<4>(operand_ty.getShape());
    ...
    result_ty = machina::GetTypeFromTFTensorShape(shape, element_ty);
  } else {
    result_ty = UnrankedTensorType::get(element_ty);
  }

  inferredReturnShapes.emplace_back(toolchain::cast<ShapedType>(result_ty));
  return success();
}
...
LogicalResult XlaReduceWindowOp::verify() {
  ...
  const auto &input_ty = toolchain::cast<ShapedType>(op.getInput().getType());
...
LogicalResult XlaSelectAndScatterOp::verify() {
  ...
  auto input_ty = toolchain::cast<ShapedType>(op.getOperand().getType());
...
LogicalResult XlaVariadicReduceOp::verify() {
  ...
  const auto &dtype = toolchain::cast<TensorType>(input_ty[0]).getElementType();
  for (const auto &ty : input_ty) {
    if (toolchain::cast<TensorType>(ty).getElementType() != dtype)
      return op.emitOpError()
...
LogicalResult XlaVariadicReduceV2Op::verify() {
  ...
  auto input_ty_0 = toolchain::cast<ShapedType>(inputs_ty[0]);
  if (input_ty_0.hasStaticShape()) {
    for (int i = 1; i < n_inputs; ++i) {
      auto input_ty_i = toolchain::cast<ShapedType>(inputs_ty[i]);
  ...
  for (int i = 0; i < n_inputs; ++i) {
    auto init_value_ty_i = toolchain::cast<ShapedType>(init_values_ty[i]);
...
LogicalResult SetStaticDimensionBoundsOp::verify() {
  ...
  mlir::ShapedType input_type = toolchain::cast<ShapedType>(op.getInput().getType());
  mlir::ShapedType static_shape_type =
      toolchain::cast<ShapedType>(op.getStaticShape().getType());
...
LogicalResult VerifyScalesAndZeroPoints(UniformQuantizedOp op, Value scales,
  ...
  ShapedType scales_type = toolchain::cast<ShapedType>(scales.getType());
  ShapedType zero_points_type = toolchain::cast<ShapedType>(zero_points.getType());
...
MutableOperandRange YieldOp::getMutableSuccessorOperands(
    RegionBranchPoint point) {
  if (auto whileOp =
          toolchain::dyn_cast<WhileRegionOp>(this->getOperation()->getParentOp())) {
  ...
  } else if (auto regionOp = toolchain::dyn_cast<GeneratorDatasetRegionOp>(
...
#include "machina/compiler/mlir/machina/ir/tf_ops_n_z.cc.inc"
