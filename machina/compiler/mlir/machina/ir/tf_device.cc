...
TensorFlowDeviceDialect::TensorFlowDeviceDialect(MLIRContext* context)
  ...
  addOperations<
#define GET_OP_LIST
#include "machina/compiler/mlir/machina/ir/tf_device.cc.inc"
      >();
...
LogicalResult ParallelExecuteOp::verify() {
  ...
  int output_index = 0;
  for (const auto& region_and_index : toolchain::enumerate(regions)) {
...
ParseResult ParseReplicateOpOperands(
    ...
    toolchain::SmallVectorImpl<toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8>>*
        replicated_inputs,
    toolchain::SmallVectorImpl<OpAsmParser::UnresolvedOperand>* packed_inputs,
    toolchain::SmallVectorImpl<OpAsmParser::UnresolvedOperand>* region_args,
    toolchain::SmallVectorImpl<Type>* region_arg_types) {
  ...
  toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8> replicated_region_args;
  toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8> packed_region_args;
  toolchain::SmallVector<Type, 8> replicated_region_arg_types;
  toolchain::SmallVector<Type, 8> packed_region_arg_types;
  do {
...
ParseResult SetReplicateOpOperands(
    toolchain::SMLoc loc, OpAsmParser* parser, OperationState* state,
    toolchain::ArrayRef<toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8>>
        replicated_inputs,
    toolchain::ArrayRef<OpAsmParser::UnresolvedOperand> packed_inputs,
    toolchain::ArrayRef<Type> region_arg_types, int32_t* n) {
  ...
  for (const auto& replicated_input_and_idx :
       toolchain::enumerate(replicated_inputs)) {
  ...
  for (const auto& packed_input_and_idx : toolchain::enumerate(packed_inputs)) {
...
ParseResult ReplicateOp::parse(OpAsmParser& parser, OperationState& result) {
  ...
  toolchain::SmallVector<toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8>, 8>
      replicated_inputs;
  toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8> packed_inputs;
  toolchain::SmallVector<OpAsmParser::UnresolvedOperand, 8> region_args;
  toolchain::SmallVector<Type, 8> region_arg_types;
  int32_t n = 0;
  ...
  for (auto argAndType : toolchain::zip(region_args, region_arg_types)) {
...
void BuildReplicateOp(
    ...
    toolchain::ArrayRef<std::pair<ValueRange, Type>> replicated_inputs,
  ...
  for (auto& replicated_input : replicated_inputs) {
    DCHECK_EQ(toolchain::size(replicated_input.first), n);
  ...
  for (const auto& output_type : replica_output_types)
    state->addTypes(toolchain::SmallVector<Type, 8>(n, output_type));
}
...
LogicalResult ReplicateOp::verify() {
  ...
  if (op.getDevices().has_value()) {
    for (auto device_attr : op.getDevices().value().getValue()) {
      ...
      bool is_device_string = toolchain::all_of(device_list, [](Attribute attr) {
  ...
  for (const auto& operand_type_and_idx :
       toolchain::enumerate(terminator.getOperandTypes())) {
...
void ReplicateOp::build(
    ...
    const toolchain::SmallDenseMap<StringRef, toolchain::SmallVector<StringRef, 4>>&
        devices,
    toolchain::ArrayRef<std::pair<ValueRange, Type>> replicated_inputs,
    ValueRange packed_inputs, TypeRange replica_output_types) {
  std::optional<DictionaryAttr> devices_attr;
  if (!devices.empty()) {
    toolchain::SmallVector<mlir::NamedAttribute, 1> device_list;
...
toolchain::ArrayRef<BlockArgument> ReplicateOp::GetReplicatedBlockArguments() {
...
// is being modified.
toolchain::ArrayRef<BlockArgument> ReplicateOp::GetPackedBlockArguments() {
...
static LogicalResult EliminatePassThroughResults(ClusterOp op,
  ...
  for (OpOperand& operand : return_op->getOpOperands()) {
    ...
    bool is_used_for_resource_write = toolchain::any_of(
