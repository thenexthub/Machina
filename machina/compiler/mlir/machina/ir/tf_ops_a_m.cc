...
OpFoldResult AddNOp::fold(FoldAdaptor adaptor) {
  ...
  auto IsKnownZero = [](Attribute attr) {
    ...
    if (mlir::isa<FloatType>(element_ty))
      return splat.getSplatValue<toolchain::APFloat>().isZero();
    if (mlir::isa<IntegerType>(element_ty))
      return splat.getSplatValue<toolchain::APInt>().getSExtValue() == 0;
    return false;
  };

  for (const auto& it : toolchain::enumerate(operands)) {
...
template <typename OpT,
          typename std::enable_if<toolchain::is_one_of<
...
static LogicalResult Verify(OpT op) {
  ...
  if (std::is_same<OpT, BatchMatMulOp>()) {
    for (const auto& dim_pairs : toolchain::zip(x_batches, y_batches)) {
...
LogicalResult BatchToSpaceOp::verify() {
  ...
  toolchain::SmallVector<int64_t, 4> input_shape(4, ShapedType::kDynamic);
  ...
  toolchain::SmallVector<int64_t, 4> crops_values;
  ...
  if (output_type.hasRank()) {
    ...
    auto check_spatial_dim = [&](int64_t spatial_dim_index,
                                 toolchain::StringRef dim_name,
                                 toolchain::StringRef crop_a_name,
                                 toolchain::StringRef crop_b_name) -> LogicalResult {
...
LogicalResult BiasAddOp::verify() {
  ...
  if (format == machina::TensorFormat::FORMAT_NHWC) {
    ...
    DCHECK_EQ(format, machina::TensorFormat::FORMAT_NCHW);
  ...
  int64_t feature_dim_idx =
      machina::GetTensorFeatureDimIndex(value_ty.getRank(), format);
...
LogicalResult BroadcastGradientArgsOp::fold(
  ...
  auto build_out_dense_element = [](SmallVectorImpl<int64_t>& shape,
    ...
    RankedTensorType type = machina::GetTypeFromTFTensorShape(
...
static LogicalResult VerifyCaseOrIfOpBranchFunctions(
    ...
    toolchain::function_ref<std::string(unsigned branch_index)> branch_name) {
  SmallVector<FunctionType, 2> branch_types;
  branch_types.reserve(branches.size());

  if (toolchain::any_of(op->getOperands(),
  ...
  for (const auto& branch : toolchain::enumerate(branches)) {
  ...
  for (int i = 0; i < expected_num_inputs; ++i) {
    ...
    if (!AreCastCompatible(branch_input_i_types)) {
      ...
      toolchain::raw_string_ostream os(input_types_str);
      toolchain::interleaveComma(branch_input_i_types, os);
      return op->emitOpError()
...
LogicalResult CaseOp::verifySymbolUses(SymbolTableCollection& symbol_table) {
  auto branch_name = [](unsigned index) {
    return toolchain::formatv("branch #{0}", index).str();
  };
...
LogicalResult CaseRegionOp::verify() {
  ...
  for (const auto& region_and_idx : toolchain::enumerate(op.getBranches())) {
    std::string description =
        toolchain::formatv("branch #{0} result", region_and_idx.index()).str();
...
class CaseOrIfRegionEliminatePassThrough
  ...
  LogicalResult matchAndRewrite(CaseOrIfRegionOp op,
    ...
    toolchain::SmallDenseMap<Value, Value, 4> result_to_extern_value;
    ...
    for (auto region_index : toolchain::seq<int>(0, branches.size()))
...
template <typename OpT, typename std::enable_if<toolchain::is_one_of<
...
LogicalResult HoistCwiseUnaryOutOfConcat::matchAndRewrite(
  ...
  bool args_same_op = toolchain::all_of(op.getValues(), [&](Value arg) -> bool {
  ...
  auto unary_operands = toolchain::map_range(op.getValues(), [](Value arg) -> Value {
...
class HoistCwiseBinaryOutOfConcat : public OpRewritePattern<TF::ConcatV2Op> {
 ...
 private:
  ...
  std::optional<HoistParams> GetHoistParams(
      ...
      const toolchain::SmallDenseMap<Value, unsigned, 4>& exceptions) const;
};
...
LogicalResult HoistCwiseBinaryOutOfConcat::matchAndRewrite(
  ...
  toolchain::SmallDenseMap<Value, unsigned, 4> exceptions;
  ...
  auto pack_or_concat = [&](bool is_scalar, Type result_type, ValueRange args,
    ...
    auto axis_type = machina::GetTypeFromTFTensorShape(
...
HoistCwiseBinaryOutOfConcat::GetHoistParams(
  ...
  auto args = [&](int operand_idx) -> SmallVector<Value, 8> {
    auto range = toolchain::map_range(op.getValues(), [&](Value arg) {
  ...
  auto is_all_tensors = [&](int operand_idx, int axis) -> bool {
    return toolchain::all_of(op.getValues(), [&](Value arg) -> bool {
      mlir::Value operand;
  ...
  if (is_all_tensors(0, axis) && is_all_scalars(1)) {
    ...
    auto rhs_type =
        machina::GetTypeFromTFTensorShape(rhs_dims, ranked.getElementType());
    ...
    std::array<int64_t, 1> lhs_dims{
        static_cast<int64_t>(op.getValues().size())};
    auto lhs_type =
        machina::GetTypeFromTFTensorShape(lhs_dims, ranked.getElementType());
    return HoistParams{args(0),
                       args(1),
                       0,
                       axis,
...
LogicalResult ConcatOffsetOp::verify() {
  ...
  for (const auto& shape_offset_idx :
       toolchain::enumerate(toolchain::zip(op.getShape(), op.getOffset()))) {
...
LogicalResult ConcatOffsetOp::fold(FoldAdaptor adaptor,
  ...
  toolchain::SmallVector<DenseIntElementsAttr, 4> shapes;
  shapes.reserve(operands.size() - 1);
  for (Attribute shape : toolchain::drop_begin(operands, 1))
  ...
  for (DenseIntElementsAttr shape : toolchain::drop_begin(shapes, 1))
  ...
  for (DenseIntElementsAttr shape : toolchain::drop_begin(shapes, 1)) {
    for (const auto& dims_and_idx : toolchain::enumerate(toolchain::zip(shape0, shape))) {
  ...
  if (getShapeType().isInteger(32)) {
    ...
    RankedTensorType offset_type = machina::GetTypeFromTFTensorShape(
...
void ConstOp::build(OpBuilder& builder, OperationState& result,
  ...
  if (auto elem_attr = mlir::dyn_cast<ElementsAttr>(value)) {
    ...
    type = machina::GetTypeFromTFTensorShape(/*shape=*/{},
                                                typed_attr.getType());
...
static LogicalResult VerifyConvOpAttributes(
  ...
  if (toolchain::any_of(strides, is_not_positive))
  ...
  if (toolchain::any_of(dilations, is_not_positive))
...
static LogicalResult Verify(OpT op) {
  ...
  if (padding == machina::Padding::EXPLICIT) {
    ...
    if (toolchain::any_of(explicit_padding, is_negative))
      return emitOptionalError(op.getLoc(),
  ...
  if (auto ty = mlir::dyn_cast<RankedTensorType>(op.getInput().getType())) {
    ...
    DCHECK(is_valid) << data_format;
    int idx = machina::GetTensorFeatureDimIndex(num_dims, format);
  ...
  if (auto ty = mlir::dyn_cast<RankedTensorType>(op.getFilter().getType())) {
    int idx = machina::GetFilterTensorInputChannelsDimIndex(
        num_dims, machina::FORMAT_HWIO);
...
static LogicalResult inferConvReturnTypeComponents(
    ...
    ArrayRef<Attribute> explicit_padding,
    toolchain::SmallVectorImpl<ShapedTypeComponents>& inferredReturnShapes) {
  ...
  // machina/core/util/tensor_format.h.
  if (input_ty.hasRank()) {
...
StringRef Conv2DOp::GetOptimalLayout(const RuntimeDevices& devices) {
  ...
  auto all_ones = [](ArrayAttr arr) -> bool {
    return toolchain::all_of(arr, [](Attribute attr) -> bool {
...
class DivNoNanOrMulNoNanConstantY : public OpRewritePattern<OpT> {
  ...
  LogicalResult matchAndRewrite(OpT op,
                                PatternRewriter& rewriter) const override {
    static_assert(
        toolchain::is_one_of<OpT, DivNoNanOp, MulNoNanOp>::value,
...
LogicalResult DynamicStitchOp::verify() {
  ...
  toolchain::SmallDenseSet<int64_t, 8> index_values;
  bool all_indices_const = true;
  int32_t max_index = -1;
  std::optional<SmallVector<int64_t, 4>> inferred_item_shape;
  for (auto it : toolchain::zip(op.getIndices(), op.getData())) {
    ...
    if (!inferred_item_shape) {
      inferred_item_shape = toolchain::to_vector<4>(item_shape);
      continue;
    }
    ...
    if (failed(mlir::verifyCompatibleShape(item_shape, *inferred_item_shape)))
      return op.emitOpError() << "has inconsistent shaped data and index "
                              ...
                              << toolchain::ArrayRef(*inferred_item_shape)
  ...
  if (all_indices_const) {
    ...
    if (inferred_item_shape) {
      ...
      auto expected_out_ty = machina::GetTypeFromTFTensorShape(
...
OpFoldResult EmptyOp::fold(FoldAdaptor adaptor) {
  ...
  if (auto float_type = mlir::dyn_cast<FloatType>(etype)) {
    auto out_type = machina::GetTypeFromTFTensorShape(out_shape, float_type);
  ...
  if (auto int_type = mlir::dyn_cast<IntegerType>(etype)) {
    auto out_type = machina::GetTypeFromTFTensorShape(out_shape, etype);
...
std::string GetAbsDeviceStr(Operation* op, uint64_t device_ordinal) {
  ...
  // toolchain::StringRef and absl::string_view works.
...
OpFoldResult EnsureShapeOp::fold(FoldAdaptor) {
  ...
  std::optional<toolchain::ArrayRef<int64_t>> shape_constraint = getShape();
...
Type InferExpandDimsOpType(Value input, Value dim) {
  ...
  SmallVector<int64_t, 4> shape = toolchain::to_vector<4>(input_ty.getShape());
...
LogicalResult FakeQuantWithMinMaxArgsOp::verify() {
  ...
  const toolchain::fltSemantics& semantics = op.getMin().getSemantics();
  float rmin, rmax;
...
static ShapedType InferFillOpType(Value dims, Value value) {
  ...
  if (matchPattern(dims, m_Constant(&dims_attr))) {
    ...
    return machina::GetTypeFromTFTensorShape(shape, etype);
  }
...
OpFoldResult FillOp::fold(FoldAdaptor adaptor) {
  ...
  type = machina::GetTypeFromTFTensorShape(shape, type.getElementType());
...
LogicalResult FoldConstantIfRegionOp::matchAndRewrite(
  ...
  auto updated_results = toolchain::to_vector<4>(yield.getOperands());

  // If the yield types do not match the IfRegion result types, add appropriate
  // casts.
  rewriter.setInsertionPoint(yield);
  for (auto it : toolchain::zip(op.getResultTypes(), updated_results)) {
...
LogicalResult MeanOp::FoldOperandsPermutation(ArrayRef<int64_t> permutation) {
  ...
  toolchain::transform(reductions_value.getValues<APInt>(),
...
#include "machina/compiler/mlir/machina/ir/tf_ops_a_m.cc.inc"
