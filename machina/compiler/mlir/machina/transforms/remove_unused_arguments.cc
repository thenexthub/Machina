...
// the same value.
toolchain::BitVector GetInvariantReturns(Region* region, int number_of_results) {
  toolchain::BitVector invariant(number_of_results, true);
...
void EraseReturnOperands(Region* region, toolchain::BitVector& erase) {
  for (Block& block : region->getBlocks()) {
    for (Operation& op : block.getOperations()) {
      if (!op.hasTrait<OpTrait::ReturnLike>()) continue;
...
void EraseResults(Operation* op, toolchain::BitVector erase) {
  ...
  for (const auto& indexed_regions : toolchain::enumerate(op->getRegions())) {
...
void RemoveUnusedArgumentsPass::runOnOperation() {
  ...
  toolchain::DenseMap<Operation*, toolchain::BitVector> args_to_erase;
  toolchain::DenseMap<Operation*, toolchain::BitVector> results_to_erase;
  toolchain::DenseMap<Operation*, toolchain::DenseMap<int, int>> args_to_remap;
  toolchain::DenseSet<Operation*> do_not_touch;  // Funcs referenced by non-call ops
  ...
  module->walk([&](SymbolUserOpInterface op) {
    if (toolchain::isa<CallOpInterface>(op.getOperation())) return;
  ...
  module->walk([&](SymbolOpInterface op) {
    ...
    auto call = toolchain::dyn_cast<CallableOpInterface>(op.getOperation());
    if (!call) return;
    Region* region = call.getCallableRegion();
    if (!region) return;  // happens e.g. for external functions

    auto func = toolchain::dyn_cast<FunctionOpInterface>(op.getOperation());
    if (!func || do_not_touch.count(func)) return;
    toolchain::BitVector unused_args(func.getNumArguments());
    toolchain::BitVector unused_results(func.getNumResults());
    toolchain::DenseMap<int, int> return_to_operand;
    toolchain::BitVector invariant_returns =
        GetInvariantReturns(region, func.getNumResults());
    toolchain::DenseMap<Value, int> argument_to_index;
