...
using RegionVec2D = toolchain::SmallVector<toolchain::SmallVector<TF::IfRegionOp, 8>, 8>;
using OperationVec2D = toolchain::SmallVector<toolchain::SmallVector<Operation*, 8>, 8>;
using MapToRegionVec2D = toolchain::SmallDenseMap<Value, RegionVec2D>;
using MapToOperationVec2D = toolchain::SmallDenseMap<Value, OperationVec2D>;
using IfOpIterConst =
    toolchain::SmallVectorTemplateCommon<mlir::TF::IfRegionOp>::const_iterator;
...
toolchain::SmallSetVector<Operation*, 4> GetAllOpsFromIf(TF::IfRegionOp if_op) {
  toolchain::SmallSetVector<Operation*, 4> all_ops;
...
bool SafeToMerge(TF::IfRegionOp first_if, TF::IfRegionOp second_if,
                 toolchain::SmallSetVector<Operation*, 4>& downstream_if_ops,
  ...
  toolchain::SmallSetVector<Operation*, 4> destination_ops =
  ...
  toolchain::SmallPtrSet<Operation*, 4> visited;
...
bool CanAddToIfSegment(
    ...
    const toolchain::SmallVector<mlir::TF::IfRegionOp, 8>& if_ops,
...
absl::flat_hash_set<Operation*> GetMoveOpsBetweenTwoIfRegions(
    ...
    toolchain::SmallSetVector<Operation*, 4> middle_if_ops,
    const std::unique_ptr<TF::SideEffectAnalysis>& side_effect_analysis) {
...
toolchain::SmallVector<Operation*, 8> GetMoveOpList(
    toolchain::SmallVector<TF::IfRegionOp, 8>& sub_if_group,
    const std::unique_ptr<TF::SideEffectAnalysis>& side_effect_analysis) {
  ...
  toolchain::SmallVector<Operation*, 8> moved_ops_ordered;
...
void GenerateSegmentsPerIfGroups(
  ...
  toolchain::SmallVector<TF::IfRegionOp, 8> sub_merged_groups;
...
toolchain::SmallVector<int, 4> GetReturnIndicesToKeep(
    TF::IfRegionOp current_if_op,
    const toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment) {
  toolchain::SmallVector<int, 4> return_indices_to_keep;
  ...
  for (const auto& index_and_value :
       toolchain::enumerate(current_if_op.getResults())) {
    if (!toolchain::all_of(index_and_value.value().getUsers(),
...
toolchain::SmallVector<toolchain::SmallVector<int, 4>> GetReturnIndicesVec(
    const toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment) {
  toolchain::SmallVector<toolchain::SmallVector<int, 4>> return_indices_vec;
  for (auto it = if_op_segment.begin(); it != if_op_segment.end(); ++it) {
    toolchain::SmallVector<int, 4> indices_to_keep_vec =
...
void ReplaceInternalUsage(toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment) {
  for (auto it = if_op_segment.begin(); it != if_op_segment.end(); ++it) {
...
void ReplaceExternalUsage(
    toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
    TF::IfRegionOp new_if_op,
    toolchain::SmallVector<toolchain::SmallVector<int, 4>>& return_indices) {
  int new_return_index = 0;
  for (const auto& index_and_value : toolchain::enumerate(if_op_segment)) {
...
void UpdateMovedOpList(
    toolchain::SmallDenseMap<Operation*, TF::IfRegionOp>& old_to_new_IfRegion_map,
    toolchain::SmallVector<Operation*, 8>& moved_ops_list) {
  toolchain::SmallDenseSet<TF::IfRegionOp> new_if_ops;
...
void CreateYieldOps(
    OpBuilder& builder, toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
    toolchain::SmallVector<toolchain::SmallVector<int, 4>>& return_indices,
    TF::IfRegionOp new_if_op, TF::IfRegionOp first_if) {
  toolchain::SmallVector<Value, 4> merged_then_yield_values;
  for (const auto& index_and_value : toolchain::enumerate(if_op_segment)) {
  ...
  toolchain::SmallVector<Value, 4> merged_else_yield_values;
  for (const auto& index_and_value : toolchain::enumerate(if_op_segment)) {
    auto if_op = index_and_value.value();
    for (auto i : return_indices[index_and_value.index()]) {
...
void MergeIfPerSegment(
    toolchain::SmallVector<TF::IfRegionOp, 8>& if_op_segment,
    toolchain::SmallVector<Operation*, 8>& moved_ops_list,
    toolchain::SmallSetVector<TF::IfRegionOp, 8>& regions_to_remove,
    toolchain::SmallDenseMap<Operation*, TF::IfRegionOp>& old_to_new_IfRegion_map) {
  TF::IfRegionOp first_if = if_op_segment[0];
  toolchain::SmallVector<Type, 4> merged_return_types;
  toolchain::SmallVector<TF::IfRegionOp, 8> sources_if_ops(
  ...
  for (const auto& index_and_value : toolchain::enumerate(return_indices)) {
  ...
  auto new_if_op = builder.create<TF::IfRegionOp>(
      ...
      toolchain::all_of(if_op_segment,
...
void OptimizeIfRegions(Block* block, ModuleOp module) {
  ...
  toolchain::SmallDenseMap<Value, toolchain::SmallVector<TF::IfRegionOp, 8>, 8>
      grouped_if_ops;
  toolchain::SmallVector<Value, 4> if_cond_order;
