...
bool AncestorUsesVirtualDevice(
  ...
  if (!op) return false;
  if (toolchain::isa<tf_device::ReplicateOp>(op)) return false;
...
void MakeShapeOpInvariant(tf_device::ReplicateOp replicate_op, int num_replicas,
  ...
  // operand.
  auto read_var_op = toolchain::dyn_cast<TF::ReadVariableOp>(input_def);
...
bool IsOpReplicateInvariant(Region* replicate_region, Operation* op) {
  ...
  if (toolchain::isa<TF::_TPUDeviceOrdinalPlaceholderOp>(op)) return false;
...
void HoistReplicateInvariantOps(tf_device::ReplicateOp replicate_op) {
  ...
  for (Operation& inner_op :
       toolchain::make_early_inc_range(replicate_op.GetBody())) {
    if (toolchain::isa<tf_device::ReturnOp>(inner_op)) continue;
