...
LogicalResult HasNoNestedEntryFunctions(
    const toolchain::SmallVector<func::FuncOp> &entry_funcs, SymbolTable &symtab) {
  ...
  for (auto &entry_func : entry_funcs) {
    toolchain::SmallVector<SymbolUserOpInterface> calls;
...
void XlaValidateInputsPass::runOnOperation() {
  ...
  toolchain::SmallVector<func::FuncOp> entry_funcs = GetEntryFunctions(module);
