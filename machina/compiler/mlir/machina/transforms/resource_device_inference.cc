...
class PerFunctionResult {
 ...
 private:
  toolchain::SmallDenseMap<int64_t, StringRef, 8> resource_id_to_device_;
...
void dump(StringRef message, Operation* op) {
  toolchain::dbgs() << message;
  op->print(toolchain::dbgs(), OpPrintingFlags().enableDebugInfo(
  ...
  WalkResult walk_res = func_op.walk<WalkOrder::PreOrder>([&](Operation* op) {
    if (auto var_handle = dyn_cast<VarHandleOp>(op)) {
      ...
      for (auto output : filter_resources(op->getResults())) {
        LLVM_DEBUG(toolchain::dbgs() << "  Processing output #"
                                << output.getResultNumber() << "\n");
        if (auto device = result->DeviceForResource(output)) {
          LLVM_DEBUG(toolchain::dbgs() << " Setting device = " << *device << "\n");
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Visiting WhileRegion\n");
...
void ResourceDeviceInference::runOnOperation() {
  ...
  toolchain::SmallDenseMap<func::FuncOp, PerFunctionResult, 4> per_function_results;
  toolchain::SetVector<func::FuncOp> worklist;
