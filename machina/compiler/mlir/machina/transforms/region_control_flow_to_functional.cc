...
struct RegionControlFlowToFunctional
 ...
 private:
  ...
  machina::OpOrArgLocNameMapper mapper;
  toolchain::SmallVector<func::FuncOp, 4> worklist;
};
...
toolchain::SmallVector<Value, 4> CollectExternValues(mlir::RegionRange regions) {
  toolchain::SetVector<Value> extern_values;

  for (Region* region : regions) {
    toolchain::SetVector<Value> region_extern_values;
  ...
  return toolchain::to_vector<4>(extern_values);
}
...
std::optional<StringRef> UnwrapSingleFunctionCall(Region& region) {
  ...
  TF::YieldOp yield =
      toolchain::dyn_cast_or_null<TF::YieldOp>(block.getTerminator());
  if (!yield) return std::nullopt;
  func::CallOp call = toolchain::dyn_cast_or_null<func::CallOp>(*block.begin());
  if (!call) return std::nullopt;
  if (block.getNumArguments() != call.getNumOperands() ||
      call.getNumResults() != yield.getNumOperands())
    return std::nullopt;
  for (auto [arg, operand] :
       toolchain::zip(block.getArguments(), call.getOperands())) {
    if (arg != operand) return std::nullopt;
  }
  for (auto [ret, operand] :
       toolchain::zip(call.getResults(), yield.getOperands())) {
...
StringRef ExtractSingleBlockRegion(
    ...
    toolchain::SmallVectorImpl<Value>& extern_values,
    toolchain::SmallVectorImpl<func::FuncOp>& worklist,
  ...
  auto input_types = toolchain::to_vector<4>(entry.getArgumentTypes());
  ...
  auto return_types = toolchain::to_vector<4>(terminator->getOperandTypes());
  ...
  for (auto it : toolchain::enumerate(extern_values)) {
  ...
  auto return_values = toolchain::to_vector<4>(terminator->getOperands());
  if (only_one_return_value) {
...
std::optional<func::CallOp> IsSingleCallRegion(Region& region,
  ...
  if (!toolchain::hasSingleElement(region)) return std::nullopt;
  ...
  if (allow_to_bool && call.getNumResults() == 1 &&
    ...
    for (auto [yield_operand, block_arg] :
         toolchain::zip(yield->getOperands().drop_front(1), block.getArguments())) {
  ...
  for (auto res_it : toolchain::zip(call.getResults(), call_consumer->getOperands()))
...
bool MatchCallArgs(func::CallOp first, func::CallOp second,
  ...
  for (auto it : toolchain::zip(first.getArgOperands(), second.getArgOperands())) {
    // Get the defining Op, skipping over casts.
    auto get_defining_op = [](Value value) {
      while (auto cast_op =
                 toolchain::dyn_cast_or_null<CastOp>(value.getDefiningOp())) {
...
struct TrivialTransformInfo {
  ...
  toolchain::SmallVector<StringRef, 2> callee_names;
...
LogicalResult RegionControlFlowToFunctional::ConvertIfOp(
  ...
  toolchain::SmallVector<Value, 4> extern_values;
...
LogicalResult RegionControlFlowToFunctional::ConvertCaseOp(
  ...
  toolchain::SmallVector<mlir::Attribute, 2> branch_symbols;
  ...
  for (const auto& item : toolchain::enumerate(case_region.getRegions())) {
    std::string branch_name =
        GetName(case_region, toolchain::formatv("_branch{0}", item.index()).str());
...
LogicalResult RegionControlFlowToFunctional::ConvertWhileOp(
  ...
  auto new_inputs = toolchain::to_vector<4>(while_region.getOperands());

  // All existing results will also be generated by the functional while.
  auto new_result_types = toolchain::to_vector<4>(while_region.getResultTypes());
...
void RegionControlFlowToFunctional::runOnOperation() {
  ...
  worklist = toolchain::to_vector<4>(module.getOps<func::FuncOp>());
  while (!worklist.empty()) {
    func::FuncOp function = worklist.pop_back_val();

    auto result = function.walk([&](Operation* op) {
      if (auto if_region = toolchain::dyn_cast<IfRegionOp>(op)) {
        if (failed(ConvertIfOp(symbol_table, if_region))) {
          op->emitOpError() << "failed to convert to functional form";
          return WalkResult::interrupt();
        }
      } else if (auto case_region = toolchain::dyn_cast<CaseRegionOp>(op)) {
        if (failed(ConvertCaseOp(symbol_table, case_region))) {
          op->emitOpError() << "failed to convert to functional form";
          return WalkResult::interrupt();
        }
      } else if (auto while_region = toolchain::dyn_cast<WhileRegionOp>(op)) {
        if (failed(ConvertWhileOp(symbol_table, while_region))) {
          op->emitOpError() << "failed to convert to functional form";
          return WalkResult::interrupt();
        }
      } else if (auto gen = toolchain::dyn_cast<GeneratorDatasetRegionOp>(op)) {
