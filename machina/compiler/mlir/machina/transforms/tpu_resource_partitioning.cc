...
bool AllResourceTypesHaveSubtypes(TypeRange resources) {
  for (Type resource : resources)
    if (!toolchain::hasSingleElement(
            toolchain::cast<tf_type::ResourceType>(
                toolchain::cast<TensorType>(resource).getElementType())
                .getSubtypes()))
      return false;
...
Type GetResourceSubtype(Type type) {
  return toolchain::cast<tf_type::ResourceType>(
             toolchain::cast<TensorType>(type).getElementType())
      .getSubtypes()
      .front();
}
...
bool IsCompositeDevice(mlir::Attribute attr) {
  const auto str_attr = toolchain::dyn_cast_if_present<StringAttr>(attr);
  return str_attr &&
         (str_attr.getValue().find("COMPOSITE") != toolchain::StringRef::npos);
}
}  // namespace
...
LogicalResult PartitionResourceReadsWrites(
  ...
  for (Value result : parallel_execute.getExecuteOutputs()) {
    ...
    auto assign_var =
        toolchain::dyn_cast<TF::AssignVariableOp>(*result.getUsers().begin());
    if (!assign_var || assign_var.getValue() != result) continue;
    auto partitioned_input =
        toolchain::dyn_cast_or_null<TF::TPUPartitionedInputV2Op>(
    ...
    toolchain::SmallVector<Type, 4> partitioned_output_types;
    ...
    for (auto [i, value] : toolchain::enumerate(partitioned_output.getOutput())) {
  ...
  for (OpOperand& operand : cluster_func->getOpOperands()) {
    auto read_var = toolchain::dyn_cast_or_null<TF::ReadVariableOp>(
    ...
    toolchain::SmallMapVector<Value, Value, 4> read_variable_ops;
    toolchain::SmallVector<Value, 4> partitioned_reads;
...
void TPUResourceReadsWritesPartitioningPass::runOnOperation() {
  toolchain::SmallVector<tf_device::ClusterFuncOp, 4> cluster_funcs;
