...
std::string GetHost(toolchain::StringRef device) {
...
bool IsOnLocalHost(toolchain::StringRef device) {
...
struct FunctionMetadata {
  ...
  toolchain::SmallVector<std::string, 4> input_devices;
  // The devices of the result values. It should have the same size as results.
  toolchain::SmallVector<std::string, 4> result_devices;
  // The operations to be included in the body of the function.
  toolchain::SmallVector<Operation *, 4> ops;
...
// the function body.
std::optional<toolchain::StringMap<FunctionMetadata>> GetFunctionMetadatas(
    func::FuncOp func_op) {
  toolchain::StringMap<FunctionMetadata> metadatas;
  WalkResult result = func_op.getBody().walk([&](Operation *op) {
    ...
    for (Value value : op->getOperands()) {
      ...
      if (BlockArgument block_arg = mlir::dyn_cast<BlockArgument>(value)) {
        ...
        if (toolchain::find(func_metadata.inputs, value) ==
      ...
      if (op->hasTrait<OpTrait::IsTerminator>()) {
        if (toolchain::find(defining_func_metadata.results, value) ==
...
void CreateFunctions(ModuleOp module_op,
  ...
  for (auto &iter : metadatas) {
    toolchain::StringRef host = iter.first();
    ...
    // function.
    for (int i : toolchain::seq<int>(0, metadata.input_devices.size())) {
      func_op.setArgAttr(i, kTFDeviceAttr,
                         StringAttr::get(context, metadata.input_devices[i]));
    }
    for (int i : toolchain::seq<int>(0, metadata.result_devices.size())) {
    ...
    IRMapping mapping;
    for (int i : toolchain::seq<int>(0, metadata.inputs.size())) {
...
void CreateRemoteRunCalls(MLIRContext *context,
                          const toolchain::StringMap<FunctionMetadata> &metadatas) {
  IRMapping mapping;
  for (auto &iter : metadatas) {
    toolchain::StringRef host = iter.first();
    ...
    Location loc = metadata.ops.front()->getLoc();
    toolchain::SmallVector<Value, 4> inputs_after_mapping;
    for (Value input : metadata.inputs) {
    ...
    for (int i : toolchain::seq<int>(0, metadata.results.size())) {
...
class ClusterTFOpsByHostPass
  ...
  void runOnOperation() override {
    ...
    for (auto func_op : original_func) {
      std::optional<toolchain::StringMap<FunctionMetadata>> metadatas =
