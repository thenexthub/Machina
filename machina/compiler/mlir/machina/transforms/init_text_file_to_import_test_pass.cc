...
void InitTextFileToImportTestPass::runOnOperation() {
  ...
  std::error_code error_code =
      toolchain::sys::fs::createTemporaryFile("text", "vocab", fd, filename);
  if (error_code) return signalPassFailure();

  toolchain::ToolOutputFile temp_file(filename, fd);
  temp_file.os() << "apple\n";
  temp_file.os() << "banana\n";
  temp_file.os() << "grape";
  temp_file.os().flush();
  ...
  for (func::FuncOp func : module.getOps<func::FuncOp>()) {
    toolchain::SmallVector<arith::ConstantOp, 4> constant_ops(
...
void InitTextFileToImportSavedModelTestPass::runOnOperation() {
  ...
  std::error_code error_code =
      toolchain::sys::fs::createUniqueDirectory("saved-model", tempdir);
  if (error_code) return signalPassFailure();
  error_code =
      toolchain::sys::fs::create_directories(Twine(tempdir) + "/assets", false);
  if (error_code) return signalPassFailure();
