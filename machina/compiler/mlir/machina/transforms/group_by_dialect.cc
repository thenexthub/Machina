...
class GroupByDialectPass
 ...
 private:
  void processFunction(mlir::func::FuncOp func, int& counter,
                       toolchain::SmallDenseSet<StringRef>& dialects,
                       Operation* module);
  void processRegion(mlir::Region& region, int& counter,
                     toolchain::SmallDenseSet<StringRef>& dialects,
                     Operation* module);

  toolchain::SmallDenseSet<StringRef> top_level_dialects_ = {"ml_program", "glue",
                                                        "func"};
};
...
void computeInputsOutputs(std::vector<Operation*>& ops,
  ...
  // All operations.
  toolchain::DenseSet<Operation*> all_operations;

  // All results of all ops.
  toolchain::DenseSet<Value> all_internal_results;
  ...
  toolchain::DenseSet<Value> inputs_seen;
  for (Operation* outer : ops) {
    outer->walk([&](Operation* op) {
  ...
  toolchain::DenseSet<Value> outputs_seen;
...
void wrapOpsInFunction(std::vector<Operation*>& ops, int function_id,
  ...
  auto call = builder.create<mlir::func::CallOp>(
      ...
      inputs);
  for (const auto& v : toolchain::enumerate(outputs)) {
...
void GroupByDialectPass::processFunction(
    ...
    toolchain::SmallDenseSet<StringRef>& dialects, Operation* module) {
  ...
  for (Block& block : region.getBlocks()) {
    ...
    for (Operation& op : block.getOperations()) {
      ...
      for (Region& region : op.getRegions()) {
        ...
        toolchain::SmallDenseSet<StringRef> parent_dialect = {dialect, "func"};
