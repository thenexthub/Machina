...
class RewriteXlaHostComputeMlir
 ...
 public:
  ...
  LogicalResult matchAndRewrite(TF::_XlaHostComputeMlirOp op,
    ...
    toolchain::SmallVector<Attribute> shape_attrs;
    ...
    if (!host_module.empty()) {
      ...
      cloned_func = toolchain::dyn_cast_or_null<func::FuncOp>(
      ...
      auto dynamic_key =
          rewriter.create<TF::_XlaCompileMlirPlaceholderProgramKeyOp>(
              func.getLoc(), /*program=*/result_type, toolchain::ArrayRef<Value>{});
      ...
      for (auto result :
           toolchain::zip(cloned_func.getArguments(), recv_at_host->getResults())) {
    ...
    if (auto sharding =
      ...
      if (!sharding.getValue().empty()) {
        ...
        func.getArgument(i).replaceAllUsesExcept(
            updated_arg, toolchain::SmallPtrSet<Operation*, 1>({updated_arg}));
      }
...
LogicalResult SetTokenInputAttrs(ModuleOp module) {
  ...
  machina::OpOrArgLocNameMapper name_mapper;
  MLIRContext* ctx = module.getContext();
  for (Region* region : regions_with_token) {
    ...
    auto token = StringAttr::get(ctx, machina::kXlaTokenArgNodeName);
    for (Operation& op : region->getOps()) {
      ...
      op.setAttr(machina::kXlaTokenInputNodesAttrName,
                 ArrayAttr::get(ctx, {token}));

      auto node_name = StringAttr::get(ctx, name_mapper.GetUniqueName(&op));
      op.setAttr(machina::kXlaOriginalOutsideCompilationNodeName, node_name);
      token = node_name;
    }
  }
  return success();
