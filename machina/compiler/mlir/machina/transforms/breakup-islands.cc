...
class BreakUpIslands : public TF::PerFunctionAggregateAnalysisConsumerPass<
 ...
 public:
  ...
  void BreakUpIsland(tf_executor::IslandOp island_op,
                     const TF::SideEffectAnalysis::Info& side_effect_analysis,
                     toolchain::DenseMap<Operation*, toolchain::SmallVector<Value, 4>>*
                         new_control_inputs);
};
...
void AddStatefulControlFlowDependencies(tf_executor::GraphOp graph_op) {
  toolchain::SmallDenseSet<Value, 8> graph_fetches;
...
void BreakUpIslands::runOnFunction(
  ...
  toolchain::DenseMap<Operation*, toolchain::SmallVector<Value, 4>> new_control_inputs;
  // Iterate in reverse order to avoid invalidating Operation* stored in
  // new_control_inputs.
  for (auto& item :
       toolchain::make_early_inc_range(toolchain::reverse(graph_op.GetBody()))) {
  ...
  // get invalidated.
  toolchain::SmallVector<Value, 8> operands;
  toolchain::SmallPtrSet<Operation*, 4> defining_ops;
  ...
  for (auto& item :
    ...
    for (Value new_control_input : toolchain::reverse(new_control_inputs_for_item)) {
...
void PopulateEmptyIsland(tf_executor::IslandOp island) {
  ...
  if (yield.getNumOperands() == 0) {
    ...
    for (auto it : toolchain::enumerate(identity_n.getResults()))
...
struct IslandSourcesAndSinks {
  ...
  toolchain::SmallPtrSet<Operation*, 4> sources;
  ...
  // executing.
  toolchain::SmallPtrSet<Operation*, 4> sinks;
};
...
void BreakUpIslands::BreakUpIsland(
  ...
  auto island_control_inputs = toolchain::to_vector<4>(island_op.getControlInputs());
  ...
  for (auto fetch : island_op.GetYield().getFetches()) {
    if (!fetch.getDefiningOp()) {
      ...
      continue;
    } else if (auto other_island_op = toolchain::dyn_cast<tf_executor::IslandOp>(
                   fetch.getDefiningOp())) {
  ...
  toolchain::SmallDenseMap<Operation*, Value, 8> new_control_for_sub_ops;
  // Control outputs of newly created islands that are sinks.
  toolchain::SmallVector<Value, 8> sink_island_controls;
  ...
  // new island.
  for (auto& sub_op : toolchain::make_early_inc_range(island_body)) {
    const auto predecessors =
        side_effect_analysis.DirectControlPredecessors(&sub_op);
    // Get the controls from the predecessors.
    toolchain::SmallVector<Value, 4> predecessor_controls;
  ...
  for (Value out : island_op.getOutputs()) {
    for (auto& use : out.getUses()) {
      ...
      if (owner->getDialect() == island_op->getDialect() &&
          !toolchain::isa<tf_executor::GraphOp, tf_executor::YieldOp,
