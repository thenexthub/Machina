...
void ModifyFunctionSignature(
    func::FuncOp func, toolchain::SmallDenseMap<Value, Value>* stack_var_to_size_var,
    toolchain::function_ref<std::optional<Type>(int64_t)> arg_to_stack_type,
    toolchain::function_ref<void(ArrayRef<BlockArgument>)> handle_new_size_vars =
        nullptr) {
...
struct PartitionedCallStackOpsInfo {
  ...
  toolchain::SmallDenseMap<int64_t, int64_t> stack_var_arg_to_size_arg;
};

LogicalResult DecomposeStackOpsInternal(
    Block*, ModuleOp, toolchain::SmallDenseMap<Value, Value>*,
    toolchain::StringMap<PartitionedCallStackOpsInfo>*);
...
LogicalResult HandleWhileOp(
    ...
    const toolchain::SmallDenseMap<Value, Value>& data_var_to_size_var,
    toolchain::StringMap<PartitionedCallStackOpsInfo>*
        decomposed_partitioned_call_callees) {
  auto body = while_op.body_function();
  toolchain::SmallDenseMap<Value, Value> body_map;
  ...
  auto add_size_vars_to_return = [&](ArrayRef<BlockArgument> new_args) {
    ...
    auto new_body_returns = toolchain::to_vector<8>(body_ret->getOperands());
  ...
  toolchain::SmallDenseMap<Value, Value> empty_map;
...
LogicalResult HandleIfOp(
  ...
  toolchain::SmallDenseMap<Value, Value> then_map;
  toolchain::SmallDenseMap<Value, Value> else_map;
  ...
  if (!signature_change) return success();
  auto new_if_operands = toolchain::to_vector<8>(if_op.getOperands());
...
LogicalResult HandlePartitionedCallOp(
  ...
  toolchain::SmallDenseMap<Value, Value> callee_map;
  ...
  if (!info.signature_change) {
    ...
    if (lowered_callee != callee) {
      ...
      lowered_callee.setName(StringAttr::get(
          callee->getContext(),
          toolchain::formatv("{0}_stack_decomposed", callee.getName()).str()));
...
LogicalResult HandleStackV2Op(
    ...
    toolchain::SmallDenseMap<Value, Value>* data_var_to_size_var) {
  ...
  auto elem_type = cutil::GetElementTypeFromAccess(
      stack.getHandle(), module, [](Operation* user) -> std::optional<Type> {
        auto push = toolchain::dyn_cast<TF::StackPushV2Op>(user);
...
LogicalResult DecomposeStackOpsInternal(
  ...
  for (auto& op : toolchain::make_early_inc_range(block->getOperations())) {
    if (toolchain::isa<TF::IdentityOp, TF::IdentityNOp>(&op)) {
      ...
      op.erase();
    } else if (auto stack = toolchain::dyn_cast<TF::StackV2Op>(&op)) {
      if (failed(HandleStackV2Op(stack, module, data_var_to_size_var))) {
        return failure();
      }
    } else if (auto push = toolchain::dyn_cast<TF::StackPushV2Op>(&op)) {
      if (failed(HandleStackPushV2Op(push, data_var_to_size_var))) {
        return failure();
      }
    } else if (auto pop = toolchain::dyn_cast<TF::StackPopV2Op>(&op)) {
      if (failed(HandleStackPopV2Op(pop, data_var_to_size_var))) {
        return failure();
      }
    } else if (auto close = toolchain::dyn_cast<TF::StackCloseV2Op>(&op)) {
    ...
    } else if (toolchain::isa<TF::WhileRegionOp>(op) ||
               toolchain::isa<TF::IfRegionOp>(op) ||
               toolchain::isa<TF::CaseRegionOp>(op)) {
      if (failed(
