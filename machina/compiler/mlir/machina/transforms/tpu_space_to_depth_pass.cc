...
void HandleFuncOp(Operation* op) {
  auto func = toolchain::cast<func::FuncOp>(op);
  UpdateFuncType(func);
}
...
void HandleConv2DStride(TF::Conv2DOp conv2d) {
  ...
  auto strides = ArrayAttr::get(context, toolchain::to_vector<4>(attrs));
...
bool IsSupportedHostInputOp(Operation* op) {
  TF::IteratorGetNextOp iter = toolchain::dyn_cast<TF::IteratorGetNextOp>(op);
...
bool HandleHostReplicatedInputs(int64_t index,
  ...
  for (auto entry : toolchain::enumerate(inputs)) {
...
void HandleCluster(tf_device::ClusterFuncOp cluster_func, int32_t block_size,
  ...
  auto maybe_replicate =
      toolchain::dyn_cast<tf_device::ReplicateOp>(cluster_func->getParentOp());

  toolchain::SmallVector<int64_t, 8> transform_input_indices;
  for (const auto& input : toolchain::enumerate(cluster_func.getOperands())) {
...
void TPUSpaceToDepthPass::runOnOperation() {
  ...
  toolchain::SmallDenseMap<unsigned, std::vector<Conv2DWithBlockSize>>
      argnum_and_convolutions;
  // A map maps block argument id to the number of users.
  toolchain::SmallDenseMap<unsigned, int> argnum_num_users;
