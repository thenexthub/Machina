...
#include "machina/compiler/mlir/machina/analysis/resource_alias_analysis.h"
...
#include "machina/compiler/mlir/machina/transforms/resource_op_lifting_cleanup.h"
...
void SetAllVarIsInitializedToTrue(Block* block) {
  ...
  for (auto op :
       toolchain::make_early_inc_range(block->getOps<TF::VarIsInitializedOp>())) {
...
void ForwardStoreToLoad(Block* block) {
  ...
  toolchain::SmallDenseMap<Value, TF::AssignVariableOp>
...
class RegionResourceHoister {
 public:
  ...
  void ReplaceResourceLoads(bool read_only) {
    toolchain::for_each(op_->getRegions(), [&](Region& region) {
 ...
 private:
  ...
  toolchain::MapVector<Value, ResourceInfo> resources_;
  toolchain::SetVector<Value> written_resources_;
...
LogicalResult RegionResourceHoister::Analyze() {
  ...
  toolchain::SetVector<Value> all_resources;
  bool is_func = false;
  ...
  for (auto resource : all_resources) {
    ...
    toolchain::BitVector written_regions(op_->getNumRegions());
...
void RegionResourceHoister::ReplaceResourceLoads(Region& region,
  ...
  for (auto read_op : toolchain::make_early_inc_range(all_reads)) {
...
void RegionResourceHoister::AppendResourceStoreValueToReturn(
  ...
  for (Region* region : regions) {
    ...
    auto new_return_operands = toolchain::to_vector<4>(old_return->getOperands());
    ...
    for (auto assign_variable_op : toolchain::make_early_inc_range(assign_ops)) {
      Value resource = assign_variable_op.getResource();
...
void RegionResourceHoister::ReplaceOpWithNewOp() {
  auto new_result_types = toolchain::to_vector<4>(op_->getResultTypes());
  ...
  for (auto it : toolchain::zip(op_->getRegions(), new_op->getRegions())) {
...
LogicalResult FindResourceArgUseInfo(
    func::FuncOp func_op,
    toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo>* result) {
  ...
  for (auto arg : TF::filter_resources(func_op.getArguments())) {
    ...
    for (auto user : arg.getUsers()) {
      ...
      info.used = true;
      if (auto read = toolchain::dyn_cast<TF::ReadVariableOp>(user)) {
        read_or_assigned = true;
        info.data_type = read.getType();
        continue;
      }

      if (auto assign = toolchain::dyn_cast<TF::AssignVariableOp>(user)) {
        read_or_assigned = true;
        info.updated = true;
...
toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> MergeArgResourceUseInfo(
    const toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo>& infos0,
    const toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo>& infos1) {
  toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> result;
...
LogicalResult RemoveUnusedResourceArgumentsAndForwardedRetvals(
    const toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo>& infos,
    func::FuncOp func_op,
    toolchain::SmallVector<int64_t, 4>* old_to_new_arg_indices = nullptr,
    toolchain::SmallDenseMap<int64_t, Type>* remaining_resource_data_types =
        nullptr) {
  // Remove return values forwarded from unused arguments.
  auto return_op = func_op.front().getTerminator();
  auto old_return_vals = toolchain::to_vector<8>(return_op->getOperands());
  int64_t skipped_retvals = 0;
  for (auto entry : toolchain::enumerate(old_return_vals)) {
  ...
  toolchain::BitVector indices_to_erase(func_op.getNumArguments());
  toolchain::SmallVector<Type, 4> new_types;
  ...
  func_op.setType(
      FunctionType::get(func_op.getContext(), new_types,
                        toolchain::to_vector<4>(return_op->getOperandTypes())));
  return success();
}
...
LogicalResult LiftArgRetResourcesForFunction(
    func::FuncOp func_op,
    const toolchain::SmallDenseMap<int64_t, Type>& resource_data_types,
    toolchain::function_ref<void(int64_t, Value)> handle_updated_arg_value) {
  ...
  // with type replaced.
  toolchain::SmallVector<Value, 4> skipped_args;
  for (auto& it : hoister.GetResources()) {
...
toolchain::SmallVector<T, 4> FilterRange(
    Range range,
    const toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo>& resource_arg_uses) {
  toolchain::SmallVector<T, 4> filtered;
  for (auto entry : toolchain::enumerate(range)) {
...
void AddLoadsStoresOutsideControlFlowOp(
    Operation* caller,
    const toolchain::SmallDenseMap<int64_t, std::pair<Type, int64_t>>&
        arg_data_type_and_updated_output_index) {
  OpBuilder builder(caller);
  auto new_operands = toolchain::to_vector<8>(caller->getOperands());
  toolchain::SmallVector<int64_t, 8> changed_indices;
...
LogicalResult HandleWhileLoop(TF::WhileOp while_op, func::FuncOp body,
  ...
  toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> body_use_info;
  toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> cond_use_info;
  ...
  toolchain::SmallVector<int64_t, 4> old_to_new_indices;
  toolchain::SmallDenseMap<int64_t, Type> remaining_resource_data_types;
  ...
  toolchain::SmallDenseMap<int64_t, std::pair<Type, int64_t>>
      arg_data_type_and_updated_output_index;
...
LogicalResult HandleCaseOrIfOp(CaseOrIfOp op, ArrayRef<func::FuncOp> branches) {
  ...
  toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> resource_arg_uses;
  if (failed(FindResourceArgUseInfo(branches.front(), &resource_arg_uses)))
    return failure();

  for (auto func : branches.drop_front()) {
    toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> branch_use_info;
  ...
  toolchain::SmallDenseMap<int64_t, int64_t> resource_arg_to_new_output;
  {
  ...
  for (auto branch : branches) {
    auto new_retvals =
        toolchain::to_vector<4>(branch.front().getTerminator()->getOperands());
...
struct PartitionedCallLiftingInfo {
  ...
  toolchain::SmallDenseMap<int64_t, int64_t> old_outputs_aliasing_old_inputs;
  // Mapping from old to new output indices in case any output is removed.
  toolchain::SmallVector<int64_t, 4> old_to_new_output_indices;
  // ResourceArgUseInfo for each old resource argument.
  toolchain::SmallDenseMap<int64_t, ResourceArgUseInfo> use_info;
...
LogicalResult HandlePartitionedCallOpCallee(
  ...
  for (auto entry :
       toolchain::enumerate(callee.front().getTerminator()->getOperands())) {
  ...
  toolchain::SmallVector<int64_t, 4> retval_indices_to_preserve;
  ...
  toolchain::SmallVector<Value, 4> new_retvals;
  // Lift resources.
  (void)LiftArgRetResourcesForFunction(
  ...
  toolchain::SmallVector<Value, 4> old_and_new_retvals;
  ...
  callee.setType(FunctionType::get(
      ...
      toolchain::to_vector<4>(new_return.getOperandTypes())));
  return success();
}
...
LogicalResult HoistForControlFlow(
    Block*, ModuleOp, bool,
    toolchain::SmallDenseMap<toolchain::StringRef, PartitionedCallLiftingInfo>*);
...
LogicalResult HoistForControlFlow(
  ...
  for (Operation& op : toolchain::make_early_inc_range(*block)) {
    if (auto while_op = toolchain::dyn_cast<TF::WhileOp>(&op)) {
    ...
    } else if (auto call_op = toolchain::dyn_cast<TF::PartitionedCallOp>(&op)) {
