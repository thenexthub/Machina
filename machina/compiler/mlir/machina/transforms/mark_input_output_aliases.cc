...
TF::AssignVariableOp FindAssignVariableOp(OpResult result) {
  ...
  if (parent) {
    if (toolchain::isa<tf_device::ParallelExecuteOp>(parent)) {
      if (auto execute =
              toolchain::dyn_cast_or_null<tf_device::ParallelExecuteOp>(parent)) {
  ...
  return toolchain::dyn_cast_or_null<TF::AssignVariableOp>(device_return);
}

// TODO(b/184420848): Explore relaxing these constraints.
LogicalResult BuildAliasingInfo(
    tf_device::ClusterFuncOp cluster_func,
    toolchain::DenseMap<Value, AliasInfo>& resource_alias_info_map) {
