...
#include "machina/compiler/mlir/utils/validators.h"  // IWYU pragma: keep

namespace mlir {
namespace TF {
namespace {

#include "machina/compiler/mlir/machina/transforms/generated_optimize.inc"
...
class SimplifyBroadcastReshape : public OpRewritePattern<BroadcastToOp> {
  ...
  LogicalResult matchAndRewrite(BroadcastToOp op,
    ...
    auto reshape_op = toolchain::dyn_cast_or_null<ReshapeOp>(user);
...
#define GEN_PASS_DEF_MACHINAOPTIMIZEPASS
