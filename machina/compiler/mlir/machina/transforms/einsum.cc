...
arith::ConstantOp createI32ConstantOp(toolchain::ArrayRef<int32_t> values,
...
arith::ConstantOp createI64ConstantOp(toolchain::ArrayRef<int64_t> values,
                                      Location loc, PatternRewriter* rewriter) {
  auto values_type = RankedTensorType::get(
...
TF::SumOp createSumOp(Value value, Location loc,
                      toolchain::ArrayRef<int32_t> redux_axes,
                      PatternRewriter* rewriter) {
  ...
  toolchain::SmallVector<int64_t> sum_shape;
...
TF::TransposeOp createTransposeOp(Value value, Location loc,
                                  toolchain::ArrayRef<int32_t> permutation,
                                  PatternRewriter* rewriter) {
...
TF::ReshapeOp createReshapeOp(Value value, ArrayRef<int64_t> shape,
  ...
  auto shape_tensor = createI64ConstantOp(
      machina::ConvertMlirShapeToTF(shape), loc, rewriter);
...
std::optional<toolchain::SmallDenseMap<char, int64_t>> EquationToMap(
    toolchain::StringRef equation) {
  toolchain::SmallDenseMap<char, int64_t> map;
...
std::optional<toolchain::SetVector<char>> GetAvailableLabels(
    toolchain::StringRef lhs, toolchain::StringRef rhs, int* lhs_named_label_count,
    int* rhs_named_label_count) {
  toolchain::SetVector<char> labels;
  for (int i = 0; i < 26; ++i) {
    labels.insert('a' + i);
    labels.insert('A' + i);
  }
...
std::string GenerateLabels(int count,
                           const toolchain::SetVector<char>& available_labels) {
...
std::tuple<std::string, std::string, std::string> FlattenEllipsis(
    toolchain::StringRef lhs, int lhs_named_label_count, toolchain::StringRef rhs,
    int rhs_named_label_count, toolchain::StringRef out, RankedTensorType lhs_ty,
    RankedTensorType rhs_ty, const toolchain::SetVector<char>& available_labels) {
...
std::optional<EinsumDimensionNumbers> GetEinsumDimensionNumbersUnary(
    toolchain::StringRef equation, RankedTensorType lhs_ty) {
  toolchain::StringRef lhs;
  toolchain::StringRef out;
...
LogicalResult rewriteToReduceSumAndTranspose(TF::EinsumOp op,
  ...
  if (!dnums.lhs.empty()) {
    ...
    toolchain::SmallVector<int32_t> reduce_idcs(dnums.lhs.size());
  ...
  toolchain::SmallVector<int32_t> lhs_transpose;
  lhs_transpose.reserve(dnums.lhs_out.size());

  toolchain::SmallDenseMap<int64_t, int64_t> out_lhs_map(dnums.lhs_out.size());
...
std::vector<int64_t> inverseTransposeVector(
    toolchain::ArrayRef<int64_t> input, toolchain::ArrayRef<int32_t> permutation) {
