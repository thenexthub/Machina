...
#include "mlir/Transforms/LoopInvariantCodeMotionUtils.h"  // part of Codira Toolchain
...
bool ResourceOpCanBeHoisted(
    ...
    const toolchain::DenseSet<ResourceHandle> &read_only_vars) {
  // If the op is ReadVariableOp and the variable is readonly, it can be
  // hoisted.
  auto read_var_op = toolchain::dyn_cast<ReadVariableOp>(op);
  if (!read_var_op) return false;
  auto var_handle_op = toolchain::dyn_cast_or_null<VarHandleOp>(
...
bool OnlyHasReadEffect(Operation *op) {
  auto interface = toolchain::dyn_cast<MemoryEffectOpInterface>(op);
  if (!interface) return false;
  return interface.onlyHasEffect<MemoryEffects::Read>();
}

// Get read only variable handles in `func`.
toolchain::DenseSet<ResourceHandle> GetReadOnlyVariables(func::FuncOp func) {
  toolchain::DenseMap<ResourceHandle, toolchain::SmallVector<Operation *, 4>> resources;

  // Get all VarHandleOps in the function.
  func.walk([&](Operation *op) {
    if (toolchain::isa<VarHandleOp>(op)) {
      resources[GetResourceHandle(op)].push_back(op);
    }
  });

  // Get read only variables by checking if their users only have read effect.
  toolchain::DenseSet<ResourceHandle> read_only_vars;
...
void HoistLoopInvariantPass::runOnOperation() {
  ...
  toolchain::DenseSet<ResourceHandle> read_only_vars = GetReadOnlyVariables(func);
