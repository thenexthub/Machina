...
toolchain::SmallSet<toolchain::StringRef, 1> GetCompositeResourceUserNames(
    Value resource) {
  ...
  toolchain::SmallSet<toolchain::StringRef, 1> composite_users;
  for (Operation* user : resource.getUsers())
    if (!toolchain::isa<TF::ReadVariableOp, TF::AssignVariableOp>(user))
...
mlir::LogicalResult ValidateVarHandle(TF::VarHandleOp var_handle_op) {
  ...
  if (!composite_ops.empty())
    return var_handle_op.emitOpError()
           ...
           << toolchain::join(composite_ops.begin(), composite_ops.end(), ", ")
           << "]";
...
mlir::LogicalResult PromoteVarHandlesToArguments(
    ...
    toolchain::SmallVectorImpl<std::string>* var_handle_shared_names) {
  Block& block = function.front();
  auto func_type = function.getFunctionType();

  auto func_arg_types = toolchain::to_vector<4>(func_type.getInputs());
  toolchain::SmallDenseMap<toolchain::StringRef, int> var_arg_index_by_name;
  for (auto var_handle_op :
       toolchain::make_early_inc_range(block.getOps<TF::VarHandleOp>())) {
    ...
    toolchain::StringRef name = var_handle_op.getSharedNameAttr().getValue();
...
LogicalResult PromoteResourcesToArguments(
    ...
    toolchain::ArrayRef<std::string> var_handle_shared_names) {
  Block& block = function.front();

  auto return_op =
      toolchain::dyn_cast_or_null<func::ReturnOp>(block.getTerminator());
  if (!return_op)
    return function.emitError() << "expects function '" << function.getName()
                                << "' to have a MLIR ReturnOp";

  toolchain::SmallVector<ResourceInfo, 4> resources(function.getNumArguments());
  auto argument_types =
      toolchain::to_vector<4>(function.getFunctionType().getInputs());
  ...
  for (Operation& op : toolchain::make_early_inc_range(block)) {
    if (auto read_op = toolchain::dyn_cast<TF::ReadVariableOp>(&op)) {
      ...
      read_op.erase();
    } else if (auto write_op = toolchain::dyn_cast<TF::AssignVariableOp>(&op)) {
      if (auto func_arg =
  ...
  auto return_operands = toolchain::to_vector<4>(return_op.getOperands());
  auto result_types = toolchain::to_vector<4>(return_op.getOperandTypes());
  toolchain::SmallVector<std::pair<int64_t, toolchain::StringRef>, 4>
      output_only_resources;
  toolchain::SmallVector<std::pair<int64_t, int64_t>, 4> input_output_alias;
  ...
  toolchain::SmallVector<int, 4> argument_indices_to_remove;
  for (auto resource_and_index : toolchain::enumerate(resources)) {
  ...
  for (int argument_index_to_remove :
       toolchain::reverse(argument_indices_to_remove)) {
...
class PromoteResourcesToArgsPass
 ...
 public:
  ...
  explicit PromoteResourcesToArgsPass(toolchain::ArrayRef<std::string> functions);
  void runOnOperation() override;
};

PromoteResourcesToArgsPass::PromoteResourcesToArgsPass(
    toolchain::ArrayRef<std::string> functions) {
  functions_ = functions;
}

void PromoteResourcesToArgsPass::runOnOperation() {
  ModuleOp module = getOperation();
  if (toolchain::size(functions_) == 0) {
    functions_ = {"main"};
  }
  ...
  for (const std::string& f : functions_) {
    ...
    toolchain::SmallVector<std::string, 4> var_handle_shared_names;
...
void PromoteVarHandlesToArgsPass::runOnOperation() {
  ...
  for (auto function : module.getOps<func::FuncOp>()) {
    ...
    const int var_handle_args_offset =
        function.getNumArguments() - var_handle_shared_names.size();
    for (auto var_name_and_index : toolchain::enumerate(var_handle_shared_names))
