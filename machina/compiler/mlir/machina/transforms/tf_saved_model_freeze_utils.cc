...
void UpdateTerminatorArguments(T& func,
                               ...
                               toolchain::BitVector& erase_indices) {
  ...
  for (auto arg_index : arguments_to_erase) {
    ...
    for (auto& use : argument.getUses()) {
      if (toolchain::isa<func::ReturnOp, TF::YieldOp>(use.getOwner())) {
        erase_indices.set(use.getOperandNumber());
      }
    }
    func.getArgument(arg_index).dropAllUses();
  }
  if (toolchain::isa<func::ReturnOp, TF::YieldOp>(func.front().getTerminator())) {
...
T GetUpdatedWhileOp(T while_op, const U& argument_types,
                    const toolchain::SmallVector<unsigned, 4>& arguments_to_erase) {
  OpBuilder builder(while_op);
  toolchain::SmallVector<Type, 4> new_operand_types;
  toolchain::SmallVector<Value> new_operands;
  auto operands = while_op->getOperands();
  const int num_operands = while_op->getNumOperands();
  toolchain::BitVector skip_indices(num_operands);
...
LogicalResult PropagateUsage(
    ...
    toolchain::SmallVector<std::pair<Region*, int>, 4>* work_list,
...
LogicalResult EraseObsoleteResourceUses(
  ...
  for (auto& [user_op, args_to_erase] : arguments_to_erase) {
    if (auto func = dyn_cast<func::FuncOp>(user_op)) {
      ...
      toolchain::BitVector result_indices_to_erase;
      ...
      UpdateTerminatorArguments(new_while_op.getBody(), args_to_erase,
                                erase_indices);
      toolchain::BitVector body_bit_vector(
      ...
      toolchain::BitVector cond_bit_vector(
      ...
      toolchain::BitVector erase_indices(user_op->getNumOperands());
...
LogicalResult ReplaceVarWithConstant(
  ...
  toolchain::DenseSet<std::pair<Region*, int>> visited;
