...
#include "mlir/Support/DebugStringHelper.h"  // part of Codira Toolchain
...
#define DCOMMENT(MSG) LLVM_DEBUG(toolchain::dbgs() << MSG << "\n")
#define DCOMMENT_OP(OP, MSG) \
  LLVM_DEBUG(OP->print(toolchain::dbgs() << MSG << " "); toolchain::dbgs() << "\n")
...
Type TypeMeet(Type lhs, Type rhs) {
  ...
  if (!lhs_shape_type.hasRank()) {
    ...
    for (auto shape_elts : toolchain::enumerate(
             toolchain::zip(lhs_shape_type.getShape(), rhs_shape_type.getShape()))) {
...
TensorType CreateTensorType(std::optional<toolchain::ArrayRef<int64_t>> shape,
                            Type element_type) {
  if (shape.has_value())
    return machina::GetTypeFromTFTensorShape(shape.value(), element_type);
...
RankedTensorType DropFirstDimension(Type type) {
  ...
  toolchain::ArrayRef<int64_t> dims_except_first =
      ranked_type.getShape().drop_front();
  return machina::GetTypeFromTFTensorShape(dims_except_first,
...
bool CanInferTensorListElementType(Value tensorlist,
  ...
  while (!worklist.empty()) {
    ...
    for (auto& use : tensorlist.getUses()) {
      if (auto push = toolchain::dyn_cast<TensorListPushBackOp>(use.getOwner())) {
        auto element_type =
      ...
      if (auto scatter = toolchain::dyn_cast<TensorListScatterIntoExistingListOp>(
              use.getOwner())) {
      ...
      if (auto set_item = toolchain::dyn_cast<TensorListSetItemOp>(use.getOwner())) {
        auto element_type =
      ...
      if (auto pop = toolchain::dyn_cast<TensorListPopBackOp>(use.getOwner())) {
        add_to_worklist(pop.getOutputHandle());
        continue;
      }
      if (auto resize = toolchain::dyn_cast<TensorListResizeOp>(use.getOwner())) {
      ...
      if (auto while_region = toolchain::dyn_cast<WhileRegionOp>(use.getOwner())) {
        DCOMMENT("\tTL WhileRegion");
        for (auto branch : while_region.getRegions())
          add_to_worklist(branch->getArgument(use.getOperandNumber()));
        continue;
      }
      if (auto yield = toolchain::dyn_cast<YieldOp>(use.getOwner())) {
...
Type GetType(Attribute shape_attr, Attribute type_attr) {
  ...
  if (shape.hasRank())
    return machina::GetTypeFromTFTensorShape(shape.getShape(),
                                                type.getValue());
  else
...
bool CanBeRefined(Type type) {
  ...
  auto can_refine_subtypes = [](TF::TensorFlowTypeWithSubtype tws) {
    return tws.GetSubtypes().empty() ||
           toolchain::any_of(tws.GetSubtypes(), CanBeRefined);
  };
...
Type GetNewArgType(Type old_arg_type, ArrayRef<int64_t> shape,
  ...
  Type new_arg_type = machina::GetTypeFromTFTensorShape(shape, element_type);
  ...
  if (auto input_ty = mlir::dyn_cast<RankedTensorType>(old_arg_type)) {
    ...
    if (!bounds.empty()) {
      ...
      new_arg_type = machina::GetTypeFromTFTensorShape(
...
struct ValuePort {
  ...
  raw_ostream& print(raw_ostream& os) const {
    ...
    os << formatv(" [{0}]", toolchain::make_range(port.begin(), port.end()));
    return os;
  }
...
struct ValuePortHasher {
  std::size_t operator()(const ValuePort& other) const {
    return hash_combine(toolchain::hash_value(other.producer.getOpaqueValue()),
...
ValuePort ComputeInputComponentFor(GatherV2Op op, ArrayRef<unsigned int> port) {
  ...
  if (!matchPattern(op.getAxis(), m_Constant(&axis)) ||
      ...
      !axis.getSplatValue<toolchain::APInt>().isZero()) {
    return {};
  }
...
ValuePort ComputeInputComponentFor(Operation* op, ArrayRef<unsigned int> port) {
  if (auto pack_op = toolchain::dyn_cast<PackOp>(op)) {
    return ComputeInputComponentFor(pack_op, port);
  }
  if (auto concat_op = toolchain::dyn_cast<ConcatV2Op>(op)) {
    return ComputeInputComponentFor(concat_op, port);
  }
  if (auto gather_op = toolchain::dyn_cast<GatherV2Op>(op)) {
...
Attribute ComputeOutputComponent(const ValuePort& value_port,
                                 ValueQueryFn values) {
  LLVM_DEBUG(value_port.print(toolchain::dbgs() << "Computing output for ") << "\n");
  ...
  if (port.empty()) {
    LLVM_DEBUG(toolchain::dbgs() << "skipping, port outside spec of " << op << "\n");
    return nullptr;
  }
...
class ShapeInference {
 public:
  ...
  void RecordValue(const ValuePort& value_port, Attribute value) {
    LLVM_DEBUG(value_port.print(toolchain::dbgs() << "\trecording ")
               << value << "\n");
 ...
 private:
  ...
  DenseMap<XlaCallModuleOp, std::unique_ptr<machina::XlaCallModuleLoader>>
...
bool ShapeInference::InferShapeForCast(Operation* op) {
  ...
  if (toolchain::all_of(result.getUses(), [this](OpOperand& use) {
  ...
  if (!new_type) {
    ...
    new_type = machina::GetTypeFromTFTensorShape(
...
bool ShapeInference::InferShapeForIf(IfOp op) {
  ...
  for (auto it : toolchain::zip(op.getResults(), then_results, else_results)) {
...
bool ShapeInference::InferShapeForCase(CaseOp op) {
  ...
  toolchain::SmallVector<TypeRange> branch_result_types;
...
bool ShapeInference::InferShapeForXlaCallModule(XlaCallModuleOp op) {
  if (!toolchain::any_of(op.getResultTypes(), CanBeRefined)) return false;

  machina::XlaCallModuleLoader* loader;
  if (auto it = xla_call_module_loaders_.find(op);
    ...
    auto l = machina::XlaCallModuleLoader::Create(
    ...
    if (!l.ok()) {
      toolchain::errs() << "Parsing error in XlaCallModule: "
                   << l.status().ToString() << "\n";
      return false;
    }
  ...
  int nr_main_token_outputs =
      toolchain::count_if(main_output_types, machina::IsTokenType);
  if (op_results.size() != main_output_types.size() - nr_main_token_outputs) {
    toolchain::errs() << "XlaCallModule has " << op_results.size()
  ...
  for (auto output_type : main_output_types) {
    if (machina::IsTokenType(output_type)) continue;
    auto output_type_ranked = mlir::dyn_cast<RankedTensorType>(output_type);
    if (output_type_ranked == nullptr) {
      toolchain::errs() << "Unsupported XlaCallModule result type: " << output_type
                   << "\n";
      return false;
    }
...
bool ShapeInference::InferShapeForRestore(Operation* op) {
  ...
  toolchain::SmallVector<mlir::Operation*> worklist;
  toolchain::append_range(worklist, op->getUsers());
  ...
  while (!worklist.empty()) {
    ...
    if (toolchain::isa<TF::CastOp, TF::IdentityOp>(use)) {
      toolchain::append_range(worklist, use->getUsers());
      continue;
    }

    TF::AssignVariableOp assign_op = toolchain::dyn_cast<TF::AssignVariableOp>(use);
    if (!assign_op) {
      continue;
    }
...
DatasetInput GetDatasetInput(Value value) {
  ...
  while (
      toolchain::isa_and_nonnull<IdentityOp, IdentityNOp>(value.getDefiningOp())) {
    value = value.getDefiningOp()->getOperand(
        mlir::cast<OpResult>(value).getResultNumber());
  }

  Operation* op = value.getDefiningOp();
  if (!toolchain::isa_and_nonnull<BatchDatasetV2Op, MapDatasetOp, RepeatDatasetOp,
...
bool ShapeInference::InferShapeForDatasetOpCommon(Operation* op, FuncOp f,
  ...
  auto input_types = toolchain::to_vector<1>(
  ...
  for (auto t : toolchain::drop_begin(op->getOperandTypes())) {
...
bool ShapeInference::InferShapeForMapDataset(MapDatasetOp op,
  ...
  if (!f || !toolchain::hasSingleElement(GetCallers(f))) return false;
...
bool ShapeInference::InferShapeForReduceDataset(ReduceDatasetOp op,
  ...
  for (auto t : toolchain::drop_begin(op.getOperandTypes(), 1 + num_states)) {
    auto meet = TypeMeet(*it, t);
    changed = changed || (meet != *it);
    *it++ = meet;
  }
...
bool ShapeInference::InferShapeForTensorListInitOps(Operation* op) {
  ...
  auto tensor_type = machina::GetTypeFromTFTensorShape({}, variant_type);
...
bool ShapeInference::InferShapeForVarHandleOp(VarHandleOp op) {
  ...
  for (auto& use : make_early_inc_range(resource.getUses())) {
    ...
    if (!toolchain::isa<AssignVariableOp>(def) && !toolchain::isa<ReadVariableOp>(def)) {
      return false;
    }
  }
...
std::optional<xla::Window> InferWindowFromDimensions(
    toolchain::SmallVector<int64_t> window_dimensions,
    toolchain::SmallVector<int64_t> window_strides,
    toolchain::SmallVector<std::pair<int64_t, int64_t>> padding,
    toolchain::SmallVector<int64_t> lhs_dilation,
    toolchain::SmallVector<int64_t> rhs_dilation) {
...
std::optional<RankedTensorType> InferWindowOutputShape(
  ...
  if (window.dimensions_size() != base_shape.getRank()) {
    toolchain::errs() << "Window has dimension " << window.dimensions_size()
  ...
  for (int64_t i = 0; i < window.dimensions_size(); ++i) {
    ...
    if (dim.size() <= 0) {
      toolchain::errs() << "Window " << window.DebugString()
  ...
  return machina::GetTypeFromTFTensorShape(output_dimensions, element_type);
}
...
bool ShapeInference::InferShapeForXlaReduceWindowOp(XlaReduceWindowOp op) {
  ...
  if (input_ty.hasStaticShape() &&
    ...
    toolchain::SmallVector<int64_t> window_dimensions_vec, window_strides_vec,
        base_dilations_vec, window_dilations_vec;
    toolchain::SmallVector<std::pair<int64_t, int64_t>> padding_pairs(
...
bool ShapeInference::InferShapeForXlaGatherOp(XlaGatherOp op) {
  ...
  if (DenseIntElementsAttr attr;
    ...
    slice_sizes_attr = attr;
             ...
             it != results_.end() &&
             toolchain::isa_and_nonnull<DenseIntElementsAttr>(it->second)) {
    slice_sizes_attr = toolchain::cast<DenseIntElementsAttr>(it->second);
  } else {
    return false;
  }

  toolchain::SmallVector<int64_t> slice_sizes;
...
std::optional<RankedTensorType> InferXlaConvOutputShape(
    toolchain::SmallVector<int64_t> input_tensor_dims,
    toolchain::SmallVector<int64_t> kernel_tensor_dims,
    toolchain::SmallVector<int64_t> window_strides,
    toolchain::SmallVector<std::pair<int64_t, int64_t>> paddings,
    toolchain::SmallVector<int64_t> lhs_dilations,
    toolchain::SmallVector<int64_t> rhs_dilations, int64_t batch_group_count,
  ...
  toolchain::SmallVector<int64_t> window_spatial_dims;
  ...
  ShapedType base_shape =
      machina::GetTypeFromTFTensorShape(input_spatial_dims, element_type);
  ...
  return machina::GetTypeFromTFTensorShape(output_dims, element_type);
}

// TODO(hanxiongwang): The logic in this function need move to Op Verify method
// when dependecy issue of adding header file
// "third_party/machina/compiler/xla/xla_data.pb.h" into
// "third_party/machina/compiler/mlir/machina/ir/tf_ops_n_z.cc" is
// resolved
...
bool ShapeInference::InferShapeForXlaConvV2Op(XlaConvV2Op op) {
  ...
  if (matchPattern(window_strides, m_Constant(&window_strides_attr)) &&
    ...
    toolchain::SmallVector<int64_t> input_tensor_dims_vec, kernel_tensor_dims_vec,
    ...
    for (const toolchain::APInt& i : window_strides_attr) {
    ...
    for (const toolchain::APInt& i : lhs_dilation_attr) {
      lhs_dilations_vec.push_back(i.getSExtValue());
    }

    for (const toolchain::APInt& i : rhs_dilation_attr) {
...
bool ShapeInference::RefineWithInferTypeOpInterface(
  ...
  if (none_of(op->getResultTypes(), CanBeRefined)) {
    LLVM_DEBUG(toolchain::dbgs() << "Skipping inference for statically shaped op '"
                            << op->getName() << "'.\n");
    return false;
  }
  ...
  for (auto [result, inferred_type] : zip(op->getResults(), inferred)) {
    ...
    if (!toolchain::isa<TF::ZerosLikeOp>(op)) {
...
ShapeHandle ShapeInference::ComputeOutputAsShape(OpResult result,
                                                 InferenceContext* ic) {
  LLVM_DEBUG(result.print(toolchain::dbgs() << "\nEvaluate partially "));
  ...
  for (unsigned int i = 0, e = dims.size(); i != e; ++i) {
    ...
    while (!worklist.empty()) {
      ...
      LLVM_DEBUG(front.print(toolchain::dbgs() << "\nWorklist front "));
      ...
      LLVM_DEBUG(ret.print(toolchain::dbgs() << "\ncomputed result = "));

      // If worklist is empty, then this is the root query op.
      if (worklist.empty()) {
        LLVM_DEBUG(toolchain::dbgs() << "[root node]\n");
        if (auto dea = mlir::dyn_cast<DenseIntElementsAttr>(ret)) {
          if (dea.getNumElements() != 1) {
            LLVM_DEBUG(toolchain::dbgs() << "Unexpected number of elements\n");
            return {};
          }
...
bool ShapeInference::ForceTypeForPassThroughOperands(Operation* op,
                                                     ...
                                                     ResultRange results) {
  bool changed = false;
  for (auto entry : toolchain::zip(operands, results)) {
...
bool ShapeInference::RefineShapeForPassThroughOps(Operation* op) {
  ...
  bool changed = false;
  for (auto entry : toolchain::zip(op->getOperands(), op->getResults())) {
...
bool CanWhileTypeBeRefinedWith(TensorType current_type,
  ...
  for (auto dim :
       toolchain::zip(current_type.getShape(), potential_refined_type.getShape())) {
...
bool ShapeInference::InferShapeForSingleOperation(Operation* op,
                                                  int64_t max_iterations) {
  LLVM_DEBUG(op->print(toolchain::dbgs() << "InferShapeForSingleOperation for ");
             toolchain::dbgs() << "\n");
  ...
  if (none_of(op->getResultTypes(), CanBeRefined)) {
    if (auto host_compute_op = dyn_cast<XlaHostComputeOp>(op)) {
      LLVM_DEBUG(toolchain::dbgs() << "Keep inference for statically shaped op '"
                              << op->getName() << "'.\n");
    } else {
      LLVM_DEBUG(toolchain::dbgs() << "Skipping inference for statically shaped op '"
                              << op->getName() << "'.\n");
  ...
  toolchain::SmallVector<ShapedTypeComponents, 4> inferred_return_shapes;
  ...
  bool changed = false;
  for (auto result : toolchain::zip(op->getResults(), inferred_return_shapes)) {
...
FailureOr<bool> ShapeInference::PropagateShapeToFunctions(
  ...
  for (func::FuncOp func : functions) {
    ...
    if (!toolchain::hasSingleElement(callers) &&
        !toolchain::all_of(callers.drop_front(), [&](Operation* caller) {
      ...
      if (toolchain::any_of(callers, [](Operation* op) {
...
FailureOr<bool> ShapeInference::PropagateShapeToRegions(
  ...
  for (auto region : regions) {
    ...
    assert(toolchain::size(input_types) == entry.getNumArguments());
    for (auto it : toolchain::zip(entry.getArguments(), input_types)) {
...
void ShapeInference::PropagateConstantToCallee(CallOpInterface call_op,
  ...
  if (!toolchain::hasSingleElement(callers)) return;
  ...
  for (auto arg : func.getArguments()) {
    ...
    LLVM_DEBUG(call_op.print(toolchain::dbgs() << "Propagate to calee: ");
               known_constant.print(toolchain::dbgs() << " constant ");
               toolchain::dbgs() << "\n");
...
void ShapeInference::PropagateConstantFromCallee(CallOpInterface call_op,
  ...
  for (auto retval :
       toolchain::enumerate(func.front().getTerminator()->getOperands())) {
    if (propagate_caller_callee_constants_) {
    ...
    if (auto known_constant = ComputeOutputComponent(vp)) {
      LLVM_DEBUG(known_constant.print(toolchain::dbgs() << "Propagate constant ");
                 call_op.print(toolchain::dbgs() << "from "); toolchain::dbgs() << "\n");
...
RankedTensorType GetCompatibleRankedTensorType(RankedTensorType lhs,
  ...
  toolchain::SmallVector<int64_t, 4> dims;
  dims.reserve(lhs.getRank());
  for (auto dim : toolchain::zip(lhs.getShape(), rhs.getShape())) {
  ...
  return machina::GetTypeFromTFTensorShape(
...
toolchain::SmallVector<Type, 4> GetWhileCompatibleTypes(
  ...
  for (auto entry :
       toolchain::zip(operand_types, result_types, region_argument_types)) {
...
FailureOr<bool> ShapeInference::PropagateShapeIntoAttachedFunctions(
  ...
  if (auto if_op = dyn_cast<TF::IfOp>(op)) {
    ...
    auto propagate_shape_to = [&](mlir::SymbolRefAttr func_sym) {
      auto func = toolchain::cast<mlir::func::FuncOp>(
      ...
      for (auto type : func.getFunctionType().getInputs()) {
        types.push_back(machina::GetTypeFromTFTensorShape(
...
LogicalResult ShapeInference::TryToFold(Operation* op) {
  LLVM_DEBUG(op->print(toolchain::dbgs() << "TryToFold "); toolchain::dbgs() << "\n");
...
LogicalResult ShapeInference::InferShapeForFunctionReturnType(
    func::FuncOp func) {
  LLVM_DEBUG(toolchain::dbgs() << "Inferring return type for: " << func.getName()
                          << "\n");
  ...
  for (OpOperand& arg_op : return_op.getOperation()->getOpOperands()) {
    ...
    if (isa_and_nonnull<CastOp, tensor::CastOp>(arg_defining_op)) {
      ...
      LLVM_DEBUG({
        toolchain::errs() << "\tfolding & updating return type ";
        result.getType().print(toolchain::errs());
        input.getType().print(toolchain::errs() << " to ");
        toolchain::errs() << "\n";
      });
...
absl::StatusOr<SmallVector<SmallVector<int64_t>>> ParseArgumentShapes(
  ...
  TF_RETURN_IF_ERROR(::machina::ParseNodeShapes(input_shapes, shapes));
...
FailureOr<bool> InferModuleShape(ModuleOp module, int64_t max_iterations,
  ...
  auto producer_or = machina::GetTfGraphProducerVersion(module);
  if (!producer_or.ok()) {
