...
std::vector<Value> ConvertTFBatchMatMulOp<BatchMatMulOpType>::sliceInput(
  ...
  if (batch_size == 1) {
    ...
    toolchain::SmallVector<Type, 4> output_types(batch_size, slice_result_type);
...
LogicalResult ConvertTFBatchMatMulOp<BatchMatMulOpType>::matchAndRewrite(
  ...
  for (int batch_idx : toolchain::seq<int>(0, bcast.output_batch_size())) {
