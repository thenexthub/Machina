...
// i.e., are ancestors.
toolchain::DenseSet<Block*> GetAllBlocksBetween(Operation* bottom, Operation* top) {
  toolchain::DenseSet<Block*> blocks;
  Operation* op = bottom;
  while (op && op != top) {
...
// `target_blocks`.
Value GetResultInBlock(Value value, toolchain::DenseSet<Block*> target_blocks) {
  while (target_blocks.find(value.getParentBlock()) == target_blocks.end()) {
    ...
    for (OpOperand& operand : value.getUses()) {
      ...
      if (!toolchain::isa<tf_device::ReturnOp>(owner)) continue;
      Operation* parent = owner->getParentOp();  // op that owns the "return"
      if (toolchain::isa<tf_device::LaunchOp>(parent)) {
        new_value = parent->getResult(operand.getOperandNumber());
      } else if (auto replicate =
                     toolchain::dyn_cast<tf_device::ReplicateOp>(parent)) {
...
Operation* FindCompilePredecessor(Operation* func_op,
  ...
  toolchain::DenseSet<Block*> blocks = GetAllBlocksBetween(preprocess_op, func_op);

  toolchain::DenseMap<Block*, Operation*> scope;
...
toolchain::SmallSetVector<Operation*, 4> GetSuccessorOps(Operation* root_op) {
  std::queue<Operation*> queue;
  toolchain::SmallSetVector<Operation*, 4> ops_to_move;

  queue.push(root_op);
  while (!queue.empty()) {
    Operation* op = queue.front();
    queue.pop();
    if (toolchain::isa<tf_device::ReturnOp>(op)) continue;
...
void CreateNewLaunchOp(OpBuilder* builder,
                       ...
                       Operation* preprocess_op,
                       toolchain::SmallSetVector<Operation*, 4>& ops_to_move,
                       toolchain::StringRef device, Operation* before_op) {
  toolchain::SmallDenseMap<Value, int> original_return_operand_map;
  ...
  for (Operation& op : toolchain::make_early_inc_range(old_block->getOperations())) {
...
void CreateReducedLaunchOp(OpBuilder* builder, Block* old_block,
  ...
  for (Operation& op : toolchain::make_early_inc_range(old_block->getOperations())) {
    if (!toolchain::isa<tf_device::ReturnOp>(op)) {
  ...
  toolchain::SmallDenseMap<Value, int> reduced_return_operand_map;
  for (OpOperand& operand :
...
LogicalResult MovePreprocessingOpInLaunch(OpBuilder* builder,
  ...
  auto original_launch_op =
      toolchain::dyn_cast<tf_device::LaunchOp>(preprocess_op->getParentOp());
...
LogicalResult VerifyAllProgramKeyOperandsReplaced(Operation* module) {
  WalkResult result = module->walk([&](Operation* op) {
    ...
    if (toolchain::dyn_cast_or_null<TF::ConstOp>(defining)) {
...
void EmbeddingProgramKeyPass::runOnOperation() {
  ...
  toolchain::SmallVector<Operation*, 6> preprocess_ops;
