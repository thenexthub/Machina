...
#include "machina/compiler/mlir/machina/transforms/sparsecore/sparsecore_passes.h.inc"
...
static constexpr toolchain::StringRef kTpuCompilationStatus =
...
bool UseEmbeddingPipelining(ModuleOp& module) {
  ...
  if (machina::GetBuildXlaOpsPassFlags()
...
struct Inliner : public InlinerInterface {
  ...
  LogicalResult UnifyReplicationInfo(func::FuncOp func) {
    ...
    for (auto& op : func.getRegion().getOps()) {
      if (auto compile_op = toolchain::dyn_cast<TF::TPUCompilationResultOp>(op)) {
  ...
  LogicalResult RemoveOutputInputPairs(func::FuncOp func) {
    toolchain::SetVector<Operation*> ops_to_erase;
    ...
    for (auto output_op :
      ...
      toolchain::SetVector<Operation*> seen_users;
      for (auto user : output_op->getUsers()) {
        if (!seen_users.insert(user)) continue;
        if (toolchain::isa<TF::TPUReplicatedInputOp>(user)) {
          if (input_op != nullptr) {
            func->emitError(
                "Found multiple TPUReplicatedInput ops but only expected 1.");
            return LogicalResult::failure();
          }
          input_op = toolchain::dyn_cast<TF::TPUReplicatedInputOp>(user);
        }
        if (toolchain::isa<func::ReturnOp>(user)) {
  ...
  LogicalResult RemoveDuplicateReplication(func::FuncOp func) {
    toolchain::SetVector<Operation*> ops_to_erase;
    toolchain::MapVector<BlockArgument, TF::TPUReplicatedInputOp> cache;
    for (auto input_op : func.getRegion().getOps<TF::TPUReplicatedInputOp>()) {
      ...
      if (!toolchain::isa<BlockArgument>(operand)) continue;
      BlockArgument arg = toolchain::dyn_cast<BlockArgument>(operand);
      ...
      if (!cache.insert({arg, input_op}).second) {
        ...
        for (auto p :
             toolchain::zip(input_op->getResults(), cache[arg]->getResults())) {
  ...
  LogicalResult InlineCallsInFunc(func::FuncOp func,
    ...
    for (auto caller :
      ...
      if (!toolchain::isa<func::FuncOp>(symbol)) {
        func.emitError() << "Invalid callee: " << caller.getF();
        return LogicalResult::failure();
      }
      auto callee =
          toolchain::dyn_cast<func::FuncOp>(symbol_table.lookup(caller.getF()));
...
LogicalResult EliminateResourceLoops(OpBuilder& builder,
  ...
  auto ComesFromBlockArgNumber = [](Value val) -> int {
    while (true) {
      if (auto block_arg = toolchain::dyn_cast<BlockArgument>(val)) {
        return block_arg.getArgNumber();
      }
      if (auto identity_op =
              toolchain::dyn_cast<TF::IdentityOp>(val.getDefiningOp())) {
  ...
  for (auto call_op :
    ...
    for (int i = 0; i < call_op->getNumResults(); ++i) {
      if (IsResourceType(call_op->getResult(i).getType())) {
        ...
        auto callee =
            toolchain::dyn_cast<func::FuncOp>(symbol_table.lookup(call_op.getF()));
...
void GatherOpsForExtraction(mlir::SetVector<Operation*>* operations,
  ...
  toolchain::SetVector<Operation*> ops_to_process(*operations);
  toolchain::SetVector<Operation*> new_ops;
    ...
    for (Operation* op : ops_to_process) {
      ...
      if (successors) {
        for (mlir::Operation* successor : op->getUsers()) {
          ...
          if (toolchain::isa<func::ReturnOp>(successor)) continue;
...
func::FuncOp CreateFnWithSignature(ModuleOp module, SymbolTable& symbol_table,
                                   const toolchain::SetVector<Value>& inputs,
                                   const toolchain::SetVector<Value>& outputs,
...
TF::StatefulPartitionedCallOp EncapsulateOpsInFunc(
    ...
    const toolchain::SetVector<Operation*>& ops,
    const toolchain::SetVector<Value>& inputs, const toolchain::SetVector<Value>& outputs,
  ...
  for (auto p : toolchain::zip(inputs, new_func.getArguments()))
    replaceAllUsesInRegionWith(std::get<0>(p), std::get<1>(p),
                               new_func.getBody());
  ...
  // new function.
  for (auto p : toolchain::zip(outputs, caller->getResults()))
    replaceAllUsesInRegionWith(std::get<0>(p), std::get<1>(p),
                               parent_func.getBody());
...
void UpdateAndInsertTPUOps(TF::StatefulPartitionedCallOp caller,
  ...
  auto func = toolchain::dyn_cast<mlir::func::FuncOp>(
...
LogicalResult FindAndExcludeOp(func::FuncOp func,
                               ...
                               toolchain::SetVector<Operation*>& merged_set,
                               OpType& found_op) {
...
LogicalResult FindOwningWhileOp(func::FuncOp body_func, ModuleOp module,
  ...
  for (auto& use : uses_optional.value()) {
    if (toolchain::isa<TF::WhileOp>(use.getUser())) {
      if (while_op != nullptr) {
        use.getUser()->emitOpError() << "multiple users of function.";
        return LogicalResult::failure();
      } else {
        while_op = toolchain::cast<TF::WhileOp>(use.getUser());
      }
    } else {
...
LogicalResult FindForwardPassOps(OpBuilder& builder,
                                 toolchain::SetVector<Operation*>& forward_pass_ops,
                                 toolchain::SetVector<Operation*>& backward_pass_ops,
                                 toolchain::SetVector<Operation*>& merged_set,
  ...
  toolchain::SetVector<int64_t> forward_variable_inputs;
  ...
  for (auto argument : loop_body_func.getArguments()) {
    ...
    if (IsResourceType(argument.getType())) {
      ...
      for (auto user : argument.getUsers()) {
        if (toolchain::isa<func::ReturnOp>(user)) continue;
        ...
        if (TF::TPUReplicatedInputOp input =
                toolchain::dyn_cast<TF::TPUReplicatedInputOp>(user)) {
  ...
  for (int64_t index : forward_variable_inputs) {
    ...
    TF::TPUReplicatedInputOp input =
        toolchain::cast<TF::TPUReplicatedInputOp>(input_ptr);
    ...
    for (Operation* next_user : input.getOutput().getUsers()) {
      if (!toolchain::isa<TF::ReadVariableOp>(next_user) &&
          !toolchain::isa<TF::AssignVariableOp>(next_user)) {
  ...
  toolchain::SetVector<Operation*> new_forward_ops;
...
LogicalResult FindBackwardPassOps(
    OpBuilder& builder, toolchain::SetVector<Operation*>& backward_pass_ops,
    toolchain::SetVector<Operation*>& merged_set, const int num_replicas) {
  ...
  toolchain::SetVector<TF::TPUReplicatedInputOp> to_clone;
  toolchain::SetVector<TF::TPUReplicatedInputOp> to_insert;
  ...
  for (Operation* op : backward_pass_ops) {
    for (OpOperand& input_value : op->getOpOperands()) {
      ...
      if (TF::TPUReplicatedInputOp input =
              toolchain::dyn_cast<TF::TPUReplicatedInputOp>(predecessor_op)) {
  ...
  toolchain::SetVector<Value> values_to_add_nodes;

  for (Operation* op : backward_pass_ops) {
    // TODO(bfontain): Should validate that all the TPU ops are in the same
    // replication region.
    // If the op is already a replicated input, no need to to anything.
    if (!IsTPUOp(op) || toolchain::isa<TF::TPUReplicatedInputOp>(op)) continue;
    for (OpOperand& input_value : op->getOpOperands())
...
LogicalResult FindCoreTPUOps(
    toolchain::SetVector<Operation*>& core_tpu_ops,
    const toolchain::SetVector<Operation*>& forward_pass_ops,
    const toolchain::SetVector<Operation*>& backward_pass_ops,
    const toolchain::SetVector<Operation*>& merged_set,
    func::FuncOp loop_body_func) {
  ...
  for (Operation& op : loop_body_func.getOps()) {
    ...
    if (merged_set.contains(&op) || toolchain::isa<func::ReturnOp>(op) ||
...
LogicalResult FindNonTPUOps(toolchain::SetVector<Operation*>& non_tpu_ops,
                            const toolchain::SetVector<Operation*>& merged_set,
                            func::FuncOp loop_body_func) {
  ...
  toolchain::SetVector<int64_t> non_tpu_args;
  for (Operation& op : loop_body_func.getOps()) {
    if (merged_set.contains(&op) || toolchain::isa<func::ReturnOp>(op) ||
  ...
  for (Operation* op : non_tpu_ops)
    for (Value input : op->getOperands())
      if (BlockArgument arg = toolchain::dyn_cast<BlockArgument>(input))
  ...
  for (OpOperand& operand : return_op->getOpOperands()) {
    if (non_tpu_args.contains(operand.getOperandNumber())) {
      if (BlockArgument argument =
              toolchain::dyn_cast<BlockArgument>(operand.get())) {
...
LogicalResult ExtractOpsAsFunc(
    ...
    toolchain::SetVector<Operation*>& ops, StringAttr replication_attr,
    TF::TPUReplicateMetadataOp metadata_op,
  ...
  toolchain::SetVector<Value> inputs;
  ...
  toolchain::SetVector<OpResult> results;
  for (Operation* op : ops) {
  ...
  toolchain::SetVector<Value> outputs;
...
LogicalResult FindSourceTPUReplicatedOutput(
  ...
  if (auto src = toolchain::dyn_cast<TF::TPUReplicatedOutputOp>(op)) {
    rep_out = src;
    return LogicalResult::success();
  }
  if (auto src = toolchain::dyn_cast<TF::IdentityOp>(op)) {
...
int FindReturnIndex(Value val) {
  ...
  for (auto user : val.getUsers()) {
    if (auto ret_op = toolchain::dyn_cast<func::ReturnOp>(user)) {
    ...
    if (auto ident_op = toolchain::dyn_cast<TF::IdentityOp>(user)) {
...
void AddAssertion(OpBuilder& builder, Location& loc, Value cond,
  ...
  auto msg = builder.create<TF::ConstOp>(
      loc, DenseStringElementsAttr::get(shape_type,
                                        toolchain::ArrayRef<StringRef>{message}));
...
LogicalResult StartStep0(OpBuilder& builder, Location& loc,
  ...
  for (auto p :
       toolchain::zip(callers.core_tpu->getResults(), new_forward->getResults()))
...
LogicalResult FinishStepNm1(OpBuilder& builder, Location& loc,
  ...
  auto new_core_tpu = func_builder.insert(callers.core_tpu->clone(ir_map));
  for (auto p :
       toolchain::zip(callers.core_tpu->getResults(), new_core_tpu->getResults()))
    ir_map.map(std::get<0>(p), std::get<1>(p));
  auto new_backward = func_builder.insert(callers.backward->clone(ir_map));
  // Add the function return;
  std::vector<Value> results;
...
void EmbeddingPipeliningPass::runOnOperation() {
  ...
  toolchain::SmallVector<Operation*> checkset(forward_pass_ops.getArrayRef());
  ...
  toolchain::SetVector<Operation*> non_tpu_ops;
  ...
  for (int ret_pos = 0; ret_pos < orig_return_op->getNumOperands(); ++ret_pos) {
    ...
    if (def_op == non_tpu_caller) {
      ...
      return signalPassFailure();
    } else if (toolchain::isa<BlockArgument>(operand)) {
      // pass
    } else {
  ...
  toolchain::SetVector<Value> new_while_operands;
  toolchain::MapVector<Value, int> loop_var_map;
  ...
  toolchain::SetVector<Value> new_body_results;
  ...
  // flag override.
  int parallel_iterations_flag = machina::GetBuildXlaOpsPassFlags()
  ...
  // the pipelining.
  for (auto p : toolchain::zip(orig_while_op->getResults(), loop_operands_n))
    replaceAllUsesInRegionWith(std::get<0>(p), std::get<1>(p),
