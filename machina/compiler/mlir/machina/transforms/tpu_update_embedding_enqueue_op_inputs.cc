...
LogicalResult ExtractEmbeddingAttribute(
    Operation* op, toolchain::StringMap<Operation*>* embedding_op_map) {
...
LogicalResult FindTPUEmbeddingOps(
    func::FuncOp func_op, toolchain::StringMap<Operation*>* enqueue_op_map,
    toolchain::StringMap<Operation*>* recv_activation_op_map,
    toolchain::StringMap<Operation*>* send_gradient_op_map) {
  auto walk_result = func_op.walk([&](Operation* op) {
    if (toolchain::isa<TF::RecvTPUEmbeddingActivationsOp>(op))
      if (failed(ExtractEmbeddingAttribute(op, recv_activation_op_map)))
        return WalkResult::interrupt();

    if (toolchain::isa<TF::SendTPUEmbeddingGradientsOp>(op))
      if (failed(ExtractEmbeddingAttribute(op, send_gradient_op_map)))
        return WalkResult::interrupt();

    if (toolchain::isa<TF::EnqueueTPUEmbeddingSparseTensorBatchOp,
...
LogicalResult UpdateEmbeddingEnqueueOpInput(
    const toolchain::StringMap<Operation*>& enqueue_op_map,
    const toolchain::StringMap<Operation*>& recv_activation_op_map,
    const toolchain::StringMap<Operation*>& send_gradient_op_map,
    OpBuilder* builder) {
  for (const auto& it : enqueue_op_map) {
    ...
    toolchain::SmallVector<StringRef, 1> mode_string_value;
...
void TPUUpdateEmbeddingEnqueueOpInputsPass::runOnOperation() {
  ...
  toolchain::StringMap<Operation*> enqueue_op_map;
  toolchain::StringMap<Operation*> recv_activation_op_map;
  toolchain::StringMap<Operation*> send_gradient_op_map;
