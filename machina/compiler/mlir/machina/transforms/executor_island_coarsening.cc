...
class CoarseningAnalysis {
 public:
  ...
  iterator_range<
      toolchain::filter_iterator<SmallVector<MergedIsland>::const_iterator,
...
CoarseningAnalysis::CoarseningAnalysis(GraphOp graph) {
  ...
  do {
    ...
    for (MergedIsland& merged_island : toolchain::reverse(merged_islands_)) {
...
MergedIsland* CoarseningAnalysis::GetResultCandidateToMergeWith(
  ...
  for (IslandOp island : merged_island.islands) {
    ...
    Block& graph_body = toolchain::cast<GraphOp>(graph).GetBody();
...
struct IslandOperandsAndResults {
  toolchain::SmallSetVector<Value, 8> operands;
  toolchain::SmallVector<IslandResult> results;
};
...
void GetNewIslandResultsAndForwardResults(
  ...
  // islands.
  toolchain::SmallPtrSet<Block*, 8> islandBlocks;
  for (IslandOp island : merged_island.islands)
    island->walk([&](Block* block) { islandBlocks.insert(block); });

  for (IslandOp island : merged_island.islands) {
    for (auto ret_vals :
         toolchain::zip(island.GetYield().getOperands(), island.getOutputs())) {
      bool result_captured = false;
      Value inner_op_result = std::get<0>(ret_vals);
      Value island_result = std::get<1>(ret_vals);
      for (auto& use : toolchain::make_early_inc_range(island_result.getUses())) {
...
IslandOp CreateNewIsland(const MergedIsland& merged_island,
                         toolchain::ArrayRef<Value> operands,
                         toolchain::ArrayRef<IslandResult> results) {
...
YieldOp CreateNewIslandYieldOp(IslandOp new_island,
                               toolchain::MutableArrayRef<IslandResult> results) {
  toolchain::SmallVector<Value, 8> yield_operands;
  yield_operands.reserve(results.size());

  for (auto [old_result, new_island] :
       toolchain::zip(results, new_island.getOutputs())) {
...
void InsertDummyIslandForFetch(FetchOp fetch) {
  toolchain::SmallVector<Value, 4> data_fetches;
  toolchain::SmallVector<Type, 4> data_types;
  toolchain::SmallVector<Value, 4> control_fetches;
