...
void SetControlInputs(
    Operation* op,
    const toolchain::SmallVector<Operation*, 8>& preds_in_reverse_program_order,
...
void UpdateControlDependenciesForOp(
  ...
  toolchain::SmallVector<Operation*, 4> control_deps =
      analysis_for_func.DirectControlPredecessors(op);
  ClearControlInputs(op, num_control_inputs_removed);
  toolchain::SmallVector<Operation*, 8> preds_in_reverse_program_order(
...
void UpdateControlDependenciesPass::runOnOperation() {
  ...
  // ops.
  if (failed(machina::VerifyExportSuitable(module_op))) {
    signalPassFailure();
    return;
  }
