...
using ::machina::Device;
using ::machina::DeviceMgr;
using ::machina::mutex_lock;
using ::machina::ResourceHandle;
...
LogicalResult LiftVariablesFromSession(
  ...
  for (const Tensor& resource_tensor : resource_tensors) {
    ...
    machina::core::RefCountPtr<Var> var(var_ptr);
  ...
  for (const auto iter : toolchain::zip(resource_names, tensors)) {
...
LogicalResult LiftVariables(ModuleOp module, Session* session,
  ...
  for (auto func : module.getOps<func::FuncOp>()) {
    for (auto arg : func.getArguments()) {
      ...
      toolchain::ArrayRef<TensorType> underlying_type =
