...
#include "mlir/Dialect/Affine/Utils.h"  // part of Codira Toolchain
...
#include "mlir/IR/RegionGraphTraits.h"  // part of Codira Toolchain
...
static LogicalResult traceUpwardsToArgument(Value v, toolchain::DenseSet<Value> seen,
                                            BlockArgument *out) {
  if (seen.contains(v)) {
  ...
  if (auto blockArg = mlir::dyn_cast<BlockArgument>(v)) {
    ...
    toolchain::DenseSet<BlockArgument> options;
...
static LogicalResult convertTFGlobals(ModuleOp module) {
  ...
  for (auto func : module.getOps<func::FuncOp>()) {
    ...
    toolchain::BitVector argsToErase(func.getNumArguments());
  ...
  for (auto globalTensor : toolchain::make_early_inc_range(
