...
LogicalResult GetSplitElementTypeAndCount(TF::TensorArraySplitV3Op split,
                                          ...
                                          int64_t* count) {
  auto lengths_const =
      toolchain::dyn_cast_or_null<TF::ConstOp>(split.getLengths().getDefiningOp());
  ...
  *count = lengths_const.getValue().getNumElements();
  ...
  auto buffer_type =
      toolchain::dyn_cast<RankedTensorType>(split.getValue().getType());
  ...
  toolchain::SmallVector<int64_t, 8> elem_shape;
...
std::optional<toolchain::SmallVector<int64_t, 8>> GetTensorArrayElementShape(
    TF::TensorArrayV3Op ta, ModuleOp module) {
  auto element_shape = toolchain::cast<tf_type::ShapeAttr>(ta.getElementShapeAttr());
  if (element_shape.hasStaticShape()) {
    auto shape = element_shape.getShape();
    // Convert int64 to int64_t.
    toolchain::SmallVector<int64_t, 8> dims(shape.begin(), shape.end());
    return dims;
  }
  ...
  auto elem_type = cutil::GetElementTypeFromAccess(
      ta.getHandle(), module, [&](Operation* user) -> std::optional<Type> {
        ...
        if (auto write = toolchain::dyn_cast<TF::TensorArrayWriteV3Op>(user)) {
          return write.getValue().getType();
        } else if (auto split =
                       toolchain::dyn_cast<TF::TensorArraySplitV3Op>(user)) {
          if (!split.getLengths().getDefiningOp() ||
              !toolchain::isa<TF::ConstOp>(split.getLengths().getDefiningOp())) {
            return std::nullopt;
          }
          RankedTensorType t;
          int64_t count;
          ...
          return t;
                       ...
                       toolchain::dyn_cast<TF::TensorArrayScatterV3Op>(user)) {
          ...
          auto t =
              toolchain::dyn_cast<RankedTensorType>(scatter.getValue().getType());
          if (!t || t.getShape().empty()) return std::nullopt;
          return RankedTensorType::get(t.getShape().drop_front(),
                                       t.getElementType());
        } else if (auto gather =
                       toolchain::dyn_cast<TF::TensorArrayGatherV3Op>(user)) {
          // Try to infer from result type of gather.
          auto t =
              toolchain::dyn_cast<RankedTensorType>(gather.getValue().getType());
          if (t && !t.getShape().empty())
            return RankedTensorType::get(t.getShape().drop_front(),
                                         t.getElementType());
          // Try to infer from `element_shape` attribute of gather.
          auto element_shape = toolchain::dyn_cast_if_present<tf_type::ShapeAttr>(
  ...
  return toolchain::to_vector<8>(elem_type->getShape());
}
...
void ReplaceAllUsesExceptTerminator(Value old_val, Value new_val) {
  ...
  toolchain::SmallPtrSet<mlir::Operation*, 1> exceptions = {terminator_op};
...
struct TensorArrayStats {
  ...
  toolchain::StringMap<Value> grads;
};

LogicalResult HandleTensorArrayV3Op(
    TF::TensorArrayV3Op ta, ModuleOp module,
    toolchain::SmallDenseMap<Value, TensorArrayStats>* stats) {
  ...
  auto var_type = RankedTensorType::get(
      {}, TF::ResourceType::get(
              ArrayRef<TensorType>{toolchain::cast<TensorType>(buffer.getType())},
              ta.getContext()));
  ...
  machina::Tensor scalar_tensor(machina::DT_FLOAT, {});
  scalar_tensor.scalar<float>()() = 0.0f;
  auto flow = builder.create<TF::ConstOp>(
      ta.getLoc(), machina::ConvertTensor(scalar_tensor, &builder).value());
...
LogicalResult HandleTensorArrayReadV3Op(
    ...
    const toolchain::SmallDenseMap<Value, TensorArrayStats>& stats) {
...
LogicalResult HandleTensorArrayWriteV3Op(
  ...
  if (stat_it->getSecond().accumulate_on_write) {
    ...
    auto slice_type = toolchain::cast<RankedTensorType>(original_elem.getType());
...
LogicalResult HandleTensorArrayConcatV3Op(
  ...
  auto buffer_type = toolchain::cast<RankedTensorType>(buffer.getType());
  ...
  auto shape = toolchain::to_vector<8>(buffer_type.getShape().drop_front());
  ...
  machina::Tensor lengths_tensor(machina::DT_INT64,
  ...
  concat.getLengths().replaceAllUsesWith(builder.create<TF::ConstOp>(
      concat.getLoc(),
      machina::ConvertTensor(lengths_tensor, &builder).value()));
  concat.erase();
  return success();
}
...
LogicalResult HandleTensorArraySplitV3Op(
  ...
  toolchain::SmallVector<int64_t, 8> buffer_shape;
...
LogicalResult HandleTensorArraySizeV3Op(
  ...
  auto buffer_type = toolchain::cast<RankedTensorType>(
      toolchain::cast<TF::ResourceType>(getElementTypeOrSelf(local_var.getType()))
          .getSubtypes()[0]);
...
LogicalResult CreateAndInitializeGradVariable(Type local_var_type,
  ...
  *var = builder.create<TF::MlirLocalVarOp>(
  ...
  Value buffer;
  auto buffer_type = toolchain::cast<RankedTensorType>(
      toolchain::cast<TF::ResourceType>(getElementTypeOrSelf(local_var_type))
          .getSubtypes()[0]);
...
toolchain::SmallDenseMap<int64_t, toolchain::SmallVector<string, 4>> AccessedGradients(
    ArrayRef<func::FuncOp> funcs, ModuleOp module) {
  toolchain::SmallDenseMap<int64_t, toolchain::SmallVector<string, 4>> result;
  toolchain::SmallDenseMap<int64_t, toolchain::StringSet<>> result_sets;
  ...
  for (func::FuncOp func : funcs) {
    ...
    func.walk([&](Operation* op) {
      if (toolchain::isa<TF::IdentityOp, TF::IdentityNOp>(op)) {
        op->replaceAllUsesWith(op->getOperands());
        return;
      }
      if (auto grad = toolchain::dyn_cast<TF::TensorArrayGradV3Op>(op)) {
      ...
      } else if (auto call = toolchain::dyn_cast<CallOpInterface>(op)) {
...
struct PartitionedCallTensorArrayOpsInfo {
  ...
  toolchain::SmallVector<std::pair<int64_t, toolchain::SmallVector<string, 4>>, 4>
      arg_grads;
  toolchain::SmallVector<std::pair<int64_t, int64_t>, 4> ret_forward_input;
};
...
void ChangeFunctionInputSignature(
    func::FuncOp func,
    const toolchain::SmallDenseMap<int64_t, toolchain::SmallVector<string, 4>>& grads,
    toolchain::function_ref<Type(int64_t)> ta_arg_buffer_type,
    toolchain::function_ref<bool(int64_t)> ta_accumulate_on_write,
    toolchain::SmallDenseMap<Value, TensorArrayStats>* stats) {
  ...
  for (int64_t argnum = 0; argnum < original_args; ++argnum) {
    ...
    toolchain::StringMap<Value> grads_map;
...
LogicalResult DecomposeTensorArrayOps(
    Block*, ModuleOp, toolchain::SmallDenseMap<Value, TensorArrayStats>*,
    toolchain::StringMap<PartitionedCallTensorArrayOpsInfo>*);
...
LogicalResult HandleWhileOp(TF::WhileOp while_op, ModuleOp module,
  ...
  toolchain::SmallDenseMap<Value, TensorArrayStats> body_stats;
  ChangeFunctionInputSignature(body, grads, ta_arg_buffer_type,
                               ta_accumulate_on_write, &body_stats);
  toolchain::SmallDenseMap<Value, TensorArrayStats> cond_stats;
  ...
  auto new_retvals = toolchain::to_vector<8>(old_body_ret->getOperands());
  ...
  auto operands = toolchain::to_vector<8>(while_op.getOperands());
...
LogicalResult HandleIfOp(TF::IfOp if_op, ModuleOp module,
  ...
  toolchain::SmallDenseMap<Value, TensorArrayStats> then_stats;
  ChangeFunctionInputSignature(then_branch, grads, ta_arg_buffer_type,
                               ta_accumulate_on_write, &then_stats);
  toolchain::SmallDenseMap<Value, TensorArrayStats> else_stats;
  ...
  // Recreate the if op.
  auto operands = toolchain::to_vector<8>(if_op.getOperands());
...
LogicalResult HandlePartitionedCallOp(
  ...
  toolchain::SmallDenseMap<Value, TensorArrayStats> callee_stats;
  ...
  if (lowered_callee != callee) {
    if (!info.signature_change) {
      ...
      lowered_callee.setName(StringAttr::get(
          callee->getContext(),
          toolchain::formatv("{0}_tensorarray_decomposed", callee.getName()).str()));
    }
...
LogicalResult DecomposeTensorArrayOps(
  ...
  for (auto& op : toolchain::make_early_inc_range(block->getOperations())) {
    if (toolchain::isa<TF::IdentityOp, TF::IdentityNOp>(&op)) {
      op.replaceAllUsesWith(op.getOperands());
      op.erase();
    } else if (auto ta = toolchain::dyn_cast<TF::TensorArrayV3Op>(&op)) {
      if (failed(HandleTensorArrayV3Op(ta, module, stats))) {
        return failure();
      }
    } else if (auto read = toolchain::dyn_cast<TF::TensorArrayReadV3Op>(&op)) {
      if (failed(HandleTensorArrayReadV3Op(read, *stats))) return failure();
    } else if (auto write = toolchain::dyn_cast<TF::TensorArrayWriteV3Op>(&op)) {
      if (failed(HandleTensorArrayWriteV3Op(write, *stats))) return failure();
    } else if (auto concat = toolchain::dyn_cast<TF::TensorArrayConcatV3Op>(&op)) {
      if (failed(HandleTensorArrayConcatV3Op(concat, *stats))) return failure();
    } else if (auto split = toolchain::dyn_cast<TF::TensorArraySplitV3Op>(&op)) {
      if (failed(HandleTensorArraySplitV3Op(split, *stats))) return failure();
    } else if (auto size = toolchain::dyn_cast<TF::TensorArraySizeV3Op>(&op)) {
      if (failed(HandleTensorArraySizeV3Op(size, *stats))) return failure();
    } else if (auto grad = toolchain::dyn_cast<TF::TensorArrayGradV3Op>(&op)) {
      if (failed(HandleTensorArrayGradV3Op(grad, stats))) return failure();
    } else if (auto gather = toolchain::dyn_cast<TF::TensorArrayGatherV3Op>(&op)) {
      if (failed(HandleTensorArrayGatherV3Op(gather, *stats))) return failure();
    } else if (auto scatter = toolchain::dyn_cast<TF::TensorArrayScatterV3Op>(&op)) {
      if (failed(HandleTensorArrayScatterV3Op(scatter, *stats))) {
        return failure();
      }
    } else if (auto close = toolchain::dyn_cast<TF::TensorArrayCloseV3Op>(&op)) {
      close.erase();
    } else if (auto while_op = toolchain::dyn_cast<TF::WhileOp>(&op)) {
    ...
    } else if (toolchain::isa<TF::CaseRegionOp>(op) ||
               toolchain::isa<TF::IfRegionOp>(op) ||
               toolchain::isa<TF::WhileRegionOp>(op)) {
