...
std::optional<LocalVarOp> IsLocalVarOp(Operation &op) {
  if (TF::MlirLocalVarOp mlir_local_var_op =
          toolchain::dyn_cast<TF::MlirLocalVarOp>(&op)) {
    return std::make_optional(LocalVarOp(mlir_local_var_op));
  }
  if (TF::VarHandleOp var_handle_op = toolchain::dyn_cast<TF::VarHandleOp>(&op)) {
    auto ANONYMOUS_NAME = ::machina::ResourceHandle::ANONYMOUS_NAME;
...
void RemoveDeadLocalVariables(Block &block) {
  toolchain::SmallVector<LocalVarOp, 8> local_vars;
  ...
  for (auto local_var : local_vars) {
    ...
    if (toolchain::all_of(users, [](const Operation *user) {
          return isa<TF::AssignVariableOp>(user) ||
                 isa<TF::DestroyResourceOp>(user);
        })) {
      for (auto user : toolchain::make_early_inc_range(users)) user->erase();
...
void EliminateUnusedResults(
    Operation *op, const toolchain::BitVector *results_to_eliminate = nullptr) {
...
func::FuncOp CloneFunctionIfNeeded(func::FuncOp func) {
  ...
  if (func_uses.has_value() && toolchain::hasSingleElement(func_uses.value()))
    return func;
...
LogicalResult EliminateUnusedResultsForIfCase(Operation *op,
  ...
  for (OpResult result : toolchain::reverse(op->getResults())) {
  ...
  toolchain::BitVector used_args(num_args);
  for (func::FuncOp func : branches) {
...
LogicalResult EliminateUnusedResultsForWhile(TF::WhileOp op) {
  ...
  toolchain::BitVector can_eliminate(op.getNumResults());
  for (OpResult result : toolchain::reverse(op.getResults())) {
    if (!result.use_empty()) continue;
    int result_idx = result.getResultNumber();
...
LogicalResult CleanupAndCanonicalize(Operation *parent_op) {
  auto walk_result = parent_op->walk([](Operation *op) {
    ...
    for (Region &region : op->getRegions()) {
      if (!toolchain::hasSingleElement(region)) return WalkResult::interrupt();
