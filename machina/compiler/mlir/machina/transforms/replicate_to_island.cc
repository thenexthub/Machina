...
#include "machina/compiler/mlir/tf2xla/transforms/split_into_island_per_op_pass.h"
...
bool RequiresReplicaIDAttribute(Operation* op) {
  return toolchain::isa<TF::EnqueueTPUEmbeddingSparseTensorBatchOp,
...
LogicalResult GetDeviceOrdinal(const std::optional<DictionaryAttr>& devices,
  ...
  auto logical_core_name =
      machina::GetDeviceAliasForLogicalCore(logical_core);
  ...
  toolchain::StringRef tpu_device =
      toolchain::cast<StringAttr>(toolchain::cast<ArrayAttr>(tpu_replica)[replica_id])
          .getValue();
  return machina::GetDeviceOrdinalFromDeviceString(op->getLoc(), tpu_device,
                                                      &device_ordinal);
}
    ...
    if (auto launch = dyn_cast<tf_device::LaunchOp>(op))
      if (auto device_by_replica = devices.value().get(launch.getDevice()))
        launch->setAttr(kDeviceAttr,
                        toolchain::cast<StringAttr>(toolchain::cast<ArrayAttr>(
...
LogicalResult ExpandReplicateIntoReplicas(
    ...
    int num_replicas, toolchain::SmallVectorImpl<tf_executor::IslandOp>& replicas,
  ...
  toolchain::SmallVector<Type, 8> output_types(terminator.getOperandTypes());
  auto control_type = tf_executor::ControlType::get(island_op.getContext());
  toolchain::SmallVector<Value, 8> replica_inputs(island_op.getControlInputs());
  ...
  IRMapping mapping;
  for (int i : toolchain::seq<int>(0, num_replicas)) {
...
LogicalResult CreateIslandsFromReplicate(const Dialect* tf_dialect,
  ...
  toolchain::SmallVector<tf_executor::IslandOp, 8> replicas;
  ...
  toolchain::SmallVector<Value, 8> replicas_outputs(replicate_op.getNumResults(),
                                               nullptr);
  for (auto replica_and_idx : toolchain::enumerate(replicas))
    for (auto replica_result_and_idx :
         toolchain::enumerate(replica_and_idx.value().getOutputs()))
  ...
  for (auto result : toolchain::zip(island_op.getOutputs(), replicas_outputs))
  ...
  if (legacy_graph_export) {
    ...
    // execute.
    toolchain::SmallVector<Value, 8> unused_replica_controls;
    for (auto& replica : replicas)
    ...
    if (!unused_replica_controls.empty()) {
      ...
      auto fetches = toolchain::to_vector<8>(fetch.getOperands());
...
void ReplicateToIslandPass::runOnOperation() {
  ...
  toolchain::SmallVector<tf_executor::IslandOp, 4> replicate_op_islands;
