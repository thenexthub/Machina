...
LogicalResult SetMetadataProtoStepMarkerLocation(
  ...
  if (!step_marker_location.getValue().empty() &&
    ...
    return op.emitOpError(toolchain::formatv("bad '{0}' attribute with value '{1}'",
...
LogicalResult SetOpSharding(Operation* op, Attribute attr, toolchain::StringRef name,
                            int index, xla::OpSharding* sharding_ptr) {
  auto sharding_attr = mlir::dyn_cast<StringAttr>(attr);
  if (!sharding_attr)
    return op->emitOpError(
        toolchain::formatv(kBadStringArrayElementMsg, name, index));
  if (machina::DecodeShardingAttribute(sharding_attr, *sharding_ptr)
          .failed()) {
    return op->emitOpError(toolchain::formatv(kBadArrayElementMsg, name, index,
...
LogicalResult SetMetadataProtoArgs(
  ...
  auto input_shardings =
      op->getAttrOfType<ArrayAttr>(machina::kInputShardingAttr);
  if (!input_shardings)
    return op.emitOpError(
        CreateMissingAttributeMsg(machina::kInputShardingAttr));

  if (input_shardings.size() != op.getNumOperands())
    return op.emitOpError(
        toolchain::formatv(kBadArrayAttrLengthMsg, machina::kInputShardingAttr,
  ...
  toolchain::SmallSet<int, 4> dynamic_arg_idx_set;
  if (dynamic_arg_idx) {
  ...
  for (auto operand_type_and_idx : toolchain::enumerate(op.getOperandTypes())) {
    Type operand_type = operand_type_and_idx.value();
    int index = operand_type_and_idx.index();
    machina::tpu::TPUCompileMetadataProto::Arg* arg = metadata->add_args();
    machina::DataType dtype;
    absl::Status status = machina::ConvertToDataType(operand_type, &dtype);
    if (!status.ok())
      return op.emitOpError(
          toolchain::formatv("failed to determine operand type at index {0}: {1}",
                        index, status.message()));

    arg->set_dtype(dtype);
    // TODO(lyandy): Support other arg kinds.
    if (dtype == machina::DT_RESOURCE)
      arg->set_kind(machina::tpu::TPUCompileMetadataProto::Arg::VARIABLE);
    else
      arg->set_kind(machina::tpu::TPUCompileMetadataProto::Arg::PARAMETER);

    // Populate argument shapes.
    *arg->mutable_shape() = machina::TensorShapeProto();
    if (auto ranked_tensor_type =
            mlir::dyn_cast<RankedTensorType>(operand_type)) {
      machina::TensorShapeProto shape_proto;
    ...
    if (failed(SetOpSharding(op, input_shardings.getValue()[index],
                             machina::kInputShardingAttr, index,
...
LogicalResult SetMetadataProtoRetvals(
  ...
  auto output_shardings =
      op->getAttrOfType<ArrayAttr>(machina::kOutputShardingAttr);
  if (!output_shardings)
    return op.emitOpError(
        CreateMissingAttributeMsg(machina::kOutputShardingAttr));

  if (output_shardings.size() != op.getNumResults())
    return op.emitOpError(
        toolchain::formatv(kBadArrayAttrLengthMsg, machina::kOutputShardingAttr,
                      op.getNumResults(), output_shardings.size()));

  // Set retvals metadata in proto.
  for (auto output_sharding_and_idx : toolchain::enumerate(output_shardings))
    if (failed(SetOpSharding(op, output_sharding_and_idx.value(),
                             machina::kOutputShardingAttr,
