...
bool IsResourceSafeForMerge(
    ...
    const toolchain::SmallDenseSet<int64_t>& resource_ids,
...
bool AddAccessedResourceIds(
    Operation* op,
    const mlir::TF::ResourceAliasAnalysis::Info& resource_analysis_info,
    toolchain::SmallDenseSet<int64_t>& resource_ids) {
...
bool IsResourceMergeable(Attribute& resource_attr, Attribute& device_attr) {
  return resource_attr &&
         ((resource_attr == device_attr) ||
          (toolchain::cast<StringAttr>(resource_attr).getValue().find("COMPOSITE") !=
           toolchain::StringRef::npos));
}
...
VariableAccessesForTPUExecute BuildVariableAccessInfo(
  ...
  auto parallel_execute = toolchain::dyn_cast<tf_device::ParallelExecuteOp>(
  ...
  for (auto operand : toolchain::enumerate(execute->getOpOperands())) {
    var_access_info.new_operand_values.push_back(operand.value().get());
    auto read_op = toolchain::dyn_cast_or_null<TF::ReadVariableOp>(
  ...
  for (Operation& op : toolchain::reverse(toolchain::make_range(
           first_read->getIterator(), execute_parent->getIterator()))) {
    if (auto read_op = toolchain::dyn_cast<TF::ReadVariableOp>(&op)) {
  ...
  toolchain::SmallPtrSet<Operation*, 8> all_assigns;
  toolchain::SmallVector<bool, 8> output_merged(execute_launch.getNumResults(),
                                           false);
  ...
  for (auto execute_output : toolchain::enumerate(execute_outputs)) {
    ...
    if (!result.hasOneUse()) {
      if (VLOG_IS_ON(2)) {
        ...
        for (auto result_user : result.getUsers()) {
          any_user_is_assign |= toolchain::isa<TF::AssignVariableOp>(result_user);
        }
    ...
    auto assign_op = toolchain::dyn_cast<TF::AssignVariableOp>(*result.user_begin());
    if (!assign_op) {
  ...
  if (last_assign != nullptr) {
    ...
    for (Operation& op :
         toolchain::make_range(std::next(execute_parent->getIterator()),
                          std::next(last_assign->getIterator()))) {
      if (auto assign_op = toolchain::dyn_cast<TF::AssignVariableOp>(&op)) {
...
int AppendTypes(toolchain::SmallVectorImpl<Type>* output_types,
...
void ReplaceParallelExecute(
  ...
  toolchain::SmallVector<Type, 8> output_types;
  ...
  for (auto region : toolchain::zip(new_parallel_execute_op->getRegions(),
...
LogicalResult MergeForOneTPUExecute(
  ...
  toolchain::SmallVector<Type, 8> new_output_types;
  ...
  toolchain::SmallVector<int64_t, 8> device_var_reads_indices;
  toolchain::SmallVector<int64_t, 8> device_var_updates_indices;
  ...
  for (auto it : toolchain::enumerate(var_access_info.new_operand_values)) {
    Type type = it.value().getType();
    if (isa<TensorType>(type) &&
        isa<TF::ResourceType>(cast<TensorType>(type).getElementType())) {
      if (!toolchain::is_contained(device_var_reads_indices, it.index()) &&
          !toolchain::is_contained(device_var_updates_indices, it.index())) {
  ...
  if (auto parallel_execute = toolchain::dyn_cast<tf_device::ParallelExecuteOp>(
          execute_launch->getParentOp()))
...
bool ParentParallelExecuteWrapsSingleOp(Operation* op) {
  auto parallel_execute =
      toolchain::dyn_cast<tf_device::ParallelExecuteOp>(op->getParentOp());
...
void TPUMergeVariablesWithExecutePass::runOnOperation() {
  ...
  module.walk([&](func::FuncOp func) {
    ...
    func.walk([&](tf_device::LaunchOp op) {
      if (op.WrapsSingleOp() &&
          toolchain::isa<TF::TPUExecuteOp>(op.GetBody().front()) &&
    ...
    for (auto execute_launch : execute_launches) {
      ...
      const bool parent_is_replicate =
          toolchain::isa<tf_device::ReplicateOp>(execute_launch->getParentOp()) ||
          (toolchain::isa<tf_device::ParallelExecuteOp>(
               execute_launch->getParentOp()) &&
           toolchain::isa<tf_device::ReplicateOp>(
