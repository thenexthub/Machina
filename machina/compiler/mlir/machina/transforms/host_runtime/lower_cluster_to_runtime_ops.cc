...
void AddNonTPULowerClusterToRuntimeOpsPassPipeline(
    OpPassManager& pm, toolchain::StringRef module_name) {
...
absl::Status RecordIfErrorStatus(const std::string error_prefix,
  ...
  if (device_type != DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT)) {
...
absl::Status RunLowerClusterToRuntimeOpsPassPipeline(
  ...
  ::machina::applyTensorflowAndCLOptions(runtime_lowering);

  if (xla_device_type == DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT)) {
  ...
  if (VLOG_IS_ON(1) ||
    ...
    ::machina::DumpMlirOpToFile(
        ...
        module, toolchain::StringRef(), &runtime_lowering);
  }
  ...
  std::string bridge_type = xla_device_type == DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT)
