...
std::string GetRandomStateVariableName() {
  return absl::StrCat("VariablesFormatState_", machina::random::New64());
}
...
Value SkipIdentity(Value v, bool allow_other_use,
                   toolchain::SmallPtrSet<Operation*, 4>* skipped = nullptr) {
...
toolchain::SmallVector<std::pair<int64_t, toolchain::SmallVector<Value, 4>>, 4>
AnnotateCompileOpAndGetExecuteArgToWhileArgsMapping(
  ...
  toolchain::SmallVector<std::pair<int64_t, toolchain::SmallVector<Value, 4>>, 4> mapping;
  ...
  toolchain::SmallDenseMap<int64_t, int64_t, 8> replicate_arg_to_execute_arg;
  for (auto index_and_arg : toolchain::enumerate(execute.getArgs())) {
  ...
  for (const auto& mirrored_index : mirrored_variable_indices_attr) {
    ...
    // arguments.
    toolchain::SmallVector<Value, 4> replicate_args;
    for (int64_t i = 0; i < num_inputs; ++i) {
      toolchain::SmallPtrSet<Operation*, 4> skipped_identities;
      ...
      bool has_other_use_in_body =
          toolchain::any_of(replicate_operand.getUsers(), [&](Operation* user) {
...
tf_device::ReplicateOp AddInputsToReplicateOp(
    ...
    const toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallVector<StringRef, 4>>&
        devices) {
  int64_t num_replicas = replicate.getN();
  ...
  // supported.
  assert(devices.find(machina::GetDeviceAliasForLogicalCore(0))
             ->getSecond()
             .size() == num_replicas);

  toolchain::SmallVector<std::pair<ValueRange, Type>, 8> new_replicated_inputs;
  toolchain::SmallVector<Value, 8> new_packed_inputs;
  toolchain::SmallVector<toolchain::SmallVector<Value, 8>, 8> replicated_inputs;
  ...
  for (auto& op : toolchain::make_early_inc_range(replicate.GetBody())) {
...
// device.
toolchain::SmallVector<TF::VarHandleOp, 4> CreateStateVars(
    const toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallVector<StringRef, 4>>&
        devices,
    Location loc, RankedTensorType key_type, OpBuilder* builder) {
  toolchain::SmallVector<TF::VarHandleOp, 4> state_vars;

  // TODO(b/148913020): Remove this constraint once model parallelism is
  // supported.
  const auto& device_list =
      devices.find(machina::GetDeviceAliasForLogicalCore(0))->getSecond();

  // Create the state variable for each device.
  for (toolchain::StringRef device : device_list) {
    state_vars.push_back(builder->create<TF::VarHandleOp>(
        loc,
        toolchain::ArrayRef<Type>{RankedTensorType::get(
            {}, TF::ResourceType::get(toolchain::ArrayRef<TensorType>{key_type},
                                      builder->getContext()))},
...
bool HandleReplicateOp(TF::WhileRegionOp while_op,
  ...
  replicate.walk([&](TF::TPUExecuteAndUpdateVariablesOp execute_op) {
    execute_launch =
        toolchain::dyn_cast<tf_device::LaunchOp>(execute_op->getParentOp());
  ...
  auto compile_launch = toolchain::dyn_cast<tf_device::LaunchOp>(compile);
  if (!compile_launch || !compile_launch.WrapsSingleOp() ||
      !toolchain::isa<TF::_TPUCompileMlirOp>(compile_launch.GetBody().front()))
    return false;
  ...
  toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallVector<StringRef, 4>> devices;
  ...
  for (auto it : device_map) {
    ...
    toolchain::SmallVector<StringRef, 4> device_list_for_alias;
  ...
  // `replicate`.
  toolchain::SmallVector<Value, 8> reformat_operands;
  ...
  auto reformat_op = builder.create<TF::TPUReshardVariablesOp>(
      execute_launch.getLoc(), toolchain::ArrayRef<Type>{}, reformat_operands);
  ...
  // replicate op.
  toolchain::SmallVector<std::pair<ValueRange, Type>, 8> unformat_replicate_operands;
  toolchain::SmallVector<Value, 8> unformat_packed_operands;
  for (const auto& entry : execute_arg_to_outer_args) {
  ...
  toolchain::SmallVector<Value, 4> state_var_vals(state_vars.size());
  for (const auto& entry : toolchain::enumerate(state_vars)) {
  ...
  machina::Tensor default_key_tensor(machina::DT_STRING, {3});
  default_key_tensor.vec<machina::tstring>()(0) = kDefaultShardingValue;
  default_key_tensor.vec<machina::tstring>()(1) = kDefaultShardingValue;
  default_key_tensor.vec<machina::tstring>()(2) = kDefaultShardingValue;
  auto default_state_key = builder.create<TF::ConstOp>(
      while_op.getLoc(),
      machina::ConvertTensor(default_key_tensor, &builder).value());
  ...
  toolchain::SmallVector<Value, 8> unformat_operands;
  ...
  auto unformat_op = builder.create<TF::TPUReshardVariablesOp>(
      while_op.getLoc(), toolchain::ArrayRef<Type>{}, unformat_operands);
