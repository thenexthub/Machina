...
LogicalResult CheckNoRecursion(ModuleOp module, CallGraph &call_graph) {
  for (toolchain::scc_iterator<const CallGraph *> scci =
           toolchain::scc_begin<const CallGraph *>(&call_graph);
       !scci.isAtEnd(); ++scci) {
    if (scci.hasCycle()) {
      auto err = module.emitError()
                 << "A recursive call graph cannot be transformed to "
                    ...
                    "recursive cycle are: ";
      toolchain::interleaveComma(*scci, err, [&](CallGraphNode *node) {
 ...
 public:
  ...
  LogicalResult Run() {
    ...
    do {
      ...
      made_changes = false;
      for (auto func :
           toolchain::make_early_inc_range(module.getOps<func::FuncOp>())) {
