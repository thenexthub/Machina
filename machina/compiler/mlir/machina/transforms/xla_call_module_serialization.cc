...
#include "machina/compiler/mlir/machina/transforms/tf_passes.h.inc"  // IWYU pragma: keep
...
FailureOr<ArrayAttr> DesymbolizeCustomCallCalledIndex(ModuleOp module) {
  ...
  toolchain::DenseMap<SymbolRefAttr, int> called_indexes;
...
FailureOr<OwningOpRef<ModuleOp>> PruneStablehloModule(
  ...
  WalkResult result = WalkReachableFunctions(
      ...
      [&](func::FuncOp f) -> WalkResult {
        ...
        auto cloned = toolchain::cast<func::FuncOp>(builder.clone(*f));
