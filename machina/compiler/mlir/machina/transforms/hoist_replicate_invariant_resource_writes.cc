...
bool IsResourceType(Type type) {
  return toolchain::isa<TF::ResourceType>(type) ||
         (toolchain::isa<TensorType>(type) &&
          toolchain::isa<TF::ResourceType>(
...
void MoveTailWritesAfterReplicate(
    ...
    toolchain::ArrayRef<TF::AssignVariableOp> tail_assign_variable_ops) {
  const auto num_replicas = replicate_op.getN();
  auto return_op = toolchain::dyn_cast<tf_device::ReturnOp>(
  ...
  // returned.
  auto new_result_types = toolchain::to_vector<4>(replicate_op->getResultTypes());
  ...
  for (auto indexed_assign : toolchain::enumerate(tail_assign_variable_ops)) {
...
SmallVector<TF::AssignVariableOp> GetTailWritesToReplicateInvariantResourceVars(
  ...
  toolchain::SmallDenseSet<Value, 16> visited_resources;
  for (auto& op :
       toolchain::reverse(replicate_op.getRegion().front().getOperations())) {
    SmallVector<Value> op_accessed_resources = GetAccessedResources(op);
    if (op_accessed_resources.empty()) continue;

    if (auto assign = toolchain::dyn_cast<TF::AssignVariableOp>(op)) {
