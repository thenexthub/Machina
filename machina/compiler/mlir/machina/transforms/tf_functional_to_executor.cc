...
void FunctionalToExecutorDialectConversion::runOnOperation() {
  ...
  if (!toolchain::hasSingleElement(func)) {
    LLVM_DEBUG(toolchain::dbgs() << "Expect single block function, skip conversion "
  ...
  if (!return_op) {
    LLVM_DEBUG(toolchain::dbgs() << "Expect function to end with return\n");
    return;
  }
  // Build GraphOp.
  ...
  for (auto item : toolchain::enumerate(graph_op.getResults())) {
