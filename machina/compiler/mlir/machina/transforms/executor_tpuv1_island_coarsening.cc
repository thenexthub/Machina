...
constexpr toolchain::StringRef kTpuStatusAttr = "_tpu_compilation_status";
constexpr toolchain::StringRef kNoReplicationCluster = "__no_replication_cluster";
...
// returns `std::nullopt`.
std::optional<toolchain::StringRef> GetTpuClusterName(Operation* op) {
...
LogicalResult SortTopologically(Block::iterator begin, Block::iterator end) {
  ...
  for (Operation& op : toolchain::make_range(begin, end))
  ...
  while (!unscheduled_ops.empty()) {
    ...
    for (Operation& op : toolchain::make_range(last_scheduled_op, end)) {
...
void CollectCandidateIslands(
    toolchain::function_ref<bool(toolchain::StringRef, Operation*)>
  ...
  for (Operation& candidate_op : toolchain::make_early_inc_range(
           toolchain::make_range(op->getIterator(), op->getBlock()->end()))) {
    ...
    std::optional<toolchain::StringRef> result =
        GetTpuClusterName(&candidate_wrapped_op);
    toolchain::StringRef candidate_cluster_name;
    if (result.has_value()) {
...
IslandOp CreateMergedIsland(IslandOp island, SmallVector<IslandOp, 16>& islands,
  ...
  for (IslandOp new_op : islands) {
    for (Value result : new_op.getOutputs()) {
      if (toolchain::any_of(result.getUsers(), [&](OpOperand user) {
  ...
  for (IslandOp island : islands) {
    ...
    for (auto result :
         toolchain::zip(island.getOutputs(), wrapped_op.getResults())) {
      if (toolchain::any_of(std::get<0>(result).getUsers(), [&](OpOperand user) {
            return !wrapped_ops.count(user.getOwner());
          }))
  ...
  for (IslandOp island : islands) {
    ...
    for (const auto& idx_result : toolchain::enumerate(island.getOutputs())) {
      Value result = idx_result.value();

      bool has_external_use = false;
      for (OpOperand& use : toolchain::make_early_inc_range(result.getUses())) {
...
LogicalResult MergeIsland(
    toolchain::function_ref<bool(StringRef, Operation*)>
        is_op_calling_func_for_cluster,
    toolchain::SmallDenseMap<StringRef, toolchain::SmallDenseSet<Operation*>>&
  ...
  std::optional<toolchain::StringRef> result = GetTpuClusterName(&wrapped_op);
  if (!result.has_value()) return success();
  toolchain::StringRef cluster_name = result.value();

  // We found a _replication_info, let's build an island for the full cluster!
  LLVM_DEBUG(toolchain::dbgs() << "Processing candidate island: "
                          << *island.getOperation() << "\n");
  ...
  for (Operation& candidate_op : toolchain::make_early_inc_range(
           toolchain::make_range(op->getBlock()->begin(), op->getBlock()->end()))) {
    IslandOp candidate_island = dyn_cast<IslandOp>(candidate_op);
    if (!candidate_island || !candidate_island.WrapsSingleOp()) continue;
...
SmallPtrSet<Operation*, 16> FindTPUPartitionedCallReachableFunctions(
  ...
  toolchain::DenseMap<func::FuncOp, toolchain::DenseSet<func::FuncOp>> caller_callee_map;
  ...
  for (auto func : module.getOps<func::FuncOp>()) {
    for (auto user : symbol_map.getUsers(func)) {
      ...
      if (toolchain::isa<TF::TPUPartitionedCallOp>(user)) {
...
bool is_valid_special_tpu_op(
    std::vector<IslandOp>& ops, toolchain::StringRef cluster_name,
    toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallDenseSet<Operation*>>&
        cluster_to_tpu_op_map) {
  for (IslandOp op : ops) {
    Operation* wrapped_op = &op.GetBody().front();
    std::optional<toolchain::StringRef> wrapped_op_cluster_name =
...
bool AddSpecialTpuOps(
    IslandOp candidate_island, toolchain::StringRef cluster_name,
    toolchain::SmallDenseMap<toolchain::StringRef, toolchain::SmallDenseSet<Operation*>>&
        cluster_to_tpu_op_map,
...
bool ExcludeIdentityOp(toolchain::SmallDenseSet<Operation*>& tpu_ops,
                       toolchain::StringRef& target_cluster_name, bool incoming) {
  for (auto iter = tpu_ops.begin(); iter != tpu_ops.end(); iter++) {
    auto island_op = toolchain::dyn_cast<IslandOp>(*iter);
    if (toolchain::dyn_cast_or_null<TF::IdentityOp>(island_op.GetBody().front())) {
...
void EraseIdentityWithNoReplicationInfo(Block& graph_body) {
  for (Operation& island_op : graph_body) {
    ...
    for (Operation& op : toolchain::make_early_inc_range(island.GetBody())) {
      std::optional<toolchain::StringRef> cluster_name = GetTpuClusterName(&op);
      if (cluster_name.has_value()) continue;
      if (auto identity_op = toolchain::dyn_cast_or_null<TF::IdentityOp>(op)) {
...
void TpuV1BridgeExecutorIslandCoarsening::runOnOperation() {
  ...
  for (func::FuncOp func_op : getOperation().getOps<func::FuncOp>()) {
    func_op.walk([&](Operation* op) {
      std::optional<toolchain::StringRef> cluster_name_opt = GetTpuClusterName(op);
  ...
  auto is_op_calling_func_for_cluster = [&](toolchain::StringRef cluster,
                                            Operation* op) {
