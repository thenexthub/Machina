...
#include "machina/compiler/mlir/machina/transforms/decompose_resource_ops.h"
...
static Type GetResourceSubtypeOrDefault(Value resource, Type element_type) {
  auto resource_type = toolchain::cast<tf_type::ResourceType>(
      toolchain::cast<TensorType>(resource.getType()).getElementType());
...
class DecomposeRngReadAndSkipOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    machina::ConcreteRngAlgorithm alg;
    if (machina::RNG_ALG_PHILOX == alg_value) {
      alg = machina::ConcreteRngAlgorithm::RNG_ALG_PHILOX;
    } else if (machina::RNG_ALG_THREEFRY == alg_value) {
      alg = machina::ConcreteRngAlgorithm::RNG_ALG_THREEFRY;
    } else if (machina::RNG_ALG_AUTO_SELECT == alg_value) {
    ...
    RankedTensorType op_type = RankedTensorType::get(
        {machina::RNG_MAX_COUNTER_SIZE + machina::RNG_KEY_SIZE},
        state_element_type);
    ...
    int counter_size = machina::GetCounterSize(alg);
    int state_size = counter_size + machina::RNG_KEY_SIZE;
    ...
    int pad = machina::RNG_MAX_COUNTER_SIZE - counter_size;
...
#include "machina/compiler/mlir/machina/transforms/generated_decompose_resource_ops.inc"
}  // namespace
