...
#include "machina/compiler/mlir/machina/transforms/cluster_ops_by_policy.h"
...
void ValuesConstraintSet::Walk(
    toolchain::function_ref<void(Value, ValueConstraint)> walk) const {
...
ValuesConstraintSet &ValuesConstraintSet::Resolve() {
  toolchain::SmallDenseSet<Value, 4> resolved;
...
using Members = toolchain::SmallVector<Member>;
...
struct ClusteringState {
  ...
  toolchain::SmallVector<Member> members;

  // Mapping from the member operation (block argument) to the member id.
  toolchain::SmallDenseMap<Source, unsigned> member_ids;
...
LogicalResult ClusteringState::VerifyValueConstraints(
  ...
  toolchain::SmallVector<Operation *> worklist;
...
LogicalResult ClusteringState::Union(unsigned a, unsigned b,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "  Clusters successfully merged\n");
...
static ClusteringState InitializeClusteringState(
  ...
  for (const auto &tuple : toolchain::enumerate(state.members)) {
    state.member_ids.try_emplace(tuple.value().source, tuple.index());
  }

  LLVM_DEBUG(toolchain::dbgs() << "Found "
...
static toolchain::SmallVector<Operation *> GetClusteringCandidates(
  ...
  if (auto op = source.dyn_cast<Operation *>()) {
    auto range = toolchain::make_filter_range(op->getUsers(), [&](Operation *user) {
  ...
  if (auto arg = source.dyn_cast<BlockArgument *>()) {
    auto range = toolchain::make_filter_range(arg->getUsers(), [&](Operation *user) {
...
static bool RunClusteringPass(ClusteringState &state,
  ...
  for (const auto &tuple : toolchain::enumerate(state.members)) {
    ...
    toolchain::SmallVector<Operation *> users =
    ...
    toolchain::sort(users, [](auto *a, auto *b) { return a->isBeforeInBlock(b); });
...
toolchain::SmallVector<Cluster> FindClustersInTheBlock(
    Block *block, const ClusteringPolicySet &policies,
  ...
  toolchain::DenseMap<unsigned, Cluster> root_clusters;
  ...
  toolchain::SmallVector<Cluster> clusters;
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Found " << clusters.size() << " clusters\n");
...
tf_device::ClusterOp CreateClusterOp(Cluster &cluster, StringAttr policy) {
  ...
  toolchain::DenseSet<Operation *> in_cluster;
  for (Operation *op : cluster.operations) in_cluster.insert(op);

  toolchain::SetVector<Value> return_values;
  toolchain::SmallVector<Type> return_types;
  ...
  toolchain::sort(cluster.operations, [](Operation *a, Operation *b) -> bool {
  ...
  for (auto tuple : toolchain::zip(return_values, cluster_op.getResults())) {
...
mlir::LogicalResult PropagateValuesConstraints(
    toolchain::ArrayRef<Operation *> root, std::function<bool(Operation *)> filter,
  ...
  toolchain::DenseMap<Operation *, ValuesConstraintSet> op_results_constraints;
  ...
  toolchain::SetVector<Operation *> worklist;
  ...
  while (!worklist.empty()) {
    ...
    if (!updated && !results.Empty()) {
      if (emit_remarks) {
        ...
        op->emitError(toolchain::formatv(
  ...
  constraints.Walk([](Value value, ValueConstraint constraint) {
    for (OpOperand &operand : value.getUses())
      operand.getOwner()->emitRemark(
          toolchain::formatv("operand #{0} constrained to: {1}",
  ...
  constraints.Walk([&](Value value, ValueConstraint constraint) {
    if (auto arg = mlir::dyn_cast<BlockArgument>(value))
      if (arg.getOwner() == &func.getBody().front())
        func.emitRemark(toolchain::formatv("input #{0} constrained to: {1}",
