...
constexpr toolchain::StringRef kOutlinedFuncPrefix = "_tpu_v1_compat_outlined_func";
...
void TPUBridgeExecutorIslandOutlining::runOnOperation() {
  ...
  toolchain::SmallVector<IslandOp, 8> islands_to_outline;
  ...
  for (IslandOp island_op : islands_to_outline) {
    ...
    toolchain::SetVector<Value> operands;
    ...
    name += toolchain::Twine(prefix_id++).str();
