...
using ResourceIdVec = toolchain::SmallVector<std::pair<TF::ResourceId, bool>>;
// MapVector provides both fast lookup and deterministic iteration order.
using IslandToIslandMapVec =
    toolchain::MapVector<IslandOp, toolchain::SmallVector<IslandOp>>;
using IslandToIslandHashMap = toolchain::SmallDenseMap<IslandOp, IslandOp>;
