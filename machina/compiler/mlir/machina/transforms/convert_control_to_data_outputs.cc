...
void CollectChainResources(
    ...
    toolchain::EquivalenceClasses<ResourceAndDevice>& resource_equivalence_classes,
    DeviceMap& devices,
    const TF::SideEffectAnalysis::Info& side_effect_analysis,
  ...
  graph_op.walk([&](IslandOp island) {
    ...
    if (auto execute = toolchain::dyn_cast<TF::TPUExecuteAndUpdateVariablesOp>(op)) {
...
void AppendFunctionResults(func::FuncOp func, int num_resources,
  ...
  auto new_result_types =
      toolchain::to_vector<4>(func.getFunctionType().getResults());
  for (int i = 0; i < num_resources; ++i) {
...
void ChainResourceOps(
  ...
  for (auto class_iter = resource_equivalence_classes.begin();
    ...
    toolchain::SmallDenseSet<Operation*> processed_ops;
...
TF::WhileOp RewriteWhileOp(TF::WhileOp while_op, int num_resource_inputs,
  ...
  auto new_operands = toolchain::to_vector<4>(while_op->getOperands());
  auto new_result_types = toolchain::to_vector<4>(while_op->getResultTypes());
...
void ConvertControlToDataOutputsPass::runOnOperation() {
  ...
  if (failed(machina::VerifyExportSuitable(module))) {
    signalPassFailure();
    return;
  }
  ...
  toolchain::SmallDenseMap<func::FuncOp, SmallVector<TF::WhileOp>>
