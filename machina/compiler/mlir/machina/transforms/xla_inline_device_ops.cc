...
void InlineDeviceOp(tf_device::ClusterOp cluster_op) {
  ...
  toolchain::SmallVector<Operation*, 4> non_terminator_ops;
  ...
  for (auto it : toolchain::zip(cluster_op.getResults(), return_op.getOperands())) {
...
void XlaInlineDeviceOpsPass::runOnOperation() {
  ...
  toolchain::SmallVector<tf_device::ClusterOp, 4> ops;
  module.walk(
