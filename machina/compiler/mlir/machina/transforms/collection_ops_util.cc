...
Value GetIndicesForElement(Value index, Value buffer, OpBuilder builder,
  ...
  toolchain::SmallVector<int64_t, 8> zeros(buffer_type.getShape().size() - 1, 0);
  auto zeros_tensor = GetR1Const(zeros, builder, loc);
  return TF::ConcatV2Op::create(
      builder, loc,
      ArrayRef<Type>{machina::GetTypeFromTFTensorShape(
...
Value GetElement(Value index, Value buffer, OpBuilder builder, Location loc,
  ...
  // size 1.
  toolchain::SmallVector<int64_t, 8> slice_size =
      toolchain::to_vector<8>(buffer_type.getShape());
  slice_size[0] = 1;
  auto size_const = GetR1Const(slice_size, builder, loc);
  auto slice_type = machina::GetTypeFromTFTensorShape(
  ...
  auto element_type = machina::GetTypeFromTFTensorShape(
...
Value SetElement(Value index, Value buffer, Value element, OpBuilder builder,
  ...
  auto slice_shape = toolchain::to_vector<8>(buffer_type.getShape());
  slice_shape[0] = 1;
  auto slice_type = machina::GetTypeFromTFTensorShape(
...
TensorType GetSizeType(OpBuilder builder) {
  return machina::GetTypeFromTFTensorShape({1}, builder.getIntegerType(32));
}
...
LogicalResult CreateInitBufferValue(ArrayRef<int64_t> element_shape,
  ...
  auto max_count_const_op = toolchain::dyn_cast<TF::ConstOp>(max_count_op);
...
LogicalResult CreateInitBufferValue(ArrayRef<int64_t> element_shape,
  ...
  if (getElementTypeOrSelf(zero.getType()) != element_dtype) {
    zero = TF::CastOp::create(
        builder, op->getLoc(),
        ArrayRef<Type>{machina::GetTypeFromTFTensorShape({}, element_dtype)},
        ArrayRef<Value>{zero});
  }
  auto buffer_type =
      machina::GetTypeFromTFTensorShape(buffer_shape, element_dtype);
...
std::optional<RankedTensorType> GetElementTypeFromAccess(
    ...
    toolchain::function_ref<std::optional<Type>(Operation*)> infer_from_op) {
  for (auto& use : collection.getUses()) {
    if (auto while_op = toolchain::dyn_cast<TF::WhileOp>(use.getOwner())) {
    ...
    } else if (auto if_op = toolchain::dyn_cast<TF::IfOp>(use.getOwner())) {
    ...
    } else if (auto call = toolchain::dyn_cast<CallOpInterface>(use.getOwner())) {
    ...
    } else if (toolchain::isa<TF::IdentityOp, TF::IdentityNOp>(use.getOwner())) {
...
int64_t GetFirstIfIndicesAreContiguous(Value indices) {
  ...
  auto const_op = toolchain::dyn_cast<TF::ConstOp>(indices_op);
...
Value GatherElements(Value indices, Value buffer, OpBuilder builder,
                     Location loc) {
  auto buffer_type = mlir::cast<RankedTensorType>(buffer.getType());
  auto result_shape = toolchain::to_vector<8>(buffer_type.getShape());
  result_shape[0] =
  ...
  if (maybe_contiguous_start >= 0) {
    toolchain::SmallVector<int64_t, 8> slice_starts(result_shape.size(), 0);
...
Value ScatterAccumulateElements(Value indices, Value updates, Value buffer,
  ...
  auto per_slice_shape = toolchain::to_vector<8>(buffer_type.getShape());
  per_slice_shape[0] = 1;
  auto slice_sizes = GetR1Const(per_slice_shape, builder, loc);
  toolchain::SmallVector<int64_t, 8> starts_in_update(buffer_type.getRank(), 0);
