...
static DenseElementsAttr GetF32Scalar(OpBuilder *builder, float value) {
  return DenseElementsAttr::get(
      machina::GetTypeFromTFTensorShape({}, builder->getF32Type()),
...
DenseIntElementsAttr GetBiasAddGradReductionIndices(int64_t rank,
  ...
  toolchain::SmallVector<int64_t, 4> dims_to_reduce(rank - 1);
...
#include "machina/compiler/mlir/machina/transforms/generated_lower_tf.inc"
...
Type InferExpandDimsType(Type ty, int64_t axis, Builder *builder) {
  ...
  auto shape = toolchain::to_vector<4>(ranked_ty.getShape());
...
Value ValuesToRank1(PatternRewriter &rewriter, Location loc, Type dtype,
  ...
  auto type = machina::GetTypeFromTFTensorShape({length}, dtype);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (mlir::isa<VariantType>(getElementTypeOrSelf(addn_op.getType())))
      return failure();
    toolchain::SmallVector<Value, 4> operands(addn_op.getInputs().begin(),
...
class LowerDynamicStitchOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto item_ty = machina::GetTypeFromTFTensorShape(
    ...
    for (auto it : toolchain::zip(indices, op.getData())) {
      ...
      auto items = rewriter.create<UnpackOp>(
          ...
          /*axis=*/0);
      for (auto index_item : toolchain::zip(index_attr, items.getResults())) {
...
class LowerInvertPermutationOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto shape_type =
        machina::GetTypeFromTFTensorShape({2}, rewriter.getIntegerType(32));
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    if (needs_cast) {
      ...
      if (original_tensor_type.hasRank()) {
        tensor_type = machina::GetTypeFromTFTensorShape(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto axis_value = rewriter.create<ConstOp>(
        loc, DenseElementsAttr::get(machina::GetTypeFromTFTensorShape(
...
class LowerSpaceToBatchNDOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto block_shape_i64_type = machina::GetTypeFromTFTensorShape(
    ...
    auto paddings_i64_type = machina::GetTypeFromTFTensorShape(
    ...
    auto full_paddings_type = machina::GetTypeFromTFTensorShape(
    ...
    toolchain::SmallVector<int64_t, 4> block_shape_ints;
    auto padded_shape = toolchain::to_vector<4>(input_shape);
    ...
    auto padded_type =
        machina::GetTypeFromTFTensorShape(padded_shape, element_type);
    // padded = pad(input, full_paddings)
    auto padded =
        rewriter.create<PadOp>(loc, padded_type, op.getInput(), full_paddings);

    auto paddings_sum_type = machina::GetTypeFromTFTensorShape(
    ...
    auto input_shape_tensor = rewriter.create<ConstOp>(
        loc,
        DenseElementsAttr::get(machina::GetTypeFromTFTensorShape(
                                   {input_rank}, rewriter.getIntegerType(64)),
                               input_shape));
    ...
    SmallVector<Type, 4> padded_shape_splits_types(
        input_rank,
        machina::GetTypeFromTFTensorShape({1}, rewriter.getIntegerType(64)));
    ...
    auto reshaped = rewriter.create<ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape(reshaped_shape_ints, element_type),
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    auto reshaped = rewriter.create<TF::ReshapeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape(reshaped_shape, element_ty), input,
    ...
    for (auto it : toolchain::enumerate(permutation)) {
      transpose_shape[it.index()] = reshaped_shape[it.value()];
    }

    auto permuted = rewriter.create<TF::TransposeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape(transpose_shape, element_ty),
        reshaped,
        rewriter.create<ConstOp>(op.getLoc(),
    ...
    auto block_shape_values =
        toolchain::to_vector<4>(block_shape.getValues<APInt>());
    ...
    auto reshaped_permuted = rewriter.create<TF::ReshapeOp>(
        op.getLoc(),
        machina::GetTypeFromTFTensorShape(reshaped_permuted_shape,
                                             element_ty),
        permuted,
    ...
    auto crop_values = toolchain::to_vector<4>(crops.getValues<APInt>());
    for (int i = 0; i < block_rank; ++i) {
    ...
    rewriter.replaceOpWithNewOp<TF::SliceOp>(
        op, machina::GetTypeFromTFTensorShape(slice_sizes, element_ty),
        reshaped_permuted,
        rewriter.create<ConstOp>(op.getLoc(),
...
class LowerSparseMatMulOp : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    MLIRContext *context = rewriter.getContext();
    toolchain::SmallVector<Value, 2> operands{op.getA(), op.getB()};
    for (Value &operand : operands) {
      ...
      if (tensor_type.hasRank()) {
        tensor_type_f32 = machina::GetTypeFromTFTensorShape(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *src_op,
    ...
    if (matchPattern(out_size, m_Constant(&out_size_cst))) {
      toolchain::SmallVector<int64_t, 2> cst_size;
    ...
    Value input_shape = rewriter.create<ShapeOp>(
        loc, machina::GetTypeFromTFTensorShape({4}, rewriter.getI64Type()),
        input);
    input_shape = rewriter.create<CastOp>(
        loc, machina::GetTypeFromTFTensorShape({4}, out_size_element_ty),
        input_shape);

    auto scalar_dim_ty =
        machina::GetTypeFromTFTensorShape({}, out_size_element_ty);
    ...
    auto in_count = rewriter.create<MulOp>(
        loc, machina::GetTypeFromTFTensorShape({}, out_size_element_ty),
        in_y, in_x);
    ...
    auto in_y_f32 = rewriter.create<CastOp>(
        loc, machina::GetTypeFromTFTensorShape({}, rewriter.getF32Type()),
        in_y);
    ...
    Value y_range = rewriter.create<RangeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({out_height_constant},
                                             rewriter.getF32Type()),
    ...
    y_range = rewriter.create<ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({out_height_constant, 1},
                                             out_size_element_ty),
        y_range,
        rewriter.create<PackOp>(
            loc, machina::GetTypeFromTFTensorShape({2}, out_size_element_ty),
    ...
    Value x_range = rewriter.create<RangeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({out_width_constant},
                                             rewriter.getF32Type()),
    ...
    Value x_indices = rewriter.create<ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({1, out_width_constant},
                                             out_size_element_ty),
        x_range,
        rewriter.create<PackOp>(
            loc, machina::GetTypeFromTFTensorShape({2}, out_size_element_ty),
    ...
    indices = rewriter.create<ReshapeOp>(
        loc,
        machina::GetTypeFromTFTensorShape({out_spatial_cst},
                                             out_size_element_ty),
        indices,
        rewriter.create<ReshapeOp>(
            loc, machina::GetTypeFromTFTensorShape({1}, out_size_element_ty),
            out_count,
    ...
    Value input_collapsed_spatial = rewriter.create<ReshapeOp>(
        ...
        input,
        rewriter.create<PackOp>(
            loc, machina::GetTypeFromTFTensorShape({3}, out_size_element_ty),
...
struct LowerRollOp : public RewritePattern {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto create_slice_op = [&](int32_t axis_i, int32_t begin_i, int32_t size_i,
      ...
      auto slice_op_ty = machina::GetTypeFromTFTensorShape(
    ...
    auto scalar_type =
        machina::GetTypeFromTFTensorShape({}, rewriter.getIntegerType(32));
