...
void MaybeCreateVarHandleForOp(Operation* op, DataFlowSolver& solver) {
  ...
  } else if (auto next = toolchain::dyn_cast<TF::IteratorGetNextOp>(op)) {
    resource = next.getIterator();
  }

  if (toolchain::dyn_cast_or_null<TF::VarHandleOp>(resource.getDefiningOp())) {
  ...
  if (auto global = toolchain::dyn_cast<tf_saved_model::GlobalTensorOp>(source)) {
    container = "";
    shared_name = global.getSymName();
  } else if (auto handle = toolchain::dyn_cast<TF::VarHandleOp>(source)) {
    container = handle.getContainer();
    shared_name = handle.getSharedName();
  } else if (auto it = toolchain::dyn_cast<TF::IteratorOp>(source)) {
  ...
  if (auto it = toolchain::dyn_cast<TF::IteratorOp>(source)) {
