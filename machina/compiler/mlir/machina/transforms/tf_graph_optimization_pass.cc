...
class GraphOptPass
 ...
 public:
  ...
  explicit GraphOptPass(std::vector<machina::GraphOptimizationPass*> passes)
...
void GraphOptPass::runOnOperation() {
  ...
  auto module_or_status = machina::tf2xla::v2::ConvertGraphToTfExecutor(
...
class GraphOptByNamePass : public GraphOptPass {
 public:
  ...
  toolchain::StringRef getDescription() const final {
    return "runs passes registered as machina::GraphOptimizationPass";
  }

 protected:
  mlir::Pass::ListOption<std::string> cl_pass_list_{
      *this, "graph-passes",
      toolchain::cl::desc("comma separated list of GraphOptimizationPass to run.")};
 ...
 private:
  void runOnOperation() override {
    if (!cl_pass_list_.empty()) {
      ...
      for (auto pass_it : toolchain::enumerate(passes_)) {
...
machina::CreateTensorFlowGraphOptimizationPass(
...
void machina::RegisterGraphOptimizationPasses() {
