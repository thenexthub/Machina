...
void ExpandParallelExecuteToIslands(
    ...
    toolchain::SmallVectorImpl<tf_executor::IslandOp>& executes,
  ...
  for (int i : toolchain::seq<int>(0, num_regions)) {
...
void CreateIslandsFromParallelExecute(
  ...
  toolchain::SmallVector<tf_executor::IslandOp, 4> executes;
  ...
  // islands.
  toolchain::SmallVector<Value, 8> parallel_execute_outputs;
  ...
  for (auto result :
       toolchain::zip(island_op.getOutputs(), parallel_execute_outputs))
  ...
  if (!island_op.getControl().use_empty()) {
    ...
    auto island_sink = tf_executor::IslandOp::create(
        builder, island_op.getLoc(), toolchain::ArrayRef<Type>{},
  ...
  if (legacy_graph_export) {
    ...
    // execute.
    toolchain::SmallVector<Value, 8> unused_execute_controls;
    for (auto& execute : executes)
...
void ParallelExecuteToIslandsPass::runOnOperation() {
  ...
  toolchain::SmallVector<tf_executor::IslandOp, 4> parallel_execute_op_islands;
