...
LogicalResult ReorderReplicateAndPartitionedInputs(
  ...
  if (!toolchain::all_of(replicated_input.getInputs(), [](Value input) {
        return toolchain::isa_and_nonnull<TF::TPUPartitionedInputV2Op>(
            input.getDefiningOp());
      }))
  ...
  auto first_partitioned_input = toolchain::cast<TF::TPUPartitionedInputV2Op>(
      replicated_input.getOperand(0).getDefiningOp());
  auto partition_dims = first_partitioned_input.getPartitionDims();
  const std::optional<::toolchain::StringRef> xla_sharding =
  ...
  for (const auto& operand : replicated_input.getInputs().drop_front()) {
    auto partitioned_input =
        toolchain::cast<TF::TPUPartitionedInputV2Op>(operand.getDefiningOp());
    const std::optional<::toolchain::StringRef> op_xla_sharding =
    ...
    if (!toolchain::equal(partition_dims, op_partition_dims)) {
  ...
  toolchain::SmallVector<toolchain::SmallVector<Value, 4>, 4>
  ...
  toolchain::SmallVector<Value, 4> operands_per_core;
  for (auto& operands_per_replica : operands_per_replica_per_core) {
    const bool is_packed =
        packed_input && toolchain::all_equal(operands_per_replica);
...
void TPUReorderReplicateAndPartitionedInputsPass::runOnOperation() {
  auto result =
      getOperation()->walk([](TF::TPUReplicatedInputOp replicated_input) {
        if (toolchain::none_of(replicated_input.getInputs(), [](Value input) {
              return toolchain::isa_and_nonnull<TF::TPUPartitionedInputV2Op>(
                  input.getDefiningOp());
            }))
