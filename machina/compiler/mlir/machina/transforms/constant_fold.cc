...
#include "machina/compiler/mlir/machina/transforms/constant_fold.h"
...
LogicalResult ConstantFoldFallbackHook(
  ...
  bool has_empty_numerical_results =
      toolchain::all_of(inst->getResultTypes(), [](Type ty) {
  ...
  if (has_empty_numerical_results &&
    ...
    for (Type ty : inst->getResultTypes()) {
      ...
      results.push_back(
          DenseElementsAttr::get(shaped_ty, toolchain::ArrayRef<Attribute>()));
    }
    return success();
  }
