...
GTEST_API_ int main(int real_argc, char** real_argv) {
  ...
  machina::AppendMarkForCompilationPassFlags(&flag_list);
  auto usage = machina::Flags::Usage(real_argv[0], flag_list);
  ...
  if (!machina::Flags::Parse(&argc, &args.front(), flag_list)) {
    LOG(ERROR) << "\n" << usage;
    return 2;
  }
