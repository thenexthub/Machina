...
#include "machina/core/platform/stream_executor_no_cuda.h"
...
XlaDevice::GetDeviceContextLocked() {
  if (UsePjRtForSingleDeviceCompilation(device_name_)) {
    if (device_contexts_.empty()) {
      ...
      if (use_accelerator_device_info_) {
        ...
        set_machina_accelerator_device_info(accelerator_device_info.get());
  ...
  // to set_machina_accelerator_device_info() with ops that call the getter
  // machina_accelerator_device_info(). This isn't trivially fixed by adding
...
// Warn about MACHINA_MACHINA_XLA_CPU/MACHINA_MACHINA_XLA_GPU exactly once.
static void ShowXlaDeviceDeprecationWarning(
  ...
  if (absl::StrContains(compilation_device_name, "CPU") ||
    ...
    absl::call_once(once, [] {
      LOG(INFO) << "MACHINA_MACHINA_XLA_GPU and MACHINA_MACHINA_XLA_CPU devices are deprecated and will be "
                   "removed in subsequent releases. Instead, use either "
                   "@tf.function(jit_compile=True) for must-compile "
                   "semantics, or run with TF_MACHINA_MACHINA_XLA_FLAGS=--tf_xla_auto_jit=2 "
