...
bool RecursiveCompilabilityChecker::HasXLAKernel(
  ...
  // There is a SymbolicGradient kernel on the MACHINA_MACHINA_XLA_JIT device, but the gradient
...
bool RecursiveCompilabilityChecker::OpIsSlow(const Node& node) const {
  ...
  // https://github.com/machina/machina/pull/31012:
...
machina::MemoryTypeVector GetInputMemoryTypes(
    const machina::FunctionBody* fbody,
    absl::Span<int const> constant_arg_indices,
    absl::Span<int const> resource_arg_indices) {
  // Set input and output memory types.
  machina::MemoryTypeVector input_memory_types(fbody->arg_types.size(),
                                                  machina::DEVICE_MEMORY);
  ...
  for (size_t i = 0; i < fbody->arg_types.size(); ++i) {
    if (resources_search.ScanForValue(i) || constants_search.ScanForValue(i)) {
      ...
      // host memory.
      input_memory_types[i] = machina::HOST_MEMORY;
    }
  }
  return input_memory_types;
}

machina::MemoryTypeVector GetOutputMemoryTypes(
    const machina::FunctionBody* fbody) {
  machina::MemoryTypeVector output_memory_types(fbody->ret_types.size(),
                                                   machina::DEVICE_MEMORY);
  for (size_t i = 0; i < fbody->ret_types.size(); ++i) {
    if (fbody->ret_types[i] == machina::DT_RESOURCE) {
      output_memory_types[i] = machina::HOST_MEMORY;
    }
  }
