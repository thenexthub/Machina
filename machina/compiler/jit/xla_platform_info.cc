...
absl::Status GetCompilationDeviceTypeAndPjRtClient(
  ...
  if (device_type == DEVICE_TPU) {
    *compilation_device_type = DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT);
    TF_ASSIGN_OR_RETURN(*pjrt_client, GetOrCreatePjRtClient(device_type));
    return absl::OkStatus();
  }
...
absl::Status BuildXlaDeviceCompiler(DeviceBase* device,
  ...
  if (platform_info.device_type() == DEVICE_TPU) {
    *xla_device_compiler = CreateXlaDeviceCompiler(
        persistor_config, DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT), nullptr);
    return absl::OkStatus();
  }
  ...
  if (device != nullptr) {
    client_options.set_intra_op_parallelism_threads(
        device->machina_cpu_worker_threads()->num_threads);
  }
...
XlaPlatformInfo XlaPlatformInfoFromDevice(DeviceBase* device_base) {
  ...
  if (device_type == DEVICE_CPU) {
    ...
    platform_id = device->machina_accelerator_device_info()
                      ->stream->parent()
                      ->GetPlatform()
                      ->id();
