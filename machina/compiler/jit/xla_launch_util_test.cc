...
class PjRtExecutionUtilTest : public OpsTestBase {
 public:
  PjRtExecutionUtilTest() {
    ...
    // Add and setup the MACHINA_MACHINA_XLA_CPU device.
    auto device_type = DeviceType(DEVICE_MACHINA_MACHINA_XLA_CPU);
    rollout_config.AllowForDeviceInXlaLaunch(device_type);
    rollout_config.AllowForDeviceInXlaCompileOnDemand(device_type);

    auto jit_device_type = DeviceType(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT);
    auto device =
    ...
    // Create PjRtClient for MACHINA_MACHINA_XLA_CPU.
...
TEST_F(PjRtExecutionUtilTest, PopulateCtxOutputs) {
  ...
  TF_EXPECT_OK(NodeDefBuilder("AddV2", "AddV2")
                   ...
                   .Input(FakeInput(DT_INT32))
                   .Attr("T", DT_INT32)
                   .Device("/job:localhost/replica:0/task:0/device:MACHINA_MACHINA_XLA_CPU:0")
                   .Finalize(node_def()));
  TF_EXPECT_OK(InitOp());
