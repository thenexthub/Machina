...
#include "machina/compiler/jit/deadness_analysis.h"
#include "machina/compiler/jit/defs.h"
#include "machina/compiler/jit/device_util.h"
#include "machina/compiler/jit/flags.h"
#include "machina/compiler/jit/resource_operation_safety_analysis.h"
#include "machina/compiler/jit/xla_cluster_util.h"
...
#include "machina/xla/service/graphcycles/graphcycles.h"
#include "machina/xla/union_find.h"
...
absl::Status IgnoreResourceOpForSafetyAnalysis(
  ...
  // If a resource operation is assigned to MACHINA_MACHINA_XLA_CPU or MACHINA_MACHINA_XLA_GPU explicitly then
  // ignore it during resource operation safety analysis.  We need this hack
  // because of two reasons:
  //
  //  1. Operations assigned to MACHINA_MACHINA_XLA_CPU and MACHINA_MACHINA_XLA_GPU have to always be compiled.
  ...
  // TensorArray and Stack on the MACHINA_MACHINA_XLA_{C|G}PU devices were the same in XLA; then
...
absl::StatusOr<bool> MarkForCompilationPassImpl::Initialize() {
  ...
  if (!debug_options_.ignore_deadness_checks) {
    MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER_LEVEL("DeadnessAnalysis", 1);
  ...
  for (Node* node : sorted_nodes) {
    ...
    if (compile_time_const_nodes[node->id()]) {
      ...
      if (op_def->is_stateful()) {
        ...
        // not compiled by XLA altogether or, if assigned to an MACHINA_MACHINA_XLA_* device
...
absl::Status MarkForCompilationPassImpl::Run() {
  ...
  // some one-time work.
  MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER_LEVEL("MarkForCompilationPassImpl::Run", 1);
...
If you want XLA:CPU, do one of the following:
 ...
 - set the TF_MACHINA_MACHINA_XLA_FLAGS to include "--tf_xla_cpu_global_jit", or
 - set cpu_global_jit to true on this session's OptimizerOptions, or
 - use experimental_jit_scope, or
 - use tf.function(jit_compile=True).

To confirm that XLA is active, pass --vmodule=xla_compilation_cache=1 (as a
proper command-line flag, not via TF_MACHINA_MACHINA_XLA_FLAGS).)";
