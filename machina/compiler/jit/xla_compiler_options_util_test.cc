...
#include "machina/compiler/jit/device_executable_persistor.h"
...
GetShapeDeterminationFns() {
  ...
  XlaShapeLayoutHelpers::LayoutPreferenceFn layout_preference_fn =
      [](const TensorShape&, DataType, std::optional<XlaArgument::Kind>) {
        return machina::XlaLayoutPreference::kTpuPreferLinearLayout;
      };
...
TEST_F(XlaCompilerOptionsTest, PjRtOptionsXlaDevice) {
  device_setup_.AddDevicesAndSetUp({DEVICE_MACHINA_MACHINA_XLA_GPU});
  Device* device = device_setup_.GetDevice(DEVICE_MACHINA_MACHINA_XLA_GPU);
  DeviceType compilation_device_type = DeviceType(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT);
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto shape, options.shape_determination_fns.shape_representation_fn(
                      ...
                      machina::XlaLayoutPreference::kTpuPreferLinearLayout));
...
TEST_F(XlaCompilerOptionsTest, PjRtOptionsPjRtBaseDevice) {
  ...
  DeviceType compilation_device_type = DeviceType(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT);
...
TEST_F(XlaCompilerOptionsTest, PjRtOptionsNonXlaDevice) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto shape, options.shape_determination_fns.shape_representation_fn(
                      TensorShape(), DT_FLOAT, false,
                      machina::XlaLayoutPreference::kNoPreference));
...
TEST_F(XlaCompilerOptionsTest, XlaOptions) {
  ...
  DeviceType device_type = DeviceType(DEVICE_MACHINA_MACHINA_XLA_GPU);
  DeviceType compilation_device_type = DeviceType(DEVICE_GPU_MACHINA_MACHINA_XLA_JIT);
...
TEST_F(XlaCompilerOptionsTest, TfRtTpuOptions) {
  ...
  xla::LocalClient* client = xla::ClientLibrary::LocalClientOrDie();
  DeviceType compilation_device_type = DeviceType(DEVICE_TPU_MACHINA_MACHINA_XLA_JIT);
