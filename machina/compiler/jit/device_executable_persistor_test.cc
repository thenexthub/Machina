...
#include "machina/compiler/jit/xla_compilation_cache.pb.h"
 ...
 protected:
  ...
  absl::Status CreatePjRtCompilerClient() {
    ...
    TF_RETURN_IF_ERROR(SetPjRtClientInTFGlobalResourceManager(
        DEVICE_CPU_MACHINA_MACHINA_XLA_JIT, xla::GetXlaPjrtCpuClient(options).value()));
    TF_ASSIGN_OR_RETURN(auto pjrt_client,
                        GetOrCreatePjRtClient(DeviceType(DEVICE_CPU_MACHINA_MACHINA_XLA_JIT)));
