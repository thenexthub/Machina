...
#include "machina/cc/ops/sendrecv_ops.h"
#include "machina/cc/ops/standard_ops.h"
#include "machina/compiler/jit/defs.h"
#include "machina/compiler/jit/mark_for_compilation_pass_test_helper.h"
#include "machina/compiler/jit/node_matchers.h"
...
using ::machina::testing::FindNodeByName;
...
TEST(XlaCompilationTest, CallXlaDeviceFuncWithResourceOp) {
  ...
  *flib.add_function() = compilable;
  ...
  string xla_cpu_device = "/job:worker/replica:0/task:0/device:MACHINA_MACHINA_XLA_CPU:0";
...
TEST(XlaCompilationTest, RandomShapeOnXlaDevice) {
  absl::string_view xla_gpu_device =
      "/job:worker/replica:0/task:0/device:MACHINA_MACHINA_XLA_GPU:0";
...
TEST(XlaCompilationTest, DontClusterMergingNodes) {
  ...
  absl::string_view xla_gpu_dev1 =
      "/job:worker/replica:0/task:0/device:MACHINA_MACHINA_XLA_GPU:1";
  std::unique_ptr<Graph> graph(new Graph(OpRegistry::Global()));
...
TEST(XlaCompilationTest, ClusterStatefulRandomOpOnXlaDevice) {
  absl::string_view xla_cpu_device =
      "/job:worker/replica:0/task:0/device:MACHINA_MACHINA_XLA_CPU:0";
...
const char* kMACHINA_MACHINA_XLA_GPU0 = "/job:worker/replica:0/task:0/device:MACHINA_MACHINA_XLA_GPU:0";
...
TEST(XlaCompilationTest, DontCreateCombinedCpuUnknownClusters) {
  ...
  FindNodeByName(graph.get(), "test/y")->set_assigned_device_name(kMACHINA_MACHINA_XLA_GPU0);
...
TEST(XlaCompilationTest, XLALiteAllowlist) {
  auto* allowlist_table = machina::GetAllowlistTable();
  ...
  absl::flat_hash_set<string> known_not_in_list =
      machina::testing::GetKnownXLAAllowlistOp();
