...
#include "machina/compiler/jit/xla_tpu_device.h"

#include "absl/types/optional.h"
#include "machina/compiler/jit/kernels/xla_ops.h"
#include "machina/compiler/jit/xla_device.h"
#include "machina/compiler/jit/xla_device_context.h"
...
#include "machina/xla/stream_executor/tpu/tpu_node_context.h"
#include "machina/xla/stream_executor/tpu/tpu_platform.h"
#include "machina/xla/stream_executor/tpu/tpu_platform_interface.h"
#include "machina/xla/stream_executor/tpu/tpu_stream_interface.h"
...
absl::StatusOr<xla::Shape> TpuShapeRepresentation(
  ...
  xla::Shape xla_shape;
  TF_RETURN_IF_ERROR(
      machina::TensorShapeToXLAShape(type, shape, &xla_shape));
  ApiConverter::StackHelper<MACHINA_MACHINA_XLA_Shape> se_shape(xla_shape);
  ApiConverter::StackHelper<MACHINA_MACHINA_XLA_Shape> tpu_shape;
  StatusHelper status;
...
absl::Status TpuPaddedShapeFn(const Tensor& tensor, xla::Shape* shape) {
  const machina::XlaTensor* xla_tensor =
      machina::XlaTensor::FromTensor(&tensor);
  ...
  StatusHelper status;
  ApiConverter::StackHelper<MACHINA_MACHINA_XLA_Shape> se_shape(on_device_shape);
  ApiConverter::StackHelper<MACHINA_MACHINA_XLA_Shape> tpu_shape;
...
absl::Status TpuNodeDeviceFactory::CreateDevices(
  ...
  registration.compilation_device_name = DEVICE_TPU_MACHINA_MACHINA_XLA_JIT;
  ...
  static XlaDeviceOpRegistrations* registrations =
      RegisterXlaDeviceKernels(DEVICE_TPU_NODE, DEVICE_TPU_MACHINA_MACHINA_XLA_JIT);
  (void)registrations;
  ...
  for (int i = 0; i < device_count; ++i) {
    ...
    options.compilation_device_name = DEVICE_TPU_MACHINA_MACHINA_XLA_JIT;
    ...
    if (!status.ok()) {
      errors::AppendToMessage(&status, "while setting up ", DEVICE_TPU_MACHINA_MACHINA_XLA_JIT,
                              " device number ", i);
      return status;
    }
...
void RegisterTpuNodeDevice(
  ...
  REGISTER_MACHINA_MACHINA_XLA_LAUNCH_KERNEL(DEVICE_TPU_NODE, XlaLocalLaunchOp, kTpuAllTypes);
  REGISTER_MACHINA_MACHINA_XLA_COMPILE_KERNEL(DEVICE_TPU_NODE, XlaCompileOp, kTpuAllTypes);
  REGISTER_MACHINA_MACHINA_XLA_RUN_KERNEL(DEVICE_TPU_NODE, XlaRunOp, kTpuAllTypes);
  REGISTER_MACHINA_MACHINA_XLA_DEVICE_KERNELS(DEVICE_TPU_NODE, kTpuAllTypes);
