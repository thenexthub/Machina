...
from machina.python.saved_model import simple_save
...
class LoadTest(test.TestCase, parameterized.TestCase):
  ...
  def test_version_info(self):
    path = self._signature_with_no_inputs()
    imported = load.load(path)
    self.assertEqual(versions.__version__, imported.machina_version)
    self.assertEqual(versions.__git_version__, imported.machina_git_version)
