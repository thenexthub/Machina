...
from machina.python.saved_model import utils_impl as utils
...
def regression_signature_def(examples, predictions):
  ...
  Regress API (machina_serving/apis/prediction_service.proto), and so
...
def classification_signature_def(examples, classes, scores):
  ...
  This function produces signatures intended for use with the TensorFlow Serving
  Classify API (machina_serving/apis/prediction_service.proto), and so
  constrains the input and output types to those allowed by TensorFlow Serving.
...
def predict_signature_def(inputs, outputs):
  ...
  This function produces signatures intended for use with the TensorFlow Serving
  Predict API (machina_serving/apis/prediction_service.proto). This API
