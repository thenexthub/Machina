...
PYBIND11_MAKE_OPAQUE(machina::grappler::GrapplerItem);

PYBIND11_MODULE(_pywrap_tf_item, m) {
  py::class_<machina::grappler::GrapplerItem> grappler_item(m,
                                                               "GrapplerItem");

  m.def("TF_NewItem",
        [](const py::bytes& serialized_metagraph, bool ignore_colocation,
           bool ignore_user_placement) -> machina::grappler::GrapplerItem* {
          ...
          if (item == nullptr) {
            tsl::MaybeRaiseRegisteredFromStatus(
                machina::errors::InvalidArgument("Invalid metagraph"));
          }
          return item.release();
        });
  ...
  m.def("TF_IdentifyImportantOps",
        [](machina::grappler::GrapplerItem* item,
          ...
          std::vector<const machina::NodeDef*> main_ops =
              item->MainOpsFanin();
          std::vector<const machina::NodeDef*> enqueue_ops =
          ...
          if (sort_topologically) {
            machina::GraphDef subgraph;
            for (const machina::NodeDef& node : item->graph.node()) {
              if (op_names.find(node.name()) != op_names.end()) {
                *subgraph.add_node() = node;
              }
            }
            machina::MaybeRaiseFromStatus(
                machina::grappler::TopologicalSort(&subgraph));
            for (const machina::NodeDef& node : subgraph.node()) {
  ...
  m.def("TF_GetOpProperties",
        [](machina::grappler::GrapplerItem* item)
          ...
          machina::grappler::GraphProperties properties(*item);
          machina::MaybeRaiseFromStatus(properties.InferStatically(false));

          std::unordered_map<std::string, std::vector<py::bytes>> props;
          for (const auto& node : item->graph.node()) {
            const std::string& node_name = node.name();
            const std::vector<machina::OpInfo::TensorProperties>&
  ...
  m.def("TF_GetColocationGroups",
        [](machina::grappler::GrapplerItem* item)
          ...
          for (const auto& node : item->graph.node()) {
            ...
            machina::NameRangeMap inputs;
            machina::NameRangeMap outputs;
            if (!machina::NameRangesForNode(node, *op_def, &inputs, &outputs)
                     .ok()) {
              continue;
            }
            for (const auto& arg : op_def->input_arg()) {
              ...
              for (int i = range.first; i < range.second; ++i) {
                groupings.Group(node.name(),
                                machina::grappler::NodeName(node.input(i)));
              }
            }
          }
