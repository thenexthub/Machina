...
void DetectDevices(
  ...
  if (!machina::DeviceFactory::AddDevices(options, "", &devices).ok()) {
    return;
  }

  for (const std::unique_ptr<machina::Device>& device : devices) {
    machina::DeviceProperties& prop = (*device_map)[device->name()];
    prop = machina::grappler::GetDeviceInfo(device->parsed_name());

    // Overwrite the memory limit since users might have requested to use only a
    // fraction of the available device memory.
    const machina::DeviceAttributes& attr = device->attributes();
    prop.set_memory_size(attr.memory_limit());
  }
}

namespace {
machina::GraphDef TF_OptimizeGraph(
    machina::grappler::Cluster* cluster,
    const std::string& serialized_config_proto,
    const machina::MetaGraphDef& metagraph, bool verbose,
  ...
  machina::GraphDef out_graph;
  {
    ...
    std::unique_ptr<machina::grappler::GrapplerItem> grappler_item =
        machina::grappler::GrapplerItemFromMetaGraphDef(graph_id, metagraph,
                                                           item_config);
    if (!grappler_item) {
      throw std::invalid_argument(
    ...
    machina::DeviceBase* cpu_device = nullptr;
    machina::grappler::MetaOptimizer optimizer(cpu_device, config_proto);

    tsl::MaybeRaiseRegisteredFromStatusWithGIL(
        optimizer.Optimize(cluster, *grappler_item, &out_graph));
    if (strip_default_attributes) {
      machina::StripDefaultAttributes(*machina::OpRegistry::Global(),
  ...
  m.def("TF_OptimizeGraphSerialized",
        [](machina::grappler::Cluster* cluster,
...

          {
            ...
            const machina::GraphDef out_graph =
