...
struct ConverterTraits {
  static const machina::DataType kTypeEnum;
...
struct Converter {
  ...
  static absl::Status Convert(TFE_Context* ctx, PyObject* obj,
    ...
    *h = machina::wrap(machina::unwrap(ctx)->CreateLocalHandle(t));
    t->Release();
    return absl::OkStatus();
  }
};
...
struct ConverterTraits<int64_t> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx,
                                               int64_t value) {
    return machina::unwrap(ctx)->CreateInt64Scalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_INT64, dim_sizes);
  }
...
struct ConverterTraits<uint64> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx, uint64 value) {
    return machina::unwrap(ctx)->CreateUint64Scalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_UINT64, dim_sizes);
  }
...
struct ConverterTraits<int32> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx,
                                               int32_t value) {
    return machina::unwrap(ctx)->CreateInt32Scalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_INT32, dim_sizes);
  }
...
struct ConverterTraits<float> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx, float value) {
    return machina::unwrap(ctx)->CreateFloatScalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_FLOAT, dim_sizes);
  }
...
struct ConverterTraits<double> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx, double value) {
    return machina::unwrap(ctx)->CreateDoubleScalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_DOUBLE, dim_sizes);
  }
...
struct ConverterTraits<Eigen::half> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx,
                                               Eigen::half value) {
    return machina::unwrap(ctx)->CreateHalfScalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_HALF, dim_sizes);
  }
...
struct ConverterTraits<tstring> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx,
                                               tstring value) {
    return machina::unwrap(ctx)->CreateStringScalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_STRING, dim_sizes);
  }
...
bool IsPyDimension(PyObject* obj) {
  ...
  bool ret =
      absl::EndsWith(PyRepr(PyType(obj)),
                     "machina.python.framework.tensor_shape.Dimension'>");
  return ret;
}
...
struct ConverterTraits<complex128> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx,
                                               complex128 value) {
    return machina::unwrap(ctx)->CreateComplex128Scalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_COMPLEX128, dim_sizes);
  }
...
struct ConverterTraits<bool> {
  static AbstractTensorInterface* CreateScalar(TFE_Context* ctx, bool value) {
    return machina::unwrap(ctx)->CreateBoolScalar(value);
  }

  static AbstractTensorInterface* CreateTensor(
      TFE_Context* ctx, absl::Span<const int64_t> dim_sizes) {
    return machina::unwrap(ctx)->CreateTensor(DT_BOOL, dim_sizes);
  }
...
TFE_TensorHandle* NumpyToTFE_TensorHandle(TFE_Context* ctx, PyObject* obj) {
  ...
  absl::Status status = machina::NdarrayToTensor(ctx, obj, &tf_tensor);
  ...
  return machina::wrap(
      machina::unwrap(ctx)->CreateLocalHandle(tf_tensor->tensor));
}
...
TFE_TensorHandle* PySeqToTFE_TensorHandle(TFE_Context* ctx, PyObject* obj,
  ...
  if (PyArray_Check(obj)) {
    ...
    if (dtype != machina::DT_INVALID) {
      if (!machina::TF_DataType_to_PyArray_TYPE(
               static_cast<TF_DataType>(dtype), &desired_np_dtype)
               .ok()) {
        PyErr_SetString(
            PyExc_TypeError,
            machina::strings::StrCat("Invalid dtype argument value ", dtype)
                .c_str());
        return nullptr;
      }
    }
  ...
  switch (state.inferred_dtype) {
    ...
    {
      AbstractTensorInterface* t = machina::unwrap(ctx)->CreateTensor(
          requested_dtype == DT_INVALID ? DT_FLOAT : requested_dtype,
          state.inferred_shape);
      auto* result =
          machina::wrap(machina::unwrap(ctx)->CreateLocalHandle(t));
      t->Release();
      return result;
    }
