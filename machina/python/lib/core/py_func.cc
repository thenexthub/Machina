...
#include "machina/python/lib/core/py_func.h"
...
bool IsCPUDevice(const Device* d) {
  return d == nullptr || d->machina_accelerator_device_info() == nullptr;
}
...
absl::Status MakeArgTuple(const PyCall* call, TFE_Context* ctx,
  ...
  for (int64_t i = 0; i < n; ++i) {
    ...
    if (call->eager) {
      Tensor t = call->ins[i];
      arg = EagerTensorFromHandle(machina::wrap(
          machina::unwrap(ctx)->CreateLocalHandleFromTFTensor(t,
                                                                 device_name)));
      if (arg == nullptr) {
        Py_DECREF(lst);
...
absl::Status ExtractTensorFromEagerTensor(const PyObject* eager_tensor,
  ...
  machina::TensorHandle* handle = down_cast<machina::TensorHandle*>(
      machina::unwrap(ctx)->TFTensorHandleFromInterface(
          machina::unwrap(EagerTensor_Handle(eager_tensor))));
...
absl::Status DoCallPyFunc(PyCall* call, bool* out_log_on_error) {
  *out_log_on_error = true;
  ...
  if (call->eager) {
    ...
    old_executor = &(machina::unwrap(ctx)->Executor());
    machina::unwrap(ctx)->SetExecutorForThread(new_executor.get());
  } else {
  ...
  if (new_executor != nullptr) {
    ...
    machina::unwrap(ctx)->SetExecutorForThread(old_executor);
  }
