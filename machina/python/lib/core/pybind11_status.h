...
#ifndef MACHINA_PYTHON_LIB_CORE_PYBIND11_STATUS_H_
#define MACHINA_PYTHON_LIB_CORE_PYBIND11_STATUS_H_
...
inline void SetRegisteredErrFromStatus(const absl::Status& status) {
  PyErr_SetObject(
      machina::PyExceptionRegistry::Lookup(status.raw_code()),
...
inline void SetRegisteredErrFromTFStatus(TF_Status* status) {
  PyErr_SetObject(machina::PyExceptionRegistry::Lookup(TF_GetCode(status)),
...
struct type_caster<machina::StatusOr<PayloadType>> {
 public:
  ...
  static handle cast(const machina::StatusOr<PayloadType>* src,
  ...
  static handle cast(machina::StatusOr<PayloadType>&& src,
                     return_value_policy policy, handle parent) {
...
#endif  // MACHINA_PYTHON_LIB_CORE_PYBIND11_STATUS_H_
