...
#include "machina/xla/tsl/python/lib/core/ml_dtypes.h"
...
absl::Status PyBytesArrayMap(PyArrayObject* array, F f) {
  Safe_PyObjectPtr iter = machina::make_safe(
      PyArray_IterNew(reinterpret_cast<PyObject*>(array)));
  while (PyArray_ITER_NOTDONE(iter.get())) {
    auto item = machina::make_safe(PyArray_GETITEM(
...
absl::Status EncodePyBytesArray(PyArrayObject* array, int64_t nelems,
  ...
  // Encode all strings.
  *size = nelems * sizeof(machina::tstring);
  std::unique_ptr<machina::tstring[]> base_ptr(
      new machina::tstring[nelems]);
  machina::tstring* dst = base_ptr.get();
...
absl::Status TF_TensorToPyArray(Safe_TF_TensorPtr tensor,
  ...
  Safe_PyObjectPtr safe_out_array =
      machina::make_safe(PyArray_Empty(dims.size(), dims.data(), descr, 0));
  if (!safe_out_array) {
...
absl::Status NdarrayToTensor(TFE_Context* ctx, PyObject* ndarray,
  ...
  if (dtype == TF_RESOURCE) {
    ...
    if (ctx) {
      *ret = make_safe(new TF_Tensor{machina::unwrap(ctx)->CreateTensor(
          static_cast<machina::DataType>(dtype), {}, 0, PyArray_DATA(array),
    ...
    if (ctx) {
      *ret = make_safe(new TF_Tensor{machina::unwrap(ctx)->CreateTensor(
          static_cast<machina::DataType>(dtype), dims.data(), dims.size(),
    ...
    if (ctx) {
      *ret = make_safe(new TF_Tensor{machina::unwrap(ctx)->CreateTensor(
          ...
          [](void* data, size_t len, void* arg) {
            delete[] reinterpret_cast<machina::tstring*>(data);
          },
          nullptr)});
    } else {
