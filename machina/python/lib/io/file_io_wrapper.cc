...
#include "machina/core/lib/io/buffered_inputstream.h"
#include "machina/core/lib/io/random_inputstream.h"
...
PYBIND11_MODULE(_pywrap_file_io, m) {
  using machina::PyTransactionToken;
  using machina::TransactionToken;
  ...
  m.def(
      ...
      [](const std::string& filename, PyTransactionToken* token) {
        absl::Status status;
        {
          ...
          status = machina::Env::Default()->FileExists(filename);
        }
        machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
      },
  ...
  m.def(
      ...
      [](const std::string& filename, PyTransactionToken* token) {
        py::gil_scoped_release release;
        absl::Status status = machina::Env::Default()->DeleteFile(filename);
        machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
      },
      py::arg("filename"), py::arg("token") = (PyTransactionToken*)nullptr);
  m.def(
      ...
      [](const std::string& filename, PyTransactionToken* token) {
        ...
        py::gil_scoped_release release;
        const auto status =
            ReadFileToString(machina::Env::Default(), filename, &data);
        pybind11::gil_scoped_acquire acquire;
        machina::MaybeRaiseRegisteredFromStatus(status);
  ...
  m.def(
      ...
      [](const std::string& filename, absl::string_view data,
        ...
        py::gil_scoped_release release;
        const auto status =
            WriteStringToFile(machina::Env::Default(), filename, data);
        machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
      },
  ...
  m.def(
      ...
      [](const std::string& dirname, PyTransactionToken* token) {
        ...
        py::gil_scoped_release release;
        const auto status =
            machina::Env::Default()->GetChildren(dirname, &results);
        pybind11::gil_scoped_acquire acquire;
        machina::MaybeRaiseRegisteredFromStatus(status);
        return results;
      },
  ...
  m.def(
      ...
      [](const std::string& pattern, PyTransactionToken* token) {
        ...
        py::gil_scoped_release release;
        const auto status =
            machina::Env::Default()->GetMatchingPaths(pattern, &results);
        pybind11::gil_scoped_acquire acquire;
        machina::MaybeRaiseRegisteredFromStatus(status);
        return results;
      },
  ...
  m.def(
      ...
      [](const std::string& dirname, PyTransactionToken* token) {
        py::gil_scoped_release release;
        const auto status = machina::Env::Default()->CreateDir(dirname);
  ...
  m.def(
      ...
      [](const std::string& dirname, PyTransactionToken* token) {
        py::gil_scoped_release release;
        const auto status =
            machina::Env::Default()->RecursivelyCreateDir(dirname);
        machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
      },
      py::arg("dirname"), py::arg("token") = (PyTransactionToken*)nullptr);
  m.def(
      ...
      [](const std::string& src, const std::string& target, bool overwrite,
        ...
        if (!overwrite && env->FileExists(target).ok()) {
          status = machina::errors::AlreadyExists("file already exists");
        } else {
  ...
  m.def(
      ...
      [](const std::string& dirname, PyTransactionToken* token) {
        ...
        auto status = machina::Env::Default()->DeleteRecursively(
            dirname, &undeleted_files, &undeleted_dirs);
        if (status.ok() && (undeleted_files > 0 || undeleted_dirs > 0)) {
          status = machina::errors::PermissionDenied(
  ...
  m.def(
      ...
      [](const std::string& dirname, PyTransactionToken* token) {
        py::gil_scoped_release release;
        const auto status = machina::Env::Default()->IsDirectory(dirname);
  ...
  m.def("HasAtomicMove", [](const std::string& path) {
    ...
    const auto status =
        machina::Env::Default()->HasAtomicMove(path, &has_atomic_move);
    machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
    return has_atomic_move;
  });

  py::class_<machina::FileStatistics>(m, "FileStatistics")
      .def_readonly("length", &machina::FileStatistics::length)
      .def_readonly("mtime_nsec", &machina::FileStatistics::mtime_nsec)
      .def_readonly("is_directory", &machina::FileStatistics::is_directory);

  m.def(
      "Stat",
      [](const std::string& filename, PyTransactionToken* token) {
        py::gil_scoped_release release;
        std::unique_ptr<machina::FileStatistics> self(
            new machina::FileStatistics);
        const auto status =
            machina::Env::Default()->Stat(filename, self.get());
  ...
  m.def("GetRegisteredSchemes", []() {
    ...
    py::gil_scoped_release release;
    const auto status =
        machina::Env::Default()->GetRegisteredFileSystemSchemes(&results);
    pybind11::gil_scoped_acquire acquire;
    machina::MaybeRaiseRegisteredFromStatus(status);
    return results;
  });

  using machina::WritableFile;
  py::class_<WritableFile>(m, "WritableFile")
      ...
      .def("flush",
           [](WritableFile* self) {
             py::gil_scoped_release release;
             machina::MaybeRaiseRegisteredFromStatusWithGIL(self->Flush());
           })
      .def("close", [](WritableFile* self) {
        py::gil_scoped_release release;
        machina::MaybeRaiseRegisteredFromStatusWithGIL(self->Close());
      });

  using machina::io::BufferedInputStream;
  py::class_<BufferedInputStream>(m, "BufferedInputStream")
      .def(py::init([](const std::string& filename, size_t buffer_size,
                       PyTransactionToken* token) {
             py::gil_scoped_release release;
             std::unique_ptr<machina::RandomAccessFile> file;
             const auto status =
                 machina::Env::Default()->NewRandomAccessFile(filename,
                                                                 &file);
             machina::MaybeRaiseRegisteredFromStatusWithGIL(status);
             std::unique_ptr<machina::io::RandomAccessInputStream>
                 input_stream(new machina::io::RandomAccessInputStream(
                     file.release(),
                     /*owns_file=*/true));
             py::gil_scoped_acquire acquire;
      ...
      .def("seek",
           [](BufferedInputStream* self, int64_t pos) {
             py::gil_scoped_release release;
             machina::MaybeRaiseRegisteredFromStatusWithGIL(self->Seek(pos));
           })
