...
#include "machina/core/lib/io/zlib_compression_options.h"
...
class PyRecordReader {
 public:
  ...
  absl::Status ReadNextRecord(machina::tstring* out) {
    if (IsClosed()) {
      return machina::errors::FailedPrecondition("Reader is closed.");
    }
  ...
  absl::Status Reopen() {
    if (!IsClosed()) {
      return machina::errors::FailedPrecondition("Reader is not closed.");
    }
    TF_RETURN_IF_ERROR(
        machina::Env::Default()->NewRandomAccessFile(filename_, &file_));
    reader_ =
        std::make_unique<machina::io::RecordReader>(file_.get(), options_);
    return absl::OkStatus();
  }

 private:
  static constexpr machina::uint64 kReaderBufferSize = 16 * 1024 * 1024;
  ...
  static machina::io::RecordReaderOptions CreateOptions(
      const std::string& compression_type) {
    auto options =
        machina::io::RecordReaderOptions::CreateRecordReaderOptions(
            compression_type);
  ...
  const machina::io::RecordReaderOptions options_;
  machina::uint64 offset_;
  std::unique_ptr<machina::RandomAccessFile> file_;
  std::unique_ptr<machina::io::RecordReader> reader_;
...
class PyRecordRandomReader {
 public:
  static absl::Status New(const std::string& filename,
    ...
    TF_RETURN_IF_ERROR(
        machina::Env::Default()->NewRandomAccessFile(filename, &file));
    auto options =
        machina::io::RecordReaderOptions::CreateRecordReaderOptions("");
    options.buffer_size = kReaderBufferSize;
    auto reader =
        std::make_unique<machina::io::RecordReader>(file.get(), options);
  ...
  absl::Status ReadRecord(machina::uint64* offset,
                          machina::tstring* out) {
    if (IsClosed()) {
 ...
 private:
  ...
  PyRecordRandomReader(std::unique_ptr<machina::RandomAccessFile> file,
                       std::unique_ptr<machina::io::RecordReader> reader)
...
class PyRecordWriter {
 public:
  static absl::Status New(const std::string& filename,
                          const machina::io::RecordWriterOptions& options,
                          PyRecordWriter** out) {
    std::unique_ptr<machina::WritableFile> file;
    TF_RETURN_IF_ERROR(
        machina::Env::Default()->NewWritableFile(filename, &file));
    auto writer =
        std::make_unique<machina::io::RecordWriter>(file.get(), options);
  ...
  absl::Status WriteRecord(absl::string_view record) {
    if (IsClosed()) {
      return machina::errors::FailedPrecondition("Writer is closed.");
    }
 ...
 private:
  PyRecordWriter(std::unique_ptr<machina::WritableFile> file,
                 std::unique_ptr<machina::io::RecordWriter> writer)
      : file_(std::move(file)), writer_(std::move(writer)) {}

  std::unique_ptr<machina::WritableFile> file_;
  std::unique_ptr<machina::io::RecordWriter> writer_;
...
PYBIND11_MODULE(_pywrap_record_io, m) {
  ...
  py::class_<PyRecordRandomReader>(m, "RandomRecordReader")
      ...
      .def("read",
           [](PyRecordRandomReader* self, machina::uint64 offset) {
             machina::uint64 temp_offset = offset;
             machina::tstring record;
             absl::Status status;
             {
               py::gil_scoped_release release;
             ...
             if (absl::IsOutOfRange(status)) {
               throw py::index_error(machina::strings::StrCat(
  ...
  using machina::io::ZlibCompressionOptions;
  ...
  using machina::io::RecordWriterOptions;
