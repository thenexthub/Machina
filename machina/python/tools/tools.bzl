...
load("//machina:machina.bzl", "clean_dep", "if_xla_available")
load("//machina:machina.default.bzl", "tfcompile_target_cpu")
load("//machina/compiler/aot:tfcompile.bzl", "target_llvm_triple")
...
def saved_model_compile_aot(
    ...
    **NOTE** Any variables passed to `variables_to_feed` *must be set by the
    ...
    saved_model_compile_aot(
      ...
      cpp_class = "machina::CompiledModel",
    ...
    #include "third_party/machina/python/tools/aot_compiled_x_plus_y.h"

    TEST(Test, Run) {
      machina::CompiledModel model;
      CHECK(model.Run());
    }
    ```
    ...
    native.genrule(
        ...
        cmd = (
            "$(location {}) aot_compile_cpu ".format(
                clean_dep("//machina/python/tools:saved_model_cli"),
            ) +
        ...
        tags = tags,
        tools = [
            "//machina/python/tools:saved_model_cli",
        ],
    )
    cc_library(
        name = name,
        ...
        deps = _maybe_force_compile(
            [
                "//machina/compiler/tf2xla:xla_compiled_cpu_runtime_standalone",
            ],
            force_compile = force_without_xla_support_flag,
        ),
    )
