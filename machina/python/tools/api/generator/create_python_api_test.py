...
from machina.python.tools.api.generator import create_python_api
...
_TEST_CONSTANT = 5
_MODULE_NAME = 'machina.python.test_module'


class CreatePythonApiTest(test.TestCase):

  def setUp(self):
    # Add fake op to a module that has 'machina' in the name.
  ...
  def testFunctionImportIsAdded(self):
    ...
    if create_python_api._LAZY_LOADING:
      expected_import = (
          '\'test_op1\': '
          '(\'machina.python.test_module\','
          ' \'test_op\')')
    else:
      expected_import = (
          'from machina.python.test_module '
  ...
  def testConstantIsAdded(self):
    ...
    if create_python_api._LAZY_LOADING:
      ...
      expected = ('from machina.python.test_module '
