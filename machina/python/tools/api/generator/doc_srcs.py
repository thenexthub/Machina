...
_MACHINA_DOC_SOURCES = {
    ...
    'summary': DocSource(
        docstring=(
            ...
            ' TensorBoard](https://www.machina.org/guide/summaries_and_tensorboard)'
            ' guide.'
        )
    ),
    'sysconfig':
    ...
    'train': DocSource(
        docstring=(
            ...
            ' [Training](https://machina.org/api_guides/python/train) guide.'
        )
    ),
}
...
def get_doc_sources(api_name):
  ...
  Args:
    api_name: API you want to generate (e.g. `machina` or `estimator`).

  Returns:
    Map from module name to DocSource object.
  """
  if api_name == tf_export.MACHINA_API_NAME:
    return _MACHINA_DOC_SOURCES
