...
from machina.python.tools.api.generator2.generator import generator
  ...
  def test_get_module(self):
    self.assertEqual(
        ...
        generator.get_module(
            'bazel/machina/keras/losses/', 'bazel/machina'
        ),
    )

  def test_generate_proxy_api_files(self):
    tmp_dir = self.create_tempdir()
    proxy_file = os.path.join(tmp_dir, 'machina/keras/losses/__init__.py')
    generator.generate_proxy_api_files(
        [proxy_file], 'keras', os.path.join(tmp_dir, 'machina/keras')
    )
...
# Generated by: machina/python/tools/api/generator2/generator/generator.py script.
