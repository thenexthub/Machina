...
try:
  from machina.python import _pywrap_tfcompile  # pylint: disable=g-import-not-at-top
except ImportError as e:
  _pywrap_tfcompile_import_error = ImportError(
      'Unable to import _pywrap_tfcompile; you must build TensorFlow '
      'with XLA.  You may need to build machina with flag '
...
_MACHINA_MACHINA_XLA_MAKEFILE_TEMPLATE = """
INC = -I{machina_includes}
LIB = -L{compiled_dir}
CXXFLAGS = {cxx_flags}
"""
...
def _xla_makefile_string(output_prefix):
  ...
  Returns:
    A string containing a filled out `_MACHINA_MACHINA_XLA_MAKEFILE_TEMPLATE`.
  """
  sysconfig = _sysconfig_module()
  output_dir, _ = os.path.split(output_prefix)
  if sysconfig:
    machina_includes = _shlex_quote(sysconfig.get_include())
  else:
    ...
    expected_header = os.path.join(
        base, 'machina', 'compiler', 'tf2xla', 'xla_compiled_cpu_function.h')
    if not os.path.exists(expected_header):
      logging.error(
          'Could not find includes path.  Missing file: {}'
          .format(expected_header))
    machina_includes = base

  return _MACHINA_MACHINA_XLA_MAKEFILE_TEMPLATE.format(
      machina_includes=machina_includes,
...
def freeze_model(checkpoint_path: str,
  ...
  Raises:
    RuntimeError: If machina was not built with XLA.
    ImportError: If machina was built with XLA but there was another
...
def aot_compile_cpu_meta_graph_def(checkpoint_path,
  ...
  if _pywrap_tfcompile_import_error:
    ...
    xla_flags = os.environ.get('MACHINA_MACHINA_XLA_FLAGS')
    if not xla_flags:
      ...
      xla_flags += ' --xla_cpu_multi_thread_eigen={}'.format(
          'true' if multithreading else 'false')
    os.environ['MACHINA_MACHINA_XLA_FLAGS'] = xla_flags
...
def _signature_to_tf2xla_config(signature_def, variable_nodes_to_feed):
  ...
  """
  from machina.compiler.tf2xla import tf2xla_pb2  # pylint: disable=g-import-not-at-top
