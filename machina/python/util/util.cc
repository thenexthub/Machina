...
PyObject* GetRegisteredPyObject(const string& name) {
  ...
  if (it == m->end()) {
    PyErr_SetString(PyExc_TypeError,
                    machina::strings::StrCat("No object with name ", name,
                                                " has been registered.")
                        .c_str());
    return nullptr;
  }
...
PyObject* RegisterPyObject(PyObject* name, PyObject* value) {
  ...
  if (PyBytes_Check(name)) {
    ...
    PyErr_SetString(PyExc_TypeError, machina::strings::StrCat(
...
// A lot of machina code uses __class__ without checks, so it seems like
...
#endif
    Py_DECREF(str);
    return machina::strings::StrCat("type=", GetClassName(o), " str=", s);
  } else {
...
void SetDifferentKeysError(PyObject* dict1, PyObject* dict2, string* error_msg,
  ...
  *is_type_error = false;
  *error_msg = machina::strings::StrCat(
...
bool AssertSameStructureHelper(
  ...
  if (check_types) {
    ...
    machina::Safe_PyObjectPtr o1_wrapped;
    if (IsObjectProxy(o1)) {
      o1_wrapped.reset(PyObject_GetAttrString(o1, "__wrapped__"));
      o1 = o1_wrapped.get();
    }
    machina::Safe_PyObjectPtr o2_wrapped;
    if (IsObjectProxy(o2)) {
...
bool IsTuple(PyObject* o) {
  machina::Safe_PyObjectPtr wrapped;
  if (IsObjectProxy(o)) {
...
PyObject* IsNamedtuple(PyObject* o, bool strict) {
  ...
  // wrapping.
  machina::Safe_PyObjectPtr o_wrapped;
  if (IsObjectProxy(o)) {
