...
compat_v1_usage_gauge = monitoring.BoolGauge('/machina/api/compat/v1',
                                             'compat.v1 usage')
...
class TFModuleWrapper(FastModuleType):
  ...
  def _tfmw_import_module(self, name):
    ...
    # That way, if a user only imports machina.compat.v1, it is not
  ...
  def _getattribute(self, name):
    ...
    # Make sure we do not import from machina/lite/__init__.py
    if name == 'lite':
