...
#ifndef MACHINA_PYTHON_UTIL_UTIL_H_
#define MACHINA_PYTHON_UTIL_UTIL_H_
...
// Implements `machina.util.nest.is_nested`.
bool IsNested(PyObject* o);

// Implements `machina.util.nest.is_nested_or_composite`.
...
// Implements the same interface as machina.util.nest.is_namedtuple
...
// Implements the same interface as machina.util.nest.same_namedtuples
...
// Implements `machina.util.nest.assert_same_structrure`.
PyObject* AssertSameStructure(PyObject* o1, PyObject* o2, bool check_types,
                              bool expand_composites);

// Implements `machina.util.nest.flatten`.
PyObject* Flatten(PyObject* nested, bool expand_composites = false);

// The machina.python.data package has its own nest utility that follows very
// slightly different semantics for its functions than the machina.python
...
// `machina.python.data.util` and in the comments for Flatten above.
...
#endif  // MACHINA_PYTHON_UTIL_UTIL_H_
