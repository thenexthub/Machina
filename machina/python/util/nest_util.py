...
from machina.python.util.compat import collections_abc as _collections_abc
from machina.python.util.custom_nest_protocol import CustomNestProtocol
...
class Modality(enum.Enum):
  ...
  - Modality.CORE follows machina_core/tf.nest semantics.
...
def is_nested(modality, structure):
  ...
  [tf.nest](https://www.machina.org/api_docs/python/tf/nest)
...
def _tf_data_yield_value(iterable):
  ...
  # depends on machina/python/util/nest.py transitively, and if we try to
...
def flatten(modality, structure, expand_composites=False):
  ...
  https://www.machina.org/api_docs/python/tf/RaggedTensor.
...
def _tf_core_flatten(structure, expand_composites=False):
  """See comments for flatten() in machina/python/util/nest.py."""
...
def _tf_core_map_structure_with_tuple_paths_up_to(
  ...
  """See comments for map_structure_with_tuple_paths_up_to() in machina/python/util/nest.py."""
  if not inputs:
