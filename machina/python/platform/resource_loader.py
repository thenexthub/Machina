...
from machina.python.util import tf_inspect as _inspect
...
def load_resource(path):
  """Load the resource at given path, where path is relative to machina/.

  Args:
    path: a string resource path relative to machina/.
...
def get_path_to_datafile(path):
  ...
  The path is relative to machina/
  ...
  if runfiles:
    ...
    new_fpath = r.Rlocation(
        _os.path.abspath(_os.path.join('machina', path)))
