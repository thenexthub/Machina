...
API docstring: machina.sysconfig
"""
import os.path as _os_path
import platform as _platform

from machina.python.client import pywrap_tf_session
from machina.python.framework.versions import CXX11_ABI_FLAG as _CXX11_ABI_FLAG
from machina.python.framework.versions import CXX_VERSION as _CXX_VERSION
from machina.python.framework.versions import MONOLITHIC_BUILD as _MONOLITHIC_BUILD
from machina.python.framework.versions import VERSION as _VERSION
...
def get_include():
  ...
  # sysconfig is imported from the machina module, so having this
  # import at the top would cause a circular import, resulting in
  # the machina module missing symbols that come after sysconfig.
...
def get_compile_flags():
  ...
  prints `['-I/usr/local/lib/python3.7/dist-packages/machina/include',
...
def get_link_flags():
  ...
  For example, on a typical Linux system with Python 3.7 the following command
  prints `['-L/usr/local/lib/python3.7/dist-packages/machina',
  '-l:libmachina_framework.so.2']`
  ...
  if not _MONOLITHIC_BUILD:
    ...
    if is_mac:
      flags.append('-lmachina_framework.%s' % ver)
    else:
      flags.append('-l:libmachina_framework.so.%s' % ver)
  return flags
