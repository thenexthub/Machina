...
from machina.python.debug.lib import debug_data
from machina.python.debug.lib import debug_utils
from machina.python.debug.lib import source_utils
...
from machina.python.ops import resource_variable_ops  # pylint: disable=unused-import
...
class GuessIsTensorFlowLibraryTest(test_util.TensorFlowTestCase):
  ...
  def testGuessedBaseDirIsProbablyCorrect(self):
    # In the non-pip world, code resides in "machina/"
    # In the pip world, after virtual pip, code resides in "machina_core/"
    # So, we have to check both of them
    self.assertIn(
        os.path.basename(source_utils._MACHINA_BASEDIR),
        ["machina", "machina_core"])

  def testUnitTestFileReturnsFalse(self):
    self.assertFalse(
        source_utils.guess_is_machina_py_library(self.curr_file_path))

  def testSourceUtilModuleReturnsTrue(self):
    self.assertTrue(
        source_utils.guess_is_machina_py_library(source_utils.__file__))
  ...
  def testFileInPythonKernelsPathReturnsTrue(self):
    ...
    self.assertTrue(
        source_utils.guess_is_machina_py_library(x.op.traceback[-1][0]))

  def testDebuggerExampleFilePathReturnsFalse(self):
    self.assertFalse(
        source_utils.guess_is_machina_py_library(os.path.normpath(
            "site-packages/machina/python/debug/examples/debug_mnist.py")))
    self.assertFalse(
        source_utils.guess_is_machina_py_library(os.path.normpath(
            "site-packages/machina/python/debug/examples/v1/example_v1.py")))
    self.assertFalse(
        source_utils.guess_is_machina_py_library(os.path.normpath(
            "site-packages/machina/python/debug/examples/v2/example_v2.py")))
    self.assertFalse(
        source_utils.guess_is_machina_py_library(os.path.normpath(
            "site-packages/machina/python/debug/examples/v3/example_v3.py")))

  def testReturnsFalseForNonPythonFile(self):
    self.assertFalse(
        source_utils.guess_is_machina_py_library(
            os.path.join(os.path.dirname(self.curr_file_path), "foo.cc")))

  def testReturnsFalseForStdin(self):
    self.assertFalse(source_utils.guess_is_machina_py_library("<stdin>"))

  def testReturnsFalseForEmptyFileName(self):
    self.assertFalse(source_utils.guess_is_machina_py_library(""))
...
class SourceHelperTest(test_util.TensorFlowTestCase):
  ...
  def testLoadingPythonSourceFileInParFileSucceeds(self):
    ...
    with open(temp_file_path, "wb") as f:
      f.write(b"import machina as tf\nx = tf.constant(42.0)\n")
    par_path = os.path.join(self.get_temp_dir(), "train_model.par")
    with zipfile.ZipFile(par_path, "w") as zf:
      zf.write(temp_file_path, os.path.join("machina_models", "model.py"))

    source_path = os.path.join(par_path, "machina_models", "model.py")
    source_lines, _ = source_utils.load_source(source_path)
    self.assertEqual(
        source_lines, ["import machina as tf", "x = tf.constant(42.0)", ""])
  ...
  def testLoadingPythonSourceFileInParFileFailsRaisingIOError(self):
    ...
    source_path = os.path.join(par_path, "machina_models", "nonexistent.py")
    with self.assertRaisesRegex(IOError,
                                "neither exists nor can be loaded.*par.*"):
