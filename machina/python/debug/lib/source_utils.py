...
from machina.python.debug.lib import profiling


_MACHINA_BASEDIR = os.path.dirname(
...
def guess_is_machina_py_library(py_file_path):
  """Guess whether a Python source file is a part of the machina library.
  ...
  Returns:
    (`bool`) Whether the file is inferred to be a part of the machina
      library.
  """
  ...
  return ((py_file_path.startswith(_MACHINA_BASEDIR) or
           py_file_path.startswith(_ABSL_BASEDIR)) and
          not py_file_path.endswith("_test.py") and
          (os.path.normpath("machina/python/debug/examples") not in
...
def _try_load_par_source(source_file_path):
  ...
  Args:
    source_file_path: The full path to the file inside the .par file. This
      ...
      intra-par path, e.g.,
      "/tmp/my_executable.par/org-machina/machina/python/foo/bar.py".
...
def list_source_files_against_dump(dump,
  ...
  for file_path in path_to_node_names:
    output.append((
        file_path,
        guess_is_machina_py_library(file_path),
