...
using machina::dtensor::AddMesh;
using machina::dtensor::AllocateDTensorDevice;
using machina::dtensor::ClearTPUCoreIDs;
using machina::dtensor::ExperimentalClearDefaultLayout;
using machina::dtensor::ExperimentalClearDefaultMesh;
using machina::dtensor::ExperimentalSetDefaultLayout;
using machina::dtensor::ExperimentalSetDefaultMesh;
using machina::dtensor::FetchLayout;
using machina::dtensor::GetStats;
using machina::dtensor::IsDTensor;
using machina::dtensor::IsSparseDTensor;
using machina::dtensor::Layout;
using machina::dtensor::Mesh;
using machina::dtensor::Pack;
using machina::dtensor::SetIteratorElementLayouts;
using machina::dtensor::SetTPUCoreIDs;
using machina::dtensor::SparsePack;
using machina::dtensor::TPUCoreIDsToLocations;
using machina::dtensor::TPUCoreLocationsToIDs;
using machina::dtensor::Unpack;
...
void ConvertToTensor(TFE_Context* ctx, PyObject* input,
  ...
  TFE_TensorHandle* handle =
      machina::ConvertToEagerTensor(ctx, input, machina::DT_INVALID);
  if (handle == nullptr) {
...
PYBIND11_MODULE(_pywrap_dtensor_device, m) {
  ...
  m.def("Pack", [](const py::handle& context, const py::handle& input_tensors,
    ...
    std::vector<machina::Safe_PyObjectPtr> py_eager_tensor_handles;
    ...
    for (Py_ssize_t i = 0; i < len; ++i) {
      ...
      if (machina::MaybeRaiseExceptionFromTFStatus(status.get(), nullptr))
        return machina::PyoOrThrow(nullptr);
    }
    ...
    machina::Safe_PyObjectPtr flat_result(PyList_New(1));
  ...
  py::class_<Mesh>(m, "Mesh")
      ...
      .def(py::init([](const machina::dtensor::MeshProto& proto) {
  ...
  py::class_<Layout>(m, "Layout")
      ...
      .def(py::init([](const machina::dtensor::LayoutProto& proto) {
