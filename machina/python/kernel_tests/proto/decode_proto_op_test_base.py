...
from machina.python.kernel_tests.proto import proto_op_test_base as test_base
from machina.python.kernel_tests.proto import test_example_pb2
...
class DecodeProtoOpTestBase(test_base.ProtoOpTestBase, parameterized.TestCase):
  ...
  def _compareProtos(self, batch_shape, sizes, fields, field_dict):
    ...
    for field in fields:
      ...
      if tf_field_name is None:
        self.fail('Unhandled machina type %d' % field.dtype)
  ...
  def testBinary(self, case):
    ...
    self._runDecodeProtoTests(
        ...
        list(case.shapes),
        batch,
        'machina.contrib.proto.TestValue',
        'binary',
        sanitize=False)
  ...
  def testPacked(self, case):
    ...
    self._runDecodeProtoTests(
        ...
        list(case.shapes),
        packed_batch,
        'machina.contrib.proto.PackedTestValue',
        'binary',
        sanitize=False)
  ...
  def testCorruptProtobuf(self, sanitize):
    ...
    msg_type = 'machina.contrib.proto.TestCase'
    field_names = ['sizes']
  ...
  def testUnexpectedType(self):
    with self.assertRaisesRegex(
      ...
      msg_type = 'machina.contrib.proto.TestValue'
      ...
      for num_fields_to_parse in range(len(all_fields_to_parse)):
        for comb in itertools.combinations(
          ...
          parsed_values = self.evaluate(
              self._decode_module.decode_proto(
                  [proto],
                  message_type='machina.contrib.proto.TestValue',
                  field_names=comb,
