...
from machina.python.kernel_tests.random import util as random_test_util
...
from machina.python.ops import stateful_random_ops as random
...
class StatefulRandomOpsTest(test.TestCase, parameterized.TestCase):
  ...
  def testOpSeedSelectionAfterSetSeed(self):
    ...
    https://github.com/machina/machina/issues/9171
    """
    shape = (3,)
  ...
  def testGetGlobalGeneratorWithXla(self):
    ...
    if not config.list_physical_devices("MACHINA_MACHINA_XLA_CPU"):
      self.skipTest("No MACHINA_MACHINA_XLA_CPU device available.")
    ...
    with ops.device("/device:MACHINA_MACHINA_XLA_CPU:0"):
