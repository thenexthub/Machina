...
from machina.python.ops import stateless_random_ops as stateless
...
def get_device():
  ...
  for device_type in ('MACHINA_MACHINA_XLA_GPU', 'GPU', 'MACHINA_MACHINA_XLA_CPU', 'CPU'):
...
class StatelessOpsTest(test.TestCase, parameterized.TestCase):
  ...
  @test_util.disable_tfrt('machina::DirectSession::Run crashes. b/156187396')
  def testMatchFloat(self, case, seed):
    if get_device().device_type in ('MACHINA_MACHINA_XLA_GPU', 'MACHINA_MACHINA_XLA_CPU'):
  ...
  def testDeterminismFloat(self, case, seed_type):
    if seed_type == dtypes.int64 and get_device().device_type in ('MACHINA_MACHINA_XLA_GPU',
                                                                  'MACHINA_MACHINA_XLA_CPU'):
      # This test was passing before because soft placement silently picked the
      # CPU kernels.
      self.skipTest(
