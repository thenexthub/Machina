...
from machina.python.data.experimental.ops import lookup_ops as data_lookup_ops
from machina.python.data.ops import readers
from machina.python.eager import context
from machina.python.feature_column import feature_column as fc_old
from machina.python.feature_column import feature_column_v2_types as fc_types
from machina.python.feature_column import serialization
from machina.python.feature_column import utils as fc_utils
...
from machina.python.training import checkpoint_utils
...
_FEATURE_COLUMN_DEPRECATION_WARNING = """\
    ...
    layers](https://www.machina.org/guide/migrate/migrating_feature_columns)
    or through the one-stop utility [`tf.keras.utils.FeatureSpace`](https://www.machina.org/api_docs/python/tf/keras/utils/FeatureSpace)
    built on top of them. See the [migration guide](https://machina.org/guide/migrate)
    for details.
    """
...
# TODO(ptucker): Move to third_party/machina/python/ops/sparse_ops.py
...
class BucketizedColumn(
  ...
  def get_config(self):
    """See 'FeatureColumn` base class."""
    from machina.python.feature_column.serialization import serialize_feature_column  # pylint: disable=g-import-not-at-top
    config = dict(zip(self._fields, self))
  ...
  def from_config(cls, config, custom_objects=None, columns_by_name=None):
    """See 'FeatureColumn` base class."""
    from machina.python.feature_column.serialization import deserialize_feature_column  # pylint: disable=g-import-not-at-top
    _check_config_keys(config, cls._fields)
    kwargs = _standardize_and_copy_config(config)
