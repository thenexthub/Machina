...
# =============================================================================
"""A Python wrapper that loads _pywrap_machina_internal.so."""
...
from machina.python.platform import self_check
...
try:
  ...
  from machina.python import pywrap_dlopen_global_flags
  _use_dlopen_global_flags = True
except ImportError:
  _use_dlopen_global_flags = False

# On UNIX-based platforms, pywrap_machina is a python library that
# dynamically loads _pywrap_machina.so.
_can_set_rtld_local = (
...
try:
  ...
  try:
    from machina.python._pywrap_machina_internal import *
  ...
  # caught internally or if someone changes the name of the target _pywrap_machina_internal.
  ...
  raise ImportError(
      ...
      f'See https://www.machina.org/install/errors '
      f'for some common causes and solutions.\n'
      f'If you need help, create an issue '
      f'at https://github.com/machina/machina/issues '
