...
PIP_PACKAGE="machina/python/tpu/profiler/pip_package"
RUNFILES="bazel-bin/machina/python/tpu/profiler/pip_package/build_pip_package.runfiles/org_machina/machina/python/tpu/profiler"
...
function main() {
  ...
  find  ${TMPDIR}/${PACKAGE_NAME} -name \*.py |
    xargs $sedi -e '
      s/^from machina.python.tpu.profiler/from '${PACKAGE_NAME}'/
  '
  echo $(ls $TMPDIR)
