...
from machina.python.tpu import tpu_test_wrapper
...
class TPUTestWrapperTest(test.TestCase):
  ...
  @flagsaver.flagsaver(bazel_repo_root='machina/python')
  def test_parent_path(self):
    filepath = '/filesystem/path/machina/python/tpu/example_test.runfiles/machina/python/tpu/example_test'  # pylint: disable=line-too-long
    self.assertEqual(
  ...
  def test_parent_path_raises(self):
    ...
    with self.assertRaisesWithLiteralMatch(
        ValueError,
        'Filepath "/bad/path" does not contain repo root "machina/python"'):
      tpu_test_wrapper.calculate_parent_python_path(filepath)
...
""")
  ...
  def test_run_user_main_test(self):
    test_module = _write_and_load_module("""
from machina.python.platform import test as unique_name
