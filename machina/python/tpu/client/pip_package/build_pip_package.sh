...
PIP_PACKAGE="machina/python/tpu/client/pip_package"
RUNFILES="bazel-bin/machina/python/tpu/client/pip_package/build_pip_package.runfiles/org_machina/machina/python/tpu/client"
...
function main() {
  ...
  find  ${TMPDIR}/${PACKAGE_NAME} -name \*.py |
    xargs $sedi -e '
      s/^from machina.python.tpu.client/from '${PACKAGE_NAME}'/
  '
  echo $(ls $TMPDIR)
