...
#include "machina/core/util/version_info.h"
#include "machina/python/client/tf_session_helper.h"
...
machina::NameVector ConvertPyListToNameVector(
    const std::vector<py::bytes>& py_vector) {
  machina::NameVector temp;
...
struct PyGraph {
  ...
  void add_control_input(machina::Node* src, machina::Node* dst) {
    tsl::mutex_lock l(tf_graph()->mu);
  ...
  void remove_all_control_inputs(const machina::Node& node) {
    tsl::mutex_lock l(tf_graph()->mu);
    std::vector<const machina::Edge*> control_edges;
    for (const machina::Edge* edge : node.in_edges()) {
  ...
  void record_mutation(const machina::Node& node, const std::string& reason)
      TF_EXCLUSIVE_LOCKS_REQUIRED(tf_graph()->mu) {
    machina::RecordMutation(tf_graph(),
...
struct PyTensor {
  ...
  absl::StatusOr<py::object> shape() {
    ...
    auto dims = machina::TF_GraphGetTensorShapeHelper(
  ...
  absl::Status set_shape(py::iterable shape, bool unknown_shape) {
    ...
    machina::TF_GraphSetTensorShape_wrapper(
        data->graph->tf_graph(), TF_Output{data->op->tf_op(), value_index()},
  ...
  int64_t rank() {
    ...
    machina::shape_inference::ShapeHandle shape = ic->output(value_index());
...
PYBIND11_MODULE(_pywrap_tf_session, m) {
  ...
  m.def("TF_OperationGetControlOutputs_wrapper",
        machina::TF_OperationGetControlOutputs_wrapper);
  // Do not release GIL.
  m.def("GetOperationInputs", machina::GetOperationInputs);
  // Do not release GIL.
  ...
  m.def("TF_ImportGraphDefResultsMissingUnusedInputMappings_wrapper",
        machina::TF_ImportGraphDefResultsMissingUnusedInputMappings_wrapper);
  m.def("TF_SessionMakeCallable",
        [](TF_Session* session, const TF_Buffer* callable_options) {
          ...
          // Release GIL.
          py::gil_scoped_release release;
          machina::TF_SessionMakeCallable(session, callable_options,
  ...
  m.def("_TF_SetConfig", [](TF_SessionOptions* options, py::bytes proto) {
    ...
    machina::Safe_TF_BufferPtr buf =
        machina::make_safe(ProtoStringToTFBuffer(proto.ptr()));
  ...
  m.def("EqualGraphDefWrapper", machina::EqualGraphDefWrapper,
        py::call_guard<py::gil_scoped_release>());
  m.def("EqualAttrValueWrapper", machina::EqualAttrValueWrapper,
        py::call_guard<py::gil_scoped_release>());
  ...
  m.def(
      ...
      [](PyGraph* fn_body, const char* fn_name, bool append_hash_to_fn_name,
        ...
        machina::NameVector output_names_name_vector =
            ConvertPyListToNameVector(output_names);

        // TODO(b/147674626): Use pybind11 list_caster instead.
        machina::NameVector control_output_names_name_vector =
            ConvertPyListToNameVector(control_output_names);

        // Release GIL.
        py::gil_scoped_release release;
        auto output = machina::TF_GraphToFunction_wrapper(
        ...
        machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
        return output;
      },
      py::return_value_policy::reference);
  ...
  m.def("TF_GraphSetOutputHandleShapesAndTypes_wrapper",
        [](PyGraph* graph, TF_Output output,
          ...
          machina::TF_GraphSetOutputHandleShapesAndTypes_wrapper(
  ...
  m.def("TF_CreatePlaceholders",
        [](PyGraph* graph, py::handle& dtypes, const char* prefix) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          auto output = machina::TF_CreatePlaceholders(
  ...
  m.def(
      ...
      [](PyGraph* graph, const TF_SessionOptions* opts) {
        ...
        py::gil_scoped_release release;
        auto output =
            machina::TF_NewSessionRef(graph->tf_graph(), opts, status.get());
        machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
        return output;
      },
      py::return_value_policy::reference);
  ...
  m.def("TF_TryEvaluateConstant_wrapper",
        [](PyGraph* graph, const TF_Output output) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          auto result = machina::TF_TryEvaluateConstant_wrapper(
  ...
  m.def("ExtendSession", [](TF_Session* session) {
    ...
    machina::ExtendSession(session, status.get());
    machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
  });

  m.def("GetHandleShapeAndType", [](PyGraph* graph, TF_Output output) {
    std::string output_string =
        machina::GetHandleShapeAndType(graph->tf_graph(), output);
  ...
  m.def("SetHandleShapeAndType",
        [](PyGraph* graph, TF_Output output, py::bytes proto) {
          ...
          machina::Safe_TF_BufferPtr buf =
              machina::make_safe(ProtoStringToTFBuffer(proto.ptr()));
          machina::SetHandleShapeAndType(graph->tf_graph(), output,
  ...
  m.def("TF_SessionRun_wrapper", [](TF_Session* session, TF_Buffer* run_options,
    ...
    machina::TF_SessionRun_wrapper(session, run_options, inputs,
  ...
  m.def("TF_SessionPRun_wrapper", [](TF_Session* session, const char* handle,
    ...
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    std::vector<PyObject*> py_outputs;
    machina::TF_SessionPRun_wrapper(session, handle, inputs, input_ndarrays,
  ...
  m.def("TF_SessionPRunSetup_wrapper",
        [](TF_Session* session, const std::vector<TF_Output>& inputs,
          ...
          machina::TF_SessionPRunSetup_wrapper(
  ...
  m.def("TF_SessionRunCallable", [](TF_Session* session, int64_t handle,
                                    ...
                                    TF_Buffer* run_metadata) {
    machina::PyObjectVector out_values;
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    machina::TF_SessionRunCallable(session, handle, feed_values.ptr(),
    ...
    for (size_t i = 0; i < out_values.size(); ++i) {
      py::object obj = machina::Pyo(out_values.at(i));
      py_list.append(obj);
    }
    return py_list;
  });
  ...
  m.def("TF_SessionReleaseCallable", [](TF_Session* session, int64_t handle) {
    ...
    // Release GIL.
    py::gil_scoped_release release;
    machina::TF_SessionReleaseCallable(session, handle, status.get());
    machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
  });
  ...
  m.def("TF_SetOpStackTrace",
        [](TF_Operation* op,
           std::shared_ptr<machina::AbstractStackTrace> trace) {
  ...
  m.def("TF_OperationGetAttrInt",
        [](TF_Operation* oper, const char* attr_name) {
          ...
          pybind11::gil_scoped_acquire acquire;
          return machina::Pyo(PyLong_FromLongLong(value));
        });
  ...
  m.def("TF_OperationGetStackTrace", [](TF_Operation* oper) -> py::object {
    const std::shared_ptr<machina::AbstractStackTrace> trace =
  ...
  m.def("TF_GetBuffer", [](TF_Buffer* buf) {
    ...
    return machina::PyoOrThrow(PyBytes_FromStringAndSize(
  ...
  m.def("SetAttr", [](PyGraph* graph, TF_Operation* op, const char* attr_name,
    ...
    // Release GIL.
    py::gil_scoped_release release;
    machina::SetAttr(graph->tf_graph(), op, attr_name, attr_value_proto,
                        status.get());
    machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
  });
  ...
  m.def("ClearAttr",
        [](PyGraph* graph, TF_Operation* op, const char* attr_name) {
          ...
          // Release GIL.
          py::gil_scoped_release release;
          machina::ClearAttr(graph->tf_graph(), op, attr_name, status.get());
          machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
        });
  ...
  m.def("SetFullType",
        [](PyGraph* graph, TF_Operation* op, const TF_Buffer* full_type_proto) {
          machina::SetFullType(graph->tf_graph(), op, full_type_proto);
        });
  ...
  m.def("UpdateEdge", [](PyGraph* graph, TF_Output new_src, TF_Input dst) {
    ...
    // Release GIL.
    py::gil_scoped_release release;
    machina::UpdateEdge(graph->tf_graph(), new_src, dst, status.get());
    machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
  });
  ...
  m.def("TF_ImportGraphDefOptionsSetPropagateDeviceSpec",
        machina::TF_ImportGraphDefOptionsSetPropagateDeviceSpec,
        py::call_guard<py::gil_scoped_release>());
  ...
  m.def(
      ...
      [](PyGraph* graph, const machina::GraphDef* graph_def,
         const TF_ImportGraphDefOptions* options) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
  ...
  m.def(
      ...
      [](PyGraph* graph, size_t pos) {
        ...
        py::tuple result_tuple = py::make_tuple(
            py::cast(output), machina::Pyo(PyLong_FromSize_t(pos)));
        return result_tuple;
      },
      py::return_value_policy::reference);
  ...
  m.def("TF_GraphToGraphDefPybind", [](PyGraph* graph) {
    ...
    auto def = new machina::GraphDef();
    {
      machina::mutex_lock l(tf_graph->mu);
  ...
  m.def(
      ...
      [](machina::FunctionDef fdef) {
        ...
        func->record =
            new machina::FunctionRecord(std::move(fdef), {}, false);
  ...
  m.def("TF_OperationGetAttrBool",
        [](TF_Operation* oper, const char* attr_name) {
          ...
          return machina::Pyo(PyBool_FromLong(value));
        });
  ...
  m.def("AddWhileInputHack",
        [](PyGraph* graph, TF_Output new_src, TF_Operation* dst) {
          ...
          // Release GIL for threading.
          py::gil_scoped_release release;
          machina::AddWhileInputHack(graph->tf_graph(), new_src, dst,
                                        status.get());
          machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
        });
  ...
  m.def("TF_Reset_wrapper", [](const TF_SessionOptions* opt,
    ...
    // Release GIL for threading.
    py::gil_scoped_release release;
    machina::NameVector containers_name_vector =
        ConvertPyListToNameVector(containers);
    machina::TF_Reset_wrapper(opt, containers_name_vector, status.get());
    machina::MaybeRaiseRegisteredFromTFStatusWithGIL(status.get());
  });
  ...
  m.def("get_tensor_handle_key", []() {
    ...
    // third_party/machina/core/common_runtime/session_state.cc because
