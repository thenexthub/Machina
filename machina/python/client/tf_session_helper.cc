...
#include "machina/python/client/session_ref.h"
...
namespace {
void MakeCallableHelper(machina::Session* session,
                        const TF_Buffer* callable_options, int64_t* out_handle,
                        TF_Status* out_status) {
  machina::CallableOptions callable_options_proto;
  ...
  machina::Session::CallableHandle handle;
...
namespace {
void RunCallableHelper(machina::Session* session, int64_t handle,
                       PyObject* feed_values, TF_Status* out_status,
                       PyObjectVector* out_values, TF_Buffer* run_metadata) {
  // Convert feed values to a vector of machina::Tensor objects.
