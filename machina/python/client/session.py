...
from machina.python.framework import device
...
from machina.python.ops import session_ops
from machina.python.platform import tf_logging as logging
from machina.python.training.experimental import mixed_precision_global_state
...
_python_session_create_counter = monitoring.Counter(
    '/machina/api/python/session_create_counter',
...
class Session(BaseSession):
  ...
  The
  [`ConfigProto`](https://www.machina.org/code/machina/core/protobuf/config.proto)
  ...
  def __init__(self, target='', graph=None, config=None):
    ...
    Args:
      target: (Optional.) The execution engine to connect to. Defaults to using
        ...
        [Distributed TensorFlow](https://machina.org/deploy/distributed) for
          more examples.
