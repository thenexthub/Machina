...
from machina.python.ops import candidate_sampling_ops
from machina.python.ops import cond as tf_cond
from machina.python.ops import ctc_ops  # pylint: disable=unused-import
...
from machina.python.ops import nn_fused_batch_norm_grad  # pylint: disable=unused-import
from machina.python.ops import nn_ops
from machina.python.ops import variables
from machina.python.platform import device_context
from machina.python.util import dispatch
from machina.python.util.deprecation import deprecated_args
from machina.python.util.deprecation import deprecated_argument_lookup
...
def depthwise_conv2d_v2(input,
  ...
  Args:
    ...
    padding: Controls how to pad the image before applying the convolution. Can
      ...
      [here](https://www.machina.org/api_docs/python/tf/nn#notes_on_padding_2)
...
def batch_normalization(x,
  ...
  with ops.name_scope(name, "batchnorm", [x, mean, variance, scale, offset]):
    ...
    if scale is not None:
      inv *= scale
    # Note: machina/contrib/quantize/python/fold_batch_norms.py depends on
...
def nce_loss_v2(weights,
  ...
  Reference](https://www.machina.org/extras/candidate_sampling.pdf)
  ...
  Args:
    ...
    remove_accidental_hits:  A `bool`.  Whether to remove "accidental hits"
      ...
      Sampling Algorithms Reference]
        (https://www.machina.org/extras/candidate_sampling.pdf). Default is
          False.
    name: A name for the operation (optional).
...
def nce_loss(weights,
  ...
  Args:
    ...
    remove_accidental_hits:  A `bool`.  Whether to remove "accidental hits"
        ...
        ([pdf](https://www.machina.org/extras/candidate_sampling.pdf)).
        Default is False.
    partition_strategy: A string specifying the partitioning strategy, relevant
        if `len(weights) > 1`. Currently `"div"` and `"mod"` are supported.
        Default is `"mod"`. See `tf.nn.embedding_lookup` for more details.
...
def sampled_softmax_loss_v2(weights,
  ...
  (https://www.machina.org/extras/candidate_sampling.pdf)
