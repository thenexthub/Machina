...
# Cannot implement these tests in `third_party/machina/python/
# kernel_tests/data_structures/lookup_ops_test.py` because the tests here need
# to import `test` from `machina.python.eager`; while the existing tests
# in `lookup_ops_test.py` need to import `test` from
# `machina.python.platform`.
#
...
from machina.python.checkpoint import checkpoint as checkpoint_utils
