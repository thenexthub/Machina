...
from machina.python.lib.core import _pywrap_py_func
from machina.python.ops import autograph_ops  # pylint: disable=unused-import
from machina.python.ops import gen_script_ops
...
def eager_py_func(func=None, inp=None, Tout=None, name=None):
  ...
  1. This allows you to use nofunc=None, inp=None, Tout=None machina code
  inside your `tf.function`.
  2. It allows you to run python control logic in a `tf.function` without
  relying on `tf.autograph` to convert the code to use machina control logic
  ...
  * you must not set `func` or `inp`
  ...
  [Eager guide](https://machina.org/guide/eager).
...
def py_func_common(func, inp, Tout, stateful=True, name=None):
  ...
  * The operation must run in the same address space as the Python program
  ...
  Args:
    ...
    Tout: A list or tuple of machina data types or a single machina data
