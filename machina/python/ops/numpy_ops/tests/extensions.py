...
from machina.python.ops import gen_bitwise_ops
from machina.python.ops import gen_collective_ops
...
import machina.python.ops.numpy_ops.tests.np_wrapper as tf_np
from machina.python.ops.parallel_for import control_flow_ops as pfor_ops
...
def grad(f, has_aux=False):
  ...
  Gradients can only be computed through numpy and machina operations and not
...
def jit(f,
  ...
  Args:
    ...
    input_signature: a list of `tf.TensorSpec`, as the input signature to
      ...
      [doc](https://www.machina.org/api_docs/python/tf/function]) of
    ...
    experimental_compile: the `experimental_compile` flag for `tf.function`. See
      the [doc](https://www.machina.org/api_docs/python/tf/function]) of
...
def conv(inp,
  ...
  See https://www.machina.org/api_docs/python/tf/nn/convolution and
  https://www.machina.org/xla/operation_semantics#conv_convolution for
  reference.
