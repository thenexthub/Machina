...
from machina.python.framework import type_spec as type_spec_module
from machina.python.keras.utils import object_identity
from machina.python.ops import array_ops
from machina.python.ops.ragged import ragged_operators  # pylint: disable=unused-import
...
# (See `MaxDimensions()` in //machina/core/framework/tensor_shape.h )
  ...
  def from_tensor(cls, tensor):
    ...
    if isinstance(tensor, tensor_lib.Tensor):
      ...
      if (type_spec.dtype == dtypes.int32 and type_spec.shape.rank is not None
        ...
        # we attempt to capture any value information machina's
