...
#include "machina/c/eager/tape.h"
...
#include "machina/core/lib/gtl/compactptrset.h"
...
#include "machina/python/eager/pywrap_gradient_exclusions.h"
#include "machina/python/eager/pywrap_tensor.h"
#include "machina/python/eager/pywrap_tfe.h"
#include "machina/python/lib/core/py_util.h"
#include "machina/python/lib/core/safe_pyobject_ptr.h"
#include "machina/python/util/stack_trace.h"
...
thread_local machina::TF_StatusPtr thread_local_tf_status(  // NOLINT
    nullptr, machina::internal::TF_StatusDeleter());        // NOLINT
...
TFE_Op* GetOp(TFE_Context* ctx, const char* op_or_function_name,
  ...
  if (!op) {
    op.reset(machina::wrap(machina::unwrap(ctx)->CreateOperation()));
  }
  tsl::Set_TF_Status_from_Status(
      status, machina::unwrap(op.get())->Reset(op_or_function_name,
                                                  raw_device_name));
...
using AttrToInputsMap =
    machina::gtl::FlatMap<string, absl::InlinedVector<InputInfo, 4UL>>;

machina::gtl::FlatMap<string, AttrToInputsMap*>* GetAllAttrToInputsMaps() {
  static auto* all_attr_to_input_maps =
      new machina::gtl::FlatMap<string, AttrToInputsMap*>;
  return all_attr_to_input_maps;
}

// This function doesn't use a lock, since we depend on the GIL directly.
AttrToInputsMap* GetAttrToInputsMapHoldingGIL(const machina::OpDef& op_def) {
  ...
  auto* output =
      machina::gtl::FindPtrOrNull(*all_attr_to_input_maps, op_def.name());
  if (output != nullptr) {
    return output;
  }
...
// This function doesn't use a lock, since we depend on the GIL directly.
machina::gtl::FlatMap<
    string, machina::gtl::FlatMap<string, machina::DataType>*>*
GetAllAttrToDefaultsMaps() {
  static auto* all_attr_to_defaults_maps = new machina::gtl::FlatMap<
      string, machina::gtl::FlatMap<string, machina::DataType>*>;
  return all_attr_to_defaults_maps;
}

machina::gtl::FlatMap<string, machina::DataType>*
GetAttrToDefaultsMapHoldingGIL(const machina::OpDef& op_def) {
#if PY_MAJOR_VERSION >= 3 && PY_MINOR_VERSION >= 4
  DCHECK(PyGILState_Check())
      << "This function needs to hold the GIL when called.";
#endif
  ...
  auto* output =
      machina::gtl::FindPtrOrNull(*all_attr_to_defaults_maps, op_def.name());
  if (output != nullptr) {
    return output;
  }

  auto* new_map = new machina::gtl::FlatMap<string, machina::DataType>;
...
struct FastPathOpExecInfo {
  ...
  const machina::gtl::FlatMap<string, machina::DataType>* default_dtypes;
  machina::gtl::FlatMap<string, machina::DataType> cached_dtypes;
};
...
bool ParseDimensionValue(const string& key, PyObject* py_value,
  ...
  machina::Safe_PyObjectPtr dimension_value(
...
bool ParseTypeValue(const string& key, PyObject* py_value, TF_Status* status,
  ...
  machina::Safe_PyObjectPtr py_type_enum(
...
bool SetOpAttrList(TFE_Context* ctx, TFE_Op* op, const char* key,
                   ...
                   machina::gtl::FlatMap<string, int64_t>* attr_list_sizes,
                   TF_Status* status) {
...
#define PARSE_LIST(c_type, parse_fn)                                    \
  ...
  for (int i = 0; i < num_values; ++i) {                                \
    machina::Safe_PyObjectPtr py_value(PySequence_ITEM(py_list, i)); \
  ...
  if (type == TF_ATTR_STRING) {
    ...
    for (int i = 0; i < num_values; ++i) {
      ...
      if (py_value.get() == Py_None) {
        ...
        for (int j = 0; j < size; ++j) {
          machina::Safe_PyObjectPtr inner_py_value(
    ...
    for (int i = 0; i < num_values; ++i) {
      ...
      //     machina.python.framework.function._DefinedFunction)
        ...
        if (name_attr == nullptr ||
          ...
          TF_SetStatus(
              ...
              absl::StrCat(
                  ...
                  "issue at "
                  "https://github.com/machina/machina/issues/new")
                  .c_str());
          return false;
        }
      }
...
void SetOpAttrListDefault(
    TFE_Context* ctx, TFE_Op* op, const machina::OpDef::AttrDef& attr,
      ...
      if (name_attr == nullptr ||
        ...
        TF_SetStatus(
            ...
            absl::StrCat(
                ...
                "at https://github.com/machina/machina/issues/new")
                .c_str());
        return false;
      }
    }
...
void SetOpAttrScalarDefault(
    TFE_Context* ctx, TFE_Op* op, const machina::AttrValue& default_value,
    const char* attr_name,
    machina::gtl::FlatMap<string, int64_t>* attr_list_sizes,
    TF_Status* status) {
  SetOpAttrValueScalar(ctx, op, default_value, attr_name, status);
  if (default_value.value_case() == machina::AttrValue::kI) {
...
machina::mutex exception_class_mutex(machina::LINKER_INITIALIZED);
...
void TFE_Py_ExecuteCancelable(TFE_Context* ctx, const char* device_name,
  ...
  auto cleaner = machina::gtl::MakeCleanup([ctx, op] { ReturnOp(ctx, op); });
  if (TF_GetCode(out_status) != TF_OK) return;

  machina::unwrap(op)->SetStackTrace(
      machina::ManagedStackTrace(machina::GetStackTrace(
          machina::StackTrace::kStackTraceInitialSize)));
  ...
  if (TF_GetCode(out_status) == TF_OK) {
    ...
    tsl::Set_TF_Status_from_Status(
        out_status,
        machina::errors::CreateWithUpdatedMessage(
...
PyObject* TFE_Py_RegisterExceptionClass(PyObject* e) {
  machina::mutex_lock l(exception_class_mutex);
...
std::string FormatErrorStatusStackTrace(const absl::Status& status) {
  machina::DCheckPyGilState();
  DCHECK(!status.ok());

  std::vector<machina::StackFrame> stack_trace =
      machina::errors::GetStackTrace(status);
  ...
  for (const machina::StackFrame& stack_frame : stack_trace) {
    ...
    machina::str_util::RemoveWhitespaceContext(&line_str);
...
int MaybeRaiseExceptionFromTFStatus(TF_Status* status, PyObject* exception) {
  ...
  if (exception == nullptr) {
    machina::mutex_lock l(exception_class_mutex);
    if (exception_class != nullptr) {
      machina::Safe_PyObjectPtr payloads(PyDict_New());
      for (const auto& payload :
           machina::errors::GetPayloads(StatusFromTF_Status(status))) {
        PyDict_SetItem(payloads.get(),
                       PyBytes_FromString(payload.first.c_str()),
                       PyBytes_FromString(payload.second.c_str()));
      }
      machina::Safe_PyObjectPtr val(Py_BuildValue(
          "siO",
...
int MaybeRaiseExceptionFromStatus(const absl::Status& status,
  ...
  if (exception == nullptr) {
    ...
    if (exception_class != nullptr) {
      machina::Safe_PyObjectPtr payloads(PyDict_New());
      for (const auto& element : machina::errors::GetPayloads(status)) {
        PyDict_SetItem(payloads.get(),
                       PyBytes_FromString(element.first.c_str()),
                       PyBytes_FromString(element.second.c_str()));
      }
      machina::Safe_PyObjectPtr val(
...
class PyTapeTensor {
 public:
  PyTapeTensor(int64_t id, machina::DataType dtype,
               const machina::TensorShape& shape)
      : id_(id), dtype_(dtype), shape_(shape) {}
  PyTapeTensor(int64_t id, machina::DataType dtype, PyObject* shape)
      : id_(id), dtype_(dtype), shape_(shape) {
  ...
  machina::DataType GetDType() const { return dtype_; }
 ...
 private:
  ...
  // tensors.
  std::variant<machina::TensorShape, PyObject*> shape_;
};

static PyTapeTensor TapeTensorFromTensor(PyObject* tensor);

class PyVSpace : public machina::eager::VSpace<PyObject, PyBackwardFunction,
                                                  PyTapeTensor> {
 public:
...
PyObject* PyTapeTensor::ZerosLike() const {
  if (GetDType() == machina::DT_RESOURCE) {
...
class VariableWatcher {
 public:
  ...
  int64_t WatchVariable(PyObject* v) {
    machina::Safe_PyObjectPtr handle(PyObject_GetAttrString(v, "handle"));
    if (handle == nullptr) {
      return -1;
    }
    int64_t id = FastTensorId(handle.get());

    machina::mutex_lock l(watched_variables_mu_);
...
typedef machina::eager::ForwardAccumulator<PyObject, PyBackwardFunction,
                                              PyTapeTensor>
    ForwardAccumulator;
...
// stack.
machina::gtl::CompactPointerSet<TFE_Py_Tape*>* GetTapeSet() {
  thread_local std::unique_ptr<machina::gtl::CompactPointerSet<TFE_Py_Tape*>>
      tape_set;
  ...
  if (tape_set == nullptr) {
    tape_set.reset(new machina::gtl::CompactPointerSet<TFE_Py_Tape*>);
  }
  return tape_set.get();
}

machina::gtl::CompactPointerSet<TFE_Py_VariableWatcher*>*
GetVariableWatcherSet() {
  ...
  if (variable_watcher_set == nullptr) {
    variable_watcher_set.reset(
        new machina::gtl::CompactPointerSet<TFE_Py_VariableWatcher*>);
  }
...
class AccumulatorSet {
 ...
 private:
  ...
  typedef machina::gtl::FlatMap<TFE_Py_ForwardAccumulator*,
                                   ListType::iterator>
      MapType;
...
class SafeTapeSet
    : public SafeSetCopy<machina::gtl::CompactPointerSet<TFE_Py_Tape*>> {
 public:
  SafeTapeSet()
      : SafeSetCopy<machina::gtl::CompactPointerSet<TFE_Py_Tape*>>(
            *GetTapeSet()) {}
};
...
bool TensorShapesAndDtypes(PyObject* tensors, std::vector<int64_t>* tensor_ids,
                           std::vector<machina::DataType>* dtypes) {
  machina::Safe_PyObjectPtr seq(
  ...
  for (int i = 0; i < len; ++i) {
    ...
    dtypes->push_back(machina::PyTensor_DataType(item));
  }
  return true;
}
...
bool DTypeNeedsHandleData(machina::DataType dtype) {
  return dtype == machina::DT_VARIANT || dtype == machina::DT_RESOURCE;
}

static PyTapeTensor TapeTensorFromTensor(PyObject* tensor) {
  if (EagerTensor_CheckExact(tensor)) {
    machina::ImmediateExecutionTensorHandle* handle =
        machina::unwrap(EagerTensor_Handle(tensor));
    int64_t id = PyEagerTensor_ID(tensor);
    machina::DataType dtype =
        static_cast<machina::DataType>(handle->DataType());
    ...
    if (MaybeRaiseExceptionFromStatus(status, nullptr)) {
      return PyTapeTensor(id, static_cast<machina::DataType>(0),
                          machina::TensorShape({}));
    } else {
  ...
  machina::DataType dtype =
      static_cast<machina::DataType>(MakeInt(dtype_enum));
  ...
  machina::Safe_PyObjectPtr shape_tuple(
...
namespace {
std::vector<machina::DataType> MakeTensorDtypeList(PyObject* tensors) {
  PyObject* seq = PySequence_Fast(tensors, "expected a sequence");
  if (seq == nullptr) {
    return {};
  }
  int len = PySequence_Fast_GET_SIZE(seq);
  PyObject** seq_array = PySequence_Fast_ITEMS(seq);
  std::vector<machina::DataType> list;
  list.reserve(len);
  for (int i = 0; i < len; ++i) {
    PyObject* tensor = seq_array[i];
    list.push_back(machina::PyTensor_DataType(tensor));
  }
  Py_DECREF(seq);
  return list;
}
...
void RegisterForwardAccumulatorCleanup(PyObject* tensor, int64_t tensor_id) {
  machina::Safe_PyObjectPtr callback(
...
void TapeSetRecordBackprop(
    ...
    const std::vector<machina::DataType>& input_dtypes,
    ...
    machina::uint64 max_gradient_tape_id) {
  if (!CouldBackprop()) {
    return;
  }
  for (TFE_Py_Tape* tape : SafeTapeSet()) {
...
bool TapeSetRecordForwardprop(
    ...
    const std::function<PyBackwardFunction*()>& backward_function_getter,
    const std::function<void(PyBackwardFunction*)>& backward_function_killer,
    const machina::eager::ForwardFunction<PyObject>* forward_function,
    PyObject* forwardprop_output_indices,
    machina::uint64* max_gradient_tape_id) {
  *max_gradient_tape_id = std::numeric_limits<machina::uint64>::max();
  if (!CouldForwardprop()) {
    return true;
  }
  auto accumulator_set = SafeAccumulatorSet();
  machina::Safe_PyObjectPtr input_seq(
  ...
  if (forwardprop_output_indices != nullptr &&
    ...
    machina::Safe_PyObjectPtr indices_fast(PySequence_Fast(
    ...
    for (AccumulatorSet::const_reverse_iterator it = accumulator_set.rbegin();
      ...
      machina::Safe_PyObjectPtr jvp_index_seq(
...
absl::Status ParseTangentOutputs(PyObject* user_output,
  ...
  machina::Safe_PyObjectPtr fast_result(
...
absl::Status CallJVPFunction(PyObject* op_name, PyObject* attrs,
  ...
  machina::Safe_PyObjectPtr py_input_tangents(
  ...
  machina::Safe_PyObjectPtr input_tuple(PySequence_Tuple(inputs));
  PyObject* to_batch = (use_batch) ? Py_True : Py_False;
  machina::Safe_PyObjectPtr callback_args(
      Py_BuildValue("OOOOOO", op_name, attrs, input_tuple.get(), results,
                    py_input_tangents.get(), to_batch));
  machina::Safe_PyObjectPtr py_result(
...
absl::Status CallOpSpecificJVPFunction(
  ...
  machina::Safe_PyObjectPtr py_result(PyObject_CallObject(
...
bool TapeSetRecordOperation(
    ...
    const machina::eager::ForwardFunction<PyObject>* forward_function) {
  std::vector<PyTapeTensor> output_info;
  machina::Safe_PyObjectPtr output_seq(PySequence_Fast(
...
PyObject* TFE_Py_TapeSetRecordOperation(PyObject* op_type,
  ...
  std::vector<machina::DataType> input_dtypes =
  ...
  if (forward_function == Py_None) {
    ...
    machina::eager::ForwardFunction<PyObject> wrapped_forward_function(
...
PyObject* TFE_Py_TapeSetRecordOperationBackprop(PyObject* op_type,
  ...
  TapeSetRecordBackprop(op_type_str, output_info, input_ids, input_dtypes,
                        ...
                        // accumulators.
                        std::numeric_limits<machina::uint64>::max());
  Py_RETURN_NONE;
}
...
PyObject* TFE_Py_TapeGradient(PyObject* tape, PyObject* target,
  ...
  machina::gtl::FlatSet<int64_t> sources_set(sources_vec.begin(),
                                                sources_vec.end());

  machina::Safe_PyObjectPtr seq =
      machina::make_safe(PySequence_Fast(target, "expected a sequence"));
  int len = PySequence_Fast_GET_SIZE(seq.get());
  PyObject** seq_array = PySequence_Fast_ITEMS(seq.get());
  ...
  if (!result.empty()) {
    ...
    machina::gtl::FlatSet<PyObject*> seen_results(result.size());
    for (int i = 0; i < result.size(); ++i) {
      if (result[i] == nullptr) {
        if (unconnected_gradients_zero) {
          // generate a zeros tensor in the shape of sources[i]
          machina::DataType dtype =
              machina::PyTensor_DataType(sources_obj[i]);
          PyTapeTensor tensor =
...
PyObject* TFE_Py_PackJVPs(PyObject* tensors) {
  if (!TapeCouldPossiblyRecord(tensors)) {
    machina::Safe_PyObjectPtr empty_tuple(PyTuple_New(0));
    machina::Safe_PyObjectPtr empty_list(PyList_New(0));
    return PyTuple_Pack(2, empty_tuple.get(), empty_list.get());
  }
  auto& accumulators = *GetAccumulatorSet();
  machina::Safe_PyObjectPtr tensors_fast(
  ...
  for (Py_ssize_t position = 0; position < len; ++position) {
    ...
    machina::DataType input_dtype(machina::PyTensor_DataType(input));
    if (input_dtype == machina::DT_INVALID) {
      return nullptr;
    }
  ...
  machina::Safe_PyObjectPtr all_indices(PyTuple_New(accumulators.size()));
  ...
  machina::Safe_PyObjectPtr new_tensors_py(PyList_New(new_tensors.size()));
...
bool CheckResourceVariable(PyObject* item) {
  if (machina::swig::IsResourceVariable(item)) {
    machina::Safe_PyObjectPtr handle(
...
bool CheckInputsOk(PyObject* seq, int start_index,
  ...
  for (int i = 0; i < op_def.input_arg_size(); i++) {
    ...
    if (!op_def.input_arg(i).number_attr().empty() ||
      ...
      machina::Safe_PyObjectPtr fast_item(
...
machina::DataType MaybeGetDType(PyObject* item) {
  if (EagerTensor_CheckExact(item) || CheckResourceVariable(item)) {
    return machina::PyTensor_DataType(item);
  }

  return machina::DT_INVALID;
}

machina::DataType MaybeGetDTypeForAttr(const string& attr,
  ...
  for (const auto& input_info : it->second) {
    ...
    if (input_info.is_list) {
      ...
      for (int i = 0; i < len; i++) {
        ...
        if (dtype != machina::DT_INVALID) return dtype;
      }
    } else {
...
PyObject* CopySequenceSettingIndicesToNull(
    PyObject* seq, const machina::gtl::FlatSet<int>& indices) {
  machina::Safe_PyObjectPtr fast_seq(
...
PyObject* RecordGradient(PyObject* op_name, PyObject* inputs, PyObject* attrs,
  ...
  if (PyErr_Occurred()) return nullptr;
  std::vector<machina::DataType> input_dtypes = MakeTensorDtypeList(inputs);
  if (PyErr_Occurred()) return nullptr;
  ...
  machina::eager::ForwardFunction<PyObject> py_forward_function(
  ...
  machina::eager::ForwardFunction<PyObject>* forward_function;
        ...
        PyBackwardFunction* function = new PyBackwardFunction(
            [op_name, attrs, num_inputs, op_inputs, op_outputs,
             forward_pass_name_scope](
              ...
              machina::Safe_PyObjectPtr skip_input_indices;
              ...
              machina::Safe_PyObjectPtr callback_args(Py_BuildValue(
              ...
              return machina::swig::Flatten(result.get());
            });
        return function;
      },
      [op_name, attrs, num_inputs, op_inputs, op_outputs,
...
void MaybeNotifyVariableAccessed(PyObject* input) {
  ...
  machina::Safe_PyObjectPtr trainable(
...
bool ReadVariableOp(const FastPathOpExecInfo& parent_op_exec_info,
                    PyObject* input, machina::Safe_PyObjectPtr* output,
                    TF_Status* status) {
  MaybeNotifyVariableAccessed(input);

  TFE_Op* op = TFE_NewOp(parent_op_exec_info.ctx, "ReadVariableOp", status);
  auto cleaner = machina::gtl::MakeCleanup([op] { TFE_DeleteOp(op); });
  if (machina::MaybeRaiseExceptionFromTFStatus(status, nullptr))
    return false;
  ...
  machina::Safe_PyObjectPtr dtype(PyObject_GetAttrString(input, "_dtype"));
  int value;
  ...
  // Get handle
  machina::Safe_PyObjectPtr handle(PyObject_GetAttrString(input, "_handle"));
...
bool ConvertToTensor(
    ...
    machina::Safe_PyObjectPtr* output_handle,
    // This gets a hint for this particular input.
    const std::function<machina::DataType()>& dtype_hint_getter,
    // This sets the dtype after conversion is complete.
    const std::function<void(const machina::DataType dtype)>& dtype_setter,
    TF_Status* status) {
  ...
  machina::DataType dtype_hint = dtype_hint_getter();

  TFE_TensorHandle* handle = machina::ConvertToEagerTensor(
      op_exec_info.ctx, input, dtype_hint, op_exec_info.device_name);
  if (handle == nullptr) {
    return machina::MaybeRaiseExceptionFromTFStatus(status, nullptr);
  }

  output_handle->reset(EagerTensorFromHandle(handle));
  dtype_setter(
      static_cast<machina::DataType>(TFE_TensorHandleDataType(handle)));
...
bool AddInputToOp(FastPathOpExecInfo* op_exec_info, PyObject* input,
                  ...
                  const machina::OpDef::ArgDef& input_arg,
                  std::vector<machina::Safe_PyObjectPtr>* flattened_attrs,
                  std::vector<machina::Safe_PyObjectPtr>* flattened_inputs,
...
bool RunCallbacks(
    ...
    const std::vector<machina::Safe_PyObjectPtr>& flattened_inputs,
    const std::vector<machina::Safe_PyObjectPtr>& flattened_attrs,
    PyObject* flattened_result) {
  DCHECK(op_exec_info.run_callbacks);

  machina::Safe_PyObjectPtr inputs(PyTuple_New(flattened_inputs.size()));
  ...
  machina::Safe_PyObjectPtr attrs(PyTuple_New(num_attrs));
...
PyObject* TFE_Py_FastPathExecute_C(PyObject* args) {
  ...
  auto* tld = machina::GetEagerContextThreadLocalData(py_eager_context);
  if (tld == nullptr) {
    return nullptr;
  }
  ...
  auto cleaner = machina::gtl::MakeCleanup([status, ctx, op] {
    ReturnStatus(status);
    ReturnOp(ctx, op);
  });
  ...
  const machina::OpDef* op_def = machina::unwrap(op)->OpDef();
  ...
  // directly.
  std::unique_ptr<std::vector<machina::Safe_PyObjectPtr>> flattened_attrs =
      nullptr;
  std::unique_ptr<std::vector<machina::Safe_PyObjectPtr>> flattened_inputs =
      nullptr;

  // TODO(nareshmodi): Encapsulate callbacks information into a struct.
  if (op_exec_info.run_callbacks) {
    flattened_attrs.reset(new std::vector<machina::Safe_PyObjectPtr>);
    flattened_inputs.reset(new std::vector<machina::Safe_PyObjectPtr>);
  }
  ...
  for (int i = 0; i < op_def->input_arg_size(); i++) {
    ...
    if (!input_arg.number_attr().empty()) {
      ...
      machina::Safe_PyObjectPtr fast_input(
      ...
      for (Py_ssize_t j = 0; j < len; j++) {
        ...
        machina::Safe_PyObjectPtr py_eager_tensor;
        if (!ConvertToTensor(
                op_exec_info, py_input, &py_eager_tensor,
                []() { return machina::DT_INVALID; },
                [](const machina::DataType dtype) {}, status)) {
          return nullptr;
        }
  ...
  if (TF_GetCode(status) != TF_OK) {
    ...
    machina::MaybeRaiseExceptionFromTFStatus(status, nullptr);
    return nullptr;
  }

  machina::Safe_PyObjectPtr flat_result(PyList_New(num_retvals));
...
// Weak reference to the Python Context (see machina/python/eager/context.py)
...
struct EagerContextThreadLocalDataDefaults {
  machina::Safe_PyObjectPtr is_eager;
  machina::Safe_PyObjectPtr device_spec;
};
...
using EagerContextThreadLocalDataMap = absl::flat_hash_map<
    PyObject*, std::unique_ptr<machina::EagerContextThreadLocalData>>;
