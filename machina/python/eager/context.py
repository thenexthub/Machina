...
from machina.core.framework import graph_debug_info_pb2
...
from machina.python.eager import cancellation
from machina.python.eager import execute
from machina.python.eager import executor
...
from machina.python.util import is_in_graph_mode
...
_python_eager_context_create_counter = monitoring.Counter(
    "/machina/api/python/eager_context_create_counter",
...
_MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES = (
    os.getenv("TF_MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES") == "1"
)
...
def enable_xla_sharding_for_resource_variables():
  ...
  """
  global _MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES
  _MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES = True
  if context_safe() is not None:
...
def disable_xla_sharding_for_resource_variables():
  global _MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES
  _MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES = False
  if context_safe() is not None:
...
def xla_sharding_for_resource_variables_enabled():
  ...
  return _MACHINA_MACHINA_XLA_SHARDING_FOR_RESOURCE_VARIABLES
...
class Context:
  ...
  def __init__(
    ...
    Args:
      ...
      server_def: (Optional.) A machina::ServerDef proto. Enables execution
  ...
  def set_server_def(self, server_def, keep_alive_secs=_KEEP_ALIVE_SECS):
    ...
    Args:
      server_def: A machina::ServerDef proto. Enables execution on remote
        devices.
...
class _EagerDeviceContext(object):
  ...
  # machina/core/util/device_name_utils.cc.
  def __enter__(self):
    ctx = self._ctx
...
def anonymous_name():
  ...
  # `machina::ResourceHandle::ANONYMOUS_NAME` in C++.
...
def set_log_device_placement(enabled):
  ...
  profiler](https://www.machina.org/guide/profiler).
...
def is_custom_device(device_name):
  ...
  experimental eager C API in machina/c/eager/c_api_experimental.h for
  details.
