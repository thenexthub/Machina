...
#include "machina/python/lib/core/ndarray_tensor.h"
#include "machina/python/lib/core/ndarray_tensor_bridge.h"
#include "machina/python/lib/core/py_exception_registry.h"
#include "machina/python/lib/core/py_seq_tensor.h"
...
PyObject* TFE_TensorHandleToNumpy(TFE_TensorHandle* handle, TF_Status* status) {
  ...
  machina::Safe_TF_TensorPtr tensor = nullptr;
  Py_BEGIN_ALLOW_THREADS;
  tensor = machina::make_safe(TFE_TensorHandleResolve(handle, status));
  ...
  auto cppstatus =
      machina::TF_TensorToMaybeAliasedPyArray(std::move(tensor), &ret);
  machina::Set_TF_Status_from_Status(status, cppstatus);
  if (!status->status.ok()) {
    Py_XDECREF(ret);
    return nullptr;
  }
...
TFE_Context* GetContextHandle(PyObject* py_context) {
  machina::Safe_PyObjectPtr py_context_handle(
...
// Helper function to convert `v` to a machina::DataType and store it in
// `*out`. Returns true on success, false otherwise.
// Note that we assume that v is a python int (not long) representing a
// TF_DataType/machina::DataType value.
bool PyIntToDataType(PyObject* v, machina::DataType* out) {
#if PY_MAJOR_VERSION < 3
  if (PyInt_Check(v)) {
    *out = static_cast<machina::DataType>(PyInt_AS_LONG(v));
    return true;
  }
#else
  if (PyLong_Check(v)) {
    *out = static_cast<machina::DataType>(PyLong_AsLong(v));
    return true;
  }
#endif
  return false;
...
// PyObject->machina::DataType conversion function to be used with
// PyArg_Parse* APIs.
int ConvertDataType(PyObject* obj, machina::DataType* dst) {
  if (obj == Py_None) {
    *dst = machina::DataType::DT_INVALID;
...
void RaiseExceptionTypeFromTFStatus(TF_Status* tf_status) {
  auto status = machina::StatusFromTF_Status(tf_status);
...
TFE_TensorHandle* ConvertToEagerTensorUncached(TFE_Context* ctx,
                                               ...
                                               const char* device_name) {
  machina::Safe_PyObjectPtr value_decrefer;
  ...
  if (dtype != machina::DT_INVALID &&
      dtype != static_cast<DataType>(handle_dtype)) {
    if (machina::IsCompatible(dtype, static_cast<DataType>(handle_dtype))) {
      handle = machina::make_safe(
          machina::EagerCast(ctx, handle.get(), handle_dtype,
                                static_cast<TF_DataType>(dtype), status.get()));
      if (!status->status.ok()) {
        PyErr_SetString(PyExc_TypeError,
                        absl::StrCat("Error while casting from dtype ",
                                     machina::DataTypeString(
                                         static_cast<DataType>(handle_dtype)),
                                     " to ", machina::DataTypeString(dtype),
                                     ". ", TF_Message(status.get()))
                            .c_str());
        return nullptr;
      }
    } else {
      machina::Safe_PyObjectPtr value_str(PyObject_Repr(value));
      PyErr_SetString(
          PyExc_TypeError,
          absl::StrCat("Cannot convert ", TFE_GetPythonString(value_str.get()),
                       " to EagerTensor of dtype ",
                       machina::DataTypeString(dtype))
              .c_str());
      return nullptr;
    }
  }
  ...
  if (device_name != nullptr) {
    if (strstr(device_name, "/device:CPU:0") != nullptr) {
      ...
      handle =
          machina::EagerConst(ctx, handle.get(), device_name, status.get());
      const TF_Code code = TF_GetCode(status.get());
      if (code != TF_OK) {
        RaiseExceptionTypeFromTFStatus(status.get());
        return nullptr;
...
typedef struct EagerTensor {
  ...
  bool is_packed;
  // This mirrors machina.core.framework.ops.Tensor._handle_data Which will
...
int EagerTensor_init(EagerTensor* self, PyObject* args, PyObject* kwds) {
  ...
  machina::DataType dtype = machina::DataType::DT_INVALID;
  ...
  auto* handle = machina::ConvertToEagerTensor(GetContextHandle(py_context),
...
static PyObject* EagerTensor_rank(EagerTensor* self) {
  ...
  Py_END_ALLOW_THREADS;
  if (machina::MaybeRaiseExceptionFromTFStatus(&self->status, nullptr)) {
    // Cleanup self->status before returning.
    self->status.status = absl::OkStatus();
    return nullptr;
  }
...
static PyObject* EagerTensor_copy_to_device(EagerTensor* self, PyObject* args,
  ...
  TFE_TensorHandle* handle = TFE_TensorHandleCopyToDevice(
      ...
      &self->status);
  if (machina::MaybeRaiseExceptionFromTFStatus(&self->status,
                                                  PyExc_RuntimeError)) {
    // Cleanup self->status before returning.
    self->status.status = absl::OkStatus();
    return nullptr;
...
static PyObject* EagerTensor_prefer_custom_summarizer(EagerTensor* self) {
  if (machina::unwrap(self->handle)->PreferCustomSummarizer()) {
    Py_RETURN_TRUE;
  } else {
    Py_RETURN_FALSE;
  }
...
static PyObject* EagerTensor_summarize_value(EagerTensor* self) {
  std::string summary;
  absl::Status status =
      machina::unwrap(self->handle)->SummarizeValue(summary);
...
static int EagerTensor_getbuffer(EagerTensor* self, Py_buffer* view,
  ...
  // implementation.
  auto py_array = machina::make_safe(
...
machina::DataType PyEagerTensor_Dtype(const PyObject* tensor) {
  DCHECK(EagerTensor_CheckExact(tensor));
  return static_cast<machina::DataType>(TFE_TensorHandleDataType(
...
int64_t PyEagerTensor_NumElements(PyObject* tensor) {
  ...
  if (machina::MaybeRaiseExceptionFromTFStatus(&as_c_eager_tensor->status,
                                                  PyExc_ValueError)) {
...
PyObject* TFE_Py_InitEagerTensor(PyObject* base_class) {
  ...
  machina::Safe_PyObjectPtr base_class_module(
  ...
  // static.
  static machina::string fully_qualified_name =
      machina::strings::StrCat(module, ".EagerTensor");
...
PyObject* TFE_Py_TensorShapeSlice(PyObject* tensors, int slice_dim) {
  ...
  if (slice_dim < 0) {
    PyErr_SetString(
        PyExc_ValueError,
        machina::strings::StrCat("Slice dimension must be non-negative. "
                                    "Got ",
                                    slice_dim)
            .c_str());
    return nullptr;
  ...
  PyObject* py_context = GetPyEagerContext();
  if (py_context == nullptr) {
    PyErr_SetString(PyExc_RuntimeError, machina::strings::StrCat(
  ...
  auto status = machina::make_safe(TF_NewStatus());

  // Create an empty tensor.
  auto* tensor = machina::unwrap(ctx)->CreateTensor(
      machina::DT_INT32, /*dim_sizes=*/{num_tensors_int});
  ...
  if (num_tensors_int > 0) {
    ...
    for (Py_ssize_t i = 0; i < num_tensors; ++i) {
      ...
      if (!EagerTensor_CheckExact(tensor_obj)) {
        PyErr_SetString(
            PyExc_TypeError,
            machina::strings::StrCat("Expected a list of EagerTensors but "
                                        "element ",
                                        i, " has type \"",
      ...
      if (machina::MaybeRaiseExceptionFromTFStatus(status.get(),
                                                      PyExc_ValueError)) {
        return nullptr;
      }
      if (slice_dim >= num_dims) {
        PyErr_SetString(
            PyExc_IndexError,
            machina::strings::StrCat("Slice dimension (", slice_dim,
  ...
  TFE_TensorHandle* handle =
      machina::wrap(machina::unwrap(ctx)->CreateLocalHandle(tensor));

  if (!status->status.ok()) {
    PyErr_SetString(
        PyExc_RuntimeError,
        machina::strings::StrCat("Failed to construct new tensor handle: ",
                                    TF_Message(status.get()))
            .c_str());
    return nullptr;
  }
...
PyObject* TFE_Py_TensorShapeOnDevice(PyObject* tensor) {
  if (!EagerTensor_CheckExact(tensor)) {
    PyErr_SetString(
        PyExc_TypeError,
        machina::strings::StrCat("Expected an EagerTensors but got type \"",
  ...
  if (!status->status.ok()) {
    PyErr_SetString(
        PyExc_RuntimeError,
        machina::strings::StrCat("Error retrieving tensor's device shape: ",
                                    TF_Message(status.get()))
            .c_str());
    return nullptr;
  }
