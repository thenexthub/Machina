...
from machina.python.framework import error_interpolation
from machina.python.framework import errors
from machina.python.framework import func_graph as func_graph_module
from machina.python.framework import function_def_to_graph
  ...
  def graph_call_attrs(self) -> Dict[str, Any]:
    ...
    attrs = {
        ...
        "xla_compile_attr": self.cached_definition.attr.get(
            attributes_lib.MACHINA_MACHINA_XLA_COMPILE, None
        ),
    }
...
def partitioned_call_op(
  ...
  Args:
    ...
    config: (Optional) A `machina::ConfigProto` proto, serialized. If `None`,
  ...
  if xla_compile_attr is not None:
    op_attrs[attributes_lib.MACHINA_MACHINA_XLA_COMPILE] = xla_compile_attr
