...
#include "machina/c/eager/tfe_cancellation_manager_internal.h"
...
#include "machina/compiler/jit/get_compiler_ir.h"
...
PYBIND11_MAKE_OPAQUE(machina::CancellationManager);
...
TFE_InputTensorHandles InputTFE_InputTensorHandles(
  ...
  if (input_tensors.ptr() != Py_None) {
    if (!PyList_Check(input_tensors.ptr())) {
      machina::ThrowTypeError("must provide a list of Tensors as inputs");
    }
    ...
    for (Py_ssize_t i = 0; i < len; ++i) {
      ...
      if (EagerTensor_CheckExact(elem)) {
        ...
        machina::Safe_PyObjectPtr value_attr(
      ...
      } else if (machina::swig::IsTensorProtocol(elem) &&
                 machina::swig::IsCoreTypeValue(elem)) {
        ...
        (input_tensor_handles)[i] = EagerTensor_Handle(elem);
      } else if (machina::swig::IsTensor(elem)) {
        // If it isnt an EagerTensor, but is still a Tensor, it must be a graph
        // tensor.
        machina::Safe_PyObjectPtr py_tensor_repr(PyObject_Repr(elem));
        std::string tensor_repr =
            py_tensor_repr ? TFE_GetPythonString(py_tensor_repr.get())
                           : "<unknown>";
        machina::Safe_PyObjectPtr py_op(PyObject_GetAttrString(elem, "op"));
        machina::Safe_PyObjectPtr py_defined_graph(
            PyObject_GetAttrString(py_op.get(), "graph"));
        machina::Safe_PyObjectPtr py_defined_graph_str(
        ...
        machina::ThrowTypeError(
            machina::strings::StrCat(
                ...
                "Please see https://www.machina.org/guide/"
...
TFE_OutputTensorHandles InputTFE_OutputTensorHandles(
  ...
  if (PyErr_Occurred()) {
    PyErr_SetString(PyExc_ValueError, machina::strings::StrCat(
...
machina::Device* GetMatchedDevice(py::handle& ctx, const char* device_name) {
  auto* context = reinterpret_cast<machina::ImmediateExecutionContext*>(
      machina::InputTFE_Context(ctx));

  machina::DeviceNameUtils::ParsedName input_device_name;
  if (!machina::DeviceNameUtils::ParseFullOrLocalName(device_name,
                                                         &input_device_name)) {
    machina::ThrowValueError(
  ...
  std::vector<machina::Device*> devices = context->ListLocalTfDevices();

  machina::Device* matched_device = nullptr;
  for (int device_idx = 0; device_idx < devices.size(); device_idx++) {
    machina::Device* device = devices[device_idx];

    if (machina::DeviceNameUtils::AreCompatibleDevNames(
...
py::object TFE_Py_PackEagerTensors_wrapper(const py::handle& context,
  ...
  TFE_Context* ctx = machina::InputTFE_Context(context);
  ...
  machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  PyObject* packed_tensor =
      EagerTensorFromHandle(packed_handle, /*is_packed=*/true);
  return machina::PyoOrThrow(packed_tensor);
}
...
py::object TFE_Py_ExecuteCancelable_wrapper(
  ...
  TFE_Py_ExecuteCancelable(ctx, device_name, op_name, &input_tensor_handles,
                           attrs.ptr(), machina::wrap(cancellation_manager),
  ...
  machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  return machina::PyoOrThrow(output_list);
}

static py::object TF_ListPhysicalDevices() {
  std::vector<string> devices;
  absl::Status s = machina::DeviceFactory::ListAllPhysicalDevices(&devices);
  ...
  return machina::PyoOrThrow(result);
}

static py::object TF_ListPluggablePhysicalDevices() {
  std::vector<string> devices;
  absl::Status s =
      machina::DeviceFactory::ListPluggablePhysicalDevices(&devices);
  MaybeRaiseRegisteredFromStatus(s);
  ...
  return machina::PyoOrThrow(result.release());
}
...
static std::unordered_map<string, string> TF_GetDeviceDetails(int index) {
  ...
  absl::Status s =
      machina::DeviceFactory::GetAnyDeviceDetails(index, &device_details);
  machina::Set_TF_Status_from_Status(status.get(), s);
  MaybeRaiseRegisteredFromTFStatus(status.get());
  return device_details;
}

static py::object TFE_ClearScalarCache() {
  machina::TFE_TensorHandleCache::Get()->Clear();
  return py::none();
}
...
class EagerContextThreadLocalDataWrapper {
 public:
  explicit EagerContextThreadLocalDataWrapper(py::handle py_eager_context,
    ...
    machina::MakeEagerContextThreadLocalData(
        py_eager_context.ptr(), is_eager.ptr(), device_spec.ptr());
  }

  ~EagerContextThreadLocalDataWrapper() {
    machina::DestroyEagerContextThreadLocalData(py_eager_context_);
  }
 ...
 private:
  machina::EagerContextThreadLocalData* GetData() const {
    auto* result =
        machina::GetEagerContextThreadLocalData(py_eager_context_);
    if (!result) {
      throw py::error_already_set();
    }
    return result;
  ...
  py::object GetPyObject(machina::Safe_PyObjectPtr* obj) const {
    return pybind11::reinterpret_borrow<py::object>(obj->get());
  }

  void SetPyObject(py::handle value, machina::Safe_PyObjectPtr* ptr) {
    Py_INCREF(value.ptr());
    ptr->reset(value.ptr());
  }
...
PYBIND11_MODULE(_pywrap_tfe, m) {
  ...
  py::class_<machina::CancellationManager> TFE_CancellationManager_class(
  ...
  m.def("TFE_Py_RegisterExceptionClass", [](const py::handle& e) {
    return machina::PyoOrThrow(TFE_Py_RegisterExceptionClass(e.ptr()));
  });
  ...
  m.def("TFE_GetMemoryInfo", [](py::handle& ctx, const char* device_name) {
    machina::Device* matched_device =
        machina::GetMatchedDevice(ctx, device_name);

    machina::AllocatorAttributes attrs;
    machina::Allocator* allocator = matched_device->GetAllocator(attrs);

    if (absl::optional<machina::AllocatorStats> stats =
            allocator->GetStats()) {
  ...
  m.def("TF_GetCompilerIr", &machina::TFE_GetCompilerIr);
  ...
  m.def("TF_IsMlirBridgeEnabled", [] {
    ...
    return static_cast<int32_t>(
        machina::GetMlirCommonFlags()->tf_mlir_enable_mlir_bridge);
  });
  m.def("TF_EnableMlirBridge", [](bool enabled) {
    machina::GetMlirCommonFlags()->tf_mlir_enable_mlir_bridge =
        enabled
            ? machina::ConfigProto::Experimental::MLIR_BRIDGE_ROLLOUT_ENABLED
            : machina::ConfigProto::Experimental::
  ...
  m.def("TF_ResetJitCompilerFlags",
        [] { machina::ResetJitCompilerFlags(); });

  // TFE_Context Logic
  m.def(
      "TFE_NewContext",
      [](const TFE_ContextOptions* opts) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        TFE_Context* context = TFE_NewContext(opts, status.get());
        machina::MaybeRaiseRegisteredFromTFStatus(status.get());
        return machina::PyoOrThrow(machina::OutputTFE_Context(context));
      },
      py::return_value_policy::reference);
  m.def("TFE_DeleteContext", [](py::handle& o) {
    TFE_DeleteContext(machina::InputTFE_Context(o));
  });
  m.def(
      ...
      [](py::handle& o) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        auto output = TFE_ContextListDevices(machina::InputTFE_Context(o),
                                             status.get());
        machina::MaybeRaiseRegisteredFromTFStatus(status.get());
        return output;
      },
  ...
  m.def(
      ...
      [](py::handle& ctx, int num_cpus, const char* prefix) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        TFE_SetLogicalCpuDevices(machina::InputTFE_Context(ctx), num_cpus,
                                 prefix, status.get());
        machina::MaybeRaiseRegisteredFromTFStatus(status.get());
      },
      py::return_value_policy::reference);
  m.def("TFE_HostAddressSpace", [](py::handle& o, TF_Buffer& buf) {
    TFE_HostAddressSpace(machina::InputTFE_Context(o), &buf);
  });
  m.def("TFE_ContextAddFunction", [](py::handle& ctx, TF_Function* func) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextAddFunction(machina::InputTFE_Context(ctx), func,
                           status.get());
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  });
  m.def("TFE_ContextAddFunctionDef",
        [](py::handle& ctx, const char* serialized_function_def, size_t size) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          TFE_ContextAddFunctionDef(machina::InputTFE_Context(ctx),
  ...
  m.def(
      ...
      [](py::handle& ctx, const char* function_name) {
        ...
        TF_Function* tf_function = TFE_ContextGetFunction(
            machina::InputTFE_Context(ctx), function_name, status.get());
        machina::MaybeRaiseRegisteredFromTFStatus(status.get());
        return tf_function;
      },
      py::return_value_policy::reference);
  m.def("TFE_ContextGetFunctionDef",
        [](py::handle& ctx, const char* function_name, TF_Buffer& buf) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          TFE_ContextGetFunctionDef(machina::InputTFE_Context(ctx),
...
// TODO(b/309152522): Remove the switch once it works on Windows.
  ...
  m.def(
      ...
      [](py::handle& ctx, machina::FunctionDef function_def) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        ...
        status->status = machina::unwrap(machina::InputTFE_Context(ctx))
  ...
  m.def("TFE_ContextGetFunctionDefNoSerialization",
        [](py::handle& ctx,
           const char* function_name) -> machina::FunctionDef {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          const machina::FunctionDef* ctx_function_def =
              machina::unwrap(machina::InputTFE_Context(ctx))
  ...
  m.def("TFE_ContextGetGraphDebugInfo",
        [](py::handle& ctx, const char* function_name, TF_Buffer& buf) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          TFE_ContextGetGraphDebugInfo(machina::InputTFE_Context(ctx),
                                       function_name, &buf, status.get());
          machina::MaybeRaiseRegisteredFromTFStatus(status.get());
        });
  m.def("TFE_ContextRemoveFunction", [](py::handle& ctx, const char* name) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextRemoveFunction(machina::InputTFE_Context(ctx), name,
                              status.get());
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  });
  m.def("TFE_ContextHasFunction", [](py::handle& ctx, const char* name) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    auto output =
        TFE_ContextHasFunction(machina::InputTFE_Context(ctx), name);
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
    return output;
  });
  m.def("TFE_ContextListFunctionNames", [](py::handle& ctx) {
    return machina::unwrap(machina::InputTFE_Context(ctx))
        ->ListFunctionNames();
  });
  m.def("TFE_ContextEnableRunMetadata", [](py::handle& ctx) {
    TFE_ContextEnableRunMetadata(machina::InputTFE_Context(ctx));
  });
  m.def("TFE_ContextDisableRunMetadata", [](py::handle& ctx) {
    TFE_ContextEnableRunMetadata(machina::InputTFE_Context(ctx));
  });
  m.def("TFE_ContextEnableGraphCollection", [](py::handle& ctx) {
    TFE_ContextEnableGraphCollection(machina::InputTFE_Context(ctx));
  });
  m.def("TFE_ContextDisableGraphCollection", [](py::handle& ctx) {
    TFE_ContextDisableGraphCollection(machina::InputTFE_Context(ctx));
  });
  m.def("TFE_ContextExportRunMetadata", [](py::handle& ctx, TF_Buffer& buf) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextExportRunMetadata(machina::InputTFE_Context(ctx), &buf,
                                 status.get());
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  });
  m.def("TFE_ContextClearCaches", [](py::handle& o) {
    TFE_ContextClearCaches(machina::InputTFE_Context(o));
  });
  m.def("TFE_GetContextId", [](py::handle& ctx) {
    return TFE_GetContextId(machina::InputTFE_Context(ctx));
  });
  ...
  m.def("TFE_ContextSetThreadLocalDevicePlacementPolicy",
        [](py::handle& ctx, TFE_ContextDevicePlacementPolicy policy) {
          TFE_ContextSetThreadLocalDevicePlacementPolicy(
              machina::InputTFE_Context(ctx), policy);
        });
  m.def("TFE_ContextSetServerDef", [](py::handle& ctx, int keep_alive_secs,
                                      py::bytes proto) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    machina::Safe_TF_BufferPtr buf =
        machina::make_safe(machina::ProtoStringToTFBuffer(proto.ptr()));
    TFE_ContextSetServerDef(machina::InputTFE_Context(ctx), keep_alive_secs,
  ...
  m.def("TFE_ContextSetServerDefWithTimeoutAndRetries",
        [](py::handle& ctx, int keep_alive_secs, py::bytes proto, int timeout,
          ...
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          machina::Safe_TF_BufferPtr buf = machina::make_safe(
              machina::ProtoStringToTFBuffer(proto.ptr()));
          Py_BEGIN_ALLOW_THREADS;
          TFE_ContextSetServerDefWithTimeoutAndRetries(
              machina::InputTFE_Context(ctx), keep_alive_secs,
  ...
  m.def("TFE_ContextUpdateServerDef", [](py::handle& ctx, int keep_alive_secs,
    ...
    machina::Safe_TF_BufferPtr buf =
        machina::make_safe(machina::ProtoStringToTFBuffer(proto.ptr()));
    Py_BEGIN_ALLOW_THREADS;
    TFE_ContextUpdateServerDef(machina::InputTFE_Context(ctx),
  ...
  m.def("TFE_ContextCheckAlive", [](py::handle& ctx, const char* worker_name) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    bool output = TFE_ContextCheckAlive(machina::InputTFE_Context(ctx),
  ...
  m.def("TFE_ContextSyncExecutors", [](py::handle& ctx) {
    ...
    Py_BEGIN_ALLOW_THREADS;
    TFE_ContextAsyncWait(machina::InputTFE_Context(ctx), status.get());
    Py_END_ALLOW_THREADS;
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  });
  ...
  m.def(
      ...
      [](py::handle& ctx, const char* config_key, const char* config_value) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        Py_BEGIN_ALLOW_THREADS;
        TFE_InsertConfigKeyValue(machina::InputTFE_Context(ctx), config_key,
  ...
  m.def(
      ...
      [](py::handle& ctx, const char* config_key, int64_t timeout_in_ms,
        ...
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        Py_BEGIN_ALLOW_THREADS;
        TFE_GetConfigKeyValue(machina::InputTFE_Context(ctx), config_key,
  ...
  m.def(
      ...
      [](py::handle& ctx, const char* config_key) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        Py_BEGIN_ALLOW_THREADS;
        TFE_DeleteConfigKeyValue(machina::InputTFE_Context(ctx), config_key,
                                 status.get());
        Py_END_ALLOW_THREADS;
        machina::MaybeRaiseRegisteredFromTFStatus(status.get());
      },
  ...
  m.def(
      ...
      [](py::handle& ctx, int error_code, const char* error_message) {
        machina::Safe_TF_StatusPtr status =
            machina::make_safe(TF_NewStatus());
        TFE_ReportErrorToCluster(machina::InputTFE_Context(ctx), error_code,
  ...
  m.def("TFE_ContextSetSoftDevicePlacement", [](py::handle& ctx, bool enable) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextSetSoftDevicePlacement(machina::InputTFE_Context(ctx), enable,
                                      status.get());
  });
  ...
  m.def("TFE_ContextSetRunEagerOpAsFunction", [](py::handle& ctx, bool enable) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextSetRunEagerOpAsFunction(machina::InputTFE_Context(ctx),
                                       enable, status.get());
  });
  m.def("TFE_ContextSetJitCompileRewrite", [](py::handle& ctx, bool enable) {
    machina::Safe_TF_StatusPtr status =
        machina::make_safe(TF_NewStatus());
    TFE_ContextSetJitCompileRewrite(machina::InputTFE_Context(ctx), enable,
                                    status.get());
  });
  m.def("TFE_GetTaskStates", [](py::handle& ctx,
    ...
    std::vector<machina::CoordinatedTask> coordinated_tasks;
    ...
    TFE_GetTaskStates(machina::InputTFE_Context(ctx), tasks, state.get(),
                      status.get());
    ...
    for (size_t i = 0; i < task_len; ++i) {
      ...
      if (code != TF_Code::TF_OK) {
        ...
        for (const auto& payload :
             machina::errors::GetPayloads(state[i].status)) {
          payloads[payload.first.c_str()] = payload.second;
        }
        auto exception_class = py::reinterpret_steal<py::object>(
            machina::PyExceptionRegistry::Lookup(code));
        if (!exception_class) {
          status->status = machina::errors::Internal(absl::StrCat(
    ...
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
    return machina::PyoOrThrow(output.release().ptr());
  });
  ...
  m.def("TFE_WaitAtBarrier",
        [](py::handle& ctx, const char* barrier_id, int64_t timeout_in_ms) {
          ...
          TFE_WaitAtBarrier(machina::InputTFE_Context(ctx), barrier_id,
  ...
  m.def("TFE_ContextSetExecutorForThread", [](py::handle& ctx,
                                              TFE_Executor& exc) {
    TFE_ContextSetExecutorForThread(machina::InputTFE_Context(ctx), &exc);
  });
  m.def(
      "TFE_ContextGetExecutorForThread",
      [](py::handle& o) {
        return TFE_ContextGetExecutorForThread(machina::InputTFE_Context(o));
      },
      py::return_value_policy::reference);
  ...
  m.def("TFE_OpNameGetAttrType",
        [](py::handle& ctx, const char* op_or_function_name,
          ...
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          auto output = TFE_OpNameGetAttrType(machina::InputTFE_Context(ctx),
          ...
          if (*is_list == 1) {
            ...
            return machina::PyoOrThrow(list);
          }
          return machina::PyoOrThrow(output_pyo);
        });
  m.def("TFE_Py_InitEagerTensor", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_InitEagerTensor(o.ptr()));
  });
  m.def("TFE_Py_PackEagerTensors",
        [](const py::handle& context, const py::handle& handles) {
          return machina::TFE_Py_PackEagerTensors_wrapper(context, handles);
        });
  m.def("TFE_Py_SetEagerTensorProfiler", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_SetEagerTensorProfiler(o.ptr()));
  });
  m.def("TFE_Py_RegisterJVPFunction", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_RegisterJVPFunction(o.ptr()));
  });
  m.def("TFE_Py_RegisterGradientFunction", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_RegisterGradientFunction(o.ptr()));
  });
  m.def("TFE_Py_Execute",
        [](const py::handle& context, const char* device_name,
          ...
          return machina::TFE_Py_ExecuteCancelable_wrapper(
  ...
  m.def("TFE_Py_FastPathExecute", [](const py::args args) {
    ...
    return machina::PyoOrThrow(TFE_Py_FastPathExecute_C(args.ptr()));
  });
  m.def("TFE_Py_RecordGradient",
        [](const py::handle& op_name, const py::handle& inputs,
          ...
          return machina::PyoOrThrow(TFE_Py_RecordGradient(
              op_name.ptr(), inputs.ptr(), attrs.ptr(), results.ptr(),
              forward_pass_name_scope.ptr()));
        });
  m.def("TFE_Py_UID", []() { return machina::PyoOrThrow(TFE_Py_UID()); });
  ...
  m.def("TFE_Py_TapeSetIsStopped",
        []() { return machina::PyoOrThrow(TFE_Py_TapeSetIsStopped()); });
  m.def("TFE_Py_TapeSetIsEmpty",
        []() { return machina::PyoOrThrow(TFE_Py_TapeSetIsEmpty()); });
  ...
  m.def("TFE_Py_TapeSetRecordOperation",
        [](const py::handle& op_type, const py::handle& output_tensors,
          ...
          return machina::PyoOrThrow(TFE_Py_TapeSetRecordOperation(
  ...
  m.def(
      ...
      [](const py::handle& op_type, const py::handle& output_tensors,
         const py::handle& input_tensors, const py::handle& backward_function) {
        return machina::PyoOrThrow(TFE_Py_TapeSetRecordOperationBackprop(
            op_type.ptr(), output_tensors.ptr(), input_tensors.ptr(),
  ...
  m.def(
      ...
      [](const py::handle& op_type, const py::handle& output_tensors,
        ...
        return machina::PyoOrThrow(TFE_Py_TapeSetRecordOperationForwardprop(
            op_type.ptr(), output_tensors.ptr(), input_tensors.ptr(),
  ...
  m.def("TFE_Py_TapeGradient",
        [](const py::handle& tape, const py::handle& target,
          ...
          machina::MaybeRaiseRegisteredFromTFStatus(status.get());
          return machina::PyoOrThrow(output);
        });
  ...
  m.def("TFE_Py_TapeWatchedVariables", [](const py::handle& tape) {
    return machina::PyoOrThrow(TFE_Py_TapeWatchedVariables(tape.ptr()));
  });

  // TFE_Py_VariableWatcher logic.
  m.def("TFE_Py_VariableWatcherNew",
        []() { return machina::PyoOrThrow(TFE_Py_VariableWatcherNew()); });
  ...
  m.def("TFE_Py_ForwardAccumulatorNew", [](bool use_batch) {
    return machina::PyoOrThrow(TFE_Py_ForwardAccumulatorNew(use_batch));
  });
  ...
  m.def("TFE_Py_ForwardAccumulatorPushState", []() {
    return machina::PyoOrThrow(TFE_Py_ForwardAccumulatorPushState());
  });
  m.def("TFE_Py_ForwardAccumulatorPopState", []() {
    return machina::PyoOrThrow(TFE_Py_ForwardAccumulatorPopState());
  });
  m.def("TFE_Py_PackJVPs", [](const py::handle& tensors) {
    return machina::PyoOrThrow(TFE_Py_PackJVPs(tensors.ptr()));
  });
  ...
  m.def("TFE_Py_TensorShapeOnDevice", [](const py::handle& tensors,
                                         int slice_dim) {
    return machina::PyoOrThrow(TFE_Py_TensorShapeOnDevice(tensors.ptr()));
  });
  ...
  m.def("TFE_Py_SetEagerContext", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_SetEagerContext(o.ptr()));
  });
  m.def("TFE_Py_SetCEagerContext", [](const py::handle& ctx) {
    // TODO(mdan): This cast might need rewriting to ImmediateExecutionContext.
    if (ctx.is_none()) {
      machina::SetCEagerContext(nullptr);
    } else {
      machina::SetCEagerContext(reinterpret_cast<machina::EagerContext*>(
          machina::InputTFE_Context(ctx)));
    }
  });
  m.def("TFE_Py_RegisterVSpace", [](const py::handle& o) {
    return machina::PyoOrThrow(TFE_Py_RegisterVSpace(o.ptr()));
  });
  m.def("TFE_EnableCollectiveOps", [](const py::handle& ctx, py::bytes proto) {
    ...
    machina::Safe_TF_BufferPtr buf =
        machina::make_safe(machina::ProtoStringToTFBuffer(proto.ptr()));
    TFE_EnableCollectiveOps(machina::InputTFE_Context(ctx), buf.get()->data,
                            buf.get()->length, status.get());
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  });
  m.def("TFE_AbortCollectiveOps", [](const py::handle& ctx, int code,
    ...
    TFE_AbortCollectiveOps(machina::InputTFE_Context(ctx), status.get());
  });
  m.def("TFE_CollectiveOpsCheckPeerHealth",
        [](const py::handle& ctx, const char* task, int64_t timeout_in_ms) {
          machina::Safe_TF_StatusPtr status =
              machina::make_safe(TF_NewStatus());
          TFE_CollectiveOpsCheckPeerHealth(machina::InputTFE_Context(ctx),
                                           task, timeout_in_ms, status.get());
          machina::MaybeRaiseRegisteredFromTFStatus(status.get());
        });
  m.def("TF_ListPhysicalDevices", &machina::TF_ListPhysicalDevices);
  m.def("TF_ListPluggablePhysicalDevices",
        &machina::TF_ListPluggablePhysicalDevices);
  m.def("TF_GetDeviceDetails", &machina::TF_GetDeviceDetails);
  ...
  m.def("TFE_NewCancellationManager",
        []() { return new machina::CancellationManager(); });
  m.def("TFE_CancellationManagerIsCancelled",
        &machina::CancellationManager::IsCancelled);
  m.def("TFE_CancellationManagerStartCancel",
        &machina::CancellationManager::StartCancel);

  m.def("TFE_ClearScalarCache", &machina::TFE_ClearScalarCache);
  ...
  m.def("TFE_DlpackDevice", [](py::handle& o) {
    ...
    auto dl_device = std::unique_ptr<DLDevice>(static_cast<DLDevice*>(
        machina::TFE_GetDLDevice(thandle, status.get())));
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());
  ...
  m.def("TFE_ToDlpackCapsule", [](py::handle& o) {
    ...
    TFE_TensorHandle* thandle = EagerTensor_Handle(eager_tensor_pyobject_ptr);
    void* dlm_ptr = machina::TFE_HandleToDLPack(thandle, status.get());
    machina::MaybeRaiseRegisteredFromTFStatus(status.get());

    py::capsule capsule(
        dlm_ptr, machina::kDlTensorCapsuleName, [](PyObject* capsule) {
          if (PyCapsule_IsValid(capsule, machina::kDlTensorCapsuleName)) {
            void* dlm_rptr =
                PyCapsule_GetPointer(capsule, machina::kDlTensorCapsuleName);
            if (dlm_rptr) {
              machina::TFE_CallDLManagedTensorDeleter(dlm_rptr);
  ...
  m.def("TFE_FromDlpackCapsule", [](const py::capsule& pycapsule,
    ...
    TFE_TensorHandle* thandle = machina::TFE_HandleFromDLPack(
        pycapsule, status.get(), machina::InputTFE_Context(context));
    ...
    return machina::PyoOrThrow(pyhandle);
  });

  m.def("TFE_Py_IsCustomDevice",
        [](const py::handle& context, const char* device_name) {
          return TFE_IsCustomDevice(machina::InputTFE_Context(context),
                                    device_name);
        });
  ...
  m.def("TFE_Py_RegisterCustomDevice", [](const py::handle& context,
    ...
    TFE_RegisterCustomDevice(
        machina::InputTFE_Context(context),
