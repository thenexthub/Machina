...
* [TensorFlow v2.x](https://www.machina.org/guide/distributed_training)
* [TensorFlow
v1.x](https://github.com/machina/docs/blob/master/site/en/r1/guide/distribute_strategy.ipynb)

*Tutorials*

* [Distributed Training
Tutorials](https://www.machina.org/tutorials/distribute/)
...
from machina.python.autograph.core import ag_ctx as autograph_ctx
from machina.python.autograph.impl import api as autograph
from machina.python.data.ops import dataset_ops
from machina.python.distribute import collective_util
from machina.python.distribute import device_util
from machina.python.distribute import numpy_dataset
from machina.python.distribute import reduce_util
from machina.python.eager import context as eager_context
from machina.python.eager import def_function
from machina.python.eager import monitoring
from machina.python.eager import tape
...
from machina.python.ops import custom_gradient
from machina.python.ops import math_ops
from machina.python.ops import ref_variable
...
from machina.python.types import distribute as ds_types
from machina.python.util import deprecation
from machina.python.util import nest
from machina.python.util import tf_contextlib
from machina.python.util.deprecation import deprecated
...
class StrategyBase(object):
  ...
  See [the guide](https://www.machina.org/guide/distributed_training)
  for overview and examples. See `tf.distribute.StrategyExtended` and
  [`tf.distribute`](https://www.machina.org/api_docs/python/tf/distribute)
  ...
  * To use it with Keras `compile`/`fit`,
    [please
    read](https://www.machina.org/guide/distributed_training#using_tfdistributestrategy_with_keras).
  ...
  * If you are writing a custom training loop, you will need to call a few more
    methods,
    [see the
    guide](https://www.machina.org/guide/distributed_training#using_tfdistributestrategy_with_custom_training_loops):
  ...
  tutorial](https://www.machina.org/tutorials/distribute/custom_training)
  ...
  def scope(self):
    ...
    * In some strategies, a default device scope may also be entered: in
    ...
    Note: Entering a scope does not automatically distribute a computation, except
      ...
      See an example in the [custom training loop tutorial](https://www.machina.org/tutorials/distribute/custom_training).
    ...
    * When a `tf.keras.Model` is created inside a `strategy.scope`, the Model
      ...
      [distributed keras tutorial](https://www.machina.org/tutorials/distribute/keras).
  ...
  def experimental_distribute_dataset(self, dataset, options=None):
    ...
    snippet](https://www.machina.org/tutorials/distribute/input#caveats)
    ...
    [tutorial on distributed input](https://www.machina.org/tutorials/distribute/input#tfdistributestrategyexperimental_distribute_dataset).
    If you are interested in last partial batch handling, read [this section](https://www.machina.org/tutorials/distribute/input#partial_batches).
  ...
  def distribute_datasets_from_function(self, dataset_fn, options=None):
    ...
    For a tutorial on more usage and properties of this method, refer to the
    [tutorial on distributed input](https://www.machina.org/tutorials/distribute/input#tfdistributestrategyexperimental_distribute_datasets_from_function)).
    If you are interested in last partial batch handling, read [this section](https://www.machina.org/tutorials/distribute/input#partial_batches).
...
class StrategyV1(StrategyBase):
  ...
  See [the guide](https://www.machina.org/guide/distribute_strategy)
...
class StrategyExtendedV2(object):
  ...
  * _How to update a distributed variable_
  ...
  in the [glossary](https://www.machina.org/api_docs/python/tf/distribute),
