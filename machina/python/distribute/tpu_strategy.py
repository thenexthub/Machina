...
from machina.python.compiler.xla.experimental import xla_sharding
from machina.python.distribute import cross_device_ops as cross_device_ops_lib
from machina.python.distribute import device_util
from machina.python.distribute import distribute_lib
from machina.python.distribute import distribute_utils
from machina.python.distribute import input_lib
from machina.python.distribute import input_util
from machina.python.distribute import numpy_dataset
from machina.python.distribute import reduce_util
from machina.python.distribute import tpu_replicated_variable
from machina.python.distribute import tpu_util
from machina.python.distribute import tpu_values
from machina.python.distribute import values
from machina.python.distribute.cluster_resolver import tpu_cluster_resolver as tpu_cluster_resolver_lib
from machina.python.distribute.v1 import input_lib as input_lib_v1
...
from machina.python.framework import device as tf_device
from machina.python.framework import device_spec
...
from machina.python.tpu import device_assignment as device_assignment_lib  # pylint: disable=unused-import
from machina.python.tpu import tpu
from machina.python.tpu import tpu_hardware_feature
from machina.python.tpu import training_loop
from machina.python.tpu.ops import tpu_ops
...
_MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT = 200
...
class TPUStrategyV2(distribute_lib.Strategy):
  ...
  behavior is enabled. See more details in https://www.machina.org/guide/tpu.
...
class TPUExtended(distribute_lib.StrategyExtendedV1):
  ...
  def _gather_to_implementation(self, value, destinations, axis, options):
    ...
    if len(value.values) <= _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT:
      output = array_ops.concat(value_list, axis=axis)
    else:
      output = array_ops.concat(
          value_list[:_MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT], axis=axis)
      for i in range(_MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT, len(value_list),
                     _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT - 1):
        output = array_ops.concat(
            [output] + value_list[i:i + _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT - 1],
            axis=axis)
  ...
  def _reduce_to(self, reduce_op, value, destinations, options):
    ...
    if len(value.values) <= _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT:
      ...
      for i in range(0, len(value_list), _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT):
        output += math_ops.add_n(value_list[i:i + _MACHINA_MACHINA_XLA_OP_BY_OP_INPUTS_LIMIT])
