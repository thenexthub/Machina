...
class KubernetesClusterResolverTest(test.TestCase):
  ...
  def testSingleItemSuccessfulRetrievalInCluster(self):
    ret = _create_pod_list(
        ('machina-abc123', 'Running', '10.1.2.3'),
    )

    cluster_resolver = KubernetesClusterResolver(
        override_client=_mock_kubernetes_client({'job-name=machina': ret}),
  ...
  def testSingleItemSuccessfulRetrieval(self):
    ret = _create_pod_list(('machina-abc123', 'Running', '10.1.2.3'),)

    cluster_resolver = KubernetesClusterResolver(
        override_client=_mock_kubernetes_client(
            {'job-name=machina': ret}))
  ...
  def testSuccessfulRetrievalWithSort(self):
    ret = _create_pod_list(
        ('machina-abc123', 'Running', '10.1.2.3'),
        ('machina-def456', 'Running', '10.1.2.4'),
        ('machina-999999', 'Running', '10.1.2.5'))
  ...
  def testNonRunningPod(self):
    ret = _create_pod_list(('machina-abc123', 'Failed', '10.1.2.3'),)

    cluster_resolver = KubernetesClusterResolver(
        override_client=_mock_kubernetes_client(
            {'job-name=machina': ret}))

    error_msg = 'Pod "machina-abc123" is not running; phase: "Failed"'
  ...
  def testMultiplePodSelectorsAndWorkers(self):
    ...
    worker2 = _create_pod_list(
        ('machina-abc124', 'Running', '10.1.2.6'),
        ('machina-def457', 'Running', '10.1.2.7'),
        ('machina-999990', 'Running', '10.1.2.8'))
    ps = _create_pod_list(
        ('machina-ps-1', 'Running', '10.1.2.1'),
        ('machina-ps-2', 'Running', '10.1.2.2'))
