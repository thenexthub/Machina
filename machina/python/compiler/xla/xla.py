...
from machina.compiler.jit.ops import xla_ops
...
from machina.python.distribute import summary_op_util
...
_MACHINA_MACHINA_XLA_COMPILE_ATTR = '_xla_compile_id'
...
class XLACompileContext(control_flow_ops.XLAControlFlowContext):
  ...
  THIS IS ONLY FOR MACHINA INTERNAL IMPLEMENTATION, DO NO USE DIRECTLY.
  ...
  def AddOp(self, op: ops.Operation):
    ...
    if _MACHINA_MACHINA_XLA_COMPILE_ATTR in op.node_def.attr:
      raise ValueError('XLA compiled computations cannot be nested, (operator '
                       'name: %s)' % op.name)

    op._set_attr(
        _MACHINA_MACHINA_XLA_COMPILE_ATTR, attr_value_pb2.AttrValue(s=self._name_as_bytes))
