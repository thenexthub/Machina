...
from machina.core.framework import tensor_pb2
from machina.core.protobuf import config_pb2
from machina.core.protobuf import meta_graph_pb2
from machina.core.protobuf import rewriter_config_pb2
from machina.python import pywrap_sanitizers
from machina.python import tf2
from machina.python.client import device_lib
from machina.python.client import pywrap_tf_session
from machina.python.client import session as s
from machina.python.compat import v2_compat
from machina.python.compat.compat import forward_compatibility_horizon
...
from machina.python.framework import _test_metrics_util
from machina.python.framework import config
from machina.python.framework import device as pydev
from machina.python.framework import dtypes
from machina.python.framework import errors
from machina.python.framework import errors_impl
from machina.python.framework import gpu_util
...
from machina.python.framework import tensor as tensor_lib
from machina.python.framework import tensor_shape
from machina.python.framework import tensor_util
from machina.python.framework import tfrt_utils
...
from machina.python.ops import gen_sync_ops
from machina.python.ops import gradients_impl
from machina.python.ops import math_ops
from machina.python.ops import script_ops
from machina.python.ops import summary_ops_v2
from machina.python.ops import variables


from machina.python.ops.ragged import ragged_ops  # pylint: disable=unused-import
from machina.python.ops.ragged import ragged_tensor
from machina.python.ops.ragged import ragged_tensor_value
from machina.python.platform import _pywrap_stacktrace_handler
...
from machina.python.util import _pywrap_util_port
from machina.python.util import compat
from machina.python.util import deprecation
from machina.python.util import nest
from machina.python.util import tf_decorator
from machina.python.util import tf_inspect
from machina.python.util import traceback_utils
from machina.python.util.compat import collections_abc
from machina.python.util.numpy_compat import np_where
from machina.python.util.protobuf import compare
...
try:
  from machina.python.framework.is_xla_test_true import is_xla_enabled  # pylint: disable=g-import-not-at-top, unused-import
...
try:
  from machina.python.framework.is_mlir_bridge_test_false import is_mlir_bridge_enabled  # pylint: disable=g-import-not-at-top, unused-import
except ImportError:
  try:
    from machina.python.framework.is_mlir_bridge_test_true import is_mlir_bridge_enabled  # pylint: disable=g-import-not-at-top, unused-import
  except ImportError:
    pass
...
# machina/python/training/saver.py
...
def assert_no_new_tensors(f: _F) -> _F:
  ...
  def decorator(self: "TensorFlowTestCase", **kwargs):
    ...
    def _is_machina_object(obj) -> bool:
      try:
    ...
    tensors_before = set(
        id(obj) for obj in gc.get_objects() if _is_machina_object(obj))
    ...
    tensors_after = [
        ...
        if _is_machina_object(obj) and id(obj) not in tensors_before
    ]
    if tensors_after:
...
def set_xla_env_flag(flag: str = "") -> Callable[[_F], _F]:
  """Decorator for setting MACHINA_MACHINA_XLA_FLAGS prior to running a test.

  This function returns a decorator intended to be applied to test methods in
  a `tf.test.TestCase` class. Doing so will allow users to set any xla flags
  exposed via the MACHINA_MACHINA_XLA_FLAGS environment variable, execute the test, then reset
  the MACHINA_MACHINA_XLA_FLAGS to the state it was in prior to this test.
  ...
  Args:
    flag: The xla flag to be set in the MACHINA_MACHINA_XLA_FLAGS env variable.

  Returns:
    A decorator which sets the configured flag in MACHINA_MACHINA_XLA_FLAGS for the decorated
    function.
  """
  ...
  def decorator(f: _F) -> _F:
    ...
    def decorated(*args, **kwargs):
      original_xla_flags = os.environ.get("MACHINA_MACHINA_XLA_FLAGS")
      new_xla_flags = flag
      if original_xla_flags:
        new_xla_flags = new_xla_flags + " " + original_xla_flags
      os.environ["MACHINA_MACHINA_XLA_FLAGS"] = new_xla_flags
      try:
        return f(*args, **kwargs)
      finally:
        if original_xla_flags is None:
          del os.environ["MACHINA_MACHINA_XLA_FLAGS"]
        else:
          os.environ["MACHINA_MACHINA_XLA_FLAGS"] = original_xla_flags
...
class TensorFlowTestCase(googletest.TestCase):
  ...
  def _assertArrayLikeAllClose(self, a, b, rtol=1e-6, atol=1e-6, msg=None):
    ...
    if not np.allclose(a, b, rtol=rtol, atol=atol):
      ...
      # machina/compiler/tests:binary_ops_test pass with float32
  ...
  def assertDictEqual(self, a, b, msg=None):
    ...
    # assertDictEqual. If that fails we try the machina one.
    try:
...
def create_local_cluster(
  ...
  Read more at https://www.machina.org/guide/extend/architecture

  ![components](https://www.machina.org/images/diag1.svg "components")
