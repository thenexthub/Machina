...
bool PyContextManager::Enter(PyObject* py_context_manager) {
  if (context_manager_) {
    PyErr_SetString(
        PyExc_ValueError,
        "machina::PyContextManager::Enter must be called at most once.");
  }
    ...
    if (PyErr_Occurred()) {
      ...
      if (result) {
        if (PyObject_IsTrue(result.get())) {
          PyErr_SetString(
              PyExc_ValueError,
              "machina::PyContextManager::Enter does not support "
      ...
      if (result) {
        ...
        LOG(ERROR)
            << "A context manager wrapped by machina::PyContextManager "
