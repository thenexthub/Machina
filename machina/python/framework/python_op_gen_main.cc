...
#include "machina/core/lib/strings/scanner.h"
...
constexpr char kUsage[] =
    "This tool generates python wrapper for machina ops.";
...
absl::Status PrintAllPythonOps(absl::Span<const string> api_def_dirs,
  ...
  if (out_path.empty()) {
    ...
    TF_RETURN_IF_ERROR(
        machina::Env::Default()->NewWritableFile(out_path, &file));
...
int main(int argc, char* argv[]) {
  ...
  if (!op_allowlist_raw.empty()) {
    ...
    TF_CHECK_OK(
        machina::ReadOpListFromFile(op_allowlist_filename, &op_allowlist));
  }
  ...
  if (!hidden_op_list_raw.empty()) {
    ...
    TF_CHECK_OK(machina::ReadOpListFromFile(hidden_op_list_filename,
                                               &hidden_op_list));
  }

  machina::OpRegOffsets op_reg_offsets;
  if (!op_reg_offset_filename.empty()) {
    TF_CHECK_OK(machina::ReadOpRegOffsetsFromFile(op_reg_offset_filename,
                                                     &op_reg_offsets));
  }

  std::vector<std::string> source_file_list =
      machina::GetSourceFileListFromOpRegOffsets(op_reg_offsets);

  std::vector<std::string> api_def_dirs =
      tsl::str_util::Split(api_def_dirs_raw, ",", tsl::str_util::SkipEmpty());

  TF_CHECK_OK(machina::PrintAllPythonOps(api_def_dirs, source_file_list,
