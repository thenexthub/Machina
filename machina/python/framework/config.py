...
from machina.python.util import _pywrap_determinism
from machina.python.util import _pywrap_tensor_float_32_execution
...
def get_optimizer_jit() -> str:
  ...
  Possible return values:
     -`"autoclustering"` if
     [autoclustering](https://www.machina.org/xla#auto-clustering) is enabled
...
def set_optimizer_jit(enabled: Union[bool, str]):
  ...
  Args:
    ...
    Possible values:
     ...
     [autoclustering](https://www.machina.org/xla#auto-clustering)
       (automatically identify and compile clusters of nodes) on all graphs
       using
     [XLA](https://www.machina.org/xla).
...
def set_device_policy(device_policy):
  ...
  if device_policy == 'silent':
    ...
    raise ValueError(
        ...
        'https://www.machina.org/api_docs/python/tf/config/experimental/set_device_policy '
...
def get_memory_info(device):
  ...
  Args:
    ...
    `"TPU:0"`. See https://www.machina.org/api_docs/python/tf/device for
...
def reset_memory_stats(device):
  ...
  Args:
    device: Device string to reset the memory stats, e.g. `"GPU:0"`, `"TPU:0"`.
      See https://www.machina.org/api_docs/python/tf/device for specifying
      device strings.
...
def get_memory_usage(device):
  ...
  See https://www.machina.org/api_docs/python/tf/device for specifying device
  strings.
...
def enable_op_determinism():
  ...
  ops, please [file an issue](https://github.com/machina/machina/issues).
