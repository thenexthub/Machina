 ...
 *
 */
// Python bindings for machina/python/framework/python_api_dispatcher.h.

#include "absl/strings/str_cat.h"
#include "pybind11/pybind11.h"  // from @pybind11
#include "pybind11/pytypes.h"  // from @pybind11
#include "pybind11/stl.h"  // from @pybind11
#include "machina/python/framework/python_api_dispatcher.h"
#include "machina/python/lib/core/safe_pyobject_ptr.h"

namespace py = pybind11;

using machina::py_dispatch::PyInstanceChecker;
using machina::py_dispatch::PyListChecker;
using machina::py_dispatch::PySignatureChecker;
using machina::py_dispatch::PythonAPIDispatcher;
using machina::py_dispatch::PyTypeChecker;
using machina::py_dispatch::PyUnionChecker;
...
PythonAPIDispatcher MakePythonAPIDispatcher(
  ...
  if (defaults.ptr() == Py_None) {
    ...
    machina::Safe_PyObjectPtr fast_defaults(
...
PYBIND11_MODULE(_pywrap_python_api_dispatcher, m) {
  ...
  py::class_<PySignatureChecker>(m, "PySignatureChecker")
      ...
      .def("CheckCanonicalizedArgs",
           [](PySignatureChecker* self, py::tuple args) {
             machina::Safe_PyObjectPtr seq(PySequence_Fast(args.ptr(), ""));
  ...
  m.def("register_dispatchable_type", [](py::handle py_class) {
    if (!machina::py_dispatch::RegisterDispatchableType(py_class.ptr())) {
      throw py::error_already_set();
    } else {
      return py_class;
    }
