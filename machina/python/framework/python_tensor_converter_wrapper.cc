...
#include "machina/python/framework/python_tensor_converter.h"
...
py::handle Convert(machina::PythonTensorConverter* self, py::handle obj,
                   py::handle dtype) {
...
PYBIND11_MODULE(_pywrap_python_tensor_converter, m) {
  py::class_<machina::PythonTensorConverter>(m, "PythonTensorConverter")
      .def(py::init(&machina::MakePythonTensorConverter))
      .def("Convert", machina::Convert);
}
