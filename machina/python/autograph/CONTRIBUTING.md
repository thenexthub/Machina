...
`machina/contrib/autograph` to `machina/python/autograph`. The move
does not impact functionality, and AutoGraph will remain accessible under
`machina.contrib.autograph` until `machina.contrib` is retired.
...
bazel test --config=opt --copt=-O3 --copt=-march=native \
  //machina/contrib/autograph/...
```

from the root of the `machina` repository. For more details see the [main TensorFlow Contributing File](../../CONTRIBUTING.md)
