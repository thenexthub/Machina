...
DLDevice GetDlContext(TFE_TensorHandle* h, TF_Status* status) {
  ...
  const char* device_name =
      machina::unwrap(h)->BackingDeviceName(&status->status);
  DeviceNameUtils::ParsedName parsed_name;
  machina::DeviceNameUtils::ParseFullName(device_name, &parsed_name);
...
absl::Status TfDataTypeFormDlDataType(const DLDataType& dtype,
  ...
  switch (dtype.code) {
      ...
      switch (dtype.bits) {
        ...
        case 64:
          *tf_dtype = TF_DataType::TF_UINT64;
          return absl::OkStatus();
        default:
          return machina::errors::InvalidArgument("Unsupported UInt bits: ",
                                                     dtype.bits);
      }
      return absl::OkStatus();
      ...
      switch (dtype.bits) {
        ...
        case 64:
          *tf_dtype = TF_DataType::TF_INT64;
          return absl::OkStatus();
        default:
          return machina::errors::InvalidArgument("Unsupported Int bits: ",
                                                     dtype.bits);
      }
      return absl::OkStatus();
      ...
      switch (dtype.bits) {
        ...
        case 64:
          *tf_dtype = TF_DataType::TF_DOUBLE;
          return absl::OkStatus();
        default:
          return machina::errors::InvalidArgument("Unsupported Float bits: ",
                                                     dtype.bits);
      }
      break;
    ...
    case DLDataTypeCode::kDLComplex:
      ...
      break;
    default:
      return machina::errors::InvalidArgument("Unsupported Type Codes: ",
                                                 dtype.code);
  }
}
...
TFE_TensorHandle* TFE_HandleFromDLPack(void* dlm, TF_Status* status,
  ...
  if (!device_name.has_value()) {
    status->status =
        machina::errors::InvalidArgument("Unsupported Device Type");
    return nullptr;
  }
  TF_DataType dtype;
