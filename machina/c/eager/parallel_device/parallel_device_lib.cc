...
class DeviceThread {
 public:
  ...
  explicit DeviceThread(const std::string& device, const bool is_async,
      ...
      : status_(TF_NewStatus()),
        ...
        op_(nullptr),
        thread_(machina::Env::Default()->StartThread(
            machina::ThreadOptions(), "parallel_device_execute",
 ...
 private:
  ...
  machina::mutex execution_mutex_;
  ...
  machina::condition_variable start_execute_;
  // The worker thread notifies that work has finished.
  machina::condition_variable finished_execute_;
  // Notifies a StartExecute that the previous Join has finished.
  machina::condition_variable finished_join_;
...
DeviceThread::~DeviceThread() {
  {
    machina::mutex_lock l(execution_mutex_);
...
void DeviceThread::Execute(TFE_Context* context, const char* operation_name,
  ...
  if (step_id_.has_value()) {
    machina::unwrap(op_.get())->SetStepId(step_id_.value());
  }
