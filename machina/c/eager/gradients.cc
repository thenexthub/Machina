...
machina::DataType TapeTensor::GetDType() const {
...
void Tape::RecordOperation(absl::Span<AbstractTensorHandle* const> inputs,
  ...
  std::vector<machina::DataType> input_dtypes(inputs.size());
...
bool Tape::ShouldRecord(
  ...
  std::vector<machina::DataType> tensor_dtypes(tensors.size());
...
absl::Status Tape::ComputeGradient(
  ...
  machina::gtl::FlatSet<int64_t> sources_set(source_tensor_ids.begin(),
