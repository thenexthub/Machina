 ...
 *
 */
#ifndef MACHINA_C_EAGER_IMMEDIATE_EXECUTION_CONTEXT_H_
#define MACHINA_C_EAGER_IMMEDIATE_EXECUTION_CONTEXT_H_
...
// LINT.ThenChange(//machina/c/eager/c_api.h)
...
class ImmediateExecutionContext : public AbstractContext {
 public:
  ...
  // Return a list of local machina::Device*.
  // TODO(tfrt-devs): We shouldn't expose legacy device in this API.
  virtual std::vector<machina::Device*> ListLocalTfDevices() = 0;

  // Return a list of all machina::Device*.
  virtual std::vector<machina::Device*> ListAllTfDevices() = 0;
  ...
  virtual ImmediateExecutionTensorHandle* CreateLocalHandleFromTFTensor(
      machina::Tensor& t, const char* d_name) = 0;

  // Convert a TFRT TensorHandle to machina::TensorHandle.
...
#endif  // MACHINA_C_EAGER_IMMEDIATE_EXECUTION_CONTEXT_H_
