...
void TestFunctionWithPackedInput(const bool remote) {
  machina::ServerDef server_def = GetServerDef(3);
...
class GraphErrorInjectionPass : public machina::GraphOptimizationPass {
 public:
  static bool enabled_;
  GraphErrorInjectionPass() {}

  absl::Status Run(
      const machina::GraphOptimizationPassOptions& options) override {
    if (!enabled_) {
    ...
    return machina::errors::Internal("Graph pass runs for more than once!");
  }
...
TEST(CAPI, DistributedFunctionGraphPassOnlyOnce) {
  ...
  machina::optimization_registration::OptimizationPassRegistration
      register_test_pass(machina::OptimizationPassRegistry::PRE_PLACEMENT, 0,
...
string VariableAddFunctionWithGraphError() {
  ...
  machina::FunctionDef def;
  CHECK(machina::protobuf::TextFormat::ParseFromString(signature, &def));
  return def.SerializeAsString();
}

class FunctionErrorInjectionPass : public machina::FunctionOptimizationPass {
 public:
  ...
  absl::Status Run(const std::string& function_name,
    ...
    for (const auto node : graph->get()->nodes()) {
      if (node->name().find(error_node_) != string::npos &&
        ...
        return machina::errors::Internal("Injected graph pass error.");
      }
    }
    return absl::OkStatus();
  }
...
void TestDistributedFunctionCancellation(bool inject_error) {
  ...
  if (inject_error) {
    ...
    machina::function_optimization_registration::
...
void TestRemoteExecuteDeleteContextWithOutstandingRPC(bool async) {
  machina::ServerDef server_def = GetServerDef(2);
  ...
  std::unique_ptr<machina::GrpcServer> worker_server;
  ASSERT_TRUE(machina::GrpcServer::Create(
                  server_def, machina::Env::Default(), &worker_server)
                  .ok());
