...
void TensorHandleCopyBetweenDevices(bool async) {
  ...
  for (int i = 0; i < num_devices; ++i) {
    ...
    auto tag = machina::strings::StrCat("Device #", i, " (", name, ")");
    // Copy to device
...
void TensorHandleSilentCopy(bool async,
  ...
  if (GetDeviceName(ctx, &gpu_device_name, "GPU")) {
    ...
    auto cpu_arg =
        machina::TensorHandleFromInterface(machina::unwrap(hcpu));
    auto gpu_arg =
        machina::TensorHandleFromInterface(machina::unwrap(hgpu));
...
class TestUnavailableErrorOp : public machina::OpKernel {
 public:
  explicit TestUnavailableErrorOp(machina::OpKernelConstruction* ctx)
      : machina::OpKernel(ctx) {}
  void Compute(machina::OpKernelContext* ctx) override {
    ctx->SetStatus(machina::errors::Unavailable("Test error."));
  }
};
REGISTER_KERNEL_BUILDER(
    Name("TestNonCommUnavailable").Device(machina::DEVICE_DEFAULT),
    TestUnavailableErrorOp);
REGISTER_KERNEL_BUILDER(
    Name("TestCommUnavailable").Device(machina::DEVICE_DEFAULT),
    TestUnavailableErrorOp);
...
string FunctionWithErrorOp(const absl::string_view op_name) {
  const std::string& func_str =
      ...
      "      value: 'error_op:out'"
      "    }";
  machina::FunctionDef def;
  CHECK(machina::protobuf::TextFormat::ParseFromString(
      machina::str_util::StringReplace(func_str, "__OP_NAME__", op_name,
                                          /*replace_all=*/true),
      &def));
...
TEST(CAPI, TestTFE_SetOpAttrs) {
  ...
  machina::AttrValue i_list_values;
  for (int i = 0; i < 4; ++i) {
  ...
  machina::AttrValue padding_value;
  *padding_value.mutable_s() = "VALID";
  machina::SetOpAttrValueScalar(ctx, op, padding_value, "padding", status);

  machina::AttrValue data_format_value;
  *data_format_value.mutable_s() = "NHWC";
  machina::SetOpAttrValueScalar(ctx, op, data_format_value, "data_format",
                                   status);
...
machina::AttrValueMap ExtractAttrs(TFE_Op* op) {
  machina::AttrValueMap attr_values;
  machina::EagerOperation* operation =
      machina::OperationFromInterface(machina::unwrap(op));
...
TEST(CAPI, TestTFE_OpInferSingleInputAttrs) {
  ...
  machina::AttrValueMap attr_values = ExtractAttrs(minOp);
  machina::AttrValueMap::const_iterator attr_found = attr_values.find("T");
  EXPECT_NE(attr_found, attr_values.cend());
  EXPECT_EQ(attr_found->second.type(), machina::DataType::DT_FLOAT);
  attr_found = attr_values.find("Tidx");
  EXPECT_NE(attr_found, attr_values.cend());
  EXPECT_EQ(attr_found->second.type(), machina::DataType::DT_INT32);
...
TEST(CAPI, TestTFE_OpInferSingleTypeInputListAttrs) {
  ...
  machina::AttrValueMap attr_values = ExtractAttrs(concatOp);
  machina::AttrValueMap::const_iterator attr_found = attr_values.find("T");
  EXPECT_NE(attr_found, attr_values.cend());
  EXPECT_EQ(attr_found->second.type(), machina::DataType::DT_FLOAT);
...
TEST(CAPI, TestTFE_OpInferMixedTypeInputListAttrs) {
  ...
  machina::AttrValueMap attr_values = ExtractAttrs(assertOp);
  machina::AttrValueMap::const_iterator attr_found = attr_values.find("T");
  EXPECT_NE(attr_found, attr_values.cend());
  EXPECT_EQ(attr_found->second.list().type(0), machina::DataType::DT_BOOL);
  EXPECT_EQ(attr_found->second.list().type(1), machina::DataType::DT_FLOAT);
  EXPECT_EQ(attr_found->second.list().type(2), machina::DataType::DT_INT32);
...
TEST(CAPI, TestTFE_OpAttrsInferenceDisabledWhenNotCallingOpAddInputList) {
  ...
  TFE_OpAddInput(concatOp, dim, status);
  CHECK_EQ(TF_OK, TF_GetCode(status)) << TF_Message(status);
  CHECK(machina::unwrap(concatOp)->OpDef());
  TFE_OpAddInput(concatOp, inputs[0], status);
  CHECK_EQ(TF_OK, TF_GetCode(status)) << TF_Message(status);
  EXPECT_FALSE(machina::unwrap(concatOp)->OpDef())
...
void TestOpAddAttrs(bool use_tfrt) {
  ...
  machina::EagerOperation* op =
      machina::OperationFromInterface(machina::unwrap(copy_op));
  op->Attrs().FillAttrValueMap(&attr_values);
  EXPECT_EQ(machina::DT_FLOAT, attr_values.find("dtype")->second.type());
...
TEST(CAPI, TestTFE_OpAttrsSerialize) {
  ...
  machina::AttrValueMap attr_values;
  machina::EagerOperation* op =
      machina::OperationFromInterface(machina::unwrap(var_op_2));
  op->Attrs().FillAttrValueMap(&attr_values);
  EXPECT_EQ(machina::DT_INT64, attr_values.find("dtype")->second.type());
...
TEST(CAPI, ShareVariableAcrossContextsWorks) {
  ...
  machina::ServerDef server_def_0 = GetServerDef(3);
  server_def_0.mutable_default_session_config()->set_isolate_session_state(
      false);
  machina::ServerDef server_def_1 =
  ...
  ASSERT_TRUE(machina::GrpcServer::Create(
                  server_def_0, machina::Env::Default(), &worker_server1)
                  .ok());
  ASSERT_TRUE(worker_server1->Start().ok());
  server_def_0.set_task_index(2);
  std::unique_ptr<machina::GrpcServer> worker_server2;
  ASSERT_TRUE(machina::GrpcServer::Create(
                  server_def_0, machina::Env::Default(), &worker_server2)
                  .ok());
...
machina::ServerDef CreateSingleHostServerDef(
    const machina::ServerDef& cluster_server_def, int task_index) {
  machina::ServerDef single_host_server_def;
  ...
  machina::ClusterDef* cluster_def =
  ...
  job_def->mutable_tasks()->insert(
      {0, machina::strings::StrCat(
              "localhost:", machina::testing::PickUnusedPortOrDie())});

  machina::JobDef* job_def2 = cluster_def->add_job();
...
machina::ServerDef GetClusterServerDef(const string& worker_job_name,
                                          int num_workers) {
  machina::ServerDef server_def = GetServerDef(worker_job_name, num_workers);
...
TEST(CAPI, SingleHostServerDefV1Works) {
  ...
  //
  machina::ServerDef cluster_server_def = GetClusterServerDef("worker", 2);
  ...
  machina::ServerDef worker_1_server_def =
  ...
  std::unique_ptr<machina::GrpcServer> worker_server1;
  ASSERT_TRUE(machina::GrpcServer::Create(worker_1_server_def,
                                             machina::Env::Default(),
                                             &worker_server1)
                  .ok());
  ASSERT_TRUE(worker_server1->Start().ok());
  ...
  machina::ServerDef worker_0_server_def =
      CreateSingleHostServerDef(cluster_server_def, 0);
  worker_0_server_def.set_task_index(0);

  std::unique_ptr<machina::GrpcServer> worker_server0;
  ASSERT_TRUE(machina::GrpcServer::Create(worker_0_server_def,
                                             machina::Env::Default(),
                                             &worker_server0)
                  .ok());
...
TEST(CAPI, SingleHostServerDefV2Works) {
  ...
  std::unique_ptr<machina::GrpcServer> worker_server0;
  ASSERT_TRUE(machina::GrpcServer::Create(cluster_server_def,
                                             machina::Env::Default(),
                                             &worker_server0)
                  .ok());
  ASSERT_TRUE(worker_server0->Start().ok());
