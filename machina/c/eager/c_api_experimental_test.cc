...
TEST(CAPI, MonitoringMultipleSampler) {
  ...
  machina::HistogramProto histogram1;
  ...
  machina::HistogramProto histogram2;
...
TEST(CAPI, Function_ident_CPU) {
  ...
  for (bool async : {false, true, false}) {
    ...
    TF_Tensor* t =
        TF_AllocateTensor(TF_INT32, nullptr, 0, 1 * sizeof(machina::int32));
    *reinterpret_cast<machina::int32*>(TF_TensorData(t)) = 42;
    ...
    ASSERT_TRUE(TF_GetCode(status) == TF_OK) << TF_Message(status);
    EXPECT_EQ(*reinterpret_cast<machina::int32*>(TF_TensorData(r)), 42);
...
TEST(CAPI, CreateLocalContextAsReset) {
  machina::ServerDef server_def = GetServerDef("worker", 2);
  ...
  int client_port = machina::testing::PickUnusedPortOrDie();
  ...
  int worker_port = machina::testing::PickUnusedPortOrDie();
  job_def->mutable_tasks()->at(0) =
      machina::strings::StrCat("localhost:", worker_port);
