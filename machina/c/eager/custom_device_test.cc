...
TEST(CUSTOM_DEVICE, ResetOperation) {
  ...
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
  ASSERT_EQ(machina::string(TFE_OpGetDevice(reused_op.get(), status.get())),
            machina::string(custom_device_name));
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
  TFE_OpReset(reused_op.get(), "Identity",
              "/job:localhost/replica:0/task:0/device:CPU:0", status.get());
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
  ASSERT_EQ(machina::string(TFE_OpGetDevice(reused_op.get(), status.get())),
            machina::string("/job:localhost/replica:0/task:0/device:CPU:0"));
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
}
...
TEST(CUSTOM_DEVICE, MakeVariable) {
  ...
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
  ASSERT_TRUE(executed);
  auto handle_cleaner = machina::gtl::MakeCleanup(
  ...
  ASSERT_TRUE(TF_GetCode(status.get()) == TF_OK) << TF_Message(status.get());
  ASSERT_TRUE(executed);
  auto value_cleaner = machina::gtl::MakeCleanup(
      [var_value]() { TFE_DeleteTensorHandle(var_value); });
  ASSERT_EQ(machina::string(name),
            machina::string(
...
TEST(CUSTOM_DEVICE, AccessVariableOnCustomDevice) {
  ...
  ASSERT_EQ(
      machina::string(name),
      machina::string(TFE_TensorHandleDeviceName(var_value, status.get())));
