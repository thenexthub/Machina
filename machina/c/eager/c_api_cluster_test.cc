...
void TestRemoteExecuteChangeServerDef(bool async) {
  ...
  // localhost).
  machina::ServerDef updated_server_def = GetServerDef("worker", 2);
  serialized = updated_server_def.SerializeAsString();

  updated_server_def.set_task_index(1);
  absl::Status s = machina::GrpcServer::Create(
      updated_server_def, machina::Env::Default(), &worker_server);
...
void TestRemoteExecuteUpdateServerDefWithFailures(bool async) {
  ...
  machina::setenv("GRPC_FAIL_FAST", "TRUE", /*overwrite=*/1);
  ...
  job_def->mutable_tasks()->insert(
      {2, machina::strings::StrCat("localhost:", port)});
  server_def.set_task_index(0);
  string serialized_update = server_def.SerializeAsString();
  TFE_ContextUpdateServerDef(ctx, 0, serialized_update.data(),
                             serialized_update.size(), status);
  ...
  // TODO(b/136478427): Figure out how to correctly shut the server down.
  worker_server.release();
  machina::unsetenv("GRPC_FAIL_FAST");
}
...
void TestConnectToCluster(bool keep_localhost_for_first_connect) {
  ...
  machina::ServerDef server_def = GetServerDef(first_name, 1);
  ...
  absl::Status status2;
  EXPECT_EQ(machina::unwrap(var_handle0)->DeviceName(&status2), dev0_name);
  ...
  TFE_TensorHandle* var_handle1 = TestVariable(ctx, 2.0, dev1_name);
  EXPECT_NE(var_handle1, nullptr);
  EXPECT_EQ(machina::unwrap(var_handle1)->DeviceName(&status2), dev1_name);
  ...
  (*server_def.mutable_cluster()->mutable_job(0)->mutable_tasks())[0] =
      absl::StrCat(second_name, ":",
                   machina::testing::PickUnusedPortOrDie());
  ...
  EXPECT_EQ(machina::unwrap(var_handle2)->DeviceName(&status2), dev2_name);
