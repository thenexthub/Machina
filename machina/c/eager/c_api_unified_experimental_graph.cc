...
#include "machina/c/eager/graph_function.h"
...
class GraphTensor : public TracingTensorHandle {
 public:
  ...
  machina::DataType DataType() const override {
    return static_cast<machina::DataType>(TF_OperationOutputType(output_));
  }
  ...
  absl::Status Shape(machina::PartialTensorShape* shape) const override {
    ...
    TF_GraphGetTensorShape(graph_, output_,
                           ...
                           &status);
    TF_RETURN_IF_ERROR(StatusFromTF_Status(&status));
    TF_RETURN_IF_ERROR(machina::TensorShapeUtils::MakeShape(dims, shape));
...
class GraphContext : public TracingContext {
 public:
  ...
  absl::Status AddParameter(DataType dtype, const PartialTensorShape& shape,
    ...
    *output = machina::down_cast<TracingTensorHandle*>(outputs[0]);
    return absl::OkStatus();
  }
