 ...
 *
 */
#ifndef MACHINA_C_EAGER_TAPE_H_
#define MACHINA_C_EAGER_TAPE_H_
...
// without using machina python code, hence left unspecified here.
//
...
class GradientTape {
 public:
  ...
  bool ShouldRecord(absl::Span<const int64_t> tensor_ids,
                    absl::Span<const machina::DataType> dtypes) const;
...
class ForwardAccumulator {
 public:
  ...
  bool ShouldRecord(absl::Span<const int64_t> tensor_ids,
                    absl::Span<const machina::DataType> dtypes);
...
GradientTape<Gradient, BackwardFunction, TapeTensor>::ComputeGradient(
  ...
  if (!state.op_tape.empty()) {
    return machina::errors::Internal("Invalid tape state.");
  }
...
ForwardAccumulator<Gradient, BackwardFunction, TapeTensor>::ForwardpropFromTape(
  ...
  */
  ...
  for (const TapeTensor& output_tensor : output_tensors) {
    ...
    Gradient* aid;
    if (output_tensor.GetDType() == machina::DT_VARIANT) {
