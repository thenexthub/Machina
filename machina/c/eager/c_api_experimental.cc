...
#include "machina/core/distributed_runtime/coordination/coordination_service_error_util.h"
...
void TFE_OpReset(TFE_Op* op_to_reset, const char* op_or_function_name,
  ...
  if (op_to_reset) {
    machina::ImmediateExecutionOperation* op =
        machina::unwrap(op_to_reset);
    op->Clear();
...
const void TFE_MonitoringStringGaugeCellValue(
  ...
  machina::string value = cell->cell.value();
  void* data = machina::port::Malloc(value.length());
...
void TFE_MonitoringSamplerCellValue(TFE_MonitoringSamplerCell* cell,
  ...
  void* data = machina::port::Malloc(content.length());
...
TFE_MonitoringBuckets* TFE_MonitoringNewExponentialBuckets(double scale,
  ...
  return new TFE_MonitoringBuckets([scale, growth_factor, bucket_count]() {
    return machina::monitoring::Buckets::Exponential(scale, growth_factor,
                                                        bucket_count);
  });
}
...
TFE_CancellationManager* TFE_NewCancellationManager() {
  return machina::wrap(new machina::CancellationManager);
}

void TFE_CancellationManagerStartCancel(
    TFE_CancellationManager* cancellation_manager) {
  machina::unwrap(cancellation_manager)->StartCancel();
}

bool TFE_CancellationManagerIsCancelled(
    TFE_CancellationManager* cancellation_manager) {
  return machina::unwrap(cancellation_manager)->IsCancelled();
}

bool TFE_CancellationManagerIsCancelling(
    TFE_CancellationManager* cancellation_manager) {
  return machina::unwrap(cancellation_manager)->IsCancelling();
}

TFE_CancellationToken TFE_CancellationManagerGetToken(
    TFE_CancellationManager* cancellation_manager) {
  return machina::unwrap(cancellation_manager)->get_cancellation_token();
}
...
bool TFE_CancellationManagerRegisterCallback(
  ...
  machina::CancelCallback callback = [callback = c_callback->callback,
                                         context = c_callback->context]() {
    callback(context);
  };
  return machina::unwrap(cancellation_manager)
...
bool TFE_CancellationManagerDeregisterCallback(
  ...
  return machina::unwrap(cancellation_manager)->DeregisterCallback(token);
}

bool TFE_CancellationManagerTryDeregisterCallback(
    TFE_CancellationManager* cancellation_manager,
    TFE_CancellationToken token) {
  return machina::unwrap(cancellation_manager)->TryDeregisterCallback(token);
}

void TFE_DeleteCancellationManager(
    TFE_CancellationManager* cancellation_manager) {
  delete machina::unwrap(cancellation_manager);
}

void TFE_OpSetCancellationManager(TFE_Op* op,
                                  TFE_CancellationManager* cancellation_manager,
                                  TF_Status* status) {
  machina::unwrap(op)->SetCancellationManager(
      machina::unwrap(cancellation_manager));
...
void TFE_ContextSetExecutorForThread(TFE_Context* ctx, TFE_Executor* executor) {
  machina::unwrap(ctx)->SetExecutorForThread(executor->executor());
}

TFE_Executor* TFE_ContextGetExecutorForThread(TFE_Context* ctx) {
  return new TFE_Executor(&machina::unwrap(ctx)->Executor());
}

void TFE_HostAddressSpace(TFE_Context* ctx, TF_Buffer* buf) {
  auto address_space = machina::DeviceNameUtils::AddressSpace(
      machina::unwrap(ctx)->HostCPUParsedName());
  auto str = machina::DeviceNameUtils::ParsedNameToString(address_space);
  void* data = machina::port::Malloc(str.length());
...
void TFE_ContextGetFunctionDef(TFE_Context* ctx, const char* function_name,
  ...
  auto* function_def = machina::unwrap(ctx)->FindFunctionDef(function_name);
...
void TFE_ContextGetGraphDebugInfo(TFE_Context* ctx, const char* function_name,
                                  TF_Buffer* buf, TF_Status* status) {
  auto function_record = machina::unwrap(ctx)->FindRecord(function_name);
  ...
  machina::GraphDebugInfo debug_info =
      machina::StackTracesMapToGraphDebugInfo(
...
TF_Tensor* TFE_AllocateHostTensor(TFE_Context* ctx, TF_DataType dtype,
  ...
  if (ctx == nullptr) {
    status->status = machina::errors::InvalidArgument("Invalid Context");
    return nullptr;
  }

  machina::AbstractTensorInterface* t =
      machina::unwrap(ctx)->CreateTensor(
          static_cast<machina::DataType>(dtype), dimvec);

  if (t == nullptr) {
    status->status =
        machina::errors::InvalidArgument("Unsupported dtype: ", dtype);
    return nullptr;
  }
...
TFE_TensorHandle* TFE_NewTensorHandleFromTensor(TFE_Context* ctx, TF_Tensor* t,
                                                TF_Status* status) {
  return machina::wrap(
      machina::unwrap(ctx)->CreateLocalHandle(t->tensor));
}
  ...
  for (int i = 0; i < *num_handles; ++i) {
    ...
    if (machina::CustomDeviceTensorHandle::classof(unwrapped_handle)) {
      ...
      auto* custom_device_handle =
          machina::down_cast<machina::CustomDeviceTensorHandle*>(
              unwrapped_handle);
      machina::ImmediateExecutionTensorHandle* result;
      status->status = custom_device_handle->device()->Pack(
          absl::Span<machina::ImmediateExecutionTensorHandle*>(
              machina::unwrap(handles), *num_handles),
          &result);
...
const char* TFE_TensorHandleDeviceType(TFE_TensorHandle* h, TF_Status* status) {
  ...
  return machina::unwrap(h)->DeviceType(&status->status);
}
...
void TFE_GetExecutedOpNames(TFE_Context* ctx, TF_Buffer* buf,
  ...
  void* data = machina::port::Malloc(op_names_str.length());
...
void TFE_SetLogicalCpuDevices(TFE_Context* ctx, int num_cpus,
  ...
  machina::SessionOptions sess_options;
  (*sess_options.config.mutable_device_count())["CPU"] = num_cpus;
  status->status =
      machina::DeviceFactory::AddCpuDevices(sess_options, prefix, &devices);
  ...
  status->status = machina::unwrap(ctx)->AddDevices(std::move(devices));
}

void TFE_InsertConfigKeyValue(TFE_Context* ctx, const char* key,
                              const char* value, TF_Status* status) {
  machina::ImmediateExecutionDistributedManager* dist_mgr =
      machina::unwrap(ctx)->GetDistributedManager();
  tsl::CoordinationServiceAgent* coord_agent =
      dist_mgr->GetCoordinationServiceAgent();
  if (coord_agent == nullptr) {
    status->status = machina::errors::FailedPrecondition(
...
void TFE_GetConfigKeyValue(TFE_Context* ctx, const char* key,
  ...
  void* data = machina::port::Malloc(value_string.length());
...
void TFE_GetTaskStates(TFE_Context* ctx, const TF_Buffer& tasks, void* states,
  ...
  std::vector<machina::CoordinatedTask> task_vec(tasks.length);
  auto* task_iter = static_cast<const machina::CoordinatedTask*>(tasks.data);
  ...
  for (size_t i = 0; i < tasks.length; ++i) {
    ...
    if (TF_GetCode(&s) != TF_Code::TF_OK) {
      machina::CoordinationServiceError error;
      *error.mutable_source_task() = result.error_payload().source_task();
      TF_SetPayload(&s, machina::CoordinationErrorPayloadKey().data(),
