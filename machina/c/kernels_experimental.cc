...
#include "machina/core/kernels/tensor_list_util.h"
#include "machina/core/kernels/variant_ops_util.h"
...
using machina::AllocatorAttributes;
using machina::mutex_lock;
using machina::ResourceBase;
using machina::Status;
using machina::Tensor;
using machina::TF_TensorFromTensor;
using machina::Var;
using machina::Variant;
using machina::errors::InvalidArgument;

struct TF_VariableInputLockHolder {
  TF_VariableInputLockHolder(
      std::vector<machina::Var*> vars,
      std::unique_ptr<std::vector<machina::mutex_lock>> locks,
      std::unique_ptr<std::vector<machina::tf_shared_lock>> shared_locks)
      : vars(std::move(vars)),
...
absl::Status EnsureSparseVariableAccess(
    ...
    void (*copyFunc)(TF_OpKernelContext* ctx, TF_Tensor* source,
                     TF_Tensor* dest),
    machina::Var* var, bool lock_held = false) {
  auto* context = reinterpret_cast<::machina::OpKernelContext*>(ctx);
...
machina::mutex* GetTrainingVariableMutex(TF_OpKernelContext* ctx,
                                            int32_t input,
                                            machina::Var** maybe_resource) {
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  *maybe_resource = nullptr;
  if (cc_ctx->input_dtype(input) == machina::DT_RESOURCE) {
    if (LookupResource(cc_ctx, HandleFromInput(cc_ctx, input), maybe_resource)
            .ok()) {
      return (*maybe_resource)->mu();
    } else {
      cc_ctx->CtxFailureWithWarning(
          machina::errors::Internal("Invalid variable reference."));
      return nullptr;
    }
  }
...
void TF_AssignVariable(TF_OpKernelContext* ctx, int input_index,
                       ...
                       TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  machina::core::RefCountPtr<machina::Var> variable;
  const machina::Tensor& value = cc_ctx->input(value_index);
  OP_REQUIRES_OK(cc_ctx, machina::LookupOrCreateResource<machina::Var>(
                             cc_ctx, HandleFromInput(cc_ctx, input_index),
                             &variable, [&value](machina::Var** ptr) {
                               *ptr = new machina::Var(value.dtype());
                               *(*ptr)->tensor() = value;
                               (*ptr)->is_initialized = true;
                               return absl::OkStatus();
                             }));
  machina::mutex_lock ml(*variable->mu());
...
void TF_AssignRefVariable(TF_OpKernelContext* ctx, int input_ref_index,
  ...
  auto copy = [copyFunc, ctx](::machina::OpKernelContext* cc_ctx,
                              ::machina::Tensor* lhs,
                              const ::machina::Tensor& rhs) {
    absl::Status s;
  ...
  ::machina::AssignRefVariable(cc_ctx, input_ref_index, output_ref_index,
...
void TF_AssignUpdateVariable(TF_OpKernelContext* ctx, int input_index,
                             ...
                             TF_Status* tf_status) {
  auto* context = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  machina::core::RefCountPtr<Var> variable;
  Status status =
...
std::string TemporaryVariableName(
    ...
    const machina::FrameAndIter& control_frame) {
  if (control_frame.frame_id != machina::kIllegalFrameId &&
      control_frame.iter_id != machina::kIllegalIterId) {
    return machina::strings::StrCat(var_name,
...
void TF_TemporaryVariable(TF_OpKernelContext* ctx, TF_DataType dtype,
                          ...
                          TF_Status* tf_status) {
  auto* context = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  machina::ResourceMgr* rm = context->resource_manager();
  ...
  std::unique_ptr<TF_Tensor, decltype(&TF_DeleteTensor)> tmp_var_tf(
      machina::TF_TensorFromTensor(tmp_var->val, &s), TF_DeleteTensor);
  OP_REQUIRES_OK(context, s);
  allocFunc(ctx, tmp_var_tf.get(), dtype, dims, num_dims, tf_status);
  s = machina::StatusFromTF_Status(tf_status);
...
void TF_MaybeLockVariableInputMutexesInOrder(
  ...
  for (auto i : input_ids) {
    if (cc_ctx->input_dtype(i) == machina::DT_RESOURCE) {
      any_resource = true;
      break;
    }
  }
  ...
  std::vector<machina::Var*> vars;
  std::vector<machina::mutex*> mutexes;
  std::vector<int32_t> acquire_order;
  for (auto input : input_ids) {
    machina::Var* var;
    machina::mutex* mutex = GetTrainingVariableMutex(ctx, input, &var);
  ...
  auto locks = std::make_unique<std::vector<machina::mutex_lock>>();
  auto shared_locks =
      std::make_unique<std::vector<machina::tf_shared_lock>>();
  locks->reserve(acquire_order.size());

  for (auto acquire : acquire_order) {
    machina::mutex* mu = mutexes[acquire];
    if (mu != nullptr) {
      if (do_lock) {
...
void TF_GetInputTensorFromVariable(TF_OpKernelContext* ctx, int input,
  ...
  auto status_setter = ::machina::gtl::MakeCleanup([cc_ctx, status]() {
    ::machina::Set_TF_Status_from_Status(status, cc_ctx->status());
  });

  absl::Status s;
  if (cc_ctx->input_dtype(input) == machina::DT_RESOURCE) {
    machina::core::RefCountPtr<machina::Var> var;
    OP_REQUIRES_OK(
    ...
    if (sparse) {
      OP_REQUIRES_OK(cc_ctx, EnsureSparseVariableAccess(ctx, isVariantType,
                                                        copyFunc, var.get()));
      *out = ::machina::TF_TensorFromTensor(*var->tensor(), &s);
      OP_REQUIRES_OK(cc_ctx, s);
      return;
    }
  ...
  *out = ::machina::TF_TensorFromTensor(
...
void TF_OpKernelContext_ForwardRefInputToRefOutput(TF_OpKernelContext* ctx,
  ...
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  if (cc_ctx->input_dtype(input_index) != machina::DT_RESOURCE) {
...
void TF_ReleaseVariableInputLockHolder(TF_VariableInputLockHolder* lockHolder) {
  if (lockHolder != nullptr) {
    ...
    for (machina::Var* var : lockHolder->vars) {
      var->Unref();
    }
    delete lockHolder;
  }
...
void TF_GetInputByName(TF_OpKernelContext* ctx, const char* inputName,
  ...
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  const ::machina::Tensor* cc_tensor = nullptr;
  absl::Status s = cc_ctx->input(inputName, &cc_tensor);

  if (!s.ok()) {
    ::machina::Set_TF_Status_from_Status(status, s);
    return;
  }
  TF_Tensor* result =
      ::machina::TF_TensorFromTensor(*cc_tensor, &status->status);
...
void TF_OpKernelConstruction_GetAttrTensorShape(TF_OpKernelConstruction* ctx,
  ...
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelConstruction*>(ctx);
...
static Status ValidateVariantType(const Variant& variant) {
  if (variant.get<T>() == nullptr) {
    const std::string type_index_name =
        ::machina::port::MaybeAbiDemangle(variant.TypeId().name());
...
static Status VariantBinaryAddFunc(
    ::machina::OpKernelContext* cc_ctx, const Variant& a, const Variant& b,
    Variant* out,
    void (*binary_add_func)(TF_OpKernelContext* ctx, TF_Tensor* a, TF_Tensor* b,
                            TF_Tensor* out));

static Status CCBinaryAddFunc(
    ::machina::OpKernelContext* cc_ctx, const Tensor& cc_a,
    const Tensor& cc_b, Tensor* cc_out,
    void (*binary_add_func)(TF_OpKernelContext* ctx, TF_Tensor* a, TF_Tensor* b,
                            TF_Tensor* out)) {
  if (cc_a.dtype() == ::machina::DT_INVALID) {
    *cc_out = cc_b;
    return absl::OkStatus();
  }
  if (cc_b.dtype() == ::machina::DT_INVALID) {
    *cc_out = cc_a;
    return absl::OkStatus();
  }
  ...
  ::machina::AllocatorAttributes attr;
  if (cc_a.dtype() == ::machina::DT_VARIANT) {
    attr.set_on_host(true);
  }
...
static Status VariantBinaryAddFunc(
    ...
    void (*binary_add_func)(TF_OpKernelContext* ctx, TF_Tensor* a, TF_Tensor* b,
                            TF_Tensor* out)) {
  auto cc_binary_add = [binary_add_func](::machina::OpKernelContext* cc_ctx,
  ...
  if (a.TypeId() == machina::TypeIndex::Make<::machina::TensorList>()) {
    TF_RETURN_IF_ERROR(ValidateVariantType<::machina::TensorList>(a));
    *out = ::machina::TensorList();

    return ::machina::TensorListBinaryAdd(
        cc_ctx, *a.get<::machina::TensorList>(),
        *b.get<::machina::TensorList>(),
        out->get<::machina::TensorList>(), cc_binary_add);
  } else if (a.TypeId() == machina::TypeIndex::Make<
                               ::machina::data::OptionalVariant>()) {
    TF_RETURN_IF_ERROR(
        ValidateVariantType<::machina::data::OptionalVariant>(a));
    *out = ::machina::data::OptionalVariant();

    return ::machina::data::OptionalBinaryAdd(
        cc_ctx, *a.get<::machina::data::OptionalVariant>(),
        *b.get<::machina::data::OptionalVariant>(),
        out->get<::machina::data::OptionalVariant>(), cc_binary_add);
  }

  const std::string type_index_name =
      ::machina::port::MaybeAbiDemangle(a.TypeId().name());
...
void TF_AddNVariant(TF_OpKernelContext* ctx,
  ...
  auto binary_add_variant =
      [binary_add_func](::machina::OpKernelContext* cc_ctx, const Variant& a,
                        const Variant& b, Variant* out) {
        return VariantBinaryAddFunc(cc_ctx, a, b, out, binary_add_func);
      };
  ::machina::AddNVariant(cc_ctx, binary_add_variant);
  ::machina::Set_TF_Status_from_Status(status, cc_ctx->status());
}

static Status ZerosLikeVariant(::machina::OpKernelContext* cc_ctx,
                               const Variant& input, Variant* out,
                               void (*zeros_like_func)(TF_OpKernelContext* ctx,
                                                       TF_Tensor* input,
                                                       TF_Tensor* out)) {
  auto cc_zeros_like_func = [zeros_like_func](
                                ::machina::OpKernelContext* cc_ctx,
                                const Tensor& cc_input, Tensor* cc_out) {
    AllocatorAttributes attr;
    if (cc_input.dtype() == ::machina::DT_VARIANT) {
      attr.set_on_host(true);
    }
    ...
    switch (cc_input.dtype()) {
      case ::machina::DT_INVALID: {
        *cc_out = Tensor(::machina::DT_INVALID);
        break;
      }
  ...
  if (input.TypeId() ==
      machina::TypeIndex::Make<::machina::TensorList>()) {
    TF_RETURN_IF_ERROR(ValidateVariantType<::machina::TensorList>(input));
    *out = ::machina::TensorList();

    return ::machina::TensorListZerosLike(
        cc_ctx, *input.get<::machina::TensorList>(),
        out->get<::machina::TensorList>(), cc_zeros_like_func);
  } else if (input.TypeId() == machina::TypeIndex::Make<
                                   ::machina::data::OptionalVariant>()) {
    TF_RETURN_IF_ERROR(
        ValidateVariantType<::machina::data::OptionalVariant>(input));
    *out = ::machina::data::OptionalVariant();

    return ::machina::data::OptionalZerosLike(
        cc_ctx, *input.get<::machina::data::OptionalVariant>(),
        out->get<::machina::data::OptionalVariant>(), cc_zeros_like_func);
  }

  const std::string type_index_name =
      ::machina::port::MaybeAbiDemangle(input.TypeId().name());
...
void TF_ZerosLikeVariant(TF_OpKernelContext* ctx,
  ...
  Tensor out(::machina::cpu_allocator(numa_node), ::machina::DT_VARIANT,
             ::machina::TensorShape({}));
  Variant* out_v = &(out.scalar<Variant>()());
  Status cc_status = ZerosLikeVariant(cc_ctx, v, out_v, zeros_like_func);
  ::machina::Set_TF_Status_from_Status(status, cc_status);
