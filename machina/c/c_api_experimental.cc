...
// struct TF_Operation { machina::Node node; };
...
void TF_EnableXLACompilation(TF_SessionOptions* options, unsigned char enable) {
  machina::ConfigProto& config = options->options.config;
  auto* optimizer_options =
      config.mutable_graph_options()->mutable_optimizer_options();
  if (enable) {
    optimizer_options->set_global_jit_level(machina::OptimizerOptions::ON_1);

    // These XLA flags are needed to trigger XLA properly from C (more generally
    // non-Python) clients. If this API is called again with `enable` set to
    // false, it is safe to keep these flag values as is.
    machina::MarkForCompilationPassFlags* flags =
        machina::GetMarkForCompilationPassFlags();
    flags->tf_xla_cpu_global_jit = true;
    flags->tf_xla_min_cluster_size = 1;
  } else {
    optimizer_options->set_global_jit_level(machina::OptimizerOptions::OFF);
  }
}

unsigned char TF_SetXlaEnableLazyCompilation(unsigned char enable) {
  machina::BuildXlaOpsPassFlags* flags =
      machina::GetBuildXlaOpsPassFlags();
...
void TF_SetXlaAutoJitMode(const char* mode) {
  machina::SetXlaAutoJitFlagFromFlagString(mode);
}

unsigned char TF_GetXlaAutoJitEnabled() {
  machina::XlaAutoJitFlag flag =
      machina::GetMarkForCompilationPassFlags()->xla_auto_jit_flag;
...
unsigned char TF_GetXlaConstantFoldingDisabled() {
  return static_cast<unsigned char>(
      machina::GetBuildXlaOpsPassFlags()->tf_xla_disable_constant_folding);
}
...
TF_Buffer* TF_CreateRunOptions(unsigned char enable_full_trace) {
  machina::RunOptions options;
  if (enable_full_trace) {
    options.set_trace_level(machina::RunOptions::FULL_TRACE);
  } else {
    options.set_trace_level(machina::RunOptions::NO_TRACE);
  }
  TF_Buffer* ret = TF_NewBuffer();
...
static std::vector<UniqueFuncPtr> CreateFunctionsFromTextProto(
  ...
  machina::GraphDef gdef;
  if (!machina::protobuf::TextFormat::ParseFromString(text_proto, &gdef)) {
...
TF_Tensor* TF_DequeueNamedTensor(TF_Session* session, int tensor_id,
  ...
  assert(session);
  {
    machina::mutex_lock c(session->graph->mu);
  ...
  TF_Operation* dequeue_op = TF_GraphOperationByName(
      session->graph,
      machina::strings::StrCat("fifo_queue_dequeue_", tensor_id).c_str());
  ...
  if (VLOG_IS_ON(1) && status->status.ok()) {
    ...
    if (machina::TF_TensorToTensor(ret, &tensor).ok()) {
...
void TF_EnqueueNamedTensor(TF_Session* session, int tensor_id,
  ...
  assert(session);
  {
    ...
    if (VLOG_IS_ON(1)) {
      VLOG(1) << "Enqueuing named tensor with id " << tensor_id
              << ", with input graph: "
              << session->graph->graph.ToGraphDefDebug().DebugString();
      machina::Tensor internal_tensor;
      if (machina::TF_TensorToTensor(tensor, &internal_tensor).ok()) {
  ...
  TF_Operation* enqueue_op = TF_GraphOperationByName(
      session->graph,
      machina::strings::StrCat("fifo_queue_enqueue_", tensor_id).c_str());
  ...
  TF_Operation* placeholder_op = TF_GraphOperationByName(
      session->graph,
      machina::strings::StrCat("arg_tensor_enqueue_", tensor_id).c_str());
...
TF_Buffer* TFE_GetServerDef(const char* text_proto, TF_Status* status) {
  ...
  if (!machina::protobuf::TextFormat::ParseFromString(text_proto,
                                                         &server_def)) {
    status->status = machina::errors::Internal(
...
void TF_MakeInternalErrorStatus(TF_Status* status, const char* errMsg) {
  status->status = machina::errors::Internal(errMsg);
}

struct TF_CheckpointReader : public machina::checkpoint::CheckpointReader {
  using machina::checkpoint::CheckpointReader::CheckpointReader;
...
TF_Tensor* TF_CheckpointReaderGetTensor(TF_CheckpointReader* reader,
  ...
  return machina::TF_TensorFromTensor(*tensor, &status->status);
}
...
struct TF_AttrBuilder : public machina::AttrBuilder {
  using machina::AttrBuilder::AttrBuilder;
...
void TF_AttrBuilderSetType(TF_AttrBuilder* builder, const char* attr_name,
  ...
  builder->Set(*iter, static_cast<machina::DataType>(value));
}

void TF_AttrBuilderSetTypeList(TF_AttrBuilder* builder, const char* attr_name,
                               const TF_DataType* values, int num_values) {
  auto iter = builder->attr_names.insert(attr_name).first;
  builder->Set(*iter, machina::gtl::ArraySlice<const machina::DataType>(
                          reinterpret_cast<const machina::DataType*>(values),
                          num_values));
}
...
void TF_AttrBuilderCheckCanRunOnDevice(TF_AttrBuilder* builder,
                                       ...
                                       TF_Status* status) {
  status->status = machina::FindKernelDef(
      machina::DeviceType(device_type), builder->BuildNodeDef(),
...
const char* TF_GetNumberAttrForOpListInput(const char* op_name, int input_index,
  ...
  status->status =
      machina::OpRegistry::Global()->LookUpOpDef(op_name, &op_def);
  if (!status->status.ok()) return nullptr;
  ...
  const machina::OpDef_ArgDef& input_arg = op_def->input_arg()[input_index];
...
int TF_OpIsStateful(const char* op_type, TF_Status* status) {
  ...
  status->status =
      machina::OpRegistry::Global()->LookUp(op_type, &op_reg_data);
  if (!status->status.ok()) {
    return 0;
  }
...
void TF_InitMain(const char* usage, int* argc, char*** argv) {
  machina::port::InitMain(usage, argc, argv);
}

int TF_PickUnusedPortOrDie() {
  return machina::internal::PickUnusedPortOrDie();
}

TFE_TensorHandle* TFE_NewTensorHandleFromScalar(TF_DataType data_type,
                                                void* data, size_t len,
                                                TF_Status* status) {
  auto dtype = static_cast<machina::DataType>(data_type);
  DCHECK(machina::DataTypeCanUseMemcpy(dtype));

  machina::Tensor tensor(dtype, machina::TensorShape({}));
  std::memcpy(machina::TensorCApi::Buffer(tensor)->data(), data, len);
...
TF_CAPI_EXPORT extern void TFE_EnableCollectiveOps(TFE_Context* ctx,
  ...
  status->status = machina::unwrap(ctx)->EnableCollectiveOps(server_def);
}
...
TF_Library* TF_LoadPluggableDeviceLibrary(const char* library_filename,
  ...
  static machina::mutex mu(machina::LINKER_INITIALIZED);
  static std::unordered_map<std::string, void*>* loaded_libs =
      new std::unordered_map<std::string, void*>();
  machina::Env* env = machina::Env::Default();
  {
    machina::mutex_lock lock(mu);
    ...
    if (it != loaded_libs->end()) {
      ...
      if (status->status.ok()) {
        TF_CHECK_OK(
            machina::RegisterPluggableDevicePlugin(lib_handle->lib_handle));
      } else {
        delete lib_handle;
        return nullptr;
      }
