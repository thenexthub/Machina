...
#include "machina/c/c_test_util.h"
...
TEST(CAPI, Tensor) {
  ...
  float* values =
      reinterpret_cast<float*>(machina::cpu_allocator()->AllocateRaw(
...
TEST(CAPI, MalformedTensor) {
  // See https://github.com/machina/machina/issues/7394
...
#if !defined(MACHINA_NO_SHARED_OBJECTS)
  {
    // Load the library.
    TF_Status* status = TF_NewStatus();
    string lib_path = machina::GetDataDependencyFilepath(
        machina::io::JoinPath("machina", "c", "test_op1.so"));
...
#endif  // !defined(MACHINA_NO_SHARED_OBJECTS)
  {
    ...
    typedef machina::protobuf::RepeatedPtrField<machina::OpDef> OpDefs;
...
void TestEncodeDecode(int line, const std::vector<string>& data) {
  ...
  for (const std::vector<int64_t>& dims :
    ...
    // Create C++ Tensor
    Tensor src(machina::DT_STRING, TensorShape(dims));
...
TEST(CAPI, DataTypeEnum) {
  EXPECT_EQ(TF_FLOAT, static_cast<TF_DataType>(machina::DT_FLOAT));
  EXPECT_EQ(TF_DOUBLE, static_cast<TF_DataType>(machina::DT_DOUBLE));
  EXPECT_EQ(TF_INT32, static_cast<TF_DataType>(machina::DT_INT32));
  EXPECT_EQ(TF_UINT8, static_cast<TF_DataType>(machina::DT_UINT8));
  EXPECT_EQ(TF_INT16, static_cast<TF_DataType>(machina::DT_INT16));
  EXPECT_EQ(TF_INT8, static_cast<TF_DataType>(machina::DT_INT8));
  EXPECT_EQ(TF_STRING, static_cast<TF_DataType>(machina::DT_STRING));
  EXPECT_EQ(TF_COMPLEX64, static_cast<TF_DataType>(machina::DT_COMPLEX64));
  EXPECT_EQ(TF_COMPLEX, TF_COMPLEX64);
  EXPECT_EQ(TF_INT64, static_cast<TF_DataType>(machina::DT_INT64));
  EXPECT_EQ(TF_BOOL, static_cast<TF_DataType>(machina::DT_BOOL));
  EXPECT_EQ(TF_QINT8, static_cast<TF_DataType>(machina::DT_QINT8));
  EXPECT_EQ(TF_QUINT8, static_cast<TF_DataType>(machina::DT_QUINT8));
  EXPECT_EQ(TF_QINT32, static_cast<TF_DataType>(machina::DT_QINT32));
  EXPECT_EQ(TF_BFLOAT16, static_cast<TF_DataType>(machina::DT_BFLOAT16));
  EXPECT_EQ(TF_QINT16, static_cast<TF_DataType>(machina::DT_QINT16));
  EXPECT_EQ(TF_QUINT16, static_cast<TF_DataType>(machina::DT_QUINT16));
  EXPECT_EQ(TF_UINT16, static_cast<TF_DataType>(machina::DT_UINT16));
  EXPECT_EQ(TF_COMPLEX128, static_cast<TF_DataType>(machina::DT_COMPLEX128));
  EXPECT_EQ(TF_HALF, static_cast<TF_DataType>(machina::DT_HALF));
  EXPECT_EQ(TF_DataTypeSize(TF_DOUBLE),
            machina::DataTypeSize(machina::DT_DOUBLE));
  EXPECT_EQ(TF_DataTypeSize(TF_STRING),
            machina::DataTypeSize(machina::DT_STRING));
...
TEST(CAPI, StatusEnum) {
  EXPECT_EQ(TF_OK, static_cast<TF_Code>(machina::error::OK));
  EXPECT_EQ(TF_CANCELLED, static_cast<TF_Code>(machina::error::CANCELLED));
  EXPECT_EQ(TF_UNKNOWN, static_cast<TF_Code>(machina::error::UNKNOWN));
  EXPECT_EQ(TF_INVALID_ARGUMENT,
            static_cast<TF_Code>(absl::StatusCode::kInvalidArgument));
  EXPECT_EQ(TF_DEADLINE_EXCEEDED,
            static_cast<TF_Code>(machina::error::DEADLINE_EXCEEDED));
  EXPECT_EQ(TF_NOT_FOUND, static_cast<TF_Code>(machina::error::NOT_FOUND));
  EXPECT_EQ(TF_ALREADY_EXISTS,
            static_cast<TF_Code>(machina::error::ALREADY_EXISTS));
  EXPECT_EQ(TF_PERMISSION_DENIED,
            static_cast<TF_Code>(machina::error::PERMISSION_DENIED));
  EXPECT_EQ(TF_UNAUTHENTICATED,
            static_cast<TF_Code>(machina::error::UNAUTHENTICATED));
  EXPECT_EQ(TF_RESOURCE_EXHAUSTED,
            static_cast<TF_Code>(machina::error::RESOURCE_EXHAUSTED));
  EXPECT_EQ(TF_FAILED_PRECONDITION,
            static_cast<TF_Code>(machina::error::FAILED_PRECONDITION));
  EXPECT_EQ(TF_ABORTED, static_cast<TF_Code>(machina::error::ABORTED));
  EXPECT_EQ(TF_OUT_OF_RANGE,
            static_cast<TF_Code>(machina::error::OUT_OF_RANGE));
  EXPECT_EQ(TF_UNIMPLEMENTED,
            static_cast<TF_Code>(machina::error::UNIMPLEMENTED));
  EXPECT_EQ(TF_INTERNAL, static_cast<TF_Code>(machina::error::INTERNAL));
  EXPECT_EQ(TF_UNAVAILABLE,
            static_cast<TF_Code>(machina::error::UNAVAILABLE));
  EXPECT_EQ(TF_DATA_LOSS, static_cast<TF_Code>(machina::error::DATA_LOSS));
}
...
TEST(CAPI, Graph) {
  ...
  EXPECT_EQ(attr_value.type(), machina::DT_INT32);
...
TEST(CAPI, ImportGraphDef) {
  ...
  machina::GraphDef graph_def_proto;
  ASSERT_TRUE(machina::ParseProtoUnlimited(&graph_def_proto, graph_def->data,
                                              graph_def->length));
  TF_Buffer graph_def_buffer;
  graph_def_buffer.data = reinterpret_cast<const void*>(&graph_def_proto);
  graph_def_buffer.length = sizeof(machina::GraphDef*);
...
TEST(CAPI, Session_Min_MACHINA_MACHINA_XLA_CPU) { RunMinTest(/*device=*/"", /*use_XLA=*/true); }
...
TEST(CAPI, Session_Min_MACHINA_MACHINA_XLA_GPU) {
  const string gpu_device = GPUDeviceName();
  // Skip this test if no GPU is available.
  if (gpu_device.empty()) return;
...
class CApiColocationTest : public ::testing::Test {
 protected:
  ...
  void SetViaStringList(TF_OperationDescription* desc,
    ...
    TF_SetAttrStringList(desc, machina::kColocationAttrName, list_ptrs.get(),
                         list_lens.get(), list.size());
  }

  void SetViaProto(TF_OperationDescription* desc,
                   const std::vector<string>& list) {
    machina::AttrValue attr;
    for (const string& v : list) {
      attr.mutable_list()->add_s(v);
    }
    string bytes;
    attr.SerializeToString(&bytes);
    TF_SetAttrValueProto(desc, machina::kColocationAttrName, bytes.data(),
                         bytes.size(), s_);
    ASSERT_EQ(TF_OK, TF_GetCode(s_)) << TF_Message(s_);
  }
  ...
  void VerifyCollocation(TF_Operation* op,
    ...
    TF_AttrMetadata m =
        TF_OperationGetAttrMetadata(op, machina::kColocationAttrName, s_);
    if (expected.empty()) {
    ...
    TF_OperationGetAttrStringList(op, machina::kColocationAttrName,
...
TEST(CAPI, SavedModel) {
  ...
  const string saved_model_dir = machina::GetDataDependencyFilepath(
      machina::io::JoinPath("machina", "cc", "saved_model", "testdata",
                               "half_plus_two", "00000123"));
  TF_SessionOptions* opt = TF_NewSessionOptions();
  TF_Buffer* run_options = TF_NewBufferFromString("", 0);
  TF_Buffer* metagraph = TF_NewBuffer();
  TF_Status* s = TF_NewStatus();
  const char* tags[] = {machina::kSavedModelTagServe};
  TF_Graph* graph = TF_NewGraph();
  TF_Session* session = TF_LoadSessionFromSavedModel(
      opt, run_options, saved_model_dir.c_str(), tags, 1, graph, metagraph, s);
  TF_DeleteBuffer(run_options);
  TF_DeleteSessionOptions(opt);
  machina::MetaGraphDef metagraph_def;
  ...
  const string input_name =
      signature_def.inputs().at(machina::kRegressInputs).name();
  const string output_name =
      signature_def.outputs().at(machina::kRegressOutputs).name();

  // Write {0, 1, 2, 3} as machina::Example inputs.
  Tensor input(machina::DT_STRING, TensorShape({4}));
  for (int64_t i = 0; i < input.NumElements(); ++i) {
    machina::Example example;
    auto* feature_map = example.mutable_features()->mutable_feature();
    (*feature_map)["x"].mutable_float_list()->add_value(i);
    input.flat<tstring>()(i) = example.SerializeAsString();
  }

  const machina::string input_op_name(
      machina::ParseTensorName(input_name).first);
  ...
  const machina::string output_op_name(
      machina::ParseTensorName(output_name).first);
 ...
 protected:
  ...
  TF_OperationDescription* init(string type) {
    ...
    return TF_NewOperation(
        ...
        ::machina::strings::StrCat("name", counter_++).c_str());
  }
...
// That will require setting up the machina build with gmock.
...
TEST_F(CApiAttributesTest, TensorShapeProto) {
  ...
  machina::PartialTensorShape(pts).AsProto(&proto);
  string bytes;
...
TEST_F(CApiAttributesTest, TensorShapeProtoList) {
  ...
  machina::PartialTensorShape(pts1).AsProto(&proto);
  proto.SerializeToString(&bytes1);

  const int64_t pts2[] = {1, 3, 5, 7};
  machina::PartialTensorShape(pts2).AsProto(&proto);
...
class DummyKernel : public machina::OpKernel {
 public:
  explicit DummyKernel(machina::OpKernelConstruction* context)
      : OpKernel(context) {}
...
REGISTER_KERNEL_BUILDER(
    Name("TestOpWithSingleKernel").Device(machina::DEVICE_CPU), DummyKernel);
