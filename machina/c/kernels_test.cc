...
TEST_F(TestKernelAttr, GetNodeDef) {
  auto my_create_func = [](TF_OpKernelConstruction* ctx) {
    ...
    const ::machina::AttrValue& value = node_def.attr().at("Attr");
    EXPECT_TRUE(value.value_case() == ::machina::AttrValue::ValueCase::kI);
...
TEST_F(TestKernelAttr, Tensor) {
  struct TensorProtoHelpers {
   public:
    static ::machina::TensorProto GenerateTensorProto() {
  ...
  AttrValue v;
  ::machina::TensorProto* tensor_proto = v.mutable_tensor();
...
TEST_F(TestKernelAttr, TensorList) {
  struct TensorProtoHelpers {
   public:
    static ::machina::TensorProto GenerateTensorProto1() {
      ::machina::TensorProto tensor_proto;
      tensor_proto.mutable_tensor_shape()->add_dim()->set_size(2);
      tensor_proto.mutable_tensor_shape()->add_dim()->set_size(2);
      tensor_proto.set_dtype(DT_INT32);
    ...
    static ::machina::TensorProto GenerateTensorProto2() {
      ::machina::TensorProto tensor_proto;
      tensor_proto.mutable_tensor_shape()->add_dim()->set_size(2);
      tensor_proto.mutable_tensor_shape()->add_dim()->set_size(3);
  ...
  AttrValue v;
  ::machina::TensorProto* tensor_proto1 = v.mutable_list()->add_tensor();
  *tensor_proto1 = TensorProtoHelpers::GenerateTensorProto1();

  ::machina::TensorProto* tensor_proto2 = v.mutable_list()->add_tensor();
...
TEST(TestKernel, TestInputAndOutputCount) {
  ...
  auto my_compute_func = [](void* kernel, TF_OpKernelContext* ctx) {
    ...
    EXPECT_EQ(123, *static_cast<machina::uint8*>(TF_TensorData(input)));
...

  {
    ...
    ASSERT_EQ(123, ctx.mutable_output(0)->scalar<machina::uint8>()());
  }
}
...
#if GOOGLE_CUDA || MACHINA_USE_ROCM
  const char* device_name_ = machina::DEVICE_GPU;
#else
  const char* device_name_ = machina::DEVICE_CPU;
#endif
};
