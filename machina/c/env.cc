...
struct TF_StringStream {
  std::vector<::machina::string>* list;
  size_t position;
};

void TF_CreateDir(const char* dirname, TF_Status* status) {
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->CreateDir(dirname));
}

void TF_DeleteDir(const char* dirname, TF_Status* status) {
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->DeleteDir(dirname));
}
...
void TF_DeleteRecursively(const char* dirname, int64_t* undeleted_file_count,
  ...
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->DeleteRecursively(dirname, &f, &d));
...
void TF_FileStat(const char* filename, TF_FileStatistics* stats,
                 TF_Status* status) {
  ::machina::FileStatistics cc_stats;
  TF_SetStatus(status, TF_OK, "");
  absl::Status s = ::machina::Env::Default()->Stat(filename, &cc_stats);
...
void TF_NewWritableFile(const char* filename, TF_WritableFileHandle** handle,
                        TF_Status* status) {
  std::unique_ptr<::machina::WritableFile> f;
  TF_SetStatus(status, TF_OK, "");
  absl::Status s = ::machina::Env::Default()->NewWritableFile(filename, &f);
  ::machina::Set_TF_Status_from_Status(status, s);
...
void TF_CloseWritableFile(TF_WritableFileHandle* handle, TF_Status* status) {
  auto* cc_file = reinterpret_cast<::machina::WritableFile*>(handle);
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(status, cc_file->Close());
  delete cc_file;
}
...
void TF_SyncWritableFile(TF_WritableFileHandle* handle, TF_Status* status) {
  auto* cc_file = reinterpret_cast<::machina::WritableFile*>(handle);
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(status, cc_file->Sync());
}

void TF_FlushWritableFile(TF_WritableFileHandle* handle, TF_Status* status) {
  auto* cc_file = reinterpret_cast<::machina::WritableFile*>(handle);
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(status, cc_file->Flush());
}
...
void TF_DeleteFile(const char* filename, TF_Status* status) {
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->DeleteFile(filename));
}
...
TF_StringStream* TF_GetChildren(const char* dirname, TF_Status* status) {
  auto* children = new std::vector<::machina::string>;

  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->GetChildren(dirname, children));
...
TF_StringStream* TF_GetLocalTempDirectories() {
  auto* tmpdirs = new std::vector<::machina::string>;

  ::machina::Env::Default()->GetLocalTempDirectories(tmpdirs);
...
char* TF_GetTempFileName(const char* extension) {
  return strdup(::machina::io::GetTempFilename(extension).c_str());
}

TF_CAPI_EXPORT extern uint64_t TF_NowNanos(void) {
  return ::machina::Env::Default()->NowNanos();
}

// Returns the number of microseconds since the Unix epoch.
TF_CAPI_EXPORT extern uint64_t TF_NowMicros(void) {
  return ::machina::Env::Default()->NowMicros();
}

// Returns the number of seconds since the Unix epoch.
TF_CAPI_EXPORT extern uint64_t TF_NowSeconds(void) {
  return ::machina::Env::Default()->NowSeconds();
}
...
void* TF_LoadSharedLibrary(const char* library_filename, TF_Status* status) {
  ...
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->LoadDynamicLibrary(library_filename,
                                                               &handle));
  return handle;
}
...
void* TF_GetSymbolFromLibrary(void* handle, const char* symbol_name,
  ...
  TF_SetStatus(status, TF_OK, "");
  ::machina::Set_TF_Status_from_Status(
      status, ::machina::Env::Default()->GetSymbolFromLibrary(
