...
#include "machina/c/experimental/stream_executor/stream_executor.h"
...
#include "machina/xla/tsl/c/tsl_status_internal.h"  // IWYU pragma: keep
...
#include "machina/core/platform/notification.h"
...
#include "machina/xla/tsl/framework/device_id_utils.h"
#include "machina/xla/tsl/platform/statusor.h"
#include "machina/core/common_runtime/next_pluggable_device/c/tf_rendezvous_c_api.h"
#include "machina/core/common_runtime/next_pluggable_device/c/tf_rendezvous_c_api_internal.h"
...
struct TF_KernelBuilder {
  ::machina::KernelDefBuilder* cc_builder;
...
TF_KernelBuilder* TF_NewKernelBuilder(
  ...
  result->cc_builder = new ::machina::KernelDefBuilder(op_name);
...
void AddTypeConstraint(TF_KernelBuilder* kernel_builder, const char* attr_name,
  ...
  // This needs to be under machina:: namespace so that
  // TF_CALL_ALL_TYPES macro can find machina::string as string.
  switch (dtype) {
...
namespace {
const machina::AttrValue* GetAttrValue(TF_OpKernelConstruction* ctx,
                                          const char* attr_name,
                                          TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelConstruction*>(ctx);
  const machina::AttrValue* attr =
      ::machina::AttrSlice(cc_ctx->def()).Find(attr_name);
  if (attr == nullptr) {
...
void TF_KernelBuilder_TypeConstraint(TF_KernelBuilder* kernel_builder,
                                     ...
                                     TF_Status* status) {
  machina::DataType dtype = static_cast<machina::DataType>(type);
  machina::AddTypeConstraint(kernel_builder, attr_name, dtype, status);
}
...
class KernelBuilderFactory
    : public ::machina::kernel_factory::OpKernelFactory {
 public:
  explicit KernelBuilderFactory(TF_KernelBuilder* builder)
      : builder_(builder) {}
  ::machina::OpKernel* Create(
      ::machina::OpKernelConstruction* context) override {
    if (builder_->compute_function)
      return new ::machina::COpKernel(context, builder_->create_function,
                                         builder_->compute_function,
                                         builder_->delete_function);
    else
      return new ::machina::CAsyncOpKernel(
...
void TF_RegisterKernelBuilder(const char* name, TF_KernelBuilder* builder,
                              TF_Status* status) {
  using machina::register_kernel::Name;
...
void TF_RegisterKernelBuilderWithKernelDef(const char* serialized_kernel_def,
  ...
  if (serialized_kernel_def == nullptr) {
    ...
    // to build a new one.
    machina::kernel_factory::OpKernelRegistrar(
        builder->cc_builder->Build(), name,
        std::make_unique<machina::KernelBuilderFactory>(builder));

    TF_SetStatus(status, TF_OK, "");
    return;
  }

  machina::KernelDef* kernel_def = new machina::KernelDef();
...
void TF_GetInput(TF_OpKernelContext* ctx, int i, TF_Tensor** tensor,
  ...
  const ::machina::Tensor& cc_tensor(cc_ctx->input(i));
...
void TF_InputRange(TF_OpKernelContext* ctx, const char* name,
  ...
  args->start = start;
  args->stop = stop;
  machina::Set_TF_Status_from_Status(args->status, status);
}
...
void TF_SetOutput(TF_OpKernelContext* ctx, int i, const TF_Tensor* tensor,
  ...
  ::machina::Tensor cc_tensor;
  absl::Status s = ::machina::TF_TensorToTensor(tensor, &cc_tensor);
...
TF_Tensor* TF_GetMutableOutput(TF_OpKernelContext* ctx, int i,
  ...
  const ::machina::Tensor& cc_tensor = *(cc_ctx->mutable_output(i));
  TF_Tensor* result =
      ::machina::TF_TensorFromTensor(cc_tensor, &status->status);
  if (TF_GetCode(status) == TF_OK) {
    return result;
...
void TF_GetSerializedFunctionDefLibrary(
  ...
  auto cc_status =
      machina::MessageToBuffer(fdef_lib, serialized_function_def_library);
...
void TF_GetSerializedConfigProto(TF_OpKernelContext* ctx,
                                 ...
                                 TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  const machina::ConfigProto* config_proto_ptr =
  ...
  auto cc_status =
      machina::MessageToBuffer(config_proto, serialized_config_proto);
  machina::Set_TF_Status_from_Status(status, cc_status);
}
...
void TF_GetSerializedResourceHandleProto(
    ...
    TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(ctx);
  const machina::ResourceHandle& handle = HandleFromInput(cc_ctx, i);
  machina::ResourceHandleProto handle_proto;
  handle.AsProto(&handle_proto);
  auto cc_status = machina::MessageToBuffer(
...
void TF_OpKernelConstruction_GetAttrSize(TF_OpKernelConstruction* ctx,
                                         ...
                                         int32_t* total_size,
                                         TF_Status* status) {
  const machina::AttrValue* attr = GetAttrValue(ctx, attr_name, status);
  if (!status->status.ok()) {
    *list_size = -1;
    *total_size = -1;
    return;
...
#define DEFINE_TF_GETATTR(func, c_type, cc_type, attr_type, list_field)        \
  ...
  void TF_OpKernelConstruction_GetAttr##func##List(                            \
    ...
    status->status =                                                           \
        machina::AttrValueHasType(*attr, "list(" attr_type ")");            \
    if (!status->status.ok()) return;                                          \
...
DEFINE_TF_GETATTR(Type, TF_DataType, machina::DataType, "type", type)
...
TF_Buffer* TF_OpKernelConstruction_GetAttrFunction(TF_OpKernelConstruction* ctx,
  ...
  cc_status = machina::MessageToBuffer(function, buffer);
  tsl::Set_TF_Status_from_Status(status, cc_status);
  if (!cc_status.ok())
    return nullptr;
  else
...
bool TF_IsHostMemoryInput(TF_OpKernelContext* ctx, int i, TF_Status* status) {
  ...
  TF_SetStatus(status, TF_OK, "");
  return cc_ctx->input_memory_type(i) == machina::HOST_MEMORY;
}
...
bool TF_IsHostMemoryOutput(TF_OpKernelContext* ctx, int i, TF_Status* status) {
  ...
  TF_SetStatus(status, TF_OK, "");
  return cc_ctx->output_memory_type(i) == machina::HOST_MEMORY;
}

int64_t TF_StepId(TF_OpKernelContext* ctx) {
  return reinterpret_cast<::machina::OpKernelContext*>(ctx)->step_id();
}
...
uint64_t TF_GetFrameId(TF_OpKernelContext* ctx) {
  return reinterpret_cast<::machina::OpKernelContext*>(ctx)
      ->frame_iter()
      .frame_id;
}
...
int TF_GetDeviceId(TF_OpKernelContext* ctx) {
  ...
  const machina::DeviceBase* device_base =
      reinterpret_cast<machina::OpKernelContext*>(ctx)->device();
#if defined(IS_MOBILE_PLATFORM) || defined(IS_SLIM_BUILD)
  if (!device_base->parsed_name().has_id) return -1;
  return device_base->parsed_name().id;
#else
  const auto* device = reinterpret_cast<const machina::Device*>(
...
TF_StringView TF_GetDeviceName(TF_OpKernelContext* ctx) {
  const auto& device_name =
      reinterpret_cast<machina::OpKernelContext*>(ctx)->device()->name();
...
TF_RendezvousThunk TF_GetRendezvous(TF_OpKernelContext* ctx) {
  TF_RendezvousThunk* thunk =
      ToC(reinterpret_cast<machina::OpKernelContext*>(ctx)->rendezvous());
...
TF_Tensor* TF_AllocateOutput(TF_OpKernelContext* context, int index,
  ...
  TF_SetStatus(status, TF_OK, "");
  auto* cc_ctx = reinterpret_cast<::machina::OpKernelContext*>(context);
  machina::gtl::ArraySlice<const int64_t> dimarray(
      reinterpret_cast<const int64_t*>(dims), num_dims);
  machina::Tensor* tensor;
  absl::Status s = cc_ctx->allocate_output(
      index, machina::TensorShape(dimarray), &tensor);
  if (!s.ok()) {
    ::machina::Set_TF_Status_from_Status(status, s);
    return nullptr;
  }
...
TF_Tensor* TF_ForwardInputOrAllocateOutput(
  ...
  machina::gtl::ArraySlice<const int64_t> output_dimarray(
      reinterpret_cast<const int64_t*>(output_dims), output_num_dims);
  machina::Tensor* output_tensor_pointer;
  absl::Status s = cc_ctx->forward_input_or_allocate_output(
      input_indices_array, output_index,
      machina::TensorShape(output_dimarray), &output_tensor_pointer,
      forwarded_input);
  if (!s.ok()) {
    ::machina::Set_TF_Status_from_Status(status, s);
    return nullptr;
...
TF_Tensor* TF_AllocateTemp(TF_OpKernelContext* context, TF_DataType dtype,
  ...
  machina::AllocatorAttributes allocator_attr;
  if (alloc_attrs && alloc_attrs->on_host) {
    allocator_attr.set_on_host(true);
  }
  absl::Status s;
  machina::Tensor tensor;
  s = cc_ctx->allocate_temp(static_cast<machina::DataType>(dtype),
                            machina::TensorShape(dimarray), &tensor,
                            allocator_attr);
  if (!s.ok()) {
    ::machina::Set_TF_Status_from_Status(status, s);
    return nullptr;
...
void TF_IncNumDeferredOps(TF_OpKernelContext* context) {
  machina::OpKernelContext* cc_ctx =
      reinterpret_cast<::machina::OpKernelContext*>(context);
