...
#ifndef MACHINA_C_C_API_INTERNAL_H_
#define MACHINA_C_C_API_INTERNAL_H_
...
struct TF_Graph {
  ...
  machina::Graph graph TF_GUARDED_BY(mu);

  // Runs shape inference.
  machina::ShapeRefiner refiner TF_GUARDED_BY(mu);

  // Maps from name of an operation to the Node* in 'graph'.
  std::unordered_map<machina::string, machina::Node*> name_map
      TF_GUARDED_BY(mu);
  ...
  machina::gtl::FlatMap<TF_Session*, machina::string> sessions
      TF_GUARDED_BY(mu);
...
struct TF_OperationDescription {
  ...
  machina::NodeBuilder node_builder;
  TF_Graph* graph;
  std::set<machina::string> colocation_constraints;
};
...
struct TF_Session {
  TF_Session(machina::Session* s, TF_Graph* g);

  machina::Session* session;
  TF_Graph* const graph;

  machina::mutex mu TF_ACQUIRED_AFTER(TF_Graph::mu);
...
struct TF_ImportGraphDefOptions {
  ...
  std::vector<machina::string> tensor_id_data;
};
...
struct TF_ImportGraphDefResults {
  ...
  std::vector<machina::string> missing_unused_key_names_data;
};
...
struct TF_Function {
  machina::FunctionRecord* record;
};

struct TF_ApiDefMap {
  explicit TF_ApiDefMap(const machina::OpList& op_list)
      :
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
        api_def_map(op_list),
#endif  // !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
...
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
  machina::ApiDefMap api_def_map TF_GUARDED_BY(lock);
#endif  // !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
...
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
struct TF_Server {
  TF_Server(std::unique_ptr<machina::ServerInterface> server);

  const machina::string target;
...
#endif  // MACHINA_C_C_API_INTERNAL_H_
