...
TF_Function* TF_GraphToFunctionWithControlOutputs(
  ...
  machina::mutex_lock l(fn_body->mu);

  // Process inputs.
  std::vector<machina::OutputTensor> input_tensors;
  std::unordered_map<const Node*, std::vector<int>> input_nodes;
  status->status = machina::ProcessInputs(fn_body, fn_name, ninputs, inputs,
                                             &input_tensors, &input_nodes);
  if (TF_GetCode(status) != TF_OK) return nullptr;

  // Process outputs.
  std::vector<machina::OutputTensor> output_tensors;
  status->status = machina::ProcessOutputs(fn_body, fn_name, noutputs,
  ...
  // Compute body nodes.
  std::vector<const Node*> body_nodes;
  status->status = machina::ComputeBodyNodes(
  ...
  status->status = machina::GraphToFunctionDef(
...
int TF_GraphGetFunctions(TF_Graph* g, TF_Function** funcs, int max_func,
                         TF_Status* status) {
  machina::FunctionDefLibrary lib;
  {
    machina::mutex_lock l(g->mu);
  ...
  for (int i = 0; i < len; ++i) {
    ...
    func->record = new machina::FunctionRecord(lib.function(i), {}, false);
    funcs[i] = func;
  }
...
TF_Function* TF_FunctionImportFunctionDef(const void* proto, size_t proto_len,
  ...
  TF_Function* func = new TF_Function();
  func->record = new machina::FunctionRecord(std::move(fdef), {}, false);
  status->status = absl::OkStatus();
  return func;
}
