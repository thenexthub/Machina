...
TF_Tensor* CreateTensor(TF_ManagedBuffer* buf, TF_DataType dtype,
  ...
  Tensor ret(static_cast<machina::DataType>(dtype),
  ...
  return new TF_Tensor{new machina::TensorInterface(std::move(ret))};
}
}  // namespace

TF_Tensor* TF_AllocateTensor(TF_DataType dtype, const int64_t* dims,
                             int num_dims, size_t len) {
  void* data = machina::allocate_tensor("TF_AllocateTensor", len,
                                           machina::cpu_allocator());
  TF_ManagedBuffer* buf =
      new TF_ManagedBuffer(data, len, machina::deallocate_buffer,
                           machina::cpu_allocator(), /*owns_memory=*/true);
...
TF_Tensor* TF_NewTensor(TF_DataType dtype, const int64_t* dims, int num_dims,
  ...
  if (dtype != TF_STRING && dtype != TF_RESOURCE &&
      machina::DataTypeCanUseMemcpy(
          static_cast<machina::DataType>(dtype)) &&
    ...
    // TF_Tensor than they do in machina::Tensor. So a copy here is a waste
    ...
    // do so.
    buf = new TF_ManagedBuffer(machina::allocate_tensor("TF_NewTensor", len),
                               len, machina::deallocate_buffer, nullptr,
                               /*owns_memory=*/true);
...
void TF_SetShape(TF_Tensor* t, const int64_t* dims, int num_dims) {
  machina::down_cast<machina::TensorInterface*>(t->tensor)->SetShape(
      dims, num_dims);
}
...
void TF_TensorBitcastFrom(const TF_Tensor* from, TF_DataType type,
  ...
  Status cc_status(
      machina::down_cast<machina::TensorInterface*>(to->tensor)
          ->BitcastFrom(
              *machina::down_cast<const machina::TensorInterface*>(
                  from->tensor),
              static_cast<machina::DataType>(type), new_dims, num_new_dims));
...
bool TensorInterface::CanMove() const {
  ...
  TensorBuffer* buf = machina::TensorCApi::Buffer(tensor_);
...
size_t TensorInterface::ByteSize() const {
  return machina::TensorCApi::Buffer(tensor_)->size();
}

void* TensorInterface::Data() const {
  return machina::TensorCApi::Buffer(tensor_)->data();
}

void TensorInterface::SetShape(const int64_t* dims, int num_dims) {
  machina::TensorShape s;
  for (int i = 0; i < num_dims; ++i) {
    s.AddDim(dims[i]);
  }
  tensor_.set_shape(s);
...
absl::Status TensorInterface::FromProto(const machina::TensorProto& from) {
...
AbstractTensorInterface* TensorInterfaceFromTensor(const Tensor& src,
  ...
  *status = absl::OkStatus();
  ...
  return new machina::TensorInterface(std::move(tensor));
}

// Non-static for testing.
TF_Tensor* TF_TensorFromTensor(const machina::Tensor& src,
                               absl::Status* status) {
  return new TF_Tensor{TensorInterfaceFromTensor(src, status)};
}

TF_Tensor* TF_TensorFromTensorShallow(const machina::Tensor& src,
                                      absl::Status* status) {
  *status = absl::OkStatus();
  if (!src.IsInitialized()) {
    *status = FailedPrecondition(
  ...
  return new TF_Tensor{new machina::TensorInterface(src)};
}

absl::Status TF_TensorToTensor(const TF_Tensor* src, Tensor* dst) {
  return machina::down_cast<const machina::TensorInterface*>(src->tensor)
      ->ToTensor(dst);
}

absl::Status TensorInterface::ToTensor(machina::Tensor* dst) const {
  *dst = tensor_;
