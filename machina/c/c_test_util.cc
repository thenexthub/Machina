...
void Split3Helper(TF_Operation* input, TF_Graph* graph, TF_Status* s,
  ...
  TF_Operation* zero = ScalarConst(
      0, graph, s, ::machina::strings::StrCat(name, "_const0").c_str());
...
bool IsPlaceholder(const machina::NodeDef& node_def) {
  ...
  for (const auto& attr : node_def.attr()) {
    if (attr.first == "dtype") {
      if (attr.second.type() == machina::DT_INT32) {
        found_dtype = true;
      } else {
        return false;
      }
...
bool IsScalarConst(const machina::NodeDef& node_def, int v) {
...
bool IsAddN(const machina::NodeDef& node_def, int n) {
...
bool IsNeg(const machina::NodeDef& node_def, const string& input) {
  return node_def.op() == "Neg" && node_def.name() == "neg" &&
         node_def.input_size() == 1 && node_def.input(0) == input;
}

bool GetGraphDef(TF_Graph* graph, machina::GraphDef* graph_def) {
...
bool GetNodeDef(TF_Operation* oper, machina::NodeDef* node_def) {
  TF_Status* s = TF_NewStatus();
  TF_Buffer* buffer = TF_NewBuffer();
...
bool GetFunctionDef(TF_Function* func, machina::FunctionDef* func_def) {
  TF_Status* s = TF_NewStatus();
  TF_Buffer* buffer = TF_NewBuffer();
...
bool GetAttrValue(TF_Operation* oper, const char* attr_name,
                  machina::AttrValue* attr_value, TF_Status* s) {
  TF_Buffer* buffer = TF_NewBuffer();
...
std::vector<std::pair<string, string>> GetGradDefs(
    const machina::GraphDef& graph_def) {
  std::vector<std::pair<string, string>> grads;
  for (const machina::GradientDef& grad : graph_def.library().gradient()) {
...
std::vector<string> GetFuncNames(const machina::GraphDef& graph_def) {
  std::vector<string> names;
  auto functions = graph_def.library().function();
  names.reserve(functions.size());
  for (const machina::FunctionDef& func : functions) {
