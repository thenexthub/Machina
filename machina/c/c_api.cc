...
#include "machina/core/platform/platform.h"  // NOLINT

#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
#include "machina/c/experimental/filesystem/modular_filesystem.h"
#include "machina/cc/framework/gradients.h"
#include "machina/cc/framework/ops.h"
#include "machina/cc/framework/scope_internal.h"
#include "machina/cc/ops/while_loop.h"
...
#include "machina/core/common_runtime/eval_const_tensor.h"
...
#include "machina/core/framework/cpp_shape_inference.pb.h"
...
#include "machina/core/framework/tensor.pb.h"  // NOLINT
...
#include "machina/core/platform/coding.h"
...
using machina::AttrValueMap;
using machina::DataType;
using machina::ExtendSessionGraphHelper;
using machina::FullTypeDef;
using machina::Graph;
using machina::GraphDef;
using machina::mutex_lock;
using machina::NameRangeMap;
using machina::NameRangesForNode;
using machina::NewSession;
using machina::Node;
using machina::NodeBuilder;
using machina::OpDef;
using machina::PartialTensorShape;
using machina::RunMetadata;
using machina::RunOptions;
using machina::Session;
using machina::Status;
using machina::string;
using machina::Tensor;
using machina::TensorId;
using machina::TensorShapeProto;
using machina::VersionDef;
using machina::errors::FailedPrecondition;
using machina::errors::InvalidArgument;
using machina::errors::OutOfRange;
using machina::gtl::ArraySlice;
...
void TF_TensorFromProto(const TF_Buffer* from, TF_Tensor* to,
  ...
  machina::TensorProto from_tensor_proto;
  status->status = BufferToMessage(from, &from_tensor_proto);
  if (!status->status.ok()) {
    return;
  }
  status->status =
      machina::down_cast<machina::TensorInterface*>(to->tensor)
...
void TF_ExtendGraph(TF_DeprecatedSession* s, const void* proto,
                    size_t proto_len, TF_Status* status) {
  GraphDef g;
  if (!machina::ParseProtoUnlimited(&g, proto, proto_len)) {
...
machina::shape_inference::ShapeHandle ShapeHandleFromDims(
    machina::shape_inference::InferenceContext* ic, int num_dims,
    const int64_t* dims) {
  if (num_dims != -1) {
    std::vector<machina::shape_inference::DimensionHandle> dim_vec;
...
void TF_GraphSetOutputHandleShapesAndTypes(TF_Graph* graph, TF_Output output,
  ...
  machina::shape_inference::InferenceContext* ic =
  ...
  auto shape_and_type_vec =
      std::vector<machina::shape_inference::ShapeAndType>(
          num_shapes_and_types);
  for (int i = 0; i < num_shapes_and_types; ++i) {
    machina::shape_inference::ShapeHandle shape_handle =
        ShapeHandleFromDims(ic, ranks[i], shapes[i]);
    shape_and_type_vec[i] = machina::shape_inference::ShapeAndType(
...
static Status TF_TensorToTensorV1(const TF_Tensor* src, Tensor* dst) {
  ...
  if (dst->dtype() == machina::DT_RESOURCE) {
    const auto tensor_interface =
        machina::down_cast<const machina::TensorInterface*>(src->tensor);
    ...
    *dst = machina::Tensor(machina::DT_RESOURCE, dst->shape());
    if (!dst->scalar<machina::ResourceHandle>()().ParseFromString(
...
TF_Library* TF_LoadLibrary(const char* library_filename, TF_Status* status) {
  ...
  status->status = machina::LoadDynamicLibrary(
...
void TF_DeleteLibraryHandle(TF_Library* lib_handle) {
  ...
  machina::port::Free(const_cast<void*>(lib_handle->op_list.data));
  delete lib_handle;
}

TF_Buffer* TF_GetAllOpList() {
  std::vector<machina::OpDef> op_defs;
  machina::OpRegistry::Global()->GetRegisteredOps(&op_defs);
  machina::OpList op_list;
...
const machina::AttrValue* GetAttrValue(TF_Operation* oper,
                                          const char* attr_name,
                                          TF_Status* status) {
  const machina::AttrValue* attr = oper->node.attrs().Find(attr_name);
  if (attr == nullptr) {
...
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
std::vector<machina::Output> OutputsFromTFOutputs(TF_Output* tf_outputs,
                                                     int n) {
  std::vector<machina::Output> outputs(n);
  for (int i = 0; i < n; ++i) {
    outputs[i] =
        machina::Output(&tf_outputs[i].oper->node, tf_outputs[i].index);
  }
  return outputs;
}

void TFOutputsFromOutputs(const std::vector<machina::Output>& outputs,
...
void TF_GraphSetTensorShape(TF_Graph* graph, TF_Output output,
  ...
  machina::shape_inference::ShapeHandle new_shape =
      machina::ShapeHandleFromDims(ic, num_dims, dims);
...
int TF_GraphGetTensorNumDims(TF_Graph* graph, TF_Output output,
  ...
  machina::shape_inference::ShapeHandle shape = ic->output(output.index);
...
void TF_ColocateWith(TF_OperationDescription* desc, TF_Operation* op) {
  desc->colocation_constraints.emplace(
      StrCat(machina::kColocationGroupPrefix, op->node.name()));
}
...
void TF_SetAttrStringList(TF_OperationDescription* desc, const char* attr_name,
                          ...
                          int num_values) {
  if (strcmp(attr_name, machina::kColocationAttrName) == 0) {
...
void TF_SetAttrFuncName(TF_OperationDescription* desc, const char* attr_name,
  ...
  machina::NameAttrList func_name;
...
void TF_SetAttrValueProto(TF_OperationDescription* desc, const char* attr_name,
  ...
  if (strcmp(attr_name, machina::kColocationAttrName) == 0) {
    if (attr_value.value_case() != machina::AttrValue::kList &&
        attr_value.value_case() != machina::AttrValue::VALUE_NOT_SET) {
      status->status =
          InvalidArgument("Expected \"list\" field for \"",
                          machina::kColocationAttrName, "\" attribute");
      return;
    }
    desc->colocation_constraints.clear();
...
void TF_OperationGetAttrString(TF_Operation* oper, const char* attr_name,
  ...
  const auto* attr = GetAttrValue(oper, attr_name, status);
  if (!status->status.ok()) return;
  if (attr->value_case() != machina::AttrValue::kS) {
    status->status =
...
void TF_OperationGetAttrStringList(TF_Operation* oper, const char* attr_name,
  ...
  const auto* attr = GetAttrValue(oper, attr_name, status);
  if (!status->status.ok()) return;
  if (attr->value_case() != machina::AttrValue::kList) {
    status->status =
...
#define DEFINE_GETATTR(func, c_type, cpp_type, list_field)                   \
  void func(TF_Operation* oper, const char* attr_name, c_type* value,        \
    ...
    cpp_type v;                                                              \
    status->status =                                                         \
        machina::GetNodeAttr(oper->node.attrs(), attr_name, &v);          \
    if (!status->status.ok()) return;                                        \
...
void TF_OperationGetAttrShape(TF_Operation* oper, const char* attr_name,
  ...
  PartialTensorShape shape;
  status->status =
      machina::GetNodeAttr(oper->node.attrs(), attr_name, &shape);
  if (!status->status.ok()) return;
...
void TF_OperationGetAttrShapeList(TF_Operation* oper, const char* attr_name,
  ...
  std::vector<PartialTensorShape> shapes;
  status->status =
      machina::GetNodeAttr(oper->node.attrs(), attr_name, &shapes);
  if (!status->status.ok()) return;
...
void TF_OperationGetAttrTensorShapeProto(TF_Operation* oper,
  ...
  const auto* attr = GetAttrValue(oper, attr_name, status);
  if (!status->status.ok()) return;
  if (attr->value_case() != machina::AttrValue::kShape) {
    status->status =
...
void TF_OperationGetAttrTensor(TF_Operation* oper, const char* attr_name,
  ...
  *value = nullptr;
  Tensor t;
  status->status = machina::GetNodeAttr(oper->node.attrs(), attr_name, &t);
  if (!status->status.ok()) return;
...
void TF_OperationGetAttrTensorList(TF_Operation* oper, const char* attr_name,
  ...
  status->status = machina::GetNodeAttr(oper->node.attrs(), attr_name, &ts);
  if (!status->status.ok()) return;
...
TF_Graph::TF_Graph()
    : graph(machina::OpRegistry::Global()),
...
void TF_ImportGraphDefOptionsRemapControlDependency(
  ...
  opts->opts.input_map[TensorId(src_name, machina::Graph::kControlSlot)] =
      TensorId(dst->node.name(), machina::Graph::kControlSlot);
}
...
static void GraphImportGraphDefLocked(TF_Graph* graph, const GraphDef& def,
  ...
  machina::ImportGraphDefResults results;
  status->status = machina::ImportGraphDef(opts->opts, def, &graph->graph,
...
TF_ImportGraphDefResults* TF_GraphImportGraphDefWithResults(
  ...
  GraphDef def;
  if (!machina::ParseProtoUnlimited(&def, graph_def->data,
                                       graph_def->length)) {
    status->status = InvalidArgument("Invalid GraphDef");
    return nullptr;
  }
...
Status CopyGraph(Graph* src_graph, Graph* dst_graph,
                 machina::ShapeRefiner* dst_refiner,
                 const TF_Output* src_inputs,
                 const std::vector<machina::Output>& dst_inputs,
                 const string& prefix,
                 const std::vector<machina::Operation>& control_deps,
                 const TF_Output* nodes_to_return, int nreturn_nodes,
                 std::vector<machina::Output>* return_nodes) {
  DCHECK(return_nodes != nullptr);
  GraphDef gdef;
  src_graph->ToGraphDef(&gdef);

  machina::ImportGraphDefOptions opts;
  opts.prefix = prefix;
  ...
  for (const machina::Operation& op : control_deps) {
...
TF_WhileParams TF_NewWhile(TF_Graph* g, TF_Output* inputs, int ninputs,
  ...
  status->status = machina::errors::Unimplemented(
      ...
      "https://github.com/machina/machina/issues if this feature is "
      "important to you");
...
void TF_FinishWhileHelper(const TF_WhileParams* params, TF_Status* status,
  ...
  machina::ops::CondGraphBuilderFn cond_fn =
      [params, parent](const machina::Scope& scope,
                       const std::vector<machina::Output>& inputs,
                       machina::Output* output) {
        DCHECK_EQ(scope.graph(), &parent->graph);
        std::vector<machina::Output> cond_output;
  ...
  machina::ops::BodyGraphBuilderFn body_fn =
      [params, parent, num_loop_vars](
          const machina::Scope& scope,
          const std::vector<machina::Output>& inputs,
  ...
  machina::OutputList loop_outputs;
  status->status = machina::ops::BuildWhileLoop(
...
void TF_AddGradientsWithPrefix(TF_Graph* g, const char* prefix, TF_Output* y,
  ...
  status->status = machina::errors::Unimplemented(
      ...
      "important to you");
#else
  std::vector<machina::Output> y_arg = OutputsFromTFOutputs(y, ny);
  std::vector<machina::Output> x_arg = OutputsFromTFOutputs(x, nx);
  std::vector<machina::Output> dy_arg;

  {
    ...
    if (dx != nullptr) {
      std::vector<machina::Output> dx_arg = OutputsFromTFOutputs(dx, ny);
      status->status =
    ...
    for (int i = first_new_node_id; i < g->graph.num_node_ids(); ++i) {
      ...
      if (!g->name_map.insert(std::make_pair(n->name(), n)).second) {
        status->status = machina::errors::Internal(
            ...
            "https://github.com/machina/machina/issues.");
      }
    }
  }
...
TF_Session::TF_Session(machina::Session* s, TF_Graph* g)
...
// that the machina/cc/saved_model:loader build target is mobile friendly.
#if defined(IS_MOBILE_PLATFORM) || defined(IS_SLIM_BUILD)
  status->status = machina::errors::Unimplemented(
  ...
  status->status =
      machina::LoadSavedModel(session_options->options, run_options_proto,
...
unsigned char TF_TryEvaluateConstant(TF_Graph* graph, TF_Output output,
  ...
  auto status_or = EvaluateConstantTensor(
      ...
      machina::EvaluateConstantTensorRunner{
...
TF_Buffer* TF_GetAllRegisteredKernels(TF_Status* status) {
  machina::KernelList kernel_list = machina::GetAllRegisteredKernels();
  TF_Buffer* ret = TF_NewBuffer();
...
TF_Buffer* TF_GetRegisteredKernelsForOp(const char* name, TF_Status* status) {
  machina::KernelList kernel_list =
      machina::GetRegisteredKernelsForOp(name);
  TF_Buffer* ret = TF_NewBuffer();
  status->status = MessageToBuffer(kernel_list, ret);
  if (!status->status.ok()) {
    TF_DeleteBuffer(ret);
...
void TF_UpdateEdge(TF_Graph* graph, TF_Output new_src, TF_Input dst,
                   TF_Status* status) {
  using machina::RecordMutation;
  mutex_lock l(graph->mu);
  machina::shape_inference::InferenceContext* ic =
      graph->refiner.GetContext(&new_src.oper->node);

  if (ic->num_outputs() <= new_src.index) {
    status->status = machina::errors::OutOfRange(
        "Cannot update edge. Output index [", new_src.index,
        "] is greater than the number of total outputs [", ic->num_outputs(),
        "].");
    return;
  }
  machina::shape_inference::ShapeHandle shape = ic->output(new_src.index);

  machina::shape_inference::InferenceContext* ic_dst =
...
void TF_RemoveAllControlInputs(TF_Graph* graph, TF_Operation* op) {
  mutex_lock l(graph->mu);
  std::vector<const machina::Edge*> control_edges;
  for (const machina::Edge* edge : op->node.in_edges()) {
    if (!edge->IsControlEdge()) continue;
    control_edges.push_back(edge);
  }
  for (const machina::Edge* edge : control_edges) {
...
TF_Buffer* TF_GetHandleShapeAndType(TF_Graph* graph, TF_Output output) {
  Node* node = &output.oper->node;
  machina::core::CppShapeInferenceResult::HandleData handle_data;
...
void TF_SetHandleShapeAndType(TF_Graph* graph, TF_Output output,
  ...
  machina::mutex_lock l(graph->mu);
  machina::shape_inference::InferenceContext* ic =
      graph->refiner.GetContext(&output.oper->node);

  std::vector<machina::shape_inference::ShapeAndType> shapes_and_types;
...
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
TF_Server::TF_Server(std::unique_ptr<machina::ServerInterface> server)
...
TF_Server* TF_NewServer(const void* proto, size_t proto_len,
  ...
  std::unique_ptr<machina::ServerInterface> out_server;
  status->status = machina::NewServer(server_def, &out_server);
  if (!status->status.ok()) return nullptr;
...
#if !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
  machina::logging::RegisterListener(listener);
#endif  // !defined(IS_MOBILE_PLATFORM) && !defined(IS_SLIM_BUILD)
}
...
#else
  status->status = machina::RegisterFilesystemPlugin(plugin_filename);
#endif  // defined(IS_MOBILE_PLATFORM) || defined(IS_SLIM_BUILD)
}
