...
protocol: "grpc"
  ...
  machina::protobuf::TextFormat::PrintToString(actual, &actual_text_proto);
...
TEST(CAPI_EXPERIMENTAL, LibraryPluggableDeviceLoadFunctions) {
  ...
  TF_Status* status = TF_NewStatus();
  string lib_path =
      machina::GetDataDependencyFilepath(machina::io::JoinPath(
          "machina", "c", "experimental", "stream_executor", "test",
...
TEST(CAPI_EXPERIMENTAL, LibraryNextPluggableDeviceLoadFunctions) {
  ...
  string lib_path =
      machina::GetDataDependencyFilepath(machina::io::JoinPath(
          "machina", "core", "common_runtime", "next_pluggable_device", "c",
