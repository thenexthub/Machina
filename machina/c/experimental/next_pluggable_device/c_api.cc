...
void TF_CreatePluginResource(TF_OpKernelContext* ctx,
                             ...
                             TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<machina::OpKernelContext*>(ctx);
  machina::PluginResource* cc_resource_ptr = new machina::PluginResource(
      plugin_resource, plugin_resource_name, delete_func);
  auto cc_status =
      cc_ctx->resource_manager()->Create<machina::PluginResource>(
...
void TF_LookupOrCreatePluginResource(
  ...
  auto cc_status = resource_mgr->LookupOrCreate<machina::PluginResource>(
...
struct TF_VariableInfo {
  ...
  // by the underlying machina::VariableInfo.
  TF_VariableInfo(int index, const std::string& name, machina::Var* var) {
    var_info = machina::VariableInfo{index, name, var};
  }

  machina::VariableInfo var_info{0, "", nullptr};
};
...
TF_VariableInfo* TF_CreateVariableInfoFromContext(TF_OpKernelContext* ctx,
                                                  ...
                                                  TF_Status* status) {
  auto* cc_ctx = reinterpret_cast<machina::OpKernelContext*>(ctx);
  const machina::Tensor& arg_tensor = cc_ctx->input(index);
  absl::Status cc_status;
  if (arg_tensor.dtype() != machina::DT_RESOURCE) {
  ...
  const machina::ResourceHandle& handle =
      arg_tensor.flat<machina::ResourceHandle>()(0);
  machina::Var* variable;
  cc_status = machina::LookupResource(cc_ctx, handle, &variable);
  return new TF_VariableInfo(index, handle.name(), variable);
}

void TF_LockVariableInfos(TF_VariableInfo** vars, int num_vars,
                          TF_Status* status) {
  std::vector<machina::VariableInfo*> variable_ptrs;
...
TF_Tensor* TF_GetTensorFromVariableInfo(TF_VariableInfo* var_info,
  ...
  machina::Tensor* tensor = var_info->var_info.var()->tensor();
  TF_Tensor* result_tensor =
      machina::TF_TensorFromTensor(*tensor, &cc_status);
  status->status = cc_status;
...
void TF_CreateAndSetPjRtCApiClient(const char* device_type, TF_Status* status,
  ...
  absl::Status s = machina::SetPjRtClientInTFGlobalResourceManager(
      machina::DeviceType(device_type), std::move(*pjrt_client));
  status->status = s;
}

void TF_ResetPjRtCClient(const char* device_type, TF_Status* status) {
  status->status =
      machina::ResetPjRtClient(machina::DeviceType(device_type));
}

PJRT_Client* TF_GetPjRtCClient(const char* device_type, TF_Status* status) {
  absl::StatusOr<xla::PjRtCApiClient*> pjrt_c_api_client =
      machina::GetPjRtCApiClient(machina::DeviceType(device_type));
...
PJRT_Buffer* TF_GetPjRtCBuffer(TF_Tensor* c_tensor, TF_Status* status) {
  ...
  auto s = machina::TF_TensorToTensor(c_tensor, &tensor);
  if (!s.ok()) {
    status->status = s;
    return nullptr;
  }
  absl::StatusOr<PJRT_Buffer*> c_buffer =
      machina::GetPjRtCBufferFromTensor(&tensor);
  if (!c_buffer.ok()) {
