...
// for testing.
#ifndef MACHINA_C_EXPERIMENTAL_STREAM_EXECUTOR_STREAM_EXECUTOR_INTERNAL_H_
#define MACHINA_C_EXPERIMENTAL_STREAM_EXECUTOR_STREAM_EXECUTOR_INTERNAL_H_
...
#include "machina/xla/stream_executor/stream_common.h"
...
class CPlatform : public Platform {
 public:
  ...
  int VisibleDeviceCount() const override {
    ...
    machina::TF_StatusPtr c_status(TF_NewStatus());
...
class CEvent : public Event {
 public:
  ...
  absl::Status Create() {
    ...
    return machina::StatusFromTF_Status(c_status.get());
  }
 ...
 public:
  ...
  absl::Status RefreshStatus() override {
    ...
    stream_executor_->get_stream_status(device_, stream_handle_,
                                        c_status.get());
    absl::Status status = machina::StatusFromTF_Status(c_status.get());
    CheckStatus(status);
    return status;
  }
  ...
  absl::Status BlockHostUntilDone() override {
    ...
    stream_executor_->record_event(device_, stream_handle, event_handle,
                                   c_status.get());
    absl::Status s = machina::StatusFromTF_Status(c_status.get());
    if (!s.ok()) {
...
#endif  // MACHINA_C_EXPERIMENTAL_STREAM_EXECUTOR_STREAM_EXECUTOR_INTERNAL_H_
