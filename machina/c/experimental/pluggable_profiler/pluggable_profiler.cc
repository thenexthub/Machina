...
#include "machina/core/profiler/lib/profiler_factory.h"
...
Status ValidateTPProfiler(const TP_Profiler& profiler) {
  ...
  TF_RETURN_IF_ERROR(
      machina::device_utils::ValidateDeviceType(profiler.device_type));
  return absl::OkStatus();
}
...
class PluggableProfiler : public machina::profiler::ProfilerInterface {
 public:
  // The caller must have validated profiler_fns and profiler.
  static std::unique_ptr<machina::profiler::ProfilerInterface>
  ...
  Status Start() override {
    ...
    return machina::StatusFromTF_Status(status.get());
  }
...
class PluggableProfilerFactory {
 public:
  ...
  std::unique_ptr<machina::profiler::ProfilerInterface>
...
Status InitPluginProfiler(TFInitProfilerFn init_fn) {
  ...
  TF_RETURN_IF_ERROR(machina::StatusFromTF_Status(status.get()));
