...
#include "machina/c/experimental/saved_model/core/revived_types/restored_resource_revival_state.h"
#include "machina/c/experimental/saved_model/core/revived_types/tf_concrete_function_revival_state.h"
#include "machina/c/experimental/saved_model/core/revived_types/tf_signature_def_function_revival_state.h"
#include "machina/c/experimental/saved_model/core/revived_types/variable.h"
...
using FunctionDefMap =
    gtl::FlatMap<absl::string_view, const machina::FunctionDef*,
                 StringPieceHasher>;

// Looks up a SavedConstant's associated tensorproto from the NodeAttrMap and
// returns a machina::Constant.
absl::Status ConstantFromSavedConstant(
    ImmediateExecutionContext* ctx,
    const machina::SavedConstant& saved_constant,
...
absl::Status ValidateSavedFunctionCompatibleWithFunctionDef(
  ...
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/eager/function.py#L2671-L2675
  // 2. convert user-provided inputs to tensors:
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/eager/function.py#L2687-L2688
  // 3. filter any non-tensor, non-variable inputs:
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/eager/function.py#L1840-L1841
  // 4. concatenate any captured inputs:
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/eager/function.py#L1912
  ...
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/saved_model/function_serialization.py#L70-L71
  // The "structured_input_signature" is guaranteed to be a tuple of the python
  // args, kwargs that correspond to the tf.function:
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/python/eager/function.py#L1974-L1979
...
absl::Status GetSignaturesMap(const SavedObjectGraph& saved_objects,
  ...
  if (signatures == nullptr) {
    ...
    // https://github.com/machina/machina/blob/a2c542a0d83227568f9214a2af9a38ae3625976f/machina/python/saved_model/save.py#L1106-L1109
    return errors::FailedPrecondition(
  ...
  if (signatures->user_object().identifier() != "signature_map") {
    ...
    // https://github.com/machina/machina/blob/c59af2913aaec235d883f50428efef1086f4c0e6/machina/python/saved_model/signature_serialization.py#L220
    return errors::FailedPrecondition(
...
absl::Status ValidateSingleConcreteFunction(
  ...
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/compiler/mlir/machina/translate/import_model.cc#L2677-L2707
...
// https://github.com/machina/machina/blob/516608035f85cec8b126712b0ff8407220206b22/machina/python/saved_model/load.py#L407
absl::Status LoadSavedVariable(ImmediateExecutionContext* ctx,
                               const SavedVariable& variable,
                               std::unique_ptr<Variable>* output) {
  const std::string& name = variable.name();
  machina::TensorShape shape(variable.shape());
  machina::DataType dtype = variable.dtype();
...
absl::Status FlattenSignature(
  ...
  // https://github.com/machina/machina/blob/1c064ab76064c58e54261b805027474885a1534d/machina/compiler/mlir/machina/translate/import_model.cc#L2775
...
gtl::FlatMap<absl::string_view, const AttrValueMap*, StringPieceHasher>
NodeToAttrMap(const machina::GraphDef& graphdef) {
  gtl::FlatMap<absl::string_view, const AttrValueMap*, StringPieceHasher>
      result;
  for (const machina::NodeDef& node : graphdef.node()) {
...
absl::Status PartiallyReviveSavedModelObjects(
  ...
  for (auto& node_and_resource_revival_state : objects->restored_resources) {
    ...
    for (const TrackableObjectGraph::TrackableObject::ObjectReference& child :
      ...
      // https://github.com/machina/machina/blob/ad66f588c1666ade8051feb42811fa27b285271c/machina/python/training/tracking/tracking.py#L277-L281
