...
#include "machina/c/experimental/saved_model/public/signature_def_param_list.h"
...
TEST_P(CSavedModelAPITest, LoadsAssetSavedModel) {
  ...
  EXPECT_EQ(TF_NumDims(result), 0);
  machina::tstring* output_value =
      static_cast<machina::tstring*>(TF_TensorData(result));
...
TEST_P(CSavedModelAPITest, LoadSavedModelWithUninitializedVariable) {
  ...
  std::string model_dir = machina::io::JoinPath(
      machina::testing::TensorFlowSrcRoot(),
  ...
  machina::TFSavedModelAPI* model_api =
      machina::down_cast<machina::TFSavedModelAPI*>(
          machina::unwrap(saved_model));
  machina::Variable* uninitialized_variable;
  ASSERT_EQ(absl::OkStatus(), model_api->GetVariable("uninitialized_variable",
                                                     &uninitialized_variable));
  ASSERT_EQ(machina::DT_FLOAT, uninitialized_variable->dtype());

  ASSERT_EQ(absl::OkStatus(),
            model_api->GetVariable("sub_module.uninitialized_variable",
                                   &uninitialized_variable));
  ASSERT_EQ(machina::DT_INT64, uninitialized_variable->dtype());
