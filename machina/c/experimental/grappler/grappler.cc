...
void TF_GetNodesToPreserveListSize(const TF_GrapplerItem* item, int* num_values,
  ...
  const std::unordered_set<std::string>& nodes =
      reinterpret_cast<const machina::grappler::GrapplerItem*>(item)
          ->NodesToPreserve();
...
void TF_GetFetchNodesListSize(const TF_GrapplerItem* item, int* num_values,
  ...
  const std::vector<std::string>& nodes =
      reinterpret_cast<const machina::grappler::GrapplerItem*>(item)->fetch;
  *num_values = nodes.size();
  *storage_size = 0;
  for (const std::string& str : nodes) {
    *storage_size += str.size();
...
void TF_InferStatically(TF_GraphProperties* graph_properties,
  ...
  TF_SetStatus(status, TF_OK, "");
  absl::Status s =
      reinterpret_cast<machina::grappler::GraphProperties*>(graph_properties)
...
void TF_GetInputPropertiesList(TF_GraphProperties* graph_properties,
  ...
  TF_SetStatus(status, TF_OK, "");
  const std::vector<machina::OpInfo::TensorProperties>& tensor_properties =
      reinterpret_cast<machina::grappler::GraphProperties*>(graph_properties)
          ->GetInputProperties(name);
  const int len =
  ...
  for (int i = 0; i < len; ++i) {
    absl::Status s =
        machina::MessageToBuffer(tensor_properties[i], properties[i]);
    if (!s.ok()) {
      tsl::Set_TF_Status_from_Status(status, s);
      return;
    }
...
TF_FunctionLibraryDefinition* TF_NewFunctionLibraryDefinition(
  ...
  absl::Status s = machina::BufferToMessage(graph_buf, &graph_def);
  if (!s.ok()) {
    tsl::Set_TF_Status_from_Status(status, s);
    return nullptr;
  }
  return reinterpret_cast<TF_FunctionLibraryDefinition*>(
      new machina::FunctionLibraryDefinition(
          machina::OpRegistry::Global(), graph_def.library()));
}

void TF_DeleteFunctionLibraryDefinition(TF_FunctionLibraryDefinition* fn_lib) {
  if (fn_lib == nullptr) return;
  delete reinterpret_cast<machina::FunctionLibraryDefinition*>(fn_lib);
}

void TF_LookUpOpDef(TF_FunctionLibraryDefinition* fn_lib, const char* name,
                    TF_Buffer* buf, TF_Status* status) {
  TF_SetStatus(status, TF_OK, "");
  const machina::OpDef* op_def_ptr = nullptr;
  absl::Status s =
      reinterpret_cast<machina::FunctionLibraryDefinition*>(fn_lib)
  ...
  s = machina::MessageToBuffer(*op_def_ptr, buf);
  if (!s.ok()) {
    tsl::Set_TF_Status_from_Status(status, s);
    return;
  }
