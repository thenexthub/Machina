...
TEST(Grappler, DeviceTypeNotSet) {
  ...
  ASSERT_EQ(status.code(), machina::error::FAILED_PRECONDITION);
  ASSERT_EQ(
      status.message(),
...
TEST(TF_GraphProperties, InputProperties) {
  ...
  for (const NodeDef& node : item.graph.node()) {
    if (node.op() == "AddN") {
      ...
      machina::OpInfo::TensorProperties in_props;
      absl::Status s = machina::BufferToMessage(in_props_buf[0], &in_props);
      TF_ASSERT_OK(s);
...
TEST(TF_GraphProperties, OutputProperties) {
  ...
  for (const NodeDef& node : item.graph.node()) {
    if (node.op() == "AddN") {
      ...
      machina::OpInfo::TensorProperties out_props;
      absl::Status s =
          machina::BufferToMessage(out_props_buf[0], &out_props);
      TF_ASSERT_OK(s);
