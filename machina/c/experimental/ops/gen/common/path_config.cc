...
PathConfig::PathConfig(const string& output_dir, const string& source_dir,
  ...
  //
  // Be flexible; accept any path string with a "machina" directory name.
  ...
  // Prefix, e.g. "third_party" given root_dir "third_party/machina/...."
  std::vector<string> source_path_components =
      machina::str_util::Split(source_dir, "/");
  ...
  // TF subdir, e.g. "c/ops" given output_dir "blah/blah/machina/c/ops"
  std::vector<string> output_path_components =
      machina::str_util::Split(output_dir, "/");
