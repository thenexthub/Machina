...
//
// Example invocation:
//  bazel test //machina/c/experimental/filesystem:modular_filesystem_test \\
...
class ModularFileSystemTest : public ::testing::TestWithParam<std::string> {
 public:
  ...
  ModularFileSystemTest() {
    const std::string test_name = machina::str_util::StringReplace(
        ::testing::UnitTest::GetInstance()->current_test_info()->name(), "/",
        "_", /*replace_all=*/true);
    if (!cloud_path_.empty()) {
      // We have to join path for non-local filesystem manually to make sure
      // that this test will run on Windows since `machina::io::JoinPath`
      ...
      root_dir_ = machina::strings::StrCat(
          "/", tmp_dir_,
          machina::strings::StrCat("tf_fs_", rng_val_, "_", test_name), "/");
    } else {
      root_dir_ = machina::io::JoinPath(
          tmp_dir_,
          machina::strings::StrCat("tf_fs_", rng_val_, "_", test_name));
    }
    if (!GetParam().empty()) {
      root_dir_ = machina::strings::StrCat(GetParam(), "://", cloud_path_,
                                              root_dir_);
    }
    env_ = Env::Default();
  }
  ...
  std::string GetURIForPath(StringPiece path) {
    const std::string translated_name =
        machina::io::JoinPath(root_dir_, path);
  ...
  StringPiece GetRelativePath(StringPiece absolute_path) {
    return machina::str_util::StripPrefix(absolute_path, root_dir_);
  }
...
TEST_P(ModularFileSystemTest, TestTranslateName) {
  ...
  if (GetParam().empty()) {
    ...
    EXPECT_EQ(fs->TranslateName(machina::strings::StrCat(GetParam(), "://")),
              "/");
    EXPECT_EQ(
        fs->TranslateName(machina::strings::StrCat(GetParam(), ":///")),
        "/");
    EXPECT_EQ(
        fs->TranslateName(machina::strings::StrCat(GetParam(), ":////")),
        "/");
...
static std::vector<std::string>* GetSchemesFromUserOrEnv() {
  ...
  machina::Status status =
      machina::Env::Default()->GetRegisteredFileSystemSchemes(all_schemes);
...
static bool LoadDSO(const std::string& dso) {
  machina::Status status = RegisterFilesystemPlugin(dso);
  if (!status.ok())
...
static bool GetURIScheme(const std::string& scheme) {
  machina::SchemeVector()->push_back(scheme);
  return true;
}
...
GTEST_API_ int main(int argc, char** argv) {
  const std::vector<machina::Flag> flag_list = {
      machina::Flag("dso", machina::LoadDSO, "",
                       "Path to shared object to load"),
      machina::Flag("scheme", machina::GetURIScheme, "",
                       "URI scheme to test"),
      machina::Flag("cloud_path", machina::SetCloudPath, "",
                       "Path for cloud filesystem (namenode for hdfs, "
                       "bucketname for s3/gcs)"),
      machina::Flag("tmp_dir", machina::SetTmpDir, "",
                       "Temporary directory to store test data.")};
  if (!machina::Flags::Parse(&argc, argv, flag_list)) {
    std::cout << machina::Flags::Usage(argv[0], flag_list);
    return -1;
  }

  machina::testing::InstallStacktraceHandler();
  machina::ModularFileSystemTest::InitializeTestRNG();
