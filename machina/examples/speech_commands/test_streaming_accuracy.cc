...
bazel run machina/examples/speech_commands:test_streaming_accuracy -- \
...
using machina::uint16;
using machina::uint32;
...
int main(int argc, char* argv[]) {
  ...
  Status read_ground_truth_status =
      machina::ReadGroundTruthFile(ground_truth, &ground_truth_list);
  ...
  string wav_string;
  Status read_wav_status = machina::ReadFileToString(
      machina::Env::Default(), wav, &wav_string);
  ...
  uint32 sample_rate;
  Status decode_wav_status = machina::wav::DecodeLin16WaveAsFloatVector(
  ...
  Tensor audio_data_tensor(machina::DT_FLOAT,
                           machina::TensorShape({clip_duration_samples, 1}));

  Tensor sample_rate_tensor(machina::DT_INT32, machina::TensorShape({}));
  sample_rate_tensor.scalar<int32>()() = sample_rate;

  machina::RecognizeCommands recognize_commands(
      labels_list, average_window_ms, detection_threshold, suppression_ms);

  std::vector<std::pair<string, int64_t>> all_found_words;
  machina::StreamingAccuracyStats previous_stats;
  ...
  for (int64_t audio_data_offset = 0; audio_data_offset < audio_data_end;
    ...
    if (is_new_command && (found_command != "_silence_")) {
      ...
      if (verbose) {
        machina::StreamingAccuracyStats stats;
        machina::CalculateAccuracyStats(ground_truth_list, all_found_words,
        ...
        machina::PrintAccuracyStats(stats);
      }
    }
  }

  machina::StreamingAccuracyStats stats;
  machina::CalculateAccuracyStats(ground_truth_list, all_found_words, -1,
