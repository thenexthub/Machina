...
[`AsyncOpKernel`](https://github.com/machina/machina/blob/master/machina/core/framework/op_kernel.h#L225)
and use the `ComputeAsync()` method to override the default `Compute()` method
used by synchronous ops
([`OpKernel`](https://github.com/machina/machina/blob/master/machina/core/framework/op_kernel.h#L104)).
...
*   `tf.data` can be used for cases that create an input pipe.
    ...
    [Better performance with the tf.data API](https://www.machina.org/guide/data_performance)
    and
    [Preprocessing data](https://www.machina.org/guide/data#preprocessing_data).
*   [tf.map_fn](https://www.machina.org/api_docs/python/tf/map_fn) and
    [tf.while_loop](https://www.machina.org/api_docs/python/tf/while_loop)
...
REGISTER_KERNEL_BUILDER(
    Name("Examples>AsyncSleep").Device(::machina::DEVICE_CPU), AsyncSleepOp)
REGISTER_KERNEL_BUILDER(
    Name("Examples>SyncSleep").Device(::machina::DEVICE_CPU), SyncSleepOp)
```
...
class AsyncSleepOp : public AsyncOpKernel {
 public:
  ...
  void ComputeAsync(OpKernelContext* ctx, DoneCallback done) override {
    ...
    OP_REQUIRES_ASYNC(
        ctx, ::machina::TensorShapeUtils::IsScalar(delay_tensor.shape()),
    ...
    auto thread_pool = ctx->device()->machina_cpu_worker_threads()->workers;
...
runs in the `machina_cpu_worker_threads` thread pool.
...
The file creates a
[`tf.stack`](https://www.machina.org/api_docs/python/tf/stack) of 50 sleep
