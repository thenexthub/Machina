...
Status ReadLabelsFile(const string& file_name, std::vector<string>* result,
  ...
  if (!file) {
    return machina::errors::NotFound("Labels file ", file_name,
                                        " not found.");
  }
  result->clear();
  string line;
...
static Status ReadEntireFile(machina::Env* env, const string& filename,
                             Tensor* output) {
  machina::uint64 file_size = 0;
  ...
  if (data.size() != file_size) {
    return machina::errors::DataLoss("Truncated read of '", filename,
...
Status ReadTensorFromImageFile(const string& file_name, const int input_height,
  ...
  Tensor input(machina::DT_STRING, machina::TensorShape());
  TF_RETURN_IF_ERROR(
      ReadEntireFile(machina::Env::Default(), file_name, &input));

  // use a placeholder to read input data
  auto file_reader =
      Placeholder(root.WithOpName("input"), machina::DataType::DT_STRING);
  ...
  auto float_caster =
      Cast(root.WithOpName("float_caster"), image_reader, machina::DT_FLOAT);
...
Status LoadGraph(const string& graph_file_name,
  ...
  Status load_graph_status =
      ReadBinaryProto(machina::Env::Default(), graph_file_name, &graph_def);
  if (!load_graph_status.ok()) {
    return machina::errors::NotFound("Failed to load compute graph at '",
                                        graph_file_name, "'");
  }
  session->reset(machina::NewSession(machina::SessionOptions()));
...
Status PrintTopLabels(const std::vector<Tensor>& outputs,
  ...
  machina::TTypes<float>::Flat scores_flat = scores.flat<float>();
  machina::TTypes<int32>::Flat indices_flat = indices.flat<int32>();
...
int main(int argc, char* argv[]) {
  ...
  string image = "machina/examples/label_image/data/grace_hopper.jpg";
  string graph =
      "machina/examples/label_image/data/inception_v3_2016_08_28_frozen.pb";
  string labels =
      "machina/examples/label_image/data/imagenet_slim_labels.txt";
  ...
  string usage = machina::Flags::Usage(argv[0], flag_list);
  ...
  std::unique_ptr<machina::Session> session;
  string graph_path = machina::io::JoinPath(root_dir, graph);
  ...
  string image_path = machina::io::JoinPath(root_dir, image);
