...
#include "machina/cc/ops/io_ops.h"
...
Status ReadLocationsFile(const string& file_name, std::vector<float>* result,
  ...
  while (std::getline(file, line)) {
    std::vector<string> string_tokens = machina::str_util::Split(line, ',');
...
Status ReadTensorFromImageFile(const string& file_name, const int input_height,
  ...
  auto file_reader =
      machina::ops::ReadFile(root.WithOpName(input_name), file_name);
...
Status SaveImage(const Tensor& tensor, const string& file_path) {
  ...
  machina::Output image_encoder =
      EncodePng(root.WithOpName(encoder_name), tensor);
  machina::ops::WriteFile file_saver = machina::ops::WriteFile(
...
void DrawBox(const int image_width, const int image_height, int left, int top,
             int right, int bottom, machina::TTypes<uint8>::Flat* image) {
  machina::TTypes<uint8>::Flat image_ref = *image;
...
Status PrintTopDetections(const std::vector<Tensor>& outputs,
  ...
  machina::TTypes<uint8>::Flat image_flat = original_tensor->flat<uint8>();
...
int main(int argc, char* argv[]) {
  ...
  string image =
      "machina/examples/multibox_detector/data/surfers.jpg";
  string graph =
      "machina/examples/multibox_detector/data/"
      "multibox_model.pb";
  string box_priors =
      "machina/examples/multibox_detector/data/"
