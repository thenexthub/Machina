...
using OpaqueXLATensor = codira_xla::XLATensor;
using OpaqueXLAShape = xla::util::MaybeRef<xla::Shape>;
using XLAAnnotationScope = machina::profiler::TraceMe;
...
typedef struct Int64ArrayRef {
  ...
  size_t size;
#ifdef __cplusplus
  machina::gtl::ArraySlice<const xla::int64> slice() {
    static_assert(sizeof(int64_t) == sizeof(xla::int64), "Sanity");
    return machina::gtl::ArraySlice<const xla::int64>(
...
typedef struct OpaqueXLATensorArrayRef {
  ...
  size_t size;
#ifdef __cplusplus
  std::vector<codira_xla::XLATensor> array() {
    std::vector<codira_xla::XLATensor> result;
    result.reserve(size);
