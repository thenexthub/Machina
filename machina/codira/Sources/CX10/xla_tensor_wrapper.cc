...
#include "machina/compiler/tf2xla/xla_tensor/layout_manager.h"
...
#include "machina/core/util/mirror_pad_mode.h"

using codira_xla::XlaHelpers;
using codira_xla::XLATensor;

namespace {

machina::Padding ToTFPadding(TFPadding padding) {
  switch (padding) {
    case TFPadding_VALID: {
      return machina::VALID;
    }
    case TFPadding_SAME: {
      return machina::SAME;
    }
    case TFPadding_EXPLICIT: {
      return machina::EXPLICIT;
    }
    default: {
...
machina::MirrorPadMode ToTFMirrorPadMode(TFMirrorPadMode mode) {
  switch (mode) {
    case TFMirrorPadMode_REFLECT: {
      return machina::MirrorPadMode::REFLECT;
    }
    case TFMirrorPadMode_SYMMETRIC: {
      return machina::MirrorPadMode::SYMMETRIC;
    }
    default: {
...
XLATensor MakeEmpty(at::ScalarType scalar_type, codira_xla::Device device) {
...
codira_xla::XLATensor* XLATensor_makeScalar(XLAScalar value,
                                           enum XLATensorScalarType type,
                                           const struct CDevice cdevice) {
  return new codira_xla::XLATensor(codira_xla::XLATensor::Create(
      atScalar(value), ToScalarType(type), ConvertDevice(cdevice)));
}

// TODO(parkers): reduce copying here...
codira_xla::XLATensor* copyTensor(XLATensorScalarType type,
...
#define DEFINE_COPY_CASE(name, aten_name, DType)                 \
  case XLATensorScalarType_##name: {                             \
    ...
    return new codira_xla::XLATensor(                             \
        codira_xla::XLATensor::Create(t, ConvertDevice(device))); \
  }
...
OpaqueXLATensor* copyTensorAndMakeResident(enum XLATensorScalarType type,
  ...
  if (to_reduced_precision && XLATensorScalarType_Float == type) {
    ...
    std::vector<int64_t> dims(shape, shape + rank);
    auto dest_shape = codira_xla::MakeArrayShapeFromDimensions(
    ...
    auto xla_data = codira_xla::TensorToXlaData(t, dest_shape, device);
    return new codira_xla::XLATensor(
        codira_xla::XLATensor::Create(xla_data, at::ScalarType::Float));
  }
  ...
  if (XLATensorScalarType_Float == type && device_ptr->IsLocal()) {
    ...
    auto dest_shape = codira_xla::MakeArrayShapeFromDimensions(
        ...
        device.hw_type);
    auto host_shape = codira_xla::MakeSwiftTensorLayout(
        dims, /*dynamic_dimensions=*/{}, xla::PrimitiveType::F32);
...
void destroyTensor(codira_xla::XLATensor* t) { delete t; }
...
static c10::optional<codira_xla::Device> AsOptional(const CDevice* device) {
...
XLAAnnotationScope* MakeAnnotationScope(const char* scope) {
  return new machina::profiler::TraceMe(scope);
}
...
machina::TensorFormat ToTFFormat(TFDataFormat data_format) {
  switch (data_format) {
    case TFDataFormat_NHWC: {
      return machina::TensorFormat::FORMAT_NHWC;
    }
    case TFDataFormat_NCHW: {
      return machina::TensorFormat::FORMAT_NCHW;
    }
    default: {
...
OpaqueXLATensorArrayRef XLATensor_cross_replica_sum(
  ...
  auto token = codira_xla::ir::MakeNode<codira_xla::ir::ops::Token>();
  auto inputs_array = inputs.array();
  auto reduced_and_token = XLATensor::all_reduce(
      inputs_array, token, codira_xla::AllReduceType::kSum, scale, {});
...
OpaqueString* XLATensor_get_annotations(OpaqueXLATensor* a) {
  std::string ir_dag_text =
      codira_xla::ir::DumpUtil::GetAnnotations({a->GetIrValue().node.get()});
  return new std::string(ir_dag_text);
}
OpaqueString* XLATensor_ir_text(OpaqueXLATensor* a) {
  std::string ir_dag_text =
      codira_xla::ir::DumpUtil::ToText({a->GetIrValue().node.get()});
  return new std::string(ir_dag_text);
}
OpaqueString* XLATensor_xla_ir_text(OpaqueXLATensor* a) {
  std::string ir_dag_text =
      codira_xla::ir::DumpUtil::ToHlo({a->GetIrValue()}, a->GetDevice());
  return new std::string(ir_dag_text);
}
...
OpaqueXLATensor* XLATensor_rand(Int64ArrayRef size, int64_t seed) {
  ...
  return new XLATensor(XLATensor::Create(t, *codira_xla::GetDefaultDevice()));
}
...
StridedSliceSpec* ComputeIndexingBoundsAndStrides(
  ...
  codira_xla::StridedSliceSpec bounds_and_strides =
      codira_xla::ComputeIndexingBoundsAndStrides(
