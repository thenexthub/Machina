...
using codira_xla::ir::LoweringContext;
using codira_xla::ir::Node;
using codira_xla::ir::NodePtr;
using codira_xla::ir::OpList;
using codira_xla::ir::Output;
using codira_xla::ir::Value;
using codira_xla::ir::XlaOpVector;
...
class XLAFunctionalWhileNode : public codira_xla::ir::Node {
 public:
  ...
  XLAFunctionalWhileNode(absl::Span<const Value> initial, const Value& n,
      ...
      : Node(codira_xla::ir::OpKind(at::aten::functional_while),
             BuildArgs(
                 initial, n,
  ...
  static xla::XlaOp zeroLike(xla::XlaOp op) {
    ...
    return xla::ConstantLiteral(
        b, xla::LiteralUtil::Zero(
               codira_xla::XlaHelpers::ShapeOfXlaOp(op).element_type()));
  }
  ...
  XlaOpVector Lower(LoweringContext* loctx) const {
    ...
    xla::XlaOp body_result;
    {
      ...
      codira_xla::ir::Util::EmissionMap emap;
      for (const auto& placeholder : placeholders_) {
        emap[placeholder.node.get()] = codira_xla::ir::Util::kEmitted;
      }
      for (size_t i = last_i + 1; i < operands().size(); ++i) {
        emap[operand(i).node] = codira_xla::ir::Util::kEmitted;
      }
      emap[index_placeholder_.node.get()] = codira_xla::ir::Util::kEmitted;
      codira_xla::ir::LoweringContext body_loctx(b, loctx->device(),
                                                std::move(emap));
      auto t = xla::Parameter(
          b, 0, codira_xla::XlaHelpers::ShapeOfXlaOp(initial), "tuple");
...
class XLAPlaceholderNode : public codira_xla::ir::Node {
 public:
  XLAPlaceholderNode(xla::Shape shape, int id)
      : Node(codira_xla::ir::OpKind(at::aten::placeholder), {}, shape, 1,
             xla::util::MHash(id)),
        id_(id) {}
  NodePtr Clone(OpList operands) const override {
    return codira_xla::ir::MakeNode<XLAPlaceholderNode>(shape(), id_);
  }
...
OpaqueXLATensorArrayRef XLATensor_functional_while(
  ...
  auto result_node = codira_xla::ir::MakeNode<XLAFunctionalWhileNode>(
  ...
  for (size_t i = 0; i < count; ++i) {
    opaque_tensors[i] = new XLATensor(
        results.data[i]->CreateFrom(codira_xla::ir::Value(result_node, i)));
  }
...
OpaqueXLATensor* XLATensor_makePlaceholder(OpaqueXLATensor* t, int id) {
  return new XLATensor(t->CreateFrom(
      codira_xla::ir::MakeNode<XLAPlaceholderNode>(t->shape(), id)));
}
