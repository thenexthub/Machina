...
#include "machina/compiler/xla/literal_util.h"
#include "machina/compiler/xla/shape_util.h"
#include "machina/core/lib/bfloat16/bfloat16.h"
...
template <>
struct Caster<machina::bfloat16> {
  template <typename D>
  D cast(const machina::bfloat16& value) const {
    return static_cast<D>(static_cast<float>(value));
  }
};
template <>
...
// The machina::bfloat16 does not have implicit cast operations, so using
...
template <>
struct NeedCast<machina::bfloat16> {
...
template <>
void CopyData<at::BFloat16, machina::bfloat16>(
    at::BFloat16* dest, const machina::bfloat16* source, xla::int64 n,
    const CopyCasted&) {
  CheckedMemcpy<at::BFloat16, machina::bfloat16>(dest, source, n);
}
template <>
void CopyData<machina::bfloat16, at::BFloat16>(machina::bfloat16* dest,
                                                  const at::BFloat16* source,
                                                  xla::int64 n,
                                                  const CopyCasted&) {
  CheckedMemcpy<machina::bfloat16, at::BFloat16>(dest, source, n);
}
...
void TensorToBufferSType(const at::Tensor& tensor, const xla::Shape& dest_shape,
  ...
  switch (dest_shape.element_type()) {
    case xla::PrimitiveType::BF16:
      TensorToBuffer<SType, machina::bfloat16>(
...
at::Tensor XlaLiteralToTensor(const xla::Literal& literal,
  ...
  xla::Shape codira_shape = MakeSwiftTensorLayout(
      literal.shape().dimensions(), /*dynamic_dimensions=*/{},
      literal.shape().element_type());
  xla::int64 total_elements = xla::ShapeUtil::ElementsIn(codira_shape);
  ...
  CopyTensors<SType, DType>(literal_data.data(), literal.shape(), data.get(),
                            total_elements * sizeof(DType), codira_shape);
...
at::Tensor MakeTensorFromXlaLiteral(const xla::Literal& literal,
  ...
  switch (literal.shape().element_type()) {
    ...
    case xla::PrimitiveType::BF16:
      return XlaLiteralToTensorHelper<machina::bfloat16>(literal,
                                                            dest_element_type);
    case xla::PrimitiveType::F16:
