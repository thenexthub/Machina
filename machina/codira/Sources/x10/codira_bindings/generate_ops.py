...
flags.DEFINE_string("codira_output", None, "path for the generated codira file")
...
def c_function_define(op):
  ...
  if not first_tensor:
    if tensor_args[0][1] == "[Tensor]":
      first_tensor = f"codira_xla::FirstTensor({tensor_args[0][0]})"
    else:
  ...
  def format_arg_ref(arg):
    ...
    if name == "shape":
      ...
      if result_dtype_arg:
        relement_type = (
            f"codira_xla::MakeXlaPrimitiveType({format_arg_ref(result_dtype_arg)},"
            f" /*device=*/nullptr)")
      return ("codira_xla::MakeArrayShapeFromDimensions(shape.slice(), {}, " +
              f"{relement_type}, "
    ...
    for extra in op["extras"]:
      if extra[0] == "canonicalize" and extra[1] == name:
        if stype == "[Int64]":
          if len(extra) == 4:
            return (f"codira_xla::ir::ops::{extra[3]}({extra[2]}_ir_value.shape(),"
                    f" {name}.slice())")
          else:
            return f"codira_xla::XlaHelpers::GetCanonicalDimensionIndices({name}.slice(), {extra[2]}_ir_value.shape().rank())"
        else:
          if len(extra) == 4:
            return (
                f"codira_xla::ir::ops::{extra[3]}({extra[2]}_ir_value, {name})")
          return f"codira_xla::XlaHelpers::GetCanonicalDimensionIndex({name}, {extra[2]}_ir_value.shape().rank())"
    ...
    if stype == "[Int64]":
      return f"codira_xla::XlaHelpers::I64List({name}.slice())"
    return name
  def unpack_arg(arg):
    name, stype, _ = arg
    if stype == "Tensor": return f"  auto {name}_ir_value = {name}->GetIrValue();\n"
    if stype == "[Tensor]":
      return f"  auto {name}_ir_value = codira_xla::UnpackIrValues({name});\n"
    return ""
  node_ctor = f"""codira_xla::ir::MakeNode<codira_xla::ir::ops::{op["op_node_name"]}>({", ".join(format_arg_ref(arg) for arg in op["args"])})"""
  result_type = None
  if op["n_results"] == 1:
  ...
  def format_result(result_i=0, dtype=None):
    ...
    if not dtype:
      return (f"new "
              f"codira_xla::XLATensor({first_tensor}->CreateFrom(codira_xla::ir::Value(result_node,"
              f" {result_i})))")
    if dtype in tensor_names:
      return f"new codira_xla::XLATensor({dtype}->CreateFrom(codira_xla::ir::Value(result_node, {result_i})))"
    result_dtype_arg = None
    for arg in args:
      if arg[0] == dtype:
        result_dtype_arg = arg
...
def codira_wrapper_define(op):
  args = op["args"]
  results = op["results"]

  def format_codira_arg(arg):
  ...
  if generics:
    generics = f"<{generics}>"
  args_gen = format_args(("\n    " + format_codira_arg(arg) for arg in args),
                         comma=",",
                         ending="\n  ")
...
"""
  ...
  return f"""
  {protection}static func {op["codira_name"]}{generics}({args_gen}) -> {results_gen} {{{defers}
{body}  }}
"""
...
def canonicalize_op(op):
  ...
  if "codira_name" not in op:
    op["codira_name"] = op["c_name"]
  if "extras" in op:
    op["extras"] = [a.split() for a in op["extras"]]
  else:
    op["extras"] = []
  # If there are type annotations, ensure codira wrapper generation.
  is_codira = not ([erase_generics(r[1]) for r in results
                  ] == [r[1] for r in results])
  if is_codira and "codira_namespace" not in op:
    op["codira_namespace"] = "_RawXLA"
  if is_codira and "generics" not in op:
    op["generics"] = {}
...
""" + "".join(c_function_define(op) for op in op_list))
  ...
  open(FLAGS.codira_output, "w+").write(HEADER + """
@_implementationOnly import x10_xla_tensor_tf_ops
@_implementationOnly import x10_xla_tensor_wrapper

extension _RawXLA {
""" + ("".join(
    codira_wrapper_define(op)
    for op in op_list
    if "codira_namespace" in op and op["codira_namespace"] == "_RawXLA")) + """
}
""")
  for op in op_list:
    if not ("codira_namespace" in op and op["codira_namespace"] == "_RawXLA"):
      print(f"""Missing codira types: {op["op_node_name"]}""")
