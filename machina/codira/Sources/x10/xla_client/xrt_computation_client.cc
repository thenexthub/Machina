...
#include "machina/compiler/xla/xla_client/xrt_computation_client.h"
...
class TensorAllocator : public machina::Allocator {
  struct AllocKey {
    struct Hash {
...
machina::DeviceNameUtils::ParsedName ParseFullXrtDevice(
    const std::string& device) {
  machina::DeviceNameUtils::ParsedName parsed_device;
  XLA_CHECK(
      machina::DeviceNameUtils::ParseFullName(device, &parsed_device) &&
...
machina::DeviceNameUtils::ParsedName ParseXrtDevice(
    const std::string& device) {
  machina::DeviceNameUtils::ParsedName parsed_device;
  XLA_CHECK(
      machina::DeviceNameUtils::ParseFullName(device, &parsed_device) &&
...
bool IsLocalDevice(const XrtComputationClient::Worker& worker,
                   const machina::DeviceNameUtils::ParsedName& parsed_device,
...
bool ParseMeshConfig(
    XrtComputationClient::Options* options,
    std::unique_ptr<machina::tpu::TopologyProto>* topology_proto) {
  ...
  (*topology_proto) = absl::make_unique<machina::tpu::TopologyProto>(
      std::move(*config.mutable_proto()));
  return true;
}

template <typename T>
T ParseProto(const machina::Tensor& tensor) {
  const machina::tstring& tensor_data =
      tensor.scalar<machina::tstring>()();
...
bool GpuIsAvailable() {
  ...
  machina::Status s =
      machina::DeviceFactory::ListAllPhysicalDevices(&devices);
  XLA_CHECK_OK(s);
...
bool ParseEnvDeviceCounts(XrtComputationClient::Options* options) {
  ...
  if (num_tpus > 0 || num_gpus > 0) {
    ...
    std::string host_port =
        absl::StrCat("localhost:", machina::internal::PickUnusedPortOrDie());
...
bool ParseEnvDevices(XrtComputationClient::Options* options) {
  ...
  int port = machina::internal::PickUnusedPortOrDie();
...
XrtComputationClient::XrtComputationClient(
  ...
  machina::ConfigProto config = CreateConfigProto(options_);
...
std::vector<Literal> XrtComputationClient::TransferFromServerImpl(
  ...
  for (size_t i = 0; i < handles.size(); ++i) {
    ...
    SessionWork* session_work = &session_work_map[session];
    machina::Scope device_scope = session->root()->WithDevice(
  ...
  for (auto& session_work : session_work_map) {
    ...
    for (size_t i = 0; i < outputs.size(); ++i) {
      ...
      XLA_CHECK(
          response.ParseFromString(outputs[i].scalar<machina::tstring>()()));
...
XrtComputationClient::ExecuteComputation(
  ...
  machina::ClientSession::FeedType feed_inputs;
  std::vector<machina::Output> exec_ops = CreateExecuteOps(
...
XrtComputationClient::RunComputations(
    ...
    const std::vector<machina::Output>& exec_ops,
    absl::Span<const Computation* const> computations,
    absl::Span<const std::string> devices,
    const machina::ClientSession::FeedType& feed_inputs) {
  ...
  for (auto& sess_replica : session_replicas) {
    ...
    auto session_runner = [&, this, session]() {
      std::vector<machina::Output> exec_nodes;
...
XrtComputationClient::ExecuteParallel(
  ...
  XrtSessionCache::SessionMap session_map;
  machina::ClientSession::FeedType feed_inputs;
  std::vector<machina::Output> exec_ops =
...
std::vector<ComputationClient::DataPtr> XrtComputationClient::ExecuteChainedXrt(
  ...
  machina::Scope device_scope = session->root()->WithDevice(xrt_device);
...
XrtComputationClient::DeconstructTuple(absl::Span<const DataPtr> tuples) {
  ...
  for (size_t i = 0; i < tuples.size(); ++i) {
    ...
    for (int64 j = 0; j < count; ++j) {
      ...
      session_work->feed_inputs.insert(
          {cached_node.holders[0], xrt_data.get_handle()});
      machina::Tensor index_tensor(machina::DT_INT32,
                                      machina::TensorShape({1}));
      index_tensor.flat<machina::int32>()(0) = j;
...
machina::Tensor XrtComputationClient::GetArgumentsInputs(
    absl::Span<const DataPtr> arguments, const std::string& device) {
  machina::Tensor inputs_tensor(
      machina::DT_INT64,
      machina::TensorShape({static_cast<int64_t>(arguments.size())}));
  for (size_t i = 0; i < arguments.size(); ++i) {
    const XrtData& xrt_data = dynamic_cast<const XrtData&>(*arguments[i]);
    XLA_CHECK_EQ(device, xrt_data.device()->name());
    inputs_tensor.flat<machina::int64>()(i) = xrt_data.get_handle();
  }
  return inputs_tensor;
}

std::vector<machina::Output> XrtComputationClient::CreateExecuteOps(
    XrtSessionCache::SessionMap* session_map,
    absl::Span<const Computation* const> computations,
...
void XrtComputationClient::ReleaseHandles(
    ...
    const std::function<const XrtSession::CachedNode&(
        XrtSession*, const machina::Scope&, const std::string&)>&
        op_generator,
  ...
  if (!released_handles.empty()) {
    ...
    for (const auto& session_and_handles : session_handles_map) {
      ...
      machina::Tensor handles_tensor(
          machina::DT_INT64, machina::TensorShape({static_cast<int64_t>(
                                    session_handles.size())}));
      auto flat_handles_tensor = handles_tensor.flat<machina::int64>();
...
void XrtComputationClient::HandleReleaser() {
  auto data_op_generator =
      [this](XrtSession* session, const machina::Scope& scope,
...
machina::tpu::TopologyProto XrtComputationClient::InitializeAndFetchTopology(
    const std::string& job, int task_no, const std::string& worker_host_port,
    const machina::ConfigProto& config) {
  machina::SessionOptions session_options;
  session_options.env = machina::Env::Default();
  session_options.target = worker_host_port;
  session_options.config = config;

  machina::Scope root = machina::Scope::NewRootScope();
  machina::ClientSession session(root, session_options);
  std::string system_device = absl::StrCat(
      "/job:", job, "/replica:0/task:", task_no, "/device:TPU_SYSTEM:0");
  machina::Scope tpu_system_scope = root.WithDevice(system_device);
  const auto unique_name =
      tpu_system_scope.GetUniqueNameForOp("ConfigureDistributedTPU");
  machina::NodeBuilder builder =
      machina::NodeBuilder(unique_name, "ConfigureDistributedTPU")
  ...
  // Cloud TPU side.
  const machina::ClusterDef cluster_def = config.cluster_def();
  ...
  XLA_CHECK_OK(session.Run({machina::Output(result, 0)}, &outputs));
  XLA_CHECK_EQ(outputs.size(), 1);

  return ParseProto<machina::tpu::TopologyProto>(outputs[0]);
}
...
void XrtComputationClient::InitializeDevices(
  ...
  if (topology_proto == nullptr) {
    ...
    if (!tpu_workers.empty()) {
      ...
      machina::tpu::TopologyProto worker_topology_proto =
...
void XrtComputationClient::CreateMeshService(
    ...
    const machina::tpu::TopologyProto* topology_proto) {
  struct Device {
...
XrtComputationClient::GetComputationResults(
    const machina::Tensor& xrt_result, const Shape& result_shape,
...
std::map<std::string, Metric> XrtComputationClient::GetMetrics() const {
  static const std::map<std::string, std::string>* metric_remap =
      new std::map<std::string, std::string>{
          {"/machina/xrt/ops/allocate", "XrtAllocate"},
          {"/machina/xrt/ops/allocate_from_tensor", "XrtAllocateFromTensor"},
          {"/machina/xrt/ops/sub_tuple", "XrtSubTuple"},
          {"/machina/xrt/ops/make_tuple", "XrtMakeTuple"},
          {"/machina/xrt/ops/compile", "XrtCompile"},
          {"/machina/xrt/ops/release_compilation", "XrtReleaseCompilation"},
          {"/machina/xrt/ops/execute", "XrtExecute"},
          {"/machina/xrt/ops/execute_chained", "XrtExecuteChained"},
          {"/machina/xrt/ops/read_literal", "XrtReadLiteral"},
          {"/machina/xrt/ops/read_tensor", "XrtReadTensor"},
          {"/machina/xrt/ops/write_literal", "XrtWriteLiteral"},
          {"/machina/xrt/ops/release_allocation", "XrtReleaseAllocation"},
          {"/machina/xrt/ops/release_all_allocations",
           "XrtReleaseAllAllocations"},
          {"/machina/xrt/ops/compact_allocations", "XrtCompactAllocations"},
          {"/machina/xrt/memory_manager/compaction", "XrtCompaction"},
          {"/machina/xrt/memory_manager/try_free_memory",
           "XrtTryFreeMemory"},
          {"/machina/xrt/executor/program_memory_evict", "XrtExecutorEvict"},
          {"/machina/xrt/ds_executor/program_memory_evict",
           "XrtExecutorEvict"}};

  std::map<std::string, Metric> metrics_data;
  xrt::XRTMetricsCollect metrics;
  metrics.add_metrics_regex("/machina/xrt/.*");
  ...
  for (auto& worker_target : options_.workers_map) {
    ...
    machina::Scope cpu_system_scope = root.WithDevice(cpu0_device);
    auto metrics_value =
        machina::ops::Const(cpu_system_scope, metrics.SerializeAsString());
    machina::Output result =
        machina::ops::XRTMetricsCollect(cpu_system_scope, metrics_value);
...
void XrtComputationClient::InitSession(XrtSession* session) const {
  struct InitNode {
    ...
    const XrtSession::CachedNode& (XrtComputationClient::*node_ctor)(
        XrtSession*, const machina::Scope&, const std::string&) const;
  } const init_nodes[] = {
...
const XrtSession::CachedNode& XrtComputationClient::GetCompileNode(
    XrtSession* session, const machina::Scope& scope,
    const std::string& device) const {
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_STRING)});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTCompile(scope, holders[0]).handle, holders));
  }
  return cache->Get();
}
...
const XrtSession::CachedNode& XrtComputationClient::GetExecuteNode(
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_INT64),
         machina::ops::Placeholder(scope, machina::DT_STRING),
         machina::ops::Placeholder(
             scope, machina::DT_INT64,
             machina::ops::Placeholder::Shape({-1}))});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTExecute(scope, holders[0], holders[1],
                                    {machina::Output(holders[2])}),
        holders));
  }
  return cache->Get();
}
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_STRING),
         machina::ops::Placeholder(scope, machina::DT_STRING)});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTExecuteChained(scope, holders[0], holders[1]),
        holders));
  }
  return cache->Get();
}
...
const XrtSession::CachedNode& XrtComputationClient::GetReadNode(
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_INT64)});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTReadLiteral(scope, holders[0]), holders));
  }
  return cache->Get();
}
...
const XrtSession::CachedNode& XrtComputationClient::GetAllocateNode(
  ...
  if (cache->Empty()) {
    ...
    machina::TensorShape tensor_shape(shape.dimensions());
    machina::TensorShape equiv_tensor_shape =
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(
            ...
            machina::ops::Placeholder::Shape(equiv_tensor_shape))});
    machina::ops::XRTAllocateFromTensor::Attrs alloc_attrs =
        machina::ops::XRTAllocateFromTensor::Layouts(layout);
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTAllocateFromTensor(scope, {holders[0].output},
...
XrtComputationClient::GetReleaseAllocationHandleNode(
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_INT64)});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTReleaseAllocationHandle(scope, holders[0]),
        holders));
  }
  return cache->Get();
}
...
const XrtSession::CachedNode& XrtComputationClient::GetReleaseCompileHandleNode(
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_INT64)});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTReleaseCompilationHandle(scope, holders[0]),
        holders));
  }
  return cache->Get();
}
...
const XrtSession::CachedNode& XrtComputationClient::GetSubTupleNode(
  ...
  if (cache->Empty()) {
    ...
    std::vector<machina::ops::Placeholder> holders(
        {machina::ops::Placeholder(scope, machina::DT_INT64),
         machina::ops::Placeholder(
             scope, machina::DT_INT32,
             machina::ops::Placeholder::Shape({1}))});
    cache->Add(std::make_shared<XrtSession::CachedNode>(
        machina::ops::XRTSubTuple(scope, holders[0], holders[1]), holders));
  }
  return cache->Get();
}

machina::DataType XrtComputationClient::XlaTypeToDataType(
    PrimitiveType dtype) {
  switch (dtype) {
...
machina::TensorShape XrtComputationClient::MakeEquivalentTensorShape(
    const Shape& shape) {
  Shape eqiv_shape =
      ShapeUtil::MakeShapeWithDescendingLayoutAndSamePhysicalLayout(shape);
  return machina::TensorShape(eqiv_shape.dimensions());
}
...
codira_xla::Device XrtComputationClient::GetDefaultDeviceStruct() const {
  return *codira_xla::GetDefaultDevice();
}
