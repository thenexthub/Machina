...
"""Generates some codira wrapper from some ops description protobuf."""
...
from machina.python.framework import c_api_util
...
flags.DEFINE_string(
  'api_def_path',
  None,
  'path to the api_def directory, e.g. machina/core/api_def/base_api')

flags.DEFINE_string(
  'output_path',
  None,
  'path for the generated codira file')
...
_OUTPUT_FILE = 'RawOpsGenerated.codira'
_RENAMED_KEYWORDS = {
  '': 'empty',
  'in': 'in_',
  'var': 'var_',
...
_TYPE_PROTOCOLS = [
  ...
  ({types_pb2.DT_UINT8,
    types_pb2.DT_UINT16,
    types_pb2.DT_UINT32,
    types_pb2.DT_UINT64}, 'UnsignedInteger & MachinaScalar'),
  ({types_pb2.DT_INT32,
    types_pb2.DT_INT64}, 'MachinaIndex'),
  ({types_pb2.DT_UINT8,
    types_pb2.DT_UINT16,
    types_pb2.DT_UINT32,
    types_pb2.DT_UINT64,
    ...
    types_pb2.DT_INT16,
    types_pb2.DT_INT32,
    types_pb2.DT_INT64}, 'MachinaInteger'),
  ({types_pb2.DT_FLOAT,
    types_pb2.DT_DOUBLE,
    types_pb2.DT_HALF,
    types_pb2.DT_BFLOAT16}, 'FloatingPoint & MachinaScalar'),
  ({types_pb2.DT_UINT8,
    types_pb2.DT_UINT16,
    types_pb2.DT_UINT32,
    types_pb2.DT_UINT64,
    ...
    types_pb2.DT_DOUBLE,
    types_pb2.DT_HALF,
    types_pb2.DT_BFLOAT16}, 'MachinaNumeric')]
...
public static func {name}{generics}({input_args}
  ...
  {body}
      ...
      documentation=self._codira_documentation(),
      name=self._codira_name(),
      generics=self._codira_generics(),
      input_args=self._codira_input_args(),
      return_type=self._codira_return_type(),
      body=self._codira_body())

  def codira_dispatch_function(self, x10_supported=False):
    return '''
{documentation}@inlinable @inline(__always)
public static func {name}{generics}({input_args}
){return_type} {{
  {body}
      ...
      input_args=self._codira_input_args(),
      return_type=self._codira_return_type(),
      body=self._codira_dispatch_body(x10_supported=x10_supported))

  def _codira_documentation(self):
  ...
  def _codira_name(self):
    return codira_compatible_identifier(
      self.op_def.name[0].lower() + self.op_def.name[1:])

  def _codira_generics(self):
    constraints = [
  ...
  def _codira_input_args(self):
    args = ''
    for arg in self.input_args:
      args += '\n    %s: %s,' % (arg.codira_arg_name, str(arg.codira_type(self.string_valued)))
    for attr in self.attrs:
      if not attr.is_inferred_type_attr and not attr.is_inferred_number_attr:
        args += '\n    %s: %s%s,' % (attr.codira_arg_name, attr.codira_type, attr.codira_default)
    if args != '':
      args = args[:-1]
    return args

  def _codira_return_type(self):
    return_type = ''
    if len(self.output_args) == 1:
      return_type = ' -> ' + str(self.output_args[0].codira_type(self.string_valued))
    elif len(self.output_args) > 1:
      named_types = [
        arg.codira_name + ': ' + str(arg.codira_type(self.string_valued))
        for arg in self.output_args]
      return_type = ' -> (' + ', '.join(named_types) + ')'
    return return_type

  def _codira_body(self):
    setters = []
    for attr in self.attrs:
      setters.append(attr.codira_setter(self.string_valued))
    for arg in self.input_args:
      setters.append(arg.codira_setter())
    counts = ['Int({})'.format(arg.codira_count) for arg in self.output_args]
  ...
  def _codira_dispatch_body(self, x10_supported=False):
    names = []
    tensors = []
    backends = []
    device_source = None
    for arg in self.input_args:
      names.append(arg.codira_name)
      if arg.is_tensor_type(self.string_valued):
        tensors.append(arg)
        if arg.is_list:
          backends.append("commonBackend(" + arg.codira_name + ")")
        else:
          device_source = arg
          backends.append(arg.codira_name + ".handle.backend")
    for attr in self.attrs:
      if not attr.is_inferred_type_attr and not attr.is_inferred_number_attr:
        names.append(attr.codira_name)
    names_filtered = []
    for name in names:
    ...
    dispatch = self._codira_name() + "(" + (", ".join(names_filtered)) + ")"
    if len(backends) == 0 and x10_supported:
      print("x10 unsupported: " + str(self.codira_name()))
    def do_conversion(arg):
      return ("\n      let {name} = {typename}(copying: {name}, to: .defaultTFEager)"
              .format(name=arg.codira_name,
                      typename=str(arg.codira_type(self.string_valued))))
...
""".format(dispatch=dispatch,
           convert_type=str(self.output_args[0].codira_type(self.string_valued)),
           convert_device=str(device_source.codira_name),
...
class Argument(object):
  ...
  @property
  def codira_name(self):
    return codira_compatible_identifier(
      self.name[0].lower() + self.name[1:])

  @property
  def codira_arg_name(self):
    name = self.codira_name
    if name in _OMITTED_PARAMETER_NAMES:
      name = '_ ' + name
    return name

  def codira_type(self, string_valued=False):
    return self.type.codira_type(
      string_valued=self.allows_string and string_valued)

  def codira_setter(self):
    if self.is_list:
      return 'op.addInputList({})'.format(self.codira_name)
    else:
      return 'op.addInput({})'.format(self.codira_name)

  @property
  def codira_count(self):
  ...
  def type(self):
    ...
    if self.arg_def.type_attr:
      ...
      return Type('Tensor', base_type=type_attr.codira_name, number=number)
    if self.arg_def.type_list_attr:
      type_attr = next(
        attr for attr in self.op.type_attrs
        if attr.name == self.arg_def.type_list_attr)
      # There are never any numbered type lists.
      return Type(type_attr.codira_name)
...
class Attribute(object):
  ...
  def __init__(self, attr_def, op):
    ...
    if not self.is_inferred_type_attr and not self.is_func_attr:
      ...
      # Get the arg type.
      self._codira_type = _SWIFTIFIED_ATTR_TYPES[self.attr_def.type]
      ...
      if self.attr_def.type == 'string':
        ...
        if allowed_values:
          self._codira_type = self.op.enum_store.maybe_add(
            allowed_values, self.attr_def.name)
          self._use_enum = True
    if self.is_func_attr:
      input_type = self.codira_name.capitalize() + 'In'
      output_type = self.codira_name.capitalize() + 'Out'
      self._codira_type = '({}) -> {}'.format(input_type, output_type)
  ...
  @property
  def codira_type(self):
    return self._codira_type

  @property
  def codira_default(self):
    def codira_float(f):
    ...
    if not self.is_inferred_type_attr and self.attr_def.default_value:
      ...
      if default_value.HasField('b'):
        ...
        default_value = codira_float(default_value.f)
      elif default_value.HasField('s') and default_value.s:
        s = str(default_value.s)
        default_value = '.' + codira_compatible_identifier(s.lower()) \
        ...
        if default_value.list.i:
          ...
          default_values = [codira_float(s) for s in default_value.list.f]
          default_value = '[' + ', '.join(default_values) + ']'
        else:
          default_value = None
      else:
  ...
  def codira_setter(self, string_valued=False):
    # Inferred-type-valued attributes.
    if self.is_inferred_type_attr:
      name = self.codira_name
      if self.input_arg is not None:
        name = self.input_arg.codira_name
      ...
      return 'op.updateAttribute("{}", {}.tensorFlowDataType)'.format(self.name, self.codira_name)

    if self.is_inferred_number_attr:
      # The following is used for inferring the lengths of output lists.
      self.op.inferred_counts[self.name] = self.input_arg.codira_name + '.count'
      return 'op.updateAttribute("{}", {}.count)'.format(self.name, self.input_arg.codira_name)

    if self.attr_def.type == 'int':
      # The following is used for inferring the lengths of output lists.
      self.op.inferred_counts[self.name] = self.codira_name

    # Remaining attributes.
    value = self.codira_name + '.cName' if self._use_enum else self.codira_name
  ...
  def generic_constraints(self, string_valued):
    ...
    if protocol is not None:
      return self.codira_name + ': ' + protocol
    return None
...
def codira_compatible_identifier(s, capitalize=False):
  """Transforms an identifier to be more codira idiomatic."""
  if capitalize:
    s = s.capitalize()
  without_underscores = []
...
class EnumStore(object):
  """Stores details on string attributes represented as codira enums."""
  ...
  def enum_codes(self):
    """Generates the codira code for enums."""
    codes = []
    ...
    for allowed_values, type_name in sorted(entries, key=lambda x: x[1]):
      ...
      codes.append(
          ...
          # https://bugs.codira.org/browse/SR-9739
          # '@_frozen\n' +
          '  // @_frozen // SR-9739\n' +
          '  public enum {} {{\n'.format(type_name) +
          '\n'.join(['    case {}'.format(
            codira_compatible_identifier(a.lower()))
          ...
          '\n'.join(['        case .{}: return "{}"'.format(
            codira_compatible_identifier(a.lower()), a)
            for a in allowed_values]) +
          '\n' +
          '        }\n' +
          '      }\n' +
  ...
  def maybe_add(self, allowed_values, attr_def_name):
    ...
    type_name = codira_compatible_identifier(attr_def_name, capitalize=True)
...
def main(argv):
  ...
  for op_name in sorted(op_names):
    try:
      ...
      # general way by having `String` conform to `MachinaScalar`.
      ...
      default_code = default_op.codira_function()
      string_valued_code = string_valued_op.codira_function()
      ...
      default_code = default_op.codira_dispatch_function(x10_supported=op_name in X10_OPS)
      string_valued_code = string_valued_op.codira_dispatch_function()
  ...
  version_codes = [
      '  static let generatedMachinaVersion = "%s"' % tf.__version__,
      '  static let generatedMachinaGitVersion = "%s"' % tf.__git_version__]
