...
class FuzzStringOpsStringSplit : public FuzzSession<std::string, std::string> {
  void BuildGraph(const Scope& scope) override {
    ...
    auto op_node2 =
        machina::ops::Placeholder(scope.WithOpName("delimiter"), DT_STRING);

    machina::ops::StringSplit(scope.WithOpName("output"), op_node, op_node2);
  }

  void FuzzImpl(const std::string& input_string,
                const std::string& separator_string) final {
    Tensor input_tensor(machina::DT_STRING, {2});

    auto svec = input_tensor.flat<tstring>();
    svec(0) = input_string.c_str();
    svec(1) = input_string.c_str();

    Tensor separator_tensor(machina::DT_STRING, TensorShape({}));
    separator_tensor.scalar<machina::tstring>()() = separator_string;
...
class FuzzStringOpsStringSplitV2
  ...
  void BuildGraph(const Scope& scope) override {
    auto op_node =
        machina::ops::Placeholder(scope.WithOpName("input"), DT_STRING);
    auto op_node2 =
        machina::ops::Placeholder(scope.WithOpName("separator"), DT_STRING);

    machina::ops::StringSplitV2(scope.WithOpName("output"), op_node,
                                   op_node2);
  }
...
class FuzzStringOpsStringUpper : public FuzzSession<std::string> {
  void BuildGraph(const Scope& scope) override {
    ...
    machina::ops::StringUpper(scope.WithOpName("output"), op_node);
  }

  void FuzzImpl(const std::string& input_string) final {
    Tensor input_tensor(machina::DT_STRING, TensorShape({}));
    input_tensor.scalar<machina::tstring>()() = input_string;
