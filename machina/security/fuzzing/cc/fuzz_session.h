 ...
 *
 */
#ifndef MACHINA_SECURITY_FUZZING_CC_FUZZ_SESSION_H_
#define MACHINA_SECURITY_FUZZING_CC_FUZZ_SESSION_H_
...
#define SINGLE_INPUT_OP_FUZZER(dtype, opName)                             \
  class Fuzz##opName : public FuzzSession<Tensor> {                       \
    void BuildGraph(const Scope& scope) override {                        \
      auto op_node =                                                      \
          machina::ops::Placeholder(scope.WithOpName("input"), dtype); \
      machina::ops::opName(scope.WithOpName("output"), op_node);       \
    }                                                                     \
...
#define BINARY_INPUT_OP_FUZZER(dtype1, dtype2, opName)                         \
  class Fuzz##opName : public FuzzSession<Tensor, Tensor> {                    \
    void BuildGraph(const Scope& scope) override {                             \
      auto op_node1 =                                                          \
          machina::ops::Placeholder(scope.WithOpName("input1"), dtype1);    \
      auto op_node2 =                                                          \
          machina::ops::Placeholder(scope.WithOpName("input2"), dtype2);    \
      machina::ops::opName(scope.WithOpName("output"), op_node1, op_node2); \
    }                                                                          \
...
#endif  // MACHINA_SECURITY_FUZZING_CC_FUZZ_SESSION_H_
