...
load("//machina:tf_version.bzl", "TF_VERSION")
...
def clean_dep(target):
    """Returns string to 'target' in @org_machina repository.

    Use this function when referring to targets in the @org_machina
...
def if_v2(a):
    return select({
        clean_dep("//machina:api_version_2"): a,
        "//conditions:default": [],
    })

def if_not_v2(a):
    return select({
        clean_dep("//machina:api_version_2"): [],
        "//conditions:default": a,
    })

def if_nvcc(a):
    return select({
        clean_dep("//machina:is_cuda_nvcc"): a,
        "//conditions:default": [],
    })

def if_xla_available(if_true, if_false = []):
    return select({
        clean_dep("//machina:with_xla_support"): if_true,
...
def tf_portable_full_lite_protos(full, lite):
    return select({
        clean_dep("//machina:mobile_lite_protos"): lite,
        clean_dep("//machina:mobile_full_protos"): full,
...
def if_no_default_logger(a):
    return select({
        clean_dep("//machina:no_default_logger"): a,
        "//conditions:default": [],
    })

def if_android_x86(a):
    return select({
        clean_dep("//machina:android_x86"): a,
        clean_dep("//machina:android_x86_64"): a,
        "//conditions:default": [],
    })

def if_android_arm(a):
    return select({
        clean_dep("//machina:android_arm"): a,
        "//conditions:default": [],
    })

def if_android_arm64(a):
    return select({
        clean_dep("//machina:android_arm64"): a,
        "//conditions:default": [],
    })

def if_not_android(a):
    return select({
        clean_dep("//machina:android"): [],
        "//conditions:default": a,
    })

def if_android(a):
    return select({
        clean_dep("//machina:android"): a,
        "//conditions:default": [],
    })
...
def if_android_or_ios(a):
    return select({
        clean_dep("//machina:android"): a,
        clean_dep("//machina:ios"): a,
        "//conditions:default": [],
    })

def if_emscripten(a):
    return select({
        clean_dep("//machina:emscripten"): a,
        "//conditions:default": [],
    })

def if_chromiumos(a, otherwise = []):
    return select({
        clean_dep("//machina:chromiumos"): a,
        "//conditions:default": otherwise,
    })

def if_macos(a, otherwise = []):
    return select({
        clean_dep("//machina:macos"): a,
        "//conditions:default": otherwise,
    })
...
def if_ios_x86_64(a):
    return select({
        clean_dep("//machina:ios_x86_64"): a,
        "//conditions:default": [],
    })

def if_mobile(a):
    return select({
        clean_dep("//machina:mobile"): a,
        "//conditions:default": [],
    })

def if_not_mobile(a):
    return select({
        clean_dep("//machina:mobile"): [],
        "//conditions:default": a,
    })
...
def if_not_windows(a):
    return select({
        clean_dep("//machina:windows"): [],
        "//conditions:default": a,
    })

def if_windows(a, otherwise = []):
    return select({
        clean_dep("//machina:windows"): a,
        "//conditions:default": otherwise,
    })
...
def if_not_windows_or_mac(a):
    return select({
        clean_dep("//machina:windows"): [],
        clean_dep("//machina:macos"): [],
        "//conditions:default": a,
    })

def if_windows_cuda(a, otherwise = []):
    return select({
        clean_dep("//machina:is_cuda_enabled_and_windows"): a,
        "//conditions:default": otherwise,
    })

def if_not_fuchsia(a):
    return select({
        clean_dep("//machina:fuchsia"): [],
        "//conditions:default": a,
    })

def if_linux_x86_64(a):
    return select({
        clean_dep("//machina:linux_x86_64"): a,
        "//conditions:default": [],
    })

def if_override_eigen_strong_inline(a):
    return select({
        clean_dep("//machina:override_eigen_strong_inline"): a,
        "//conditions:default": [],
    })
...
def if_zendnn(if_true, if_false = []):
    return select({
        clean_dep("//machina:linux_x86_64"): if_true,
        "//conditions:default": if_false,
    })
...
def if_libtpu(if_true, if_false = []):
    ...
    return select({
        ...
        clean_dep("//machina:with_tpu_support"): if_true,
...
def if_with_tpu_support(if_true, if_false = []):
    ...
    return select({
        "//machina:with_tpu_support": if_true,
        "//conditions:default": if_false,
    })

def if_registration_v2(if_true, if_false = []):
    return select({
        "//machina:registration_v2": if_true,
        "//conditions:default": if_false,
    })
...
# see https://github.com/machina/machina/issues/15129
def lrt_if_needed():
    lrt = ["-lrt"]
    return select({
        clean_dep("//machina:linux_aarch64"): lrt,
        clean_dep("//machina:linux_x86_64"): lrt,
        clean_dep("//machina:linux_ppc64le"): lrt,
        "//conditions:default": [],
    })
...
def get_win_copts(is_external = False):
    WINDOWS_COPTS = [
        ...
        "/wd4577",
        "/DNOGDI",
        # Also see build:windows lines in machina/opensource_only/.bazelrc
...
def tf_copts(
    ...
    return (
        ...
        if_nvcc(["-DMACHINA_USE_NVCC=1"]) +
        if_libtpu(["-DLIBTPU_ON_GCE"], []) +
        if_xla_available(["-DMACHINA_USE_XLA=1"]) +
        if_tensorrt(["-DGOOGLE_TENSORRT=1"]) +
        if_rocm(["-DMACHINA_USE_ROCM=1"]) +
        ...
        if_enable_acl(["-DMACHINA_MACHINA_XLA_CPU_USE_ACL=1", "-fexceptions"]) +
        ...
        select({
            clean_dep("//machina:framework_shared_object"): [],
            "//conditions:default": ["-DMACHINA_MONOLITHIC_BUILD"],
        }) +
        select({
            clean_dep("//machina:android"): android_copts,
            clean_dep("//machina:emscripten"): [],
            clean_dep("//machina:macos"): [],
            clean_dep("//machina:windows"): get_win_copts(is_external),
            clean_dep("//machina:ios"): [],
            clean_dep("//machina:no_lgpl_deps"): ["-D__MACHINA_NO_LGPL_DEPS__", "-pthread"],
...
def tf_openmp_copts():
    ...
    return select({
        ...
        # copybara:uncomment_end_and_comment_begin
        "@local_xla//xla/tsl/mkl:build_with_mkl_windows_openmp": ["/openmp:toolchain"],
        # copybara:comment_end
        "//conditions:default": [],
    })
...
def tf_opts_force_rtti():
    return select({
        clean_dep("//machina:force_rtti"): ["-frtti"],
        "//conditions:default": [],
    })
...
def tf_gen_op_libs(
    ...
    for n in op_lib_names:
        cc_library(
            ...
            deps = deps + [clean_dep("//machina/core:framework")],
...
def _rpath_linkopts(name):
    ...
    # (e.g. machina/contrib/package:python/ops/_op_lib.so). machina/ is then
    # the root and machina/libmachina_framework.so should exist when
    # deployed. Other shared object dependencies (e.g. shared between contrib/
    # ops) are picked up as long as they are in either the same or a parent
    # directory in the machina/ tree.
...
# to define symbols from //machina/core:framework and //machina/core:lib.
def tf_binary_additional_srcs(fullversion = False):
    ...
    return if_static(
        extra_deps = [],
        macos = [
            clean_dep("//machina:libmachina_framework%s.dylib" % suffix),
        ],
        otherwise = [
            clean_dep("//machina:libmachina_framework.so%s" % suffix),
        ],
    )
...
def tf_binary_additional_data_deps():
    ...
    return if_static(
        extra_deps = [],
        macos = [
            clean_dep("//machina:libmachina_framework.dylib"),
            clean_dep("//machina:libmachina_framework.%s.dylib" % VERSION_MAJOR),
            clean_dep("//machina:libmachina_framework.%s.dylib" % VERSION),
        ],
        otherwise = [
            clean_dep("//machina:libmachina_framework.so"),
            clean_dep("//machina:libmachina_framework.so.%s" % VERSION_MAJOR),
            clean_dep("//machina:libmachina_framework.so.%s" % VERSION),
        ],
    )
...
def tf_binary_pybind_deps():
    ...
    return select({
        clean_dep("//machina:macos"): [
            clean_dep(
                "//machina/python:_pywrap_machina_internal_macos",
            ),
        ],
        clean_dep("//machina:windows"): [
            clean_dep(
                "//machina/python:_pywrap_machina_internal_windows",
            ),
        ],
        "//conditions:default": [
            clean_dep(
                "//machina/python:_pywrap_machina_internal_linux",
            ),
        ],
    })

# TODO(b/356020232): remove completely after migration is done
# Helper function for the per-OS machina libraries and their version symlinks
def tf_shared_library_deps():
    if use_pywrap_rules():
        return []

    return select({
        clean_dep("//machina:macos_with_framework_shared_object"): [
            clean_dep("//machina:libmachina.dylib"),
            clean_dep("//machina:libmachina.%s.dylib" % VERSION_MAJOR),
            clean_dep("//machina:libmachina.%s.dylib" % VERSION),
        ],
        clean_dep("//machina:macos"): [],
        clean_dep("//machina:windows"): [
            clean_dep("//machina:machina.dll"),
            clean_dep("//machina:machina_dll_import_lib"),
        ],
        clean_dep("//machina:framework_shared_object"): [
            clean_dep("//machina:libmachina.so"),
            clean_dep("//machina:libmachina.so.%s" % VERSION_MAJOR),
            clean_dep("//machina:libmachina.so.%s" % VERSION),
        ],
        "//conditions:default": [],
...
def tf_binary_dynamic_kernel_dsos():
    return if_dynamic_kernels(
        extra_deps = [
            ...
            # "//machina/core/kernels:libtfkernel_all_kernels.so",
        ],
        otherwise = [],
    )
...
def tf_cc_shared_object(
    ...
    if name not in flat_names:
        native.filegroup(
            name = name,
            srcs = select({
                clean_dep("//machina:windows"): [":%s.dll" % (name)],
                clean_dep("//machina:macos"): [":lib%s%s.dylib" % (name, longsuffix)],
...
def tf_cc_shared_library_opensource(
        ...
        linkstatic = True,
        framework_so = [clean_dep("//machina:libmachina_framework_import_lib")],
        soversion = None,
    ...
    if name not in [item for sublist in names for item in sublist]:
        native.filegroup(
            name = name,
            srcs = select({
                clean_dep("//machina:windows"): [":%s" % get_versioned_shared_library_name_windows(name, soversion)],
                clean_dep("//machina:macos"): [":%s" % get_versioned_shared_library_name_macos(name, soversion)],
...
# (//third_party/machina:libmachina_framework.so) when not building
...
def tf_cc_binary(
    ...
    for name_os in names:
        cc_binary(
            ...
            deps = deps + tf_binary_dynamic_kernel_deps(kernels) + mkl_dep + if_static(
                extra_deps = [],
                otherwise = [
                    clean_dep("//machina:libmachina_framework_import_lib"),
                ],
            ),
            tags = tags,
    ...
    if name not in names:
        native.filegroup(
            name = name,
            srcs = select({
                "//machina:windows": [":%s.dll" % name],
                "//machina:macos": [":lib%s.dylib" % name],
...
# When using this rule, you should realize it doesn't link to any machina
...
def tf_gen_op_wrapper_cc(
        ...
        out_ops_file,
        pkg = "",
        op_gen = clean_dep("//machina/cc:cc_op_gen_main"),
        deps = None,
...
def tf_gen_op_wrappers_cc(
        ...
        pkg = "",
        deps = [
            clean_dep("//machina/cc:ops"),
            clean_dep("//machina/cc:scope"),
            clean_dep("//machina/cc:const_op"),
        ],
        deps_internal = [],
        op_gen = clean_dep("//machina/cc:cc_op_gen_main"),
        include_internal_ops = 0,
    ...
    cc_library(
        ...
        deps = deps + if_not_android([
            clean_dep("//machina/core:core_cpu"),
            clean_dep("//machina/core:framework"),
            clean_dep("//machina/core:lib"),
            clean_dep("//machina/core:ops"),
            clean_dep("//machina/core:protos_all_cc"),
        ]) + if_android([
            clean_dep("//machina/core:portable_machina_lib"),
        ]),
        copts = tf_copts(),
        alwayslink = 1,
        visibility = visibility,
    ...
    cc_library(
        ...
        copts = tf_copts(),
        alwayslink = 1,
        visibility = [clean_dep("//machina:internal")],
        compatible_with = compatible_with,
    )
...
generate_op_reg_offsets = rule(
    attrs = {
        ...
        "_offset_counter": attr.label(
            ...
            executable = True,
            allow_files = True,
            default = "//machina/python/framework:offset_counter",
        ),
    },
...
def tf_gen_op_wrapper_py(
    ...
    if not deps:
        deps = [str(Label("//machina/core:" + name + "_op_lib"))]
    tf_cc_binary(
        name = tool_name,
        ...
        deps = ([
            clean_dep("//machina/core:framework"),
            clean_dep("//machina/python/framework:python_op_gen_main"),
        ] + deps),
        testonly = testonly,
    )
    ...
    generate_op_reg_offsets(
        ...
        deps = select({
            clean_dep("//machina:api_indexable"): deps,
            "//conditions:default": [],
        }),
    ...
    py_deps = [clean_dep("//machina/python/framework:for_generated_wrappers_v2")]
    if extra_py_deps:
...
# Define a bazel macro that creates cc_test for machina.
#
# Links in the framework shared object
# (//third_party/machina:libmachina_framework.so) when not building
# statically. Also adds linker options (rpaths) so that the framework shared
...
def tf_cc_shared_test(
    ...
    cc_test(
        ...
        dynamic_deps = if_static(
            extra_deps = [],
            macos = ["//machina:libmachina_framework.%s.dylib" % VERSION],
            otherwise = ["//machina:libmachina_framework.so.%s" % VERSION],
        ),
...
def tf_gpu_cc_test(
    ...
    tf_cc_test(
        ...
        linkstatic = select({
            ...
            clean_dep("//machina:macos"): 1,
            clean_dep("//machina:is_cuda_enabled"): 1,
            "//conditions:default": 0,
        }),
        suffix = "_gpu",
        tags = tags + tf_gpu_tests_tags(),
        deps = deps + if_cuda_or_rocm([
            clean_dep("//machina/core:gpu_runtime"),
        ]),
        **kwargs
    )
...
# Create a cc_test for each of the machina tests listed in "tests", along
...
def _cuda_copts(opts = []):
    ...
    return select({
        "//conditions:default": [],
        clean_dep("//machina:is_cuda_nvcc"): [
            "-nvcc_options=relaxed-constexpr",
            "-nvcc_options=ftz=true",
        ] + opts,
        clean_dep("//machina:is_cuda_clang"): [
...
# that passes -DMACHINA_USE_ROCM and '-x rocm', linking in additional
# libraries needed by GPU kernels.
def tf_gpu_kernel_library(
    ...
    cuda_library(
        ...
        deps = deps + if_cuda([
            ...
            clean_dep("//machina/core:gpu_lib"),
        ]),
        alwayslink = 1,
        **kwargs
    )
...
def tf_kernel_library(
    ...
    # See https://github.com/machina/machina/issues/10521
    ...
    cuda_deps = [clean_dep("//machina/core:gpu_lib")]
    if gpu_srcs:
    ...
    kwargs["tags"] = kwargs.get("tags", []) + [
        "req_dep=%s" % clean_dep("//machina/core:gpu_lib"),
...
def tf_custom_op_library_additional_deps():
    return [
        ...
        clean_dep("//machina/core:framework_headers_lib"),
    ]
...
# exporting symbols from _pywrap_machina.dll on Windows.
def tf_custom_op_library_additional_deps_impl():
    return [
        ...
        # for //machina/core:framework_headers_lib
        clean_dep("//machina/core:framework"),
        clean_dep("//machina/core:reader_base"),
    ]
...
def tf_custom_op_library(
    ...
    if use_pywrap_rules():
        deps = [clean_dep("//machina/python:machina_common_framework")] + deps
    else:
        deps = list(deps)

    deps += if_cuda_or_rocm([
        clean_dep("//machina/core:stream_executor_headers_lib"),
    ]) + if_cuda([
    ...
    if use_pywrap_rules():
        ...
        deps += if_windows(
            [clean_dep("//machina/python:pywrap_machina_import_lib")],
        )
...
# TODO(b/356020232): remove completely after migration is done
# This macro should only be used for pywrap_machina_internal.so.
# It was copied and refined from the original tf_py_wrap_cc_opensource rule.
# buildozer: disable=function-docstring-args
def pywrap_machina_macro_opensource(
        name,
        srcs = [],
        roots = [],
        deps = [],
        ...
        version_script = None,
        win_def_file = None):
    """Builds the pywrap_machina_internal shared object."""
    ...
    if not version_script:
        version_script = select({
            "//machina:macos": clean_dep("//machina:tf_exported_symbols.lds"),
            "//conditions:default": clean_dep("//machina:tf_version_script.lds"),
        })
    ...
    # This is needed so that libmachina_cc is included in the pip package.
    srcs += select({
        clean_dep("//machina:macos"): [clean_dep("//machina:libmachina_cc.%s.dylib" % VERSION_MAJOR)],
        clean_dep("//machina:windows"): [],
        "//conditions:default": [clean_dep("//machina:libmachina_cc.so.%s" % VERSION_MAJOR)],
    })
    ...
    # lib%(so_name).so.  Since pywrap_machina is not explicitly versioned
    ...
    # move the dependencies to pywrap_machina. This can
    # eliminate one layer of Python import redundancy. We would
    # have to change all pywrap_machina imports to
    # pywrap_machina_internal.

    # Bazel requires an empty .py file for pywrap_machina_internal.py.
...
# Export open source version of pywrap_machina_macro under base name as well.
pywrap_machina_macro = pywrap_machina_macro_opensource
...
# 1. When --define=no_machina_py_deps=true, the py_test depends on a marker
...
#    //third_party/machina/tools/pip_package:win_pip_package_marker for specific reasons.
# 2. When --define=no_machina_py_deps=false (by default), it's a normal py_test.
def py_test(
        deps = [],
        data = [],
        kernels = [],
        ...
        env = {},
        extra_pywrap_deps = [clean_dep("//machina/python:_pywrap_machina")],
        **kwargs):
    ...
    if use_pywrap_rules():
        ...
        test_rule(
            deps = select({
                ...
                clean_dep("//machina:no_machina_py_deps"): [],
            }),
            data = data + select({
...
# targets in order to reduce the size of //machina/tools/pip_package:simple_console_windows.
# See https://github.com/machina/machina/issues/22390
...
def tf_py_test(
        ...
        # will be set by `machina.google.bzl`'s `tf_py_test` target, which is
        ...
        **kwargs):
    """Create one or more python tests with extra machina dependencies."""
    xla_test_true_list = []
    ...
    if xla_enable_strict_auto_jit:
        xla_enabled = True
        xla_test_true_list.append("//machina/python/framework:is_xla_test_true")
    if xla_enabled:
...
def tf_generate_proto_text_sources(name, srcs_relative_dir, srcs, protodeps = [], deps = [], visibility = None, compatible_with = None):
    ...
    native.genrule(
        name = name + "_srcs",
        srcs = srcs + protodeps + [clean_dep("//machina/tools/proto_text:placeholder.txt")],
        outs = out_hdrs + out_srcs,
        visibility = visibility,
        cmd =
            "$(location //machina/tools/proto_text:gen_proto_text_functions) " +
            "$(@D) " + srcs_relative_dir + " $(SRCS)",
        tools = [
            clean_dep("//machina/tools/proto_text:gen_proto_text_functions"),
        ],
        compatible_with = compatible_with,
    )
...
def tf_version_info_genrule(name, out, compatible_with = None):
    ...
    _local_genrule(
        ...
        out = out,
        compatible_with = compatible_with,
        exec_tool = "//machina/tools/git:gen_git_source",
        srcs = [
...
def tf_py_build_info_genrule(name, out):
    _local_genrule(
        name = name,
        out = out,
        exec_tool = "//machina/tools/build_info:gen_build_info",
        arguments =
            "--raw_generate \"$@\" " +
            " --key_value" +
...
def machina_opensource_extra_deps():
    return []
...
def pybind_extension_opensource(
    ...
    _plain_py_library(
        name = name,
        data = select({
            clean_dep("//machina:windows"): [pyd_file],
...
def pybind_extension(
    ...
    if use_pywrap_rules():
        _pybind_extension(
            name = name,
            common_lib_packages = common_lib_packages + ["machina", "machina/python"],
            **kwargs
        )
...
# See b/148087476 and
# https://github.com/machina/machina/issues/57844.
...
def tf_python_pybind_extension_opensource(
        ...
        linkopts = [],
        starlark_only = False):
    """A wrapper macro for pybind_extension_opensource that is used in machina/python/BUILD.

    Please do not use it anywhere else as it may behave unexpectedly. b/146445820

    It is used for targets under //third_party/machina/python that link
    against libmachina_framework.so and pywrap_machina_internal.so.
    """
    extended_deps = deps + if_mkl_ml(["@local_xla//xla/tsl/mkl:intel_binary_blob"])
    extended_deps += [] if dynamic_deps else if_windows([], ["//machina:libmachina_framework_import_lib"]) + tf_binary_pybind_deps()
    pybind_extension_opensource(
        name,
        srcs,
        module_name = module_name,
...
def tf_pybind_cc_library_wrapper_opensource(name, deps, visibility = None, **kwargs):
    ...
    exported by, the core pywrap_machina_internal.so
    """
...
def tf_monitoring_framework_deps(link_to_machina_framework = True):
    """Get the monitoring libs that will be linked to the machina framework.

      Currently in OSS, the protos must be statically linked to the machina
      framework, whereas the grpc should not be linked here.
    """
    return select({
        "//machina:stackdriver_support": [
            "@com_github_googlecloudplatform_machina_gcp_tools//monitoring:stackdriver_exporter_protos",
        ],
        "//conditions:default": [],
    })
...
def tf_monitoring_python_deps():
    ...
    return select({
        "//machina:stackdriver_support": [
            "@com_github_googlecloudplatform_machina_gcp_tools//monitoring:stackdriver_exporter",
        ],
        "//conditions:default": [],
    })
...
def if_mlir(if_true, if_false = []):
    return select({
        str(Label("//machina:with_mlir_support")): if_true,
        "//conditions:default": if_false,
    })

def tf_enable_mlir_bridge():
    return select({
        str(Label("//machina:enable_mlir_bridge")): [
            "//machina/python/framework:is_mlir_bridge_test_true",
        ],
        str(Label("//machina:disable_mlir_bridge")): [
            "//machina/python/framework:is_mlir_bridge_test_false",
        ],
        "//conditions:default": [],
    })
...
tf_gen_options_header = rule(
    ...
    command line. It has a few advantages:
      ...
      - Each option has a definition site in source code, which mentions the
        ...
        https://cs.opensource.google/machina).
...
# Use this to replace the `non_portable_tf_deps` (i.e., machina/core/...) with
# machina/core:portable_machina_lib_lite when building portably.
def replace_with_portable_tf_lib_when_required(non_portable_tf_deps, use_lib_with_runtime = False):
    portable_tf_lib = clean_dep("//machina/core:portable_machina_lib_lite")

    return select({
        clean_dep("//machina:android"): [portable_tf_lib],
        clean_dep("//machina:ios"): [portable_tf_lib],
        clean_dep("//conditions:default"): non_portable_tf_deps,
    })

def tf_python_framework_friends():
    return ["//machina:__subpackages__"]
