...
void WriteClassDecl(const OpInfo& op_info, WritableFile* h) {
  ...
  if (op_info.output_types.empty()) {
    ...
    strings::StrAppend(&class_decl,
                       "  operator ::machina::Operation() const { "
                       "return operation; }\n");
    ...
    if (op_info.is_list_output[0]) {
      ...
      // output.
      strings::StrAppend(&class_decl,
                         "  ::machina::Output operator[](size_t index) "
                         "const { return ",
      ...
      // Output.
      strings::StrAppend(&class_decl,
                         "  operator ::machina::Output() const { return ",
                         op_info.output_names[0], "; }\n");
      strings::StrAppend(&class_decl,
                         "  operator ::machina::Input() const { return ",
                         op_info.output_names[0], "; }\n");
      // Write node() to get the Node* directly.
      strings::StrAppend(&class_decl,
                         "  ::machina::Node* node() const { return ",
...
void GetOutput(const OpInfo& op_info, string* out) {
  ...
  strings::StrAppend(out, "  ::machina::NameRangeMap _outputs_range;\n");
  strings::StrAppend(out,
                     "  ::machina::Status _status_ = "
                     "::machina::NameRangesForNode(*ret, ret->op_def(), "
