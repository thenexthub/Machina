...
string WriteClassFuzzDef(const OpInfo& op_info) {
      ...
      absl::StrJoin(
          ...
          [op_info](string* out, const OpDef_ArgDef arg) {
            ...
            if (ArgIsList(arg)) {
              strings::StrAppend(
                  out, "    Input ", arg.name(),
                  "_0 = ", "machina::ops::Placeholder(scope.WithOpName(\"",
                  arg.name(), "\"), ", type, ");\n");
              strings::StrAppend(
                  out, "    Input ", arg.name(),
                  "_1 = ", "machina::ops::Placeholder(scope.WithOpName(\"",
                  arg.name(), "\"), ", type, ");\n");
              strings::StrAppend(
              ...
              strings::StrAppend(
                  out, "    auto ", arg.name(), " = ",
                  "machina::ops::Placeholder(scope.WithOpName(\"",
                  arg.name(), "\"), ", type, ");\n");
            }
          }),
  ...
  string constructor_call_str = absl::Substitute(
      "    machina::ops::$0(scope.WithOpName(\"output\")$1);\n",
      op_info.op_name,
      strings::StrCat(
