...
absl::Status MirrorPadGrad(const Scope& scope, const Operation& op,
  ...
  grad_outputs->push_back(machina::ops::internal::MirrorPadGrad(
...
absl::Status ConcatGradHelper(const Scope& scope, const Operation& op,
  ...
  auto builder =
      ::machina::NodeBuilder(unique_name, "ConcatOffset")
          .Input(::machina::ops::AsNodeOut(scope, op.input(dim_index)))
          .Input(::machina::ops::AsNodeOutList(scope, shapes.output));
  scope.UpdateBuilder(&builder);
  ::machina::Node* concat_offset_node;
