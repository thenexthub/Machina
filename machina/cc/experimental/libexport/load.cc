 ...
 *
 */
#include "machina/cc/experimental/libexport/load.h"
...
absl::StatusOr<TFPackage> TFPackage::Load(const std::string& path) {
  ...
  const std::string variables_dir =
      machina::io::JoinPath(path, machina::kSavedModelVariablesDirectory);
  ...
  if (Env::Default()->FileExists(variables_dir).ok()) {
    ...
    tf_package.variables_filepath_ = machina::io::JoinPath(
        variables_dir, machina::kSavedModelVariablesFilename);
    tf_package.variable_reader_ = std::make_unique<machina::BundleReader>(
        machina::Env::Default(), tf_package.variables_filepath_);
    machina::Tensor object_graph_tensor;
    TF_RETURN_IF_ERROR(tf_package.variable_reader_->Lookup(
        machina::kObjectGraphProtoKey, &object_graph_tensor));
    const auto* object_graph_string =
        reinterpret_cast<const machina::tstring*>(
...
absl::StatusOr<const machina::NodeDef*> TFPackage::GetGraphDefNode(
    std::string name) {
