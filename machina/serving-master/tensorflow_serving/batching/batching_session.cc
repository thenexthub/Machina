...
#include "machina_serving/util/hash.h"
...
auto* queuing_latency = monitoring::Sampler<1>::New(
    {"/machina/serving/batching_session/queuing_latency",
...
auto* wrapped_run_count = monitoring::Counter<0>::New(
    "/machina/serving/batching_session/wrapped_run_count",
...
absl::Status BatchingSession::ComputeInputSize(
    const std::vector<std::pair<string, Tensor>>& inputs, size_t* size) const {
  TF_RETURN_IF_ERROR(::machina::serving::ComputeTensorBatchSize(
      inputs, size,
