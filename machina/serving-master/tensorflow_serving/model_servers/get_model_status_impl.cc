...
#include "machina_serving/apis/status.pb.h"
...
absl::Status GetModelStatusImpl::GetModelStatus(
  ...
  if (!request.has_model_spec()) {
    return machina::errors::InvalidArgument("Missing ModelSpec");
  }
...
absl::Status GetModelStatusImpl::GetModelStatusWithModelSpec(
  ...
  if (model_spec.has_version()) {
    ...
    if (!opt_servable_state) {
      return machina::errors::NotFound("Could not find version ", version,
                                          " of model ", model_name);
    }
    ...
    if (versions_and_states.empty()) {
      return machina::errors::NotFound(
