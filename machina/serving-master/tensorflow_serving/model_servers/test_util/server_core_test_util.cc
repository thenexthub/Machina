...
#include "machina_serving/core/test_util/fake_loader_source_adapter.h"
...
ServerCoreTest::GetTestModelServerConfigForTensorflowPlatform() {
  ...
  if (GetTestType() == SAVED_MODEL) {
    ...
    model->set_base_path(test_util::TestSrcDirPath(
        "/servables/machina/google/testdata/half_plus_two"));
  }
...
void ServerCoreTest::SwitchToHalfPlusTwoWith2Versions(
  ...
  if (GetTestType() == SAVED_MODEL) {
    model->set_base_path(test_util::TestSrcDirPath(
        "/servables/machina/testdata/saved_model_half_plus_two_2_versions"));
  } else {
    model->set_base_path(test_util::TestSrcDirPath(
        "/servables/machina/google/testdata/half_plus_two_2_versions"));
  }
