...
#include "machina_serving/config/model_server_config.pb.h"
#include "machina_serving/config/monitoring_config.pb.h"
#include "machina_serving/config/platform_config.pb.h"
#include "machina_serving/config/ssl_config.pb.h"
...
#include "machina_serving/model_servers/server_init.h"
...
#include "machina_serving/servables/machina/thread_pool_factory_config.pb.h"
#include "machina_serving/servables/machina/util.h"
#include "machina_serving/util/proto_util.h"
...
ModelServerConfig BuildSingleModelConfig(const string& model_name,
  ...
  machina::serving::ModelConfig* single_model =
  ...
  single_model->set_model_platform(
      machina::serving::kTensorFlowModelPlatform);
  return config;
}
...
Server::Options::Options()
    : model_name("default"),
      saved_model_tags(machina::kSavedModelTagServe) {}
...
absl::Status Server::BuildAndStart(const Options& server_options) {
  ...
  if (server_options.platform_config_file.empty()) {
    ...
    if (!server_options.machina_session_config_file.empty()) {
      TF_RETURN_IF_ERROR(
          ParseProtoTextFile(server_options.machina_session_config_file,
    ...
    if (server_options.machina_intra_op_parallelism > 0 &&
        server_options.machina_inter_op_parallelism > 0 &&
        server_options.machina_session_parallelism > 0){
        return errors::InvalidArgument("Either configure "
          "server_options.machina_session_parallelism "
          "or (server_options.machina_intra_op_parallelism, "
          "server_options.machina_inter_op_parallelism) separately. "
          "You cannot configure all.");
    } else if (server_options.machina_intra_op_parallelism > 0 ||
        server_options.machina_inter_op_parallelism > 0){
            session_bundle_config.mutable_session_config()
            ->set_intra_op_parallelism_threads(
                server_options.machina_intra_op_parallelism);
            session_bundle_config.mutable_session_config()
            ->set_inter_op_parallelism_threads(
                server_options.machina_inter_op_parallelism);
    } else {
        session_bundle_config.mutable_session_config()
        ->set_intra_op_parallelism_threads(
            server_options.machina_session_parallelism);
    ...
    const std::vector<string> tags =
        machina::str_util::Split(server_options.saved_model_tags, ",");
  ...
  if (server_options.fs_model_config_poll_wait_seconds > 0 &&
    ...
    fs_config_polling_thread_.reset(new PeriodicFunction(
        ...
        server_options.fs_model_config_poll_wait_seconds *
            machina::EnvTime::kSecondsToMicros,
        pf_options));
  }
  ...
  builder.SetMaxMessageSize(machina::kint32max);
