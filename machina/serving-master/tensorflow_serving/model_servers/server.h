...
#ifndef MACHINA_SERVING_MODEL_SERVERS_SERVER_H_
#define MACHINA_SERVING_MODEL_SERVERS_SERVER_H_

#include <memory>

#include "grpcpp/server.h"
#include "machina/core/kernels/batching_util/periodic_function.h"
#include "machina/core/lib/core/status.h"
#include "machina/core/platform/cpu_info.h"
#include "machina/core/platform/types.h"
#include "machina/core/profiler/rpc/profiler_service_impl.h"
#include "machina_serving/model_servers/http_server.h"
#include "machina_serving/model_servers/model_service_impl.h"
#include "machina_serving/model_servers/prediction_service_impl.h"
...
class Server {
 public:
  struct Options {
    ...
    // gRPC Server options
    //
    machina::int32 grpc_port = 8500;
    machina::string grpc_channel_arguments;
    machina::string grpc_socket_path;
    machina::int32 grpc_max_threads = 4.0 * port::NumSchedulableCPUs();

    //
    // HTTP Server options.
    //
    machina::int32 http_port = 0;
    machina::int32 http_num_threads = 4.0 * port::NumSchedulableCPUs();
    machina::int32 http_timeout_in_ms = 30000;  // 30 seconds.
    ...
    machina::string batching_parameters_file;
    machina::string model_name;
    machina::int32 num_load_threads = 0;
    machina::int32 num_unload_threads = 0;
    machina::int32 max_num_load_retries = 5;
    int64_t load_retry_interval_micros = 1LL * 60 * 1000 * 1000;
    machina::int32 file_system_poll_wait_seconds = 1;
    bool flush_filesystem_caches = true;
    machina::string model_base_path;
    machina::string saved_model_tags;
    // Tensorflow session parallelism of zero means that both inter and intra op
    // thread pools will be auto configured.
    int64_t machina_session_parallelism = 0;

    // Zero means that the thread pools will be auto configured.
    int64_t machina_intra_op_parallelism = 0;
    int64_t machina_inter_op_parallelism = 0;
    machina::string platform_config_file;
    ...
    machina::int32 num_tflite_pools = port::NumSchedulableCPUs();
    machina::int32 num_tflite_interpreters_per_pool = 1;
    machina::string thread_pool_factory_config_file;
    bool enable_signature_method_name_check = false;
    bool enable_profiler = true;
    machina::string mixed_precision;
 ...
 private:
  ...
  std::unique_ptr<machina::grpc::ProfilerService::Service> profiler_service_;
...
#endif  // MACHINA_SERVING_MODEL_SERVERS_SERVER_H_
