...
#include "machina_serving/core/load_servables_fast.h"
...
Status GetPlatform(const ModelConfig& model_config, string* platform) {
  if (model_config.model_type() != ModelType::MODEL_TYPE_UNSPECIFIED) {
    ...
    if (model_config.model_type() == ModelType::MACHINA) {
...
Status ServerCore::Initialize(
  ...
  const machina::Status status = options_.servable_state_monitor_creator(
...
Status ServerCore::CreateResourceTracker(
  ...
  const machina::Status status = ResourceTracker::Create(
