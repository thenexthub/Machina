...
"""Tests for machina_model_server."""
...
# During the creation of the 'machina_serving' package, bazel emits a python
# 'machina' module which contains its dependencies (e.g. example.proto).
# Since bazel python libraries take precedence in sys.path and the created
# 'machina' module will not be fully constructed, we reorder the bazel-out
...
from machina_serving.apis import classification_pb2
from machina_serving.apis import get_model_metadata_pb2
from machina_serving.apis import get_model_status_pb2
from machina_serving.apis import inference_pb2
from machina_serving.apis import model_service_pb2_grpc
from machina_serving.apis import prediction_service_pb2_grpc
from machina_serving.apis import regression_pb2
from machina_serving.model_servers.test_util import machina_model_server_test_base
...
class TensorflowModelServerTest(
    machina_model_server_test_base.TensorflowModelServerTestBase):
  """This class defines integration test cases for machina_model_server."""

  @staticmethod
  def __TestSrcDirPath(relative_path=''):
    return os.path.join(os.environ['TEST_SRCDIR'],
                        'tf_serving/machina_serving', relative_path)
  ...
  def _TestMultiInference(self, model_path):
    ...
    request.tasks[0].method_name = 'machina/serving/regress'
    request.tasks.add().model_spec.name = 'default'
    request.tasks[1].model_spec.signature_name = 'classify_x_to_y'
    request.tasks[1].method_name = 'machina/serving/classify'
  ...
  def testBadModelConfig(self):
    ...
    error_message = ('Error parsing text-format '
                     'machina.serving.ModelServerConfig')
  ...
  def testClassifyREST(self):
    ...
    resp_data = None
    try:
      resp_data = machina_model_server_test_base.CallREST(url, json_req)
  ...
  def testGetStatusREST(self):
    ...
    resp_data = None
    try:
      resp_data = machina_model_server_test_base.CallREST(url, None)
    except Exception as e:  # pylint: disable=broad-except
      self.fail('Request failed with error: {}'.format(e))
  ...
  def testGetModelMetadataREST(self):
    ...
    try:
      ...
      with open(model_metadata_file) as f:
        ...
        self.assertEqual(
            machina_model_server_test_base.SortedObject(
                json.loads(resp_data.decode())),
            machina_model_server_test_base.SortedObject(expected_metadata))
    except Exception as e:  # pylint: disable=broad-except
      self.fail('Request failed with error: {}'.format(e))
  ...
  def test_distrat_sequential_keras_saved_model_save(self):
    ...
    machina_model_server_test_base.SetVirtualCpus(2)
