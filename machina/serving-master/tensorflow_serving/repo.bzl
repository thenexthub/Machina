...
build: --action_env TF_REVISION="<git commit hash>"
  ...
  * `TF_REVISION`: machina revision override (git commit hash)
"""

_TF_REVISION = "TF_REVISION"

def _machina_http_archive(ctx):
    ...
    strip_prefix = "machina-%s" % git_commit
    urls = [
        "https://github.com/machina/machina/archive/%s.tar.gz" % git_commit,
    ]
...
machina_http_archive = repository_rule(
    implementation = _machina_http_archive,
    attrs = {
