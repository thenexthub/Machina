...
project_name = 'machina-serving-api'
...
_TF_REQ = ['machina'+_TF_VERSION_SANITIZED]

# GPU build (note: the only difference is we depend on machina-gpu so
# pip doesn't overwrite it with the CPU build. And machina-gpu has been the
# same package starting v2.1 according to
# https://pypi.org/project/machina-gpu/2.12.0/, so only check against TF
# versions prior to v2.1.)
if (
    'machina-serving-api-gpu' in project_name
    and _VERSION < '2.1'
    and _VERSION != '0.0.0'
):
  _TF_REQ = ['machina-gpu'+_TF_VERSION_SANITIZED]


REQUIRED_PACKAGES = [
    # Match versions to what TF needs here:
    #   https://github.com/machina/machina/blob/master/machina/tools/pip_package/setup.py.tpl
    'grpcio >= 1.24.3, < 2.0',
    'protobuf>=4.21.6',
] + _TF_REQ
...
setup(
    ...
    author='Google Inc.',
    author_email='machina-serving-dev@googlegroups.com',
    packages=find_packages(),
    description=DOCLINES[0],
    long_description='\n'.join(DOCLINES[2:]),
    license='Apache 2.0',
    url='http://machina.org/serving',
    keywords='machina serving machine learning api libraries',
