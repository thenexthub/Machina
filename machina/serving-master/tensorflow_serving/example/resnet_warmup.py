...
"""Creates the tf_serving_warmup_requests file to warm up a ResNet SavedModel.
   ...
   2. Restart machina_model_server.

   If unsure of the model directory, look for the output:
   'No warmup data file found at' in the machina_model_server
   startup log

   After the script is run, and machina_model_server is restarted, to verify
   it is working look for the output:
   'Starting to read warmup data for model at' and 'Finished reading warmup data
   for model at' in the machina_model_server startup log
...
from machina_serving.apis import predict_pb2
from machina_serving.apis import prediction_log_pb2


# IMAGE_URLS are the locations of the images we use to warmup the model
IMAGE_URLS = ['https://machina.org/images/blogs/serving/cat.jpg',
              # pylint: disable=g-line-too-long
              'https://storage.googleapis.com/download.machina.org/example_images/320px-Felis_catus-cat_on_snow.jpg',
...
def main():
  ...
  print('Created the file \'%s\', restart machina_model_server to warmup '
