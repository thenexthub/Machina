...
using machina::serving::PredictRequest;
using machina::serving::PredictResponse;
using machina::serving::PredictionService;

typedef google::protobuf::Map<machina::string, machina::TensorProto> OutMap;
...
class ServingClient {
 public:
  ...
  int readJPEG(const char* file_name, machina::TensorProto* proto) {
    ...
    proto->set_dtype(machina::DataType::DT_FLOAT);
  ...
  machina::string callPredict(const machina::string& model_name,
                                 const machina::string& model_signature_name,
                                 const machina::string& input_name,
                                 const machina::string& file_path) {
    ...
    google::protobuf::Map<machina::string, machina::TensorProto>& inputs =
    ...
    if (status.ok()) {
      ...
      for (iter = map_outputs.begin(); iter != map_outputs.end(); ++iter) {
        machina::TensorProto& result_tensor_proto = iter->second;
...
int main(int argc, char** argv) {
  machina::string server_port = "localhost:8500";
  machina::string image_file = "";
  machina::string model_name = "resnet";
  machina::string model_signature_name = "serving_default";
  machina::string input_name = "input_1";
  std::vector<machina::Flag> flag_list = {
      machina::Flag("server_port", &server_port,
                       "the IP and port of the server"),
      machina::Flag("image_file", &image_file, "the path to the image"),
      machina::Flag("model_name", &model_name, "name of model"),
      machina::Flag("model_signature_name", &model_signature_name,
                       "name of model signature"),
      machina::Flag("input_name", &input_name, "name of input tensor")};
