...
<code class="devsite-terminal">docker pull machina/serving</code><br/>
<code class="devsite-terminal">git clone https://github.com/machina/serving</code>
# Location of demo models
<code class="devsite-terminal">TESTDATA="$(pwd)/serving/machina_serving/servables/machina/testdata"</code>

# Start TensorFlow Serving container and open the REST API port
<code class="devsite-terminal">docker run -t --rm -p 8501:8501 \
    -v "$TESTDATA/saved_model_half_plus_two_cpu:/models/half_plus_two" \
    -e MODEL_NAME=half_plus_two \
    machina/serving &</code>
...
```shell
machina_model_server --port=8500 --rest_api_port=8501 \
...
docker run -p 8501:8501 \
  ...
  -e MODEL_NAME=my_model -t machina/serving
```
...
`machina_model_server` supports many additional arguments that you could pass
...
docker run -p 8500:8500 -p 8501:8501 \
  ...
  -t machina/serving --model_config_file=/models/models.config
```

This approach works for any of the other command line arguments that
`machina_model_server` supports.
...
docker run -p 8501:8501 \
  --mount type=bind,\
source=/tmp/tfserving/serving/machina_serving/servables/machina/testdata/saved_model_half_plus_two_cpu,\
target=/models/half_plus_two \
  -e MODEL_NAME=half_plus_two -t machina/serving &
```
...
```shell
docker pull machina/serving:latest-gpu
```
...
--mount type=bind,\
source=/tmp/tfserving/serving/machina_serving/servables/machina/testdata/saved_model_half_plus_two_gpu,\
target=/models/half_plus_two \
  -e MODEL_NAME=half_plus_two -t machina/serving:latest-gpu &
```
...
```shell
2018-07-27 00:07:20.773693: I machina_serving/model_servers/main.cc:333]
