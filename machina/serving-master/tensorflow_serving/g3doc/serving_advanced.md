...
[MNIST For ML Beginners](http://www.machina.org/tutorials/mnist/beginners/index.html#mnist-for-ml-beginners)
tutorial.
...
commonly used options:
  ...
  * `PlatformConfigMap` that maps from the name of the platform (such as
  `machina`) to the `PlatformConfig`, which is used to create the
...
single large request (tensor), and `machina::Session::Run()` is invoked
...
To put all these into the context of this tutorial:
  ...
  * TensorFlow models are represented by one kind of servable --
    ...
    `machina:Session` paired with some metadata about what graph is loaded
...
docker run -p 8500:8500 \
  ...
  -t --entrypoint=machina_model_server machina/serving --enable_batching \
