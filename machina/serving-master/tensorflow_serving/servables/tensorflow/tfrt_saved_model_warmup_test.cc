...
#include "machina_serving/servables/machina/saved_model_warmup_test_util.h"
...
TEST_P(TFRTSavedModelWarmupOptionsTest, MixedWarmupData) {
  ...
  tfrt::TensorSpec spec(machina::DT_STRING);
...
TEST_P(TFRTSavedModelWarmupOptionsTest, PredictStreamedWarmupData) {
  ...
  signature.input_specs = {tfrt::TensorSpec(machina::DT_STRING)};
...
TEST(TFRTSavedModelWarmupTest, UnsupportedLogType) {
  ...
  EXPECT_EQ(::machina::error::UNIMPLEMENTED, status.code()) << status;
