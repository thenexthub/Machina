...
#include "machina_serving/util/threadpool_executor.h"
...
auto* model_warm_up_latency = monitoring::Sampler<2>::New(
    {
        "/machina/serving/model_warmup_latency",
...
absl::Status RunSavedModelWarmupUntracked(
  ...
  if (!machina::Env::Default()->FilesExist({warmup_path}, nullptr)) {
  ...
  std::unique_ptr<machina::RandomAccessFile> tf_record_file;
  TF_RETURN_IF_ERROR(machina::Env::Default()->NewRandomAccessFile(
  ...
  int num_model_warmup_threads =
      model_warmup_options.has_num_model_warmup_threads()
          ...
          : 1;
  std::unique_ptr<machina::io::SequentialRecordReader> tf_record_file_reader;
  absl::Status status;
  int num_warmup_records = 0;
  if (num_model_warmup_threads <= 1) {
    tf_record_file_reader.reset(
        new machina::io::SequentialRecordReader(tf_record_file.get()));
    tstring record;
    status = tf_record_file_reader->ReadRecord(&record);
    machina::serving::PredictionLog prediction_log;
    while (status.ok()) {
    ...
    struct SharedState {
      ...
      // executed.
      ::machina::condition_variable done ABSL_GUARDED_BY(mu);
      std::unique_ptr<machina::io::SequentialRecordReader>
    ...
    {
      ::machina::mutex_lock lock(state->mu);
