...
#include "machina_serving/servables/machina/tfrt_saved_model_source_adapter.h"
...
#include "machina_serving/resources/resource_util.h"
...
#include "machina_serving/servables/machina/file_acl.h"
...
// copybara:strip_begin (Do not leak in machina serving OSS.)
namespace {
...
absl::Status IsOrbaxModelDirectory(absl::string_view path) {
  const std::string orbax_model_manifest_pb_path =
      machina::io::JoinPath(path, kOrbaxModelManifestPb);
  const std::string orbax_model_manifest_version_path =
      machina::io::JoinPath(path, kOrbaxModelManifestVersionTxt);
