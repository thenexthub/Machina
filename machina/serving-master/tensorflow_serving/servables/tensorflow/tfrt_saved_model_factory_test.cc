...
TEST_F(TfrtSavedModelFactoryTest, BasicWithSavedModelConfigAndGraphRewrite) {
  TF_ASSERT_OK(machina::serving::ResetGraphRewriterForTesting());
  bool rewriter_was_called = false;
  TF_ASSERT_OK(machina::serving::SetGraphRewriter([&](MetaGraphDef* graph) {
...
TEST_F(TfrtSavedModelFactoryTest, Batch) {
  ...
  std::vector<machina::Tensor> input_tensors;
  input_tensors.push_back(input_tensor);
  std::vector<machina::Tensor> output_tensors1, output_tensors2;
  tfrt::SavedModel::RunOptions run_options;  // Default RunOptions.
