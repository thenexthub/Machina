...
#include "machina_serving/servables/machina/remote_op_config_rewriter.pb.h"
...
TEST(SavedModeConfigTest, MissingSavedModelConfig) {
  ...
  machina::GraphOptions graph_options;
...
TEST(ModelRuntimeConfigTest, OverwriteRuntimeConfig) {
  ...
  machina::tfrt_stub::TestConfig1 old_test_config1;
  ...
  EXPECT_THAT(
      runtime_config.ToProto(), EqualsProto(R"pb(
        config {
          type_url: "type.googleapis.com/machina.tfrt_stub.TestConfig1"
          value: "\n\rtest config 1"
        }
      )pb"));
}
