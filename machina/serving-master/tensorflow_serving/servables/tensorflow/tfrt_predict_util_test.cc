...
#include "machina_serving/servables/machina/tfrt_predict_util.h"
...
#include "machina_serving/servables/machina/test_util/mock_tfrt_saved_model.h"
 ...
 public:
  static void SetUpTestSuite() {
    ...
    model_config->set_base_path(
        test_util::TestSrcDirPath("servables/machina/testdata/"
...
TEST_F(PredictImplTest, PredictionSuccess) {
  ...
  tensor_proto.set_dtype(machina::DT_FLOAT);
  ...
  output_tensor_proto.set_dtype(machina::DT_FLOAT);
...
TEST_F(PredictImplTest, PredictionInvalidTensor) {
  ...
  tensor_proto.set_dtype(machina::DT_BOOL);
  (*request.mutable_inputs())[kInputTensorKey] = tensor_proto;

  auto status = CallPredict(GetServerCore(), request, &response);
  EXPECT_EQ(status.code(), machina::error::Code::INVALID_ARGUMENT);
...
TEST_F(PredictImplTest, PredictionMissingFunction) {
  ...
  EXPECT_EQ(status.code(), machina::error::Code::FAILED_PRECONDITION);
...
TEST_F(PredictImplTest, PredictionRunError) {
  ...
  tfrt::TensorSpec spec(machina::DT_FLOAT);
...
TEST_F(PredictImplTest, PredictionUnmatchedOutputNumber) {
  ...
  auto status =
      RunPredict(tfrt_stub::SavedModel::RunOptions(), kTestModelVersion,
                 saved_model.get(), request, &response);
  EXPECT_EQ(status.code(), machina::error::Code::UNKNOWN);
...
TEST_F(PredictImplTest, OutputFilters) {
  ...
  machina::SignatureDef signature_def;
  machina::TensorInfo tensor_info1, tensor_info2, tensor_info3;
  ...
  signature_def.set_method_name("machina/serving/predict");
  ...
  output_tensor_proto1.set_dtype(machina::DT_FLOAT);
...
TEST_F(PredictImplTest, OutputFiltersFullSet) {
  ...
  output_tensor_proto2.set_dtype(machina::DT_FLOAT);
...
TEST_F(PredictImplTest, PredictionTimeout) {
  ...
  EXPECT_EQ(status.code(), machina::error::Code::DEADLINE_EXCEEDED);
