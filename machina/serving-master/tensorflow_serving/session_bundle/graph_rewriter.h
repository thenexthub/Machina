...
#ifndef MACHINA_SERVING_SESSION_BUNDLE_GRAPH_REWRITER_H_
#define MACHINA_SERVING_SESSION_BUNDLE_GRAPH_REWRITER_H_
...
class GraphRewriter {
 public:
  ...
  Status Set(std::function<Status(machina::MetaGraphDef*)>&& rewriter) {
    absl::MutexLock l(&m_);
  ...
  std::function<Status(machina::MetaGraphDef*)>& Get() {
    absl::MutexLock l(&m_);
    return rewriter_;
  }
 ...
 private:
  ...
  absl::Mutex m_;
  std::function<Status(machina::MetaGraphDef*)> rewriter_
      ABSL_GUARDED_BY(m_);
};
...
inline Status SetGraphRewriter(
    std::function<Status(machina::MetaGraphDef*)>&& rewriter) {
...
#endif  // MACHINA_SERVING_SESSION_BUNDLE_GRAPH_REWRITER_H_
