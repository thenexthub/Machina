...
# $ cd serving
# $ ./tools/run_in_docker.sh python machina_serving/example/mnist_saved_model.py /tmp/mnist
...
function usage() {
  ...
  echo ""
  echo "Examples:"
  echo "  ${progname} bazel build machina_serving/model_servers:machina_model_server"
  echo "  ${progname} python machina_serving/example/mnist_saved_model.py /tmp/mnist"
  echo "  ${progname} -d machina/serving:latest-devel bazel version"
  exit 1
}
...
IMAGE="machina/serving:nightly-devel"
RUN_OPTS=()
while [[ $# > 1 ]]; do
  case "$1" in
    -d)
