...
#include "machina/core/distributed_runtime/eager/eager_service_impl.h"
...
#include "machina/core/distributed_runtime/eager/cluster_function_library_runtime.h"
...
#include "machina/core/distributed_runtime/message_wrappers.h"
#include "machina/core/distributed_runtime/rpc_collective_executor_mgr.h"
#include "machina/core/distributed_runtime/session_mgr.h"
...
#include "machina/core/nccl/collective_communicator.h"
...
absl::Status GetNumRetvals(
    ...
    const google::protobuf::Map<string, machina::AttrValue>& attrs, int* num_retvals) {
  const machina::OpRegistrationData* op_reg_data = nullptr;
  auto status = machina::OpRegistry::Global()->LookUp(op_name, &op_reg_data);
  if (absl::IsNotFound(status)) {
    status = func_lib_def->LookUp(op_name, &op_reg_data);
  }
  TF_RETURN_IF_ERROR(status);

  const machina::OpDef& op_def = op_reg_data->op_def;
...
absl::Status GetEagerOperationAndNumRetvals(const Operation& operation,
  ...
  std::optional<machina::EagerFunctionParams> remote_func_params =
      std::nullopt;
...
absl::Status TensorHandleProto(TensorHandle* handle, TensorProto* proto) {
  const machina::Tensor* t = nullptr;
...
absl::Status EagerServiceImpl::CreateContext(
  ...
  auto session_name =
      machina::strings::StrCat("eager_", request->context_id());
  if (VLOG_IS_ON(2)) {
  ...
  machina::DeviceMgr* device_mgr = worker_session->device_mgr();
  ...
  machina::EagerContext* ctx = new machina::EagerContext(
      opts, machina::ContextDevicePlacementPolicy::DEVICE_PLACEMENT_SILENT,
  ...
  std::unique_ptr<machina::eager::EagerClientCache> remote_eager_workers;
  ...
  auto remote_mgr =
      std::make_unique<machina::eager::RemoteMgr>(/*is_master=*/false, ctx);
...
absl::Status EagerServiceImpl::UpdateContext(
  ...
  machina::EagerContext* ctx = server_context->Context();
  ...
  const machina::DeviceMgr* device_mgr = worker_session->device_mgr();
...
absl::Status EagerServiceImpl::CreateMasterContext(
    const machina::uint64 context_id, EagerContext* context) {
  {
    mutex_lock l(contexts_mu_);
...
absl::Status EagerServiceImpl::ExecuteOp(CallOptions* call_opts,
  ...
  machina::EagerOperation op(eager_context);
  int num_retvals = 0;
  ...
  TF_RETURN_IF_ERROR(op.Execute(
      absl::MakeSpan(
          reinterpret_cast<machina::AbstractTensorHandle**>(retvals.data()),
          num_retvals),
      &num_retvals));
...
absl::Status EagerServiceImpl::KeepAlive(const KeepAliveRequest* request,
  ...
  machina::EagerContext* ctx = context->Context();
...
absl::Status EagerServiceImpl::SendTensor(const SendTensorOp& send_tensor,
                                          EagerContext* eager_context) {
  absl::InlinedVector<machina::TensorHandle*, 2UL> tensors;
...
absl::Status EagerServiceImpl::SendPackedHandle(
  ...
  std::vector<machina::TensorHandle*> handles;
  ...
  machina::TensorHandle* packed_handle = nullptr;
  std::vector<machina::TensorHandle*> handles_to_pack = handles;
