...
#include "machina/core/distributed_runtime/rpc/rpc_rendezvous_mgr.h"
...
class EagerServiceImplTest : public ::testing::Test {
 ...
 protected:
  ...
  machina::RpcRendezvousMgr rendezvous_mgr_;
...
void SetTensorProto(TensorProto* tensor_proto) {
  ...
  TF_ASSERT_OK(machina::TF_TensorToTensor(t, &tensor));
...
machina::NodeDef MatMulFunctionNodeDef() {
  machina::NodeDef def;
...
machina::FunctionDef MatMulFunction() {
...
machina::FunctionDef MatMulTransposeFunction() {
  machina::FunctionDef def;
  CHECK(machina::protobuf::TextFormat::ParseFromString(
      "    signature {"
      "      name: 'MatMulFunction'"
...
machina::FunctionDef MatMulNestedFunction() {
  machina::FunctionDef def;
  CHECK(machina::protobuf::TextFormat::ParseFromString(
      "    signature {"
...
machina::FunctionDef SingleRecvNodeFunction() {
  machina::FunctionDef def;
  CHECK(machina::protobuf::TextFormat::ParseFromString(
      "    signature {"
...
TEST_F(EagerServiceImplTest, BasicTest) {
  ...
  AttrValue val;
  val.set_type(machina::DataType::DT_FLOAT);
  ...
  machina::TensorHandle* tensor_handle;
...
class FunctionWithRemoteInputsTest : public EagerServiceImplTest {
 public:
  ...
  bool MatMulHasAttrWithDefaultValue(const machina::FunctionDef& fdef) {
...
TEST_F(EagerServiceImplTest, SendPackedHandleTest) {
  ...
  machina::TensorHandle* packed_handle;
  TF_ASSERT_OK(eager_service_impl.GetTensorHandle(
...
TEST_F(EagerServiceImplTest, RequestsToMasterTest) {
  tsl::core::RefCountPtr<machina::Rendezvous> rendezvous =
      tsl::core::RefCountPtr<machina::Rendezvous>(
          new machina::IntraProcessRendezvous(device_mgr_.get()));
  ...
  auto remote_mgr =
      std::make_unique<machina::eager::RemoteMgr>(/*is_master=*/true, ctx);
