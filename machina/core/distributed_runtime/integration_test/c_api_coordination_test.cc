...
void ConfigCoordinationService(machina::ServerDef* server_def,
...
string SetConfigKeyValueFn() {
  FunctionDef fdef;
  machina::protobuf::TextFormat::ParseFromString(
      "    signature {"
...
TEST(CAPI, MultiClientCoordinationSetGetConfigs) {
  ...
  auto worker_thread_fn = [&](int worker_id) {
    ...
    const std::string& key = machina::strings::StrCat("key", worker_id);
    TFE_InsertConfigKeyValue(
        ctx, key.c_str(),
        machina::strings::StrCat("value", worker_id).c_str(), status);
    EXPECT_EQ(TF_OK, TF_GetCode(status)) << TF_Message(status);
    counter1.Block();

    const int next_id = (worker_id + 1) % cluster_size;
    // Setting next_key errors out because it has been set by another worker
    const std::string& next_key = machina::strings::StrCat("key", next_id);
    ...
    std::string value_str{static_cast<const char*>(value_buf->data),
                          value_buf->length};
    EXPECT_EQ(value_str, machina::strings::StrCat("value", next_id));
...
TEST_P(SingleClientCoordinationServiceTest, TestSetGetConfigInOp) {
  ...
  std::unique_ptr<machina::ServerInterface> w2;
  ASSERT_TRUE(
  ...
  if (!use_worker0_as_client) {
    ...
    const int client_port = machina::testing::PickUnusedPortOrDie();
