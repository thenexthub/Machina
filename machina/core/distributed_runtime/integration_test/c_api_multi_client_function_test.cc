...
std::string SendFunction(const std::string& send_device,
                         ...
                         const machina::int64 send_device_incarnation) {
...
TEST_P(MultiClientSendRecvTest, TestMultiClientSendRecv) {
  ...
  auto worker_thread_fn = [&](int worker_id) {
    ...
    std::vector<machina::DeviceAttributes> device_attrs;
    machina::unwrap(ctx)->ListDevices(&device_attrs);
    machina::uint64 send_device_incarnation = 0;
    ...
    if (worker_id == 0) {
      // Sender worker.
      machina::Env::Default()->SleepForMicroseconds(params.delay_send_sec *
                                                       1000);
      ...
      // Receiver worker.
      machina::Env::Default()->SleepForMicroseconds(params.delay_recv_sec *
                                                       1000);
      ...
      for (int s = 1; s <= params.num_steps; s++) {
        ...
        if (params.use_tfrt) {
          // TODO (@chienchunh): Add support for step id configuration in TFRT.
          EXPECT_TRUE(machina::unwrap(recv_func)
                          ->Reset("RecvFunction", recv_device.c_str())
                          .ok());
        } else {
          machina::EagerOperation* op =
              machina::OperationFromInterface(machina::unwrap(recv_func));
