...
int main(int argc, char* argv[]) {
  ...
  machina::string job_spec;
  machina::string job_name;
  machina::string host_port;
  int num_cpus = 1;
  int num_gpus = 0;
  int task_index = 0;
  int replica = 0;
  std::vector<machina::Flag> flag_list = {
      machina::Flag("tf_jobs", &job_spec, "job specification"),
      machina::Flag("tf_job", &job_name, "job name"),
      machina::Flag("tf_task", &task_index, "task index"),
      machina::Flag("tf_replica", &replica, "task replica"),
      machina::Flag("host_port", &host_port, "listen address"),
      machina::Flag("num_cpus", &num_cpus, "number of CPUs"),
      machina::Flag("num_gpus", &num_gpus, "number of GPUs"),
  };
  ...
  machina::ServerDef def;
  absl::Status s =
      machina::FillServerDef(job_spec, job_name, num_cpus, num_gpus,
  ...
  std::unique_ptr<machina::ServerInterface> svr;
  s = machina::NewServer(def, &svr);
