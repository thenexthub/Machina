...
#include "machina/core/protobuf/bfc_memory_map.pb.h"
...
int main(int argc, char** argv) {
  ...
  std::vector<machina::Flag> flag_list = {
      machina::Flag("path", &path,
                       "Path of GPU BFCAllocator memory dump file"),
      machina::Flag("by_addr", &by_addr,
                       "Whether to print Chunks by memory address"),
      machina::Flag("by_ac", &by_ac,
                       "Whether to print Chunks by action count"),
      machina::Flag("by_size", &by_size,
                       "Whether to print Chunks by decreasing size"),
      machina::Flag("by_age", &by_age,
                       "If true, replace absolute action count with age "
                       "(max_action_count - action_count) in display."),
      machina::Flag("op_name", &op_name,
                       "Whether to print only chunks whose Op name matches "
                       "the supplied regexp."),
      machina::Flag("freed_at", &freed_at,
                       "Whether to display the freed_at value (only relevant "
                       "with timestamped allocator)."),
      machina::Flag("chunk_type", &chunk_type,
                       "One of {'U', 'F', 'A'} meaning in Use, Free, or All "
                       "(default).  Displays only Chunks of this type."),
      machina::Flag("size_history", &size_history,
                       "If true, show the size history."),
  };
  bool parse_result = machina::Flags::Parse(&argc, argv, flag_list);
  if (!parse_result || path.empty()) {
    std::cerr << machina::Flags::Usage(argv[0], flag_list);
    return -1;
  }
  machina::port::InitMain(argv[0], &argc, &argv);
  machina::MemoryDump md = machina::ReadDumpFile(path);
  machina::PrintSummary(md);
  if (chunk_type != "A") {
    md = FilterByChunkType(md, chunk_type[0]);
  }
  if (by_addr) machina::PrintChunksByAddress(md, by_age, freed_at);
  if (by_ac) machina::PrintChunksByActionCount(md, by_age, freed_at);
  if (by_size) machina::PrintChunksBySize(md, by_age, freed_at);
  if (!op_name.empty()) {
    machina::PrintChunksByOpName(md, op_name, by_age, freed_at);
  }
  if (size_history) machina::PrintSizeHistory(md, by_age);
}
