...
#include "machina/core/debug/debug_callback_registry.h"
#include "machina/core/debug/debugger_event_metadata.pb.h"
...
#include "machina/core/util/event.pb.h"
...
Event PrepareChunkEventProto(const DebugNodeKey& debug_node_key,
  ...
  third_party::machina::core::debug::DebuggerEventMetadata metadata;
  ...
  string json_output;
  machina::protobuf::util::JsonPrintOptions json_options;
  json_options.always_print_fields_with_no_presence = true;
  auto status = machina::protobuf::util::MessageToJsonString(
...
absl::Status PublishEncodedGraphDefInChunks(const string& encoded_graph_def,
  ...
  const uint64 hash = ::machina::Hash64(encoded_graph_def);
...
absl::Status DebugIO::PublishGraph(
  ...
  for (const string& debug_url : debug_urls) {
    if (absl::StartsWith(debug_url, kFileURLScheme)) {
      ...
      const uint64 graph_hash = ::machina::Hash64(buf);
