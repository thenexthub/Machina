...
#include "machina/core/debug/debug_grpc_testlib.h"
...
::grpc::Status TestEventListenerImpl::SendEvents(
    ...
    ::grpc::ServerReaderWriter<::machina::EventReply, ::machina::Event>*
        stream) {
  Event event;
  ...
  while (stream->Read(&event)) {
    if (event.has_log_message()) {
      ...
      std::vector<string> name_items =
          machina::str_util::Split(val.node_name(), ':');
      ...
      auto status = machina::protobuf::util::JsonStringToMessage(
