...
machina::GraphDef CreateTestProto() {
  machina::GraphDef g;
  machina::NodeDef* node = g.add_node();
  node->set_name("name1");
  node->set_op("op1");
  node = g.add_node();
  node->set_name("name2");
...
TEST_F(DefaultEnvTest, ReadWriteBinaryProto) {
  const machina::GraphDef proto = CreateTestProto();
