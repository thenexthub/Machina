...
#include "machina/cc/ops/tpu_configuration_ops.h"
...
absl::Status InitializeFromSession(absl::string_view session_target,
  ...
  machina::SessionOptions s_opts;
  s_opts.target = std::string(session_target);

  std::unique_ptr<machina::Session> sess(machina::NewSession(s_opts));
...
absl::Status InitializeTPUSystemGlobally() {
  ...
  return InitializeTPUSystemGlobally(machina::Env::Default(), &tpu_topology);
}
