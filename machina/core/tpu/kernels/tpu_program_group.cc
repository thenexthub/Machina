 ...
 *
 */
#include "machina/core/tpu/kernels/tpu_program_group.h"
...
TPUExecutableInfoProto TpuProgramGroup::ConstructExecutableInfo(
    const MACHINA_MACHINA_XLA_TpuProgram* xla_tpu_program) {
...
void TpuProgramGroup::Initialize(
    absl::Span<MACHINA_MACHINA_XLA_TpuProgram* const> xla_tpu_programs) {
  ...
  for (size_t i = 0; i < tpu_programs_.size(); ++i) {
    const MACHINA_MACHINA_XLA_TpuProgram* xla_tpu_program = tpu_programs_[i];
...
bool TpuProgramGroup::has_sharding_program() const {
  for (const MACHINA_MACHINA_XLA_TpuProgram* tpu_program : tpu_programs_) {
...
void TpuProgramGroup::UnloadAndDestroyPrograms() {
  for (MACHINA_MACHINA_XLA_TpuProgram* tpu_program : tpu_programs_) {
    StatusHelper status;
...
const std::vector<MACHINA_MACHINA_XLA_TpuProgram*>& TpuProgramGroup::tpu_programs() const {
...
const MACHINA_MACHINA_XLA_TpuProgram* TpuProgramGroup::tpu_program(int index) const {
  CHECK_GE(index, 0);
  CHECK_LT(index, tpu_programs_.size());
...
void TpuProgramGroup::set_tpu_programs(
    absl::Span<MACHINA_MACHINA_XLA_TpuProgram* const> tpu_programs) {
...
Status TpuProgramGroup::CompileAndBuild(
  ...
  size_t count = 0;
  MACHINA_MACHINA_XLA_TpuProgram** xla_tpu_programs = nullptr;
  StatusHelper status;
  ...
  TpuProgramGroup* tpu_program_group =
      machina::down_cast<TpuProgramGroup*>(tpu_program_group_interface);
...
std::vector<MACHINA_MACHINA_XLA_TpuProgram*> TpuProgramGroup::tpu_programs(
    TpuProgramShardingType sharding_type) const {
  std::vector<MACHINA_MACHINA_XLA_TpuProgram*> tpu_programs;
