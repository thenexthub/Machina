...
#include "machina/core/tpu/kernels/tpu_functional_ops.h"
...
#include "machina/core/tpu/kernels/tpu_op_util.h"
  ...
  for (Node* node : graph->op_nodes()) {
    if (node->type_string() == "TPUCompile" ||
      ...
      if (node->type_string() == "_TPUCompileMlir") {
        ...
        fingerprint = machina::Fingerprint64(mlir_module);
      } else {
...
absl::Status TPUPartitionedCallOp::InferShapesWithResourceVar(
  ...
  TF_RETURN_IF_ERROR(machina::InferShapes(
