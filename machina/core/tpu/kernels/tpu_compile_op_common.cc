...
#include "machina/core/protobuf/tpu/compilation_result.pb.h"
#include "machina/core/protobuf/tpu/compile_metadata.pb.h"
#include "machina/core/tpu/kernels/tpu_compilation_cache_entry_unloader.h"
...
#include "machina/core/tpu/kernels/tpu_util.h"
...
void TpuCompileOpKernelCommon::Compute(OpKernelContext* ctx) {
  ...
  if (!compile_status.ok() &&
    ...
    compile_status = machina::errors::CreateWithUpdatedMessage(
...
absl::Status TpuCompileOpKernelCommon::CompileLocallyAndFillHostCache(
  ...
  tsl::OkOrSetErrorCounterPayload(
      machina::core::platform::ErrorSourceProto::TPU_COMPILE_OP, status);
  return status;
}
