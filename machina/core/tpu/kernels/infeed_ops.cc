...
#include "machina/core/tpu/kernels/infeed_ops.h"
...
#include "machina/core/framework/variant_encode_decode.h"  // IWYU pragma: keep
...
typedef machina::tpu::NoncopyableBuffer LinearizerBuffer;
...
xla::Shape GetTPUInfeedLayout(const xla::Shape& shape) {
  ...
  MACHINA_MACHINA_XLA_Shape c_infeed_shape;
...
// Transposes the given tensor using the machina C++ transpose implementation
...
struct LinearizedBuffersWrapper {
  ...
  // These functions are machina::Variant requirements.
...
absl::Status InfeedEnqueuePrelinearizedBufferOp::DoWork(OpKernelContext* ctx,
  ...
  const LinearizedBuffersWrapper* wrapper =
      input_tensor.scalar<machina::Variant>()()
