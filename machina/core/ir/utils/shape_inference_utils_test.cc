...
class ShapeInferenceTest : public ::testing::Test {
 protected:
  ...
  void VerifyInferredShapes(OpRange &&ops,
                            ...
                            bool check_type) {
    for (auto it : toolchain::zip(ops, inferred_result)) {
...
TEST_F(ShapeInferenceTest, TestShapeAndTypeInference) {
  ...
  auto op_result_as_shape_fn = [](InferenceContext &ic,
    ...
    for (auto element : toolchain::enumerate(attr.getValues<APInt>()))
...
TEST_F(ShapeInferenceTest, TestInferenceFailure) {
  ...
  auto get_empty_attr_values_fn =
      [](Operation *, toolchain::StringRef, const machina::OpRegistrationData *,
         bool, machina::AttrValueMap *) { return absl::OkStatus(); };
  ...
  auto error_attr_values_fn = [](Operation *, toolchain::StringRef,
                                 const machina::OpRegistrationData *, bool,
                                 machina::AttrValueMap *) {
    return machina::errors::Unknown("Intended error");
  };
