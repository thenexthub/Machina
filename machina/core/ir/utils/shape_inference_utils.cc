...
// Get the machina op name.
toolchain::StringRef GetTensorFlowOpName(Operation* inst) {
  toolchain::StringRef op_name = inst->getName().stripDialect();
...
std::optional<machina::PartialTensorShape> GetShapeFromMlirType(Type t) {
  if (auto ranked_type = toolchain::dyn_cast<RankedTensorType>(t)) {
    machina::PartialTensorShape shape;
    const absl::Status status =
        machina::PartialTensorShape::BuildPartialTensorShape(
...
std::optional<machina::PartialTensorShape> GetShapeFromMlirAttr(Value v) {
  // Function arguments may have shape attr to describe its output shape.
  if (auto arg = dyn_cast<BlockArgument>(v)) {
    Operation* parent_op = arg.getOwner()->getParentOp();
    if (auto func_op = toolchain::dyn_cast<FunctionOpInterface>(parent_op)) {
      ...
      auto shape_attr = toolchain::dyn_cast<tf_type::ShapeAttr>(attrs[0]);
      if (shape_attr && shape_attr.hasRank())
        return machina::PartialTensorShape(shape_attr.getShape());
    }
  }
  return std::nullopt;
}
...
std::unique_ptr<std::vector<
    std::pair<machina::PartialTensorShape, machina::DataType>>>
GetSubtypesHelper(Type type) {
  auto type_with_subtypes =
      toolchain::dyn_cast<T>(toolchain::cast<TensorType>(type).getElementType());
...
LogicalResult ReportErrorFromShapeFunction(std::optional<Location> location,
                                           ...
                                           toolchain::StringRef error_message) {
...
std::optional<SmallVector<int64_t, 8>> GetShapeFromHandle(
  ...
  for (int dim : toolchain::seq<int>(0, context.Rank(sh)))
...
LogicalResult InferReturnTypeComponentsForTFOp(
  ...
  toolchain::StringRef op_name = GetTensorFlowOpName(op);
  ...
  machina::AttrValueMap attributes;
  ...
  std::vector<machina::PartialTensorShape> input_shapes(num_operands);
  ...
  InferenceContext c(graph_version, machina::AttrSlice(&attributes),
  ...
  std::vector<const machina::Tensor*> input_tensors(num_operands);
  std::vector<machina::Tensor> tensors(num_operands);
  ...
  auto requires_inputs = [&]() {
    return any_of(toolchain::seq<int>(0, c.num_inputs()), [&](int input) {
  ...
  while (requires_inputs()) {
    ...
    for (int input : toolchain::seq<int>(0, c.num_inputs())) {
      if (input_tensors[input]) continue;

      if (c.requested_input_tensor(input)) {
        if (auto attr = toolchain::dyn_cast_if_present<ElementsAttr>(
                operand_as_constant_fn(op->getOperand(input)))) {
          VLOG(4) << "Requesting " << input << " as constant\n";
          machina::Tensor* input_tensor = &tensors.at(input);
  ...
  for (int output : toolchain::seq<int>(0, c.num_outputs())) {
