...
#include "machina/core/ir/types/attributes.cc.inc"
#include "machina/core/ir/types/attributes_enum.cc.inc"

#define GET_TYPEDEF_CLASSES
#include "machina/core/ir/types/types.cc.inc"

// Generated definitions.
#include "machina/core/ir/types/dialect.cpp.inc"
...
#include "machina/core/ir/types/types.def"
           >();
}
...
Type TFTypeDialect::parseType(DialectAsmParser& parser) const {
  StringRef type_tag;
  toolchain::SMLoc loc = parser.getNameLoc();
...
#include "machina/core/ir/types/types.def"
  ...
  if (failed(generatedTypePrinter(type, printer)))
    toolchain::report_fatal_error("unexpected machina graph type kind");
}
...
FailureOr<FullTypeAttr> RawFullTypeAttrParser(AsmParser& parser) {
  ...
  toolchain::StringRef type_id_str;
...
static void RawFullTypeAttrPrint(FullTypeAttr tfattr, AsmPrinter& printer) {
  ...
  if (!tfattr.getArgs().empty()) {
    printer << "<";
    toolchain::interleaveComma(tfattr.getArgs(), printer, [&](Attribute arg) {
...
void ShapeAttr::print(AsmPrinter& os) const {
  ...
  if (hasRank()) {
    ...
    toolchain::interleave(getShape(), os, print_dim, "x");
  } else {
    os << "*";
  }
  os << ">";
...
Attribute ShapeAttr::parse(AsmParser& parser, Type type) {
  ...
  return ShapeAttr::get(parser.getContext(), toolchain::ArrayRef(shape));
}
...
bool GetCastCompatibleShape(ArrayRef<int64_t> a_shape,
  ...
  for (auto dims : toolchain::zip(a_shape, b_shape)) {
...
OperandShapeIterator::OperandShapeIterator(Operation::operand_iterator it)
    : toolchain::mapped_iterator<Operation::operand_iterator,
                            std::optional<ArrayRef<int64_t>> (*)(Value)>(
          it, &GetShape) {}

ResultShapeIterator::ResultShapeIterator(Operation::result_iterator it)
    : toolchain::mapped_iterator<Operation::result_iterator,
                            std::optional<ArrayRef<int64_t>> (*)(Value)>(
          it, &GetShape) {}
...
bool TensorFlowType::classof(Type type) {
  return toolchain::isa<TFTypeDialect>(type.getDialect());
}
...
TensorFlowType TensorFlowRefType::get(Type type) {
  ...
  } else if (toolchain::isa<mlir::Float8E4M3FNType>(type)) {
    return Float8E4M3FNRefType::get(ctx);
  } else if (toolchain::isa<mlir::Float8E5M2Type>(type)) {
    return Float8E5M2RefType::get(ctx);
  } else if (toolchain::isa<mlir::Float8E4M3FNUZType>(type)) {
    return Float8E4M3FNUZRefType::get(ctx);
  } else if (toolchain::isa<mlir::Float8E4M3B11FNUZType>(type)) {
    return Float8E4M3B11FNUZRefType::get(ctx);
  } else if (toolchain::isa<mlir::Float8E5M2FNUZType>(type)) {
...
// NOLINTNEXTLINE
#include "machina/core/ir/types/types.def"
  llvm_unreachable("unexpected machina ref type kind");
}
...
Type TensorFlowTypeWithSubtype::RemoveSubtypes() {
  ...
  llvm_unreachable("unexpected machina type with subtypes kind");
}
...
bool BroadcastCompatible(TypeRange lhs, TypeRange rhs) {
  ...
  for (auto types : toolchain::zip(lhs, rhs)) {
    ...
    if (lhs_et != rhs_et) {
      ...
      if (!lhs_wst_st.empty() && !rhs_wst_st.empty()) {
        for (auto subtypes : toolchain::zip(lhs_wst_st, rhs_wst_st)) {
...
Type GetCastCompatibleType(Type a, Type b, bool may_ignore_ref_type_a) {
  ...
  if (!a_tt && !b_tt) {
    ...
    for (auto subtypes : toolchain::zip(a_wst_st, b_wst_st)) {
      Type refined_st =
