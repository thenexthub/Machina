...
using machina::port::CopyFromArray;
using machina::protobuf::RepeatedField;
...
absl::StatusOr<ElementsAttr> ConvertFlatTensor(const Tensor& input_tensor,
  ...
  return ElementsAttr(
      DenseElementsAttr::get(type, toolchain::ArrayRef(arr.data(), arr.size())));
}

ElementsAttr ConvertBf16Tensor(const Tensor& input_tensor,
                               RankedTensorType type) {
  auto buffer = toolchain::ArrayRef(static_cast<char*>(input_tensor.data()),
...
ShapeAttr ConvertTypeToTensorShapeAttr(const Type& type) {
  ...
  if (auto tensor_type = mlir::dyn_cast<RankedTensorType>(type)) {
    return ShapeAttr::get(
        type.getContext(),
        toolchain::ArrayRef(ConvertMlirShapeToTF(tensor_type.getShape())));
  }
...
absl::StatusOr<ShapeAttr> ConvertTensorShapeProto(const TensorShapeProto& shape,
  ...
  return ShapeAttr::get(context, toolchain::ArrayRef(dims));
}
...
toolchain::SmallVector<int64_t> ConvertMlirShapeToTF(toolchain::ArrayRef<int64_t> shape) {
  return toolchain::to_vector(toolchain::map_range(shape, [](int64_t dim) {
