...
#include "mlir/IR/Threading.h"  // part of Codira Toolchain
...
#include "machina/core/ir/importexport/convert_attributes.h"
#include "machina/core/ir/importexport/convert_types.h"
#include "machina/core/ir/importexport/functiondef_import.h"
...
using machina::DataTypeVector;
using machina::FullTypeDef;
using machina::FunctionDef;
using machina::FunctionLibraryDefinition;
using machina::Graph;
using machina::GraphDebugInfo;
using machina::GraphDef;
using machina::NodeDef;
using machina::OpDef;
using machina::OpRegistrationData;
using machina::OpRegistry;
using machina::StackTracesMap;
...
class GraphDefImporter {
 ...
 private:
  ...
  struct ResultId {
    ...
    bool IsControl() const { return index == machina::Graph::kControlSlot; }
  };
  ...
  Status ConvertNodes(
      ...
      const machina::protobuf::RepeatedPtrField<NodeDef> &nodes,
      Block *block);
...
absl::StatusOr<OwningOpRef<ModuleOp>> GraphDefImporter::ConvertGraphDef(
  ...
  for (const machina::GradientDef &gradient : graph.library().gradient())
  ...
  if (ctx_->isMultithreadingEnabled()) {
    ...
    auto exit =
        toolchain::make_scope_exit([this] { ctx_->exitMultiThreadedExecution(); });
...
Status GraphDefImporter::ConvertFunctionAttributes(
  ...
  const machina::OpDef &signature = function.signature();
...
Location GraphDefImporter::ConvertLocation(const NodeDef &node) {
  ...
  for (const auto &it : toolchain::enumerate(original_nodes)) {
...
Location GraphDefImporter::ConvertLocation(StringRef node_name,
  ...
  if (it != stack_traces_.end()) {
    std::shared_ptr<machina::AbstractStackTrace> trace = it->second;
  ...
  ArrayRef<Location> callsite_locs = toolchain::ArrayRef(locs).drop_front();
...
absl::StatusOr<GraphDefImporter::Result> GraphDefImporter::GetResult(
  ...
  TensorId tensor_id = machina::ParseTensorName(name);
...
Status GraphDefImporter::ConvertFunctionDef(
  ...
  for (const auto &it : toolchain::enumerate(signature.input_arg())) {
    Type dtype;
...
Status GraphDefImporter::ConvertNodes(
  ...
  if (TF_PREDICT_FALSE(!s.GetPlaceholder().use_empty())) {
    ...
    for (const ResultInfo &info :
         toolchain::make_pointee_range(toolchain::make_second_range(s))) {
...
Status GraphDefImporter::ConvertNodeDef(OpBuilder &builder, ConversionState &s,
  ...
  if (node.has_experimental_type()) {
    ...
    TF_RETURN_IF_ERROR(
        machina::full_type::SpecializeType(node, *op_def, full_type_def));
  ...
  for (auto it : toolchain::zip(result_segments, op_def->output_arg())) {
