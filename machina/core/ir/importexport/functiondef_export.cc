...
absl::StatusOr<FunctionDef> ConvertGenericFunctionToFunctionDef(
  ...
  for (int arg_num : toolchain::seq<int>(0, func_op.getNumArguments())) {
  ...
  // FunctionDef.
  auto return_op = toolchain::cast<tfg::ReturnOp>(
      func_op.SingleBlock::getBody()->getTerminator());
  ArrayAttr results_attr = func_op.getAllResultAttrs();
  for (const auto &indexed_result : toolchain::enumerate(return_op->getOperands())) {
  ...
  if (unique_ids_keys) {
    ...
    for (auto key_value : toolchain::zip(unique_ids_keys.getValues<int32_t>(),
