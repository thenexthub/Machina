...
static void NormalizeNode(NodeDef* node, bool add_fulltype) {
  ...
  (void)machina::OpRegistry::Global()->LookUpOpDef(node->op(), &op_def);
  ...
  if (add_fulltype) {
    if (node->has_experimental_type()) {
      ...
      (void)machina::OpRegistry::Global()->LookUp(node->op(), &op_reg_data);
...
void NormalizeTensorData(GraphDef& graphdef, bool add_fulltype) {
  ...
  toolchain::sort(*library->mutable_function(),
  ...
  toolchain::sort(*graphdef.mutable_node(),
  ...
  for (int func_id = 0; func_id < library->function_size(); ++func_id) {
    ...
    toolchain::sort(*func->mutable_node_def(), [](NodeDef& lhs, NodeDef& rhs) {
      return lhs.name() < rhs.name();
    });
    ...
    for (const auto& it : *func->mutable_ret()) {
      ...
      toolchain::SmallVector<int> to_erase;
