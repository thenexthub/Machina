...
#include "machina/core/ir/importexport/functiondef_export.h"
...
class GraphDefExporter {
 public:
  GraphDefExporter(
      ...
      toolchain::PointerUnion<SymbolTable *, const FunctionLibraryDefinition *>
          function_table)
...
static Status ConvertAttributes(
    machina::protobuf::Map<std::string, AttrValue> *map,
...
absl::StatusOr<std::optional<GradientDef>> GraphDefExporter::ExportFunction(
  ...
  if (DenseIntElementsAttr keys = func.getResourceArgUniqueIdsKeysAttr()) {
    ...
    for (auto kv :
         toolchain::zip(keys.getValues<int32_t>(), values.getValues<int32_t>()))
  ...
  for (int i = 0, e = func.getNumArguments(); i < e; i += 2) {
    ...
    if (toolchain::any_of(attrs, [](const NamedAttribute &attr) {
  ...
  for (auto it :
       toolchain::zip(func.getResultTypes(),
  ...
  for (auto it :
       toolchain::zip(return_op.getControlRetAttrs().getAsRange<DictionaryAttr>(),
...
Status ConvertToGraphDef(ModuleOp module, machina::GraphDef *graph) {
