...
class ValueMapManager {
 public:
  ValueMapManager(
      toolchain::StringMap<toolchain::StringMap<SmallVector<Value, 1>>>& values_map,
  ...
  Status DefineOperation(Operation* op, StringRef node_name) {
    toolchain::StringMap<SmallVector<Value, 1>>& op_info = values_map_[node_name];
  ...
  absl::StatusOr<Value> GetValueOrCreatePlaceholder(StringRef full_name) {
    ...
    if (is_control_dep) full_name = full_name.drop_front();
    {
      ...
      if (colon_sep != StringRef::npos) {
        ...
        if (!toolchain::getAsUnsignedInteger(output_name.drop_front(colon_sep + 1),
                                        10, value)) {
          if (LLVM_LIKELY(
                  value <=
                  std::numeric_limits<toolchain::SmallVectorSizeType<Value>>::max() -
                      1))
            output_num = value;
          else
 ...
 private:
  toolchain::StringMap<toolchain::StringMap<SmallVector<Value, 1>>>& values_map_;
  OpBuilder& builder_;
  Location loc_;
...
absl::StatusOr<NamedAttrList> ConvertArgDefAttributes(const OpDef::ArgDef& arg,
  ...
  Type input_type;
  if (arg.type() != machina::DT_INVALID) {
...
Status ImportGenericFunction(
  ...
  for (const auto& enumerated_input : toolchain::enumerate(signature.input_arg())) {
  ...
  // input names.
  for (auto type_and_name : toolchain::zip(arg_types, arg_names)) {
    Value arg = body->addArgument(std::get<0>(type_and_name), unknown_loc);
    toolchain::StringMap<SmallVector<Value, 1>>& values =
  ...
  res_num = 0;
  toolchain::StringMap<int> output_name_to_position;
  for (const OpDef::ArgDef& output : signature.output_arg()) {
  ...
  res_num = 0;
  toolchain::StringMap<int> control_output_to_position;
  for (const std::string& output : signature.control_output()) {
  ...
  for (const auto& indexed_val : toolchain::enumerate(ret_vals)) {
