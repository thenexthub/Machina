...
#include "machina/core/ir/importexport/mangling.h"
...
Status ConvertAttribute(Type type, bool remove_ref_type, AttrValue* value) {
  ...
  if (machina::IsRefType(dtype)) dtype = machina::RemoveRefType(dtype);
...
absl::StatusOr<AttrValue> ConvertAttribute(Attribute attr) {
  ...
  TF_RETURN_IF_ERROR(
      toolchain::TypeSwitch<Attribute, Status>(attr)
...
absl::StatusOr<tf_type::FullTypeAttr> ConvertAttribute(
  ...
  for (const machina::FullTypeDef& it : full_type.args()) {
  ...
  switch (full_type.attr_case()) {
    case machina::FullTypeDef::AttrCase::kS:
      attr = builder.getStringAttr(full_type.s());
      break;
    case machina::FullTypeDef::AttrCase::kI:
      attr = builder.getI64IntegerAttr(full_type.i());
      break;
    case machina::FullTypeDef::ATTR_NOT_SET:
      break;
    default:
...
absl::StatusOr<machina::FullTypeDef> ConvertAttribute(
  ...
  using FullTypeDef = machina::FullTypeDef;
  ...
  if (full_type.getAttr()) {
    bool converted = toolchain::TypeSwitch<Attribute, bool>(full_type.getAttr())
  ...
  ret.set_type_id(
      static_cast<machina::FullTypeId>(full_type.getTypeId().getInt()));
...
absl::StatusOr<ArrayAttr> ConvertHandleData(
  ...
  for (const auto& handle : handle_data) {
    if (handle.dtype() == machina::DT_INVALID)
