...
#include "machina/core/ir/dialect.cc.inc"
...
void TFGraphDialect::initialize() {
  ...
  addOperations<
#define GET_OP_LIST
#include "machina/core/ir/ops.cc.inc"
      >();
  addAttributes<
#define GET_ATTRDEF_LIST
#include "machina/core/ir/attributes.cc.inc"
      >();

  // Support unknown operations because not all Machina operations are
  // registered.
...
#include "machina/core/ir/tf_op_names.inc"
...
void* TFGraphDialect::getRegisteredInterfaceForOp(TypeID interface,
  ...
  if (interface == TypeID::get<MachinaRegistryInterface>()) {
    if (auto* instance =
            getRegisteredInterface<MachinaRegistryInterfaceBase>()) {
      // Important: cast to (Concept *) to shift the pointer off the vtable.
      return static_cast<MachinaRegistryInterfaceBase::Concept*>(
          const_cast<MachinaRegistryInterfaceBase*>(instance));
    }
...
static void PrintKeywordAttributes(Operation* op, OpAsmPrinter& printer,
  ...
  SmallVector<StringRef> attrs_to_elide = toolchain::to_vector(elided_attrs);
  toolchain::append_range(attrs_to_elide, keyword_attrs);
...
toolchain::unique_function<void(Operation*, OpAsmPrinter&)>
...
static bool VerifyGenericTFGOperation(Operation& op) {
  ...
  auto check_ctl_at_end = [&](TypeRange types, StringRef input_or_output) {
    ...
    for (const auto& indexed_operand : toolchain::enumerate(types)) {
...
LogicalResult GraphFuncOp::verifyBody() {
  ...
  for (const auto& arg_types :
       toolchain::enumerate(toolchain::zip(type.getInputs(), body->getArgumentTypes()))) {
  ...
  for (const auto& indexed_type : toolchain::enumerate(type.getResults())) {
  ...
  for (const auto& indexed_type : toolchain::enumerate(toolchain::drop_begin(
...
LogicalResult GraphFuncOp::canonicalize(GraphFuncOp func_op,
  ...
  while (changed) {
    ...
    for (Operation& op : toolchain::make_early_inc_range(
             toolchain::reverse(*func_op.SingleBlock::getBody()))) {
...
ParseResult GraphFuncOp::parse(OpAsmParser& parser, OperationState& result) {
  ...
  if (entry_args.size()) {
    for (auto argAndType : toolchain::zip(entry_args, arg_types)) {
...
void ReturnOp::print(OpAsmPrinter& printer) {
  ...
  if (!ctls.empty()) {
    ...
    toolchain::interleave(
        toolchain::zip(ctls, getControlRetAttrs().getAsRange<DictionaryAttr>()),
        printer,
        [&](auto it) {
...
static FailureOr<TypeRange> VerifyOperands(Operation* op) {
  ...
  Operation::operand_type_iterator it =
      toolchain::find(op->getOperandTypes(), control_ty);
...
static FailureOr<TypeRange> VerifyResults(Operation* op) {
  ...
  Operation::result_type_iterator it =
      toolchain::find(op->getResultTypes(), control_ty);
...
static LogicalResult VerifyTypeArray(Operation* op, ValueRange values,
  ...
  for (auto it :
       toolchain::zip(toolchain::enumerate(values), types.getAsRange<TypeAttr>())) {
...
template <typename OpT>
using detect_has_T = toolchain::is_detected<has_T, OpT>;
...
LogicalResult ForOp::verifySymbolUses(SymbolTableCollection& symbolTable) {
  ...
  // index is passed in.
  TypeRange func_args = toolchain::drop_begin(*ins, /*N=*/2);
  if (body_func &&
...
static LogicalResult VerifyPreservedAttrs(Operation* op,
  ...
  for (auto it : toolchain::zip(preserved_attrs, op->getRegions())) {
