...
__device__ int machina_philox_thread_counter;
...
__global__ void FillKernel(
  ...
  auto old_counter_value = atomicAdd(&machina_philox_thread_counter, 1);
...
void UpdateVariableAndFill_Philox<GPUDevice, Distribution>::operator()(
  ...
  int zero = 0;
#if GOOGLE_CUDA
  cudaMemcpyToSymbol(machina_philox_thread_counter, &zero, sizeof(int));
#else  // MACHINA_USE_ROCM
  int status = hipMemcpyToSymbol(HIP_SYMBOL(machina_philox_thread_counter),
                                 &zero, sizeof(int));
