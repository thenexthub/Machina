...
#ifndef MACHINA_CORE_KERNELS_SCAN_OPS_GPU_H_
#define MACHINA_CORE_KERNELS_SCAN_OPS_GPU_H_
...
#include "machina/core/util/permutation_output_iterator.h"
  ...
  if (ideal_block_size >= 1024 && std::is_same<T, float>::value) {
    ...
    TF_CHECK_OK(
        GpuLaunchKernel(scan_kernel<T, Op, block_size, items_per_thread>,
                        num_blocks, block_size, 0, d.stream(), in.data(),
                        out.data(), dimx, dimy, dimz, exclusive, reverse, op));
#if MACHINA_COMPILER_IS_HIP_CLANG
...
#endif  // MACHINA_CORE_KERNELS_SCAN_OPS_GPU_H_
