...
#ifndef MACHINA_CORE_KERNELS_SEGMENT_REDUCTION_OPS_IMPL_H_
#define MACHINA_CORE_KERNELS_SEGMENT_REDUCTION_OPS_IMPL_H_
 ...
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (input.dtype() == DT_BFLOAT16 || input.dtype() == DT_HALF) {
      ...
      temp = machina::Tensor(DT_FLOAT, temp_shape);
    }
...
struct SparseSegmentGradFunctor<CPUDevice, T, Index, SegmentId> {
  void operator()(OpKernelContext* context,
    ...
    if (output->dtype() == DT_BFLOAT16 || output->dtype() == DT_HALF) {
      temp = machina::Tensor(DT_FLOAT, output->shape());
    }
    auto temp_flat = temp.flat_outer_dims<float>();
...
#endif  // MACHINA_CORE_KERNELS_SEGMENT_REDUCTION_OPS_IMPL_H_
