...
#ifndef MACHINA_CORE_KERNELS_SPARSE_MATMUL_OP_H_
#define MACHINA_CORE_KERNELS_SPARSE_MATMUL_OP_H_
...
#include "machina/xla/tsl/platform/windows/intrinsics_port.h"
#endif
...
EIGEN_DEVICE_FUNC inline Packet pexpand_bf16_l(const Packet& from) {
  ...
  tmp = (reinterpret_cast<const machina::uint32&>(from)) & 0xffff0000;
#else
  tmp = (reinterpret_cast<const machina::uint32&>(from) << 16) & 0xffff0000;
#endif
...
EIGEN_DEVICE_FUNC inline Packet4f pexpand_bf16_l(const Packet4f& from) {
  ...
  machina::uint32 p[4];
  pstoreu(r, from);
  machina::uint32* ir = reinterpret_cast<machina::uint32*>(r);
...
EIGEN_STRONG_INLINE Packet4f pload4bf16<Packet4f>(const float* from) {
  machina::uint32 p[4];
  const machina::uint32* ir =
      reinterpret_cast<const machina::uint32*>(from);
  p[0] = (ir[0] << 16) & 0xffff0000;
  p[1] = ir[0] & 0xffff0000;
  p[2] = (ir[1] << 16) & 0xffff0000;
  p[3] = ir[1] & 0xffff0000;
...
#endif  // MACHINA_CORE_KERNELS_SPARSE_MATMUL_OP_H_
