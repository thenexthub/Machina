...
#include "machina/core/kernels/meta_support.h"
...
#if defined(GEMMLOWP_NEON_32) && !defined(MACHINA_DISABLE_META) && \
    !defined(__APPLE__)
#define MACHINA_USE_META (1)
#endif
...
int GetWorkersCount(OpKernelContext* tf_context) {
  if (g_num_threads == 0) {
    return tf_context->device()->machina_cpu_worker_threads()->num_threads;
  }
...
void QuantizedGemmImpl(OpKernelContext* tf_context, const quint8* a_data,
  ...
  if (g_use_local_context) {
    ...
    auto& workers = *(tf_context->device()->machina_cpu_worker_threads());
