...
class FuzzOneHot : public FuzzSession {
  void BuildGraph(const Scope& scope) override {
    auto input =
        machina::ops::Placeholder(scope.WithOpName("input"), DT_UINT8);
    auto depth =
        machina::ops::Placeholder(scope.WithOpName("depth"), DT_INT32);
    auto on = machina::ops::Placeholder(scope.WithOpName("on"), DT_UINT8);
    auto off = machina::ops::Placeholder(scope.WithOpName("off"), DT_UINT8);
    (void)machina::ops::OneHot(scope.WithOpName("output"), input, depth, on,
                                  off);
  }
  ...
  void FuzzImpl(const uint8_t* data, size_t size) override {
    ...
    Tensor input_tensor(machina::DT_UINT8, TensorShape({input_size}));
    Tensor depth_tensor(machina::DT_INT32, TensorShape({}));
    Tensor on_tensor(machina::DT_UINT8, TensorShape({}));
    Tensor off_tensor(machina::DT_UINT8, TensorShape({}));
