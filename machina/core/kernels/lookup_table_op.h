...
#ifndef MACHINA_CORE_KERNELS_LOOKUP_TABLE_OP_H_
#define MACHINA_CORE_KERNELS_LOOKUP_TABLE_OP_H_
 ...
 public:
  ...
  explicit LookupTableOp(OpKernelConstruction* ctx)
    ...
    if (ctx->output_type(0) == DT_RESOURCE) {
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_temp(machina::DT_RESOURCE,
                                        machina::TensorShape({}), &table_));
    } else {
      OP_REQUIRES_OK(ctx,
                     ctx->allocate_temp(machina::DT_STRING,
                                        machina::TensorShape({2}), &table_));
    }
    OP_REQUIRES_OK(
...
class AnonymousLookupTableOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    Tensor table_tensor;
    OP_REQUIRES_OK(
        ctx, ctx->allocate_temp(machina::DT_RESOURCE,
                                machina::TensorShape({}), &table_tensor));
    if (ctx->track_allocations()) {
...
#endif  // MACHINA_CORE_KERNELS_LOOKUP_TABLE_OP_H_
