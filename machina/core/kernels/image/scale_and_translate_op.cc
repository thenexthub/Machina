...
#include "machina/core/kernels/image/scale_and_translate_op.h"
...
absl::Status ComputeSpansCore(OpKernelContext* context, const Kernel& kernel,
  ...
  TF_RETURN_IF_ERROR(context->allocate_temp(
      machina::DT_INT32, machina::TensorShape({output_size}),
      &spans->starts, alloc_attr));
  auto starts_vec = spans->starts.vec<int32>();
  TF_RETURN_IF_ERROR(context->allocate_temp(
      machina::DT_FLOAT,
      machina::TensorShape({spans->span_size * output_size}),
...
absl::Status ComputeGradSpansCore(OpKernelContext* context, const Spans& spans,
  ...
  alloc_attr.set_on_host(true);
  TF_RETURN_IF_ERROR(context->allocate_temp(
      machina::DT_INT32, machina::TensorShape({forward_input_size}),
      &grad_spans->starts, alloc_attr));
  auto grad_starts_vec = grad_spans->starts.vec<int32>();
  TF_RETURN_IF_ERROR(context->allocate_temp(
      machina::DT_FLOAT,
      machina::TensorShape({grad_spans->span_size * forward_input_size}),
