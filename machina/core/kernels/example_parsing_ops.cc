 ...
 protected:
  ...
  absl::Status ParseExampleVector(const example::FastParseExampleConfig& config,
    ...
    return FastParseExample(
        ...
        ctx->device()->machina_cpu_worker_threads()->workers, result);
  }
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES_OK(
        ctx, FastParseSequenceExample(
                 ...
                 ctx->device()->machina_cpu_worker_threads()->workers,
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    for (int i = 0; i < json_examples->NumElements(); ++i) {
      ...
      auto status = protobuf::util::JsonToBinaryStream(
          resolver_.get(), "type.googleapis.com/machina.Example", &in, &out);
