...
template <typename T, std::enable_if_t<toolchain::is_one_of<T, float, double>::value,
                                       bool> = true>
...
template <typename T,
          std::enable_if_t<toolchain::is_one_of<T, Eigen::half>::value, bool> = true>
T baseline_floor_div(T lhs, T rhs) {
  return static_cast<T>(std::floor(static_cast<float>(lhs / rhs)));
}

template <typename T, std::enable_if_t<toolchain::is_one_of<T, int8_t, int16_t,
...
template <typename T,
          std::enable_if_t<
              toolchain::is_one_of<T, uint8_t, uint16_t, uint32_t, uint64_t>::value,
              bool> = true>
T baseline_floor_div(T lhs, T rhs) {
  return lhs / rhs;
}
