...
def if_mlir_generated_gpu_kernels_enabled(if_true, if_false = []):
    return select({
        "//machina/core/kernels/mlir_generated:is_gpu_enabled": if_true,
        "//conditions:default": if_false,
    })

def if_mlir_generated_experimental_kernels_enabled(if_true, if_false = []):
    return select({
        "//machina/core/kernels/mlir_generated:is_experimental_enabled": if_true,
        "//conditions:default": if_false,
    })
...
_gen_kernel_bin_rule = rule(
    attrs = {
        ...
        "_tool": attr.label(
            executable = True,
            default = Label("//machina/compiler/mlir/tools/kernel_gen:hlo_to_kernel"),
            cfg = "exec",
        ),
...
def _gen_kernel_library(
    ...
    """ Generate a library with GPU kernels for a specific machina op.

    Args:
      name: The name of the produced library with kernels.
      op: The name of the machina op.
    ...
    if cuda_gpu_architectures() or rocm_gpu_architectures():
        for (type, output_type, jit, jit_i64_indexed_for_large_tensors) in all_kernels:
            ...
            test_args = [
                "$(location //machina/compiler/mlir/tools/kernel_gen:hlo_to_kernel)",
            ...
            sh_test(
                ...
                data = [
                    ...
                    "//machina/compiler/mlir/tools/kernel_gen:hlo_to_kernel",
                ],
            )
    ...
    kernel_deps = [
        ...
        for (type, output_type, jit, jit_i64_indexed_for_large_tensors) in all_kernels
    ] + ["//machina/compiler/mlir/tools/kernel_gen:tf_framework_c_interface"]
    cc_library(
        name = name,
        deps = if_gpu_is_configured(kernel_deps + [
            "//machina/compiler/mlir/tools/kernel_gen:tf_gpu_runtime_wrappers",
        ]),
        linkstatic = 1,
        tags = tags,
    )
