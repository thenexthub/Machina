...
#ifndef MACHINA_CORE_KERNELS_RANDOM_OP_GPU_H_
#define MACHINA_CORE_KERNELS_RANDOM_OP_GPU_H_

#if defined(__CUDACC__) || MACHINA_USE_ROCM

#include "machina/core/kernels/random_op.h"
#include "machina/core/kernels/random_ops_util.h"
...
class SampleCopier {
 public:
  inline __device__ void operator()(
      T* __restrict__ buf,
      const machina::random::Array<T, ElementCount>& array) const {
#pragma unroll
...
class SampleCopier<float, 4> {
 public:
  ...
  inline __device__ void operator()(
      ...
      const machina::random::Array<float, 4>& array) const {
...
class SampleCopier<int32, 4> {
 public:
  ...
  inline __device__ void operator()(
      ...
      const machina::random::Array<int32, 4>& array) const {
    ::int4 vec;
    vec.x = array[0];
    vec.y = array[1];
    vec.z = array[2];
...
class SampleCopier<double, 2> {
 public:
  ...
  inline __device__ void operator()(
      ...
      const machina::random::Array<double, 2>& array) const {
    double2 vec;
    vec.x = array[0];
    vec.y = array[1];
...
class SampleCopier<int64, 2> {
 public:
  ...
  inline __device__ void operator()(
      ...
      const machina::random::Array<int64, 2>& array) const {
    longlong2 vec;
    vec.x = array[0];
    vec.y = array[1];
...
#endif  // defined(__CUDACC__) || MACHINA_USE_ROCM

#endif  // MACHINA_CORE_KERNELS_RANDOM_OP_GPU_H_
