...
#ifndef MACHINA_CORE_KERNELS_BATCHING_UTIL_BASIC_BATCH_SCHEDULER_H_
#define MACHINA_CORE_KERNELS_BATCHING_UTIL_BASIC_BATCH_SCHEDULER_H_
...
//        table servable and a machina session servable).
...
//        lookups and machina runs may both be performed in the batch thread,
//        or alternatively the vocabulary lookup may occur in the request thread
//        with only the machina run performed in the batch thread.
//
...
//     table's version number must match that of the machina session),
...
#endif  // MACHINA_CORE_KERNELS_BATCHING_UTIL_BASIC_BATCH_SCHEDULER_H_
