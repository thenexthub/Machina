...
#include "machina/core/kernels/gather_nd_op.h"
...
#include "machina/core/kernels/training_op_helpers.h"
...
Status DoScatterOnCpu(OpKernelContext* c, Tensor* params, const Tensor& indices,
  ...
  c->device()->machina_accelerator_device_info()->event_mgr->ThenExecute(
      stream, [host_params] {});
  return OkStatus();
}
...
absl::Status DoScatter(OpKernelContext* c, Tensor* params,
  ...
  if (std::is_same<Device, GPUDevice>::value &&
      machina::OpDeterminismRequired() && !DisableScatterOpDeterminism()) {
...
class ResourceScatterUpdateOp : public OpKernel {
 ...
 private:
  ...
  void DoCompute(OpKernelContext* c, Var* v) {
    ...
    if (isCPUDevice<Device>() && op == machina::scatter_op::UpdateOp::DIV) {
