...
#ifndef MACHINA_CORE_KERNELS_CWISE_OPS_H_
#define MACHINA_CORE_KERNELS_CWISE_OPS_H_
...
struct safe_scalar_binary_pow_op {
  ...
  EIGEN_DEVICE_FUNC inline Scalar operator()(const Scalar& a,
    ...
    const Exponent safe_b = machina::internal::SubtleMustCopy(b);
...
struct safe_div_or_mod_op {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE T operator()(const T& a,
                                                     const T& b) const {
    const T safe_b = machina::internal::SubtleMustCopy(b);
    if (TF_PREDICT_TRUE(safe_b != 0)) {
      // Avoid FPE for INT_MIN/-1.
      const T safe_a = machina::internal::SubtleMustCopy(a);
...
struct base {
  ...
  // speaking, the machina operation has the signature:
...
#endif  // MACHINA_CORE_KERNELS_CWISE_OPS_H_
