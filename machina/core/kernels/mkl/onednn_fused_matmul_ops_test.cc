...
//    machina/core/kernels/mkl/mkl_fused_ops_test.cc
//
...
class FusedMatMulOpsTest : public OpsTestBase {
 ...
 protected:
  ...
  bool HasQuantizationSupport() {
    return TestCPUFeature(machina::port::CPUFeature::AVX_VNNI_INT8) ||
           TestCPUFeature(machina::port::CPUFeature::AVX512_VNNI) ||
  ...
  void RunAndFetch(const machina::Scope& root,
    ...
    machina::RewriterConfig* cfg =
        session_options.config.mutable_graph_options()
            ->mutable_rewrite_options();
    cfg->set_constant_folding(machina::RewriterConfig::OFF);
    cfg->set_layout_optimizer(machina::RewriterConfig::OFF);
    cfg->set_remapping(machina::RewriterConfig::OFF);

    std::unique_ptr<machina::Session> session(
        machina::NewSession(session_options));
  ...
  Output ActivationOp(Scope& root, string op, Output x, string name) {
    ...
    //    machina/python/grappler/remapper_test.py
    if (op == "Relu") {
