...
#include "machina/c/experimental/stream_executor/stream_executor_test_util.h"
...
TEST_F(WhileOpTest, WhileOpCPUBuildWithPluggableDevice) {
  ...
  se_.host_memory_allocate = [](const SP_Device* const device, uint64_t size) {
    ...
    return malloc(size);
#else
    return machina::port::AlignedMalloc(size, EIGEN_MAX_ALIGN_BYTES);
#endif
  };
...
#else
    mem->opaque = machina::port::AlignedMalloc(size, EIGEN_MAX_ALIGN_BYTES);
#endif
    mem->size = size;
  };
