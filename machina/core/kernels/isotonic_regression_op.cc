...
class IsotonicRegressionOp : public machina::OpKernel {
 public:
  explicit IsotonicRegressionOp(machina::OpKernelConstruction* context)
  ...
  void Compute(machina::OpKernelContext* context) override {
    ...
    OP_REQUIRES(context,
                machina::FastBoundsCheck(input.dimensions()[0], int_max) &&
                    machina::FastBoundsCheck(input.dimensions()[1], int_max),
                machina::errors::InvalidArgument("Tensor too large"));
    ...
    machina::Tensor* segments_tensor = nullptr;
