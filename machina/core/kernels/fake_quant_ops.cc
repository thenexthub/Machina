...
#include "machina/core/kernels/fake_quant_ops_functor.h"
...
using machina::BinaryElementWiseOp;
using machina::DEVICE_CPU;
#if (defined(GOOGLE_CUDA) && GOOGLE_CUDA) || \
    (defined(MACHINA_USE_ROCM) && MACHINA_USE_ROCM)
using machina::DEVICE_GPU;
#endif  // GOOGLE_CUDA || MACHINA_USE_ROCM
...
using machina::TTypes;  // NOLINT This is needed in CUDA mode, do not remove.
using machina::UnaryElementWiseOp;
...
auto* using_fake_quant = monitoring::Gauge<bool, 0>::New(
    "/machina/api/op/using_fake_quantization",
