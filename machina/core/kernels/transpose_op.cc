...
#include "machina/core/kernels/transpose_op.h"
...
absl::Status TransposeCpuOp::DoTranspose(OpKernelContext* ctx, const Tensor& in,
  ...
  return ::machina::DoTranspose(ctx->eigen_device<CPUDevice>(), in, perm,
                                   out);
}
...
absl::Status ConjugateTransposeCpuOp::DoTranspose(OpKernelContext* ctx,
                                                  ...
                                                  Tensor* out) {
  typedef Eigen::ThreadPoolDevice CPUDevice;
  return ::machina::DoConjugateTranspose(ctx->eigen_device<CPUDevice>(), in,
                                            perm, out);
}
...
#undef REGISTER
  ...
  return ::machina::DoTranspose(ctx->eigen_device<GPUDevice>(), in, perm,
                                   out);
}
Status ConjugateTransposeGpuOp::DoTranspose(OpKernelContext* ctx,
                                            ...
                                            Tensor* out) {
  typedef Eigen::GpuDevice GPUDevice;
  return ::machina::DoConjugateTranspose(ctx->eigen_device<GPUDevice>(), in,
                                            perm, out);
}
