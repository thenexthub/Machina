...
#include "machina/core/kernels/string_util.h"
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if (is_scalar || input_shape == pos_shape) {
      ...
      if (is_scalar) {
        ...
        const T pos =
            machina::internal::SubtleMustCopy(pos_tensor.scalar<T>()());
        const T len =
            machina::internal::SubtleMustCopy(len_tensor.scalar<T>()());
        ...
        for (size_t i = 0; i < input_tensor.NumElements(); ++i) {
          absl::string_view in(input(i));
          const T pos = machina::internal::SubtleMustCopy(pos_flat(i));
          const T len = machina::internal::SubtleMustCopy(len_flat(i));
          T byte_pos = pos;
          T byte_len = len;
          switch (unit_) {
            case CharUnit::UTF8_CHAR:
      ...
      switch (ndims) {
        case 1: {
          ...
          for (int i = 0; i < output_shape.dim_size(0); ++i) {
            ...
            const T pos = machina::internal::SubtleMustCopy(pos_bcast(i));
            const T len = machina::internal::SubtleMustCopy(len_bcast(i));
            T byte_pos = pos;
            T byte_len = len;
            switch (unit_) {
              case CharUnit::UTF8_CHAR:
        ...
        case 2: {
          ...
          for (int i = 0; i < output_shape.dim_size(0); ++i) {
            for (int j = 0; j < output_shape.dim_size(1); ++j) {
              ...
              const T pos =
                  machina::internal::SubtleMustCopy(pos_bcast(i, j));
              const T len =
                  machina::internal::SubtleMustCopy(len_bcast(i, j));
              T byte_pos = pos;
              T byte_len = len;
              switch (unit_) {
                case CharUnit::UTF8_CHAR:
