...
#include "machina/core/runtime_fallback/util/fallback_test_util.h"
...
std::unique_ptr<tfrt::CoreRuntime> CreateCoreRuntime() {
  auto diag_handler = [](const tfrt::DecodedDiagnostic& diag) {
    toolchain::errs() << "Encountered runtime error: " << diag.message() << "\n";
  };
...
void CoreRuntimeDriver::InitializeCpuRuntimeFallbackOpHandler() {
  auto fallback_op_handler = machina::tfd::CreateRuntimeFallbackOpHandler(
  ...
  cpu_op_handler.get()->AddImplicitConversion(
      machina::tfd::RuntimeFallbackTensor::kTensorType,
...
void CoreRuntimeDriver::InitializeCpuKernelFallbackOpHandler() {
  auto fallback_op_handler = machina::tfd::CreateKernelFallbackOpHandler(
  ...
  cpu_op_handler.get()->AddImplicitConversion(
      machina::KernelFallbackTensor::kTensorType,
      DenseHostTensor::kTensorType);
  cpu_op_handler.get()->AddImplicitConversion(
      machina::KernelFallbackTensor::kTensorType,
      AnyScalarHostTensor::kTensorType);
...
ExecutionContext CoreRuntimeDriver::CreateExecutionContext(
  ...
  auto exec_ctx = machina::tfd::CreateFallbackTestExecutionContext(
