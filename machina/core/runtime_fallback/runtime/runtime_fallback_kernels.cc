...
#include "machina/core/runtime_fallback/kernel/kernel_fallback_execute_compat.h"
...
#include "machina/core/runtime_fallback/runtime/runtime_fallback_op_handler.h"
...
// Create RuntimeFallbackTensor from machina::TensorHandle.
...
static absl::Status DecodeDenseAttrToTensorInterface(
  ...
  *result = machina::TensorInterface(std::move(t));
  return absl::OkStatus();
}

// Handle attributes.
//
// Refer to machina/core/framework/attr_value.proto and
// machina/c/eager/c_api.h.
//
    ...
    if (IsUnusedAttribute(entry.name)) {
      ...
      if (entry.element_count == 0) {
        ...
        *status_ptr = eager_op->SetAttrFunctionName(
        ...
        toolchain::ArrayRef<int64_t> int_array =
        ...
        *status_ptr = eager_op->SetAttrIntList(entry.name, int_array.data(),
        ...
        toolchain::ArrayRef<float> float_array =
        ...
        *status_ptr = eager_op->SetAttrFloatList(entry.name, float_array.data(),
        ...
        toolchain::ArrayRef<bool> bool_array =
        ...
        *status_ptr = eager_op->SetAttrBoolList(
        ...
        // to convert then to machina data types first.
        auto bef_dtypes =
            toolchain::ArrayRef(static_cast<const tfrt::DType*>(op_attr.GetData()),
                           op_attr.element_count);

        toolchain::SmallVector<machina::DataType, 4> tf_dtypes;
        ...
        *status_ptr =
            machina::errors::Internal("unsupported array attribute type");
      }
    } else {
      if (entry.type == OpAttrType::I64) {
        ...
        *status_ptr = eager_op->SetAttrType(entry.name, tf_dtype);
        ...
        machina::TensorInterface interface;
        *status_ptr =
        ...
        *status_ptr = eager_op->SetAttrTensor(entry.name, &interface);
        ...
        if (IsDataTypeAttribute(attr_base.type()) &&
          ...
          // Handle list(string).
          toolchain::SmallVector<const void*, 8> values;
          toolchain::SmallVector<size_t, 8> lengths;
          ...
          *status_ptr =
          ...
          toolchain::SmallVector<int, 8> ranks;
          toolchain::SmallVector<const int64_t*, 8> dims;
          ...
          *status_ptr =
              machina::errors::Internal("unsupported list attribute type");
        }
      } else {
        *status_ptr =
            machina::errors::Internal("unsupported scalar attribute type");
      }
    }
  });
  return status;
...
absl::Status CallEagerExecute(
    ...
    toolchain::ArrayRef<TensorHandle*> input_tensor_handles, const OpAttrsRef& attrs,
...
AsyncValueRef<Chain> RuntimeFallbackExecute(
    const tfrt::ExecutionContext& exec_ctx, EagerContext* eager_ctx,
    const char* op_name, const char* device_name,
    toolchain::ArrayRef<Tensor*> arguments, const OpAttrsRef& attrs,
    toolchain::MutableArrayRef<RCReference<AsyncValue>> results) {
  ...
  toolchain::SmallVector<TensorHandle*, 4> input_tensor_handles;
  input_tensor_handles.reserve(arguments.size());
  for (Tensor* input_tensor : arguments) {
    input_tensor_handles.push_back(
        toolchain::cast<RuntimeFallbackTensor>(input_tensor)->GetTensorHandle());
  }

  int num_retvals = results.size();
  toolchain::SmallVector<machina::AbstractTensorHandle*, 4> result_tensor_handles(
      num_retvals);
  absl::Status status;
...
AsyncValueRef<Chain> RuntimeFallbackExecute(
    ...
    const char* device_name, toolchain::ArrayRef<Tensor*> arguments,
    const OpAttrsRef& attrs,
    toolchain::MutableArrayRef<RCReference<AsyncValue>> results) {
  // Get EagerContext.
...
static void RuntimeFallbackKernel(
  ...
  toolchain::SmallVector<machina::AbstractTensorHandle*, 4> retvals(num_retvals);
...
// Convert the tfrt::TensorHandle to machina::Tensor. `device` is the target
// device for the converted machina::Tensor.
//
...
void CoreRTTensorHandleToFallbackTensorInternal(
    toolchain::ArrayRef<tfrt::AsyncValue*> tensorhandle_args,
    toolchain::MutableArrayRef<tfrt::RCReference<tfrt::AsyncValue>>
        tf_tensor_results,
  ...
  auto set_result = [&](tfrt::RCReference<tfrt::AsyncValue>& result,
                        toolchain::Expected<machina::Tensor> tf_tensor) {
  ...
  // will always convert it to machina::Tensor which is itself refcounted.
...
// The BEF kernel for tfrt::TensorHandle to machina::Tensor conversion.
...
// Convert the machina::Tensor to tfrt::TensorHandle. `device` is the device
// for the input machina::Tensor.
//
...
static void FallbackTensorToCoreRTTensorHandleInternal(
    toolchain::ArrayRef<tfrt::AsyncValue*> tf_tensor_args,
    toolchain::MutableArrayRef<tfrt::RCReference<tfrt::AsyncValue>>
  ...
  for (int i = 0; i < tf_tensor_args.size(); ++i) {
    ...
    auto& tf_tensor = av->get<machina::tfrt_stub::FallbackTensor>().tensor();
...
// The BEF kernel for machina::Tensor to tfrt::TensorHandle conversion.
...
tfrt::Chain PrintFallbackTensor(
    const machina::tfrt_stub::FallbackTensor& arg, const tfrt::Chain& ch) {
  std::string message;
  toolchain::raw_string_ostream(message) << arg.tensor().DebugString() << "\n";
...
// Since the arguments and results are machina::Tensors, internally it
// does conversions between RuntimeFallbackTensor and machina::Tensor.
static void RuntimeFallbackExecuteOp(
  ...
  // The device name might not be in the format expected by machina. In that
  ...
  // First we convert machina::Tensor to RuntimeFallbackTensors.
  toolchain::SmallVector<RuntimeFallbackTensor, 4> tfrt_tensor_args;
  tfrt_tensor_args.reserve(args.size());
  for (int i = 0; i < args.size(); ++i) {
    auto* av = args[i];
    auto tf_tensor = av->get<machina::Tensor>();
  ...
  toolchain::SmallVector<tfrt::Tensor*, 4> tfrt_tensor_arg_ptrs;
  tfrt_tensor_arg_ptrs.reserve(args.size());
  for (auto& tensor : tfrt_tensor_args) tfrt_tensor_arg_ptrs.push_back(&tensor);

  toolchain::SmallVector<RCReference<tfrt::AsyncValue>, 4> tfrt_tensor_results;
  ...
  // RuntimeFallbackTensor, back to machina::Tensor that is expected by the
  // BEF kernel.
  for (int i = 0; i < results.size(); ++i) {
    auto& runtime_fallback_tensor =
        tfrt_tensor_results[i]->get<RuntimeFallbackTensor>();
    const machina::Tensor* tf_tensor = nullptr;
    absl::Status s =
        runtime_fallback_tensor.GetTensorHandle()->Tensor(&tf_tensor);
    DCHECK(s.ok()) << s;
    results[i] =
        tfrt::MakeAvailableAsyncValueRef<machina::Tensor>(*tf_tensor);
  }
}
...
static OwnedTensorHandle ConvertTFRTTensorToTFTensorHandle(
    tfrt::Tensor* tensor) {
  if (auto* dht = toolchain::dyn_cast<tfrt::DenseHostTensor>(tensor)) {
  ...
  if (auto* sht = toolchain::dyn_cast<tfrt::StringHostTensor>(tensor)) {
...
static toolchain::Expected<tfrt::Value> ConvertTFTensorHandleToTFRTTensor(
  ...
  // toolchain::unique_function), and it gets released when HostBuffer deallocator is
