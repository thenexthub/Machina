 ...
 *
 */
#include "machina/core/runtime_fallback/kernel/kernel_fallback_execute.h"
...
namespace {
using machina::KernelFallbackTensor;
...
bool KernelFallbackExecute(
  ...
  toolchain::SmallVector<RCReference<AsyncValue>, 4> inputs;
  inputs.reserve(arguments.size());
  for (AsyncValue* input : arguments) {
    inputs.push_back(FormRef(input));
  }
  toolchain::SmallVector<RCReference<AsyncValue>, 4> outputs(results.begin(),
                                                        results.end());
       ...
       output_type = output_type]() mutable {
        ...
        if (op_kernel_context.error().has_value()) {
          ...
          for (int i = 0, e = outputs.size(); i != e; ++i) {
            // Expected result could be either a machina::Tensor
            ...
            if (output_type == KernelFallbackOutputType::TENSOR) {
              outputs[i]->emplace<machina::Tensor>(
