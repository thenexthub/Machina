 ...
 *
 */
#ifndef MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TENSOR_UTIL_H_
#define MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TENSOR_UTIL_H_
...
tfrt::AsyncValueRef<TensorWrapperType> TransferTensorToDevice(
  ...
  const bool src_cpu =
      src_device->machina_accelerator_device_info() == nullptr;
  const bool dst_cpu =
      dst_device->machina_accelerator_device_info() == nullptr;
  ...
  if (!dst_cpu && (src.dtype() != machina::DT_VARIANT &&
    ...
    return tfrt::MakeErrorAsyncValueRef(absl::InternalError(tfrt::StrCat(
        "Can't copy Tensor with type ", machina::DataTypeString(src.dtype()),
  ...
  machina::Tensor dst(dst_device->GetAllocator(attr), src.dtype(),
                         src.shape());
  ...
  bool enqueued = tfrt::EnqueueBlockingWork(
      exec_ctx.host(), [result = result.CopyRef(), src_cpu, dst_cpu, src_device,
        ...
        if (!src_cpu) {
          src_device_context =
              src_device->machina_accelerator_device_info()->default_context;
        }
        machina::DeviceContext* dst_device_context = nullptr;
        if (!dst_cpu) {
          dst_device_context =
              dst_device->machina_accelerator_device_info()->default_context;
        }
        ...
        absl::Status status;
        machina::CopyTensor::ViaDMA(
            "copy", src_device_context, dst_device_context, src_device,
            dst_device, machina::AllocatorAttributes(),
...
#endif  // MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TENSOR_UTIL_H_
