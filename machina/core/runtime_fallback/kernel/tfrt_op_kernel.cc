...
absl::Status TFRTOpKernelConstruction::GetAttr(absl::string_view attr_name,
  ...
  bool success = attributes_.GetString(
      toolchain::StringRef(attr_name.data(), attr_name.size()), &view);
  if (!success) {
...
absl::Status TFRTOpKernelConstruction::GetAttr(absl::string_view attr_name,
  ...
  bool success = attributes_.Get<tfrt::OpAttrType>(
      toolchain::StringRef(attr_name.data(), attr_name.size()), &attrtype);
  if (!success) {
    return MissingAttributeError(attr_name);
  }
...
absl::Status TFRTOpKernelConstruction::GetAttr(
  ...
  toolchain::ArrayRef<int32> arrayref;
  bool success = attributes_.GetArray<int32>(
      toolchain::StringRef(attr_name.data(), attr_name.size()), &arrayref);
  if (!success) {
    return MissingAttributeError(attr_name);
  }
  *value = arrayref;
...
std::string FillFailureMessage(const char* file, int line,
                               const absl::Status& s) {
  std::string error;
  toolchain::raw_string_ostream sstr(error);
...
TFRTOpKernelContext::TFRTOpKernelContext(
    toolchain::ArrayRef<tfrt::RCReference<tfrt::AsyncValue>> inputs, int num_outputs,
...
const TFRTOpMeta* TFRTOpMetaMap::GetOpMeta(absl::string_view op_name) const {
  auto it = op_metas_.find(toolchain::StringRef(op_name.data(), op_name.size()));
...
toolchain::ManagedStatic<TFRTOpMetaMap> tfrt_forwarding_op_meta_map;

toolchain::ManagedStatic<TFRTOpKernelFactories> tfrt_forwarding_kernel_factories;
...
absl::Status ValidKernelAttr(absl::string_view kernel_class_name,
                             ...
                             const toolchain::StringMap<DataType>& constraints) {
