...
#include "machina/core/runtime_fallback/kernel/kernel_fallback_utils.h"
#include "machina/core/runtime_fallback/runtime/op_logger.h"
...
#include "machina/core/tfrt/fallback/device_with_custom_allocator.h"
#include "machina/core/tfrt/fallback/op_kernel_runner.h"
#include "machina/core/tfrt/fallback/op_kernel_runner_cache.h"
...
using ::machina::tfrt_stub::OpKernelRunState;
...
static toolchain::Expected<absl::InlinedVector<machina::Tensor, 4UL>>
ConvertInputTensors(toolchain::ArrayRef<tfrt::Tensor*> arguments) {
...
static absl::Status ValidateInputTypes(
    ...
    const absl::InlinedVector<machina::Tensor, 4UL>& input_tf_tensors,
    const DataTypeVector& input_types) {
  const size_t n_inputs = input_tf_tensors.size();

  if (input_types.size() != n_inputs) {
    return machina::errors::InvalidArgument("expected ", input_types.size(),
                                               " inputs, got ", n_inputs);
  }
...
// Execute a machina::OpKernel Asynchronously. `kernel_runner` and
...
static void KernelFallbackExecuteCompatAsyncInternal(
  ...
  toolchain::SmallVector<AsyncValueRef<TensorType>, 4> result_refs;
  ...
  struct AsyncState {
    ...
    toolchain::SmallVector<tfrt::AsyncValueRef<TensorType>, 4> result_refs;
  };
...
// Execute a machina::OpKernel synchronously. `kernel_runner` and
// `input_tf_tensors` are expected to be alive during the call to this function.
...
tfrt::AsyncValueRef<tfrt::Chain> KernelFallbackExecuteCompatCoreRuntimeDispatch(
  ...
  if (!expected_input_tf_tensors) {
    status = machina::errors::Internal(
...
std::string GetTracingMetadata(toolchain::ArrayRef<tfrt::AsyncValue*> args,
                               const tfrt::ExecutionContext& exec_ctx,
                               const OpKernelRunner& kernel_runner) {
  ...
  toolchain::raw_string_ostream input_string_stream(input_string);
  ...
  toolchain::raw_string_ostream attr_string_stream(attr_string);
...
// expected to machina::tfrt_stub::FallbackTensor.
TF_ATTRIBUTE_ALWAYS_INLINE static void KernelFallbackExecuteOpInternal(
    toolchain::ArrayRef<tfrt::AsyncValue*> args,
    toolchain::MutableArrayRef<tfrt::RCReference<tfrt::AsyncValue>> results,
    tfrt::AsyncValueRef<tfrt::Chain>* op_chain,
  ...
  for (int i = 0; i < args.size(); ++i) {
    auto* arg = args[i];
    auto& fallback_tensor = arg->get<machina::tfrt_stub::FallbackTensor>();
    ...
    // the argument is unique but mutable, then machina optimizations like
...
// The BEF kernel for creating machina::OpKernel to be used in kernel
...
toolchain::Expected<tfrt::Chain> FallbackSetResource(
    tfrt::Argument<tfrt::Chain> in_ch,
    tfrt::Argument<machina::tfrt_stub::FallbackTensor> arg,
  ...
  resource_array->SetResource(
      index,
      machina::tfrt_stub::ImmutableTensor::Create(arg.get().tensor()));
...
void FallbackGetResource(tfrt::Argument<tfrt::Chain> in_ch,
  ...
  toolchain::ArrayRef<int64_t> indices = indices_attr.GetValue<int64_t>();
...
void FallbackAsyncExecuteOpSeq(tfrt::AsyncKernelFrame* frame) {
  ...
  toolchain::ArrayRef<tfrt::AsyncValue*> args = all_args.drop_front();
...
void KernelFallbackExecuteOpCustomAllocatorInternal(
  ...
  auto* allocator = args.front()->get<machina::Allocator*>();
  ...
  if (!fallback_tensor.is_immutable()) {
    ...
    for (int i = 0; i < results.size(); ++i) {
      auto immutable_tensor =
          machina::tfrt_stub::ImmutableTensor::Create(tensor);
    ...
    for (int i = 0; i < results.size(); ++i) {
      results[i] = tfrt::MakeAvailableAsyncValueRef<
          machina::tfrt_stub::FallbackTensor>(fallback_tensor);
    }
  }
}

toolchain::Expected<machina::tfrt_stub::FallbackTensor> ConstStringTensor(
    tfrt::ArrayAttr shape, tfrt::AggregateAttr value,
    const tfrt::ExecutionContext& context) {
  toolchain::SmallVector<int64_t> dims;
  ...
  return machina::tfrt_stub::FallbackTensor(tensor);
}

toolchain::Expected<machina::tfrt_stub::FallbackTensor> ConstTensorProto(
  ...
  return machina::tfrt_stub::FallbackTensor(std::move(tensor));
}

// Returns true if the machina::DataType is trivially copyable.
bool IsTriviallyCopyableTensorflowDataType(machina::DataType dtype) {
  static const auto* const non_trivially_copyable_dtypes =
      new absl::flat_hash_set<machina::DataType>{
          machina::DataType::DT_STRING, machina::DataType::DT_RESOURCE,
          machina::DataType::DT_VARIANT};
  return !non_trivially_copyable_dtypes->contains(dtype);
}

toolchain::Expected<machina::tfrt_stub::FallbackTensor> ConstDenseTensor(
  ...
  machina::Tensor tensor(dtype, machina::TensorShape(value.shape()));
  std::memcpy(tensor.data(), value.GetElements(), tensor.TotalBytes());
  return machina::tfrt_stub::FallbackTensor(tensor);
}

toolchain::Expected<bool> Predicate(
    const machina::tfrt_stub::FallbackTensor& input,
    const tfrt::ExecutionContext& exec_ctx) {
...
void BatchFunction(
    tfrt::RepeatedArguments<machina::tfrt_stub::FallbackTensor> args,
  ...
  auto attr_builder = [op_attr_array,
                       f](machina::AttrValueMap* attr_value_map) {
  ...
  input_tf_tensors.push_back(machina::Tensor(exec_ctx_intptr));
...
class TestAllocator : public machina::AllocatorWrapper {
 public:
  TestAllocator() : machina::AllocatorWrapper(machina::cpu_allocator()) {}
...
machina::Allocator* GetTestAllocator() {
