...
using ::machina::tfrt_stub::OpKernelRunnerTable;

void FallbackResourceArray::SetResource(
    int index, machina::tfrt_stub::ImmutableTensor tensor) {
  ...
  resources_.push_back(std::make_unique<machina::tfrt_stub::ImmutableTensor>(
      std::move(tensor)));
...
KernelFallbackCompatRequestState::KernelFallbackCompatRequestState(
  ...
  if (user_intra_op_threadpool != nullptr) {
    custom_cpu_device_ = machina::RenamedDevice::NewRenamedDevice(
    ...
    for (auto* device : device_manager_->ListDevices()) {
      custom_device_[device] = machina::RenamedDevice::NewRenamedDevice(
...
KernelFallbackCompatRequestState::KernelFallbackCompatRequestState(
    ...
    : KernelFallbackCompatRequestState(
          ...
          // third_party/machina/core/common_runtime/direct_session.cc
