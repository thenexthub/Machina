 ...
 *
 */
#include "machina/core/runtime_fallback/kernel/kernel_fallback_op_handler.h"
...
class KernelFallbackOpHandler : public tfrt::OpHandler {
 public:
  ...
  toolchain::Expected<tfrt::CoreRuntimeOp> MakeOp(
 ...
 private:
  ...
  friend toolchain::Expected<tfrt::OpHandler*> CreateKernelFallbackOpHandler(
      tfrt::CoreRuntime* runtime, tfrt::RCReference<tfrt::Device> device);

  toolchain::Error Initialize();
...
struct KernelFallbackOpHandlerCompatTraits {
  ...
  static void Dispatch(const CompatOpEntry& op_entry,
                       ...
                       toolchain::ArrayRef<tfrt::Tensor*> inputs,
                       const OpAttrsRef& attrs,
                       toolchain::ArrayRef<TensorMetadata> result_mds,
  ...
  return CoreRuntimeOp(
      [op_name = op_name.str(), this](const OpInvocation& invocation) {
        ...
        if (!fallback_op_entry.fallback_request_state) {
          propagate_error(machina::errors::NotFound(
        ...
        auto kernel_runner_or_status = runner_cache->GetOrCreate(
            ...
            [&attrs = invocation.attrs, host = invocation.exec_ctx.host()](
              ...
              if (auto error =
                ...
                return machina::errors::InvalidArgument(tfrt::StrCat(error));
...
toolchain::Expected<tfrt::OpHandler*> CreateKernelFallbackOpHandler(
    tfrt::CoreRuntime* runtime, tfrt::RCReference<tfrt::Device> device) {
...
toolchain::Error KernelFallbackOpHandler::Initialize() {
