...
#ifndef MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
#define MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
...
#include "machina/core/runtime_fallback/kernel/attr_util.h"
...
absl::Status TFRTOpKernelConstruction::GetAttr(absl::string_view attr_name,
  ...
  bool success = attributes_.Get<T>(
      toolchain::StringRef(attr_name.data(), attr_name.size()), value);
  if (!success) {
...
class TFRTOpKernelContext {
 public:
  explicit TFRTOpKernelContext(
      toolchain::ArrayRef<tfrt::RCReference<tfrt::AsyncValue>> inputs,
 ...
 private:
  toolchain::ArrayRef<tfrt::RCReference<tfrt::AsyncValue>> inputs_;
...
class TFRTOpMetaMap {
 ...
 private:
  toolchain::StringMap<TFRTOpMeta> op_metas_;
};

extern toolchain::ManagedStatic<TFRTOpMetaMap> tfrt_forwarding_op_meta_map;
...
struct TFRTOpKernelReg {
  ...
  toolchain::StringMap<DataType> type_constraints;
};
...
class TFRTOpKernelFactories {
 ...
 private:
  toolchain::StringMap<std::vector<TFRTOpKernelReg>> factories_;
};

// TODO(lauj) Should we move these kernel registrations to tfrt::KernelRegistry?
extern toolchain::ManagedStatic<TFRTOpKernelFactories>
...
#define REGISTER_KERNEL_FALLBACK_KERNEL_UNIQ(ctr, name, ...)             \
  static bool global_tfrt_forwarding_kernel_##ctr##_registered_ = []() { \
    ::machina::tfrt_forwarding_kernel_factories->RegisterFactory(     \
...
#endif  // MACHINA_CORE_RUNTIME_FALLBACK_KERNEL_TFRT_OP_KERNEL_H_
