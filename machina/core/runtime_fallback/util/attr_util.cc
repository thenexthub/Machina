...
using ::machina::protobuf::RepeatedFieldBackInserter;
...
toolchain::Expected<machina::Tensor> DecodeDenseAttrToTfTensor(
    const DenseAttr& dense_attr, HostContext* host) {
  toolchain::Expected<DenseHostTensor> dht =
...
toolchain::Error FillAttrValueMapUsingArray(const OpAttrsRawEntry& entry,
                                       AttrValue& attr_tmp,
...
toolchain::Error FillAttrValueMapUsingAggregate(const OpAttrsRawEntry& entry,
                                           AttrValue& attr_tmp,
                                           const OpAttrsRef& attrs) {
...
toolchain::Error FillAttrValueMapUsingScalar(const OpAttrsRawEntry& entry,
...
#include "machina/core/runtime_fallback/util/attr_type.def"  // NOLINT
    default:
...
absl::Status ParseTensorAttrValue(absl::string_view attr_value,
  ...
  if (std::is_base_of<machina::protobuf::Message,
                      machina::TensorProto>()) {
...
machina::Tensor CreateTfTensorFromDenseAttr(tfrt::DenseAttr attr) {
  machina::TensorShape shape(absl::InlinedVector<int64_t, 4>(
      attr.shape().begin(), attr.shape().end()));
  machina::DataType dtype = ConvertBefAttrTypeToTfDataType(attr.dtype());

  machina::Tensor tensor(dtype, shape);
...
absl::Status SetUpScalarAttr(tfrt::TypedAttrBase bef_attr,
                             machina::AttrValue* tf_attr) {
  if (auto shape_attr = bef_attr.dyn_cast<tfrt::ShapeAttr>()) {
    if (shape_attr.HasRank()) {
      machina::PartialTensorShape tf_shape(shape_attr.GetShape());
      tf_shape.AsProto(tf_attr->mutable_shape());
    } else {
      machina::PartialTensorShape unranked_shape;
    ...
    return machina::errors::Internal("Failed to set up attribute.");
  }
...
void AddShapeToAttrList(tfrt::ShapeAttr shape,
                        machina::AttrValue::ListValue* list) {
  if (shape.HasRank()) {
    machina::PartialTensorShape tf_shape(shape.GetShape());
...
absl::Status SetUpListAttr(tfrt::AggregateAttr aggregate_attr,
  ...
  for (int i = 0; i < aggregate_attr.GetNumElements(); ++i) {
    ...
    if (auto shape_attr = base.dyn_cast<tfrt::ShapeAttr>()) {
      ...
      return machina::errors::Internal("Failed to set up list attr.");
    }
  }
  return absl::OkStatus();
}
