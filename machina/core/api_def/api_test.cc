...
// Test that validates machina/core/api_def/base_api/api_def*.pbtxt files.
...
#include "machina/core/api_def/excluded_ops.h"
...
string DefaultApiDefDir() {
  return GetDataDependencyFilepath(
      io::JoinPath("machina", "core", "api_def", "base_api"));
}

string PythonApiDefDir() {
  return GetDataDependencyFilepath(
      io::JoinPath("machina", "core", "api_def", "python_api"));
}
...
void GetGoldenApiDefs(Env* env, const string& api_files_dir,
  ...
  for (auto& file_path : matching_paths) {
    ...
    ApiDefs api_defs;
    QCHECK(machina::protobuf::TextFormat::ParseFromString(file_contents,
                                                             &api_defs))
...
TEST_F(BaseApiTest, AllOpsAreInApiDef) {
  ...
  for (const auto& op : ops_.op()) {
    ...
    EXPECT_TRUE(api_defs_map_.find(op.name()) != api_defs_map_.end())
        ...
        << "under machina/core/api_def/base_api/ directory.";
  }
}
