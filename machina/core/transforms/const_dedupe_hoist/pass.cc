...
struct EquivalentConst : public toolchain::DenseMapInfo<Operation*> {
  static unsigned getHashValue(const Operation* op_c) {
    auto* op = const_cast<Operation*>(op_c);
    auto hash = toolchain::hash_value("");
    ...
    for (auto attr : op->getAttrs()) {
      ...
      hash = toolchain::hash_combine(hash, attr.getValue());
    }
    return hash;
  }
  ...
  static bool isEqual(const Operation* lhs_c, const Operation* rhs_c) {
    ...
    for (auto it : toolchain::zip(lhs->getAttrs(), rhs->getAttrs())) {
...
void DedupeAndHoistConstantPass::PropagateEdges(Operation* op) {
  ...
  for (Operation* user : users) {
    ...
    auto add_ctl_operands = [&](Operation* operation) {
      ...
      auto op_operands =
          toolchain::make_filter_range(TFOp(operation).getControlOperands(),
...
Operation* DedupeAndHoistConstantPass::BuildIdentity(Operation* input,
  ...
  auto attrs = toolchain::to_vector(
      toolchain::make_filter_range(input->getAttrs(), [&](NamedAttribute attr) {
