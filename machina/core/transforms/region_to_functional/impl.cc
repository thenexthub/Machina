...
static SmallVector<Value> GetForwardedValues(ValueRange added,
  ...
  toolchain::append_range(args, block_args.slice(prev.size(), added.size()));
  return args;
}
...
struct ConvertWhileLikeRegionOpToExplicitCapture
  ...
  WhileLikeRegionOp RebuildWith(WhileLikeRegionOp op, ValueRange added,
    ...
    SmallVector<Value> operands = toolchain::to_vector(op.getInit());
    toolchain::append_range(operands, added);
    SmallVector<Type> results = toolchain::to_vector(op.getOuts().getTypes());
    toolchain::append_range(results, added.getTypes());
...
FailureOr<std::vector<Value>> BasePattern::CollectValuesDefinedAboveAll(
  ...
  for (Region &region : toolchain::make_pointee_range(regions))
    CollectValuesDefinedAbove(region, data_set, ctl_only);
  toolchain::SmallVector<Value, 0> data_sv = data_set.takeVector();
  ...
  for (const auto &ctl : toolchain::enumerate(ctl_only.takeVector())) {
    Operation *const_op =
...
void BasePattern::IsolateRegions(RegionRange regions,
  ...
  Value data, ctl;
  for (Region &region : toolchain::make_pointee_range(regions)) {
    for (auto it : toolchain::zip(datas, ctls)) {
      std::tie(data, ctl) = it;
...
NamedAttrList BasePattern::BuildAttributes(RegionAttr preserved,
  ...
  for (const auto &it : toolchain::enumerate(arguments)) {
...
GraphFuncOp BasePattern::CreateFunc(Location loc, const Twine &sym_name,
  ...
  SmallVector<BlockArgument> args =
      toolchain::to_vector(GetLoopRegionDataArgs(region));
  SmallVector<BlockArgument> ctls =
      toolchain::to_vector(GetLoopRegionControlTokens(region));
  // TODO(jeffniu): Change GraphFuncOp to use the same argument order as region
  // loop ops.
  for (auto it : toolchain::zip(args, ctls)) {
  ...
  toolchain::BitVector indices(region.getNumArguments());
...
FuncAttr BasePattern::Outline(Operation *op, PatternRewriter &rewriter,
  ...
  if (StringAttr existing_name = GetFunctionName(preserved)) {
    ...
    if (auto op_name =
            op->getAttrOfType<StringAttr>(dialect_.getNameAttrIdentifier())) {
      toolchain::raw_string_ostream os(new_func_name);
      os << "_tfg_region_specialized_";
      for (char c : toolchain::map_range(
               op_name.getValue(), [](char c) { return isalnum(c) ? c : '_'; }))
        os << c;
      os << '_' << toolchain::to_string(region.getRegionNumber());
      os.flush();
    }
  }
...
void BasePattern::ReuseAllOrOutline(
  ...
  if (toolchain::all_of(regions, get_reusable_func)) return;
...
static bool HasNestedRegions(Region &region) {
  return toolchain::any_of(region.getOps(),
...
static bool RegionEqualTo(Region &region, GraphFuncOp func) {
  ...
  for (const auto &it : toolchain::enumerate(GetLoopRegionDataArgs(region))) {
  ...
  auto map_value_range = [](ValueRange lhs_range, ValueRange rhs_range,
    ...
    for (auto it : toolchain::zip(lhs_range, rhs_range))
  ...
  auto compare_ops = [&](Operation &lhs, Operation &rhs) {
    ...
    for (auto it : toolchain::zip(lhs_attrs, rhs_attrs)) {
  ...
  if (!toolchain::all_of_zip(region.front().without_terminator(),
...
ConvertToExplicitCapture<OpT>::Run(OpT op, PatternRewriter &rewriter) {
  ...
  for (auto it : toolchain::zip(op->getRegions(), new_op->getRegions()))
...
LogicalResult ConvertIfLikeOp<IfLikeRegionOp, IfLikeOp>::matchAndRewrite(
  ...
  SmallVector<Value> operands = toolchain::to_vector(args);
  toolchain::append_range(operands, op.getCtls());
...
LogicalResult ConvertCaseLikeOp<CaseLikeRegionOp, CaseLikeOp>::matchAndRewrite(
  ...
  if (toolchain::any_of(op.getBranches(), HasNestedRegions)) return failure();
  if (ArrayAttr preserved = op.getRegionAttrsAttr()) {
    if (toolchain::any_of(preserved.getAsRange<RegionAttr>(), [&](auto preserved) {
  ...
  for (const auto &it : toolchain::enumerate(op.getBranches())) {
...
ConvertWhileLikeOp<WhileLikeRegionOp, WhileLikeOp>::matchAndRewrite(
  ...
  if (!body_ref || !cond_ref) {
    ...
    toolchain::append_range(cond_rets, cond_op.getCtls());
...
LogicalResult ConvertForOp::matchAndRewrite(ForRegionOp op,
  ...
  toolchain::append_range(func_args, op.getInit());
