...
static SmallVector<toolchain::detail::ValueOfRange<RangeT>> FilterByIndex(
    RangeT &&range, const toolchain::BitVector &indices) {
  SmallVector<toolchain::detail::ValueOfRange<RangeT>> result;
  for (const auto &it : toolchain::enumerate(range))
...
static SmallVector<unsigned> GetPassthroughIndices(Region &region) {
  ...
  for (auto it : toolchain::zip(toolchain::enumerate(yield.getArgs()), iter_args)) {
...
static void EraseIterArgsFromTerminator(Operation *terminator,
  ...
  SmallVector<unsigned> cond_indices = toolchain::to_vector(indices);
...
struct EliminatePassthroughIterArgs {
  ...
  static size_t Run(OpT op, IRRewriter &rewriter) {
    ...
    toolchain::BitVector remove(op.getInit().size());
    ...
    SmallVector<Value> results = toolchain::to_vector(ValueRange(new_op.getOuts()));
    for (const auto &it : toolchain::enumerate(indices)) {
...
struct EliminateForPassthroughIterArgs
    : public EliminatePassthroughIterArgs<EliminateForPassthroughIterArgs,
                                          ForRegionOp> {
  static ForRegionOp RebuildOp(const toolchain::BitVector &indices, ForRegionOp op,
                               IRRewriter &rewriter) {
...
struct EliminateWhileLikePassthroughIterArgs
    : public EliminatePassthroughIterArgs<
          ...
          WhileLikeRegionOp> {
  static WhileLikeRegionOp RebuildOp(const toolchain::BitVector &indices,
