...
#include "machina/core/ir/importexport/convert_types.h"
...
static ElementsAttr CreateElementsAttrOfTypeValues(Type element_type,
  ...
  if (element_type.isIntOrIndex()) {
    const_attr = DenseElementsAttr::get(
        tensor_shape, toolchain::to_vector(value_attr.getValues<APInt>()));
  } else {
    const_attr = DenseElementsAttr::get(
        tensor_shape, toolchain::to_vector(value_attr.getValues<APFloat>()));
  }
  return const_attr;
}
...
static void AddControlOperand(Operation *op, Value control,
  ...
  if (toolchain::is_contained(op->getOperands(), control)) return;
...
static FailureOr<TFOp> ReplaceOpWithNoOp(OpBuilder &builder, TFOp op) {
  ...
  for (Value value : op->getOperands()) {
    ...
    if (!toolchain::is_contained(state.operands, control))
...
static FailureOr<TFOp> ReplaceOpWithSnapshot(OpBuilder &builder, TFOp op,
  ...
  toolchain::append_range(state.operands,
...
static FailureOr<TFOp> ReplaceOpWithBroadcastTo(OpBuilder &builder, TFOp op,
  ...
  for (const auto &it : toolchain::enumerate(op.getNonControlOperands())) {
    int idx = it.index();
    Value v = it.value();
    if (idx == idx_to_replace) continue;
    if (toolchain::is_contained(control_operands, v)) continue;
...
bool OpPropertyHelper::IsFreeOfSideEffect(TFOp op) {
  machina::OpRegistry *op_registry = machina::OpRegistry::Global();
...
bool OpPropertyHelper::MaybeFoldable(TFOp op) {
  ...
  if (ShouldPreserveOp(op) &&
      !(toolchain::any_of(  // Is a fetch node
  ...
  // TODO(rmlarsen): Only do this for MACHINA_MACHINA_XLA_* devices.
  for (const machina::OpDef::ArgDef &output_arg : op_def->output_arg()) {
    if (output_arg.type() == machina::DT_VARIANT) {
      return false;
    }
  }
...
bool OpPropertyHelper::ShouldPreserveOp(TFOp op) {
  ...
  return toolchain::any_of(op.controlRet().getUsers(), [&](TFOp child_op) {
...
static bool IsValidConstShapeForMulConvPushDown(StringAttr data_format,
  ...
  if (data_format == "NHWC" || data_format == "NDHWC") {
    ...
    if (filter_shape.getShape() != toolchain::ArrayRef(broadcast_shape))
      return false;
...
class EvaluateConstant : public FolderPatternBase<EvaluateConstant> {
 public:
  explicit EvaluateConstant(OpPropertyHelper &helper)
      : FolderPatternBase<EvaluateConstant>(MatchAnyOpTypeTag(), helper),
        ...
        resource_mgr_(std::make_unique<machina::ResourceMgr>()) {}
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (toolchain::all_of(op->getResults().drop_back(),
    ...
    for (const auto &it : toolchain::enumerate(result)) {
    ...
    if (const_ops.size() == 1) {
      ...
      for (const auto &it : toolchain::enumerate(const_ops)) {
        if (!it.value()) continue;
        for (OpOperand &use :
             toolchain::make_early_inc_range(op->getResult(it.index()).getUses())) {
 ...
 private:
  ...
  std::unique_ptr<machina::ResourceMgr> resource_mgr_;
};
...
class MaterializeShapeNOp : public FolderPatternBase<MaterializeShapeNOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
                                PatternRewriter &rewriter) const override {
    for (const auto &it : toolchain::enumerate(TFOp(op).getNonControlOperands())) {
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto get_shape = [this](Operation *op,
      ...
      if (dialect_->IsShape(op)) {
        ...
        toolchain::append_range(shape, type.getShape());
      } else {
        ...
        if (element_type.isInteger(32)) {
          toolchain::append_range(shape, attr.getValues<int32_t>());
        } else if (element_type.isInteger(64)) {
          toolchain::append_range(shape, attr.getValues<int64_t>());
        } else {
          return false;
        }
      }
    ...
    machina::BCast::Vec s0_vec(s0_shape.begin(), s0_shape.end());
    machina::BCast::Vec s1_vec(s1_shape.begin(), s1_shape.end());
    machina::BCast bcast(s0_vec, s1_vec);
    if (!bcast.IsValid()) return failure();

    machina::BCast::Vec reduce_dims[2];
    ...
    for (int j = 0; j < 2; ++j) {
      ...
      ElementsAttr const_attr = CreateElementsAttrOfTypeValues(
          ...
          toolchain::ArrayRef<int64_t>(reduce_dims[j].data(), reduction_indices));
      FailureOr<TFOp> const_op = CreateConstantTensorOp(
    ...
    for (OpOperand &user :
         toolchain::make_early_inc_range(op->getResult(0).getUses())) {
      rewriter.startOpModification(user.getOwner());
      user.set(const_values[0]);
      rewriter.finalizeOpModification(user.getOwner());
    }
    for (OpOperand &user :
         toolchain::make_early_inc_range(op->getResult(1).getUses())) {
      rewriter.startOpModification(user.getOwner());
 ...
 protected:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (Value operand : TFOp(op).getNonControlOperands()) {
      ...
      for (OpOperand &user :
           toolchain::make_early_inc_range(op->getResults()[0].getUses())) {
        rewriter.startOpModification(user.getOwner());
        user.set((*const_out)->getResult(0));
        rewriter.finalizeOpModification(user.getOwner());
      }
      for (OpOperand &user :
           toolchain::make_early_inc_range(op->getResults()[1].getUses())) {
        rewriter.startOpModification(user.getOwner());
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (multiples_attr.getElementType().isInteger(32)) {
      if (toolchain::any_of(multiples_attr.getValues<int32_t>(),
                       [](int v) { return v != 1; })) {
        return failure();
      }
    } else {
      if (toolchain::any_of(multiples_attr.getValues<int64_t>(),
 ...
 protected:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (paddings_attr.getElementType().isInteger(32)) {
      if (toolchain::any_of(paddings_attr.getValues<int32_t>(),
                       [](int v) { return v != 0; })) {
        return failure();
      }
    } else {
      if (toolchain::any_of(paddings_attr.getValues<int64_t>(),
...
class SimplifySqueezeOp : public FolderPatternBase<SimplifySqueezeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (toolchain::any_of(shape_type.getShape(), [](int64_t s) { return s <= 1; }))
      return failure();
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    auto simplify_result = [&](OpResult result, const bool const_value,
      ...
      for (OpOperand &user : toolchain::make_early_inc_range(result.getUses())) {
...
class SimplifyReshapeOp : public FolderPatternBase<SimplifyReshapeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // that toolchain::zip implementation requires copy assignment (it shouldn't)
    SmallVector<APInt> new_shape(shape_attr.getValues<APInt>());

    if (input_shape.getRank() != new_shape.size()) return failure();
    for (const auto &it : toolchain::zip(input_shape.getShape(), new_shape)) {
...
class ConstantPushDownBase : public Base<ConcreteType> {
 protected:
  ...
  bool IsOperandsSafeToMove(Operation *op_child, Operation *const_child) const {
    ...
    if (toolchain::any_of(op_child->getResults().drop_back(),
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!dialect_->IsConstant(y_node)) {
      ...
      if (c_shape.hasRank() && x_shape.hasRank() && c_shape.getRank() > 0) {
        for (auto it : toolchain::zip(c_shape.getShape(), x_shape.getShape())) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (OpOperand &operand : op->getOpOperands()) {
      ...
      if (!toolchain::is_contained(control_operands, control))
...
class PartialAssocOpConstFolding
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (toolchain::any_of(non_control_operands, [](Value v) {
          Operation *v_op = v.getDefiningOp();
          return v_op &&
...
class MergeConcatOp : public FolderPatternBase<MergeConcatOp> {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // case of it.
    if (toolchain::all_of(concat_non_control_operands.drop_back(), [&](Value v) {
    ...
    for (Value v : non_control_operands) {
      ...
      if (!op_device.empty() && machina::DeviceNameUtils::SplitDeviceName(
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    for (auto interval : constant_input_runs) {
      ...
      for (auto i : toolchain::seq<int>(interval.first + 1, interval.second)) {
    ...
    if (!inputs_to_delete.empty()) {
      OperationState state(op->getLoc(), op->getName());
      for (const auto &it : toolchain::enumerate(non_control_operands)) {
