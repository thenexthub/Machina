...
static Type GetReifiedType(Type orig, ShapeAttr shape) {
  ...
  if (shape.hasRank()) {
    ...
    SmallVector<int64_t> dims = toolchain::to_vector(shape.getShape());
...
ArrayAttr ConsolidateAttributesPassImpl::reifyAndDropFunctionArgumentAttributes(
  ...
  for (auto i : toolchain::seq<unsigned>(0, num_args)) {
...
ArrayAttr ConsolidateAttributesPassImpl::reifyAndDropFunctionResultAttributes(
  ...
  for (const auto &it :
       toolchain::enumerate(res_attrs.getAsRange<DictionaryAttr>())) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    rewriter.modifyOpInPlace(op, [&] {
      ...
      for (auto it :
           toolchain::zip(results, output_shapes.getAsRange<ShapeAttr>())) {
...
class DropAttributes : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!toolchain::count_if(attr_ids_, [&](StringAttr attr_id) {
...
void PrepareAttributesForExportPassImpl::prepareFunctionAttributes(
  ...
  for (auto it : toolchain::zip(func.getArgumentTypes(),
  ...
  for (auto it : toolchain::zip(func.getResultTypes(),
