...
static bool CanBeRefined(Operation *op) {
  return toolchain::any_of(op->getResultTypes(),
...
void ShapeInference::TryToCacheResultsTensorValue(Operation *op) {
  ...
  if (toolchain::all_of(op->getResults().drop_back(), [this](Value value) {
    ...
    for (OpOperand &operand : op->getOpOperands()) {
      ...
      if (return_shape.getElementType().isInteger(32)) {
        tensor_value = DenseElementsAttr::get<int>(
            return_shape,
            SmallVector<int>(toolchain::map_range(
...
void ShapeInference::runOnOperation() {
  ...
  auto op_result_as_shape_fn = [this](InferenceContext &ic,
    ...
    for (const auto &element : toolchain::enumerate(attr.getValues<APInt>()))
  ...
  auto infer_and_update_shapes = [&](Operation *op) -> bool {
    ...
    bool updated = false;
    for (auto it : toolchain::zip(op->getResults().drop_back(), results)) {
  ...
  getOperation()->walk([&](GraphFuncOp func) {
    ...
    for (const auto &indexed_type : toolchain::enumerate(func_type.getResults())) {
