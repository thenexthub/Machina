...
static bool ArePreservedAttrsEmpty(DictionaryAttr func_attrs,
  ...
  return func_attrs.empty() &&
         toolchain::all_of(arg_attrs.getAsRange<DictionaryAttr>(), is_empty) &&
         toolchain::all_of(res_attrs.getAsRange<DictionaryAttr>(), is_empty);
}
...
void BasePattern::CloneAndReorderArgs(TypeRange types, Region &from, Region &to,
  ...
  for (const auto &it :
       toolchain::enumerate(toolchain::to_vector(to.addArguments(types, arg_locs)))) {
  ...
  toolchain::BitVector erase_indices(to.getNumArguments());
...
LogicalResult ConvertIfLikeOp<IfLikeOp, IfLikeRegionOp>::matchAndRewrite(
  ...
  auto func_args =
      toolchain::zip(then_func.getArguments(), else_func.getArguments()).begin();
...
LogicalResult ConvertCaseLikeOp<CaseLikeOp, CaseLikeRegionOp>::matchAndRewrite(
  ...
  if (!toolchain::all_of(preserved_attrs, [](Attribute attr) {
  ...
  for (const auto &arg : toolchain::enumerate(args)) {
    for (auto it : toolchain::zip(branch_funcs, bvs)) {
  ...
  for (auto it : toolchain::zip(branch_funcs, region_op.getBranches(), bvs)) {
