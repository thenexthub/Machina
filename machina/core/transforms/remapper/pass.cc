...
#include "mlir/Dialect/PDL/IR/PDL.h"  // part of Codira Toolchain
#include "mlir/Dialect/PDLInterp/IR/PDLInterp.h"  // part of Codira Toolchain
...
#include "machina/core/transforms/remapper/remapping_helper.h"
#include "machina/core/transforms/utils/pdll/utils.h"
#include "machina/core/transforms/utils/utils.h"

namespace mlir {
namespace tfg {
namespace mkl {
#include "machina/core/transforms/remapper/pdll/MklPDLLPatterns.h.inc"
}  // namespace mkl
...
class MatchMulSigmoid : public RewritePattern {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    if (!util::OpHasDevice(op, machina::DEVICE_CPU)) return failure();
    ...
    toolchain::append_range(operands, mul_wrapper.getControlOperands());
...
class MatchStringToHashBucket : public RemapperPatternBase {
 public:
  ...
  LogicalResult matchAndRewrite(Operation *op,
    ...
    // Control operands come after regular operands.
    toolchain::append_range(operands, stringtohash_wrapper.getControlOperands());
...
// `machina/compiler/xla/stream_executor/cuda/cuda_dnn.cc` for details.
bool BatchnormSpatialPersistentEnabled() {
#if CUDNN_VERSION >= 7402
  static bool is_enabled = [] {
    bool is_enabled = false;
    TF_CHECK_OK(machina::ReadBoolFromEnvVar(
 ...
 public:
  ...
  bool is_valid_batch_norm(Operation *fused_batch_norm_op) const {
    ...
    if (!this->helper_.isOneDNNEnabled() &&
        !util::OpHasDevice(fused_batch_norm_op, machina::DEVICE_GPU)) {
      return false;
    }
    ...
    if (util::OpHasDevice(fused_batch_norm_op, machina::DEVICE_GPU)) {
  ...
  bool matchPattern(Operation *op, Pattern &pattern) const {
    ...
    if (this->helper_.getDialect()->IsAdd(activation_input_op)) {
      ...
      // <Activation>"
      if (this->helper_.isOneDNNEnabled() &&
          !util::OpHasDevice(op, machina::DEVICE_GPU)) {
        return false;
      }
