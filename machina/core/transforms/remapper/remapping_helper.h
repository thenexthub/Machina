 ...
 *
 */
#ifndef MACHINA_CORE_TRANSFORMS_REMAPPER_REMAPPING_HELPER_H_
#define MACHINA_CORE_TRANSFORMS_REMAPPER_REMAPPING_HELPER_H_
...
class OpPropertyHelper : public OpCatHelper {
 public:
  ...
  bool IsGpuCompatible(const ContractionBiasAddActivation& pattern) const {
    ...
    if (!util::OpHasDevice(pattern.contraction, machina::DEVICE_GPU))
      return false;
  ...
  bool IsCpuCompatible(Operation* contraction_op) const {
    if (!util::OpHasDevice(contraction_op, machina::DEVICE_CPU))
      return false;
    if (dialect_->IsConv2D(contraction_op) ||
...
#endif  // MACHINA_CORE_TRANSFORMS_REMAPPER_REMAPPING_HELPER_H_
