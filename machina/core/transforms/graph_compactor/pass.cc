...
static void EncodeName(unsigned counter, std::string &output) {
  ...
  EncodeName(counter, output, toolchain::ArrayRef(valid_chars, valid_first_chars),
             toolchain::ArrayRef(valid_chars, valid_trailing_chars));
}
...
class StripDefaultAttrsPass
 ...
 private:
  ...
  const machina::OpRegistry *registry_;
};
}  // namespace
...
LogicalResult StripDefaultAttrsPass::removeDefaultValuedAttrs(Operation *op) {
  ...
  toolchain::BitVector indices_to_remove(attrs.size());
  Builder b(&getContext());
  for (const machina::OpDef::AttrDef &attr : op_reg_data->op_def.attr()) {
  ...
  for (const auto &it : toolchain::enumerate(attrs)) {
...
LogicalResult AddDefaultAttrsPass::addDefaultValuedAttrs(Operation *op) {
  ...
  if (toolchain::all_of(op_reg_data->op_def.attr(),
