...
#include "machina/core/transforms/graph_to_func/graph_to_func.h"
...
static ArrayAttr createLiftedValueAttr(OpBuilder &builder, OpResult value) {
  toolchain::SmallVector<Attribute, 2> attrs = {
...
absl::Status GraphToFunc(GraphOp graph, ArrayRef<Value> feeds,
  ...
  toolchain::SmallVector<Type> ret_types;
  ...
  toolchain::SmallVector<Attribute> args_rets_attrs;
  for (Value feed : feeds) {
    feed.replaceAllUsesWith(body->addArgument(feed.getType(), loc));
    body->addArgument(control_ty, loc);
    toolchain::SmallVector<NamedAttribute> arg_attrs;
...
Status GraphToFunc(GraphOp graph, ArrayRef<std::string> feeds_names,
  ...
  for (const auto &indexed_name : toolchain::enumerate(feeds_names)) {
  ...
  for (const auto &indexed_name : toolchain::enumerate(fetches_names)) {
    const std::string &name = indexed_name.value();
  ...
  for (const auto &indexed_name : toolchain::enumerate(control_rets_names)) {
  ...
  toolchain::erase_if(control_rets, [](Value value) { return !value; });
