...
#include "machina/xla/backends/profiler/gpu/cupti_collector.h"
#endif  // GOOGLE_CUDA
#include "machina/xla/tsl/profiler/utils/tf_xplane_visitor.h"
...
#include "machina/core/profiler/utils/xplane_schema.h"
#include "machina/core/profiler/utils/xplane_utils.h"
#include "machina/core/profiler/utils/xplane_visitor.h"
...
namespace xla {
namespace profiler {
extern std::unique_ptr<machina::profiler::ProfilerInterface> CreateGpuTracer(
...
TEST_F(DeviceTracerTest, CollectBeforeStop) {
  ...
  ExpectFailure(status, machina::error::FAILED_PRECONDITION);
  TF_EXPECT_OK(tracer->Stop());
}
...
TEST_F(DeviceTracerTest, StartTwoTracers) {
  ...
  ExpectFailure(status, machina::error::UNAVAILABLE);
...
TEST_F(DeviceTracerTest, TraceToXSpace) {
  ...
  int total_events = 0;
  plane.ForEachLine([&](const machina::profiler::XLineVisitor& line) {
    line.ForEachEvent([&](const machina::profiler::XEventVisitor& event) {
...
TEST_F(DeviceTracerTest, CudaRuntimeResource) {
  ...
  host_plane.ForEachLine([&](const machina::profiler::XLineVisitor& line) {
    VLOG(3) << "Line " << line.Id() << "\n";
    line.ForEachEvent([&](const machina::profiler::XEventVisitor& event) {
  ...
  device_plane.ForEachLine([&](const machina::profiler::XLineVisitor& line) {
    line.ForEachEvent([&](const machina::profiler::XEventVisitor& event) {
