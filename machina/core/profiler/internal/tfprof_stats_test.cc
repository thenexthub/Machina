...
#include "machina/core/profiler/internal/tfprof_stats.h"
...
class TFProfStatsTest : public ::testing::Test {
 protected:
  TFProfStatsTest() {
    ...
    std::unique_ptr<machina::GraphDef> graph_pb(new machina::GraphDef());
    TF_CHECK_OK(
        ReadProtoFile(Env::Default(), graph_path, graph_pb.get(), false));

    std::unique_ptr<machina::RunMetadata> run_meta_pb(
        new machina::RunMetadata());
