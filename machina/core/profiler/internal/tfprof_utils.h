...
#ifndef MACHINA_CORE_PROFILER_INTERNAL_TFPROF_UTILS_H_
#define MACHINA_CORE_PROFILER_INTERNAL_TFPROF_UTILS_H_
...
absl::Status ReadProtoFile(Env* env, const string& fname, T* proto,
  ...
  if (binary_first) {
    if (ReadBinaryProto(machina::Env::Default(), fname, proto).ok()) {
      return absl::Status();
    ...
    if (protobuf::TextFormat::ParseFromString(out, proto)) {
      return absl::Status();
    } else if (ReadBinaryProto(machina::Env::Default(), fname, proto).ok()) {
      return absl::Status();
    }
  }
...
#endif  // MACHINA_CORE_PROFILER_INTERNAL_TFPROF_UTILS_H_
