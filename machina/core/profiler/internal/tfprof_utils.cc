...
#include "machina/core/platform/regexp.h"
  ...
  *cmd = pieces[0];
  ...
  for (int i = 1; i < pieces.size(); ++i) {
    if (pieces[i] == string(machina::tfprof::kOptions[0])) {
      ...
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[13]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      ...
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[14]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      opts->start_name_regexes =
          absl::StrSplit(StripQuote(pieces[i + 1]), ',', absl::SkipEmpty());
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[15]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      opts->trim_name_regexes =
          absl::StrSplit(StripQuote(pieces[i + 1]), ',', absl::SkipEmpty());
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[16]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      opts->show_name_regexes =
          absl::StrSplit(StripQuote(pieces[i + 1]), ',', absl::SkipEmpty());
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[17]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      opts->hide_name_regexes =
          absl::StrSplit(StripQuote(pieces[i + 1]), ',', absl::SkipEmpty());
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[18]) {
    ...
    } else if (pieces[i] == machina::tfprof::kOptions[19]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
      ...
      ++i;
    } else if (pieces[i] == machina::tfprof::kOptions[20]) {
      if (pieces.size() <= i + 1) {
        return ReturnError(pieces, i);
      }
...
void PrintHelp() {
  absl::PrintF(
      "See https://github.com/machina/machina/tree/master/machina/core/profiler/"
