...
// It is not easy to link xla/client/lib into the machina core lib, so we
...
absl::Status ApproxTopKShape(shape_inference::InferenceContext* c) {
  ...
  int64_t output_dim_value = [&] {
    ...
    return machina::MathUtil::CeilOfRatio<int64_t>(
               machina::MathUtil::CeilOfRatio<int64_t>(reduction_dim_value,
                                                          tpu_tiling),
               (1 << log2_reduction)) *
           tpu_tiling;
  }();
...
// LINT.ThenChange(//machina/compiler/xla/hlo/builder/lib/approx_topk_shape.cc)
...
REGISTER_OP("IsotonicRegression")
    ...
    .SetShapeFn([](::machina::shape_inference::InferenceContext* context) {
