...
absl::Status PrepareCopy(Device* device, const DeviceContext* ctx,
  ...
  auto di = device->machina_accelerator_device_info();
  if (di == nullptr) {
...
void GPUUtil::DeviceToDeviceCopy(
  ...
  if (total_bytes > 0) {
    ...
    // dst->machina_accelerator_device_info()->default_context which is
    // GPUDeviceContext.
    stream_executor::Stream* recv_stream =
        static_cast<const GPUDeviceContext*>(
            dst->machina_accelerator_device_info()->default_context)
            ->stream();
...
void GPUUtil::CopyGPUTensorToCPU(Device* gpu_device,
  ...
  if (pjrt_tensor_buffer != nullptr) {
    ...
    if (pjrt_tensor_buffer->pjrt_buffer()->has_dynamic_dimensions()) {
      ...
      auto status = machina::HostTensorToMutableBorrowingLiteral(
    ...
    if (do_staging) {
      {
        ...
        staging_buffer = host_memory_allocator->AllocateRaw(
            machina::Allocator::kAllocatorAlignment, total_bytes);
...
absl::Status GPUUtil::Sync(Device* gpu_device) {
  VLOG(1) << "GPUUtil::Sync";
  auto* dev_info = gpu_device->machina_accelerator_device_info();
  if (!dev_info) {
