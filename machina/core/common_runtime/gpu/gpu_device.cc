...
#include "machina/core/common_runtime/device_id_utils.h"
#include "machina/core/common_runtime/gpu/gpu_device.h"
#include "machina/core/common_runtime/gpu/gpu_id_manager.h"
#include "machina/core/common_runtime/gpu/gpu_process_state.h"
#include "machina/core/common_runtime/gpu/gpu_util.h"
#include "machina/core/common_runtime/gpu_device_context.h"
...
#include "machina/core/platform/rocm.h"
#endif
...
#include "machina/core/profiler/lib/scoped_annotation.h"
...
#include "machina/core/util/stream_executor_util.h"
...
class EigenGpuStreamDevice : public ::Eigen::StreamInterface {
 public:
  ...
  void Reinitialize(OpKernelContext* context, gpuStream_t gpu_stream,
                    ...
                    ::machina::Allocator* alloc, char* scratch) {
 ...
 private:
  struct AsyncFreeData {
    AsyncFreeData(::machina::Allocator* a, void* p, const string& o,
                  const int64_t s)
        : allocator_(a), address_(p), operation_(o), step_id_(s) {}
    ::machina::Allocator* allocator_;
    void* address_;
  ...
  ::machina::Allocator* allocator_;  // Not owned.
...
Status BaseGPUDevice::Init(const SessionOptions& options) {
  ...
  // TODO(machina-team): set up nccl stream when MACHINA_USE_ROCM is set.
  ...
  set_machina_accelerator_device_info(accelerator_device_info_);
  ...
  if (gpu_thread_mode != "global") {
    ...
    if (gpu_thread_mode == "gpu_private") {
      ...
      thread_pool_.reset(new thread::ThreadPool(
          ...
          /*allocator=*/nullptr));
      set_machina_device_thread_pool(thread_pool_.get());
      ...
      static thread::ThreadPool* thread_pool = new thread::ThreadPool(
          ...
          !options.config.experimental().disable_thread_spinning(),
          /*allocator=*/nullptr);
      set_machina_device_thread_pool(thread_pool);
    } else {
...
Status BaseGPUDeviceFactory::CreateDevices(
  ...
  if (obtained_pjrt_client.ok()) {
    pjrt_se_client = machina::down_cast<xla::StreamExecutorGpuClient*>(
  ...
  for (int di = 0; di < tf_device_specs.size(); ++di) {
    ...
    if (should_create_new_pjrt_client ||
      ...
      auto* pjrt_se_client =
          machina::down_cast<xla::StreamExecutorGpuClient*>(
              *obtained_pjrt_client);
...
static string GetShortDeviceDescription(
  ...
  // LINT.ThenChange(//machina/python/framework/gpu_util.py)
#elif MACHINA_USE_ROCM
...
Status BaseGPUDeviceFactory::GetValidDeviceIds(
  ...
  if (!handle_or.ok()) {
    LOG(WARNING) << "Cannot dlopen some GPU libraries. Please make sure the "
                    ...
                    "https://www.machina.org/install/gpu for how to "
