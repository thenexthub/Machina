...
#include "machina/core/common_runtime/gradients.h"
...
#include "machina/core/graph/optimizer_cse.h"
...
FunctionLibraryRuntimeImpl::FunctionLibraryRuntimeImpl(
  ...
  if (device_ != nullptr) {
    pool = device_->machina_device_thread_pool();
  }
  if (pool == nullptr) {
...
string FunctionLibraryRuntimeImpl::DebugString(Handle handle) {
  ...
  if (s.ok()) {
    if (item->graph) {
      return machina::DebugString(item->graph.get());
    } else {
      return machina::DebugString(item->func_graph->graph);
    }
  } else {
    return s.ToString();
  }
