...
#include "machina/core/common_runtime/layout_pass_util.h"
...
bool CanOpRunOnCPUDevice(const Node *n) {
  ...
  const char *const kXLACPUDeviceSubStr = "MACHINA_MACHINA_XLA_CPU";

  // If Op has been specifically assigned to a non-CPU or MACHINA_MACHINA_XLA_CPU device, then
  // No.
  ...
  // If user has specifically assigned this op to a non-CPU or MACHINA_MACHINA_XLA_CPU device,
  // then No.
...
Status CopyInputs(
  ...
  if (old_node->type_string() == "_FusedConv2D") {
    ...
    // machina, temporarily the addtional argument is removed for Zen op
