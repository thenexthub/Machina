...
#include "machina/core/common_runtime/placer.h"
...
using FDH = ::machina::FunctionDefHelper;
...
REGISTER_KERNEL_BUILDER(Name("TestXlaOp").Device("MACHINA_MACHINA_XLA_CPU").Priority(2),
                        DummyOp);
...
class PlacerTest : public ::testing::Test {
 protected:
  ...
  explicit PlacerTest(int num_devices) {
    // Build a set of num_devices GPU, num_devices CPU devices, and one MACHINA_MACHINA_XLA_CPU
    // device.
    ...
    local_devices_.emplace_back(FakeDevice::MakeDevice(
        "/job:a/replica:0/task:0/device:MACHINA_MACHINA_XLA_CPU:0", "MACHINA_MACHINA_XLA_CPU"));
    devices_.AddDevice(local_devices_.back().get());
    local_devices_.emplace_back(FakeDevice::MakeDevice(
...
// Test that if the node supports MACHINA_MACHINA_XLA_CPU and FakeCPU, it will be placed on
// MACHINA_MACHINA_XLA_CPU if and only if the node is assigned to the MACHINA_MACHINA_XLA_CPU device.
TEST_F(PlacerTest, TestXlaOpPlacement) {
  ...
  GetNodeByName(g, "n2")->set_assigned_device_name(
      "/job:a/replica:0/task:0/device:MACHINA_MACHINA_XLA_CPU:0");

  TF_EXPECT_OK(Place(&g));
  EXPECT_DEVICE_TYPE(g, "in", "FakeCPU");
  // n1 should be placed on FakeCPU even if the op supports MACHINA_MACHINA_XLA_CPU with higher
  // priority than FakeCPU.
  EXPECT_DEVICE_TYPE(g, "n1", "FakeCPU");
  // n2 should be placed on MACHINA_MACHINA_XLA_CPU because it supports MACHINA_MACHINA_XLA_CPU and it is
  // assigned to a MACHINA_MACHINA_XLA_CPU device.
  EXPECT_DEVICE_TYPE(g, "n2", "MACHINA_MACHINA_XLA_CPU");
}
