 ...
 *
 */
#ifndef MACHINA_CORE_COMMON_RUNTIME_EAGER_TENSOR_HANDLE_H_
#define MACHINA_CORE_COMMON_RUNTIME_EAGER_TENSOR_HANDLE_H_
...
class TensorHandle : public ImmediateExecutionTensorHandle {
  ...
  TensorHandle(machina::Tensor&& t, Device* d, Device* op_device,
               Device* resource_device, EagerContext* ctx);
  // TensorHandle for dtype == DT_RESOURCE
  TensorHandle(machina::Tensor&& t, Device* d, Device* op_device,
               EagerContext* ctx);
  TensorHandle(Device* d, Device* op_device, Device* resource_device,
               machina::DataType dtype, EagerContext* ctx);

#if !defined(IS_MOBILE_PLATFORM)
  TensorHandle(int64_t op_id, int32_t output_num, const string& remote_task,
               machina::DataType dtype, Device* device, EagerContext* ctx,
               bool unknown_device);
  TensorHandle(int64_t op_id, int32_t output_num, machina::DataType dtype,
               Device* device, bool is_ready, EagerContext* ctx);
#endif  // IS_MOBILE_PLATFORM

 public:
  // TensorHandle with no assigned device
  static TensorHandle* CreateLocalHandle(const machina::Tensor& t);
  static TensorHandle* CreateLocalHandle(machina::Tensor&& t, Device* d,
  ...
  absl::Status TensorFromDevice(const Device* d,
                                const machina::Tensor** t) const;

  // Return the TensorValue from the specified device which could be either the
  // default device or a local mirror. The device pointer should be nullptr if
  // requesting the HostCPU.
  absl::Status TensorValue(const Device* d, machina::TensorValue* t);
  ...
  absl::Status Shape(machina::TensorShape* shape);
  ...
  absl::Status AddLocalMirror(machina::Tensor&& tensor, const Device* d);
  ...
  absl::Status SetTensor(machina::Tensor&& tensor, const Device* d);
  ...
  absl::Status CopyToDevice(const EagerContext& ctx, machina::Device* d,
  ...
  machina::FullTypeDef FullType() const override { return full_type_; }
 ...
 private:
  ...
  machina::Device* resource_device_;
  ...
  std::unordered_map<const machina::Device*, LocalTensorHandleData>
...
#endif  // MACHINA_CORE_COMMON_RUNTIME_EAGER_TENSOR_HANDLE_H_
