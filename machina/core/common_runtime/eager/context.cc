...
#include "machina/core/common_runtime/stats_publisher_interface.h"
#include "machina/core/common_runtime/eager/rendezvous_cache.h"
...
#include "machina/core/common_runtime/collective_executor_mgr.h"
#include "machina/core/common_runtime/collective_param_resolver_local.h"
...
#include "machina/core/distributed_runtime/cluster_function_library_runtime.h"
#include "machina/core/distributed_runtime/collective_param_resolver_distributed.h"
#include "machina/core/distributed_runtime/device_resolver_distributed.h"
...
bool ReadBoolFromEnvVar(absl::string_view env_var_name, bool default_val) {
  bool val;
  if (machina::ReadBoolFromEnvVar(env_var_name, default_val, &val).ok()) {
    return val;
  }
  return default_val;
}
...
auto* eager_context_created =
    monitoring::Gauge<bool, 0>::New("/machina/core/eager_context_created",
...
EagerContext::EagerContext(
  ...
  // provided). For builds using "machina/compiler/xla/tsl/platform/default",
...
void EagerContext::ResetPFLR(const DeviceMgr* device_mgr, Env* env,
  ...
  const machina::SessionMetadata* session_metadata = nullptr;
...
void EagerContext::SetExecutorForThread(EagerExecutor* executor) {
  machina::mutex_lock l(executor_map_mu_);
...
absl::Status EagerContext::FindFunctionOpData(
    const string& name, const machina::OpRegistrationData** op_data) {
...
void EagerContext::ListDevices(
    std::vector<machina::DeviceAttributes>* device_attributes) {
  ...
  if (!remote_devices.empty()) {
    if (!remote_device_mgr()) {
      remote_device_manager_.Reset(
          std::make_unique<machina::DynamicDeviceMgr>());
    }
    TF_RETURN_IF_ERROR(
...
machina::ImmediateExecutionContext::CacheStats
...
absl::Status EagerContext::SetRemoteDeviceFilters(
  ...
  for (auto& filter : device_filters) {
    ...
    if (DeviceNameUtils::ParseFullName(filter, &parsed_filter)) {
      ...
      return machina::errors::InvalidArgument("Invalid filter: ", filter);
    }
  }
