...
#ifndef MACHINA_CORE_COMMON_RUNTIME_EAGER_ATTR_BUILDER_H_
#define MACHINA_CORE_COMMON_RUNTIME_EAGER_ATTR_BUILDER_H_
...
// a.NumInputs(2);
// a.Set("T", TF_FLOAT);
// machina::Fprint128 cache_key = a.CacheKey("cpu:0");
...
class AttrBuilder : public AbstractOpAttrs {
 public:
  ...
  machina::Fprint128 CacheKey(absl::string_view device);
  ...
  void GetNameAttrList(machina::NameAttrList* name_and_attrs) const override;
  ...
  bool GetType(absl::string_view attr_name,
               machina::DataType* result) const override;
  absl::Status GetTypeList(
      absl::string_view attr_name,
      absl::InlinedVector<DataType, 4>* type_list) const override;

 private:
  machina::Fprint128 BuildCacheKeyForDevice(absl::string_view device) const;
  ...
  std::optional<machina::Fprint128> cached_cache_key_;
...
absl::Status AttrBuilder::Get(absl::string_view attr_name,
                              machina::DataType* value) const;
}  // namespace machina

#endif  // MACHINA_CORE_COMMON_RUNTIME_EAGER_ATTR_BUILDER_H_
