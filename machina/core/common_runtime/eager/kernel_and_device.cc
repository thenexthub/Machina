...
absl::Status KernelAndDeviceOp::Init(
  ...
  for (size_t i = 0; i < input_alloc_attrs_.size(); ++i) {
    bool host = kernel_->input_memory_types()[i] == machina::HOST_MEMORY;
  ...
  if (config_it != ndef.attr().end()) {
    if (!options.config_proto.ParseFromString(config_it->second.s())) {
      return errors::InvalidArgument(
          "Failed to parse config_proto attribute as machina::ConfigProto "
          "proto.");
    }
...
machina::Device* KernelAndDeviceOp::OutputDevice(int idx) const {
...
machina::Device* KernelAndDeviceFunc::OutputDevice(int idx) const {
...
machina::Device* KernelAndDeviceOp::OutputResourceDevice(int idx) const {
...
machina::Device* KernelAndDeviceFunc::OutputResourceDevice(int idx) const {
  if (output_dtypes_[idx] == DT_RESOURCE) {
    return output_devices_[idx];
  }
  return nullptr;
