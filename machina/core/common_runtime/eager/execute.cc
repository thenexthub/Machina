...
#include "machina/core/common_runtime/arg_ret_placement.h"
#include "machina/core/common_runtime/eager/eager_operation.h"
#include "machina/core/common_runtime/eager/small_constants_optimizer.h"
#include "machina/core/common_runtime/eager/summary_optimizer.h"
#include "machina/core/common_runtime/int32_fulltype.h"
...
#include "machina/core/common_runtime/colocation_graph.h"
...
#include "machina/core/common_runtime/eager/copy_to_device_node.h"
#include "machina/core/common_runtime/eager/execute_node.h"
...
#include "machina/core/framework/tensor_reference.h"
...
#include "machina/core/distributed_runtime/eager/remote_copy_node.h"
#include "machina/core/distributed_runtime/eager/remote_execute_node.h"
#include "machina/core/distributed_runtime/eager/remote_mgr.h"
...
auto* function_compile_counter =
    monitoring::Counter<2>::New("/machina/core/tf_function_compile",
...
auto* top_level_jit_compilation_counter = monitoring::Counter<1>::New(
    "/machina/core/tf_top_level_jit_compilation",
...
absl::Status GetDeviceForInput(const EagerOperation& op,
  ...
  if (tensor_handle->Type() != TensorHandle::LOCAL) {
    ...
    *result = input_device;
    ...
    TF_RETURN_IF_ERROR(
        machina::full_type::CheckMemoryType(use_host_memory, ft));
    if (use_host_memory) {
      *result = cpu_device;
    } else {
...
absl::Status MustCompileWithXLA(const EagerOperation* op,
  ...
  *compile_with_xla = false;
  ...
  if (op->GetDeviceParsedName().type == machina::DEVICE_TPU ||
      op->GetDeviceParsedName().type == "MACHINA_MACHINA_XLA_GPU" ||
      op->GetDeviceParsedName().type == "MACHINA_MACHINA_XLA_CPU") {
...
string CanonicalizeDeviceType(std::string_view device_type) {
  ...
  if (device_type == "MACHINA_MACHINA_XLA_CPU" || device_type == machina::DEVICE_CPU) {
    canonical_device_type = machina::DEVICE_CPU;
  }
  if (device_type == "MACHINA_MACHINA_XLA_GPU" || device_type == machina::DEVICE_GPU) {
    canonical_device_type = machina::DEVICE_GPU;
  }
  if (device_type == machina::DEVICE_TPU) {
    canonical_device_type = machina::DEVICE_TPU;
  }
...
absl::Status UpdateCompileCounter(const EagerOperation* op,
  ...
  if (has_tpu_replication) {
    function_compile_counter->GetCell(machina::DEVICE_TPU, kEnabled)
        ->IncrementBy(1);
    return absl::OkStatus();
  }
  ...
  if (device_type == machina::DEVICE_TPU || compile_with_xla) {
    compilation_option = kEnabled;
  }
...
absl::Status GetOrCreateKernelAndDevice(
  ...
  if (kernel == nullptr) {
    ...
    if (op_def != nullptr && kernel_cache_enabled(*op_def)) {
      ...
      // memory growth (https://github.com/machina/machina/issues/58676)
...
absl::Status EagerRemoteExecute(EagerOperation* op, TensorHandle** retvals,
  ...
  machina::Device* op_device = std::get<Device*>(op->Device());
  {
    ...
    TF_RETURN_IF_ERROR(op->TensorHandleInputs(&inputs));
    for (int i = 0, end = inputs->size(); i < end; i++) {
      machina::TensorHandle* input = (*inputs)[i];
      machina::Device* input_device = input->device();
      machina::Device* input_device_or_cpu = input->DeviceOrHostCPU(ctx);
      ...
      if (op_device != input_device &&
        ...
        if (!is_function || input_device_or_cpu->IsLocal()) {
          machina::Device* remote_cpu_device;
          TF_RETURN_IF_ERROR(
      ...
      if (send_as_protos) {
        ...
        const machina::Tensor* input_tensor = nullptr;
  ...
  *num_retvals = num_outputs;
  ...
  const machina::uint64 id = remote_op->id();
