...
#include "machina/core/distributed_runtime/eager/remote_tensor_handle_data.h"
...
TensorHandle* TensorHandle::CreateLocalHandle(const machina::Tensor& t) {
  // TODO(b/136608821): Move away from nullptr
  machina::Tensor tensor = t;
...
TensorHandle* TensorHandle::CreateLocalHandle(machina::Tensor&& t, Device* d,
                                              Device* op_device,
                                              EagerContext* ctx) {
...
TensorHandle::TensorHandle(machina::Tensor&& t, Device* d, Device* op_device,
...
TensorHandle::TensorHandle(Device* d, Device* op_device,
                           Device* resource_device, machina::DataType dtype,
                           EagerContext* ctx)
    : ImmediateExecutionTensorHandle(kEager),
      dtype(dtype),
...
absl::Status TensorHandle::CreatePackedHandle(
    std::vector<TensorHandle*>&& handles, const machina::DataType dtype,
    const machina::TensorShape& shape, const string& device_name,
...
absl::Status TensorHandle::CreatePackedHandle(
  ...
  machina::DataType dtype = handles.at(0)->dtype;
...
TensorHandle* TensorHandle::CreateUnshapedRemoteHandle(
    ...
    machina::DataType dtype, Device* d, EagerContext* ctx,
...
TensorHandle::TensorHandle(int64_t op_id, int32_t output_num,
                           ...
                           machina::DataType dtype, Device* d,
...
TensorHandle* TensorHandle::CreateLazyRemoteHandle(
    int64_t op_id, int32_t output_num, machina::DataType dtype, Device* d,
...
machina::DataType TensorHandle::DataType() const { return dtype; }
...
absl::Status TensorHandle::Tensor(const machina::Tensor** t) const {
...
absl::Status TensorHandle::TensorFromDevice(
    const Device* d, const machina::Tensor** t) const {
...
absl::Status TensorHandle::Shape(machina::TensorShape* shape) {
...
absl::Status TensorHandle::Shape(machina::PartialTensorShape* shape) const {
...
absl::Status TensorHandle::RemoteAddress(const Device* d,
  ...
  const machina::RemoteTensorHandleData* remote_data = nullptr;
...
absl::Status TensorHandle::AddLocalMirror(machina::Tensor&& tensor,
                                          const Device* d) {
  if (d == device_) {
    return errors::Internal(
...
absl::Status TensorHandle::SetTensor(machina::Tensor&& t, const Device* d) {
...
absl::Status TensorHandle::CopyToDevice(const EagerContext& ctx,
                                        machina::Device* d,
                                        machina::Tensor* output) const {
  machina::Device* dstd = (d == nullptr) ? ctx.HostCPU() : d;
  machina::Device* srcd = DeviceOrHostCPU(ctx);
  const bool dst_cpu = dstd->machina_accelerator_device_info() == nullptr;
  const bool src_cpu = srcd->machina_accelerator_device_info() == nullptr;
  bool is_same_device =
      (srcd == dstd) || (srcd->name() == dstd->name()) || (dst_cpu && src_cpu);

  const machina::Tensor* src = nullptr;
  TF_RETURN_IF_ERROR(Tensor(&src));
  if (is_same_device) {
    *output = *src;
    return absl::OkStatus();
  }
  if (!dst_cpu && (src->dtype() != machina::DT_VARIANT &&
                   !machina::DataTypeCanUseMemcpy(src->dtype()))) {
    return machina::errors::InvalidArgument(
        "Can't copy Tensor with type ",
        machina::DataTypeString(src->dtype()), " to device ", dstd->name(),
        ".");
  }
  machina::AllocatorAttributes attr;
  if (src->dtype() == machina::DT_VARIANT) {
    attr.set_on_host(true);
  }
  const auto* dstd_info = dstd->machina_accelerator_device_info();
  machina::Tensor dst(dstd->GetAllocator(attr), src->dtype(), src->shape());
  if (src->shape().num_elements() == 0) {
    *output = dst;
    return absl::OkStatus();
  }
  machina::DeviceContext* src_device_context = nullptr;
  if (!src_cpu) {
    src_device_context =
        srcd->machina_accelerator_device_info()->default_context;
  }
  machina::DeviceContext* dst_device_context = nullptr;
  if (!dst_cpu) {
  ...
  machina::Notification n;
  absl::Status status;
  machina::CopyTensor::ViaDMA("copy", src_device_context, dst_device_context,
                                 srcd, dstd, machina::AllocatorAttributes(),
                                 machina::AllocatorAttributes(), src, &dst,
