...
machina::gtl::FlatMap<string, const AttrTypeMap*>* OpNameToAttrTypeMap() {
  static auto* const m =
      new machina::gtl::FlatMap<string, const AttrTypeMap*>;
  return m;
}
...
DEFINE_GET_ATTR(machina::DataType, type, "type");
...
void CombineUnordered(const machina::Fprint128& a,
                      machina::Fprint128* b) {
  b->low64 += a.low64;
  b->high64 += a.high64;
}

inline machina::Fprint128 CacheKeyHelper(absl::string_view s,
                                            const machina::Fprint128& b) {
  machina::Fprint128 a = machina::Fingerprint128(s);
  return FingerprintCat128(a, b);
}

inline machina::Fprint128 CacheKeyHelper(absl::string_view s, uint64 b) {
  return CacheKeyHelper(s, {b, b});
}

}  // namespace

machina::Fprint128 AttrBuilder::CacheKey(const absl::string_view device) {
...
machina::Fprint128 AttrBuilder::BuildCacheKeyForDevice(
    const absl::string_view device) const {
  machina::Fprint128 f = machina::Fingerprint128(op_name());
  f = tsl::FingerprintCat128(f, machina::Fingerprint128(device));
  for (const auto& p : encoded_attrs_) {
    CombineUnordered(
        CacheKeyHelper(p.first, machina::Fingerprint128(p.second)), &f);
  }
  return f;
}

void AttrBuilder::GetNameAttrList(
    machina::NameAttrList* name_and_attrs) const {
