...
ImmediateExecutionTensorHandle* EagerContext::CopyTensorHandleToDevice(
  ...
  *status =
      EagerCopyToDevice(input, this, &this->Executor(), device, false,
                        reinterpret_cast<machina::TensorHandle**>(&result));
  if (status->ok()) {
    return result;
  }
  return nullptr;
...
ImmediateExecutionTensorHandle* EagerContext::CreateLocalHandleFromTFTensor(
    machina::Tensor& t, const char* d_name) {
...
absl::Status EagerOperation::Execute(absl::Span<AbstractTensorHandle*> retvals,
  ...
  // physical devices.
  machina::TensorHandle** retval_array =
      reinterpret_cast<machina::TensorHandle**>(retvals.data());
