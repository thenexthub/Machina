...
class EagerOpRewriteTest : public ::testing::Test {
 public:
  ...
  std::unique_ptr<machina::EagerOperation> CreateOp(const string op_name) {
    std::unique_ptr<DeviceMgr> device_mgr =
        std::make_unique<StaticDeviceMgr>(DeviceFactory::NewDevice(
            "CPU", {}, "/job:localhost/replica:0/task:0/device:CPU:0"));
    bool async = false;
    auto rendezvous =
        tsl::core::RefCountPtr<machina::IntraProcessRendezvous>(
            new machina::IntraProcessRendezvous(device_mgr.get()));
    eager_ctx_ = new machina::EagerContext(
        SessionOptions(),
    ...
    std::unique_ptr<machina::EagerOperation> op(
        new machina::EagerOperation(eager_ctx_));
    EXPECT_EQ(OkStatus(),
 ...
 protected:
  machina::EagerContext* eager_ctx_;
};
