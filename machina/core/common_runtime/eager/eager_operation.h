 ...
 *
 */
#ifndef MACHINA_CORE_COMMON_RUNTIME_EAGER_EAGER_OPERATION_H_
#define MACHINA_CORE_COMMON_RUNTIME_EAGER_EAGER_OPERATION_H_
...
class EagerOperation : public ImmediateExecutionOperation {
 public:
  explicit EagerOperation(machina::EagerContext* ctx)
  ...
  const machina::OpDef* OpDef() const override { return op_def_; };
  ...
  machina::EagerContext& EagerContext() const { return ctx_; }
 ...
 private:
  ...
  const machina::OpDef* GetOpDef(absl::Status* status);
  ...
  const machina::OpDef* op_def_;  // op definition from protobuf
...
#endif  // MACHINA_CORE_COMMON_RUNTIME_EAGER_EAGER_OPERATION_H_
