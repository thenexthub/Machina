...
absl::Status CustomDeviceOpHandler::Execute(
    ...
    int* num_retvals) {
  machina::CustomDevice* custom_device = nullptr;
  ...
  if (op->HasCustomDeviceInput()) {
    ...
    for (int i = 0; i < inputs.size(); ++i) {
      ...
      // TODO(b/175427838): It would be nice to be able to use machina::isa
      // here.
      if (machina::CustomDeviceTensorHandle::classof(inputs[i])) {
        machina::CustomDeviceTensorHandle* previous =
            machina::down_cast<machina::CustomDeviceTensorHandle*>(
                inputs[i]);
        machina::ImmediateExecutionTensorHandle* new_tensor;
  ...
  return op->Execute(
      absl::MakeSpan(
          reinterpret_cast<machina::AbstractTensorHandle**>(retvals),
          *num_retvals),
      num_retvals);
}
