 ...
 *
 */
#ifndef MACHINA_CORE_COMMON_RUNTIME_EAGER_CONTEXT_H_
#define MACHINA_CORE_COMMON_RUNTIME_EAGER_CONTEXT_H_
...
// "machina/core/platform/platform.h" must be included first before using
// IS_MOBILE_PLATFORM.
#if !defined(IS_MOBILE_PLATFORM)
#include "machina/core/distributed_runtime/eager/eager_client.h"
#include "machina/core/distributed_runtime/rendezvous_mgr_interface.h"
...
class EagerContext : public ImmediateExecutionContext, public core::RefCounted {
 public:
  ...
  AbstractTensorInterface* CreateStringScalar(
      machina::tstring value) override;
  AbstractTensorInterface* CreateComplex128Scalar(
      machina::complex128 value) override;
  ...
  // Create an abstract tensor handle from machina::Tensor.
  ImmediateExecutionTensorHandle* CreateLocalHandleFromTFTensor(
      machina::Tensor& t, const char* d_name) override;
  ...
  // machina::TensorHandle. In current runtime EagerContext, just forward
  // the input since the input tensor handle is already a
  // machina::TensorHandle.
  ...
  machina::ImmediateExecutionContext::CacheStats GetCacheStats() override;
  ...
  Rendezvous::Factory RendezvousFactory(
    ...
    // Ref: machina/c/eager/c_api.cc;l=143;rcl=396387348
  ...
  machina::DeviceMgr* local_device_mgr() const {
    return local_device_manager_.Get();
  }
  const machina::DynamicDeviceMgr* remote_device_mgr() const {
    return remote_device_manager_.Get();
  }

  machina::DynamicDeviceMgr* GetOwnedRemoteDeviceMgr() {
  ...
  //
  // - server: A ServerInterface that exports the machina.WorkerService.
  ...
  machina::ServerInterface* GetServer() { return server_.get(); }
  ...
  machina::Env* TFEnv() const { return env_; }
 ...
 private:
  ...
  void UpdateGlobalRendezvousDeviceManager(machina::DeviceMgr* device_mgr);
...
#endif  // MACHINA_CORE_COMMON_RUNTIME_EAGER_CONTEXT_H_
