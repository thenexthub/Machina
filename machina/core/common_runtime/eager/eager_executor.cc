...
EagerExecutor::EagerExecutor(bool async, bool enable_streaming_enqueue,
    ...
    : next_node_id_(0),
      ok_(true),
      thread_(async ? machina::Env::Default()->StartThread(
                          machina::ThreadOptions(), "eager_async_executor",
...
EagerExecutor::~EagerExecutor() {
  machina::mutex_lock l(node_queue_mutex_);
...
absl::Status EagerExecutor::WaitForAllPendingNodesLocked(mutex_lock* lock) {
  machina::condition_variable cond;
