...
#include "machina/cc/ops/array_ops_internal.h"
...
#include "machina/core/platform/null_file_system.h"
...
class TestReadOnlyMemoryRegion : public ::machina::ReadOnlyMemoryRegion {
 public:
...
class TestTFFileSystem : public ::machina::NullFileSystem {
 public:
  TestTFFileSystem()
      : ::machina::NullFileSystem(),
        data_tensor_(test::AsTensor<double>({1., 2., 3., 4.}, {2, 2})) {}

  using ::machina::NullFileSystem::NewReadOnlyMemoryRegionFromFile;

  absl::Status NewReadOnlyMemoryRegionFromFile(
      const string& fname, ::machina::TransactionToken* token,
      std::unique_ptr<::machina::ReadOnlyMemoryRegion>* result) override {
    ...
    *result = std::unique_ptr<::machina::ReadOnlyMemoryRegion>(
        new TestReadOnlyMemoryRegion(sp.data(), sp.size()));
    return absl::OkStatus();
  }

 protected:
  ::machina::Tensor data_tensor_;
};

// A test TF environment that checks that the environment was used.
class TestTFEnvironment : public ::machina::EnvWrapper {
 public:
  using tf_base = ::machina::EnvWrapper;
  TestTFEnvironment() : ::machina::EnvWrapper(Default()) {}
  absl::Status GetFileSystemForFile(
      const string& fname, ::machina::FileSystem** result) override {
    was_used_ = true;
    if (fname == "test://test") {
