...
#include "machina/core/common_runtime/optimize_function_graph_utils.h"
#include "machina/core/common_runtime/partitioning_utils.h"
#include "machina/core/common_runtime/placer.h"
#include "machina/core/common_runtime/rendezvous_util.h"
#include "machina/core/common_runtime/replicate_per_replica_nodes.h"
#include "machina/core/common_runtime/single_threaded_executor.h"
...
#include "machina/core/util/reffed_status_callback.h"
...
absl::Status ProcessFunctionLibraryRuntime::GetDeviceContext(
  ...
  *device_context = nullptr;
  ...
  if (device->IsRemoteCallAllowed()) {
    auto* dev_info = flr->device()->machina_accelerator_device_info();
    if (dev_info) {
...
absl::Status ProcessFunctionLibraryRuntime::InstantiateMultiDevice(
  ...
  auto instantiate_component = [this, dev_set, &data_lib_def, &control_ret,
    ...
    bool ints_on_device =
        (device_type == "TPU" || device_type == "MACHINA_MACHINA_XLA_CPU" ||
         device_type == "MACHINA_MACHINA_XLA_GPU" || options.int_args_and_retvals_on_device);
...
absl::Status ProcessFunctionLibraryRuntime::RunMultiDeviceSync(
  ...
  for (const string& target : subgraph_keys) {
    ...
    if (flr != nullptr) {
      ...
      // runner
      thread::ThreadPool* pool = flr->device()->machina_device_thread_pool();
