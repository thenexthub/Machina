...
REGISTER_KERNEL_BUILDER(Name("Test1").Device(machina::DEVICE_CPU),
                        DummyKernel);
...
class TestOp2 : public ::machina::OpKernel {
 public:
  explicit TestOp2(::machina::OpKernelConstruction* context)
      : OpKernel(context) {
    absl::Status status = context->MatchSignature({::machina::DT_INT32},
                                                  {::machina::DT_INT32});
...
class TestOp3Cpu : public machina::OpKernel {
 public:
...
class TestOp3Gpu : public machina::OpKernel {
 public:
...
class TestOp5Cpu : public machina::OpKernel {
 public:
...
class TestOp5Gpu : public machina::OpKernel {
 public:
...
class StrippedNode : public machina::OpKernel {
 public:
...
class RefInputs : public machina::OpKernel {
 public:
...
TEST_F(OpKernelTest, AllocateOutput) {
  ...
  EXPECT_THAT(s, machina::testing::StatusIs(error::INTERNAL));
...
REGISTER_KERNEL_BUILDER(Name("ListOut").Device(machina::DEVICE_CPU),
                        DummyKernel);
...
class GetAttrKernel : public ::machina::OpKernel {
 public:
...
TEST(RegisteredKernels, CanLogAllRegisteredKernels) {
  machina::LogAllRegisteredKernels();
}
