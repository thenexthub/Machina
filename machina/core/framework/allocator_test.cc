...
#include "machina/core/profiler/protobuf/memory_profile.pb.h"
...
TEST(CPUAllocatorTest, ProfilerReporting) {
  ...
  const auto plane = ::tsl::profiler::FindPlaneWithName(
      xspace, ::machina::profiler::kHostThreadsPlaneName);
  ::machina::profiler::XPlaneVisitor xplane(plane);

  ASSERT_EQ(plane->name(), ::machina::profiler::kHostThreadsPlaneName)
  ...
  ::machina::profiler::XEventVisitor e0(&xplane, &line, &events[0]);
      ...
      << "XSpace: " << xspace.DebugString();
  {
    ...
    e0.ForEachStat([&](const ::machina::profiler::XStatVisitor& stat) {
  ...
  ::machina::profiler::XEventVisitor e1(&xplane, &line, &events[1]);
  EXPECT_EQ(e1.Name(), "MemoryDeallocation")
      << "XSpace: " << xspace.DebugString();
  {
    absl::optional<std::string> bytes_allocated, peak_bytes_in_use,
        allocation_bytes;
    e1.ForEachStat([&](const ::machina::profiler::XStatVisitor& stat) {
      if (stat.Name() == "bytes_allocated") {
        bytes_allocated = stat.ToString();
      } else if (stat.Name() == "peak_bytes_in_use") {
        peak_bytes_in_use = stat.ToString();
