...
class CreateTestVariant {
 public:
  explicit CreateTestVariant(const ::machina::Scope& scope,
                             const Input& value) {
    if (!scope.ok()) return;
    ...
    auto builder = ::machina::NodeBuilder(unique_name, "CreateTestVariant")
                       .Input(_value);
  ...
  operator ::machina::Output() const { return output_; }
  // Intentionally not marked as explicit.
  // NOLINTNEXTLINE google-explicit-constructor
  operator ::machina::Input() const { return output_; }

  ::machina::Node* node() const { return output_.node(); }

  ::machina::Output output_;
};
