...
class TensorEqMatcherImpl : public ::testing::MatcherInterface<const Tensor&> {
 public:
  ...
  void DescribeTo(::std::ostream* os) const override {
    *os << "data type is " << machina::DataTypeString(target_.dtype())
        << ", and shape is " << target_.shape();
    switch (target_.dtype()) {
#define CASE_TYPE(T)                                       \
  case machina::DataTypeToEnum<T>::value: {             \
    *os << ", and tensor data ";                           \
...
#undef CASE_TYPE
      default: {
        DLOG(FATAL) << "TensorEq matcher unsupported dtype: "
                    << machina::DataTypeString(target_.dtype());
      }
    }
  }

  void DescribeNegationTo(::std::ostream* os) const override {
    *os << "data type is not " << machina::DataTypeString(target_.dtype())
...
#undef CASE_TYPE
  ...
  bool MatchAndExplain(
    ...
    *listener << "whose data type " << machina::DataTypeString(value.dtype())
