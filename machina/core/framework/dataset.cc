...
#include "machina/core/platform/resource.h"
...
class DatasetVariantWrapper {
 public:
  ...
  string TypeName() const { return "machina::DatasetVariantWrapper"; }
...
const char kWrappedDatasetVariantTypeName[] =
    "machina::data::WrappedDatasetVariant";
...
class WrappedDatasetVariantWrapper {
 public:
  ...
  string TypeName() const { return "machina::WrappedDatasetVariantWrapper"; }

  string DebugString() const {
    return "machina::WrappedDatasetVariantWrapper::DebugString";
  }
...
int32_t GetRunnerThreadpoolSizeFromOpKernelContext(OpKernelContext* ctx) {
  thread::ThreadPool* thread_pool =
      ctx->device()->machina_device_thread_pool();
  if (thread_pool) {
...
void BackgroundWorker::WorkerLoop() {
  machina::ResourceTagger tag(kTFDataResourceTag, "Background");
  while (true) {
...
class RunnerImpl : public Runner {
 public:
  void Run(const std::function<void()>& f) override {
    machina::ResourceTagger tag(kTFDataResourceTag, "Runner");
    f();
