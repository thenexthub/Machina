...
auto* counter_with_labels =
    Counter<1>::New("/machina/test/counter_with_labels",
...
auto* init_counter_without_labels = Counter<0>::New(
    "/machina/test/init_counter_without_labels",
...
auto* counter_without_labels = Counter<0>::New(
    "/machina/test/counter_without_labels", "Counter without any labels.");
...
auto* dead_counter_without_labels = Counter<0>::New(
    "/machina/test/dead_counter_without_labels",
...
TEST(LabeledCounterTest, SameName) {
  auto* same_counter = Counter<1>::New("/machina/test/counter_with_labels",
                                       "Counter with one label.", "MyLabel");
