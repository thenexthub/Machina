...
#include "machina/xla/tsl/lib/io/snappy/snappy_inputbuffer.h"
#include "machina/xla/tsl/lib/io/snappy/snappy_outputbuffer.h"
...
#include "machina/core/lib/io/zlib_inputstream.h"
#include "machina/core/lib/io/zlib_outputbuffer.h"
...
absl::Status TFRecordWriter::Initialize(machina::Env* env) {
...
absl::Status CustomWriter::Initialize(machina::Env* env) {
  TF_RETURN_IF_ERROR(env->NewAppendableFile(filename_, &dest_));
...
void AsyncWriter::Consume(ElementOrEOF* be) {
  mutex_lock l(mu_);
  mu_.Await(machina::Condition(this, &AsyncWriter::ElementAvailable));
  *be = deque_.front();
  deque_.pop_front();
}
