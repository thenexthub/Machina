...
#include "machina/core/data/compression_utils.h"
...
constexpr char kIteratorVariantTypeName[] = "machina::Iterator";
...
VariantTensorDataReader::VariantTensorDataReader(
    const std::vector<const machina::VariantTensorData*>& data) {
...
absl::Status VariantTensorDataWriter::WriteTensorInternal(absl::string_view n,
  ...
  if (data_.count(name) == 0) {
    ...
    data_[name]->set_type_name("machina::Iterator");
  }
