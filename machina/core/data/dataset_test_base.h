...
#ifndef MACHINA_CORE_DATA_DATASET_TEST_BASE_H_
#define MACHINA_CORE_DATA_DATASET_TEST_BASE_H_
...
typedef std::vector<
    std::pair<string, machina::FunctionDefHelper::AttrValueWrapper>>
    AttributeVector;
 ...
 public:
  ...
  class TestVariant {
   public:
    ...
    constexpr static const char kTypeName[] = "machina::data::TestVariant";

    string TypeName() const { return kTypeName; }

    // Encodes the contents of this object into `data`.  This function signature
    // is required for objects to be stored in `machina::Variant`s.  See the
    // docs for `machina::Variant` for more information and see
    // `machina::Variant::Encode` for how this is used.
    ...
    // signature is required for objects to be stored in `machina::Variant`s.
    // See the docs for `machina::Variant` for more information and see
    // `machina::Variant::Decode` for how this is used.
...
#endif  // MACHINA_CORE_DATA_DATASET_TEST_BASE_H_
