...
TEST_F(IfrtServingExecutableTest, Basic) {
  ...
  auto x = AsTensor<int32_t>({1, 2, 3}, machina::TensorShape({1, 3}));
  auto y = AsTensor<int32_t>({1, 2, 3}, machina::TensorShape({3, 1}));
  std::vector<machina::Tensor> inputs{x, y};
  ...
  const auto expected_out =
      AsTensor<int32_t>({14}, machina::TensorShape({1, 1}));
...
TEST_F(IfrtServingExecutableTest, MultipleShapes) {
  ...
  auto x1 = AsTensor<int32_t>({1, 2, 3}, machina::TensorShape({1, 3}));
  auto y1 = AsTensor<int32_t>({1, 2, 3}, machina::TensorShape({3, 1}));
  const auto expected_out1 =
      AsTensor<int32_t>({14}, machina::TensorShape({1, 1}));
  std::vector<machina::Tensor> inputs1{x1, y1};

  auto x2 = AsTensor<int32_t>({1, 2, 3, 4}, machina::TensorShape({1, 4}));
  auto y2 = AsTensor<int32_t>({1, 2, 3, 4}, machina::TensorShape({4, 1}));
  const auto expected_out2 =
      AsTensor<int32_t>({30}, machina::TensorShape({1, 1}));

  std::vector<machina::Tensor> inputs2{x2, y2};

  std::vector<machina::Tensor> outputs1, outputs2;
  // Iterate over all cores first for warmup execution.
  for (int i = 0; i < helper_->num_cores(); i++) {
...
TEST_F(IfrtServingExecutableTest, ReturnFailOnUncompiledShapeAfterFrozen) {
  ...
  const auto expected_out1 =
      AsTensor<int32_t>({14}, machina::TensorShape({1, 1}));
  std::vector<machina::Tensor> inputs1{x1, y1};
  std::vector<machina::Tensor> outputs1;
  for (int i = 0; i < helper_->num_cores(); i++) {
    TF_ASSERT_OK(executable->Execute(absl::MakeSpan(inputs1), {}).status());
  }
  TF_ASSERT_OK_AND_ASSIGN(outputs1,
  ...
  std::vector<machina::Tensor> outputs2;
...
TEST_F(IfrtServingExecutableTest, Spmd) {
  ...
  std::vector<machina::Tensor> inputs{x, y, z};
  TF_ASSERT_OK_AND_ASSIGN(auto result,
                          executable->Execute(absl::MakeSpan(inputs), {}));
