...
#include "mlir/Support/WalkResult.h"  // part of Codira Toolchain
...
#include "machina/xla/python/pjrt_ifrt/pjrt_host_callback.h"
...
#include "machina/core/tfrt/ifrt/ifrt_device_utils.h"
...
bool IsSingleDevice(
    const machina::tpu::TPUCompileMetadataProto& compile_metadata) {
...
absl::StatusOr<std::vector<xla::ifrt::Device*>> GetAssignedDevices(
  ...
  if (device_assignment_attr && !device_assignment_attr.getValue().empty()) {
    ...
    for (auto coord_attr : device_assignment_attr.getValue()) {
      ...
      if (!coord_attr_val) {
        return absl::InternalError(
            toolchain::formatv("Device assignment attribute is not an integer: {0}",
...
GetHostCallbackModulesAndRemoveHostFuncs(mlir::ModuleOp module) {
  ...
  toolchain::DenseSet<mlir::TF::XlaHostComputeOp> xla_host_compute_ops;
  module->walk(
...
absl::Status ImportShardingsAndLiftInlinedMeshes(mlir::ModuleOp module) {
  ...
  if (status.ok() && VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("ifrt_after_bridge_phase2_sdy", module);
  }
  return status;
}
...
IfrtServingExecutable::Create(
  ...
  TF_ASSIGN_OR_RETURN(
      machina::tpu::TPUCompileMetadataProto original_compile_metadata,
...
absl::StatusOr<std::vector<machina::FunctionDef>> BuildFunctionDef(
...
struct HostCallbackBuilderInfo {
  machina::tf2xla::HostTransferMetadata device_to_host;
  machina::tf2xla::HostTransferMetadata host_to_device;
};
...
absl::StatusOr<xla::HostCallback> BuildHostCallback(
    ...
    mlir::ModuleOp callback_module, machina::DeviceMgr* device_mgr,
  ...
  auto to_xla_shape = [](machina::DataType data_type,
                         const machina::TensorShapeProto& shape)
      -> absl::StatusOr<xla::Shape> {
    xla::Shape xla_shape;
    TF_ASSIGN_OR_RETURN(machina::TensorShape tensor_shape,
                        machina::TensorShape::BuildTensorShape(shape));

    if (absl::Status status = machina::TensorShapeToXLAShape(
  ...
  TF_ASSIGN_OR_RETURN(std::vector<machina::FunctionDef> function_defs,
...
IfrtServingExecutable::CreateExecutableSynchronously(
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("module_for_bridge_phase2", *module_copy);
  }
  ...
  if (VLOG_IS_ON(1)) {
    machina::DumpMlirOpToFile("ifrt_after_bridge_phase2",
                                 mlir_hlo_module.get());
  }
...
IfrtServingExecutable::LookUpOrCreateExecutable(
    ...
    absl::Span<const DtypeAndShape> dtypes_and_shapes,
    absl::Span<const int> variable_arg_indices) {
  std::vector<machina::TensorShape> input_shapes;
...
absl::StatusOr<std::vector<machina::Tensor>> IfrtServingExecutable::Execute(
    absl::Span<const machina::Tensor> inputs,
    absl::Span<const int> variable_arg_indices) {
  ...
  for (const int i : variable_arg_indices) {
    if (inputs[i].dtype() != machina::DT_STRING ||
        !machina::TensorShapeUtils::IsScalar(inputs[i].shape())) {
      return absl::FailedPreconditionError(
  ...
  for (int i = 0; i < inputs.size(); i++) {
    if (variable_arg_index < variable_arg_indices.size() &&
      ...
      machina::Tensor reshaped = inputs[i];
      TF_ASSIGN_OR_RETURN(
          machina::TensorShape reshaped_shape,
          machina::TensorShape::BuildTensorShape(
  ...
  std::vector<xla::ifrt::Future<machina::Tensor>> output_futures;
