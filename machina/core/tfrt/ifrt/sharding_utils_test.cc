...
struct ReshardToTensorTestParam {
  ...
  machina::Tensor expected_out_tensor;
  std::vector<int> device_indices;
  xla::HloSharding sharding;
};

struct TensorToArrayTestParam {
  machina::Tensor in_tensor;
...
machina::Tensor AsInt4Tensor(absl::Span<const int> values,
                                const TensorShape& shape) {
  machina::Tensor tensor(machina::DT_INT4, shape);
...
TEST_P(TensorToArrayTest, MakeArrayFromTensor) {
  ...
  for (int i = 0; i < disassembled_arrays.size(); ++i) {
    ...
    machina::Tensor host_tensor(expected_out_tensor.dtype(),
