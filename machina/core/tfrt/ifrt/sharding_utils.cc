...
#include "machina/xla/python/ifrt/index.h"
#include "machina/xla/python/ifrt/index_domain.h"
...
#include "machina/core/tpu/kernels/sharding_utils.h"
...
SplitAndCreateArraysFromHostBuffer(
    xla::ifrt::Client& ifrt_client, const machina::Tensor& input_tensor,
  ...
  machina::DataType tensor_data_type = input_tensor.dtype();
  std::vector<int32_t> paddings(num_partitions_per_axis.size(), 0);
  std::vector<machina::Tensor> split_tensors;
  split_tensors.resize(num_slices);

  auto allocate_output_fn =
      [&](int i, const machina::TensorShape& output_slice_shape,
        ...
        split_tensors[i] =
            machina::Tensor(tensor_data_type, output_slice_shape);
  ...
  auto assign_or_copy_value_fn =
      [&](const machina::Tensor& input) -> absl::Status {
...
#define CASE(type)                                                             \
  case machina::DataTypeToEnum<type>::value: {                              \
...
//
absl::StatusOr<machina::Tensor> MakeTensorFromDisassembledTensors(
    xla::ifrt::Client& ifrt_client,
    absl::Span<const machina::Tensor> disassembled_tensors,
    const std::vector<int>& num_concats,
    machina::DataType output_tensor_type,
    const machina::TensorShape& output_tensor_shape,
    const tsl::thread::ThreadPool& thread_pool) {
  Eigen::ThreadPoolDevice thread_pool_device(thread_pool.AsEigenThreadPool(),
                                             thread_pool.NumThreads());
  ...
  machina::Tensor output_tensor(output_tensor_type, output_tensor_shape);

  auto assign_or_copy_value_fn = [&](const Tensor& tensor) {
    output_tensor = tensor;
    return absl::OkStatus();
  };
  auto get_output_fn = [&]() -> absl::StatusOr<machina::Tensor*> {
...
absl::StatusOr<int> VerifyIndexDomainsAndGetReplicas(
    ...
    const machina::TensorShape& tensor_shape) {
...
absl::StatusOr<xla::ifrt::ArrayRef> CreateArrayFromHostTensorForSingleDevice(
    xla::ifrt::Client& ifrt_client, const machina::Tensor& tensor,
...
absl::StatusOr<xla::ifrt::Future<machina::Tensor>> MakeTensorFromArrayHelper(
    xla::ifrt::Client& ifrt_client, xla::ifrt::Array& input_array,
    const xla::HloSharding& hlo_sharding,
    const xla::ifrt::DeviceListRef& device_list,
    tsl::thread::ThreadPool& thread_pool) {
  TF_ASSIGN_OR_RETURN(machina::DataType data_type,
                      ToTensorDataType(input_array.dtype()));
  machina::TensorShape tensor_shape = ToTensorShape(input_array.shape());

  VLOG(2) << "Create tensor from array based on sharding: "
          << hlo_sharding.ToString();

  xla::ifrt::Promise<machina::Tensor> promise =
      xla::ifrt::Future<machina::Tensor>::CreatePromise();
  xla::ifrt::Future<machina::Tensor> output_tensor_future(promise);
  ...
  if (hlo_sharding.IsReplicated()) {
    ...
    machina::Tensor output_tensor(data_type, tensor_shape);
  ...
  std::vector<machina::Tensor> input_tensors;
  ...
  for (const auto& [index_domain, array] : index_domain_device_arrays) {
    machina::TensorShape tensor_shape = ToTensorShape(index_domain.shape());
    TF_ASSIGN_OR_RETURN(machina::DataType dtype,
                        ToTensorDataType(array->dtype()));
    machina::Tensor tensor(dtype, tensor_shape);
...
xla::ifrt::Future<machina::Tensor> MakeTensorFromArray(
    xla::ifrt::Client& ifrt_client, xla::ifrt::Array& input_array,
    const xla::HloSharding& hlo_sharding,
    const xla::ifrt::DeviceListRef& device_list,
    tsl::thread::ThreadPool& thread_pool) {
  absl::StatusOr<xla::ifrt::Future<machina::Tensor>> output_tensor_future =
      MakeTensorFromArrayHelper(ifrt_client, input_array, hlo_sharding,
                                device_list, thread_pool);
  if (!output_tensor_future.ok()) {
    return xla::ifrt::Future<machina::Tensor>(
...
std::optional<absl::InlinedVector<int64_t, 4>> GetByteStrides(
    machina::DataType dtype, const machina::TensorShape& shape) {
  xla::Shape xla_shape;
  if (auto status = machina::TensorShapeToXLAShape(dtype, shape, &xla_shape);
      !status.ok()) {
    return std::nullopt;
  }
