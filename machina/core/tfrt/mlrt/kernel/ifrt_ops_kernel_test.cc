...
using machina::test::ExpectEqual;
...
std::string EncodeRestoreDtypesInt32(int num_outputs) {
  ...
  auto ctor = mlrt::bc::New<mlrt::bc::Vector<machina::DataType>>(
      &allocator, num_outputs);

  for (int i = 0; i < num_outputs; ++i) {
    ctor.ConstructAt(i, machina::DT_INT32);
  }
...
class KernelTest : public ::testing::TestWithParam<bool> {
 protected:
  ...
  machina::SessionOptions session_options_;
  machina::FunctionDefLibrary fdef_lib_;
  ...
  machina::ifrt_serving::IfrtModelContext* ifrt_model_context_;
};
...
TEST_P(KernelTest, IfrtLoadVariableOpCanGetTensorFromResourceManager) {
  ...
  TF_CHECK_OK(machina::Tensor::BuildTensor(DT_INT32, {}, &input_tensor));
...
TEST_P(KernelTest, IfrtLoadVariableOp) {
  ...
  auto input_tensor_future =
      xla::ifrt::Future<machina::Tensor>(input_tensor_promise);
...
TEST_P(KernelTest, IfrtRestoreVariableOp) {
  ...
  xla::ifrt::Future<machina::Tensor> uninitialized_entry =
  ...
  xla::ifrt::Future<machina::Tensor> restored_future =
      ifrt_model_context_->GetRestoreTensorRegistry().GetRestoredTensor(
          absl::StrCat(kVariableRuntimeName, 0));
  absl::StatusOr<machina::Tensor> restored_tensor = restored_future.Await();
...
TEST_P(KernelTest, IfrtRestoreVariableOp4Variables) {
  ...
  machina::Tensor slice_tensor = AsTensor<tsl::tstring>(
