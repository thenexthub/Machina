 ...
 *
 */
#ifndef MACHINA_CORE_TFRT_MLRT_KERNEL_KERNEL_RUNNER_UTILS_H_
#define MACHINA_CORE_TFRT_MLRT_KERNEL_KERNEL_RUNNER_UTILS_H_
...
void LaunchAsyncOpKernel(const tfrt_stub::OpKernelRunner& kernel_runner,
                         ...
                         std::shared_ptr<machina::DeviceBase> custom_device);
...
void ExecuteKernelRunner(
  ...
  for (int i = 0; i < args.size(); ++i) {
    ...
    // buffer forwarding optimization in many machina OpKernels.
  ...
  if constexpr (!IsAsync) {
    ...
    for (int i = 0; i < op_kernel_context.num_outputs(); ++i) {
      ...
      results[i].template Emplace<machina::tfrt_stub::FallbackTensor>(
          std::move(*op_kernel_context.mutable_output(i)));
    }
  } else {
    std::shared_ptr<machina::DeviceBase> custom_device = nullptr;
    if constexpr (Frame::kUseCustomDevice) {
  ...
  for (int i = 0; i < last_uses.size(); ++i) {
    if (last_uses[i]) {
      reg_span[i].template Destroy<machina::tfrt_stub::FallbackTensor>();
    }
  }
}
...
#endif  // MACHINA_CORE_TFRT_MLRT_KERNEL_KERNEL_RUNNER_UTILS_H_
