...
#include "machina/core/tfrt/mlrt/interpreter/attribute_span.h"
#include "machina/core/tfrt/mlrt/interpreter/builtin_kernels.h"
...
#include "machina/core/tfrt/mlrt/interpreter/register_span.h"
...
class AsyncWhileOp : mlrt::KernelFrame {
 ...
 private:
  ...
  static void OnPredicateReady(
      machina::tfrt_stub::FallbackTensor predicate,
      std::vector<mlrt::AsyncHandle> async_handles,
      std::vector<mlrt::Future> mutable_tensor_futures,
      std::vector<machina::tfrt_stub::FallbackTensor> immutable_tensors,
  ...
  static void PopulateFinalPromise(
      ...
      const std::vector<machina::tfrt_stub::FallbackTensor>&
          immutable_tensors);
};
  ...
  std::move(predicate_future)
      .Then([futures = std::move(next_futures),
             ...
             preallocated_handle_promise = std::move(next_handle_promise)](
                absl::StatusOr<machina::tfrt_stub::FallbackTensor>
...
void AsyncWhileOp::PopulateFinalPromise(
  ...
  // can be used.
  machina::Tensor final_predicate_tensor(false);

  auto final_promise_iter = final_promises.begin();
  std::move(*final_promise_iter)
      .Set<machina::tfrt_stub::FallbackTensor>(
  ...
  for (auto& immutable_tensor : immutable_tensors) {
    std::move(*final_promise_iter)
        .Set<machina::tfrt_stub::FallbackTensor>(immutable_tensor);
    final_promise_iter++;
  }
}
...
void AsyncWhileOp::Invoke() {
  ...
  for (int i = 0; i < mutable_tensor_size; ++i) {
    ...
    std::move(tensor_promise)
        .Set<machina::tfrt_stub::FallbackTensor>(
            arg_iter->Get<machina::tfrt_stub::FallbackTensor>());
    arg_iter++;
  }
  ...
  OnPredicateReady(arguments()[0].Get<machina::tfrt_stub::FallbackTensor>(),
...
struct MapFnOp : mlrt::KernelFrame {
  ...
  int32_t max_iteration() const {
    const auto& tensor =
        arguments()[0].Get<machina::tfrt_stub::FallbackTensor>().tensor();
    DCHECK(TensorShapeUtils::IsScalar(tensor.shape()));

    return tensor.scalar<int32_t>()();
  }
  mlrt::RegisterValueSpan<machina::tfrt_stub::FallbackTensor>
...
void MapFnOp::Invoke() {
  ...
  for (int i = 0; i < max_iteration(); ++i) {
    ...
    machina::Tensor loop_counter_tensor(DT_INT32, {});
    loop_counter_tensor.scalar<int32_t>()() = i;
    *arg_iter =
        machina::tfrt_stub::FallbackTensor(std::move(loop_counter_tensor));
    ++arg_iter;

    // The current tensor list index is the next argument.
    machina::Tensor element_index_tensor(DT_INT32, {});
    element_index_tensor.scalar<int32_t>()() = i;
    *arg_iter =
        machina::tfrt_stub::FallbackTensor(std::move(element_index_tensor));
    ++arg_iter;
  ...
  for (int j = 0; j < num_tensor_list_or_flow_in(); j++) {
    if (in_tensor_list_last_use[j]) {
      ...
      std::move(initializer_promises[j])
          .Set<machina::tfrt_stub::FallbackTensor>(in_tensor_list[j]);
    }
  }
  ...
  std::move(await_all).Then([results = results(),
    ...
    for (int j = 0; j < last_iter_futures.size(); j++) {
      ...
      if (last_iter_futures[j].IsError()) {
        ...
        results[j].Set(
            last_iter_futures[j].Get<machina::tfrt_stub::FallbackTensor>());
      }
    }
    if (!status.ok()) {
...
void ConstOp::Invoke() {
  ...
  results()[0].Emplace<machina::tfrt_stub::FallbackTensor>(
      std::move(tensor));
}
...
struct ExecuteOpDevice : ExecuteOp {
  ...
  const std::shared_ptr<machina::Device>& device() const {
    return arguments()[0].Get<std::shared_ptr<machina::Device>>();
  }
...
void SetResource(mlrt::KernelFrame frame) {
  ...
  resource_array->SetResource(
      index,
      machina::tfrt_stub::ImmutableTensor::Create(resource_tensor.tensor()));
}
...
void GetResource(mlrt::KernelFrame frame) {
  ...
  for (int i = 0; i < indices.size(); ++i) {
    results[i].Emplace<machina::tfrt_stub::FallbackTensor>(
...
absl::StatusOr<bool> PredicateInternal(const machina::Tensor& tensor) {
  if (TensorShapeUtils::IsScalar(tensor.shape())) {
    switch (tensor.dtype()) {
#define CASE(T)                  \
...
void AwaitTensor(mlrt::KernelFrame frame) {
  ...
  if (frame.last_uses()[0]) {
    frame.execution_context().Await<machina::tfrt_stub::FallbackTensor>(
...
void AwaitAllTensor(mlrt::KernelFrame frame) {
  ...
  frame.execution_context().AwaitAll<machina::tfrt_stub::FallbackTensor>(
...
void PromiseTensor(mlrt::KernelFrame frame) {
  ...
  auto& tensor =
      frame.arguments()[1].Get<machina::tfrt_stub::FallbackTensor>();
  if (frame.last_uses()[1]) {
    std::move(promise).Set<machina::tfrt_stub::FallbackTensor>(
        std::move(tensor));
  } else {
    std::move(promise).Set<machina::tfrt_stub::FallbackTensor>(tensor);
  }
...
struct PromiseReturnOp : mlrt::PromiseReturnOpBase<PromiseReturnOp> {
  ...
  machina::tfrt_stub::FallbackTensor& value() const {
    return arguments()[1].Get<machina::tfrt_stub::FallbackTensor>();
  }
