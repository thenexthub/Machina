...
using machina::ifrt_serving::IfrtModelContext;
...
struct MlrtIfrtRestoreVariableKernel : mlrt::KernelFrame {
  ...
  machina::tfrt_stub::FallbackTensor prefix() const {
    DCHECK_GT(arguments().size(), 3);
    return arguments()[0].Get<machina::tfrt_stub::FallbackTensor>();
  }
  machina::tfrt_stub::FallbackTensor tensor_names() const {
    DCHECK_GT(arguments().size(), 3);
    return arguments()[1].Get<machina::tfrt_stub::FallbackTensor>();
  }
  machina::tfrt_stub::FallbackTensor shape_and_slices() const {
    DCHECK_GT(arguments().size(), 3);
    return arguments()[2].Get<machina::tfrt_stub::FallbackTensor>();
  }

  mlrt::bc::Vector<machina::DataType> restored_dtypes() const {
    return attributes().GetAs<mlrt::bc::Vector<machina::DataType>>(0);
  }
  ...
  std::vector<machina::tfrt_stub::FallbackTensor> var_handles() const {
    DCHECK_GT(arguments().size(), 3);
    std::vector<machina::tfrt_stub::FallbackTensor> result;
    result.reserve(arguments().size() - 3);
    for (int i = 3; i < arguments().size(); ++i) {
      result.push_back(
          arguments()[i].Get<machina::tfrt_stub::FallbackTensor>());
    }
    return result;
  }
...
absl::Status MlrtIfrtRestoreVariableKernel::InvokeHelper() {
  ...
  std::vector<machina::DataType> restored_dtypes_vec(
...
class MlrtIfrtLoadVariableKernel : public mlrt::KernelFrame {
 public:
  ...
  const machina::Tensor& variable_handler_tensor() const {
    DCHECK_GE(arguments().size(), 1);
    const machina::Tensor& ret =
...
absl::Status MlrtIfrtLoadVariableKernel::InvokeHelper() {
  ...
  if (used_by_host()) {
    if (ifrt_restore_tensor_registry.SetUsedByHost(runtime_name).ok()) {
          ...
          [tensor_promise = std::move(tensor_promise)](
            ...
            std::move(tensor_promise)
                .Set<machina::tfrt_stub::FallbackTensor>(
                    machina::tfrt_stub::FallbackTensor(*restored_tensor));
          });
    } else {
      ...
      if (machina::Tensor* t = variable->tensor(); t != nullptr) {
        std::move(tensor_promise)
            .Set<machina::tfrt_stub::FallbackTensor>(
                machina::tfrt_stub::FallbackTensor(*t));
      } else {
        std::move(tensor_promise)
  ...
  machina::Tensor key_tensor(machina::DT_STRING, {});
  key_tensor.scalar<tsl::tstring>()() = runtime_name;
  results()[0].Set(machina::tfrt_stub::FallbackTensor(key_tensor));
