...
TEST(AttributeTest, TensorAttr) {
  ...
  EXPECT_EQ(tensor_attr.dtype(), machina::DT_INT64);
...
TEST(AttributeTest, BoolTensorAttr) {
  ...
  EXPECT_EQ(tensor_attr.dtype(), machina::DT_BOOL);
  EXPECT_THAT(tensor_attr.shape(), ::testing::ElementsAreArray({4}));
...
TEST(AttributeTest, ShapeAttr) {
  ...
  auto shape_attr = mlir::TF::ShapeAttr::get(
      &mlir_context, toolchain::ArrayRef<int64_t>(data.begin(), data.end()),
      /*unranked=*/false);
...
TEST(AttributeTest, DtypeArrayAttr) {
  ...
  auto arr_attr = mlir::ArrayAttr::get(
      &mlir_context, toolchain::ArrayRef<mlir::Attribute>(arr.begin(), arr.end()));
  ...
  mlrt::bc::Vector<machina::DataType> dtype_arr(attr_buffer.data());
