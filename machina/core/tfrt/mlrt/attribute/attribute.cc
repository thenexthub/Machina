...
absl::StatusOr<std::string> EncodeTensorflowAttribute(
  ...
  if (auto dense_attr = mlir::dyn_cast<mlir::DenseElementsAttr>(attr)) {
    ...
    TF_RETURN_IF_ERROR(machina::ConvertToDataType(element_type, &dtype));

    if (dtype == machina::DT_STRING) {
  ...
  if (auto type_attr = mlir::dyn_cast<mlir::TypeAttr>(attr)) {
    machina::DataType dtype;
    TF_RETURN_IF_ERROR(
        machina::ConvertToDataType(type_attr.getValue(), &dtype));
