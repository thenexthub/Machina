...
TEST(SerializeBEFTest, HandlesCompleteProcess) {
  ...
  const std::string saved_model_mlir_path =
      "third_party/machina/compiler/mlir/tfrt/tests/saved_model/testdata/"
      "test.mlir";
  ...
  TF_ASSERT_OK(machina::tfrt_stub::SerializeBEF(old_bef, filepath));
...
TEST(SerializeMLRTTest, HandlesSerializeAndDeserializeProcess) {
  ...
  TF_ASSERT_OK(
      machina::tfrt_stub::SerializeMLRTBytecode(old_bytecode, filepath));
