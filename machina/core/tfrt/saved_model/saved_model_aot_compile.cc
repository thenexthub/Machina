...
#include "machina/compiler/jit/pjrt_device_compiler_client.h"
#include "machina/compiler/jit/tf_graph_to_hlo_compiler.h"
#include "machina/compiler/jit/xla_compiler_options_util.h"
...
#include "machina/xla/pjrt/gpu/se_gpu_pjrt_compiler.h"
...
absl::Status CompileTfGraphToHlo(
  ...
  auto device =
      std::make_unique<VirtualDevice>(machina::Env::Default(), device_proto);
  ...
  *options = GenerateCompilerOptionsForPjRt(
  ...
  options->device_type = DeviceType("MACHINA_MACHINA_XLA_GPU_JIT");
...
absl::Status UpdateGraphDefWithInputShapes(
    ...
    const absl::flat_hash_map<std::string, machina::TensorShapeProto>&
        input_shapes,
  ...
  absl::flat_hash_map<std::string, machina::TensorShapeProto>
      graph_input_shapes;
...
absl::StatusOr<AotResult> AotCompileSavedModel(
  ...
  TF_ASSIGN_OR_RETURN(machina::MetaGraphDef meta_graph_def,
  ...
  ASSIGN_OR_RETURN_IN_IMPORT(
      std::unique_ptr<machina::tfrt_stub::FallbackState> fallback_state,
...
absl::StatusOr<std::string> AotCompileToGpuPjRtLoadedExecutableWithDevice(
  ...
  auto se_client = absl::WrapUnique(
      machina::down_cast<xla::StreamExecutorGpuClient*>(client.release()));
...
absl::StatusOr<AotResult::ExecutableMap> AotCompileXlaFunctionsInMetaGraphDef(
    ...
    const absl::flat_hash_map<std::string, machina::TensorShapeProto>&
        input_shapes,
    const machina::FunctionDefLibrary& fdef_lib,
    const machina::SessionOptions& session_options,
