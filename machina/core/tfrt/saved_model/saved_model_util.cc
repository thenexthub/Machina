...
#include "machina/core/tfrt/saved_model/saved_model_import_input.h"
...
std::vector<std::string> FindNamesForValidSignatures(
  ...
  auto is_ref_type_tensor_info = [](const auto& named_tensor_info) {
    return machina::IsRefType(named_tensor_info.second.dtype());
  };
...
machina::Tensor CreateScalarStringTensor(absl::string_view str) {
  return machina::Tensor(machina::tstring(str));
}
...
absl::StatusOr<machina::Tensor> CreateTensorFromBoundInput(
  ...
  if (auto asset = toolchain::dyn_cast<mlir::tf_saved_model::AssetOp>(bound_input)) {
...
absl::StatusOr<InitializersAndSignatures> GetInitializersAndSignatures(
  ...
  absl::flat_hash_map<std::string, std::vector<machina::Tensor>>
        ...
        if (should_initialize_inputs) {
          ...
          for (auto* bound_input : sig_info.bound_inputs) {
            ...
            if (!capture.ok()) {
              ...
              init_inputs.push_back(machina::Tensor());
            } else {
...
absl::StatusOr<machina::MetaGraphDef> ReadSavedModel(
  ...
  TF_RETURN_IF_ERROR(machina::ReadMetaGraphDefFromSavedModel(
...
absl::StatusOr<mlir::OwningOpRef<mlir::ModuleOp>> ImportSavedModel(
    mlir::MLIRContext* context, const machina::MetaGraphDef& meta_graph_def,
  ...
  TF_ASSIGN_OR_RETURN(
      auto module,
      machina::ConvertSavedModelV1ToMlirLite(
          import_input,
