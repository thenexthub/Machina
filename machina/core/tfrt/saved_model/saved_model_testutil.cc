...
std::unique_ptr<machina::tfrt_stub::Runtime> DefaultTfrtRuntime(
    int num_threads) {
  return machina::tfrt_stub::Runtime::Create(
...
void ComputeCurrentTFResult(const std::string& saved_model_dir,
  ...
  TF_CHECK_OK(machina::LoadSavedModel(session_options, /*run_options=*/{},
                                         saved_model_dir,
                                         /* tags = */ {"serve"}, bundle));
  ...
  std::vector<std::pair<std::string, machina::Tensor>> session_inputs;
  session_inputs.reserve(inputs.size());
  for (const auto& iter : toolchain::zip(input_names, inputs)) {
...
void ExpectTensorEqual(const machina::Tensor& x, const machina::Tensor& y,
  ...
  switch (y.dtype()) {
    case machina::DT_STRING:
      machina::test::ExpectTensorEqual<machina::tstring>(x, y);
      break;
    case machina::DT_FLOAT:
    case machina::DT_DOUBLE:
      if (error) {
        machina::test::ExpectClose(x, y, *error, /*rtol=*/0.0);
      } else {
        machina::test::ExpectEqual(x, y);
      }
      break;
    default:
      machina::test::ExpectEqual(x, y);
