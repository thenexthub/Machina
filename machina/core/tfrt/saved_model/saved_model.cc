...
#include "machina/compiler/mlir/tfrt/transforms/mlrt/import_model.h"
...
#include "machina/core/tfrt/graph_executor/export_mlir.h"
#include "machina/core/tfrt/graph_executor/graph_execution_options.h"
#include "machina/core/tfrt/graph_executor/graph_executor.h"
...
#include "machina/core/tfrt/mlrt/kernel/batch_kernel.h"
#include "machina/core/tfrt/mlrt/kernel/kernel.h"
...
auto* inferred_model_type_count = monitoring::Counter<3>::New(
    "/machina/tfrt/inferred_model_type",
...
auto* saved_model_input_spec_validation_failure =
    machina::monitoring::Gauge<bool, 1>::New(
        "/machina/tfrt/saved_model/input_spec_validation_failure",
...
absl::Status PrepareRestore(mlir::MLIRContext* context,
                            ...
                            const machina::MetaGraphDef& meta_graph_def,
...
absl::Status RunBytecodeInitializers(
  ...
  for (const auto& p : initializers_and_signatures.initializers) {
    ...
    std::vector<machina::Tensor> outputs;
    const std::vector<machina::Tensor> empty_inputs;
    const std::vector<machina::Tensor>& initializer_inputs =
...
absl::Status IsInputSpecsCorrect(absl::string_view name,
                                 ...
                                 absl::Span<const machina::Tensor> inputs) {
...
absl::Status CheckInputSpecs(
    const machina::SessionMetadata& model_metadata,
    const SavedModel::RunOptions& run_options, absl::string_view signature_name,
    const internal::Signature& signature,
    absl::Span<const machina::Tensor> input_tensors) {
  ...
  if (!status.ok()) {
    ...
    if (!run_options.validate_input_specs_dry_run) {
      return machina::errors::InvalidArgument(error_string);
    }
...
absl::Status PreprocessSignature(
    const machina::SessionMetadata& model_metadata,
    const SavedModel::RunOptions& run_options, absl::string_view signature_name,
    const machina::SignatureDef& signature_def,
    const internal::Signature& signature,
    absl::Span<const machina::Tensor> input_tensors,
...
void GetSignaturesFromSignatureDef(
    ...
    const google::protobuf::Map<std::string, machina::SignatureDef>& signature_defs,
    const SavedModel::Options& options) {
  for (const auto& p : signature_defs) {
    const std::string& signature_name = p.first;
    const machina::SignatureDef& signature_def = p.second;
    ...
    for (const auto& p : signature_def.inputs()) {
      ...
      const machina::TensorInfo& tensor_info = p.second;
...
absl::StatusOr<std::unique_ptr<SavedModel>> SavedModelImpl::LoadSavedModel(
    Options options, machina::MetaGraphDef meta_graph_def,
  ...
  if (aot_exist) {
    ...
    machina::tf_mlrt::RegisterTfMlrtKernels(*kernel_registry);
    machina::tf_mlrt::RegisterTfMlrtBatchKernels(*kernel_registry);

    if (options.graph_execution_options.enable_mlrt) {
      ASSIGN_OR_RETURN_IN_COMPILE(
          bytecode, machina::mlrt_compiler::ConvertTfMlirToBytecode(
                        options.graph_execution_options.compile_options,
                        *fallback_state, mlir_module.get(), model_context));
    } else {
      RETURN_IF_ERROR_IN_COMPILE(machina::ConvertTfMlirToBef(
...
SavedModelImpl::SavedModelImpl(
    ...
    machina::MetaGraphDef meta_graph_def, tfrt::BefBuffer bef,
...
const machina::MetaGraphDef& SavedModelImpl::GetMetaGraphDef() const {
...
absl::Status SavedModelImpl::Run(const RunOptions& run_options,
  ...
  if (options_.enable_lazy_loading &&
    ...
    std::vector<std::pair<std::string, machina::Tensor>> input_tensors;
...
struct SavedModelImpl::JoinedSignature {
  ...
  machina::GraphImportConfig::InputArrays input_nodes;
...
absl::Status SavedModelImpl::RunMultipleSignatures(
  ...
  // fetch nodes.
  std::vector<std::pair<std::string /*tensor_name*/, machina::Tensor>>
      flat_inputs;
  ...
  std::vector<machina::Tensor> flat_outputs;
...
SavedModelImpl::ImportSubgraph(
    ...
    const machina::GraphImportConfig::InputArrays& input_nodes,
...
absl::StatusOr<JoinedSignature> JoinSignatures(
    ...
    const machina::protobuf::Map<std::string, machina::SignatureDef>&
        signature_def_map) {
  ...
  for (const auto& name : names) {
    ...
    for (const auto& iter : signature_def.inputs()) {
      ...
      joined_signature.input_nodes.insert(
          std::pair<std::string, machina::ArrayInfo>(tensor_info.name(),
                                                        std::move(array_info)));
    }
...
SavedModelImpl::LoadJoinedSignature(const JoinedSignature& joined_signature) {
  ...
  if (options_.graph_execution_options.enable_mlrt) {
    ASSIGN_OR_RETURN_IN_COMPILE(
        ...
        machina::mlrt_compiler::ConvertTfMlirToBytecode(
    ...
    TF_RETURN_IF_ERROR(machina::ConvertTfMlirToBef(
...
SavedModelImpl::GetOrCreateLoadingResult(const RunOptions& run_options,
  ...
  machina::mutex_lock l(loading_result_cache_mu_);
