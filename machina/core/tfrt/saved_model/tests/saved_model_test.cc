...
#include "machina/core/tfrt/graph_executor/test_config.pb.h"
#include "machina/core/tfrt/run_handler_thread_pool/run_handler_concurrent_work_queue.h"
...
TEST(SavedModelTest, BasicV2) {
  ...
  // Set input 'x' to [[1, 1, 1]]
  std::vector<machina::Tensor> inputs;
  inputs.emplace_back(machina::DT_INT32,
                      /*shape=*/machina::TensorShape{1, 3});
...
TEST(SavedModelTest, BasicInlineExecution) {
  ...
  runtime->SetCreateRequestQueueFn(
      [](int64_t) -> absl::StatusOr<std::unique_ptr<WorkQueueInterface>> {
        return machina::tfrt_stub::WrapDefaultWorkQueue(
...
TEST(SavedModelTest, VariableOnTpu) {
  ...
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/variable_on_tpu");
...
std::vector<machina::Tensor> CreateExpectedOutputs(
    const FunctionMetadata& function_metadata,
    const std::vector<std::pair<std::string, machina::Tensor>>&
        named_outputs) {
  std::vector<machina::Tensor> outputs;
  absl::flat_hash_map<std::string, machina::Tensor> name_to_outputs;
...
TEST(SavedModelTest, RunMultipleSignatures) {
  ...
  std::vector<machina::Tensor> toy_inputs;
  toy_inputs.push_back(CreateTfTensor<int32_t>(/*shape=*/{1, 3},
                                               /*data=*/{1, 1, 1}));
  std::vector<machina::Tensor> another_toy_inputs;
  another_toy_inputs.push_back(CreateTfTensor<int32_t>(/*shape=*/{1, 3},
                                                       /*data=*/{2, 2, 2}));

  std::vector<machina::Tensor> yet_another_toy_inputs;
  ...
  // copyable.
  std::vector<std::vector<machina::Tensor>> inputs;
  ...
  std::vector<std::vector<machina::Tensor>> outputs;
...

  {
    ...
    std::vector<machina::Tensor> expected_another_toy_outputs =
...

  {
    ...
    expected_yet_another_toy_named_outputs.push_back(
        {"r33", CreateTfTensor<int32_t>(/*shape=*/{1, 3},
                                        /*data=*/{24, 24, 24})});
    std::vector<machina::Tensor> expected_yet_another_toy_outputs =
...
class SavedModelRunByTensorNamesTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    //  y = tf.compat.v1.get_variable(name='y', initializer=[1, 2, 3])
    //  r = tf.matmul(x, y)
    auto saved_model_dir = machina::GetDataDependencyFilepath(
        "machina/core/tfrt/saved_model/tests/toy_v1/1");
  ...
  std::vector<std::pair<std::string, machina::Tensor>> inputs_;
...
TEST(SavedModelTest, FunctionMetadata) {
  ...
  EXPECT_THAT(
      ...
      ::testing::ElementsAreArray({TensorSpec(machina::DT_INT32, {1, 3})}));
  ...
  EXPECT_THAT(function_metadata->GetOutputSpecs(),
              ...
              ::testing::ElementsAreArray({::testing::Field(
                  &TensorSpec::dtype, machina::DT_INT32)}));
}
...
TEST(SavedModelTest, RefTypeTensorInput) {
  ...
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/ref_type_tensor_input");
...
TEST(ControlFlowTest, CtrlFlow) {
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/if_v1");
  ...
  machina::Tensor x(machina::DT_INT32, machina::TensorShape({}));
  std::copy(std::begin(x_data), std::end(x_data), x.flat<int32_t>().data());
  std::vector<machina::Tensor> tf_inputs = {x};
  std::vector<machina::Tensor> tf_outputs;
...
TEST(SavedModelTest, ResourceGather) {
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/resource_gather_v1");
...
TEST(SavedModelTest, DTypeCoverage) {
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/dtype_coverage_v1");
  ...
  std::vector<machina::Tensor> tf_inputs;
  std::vector<machina::Tensor> tf_outputs;
  ComputeCurrentTFResult(saved_model_dir, /*signature_name=*/"serving_default",
                         function_metadata->GetInputNames(), tf_inputs,
                         function_metadata->GetOutputNames(), &tf_outputs);
...
TEST(SavedModelTest, Error) {
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/error_v1");
...
TEST_P(SavedModelPowTest, Pow) {
  std::string saved_model_dir =
      machina::GetDataDependencyFilepath(GetParam().path);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    ::testing::Values(
        PowTestParam{"machina/core/tfrt/saved_model/tests/pow", false},
        PowTestParam{"machina/core/tfrt/saved_model/tests/pow_v2", false},
        PowTestParam{"machina/core/tfrt/saved_model/tests/pow_v2", true}));

TEST(SavedModelPowTest, MapDataset) {
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/data");
...
TEST(SavedModelTest, ControlFlowV1) {
  ...
  // conflicts).
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/control_flow_v1");
...
TEST(SavedModelTest, WhileLoopV1) {
  ...
  // fetch nodes.
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/while_v1");
...
TEST(SavedModelTest, SparseTensorInput) {
  ...
  // once it is supported.
  std::string saved_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/sparse_tensor_input");
...
TEST(SavedModelTest, EmitUnifiedModelIdFromFingerprintUUID) {
  ...
  auto unified_model_id =
      CellReader<std::string>("/machina/tfrt/saved_model/unified_model_id");
...
TEST(SavedModelTest, EmitUnifiedModelIdWhenNoFingerprintFile) {
  tsl::Env* env = ::tsl::Env::Default();
  std::string source_model_dir = machina::GetDataDependencyFilepath(
      "machina/core/tfrt/saved_model/tests/toy_v1/1");
  std::string saved_model_dir =
