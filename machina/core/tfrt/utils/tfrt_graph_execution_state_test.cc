...
#include "machina/cc/ops/state_ops.h"
...
TEST_F(OptimizeGraphTest, OptimizeFunctions) {
  ...
  machina::FunctionDefLibrary fdef_lib;
  {
    ...
    auto fdef = machina::FunctionDefHelper::Create(
    ...
    *fdef_lib.add_function() = fdef;
    ...
    std::vector<machina::Output> inputs = {a};
    std::vector<machina::DataType> output_dtypes = {
        fdef.signature().output_arg(0).type()};
    machina::NameAttrList func_attr;
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto fallback_state,
      machina::tfrt_stub::FallbackState::Create({}, fdef_lib));
...
TEST_F(OptimizeGraphTest, OptimizeFunctionsUsedByFunctionNodes) {
  ...
  machina::FunctionDefLibrary fdef_lib;
  {
    ...
    const Tensor kThree = test::AsScalar<float>(3.0);
    auto pow3_fdef = machina::FunctionDefHelper::Create(
        "Pow3", {"x: float"}, {"y: float"}, {},
        {{{"three"}, "Const", {}, {{"dtype", DT_FLOAT}, {"value", kThree}}},
         {{"pow3"}, "Pow", {"x", "three:output:0"}, {{"T", DT_FLOAT}}}},
        {{"y", "pow3:z:0"}});
    ...
    auto base2pow3_fdef = machina::FunctionDefHelper::Create(
        ...
        {{{"one"}, "Const", {}, {{"dtype", DT_FLOAT}, {"value", kOne}}},
         ...
         {{"pcall"},
          ...
          {{"Tin", DataTypeSlice({DT_FLOAT})},
           ...
           {"f", machina::FunctionDefHelper::FunctionRef(
                     "Pow3", {{"T", DT_FLOAT}})}}}},
        {{"y", "pcall:output:0"}});
...
TEST_F(ExtendGraphTest, ExtendGraph) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto fallback_state,
      machina::tfrt_stub::FallbackState::Create(session_options, {}));
