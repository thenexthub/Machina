...
#include "machina/core/tfrt/utils/graph_partition.h"
...
TEST_F(GraphPartitionTest, InsertTransferOpsWithTwoDevice) {
  ...
  for (const FunctionDef& fdef : new_graphdef.library().function()) {
    ...
    ASSERT_TRUE(fdef.attr().contains(machina::kNoInlineAttr));
    EXPECT_TRUE(fdef.attr().at(machina::kNoInlineAttr).b());
  ...
  for (NodeDef* node : partitioned_call_nodes) {
    ...
    ASSERT_TRUE(node->attr().contains(machina::kNoInlineAttr));
    EXPECT_TRUE(node->attr().at(machina::kNoInlineAttr).b());
