...
TEST(TensorUtilTest, SHTToTFTensor) {
  ...
  EXPECT_EQ(tf_tensor.flat<machina::tstring>()(0), "hello");
}

TEST(TensorUtilTest, KNFBToTFTensor) {
  std::unique_ptr<HostContext> host = CreateHostContext();
  machina::Tensor original_tf_tensor(0.1f);
  machina::KernelFallbackTensor knfbt(original_tf_tensor);
...
TEST(TensorUtilTest, ScalarHostTensorToTFTensor) {
  ...
  ASSERT_EQ(tf_tensor->dtype(), machina::DT_INT32);
...
machina::Tensor CreateTFTensor(const T (&data)[N],
                                  const machina::TensorShape& shape) {
  machina::Tensor tensor(machina::DataTypeToEnum<T>::value, shape);
...
TEST(TensorUtilTest, TFTensorAndTensorHandleBasic) {
  ...
  machina::TensorShape shape({2, 2});

  machina::Tensor tensors[] = {CreateTFTensor(i1_data, shape),
  ...
  for (const auto& tf_tensor : tensors) {
    ...
    EXPECT_EQ(tf_tensor.tensor_data(), tf_tensor_out.tensor_data())
        ...
        << machina::DataType_Name(tf_tensor.dtype());
  }
}
...
TEST(TensorUtilTest, TFTensorAndTensorHandleString) {
  ...
  machina::Tensor tf_tensor(machina::DT_STRING,
                               machina::TensorShape({2}));
  auto tf_data = tf_tensor.flat<machina::tstring>();
  ...
  auto tf_data_2 = tf_tensor_2.flat<machina::tstring>();
...
TEST(TensorUtilTest, TFTensorToDHT) {
  ...
  ASSERT_FALSE(!dht);
  toolchain::SmallVector<int32_t, 4> dims;
