...
absl::StatusOr<absl::flat_hash_set<std::string>> PreprocessGraph(
    machina::GraphDef& graph_def, bool run_placer_grappler_on_functions) {
  if (VLOG_IS_ON(1)) {
    DumpGraphDefToFile("before_generate_resource_shared_name_graph_def",
                       graph_def);
  }

  TF_RETURN_IF_ERROR(machina::GenerateResourceSharedNameIfEmpty(
      graph_def, machina::OpRegistry::Global()));
...
bool GetTf2xlaMlirBridgeState(
  ...
  if (auto mlir_bridge_config =
          runtime_config->Get<machina::tf2xla::v1::MlirBridgeConfig>();
...
TfrtGraphExecutionState::Create(
    ...
    machina::GraphDef graph_def, const FallbackState& fallback_state,
...
machina::GraphDef CreateGraphDefFromGraphAndFlibDef(
...
absl::StatusOr<std::unique_ptr<machina::Graph>> CreatePrunedGraph(
    machina::GraphDef graph_def, const CallableOptions& callable_options) {
  // clang-tidy off
...
TfrtGraphExecutionState::CreateOptimizedGraph(
  ...
  machina::BuildGraphOptions build_graph_options;
  ...
  TF_RETURN_WITH_CONTEXT_IF_ERROR(
      FunctionalizeControlFlow(
          result.graph.get(),
          const_cast<machina::FunctionLibraryDefinition*>(
...
absl::Status EliminateRefVariablesFromV1ControlFlow(
  ...
  machina::GraphDef updated_graph_def;
...
absl::Status OptimizeFunctions(
  ...
  for (FunctionDef& fdef : *flib_proto.mutable_function()) {
    ...
    machina::Graph* graph = fbody->graph;
    machina::GraphDef graph_def;
    graph->ToGraphDef(&graph_def);
    ...
    *graph_def.mutable_library() = flib.ToProto();
    ...
    std::unique_ptr<machina::FunctionLibraryDefinition> optimized_flib;
    machina::BuildGraphOptions build_graph_options;
...
absl::StatusOr<std::unique_ptr<machina::Graph>>
