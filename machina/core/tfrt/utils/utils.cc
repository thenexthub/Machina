...
void CreateDummyTfDevices(
  ...
  for (const auto& name : device_names) {
    machina::DeviceAttributes device_attrs =
        machina::Device::BuildDeviceAttributes(
            name, machina::DEVICE_TPU_SYSTEM, machina::Bytes(16ULL << 30),
            machina::DeviceLocality(), "device: TFRT TPU SYSTEM device");
    dummy_tf_devices->push_back(std::make_unique<machina::VirtualDevice>(
        machina::Env::Default(), device_attrs));
  }
}
