...
#include "machina/core/tfrt/runtime/tf_threadpool_concurrent_work_queue.h"
...
class TfrtSession : public machina::Session {
 public:
  ...
  absl::Status CreateLocked(GraphDef graph)
    ...
    machina::tfrt_stub::UpdateTpuTargetByBridgeCompatibility(options, graph);
    ...
    auto session_options =
        machina::tfrt_stub::CreateDefaultSessionOptions(options);
    ...
    TF_ASSIGN_OR_RETURN(
        auto fallback_state,
        machina::tfrt_stub::FallbackState::CreateWithDeviceMgr(
    ...
    TF_RETURN_IF_ERROR(
        machina::ConvertGraphDefToGraph(opts, graph, initial_graph.get()));
    ...
    TF_ASSIGN_OR_RETURN(
        graph_executor_,
        machina::tfrt_stub::GraphExecutor::Create(
  ...
  absl::Status RunInternal(
    ...
    std::unique_ptr<machina::tfrt_stub::WorkQueueInterface> work_queue;
    ...
    if (inter_op_thread_pools_.run_in_caller_thread() ||
      ...
      work_queue =
          std::make_unique<machina::tfrt_stub::TfThreadPoolWorkQueue>(
  ...
  absl::Status RunCallable(
    ...
    for (const auto& it :
         toolchain::zip(callable.callable_options.feed(), feed_tensors)) {
 ...
 private:
  ...
  machina::tfrt_stub::GraphExecutionOptions GetGraphExecutionOptions()
      const {
    // TODO(jingdong): Check option configurations.
    ::machina::tfrt_stub::GraphExecutionOptions options(runtime_);
  ...
  std::unique_ptr<::machina::tfrt_stub::GraphExecutor> graph_executor_;

  machina::tfrt_stub::Runtime* runtime_ = nullptr;
  ...
  machina::BackendCompiler* backend_compiler_ = nullptr;
  std::unique_ptr<StaticDeviceMgr> device_manager_;
};

std::unique_ptr<machina::tfrt_stub::WorkQueueInterface>
 ...
 public:
  ...
  absl::StatusOr<TfrtSessionInterOpThreadPools> UpdateAndGetInterOpThreadPools(
    ...
    if (session_inter_op_thread_pool_size > 0) {
      ...
      for (const auto& it :
           toolchain::enumerate(options.config.session_inter_op_thread_pool())) {
...
absl::Status TfrtSessionFactory::InitializeLocked(
  ...
  if (options.runtime != nullptr) {
    ...
    owned_runtime_ = machina::tfrt_stub::Runtime::Create(
