 ...
 *
 */
#ifndef MACHINA_CORE_TFRT_TFRT_SESSION_TFRT_SESSION_H_
#define MACHINA_CORE_TFRT_TFRT_SESSION_TFRT_SESSION_H_
...
struct TfrtSessionOptions {
  ...
  machina::tfrt_stub::Runtime* runtime = nullptr;
  bool enable_mlrt = false;
  // Should only set one of `use_tpu` and `use_gpu` and `backend_compiler`.
  bool use_tpu = false;
  bool use_gpu = false;
  machina::BackendCompiler* backend_compiler = nullptr;
};

// Factory class to create `TfrtSession` instances.
class TfrtSessionFactory : public machina::SessionFactory {
 public:
  TfrtSessionFactory();
 ...
 private:
  ...
  machina::tfrt_stub::Runtime* runtime_ TF_GUARDED_BY(mutex_) = nullptr;
  std::unique_ptr<machina::tfrt_stub::Runtime> owned_runtime_
  ...
  machina::BackendCompiler* backend_compiler_ TF_GUARDED_BY(mutex_);
...
#endif  // MACHINA_CORE_TFRT_TFRT_SESSION_TFRT_SESSION_H_
