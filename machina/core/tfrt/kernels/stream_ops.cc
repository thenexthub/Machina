 ...
 *
 */
#include "machina/core/tfrt/kernels/stream_ops.h"
...
PwStreamResultsOp::PwStreamResultsOp(machina::OpKernelConstruction* ctx)
    : OpKernel(ctx) {
  ...
  auto interface = machina::tfrt_stub::GetGlobalStreamInterfaceFactory()
                       .CreateWorkerStreamInterface()(controller_address_);
  OP_REQUIRES_OK(ctx, interface.status());
  stream_ = *std::move(interface);
}

void PwStreamResultsOp::Compute(machina::OpKernelContext* ctx) {
  OP_REQUIRES(
  ...
  if (names_.back() == kBatchedStepIdName) {
    ...
    step_ids = machina::Tensor(static_cast<int64_t>(ctx->step_id()));
  }

  std::vector<machina::Tensor> batched_tensors;
...
REGISTER_KERNEL_BUILDER(Name("PwStreamResults").Device(machina::DEVICE_CPU),
                        PwStreamResultsOp);
