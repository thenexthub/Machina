...
#include "machina/core/tfrt/kernels/ifrt_program_ops.h"
...
IfrtCallOp::IfrtCallOp(machina::OpKernelConstruction* ctx) : OpKernel(ctx) {
  OP_REQUIRES_OK(ctx, ctx->GetAttr("program_id", &program_id_));
  OP_REQUIRES_OK(ctx,
                 ctx->GetAttr("variable_arg_indices", &variable_arg_indices_));
}

void IfrtCallOp::Compute(machina::OpKernelContext* ctx) {
  absl::call_once(init_once_, [&]() {
    executable_ = machina::ifrt_serving::ServingExecutableRegistry::Lookup(
        program_id_);
  });
  ...
  machina::OpOutputList outputs(ctx, 0, results->size());
  for (int i = 0; i < results->size(); ++i) {
    outputs.set(i, (*results)[i]);
  }
}

REGISTER_KERNEL_BUILDER(Name("IfrtCall").Device(machina::DEVICE_CPU),
                        IfrtCallOp);
