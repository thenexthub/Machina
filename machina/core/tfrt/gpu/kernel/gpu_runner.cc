...
tfrt::AsyncValueRef<tfrt_stub::FallbackTensor> TransferTensorToDevice(
    ...
    Device* gpu_device) {
  const machina::Tensor& src = tensor.tensor();
  machina::AllocatorAttributes attr;
  attr.set_use_pjrt_allocator(true);

  machina::Tensor dst(gpu_device->GetAllocator(attr), src.dtype(),
                         src.shape());
  ...
  DeviceContext* pjrt_device_context =
      gpu_device->machina_accelerator_device_info()->pjrt_context;
...
tfrt::AsyncValueRef<tfrt_stub::FallbackTensor> TransferTensorFromDevice(
  ...
  machina::AllocatorAttributes attr;
  machina::Tensor dst(cpu_device->GetAllocator(attr), src.dtype(),
                         src.shape());
  if (src.shape().num_elements() == 0) {
    return tfrt::MakeAvailableAsyncValueRef<tfrt_stub::FallbackTensor>(dst);
  }
...
TransferOutputsToHostIfNeeded(
  ...
  toolchain::SmallVector<tfrt::AsyncValueRef<tfrt_stub::FallbackTensor>> results;
...
std::vector<XlaCompiler::Argument> BuildXlaCompilerArguments(
  ...
  for (int input_num = 0; input_num < inputs.size(); ++input_num) {
    const machina::Tensor& input = inputs[input_num].tensor();
...
ExecuteProgram(
    ...
    const toolchain::SmallVector<tfrt::AsyncValueRef<tfrt_stub::FallbackTensor>>&
        transferred_args,
    const XlaCompiler::CompilationResult* compilation_result,
...
GpuRunner::Run(GpuRunInputs run_inputs) {
  ...
  toolchain::SmallVector<tfrt::RCReference<tfrt::AsyncValue>, 4>
