...
absl::Status RunMlrtFunction(
  ...
  // Set up tf_mlrt::Context which is used for executing machina::OpKernel.
  ...
  if (!execution_context.status().ok()) {
    outputs->resize(mlrt_outputs.size(), machina::Tensor());
...
absl::StatusOr<std::unique_ptr<RequestInfo>> CreateRequestInfo(
    ...
    machina::tfrt_stub::WorkQueueInterface* work_queue,
    ...
    machina::tfrt_stub::FallbackState& fallback_state,
  ...
  // Create a `machina::Executor::Args::Runner` with the above request queue.
  ...
  // thread pool in `machina::Device` will be used.
  DCHECK(runner_table);
...
absl::Status GraphExecutionRunOnFunction(
  ...
  if (run_options.deadline.has_value()) {
    ...
    if (absl::ToChronoTime(absl::Now()) > deadline) {
      return machina::errors::DeadlineExceeded(kDeadlineExceededMessage);
    }
  ...
  toolchain::SmallVector<tfrt::AsyncValue*, 4> arguments;
  auto cleanup = machina::gtl::MakeCleanup([&]() {
  ...
  if (arguments.size() != func->argument_types().size())
    return machina::errors::Internal("incorrect number of inputs.");

  toolchain::SmallVector<tfrt::RCReference<tfrt::AsyncValue>, 4> chain_and_results;
  ...
  auto results = toolchain::drop_begin(chain_and_results, 1);

  machina::StatusGroup status_group;
  ...
  for (tfrt::RCReference<tfrt::AsyncValue>& result : results) {
    ...
    if (result->IsError()) {
      ...
      outputs->push_back(machina::Tensor());
      continue;
    }
...
GraphExecutor::GraphExecutor(
    ...
    std::unique_ptr<machina::tfrt_stub::TfrtGraphExecutionState>
...
absl::Status GraphExecutor::Run(
  ...
  // compilation.
  std::vector<machina::DataType> sorted_input_dtypes;
  ...
  std::vector<machina::Tensor> flat_inputs;
  ...
  if (do_recompilation) {
    ...
    machina::mutex_lock l(num_recompilations_mu_);
...
GraphExecutor::ImportAndCompileClientGraph(
  ...
  if (options_.compile_options.compile_to_sync_tfrt_dialect) {
    if (kernel_registry_ == nullptr) {
      return machina::errors::Internal("Missing kernel registry in MLRT.");
    }
    ...
    tfrt::BefBuffer bef;
    TF_RETURN_IF_ERROR(
        machina::ConvertTfMlirToBef(options_.compile_options, module.get(),
  ...
  auto* latency_sampler =
      machina::tfrt_metrics::GetTfrtGraphExecutorLatencySampler(
...
GraphExecutor::ImportClientGraphToMlirModule(
  ...
  TF_ASSIGN_OR_RETURN(
      auto module,
      machina::tf2xla::v2::ConvertGraphToTfExecutor(
...
GraphExecutor::GetOrCreateLoadedClientGraph(
    ...
    absl::Span<const machina::DataType> input_tensor_dtypes,
    absl::Span<const std::string> output_tensor_names,
    absl::Span<const std::string> target_tensor_names,
    machina::tfrt_stub::WorkQueueInterface* work_queue,
  ...
  machina::mutex_lock l(loaded_client_graphs_mu_);
...
absl::Status GraphExecutor::RunWithSyncInterpreter(
    ...
    absl::Span<const machina::DataType> input_dtypes,
    absl::Span<const std::string> output_tensor_names,
    absl::Span<const std::string> target_tensor_names,
  ...
  machina::tfd::KernelFallbackCompatRequestState kernel_fallback_state(
  ...
  auto tf_context = std::make_unique<machina::tf_mlrt::Context>(
...
CostRecorder* GraphExecutor::LoadedClientGraph::MaybeGetCostRecorder(
  ...
  *do_recompilation = false;
  machina::mutex_lock l(cost_analysis_data_.mu);
...
absl::Status GraphExecutor::LoadedClientGraph::UpdateCost(
  ...
  if (executable_context()->IsForMlrt()) {
    ...
    TF_ASSIGN_OR_RETURN(
        auto bytecode_buffer,
        machina::mlrt_compiler::ConvertTfMlirWithOpKeysToBytecode(
    ...
    if (bef.empty()) {
      return diag_handler.Combine(
          machina::errors::Internal("failed to convert MLIR to BEF."));
    }
    bef.shrink_to_fit();
  ...
  }
  {
    ...
    machina::mutex_lock lock(executable_context_mu_);
...
void GraphExecutor::LoadedClientGraph::UpdateCostAnalysisData(
    absl::Time now, bool do_recompilation) {
  machina::mutex_lock lock(cost_analysis_data_.mu);
  if (!do_recompilation) {
...
void RegisterMlirDialect(mlir::DialectRegistry& registry,
                         machina::BackendCompiler* backend_compiler) {
