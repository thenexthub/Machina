 ...
 *
 */
#ifndef MACHINA_CORE_TFRT_GRAPH_EXECUTOR_GRAPH_EXECUTION_OPTIONS_H_
#define MACHINA_CORE_TFRT_GRAPH_EXECUTOR_GRAPH_EXECUTION_OPTIONS_H_
...
struct GraphExecutionOptions {
  explicit GraphExecutionOptions(const machina::tfrt_stub::Runtime* rt)
      : runtime(rt) {
    DCHECK(runtime);
  }
  ...
  // Runtime configuration. Refer to machina::tfrt_stub::Runtime class for
  // more details. It must not be nullptr;
  const machina::tfrt_stub::Runtime* runtime = nullptr;

  // Model metadata used for monitoring and tracing.
  machina::SessionMetadata model_metadata;
  ...
  machina::TfrtCompileOptions compile_options;
};
...
struct GraphExecutionRunOptions {
  ...
  // in the machina::tfrt_stub::Runtime will be used.
  machina::tfrt_stub::WorkQueueInterface* work_queue = nullptr;
  ...
  std::function<void(absl::flat_hash_map<std::string, machina::Tensor>)>
...
machina::SessionOptions CreateDefaultSessionOptions(
    const GraphExecutionOptions& options);

// Updates TPU target to fallback if bridge uncompatible, otherwise TPU runtime.
void UpdateTpuTargetByBridgeCompatibility(
    machina::tfrt_stub::GraphExecutionOptions& options,
    const machina::GraphDef& graph_def);

}  // namespace tfrt_stub
}  // namespace machina

#endif  // MACHINA_CORE_TFRT_GRAPH_EXECUTOR_GRAPH_EXECUTION_OPTIONS_H_
