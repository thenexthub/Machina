...
class GraphExecutorForTestingCostAnalysis : public GraphExecutor {
 public:
  int num_recompilations() {
    machina::mutex_lock lock(num_recompilations_mu_);
...
TEST_P(GraphExecutorTest, Vanilla) {
  ...
  TF_ASSERT_OK_AND_ASSIGN(
      auto fallback_state,
      machina::tfrt_stub::FallbackState::Create(
...
TEST_P(GraphExecutorTest, OnlineCostAnalysisOptionsOverrideToOnce) {
  ...
  machina::tf2xla::v1::MlirBridgeConfig mlir_bridge_config;
...
class TestIsCancelledKernel : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* ctx) override {
    ctx->set_output(
        0, machina::Tensor(ctx->cancellation_manager()->IsCancelled()));
  }
};
...
TEST_P(GraphExecutorTest, Cancellation) {
  ...
  machina::GraphDefBuilder builder(
      machina::GraphDefBuilder::kFailImmediately);

  const machina::TensorShape tensor_shape({10, 9});
  machina::Node* input = machina::ops::SourceOp(
      "Placeholder", builder.opts()
                         .WithName("input")
                         .WithAttr("dtype", machina::DT_INT32)
                         .WithAttr("shape", tensor_shape));
  machina::ops::SourceOp("TestIsCancelled",
                            builder.opts()
                                .WithName("is_cancelled")
                                .WithAttr("T", machina::DT_BOOL));
  machina::ops::UnaryOp("TestCancel", input,
                           builder.opts()
                               .WithName("test_cancel")
                               .WithAttr("T", machina::DT_INT32));
