...
#include "machina/core/tfrt/utils/bridge_graph_analysis.h"
...
machina::SessionOptions CreateDefaultSessionOptions(
  ...
  *config.mutable_graph_options() = options.compile_options.graph_options;
  ...
  if (options.enable_grappler_function_optimizer) {
    config.mutable_graph_options()
        ->mutable_rewrite_options()
        ->set_function_optimization(machina::RewriterConfig::ON);
  } else {
    config.mutable_graph_options()
        ->mutable_rewrite_options()
        ->set_function_optimization(machina::RewriterConfig::OFF);
  }
...
void UpdateTpuTargetByBridgeCompatibility(
  ...
  if (options.compile_options.device_target ==
      machina::TfrtDeviceInfraTarget::kBridgeFallback) {
    ...
    if (!s.ok()) {
      ...
      options.compile_options.device_target =
          machina::TfrtDeviceInfraTarget::kTfFallback;
    } else {
      options.compile_options.device_target =
          machina::TfrtDeviceInfraTarget::kTpurt;
    }
  }
  ...
  if (options.compile_options.device_target !=
          machina::TfrtDeviceInfraTarget::kCpu &&
      options.compile_options.device_target !=
          machina::TfrtDeviceInfraTarget::kGpu) {
