...
#ifndef MACHINA_CORE_TFRT_RUN_HANDLER_THREAD_POOL_RUN_HANDLER_H_
#define MACHINA_CORE_TFRT_RUN_HANDLER_THREAD_POOL_RUN_HANDLER_H_
...
class RunHandler {
 public:
  ...
  machina::thread::ThreadPoolInterface* AsIntraThreadPoolInterface() const;
...
class RunHandlerEnvironment {
 public:
  typedef machina::Thread EnvThread;
  struct TaskImpl {
    TaskFunction f;
    machina::Context context;
    uint64_t trace_id;
  };
  machina::Env* const env_;
  const machina::ThreadOptions thread_options_;
 ...
 public:
  ...
  RunHandlerEnvironment(machina::Env* env,
                        const machina::ThreadOptions& thread_options,
...
class ThreadWorkSource {
 public:
  ...
  void SetWaiter(uint64_t version, Waiter* waiter, machina::mutex* mutex);
 ...
 private:
  ...
  machina::mutex blocking_queue_op_mu_;
  char pad_[128];
  machina::mutex waiters_mu_;
  Waiter queue_waiters_ TF_GUARDED_BY(waiters_mu_);
  std::atomic<int64_t> traceme_id_;

  machina::mutex run_handler_waiter_mu_;
  uint64_t version_ TF_GUARDED_BY(run_handler_waiter_mu_);
  machina::mutex* sub_thread_pool_waiter_mu_
...
class RunHandlerThreadPool {
 public:
  ...
  RunHandlerThreadPool(Options options, machina::Env* env,
                       const machina::ThreadOptions& thread_options,
                       const std::string& name,
 ...
 private:
  struct ThreadData {
    ...
    machina::condition_variable sources_not_empty;
    std::unique_ptr<machina::Thread> thread;
    int current_index;
...
class RunHandlerWorkQueue : public machina::tfrt_stub::WorkQueueInterface {
 public:
  ...
  void Await(
      toolchain::ArrayRef<tfrt::RCReference<tfrt::AsyncValue>> values) override;
...
#endif  // MACHINA_CORE_TFRT_RUN_HANDLER_THREAD_POOL_RUN_HANDLER_H_
