...
#include "machina/core/tfrt/run_handler_thread_pool/run_handler.h"
...
RunHandlerEnvironment::RunHandlerEnvironment(
    machina::Env* env, const machina::ThreadOptions& thread_options,
...
RunHandlerEnvironment::EnvThread* RunHandlerEnvironment::CreateThread(
  ...
  return env_->StartThread(thread_options_, name_, [=]() {
    ...
    machina::port::ScopedFlushDenormal flush;
    // Set the processor rounding mode to ROUND TO NEAREST.
    machina::port::ScopedSetRound round(FE_TONEAREST);
    if (thread_options_.numa_node != machina::port::kNUMANoAffinity) {
      machina::port::NUMASetThreadNodeAffinity(thread_options_.numa_node);
    }
    f();
  });
}
...
RunHandlerEnvironment::Task RunHandlerEnvironment::CreateTask(TaskFunction f) {
  ...
  return Task{
      std::unique_ptr<TaskImpl>(new TaskImpl{
          std::move(f),
          machina::Context(machina::ContextKind::kThread),
          id,
      }),
  };
}
...
void RunHandlerEnvironment::ExecuteTask(const Task& t) {
  machina::WithContext wc(t.f->context);
  tsl::tracing::ScopedRegion region(tsl::tracing::EventCategory::kRunClosure,
                                    t.f->trace_id);
  t.f->f();
}

void WaitOnWaiter(Waiter* waiter, Waiter* queue_head, machina::mutex* mutex,
                  int sleep_micros, bool adaptive_sleep_time) {
  {
    machina::mutex_lock l(*mutex);
  ...
  {
    machina::mutex_lock l(waiter->mu);
...
Task ThreadWorkSource::EnqueueTask(Task t, bool is_blocking,
  ...
  if (enable_wake_up) {
    ...
    machina::mutex* waiter_queue_mu;
    {
      ...
      // finishes.
      machina::tf_shared_lock lock(run_handler_waiter_mu_);
      waiter_queue = sub_thread_pool_waiter_;
      waiter_queue_mu = sub_thread_pool_waiter_mu_;
    }
    {
      machina::mutex_lock l(*waiter_queue_mu);
...
void ThreadWorkSource::SetWaiter(uint64_t version, Waiter* waiter,
  ...
  machina::mutex_lock l(run_handler_waiter_mu_);
...
RunHandlerThreadPool::RunHandlerThreadPool(
    Options options, machina::Env* env,
    const machina::ThreadOptions& thread_options, const std::string& name,
    Eigen::MaxSizeVector<machina::mutex>* waiters_mu,
...
RunHandlerThreadPool::~RunHandlerThreadPool() {
  ...
  for (size_t i = 0; i < thread_data_.size(); ++i) {
    {
      machina::mutex_lock l(thread_data_[i].mu);
...
void RunHandlerThreadPool::SetThreadWorkSources(
  ...
  machina::mutex_lock l(thread_data_[tid].mu);
...
void RunHandlerThreadPool::WorkerLoop(int thread_id,
  ...
  while (!cancelled_) {
    ...
    {
      machina::mutex_lock l(thread_data_[thread_id].mu);
...
void RunHandlerThreadPool::WaitForWorkInSubThreadPool(int thread_id,
  ...
  if (!is_blocking) {
    machina::Env::Default()->SleepForMicroseconds(
        non_blocking_thread_sleep_time_);
    return;
  }
...
class RunHandler::Impl {
 public:
  ...
  machina::thread::ThreadPoolInterface* thread_pool_interface() {
 ...
 private:
  class RunHandlerEigenThreadPool
      : public machina::thread::ThreadPoolInterface {
   public:
    ...
    void Schedule(std::function<void()> fn) override {
      run_handler_->ScheduleIntraOpClosure(machina::tfrt_stub::WrapWork(
...
class RunHandlerPool::Impl {
 public:
      ...
      : max_handlers_(options.max_concurrent_handler),
        ...
        run_handler_thread_pool_(new internal::RunHandlerThreadPool(
            internal::RunHandlerThreadPool::Options(
                ...
                options.sub_thread_request_percentage),
            machina::Env::Default(), machina::ThreadOptions(),
    ...
    RunHandler::Impl* handler_impl;
    {
      ...
      if (!has_free_handler()) {
        tsl::profiler::TraceMe activity(
            ...
            tsl::profiler::TraceMeLevel::kInfo);
        if (timeout_in_ms == 0) {
          mu_.Await(machina::Condition(this, &Impl::has_free_handler));
        } else if (!mu_.AwaitWithDeadline(
                       machina::Condition(this, &Impl::has_free_handler),
                       machina::EnvTime::NowNanos() +
  ...
  void ReleaseHandler(RunHandler::Impl* handler) TF_LOCKS_EXCLUDED(mu_) {
    ...
    uint64_t now = machina::EnvTime::NowMicros();
  ...
  void Quiesce() TF_LOCKS_EXCLUDED(mu_) {
    while (true) {
      ...
      machina::Env::Default()->SleepForMicroseconds(sleep_time);
    }
  }
 ...
 private:
  ...
  Eigen::MaxSizeVector<machina::mutex> waiters_mu_;
  ...
  machina::histogram::Histogram time_hist_ TF_GUARDED_BY(mu_);
...
void RunHandlerPool::Impl::LogInfo() {
  if (iterations_++ % 50000 == 10 && VLOG_IS_ON(1)) {
    ...
    uint64_t now = machina::Env::Default()->NowMicros();
    ...
    for (int i = 0; i < num_active_requests; ++i) {
      ...
      times_str += machina::strings::StrCat(
          (now - (*it)->start_time_us()) / 1000.0, " ms.");
      ids_str += machina::strings::StrCat((*it)->tws()->GetTracemeId());
      ++it;
    }
...
void RunHandler::Impl::Reset(int64_t step_id,
                             const RunHandlerOptions& options) {
  start_time_us_ = machina::Env::Default()->NowMicros();
  step_id_ = step_id;
  options_ = options;
...
machina::thread::ThreadPoolInterface*
...
void RunHandlerWorkQueue::AddTask(TaskFunction work) {
  run_handler_->ScheduleInterOpClosure(machina::tfrt_stub::WrapWork(
