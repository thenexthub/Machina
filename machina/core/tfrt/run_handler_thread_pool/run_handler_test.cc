...
TEST(RunHandlerUtilTest, TestBasicScheduling) {
  ...
  machina::BlockingCounter counter(2 * num_handlers * num_threads);

  machina::thread::ThreadPool test_pool(machina::Env::Default(), "test",
                                           num_handlers);
  for (int i = 0; i < num_handlers; ++i) {
    test_pool.Schedule([&counter, &barrier, &pool, i, num_threads]() {
      auto handler = pool->Get(i);
      machina::BlockingCounter local_counter(2 * num_threads);
...
TEST_P(RunHandlerThreadPoolTest, EnqueueTask) {
  Eigen::MaxSizeVector<machina::mutex> waiters_mu(2);
  waiters_mu.resize(2);
...
TEST_P(RunHandlerThreadPoolTest, RoundRobinExecution) {
  Eigen::MaxSizeVector<machina::mutex> waiters_mu(1);
  waiters_mu.resize(1);
  ...
  machina::condition_variable function_start;
  machina::condition_variable function_end;
