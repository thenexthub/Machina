...
absl::Status CheckOpDefCompatibility(const machina::OpDef& op_def) {
...
// Create a machina::NodeDef from the machina::OpDef and the attributes.
absl::StatusOr<machina::NodeDef> BuildNodeDef(
    const machina::OpDef& op_def, absl::string_view node_name, int num_args,
...
absl::Status CreateOpKernel(machina::FunctionLibraryRuntime* flr,
                            machina::NodeDef ndef,
                            std::unique_ptr<machina::OpKernel>* result) {
  std::shared_ptr<const machina::NodeProperties> props;
  TF_RETURN_IF_ERROR(machina::NodeProperties::CreateFromNodeDef(
      std::move(ndef), flr->GetFunctionLibraryDefinition(), &props));
  machina::OpKernel* k = nullptr;
...
absl::StatusOr<OpKernelRunner> OpKernelRunner::Create(
    ...
    const std::function<absl::Status(machina::AttrValueMap*)>& attr_builder,
...
absl::StatusOr<OpKernelRunner> OpKernelRunner::Create(
  ...
  TF_RETURN_IF_ERROR(machina::OpRegistry::Global()->LookUpOpDef(
...
OpKernelRunner::OpKernelRunner(
    ...
    std::unique_ptr<machina::OpKernel> op_kernel)
