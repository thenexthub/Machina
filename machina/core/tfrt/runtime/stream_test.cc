...
TEST_F(StreamTest, Simple) {
  ...
  std::vector<absl::flat_hash_map<std::string, machina::Tensor>> outputs;
  ScopedStreamCallback scoped_stream_callback;
  {
    TF_ASSERT_OK_AND_ASSIGN(
        ...
        GetGlobalStreamCallbackRegistry().Register(
            ...
            [&](absl::flat_hash_map<std::string, machina::Tensor> arg) {
              outputs.push_back(std::move(arg));
            }));

    std::vector<absl::flat_hash_map<std::string, machina::Tensor>> expected =
