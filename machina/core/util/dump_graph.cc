...
absl::Status WriteProtoToUniqueFile(const machina::protobuf::Message& proto,
                                    WritableFile* file) {
  string s;
  string format = GetDumpGraphFormatLowerCase();
  if (format == "txt" &&
      !::machina::protobuf::TextFormat::PrintToString(proto, &s)) {
...
absl::Status WriteProtoToUniqueFile(
    const machina::protobuf::MessageLite& proto, WritableFile* file) {
  string s;
...
string DumpProtoToFile(const string& name,
                       machina::protobuf::Message const& proto,
                       const string& dirname) {
  return DumpToFile(
