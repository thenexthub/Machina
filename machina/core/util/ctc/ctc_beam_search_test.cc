...
// words.
#include "machina/core/util/ctc/ctc_beam_search.h"
...
class DictionaryBeamScorer
    : public machina::ctc::BaseBeamScorer<T, BeamState> {
 public:
  DictionaryBeamScorer()
      : machina::ctc::BaseBeamScorer<T, BeamState>(),
        dictionary_({{3}, {3, 1}}) {}
...
void ctc_beam_search_decoding_with_and_without_dictionary() {
  ...
  typename machina::ctc::CTCBeamSearchDecoder<T>::DefaultBeamScorer
      default_scorer;
  machina::ctc::CTCBeamSearchDecoder<T> decoder(num_classes, 10 * top_paths,
                                                   &default_scorer);

  // Dictionary decoder, allowing only two dictionary words : {3}, {3, 1}.
  DictionaryBeamScorer<T, HistoryBeamState<T>> dictionary_scorer;
  machina::ctc::CTCBeamSearchDecoder<T, HistoryBeamState<T>>
  ...
  std::vector<typename machina::ctc::CTCDecoder<T>::Output> expected_output =
      {
          {{1, 3}, {1, 3, 1}, {3, 1, 3}},
      };
  ...
  std::vector<typename machina::ctc::CTCDecoder<T>::Output>
  ...
  std::vector<typename machina::ctc::CTCDecoder<T>::Output> outputs(
      top_paths);
  for (typename machina::ctc::CTCDecoder<T>::Output& output : outputs) {
  ...
  std::vector<typename machina::ctc::CTCDecoder<T>::Output> dict_outputs(
      top_paths);
  for (typename machina::ctc::CTCDecoder<T>::Output& output : dict_outputs) {
    output.resize(batch_size);
  }
  EXPECT_TRUE(
...
void ctc_beam_search_decoding_all_beam_elements_have_finite_scores() {
  ...
  typename machina::ctc::CTCBeamSearchDecoder<T>::DefaultBeamScorer
      default_scorer;
  machina::ctc::CTCBeamSearchDecoder<T> decoder(num_classes, top_paths,
                                                   &default_scorer);
...
class RapidlyDroppingLabelScorer
    : public machina::ctc::BaseBeamScorer<T, LabelState> {
 public:
...
void ctc_beam_search_label_selection() {
  ...
  machina::ctc::CTCBeamSearchDecoder<T, LabelState> decoder(
