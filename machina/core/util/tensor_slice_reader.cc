...
#include "machina/core/framework/versions.h"
...
#include "machina/core/util/tensor_slice_util.h"
...
absl::Status TensorSliceReader::GetTensor(
    const string& name, std::unique_ptr<machina::Tensor>* out_tensor) const {
  DataType type;
  TensorShape shape;
  TensorSlice slice;
  {
  ...
  std::unique_ptr<machina::Tensor> t(new machina::Tensor);
  absl::Status s = machina::Tensor::BuildTensor(type, shape, t.get());
  if (!s.ok()) return s;
