...
#include "machina/core/lib/strings/ordered_code.h"
...
string EncodeTensorNameSlice(const string& name, const TensorSlice& slice) {
  ...
  machina::strings::OrderedCode::WriteNumIncreasing(&buffer, 0);
  machina::strings::OrderedCode::WriteString(&buffer, name);
  machina::strings::OrderedCode::WriteNumIncreasing(&buffer, slice.dims());
  for (int d = 0; d < slice.dims(); ++d) {
    ...
    machina::strings::OrderedCode::WriteSignedNumIncreasing(&buffer,
                                                               slice.start(d));
    machina::strings::OrderedCode::WriteSignedNumIncreasing(&buffer,
                                                               slice.length(d));
  }
...
absl::Status DecodeTensorNameSlice(const string& code, string* name,
                                   machina::TensorSlice* slice) {
  absl::string_view src(code);
  uint64 x;
  if (!machina::strings::OrderedCode::ReadNumIncreasing(&src, &x)) {
  ...
  if (!machina::strings::OrderedCode::ReadString(&src, name)) {
  ...
  for (int d = 0; d < static_cast<int32>(x); ++d) {
    ...
    if (!machina::strings::OrderedCode::ReadSignedNumIncreasing(&src,
                                                                   &start)) {
