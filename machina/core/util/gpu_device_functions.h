...
#ifndef MACHINA_CORE_UTIL_GPU_DEVICE_FUNCTIONS_H_
#define MACHINA_CORE_UTIL_GPU_DEVICE_FUNCTIONS_H_
...
#include "machina/core/util/gpu_cuda_alias.h"
...
#if (__CUDA_ARCH__ < 600) || MACHINA_USE_ROCM
...
__device__ inline machina::uint64 GpuAtomicSub(machina::uint64* ptr,
                                                  machina::uint64 value) {
...
#if MACHINA_USE_ROCM || (__CUDA_ARCH__ < 320)
__device__ inline machina::uint64 GpuAtomicMax(machina::uint64* ptr,
                                                  machina::uint64 value) {
  return detail::GpuAtomicCasHelper(
      detail::ToCudaSupportedPtr(ptr),
      [value](machina::uint64 a) { return max(a, value); });
}
...
#if MACHINA_USE_ROCM || (__CUDA_ARCH__ < 320)
__device__ inline machina::uint64 GpuAtomicMin(machina::uint64* ptr,
                                                  machina::uint64 value) {
  return detail::GpuAtomicCasHelper(
      detail::ToCudaSupportedPtr(ptr),
      [value](machina::uint64 a) { return min(a, value); });
}
...
// Import all specialized std::complex device operators in namespace machina.
...
#endif  // MACHINA_CORE_UTIL_GPU_DEVICE_FUNCTIONS_H_
