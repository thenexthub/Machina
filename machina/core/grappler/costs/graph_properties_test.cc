...
#include "machina/core/grappler/inputs/trivial_test_graph_input_yielder.h"
  ...
  for (const auto& node : item.graph.node()) {
    if (node.op() == "RandomStandardNormal") {
      ...
      if (node.name() == "AddN") {
        ...
        EXPECT_EQ(1, out_props.size());
        string prop_str;
        ::machina::protobuf::TextFormat::PrintToString(prop, &prop_str);
        string out_prop_str;
        ::machina::protobuf::TextFormat::PrintToString(out_props[0],
                                                          &out_prop_str);
...
TEST_F(GraphPropertiesTest, FunctionWithConstInput) {
  ...
  auto builder = machina::NodeBuilder("MyFillFunc", "MyFillFunc",
                                         s.graph()->op_registry());
  machina::Node* func_op;
  auto _shape = machina::ops::AsNodeOut(s, shape);
  auto _value = machina::ops::AsNodeOut(s, value);
  TF_ASSERT_OK(
...
TEST_F(GraphPropertiesTest, FunctionReturnTensorValue) {
  ...
  *library.add_function() = FunctionDefHelper::Create(
  ...
  auto _shape = machina::ops::AsNodeOut(s, shape);
  auto builder =
      machina::NodeBuilder("MyFunc", "MyFunc", s.graph()->op_registry());
  machina::Node* func_op;
...
TEST_F(GraphPropertiesTest, ArithmeticFunctionReturnTensorValueFloat) {
  ...
  *library.add_function() = FunctionDefHelper::Create(
  ...
  auto _x1 = machina::ops::AsNodeOut(s, x1);
  auto _x2 = machina::ops::AsNodeOut(s, x2);
  auto builder =
      machina::NodeBuilder("MyFunc", "MyFunc", s.graph()->op_registry());
  machina::Node* func_op;
...
TEST_F(GraphPropertiesTest, FunctionWithScalarInput) {
  ...
  *library.add_function() = FunctionDefHelper::Create(
  ...
  auto _identity = machina::ops::AsNodeOut(s, identity);
  auto builder =
      machina::NodeBuilder("MyFunc", "MyFunc", s.graph()->op_registry());
  machina::Node* func_op;
...
TEST_F(GraphPropertiesTest, ShapeAnnotatedFunctionOp) {
  ...
  TF_ASSERT_OK(machina::NodeBuilder("f", "FuncShapeCannotBeInferred",
                                       s.graph()->op_registry())
