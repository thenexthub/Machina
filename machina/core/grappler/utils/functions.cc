...
class MakeFunctionDefHelper {
 ...
 private:
  ...
  absl::flat_hash_map<string, machina::NameRangeMap> function_body_outputs_;
};
...
absl::Status MakeFunctionDefHelper::Initialize(
  ...
  for (const NodeDef& node : item.function_body().node()) {
    ...
    machina::NameRangeMap outputs_range_map;
    TF_RETURN_IF_ERROR(machina::NameRangesForNode(
...
absl::Status MakeFunctionDefHelper::AsFunctionDefInput(
  ...
  if (is_body_output != function_body_outputs_.end()) {
    const machina::NameRangeMap& outputs_range_map = is_body_output->second;
