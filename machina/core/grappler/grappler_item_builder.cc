...
#include "machina/core/platform/protobuf_internal.h"
...
void InitializeTensor(DataType type, Tensor* tensor) {
  ...
  if (type == DT_FLOAT) {
    ...
    // is_simple_type<> in machina/core/framework/type_traits.h, and
...
absl::Status RuntimeGraphOptimizer(const GraphDef& graph_def_arg,
  ...
  if (cfg.apply_optimizations) {
    optimizer_opts->set_opt_level(::machina::OptimizerOptions::L1);
  } else {
    optimizer_opts->set_opt_level(::machina::OptimizerOptions::L0);
  }
  ...
  // Optimize the graph.
  ::machina::GraphOptimizer optimizer(*optimizer_opts);
  optimizer.Optimize(flr, env, cpu_device, &graphptr,
                     machina::GraphOptimizer::Options());
      ...
      if (!any_assets.empty()) {
        if (std::is_base_of<protobuf::Message, AssetFileDef>()) {
          for (const auto& any_asset : any_assets) {
            ...
            if (!ParseAny(any_asset, &asset_file_def, "machina.AssetFileDef")
                     .ok()) {
