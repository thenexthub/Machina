...
#include "machina/core/grappler/optimizers/inference/batch_op_rewriter.h"
...
using ::machina::RewriterConfig_CustomGraphOptimizer;
using ::machina::Status;
using ::machina::grappler::GrapplerItem;
...
void AddBatchOp(GraphDef* graph, int num_batch_threads = 16,
  ...
  auto set_batch_node_attribute = [&](const int32_t num_batch_threads,
    ...
    ::machina::graph_transforms::SetNodeAttr("num_batch_threads",
                                                num_batch_threads, batch_op);
    ::machina::graph_transforms::SetNodeAttr("max_batch_size",
                                                max_batch_size, batch_op);
    ::machina::graph_transforms::SetNodeAttr("batch_timeout_micros",
                                                batch_timeout_micros, batch_op);
    ::machina::graph_transforms::SetNodeAttr("allowed_batch_sizes",
                                                allowed_batch_sizes, batch_op);
    ::machina::graph_transforms::SetNodeAttr("max_enqueued_batches",
                                                max_enqueued_batches, batch_op);
    ::machina::graph_transforms::SetNodeAttr("enable_large_batch_splitting",
                                                !disable_large_batch_splitting,
                                                batch_op);

    if (!mixed_priority_policy.empty()) {
      ::machina::graph_transforms::SetNodeAttr(
    ...
    if (!reserved_int_attrs.empty()) {
      ::machina::graph_transforms::SetNodeAttr(kEnableAdaptiveSchedulerAttr,
                                                  true, batch_op);
