...
using ::machina::grappler::Cluster;
...
void UpdateBatchOps(GraphDef* graph, BatchOpRewriteFunction rewrite_fn) {
  ...
  // `::machina::graph_transforms::SetNodeAttr` used by rewrite functions
...
Status BatchOpRewriter::Init(
    const ::machina::RewriterConfig_CustomGraphOptimizer* config) {
  ...
  *optimized_graph = item.graph;
  ...
  if (config_.enable_adaptive_shared_batching_thread_pool()) {
    ...
    UpdateBatchOps(optimized_graph, [](NodeDef* batch_op) {
      ::machina::graph_transforms::SetNodeAttr(kNumBatchThreadsAttr, 0,
                                                  batch_op);
    });
  }
  ...
  if (config_.has_global_prioritization()) {
    ...
    UpdateBatchOps(optimized_graph, [&](NodeDef* batch_op) {
      ::machina::graph_transforms::SetNodeAttr(
          ...
          batch_op);
      ::machina::graph_transforms::SetNodeAttr("mixed_priority_policy",
