...
#if GOOGLE_CUDA || MACHINA_USE_ROCM || INTEL_MKL
...
#include "machina/cc/ops/control_flow_ops_internal.h"
#include "machina/cc/ops/list_ops.h"
...
#include "machina/core/grappler/graph_view.h"
...
class AutoMixedPrecisionTest : public GrapplerTest {
 protected:
  ...
  void TestSimpleUnaryInferOp(
      ...
      const std::function<Output(const machina::Scope&, Output)>&
          test_op_factory) {
    int size = 128;
...
TEST_P(AutoMixedPrecisionParamTest, TensorListThroughFunction) {
  ...
  auto _infer1 = machina::ops::AsNodeOut(s, infer1);
  auto _tl1w1_handle = machina::ops::AsNodeOut(s, tl1w1.output_handle);
  auto builder =
      machina::NodeBuilder("Func1", "Func1", s.graph()->op_registry());
  machina::Node* func1_op;
...
TEST_P(AutoMixedPrecisionParamTest, EluOp) {
  TestSimpleUnaryInferOp(
      -5, 5, 1.0e-3, 1.0e-3,
      [](const machina::Scope& scope, Output input) -> Output {
...
TEST_F(AutoMixedPrecisionCpuTest, Simple) {
  machina::Scope s = machina::Scope::NewRootScope().WithDevice(
      "/job:localhost/replica:0/task:0/device:CPU:0");
  Output input = ops::Const(s.WithOpName("input"), 1.f / 32, {32, 32});
  Output deny1 = ops::Exp(s.WithOpName("deny1"), input);
  Output clr1 = ops::Relu(s.WithOpName("clr1"), deny1);
...
class AutoMixedPrecisionSimulateGpuTest : public GrapplerTest {
 protected:
  ...
  void TestSimple(machina::Scope s, bool is_optimized) {
    Output input = ops::Const(s.WithOpName("input"), 1.f / 32, {32, 32});
    Output deny1 = ops::Exp(s.WithOpName("deny1"), input);
    Output clr1 = ops::Relu(s.WithOpName("clr1"), deny1);
    Output infer1 = ops::Sqrt(s.WithOpName("infer1"), clr1);
...
TEST_F(AutoMixedPrecisionSimulateGpuTest, Simple_NoGpu) {
  TestSimple(machina::Scope::NewRootScope(), /* is_optimized= */ false);
}
...
TEST_F(AutoMixedPrecisionSimulateGpuTest, Simple_SimulatedGpu) {
  setenv("TF_AUTO_MIXED_PRECISION_GRAPH_REWRITE_SIMULATE_GPU", "true",
         1 /* replace */);
  TestSimple(machina::Scope::NewRootScope(), /* is_optimized= */ true);
}

TEST_F(AutoMixedPrecisionSimulateGpuTest, Simple_SimulatedGpu_CpuScope) {
  setenv("TF_AUTO_MIXED_PRECISION_GRAPH_REWRITE_SIMULATE_GPU", "true",
         1 /* replace */);
  TestSimple(machina::Scope::NewRootScope().WithDevice(
                 "/job:localhost/replica:0/task:0/device:CPU:0"),
             /* is_optimized= */ false);
}
...
#endif  // GOOGLE_CUDA || MACHINA_USE_ROCM || INTEL_MKL
