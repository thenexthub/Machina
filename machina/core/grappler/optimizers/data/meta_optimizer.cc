...
#include "machina/core/grappler/optimizers/data/meta_optimizer.h"
...
using ConfigMap =
    std::map<string, machina::RewriterConfig_CustomGraphOptimizer>;
  ...
  for (const auto& option_string : options) {
    ...
    if (!optimizer_config) {
      (*result)[optimizer_name] =
          machina::RewriterConfig_CustomGraphOptimizer();
