...
#include "machina/core/kernels/data/shard_dataset_op.h"
...
using machina::data::AutoShardPolicy;
...
bool IsDatasetNodeOfType(const NodeDef& node,
  ...
  for (const auto& dataset_op_name : arr) {
    if (machina::data::MatchesAnyVersion(/*op_prefix=*/dataset_op_name,
                                            /*op_to_match=*/node.op())) {
      return true;
    }
  }
...
absl::Status AddShardNode(MutableGraphView* graph, const NodeDef& add_before,
  ...
  if (absl::StrContains(add_after->op(), "Dataset")) {
    ...
    if (add_after->attr().count(kOutputShapes) > 0) {
      ...
      machina::TensorShapeProto* shape =
