...
TEST_F(SingleMachineTest, MultipleItems) {
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    string s1;
    ::machina::protobuf::TextFormat::PrintToString(metadata1, &s1);
    string s2;
    ::machina::protobuf::TextFormat::PrintToString(metadata2, &s2);
    EXPECT_EQ(s1, s2);
  }
}
...
#if !(MACHINA_USE_ROCM)  // fails with ROCm (investigate)
