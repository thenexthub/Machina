...
echo "XLA Flags JSON: $MACHINA_MACHINA_XLA_FLAGS_JSON"
...
if echo "$MACHINA_MACHINA_XLA_FLAGS_JSON" | jq -e '. | arrays and length > 0' > /dev/null; then
    mapfile -t xla_flags_array < <(echo "$MACHINA_MACHINA_XLA_FLAGS_JSON" | jq -r '.[]')
fi
