...
_MACHINA_MACHINA_XLA_SRC_ROOT = pathlib.Path(__file__).absolute().parent.parent.parent
...
def main():
  ...
  if not (external := _MACHINA_MACHINA_XLA_SRC_ROOT / "external").exists():
    logging.info("Symlinking `xla/bazel-xla/external` to `xla/external`")
    external.symlink_to(_MACHINA_MACHINA_XLA_SRC_ROOT / "bazel-xla" / "external")
  ...
  with (_MACHINA_MACHINA_XLA_SRC_ROOT / "compile_commands.json").open("w") as f:
    json.dump(
        [
            command.to_dumpable_json(directory=str(_MACHINA_MACHINA_XLA_SRC_ROOT))
