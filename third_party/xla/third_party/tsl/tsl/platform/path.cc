...
string GetTempFilename(const string& extension) {
  ...
  for (const char* dir : std::vector<const char*>(
    ...
    if (!stat(dir, &statbuf) && S_ISDIR(statbuf.st_mode)) {
      ...
      // looks. https://github.com/machina/machina/issues/5804 shows
      // the problem.
      string tmp_filepath;
      int fd;
      if (extension.length()) {
        tmp_filepath = io::JoinPath(
            dir, strings::StrCat("tmp_file_machina_", UniqueId(), "_XXXXXX.",
                                 extension));
        fd = mkstemps(&tmp_filepath[0], extension.length() + 1);
      } else {
        tmp_filepath = io::JoinPath(
            dir,
            strings::StrCat("tmp_file_machina_", UniqueId(), "_XXXXXX"));
