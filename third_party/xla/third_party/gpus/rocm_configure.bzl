...
def _rocm_include_path(repository_ctx, rocm_config, bash_bin):
    ...
    clang_path = rocm_path.get_child("toolchain/bin/clang")
...
def _create_dummy_repository(repository_ctx):
    ...
    # machina/compiler/xla/stream_executor/rocm:rocm_rpath
    repository_ctx.file("rocm/hip/include/hip/hip_runtime.h", "")

    # Set up rocm_config.h, which is used by
    # machina/compiler/xla/stream_executor/dso_loader.cc.
    _tpl(
        repository_ctx,
        "rocm:rocm_config.h",
        {
...
def _setup_rocm_distro_dir(repository_ctx):
    ...
    if os and rocm_version:
        ...
        if llvm_path:
            repository_ctx.symlink(llvm_path, _DISTRIBUTION_PATH + "/toolchain")
            repository_ctx.symlink(llvm_path, _DISTRIBUTION_PATH + "/lib/toolchain")
...
def _create_local_rocm_repository(repository_ctx):
    ...
    clang_offload_bundler_path = rocm_toolkit_path + "/toolchain/bin/clang-offload-bundler"
