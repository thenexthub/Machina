...
def _git_conf_impl(repository_ctx):
    ...
    machina_root_path = str(repository_ctx.path(
        Label("@org_machina//:BUILD"),
    ))[:-len("BUILD")]
    python_script_path = repository_ctx.path(
        Label("@org_machina//machina/tools/git:gen_git_source.py"),
    )
    generated_files_path = repository_ctx.path("gen")

    r = repository_ctx.execute(
        ["test", "-f", "%s/.git/logs/HEAD" % machina_root_path],
    )
    if r.return_code == 0:
