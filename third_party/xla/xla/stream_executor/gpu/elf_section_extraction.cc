...
#include "machina/xla/stream_executor/gpu/elf_section_extraction.h"
...
static absl::StatusOr<std::string> GetSectionContentsFromELFDump(
    const toolchain::object::Binary& binary, absl::string_view section_name) {
  const toolchain::object::ELF64LEObjectFile* executable_elf_object_file_ptr =
      toolchain::dyn_cast<const toolchain::object::ELF64LEObjectFile>(&binary);
...
static absl::StatusOr<std::string> GetSectionContentsFromStaticLibraryDump(
    const toolchain::object::Archive& archive, absl::string_view section_name) {
  toolchain::Error archive_error = toolchain::Error::success();
  for (const toolchain::object::Archive::Child& child :
       archive.children(archive_error)) {
    if (archive_error) {
      return absl::InternalError(toolchain::toString(std::move(archive_error)));
    }

    toolchain::Expected<std::unique_ptr<toolchain::object::Binary>> binary =
        child.getAsBinary();
    if (!binary) {
      continue;
    }
...
absl::StatusOr<std::string> GetSectionContentsFromLibraryDump(
  ...
  const auto buffer = toolchain::MemoryBuffer::getMemBuffer(
      library_dump,
      /*BufferName=*/"", /*RequiresNullTerminator=*/false);

  toolchain::Expected<std::unique_ptr<toolchain::object::Binary>> binary =
      toolchain::object::createBinary(buffer->getMemBufferRef());
  if (!binary) {
    return absl::InternalError(toolchain::toString(binary.takeError()));
  }

  if (binary.get()->isArchive()) {
    return GetSectionContentsFromStaticLibraryDump(
        *toolchain::cast<const toolchain::object::Archive>(binary.get().get()),
        section_name);
  }
  if (binary.get()->isELF()) {
