...
GpuCommandBuffer::CreateDnnGraphCommand(
  ...
  GpuCommandBuffer& nested_gpu =
      machina::down_cast<GpuCommandBuffer&>(*nested);
  TF_RETURN_IF_ERROR(
  ...
  if (mode_ == Mode::kPrimary && state_ == State::kCreate) {
    ...
    if (instantiated.code() == absl::StatusCode::kResourceExhausted) {
      return absl::ResourceExhaustedError(absl::StrFormat(
          ...
          "MACHINA_MACHINA_XLA_CLIENT_MEM_FRACTION (b) Disable command buffers with "
          "'MACHINA_MACHINA_XLA_FLAGS=--xla_gpu_enable_command_buffer=' (empty set). Original "
          "error: %s",
