...
absl::StatusOr<std::vector<uint8_t>> GetFatbinFromArchive(
    toolchain::StringRef archive_path, toolchain::StringRef fatbin_section_prefix) {
  ...
  const auto buffer = toolchain::MemoryBuffer::getMemBuffer(
      toolchain::StringRef(file_contents),
      /*BufferName=*/"", /*RequiresNullTerminator=*/false);

  auto archive_ptr = toolchain::object::Archive::create(buffer->getMemBufferRef());

  if (!archive_ptr) {
    return absl::InternalError(toolchain::toString(archive_ptr.takeError()));
  }

  const toolchain::object::Archive* archive = archive_ptr.get().get();
  ...
  for (const auto& child : archive->children(archive_error)) {
    ...
    auto executable_elf_object_file_ptr =
        toolchain::dyn_cast<toolchain::object::ELF64LEObjectFile>(binary.get());
