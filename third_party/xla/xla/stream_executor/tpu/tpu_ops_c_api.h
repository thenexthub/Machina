...
==============================================================================*/
#ifndef MACHINA_MACHINA_XLA_STREAM_EXECUTOR_TPU_TPU_OPS_C_API_H_
#define MACHINA_MACHINA_XLA_STREAM_EXECUTOR_TPU_TPU_OPS_C_API_H_
...
#include "machina/xla/stream_executor/tpu/libtftpu.h"
...
typedef struct MACHINA_MACHINA_XLA_TpuProgram MACHINA_MACHINA_XLA_TpuProgram;

// Enum for choosing sharding/unsharding program from a `MACHINA_MACHINA_XLA_TpuProgram` obj.
...
typedef struct MACHINA_MACHINA_XLA_TpuMeshState MACHINA_MACHINA_XLA_TpuMeshState;

typedef struct MACHINA_MACHINA_XLA_TpuEmbeddingEngineState MACHINA_MACHINA_XLA_TpuEmbeddingEngineState;

typedef struct TpuEmbedding_TensorBatchFixedState
    TpuEmbedding_TensorBatchFixedState;

typedef struct MACHINA_MACHINA_XLA_DeviceAssignment {
  const char* bytes;
  size_t size;
} MACHINA_MACHINA_XLA_DeviceAssignment;
...
typedef struct MACHINA_MACHINA_XLA_TpuNodeContext MACHINA_MACHINA_XLA_TpuNodeContext;
...
// The API allocates the `MACHINA_MACHINA_XLA_TpuProgram*[]` array `tpu_programs` and creates
// `MACHINA_MACHINA_XLA_TpuProgram` object(s) using the `TpuProgram_New` API. The caller is
// responsible to deallocate both the `MACHINA_MACHINA_XLA_TpuProgram*[]` array and the
// `MACHINA_MACHINA_XLA_TpuProgram` object(s) using `TpuProgram_FreeArray` and `TpuProgram_Free`
// API respectively.
TFTPU_CAPI_EXPORT void TpuCompile_CompileAndBuild(
    TpuSerializedProto compilation_request, const MACHINA_MACHINA_XLA_TpuMeshState* mesh_state,
    MACHINA_MACHINA_XLA_TpuProgram** tpu_programs[], size_t* count, TF_Status* status);

// Creates a new TPU mesh state object.
TFTPU_CAPI_EXPORT MACHINA_MACHINA_XLA_TpuMeshState* TpuMeshState_Create();

// Deletes the given TPU `mesh_state` object. Once deleted the object is
// unusable.
TFTPU_CAPI_EXPORT void TpuMeshState_Free(MACHINA_MACHINA_XLA_TpuMeshState* mesh_state);

// Returns a pointer to an opaque mesh data structure used internally.
TFTPU_CAPI_EXPORT void* TpuMeshState_MeshCommonState(
    MACHINA_MACHINA_XLA_TpuMeshState* mesh_state);

// Creates a new TPU embedding engine state object.
TFTPU_CAPI_EXPORT MACHINA_MACHINA_XLA_TpuEmbeddingEngineState* TpuEmbeddingEngineState_Create();
...
TFTPU_CAPI_EXPORT void TpuEmbeddingEngineState_Free(
    MACHINA_MACHINA_XLA_TpuEmbeddingEngineState* engine_state);
...
typedef struct TpuExecutable_LoadProgramAndEnqueueToStream_Params {
  ...
  const MACHINA_MACHINA_XLA_TpuProgram* program;
  ...
  int32_t rng_seed;
  MACHINA_MACHINA_XLA_DeviceAssignment* device_assignment;
  SE_Stream* stream;
...
TFTPU_CAPI_EXPORT void HardwareLayout_HostShapeToDeviceShape(
    MACHINA_MACHINA_XLA_Shape* host_shape, MACHINA_MACHINA_XLA_Shape* device_shape);
TFTPU_CAPI_EXPORT int64_t HardwareLayout_ShapeSize(MACHINA_MACHINA_XLA_Shape* shape);
TFTPU_CAPI_EXPORT int64_t HardwareLayout_ShapeSizeCompact(MACHINA_MACHINA_XLA_Shape* shape);
TFTPU_CAPI_EXPORT int64_t HardwareLayout_ShapeSizeCompactRaw(MACHINA_MACHINA_XLA_Shape* shape);
...
typedef struct TpuExecute_RuntimeInputToPaddedData_Params {
  ...
  MACHINA_MACHINA_XLA_Shape* runtime_shape;
  MACHINA_MACHINA_XLA_Shape* compile_time_shape;
...
typedef struct WaitForDistributedTpuOp_DoWork_Params {
  ...
  machina::TpuMeshCommonState* tpu_mesh_common_state;
...
TFTPU_CAPI_EXPORT MACHINA_MACHINA_XLA_TpuProgram* TpuProgram_New();

// Destroys the `tpu_program`.
TFTPU_CAPI_EXPORT void TpuProgram_Free(MACHINA_MACHINA_XLA_TpuProgram* tpu_program);

// Creates an array of `MACHINA_MACHINA_XLA_TpuProgram*`.
TFTPU_CAPI_EXPORT MACHINA_MACHINA_XLA_TpuProgram** TpuProgram_NewArray(size_t count);

// Destroys an array of `MACHINA_MACHINA_XLA_TpuProgram*`.
TFTPU_CAPI_EXPORT void TpuProgram_FreeArray(MACHINA_MACHINA_XLA_TpuProgram* tpu_program[]);

// Unloads and destroys the `tpu_program`. Once the TPU program is unloaded and
// destroyed, it is in an unusable state.
TFTPU_CAPI_EXPORT void TpuProgram_UnloadAndDestroy(MACHINA_MACHINA_XLA_TpuProgram* tpu_program,
                                                   TF_Status* status);

// Gets TPU program size in bytes from the `tpu_program`.
TFTPU_CAPI_EXPORT int64_t
TpuProgram_GetProgramSize(const MACHINA_MACHINA_XLA_TpuProgram* tpu_program);
...
TFTPU_CAPI_EXPORT bool TpuProgram_LogProgramMemorySummary(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program);

// Gets TPU program executable info from the `tpu_program`.
TFTPU_CAPI_EXPORT void TpuProgram_GetExecutableInfo(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program, TpuSerializedProto* executable_info,
    TF_Status* status);

// Gets host transfer info proto.
TFTPU_CAPI_EXPORT void TpuProgram_GetHostTransferInfo(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program, TpuSerializedProto* host_transfer_info,
    TF_Status* status);

// Gets HLO metadata proto.
TFTPU_CAPI_EXPORT void TpuProgram_GetHloMetadata(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program, TpuSerializedProto* hlo_metadata,
    TF_Status* status);

// Gets may modify variables boolean value.
TFTPU_CAPI_EXPORT void TpuProgram_GetMayModifyVariables(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program, bool* may_modify_variables);
...
TFTPU_CAPI_EXPORT MACHINA_MACHINA_XLA_TpuProgram* TpuProgram_GetTpuProgram(
    MACHINA_MACHINA_XLA_TpuProgram* tpu_program, TpuProgramShardingType type);

// Gets TPU executable proto from a `tpu_program`.
TFTPU_CAPI_EXPORT void TpuProgram_SerializeTpuExecutable(
    const MACHINA_MACHINA_XLA_TpuProgram* tpu_program, TpuExecutableSerializedProto* executable,
    TF_Status* status);
...
// Deserializes the `GetTpuProgramResponse` proto into an `MACHINA_MACHINA_XLA_TpuProgram`.
TFTPU_CAPI_EXPORT void TpuProgram_DeserializeFromGetTpuProgramResponseProto(
    TpuSerializedProto get_tpu_program_response, MACHINA_MACHINA_XLA_TpuProgram* tpu_program,
    TF_Status* status);

TFTPU_CAPI_EXPORT TpuProgramFingerprint
TpuProgram_GetFingerprint(const MACHINA_MACHINA_XLA_TpuProgram* tpu_program);
...
TFTPU_CAPI_EXPORT int TpuTopology_AvailableCoreCount(
    const MACHINA_MACHINA_XLA_TpuMeshState* mesh_state, TpuCoreTypeEnum tpu_core_type);
...
MACHINA_MACHINA_XLA_TpuNodeContext* TpuNodeContext_Create(int device_ordinal,
                                          TF_Status* status);
void TpuNodeContext_Free(MACHINA_MACHINA_XLA_TpuNodeContext* node_context);
...
typedef struct TpuEmbeddingEngine_Finalize_Params {
  int32_t struct_size;
  void* priv;
  const MACHINA_MACHINA_XLA_TpuMeshState* tpu_mesh_state;
...
typedef struct TpuEmbeddingEngine_RecvActivationsComputation_Params {
  ...
  MACHINA_MACHINA_XLA_Shape* deduplication_data_shape;
...
typedef struct TpuEmbeddingEngine_SendTPUEmbeddingGradientsComputation_Params {
  ...
  TpuSerializedProto hbm_buffers_config;
  TpuSerializedProto tpu_topology;
  MACHINA_MACHINA_XLA_Shape* learning_rate_tuple_shape;
  MACHINA_MACHINA_XLA_Shape* deduplication_data_shape;
  MACHINA_MACHINA_XLA_Shape* gradient_tuple_shape;
  TpuSerializedProto* op_sharding;
  // out
  TpuSerializedProto* xla_computation;
  TF_Status* status;
...
#endif  // MACHINA_MACHINA_XLA_STREAM_EXECUTOR_TPU_TPU_OPS_C_API_H_
