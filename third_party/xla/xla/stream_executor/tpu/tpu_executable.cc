...
static SE_ExecutableRunOptions ToC(
  ...
  if (options.run_options().host_to_device_stream() != nullptr) {
    se_options.host_to_device_stream =
        static_cast<machina::tpu::TpuStream*>(
  ...
  se_options.stream =
      static_cast<machina::tpu::TpuStream*>(impl)->se_stream();
  return se_options;
}
...
absl::StatusOr<ExecutionOutput> TpuExecutable::ExecuteAsyncOnStream(
  ...
  for (int i = 0; i < arguments.size(); ++i) {
    ...
    se_args[i]->unowned_indices = new MACHINA_MACHINA_XLA_ShapeIndex[unowned_indices.size()];
    int j = 0;
