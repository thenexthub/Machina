...
TEST_F(LLVMCompilerTest, HooksTest) {
  ...
  auto pre_opt_hook = [&pre_opt_hook_call_count](const toolchain::Module&) {
    ++pre_opt_hook_call_count;
    return absl::OkStatus();
  };
  auto post_opt_hook = [&post_opt_hook_call_count](const toolchain::Module&) {
  ...
  LLVMCompiler* compiler =
      machina::down_cast<xla::LLVMCompiler*>(backend().compiler());
