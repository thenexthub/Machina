...
HloInstruction* RaggedToDense(HloInstruction* ragged_operand,
  ...
  toolchain::SmallVector<int64_t> slice_starts(slice_shape.dimensions().size(), 0);
  toolchain::SmallVector<int64_t> slice_limits(slice_shape.dimensions().size());
  for (int i = 0; i < slice_shape.dimensions().size(); ++i) {
    slice_limits[i] = slice_shape.dimensions(i);
  }
  toolchain::SmallVector<int64_t> slice_strides(slice_shape.dimensions().size(), 1);
  ...
  toolchain::SmallVector<int64_t> new_shape_dims;
  ...
  toolchain::SmallVector<int64_t> broadcast_dims(
  ...
  toolchain::SmallVector<int64_t> old_dims_new_shape(old_shape.dimensions().size());
...
HloInstruction* TransposeIndexToFront(HloInstruction* instr, int index) {
  ...
  toolchain::SmallVector<int64_t> transpose_dims;
