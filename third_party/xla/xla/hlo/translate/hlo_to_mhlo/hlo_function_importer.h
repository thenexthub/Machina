...
#ifndef MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_FUNCTION_IMPORTER_H_
#define MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_FUNCTION_IMPORTER_H_
...
class HloFunctionImporter {
 public:
  ...
  static absl::StatusOr<mlir::Value> ImportInstructions(
      ...
      const toolchain::SmallVectorImpl<mlir::Value>& arguments,
  ...
  static void FlattenTupleType(
      mlir::Type type, toolchain::SmallVectorImpl<mlir::Type>& flattened_types);
  ...
  static void FlattenTupleValue(
      ...
      toolchain::SmallVectorImpl<mlir::Value>& flattened_values);

  // FlattenTupleValues flattens the values in (nested) tuple-typed 'values' and
  // returns the flattened values.
  static toolchain::SmallVector<mlir::Value> FlattenTupleValues(
 ...
 private:
  ...
  absl::StatusOr<toolchain::SmallVector<mlir::Value, 4>> GetOperands(
      const HloInstruction* instruction);
  ...
  mlir::DenseI64ArrayAttr ConvertArray(toolchain::ArrayRef<int64_t> elements);

  // Converts Array ref to a DenseBoolArrayAttr.
  mlir::DenseBoolArrayAttr ConvertArray(toolchain::ArrayRef<bool> elements);

  // Converts Array ref to an DenseIntElementsAttr.
  mlir::DenseIntElementsAttr Convert(toolchain::ArrayRef<int64_t> elements);

  // Converts Array ref of bools to a DenseIntElementsAttr of I1 type.
  mlir::DenseIntElementsAttr Convert(toolchain::ArrayRef<bool> elements);
  ...
  mlir::NamedAttribute ConvertPadding(toolchain::ArrayRef<int64_t> padding);
...
#endif  // MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_FUNCTION_IMPORTER_H_
