...
absl::StatusOr<mlir::Operation*> ImportOldStyleAsyncStart(
  ...
  toolchain::SmallVector<mlir::NamedAttribute> async_attributes;
  ...
  toolchain::SmallVector<mlir::Location, 1> locs(Untuple(result_types[0]).size(),
                                            loc);
  auto sync_operand =
      async_builder
...
absl::StatusOr<mlir::Operation*> ImportOldStyleAsyncDone(
  ...
  auto async_bundle = toolchain::cast<mlir::mhlo::AsyncBundleType>(
      async_start.getResult().getType());

  auto start_tuple =
      toolchain::dyn_cast<mlir::TupleType>(async_bundle.getTypes()[1]);
  if (start_tuple && toolchain::isa<mlir::TupleType>(start_tuple.getType(0))) {
...
absl::StatusOr<mlir::Operation*> ImportSend(
  ...
  toolchain::ArrayRef<mlir::Value> args = operands;
...
absl::StatusOr<mlir::Operation*> ImportRecv(
  ...
  auto result_types = toolchain::cast<mlir::TupleType>(result_type).getTypes();
  if (result_types.size() != 3)
  ...
  if (isPipelined) {
    ...
    toolchain::SmallVector<mlir::Type> async_result_types = {result_types[0],
                                                        result_types[2]};
  ...
  if (!IsEmptyTuple(result_types[0])) {
    auto recv = builder->create<mlir::stablehlo::RecvOp>(
        loc, toolchain::SmallVector<mlir::Type>{result_types[0], result_types[2]},
        operands, attributes);
    if (instruction->has_sharding()) {
      const HloSharding& sharding = instruction->sharding();
      if (sharding.IsTuple() && sharding.tuple_elements().size() == 3) {
  ...
  auto recv = builder->create<mlir::stablehlo::RecvOp>(
      loc, toolchain::SmallVector<mlir::Type>{result_types[2]}, operands,
      attributes);
  auto empty_tuple = builder->create<mlir::stablehlo::TupleOp>(
      loc, toolchain::ArrayRef<mlir::Value>{});

  return builder->create<mlir::stablehlo::TupleOp>(
      loc,
      toolchain::ArrayRef<mlir::Value>{empty_tuple.getResult(), recv.getResult(0)});
}
