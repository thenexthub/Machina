...
#include "machina/xla/hlo/translate/hlo_to_mhlo/hlo_function_importer.h"
...
#include "machina/xla/hlo/ir/hlo_sharding_metadata.h"
#include "machina/xla/hlo/translate/attributes.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/async_importer.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/attribute_importer.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/custom_call_importer.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/hlo_utils.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/location_importer.h"
...
#include "machina/xla/protobuf_util.h"
...
// original HLO naming.
std::string SanitizeFunctionName(toolchain::StringRef name) {
  std::string output(name);
  toolchain::for_each(output, [](char& x) { x = x == '-' ? '_' : x; });
  return output;
}
...
bool FoldGetTupleElementOfTuple(mlir::stablehlo::GetTupleElementOp op) {
  if (auto tupleOp =
    ...
    toolchain::SmallVector<Value> new_operand{tupleOp.getOperand(idx)};
...
void CleanUpTupleOps(mlir::Block* block, mlir::OpBuilder* builder) {
  bool changed = true;
  toolchain::SmallVector<Value> folded_results;

  while (changed) {
    changed = false;
    for (Operation& op : toolchain::make_early_inc_range(block->getOperations())) {
      if (auto get_tuple_op =
              toolchain::dyn_cast<mlir::stablehlo::GetTupleElementOp>(op)) {
        changed = FoldGetTupleElementOfTuple(get_tuple_op);
      } else if (toolchain::isa<mlir::stablehlo::TupleOp>(op) &&
...
toolchain::ArrayRef<int64_t> ToArrayRef(absl::Span<const int64_t> span) {
  return toolchain::ArrayRef<int64_t>(span.data(), span.size());
}

Operation* CreateReturnOp(mlir::OpBuilder& builder, mlir::Location loc,
                          mlir::ValueRange operands,
                          mlir::Dialect* parent_dialect) {
  LLVM_DEBUG(toolchain::dbgs() << "CreateReturnOp: "
                          << parent_dialect->getNamespace() << '\n');
  if (toolchain::isa<mlir::mhlo::MhloDialect>(parent_dialect)) {
  ...
  if (toolchain::isa<mlir::stablehlo::StablehloDialect>(parent_dialect)) {
...
Operation* WrapInTuple(mlir::OpBuilder* builder, Operation* op) {
  LLVM_DEBUG(toolchain::dbgs() << "WrapInTuple: " << op->getName()
                          << op->getResultTypes() << '\n');
...
Operation* GetTupleElementOp(mlir::OpBuilder* builder, Value value,
                             int64_t index,
                             toolchain::SmallVector<NamedAttribute>&& attributes) {
...
absl::StatusOr<mlir::Value> createConstantZeroLike(mlir::Value operand,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "CreateConstantZeroLike: " << operand << ", "
                          << type << '\n');
  ...
  auto bounded_dim = std::distance(type.getShape().begin(),
                                   toolchain::find_if(type.getShape(), [](auto dim) {
...
void HloFunctionImporter::ReplaceBlockArgumentsWithImplicitOperands(
    mlir::Operation* op, toolchain::ArrayRef<mlir::Value> implicit_operands) {
...
static bool IsNestedTupleInData(Type type) {
  ...
  assert(toolchain::isa<mlir::stablehlo::TokenType>(tuple_type.getType(1)) &&
...
void HloFunctionImporter::FlattenTupleType(
    Type type, toolchain::SmallVectorImpl<Type>& flattened_types) {
  auto tuple_type = mlir::dyn_cast<mlir::TupleType>(type);
  if (!tuple_type) {
...
void HloFunctionImporter::FlattenTupleValue(
    ...
    toolchain::SmallVectorImpl<Value>& flattened_values) {
  auto tuple_type = toolchain::dyn_cast<mlir::TupleType>(value.getType());
  if (!tuple_type) {
...
toolchain::SmallVector<Value> HloFunctionImporter::FlattenTupleValues(
    mlir::OpBuilder* func_builder, mlir::Location loc, mlir::ValueRange values,
    std::optional<int> reserve_size) {
  toolchain::SmallVector<Value> flattened_values;
  if (reserve_size) {
...
absl::StatusOr<FuncOp> HloFunctionImporter::ImportAsFunc(
  ...
  if (flatten_computation_args_result_) {
    toolchain::SmallVector<Type> flattened_args;
    for (Type type : args) {
      FlattenTupleType(type, flattened_args);
    }
    toolchain::SmallVector<Type> flattened_rets;
  ...
  for (auto instruction : computation.parameter_instructions()) {
    ...
    if (flatten_computation_args_result_) {
      ...
      if (parameter->parameter_replicated_at_leaf_buffers().has_value()) {
        ...
        toolchain::SmallVector<bool> replicated_at_leaf_buffers;
        for (auto b :
  ...
  if (!is_token && computation.root_instruction()->has_sharding()) {
    ...
    for (const auto& [ret_index, ret_sharding] :
         toolchain::enumerate(ret_shardings)) {
...
absl::Status HloFunctionImporter::ImportAsRegion(
  ...
  if (!toolchain::isa<FuncOp>(region->getParentOp()) ||
      flatten_computation_args_result_) {
    for (auto arg : args) {
      toolchain::SmallVector<Type> flattened_arg_types;
...
absl::StatusOr<Value> HloFunctionImporter::ImportInstructionsImpl(
    ...
    const toolchain::SmallVectorImpl<Value>& arguments, mlir::OpBuilder* builder) {
...
absl::Status HloFunctionImporter::ImportInstructions(
  ...
  toolchain::SmallVector<Value, 4> arguments(block->args_begin(), block->args_end());
  mlir::OpBuilder builder = mlir::OpBuilder::atBlockEnd(block);

  mlir::Location loc = builder.getUnknownLoc();

  bool is_func = toolchain::isa<FuncOp>(block->getParentOp());
  ...
  if (flatten_region_arg_tuple) {
    ...
    // otherwise.
    toolchain::SmallVector<Value> effective_arguments;

    toolchain::SmallVector<Type> computation_arg_types;
    ...
    for (Type computation_arg_type : computation_arg_types) {
      ...
      // using arg2 and arg3.
      toolchain::SmallVector<Type> flattened_arg_type;
      FlattenTupleType(orig_tuple_arg_type, flattened_arg_type);

      mlir::ValueRange sub_args(toolchain::ArrayRef<Value>(
  ...
  if (flatten_region_arg_tuple) {
    ...
    toolchain::SmallVector<Value> flattened_return_operands;
...
absl::StatusOr<Value> HloFunctionImporter::ImportInstructions(
    const HloComputation& computation,
    const toolchain::SmallVectorImpl<Value>& arguments,
...
absl::StatusOr<mlir::Operation*> HloFunctionImporter::ImportInstructionImpl(
  ...
  toolchain::SmallVector<NamedAttribute, 10> attributes;
  ...
  toolchain::SmallVector<NamedAttribute, 4> frontend_attributes;
  ...
  switch (instruction->opcode()) {
    ...
    case HloOpcode::kAsyncDone: {
      ...
      if (instruction->opcode() == HloOpcode::kAsyncStart) {
        auto bundle_result_type = mlir::mhlo::AsyncBundleType::get(
            context_, toolchain::cast<mlir::TupleType>(result_type).getTypes());
    ...
    case HloOpcode::kBatchNormTraining:
      ...
      if (instruction->opcode() == HloOpcode::kBatchNormGrad) {
        ...
        toolchain::SmallVector<Type> flattened_ret_types;
    ...
    case HloOpcode::kRaggedAllToAll: {
      toolchain::SmallVector<NamedAttribute> backendConfigAttributes;
    ...
    case HloOpcode::kCall: {
      ...
      if (instruction->is_composite()) {
        ...
        toolchain::SmallVector<NamedAttribute, 4> fe_attrs_without_composite_attrs;
      ...
      toolchain::SmallVector<Value> flattened_operands = FlattenTupleValues(
    ...
    case HloOpcode::kCustomCall: {
      ...
      if (!called_computations.empty()) {
        toolchain::SmallVector<mlir::Attribute> callees;
    ...
    case HloOpcode::kDynamicSlice: {
      ...
      return func_builder
          ->create<mlir::stablehlo::DynamicSliceOp>(
              ...
              toolchain::ArrayRef(operands).drop_front(), ConvertArray(slice_sizes))
          .getOperation();
    }
    case HloOpcode::kDynamicUpdateSlice: {
      return func_builder
          ->create<mlir::stablehlo::DynamicUpdateSliceOp>(
              ...
              toolchain::ArrayRef<Value>(operands.begin() + 2, operands.end()))
          .getOperation();
    }
    case HloOpcode::kInfeed: {
      ...
      toolchain::SmallVector<mlir::Attribute> flattened_attr;
      TF_RETURN_IF_ERROR(
          ConvertShapeToMlirLayout(instruction->shape(), flattened_attr));
      attributes.push_back(builder_->getNamedAttr(
          "layout", builder_->getArrayAttr(toolchain::ArrayRef(flattened_attr))));
    ...
    case HloOpcode::kOutfeed: {
      ...
      toolchain::SmallVector<Value> flattened_operands;
    ...
    case HloOpcode::kPad: {
      ...
      toolchain::SmallVector<int64_t, 4> edge_padding_low;
      toolchain::SmallVector<int64_t, 4> edge_padding_high;
      toolchain::SmallVector<int64_t, 4> interior_padding;
    ...
    case HloOpcode::kScatter: {
      ...
      toolchain::SmallVector<Type> flattened_types;
    ...
    case HloOpcode::kSelectAndScatter: {
      ...
      toolchain::SmallVector<int64_t, 4> window_strides, window_dimensions;
    ...
    case HloOpcode::kSort: {
      ...
      toolchain::SmallVector<Type, 4> return_types = {result_type};
      if (mlir::TupleType tuple_ty =
              mlir::dyn_cast<mlir::TupleType>(result_type)) {
        return_types = toolchain::to_vector<6>(tuple_ty.getTypes());
      }
    ...
    case HloOpcode::kConditional: {
      toolchain::SmallVector<Type, 4> rets;
      toolchain::SmallVector<Value> flattened_operands =
      ...
      toolchain::ArrayRef<Value> implicit_operands(flattened_operands.begin() + 1,
      ...
      for (const auto& index_and_computation :
           toolchain::enumerate(instruction->branch_computations())) {
    ...
    case HloOpcode::kAllGather: {
      ...
      toolchain::SmallVector<Type> result_types = {result_type};
      if (result_tuple_ty) {
        result_types = toolchain::to_vector(result_tuple_ty.getTypes());
      }
      attributes.push_back(builder_->getNamedAttr(
          "all_gather_dim",
    ...
    case HloOpcode::kAllToAll: {
      ...
      auto replica_groups_attr = toolchain::cast<DenseIntElementsAttr>(
      ...
      auto result_tuple_ty = toolchain::dyn_cast<mlir::TupleType>(result_type);
      if (result_tuple_ty) {
        ...
        toolchain::SmallVector<Type, 4> return_types =
            toolchain::to_vector<4>(result_tuple_ty.getTypes());
        mlir::mhlo::ChannelHandleAttr channel_handle_attr{};
        if (all_to_all->channel_id().has_value()) {
          channel_handle_attr = toolchain::cast<mlir::mhlo::ChannelHandleAttr>(
      ...
      if (all_to_all->channel_id().has_value()) {
        channel_handle_attr = toolchain::cast<mlir::stablehlo::ChannelHandleAttr>(
    ...
    case HloOpcode::kWhile: {
      toolchain::SmallVector<Value> flattened_operands;
      toolchain::SmallVector<Type> flattened_operand_types;
    ...
    case HloOpcode::kReduceWindow: {
      ...
      toolchain::SmallVector<int64_t, 4> sizes, strides, base_dilations,
          win_dilations;
      toolchain::SmallVector<int64_t, 8> padding;
    ...
    case HloOpcode::kConvolution: {
      toolchain::SmallVector<int64_t, 4> strides, lhs_dilations, rhs_dilations;
      toolchain::SmallVector<bool, 4> reversals;
      toolchain::SmallVector<int64_t, 8> paddings;
      for (const auto& dim : instruction->window().dimensions()) {
        strides.push_back(dim.stride());
...
HloFunctionImporter::ImportInstructionWithLayout(
    ...
    const toolchain::SmallVectorImpl<mlir::Value>& operands,
    mlir::OpBuilder* func_builder, DynamicShapeHandlingMode mode) {
  LLVM_DEBUG(toolchain::dbgs() << "Importing instruction: "
                          << HloOpcodeString(instruction->opcode()) << '\n');
  LLVM_DEBUG({
    toolchain::dbgs() << "  operands: (";
    toolchain::interleaveComma(operands, toolchain::dbgs(),
                          [](Value v) { toolchain::dbgs() << v.getType(); });
    toolchain::dbgs() << ")\n";
  });
  TF_ASSIGN_OR_RETURN(
      mlir::Operation * op,
  ...
  if (op == nullptr) {
    LLVM_DEBUG(toolchain::dbgs() << "  instruction skipped.\n");
    return op;
  }

  // Print generic in debug since module may be invalid while printing.
  LLVM_DEBUG({
    op->print(toolchain::dbgs(), mlir::OpPrintingFlags().printGenericOpForm());
    toolchain::dbgs() << "\n";
  });
...
absl::StatusOr<toolchain::SmallVector<mlir::Value, 4>>
HloFunctionImporter::GetOperands(const HloInstruction* instruction) {
  toolchain::SmallVector<mlir::Value, 4> operands;
...
mlir::DenseIntElementsAttr HloFunctionImporter::ConvertDimensions(
  ...
  toolchain::SmallVector<APInt, 8> dimensions;
...
mlir::NamedAttribute HloFunctionImporter::ConvertPadding(
    toolchain::ArrayRef<int64_t> padding) {
  auto ty =
...
absl::Status HloFunctionImporter::ConvertShapeToMlirLayout(
    const Shape& shape,
    toolchain::SmallVectorImpl<mlir::Attribute>& flattened_attr) {
  if (shape.IsToken()) {
    return absl::OkStatus();
  }
  if (shape.IsTuple()) {
  ...
  if (shape.IsArray()) {
    ...
    toolchain::ArrayRef<mlir::Attribute> array_ref(minor_to_major);
...
mlir::Attribute ConvertInputOutputAlias(const HloInputOutputAliasConfig& alias,
  ...
  toolchain::SmallVector<mlir::Attribute> element_attrs;
