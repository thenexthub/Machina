...
::mlir::DenseElementsAttr CreateDenseAttrFromLiteral(
  ...
  if constexpr (is_intN_v<CppType>) {
    ...
    std::vector<toolchain::APFloat> apfloats;
    apfloats.reserve(literal.element_count());
    for (size_t i = 0; i < literal.element_count(); i++) {
      toolchain::APFloat apfloat{static_cast<float>(data_span[i])};
      bool losesInfo;
      toolchain::APFloat::opStatus status =
          apfloat.convert(toolchain::APFloat::Float4E2M1FN(),
                          toolchain::APFloat::rmNearestTiesToEven, &losesInfo);
      CHECK_EQ(status, toolchain::APFloat::opOK)
    ...
    return ::mlir::DenseElementsAttr::get(
        type, toolchain::ArrayRef(data_span.data(), data_span.size()));
  }
}
...
absl::StatusOr<AffineMap> GetPermutationIfAvailable(const Shape& shape,
  ...
  toolchain::SmallVector<int64_t, 4> strides(shape.dimensions().size(), 1);
...
absl::StatusOr<mlir::MemRefType> ConvertTensorShapeToMemRefType(
  ...
  toolchain::SmallVector<int64_t, 4> array(dimensions.begin(), dimensions.end());
...
mlir::Value CreateTupleValue(mlir::OpBuilder* func_builder, mlir::Location loc,
  ...
  toolchain::SmallVector<mlir::Value> flatten_sub_values;
...
mlir::Operation* WrapVariadicResultsInTuple(mlir::OpBuilder* builder,
  ...
  assert(result_types.size() != 1 ||
         !toolchain::isa<mlir::TupleType>(result_types[0]) &&
...
bool IsEmptyTuple(const mlir::Type& type) {
  if (auto tuple_type = toolchain::dyn_cast<mlir::TupleType>(type)) {
...
mlir::TypeRange Untuple(const mlir::Type& type) {
  if (toolchain::isa<mlir::TupleType>(type)) {
    return toolchain::dyn_cast<mlir::TupleType>(type).getTypes();
  }
  return type;
}
