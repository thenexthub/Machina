...
#include "machina/xla/hlo/translate/hlo_to_mhlo/hlo_module_importer.h"
...
absl::Status HloModuleImporter::Import(const HloModule& hlo_module) {
  auto module = toolchain::cast<mlir::ModuleOp>(symbol_table_.getOp());
  ...
  if (!import_all_computation_) {
    ...
    LLVM_DEBUG(toolchain::dbgs() << "Emit StableHLO: " << emit_stablehlo_ << "\n");
    if (!emit_stablehlo_) {
