...
#ifndef MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
#define MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
...
mlir::DenseIntElementsAttr CreateDenseIntElementsAttrFromVector(
    const toolchain::ArrayRef<int64_t> vector, mlir::Builder builder,
...
static absl::StatusOr<TypeT> ConvertTensorShapeToType(const Shape& xla_ty,
  ...
  toolchain::SmallVector<int64_t, 4> shape(rank, mlir::ShapedType::kDynamic);
  toolchain::SmallVector<int64_t, 4> bounds(rank, mlir::ShapedType::kDynamic);
...
static absl::StatusOr<mlir::Type> ConvertShapeToType(const Shape& shape,
                                                     mlir::Builder builder) {
  if (shape.IsTuple()) {
    toolchain::SmallVector<mlir::Type, 4> contents;
...
static std::pair<mlir::Attribute, mlir::ArrayAttr> GetLayoutAttribute(
  ...
  if (shape.IsTuple()) {
    ...
    toolchain::SmallVector<mlir::Attribute> tile_attrs;
  ...
  toolchain::SmallVector<mlir::Attribute> vec_of_tiles;
  toolchain::SmallVector<int64_t> layout_vec;
  if (maybe_layout.has_value() || shape.has_layout()) {
    ...
    for (const Tile& tile : layout.tiles()) {
      toolchain::SmallVector<int64_t> tile_vec = {tile.dimensions().begin(),
                                             tile.dimensions().end()};
...
static bool HasCustomLayout(const Shape& shape) {
  if (shape.IsTuple()) {
    return toolchain::any_of(shape.tuple_shapes(), HasCustomLayout);
  }
  return shape.has_layout() && !shape.layout().minor_to_major().empty() &&
         shape.layout() != LayoutUtil::GetDefaultLayoutForShape(shape);
}

inline toolchain::StringRef ToStringRef(absl::string_view str) {
  return toolchain::StringRef(str.data(), str.size());
}

}  // namespace xla

#endif  // MACHINA_MACHINA_XLA_HLO_TRANSLATE_HLO_TO_MHLO_HLO_UTILS_H_
