...
Shape TypeToShape(mlir::Type type) {
  ...
  if (type.isIntOrFloat()) {
    ...
    toolchain::SmallVector<int64_t, 4> span(v.getShape().begin(),
                                       v.getShape().end());
    ...
    toolchain::SmallVector<int64_t, 4> bounds;
    if (auto extn = mlir::dyn_cast_or_null<mlir::mhlo::TypeExtensionsAttr>(
            t.getEncoding())) {
      bounds = toolchain::to_vector<4>(extn.getBounds());
    } else if (auto extn =
    ...
    if (auto sparse = mlir::sparse_tensor::getSparseTensorEncoding(type)) {
      ...
      auto final_ordering = mlir::applyPermutationMap(
          dimToLvl, toolchain::ArrayRef<int64_t>(ordering));
    ...
    toolchain::SmallVector<Shape, 4> shapes;
