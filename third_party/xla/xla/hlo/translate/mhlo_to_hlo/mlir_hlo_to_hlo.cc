...
#include "machina/xla/hlo/translate/mhlo_to_hlo/mlir_hlo_to_hlo.h"
...
#include "machina/xla/hlo/builder/lib/approx_topk.h"
#include "machina/xla/hlo/builder/lib/approx_topk_shape.h"
#include "machina/xla/hlo/builder/lib/matrix.h"  // IWYU pragma: keep
...
#include "machina/xla/hlo/ir/dynamic_parameter_binding.h"
...
#include "machina/xla/hlo/translate/attributes.h"
#include "machina/xla/hlo/translate/hlo_to_mhlo/hlo_utils.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/attribute_exporter.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/layout_util.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/literal_exporter.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/location_exporter.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/module_attributes_exporter.h"
#include "machina/xla/hlo/translate/mhlo_to_hlo/stack_frame_index_builder.h"
...
#include "machina/xla/mlir/utils/error_util.h"
#include "machina/xla/mlir/utils/type_util.h"
#include "machina/xla/mlir_hlo/mhlo/IR/hlo_ops.h"
#include "machina/xla/mlir_hlo/mhlo/transforms/passes.h"
#include "machina/xla/mlir_hlo/stablehlo_ext/transforms/passes.h"
...
#include "machina/xla/service/source_target_pairs.h"
...
static mlir::LogicalResult GetXlaOp(
    mlir::Value val, const toolchain::DenseMap<mlir::Value, xla::XlaOp>& val_map,
...
static int ConvertAPInt(toolchain::APInt i) { return i.getSExtValue(); }
...
static double ConvertAPFloat(toolchain::APFloat value) {
  const auto& semantics = value.getSemantics();
  bool losesInfo = false;
  if (&semantics != &toolchain::APFloat::IEEEdouble())
    value.convert(toolchain::APFloat::IEEEdouble(),
                  toolchain::APFloat::rmNearestTiesToEven, &losesInfo);
...
static std::vector<xla::Shape> ConvertTypesToShapesWithLayout(
  ...
  for (auto [type, layout] : toolchain::zip(value_types, layouts)) {
...
static xla::Layout ExtractLayout(
    ...
    toolchain::StringRef attr_name = kDefaultLayoutAttrName) {
  if (auto attr = op->getAttrOfType<mlir::DenseIntElementsAttr>(attr_name)) {
    toolchain::SmallVector<int64_t, 4> minor_to_major;
    DCHECK_EQ(rank, attr.size());
    minor_to_major.reserve(attr.size());
    for (const toolchain::APInt& i : attr) {
...
static mlir::FailureOr<xla::Shape> ExtractXlaShape(mlir::Operation* op) {
  if (auto attr = op->getAttrOfType<mlir::StringAttr>(kDefaultLayoutAttrName)) {
    ...
    for (auto [index, result] : toolchain::enumerate(op->getResults())) {
...
#define I64_ARRAY_ATTR_TO_VECTOR(attribute)               \
  static std::vector<int64_t> Convert_##attribute(        \
      std::optional<toolchain::ArrayRef<int64_t>> attribute) { \
...
static std::vector<int64_t> Convert_ArrayRef(toolchain::ArrayRef<int64_t> values) {
  return {values.begin(), values.end()};
}
...
static xla::ComparisonDirection Convert_comparison_direction(
    toolchain::StringRef comparison_direction_string) {
...
static void ExtractFrontendAttributesFromFunction(
    ...
    toolchain::SmallVectorImpl<std::optional<xla::FrontendAttributes>>* fe_attrs) {
...
static bool SomeOptionalShardingsAreSet(
    toolchain::ArrayRef<std::optional<xla::OpSharding>> shardings) {
  return toolchain::any_of(shardings,
...
static void ExtractShardingsFromFunction(
    mlir::func::FuncOp function,
    toolchain::SmallVectorImpl<std::optional<xla::OpSharding>>* arg_shardings,
    toolchain::SmallVectorImpl<std::optional<xla::OpSharding>>* ret_shardings) {
...
std::optional<xla::OpSharding> CreateTupleSharding(
    toolchain::ArrayRef<std::optional<xla::OpSharding>> tuple_shardings) {
...
xla::XlaOp CreateTupleIfMultipleOps(
    xla::XlaBuilder* builder, toolchain::ArrayRef<xla::XlaOp> ops,
    toolchain::ArrayRef<std::optional<xla::OpSharding>> shardings) {
  if (ops.size() == 1) {
    return ops[0];
  }
...
//   `op_sharding`.
toolchain::SmallVector<std::optional<xla::OpSharding>> GetResultShardings(
    std::optional<xla::OpSharding> op_sharding, int64_t num_results) {
  if (!op_sharding) {
    return {};
  }
  toolchain::SmallVector<std::optional<xla::OpSharding>> res_shardings;
...
toolchain::SmallVector<std::optional<xla::OpSharding>> GetXlaOpShardings(
    toolchain::ArrayRef<xla::XlaOp> xla_ops) {
  toolchain::SmallVector<std::optional<xla::OpSharding>> shardings;
...
class ConvertToHloModule {
 public:
  using ValueLoweringMap = toolchain::DenseMap<Value, xla::XlaOp>;
  using FunctionLoweringMap =
      toolchain::DenseMap<mlir::func::FuncOp, xla::XlaComputationId>;
  ...
  LogicalResult LowerRegionAsComputation(
      ...
      toolchain::ArrayRef<mlir::Value> implicit_operands = {},
      toolchain::ArrayRef<mlir::Value> implicit_results = {},
      bool ensure_single_arg = false,
      toolchain::ArrayRef<std::optional<xla::OpSharding>> arg_shardings = {},
      toolchain::ArrayRef<std::optional<xla::OpSharding>> ret_shardings = {});
  ...
  LogicalResult LowerBasicBlockAsFunction(
      ...
      toolchain::ArrayRef<std::optional<xla::OpSharding>> arg_shardings,
      toolchain::ArrayRef<std::optional<xla::OpSharding>> ret_shardings,
      toolchain::ArrayRef<std::optional<xla::FrontendAttributes>> fe_attrs,
  ...
  LogicalResult LowerReturn(
      ...
      toolchain::ArrayRef<std::optional<xla::OpSharding>> ret_shardings,
      toolchain::ArrayRef<mlir::Value> implicit_results, xla::XlaBuilder* builder,
      ConvertToHloModule::ValueLoweringMap* value_lowering,
 ...
 private:
  LogicalResult SetEntryTupleShapesAndLeafReplication(
      ...
      toolchain::SmallVectorImpl<xla::Shape>* arg_shapes,
...
struct OpLoweringContext {
  toolchain::DenseMap<mlir::Value, xla::XlaOp>* values;
...
mlir::LogicalResult GetTuple(mlir::Operation* op,
                             ...
                             toolchain::SmallVectorImpl<xla::XlaOp>& results) {
...
mlir::LogicalResult GetXlaOps(mlir::Operation* op,
                              toolchain::ArrayRef<mlir::Value> values,
                              OpLoweringContext ctx,
                              toolchain::SmallVectorImpl<xla::XlaOp>& results) {
  results.reserve(values.size());
  for (mlir::Value value : values) {
...
bool SimplyReturnedOp(mlir::Operation* op) {
  for (auto operand : op->getOperands()) {
    if (!toolchain::isa<mlir::BlockArgument>(operand)) return false;
  }
  ...
  if (toolchain::isa<mlir::func::ReturnOp>(first_user)) return true;
  return false;
}
...
LogicalResult ExportXlaOp(AllGatherOp op, OpLoweringContext ctx) {
  ...
  for (const auto& indexed_pair :
       toolchain::enumerate(toolchain::zip(op.getOperandTypes(), op.getResultTypes()))) {
...
LogicalResult ExportXlaOp(SendOp op, OpLoweringContext ctx) {
  ...
  toolchain::SmallVector<xla::XlaOp> operands;
...
LogicalResult ExportXlaOp(RecvOp op, OpLoweringContext ctx) {
  ...
  std::vector<xla::Shape> subshapes;
  for (const auto& item : toolchain::enumerate(result_types)) {
  ...
  if (subshapes.size() == 1) {
    ...
    for (const auto& item : toolchain::enumerate(op.getResults())) {
      if (item.index() == num_results - 1) break;
...
LogicalResult ExportXlaOp(IfOp op, OpLoweringContext ctx) {
  ...
  toolchain::SetVector<mlir::Value> implicit_true_operand_set,
  ...
  toolchain::SmallVector<mlir::Value> implicit_true_operands =
      implicit_true_operand_set.takeVector();
  toolchain::SmallVector<mlir::Value> implicit_false_operands =
      implicit_false_operand_set.takeVector();

  toolchain::SmallVector<std::optional<xla::OpSharding>> ret_shardings =
      GetResultShardings(ctx.builder->sharding(), op->getNumResults());

  toolchain::SmallVector<xla::XlaOp> true_args;
  if (failed(GetXlaOps(op, implicit_true_operands, ctx, true_args)))
    return failure();

  toolchain::SmallVector<xla::XlaOp> false_args;
  if (failed(GetXlaOps(op, implicit_false_operands, ctx, false_args)))
    return failure();

  toolchain::SmallVector<std::optional<xla::OpSharding>> true_arg_shardings,
      false_arg_shardings;
...
LogicalResult ExportXlaOp(CaseOp op, OpLoweringContext ctx) {
  toolchain::DenseMap<mlir::Value, xla::XlaOp>& value_map = *ctx.values;
  // OperandRange operands = op.branch_operands();
  MutableArrayRef<Region> branches = op.getBranches();
  toolchain::SmallVector<xla::XlaOp, 4> branch_operands(branches.size());
  ...
  for (unsigned i = 0; i < branches.size(); ++i) {
    toolchain::SetVector<mlir::Value> implicit_operand_set;
    getUsedValuesDefinedAbove(branches[i], branches[i], implicit_operand_set);
    toolchain::SmallVector<mlir::Value> implicit_operands =
    ...
    toolchain::SmallVector<xla::XlaOp> args;
    if (failed(GetXlaOps(op, implicit_operands, ctx, args))) return failure();

    toolchain::SmallVector<std::optional<xla::OpSharding>> arg_shardings;
...
LogicalResult ExportXlaOp(WhileOp op, OpLoweringContext ctx) {
  ...
  toolchain::SmallVector<xla::XlaOp> implicit_args;
  ...
  toolchain::SmallVector<std::optional<xla::OpSharding>> implicit_shardings;
...
LogicalResult ExportXlaOp(ReduceOp op, OpLoweringContext ctx) {
  ...
  toolchain::SmallVector<xla::XlaOp> operands, init_values;
...
LogicalResult ExportXlaOp(CustomCallOp op, OpLoweringContext ctx) {
  ...
  if (*xla_api_version == xla::CustomCallApiVersion::API_VERSION_TYPED_FFI) {
    ...
    if (auto dict = mlir::dyn_cast_or_null<mlir::DictionaryAttr>(
      ...
      toolchain::raw_string_ostream(backend_config) << dict;
    }
  } else {
    // Forward backend config string to the HLO instruction.
    if (auto str = mlir::dyn_cast_or_null<mlir::StringAttr>(
            op.getBackendConfig().value_or(mlir::Attribute()))) {
      toolchain::raw_string_ostream(backend_config) << str.strref();
    }
  }
...
LogicalResult ExportXlaOp(ScatterOp op, OpLoweringContext ctx) {
  ...
  toolchain::SmallVector<xla::XlaOp> operands;
  toolchain::SmallVector<xla::XlaOp> updates;
  if (failed(GetTuple(op, op.getInputs(), ctx, operands))) return failure();
...
mlir::LogicalResult ExportXlaOp(mlir::stablehlo::SetDimensionSizeOp op,
  ...
  xla::XlaOp xla_result;
  if (auto constant = toolchain::dyn_cast_or_null<mlir::mhlo::ConstantOp>(
...
LogicalResult ExportXlaOp(FusionOp op, OpLoweringContext ctx) {
  ...
  auto aliasInfo =
      xla::ConvertOutputOperandAliasing<mlir::mhlo::OutputOperandAliasAttr>(
          op.getOutputOperandAliases());
  auto output_operand_aliasing = absl::MakeSpan(*aliasInfo);
  toolchain::SmallVector<xla::XlaOp, 4> operands;
...
#include "machina/xla/hlo/translate/mhlo_to_hlo/hlo_op_writer.inc"
...
LogicalResult ConvertToHloModule::LowerStablehloCompositeCall(
  ...
  toolchain::raw_string_ostream(composite_attributes)
...
LogicalResult ConvertToHloModule::LowerReturn(
  ...
  if ((options_.return_tuple && is_entry_function) || num_return_values != 1) {
    ...
    // NOTE: we can't use operand_range in toolchain::concat.
...
LogicalResult ConvertToHloModule::RunOnFunction(mlir::func::FuncOp f) {
  ...
  if (!toolchain::hasSingleElement(f)) {
  ...
  toolchain::SmallVector<std::optional<xla::OpSharding>, 4> arg_shardings;
  toolchain::SmallVector<std::optional<xla::OpSharding>, 4> ret_shardings;
  toolchain::SmallVector<std::optional<xla::FrontendAttributes>, 4> arg_fe_attrs;
  if (entry_function) {
...
LogicalResult ConvertToHloModule::SetEntryTupleShardings(
  ...
  if (!arg_shardings.empty() && SomeOptionalShardingsAreSet(arg_shardings)) {
    xla::OpSharding sharding;
    sharding.set_type(xla::OpSharding::TUPLE);
    for (const auto& arg_sharding : toolchain::enumerate(arg_shardings)) {
...
LogicalResult ConvertToHloModule::LowerBasicBlockAsFunction(
  ...
  if (is_entry_function && options_.use_tuple_args) {
    toolchain::SmallVector<xla::Shape, 4> arg_shapes;
    std::vector<bool> leaf_replication;
    ...
    if (ensure_single_arg) {
      ...
      if (args_size > 1) {
        ...
        for (auto [implicit_index, implicit_operand] :
             toolchain::enumerate(implicit_operands)) {
  ...
  // LLVM_DEBUG(toolchain::dbgs() << "Created: " << result->name() << "\n");
  return success();
}
...
LogicalResult ConvertToHloModule::LowerRegionAsComputation(
    mlir::Region* region, xla::XlaComputationId& func,
    toolchain::ArrayRef<mlir::Value> implicit_operands,
    toolchain::ArrayRef<mlir::Value> implicit_results, bool ensure_single_arg,
    toolchain::ArrayRef<std::optional<xla::OpSharding>> arg_shardings,
    toolchain::ArrayRef<std::optional<xla::OpSharding>> ret_shardings) {
...
absl::Status BuildHloFromMlirHlo(mlir::Block& block, xla::XlaBuilder& builder,
                                 toolchain::ArrayRef<xla::XlaOp> xla_params,
                                 std::vector<xla::XlaOp>& returns,
                                 MlirToHloConversionOptions options) {
