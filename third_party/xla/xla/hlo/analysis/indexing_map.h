...
#ifndef MACHINA_MACHINA_XLA_HLO_ANALYSIS_INDEXING_MAP_H_
#define MACHINA_MACHINA_XLA_HLO_ANALYSIS_INDEXING_MAP_H_
...
// For use in toolchain::hash_combine.
inline size_t hash_value(const Interval& range) {
  return toolchain::hash_combine(range.lower, range.upper);
}
...
class IndexingMap {
 public:
  ...
  struct Variable {
    Variable() = default;
    explicit Variable(Interval bounds, toolchain::StringRef name = "")
        : bounds(bounds), name(name) {}
    Variable(int64_t lb, int64_t ub, toolchain::StringRef name = "")
  ...
  IndexingMap(mlir::AffineMap affine_map, std::vector<Variable> dimensions,
              std::vector<Variable> range_vars, std::vector<Variable> rt_vars,
              const toolchain::MapVector<mlir::AffineExpr, Interval>& constraints);
  ...
  const toolchain::MapVector<mlir::AffineExpr, Interval>& GetConstraints() const {
    return constraints_;
  }
  ...
  bool ConstraintsSatisfied(
      toolchain::ArrayRef<mlir::AffineExpr> dim_const_exprs,
      toolchain::ArrayRef<mlir::AffineExpr> symbol_const_exprs) const;

  // Evaluates indexing map results at a given point.
  toolchain::SmallVector<int64_t, 4> Evaluate(
      toolchain::ArrayRef<mlir::AffineExpr> dim_const_exprs,
      toolchain::ArrayRef<mlir::AffineExpr> symbol_const_exprs) const;
  ...
  toolchain::SmallBitVector RemoveUnusedSymbols();
  ...
  toolchain::SmallBitVector RemoveUnusedVars();
 ...
 private:
  ...
  bool CompressVars(const toolchain::SmallBitVector& unused_dims,
  ...
  toolchain::MapVector<mlir::AffineExpr, Interval> constraints_;
...
inline size_t hash_value(const IndexingMap::Variable& dim_var) {
  return toolchain::hash_combine(dim_var.bounds);
}
...
H AbslHashValue(H h, const IndexingMap& indexing_map) {
  toolchain::hash_code affine_map_hash =
      toolchain::hash_combine(indexing_map.GetAffineMap());
  toolchain::SmallVector<size_t> constraint_hashes;
  constraint_hashes.reserve(indexing_map.GetConstraintsCount());
  for (const auto& [expr, interval] : indexing_map.GetConstraints()) {
    constraint_hashes.push_back(toolchain::hash_combine(expr, interval));
  }
...
IndexingMap ConvertRangeVariablesToDimensions(
    const IndexingMap& map, toolchain::ArrayRef<int64_t> range_var_indices);

}  // namespace xla

#endif  // MACHINA_MACHINA_XLA_HLO_ANALYSIS_INDEXING_MAP_H_
