...
HloInstructionIndexing ComputeInputToOutputBroadcastOpIndexing(
  ...
  for (auto [output_dim_id, output_dim] :
       toolchain::enumerate(output_shape.dimensions())) {
    auto bcast_dim =
...
HloInstructionIndexing ComputeOutputToInputConcatenateOpIndexing(
  ...
  int64_t offset = 0;
  for (const auto [operand_id, operand] : toolchain::enumerate(concat->operands())) {
...
HloInstructionIndexing ComputeOutputToInputFusionOpIndexing(
  ...
  for (auto [operand_id, operand] : toolchain::enumerate(fusion->operands())) {
...
HloInstructionIndexing ComputeOutputToInputDotOpIndexing(
  ...
  for (auto [lhs_batch_dim, rhs_batch_dim] :
       toolchain::zip(lhs_batch_dims, rhs_batch_dims)) {
  ...
  for (auto [lhs_contracting_dim, rhs_contracting_dim] :
       toolchain::zip(lhs_contracting_dims, rhs_contracting_dims)) {
...
HloInstructionIndexing ComputeOutputToInputDynamicSliceOpIndexing(
  ...
  for (auto [dim, slice_size] :
       toolchain::enumerate(dynamic_slice->dynamic_slice_sizes())) {
...
HloInstructionIndexing ComputeOutputToInputDynamicUpdateSliceOpIndexing(
  ...
  for (auto [dim, slice_size] : toolchain::enumerate(update_shape.dimensions())) {
    exprs.push_back(getAffineDimExpr(dim, mlir_context) -
                    getAffineSymbolExpr(dim, mlir_context));
...
HloInstructionIndexing ComputeOutputToInputGatherOpIndexing(
  ...
  for (auto [operand_dim_id, slice_size] :
       toolchain::enumerate(gather->gather_slice_sizes())) {
...
IndexingMap ComputeOutputToInputPadOpIndexingImpl(
  ...
  for (const auto [output_dim, pad_low, pad_high, pad_interior] :
       toolchain::zip(output_dims, padding_low, padding_high, padding_interior)) {
...
HloInstructionIndexing ComputeOutputToInputReduceOpIndexing(
  ...
  for (auto [input_dim_id, input_dim] :
       toolchain::enumerate(input_shape.dimensions())) {
...
IndexingMap ComposeIndexingMapsForWindow(
  ...
  for (const auto& [dim_id, window_config] :
       toolchain::enumerate(window.dimensions())) {
...
HloInstructionIndexing ComputeOutputToInputConvolutionOpIndexing(
  ...
  toolchain::MapVector<AffineExpr, Interval> input_constraints;
...
AffineExpr LinearizeShape(absl::Span<const int64_t> dims,
  ...
  for (auto [stride, dimension_expr] : toolchain::zip(strides, dimension_exprs)) {
...
HloInstructionIndexing ComputeReverseOpIndexing(
  ...
  for (auto [output_dim_id, output_dim] : toolchain::enumerate(output_dims)) {
...
toolchain::SmallVector<AffineExpr, 4> DelinearizeInBoundsIndex(
    AffineExpr linear, absl::Span<const int64_t> sizes) {
  toolchain::SmallVector<AffineExpr, 4> result;
  ...
  for (auto [size, stride] : toolchain::zip(sizes, strides)) {
...
HloInstructionIndexing HloInstructionIndexing::FromIndexingMaps(
  ...
  for (const auto& [index, map] : toolchain::enumerate(indexing_maps)) {
...
HloInstructionIndexing HloInstructionIndexing::FromOperandIndexing(
  ...
  for (const auto& [index, rt_map] : toolchain::enumerate(operand_indexing)) {
...
std::string HloInstructionIndexing::ToString(
  ...
  for (const auto& [operand_id, operand_indexes] :
       toolchain::enumerate(indexing_maps)) {
...
GroupedByOpIndexing GroupIndexingMapsByProducers(
  ...
  for (const auto& [operand_id, indexing_maps] :
       toolchain::enumerate(indexing.indexing_maps)) {
...
GroupedByOpIndexing ComputeGroupedOutputToInputIndexing(
  ...
  for (; it != post_order.rend(); ++it) {
    ...
    for (const auto& [producer_operand_id, producer_operand_indexing] :
         toolchain::enumerate(producer_indexing.indexing_maps)) {
...
std::string OperandIndexing::ToString() const {
  ...
  if (!rt_vars_.empty()) {
    ...
    for (const auto& [id, rt_var] : toolchain::enumerate(rt_vars_)) {
...
toolchain::SmallBitVector OperandIndexing::RemoveUnusedSymbols() {
  const int64_t range_vars_count = map_.GetRangeVarsCount();
  toolchain::SmallBitVector removed = map_.RemoveUnusedSymbols();
  std::vector<RuntimeVarIndexing> updated_rt_vars;
  updated_rt_vars.reserve(rt_vars_.size());
  for (const auto& [i, rt] : toolchain::enumerate(rt_vars_)) {
