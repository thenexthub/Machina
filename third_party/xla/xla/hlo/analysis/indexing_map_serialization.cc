...
class Parser {
 public:
  explicit Parser(toolchain::StringRef input) : input_(input), it_(input.begin()) {
  ...
  bool ParseCommaSeparatedVarList(
      Delimeter delimeter,
      toolchain::function_ref<bool(Parser& parser)> parse_element_fn);
 ...
 private:
  ...
  toolchain::StringRef input_;
  toolchain::StringRef::iterator it_;
  Token current_token_;
};

bool Parser::ParseVarName(std::string* var_name) {
  if (current_token_.kind != Token::Kind::kVarName) {
    toolchain::errs() << "Expected var name, got: " << current_token_.spelling
                 << "\n";
    return false;
  }
...
bool Parser::ParseInt(int64_t* value) {
  if (current_token_.kind != Token::Kind::kIntLiteral ||
    ...
    toolchain::errs() << "Expected int literal, got: " << current_token_.spelling
                 << "\n";
    return false;
  }
  Advance();
...
bool Parser::ParseBool(bool* boolean) {
  if (current_token_.kind != Token::Kind::kBoolLiteral) {
    toolchain::errs() << "Expected bool literal, got: " << current_token_.spelling
                 << "\n";
    return false;
  }
...
std::pair<Token::Kind, Token::Kind> Parser::GetDelimiterPair(
  ...
  switch (delimeter) {
    ...
    default:
      toolchain::errs() << "Unsupported delimiter: " << static_cast<int>(delimeter)
                   << "\n";
...
bool ParseAffineExprsWithMLIR(ArrayRef<std::string> dim_var_names,
  ...
  if (!affine_map_attr) {
    toolchain::errs() << "Failed to parse affine map: " << ss.str() << "\n";
    return false;
  }
  AffineMap affine_map = mlir::cast<AffineMapAttr>(affine_map_attr).getValue();
  affine_exprs = toolchain::to_vector(affine_map.getResults());
  return true;
}
...
void PrintAffineExprImpl(const AffineExpr affine_expr,
                         ...
                         bool add_parentheses, toolchain::raw_ostream& os) {
...
std::optional<IndexingMap> ParseIndexingMap(toolchain::StringRef input,
                                            MLIRContext* context) {
  Parser parser(input);
  ...
  if (!ParseVarNames(parser, Delimeter::kParen, dim_var_names) ||
    ...
    toolchain::errs() << "Failed to parse variable names\n";
    return std::nullopt;
  }
  ...
  if (!parser.ConsumeToken(Token::Kind::kArrow) ||
    ...
    toolchain::errs() << "Failed to parse affine map results\n";
    return std::nullopt;
  }
  ...
  if (dim_var_names.empty() && range_var_names.empty() &&
    ...
    if (num_affine_map_results != 0 ||
      ...
      toolchain::errs() << "Expected an empty indexing map\n";
      return std::nullopt;
    }
  ...
  if (!parser.ConsumeToken(Token::Kind::kComma) ||
    ...
    toolchain::errs() << "Failed to parse domain keyword\n";
    return std::nullopt;
  }
  // Parse dimension variables.
  std::vector<IndexingMap::Variable> dim_vars;
  for (const auto& [dim_id, dim_name] : toolchain::enumerate(dim_var_names)) {
    std::string var_name;
    Interval interval;
    if (!parser.ParseVarName(&var_name) ||
      ...
      toolchain::errs() << "Failed to parse DimVar " << dim_name << " interval\n";
      return std::nullopt;
    }
    if (var_name != dim_name) {
      toolchain::errs() << "Dimension name mismatch " << dim_name
                   << " != " << var_name << "\n";
      return std::nullopt;
    }
  ...
  for (const auto& [index, name] : toolchain::enumerate(range_var_names)) {
    std::string var_name;
    Interval interval;
    if (!parser.ParseVarName(&var_name) ||
        !parser.ConsumeToken(Token::Kind::kKeywordIn) ||
        ...
        (parser.GetCurrentToken().kind != Token::Kind::kEOF &&
         !parser.ConsumeToken(Token::Kind::kComma))) {
      toolchain::errs() << "Failed to parse RangeVar " << name << " interval\n";
      return std::nullopt;
    }
    if (var_name != name) {
      toolchain::errs() << "Range var name mismatch " << name << " != " << var_name
                   << "\n";
      return std::nullopt;
    }
  ...
  for (const auto& [index, name] : toolchain::enumerate(rt_var_names)) {
    std::string var_name;
    Interval interval;
    if (!parser.ParseVarName(&var_name) ||
        !parser.ConsumeToken(Token::Kind::kKeywordIn) ||
        ...
        (parser.GetCurrentToken().kind != Token::Kind::kEOF &&
         !parser.ConsumeToken(Token::Kind::kComma))) {
      toolchain::errs() << "Failed to parse RuntimeVar " << name << " interval\n";
      return std::nullopt;
    }
    if (var_name != name) {
      toolchain::errs() << "Runtime var name mismatch " << name << " != " << var_name
                   << "\n";
      return std::nullopt;
    }
  ...
  while (!parser.ConsumeToken(Token::Kind::kEOF)) {
    ...
    if (!parser.ParseAffineExprString(&affine_expr_str) ||
        ...
        (parser.GetCurrentToken().kind != Token::Kind::kEOF &&
         !parser.ConsumeToken(Token::Kind::kComma))) {
      toolchain::errs() << "Failed to parse constraint\n";
      return std::nullopt;
    }
    affine_expr_strs.push_back(affine_expr_str);
  ...
  if (!ParseAffineExprsWithMLIR(dim_var_names, symbol_var_names,
    ...
    toolchain::errs() << "Failed to parse affine expressions\n";
    return std::nullopt;
  }
  ...
  for (const auto& [expr, bounds] :
       toolchain::zip(constraint_exprs, constraint_bounds)) {
...
std::string ToString(AffineMap affine_map,
  ...
  toolchain::interleaveComma(affine_map.getResults(), ss, [&](AffineExpr expr) {
...
std::string ToString(const IndexingMap& indexing_map,
  ...
  for (const auto& [index, dim_var] : toolchain::enumerate(dim_vars)) {
    ss << dim_names[index] << " in " << dim_var.bounds;
    if (--remaining_vars_to_print > 0) {
      ss << ", ";
    }
  }
  for (const auto& [index, range_var] : toolchain::enumerate(range_vars)) {
    ss << symbol_names[index] << " in " << range_var.bounds;
    if (--remaining_vars_to_print > 0) {
      ss << ", ";
    }
  }
  for (const auto& [index, rt_var] : toolchain::enumerate(rt_vars)) {
...
SmallVector<std::string> GetRangeVarNames(const IndexingMap& map) {
  ...
  for (const auto& [index, range_var] : toolchain::enumerate(map.GetRangeVars())) {
...
SmallVector<std::string> GetRTVarNames(const IndexingMap& map) {
  ...
  for (const auto& [index, rt_var] : toolchain::enumerate(map.GetRTVars())) {
...
SmallVector<std::string> GetDimVarNames(const IndexingMap& map) {
  ...
  for (const auto& [index, dim_var] : toolchain::enumerate(map.GetDimVars())) {
