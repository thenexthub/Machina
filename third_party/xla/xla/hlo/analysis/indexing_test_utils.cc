...
#include "machina/xla/hlo/analysis/indexing_test_utils.h"
...
std::optional<int64_t> SafeEvaluateAffineExpr(mlir::AffineExpr expr,
  ...
  switch (binary.getKind()) {
    case mlir::AffineExprKind::Add:
      if (toolchain::AddOverflow(*lhs, *rhs, result)) {
        return std::nullopt;
      }
      return result;
    case mlir::AffineExprKind::Mul:
      if (toolchain::MulOverflow(*lhs, *rhs, result)) {
        return std::nullopt;
      }
      return result;
    case mlir::AffineExprKind::FloorDiv:
      return result_division_is_undefined
                 ? std::nullopt
                 : std::make_optional(toolchain::divideFloorSigned(*lhs, *rhs));
    case mlir::AffineExprKind::CeilDiv:
      return result_division_is_undefined
                 ? std::nullopt
                 : std::make_optional(toolchain::divideCeilSigned(*lhs, *rhs));
    case mlir::AffineExprKind::Mod:
      return rhs <= 0 ? std::nullopt
                      : std::make_optional(toolchain::mod(*lhs, *rhs));
    default:
  ...
  TF_RETURN_IF_ERROR(EnumerateDomain(
      ...
      [&](absl::Span<int64_t const> dims,
        ...
        for (auto [coordinate, interval] :
             toolchain::zip(codomain_point, expected_codomain)) {
