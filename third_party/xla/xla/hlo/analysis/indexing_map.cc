...
AffineExpr AffineExprSimplifier::RewriteMod(AffineBinaryOpExpr mod) {
  ...
  int64_t offset = toolchain::divideFloorSigned(lhs.lower, m) * -m;
...
mlir::AffineExpr AffineExprSimplifier::RewriteSum(
  ...
  toolchain::SmallDenseMap<AffineExpr, int64_t /* multiplier */> summands;
  ...
  toolchain::sort(expanded_summands,
    ...
    default: {
      ...
      switch (expr->getKind()) {
        case AffineExprKind::Mul: {
          ...
          range->lower = toolchain::divideCeilSigned(range->lower, factor);
          range->upper = toolchain::divideFloorSigned(range->upper, factor);
          *expr = lhs;
          return true;
        }
        case AffineExprKind::FloorDiv: {
...
Interval Interval::operator+(const Interval& rhs) const {
  ...
  bool lower_overflow = toolchain::AddOverflow(lower, rhs.lower, out_lower);
  bool upper_overflow = toolchain::AddOverflow(upper, rhs.upper, out_upper);
...
Interval Interval::FloorDiv(int64_t rhs) const {
  auto saturate_div = [](int64_t lhs, int64_t rhs) {
    ...
    return toolchain::divideFloorSigned(lhs, rhs);
  };
...
IndexingMap::IndexingMap(
    ...
    std::vector<IndexingMap::Variable> range_vars,
    std::vector<IndexingMap::Variable> rt_vars,
    const toolchain::MapVector<AffineExpr, Interval>& constraints)
    : affine_map_(affine_map),
      dim_vars_(std::move(dimensions)),
      range_vars_(std::move(range_vars)),
      rt_vars_(std::move(rt_vars)),
...
Interval RangeEvaluator::ComputeExpressionRange(AffineExpr expr) {
  ...
  switch (expr.getKind()) {
    ...
    case AffineExprKind::FloorDiv: {
      ...
      int64_t a = toolchain::divideFloorSigned(lhs.lower, d);
      int64_t b = toolchain::divideFloorSigned(lhs.upper, d);
...
namespace {
bool EqualConstraints(const toolchain::MapVector<mlir::AffineExpr, Interval>& lhs,
                      const toolchain::MapVector<mlir::AffineExpr, Interval>& rhs) {
...
struct UsedParameters {
  toolchain::DenseSet<int64_t> dimension_ids;
  toolchain::DenseSet<int64_t> symbol_ids;
};
...
bool IndexingMap::CompressVars(const toolchain::SmallBitVector& unused_dims,
                               const toolchain::SmallBitVector& unused_symbols) {
...
void IndexingMap::ResetToKnownEmpty() {
  ...
  affine_map_ = AffineMap::get(
      ...
      toolchain::SmallVector<AffineExpr>(affine_map_.getNumResults(), zero),
      GetMLIRContext());
...
bool IndexingMap::MergeModConstraints() {
  ...
  toolchain::MapVector<AffineExpr, toolchain::SmallVector<AffineBinaryOpExpr, 2>>
      grouped_constraints;
  for (const auto& [expr, _] : constraints_) {
  ...
  for (const auto& [lhs, binops] : grouped_constraints) {
    toolchain::MapVector<int64_t, toolchain::SmallVector<AffineBinaryOpExpr, 2>>
        mod_groups;
...
IndexingMap ComposeIndexingMaps(const IndexingMap& first,
  ...
  for (const IndexingMap::Variable& range_var :
       toolchain::concat<const IndexingMap::Variable>(second.GetRangeVars(),
                                                 first.GetRangeVars())) {
  ...
  for (const IndexingMap::Variable& rt_var :
       toolchain::concat<const IndexingMap::Variable>(second.GetRTVars(),
                                                 first.GetRTVars())) {
  ...
  for (const auto& [expr, range] :
       toolchain::zip(remapped_constraints.getResults(), constraints_ranges)) {
  ...
  for (auto [index, expr] :
       toolchain::enumerate(first.GetAffineMap().getResults())) {
...
bool IndexingMap::RescaleSymbols() {
  ...
  toolchain::DenseSet<AffineExpr> to_delete;
  toolchain::DenseMap<AffineExpr, AffineExpr> to_replace;
  ...
  toolchain::MapVector<mlir::AffineExpr, Interval> new_constraints;
  for (const auto& [expr, range] : constraints_) {
...
IndexingMap IndexingMap::ConvertSymbolsToDimensions() const {
  ...
  toolchain::append_range(new_dim_vars, GetDimVars());
  ...
  for (const IndexingMap::Variable& var :
       toolchain::concat<const IndexingMap::Variable>(range_vars_, rt_vars_)) {
