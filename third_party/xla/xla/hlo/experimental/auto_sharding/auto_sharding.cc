...
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_cost_graph.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_device_mesh.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_iopddl.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_memory.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_option.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_solver.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_strategy.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_util.h"
#include "machina/xla/hlo/experimental/auto_sharding/auto_sharding_wrapper.h"
#include "machina/xla/hlo/experimental/auto_sharding/cluster_environment.h"
#include "machina/xla/hlo/experimental/auto_sharding/matrix.h"
#include "machina/xla/hlo/experimental/auto_sharding/metrics.h"
#include "machina/xla/hlo/experimental/auto_sharding/profiling_result.h"
...
#include "machina/xla/hlo/utils/hlo_sharding_util.h"
#include "machina/xla/service/buffer_value.h"
#include "machina/xla/service/call_graph.h"
#include "machina/xla/service/computation_layout.h"
  ...
  switch (ins->opcode()) {
    case HloOpcode::kGather: {
      if (operand_shardings.shardings[0].has_value() &&
        ...
        // third_party/machina/compiler/xla/service/spmd/spmd_partitioner_test.cc),
...
absl::StatusOr<bool> AutoShardingImplementation::RunAutoSharding(
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(10, spmd::PrintLivenessSet(liveness_set));
  const HloInstructionSequence& sequence =
      hlo_live_range->flattened_instruction_sequence();
  ...
  for (size_t mesh_idx = 0; mesh_idx < partial_mesh_shapes.size(); ++mesh_idx) {
    ...
    MACHINA_MACHINA_XLA_VLOG_LINES(6, module->ToString());
    ...
    MACHINA_MACHINA_XLA_VLOG_LINES(8, PrintStrategyMap(strategy_map, sequence));
    ...
    MACHINA_MACHINA_XLA_VLOG_LINES(5, PrintAutoShardingSolution(
                          sequence, liveness_set, strategy_map, strategy_groups,
                          cost_graph, output.s_val, output.cost));
    MACHINA_MACHINA_XLA_VLOG_LINES(6, PrintSolutionMemoryUsage(liveness_set, strategy_map,
...
absl::Time DumpModuleAndRecordPassStart(const HloModule* module) {
  MACHINA_MACHINA_XLA_VLOG_LINES(6,
...
void RecordPassEndAndDumpModule(absl::Time start_time,
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(6, absl::StrCat("After auto sharding:\n", module->ToString()));
