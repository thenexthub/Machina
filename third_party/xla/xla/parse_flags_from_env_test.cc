...
static void TestParseFlagsFromEnv(const char* msg) {
  ...
  ResetFlagsFromEnvForTesting("TF_MACHINA_MACHINA_XLA_FLAGS", &pargc, &pargv);
  ...
  ParseFlagsFromEnvAndDieIfUnknown("TF_MACHINA_MACHINA_XLA_FLAGS", flag_list);
...
TEST(ParseFlagsFromEnv, Basic) {
  // Prepare environment.
  tsl::setenv("TF_MACHINA_MACHINA_XLA_FLAGS", kTestFlagString, true /*overwrite*/);
...
TEST(ParseFlagsFromEnv, File) {
  ...
  fclose(fp);
  // Prepare environment.
  tsl::setenv("TF_MACHINA_MACHINA_XLA_FLAGS", tmp_file.c_str(), true /*overwrite*/);
...
TEST(ParseFlagsFromEnv, EnvAndFlag) {
  ...
  for (int i = 0; i != TF_ARRAYSIZE(test); i++) {
    if (test[i].env == nullptr) {
      ...
      tsl::unsetenv("TF_MACHINA_MACHINA_XLA_FLAGS");
    } else {
      tsl::setenv("TF_MACHINA_MACHINA_XLA_FLAGS", test[i].env, /*overwrite=*/true);
    }
...
TEST(ParseFlagsFromEnv, ErrorOutOnFlagFailure) {
  ...
  if (env == nullptr) {
    ...
    tsl::setenv("TF_MACHINA_MACHINA_XLA_FLAGS", env, /*overwrite=*/true);
  }
  tsl::SubProcess child;
  std::vector<std::string> argv;
  argv.push_back(binary_name);
...
TEST(ParseFlagsFromEnv, UknownFlagErrorMessage) {
  ...
  EXPECT_THAT(
      stderr_str,
      ::testing::EndsWith("Unknown flags in TF_MACHINA_MACHINA_XLA_FLAGS: "
...
int main(int argc, char* argv[]) {
  ...
  xla::ParseFlagsFromEnvAndDieIfUnknown("TF_MACHINA_MACHINA_XLA_FLAGS", flag_list);
