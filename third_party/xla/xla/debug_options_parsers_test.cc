...
void SetXlaFlagsEnvVar(const std::string& value) {
  ...
  ResetFlagsFromEnvForTesting("MACHINA_MACHINA_XLA_FLAGS", &pargc, &pargv);
  tsl::setenv("MACHINA_MACHINA_XLA_FLAGS", value.c_str(), true /*overwrite*/);
}
...
TEST(FuelTest, FuelPassCountsAreSeparate) {
  tsl::setenv("MACHINA_MACHINA_XLA_FLAGS", "--xla_fuel=ABC=1,PQR=2", /*overwrite=*/true);
...
TEST(FuelTest,
  ...
  tsl::setenv("MACHINA_MACHINA_XLA_FLAGS", "--xla_fuel=MNO=1,XYZ=2", /*overwrite=*/true);
  // Parse flags from the environment variable.
  int* pargc;
  std::vector<char*>* pargv;
  ResetFlagsFromEnvForTesting("MACHINA_MACHINA_XLA_FLAGS", &pargc, &pargv);
...
TEST(ParsingDebugOptionsTest, EnvOverwritesDebugOptionsFile) {
  ...
  ResetFlagValues();
  tsl::setenv("MACHINA_MACHINA_XLA_FLAGS",
