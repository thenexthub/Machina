...
class CpuOptProvider : public CompiledOptProvider {
 public:
  ...
  std::set<std::string> SupportedStages() override {
    ...
    supported.insert({"toolchain-before-optimizations"});
    return supported;
  }
  ...
  void RegisterProviderPasses(HloModule& module) override {
    ...
    absl::StatusOr<std::unique_ptr<toolchain::TargetMachine>> jit_target_machine =
 ...
 private:
  toolchain::CodeGenOptLevel CodeGenOptLevel(const HloModuleConfig& module_config) {
