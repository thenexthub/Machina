...
#include "machina/xla/hlo/translate/mhlo_to_hlo/translate.h"
#include "machina/xla/hlo/translate/stablehlo_to_hlo/translate.h"
#include "machina/xla/service/hlo_module_config.h"
#include "machina/xla/service/hlo_runner.h"
#include "machina/xla/service/platform_util.h"
#include "machina/xla/tools/run_hlo_module.h"
#include "machina/xla/tools/run_hlo_module.pb.h"
...
// to first consider the debug_options from that file, then MACHINA_MACHINA_XLA_FLAGS, and then
...
int main(int argc, char** argv) {
  ...
  std::vector<tsl::Flag> flag_list = {
      ...
      tsl::Flag(
          ...
          "xla_cpu_use_thunk_runtime flag in MACHINA_MACHINA_XLA_FLAGS. This option doesn't "
      ...
      tsl::Flag("debug_options_file", &unused_debug_options_filename,
                ...
                "MACHINA_MACHINA_XLA_FLAGS > debug_options_file > default flags.")};
  ...
  for (int c = 1; c < argc; c++) {
    ...
    if (opts.input_format == "stablehlo" || opts.input_format == "mhlo") {
      ...
      auto output = std::make_unique<toolchain::ToolOutputFile>(
          hlo_filename, error, toolchain::sys::fs::OF_None);
      if (error) {
        LOG(QFATAL) << "cannot open output file '" << std::string(hlo_filename)
                    << "': " << error.message();
      }

      auto input = toolchain::MemoryBuffer::getFile(input_filename);
