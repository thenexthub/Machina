...
#include "machina/xla/tools/multihost_hlo_runner/functional_hlo_runner.h"
...
#include "machina/xla/runtime/large_hlo_snapshot_serialization/serialization.h"
...
#include "machina/xla/tools/hlo_control_flow_flattening.h"
#include "machina/xla/tsl/platform/env.h"
#include "machina/xla/tsl/platform/errors.h"
#include "machina/xla/tsl/platform/file_system.h"
#include "machina/xla/tsl/platform/file_system_helper.h"
#include "machina/xla/tsl/platform/status.h"
#include "machina/xla/tsl/platform/statusor.h"
#include "machina/xla/tsl/util/fixed_option_set_flag.h"
...
absl::StatusOr<PerDeviceLiteralVecType> RunInternal(
  ...
  for (int repeat = 0; repeat < running_options.num_repeats; ++repeat) {
    VLOG(1) << "FunctionalHloRunner: ExecuteOnDevices started (repeat = "
            << repeat << ").";
    {
      MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER("FunctionalHloRunner::ExecuteOnDevices");
...
absl::StatusOr<CompileOptions> CreateCompileOptions(
  ...
  if (task_id == 0) {
    ...
    // from parsed MACHINA_MACHINA_XLA_FLAGS env (already populated in debug_options).
...
void HLORunnerProfiler::UploadSession() {
  xspace_ = std::make_unique<machina::profiler::XSpace>();
...
const machina::profiler::XSpace* HLORunnerProfiler::GetXSpace() {
  return xspace_.get();
}
