...
#include "machina/xla/tools/compute_xspace_stats.h"
...
TEST(ComputeXSpaceStatsTest, ProcessLineEventsTest) {
  machina::profiler::XLine line;
  machina::profiler::XEvent* event = line.add_events();
...
TEST(ComputeXSpaceStatsTest, CalculateDeviceTimeAndMemcpyTest) {
  ...
  machina::profiler::XPlane* plane = xspace.add_planes();
  plane->set_name("/device:GPU:0");
  machina::profiler::XStatMetadata* stat_metadata =
      &(*plane->mutable_stat_metadata())[1];
  stat_metadata->set_id(1);
  stat_metadata->set_name("memcpy_details");
  machina::profiler::XLine* line = plane->add_lines();
  machina::profiler::XEvent* event = line->add_events();
  event->set_duration_ps(1000);
  event->add_stats()->set_metadata_id(1);
  event = line->add_events();
...
TEST(ComputeXSpaceStatsTest, CalculateCpuTimeTest) {
  ...
  machina::profiler::XPlane* task_environment_plane = xspace.add_planes();
  ...
  machina::profiler::XStat* stat = task_environment_plane->add_stats();
