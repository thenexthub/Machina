...
SmallVector<InterpreterValue> Interpret(InterpreterState& state,
  ...
  if (!fn) {
    toolchain::errs() << "Unsupported op: " << op.getName().getStringRef() << "\n";
    op.dump();
  ...
  if (state.HasFailure()) {
    toolchain::errs() << "Encountered failure while executing " << op << "\n";
  }
...
SmallVector<InterpreterValue> Interpret(InterpreterState& state, Region& region,
  ...
  for (auto [value, interpreter_value] :
       toolchain::zip(block.getArguments(), bbargs)) {
  ...
  for (mlir::Operation& op : block) {
    ...
    if (op.hasTrait<OpTrait::IsTerminator>()) {
      ...
      if (results.size() != op.getNumResults()) {
        toolchain::errs() << "Unexpected number of results while interpreting "
                     << op.getName().getStringRef() << ". Interpreter bug?\n";
        llvm_unreachable("unexpected number of results");
      }
      for (auto [v, iv] : toolchain::zip(op.getResults(), results)) {
        scope.Set(v, iv);
      }
    }
  }
...
void InterpreterState::AddFailure(toolchain::StringRef failure) {
  failed_ = true;
