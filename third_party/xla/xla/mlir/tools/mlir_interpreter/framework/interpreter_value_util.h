...
#ifndef MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_UTIL_H_
#define MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_UTIL_H_
...
struct InterpreterValueMapVisitor {
  ...
  InterpreterValue operator()(const TensorOrMemref<T>& t) {
    if constexpr (Fn::template SupportedType<T>()) {
      ...
      toolchain::errs() << toolchain::getTypeName<Fn>()
                   << " unsupported type: " << toolchain::getTypeName<T>() << "\n";
...
struct InterpreterValueBiMapVisitor {
  ...
  InterpreterValue operator()(const Tuple& lhs_t) {
    ...
    Tuple out;
    for (const auto& [lhs_v, rhs_v] : toolchain::zip(lhs_t.values, rhs_t.values)) {
...
#endif  // MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_UTIL_H_
