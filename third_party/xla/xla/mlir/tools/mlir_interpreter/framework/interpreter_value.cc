...
void InterpreterValue::Print(toolchain::raw_ostream& os) const {
...
InterpreterValue InterpreterValue::ExtractElement(
    toolchain::ArrayRef<int64_t> indices) const {
  return std::visit(
...
void InterpreterValue::InsertElement(toolchain::ArrayRef<int64_t> indices,
...
void InterpreterValue::Fill(
    const std::function<InterpreterValue(toolchain::ArrayRef<int64_t> indices)>& f) {
  std::visit(
      [&](auto& it) {
        using T = std::decay_t<decltype(it)>;
        if constexpr (is_tensor_or_memref_v<T>) {
...
InterpreterValue InterpreterValue::TypedAlike(
    toolchain::ArrayRef<int64_t> shape) const {
  return std::visit(
      [&](const auto& it) -> InterpreterValue {
        using T = std::decay_t<decltype(it)>;
        if constexpr (is_tensor_or_memref_v<T>) {
...
InterpreterValue InterpreterValue::MakeTensor(mlir::Type element_type,
  ...
  auto vector_ty = toolchain::dyn_cast<VectorType>(element_type);
  if (vector_ty) {
    toolchain::copy(vector_ty.getShape(), std::back_inserter(shape));
  }
...
bool Tuple::operator==(const Tuple& other) const {
  ...
  for (const auto& [lhs, rhs] : toolchain::zip(values, other.values)) {
