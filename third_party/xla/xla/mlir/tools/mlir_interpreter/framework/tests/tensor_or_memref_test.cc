...
std::optional<int64_t> GetCollapsedStrideNaive(toolchain::ArrayRef<int64_t> dims,
  ...
  toolchain::SmallBitVector v(view.GetNumElements());
  for (const auto& indices : f.Indices()) {
    SmallVector<int64_t> view_indices(view.num_dimensions());
    for (auto [dim, index] : toolchain::zip(dims, indices)) {
