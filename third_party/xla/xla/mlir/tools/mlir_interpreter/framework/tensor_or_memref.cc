...
bool BufferView::InBounds(toolchain::ArrayRef<int64_t> view_indices) const {
  if (view_indices.size() > sizes.size()) {
    return false;
  }
  for (auto [index, size] : toolchain::zip(view_indices, sizes)) {
...
LogicalResult BufferView::Slice(int64_t dim_index, int64_t dim_offset) {
  toolchain::SmallVector<int64_t> offsets(num_dimensions(), 0);
...
LogicalResult BufferView::Subview(ArrayRef<int64_t> subview_offsets,
  ...
  for (auto [in_size, subview_offset, subview_size, subview_stride] :
       toolchain::zip(sizes, subview_offsets, subview_sizes, subview_strides)) {
  ...
  for (auto [in_stride, subview_stride] : toolchain::zip(strides, subview_strides)) {
    in_stride *= subview_stride;
  }
  sizes = toolchain::to_vector(subview_sizes);
  return success();
}
...
std::optional<int64_t> BufferView::GetCollapsedStride(
    toolchain::ArrayRef<int64_t> dims) const {
  using StrideAndDim = std::pair<int64_t, int64_t>;
  toolchain::SmallVector<StrideAndDim> strides_and_dims;
  for (auto dim : dims) {
    if (sizes[dim] != 1) {
  ...
  toolchain::sort(strides_and_dims);
