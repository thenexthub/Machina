...
#ifndef MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_H_
#define MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_H_
...
struct InterpreterValue {
  ...
  // itself.
  InterpreterValue ExtractElement(toolchain::ArrayRef<int64_t> indices) const;
  // Sets the element at the given index. If the value is a scalar, sets its
  // value.
  void InsertElement(toolchain::ArrayRef<int64_t> indices,
                     const InterpreterValue& value);
  // Initializes all elements of the underlying tensor.
  void Fill(
      const std::function<InterpreterValue(toolchain::ArrayRef<int64_t> indices)>&
          f);
  ...
  InterpreterValue CoerceLayout(toolchain::ArrayRef<int64_t> layout) const;
  ...
  // interpreter value.
  InterpreterValue TypedAlike(toolchain::ArrayRef<int64_t> shape) const;
...
auto DispatchScalarType(mlir::Type ty, Fn&& functor) {
  ...
  toolchain::errs() << "DispatchScalarType unimplemented for " << ty << "\n";
...
#endif  // MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_INTERPRETER_VALUE_H_
