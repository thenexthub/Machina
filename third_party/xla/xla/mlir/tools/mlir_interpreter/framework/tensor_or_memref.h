...
#ifndef MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_TENSOR_OR_MEMREF_H_
#define MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_TENSOR_OR_MEMREF_H_
...
struct BufferView {
  int64_t offset;
  toolchain::SmallVector<int64_t> sizes;    // [10, 11, 12]
  toolchain::SmallVector<int64_t> strides;  // [132, 12, 1]
   ...
   public:
    class Iterator {
     public:
      ...
      using value_type = toolchain::SmallVector<int64_t>;
      using difference_type = std::ptrdiff_t;
      using pointer = toolchain::SmallVector<int64_t>*;
      using reference = toolchain::SmallVector<int64_t>&;

      const toolchain::SmallVector<int64_t>& operator*() const {
     ...
     private:
      ...
      Iterator(const BufferView* view, toolchain::SmallVector<int64_t> indices,
      ...
      toolchain::SmallVector<int64_t> view_indices_;
  ...
  std::optional<int64_t> GetPhysicalIndex(
      toolchain::ArrayRef<int64_t> view_indices) const;
  ...
  // possible.
  std::optional<int64_t> GetCollapsedStride(toolchain::ArrayRef<int64_t> dims) const;

  bool InBounds(toolchain::ArrayRef<int64_t> view_indices) const;
...
class Buffer {
 ...
 public:
  ...
  void Deallocate(mlir::Operation* op) {
    if (is_alloca_) {
      ...
      std::string failure;
      toolchain::raw_string_ostream os(failure);
      os << "double-free\n";
  ...
  void SetFailure(toolchain::StringRef failure) const {
    this->failure_ = failure.str();
  }
  toolchain::StringRef GetFailure() const { return failure_; }
 ...
 private:
  ...
  toolchain::SmallVector<char> storage_;
...
struct TensorOrMemref {
  ...
  TensorOrMemref<T> Clone(ArrayRef<int64_t> layout = {}) const {
    ...
    for (auto [src_index, dst_index] :
         toolchain::zip(view.Indices(true), out.view.Indices(true))) {
...
#endif  // MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_TENSOR_OR_MEMREF_H_
