...
==============================================================================*/
#ifndef MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_REGISTRATION_H_
#define MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_REGISTRATION_H_
...
#include "toolchain/Support/TypeName.h"
...
auto TypedInterpreterOpConvertArg(MutableArrayRef<InterpreterValue> args,
  ...
  auto fail = [&]() {
    state.AddFailure(absl::StrCat("Unable to convert argument (variant index ",
                                  ...
                                  toolchain::getTypeName<ArgT>().str()));
    return ArgT{};
  };
...
void RegisterTypedInterpreterOpImpl(Ret (*fn)(InterpreterState&, Op, T... args),
  ...
  RegisterInterpreterOp(
      ...
      [fn](MutableArrayRef<InterpreterValue> args, mlir::Operation* op,
        ...
        auto cast = toolchain::dyn_cast<Op>(op);
        if (!cast) {
          state.AddFailure(absl::StrCat(
              "failed to cast op '", op->getName().getStringRef().str(),
              "' to expected type (", toolchain::getTypeName<Op>().str(), ")"));
          return {};
        }
        int64_t used_args = 0;
        for (auto i : toolchain::seq<uint64_t>(0, sizeof...(T))) {
...
#endif  // MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_INTERPRETER_FRAMEWORK_REGISTRATION_H_
