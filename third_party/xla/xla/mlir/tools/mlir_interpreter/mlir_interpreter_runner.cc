...
struct Options {
  toolchain::cl::opt<std::string> input_filename{toolchain::cl::Positional,
                                            ...
                                            toolchain::cl::init("-")};
  toolchain::cl::opt<bool> run_all_functions{
      "run-all", toolchain::cl::desc("Run all functions in the module"),
      toolchain::cl::init(false)};
};

static mlir::OwningOpRef<mlir::Operation *> ParseMlirInput(
    toolchain::StringRef input_filename, mlir::MLIRContext *context) {
...
static mlir::LogicalResult Run(mlir::ModuleOp module,
  ...
  toolchain::outs() << "@" << function.getName().str() << "()\n";
  if (function.getBody().getBlocks().front().getNumArguments() > 0) {
    toolchain::errs() << "Function arguments are not supported.";
    return mlir::failure();
  }
  ...
  if (!results->empty()) {
    toolchain::outs() << "Results:\n";
    for (const auto &result : *results) {
      toolchain::outs() << result.ToString() << "\n";
    }
  }
...
int main(int argc, char *argv[]) {
  // Flush toolchain::outs before writing errors.
  toolchain::errs().tie(&toolchain::outs());

  Options options;
  toolchain::cl::ParseCommandLineOptions(argc, argv, "MLIR interpreter driver\n");
  ...
  if (!parsed_input) {
    toolchain::errs() << "Failed to parse module.\n";
    return 1;
  }
  auto module = toolchain::dyn_cast<mlir::ModuleOp>(**parsed_input);
  if (!module) {
    toolchain::errs() << "Parsing returned something that's not a module.\n";
    return 1;
  }
  ...
  if (options.run_all_functions) {
    ...
    if (!main) {
      toolchain::errs() << "no main function found.\n";
      return 1;
    }
    if (!Run(module, toolchain::cast<mlir::func::FuncOp>(main)).succeeded()) {
      return 1;
    }
  }
  return 0;
