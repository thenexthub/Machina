...
#include "machina/xla/mlir/tools/mlir_interpreter/dialects/symbol_finder.h"
...
toolchain::SmallVector<InterpreterValue> Call(MutableArrayRef<InterpreterValue> args,
                                         mlir::Operation* op,
                                         InterpreterState& state) {
  auto call = toolchain::cast<func::CallOp>(op);
  auto callee =
      toolchain::cast<func::FuncOp>(state.GetSymbols().lookup(call.getCallee()));
