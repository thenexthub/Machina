...
InterpreterValue Bitcast(InterpreterState&, vector::BitCastOp op,
  ...
  view.sizes = toolchain::to_vector(ty.getShape());
...
InterpreterValue Broadcast(InterpreterState&, vector::BroadcastOp broadcast,
  ...
  toolchain::copy(toolchain::reverse(result_view.strides), strides.rbegin());
  ...
  result_view.sizes =
      toolchain::to_vector(broadcast.getResultVectorType().getShape());
  return result;
}
...
InterpreterValue MaskImpl(mlir::Operation* op, ArrayRef<int64_t> mask_sizes) {
  ...
  BufferView iter;
  iter.sizes = toolchain::to_vector(mask_sizes);
...
InterpreterValue Gather(InterpreterState& state, vector::GatherOp op,
  ...
  for (const auto& out_index : out.View().Indices()) {
    ...
    auto in_index = toolchain::to_vector(offsets);
...
toolchain::SmallVector<InterpreterValue> Mask(
...
InterpreterValue ReductionImpl(InterpreterState& state,
  ...
  for (auto [dim, size] : toolchain::enumerate(v.View().sizes)) {
    if (!toolchain::is_contained(dims, dim)) {
  ...
  return DispatchScalarType(element_type, [&](auto dummy) -> InterpreterValue {
    ...
    for (const auto& result_index : result.view.Indices()) {
      ...
      for (auto [dim, index] :
           toolchain::reverse(toolchain::zip(kept_dims, result_index))) {
...
InterpreterValue OuterProduct(InterpreterState&,
  ...
  return DispatchScalarType(ty, [&](auto dummy) -> InterpreterValue {
    ...
    if (std::holds_alternative<T>(rhs.storage)) {
      ...
      for (int64_t i : toolchain::seq(int64_t{0}, lhs_t.view.sizes[0])) {
        result.at(i) = combiner(result.at(i), lhs_t.at(i) * rhs_scalar);
      }
    } else {
      const TT& rhs_t = std::get<TT>(rhs.storage);
      for (int64_t i : toolchain::seq(int64_t{0}, lhs_t.view.sizes[0])) {
        for (int64_t j : toolchain::seq(int64_t{0}, rhs_t.view.sizes[0])) {
          result.at({i, j}) =
...
InterpreterValue ShapeCast(InterpreterState&, vector::ShapeCastOp op,
  ...
  out_view.sizes = toolchain::to_vector(
...
InterpreterValue Shuffle(InterpreterState& state, vector::ShuffleOp shuffle,
  ...
  for (auto [dst_index, src_index] : toolchain::enumerate(mask)) {
...
InterpreterValue Splat(InterpreterState&, vector::SplatOp op,
  ...
  auto& view = out.View();
  view.sizes = toolchain::to_vector(
      mlir::cast<ShapedType>(op->getResultTypes()[0]).getShape());
...
std::optional<InterpreterValue> ExtractMemorySlice(
  ...
  toolchain::SmallVector<bool> in_bounds(offsets.size());
  if (in_bounds_attr) {
    toolchain::copy(in_bounds_attr->getAsValueRange<BoolAttr>(),
...
toolchain::SmallVector<InterpreterValue> TransferWrite(
