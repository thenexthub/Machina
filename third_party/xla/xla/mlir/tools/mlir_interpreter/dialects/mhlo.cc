...
#include "machina/xla/mlir/tools/mlir_interpreter/dialects/comparators.h"
#include "machina/xla/mlir/tools/mlir_interpreter/dialects/cwise_math.h"
#include "machina/xla/mlir/tools/mlir_interpreter/dialects/util.h"
...
#include "machina/xla/mlir/tools/mlir_interpreter/framework/interpreter_value_util.h"
#include "machina/xla/mlir/tools/mlir_interpreter/framework/registration.h"
#include "machina/xla/mlir/tools/mlir_interpreter/framework/tensor_or_memref.h"
...
InterpreterValue BitcastConvert(InterpreterState&, mhlo::BitcastConvertOp op,
  ...
  out_view.sizes = toolchain::to_vector(ty.getShape());
  return result;
}
...
InterpreterValue BroadcastInDim(InterpreterState&,
  ...
  out.Fill([&](toolchain::ArrayRef<int64_t> out_indices) {
    toolchain::SmallVector<int64_t> in_indices;
    for (auto [inDim, out_dim] : toolchain::enumerate(boradcast_dims)) {
...
toolchain::SmallVector<int64_t> ClampStarts(ArrayRef<int64_t> starts,
  ...
  for (auto [start, slice_size, tensor_size] :
       toolchain::zip(starts, slice_sizes, tensor_sizes)) {
    result.push_back(
...
InterpreterValue DynamicSlice(InterpreterState&, mhlo::DynamicSliceOp slice,
  ...
  auto result = in.TypedAlike(
      toolchain::to_vector(slice.getSliceSizes().getValues<int64_t>()));
  auto clamed_starts =
      ClampStarts(starts, result.View().sizes, in.View().sizes);
  // TODO(jreiffers): Skip the copy.
  result.Fill([&](toolchain::ArrayRef<int64_t> out_indices) {
    toolchain::SmallVector<int64_t> in_indices;
    for (auto [start, index] : toolchain::zip(clamed_starts, out_indices)) {
...
InterpreterValue DynamicUpdateSlice(InterpreterState&,
  ...
  for (auto in_indices : updates.View().Indices()) {
    toolchain::SmallVector<int64_t> out_indices;
    for (auto [start, index] : toolchain::zip(clamed_starts, in_indices)) {
...
InterpreterValue Slice(InterpreterState&, mhlo::SliceOp slice,
  ...
  toolchain::SmallVector<int64_t> sizes;
  for (auto [start, limit, stride] : toolchain::zip(starts, limits, strides)) {
  ...
  result.Fill([&](toolchain::ArrayRef<int64_t> out_indices) {
    ...
    for (auto [start, stride, index] :
         toolchain::zip(starts, strides, out_indices)) {
...
toolchain::SmallVector<InterpreterValue> Constant(InterpreterState&,
...
InterpreterValue Pad(InterpreterState&, mhlo::PadOp pad, InterpreterValue arg,
  ...
  toolchain::SmallVector<int64_t> sizes;
  for (auto [size, lo, in, hi] : toolchain::zip(arg.View().sizes, los, ins, his)) {
    sizes.push_back(size + lo + hi + (size - 1) * in);
  }

  auto result = arg.TypedAlike(sizes);
  result.Fill([&](toolchain::ArrayRef<int64_t>) { return padding_value; });

  for (const auto& in_indices : arg.View().Indices()) {
    toolchain::SmallVector<int64_t> out_indices;
    for (auto [in_index, in, lo] : toolchain::zip(in_indices, ins, los)) {
...
toolchain::SmallVector<InterpreterValue> Gather(
    MutableArrayRef<InterpreterValue> args, mlir::Operation* op,
    InterpreterState& state) {
  auto gather = toolchain::dyn_cast<mhlo::GatherOp>(op);
  auto dynamic_gather = toolchain::dyn_cast<mhlo::DynamicGatherOp>(op);
  ...
  auto slice_sizes =
      gather ? toolchain::to_vector(gather.getSliceSizes().getValues<int64_t>())
  ...
  for (int64_t i = 0; i < output_rank; ++i) {
    if (!toolchain::is_contained(offset_dims, i)) {
  ...
  for (auto [output_dim, slice_dim] :
       toolchain::zip(offset_dims, non_collapsed_slice_dims)) {
    output_sizes[output_dim] = slice_sizes[slice_dim];
  }
  for (auto [batch_index, output_dim] : toolchain::enumerate(batch_dims)) {
  ...
  for (auto start_indices_index : start_indices_view.Indices()) {
    ...
    for (auto [i, dim] : toolchain::enumerate(start_index_map)) {
      if (index_vector_dim < start_indices_view.num_dimensions()) {
...
toolchain::SmallVector<InterpreterValue> Scatter(
  ...
  toolchain::SmallVector<int64_t> batch_dims;
  for (int64_t dim = 0; dim < operand_rank; ++dim) {
    if (!toolchain::is_contained(inserted_window_dims, dim)) {
      batch_dims.push_back(dim);
    }
  }

  toolchain::SmallVector<int64_t> update_scatter_dims;
  for (int64_t dim = 0; dim < updates_rank; ++dim) {
    if (!toolchain::is_contained(update_window_dims, dim)) {
      update_scatter_dims.push_back(dim);
    }
  }

  toolchain::SmallVector<InterpreterValue> n_results;
  for (auto& inputs : n_inputs) {
    n_results.push_back(inputs.Clone());
  }

  for (auto [inputs, updates, results] :
       toolchain::zip(n_results, n_updates, n_results)) {
    const auto& updates_view = updates.View();
    for (const auto& update_indices : updates_view.Indices()) {
      toolchain::SmallVector<int64_t> input_indices(operand_rank);
      toolchain::SmallVector<int64_t> max_indices(operand_rank);
      toolchain::SmallVector<int64_t> min_indices(operand_rank);
      toolchain::SmallVector<int64_t> scatter_indices_index(indices_rank);

      for (auto [index, dim] : toolchain::enumerate(update_scatter_dims)) {
        scatter_indices_index[index >= index_vector_dim ? index + 1 : index] +=
            update_indices[dim];
      }

      for (auto [update_dim, operand_dim] :
           toolchain::zip(update_window_dims, batch_dims)) {
        input_indices[operand_dim] = update_indices[update_dim];
        max_indices[operand_dim] = updates_view.sizes[update_dim] - 1;
      }

      for (auto [index, dim] : toolchain::enumerate(scatter_dims_to_operand_dims)) {
...
InterpreterValue Transpose(InterpreterState&, mhlo::TransposeOp transpose,
  ...
  return TransposeImpl(tensor, toolchain::to_vector(permutation));
}
...
toolchain::SmallVector<InterpreterValue> While(
...
InterpreterValue Copy(InterpreterState&, mhlo::CopyOp op,
  ...
  return tensor.Clone(
      toolchain::to_vector(cast<DenseIntElementsAttr>(layout).getValues<int64_t>()));
}

toolchain::SmallVector<InterpreterValue> Fusion(InterpreterState& state,
                                           mhlo::FusionOp fusion,
                                           ArrayRef<InterpreterValue> args) {
  return Interpret(state, fusion.getRegion(), args);
}

toolchain::SmallVector<InterpreterValue> Reduce(InterpreterState& state,
  ...
  for (const auto& index : operands[0].View().Indices()) {
    ...
    for (auto [result, value] : toolchain::zip(results, new_values)) {
...
SmallVector<InterpreterValue> Sort(InterpreterState& state, mhlo::SortOp op,
  ...
  for (const auto& offset : iter_view.Indices()) {
    ...
    auto copy = offset;
    for (auto [dst, src] : toolchain::enumerate(indices)) {
      for (auto [input, result] : toolchain::zip(inputs, results)) {
        copy[dim] = src;
...
InterpreterValue DotGeneralImpl(InterpreterValue& lhs, InterpreterValue& rhs,
  ...
  for (int64_t i = 0; i < lhsv.num_dimensions(); i++) {
    if (!toolchain::is_contained(lhs_contracting, i) &&
        !toolchain::is_contained(lhs_batch, i)) {
      dimensions.push_back(lhsv.sizes[i]);
      lhs_non_batch.push_back(i);
    }
  }
  for (int64_t i = 0; i < rhs.View().num_dimensions(); i++) {
    if (!toolchain::is_contained(rhs_contracting, i) &&
        !toolchain::is_contained(rhs_batch, i)) {
