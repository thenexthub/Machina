...
toolchain::SmallVector<InterpreterValue> Broadcast(InterpreterState&,
                                              linalg::BroadcastOp broadcast,
                                              const InterpreterValue& input,
                                              InterpreterValue init) {
  auto broadcast_dims = toolchain::to_vector(broadcast.getDimensions());
  toolchain::sort(broadcast_dims);
  ...
  init.Fill([&](toolchain::ArrayRef<int64_t> indices) {
    return out.ExtractElement(indices);
  });
  return {};
}

toolchain::SmallVector<InterpreterValue> Generic(
  ...
  for (auto& value : toolchain::concat<InterpreterValue>(inputs, outputs_ref)) {
  ...
  run = [&](int64_t loop_index) {
    ...
    if (loop_index < ranges.size()) {
      ...
      toolchain::SmallVector<InterpreterValue> bbargs;
      // Build bbargs: 1. inputs, 2. outputs.
      for (auto [input, map] : toolchain::zip(inputs, indexing_maps)) {
        auto indices = EvalAffineMap(map, ivs);
        bbargs.push_back(input.ExtractElement(indices));
      }
      toolchain::SmallVector<toolchain::SmallVector<int64_t>> output_indices;
      for (auto [output, map] : toolchain::zip(outputs, output_maps)) {
      ...
      for (auto [output, indices, yield] :
           toolchain::zip(outputs, output_indices, yielded)) {
...
toolchain::SmallVector<InterpreterValue> Map(InterpreterState& state,
                                        linalg::MapOp op,
  ...
  for (const auto& indices : output.View().Indices()) {
    ...
    toolchain::SmallVector<InterpreterValue> args;
...
toolchain::SmallVector<InterpreterValue> Reduce(InterpreterState& state,
  ...
  for (auto [ty, init] : toolchain::zip(reduce.getInits().getTypes(), inits)) {
  ...
  for (const auto& index : ins[0].View().Indices()) {
    ...
    for (auto [out, value] : toolchain::zip(output, new_values)) {
...
toolchain::SmallVector<InterpreterValue> Fill(InterpreterState&, linalg::FillOp op,
  ...
  output.Fill([&](toolchain::ArrayRef<int64_t>) { return value; });
  if (op.getNumResults() == 0) {
    return {};
  }
  return {std::move(output)};
