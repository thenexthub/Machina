...
SmallVector<int64_t> ReplaceDynamicVals(toolchain::ArrayRef<int64_t> static_vals,
...
InterpreterValue ReshapeTensor(const InterpreterValue& in,
  ...
  for (const auto& [in_index, out_index] :
       toolchain::zip(in.View().Indices(), out.View().Indices())) {
...
toolchain::SmallVector<InterpreterValue> NoOpTerminator(
    ...
    InterpreterState&) {
  return toolchain::to_vector(args);
}
...
int64_t EvalAffineExpr(AffineExpr expr, ArrayRef<int64_t> dims,
  ...
  switch (expr.getKind()) {
    ...
    case AffineExprKind::Mod:
      return toolchain::mod(lhs, rhs);
    case AffineExprKind::FloorDiv:
      return toolchain::divideFloorSigned(lhs, rhs);
    case AffineExprKind::CeilDiv:
      return toolchain::divideCeilSigned(lhs, rhs);
