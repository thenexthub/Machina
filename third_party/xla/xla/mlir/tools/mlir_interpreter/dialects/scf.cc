...
class ParallelSideChannel : public InterpreterSideChannel {
 public:
  explicit ParallelSideChannel(toolchain::SmallVector<InterpreterValue>& results)
      : results_(results) {}
...
toolchain::SmallVector<InterpreterValue> For(InterpreterState& state, scf::ForOp op,
                                        int64_t lb, int64_t ub, int64_t step,
                                        ArrayRef<InterpreterValue> inits) {
  toolchain::SmallVector<InterpreterValue> results;
  ...
  for (; lb < ub; lb += step) {
    ...
    toolchain::copy(results, std::back_inserter(inputs));
...
toolchain::SmallVector<InterpreterValue> ForAll(
  ...
  SmallVector<InterpreterValue> outs = toolchain::to_vector(inits);
  for (auto [type, out] : toolchain::zip(op.getOutputs().getTypes(), outs)) {
  ...
  for (auto [lb, ub, step] : toolchain::zip(lbs, ubs, steps)) {
    if (step == 0) {
  ...
  for (const auto& indices : view.Indices()) {
    ...
    for (auto [i, lb, step] : toolchain::zip(indices, lbs, steps)) {
      args.push_back(InterpreterValue{i * step + lb});
    }
    toolchain::copy(outs, std::back_inserter(args));
...
toolchain::SmallVector<InterpreterValue> If(InterpreterState& state, scf::IfOp op,
                                       bool condition) {
  if (condition) {
...
toolchain::SmallVector<InterpreterValue> Parallel(InterpreterState& state,
