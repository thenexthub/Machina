...
InterpreterValue FromElements(InterpreterState&, tensor::FromElementsOp op,
  ...
  auto result = InterpreterValue::MakeTensor(ty.getElementType(),
                                             toolchain::to_vector(ty.getShape()));
  for (auto [index, element] : toolchain::zip(result.View().Indices(), elements)) {
    result.InsertElement(index, element);
  }
  return result;
}

toolchain::SmallVector<InterpreterValue> CollapseShape(
...
toolchain::SmallVector<InterpreterValue> ExpandShape(
    InterpreterState&, tensor::ExpandShapeOp op,
    const InterpreterValue& tensor) {
  auto ty = cast<ShapedType>(op->getResultTypes().front());
  auto sizes = toolchain::to_vector(ty.getShape());
  for (const auto& [src_index, dst_indices] :
       toolchain::enumerate(op.getReassociationIndices())) {
...
toolchain::SmallVector<InterpreterValue> ExtractSlice(
  ...
  out.Fill([&](toolchain::ArrayRef<int64_t> indices) {
    toolchain::SmallVector<int64_t> src_indices;
...
toolchain::SmallVector<InterpreterValue> InsertSlice(
  ...
  toolchain::SmallVector<int64_t> inserted_dims;
  auto& src_view = src.View();
  auto* src_size_it = src_view.sizes.begin();
  for (auto [dim, size] : toolchain::enumerate(static_sizes)) {
  ...
  for (const auto& src_indices : src_view.Indices()) {
    toolchain::SmallVector<int64_t> src_with_inserted_dims = src_indices;
    for (int64_t dim : inserted_dims) {
      src_with_inserted_dims.insert(src_with_inserted_dims.begin() + dim, 0);
    }
    toolchain::SmallVector<int64_t> dst_indices;
    for (auto [src_index, stride, offset] :
         toolchain::zip(src_with_inserted_dims, v.strides, v.offsets)) {
...
InterpreterValue Pad(InterpreterState& state, tensor::PadOp pad,
  ...
  toolchain::SmallVector<int64_t> result_sizes;
  for (auto [size, low, high] : toolchain::zip(view.sizes, lows, highs)) {
    result_sizes.push_back(size + low + high);
  }

  auto result = tensor.TypedAlike(result_sizes);
  result.Fill([&](toolchain::ArrayRef<int64_t> out_index) -> InterpreterValue {
    toolchain::SmallVector<int64_t> in_index;
    for (auto [index, low] : toolchain::zip(out_index, lows)) {
