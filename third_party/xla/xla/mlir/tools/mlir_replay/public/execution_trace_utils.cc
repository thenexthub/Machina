...
toolchain::SmallVector<mlir::Attribute> ValueToAttribute(
  ...
  if (std::holds_alternative<Tuple>(value.storage)) {
    ...
    for (const auto& [v, ty] : toolchain::zip(t.values, types)) {
...
TensorOrMemref<T> ArrayLiteralToTensor(const xla::Literal& literal) {
  ...
  if (literal.shape().has_layout()) {
    toolchain::copy(literal.shape().layout().minor_to_major(),
...
absl::StatusOr<InterpreterValue> TracedValueToValue(
  ...
  auto extract = [&](auto dummy, auto& elements) -> InterpreterValue {
    ...
    auto result =
        TensorOrMemref<T>::Empty(toolchain::to_vector(traced_value.shape()));
...
toolchain::SmallVector<const InstructionTrace*> FindOpExecutionsInTrace(
    const ExecutionTrace& trace, mlir::Operation* op) {
  toolchain::SmallVector<int64_t> region_indices;
  toolchain::SmallVector<int64_t> op_indices;

  std::function<void(mlir::Operation*)> get_op_path;
  get_op_path = [&](mlir::Operation* op) {
    auto* parent = op->getParentOp();
    if (!toolchain::isa<func::FuncOp>(parent)) {
      get_op_path(parent);
  ...
  toolchain::SmallVector<const InstructionTrace*> result;
