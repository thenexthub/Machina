...
#include "machina/xla/mlir/tools/mlir_replay/mlir_replay_lib.h"
...
absl::StatusOr<SmallVector<InterpreterValue>> LoadArgs(
  ...
  for (const auto& [arg, type] : toolchain::zip(snapshot.arguments(), types)) {
...
mlir::interpreter::InterpreterValue RandomTensor(absl::BitGenRef bitgen,
  ...
  if (shaped_ty) {
    shape = toolchain::to_vector(shaped_ty.getShape());
  }
...
mlir::FailureOr<mlir::interpreter::InterpreterValue> MakeRandomInput(
  ...
  toolchain::errs() << "Unsupported type: ";
  type.print(toolchain::errs());
  toolchain::errs() << "\n";
  return failure();
}
...
std::vector<int64_t> ExtractXlaBufferAssignment(func::FuncOp main) {
  ...
  if (auto inner_mapping = main->getAttrOfType<ArrayAttr>(
    ...
    toolchain::copy(toolchain::map_range(inner_mapping.getAsValueRange<IntegerAttr>(),
  ...
  for (int64_t arg_index : toolchain::seq<int64_t>(0, main.getNumArguments())) {
...
absl::StatusOr<SmallVector<InterpreterValue>> Run(
  ...
  source_mgr->AddNewSourceBuffer(toolchain::MemoryBuffer::getMemBuffer(mlir_ir),
                                 mlir::SMLoc());
  ...
  for (const std::string& candidate : entry) {
    main = toolchain::dyn_cast_or_null<func::FuncOp>(symbols.lookup(candidate));
