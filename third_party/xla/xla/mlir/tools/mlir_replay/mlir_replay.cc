...
bool ResultsMatch(const xla::HloSnapshot& snapshot,
                  const toolchain::SmallVector<mlir::interpreter::InterpreterValue>&
                      first_pass_results,
...
void TestAll(mlir::MLIRContext& context, const ReplayOptions& opts) {
  ...
  for (const auto& trace_path : traces) {
    ...
    if (!failures.empty()) {
      toolchain::errs() << "Failures for " << trace_path << ":\n  "
...
int main(int argc, char* argv[]) {
  ...
  toolchain::SmallVector<mlir::interpreter::InterpreterValue> previous_results;
  int pass_id = 0;
  for (auto& state : trace.passes()) {
    toolchain::outs() << "Running IR after " << state.after_pass() << ".\n";
    ...
    if (results.status().ok()) {
      ...
      toolchain::errs() << results.status().ToString() << "\n";
