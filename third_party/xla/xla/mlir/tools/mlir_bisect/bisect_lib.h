...
#ifndef MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_BISECT_BISECT_LIB_H_
#define MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_BISECT_BISECT_LIB_H_
...
class BisectState {
 public:
  ...
  toolchain::SmallVector<const interpreter::InstructionTrace*> GetExecutions(
...
std::pair<OwningOpRef<ModuleOp>, Op> CloneModuleFor(Op op) {
  ...
  return {std::move(module), toolchain::cast<Op>(op_clone)};
}
...
void RegisterReduceStrategy(StringRef name,
  ...
  RegisterReduceStrategy(
      name, [fn](BisectState& state, Operation* op) -> CandidateVector {
        if (auto cast = toolchain::dyn_cast<Op>(op)) {
...
#endif  // MACHINA_MACHINA_XLA_MLIR_TOOLS_MLIR_BISECT_BISECT_LIB_H_
