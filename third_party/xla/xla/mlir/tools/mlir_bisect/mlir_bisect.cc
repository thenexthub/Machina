...
#include "machina/xla/mlir/tools/mlir_bisect/bisect_lib.h"
#include "machina/xla/mlir/tools/mlir_bisect/test_passes.h"
#include "machina/xla/mlir/tools/mlir_interpreter/framework/interpreter.h"
#include "machina/xla/mlir/tools/mlir_replay/public/execution_trace.pb.h"
#include "machina/xla/mlir/tools/mlir_replay/public/execution_trace_utils.h"
...
struct Options {
  toolchain::cl::opt<std::string> input_filename{toolchain::cl::Positional,
                                            toolchain::cl::desc("<input file>"),
                                            toolchain::cl::init("-")};
  toolchain::cl::opt<std::string> hlo_snapshot{
      "hlo-snapshot",
      toolchain::cl::desc(
          "If set, get argument values from the given snapshot. If not set, "
          "the input function must not have any arguments."),
      toolchain::cl::init("")};
  toolchain::cl::opt<std::string> debug_strategy{
      "debug-strategy",
      toolchain::cl::desc("If set, print all reductions for the given strategy and "
                     "exit. For testing."),
      toolchain::cl::init("")};
  toolchain::cl::opt<std::string> expected_error{
      "expected-error",
      toolchain::cl::desc("If set, expect the given error message after applying "
                     "the pass instead of a successful execution."),
      toolchain::cl::init("")};
  toolchain::cl::opt<int64_t> max_steps_per_run{
      "max-steps-per-run",
      toolchain::cl::desc("Maximum number of steps to execute for each attempt."),
      toolchain::cl::init(100000)};
  mlir::PassPipelineCLParser pass_pipeline{"", "Passes to run"};
  toolchain::cl::opt<bool> canonicalize{
      "enable-canonicalization",
      toolchain::cl::desc("If set, canonicalize candidates before trying them. Set "
...
OwningOpRef<ModuleOp> ParseMlirInput(toolchain::StringRef input_filename,
                                     MLIRContext* context) {
  ...
  auto source_mgr = std::make_shared<toolchain::SourceMgr>();
...
LogicalResult RunPipeline(ModuleOp module, const Options& options) {
  ...
  if (failed(options.pass_pipeline.addToPipeline(pm, error_handler)) ||
    ...
    toolchain::errs() << "pipeline failed\n";
    return failure();
  }
  return success();
}
...
LogicalResult Run(mlir::Operation* module, interpreter::ExecutionTrace* trace,
  ...
  auto results_before_pass = interpreter::RunInterpreter(
      symbol_table, toolchain::cast<func::FuncOp>(symbol_table.lookup("main")), {},
      interpreter_options);

  if (!results_before_pass.ok()) {
    toolchain::errs() << "Interpreter failed\n";
    return failure();
  }
  ...
  OwningOpRef<ModuleOp> clone(toolchain::cast<ModuleOp>(module).clone());
  ...
  if (!options.expected_error.empty()) {
    ...
    interpreter_options.error_handler = [&](toolchain::StringRef failure) {
  ...
  auto results_after_pass = interpreter::RunInterpreter(
      symbol_table_after,
      toolchain::cast<func::FuncOp>(symbol_table_after.lookup("main")), {},
  ...
  if (!results_after_pass.ok()) {
    ...
    toolchain::errs() << "Expected error not seen\n";
    return failure();
  }
  ...
  toolchain::errs() << "results before:\n";
  for (auto& result : *results_before_pass) {
    toolchain::errs() << "  " << result.ToString() << "\n";
  }
  toolchain::errs() << "\nresults after:\n";
...
OwningOpRef<ModuleOp> ReduceModule(OwningOpRef<ModuleOp> module,
  ...
  auto strategies = toolchain::to_vector(mlir::bisect::detail::GetStrategies());
  ...
  auto apply_step = [&]() -> std::optional<OwningOpRef<ModuleOp>> {
    for (auto it = strategies.begin(); it != strategies.end(); ++it) {
      for (auto& candidate_fn :
        ...
        toolchain::outs() << "module after " << it->first << ":\n"
                     << *candidate << "\n\n";
...
void ReplaceArgsWithConstants(ModuleOp module,
  ...
  auto main = toolchain::cast<func::FuncOp>(module.lookupSymbol("main"));
  OpBuilder b(main.getBody());
  for (auto [arg, bbarg] :
       toolchain::zip(snapshot.arguments(), main.getBody().getArguments())) {
    ...
    auto constant = b.create<arith::ConstantOp>(
        main.getLoc(), bbarg.getType(), toolchain::cast<TypedAttr>(attr.front()));
  ...
  for (auto arg :
    ...
    CHECK(toolchain::isa<MemRefType>(arg.getType())) << "unsupported argument";
    arg.replaceAllUsesWith(b.create<memref::AllocOp>(
        module.getLoc(), toolchain::cast<MemRefType>(arg.getType())));
  }
...
int main(int argc, char* argv[]) {
  toolchain::errs().tie(&toolchain::outs());
  toolchain::outs().tie(&toolchain::errs());
  int dummy_argc = 1;
  tsl::port::InitMain("", &dummy_argc, &argv);

  Options options;
  toolchain::cl::ParseCommandLineOptions(argc, argv, "MLIR bisect tool\n");
  ...
  if (!options.debug_strategy.empty()) {
    ...
    for (auto& candidate : mlir::bisect::detail::GetCandidates(
      ...
      toolchain::outs() << *new_module << "\n\n";
      if (!mlir::verify(*new_module).succeeded()) {
        some_failed = true;
        toolchain::errs() << "verification failed\n";
      }
    }
  ...
  toolchain::outs() << "Final module:\n" << *module << "\n";
  if (options.pass_pipeline.hasAnyOccurrences() &&
      mlir::succeeded(mlir::bisect::RunPipeline(*module, options))) {
    toolchain::outs() << "Final module after running pipeline:\n" << *module << "\n";
  }
  return 0;
}
