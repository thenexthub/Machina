...
toolchain::SmallVector<std::function<OwningOpRef<ModuleOp>()>> ReplaceOpWithConstant(
    BisectState& state, Operation* op) {
  toolchain::SmallVector<std::function<OwningOpRef<ModuleOp>()>> result;
  if (op->hasTrait<OpTrait::ConstantLike>() || IsTopLevelOp(op) ||
      IsTerminator(op) || op->use_empty() || op->getNumResults() == 0) {
    return result;
  }

  auto mii = toolchain::dyn_cast<MemoryEffectOpInterface>(op);
    ...
    result.push_back([execution, op]() -> OwningOpRef<ModuleOp> {
      ...
      for (int64_t i = 0; i < op->getNumResults(); ++i) {
        ...
        op_clone->getResults()[i].replaceAllUsesWith(arith::ConstantOp::create(
            ...
            toolchain::cast<TypedAttr>(attribute.front())));
      }
...
toolchain::SmallVector<std::function<OwningOpRef<ModuleOp>()>>
...
toolchain::SmallVector<std::function<OwningOpRef<ModuleOp>()>> ReplaceOpWithValue(
    BisectState&, Operation* op) {
  toolchain::SmallVector<std::function<OwningOpRef<ModuleOp>()>> ret;
  if (op->hasTrait<OpTrait::ConstantLike>() || IsTopLevelOp(op) ||
      IsTerminator(op)) {
    return ret;
  }
  ...
  toolchain::DenseMap<mlir::Type, toolchain::SmallVector<std::pair<Operation*, int64_t>>>
      candidates_by_type;
  for (auto* pred = op->getPrevNode(); pred != nullptr;
       pred = pred->getPrevNode()) {
    for (auto [index, result] : toolchain::enumerate(pred->getResults())) {
