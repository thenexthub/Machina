...
struct BarePtrFuncOpConversion : public ConvertOpToLLVMPattern<func::FuncOp> {
  ...
  mlir::func::FuncOp convertFuncOpToLLVMFuncOp(
    ...
    // machina/compiler/xla/service/cpu/ir_function.cc
    //
    ...
    auto new_func_op = rewriter.create<mlir::func::FuncOp>(
        loc, funcOp.getName(), llvm_type, toolchain::SmallVector<NamedAttribute>());
    auto locs = toolchain::SmallVector<mlir::Location>(arg_types.size(), loc);
    Block *new_entry =
        ...
        if (funcOp->hasAttr("xla_framework.result_inner_mapping")) {
          ...
          rewriter.create<LLVM::StoreOp>(
              ...
              rewriter.create<LLVM::GEPOp>(loc, ptr_type, ptr_type, first_load,
                                           toolchain::ArrayRef(second_index)));
...
#include "machina/xla/mlir/framework/transforms/passes.h.inc"
 ...
 public:
  ...
  void runOnOperation() override {
    ...
    target.addDynamicallyLegalOp<func::FuncOp>([](func::FuncOp op) {
      if (toolchain::any_of(toolchain::concat<const Type>(op.getArgumentTypes(),
                                                op.getResultTypes()),
                       [](Type type) {
