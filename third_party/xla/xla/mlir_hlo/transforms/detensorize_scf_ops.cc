...
struct RegionOpPattern : public OpRewritePattern<T> {
  ...
  LogicalResult matchAndRewrite(T op,
    ...
    if (!toolchain::any_of(op->getOperands(), isUnitTensor) &&
        !toolchain::any_of(op->getResults(), isUnitTensor))
      return failure();

    auto* result = rewriter.clone(*op.getOperation());

    auto unitTensors = [](auto&& range) {
      return toolchain::make_filter_range(toolchain::enumerate(range), [](auto it) {
    ...
    toolchain::SmallVector<Value> results = result->getResults();
