...
void UnbufferizePass::runOnOperation() {
  ...
  IRMapping mapping;
  toolchain::SmallDenseSet<BlockArgument> insertedArgs;
  funcOp->walk([&](bufferization::ToTensorOp op) {
    ...
    if (newValue == nullptr) {
      ...
      if (toolchain::failed(funcOp.insertArgument(
  ...
  if (toolchain::failed(funcOp.eraseArguments(argsToErase))) {
    return signalPassFailure();
  }
  auto resultIndices = toolchain::to_vector(toolchain::seq<unsigned>(0, results.size()));
  if (toolchain::failed(funcOp.insertResults(
