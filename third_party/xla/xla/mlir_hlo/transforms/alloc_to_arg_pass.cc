...
void AllocToArgPass::runOnOperation() {
  ...
  auto returnOp = toolchain::cast<func::ReturnOp>(bodyBlock.getTerminator());

  IRRewriter rewriter(&getContext());
  BitVector resultsToErase(funcOp.getNumResults());
  Location loc = returnOp.getLoc();

  for (auto [i, result] : toolchain::enumerate(returnOp.getOperands())) {
    Operation *resultDef = result.getDefiningOp();
    Type resultTy = result.getType();

    // Case: plain alloc.
    if (auto allocOp = toolchain::dyn_cast_or_null<memref::AllocOp>(resultDef)) {
    ...
    if (auto expandOp =
            toolchain::dyn_cast_or_null<memref::ExpandShapeOp>(resultDef)) {
      Operation *expandDef = expandOp.getOperand(0).getDefiningOp();
      if (auto allocOp = toolchain::dyn_cast_or_null<memref::AllocOp>(expandDef)) {
        resultsToErase.set(i);
        auto attrs = funcOp.getResultAttrDict(i);
        if (failed(funcOp.insertArgument(funcOp.getNumArguments(), resultTy,
                                         attrs, loc))) {
