...
SmallVector<RegionEdge> getSuccessorRegions(RegionBranchOpInterface op,
  ...
  for (const auto& successor : successors) {
    ...
    if (successor.isParent()) {
      ...
      edge.successorValueIndex = toolchain::isa<scf::ForOp>(op) ? 1 : 0;
    }
  }
