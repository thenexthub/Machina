...
T findOp(Operation* start, std::function<bool(T)> predicate) {
  while (start) {
    if (T t = toolchain::dyn_cast<T>(start)) {
...
bool hasUsesBetween(Operation* start, Operation* end, Value v) {
  return anyMatchBetween(start, end, [v](Operation* op) {
    return toolchain::is_contained(op->getOperands(), v);
  });
}
...
findAllocsAndDeallocs(Block& block) {
  ...
  for (auto& op : block) {
    if (auto alloc = toolchain::dyn_cast<memref::AllocOp>(op)) {
      if (alloc.getDynamicSizes().empty()) {
        allocations[alloc.getResult().getType()].push_back(alloc);
      }
    } else if (auto dealloc = toolchain::dyn_cast<memref::DeallocOp>(op)) {
...
RegionBranchOpInterface doubleBuffer(RegionBranchOpInterface op) {
  ...
  for (unsigned i = 0; i < op->getNumRegions(); ++i) {
    ...
    for (auto [type, allocs] : allocations) {
      for (auto [alloc, dealloc] : toolchain::zip(allocs, deallocations[type])) {
        doubleBuffer(op, alloc, dealloc);
        deallocsToFix.push_back(dealloc);

        if (toolchain::isa<scf::WhileOp>(op)) {
...
bool doubleBuffer(Block& block) {
  ...
  while (op) {
    ...
    if (toolchain::isa<scf::ForOp, scf::WhileOp>(op)) {
...
bool isRestrictBbArg(Value value) {
  auto bbarg = toolchain::dyn_cast<BlockArgument>(value);
  auto func =
      toolchain::dyn_cast<func::FuncOp>(value.getParentBlock()->getParentOp());
...
void eliminateCopies(Block& block, Block& root) {
  ...
  while (op) {
    ...
    auto copy = toolchain::dyn_cast_or_null<memref::CopyOp>(op);
    op = op->getNextNode();

    auto dealloc = toolchain::dyn_cast_or_null<memref::DeallocOp>(op);
    if (!copy || !dealloc ||
    ...
    auto sourceAlloc = toolchain::dyn_cast_or_null<memref::AllocOp>(
    ...
    if (!targetIsFirstUseOfRestrictBbArg) {
      auto targetAlloc = toolchain::dyn_cast_or_null<memref::AllocOp>(
...
bool reuseBuffers(Block& block, BufferReuseMode mode) {
  ...
  bool result = false;
  while (op) {
    if (auto dealloc = toolchain::dyn_cast<memref::DeallocOp>(op)) {
      ...
      if (!alloc || (mode == BufferReuseMode::CONSERVATIVE &&
                     anyMatchBetween(dealloc, alloc, [&](Operation* op) {
                       return toolchain::isa<memref::AllocOp>(op) &&
...
SmallVector<Value> hoistAllocs(Operation* parent, Region& region,
  ...
  while (op) {
    auto alloc = toolchain::dyn_cast<memref::AllocOp>(op);
    if (alloc && alloc.getDynamicSizes().empty()) {
      auto dealloc = toolchain::find_if(op->getUsers(), [&](Operation* user) {
        return toolchain::isa<memref::DeallocOp>(user) &&
      ...
      auto* reusable = toolchain::find_if(freeAllocs, [&](Value free) {
...
bool hoistAllocs(Block& block) {
  ...
  while (op) {
    ...
    if (auto rbi = toolchain::dyn_cast<RegionBranchOpInterface>(op)) {
...
bool simplifyLoopDeallocs(Block& block) {
  ...
  for (auto& op : block.getOperations()) {
    auto rbi = toolchain::dyn_cast<RegionBranchOpInterface>(op);
    if (!rbi) continue;
    ...
    toolchain::DenseSet<Value> operands{rbi->getOperands().begin(),
                                   rbi->getOperands().end()};
    toolchain::DenseMap<Value, ResultInfo> results;
    ...
    for (auto result : rbi->getResults()) {
      if (result.use_empty()) {
        ...
        if (auto dealloc =
                toolchain::dyn_cast<memref::DeallocOp>(*result.getUsers().begin())) {
    ...
    toolchain::EquivalenceClasses<Value> eq;
    auto getAliases = [&](RegionBranchPoint point) {
      for (const auto& edge : getSuccessorRegions(rbi, point)) {
        for (auto [pred, succ] : toolchain::zip(edge.getPredecessorOperands(),
    ...
    for (auto it = eq.begin(), e = eq.end(); it != e; ++it) {
      ...
      toolchain::SmallVector<memref::DeallocOp> deallocs;
      bool failed = false;
      for (auto member = eq.member_begin(**it);
        ...
        } else if (auto bbarg = toolchain::dyn_cast<BlockArgument>(*member)) {
      ...
      if (equivalentOperands.size() == deallocs.size() && !failed) {
        ...
        for (auto [dealloc, operand] :
             toolchain::zip(deallocs, toolchain::to_vector(equivalentOperands))) {
...
void promoteBuffers(Block& block) {
  ...
  for (auto* op = &block.front(); op;) {
    ...
    if (alloc) {
      ...
      auto dealloc = toolchain::find_if(alloc->getUsers(), [&](Operation* user) {
        return user->getBlock() == &block && toolchain::isa<memref::DeallocOp>(user);
      });
      ...
      if (dealloc != alloc->getUsers().end()) {
        if (op == *dealloc) {
          ...
          if (remainingAllowedStackUse >= numElements) {
            ...
            promoteToStack(toolchain::cast<memref::DeallocOp>(*dealloc));
          }
        }
      }
    }
