...
static LogicalResult walkReturnOperations(
    Region *region,
    toolchain::function_ref<LogicalResult(RegionBranchTerminatorOpInterface)> func) {
...
class BufferDeallocation : public BufferPlacementTransformationBase {
 public:
  using AliasAllocationMapT =
      toolchain::DenseMap<Value, mlir::bufferization::AllocationOpInterface>;
 ...
 private:
  ...
  LogicalResult introduceClones() {
    ...
    toolchain::SmallDenseSet<std::tuple<Value, Block *>> visitedValues;
  ...
  LogicalResult introduceBlockArgCopy(BlockArgument blockArg) {
    ...
    regionInterface.getSuccessorRegions(/*point=*/RegionBranchPoint::parent(),
                                        successorRegions);
    auto *it =
        toolchain::find_if(successorRegions, [&](RegionSuccessor &successorRegion) {
          return successorRegion.getSuccessor() == argRegion;
        });
    ...
    size_t operandIndex =
        toolchain::find(it->getSuccessorInputs(), blockArg).getIndex() +
  ...
  LogicalResult introduceClonesForRegionSuccessors(
    ...
    for (Region &region : regions) {
      ...
      RegionSuccessor *regionSuccessor =
          toolchain::find_if(successorRegions, regionPredicate);
      ...
      // bindings.
      size_t operandIndex =
          toolchain::find(regionSuccessor->getSuccessorInputs(), argValue)
              .getIndex();
