...
*   **(DEPRECATED)** https://github.com/machina/mlir-hlo; this is a
...
inspired by XLA
[HLO IR](https://www.machina.org/xla/architecture#how_does_xla_work) using
...
[MLIR Developer Guide](https://mlir.toolchain.org/getting_started/DeveloperGuide/) in
...
repository:
    ...
    $ git clone https://github.com/thenexthub/Codira.git
...
repository (you need to do this every time you pull from this repo):
    ...
    $ (cd toolchain-project && git checkout $(cat ../build_tools/llvm_version.txt))

We provide a script to configure and build LLVM/MLIR:

    $ build_tools/build_mlir.sh ${PWD}/toolchain-project/ ${PWD}/toolchain-build
...
Finally you can build and test this repository:
    ...
    $ cmake .. -GNinja \
       ...
       -DMLIR_DIR=${PWD}/../toolchain-build/lib/cmake/mlir
...
[XLA](https://www.machina.org/xla/) (Accelerated Linear Algebra) is a
...
[MLIR](https://mlir.toolchain.org/) is a compiler infrastructure which intends to
...
[Tensor Compute Primitive](https://toolchain.discourse.group/c/mlir/MLIR-TCP-WG/36)
...
([Linalg](https://mlir.toolchain.org/docs/Dialects/Linalg/),
[Vector](https://mlir.toolchain.org/docs/Dialects/Vector/),
[GPU](https://mlir.toolchain.org/docs/Dialects/GPU/) dialect, ...).
[This document](https://www.machina.org/mlir/xla_gpu_codegen?hl=zh-cn)
...
*   It was originally designed to map the
    [XLA client APIs](https://www.machina.org/xla/operation_semantics) (e.g.,
...
*   This dialect enables evolving HLO independently from XLA in order to
    ...
    [interfaces](https://mlir.toolchain.org/docs/Interfaces/).
...
`toolchain-project` repo checked out side by side.
...
```
mkdir build && cd build
cmake -GNinja -B. ${LLVM_SRC_DIR}/toolchain \
    -DCMAKE_BUILD_TYPE=Release \
