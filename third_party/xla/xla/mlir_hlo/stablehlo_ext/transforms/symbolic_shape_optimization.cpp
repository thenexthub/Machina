...
struct SimplifyBroadcasts : public mlir::OpRewritePattern<shape::BroadcastOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<ArrayRef<SymbolicExpr>> shapesInfo;
    ...
    SmallVector<std::optional<SymbolicBroadcastDimension>> symResult(
        rank, std::nullopt);
    for (const auto &sInfo : toolchain::enumerate(shapesInfo)) {
      size_t dimOffset = rank - sInfo.value().size();
      for (const auto &symExpr : toolchain::enumerate(sInfo.value())) {
    ...
    auto elements = toolchain::to_vector<8>(
        toolchain::map_range(symResult, [&](const auto &symResultDim) {
...
LogicalResult analyzeDynamicBroadcastInDimExpandingBehavior(
    ...
    toolchain::SmallSetVector<int64_t, 4> *knownExpandingDims,
    toolchain::SmallSetVector<int64_t, 4> *knownNonexpandingDims) {
...
struct AnnotateExpandingDimensionsInDynamicBroadcastInDim
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallSetVector<int64_t, 4> knownExpandingDims, knownNonexpandingDims;
    ...
    auto insertAll = [](toolchain::SmallSetVector<int64_t, 4> &dst,
    ...
    auto isEqual = [](toolchain::SmallSetVector<int64_t, 4> &set,
                      ArrayRef<int64_t> attr) {
      return static_cast<int64_t>(set.size()) == attr.size() &&
             toolchain::all_of(attr, [&](auto it) { return set.count(it); });
    };
...
bool isProduct(AffineExpr expr,
               toolchain::function_ref<void(AffineConstantExpr)> cbkConstantFactor,
               toolchain::function_ref<void(AffineSymbolExpr)> cbkSymbolicFactor) {
...
bool isSymbolicProduct(const SymbolicExpr &symbolicExpr,
                       toolchain::function_ref<void(int64_t)> cbkConstantFactor,
                       toolchain::function_ref<void(Symbol)> cbkSymbolicFactor) {
  return isProduct(
      symbolicExpr.expr,
...
struct SymbolicProduct {
  ...
  toolchain::SmallVector<Symbol> symbolic;
...
SymbolicProduct eliminateCommonFactors(SymbolicProduct &a, SymbolicProduct &b) {
  ...
  while (i < static_cast<int64_t>(a.symbolic.size())) {
    auto *it = toolchain::find(b.symbolic, a.symbolic[i]);
...
SmallVector<int64_t> concretizeOperandShape(
  ...
  for (auto it : toolchain::zip(operandShape, operandShapeInfo)) {
  ...
  for (const DimensionGroup &g : dimGroups) {
    if (g.kind == kind) {
      ...
      reassociation.push_back(
          toolchain::to_vector(toolchain::seq<int64_t>(i, i + g.size)));
      i += g.size;
      continue;
    }
...
bool isKnownBroadcastable(ShapeComponentAnalysis &analysis,
  ...
  for (Value shape : bcastedShapes) {
    ...
    for (auto zip : toolchain::zip(toolchain::reverse(*outputShapeDims),
                              toolchain::reverse(*shapeDims))) {
...
std::optional<Value> simplifyBroadcast(ShapeComponentAnalysis &analysis,
  ...
  size_t maxRank = 0;
  for (const auto &shape : toolchain::enumerate(shapes)) {
  ...
  for (const auto &shape : toolchain::enumerate(shapesFound)) {
    for (const auto &dim : toolchain::enumerate(toolchain::reverse(shape.value()))) {
  ...
  if (toolchain::all_equal(shapeAndRankForDim) && shapeAndRankForDim[0].first) {
