...
toolchain::LogicalResult isSupportedQuantCompositeOp(stablehlo::CompositeOp op) {
  ...
  if (op.getName() == "quant.fake_quant") {
    ...
    auto dtypeAttr = toolchain::dyn_cast_or_null<TypeAttr>(
...
LogicalResult getQuantCompositeAttributes(
  ...
  // Extract scales
  auto scaleAttr = toolchain::dyn_cast_or_null<DenseFPElementsAttr>(
  ...
  auto zeroPointAttr = toolchain::dyn_cast_or_null<DenseIntElementsAttr>(
      op.getCompositeAttributes().get("zero_point"));
  if (zeroPointAttr) {
    auto tempVec = toolchain::to_vector(zeroPointAttr.getValues<int64_t>());
    zeroPoints.assign(tempVec.begin(), tempVec.end());
  }

  // Extract quantized dimension. If not present, set to -1.
  quantizedDimension = -1;
  auto quantDimensionAttr = toolchain::dyn_cast_or_null<IntegerAttr>(
  ...
  auto storageTypeMinAttr = toolchain::dyn_cast_or_null<IntegerAttr>(
  ...
  auto storageTypeMaxAttr = toolchain::dyn_cast_or_null<IntegerAttr>(
...
class RewriteQuantizeCompositeOp
  ...
  LogicalResult matchAndRewrite(stablehlo::CompositeOp op,
    ...
    RankedTensorType outputQuantizedType = RankedTensorType::get(
        toolchain::cast<ShapedType>(op.getResults().front().getType()).getShape(),
...
class RewriteDequantizeCompositeOp
  ...
  LogicalResult matchAndRewrite(stablehlo::CompositeOp op,
    ...
    Type quantizedElementType = stablehlo::getQuantizedElementType(
        ...
        quantizedDimension, storageTypeMin, storageTypeMax);
    auto quantizedType =
        toolchain::cast<ShapedType>(op.getResults().front().getType())
    ...
    if (isa<BlockArgument>(op.getOperand(0))) {
      ...
      int updatedArgIdx = -1;
      for (auto [i, arg] : toolchain::enumerate(funcOp.getArguments())) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(stablehlo::CompositeOp op,
    ...
    RankedTensorType quantizedType = RankedTensorType::get(
        toolchain::cast<ShapedType>(op.getType(0)).getShape(), quantizedElementType);
    auto stablehloQuantizeOp = rewriter.create<stablehlo::UniformQuantizeOp>(
