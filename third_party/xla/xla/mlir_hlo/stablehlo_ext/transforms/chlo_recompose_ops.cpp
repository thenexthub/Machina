...
FailureOr<Attribute> deserializeRaggedDotDimensionNumbersAttr(
  ...
  auto arrayAttr = toolchain::dyn_cast<ArrayAttr>(attr.getValue());
  if (!arrayAttr || arrayAttr.size() != 6)
    return op->emitError() << "ragged_dot_dimension_numbers is not an "
                              "ArrayAttr with 6 elements";
  auto lhsBatch = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[0]);
  auto rhsBatch = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[1]);
  auto lhsContract = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[2]);
  auto rhsContract = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[3]);
  auto lhsRagged = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[4]);
  auto rhsGroup = toolchain::dyn_cast<DenseIntElementsAttr>(arrayAttr[5]);
  ...
  return chlo::RaggedDotDimensionNumbersAttr::get(
      op->getContext(), toolchain::to_vector(lhsBatch.getValues<int64_t>()),
      toolchain::to_vector(rhsBatch.getValues<int64_t>()),
      toolchain::to_vector(lhsContract.getValues<int64_t>()),
      toolchain::to_vector(rhsContract.getValues<int64_t>()),
      toolchain::to_vector(lhsRagged.getValues<int64_t>()),
      toolchain::to_vector(rhsGroup.getValues<int64_t>()));
}
...
FailureOr<DictionaryAttr> getCustomCallOpAttributes(stablehlo::CustomCallOp op,
  ...
  auto attrs = toolchain::dyn_cast_or_null<DictionaryAttr>(
...
LogicalResult recomposeChloOpFromCustomCall(stablehlo::CustomCallOp op,
  ...
  if (!toolchain::all_of(op->getAttrs(), isSupportedAttrName) ||
...
LogicalResult recomposeChloOpFromCustomCall(
  ...
  if (!toolchain::is_contained(customCallNames, op.getCallTargetName()))
    return rewriter.notifyMatchFailure(
...
struct ChloRecomposeOpsPass
  ...
  void runOnOperation() override {
    ...
    toolchain::SmallVector<Operation*> candidateOps;
    moduleOp.walk(
