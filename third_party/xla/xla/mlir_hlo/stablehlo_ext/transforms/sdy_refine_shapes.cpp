...
LogicalResult refineValues(
  ...
  for (auto it : toolchain::zip(blockArguments, types)) {
  ...
  for (auto it : toolchain::zip(blockArguments, refinedTypes)) {
    // Cannot use structured bindings to simplify this because capturing
    // structured bindings in a lambda is a C++ 20 extension.
    ...
    if (toolchain::none_of(value.getUses(), isFuncReturn)) continue;
...
LogicalResult refineManualComputationBody(
  ...
  for (auto [arg, globalType, inSharding] :
       toolchain::zip_equal(manualComputation.getBody().getArguments(),
  ...
  for (auto [localType, sharding] :
       toolchain::zip_equal(sdy::getBodyTerminatorOpOperandTypes(manualComputation),
...
LogicalResult refineNamedComputationOpPattern(
  ...
  return stablehlo::refineReturnTypes(
      ...
      toolchain::to_vector(namedComputation.getBody().front().getTerminator()
                      ->getOperandTypes()));
}
