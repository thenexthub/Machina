...
LogicalResult DynamicReduceWindowOpAdaptor::verify() {
  ...
  for (auto [index, input] : toolchain::enumerate(inputs)) {
  ...
  for (auto [index, initValue] : toolchain::enumerate(initValues)) {
  ...
  for (auto [inputType, initValueType] :
       toolchain::zip(inputTypes, initValueTypes)) {
  ...
  toolchain::append_range(expectedBodyInputs, initValueTypes);
  toolchain::append_range(expectedBodyInputs, initValueTypes);
  SmallVector<Type> expectedBodyOutputs;
  toolchain::append_range(expectedBodyOutputs, initValueTypes);
  ...
  for (auto [resultType, initValueType] :
       toolchain::zip(resultTypes, initValueTypes)) {
