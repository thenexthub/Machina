...
class GatherIsSlice : public OpRewritePattern<GatherOp> {
  ...
  LogicalResult matchAndRewrite(GatherOp gather,
    ...
    for (const auto& it : toolchain::enumerate(dimensionNumbers.getOffsetDims())) {
    ...
    for (const auto& it : toolchain::enumerate(resultTy.getShape())) {
    ...
    toolchain::SmallVector<Value, 4> sliceStartIndices;
