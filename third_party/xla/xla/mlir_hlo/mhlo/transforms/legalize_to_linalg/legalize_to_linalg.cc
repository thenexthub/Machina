...
bool verifyHloOpBufferOrTensorSemantics(Operation* op) {
  ...
  if (!toolchain::all_of(op->getOperands(), verifyType)) return false;
  return toolchain::all_of(op->getResults(), verifyType);
}
...
AffineMap getTransposeMapForReduction(MLIRContext* context, int rank,
  ...
  toolchain::SmallSetVector<int, 4> s;
...
static bool hasCanonicalDimensionNumbers(
  ...
  const int inputSpatialRank =
      toolchain::size(dimensionNumbers.getInputSpatialDimensions());
  ...
  const int kernelSpatialRank =
      toolchain::size(dimensionNumbers.getKernelSpatialDimensions());
  ...
  const int outputSpatialRank =
      toolchain::size(dimensionNumbers.getOutputSpatialDimensions());
...
struct RngUniformConversion : public OpConversionPattern<mhlo::RngOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    // https://reviews.toolchain.org/D101364
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        [&](OpBuilder& b, Location loc, ValueRange args) {
          toolchain::SmallVector<Value> updateVec = {b.create<arith::ConstantOp>(
...
SmallVector<utils::IteratorType, 3> getEinsumLoopsAttrs(
    const toolchain::SmallSetVector<StringRef, 4>& inputInd,
    const toolchain::SmallSetVector<StringRef, 4>& reductionDims) {
...
toolchain::SmallSetVector<StringRef, 4> findSummationAxes(
    const toolchain::SmallSetVector<StringRef, 4>& inputSet,
    const toolchain::SmallSetVector<StringRef, 4>& outputSet) {
  toolchain::SmallSetVector<StringRef, 4> summationAxes;
...
class EinsumToLinalgConverter : public OpConversionPattern<mhlo::EinsumOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallSetVector<StringRef, 4> inputInd;
    toolchain::SmallSetVector<StringRef, 4> outputInd;

    inputInd.insert(lhsEin.begin(), lhsEin.end());
    inputInd.insert(rhsEin.begin(), rhsEin.end());
    outputInd.insert(outEin.begin(), outEin.end());

    toolchain::SmallSetVector<StringRef, 4> reductionAxe =
    ...
    for (const auto& it : toolchain::enumerate(inputInd)) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    Value opResult = mhlo::MhloOpToStdScalarOp::mapOp(
        mhloOp, argType.getElementType(), toolchain::ArrayRef<Value>{lhs, rhs},
...
class BroadcastConverter
 ...
 public:
  ...
  static SmallVector<AffineMap, 2> getIndexingMaps(OpTy broadcastOp,
    ...
    unsigned numPrependedDims = toolchain::size(broadcastOp.getBroadcastSizes());
...
class BroadcastOpToBroadcastConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    SmallVector<int64_t> dimensions =
        toolchain::to_vector(toolchain::seq<int64_t>(0, numPrependedDims));
...
Value collapseExpandingDims(PatternRewriter& rewriter, Location loc,
                            ...
                            toolchain::function_ref<bool(int64_t)> isExpandingDim) {
  ...
  for (const auto& [idx, dim] : toolchain::enumerate(dimensions)) {
...
Value transposeBroadcastOperand(PatternRewriter& rewriter, Location loc,
  ...
  if (toolchain::is_sorted(dimensions)) return operand;

  SmallVector<int64_t> permutation =
      toolchain::to_vector(toolchain::seq<int64_t>(0, dimensions.size()));
  toolchain::sort(permutation, [&](int64_t lhs, int64_t rhs) {
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    SmallVector<int64_t> broadcastDimensions =
        toolchain::to_vector(op.getBroadcastDimensions().getValues<int64_t>());
    ...
    for (int64_t dim : toolchain::seq<int64_t>(0, resultTy.getRank())) {
      if (!toolchain::is_contained(broadcastDimensions, dim))
...
class HloDynamicBroadcastInDimConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto bcastDims = toolchain::to_vector(
        toolchain::map_range(op.getBroadcastDimensions(), [](const APInt& d) {
          return static_cast<int64_t>(d.getLimitedValue());
        }));
    for (const auto& it : toolchain::enumerate(operandType.getShape())) {
    ...
    if (!toolchain::all_of(dimExprs, [](AffineExpr expr) { return expr; }))
      return failure();
    ...
    rewriter.replaceOpWithNewOp<linalg::GenericOp>(
        ...
        toolchain::ArrayRef({AffineMap::get(/*dimCount=*/nloops, /*symbolCount=*/0,
...
class DynamicBroadcastInDimOpToBroadcastConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // Use static type info.
    for (const auto& [idx, dim] : toolchain::enumerate(operandTy.getShape())) {
    ...
    // Fail if unknown expansion behavior remains.
    if (toolchain::any_of(expansionBehavior, [](auto v) { return !v.has_value(); }))
      return failure();
 ...
 private:
  static Value getBroadcastOperand(
    ...
    SmallVector<int64_t> updatedOperandShape =
        toolchain::to_vector(operandTy.getShape());
  ...
  static ShapedType getBroadcastResultType(
    ...
    auto broadcastResultShape = toolchain::to_vector(resultTy.getShape());

    for (const auto& [operandIndex, resultIndex] :
         toolchain::enumerate(dimensions)) {
...
class TransposeConverter
 ...
 public:
  ...
  static SmallVector<AffineMap, 2> getIndexingMaps(OpTy op, Builder* b) {
    ...
    for (const auto& permutation : toolchain::enumerate(op.getPermutation())) {
...
class TransposeOpToTransposeConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    auto permutation = rewriter.getDenseI64ArrayAttr(
        toolchain::to_vector(op.getPermutation().getValues<int64_t>()));
...
class RealDynamicSliceConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto i : toolchain::seq<unsigned>(0, argType.getRank())) {
...
class ReshapeOpConverter : public OpConversionPattern<mhlo::ReshapeOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (toolchain::is_contained(resultType.getShape(), 0)) {
    ...
    if (resultType.getRank() == 0 && !operandType.hasStaticShape()) {
      ...
      auto staticType = RankedTensorType::get(
          toolchain::SmallVector<int64_t>(operandType.getRank(), 1), elemType);
    ...
    if (std::optional<SmallVector<ReassociationIndices>> reassociationMap =
      ...
      if (resultType.getRank() < operandType.getRank()) {
        ...
        for (const auto& map : toolchain::enumerate(*reassociationMap)) {
  ...
  LogicalResult matchAndRewrite(
    ...
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        ValueRange{getEmptyTensorFor(rewriter, loc, resultShapedType, iotaOp,
                                     adaptor.getOperands())},
        toolchain::ArrayRef(rewriter.getMultiDimIdentityMap(nloops)),
        getNParallelLoopsAttrs(nloops),
        [&](OpBuilder& nestedBuilder, Location nestedLoc, ValueRange /*args*/) {
  ...
  LogicalResult matchAndRewrite(
    ...
    rewriter.replaceOpWithNewOp<linalg::GenericOp>(
        ...
        [&](OpBuilder& nestedBuilder, Location loc, ValueRange) {
          ...
          for (const auto& it : toolchain::enumerate(adaptor.getOperands())) {
            Value arg = it.value();
...
class DynamicSliceConverter : public OpConversionPattern<mhlo::DynamicSliceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (const auto& en : toolchain::enumerate(
             toolchain::zip(adaptor.getStartIndices(),
...
class DynamicUpdateSliceConverter
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // https://www.machina.org/xla/operation_semantics#dynamicupdateslice
    ...
    Value zero = rewriter.create<arith::ConstantIndexOp>(loc, 0);
    for (const auto& en : toolchain::enumerate(adaptor.getStartIndices())) {
...
class MapOpToGenericConverter : public OpConversionPattern<mhlo::MapOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (const auto& it : toolchain::enumerate(op.getOperation()->getOperands())) {
...
class MapOpToMapConverter : public OpConversionPattern<mhlo::MapOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (Value operand : toolchain::drop_begin(adaptor.getOperands(), 1)) {
...
class ReduceOpToGenericConverter : public OpConversionPattern<mhlo::ReduceOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (toolchain::any_of(adaptor.getInputs(), [](Value v) {
    ...
    for (auto [operand, initValue, resultType] :
         toolchain::zip(adaptor.getInputs(), adaptor.getInitValues(), resultTypes)) {
    ...
    // the original RHS.
    for (const auto& [idx, val] : toolchain::enumerate(op.getInputs())) {
      signatureConverter.addInputs(
    ...
    for (const auto& [idx, val] : toolchain::enumerate(op.getInitValues())) {
      signatureConverter.addInputs(
          /*origInputNo=*/idx,
...
struct ReduceOpToReduceConverter : public OpConversionPattern<mhlo::ReduceOp> {
  ...
  LogicalResult matchAndRewrite(
      ...
      ConversionPatternRewriter& rewriter) const final {
    auto reductionDims =
        toolchain::to_vector(op.getDimensions().getValues<int64_t>());
    // mhlo.reduce doesn't specify the order of the reduction dimensions.
    toolchain::sort(reductionDims);
    ...
    for (auto [operand, initValue, resultType] :
      ...
      SmallVector<Value, 8> dynShape;
      for (const auto& [index, dim] : toolchain::enumerate(
               mlir::cast<ShapedType>(operand.getType()).getShape())) {
        if (!toolchain::is_contained(reductionDims, index)) {
    ...
    for (auto [result, resultType] :
         toolchain::zip(linalgOp.getResults(), resultTypes)) {
      results.push_back(
...
struct SelectAndScatterNoOverlapConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<int64_t> pad(rank * 2, 0);
    if (op.getPadding().has_value())
      pad = toolchain::to_vector(op.getPaddingAttr().getValues<int64_t>());
    ...
    auto strides =
        toolchain::to_vector(op.getWindowStridesAttr().getValues<int64_t>());
    auto window =
        toolchain::to_vector(op.getWindowDimensionsAttr().getValues<int64_t>());
    ...
    if (toolchain::all_of(window, [](auto sz) { return sz == 1; })) {
      return rewriter.notifyMatchFailure(op,
    ...
    toolchain::SmallVector<int64_t> broadcastShape;
    toolchain::SmallVector<Value> broadcastDynDims;
    toolchain::SmallVector<AffineExpr> broadcastExprs;
...
struct PadOpNegativePaddingConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    SmallVector<OpFoldResult, 4> sizes(toolchain::map_range(
        op.getType().getShape(),
...
struct PadOpConversion : public OpConversionPattern<mhlo::PadOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (toolchain::any_of(op.getEdgePaddingLow().getValues<APInt>(), isNegative) ||
        toolchain::any_of(op.getEdgePaddingHigh().getValues<APInt>(), isNegative))
      return failure();
    ...
    if (toolchain::all_of(op.getInteriorPadding().getValues<APInt>(),
    ...
    auto sizes = toolchain::to_vector<4>(toolchain::map_range(
        toolchain::seq<int64_t>(0, operandType.getRank()),
    ...
    auto strides = toolchain::to_vector<4>(
        toolchain::map_range(op.getInteriorPadding().getValues<IntegerAttr>(),
...
Value applyConvolutionPadding(Location loc, Value input,
                              ...
                              toolchain::ArrayRef<int64_t> dimMappings,
                              OpBuilder& rewriter) {
  ...
  if (padding) {
    ...
    for (auto i : toolchain::seq<int64_t>(0, padding.size() / 2)) {
  ...
  if (lhsDilation) {
    ...
    for (auto i : toolchain::seq<int64_t>(0, lhsDilation.size())) {
      auto dim = dimMappings[i];
...
Value applyConvolutionReversal(Location loc, OpBuilder& b, ConvolutionOp op,
  ...
  toolchain::SmallVector<int64_t> reversedDims;
  for (const auto& [idx, reversed] :
       toolchain::enumerate(reversals.value().getValues<bool>())) {
    if (reversed) {
...
struct NormalConvolutionOpConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<int64_t> spatialDimMapping(rank - 2);
...
struct ConvolutionOpGeneralConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    // Immediately emit an EmptyOp for output tensors with zero dimension.
    if (toolchain::is_contained(reshapedResultShape, 0)) {
    ...
    auto updateDimMappingFromOffset =
        [](toolchain::SmallVectorImpl<int64_t>& mapping, int64_t offset) {
          for (auto& mappingElt : toolchain::drop_begin(mapping, offset)) {
            mappingElt += 1;
          }
        };
    ...
    auto reshapeShapeVector = [](toolchain::ArrayRef<int64_t> oldShape,
                                 toolchain::SmallVectorImpl<int64_t>& newShape,
                                 int64_t reshapedDim, int64_t factor) {
      newShape.reserve(oldShape.size() + 1);
      for (auto i : toolchain::seq<int64_t>(0, oldShape.size())) {
        if (i == reshapedDim) {
    ...
    Value convolved =
        rewriter
            .create<linalg::GenericOp>(
                loc,
                /*resultTensors=*/
                toolchain::ArrayRef<Type>(zeroTensor.getType()),
                /*inputs=*/
                toolchain::ArrayRef<Value>({modifiedLhs, modifiedRhs}),
                /*outputs=*/toolchain::ArrayRef<Value>(zeroTensor), inferredMaps,
                iterationLoops,
                /*bodyBuild=*/
...
struct DepthwiseConvolutionOpConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    // Apply padding and input dilation.
    toolchain::SmallVector<int64_t> spatialDimMapping(spatialRank);
    std::iota(spatialDimMapping.begin(), spatialDimMapping.end(), 1);
    input = applyConvolutionPadding(loc, input, op.getPaddingAttr(),
                                    op.getLhsDilationAttr(), spatialDimMapping,
                                    rewriter);

    auto filterDims = toolchain::to_vector<4>(
...
struct ReduceWindowOpOnTensorsGenericConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    Location loc = op.getLoc();
    toolchain::SmallVector<Value> initValues = adaptor.getInitValues();
    toolchain::SmallVector<Type> resultTypes;
    if (failed(typeConverter->convertTypes(op.getResultTypes(), resultTypes)))
      return failure();
    auto numOperands = initValues.size();

    toolchain::SmallVector<int64_t> windowDimensions =
        extract1DVector(op.getWindowDimensions());

    toolchain::SmallVector<int64_t> padding;
    if (op.getPadding()) {
      padding = extract1DVector(*op.getPadding());
    }

    toolchain::SmallVector<int64_t> baseDilations;
    if (op.getBaseDilations()) {
      baseDilations = extract1DVector(*op.getBaseDilations());
    }

    toolchain::SmallVector<int64_t> windowStrides(windowDimensions.size(), 1);
    if (op.getWindowStrides()) {
      windowStrides = extract1DVector(*op.getWindowStrides());
    }

    toolchain::SmallVector<int64_t> windowDilations(windowDimensions.size(), 1);
    ...
    toolchain::SmallVector<Value> broadcastValues;
    ...
    toolchain::SmallVector<Value> inputs = toolchain::to_vector(adaptor.getInputs());

    // Pad as necessary.
    if (toolchain::any_of(padding, [](int64_t v) { return v != 0; }) ||
        toolchain::any_of(baseDilations, [](int64_t v) { return v != 1; })) {
      toolchain::SmallVector<int64_t> staticLows(rank, 0);
      toolchain::SmallVector<int64_t> staticHighs(rank, 0);
      ...
      toolchain::SmallVector<int64_t> staticInteriors(rank, 0);
      for (const auto& dilation : toolchain::enumerate(baseDilations)) {
      ...
      for (auto values : toolchain::zip(inputs, initValues)) {
    ...
    for (const auto& [i, type] : toolchain::enumerate(resultTypes)) {
    ...
    for (const auto& [i, input] :
         toolchain::enumerate(ArrayRef<Value>(inputs).drop_back())) {
      signatureConverter.addInputs(
...
struct ReduceWindowOpConversion
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto it : toolchain::zip(op.getResults(), operands, initValues)) {
      ...
      for (const auto& en : toolchain::enumerate(resultType.getShape())) {
...
/// https://www.machina.org/xla/operation_semantics#gather. The compiler
...
struct GatherConversion : public OpConversionPattern<mhlo::GatherOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    for (int dim = 0; dim < resultRank; ++dim)
      if (!toolchain::is_contained(offsetDims, dim)) batchDims.push_back(dim);
    ...
    for (const auto& it : toolchain::enumerate(startIndexMap))
    ...
    for (auto [operandDim, indicesDim] :
         toolchain::zip_equal(operandBatchingDims, startIndicesBatchingDims)) {
      indexFromBatching[operandDim] =
          gatherIndex[indicesDim + (indicesDim < indexVectorDim ? 0 : 1)];
    }

    auto isCollapsedOrBatching = [&](int64_t dim) {
      return toolchain::is_contained(collapsedSliceDims, dim) ||
             toolchain::is_contained(operandBatchingDims, dim);
    };
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto getMap = [&](int64_t rank, ArrayRef<int64_t> batchingDims,
      ...
      toolchain::SmallVector<AffineExpr> indices(rank);
      for (const auto& i : toolchain::enumerate(batchingDims)) {
        indices[i.value()] = rewriter.getAffineDimExpr(i.index());
      }
      for (const auto& i : toolchain::enumerate(contractingDims)) {
...
class PointwiseToLinalgMapConverter : public OpConversionPattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!toolchain::all_of(adaptor.getOperands(), [&](Value v) {
          int64_t r = getRank(v);
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    SmallVector<OpFoldResult> strides(resultType.getRank(),
                                      rewriter.getIndexAttr(1));
    SmallVector<OpFoldResult> sizes(toolchain::map_range(
