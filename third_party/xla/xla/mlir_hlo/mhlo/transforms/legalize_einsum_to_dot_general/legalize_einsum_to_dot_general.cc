  ...
  LogicalResult matchAndRewrite(EinsumOp einsum,
    ...
    auto collectOperandDims =
        [resultTokens](
          ...
          toolchain::SmallDenseSet<char> othersSet(others.begin(), others.end());
          toolchain::SmallDenseSet<char> resultTokensSet(resultTokens.begin(),
                                                    resultTokens.end());
          for (const auto &en : toolchain::enumerate(operandTokens)) {
    ...
    for (const auto &it : toolchain::enumerate(lhsBatchingDims)) {
