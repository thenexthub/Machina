...
static void prepareBroadcastInDim(BroadcastInDimOp bcast) {
  ...
  if (toolchain::is_sorted(dims.getValues<int64_t>())) return;
  ...
  SmallVector<int64_t> transposedDim =
      to_vector(toolchain::seq<int64_t>(0, dims.size()));
  auto rawDims = dims.getValues<int64_t>();
  toolchain::sort(transposedDim, [&](int64_t lhs, int64_t rhs) {
...
static void prepareExplicitCapturedConstants(Operation *op) {
  for (Region &region : op->getRegions()) {
    ...
    toolchain::SetVector<Value> implicitInputs;
