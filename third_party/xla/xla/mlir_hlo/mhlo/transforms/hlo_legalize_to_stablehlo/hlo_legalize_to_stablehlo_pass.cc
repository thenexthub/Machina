...
struct AddDependencyOpToStablehloTokenConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!toolchain::isa<stablehlo::TokenType>(adaptor.getToken().getType()))
...
bool hasMhloOperand(Operation* op) {
  return toolchain::any_of(op->getOperandTypes(), [](Type type) {
    ...
    if (toolchain::isa<mhlo::MhloDialect>(type.getDialect())) return true;

    // Check for tensor<X, #stablehlo.bounds<...>>
    if (auto rankedType = dyn_cast<RankedTensorType>(type)) {
      return toolchain::isa_and_nonnull<mhlo::TypeExtensionsAttr>(
