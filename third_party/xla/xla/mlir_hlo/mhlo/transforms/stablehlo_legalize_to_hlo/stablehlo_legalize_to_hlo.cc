...
LogicalResult rewriteCustomCallAsMhloOp(stablehlo::CustomCallOp stablehloOp,
  ...
  if (!toolchain::all_of(stablehloOp->getAttrs(), isSupportedAttrName) ||
 ...
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    for (auto [stablehloRegion, hloRegion] :
         toolchain::zip(stablehloOp->getRegions(), hloOp->getRegions())) {
...
struct AddDependencyOpToMhoTokenConverter
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!toolchain::isa<mhlo::TokenType>(adaptor.getToken().getType()))
...
bool hasStablehloOperand(Operation* op) {
  return toolchain::any_of(op->getOperandTypes(), [](Type type) {
    // Check for !stablehlo.token
    if (toolchain::isa<stablehlo::StablehloDialect>(type.getDialect())) return true;

    // Check for tensor<X, #stablehlo.bounds<...>>
    if (auto rankedType = dyn_cast<RankedTensorType>(type)) {
      return toolchain::isa_and_nonnull<stablehlo::TypeExtensionsAttr>(
...
struct UpdateOperandsInUnknownOp : public ConversionPattern {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (toolchain::isa<mhlo::MhloDialect, stablehlo::StablehloDialect>(
            op->getDialect()))
