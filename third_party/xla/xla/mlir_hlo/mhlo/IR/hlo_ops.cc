...
struct AsyncBundleTypeStorage final
    : public TypeStorage,
      public toolchain::TrailingObjects<AsyncBundleTypeStorage, Type> {
...
static void replaceOpWithRegion(PatternRewriter& rewriter, Operation* op,
  ...
  assert(toolchain::hasSingleElement(region) && "expected single-block region");
...
LogicalResult AsyncDoneOp::inferReturnTypes(
  ...
  toolchain::append_range(inferredReturnTypes,
  ...
  if (getOperandLayouts().has_value() || getResultLayouts().has_value()) {
    ...
    auto verifyTypesAndLayouts =
      ...
      for (const auto& indexedTypeAndLayout :
           toolchain::enumerate(toolchain::zip(types, layouts))) {
...
LogicalResult DotGeneralOp::verify() {
  bool isDefaultPrecisionConfig =
      ...
      toolchain::all_of(getPrecisionConfig().value(), [](Attribute attr) {
...
LogicalResult DotGeneralOp::reifyReturnTypeShapes(
  ...
  for (int64_t i = 0; i < lhsType.getRank(); i++) {
    if (!toolchain::is_contained(dimNumbers.getLhsContractingDimensions(), i) &&
        !toolchain::is_contained(dimNumbers.getLhsBatchingDimensions(), i)) {
      dimensions.push_back(
          builder.create<tensor::DimOp>(getLoc(), adaptor.getLhs(), i));
    }
  }
  for (int64_t i = 0; i < rhsType.getRank(); i++) {
    if (!toolchain::is_contained(dimNumbers.getRhsContractingDimensions(), i) &&
        !toolchain::is_contained(dimNumbers.getRhsBatchingDimensions(), i)) {
      dimensions.push_back(
...
LogicalResult checkRaggedDotConstraints(
  ...
  Mode mode;
  if (toolchain::is_contained(lhsBatchingDimensions, lhsRaggedDim)) {
    mode = kBatch;
  } else if (toolchain::is_contained(lhsContractingDimensions, lhsRaggedDim)) {
    mode = kContracting;
  } else {
      ...
      case kBatch:
        prefixDims.resize(
            std::distance(lhsBatchingDimensions.begin(),
                          toolchain::find(lhsBatchingDimensions, lhsRaggedDim)));
        break;
      case kContracting:
        prefixDims.insert(prefixDims.end(), lhsContractingDimensions.begin(),
                          toolchain::find(lhsContractingDimensions, lhsRaggedDim));
        break;
      case kNonContracting:
        for (int64_t i = 0; i < lhsRaggedDim; ++i) {
          if (!toolchain::is_contained(lhsBatchingDimensions, i) &&
              !toolchain::is_contained(lhsContractingDimensions, i)) {
...
LogicalResult inferRaggedDotOp(
  ...
  if (toolchain::is_contained(lhsContractingDimensions, lhsRaggedDim)) {
  ...
  for (int64_t i = 0; i < rankedRhsType.getRank(); i++)
    if (!toolchain::is_contained(rhsBatchingDimensions, i) &&
        !toolchain::is_contained(rhsContractingDimensions, i) &&
        !toolchain::is_contained(rhsGroupDimensions, i))
...
LogicalResult ResultAccuracyAttr::verify(
    ::toolchain::function_ref<::mlir::InFlightDiagnostic()> emitError, APFloat atol,
...
LogicalResult FftOp::inferReturnTypeComponents(
  ...
  return hlo::inferFftOp(
      ...
      toolchain::to_vector(adaptor.getFftLength().getValues<int64_t>()),
      inferredReturnShapes);
}
...
struct GatherSlice : public OpRewritePattern<GatherOp> {
  ...
  LogicalResult matchAndRewrite(GatherOp gather,
    ...
    auto sliceEnd =
        toolchain::to_vector<8>(gather.getSliceSizes().getValues<int64_t>());
    toolchain::SmallVector<int64_t, 8> sliceStart(sliceEnd.size(), 0);
    for (auto it :
         toolchain::zip(dnums.getStartIndexMap(), index.getValues<APInt>())) {
    ...
    toolchain::SmallVector<int64_t, 8> sliceStride(sliceEnd.size(), 1);
    toolchain::SmallVector<int64_t, 8> sliceShape(sliceEnd.size());
    ...
    if (!collapsedSliceDims.empty()) {
      toolchain::SmallVector<int64_t, 8> reshapeShape;
      for (size_t i = 0; i < sliceShape.size(); ++i) {
        if (toolchain::count(collapsedSliceDims, i) == 0) {
...
// following https://www.machina.org/xla/operation_semantics#gather
...
LogicalResult reifyGatherShape(Op* op, OpBuilder& builder, ValueRange operands,
  ...
  toolchain::transform(sliceSizes, sliceSizes.begin(),
...
LogicalResult GatherOp::inferReturnTypeComponents(
  ...
  return hlo::inferGatherOp(
      ...
      toolchain::to_vector(adaptor.getSliceSizes().getValues<int64_t>()),
      inferredReturnShapes);
}
...
struct ConvolutionIsDot : public OpRewritePattern<mhlo::ConvolutionOp> {
  ...
  LogicalResult matchAndRewrite(mhlo::ConvolutionOp op,
    ...
    toolchain::SmallVector<int64_t> lhsShape;
    toolchain::SmallVector<int64_t> rhsShape;
    ...
    toolchain::SmallVector<int64_t> perms;
...
struct UnpackRepackSameTuple : public OpRewritePattern<TupleOp> {
  ...
  LogicalResult matchAndRewrite(TupleOp op,
    ...
    for (const auto& elementAndIdx :
         toolchain::enumerate(op.getVal().drop_front(1))) {
...
LogicalResult BroadcastOp::inferReturnTypeComponents(
  ...
  return hlo::inferBroadcastOp(
      ...
      toolchain::to_vector(adaptor.getBroadcastSizes().getValues<int64_t>()),
      inferredReturnShapes);
}
...
LogicalResult BroadcastOp::reifyReturnTypeShapes(
  ...
  for (auto index : toolchain::seq<int64_t>(0, operandType.getRank())) {
    shapeValues.push_back(
...
LogicalResult BroadcastInDimOp::verify() {
  return hlo::verifyBroadcastInDimOp(
      getLoc(), getOperand(),
      toolchain::to_vector(getBroadcastDimensions().getValues<int64_t>()),
      getResult());
}
...
OpFoldResult BroadcastInDimOp::fold(FoldAdaptor adaptor) {
  ...
  if (type == getOperand().getType()) {
    ...
    if (!std::equal(broadcastValues.begin(), broadcastValues.end(),
                    toolchain::seq<int64_t>(0, type.getRank()).begin())) {
      return {};
    }
    return getOperand();
  }
 ...
 public:
  ...
  LogicalResult matchAndRewrite(BroadcastInDimOp op,
    ...
    if (operandType.hasStaticShape() && resultType.hasStaticShape()) {
      ...
      if (toolchain::is_sorted(bsDimIndices) && sameTotalElements) {
    ...
    if (auto broadcastInDimOp = toolchain::dyn_cast_or_null<BroadcastInDimOp>(
...
LogicalResult DynamicBroadcastInDimOp::verify() {
  ...
  return hlo::verifyDynamicBroadcastInDimOp(
      ...
      getKnownExpandingDimensionsAttr()
          ? std::optional<SmallVector<int64_t>>(toolchain::to_vector(
...
OpTy refineOpWithNewOp(PatternRewriter& rewriter, Operation* op,
  ...
  toolchain::SmallVector<Value> replacementResults;
  ...
  for (auto [opResult, newOpResult] :
    ...
    if (toolchain::any_of(opResult.getUsers(), [&](Operation* user) {
...
class ConcatenateOperandRemoval : public OpRewritePattern<ConcatenateOp> {
 public:
  ...
  LogicalResult matchAndRewrite(ConcatenateOp op,
    ...
    toolchain::SmallVector<Value, 6> newOperands;
...
class ConcatenateForwarding : public OpRewritePattern<ConcatenateOp> {
  ...
  LogicalResult matchAndRewrite(ConcatenateOp op,
    ...
    int operandCount = 0;
    toolchain::for_each(op.getVal(), [&](Value val) {
...
LogicalResult ConcatenateOp::reifyReturnTypeShapes(
  ...
  for (size_t inputId = 0; inputId < inputs.size(); ++inputId) {
    ...
    for (const auto& element : toolchain::enumerate(operandType.getShape())) {
...
struct DynamicSliceToSlice : public OpRewritePattern<DynamicSliceOp> {
  ...
  LogicalResult matchAndRewrite(DynamicSliceOp dynamicSlice,
    ...
    for (const auto& indexAndSliceStart :
         toolchain::enumerate(dynamicSlice.getStartIndices())) {
      APInt val;
...
struct RealDSliceToDSlice : public OpRewritePattern<RealDynamicSliceOp> {
  ...
  LogicalResult matchAndRewrite(RealDynamicSliceOp op,
    ...
    if (!toolchain::all_of(stridesAttr.getValues<APInt>(),
...
LogicalResult MapOp::inferReturnTypeComponents(
  ...
  return hlo::inferMapOp(
      ...
      toolchain::to_vector(adaptor.getDimensions().getValues<int64_t>()),
      adaptor.getComputation(), inferredReturnShapes);
}
...
LogicalResult ReduceWindowOp::inferReturnTypeComponents(
  ...
  return hlo::inferReduceWindowOp(
      ...
      toolchain::to_vector(adaptor.getWindowDimensions().getValues<int64_t>()),
      adaptor.getWindowStrides()
          ? toolchain::to_vector(adaptor.getWindowStrides()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
      adaptor.getBaseDilations()
          ? toolchain::to_vector(adaptor.getBaseDilations()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
      adaptor.getWindowDilations()
          ? toolchain::to_vector(adaptor.getWindowDilations()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
...
LogicalResult ReduceWindowOp::verify() {
  ...
  return hlo::verifyReduceWindowOp(
      ...
      toolchain::to_vector(getWindowDimensions().getValues<int64_t>()),
      getWindowStrides()
          ? toolchain::to_vector(getWindowStrides()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
      getBaseDilations()
          ? toolchain::to_vector(getBaseDilations()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
      getWindowDilations()
          ? toolchain::to_vector(getWindowDilations()->getValues<int64_t>())
          : ArrayRef<int64_t>{},
      getPadding(), getBody());
}
...
void ReduceWindowOp::build(
  ...
  toolchain::SmallVector<Type> blockArgTypes;
  toolchain::SmallVector<Location> locs;
  ...
  for (auto i : inputs) {
    ...
    blockArgTypes.push_back(
        iType.cloneWith(toolchain::ArrayRef<int64_t>(), iType.getElementType()));
  ...
  toolchain::SmallVector<mlir::Type, 2> inferredReturnTypes;
  if (mlir::succeeded(ReduceWindowOp::inferReturnTypes(
    ...
    toolchain::report_fatal_error("Failed to infer result type(s).");
}
...
OpFoldResult ReverseOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::all_of(dims.getValues<int64_t>(), [&](int64_t dim) {
...
static LogicalResult tryFoldZeroDimReduction(
  ...
  for (auto [operand, opResult] :
       toolchain::zip_equal(reduceOp.getInputs(), reduceOp.getResults())) {
...
static LogicalResult tryFoldOutsideValuesReduction(
  ...
  for (auto [result, opResult] :
       toolchain::zip_equal(retOp.getResults(), reduceOp.getResults())) {
...
static bool hasSameOperandAndResultTypes(Operation& op) {
  ...
  return toolchain::all_of(op.getOperandTypes(), typeMatch) &&
         toolchain::all_of(op.getResultTypes(), typeMatch);
}

void ReduceOp::print(OpAsmPrinter& p) {
  auto dimensions = toolchain::to_vector(getDimensions().getValues<int64_t>());
...
LogicalResult ReduceOp::verify() {
  ...
  return hlo::verifyReduceOp(
      getLoc(), getInputs(), getInitValues(),
      toolchain::to_vector(getDimensions().getValues<int64_t>()), getBody());
}
...
struct LowerBoolSplatConstantsIntoRegion : public OpRewritePattern<ReduceOp> {
  ...
  LogicalResult matchAndRewrite(ReduceOp op,
    ...
    for (auto inpAndBarg : toolchain::zip(op.getOperands(), bb.getArguments())) {
...
static LogicalResult convertEmptyReduces(ReduceOp op,
  ...
  if (zeroExtent) {
    ...
    if (t.hasStaticShape()) {
      for (auto [init, out] : toolchain::zip(op.getInitValues(), op.getResults())) {
    ...
    for (auto [init, shape, out] :
         toolchain::zip(op.getInitValues(), shapes, op.getResults())) {
...
LogicalResult ReverseOp::verify() {
  ...
  return hlo::verifyReverseOp(
      getLoc(), getOperand(),
      toolchain::to_vector(getDimensions().getValues<int64_t>()));
}
...
LogicalResult PadOp::inferReturnTypes(
  ...
  return hlo::inferPadOp(
      ...
      toolchain::to_vector(adaptor.getEdgePaddingLow().getValues<int64_t>()),
      toolchain::to_vector(adaptor.getEdgePaddingHigh().getValues<int64_t>()),
      toolchain::to_vector(adaptor.getInteriorPadding().getValues<int64_t>()),
      inferredReturnTypes);
}
...
OpFoldResult padOpFoldHelper(DenseElementsAttr input, DenseElementsAttr padding,
  ...
  toolchain::SmallVector<T, 4> result(returnType.getNumElements(),
                                 padding.getValues<T>()[0]);

  auto nextIndex = [](toolchain::SmallVector<uint64_t, 8>& index,
  ...
  toolchain::SmallVector<uint64_t, 8> index(input.getType().getRank(), 0);
...
OpFoldResult PadOp::fold(FoldAdaptor adaptor) {
  ...
  if (toolchain::all_of(getEdgePaddingLow().getValues<APInt>(), isZero) &&
      toolchain::all_of(getEdgePaddingHigh().getValues<APInt>(), isZero) &&
      toolchain::all_of(getInteriorPadding().getValues<APInt>(), isZero))
    return getOperand();

  // If any padding is negative then it isn't supported by the folder (yet).
  auto isNegative = [](const APInt& i) { return i.slt(0); };
  if (toolchain::any_of(getEdgePaddingLow().getValues<APInt>(), isNegative) ||
      toolchain::any_of(getEdgePaddingHigh().getValues<APInt>(), isNegative) ||
      toolchain::any_of(getInteriorPadding().getValues<APInt>(), isNegative))
    return {};
...
LogicalResult PadOp::reifyReturnTypeShapes(
  ...
  toolchain::SmallVector<int32_t> padHigh;
  toolchain::SmallVector<int32_t> padLow;
  toolchain::SmallVector<int32_t> padInterior;
  ...
  toolchain::SmallVector<Value> dimensions;
...
struct PadEmptyTensor : public OpRewritePattern<PadOp> {
  ...
  LogicalResult matchAndRewrite(PadOp op,
    ...
    if (toolchain::all_of(operandTy.getShape(), [](int64_t d) { return d != 0; })) {
      return failure();
    }
    ...
    toolchain::SmallVector<Value> reifiedShapes;
...
LogicalResult DynamicPadOp::reifyReturnTypeShapes(
  ...
  for (int idx : toolchain::seq<int>(0, operandType.getShape().size())) {
    Value valueDim =
...
static LogicalResult inlineCaseConstantCondition(CaseOp caseOp,
  ...
  // https://www.machina.org/xla/operation_semantics#conditional
  if (index < 0 || index >= caseOp.getNumRegions())
    index = caseOp.getNumRegions() - 1;

  Region& region = caseOp.getRegion(index);
  if (!toolchain::hasSingleElement(region)) return failure();
...
struct Round {
  std::optional<APFloat> operator()(const APFloat& f) {
    APFloat r = f;
    r.roundToIntegral(toolchain::RoundingMode::NearestTiesToAway);
    return r;
  }
};
...
struct RoundNearestEven {
  std::optional<APFloat> operator()(const APFloat& f) {
    APFloat r = f;
    r.roundToIntegral(toolchain::RoundingMode::NearestTiesToEven);
    return r;
  }
};
...
#define UNARY_FOLDER_UPCAST_TO_F64(Op, Func, Validate)               \
  struct Op##Folder {                                                \
    std::optional<APFloat> operator()(const APFloat& input) {        \
      APFloat f = input;                                             \
      const toolchain::fltSemantics& oldSemantics = f.getSemantics();     \
                                                                     \
      bool unusedLoseInfo;                                           \
...
static Attribute BinaryFolder(Op* op, ArrayRef<Attribute> attrs) {
  ...
  for (const auto zip :
       toolchain::zip(lhs.getValues<ValType>(), rhs.getValues<ValType>())) {
...
LogicalResult SliceOp::inferReturnTypes(
  ...
  return hlo::inferSliceOp(
      location, adaptor.getOperand().getType(),
      toolchain::to_vector(adaptor.getStartIndices().getValues<int64_t>()),
      toolchain::to_vector(adaptor.getLimitIndices().getValues<int64_t>()),
      toolchain::to_vector(adaptor.getStrides().getValues<int64_t>()),
      inferredReturnTypes);
}
...
static void sliceElements(I values, ArrayRef<int64_t> sizes,
                          ...
                          toolchain::SmallVectorImpl<E>* outValues) {
...
static Attribute foldSlice(SliceOp* op, I values) {
  auto start = toolchain::to_vector<6>(op->getStartIndices().getValues<int64_t>());
  auto limit = toolchain::to_vector<6>(op->getLimitIndices().getValues<int64_t>());
  auto stride = toolchain::to_vector<6>(op->getStrides().getValues<int64_t>());
  ...
  toolchain::SmallVector<int64_t, 6> sizes;
  ...
  toolchain::SmallVector<E, 6> outValues;
...
struct SimplifyConcatSlice : public OpRewritePattern<SliceOp> {
  ...
  LogicalResult matchAndRewrite(SliceOp slice,
    ...
    toolchain::SmallVector<APInt, 6> newStart(start);
    toolchain::SmallVector<APInt, 6> newLimit(limit);
...
static LogicalResult sortDropEmptyUseArgs(SortOp op,
  ...
  for (const auto& en : toolchain::enumerate(op.getInputs())) {
...
OpFoldResult TransposeOp::fold(FoldAdaptor adaptor) {
  ...
  for (const auto& it : toolchain::enumerate(getPermutation().getValues<APInt>())) {
...
LogicalResult TransposeOp::inferReturnTypes(
  ...
  return hlo::inferTransposeOp(
      ...
      toolchain::to_vector(adaptor.getPermutation().getValues<int64_t>()),
      inferredReturnTypes);
}
...
static Attribute CompareFolder(CompareOp op, ArrayRef<Attribute> attrs) {
  ...
  values.reserve(lhs.getNumElements());
  for (const auto zip :
       toolchain::zip(lhs.getValues<SrcType>(), rhs.getValues<SrcType>())) {
    values.push_back(
...
LogicalResult ScatterOp::fold(
    FoldAdaptor adaptor, toolchain::SmallVectorImpl<OpFoldResult>& foldResults) {
  ...
  if (updateType == baseType && updateType.hasStaticShape() &&
      ...
      toolchain::hasSingleElement(getUpdateComputation().front())) {
  ...
  toolchain::SmallVector<Attribute, 8> results(base.getValues<Attribute>());
  toolchain::SmallVector<uint64_t, 8> updateIndex(updateType.getRank(), 0);
  toolchain::SmallVector<uint64_t, 8> indexIndex;
  indexIndex.reserve(indexType.getRank());
  toolchain::SmallVector<int64_t, 8> baseIndex;
  ...
  do {
    ...
    for (int64_t i = 0; i < static_cast<int64_t>(updateIndex.size()); ++i) {
      if (!toolchain::is_contained(updateWindowDims, i))
    ...
    for (auto [operandDim, indicesDim] :
         toolchain::zip_equal(inputBatchingDims, scatterIndicesBatchingDims)) {
    ...
    for (uint64_t i = 0; i < baseIndex.size(); ++i) {
      if (toolchain::is_contained(insertedWindowDims, i) ||
          toolchain::is_contained(inputBatchingDims, i))
        continue;
...
struct ScatterFullReplace : public OpRewritePattern<ScatterOp> {
  ...
  LogicalResult matchAndRewrite(ScatterOp scatter,
    ...
    auto dimensions =
        toolchain::to_vector(toolchain::seq<int64_t>(0, baseType.getRank()));
...
static LogicalResult whileCanonicalization(WhileOp whileOp,
  ...
  if (!toolchain::any_of(toolchain::zip(whileOp->getOperands(), body->getArguments(),
  ...
  for (const auto& enumeratedOperands : toolchain::enumerate(toolchain::zip(
  ...
  for (int idx : toolchain::reverse(invariantArgIdxs))
  ...
  for (auto results : toolchain::zip(resultsToReplace, newWhileOp->getResults()))
...
static ParseResult parseStruct(
    ...
    ArrayRef<toolchain::function_ref<ParseResult()>> parseFuncs,
  ...
  while (failed(parser.parseOptionalGreater())) {
    bool foundOne = false;
    for (const auto& it : toolchain::enumerate(keywords)) {
    ...
    if (!foundOne) {
      auto parseError = parser.emitError(parser.getCurrentLocation())
                        << "expected one of: ";
      toolchain::interleaveComma(keywords, parseError, [&](StringRef kw) {
        parseError << '`' << kw << '`';
      });
      return parseError;
    }
...
static void printField(AsmPrinter& printer, StringRef name, ArrayRef<T> field,
  ...
  if (!field.empty()) {
    ...
    toolchain::interleaveComma(field, printer);
    printer << "]";
    separator = ", ";
  }
}
...
Attribute RaggedDotDimensionNumbersAttr::parse(AsmParser& parser, Type type) {
  ...
  if (failed(parseStruct(
          ...
          {[&]() {
             ...
             dotDimensionNumbers =
                 toolchain::dyn_cast<DotDimensionNumbersAttr>(attr);
...
void printConvolutionDimensions(AsmPrinter& p, ConvDimensionNumbersAttr dnums) {
  ...
  auto printDim =
      [&](ArrayRef<int64_t> spatialDims,
        ...
        toolchain::SmallVector<int64_t> dims(numDims, kUnknownDim);
        ...
        for (const std::pair<int64_t, NonSpatialDim>& nonSpatialDim :
          ...
          if (nonSpatialDim.first < 0 ||
            ...
            toolchain::report_fatal_error("Invalid non-spatial dimension.");
          dims[nonSpatialDim.first] = nonSpatialDim.second;
        }
        for (const auto& spatialDim : toolchain::enumerate(spatialDims)) {
          if (spatialDim.value() < 0 ||
              static_cast<size_t>(spatialDim.value()) >= dims.size())
            toolchain::report_fatal_error("Invalid spatial dimension.");
        ...
        p << '[';
        toolchain::interleaveComma(dims, p, [&](int64_t dim) {
          if (dim == kUnknownDim) {
            p << "?";
          } else if (dim >= 0) {
            p << dim;
...
ParseResult parseConvolutionDimensions(AsmParser& parser,
  ...
  using parse_dim_result_t =
      std::pair<toolchain::SmallVector<int64_t>,
                toolchain::SmallDenseMap<NonSpatialDim, int64_t, 4,
  ...
  auto parseDims =
    ...
    toolchain::SmallDenseMap<int64_t, int64_t> spatialDimsMap;
    ...
    // error later.
    toolchain::SmallVector<int64_t> unspecifiedSpatialDims;
    ...
    if (!unspecifiedSpatialDims.empty()) {
      ...
      toolchain::interleaveComma(unspecifiedSpatialDims, diag);
      diag << " not specified";
      return diag;
    }
  ...
  toolchain::SmallVector<int64_t> inputSpatialDimensions = parsedDims.first;
  ...
  toolchain::SmallVector<int64_t> kernelSpatialDimensions = parsedDims.first;
  ...
  toolchain::SmallVector<int64_t> outputSpatialDimensions = parsedDims.first;
...
Attribute ArgResultAliasAttr::parse(AsmParser& parser, Type type) {
  if (failed(parser.parseLess())) return {};
  toolchain::SmallVector<int64_t> argTupleIndices;
  ...
  toolchain::SmallVector<int64_t> resultIndices;
  ...
  toolchain::SmallVector<bool, 3> parseEqual = {true, true, false};
...
static LogicalResult verifyArgResultAliasAttr(StringAttr attrName,
  ...
  auto tupleIndices = toolchain::concat<const int64_t>(
      aliasAttr.getArgTupleIndices(), aliasAttr.getResultTupleIndices());
  if (toolchain::any_of(tupleIndices, [](const int64_t val) { return val < 0; }) ||
...
// arguments.
static void buildSortComparisonBody(toolchain::ArrayRef<Type> elementTypes,
...
SortOp createSortOp(PatternRewriter* rewriter, const Location& loc,
                    const toolchain::ArrayRef<Value>& operands,
                    const toolchain::ArrayRef<Type>& elementTypes, int64_t dimension,
