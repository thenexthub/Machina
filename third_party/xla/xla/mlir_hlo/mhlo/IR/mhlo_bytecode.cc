...
#define _LOG_CALL_TO(func)                                                     \
  DEBUG_WITH_TYPE(                                                             \
      "mhlo-bytecode",                                                         \
      toolchain::errs() << "Called: " << _EXTRACT_AFTER(LLVM_PRETTY_FUNCTION, func) \
                   << '\n')
...
#define LOG_NOT_IMPLEMENTED \
  DEBUG_WITH_TYPE(          \
      "mhlo-bytecode",      \
      toolchain::errs() << "***Not Implemented: " << LLVM_PRETTY_FUNCTION << '\n')
...
ArgResultAliasAttr MhloBytecodeInterface::readArgResultAliasAttr(
  ...
  int64_t resultIndex;
  toolchain::SmallVector<int64_t> resultTupleIndices;
...
ConvDimensionNumbersAttr MhloBytecodeInterface::readConvDimensionNumbersAttr(
  ...
  toolchain::SmallVector<int64_t> inputSpatialDimensions;

  int64_t kernelInputFeatureDimension, kernelOutputFeatureDimension;
  toolchain::SmallVector<int64_t> kernelSpatialDimensions;

  int64_t outputBatchDimension, outputFeatureDimension;
  toolchain::SmallVector<int64_t> outputSpatialDimensions;
...
DotDimensionNumbersAttr MhloBytecodeInterface::readDotDimensionNumbersAttr(
    DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> lhsBatchingDimensions, rhsBatchingDimensions,
...
MhloBytecodeInterface::readGatherDimensionNumbersAttr(
    DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> offsetDims, collapsedSliceDims,
...
OutputOperandAliasAttr MhloBytecodeInterface::readOutputOperandAliasAttr(
    DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> outputTupleIndices, operandTupleIndices;
...
MhloBytecodeInterface::readScatterDimensionNumbersAttr(
    DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> updateWindowDims, insertedWindowDims,
...
TypeExtensionsAttr MhloBytecodeInterface::readTypeExtensionsAttr(
    DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> bounds;
...
AsyncBundleType MhloBytecodeInterface::readAsyncBundleType(
  ...
  toolchain::SmallVector<Type> types;
