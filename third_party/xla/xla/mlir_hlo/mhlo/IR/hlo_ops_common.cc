...
LogicalResult verifyCollectivePermuteSourceTargetPairs(
  ...
  toolchain::DenseSet<int64_t> sources;
  toolchain::DenseSet<int64_t> targets;
...
LogicalResult verifyReduceScatter(Operation *op, TypeRange operandTypes,
  ...
  for (auto it : toolchain::zip(operandTypes, resultTypes)) {
    ...
    for (uint64_t index : toolchain::seq<uint64_t>(0, operandType.getRank())) {
...
void printWindowAttribute(OpAsmPrinter &p, DenseElementsAttr attribute) {
  if (attribute.getElementType().isInteger(/*width=*/1)) {
    // boolean attribute.
    toolchain::interleaveComma(attribute.getValues<bool>(), p,
                          [&](bool b) { p << (b ? 1 : 0); });
    return;
  }
  if (attribute.getType().getRank() == 2) {
    ...
    toolchain::interleaveComma(attribute.getValues<int64_t>(), p);
  }
}
}  // namespace
...
void printWindowAttributes(OpAsmPrinter &p, Operation * /*op*/,
  ...
  auto nonNullAttributes = toolchain::make_filter_range(
      printedAttributes,
      [](const pair_t &a) { return static_cast<bool>(a.first); });

  toolchain::interleaveComma(nonNullAttributes, p, [&](const pair_t &a) {
    p << a.second << " = [";
...
ParseResult parseWindowAttributes(OpAsmParser &parser,
  ...
  toolchain::StringSet<> allowedAttributeNames{
  ...
  while (parser.parseOptionalKeyword(&attributeName).succeeded()) {
    ...
    // integers to parse.
    toolchain::SmallVector<int64_t> values;
    auto int64Parser = [&]() {
    ...
    if (attributeName == "pad") {
      ...
      if (attributeName == "reverse") {
        ...
        auto boolVector = toolchain::to_vector<4>(
            toolchain::map_range(values, [](int64_t v) { return v != 0; }));
