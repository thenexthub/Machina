...
std::pair<ArithOp, ArithOp> runThreeFry2xi32(ArithOp key0, ArithOp key1,
  ...
  // algorithm.
  toolchain::SmallVector<int32_t, 8> rotations = {13, 15, 26, 6, 17, 29, 16, 24};
...
LogicalResult generateLinalgThreeFry32(OpBuilder &builder, Location loc,
  ...
  toolchain::SmallVector<int64_t> collapseShape(resultTy.getShape());
  ...
  toolchain::SmallVector<int64_t> offset(resultTy.getRank(), 0);
  toolchain::SmallVector<int64_t> stride(resultTy.getRank(), 1);
...
LogicalResult generateLinalgPhilox32(OpBuilder &builder, Location loc,
  ...
  // Collapse the concat dimension back into the parent.
  toolchain::SmallVector<int64_t> collapseShape(intermediateType.getShape());
