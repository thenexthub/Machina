...
///     absent={}
///   linalg.yield %result
Value preSparsify(Operation* op, toolchain::SmallVector<Value, 2>& values, Type rtp,
                  OpBuilder* b);
...
class PointwiseToLinalgConverter : public OpConversionPattern<OpTy> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    auto it = toolchain::find_if_not(adaptor.getOperands(), isScalar);
    Value maxRankArg =
    ...
    auto linalgOp = rewriter.create<linalg::GenericOp>(
        ...
        [&](OpBuilder& nestedBuilder, Location /*nested_loc*/,
          ...
          auto argvec = toolchain::to_vector<2>(args.take_front(inputs.size()));
