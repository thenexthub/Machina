...
// IWYU pragma: no_include "toolchain/IR/Intrinsics.gen.inc"
...
#include "toolchain/Transforms/Utils/BasicBlockUtils.h"
...
#include "machina/xla/comparison_util.h"
#include "machina/xla/hlo/ir/hlo_casting_utils.h"
...
#include "machina/xla/service/algorithm_util.h"
#include "machina/xla/service/float8_fnuz_ir_emitter.h"
#include "machina/xla/service/llvm_ir/ir_array.h"
#include "machina/xla/service/llvm_ir/llvm_loop.h"
...
#include "machina/xla/window_util.h"
...
absl::StatusOr<toolchain::Value*> EmitReducePrecisionIR(
  ...
  toolchain::Type* float_type = x->getType();
  int64_t nbits = float_type->getPrimitiveSizeInBits();
  toolchain::IntegerType* int_type = b->getIntNTy(nbits);
  ...
  toolchain::Value* x_as_int = b->CreateBitCast(x, int_type);
  ...
  toolchain::Value* x_abs_bits =
      b->CreateAnd(x_as_int, toolchain::ConstantInt::get(int_type, ~sign_bit_mask));

  APInt exp_bits_mask(nbits, 1);
  exp_bits_mask = ((exp_bits_mask << src_exponent_bits) - 1)
                  << src_mantissa_bits;
  auto x_is_nan = b->CreateICmpUGT(
      x_abs_bits, toolchain::ConstantInt::get(int_type, exp_bits_mask));
  ...
  if (dest_mantissa_bits < src_mantissa_bits) {
    ...
    toolchain::Value* x_last_mantissa_bit = b->CreateLShr(
        b->CreateAnd(x_as_int,
                     toolchain::ConstantInt::get(int_type, last_mantissa_bit_mask)),
        (src_mantissa_bits - dest_mantissa_bits));
    toolchain::Value* x_rounding_bias =
        b->CreateAdd(x_last_mantissa_bit,
                     toolchain::ConstantInt::get(int_type, base_rounding_bias));
    ...
    toolchain::Value* x_rounded = b->CreateAdd(x_as_int, x_rounding_bias);
    x_rounded = b->CreateAnd(x_rounded,
                             toolchain::ConstantInt::get(int_type, truncation_mask));
    if (quiet_nans) {
  ...
  if (dest_exponent_bits < src_exponent_bits) {
    ...
    toolchain::Value* x_exponent =
        b->CreateAnd(x_as_int, toolchain::ConstantInt::get(int_type, exp_bits_mask));
    toolchain::Value* x_overflows = b->CreateICmpUGT(
        x_exponent, toolchain::ConstantInt::get(
                        int_type, reduced_max_exponent << src_mantissa_bits));
    toolchain::Value* x_underflows = b->CreateICmpULE(
        x_exponent, toolchain::ConstantInt::get(
                        int_type, reduced_min_exponent << src_mantissa_bits));

    // Compute appropriately-signed values of zero and infinity.
    toolchain::Value* x_signed_zero =
        b->CreateAnd(x_as_int, toolchain::ConstantInt::get(int_type, sign_bit_mask));
    toolchain::Value* x_signed_inf = b->CreateOr(
        x_signed_zero, toolchain::ConstantInt::get(int_type, exp_bits_mask));
  ...
  toolchain::Value* result = b->CreateBitCast(x_as_int, float_type);
  ...
  if (dest_mantissa_bits > 0) {
    if (quiet_nans) {
      ...
      toolchain::Value* x_with_qnan_bit_set =
          b->CreateOr(x_as_int, toolchain::ConstantInt::get(int_type, qnan_mask));
    ...
    result = b->CreateSelect(x_is_nan,
                             toolchain::ConstantFP::getInfinity(float_type), result);
  }
...
toolchain::Value* handle_halfway_points_FxToF8(toolchain::Value* fx_abs_bits,
                                          toolchain::Value* f8_bits,
  ...
  const toolchain::fltSemantics* fx_semantics;
  toolchain::IntegerType* ix_type;

  if constexpr (fx_type == F16) {
    fx_semantics = &toolchain::APFloat::IEEEhalf();
    ix_type = b->getInt16Ty();
  } else if constexpr (fx_type == F32) {
    fx_semantics = &toolchain::APFloat::IEEEsingle();
    ix_type = b->getInt32Ty();
  } else if constexpr (fx_type == F64) {
    fx_semantics = &toolchain::APFloat::IEEEdouble();
    ix_type = b->getInt64Ty();
  }

  auto ix_const = [fx_semantics, ix_type](APFloat val) {
    bool losesInfo;
    val.convert(*fx_semantics, toolchain::RoundingMode::NearestTiesToEven,
                &losesInfo);
    return toolchain::ConstantInt::get(ix_type, val.bitcastToAPInt());
  };

  toolchain::IntegerType* i8_type = b->getInt8Ty();
  auto i8_const = [i8_type](int val) {
    return toolchain::ConstantInt::get(i8_type, val);
  };
  ...
  for (int i = arr_sz - 1; i >= 0; i--) {
    Value* comparison;
    toolchain::Constant* half_way_point = ix_const(halfway_points[i]);
...
absl::StatusOr<toolchain::Value*> EmitF16ToF8e5m2(toolchain::Value* f16_value,
                                             toolchain::IRBuilderBase* b) {
  TF_ASSIGN_OR_RETURN(
      toolchain::Value * reduced_precision,
      EmitReducePrecisionIR(
          ...
          /*quiet_nans=*/true, b));
  toolchain::Value* as_int16 = b->CreateBitCast(reduced_precision, b->getInt16Ty());
  toolchain::Value* shifted = b->CreateLShr(as_int16, 8);
  toolchain::Value* truncated = b->CreateTrunc(shifted, b->getInt8Ty());
  return b->CreateBitCast(truncated, b->getInt8Ty());
}

toolchain::Value* EmitF8e5m2ToF16(toolchain::Value* f8_value, toolchain::IRBuilderBase* b) {
  toolchain::Value* as_int8 = b->CreateBitCast(f8_value, b->getInt8Ty());
  toolchain::Value* as_int16 = b->CreateZExt(as_int8, b->getInt16Ty());
  toolchain::Value* shifted = b->CreateShl(as_int16, 8);
...
// bits wide.
template <PrimitiveType fx_type, int f8_exponent_bits>
absl::StatusOr<toolchain::Value*> EmitFxToF8e(toolchain::Value* fx_value,
                                         toolchain::IRBuilderBase* b) {
  static_assert(fx_type == F16 || fx_type == F32 || fx_type == F64);
  static_assert(3 <= f8_exponent_bits && f8_exponent_bits <= 4);
  ...
  auto ix_const = [ix_type](uint64_t val) {
    return toolchain::ConstantInt::get(ix_type, val);
  };

  toolchain::IntegerType* i8_type = b->getInt8Ty();
  toolchain::Constant* infinity = toolchain::ConstantInt::get(
      ix_type, toolchain::APFloat::getInf(*fx_semantics).bitcastToAPInt());
  toolchain::ConstantInt* nosign_mask =
      ix_const(ix_type->getBitMask() ^ ix_type->getSignBit());
  toolchain::ConstantInt* sign_mask = ix_const(ix_type->getSignBit());
  toolchain::ConstantInt* sign_shift = ix_const(fx_width - 8);
  toolchain::ConstantInt* fx_exponent_bias_difference =
      ix_const(exponent_bias_difference << fx_mantissa_bits);
  toolchain::ConstantInt* fx_doubled_exponent_bias_difference =
      ix_const(exponent_bias_difference << (fx_mantissa_bits + 1));
  toolchain::ConstantInt* mantissa_bits_difference =
      ix_const(fx_mantissa_bits - f8_mantissa_bits);
  toolchain::ConstantInt* min_normal_value =
  ...
  toolchain::Value* fx_abs_bits = b->CreateAnd(fx_as_int, nosign_mask);
...
toolchain::Value* EmitToF16F8e(toolchain::Value* f8_value, toolchain::IRBuilderBase* b) {
  using toolchain::APInt;
  using toolchain::Value;
  static_assert(3 <= f8_exponent_bits && f8_exponent_bits <= 4);

  toolchain::IntegerType* i8_type = b->getInt8Ty();
  toolchain::IntegerType* i16_type = b->getInt16Ty();
  auto i8_const = [i8_type](int val) {
    return toolchain::ConstantInt::get(i8_type, val);
  };
  auto i16_const = [i16_type](int val) {
    return toolchain::ConstantInt::get(i16_type, val);
  };
...
toolchain::Value* EmitF16ToF8e4m3fn(toolchain::Value* f16_value, toolchain::IRBuilderBase* b) {
  using toolchain::APInt;
  using toolchain::Value;
  ...
  toolchain::Value* f16_abs_bits = b->CreateAnd(f16_as_int, i16_const(0x7FFF));
...
toolchain::Value* EmitF8e4m3fnToF16(toolchain::Value* f8_value, toolchain::IRBuilderBase* b) {
  using toolchain::APInt;
  using toolchain::Value;
...
toolchain::Value* EmitF16ToF8e4m3b11fnuz(toolchain::Value* f16_value,
                                    toolchain::IRBuilderBase* b) {
  using toolchain::APInt;
  using toolchain::Value;
  ...
  auto f16_abs_value = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs,
                                                    {f16_value}, {type}, b);
  auto f16_zero_or_underflow = toolchain::ConstantFP::get(type, 0x1.004p-14);
  auto is_zero = b->CreateFCmpOLT(f16_abs_value, f16_zero_or_underflow);
  auto f8_overflow_threshold = toolchain::ConstantFP::get(type, 0x1.fp+4);
  ...
  f16_value = b->CreateFMul(
      f16_value, toolchain::ConstantFP::get(f16_value->getType(), 1 << (11 - 7)));
...
toolchain::Value* EmitF8e4m3b11fnuzToF16(toolchain::Value* f8_value,
                                    toolchain::IRBuilderBase* b) {
  using toolchain::APInt;
  using toolchain::Value;
  ...
  f16_value = b->CreateFMul(
      f16_value,
      toolchain::ConstantFP::get(f16_value->getType(), 1.0 / (1 << (11 - 7))));
  f16_value = b->CreateSelect(
      is_nan,
      toolchain::ConstantFP::get(f16_value->getType(),
                            std::numeric_limits<double>::quiet_NaN()),
      f16_value);

  toolchain::Value* max_like_sign = toolchain::ConstantFP::get(
      f16_value->getType(),
...
absl::StatusOr<toolchain::Value*> EmitF16ToF4e2m1fn(toolchain::Value* f16_value,
                                               toolchain::IRBuilderBase* b) {
  auto i8_const = [&](int val) {
    return toolchain::ConstantInt::get(b->getInt8Ty(), val);
  };
  auto i16_const = [&](int val) {
    return toolchain::ConstantInt::get(b->getInt16Ty(), val);
  };
  ...
  toolchain::Value* f16_bits = b->CreateBitCast(f16_value, b->getInt16Ty());
  toolchain::Value* f16_abs_bits = b->CreateAnd(f16_bits, i16_const(0x7FFF));
  ...
  toolchain::Value* f4_normal = b->CreateSub(f4_bits, i8_const(f4_exponent_offset));
  ...
  toolchain::Value* is_underflow = b->CreateICmpSLE(f4_normal, i8_const(1));
  ...
  //     f4_normal
  toolchain::Value* f4_result = b->CreateSelect(
      is_underflow,
...
toolchain::Value* EmitF4e2m1fnToF16(toolchain::Value* f8_value, toolchain::IRBuilderBase* b) {
  auto i16_const = [&](int val) {
    return toolchain::ConstantInt::get(b->getInt16Ty(), val);
  };
  constexpr int mantissa_diff = 9;  // 10 for F16, 1 for F4
  ...
  toolchain::Value* as_int16 = b->CreateZExt(f8_value, b->getInt16Ty());

  // Get the sign and shift it to F16 position.
  //   f4_sign = as_int16 >> 3
  //   f16_sign_bit = f4_sign << 15
  toolchain::Value* f4_sign = b->CreateLShr(as_int16, 3);
  toolchain::Value* f16_sign_bit = b->CreateShl(f4_sign, 15);

  // Get exponent and mantissa bits without the sign.
  //   f4_bits = as_int16 & 0x7
  //   f16_bits = f4_bits << (f16_mantissa - f4_mantissa)
  toolchain::Value* f4_bits = b->CreateAnd(as_int16, i16_const(0x7));
  toolchain::Value* f16_bits = b->CreateShl(f4_bits, mantissa_diff);

  // Convert F16 exponent to F4 exponent by readjusting the exponent bias.
  //   f4_normal = f4_bits - ((f16_bias - f4_bias) << f4_mantissa)
  constexpr int f16_exponent_offset = bias_diff << 10;
  toolchain::Value* f16_normal =
  ...
  //   is_zero = f4_bits == 0
  toolchain::Value* is_denorm_or_zero = b->CreateICmpULE(f4_bits, i16_const(1));
  toolchain::Value* is_zero = b->CreateICmpEQ(f4_bits, i16_const(0));

  // Chain of selects that handles the special cases.
  //   f16_result = is_denorm_or_zero ? (is_zero ? 0.0 : 0.5) : f16_normal
  toolchain::Value* f16_result = b->CreateSelect(
      is_denorm_or_zero,
  ...
  toolchain::Value* f16_signed_result = b->CreateOr(f16_result, f16_sign_bit);
  return b->CreateBitCast(f16_signed_result, b->getHalfTy());
}

toolchain::Value* EmitF32ToF8e8m0fnu(toolchain::Value* f32_value,
                                toolchain::IRBuilderBase* b) {
  auto i32_const = [&](int val) {
    return toolchain::ConstantInt::get(b->getInt32Ty(), val);
  };

  // Cast the input value to an integer for bitwise manipulation.
  //   as_int32 = bitcast(f32_value, int)
  toolchain::Value* as_int32 = b->CreateBitCast(f32_value, b->getInt32Ty());
  ...
  toolchain::Value* is_zero_or_negative = b->CreateICmpSLE(as_int32, i32_const(0));
  toolchain::Value* is_overflow_or_nan =
      b->CreateICmpSGE(as_int32, i32_const(0x7F400000));  // 1.5 * 2^127
  toolchain::Value* is_nan = b->CreateOr(is_zero_or_negative, is_overflow_or_nan);
  ...
  toolchain::Value* rounded =
      b->CreateAdd(as_int32, i32_const(0x400000));  // 1.0 * 2^-127
  toolchain::Value* f8_normal = b->CreateAShr(rounded, 23);

  // Chain of selects that handles the special cases.
  //   f8_result = is_nan ? 0xFF : (is_denorm ? 0x00 : f8_normal)
  toolchain::Value* f8_result =
...
toolchain::Value* EmitF8e8m0fnuToF32(toolchain::Value* f8_value, toolchain::IRBuilderBase* b) {
  auto i32_const = [&](int val) {
    return toolchain::ConstantInt::get(b->getInt32Ty(), val);
  };
  ...
  toolchain::Value* as_int32 = b->CreateZExt(f8_value, b->getInt32Ty());

  // Check if the input is a denormal or NaN.
  //   is_zero = as_int32 == 0x00
  //   is_nan = as_int32 == 0xFF
  toolchain::Value* is_zero = b->CreateICmpEQ(as_int32, i32_const(0));
  toolchain::Value* is_nan = b->CreateICmpEQ(as_int32, i32_const(0xFF));

  // Shift exponent to the left for the normal case.
  //   f32_normal = as_int32 << mantissa_diff
  toolchain::Value* f32_normal = b->CreateShl(as_int32, 23);

  // Chain of selects that handles the special cases.
  //   f32_result = is_nan ? 0x7FC00000 : (is_zero ? 0x1p-127 : f32_normal)
  toolchain::Value* f32_result = b->CreateSelect(
...
toolchain::Value* EmitIntegralToFloating(toolchain::Value* integer_value,
                                    PrimitiveType from_type,
                                    PrimitiveType to_type, toolchain::Module* module,
                                    toolchain::IRBuilderBase* b) {
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitUnaryOp(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitIntegerUnaryOp(
    const HloInstruction* op, toolchain::Value* operand_value) {
  switch (op->opcode()) {
      ...
      if (to_type == PRED) {
        return b_->CreateZExt(
            ICmpNE(operand_value,
                   toolchain::ConstantInt::get(operand_value->getType(), 0)),
    ...
    case HloOpcode::kClz: {
      ...
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::ctlz,
    ...
    case HloOpcode::kPopulationCount: {
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::ctpop,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    }
    default:
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitFloatUnaryOp(
    const HloInstruction* op, toolchain::Value* operand_value) {
  switch (op->opcode()) {
    case HloOpcode::kConvert: {
      PrimitiveType from_type = op->operand(0)->shape().element_type();
      ...
      if (to_type == PRED) {
        return b_->CreateZExt(
            FCmpUNE(operand_value,
                    toolchain::ConstantFP::get(operand_value->getType(), 0.0)),
            llvm_ir::PrimitiveTypeToIrType(PRED, module_->getContext()));
      }
      auto* to_ir_type =
          llvm_ir::PrimitiveTypeToIrType(to_type, module_->getContext());
      ...
      if (primitive_util::IsSignedIntegralType(to_type)) {
        int64_t min_int = toolchain::minIntN(to_width);
        int64_t max_int = toolchain::maxIntN(to_width);
        auto zero_int = toolchain::ConstantInt::get(to_ir_type, 0);
        auto min_value_int = toolchain::ConstantInt::get(to_ir_type, min_int);
        auto max_value_int = toolchain::ConstantInt::get(to_ir_type, max_int);
        auto min_value_float = toolchain::ConstantFP::get(from_ir_type, min_int);
        auto max_value_float = toolchain::ConstantFP::get(from_ir_type, max_int);
        auto clamped = FPToSI(
            operand_value,
            llvm_ir::PrimitiveTypeToIrType(to_type, module_->getContext()));
      ...
      if (primitive_util::IsUnsignedIntegralType(to_type)) {
        uint64_t min_int = 0;
        uint64_t max_int = toolchain::maxUIntN(to_width);
        auto min_value_int = toolchain::ConstantInt::get(to_ir_type, min_int);
        auto max_value_int = toolchain::ConstantInt::get(to_ir_type, max_int);
        auto min_value_float = toolchain::ConstantFP::get(from_ir_type, min_int);
        auto max_value_float = toolchain::ConstantFP::get(from_ir_type, max_int);
    ...
    case HloOpcode::kFloor:
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::floor,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    case HloOpcode::kCeil:
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::ceil,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    case HloOpcode::kAbs:
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    case HloOpcode::kRoundNearestAfz:
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::round,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    // TODO(b/238238423): toolchain::Intrinsic::nearbyint is equivalent to roundeven
    // as TF and JAX default to FE_TONEAREST. Call toolchain::Intrinsic::roundeven
    // instead once GPU emitter supports lowering LLVM.
    case HloOpcode::kRoundNearestEven:
      return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::nearbyint,
                                          {operand_value},
                                          {operand_value->getType()}, b_);
    case HloOpcode::kSign: {
      auto type = operand_value->getType();
      auto zero = toolchain::ConstantFP::get(type, 0.0);
      auto ne0_i1 = FCmpONE(operand_value, zero);
      auto ne0_float = UIToFP(ne0_i1, type);
      toolchain::Value* result = llvm_ir::EmitCallToIntrinsic(
          toolchain::Intrinsic::copysign, {ne0_float, operand_value},
          {operand_value->getType()}, b_);
    ...
    case HloOpcode::kIsFinite: {
      ...
      auto abs_value = llvm_ir::EmitCallToIntrinsic(
          toolchain::Intrinsic::fabs, {operand_value}, {type}, b_);
      auto infinity = toolchain::ConstantFP::getInfinity(type);
    ...
    case HloOpcode::kImag:
      return toolchain::ConstantFP::get(operand_value->getType(), 0.0);
    default:
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexUnaryOp(
    const HloInstruction* op, toolchain::Value* operand_value) {
  ...
  switch (op->opcode()) {
    ...
    case HloOpcode::kLog1p: {
      ...
      toolchain::Type* llvm_ty = a->getType();
      auto one = toolchain::ConstantFP::get(llvm_ty, 1.0);
      auto half = toolchain::ConstantFP::get(llvm_ty, 0.5);

      auto a1 = FAdd(a, one);
      auto abs_a1 = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {a1},
                                                 {llvm_ty}, b_);
      auto abs_b = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {b},
                                                {llvm_ty}, b_);
    ...
    case HloOpcode::kExpm1: {
      ...
      auto a = EmitExtractReal(operand_value);
      auto b = EmitExtractImag(operand_value);
      auto zero = toolchain::ConstantFP::get(b->getType(), 0.0);
      auto one = toolchain::ConstantFP::get(b->getType(), 1.0);
    ...
    case HloOpcode::kSin: {
      ...
      auto half_exp_y = FMul(toolchain::ConstantFP::get(type, 0.5), exp_y);
      auto half_exp_neg_y = FDiv(toolchain::ConstantFP::get(type, 0.5), exp_y);
      ...
      toolchain::Value* real_result = nullptr;
      toolchain::Value* imag_result = nullptr;
    ...
    case HloOpcode::kTanh: {
      ...
      */
      ...
      toolchain::Value* a0 = EmitExtractReal(operand_value);
      toolchain::Value* b0 = EmitExtractImag(operand_value);
      toolchain::Type* type = a0->getType();
      toolchain::Value* neg_one = toolchain::ConstantFP::get(type, -1.0);

      auto a = op_is_tan ? FMul(neg_one, b0) : a0;
      auto b = op_is_tan ? a0 : b0;

      toolchain::Value* two_a = FAdd(a, a);
      toolchain::Value* neg_2a = FMul(neg_one, two_a);
      ...
      // identical results.
      TF_ASSIGN_OR_RETURN(toolchain::Value * exp_2a_m1,
                          EmitExpm1(component_type, two_a));
      TF_ASSIGN_OR_RETURN(toolchain::Value * exp_neg_2a_m1,
                          EmitExpm1(component_type, neg_2a));
      toolchain::Value* real_numerator = FSub(exp_2a_m1, exp_neg_2a_m1);
      ...
      TF_ASSIGN_OR_RETURN(toolchain::Value * cos_b, EmitCos(component_type, b));
      toolchain::Value* four = toolchain::ConstantFP::get(type, 4.0);
      toolchain::Value* cos_b_sq = FMul(cos_b, cos_b);
      toolchain::Value* two_cos_2b_p2 = FMul(cos_b_sq, four);

      // Similarly we can compute sin(2b) with the formula sin(2b) =
      // 2*sin(b)*cos(b).
      TF_ASSIGN_OR_RETURN(toolchain::Value * sin_b, EmitSin(component_type, b));
      toolchain::Value* imag_numerator = FMul(four, FMul(cos_b, sin_b));
      ...
      toolchain::Value* denom = FAdd(exp_sum_m2, two_cos_2b_p2);
      ...
      toolchain::Value* inf = toolchain::ConstantFP::getInfinity(type);
      toolchain::Value* is_inf = FCmpOEQ(exp_sum_m2, inf);
      toolchain::Value* real_limit = llvm_ir::EmitCallToIntrinsic(
          toolchain::Intrinsic::copysign, {neg_one, a}, {type}, b_);

      toolchain::Value* real =
          Select(is_inf, real_limit, FDiv(real_numerator, denom));
      toolchain::Value* imag = FDiv(imag_numerator, denom);
      ...
      if (!(b_->getFastMathFlags().noNaNs() &&
        ...
        toolchain::Value* abs_a = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs,
                                                          {a}, {type}, b_);
        toolchain::Value* zero = toolchain::ConstantFP::get(type, 0.0);
        toolchain::Value* nan = toolchain::ConstantFP::getNaN(type);

        toolchain::Value* a_is_inf = FCmpOEQ(abs_a, inf);
        toolchain::Value* b_is_zero = FCmpOEQ(b, zero);

        // imag_numerator = 2sin(2b), so sin(2b) is NaN if and only if
        // imag_numerator is NaN.
        toolchain::Value* sin_2b_is_nan =
            b_->CreateFCmpUNO(imag_numerator, imag_numerator);

        toolchain::Value* real_is_nan =
            b_->CreateAnd(sin_2b_is_nan, b_->CreateNot(a_is_inf));
        toolchain::Value* imag_is_zero =
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitBinaryOp(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitFloatBinaryOp(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  switch (op->opcode()) {
    ...
    case HloOpcode::kCompare: {
      ...
      switch (op->comparison_direction()) {
        case ComparisonDirection::kEq:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OEQ, lhs_value,
                                         rhs_value, b_, op->name());
        case ComparisonDirection::kNe:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_UNE, lhs_value,
                                         rhs_value, b_, op->name());
        case ComparisonDirection::kLt:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OLT, lhs_value,
                                         rhs_value, b_, op->name());
        case ComparisonDirection::kGt:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OGT, lhs_value,
                                         rhs_value, b_, op->name());
        case ComparisonDirection::kLe:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OLE, lhs_value,
                                         rhs_value, b_, op->name());
        case ComparisonDirection::kGe:
          return llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OGE, lhs_value,
                                         rhs_value, b_, op->name());
      }
    }
...
absl::StatusOr<std::tuple<toolchain::Value*, toolchain::Value*, toolchain::Value*>>
ElementalIrEmitter::EmitComplexAbsHelper(PrimitiveType prim_type,
                                         toolchain::Value* real, toolchain::Value* imag,
                                         bool return_sqrt) {
  toolchain::Value* abs_real = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::fabs, {real}, {real->getType()}, b_);
  toolchain::Value* abs_imag = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::fabs, {imag}, {imag->getType()}, b_);
  toolchain::Value* max = EmitFloatMax(abs_real, abs_imag, "");
  toolchain::Value* min = EmitFloatMin(abs_real, abs_imag, "");

  toolchain::Value* div = FDiv(min, max);
  toolchain::Value* div_sq = FMul(div, div);
  toolchain::Value* one = toolchain::ConstantFP::get(max->getType(), 1);
  toolchain::Value* one_p_div_sq = FAdd(one, div_sq);
  TF_ASSIGN_OR_RETURN(toolchain::Value * sqrt, EmitSqrt(prim_type, one_p_div_sq));
  return std::make_tuple(min, max, return_sqrt ? sqrt : one_p_div_sq);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexAbs(
    PrimitiveType prim_type, toolchain::Value* operand_value) {
  toolchain::Value* min;
  toolchain::Value* max;
  toolchain::Value* sqrt;
  toolchain::Value* real = EmitExtractReal(operand_value);
  toolchain::Value* imag = EmitExtractImag(operand_value);
  TF_ASSIGN_OR_RETURN(
      std::tie(min, max, sqrt),
      EmitComplexAbsHelper(prim_type, real, imag, /*return_sqrt=*/true));
  toolchain::Value* result = FMul(max, sqrt);
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitSqrtComplexAbs(
    PrimitiveType prim_type, toolchain::Value* operand_value) {
  toolchain::Value* min;
  toolchain::Value* max;
  toolchain::Value* one_p_div_sq;
  toolchain::Value* real = EmitExtractReal(operand_value);
  toolchain::Value* imag = EmitExtractImag(operand_value);
  TF_ASSIGN_OR_RETURN(
      std::tie(min, max, one_p_div_sq),
      EmitComplexAbsHelper(prim_type, real, imag, /*return_sqrt=*/false));
  TF_ASSIGN_OR_RETURN(toolchain::Value * sqrt_max, EmitSqrt(prim_type, max));
  TF_ASSIGN_OR_RETURN(toolchain::Value * pow,
                      EmitPow(prim_type, one_p_div_sq,
                              toolchain::ConstantFP::get(max->getType(), .25), ""));
  toolchain::Value* result = FMul(sqrt_max, pow);
  // When (min, max) are (0, 0), (inf, inf), or (NaN, ...), `result` is NaN.
  // In such cases, we return `min` instead of `result`.
  return Select(FCmpUNO(result, result), min, result);
}
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitRsqrtComplexAbs(
    PrimitiveType prim_type, toolchain::Value* operand_value) {
  toolchain::Value* min;
  toolchain::Value* max;
  toolchain::Value* sqrt;
  ...
  TF_ASSIGN_OR_RETURN(
      std::tie(min, max, sqrt),
      EmitComplexAbsHelper(prim_type, real, imag, /*return_sqrt=*/true));
  TF_ASSIGN_OR_RETURN(toolchain::Value * rsqrt_max, EmitRsqrt(prim_type, max));
  TF_ASSIGN_OR_RETURN(toolchain::Value * rsqrt_sqrt, EmitRsqrt(prim_type, sqrt));
  toolchain::Value* result = FMul(rsqrt_max, rsqrt_sqrt);
  TF_ASSIGN_OR_RETURN(toolchain::Value * rsqrt_min, EmitRsqrt(prim_type, min));
  // When (min, max) are (0, 0), (inf, inf), or (NaN, ...), `result` is NaN.
  // In such cases, we return rsqrt(min) instead of `result`.
  return Select(FCmpUNO(result, result), rsqrt_min, result);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexAdd(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  return EmitComposeComplex(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexSubtract(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  return EmitComposeComplex(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexMultiply(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  return EmitComposeComplex(
      op,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexDivide(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  ...
  auto b_r_abs =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {b_r}, {type}, b_);
  auto b_i_abs =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {b_i}, {type}, b_);
  ...
  auto zero = toolchain::ConstantFP::get(type, 0.0);
  auto one = toolchain::ConstantFP::get(type, 1.0);
  auto inf = toolchain::ConstantFP::getInfinity(type);
  ...
  auto inf_with_sign_of_b_r = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::copysign, {inf, b_r}, {type}, b_);
  ...
  auto a_r_abs =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {a_r}, {type}, b_);
  auto a_i_abs =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {a_i}, {type}, b_);
  ...
  auto a_r_inf_with_sign = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::copysign, {Select(a_r_infinite, one, zero), a_r}, {type},
      b_);
  auto a_i_inf_with_sign = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::copysign, {Select(a_i_infinite, one, zero), a_i}, {type},
      b_);
  ...
  auto b_r_inf_with_sign = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::copysign, {Select(b_r_infinite, one, zero), b_r}, {type},
      b_);
  auto b_i_inf_with_sign = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::copysign, {Select(b_i_infinite, one, zero), b_i}, {type},
      b_);
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexLog(
    const HloInstruction* op, toolchain::Value* operand_value) {
  ...
  auto a = EmitExtractReal(operand_value);
  auto b = EmitExtractImag(operand_value);
  TF_ASSIGN_OR_RETURN(toolchain::Value * angle, EmitAtan2(component_type, b, a, ""));
  TF_ASSIGN_OR_RETURN(toolchain::Value * abs,
                      EmitComplexAbs(component_type, operand_value));
  TF_ASSIGN_OR_RETURN(toolchain::Value * log_abs, EmitLog(component_type, abs));
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexSqrt(
    const HloInstruction* op, PrimitiveType prim_type,
    toolchain::Value* operand_value) {
  toolchain::Type* type = static_cast<toolchain::StructType*>(operand_value->getType())
                         ->getElementType(0);

  TF_ASSIGN_OR_RETURN(toolchain::Value * r,
                      EmitSqrtComplexAbs(prim_type, operand_value));

  toolchain::Value* a = EmitExtractReal(operand_value);
  toolchain::Value* b = EmitExtractImag(operand_value);
  TF_ASSIGN_OR_RETURN(toolchain::Value * t, EmitAtan2(prim_type, b, a, ""));

  toolchain::Value* c = toolchain::ConstantFP::get(type, 0.5);
  toolchain::Value* angle = FMul(t, c);
  TF_ASSIGN_OR_RETURN(toolchain::Value * cos, EmitCos(prim_type, angle));
  TF_ASSIGN_OR_RETURN(toolchain::Value * sin, EmitSin(prim_type, angle));

  toolchain::Value* real_part;
  toolchain::Value* imag_part;

  toolchain::Value* zero = toolchain::ConstantFP::get(type, 0);

  if (!(b_->getFastMathFlags().noNaNs() && b_->getFastMathFlags().noInfs())) {
    toolchain::Value* inf = toolchain::ConstantFP::getInfinity(type);
    toolchain::Value* neg_inf = toolchain::ConstantFP::getInfinity(type, true);
    toolchain::Value* nan = toolchain::ConstantFP::getNaN(type);
    toolchain::Value* abs_b = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs,
                                                      {b}, {b->getType()}, b_);
    ...
    toolchain::Value* b_signed_inf = llvm_ir::EmitCallToIntrinsic(
        toolchain::Intrinsic::copysign, {inf, b}, {b->getType()}, b_);
    imag_part =
...
// where r = |a+bi| and t = atan2(b,a).
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexRsqrt(
    const HloInstruction* op, PrimitiveType prim_type,
    toolchain::Value* operand_value) {
  toolchain::Type* type = static_cast<toolchain::StructType*>(operand_value->getType())
                         ->getElementType(0);
  ...
  toolchain::Value* real_part = FMul(r, cos);
  toolchain::Value* imag_part = FMul(r, sin);

  if (!(b_->getFastMathFlags().noNaNs() && b_->getFastMathFlags().noInfs())) {
    toolchain::Value* zero = toolchain::ConstantFP::get(type, 0);
    toolchain::Value* neg_one = toolchain::ConstantFP::get(type, -1);
    toolchain::Value* inf = toolchain::ConstantFP::getInfinity(type);
    toolchain::Value* nan = toolchain::ConstantFP::getNaN(type);
    toolchain::Value* a_signed_zero = llvm_ir::EmitCallToIntrinsic(
        toolchain::Intrinsic::copysign, {zero, a}, {a->getType()}, b_);
    toolchain::Value* b_signed_zero = llvm_ir::EmitCallToIntrinsic(
        toolchain::Intrinsic::copysign, {zero, b}, {b->getType()}, b_);
    toolchain::Value* neg_b_signed_zero = FMul(b_signed_zero, neg_one);
    ...
    toolchain::Value* is_zero_zero = And(FCmpOEQ(b, zero), FCmpOEQ(a, zero));
    real_part = Select(
        is_zero_zero, inf,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexPower(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  PrimitiveType component_type =
      primitive_util::ComplexComponentType(op->shape().element_type());
  ...
  toolchain::Value* inf = toolchain::ConstantFP::getInfinity(a->getType());
  auto zero = toolchain::ConstantFP::get(a->getType(), 0);
  auto one = toolchain::ConstantFP::get(a->getType(), 1);
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitComplexBinaryOp(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
  switch (op->opcode()) {
    case HloOpcode::kAdd:
    ...
    case HloOpcode::kCompare: {
      switch (op->comparison_direction()) {
        case ComparisonDirection::kEq:
          return And(llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OEQ,
                                             EmitExtractReal(lhs_value),
                                             EmitExtractReal(rhs_value), b_),
                     llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_OEQ,
                                             EmitExtractImag(lhs_value),
                                             EmitExtractImag(rhs_value), b_));
        case ComparisonDirection::kNe:
          return Or(llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_UNE,
                                            EmitExtractReal(lhs_value),
                                            EmitExtractReal(rhs_value), b_),
                    llvm_ir::EmitComparison(toolchain::CmpInst::FCMP_UNE,
                                            EmitExtractImag(lhs_value),
                                            EmitExtractImag(rhs_value), b_));
        default:
          return Unimplemented(
    ...
    case HloOpcode::kAtan2: {
      ...
      auto negative_one = toolchain::ConstantFP::get(type, -1.0);
...
toolchain::Value* ElementalIrEmitter::EmitFloatMax(toolchain::Value* lhs_value,
                                              toolchain::Value* rhs_value,
                                              absl::string_view name) {
  return llvm_ir::EmitFloatMax(lhs_value, rhs_value, b_, fast_min_max(), name);
}

toolchain::Value* ElementalIrEmitter::EmitFloatMin(toolchain::Value* lhs_value,
                                              toolchain::Value* rhs_value,
                                              absl::string_view name) {
  return llvm_ir::EmitFloatMin(lhs_value, rhs_value, b_, fast_min_max(), name);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitLog(
    PrimitiveType prim_type, toolchain::Value* value) {
  return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::log, {value},
                                      {value->getType()}, b_);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitLog1p(
    PrimitiveType prim_type, toolchain::Value* value) {
  toolchain::Function* log1p = codegen::intrinsics::Log1p::GetOrInsertDeclaration(
      module_, IntrinsicType::S(prim_type));
  return b_->CreateCall(log1p, {value});
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitSqrt(PrimitiveType,
                                                          toolchain::Value* value) {
  return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::sqrt, {value},
                                      {value->getType()}, b_);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitRsqrt(
    PrimitiveType prim_type, toolchain::Value* value) {
  TF_ASSIGN_OR_RETURN(auto sqrt, EmitSqrt(prim_type, value));
  return FDiv(toolchain::ConstantFP::get(sqrt->getType(), 1.0), sqrt);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitSin(
    PrimitiveType prim_type, toolchain::Value* value) {
  return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::sin, {value},
                                      {value->getType()}, b_);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitCos(
    PrimitiveType prim_type, toolchain::Value* value) {
  return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::cos, {value},
                                      {value->getType()}, b_);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitCosm1(
    PrimitiveType prim_type, toolchain::Value* value) {
  auto x = value;
  auto type = llvm_ir::PrimitiveTypeToIrType(prim_type, module_->getContext());
  auto negative_half = toolchain::ConstantFP::get(type, -0.5);
  auto negative_one = toolchain::ConstantFP::get(type, -1.0);
  ...
  return Select(FCmpOGT(xx, toolchain::ConstantFP::get(type, 0.61685)), for_large_x,
                for_small_x);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitExp(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitExpm1(
    PrimitiveType prim_type, toolchain::Value* value) {
  auto x = value;
  auto type = llvm_ir::PrimitiveTypeToIrType(prim_type, module_->getContext());
  auto one = toolchain::ConstantFP::get(type, 1.0);
  auto half = toolchain::ConstantFP::get(type, 0.5);
  auto zero = toolchain::ConstantFP::get(type, 0.0);
  ...
  auto abs_x =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {x}, {type}, b_);
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitPow(
    PrimitiveType prim_type, toolchain::Value* lhs, toolchain::Value* rhs,
    absl::string_view name) {
  return llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::pow, {lhs, rhs},
                                      {lhs->getType()}, b_, name);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitCbrt(
    PrimitiveType prim_type, toolchain::Value* value) {
  auto type = llvm_ir::PrimitiveTypeToIrType(prim_type, module_->getContext());
  auto third = toolchain::ConstantFP::get(type, 1.0 / 3.0);
  auto abs_value =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {value}, {type}, b_);
  TF_ASSIGN_OR_RETURN(toolchain::Value * abs_res,
                      EmitPow(prim_type, abs_value, third, ""));
  auto signed_res = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::copysign,
                                                 {abs_res, value}, {type}, b_);
  return signed_res;
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitAtan2(
    PrimitiveType prim_type, toolchain::Value* lhs, toolchain::Value* /*rhs*/,
    absl::string_view /*name*/) {
  return Unimplemented("atan2");
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitTanh(
    PrimitiveType prim_type, toolchain::Value* value) {
  return Unimplemented("tanh");
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitErf(
    PrimitiveType prim_type, toolchain::Value* value) {
  return Unimplemented("erf");
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitTan(
    PrimitiveType prim_type, toolchain::Value* value) {
  auto sin_x = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::sin, {value},
                                            {value->getType()}, b_);
  auto cos_x = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::cos, {value},
                                            {value->getType()}, b_);
  return FDiv(sin_x, cos_x);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitReducePrecision(
...
static toolchain::Value* SaturateShiftIfNecessary(toolchain::IRBuilderBase* b,
                                             toolchain::Value* lhs, toolchain::Value* rhs,
                                             toolchain::Value* shift_result,
                                             bool saturate_to_sign_bit) {
  toolchain::IntegerType* integer_type =
      toolchain::cast<toolchain::IntegerType>(lhs->getType());
  unsigned integer_bitsize = integer_type->getBitWidth();
  toolchain::ConstantInt* integer_bitsize_constant =
      toolchain::ConstantInt::get(integer_type, integer_bitsize);
  toolchain::ConstantInt* zero = toolchain::ConstantInt::get(integer_type, 0);
  toolchain::ConstantInt* minus_one = toolchain::ConstantInt::get(integer_type, -1);
  toolchain::Value* saturated_value;
  ...
  toolchain::Value* shift_amt_in_range =
...
toolchain::Value* ElementalIrEmitter::GetOne(toolchain::Type* type) {
  return toolchain::ConstantInt::get(toolchain::cast<toolchain::IntegerType>(type), 1);
}

toolchain::Value* ElementalIrEmitter::GetZero(toolchain::Type* type) {
  return toolchain::ConstantInt::get(toolchain::cast<toolchain::IntegerType>(type), 0);
}

toolchain::Value* ElementalIrEmitter::GetIntSMin(toolchain::Type* type) {
  auto* integer_type = toolchain::cast<toolchain::IntegerType>(type);
  return toolchain::ConstantInt::get(integer_type, toolchain::APInt::getSignedMinValue(
                                                  integer_type->getBitWidth()));
}

toolchain::Value* ElementalIrEmitter::GetMinusOne(toolchain::Type* type) {
  auto* integer_type = toolchain::cast<toolchain::IntegerType>(type);
  return toolchain::ConstantInt::get(
      integer_type, toolchain::APInt::getAllOnes(integer_type->getBitWidth()));
}

toolchain::Value* ElementalIrEmitter::IsZero(toolchain::Value* v) {
  return ICmpEQ(v, toolchain::ConstantInt::get(v->getType(), 0));
}

toolchain::Value* ElementalIrEmitter::IsIntMinDivisionOverflow(toolchain::Value* lhs,
                                                          toolchain::Value* rhs) {
  return And(ICmpEQ(lhs, GetIntSMin(lhs->getType())),
             ICmpEQ(rhs, GetMinusOne(rhs->getType())));
}

toolchain::Value* ElementalIrEmitter::EmitIntegerDivide(toolchain::Value* lhs,
                                                   toolchain::Value* rhs,
                                                   bool is_signed) {
  ...
  if (!is_signed) {
    toolchain::Value* udiv_is_unsafe = IsZero(rhs);
    toolchain::Value* safe_rhs = Select(udiv_is_unsafe, GetOne(lhs->getType()), rhs);
    toolchain::Value* safe_div = UDiv(lhs, safe_rhs);
    return Select(udiv_is_unsafe, GetMinusOne(lhs->getType()), safe_div);
  }

  toolchain::Value* has_zero_divisor = IsZero(rhs);
  toolchain::Value* has_int_min_overflow = IsIntMinDivisionOverflow(lhs, rhs);
  toolchain::Value* sdiv_is_unsafe = Or(has_int_min_overflow, has_zero_divisor);
  toolchain::Value* safe_rhs = Select(sdiv_is_unsafe, GetOne(lhs->getType()), rhs);
  toolchain::Value* safe_div = SDiv(lhs, safe_rhs);
...
toolchain::Value* ElementalIrEmitter::EmitIntegerRemainder(toolchain::Value* lhs,
                                                      toolchain::Value* rhs,
                                                      bool is_signed) {
  ...
  if (!is_signed) {
    toolchain::Value* urem_is_unsafe = IsZero(rhs);
    toolchain::Value* safe_rhs = Select(urem_is_unsafe, GetOne(lhs->getType()), rhs);
    toolchain::Value* safe_rem = URem(lhs, safe_rhs);
    return Select(urem_is_unsafe, lhs, safe_rem);
  }

  toolchain::Value* has_zero_divisor = IsZero(rhs);
  toolchain::Value* has_int_min_overflow = IsIntMinDivisionOverflow(lhs, rhs);
  toolchain::Value* srem_is_unsafe = Or(has_int_min_overflow, has_zero_divisor);
  toolchain::Value* safe_rhs = Select(srem_is_unsafe, GetOne(lhs->getType()), rhs);
  toolchain::Value* safe_rem = SRem(lhs, safe_rhs);
...
toolchain::Value* ElementalIrEmitter::EmitIntegerPow(toolchain::Value* base,
                                                toolchain::Value* exponent,
                                                bool is_signed) {
  ...
  int bits = 6;  // Everything else would overflow for any exponent > 1, as 2^64
                 ...
                 // that's 1 << 6.
  toolchain::Value* accumulator = toolchain::ConstantInt::get(base->getType(), 1);
  toolchain::Value* one = toolchain::ConstantInt::get(exponent->getType(), 1);
  toolchain::Value* zero = toolchain::ConstantInt::get(exponent->getType(), 0);
  toolchain::Value* original_base = base;
  toolchain::Value* original_exponent = exponent;
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitPredBinaryOp(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value) {
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitIntegerBinaryOp(
    const HloInstruction* op, toolchain::Value* lhs_value, toolchain::Value* rhs_value,
    bool is_signed) {
  switch (op->opcode()) {
    ...
    case HloOpcode::kCompare: {
      switch (op->comparison_direction()) {
        case ComparisonDirection::kEq:
          return llvm_ir::EmitComparison(toolchain::CmpInst::ICMP_EQ, lhs_value,
                                         rhs_value, b_);
        case ComparisonDirection::kNe:
          return llvm_ir::EmitComparison(toolchain::CmpInst::ICMP_NE, lhs_value,
                                         rhs_value, b_);
        case ComparisonDirection::kLt:
          return llvm_ir::EmitComparison(
              is_signed ? toolchain::CmpInst::ICMP_SLT : toolchain::CmpInst::ICMP_ULT,
              lhs_value, rhs_value, b_);
        case ComparisonDirection::kGt:
          return llvm_ir::EmitComparison(
              is_signed ? toolchain::CmpInst::ICMP_SGT : toolchain::CmpInst::ICMP_UGT,
              lhs_value, rhs_value, b_);
        case ComparisonDirection::kLe:
          return llvm_ir::EmitComparison(
              is_signed ? toolchain::CmpInst::ICMP_SLE : toolchain::CmpInst::ICMP_ULE,
              lhs_value, rhs_value, b_);
        case ComparisonDirection::kGe:
          return llvm_ir::EmitComparison(
              is_signed ? toolchain::CmpInst::ICMP_SGE : toolchain::CmpInst::ICMP_UGE,
              lhs_value, rhs_value, b_);
      }
    }
    case HloOpcode::kMinimum:
...
toolchain::Value* ElementalIrEmitter::EmitIntegralMax(toolchain::Value* lhs_value,
                                                 toolchain::Value* rhs_value,
                                                 bool is_signed) {
  return Select(b_->CreateICmp(is_signed ? toolchain::ICmpInst::ICMP_SGE
                                         : toolchain::ICmpInst::ICMP_UGE,
                               lhs_value, rhs_value),
                lhs_value, rhs_value);
}

toolchain::Value* ElementalIrEmitter::EmitIntegralMin(toolchain::Value* lhs_value,
                                                 toolchain::Value* rhs_value,
                                                 bool is_signed) {
  return Select(b_->CreateICmp(is_signed ? toolchain::ICmpInst::ICMP_SLE
                                         : toolchain::ICmpInst::ICMP_ULE,
                               lhs_value, rhs_value),
                lhs_value, rhs_value);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalSelect(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& index) {
  TF_ASSIGN_OR_RETURN(toolchain::Value * pred_value,
                      operand_to_generator.at(hlo->operand(0))(index));
  TF_ASSIGN_OR_RETURN(toolchain::Value * on_true_value,
                      operand_to_generator.at(hlo->operand(1))(index));
  TF_ASSIGN_OR_RETURN(toolchain::Value * on_false_value,
                      operand_to_generator.at(hlo->operand(2))(index));
  return Select(Trunc(pred_value, b_->getInt1Ty()), on_true_value,
                on_false_value);
}

absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalClamp(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& index) {
  TF_ASSIGN_OR_RETURN(toolchain::Value * min_value,
                      operand_to_generator.at(hlo->operand(0))(index));
  TF_ASSIGN_OR_RETURN(toolchain::Value * arg_value,
                      operand_to_generator.at(hlo->operand(1))(index));
  TF_ASSIGN_OR_RETURN(toolchain::Value * max_value,
                      operand_to_generator.at(hlo->operand(2))(index));
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalConcatenate(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& source_index) {
  const int64_t concat_dim = hlo->dimensions(0);
  toolchain::BasicBlock* init_block = b_->GetInsertBlock();

  toolchain::BasicBlock* exit_block;
  ...
  // index inputs.
  std::vector<toolchain::BasicBlock*> emit_operand_blocks(
      to_unique_operand_id.size(), nullptr);
  std::vector<toolchain::PHINode*> source_index_phis(to_unique_operand_id.size(),
                                                nullptr);
  for (const HloInstruction* operand : hlo->operands()) {
    ...
    std::vector<toolchain::Value*> operand_multi_index = source_index.multidim();
    ...
    b_->SetInsertPoint(toolchain::BranchInst::Create(
  ...
  std::function<toolchain::BasicBlock*(
      ...
      emit_tree =
          [&](absl::Span<const std::pair<int64_t, const HloInstruction*>>
            ...
            toolchain::Constant* pivot_const = toolchain::ConstantInt::get(
                source_index[concat_dim]->getType(), pivot.first);
            toolchain::Value* comp =
                b_->CreateICmpULT(source_index[concat_dim], pivot_const);

            toolchain::BasicBlock* left_block = emit_tree(operands.subspan(0, mid));
            toolchain::BasicBlock* right_block = emit_tree(operands.subspan(mid));
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalDynamicSlice(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& index) {
  ...
  toolchain::Type* index_type = index.GetType();
  std::vector<toolchain::Value*> slice_start_multi_index(rank);
  for (int64_t i = 0; i < rank; ++i) {
    auto index_typed_const = [&](uint64_t c) -> toolchain::Constant* {
      return toolchain::ConstantInt::get(index_type, c);
    };
    llvm_ir::IrArray::Index zero_index(index_type);
    TF_ASSIGN_OR_RETURN(
        toolchain::Value * start_index_value,
  ...
  std::vector<toolchain::Value*> input_multi_index(rank);
  for (int64_t i = 0; i < rank; ++i) {
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalGather(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& index) {
  ...
  // generate.
  std::vector<toolchain::Value*> operand_multi_index;
  ...
  std::vector<toolchain::Value*> gather_index_index_components;
  {
  ...
  auto add_to_operand_index = [&](toolchain::Value* index_component, int64_t dim) {
    ...
    auto clamped_index = EmitIntegralMin(
        toolchain::ConstantInt::get(extended_type, largest_valid_start_index),
        EmitIntegralMax(toolchain::ConstantInt::get(extended_type, 0),
  ...
  if (indices_shape.dimensions().size() == dim_numbers.index_vector_dim()) {
    ...
    TF_ASSIGN_OR_RETURN(toolchain::Value * gather_dim_component,
...
ElementalIrEmitter::EmitElementalDynamicUpdateSlice(
  ...
  const int64_t rank = input_hlo->shape().dimensions().size();
  std::vector<toolchain::Value*> slice_start_multi_index(rank);
  std::vector<toolchain::Value*> slice_limit_multi_index(rank);
  // Slice intersection gathers (ANDs) conditions on all ranks for which
  // 'input' is set to 'update'
  toolchain::Value* slice_intersection = b_->getTrue();

  for (int64_t i = 0; i < rank; ++i) {
    toolchain::Type* index_type = index[0]->getType();
    auto index_typed_const = [&](uint64_t c) -> toolchain::Constant* {
      return toolchain::ConstantInt::get(index_type, c);
    };
    ...
    start_index_value =
        b_->CreateIntCast(start_index_value, index_type, is_signed);
    toolchain::Value* update_dim_size =
  ...
  toolchain::AllocaInst* ret_value_addr = llvm_ir::EmitAllocaAtFunctionEntry(
  ...
  std::vector<toolchain::Value*> update_multi_index(rank);
  for (int64_t i = 0; i < rank; ++i) {
  ...
  llvm_ir::IrArray::Index update_index(update_multi_index, update_hlo->shape(),
                                       index.GetType());
  TF_ASSIGN_OR_RETURN(toolchain::Value * true_value,
  ...
  TF_ASSIGN_OR_RETURN(toolchain::Value * false_value,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalPad(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
    const llvm_ir::IrArray::Index& padded_index) {
  std::vector<toolchain::Value*> multi_index = padded_index.multidim();
  toolchain::Value* in_bounds = b_->getTrue();
  ...
  TF_ASSIGN_OR_RETURN(toolchain::Value * operand_value,
                      operand_to_generator.at(hlo->operand(0))(index));
  Store(operand_value, ret_value_addr);

  SetToFirstInsertPoint(if_data.false_block, b_);
  TF_ASSIGN_OR_RETURN(toolchain::Value * padding_value,
                      operand_to_generator.at(hlo->operand(1))(
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalDot(
    const HloInstruction* hlo,
    const ElementalIrEmitter::HloToElementGeneratorMap& operand_to_generator,
  ...
  toolchain::Type* index_type = dot_result_index.GetType();
  auto index_typed_const = [&](uint64_t c) -> toolchain::Constant* {
    return toolchain::ConstantInt::get(index_type, c);
  };
  ...
  toolchain::Type* primitive_type_llvm =
      llvm_ir::PrimitiveTypeToIrType(primitive_type, module_->getContext());
  if (primitive_type == BF16) {
    primitive_type_llvm =
        llvm_ir::PrimitiveTypeToIrType(F32, module_->getContext());
  }
  toolchain::AllocaInst* accumulator_alloca =
      llvm_ir::EmitAllocaAtFunctionEntry(primitive_type_llvm, "dot_acc", b_);
  Store(toolchain::Constant::getNullValue(primitive_type_llvm), accumulator_alloca);
  ...
  std::vector<toolchain::Value*> lhs_multi_index(lhs_dims, nullptr),
  ...
  toolchain::Value* current_accumulator =
      Load(accumulator_alloca->getAllocatedType(), accumulator_alloca);
  TF_ASSIGN_OR_RETURN(toolchain::Value * lhs_value, lhs_generator(lhs_index));
  TF_ASSIGN_OR_RETURN(toolchain::Value * rhs_value, rhs_generator(rhs_index));
  ...
  toolchain::Value* next_accumulator =
  ...
  switch (hlo->opcode()) {
    ...
    case HloOpcode::kTanh:
      return [this, hlo, &operand_to_generator](
                 const IrArray::Index& index) -> absl::StatusOr<toolchain::Value*> {
        TF_ASSIGN_OR_RETURN(toolchain::Value * operand_value,
                            operand_to_generator.at(hlo->operand(0))(index));
    ...
    case HloOpcode::kSubtract:
      return [this, hlo, &operand_to_generator](
        ...
        TF_ASSIGN_OR_RETURN(toolchain::Value * lhs_value,
                            operand_to_generator.at(lhs)(index));
        TF_ASSIGN_OR_RETURN(toolchain::Value * rhs_value,
    ...
    case HloOpcode::kReverse:
      return [this, hlo,
        ...
        std::vector<toolchain::Value*> source_multi_index = target_index.multidim();
      ...
      return [this, hlo](const IrArray::Index& target_index)
        ...
        IrArray::Index elem_index =
            iota->shape().dimensions().size() > 1
                ...
                : target_index;
        toolchain::Value* elem_index_linear = elem_index.linear();
...
toolchain::Value* ElementalIrEmitter::EmitExtractReal(toolchain::Value* value) {
  return ExtractValue(value, {0});
}

toolchain::Value* ElementalIrEmitter::EmitExtractImag(toolchain::Value* value) {
  return ExtractValue(value, {1});
}

toolchain::Value* ElementalIrEmitter::EmitComposeComplex(const HloInstruction* op,
                                                    toolchain::Value* real,
                                                    toolchain::Value* imag) {
  auto cplx_type = llvm_ir::PrimitiveTypeToIrType(op->shape().element_type(),
                                                  module_->getContext());
  auto complex =
      InsertValue(toolchain::ConstantAggregateZero::get(cplx_type), real, {0});
  if (imag != nullptr) {
...
toolchain::Value* ElementalIrEmitter::EmitMulAdd(toolchain::Value* lhs, toolchain::Value* rhs,
  ...
  if (primitive_util::IsComplexType(primitive_type)) {
    ...
    toolchain::FastMathFlags reassoc_flag = b_->getFastMathFlags();
    reassoc_flag.setAllowReassoc(true);

    toolchain::Value* cast_mul = FPCast(FMul(lhs, rhs), accumulator->getType());
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalMap(
    const HloMapInstruction* map_instr,
    absl::Span<toolchain::Value* const> elemental_operands) {
  TF_ASSIGN_OR_RETURN(
      std::vector<toolchain::Value*> values,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalReduceWindow(
  ...
  std::vector<toolchain::Type*> accum_types;
  std::vector<toolchain::Value*> accum_ptrs;
  for (int64_t operand_index = 0; operand_index < input_count;
    ...
    toolchain::Type* llvm_type = llvm_ir::PrimitiveTypeToIrType(
        operand_element_type, module_->getContext());
    accum_types.push_back(llvm_type);
    toolchain::Value* accum_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
    ...
    accum_ptrs.push_back(accum_ptr);
    {
      ...
      TF_ASSIGN_OR_RETURN(
          toolchain::Value* const init_value,
  ...
  std::vector<toolchain::Value*> input_multi_index(index.size());
  toolchain::Value* in_bounds = b_->getInt1(true);
  for (size_t i = 0; i < index.size(); ++i) {
    toolchain::Value* stridden_index =
    ...
    toolchain::Value* dilation_condition =
  ...
  std::vector<toolchain::Value*> input_values(reduce_window->operand_count());
  ...
  for (int64_t operand_idx = 0; operand_idx < input_count; ++operand_idx) {
    TF_ASSIGN_OR_RETURN(toolchain::Value * input_value,
                        input_generators[operand_idx](input_index));
    input_values[input_count + operand_idx] = input_value;
    input_values[operand_idx] =
        Load(toolchain::cast<toolchain::AllocaInst>(accum_ptrs[operand_idx])
                 ->getAllocatedType(),
             accum_ptrs[operand_idx]);
  }
  TF_ASSIGN_OR_RETURN(std::vector<toolchain::Value*> accum_values,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitElementalReduce(
  ...
  std::vector<toolchain::Value*> accumulator_addrs;
  std::vector<toolchain::Type*> accumulator_types;
  toolchain::Type* index_type = index.GetType();
  for (int i = 0; i < accumulators_count; i++) {
    ...
    toolchain::Type* accumulator_llvm_type =
    ...
    toolchain::AllocaInst* accumulator_addr = llvm_ir::EmitAllocaAtFunctionEntry(
  ...
  std::vector<toolchain::Value*> input_multi_index =
  ...
  std::vector<toolchain::Value*> reduction_operands;
  for (toolchain::Value* accum : accumulator_addrs) {
    toolchain::Value* accum_value =
        Load(toolchain::cast<toolchain::AllocaInst>(accum)->getAllocatedType(), accum);
    reduction_operands.push_back(accum_value);
  }

  for (int i = 0; i < accumulators_count; i++) {
    TF_ASSIGN_OR_RETURN(toolchain::Value* const input_element,
                        input_generators[i](input_index));
    reduction_operands.push_back(input_element);
  }

  TF_ASSIGN_OR_RETURN(
      std::vector<toolchain::Value*> results,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitAccumResult(
  ...
  if (is_variadic) {
    ...
    toolchain::Value* returned_structure = toolchain::UndefValue::get(
        toolchain::StructType::get(b()->getContext(), accumulator_types));
    for (int64_t i = 0; i < accumulator_addrs.size(); i++) {
      toolchain::Value* accumulator_value =
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EmitConvolution(
  ...
  std::vector<toolchain::Value*> output_spatial(num_spatial_dims);
  for (int i = 0; i < num_spatial_dims; ++i) {
    output_spatial[i] = index[dnums.output_spatial_dimensions(i)];
  }
  toolchain::Value* output_feature = index[dnums.output_feature_dimension()];
  toolchain::Value* batch = index[dnums.output_batch_dimension()];
  ...
  toolchain::Type* accumulator_type =
      lhs_element_type == F16 ? b_->getFloatTy() : lhs_llvm_type;
  toolchain::AllocaInst* sum_address = llvm_ir::EmitAllocaAtFunctionEntry(
      accumulator_type, "convolution_sum_address", b_);
  toolchain::Value* constant_zero = toolchain::Constant::getNullValue(accumulator_type);
  Store(constant_zero, sum_address);

  llvm_ir::ForLoopNest loops(IrName(convolution, "inner"), b_);
  std::vector<toolchain::Value*> kernel_spatial(num_spatial_dims);
  for (int i = 0; i < num_spatial_dims; ++i) {
    kernel_spatial[i] =
        loops
            .AddLoop(
  ...
  const int64_t output_group_size =
      ...
      feature_group_count;
  toolchain::Value* input_feature =
      loops.AddLoop(0, input_group_size, "iz")->GetIndVarValue();

  SetToFirstInsertPoint(loops.GetInnerLoopBodyBasicBlock(), b_);

  toolchain::Value* group_id = SDiv(output_feature, b_->getInt64(output_group_size));
  toolchain::Value* lhs_input_feature =
  ...
  // of the array.
  const auto calculate_input_index = [this](toolchain::Value* output_index,
                                            toolchain::Value* kernel_index,
                                            const WindowDimension& window_dim) {
    toolchain::Value* strided_index =
        NSWMul(output_index, b_->getInt64(window_dim.stride()));
    toolchain::Value* dilated_kernel_index =
  ...
  std::vector<toolchain::Value*> input_spatial(num_spatial_dims);
  for (int i = 0; i < num_spatial_dims; ++i) {
  ...
  const auto not_in_hole = [&](toolchain::Value* input_index,
                               int64_t base_dilation) {
    toolchain::Value* remainder = SRem(input_index, b_->getInt64(base_dilation));
    return ICmpEQ(remainder, b_->getInt64(0));
  };

  toolchain::Value* in_bounds_condition = b_->getInt1(true);
  for (int i = 0; i < num_spatial_dims; ++i) {
    toolchain::ConstantInt* input_bound = b_->getInt64(window_util::DilatedBound(
        lhs->shape().dimensions(dnums.input_spatial_dimensions(i)),
        window.dimensions(i).base_dilation()));
    toolchain::Value* dim_in_bound = ICmpULT(input_spatial[i], input_bound);
    toolchain::Value* dim_not_in_hole =
        not_in_hole(input_spatial[i], window.dimensions(i).base_dilation());
    toolchain::Value* dim_ok = And(dim_in_bound, dim_not_in_hole);
  ...
  const auto undilate = [&](toolchain::Value* input_index, int64_t base_dilation) {
  ...
  std::vector<toolchain::Value*> input_multi_index(num_dims);
  for (int i = 0; i < num_spatial_dims; ++i) {
  ...
  std::vector<toolchain::Value*> kernel_multi_index(num_dims);
  for (int i = 0; i < num_spatial_dims; ++i) {
  ...
  llvm_ir::IrArray::Index input_index(input_multi_index, lhs->shape(),
                                      b_->getInt64Ty());
  TF_ASSIGN_OR_RETURN(toolchain::Value* const input_value,
                      input_generator(input_index));
  llvm_ir::IrArray::Index kernel_index(kernel_multi_index, rhs->shape(),
                                       b_->getInt64Ty());
  TF_ASSIGN_OR_RETURN(toolchain::Value* const kernel_value,
...
absl::StatusOr<toolchain::Value*> ElementalIrEmitter::EvaluatePolynomial(
    toolchain::Type* type, toolchain::Value* x, absl::Span<const double> coefficients) {
  toolchain::Value* poly = toolchain::ConstantFP::get(type, 0.0);
  for (const double c : coefficients) {
    poly = FAdd(FMul(poly, x), toolchain::ConstantFP::get(type, c));
  }
  return poly;
}
