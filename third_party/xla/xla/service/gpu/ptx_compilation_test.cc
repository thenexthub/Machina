...
#include "machina/xla/stream_executor/cuda/ptx_compilation_method.h"
#include "machina/xla/stream_executor/cuda/ptx_compiler_support.h"
#include "machina/xla/stream_executor/cuda/ptx_linking_method.h"
...
TEST_P(NVPTXCompilationTests, CompareBinaryOutput) {
  ...
  auto get_text_sections = [&](absl::Span<const uint8_t> binary)
    ...
    auto buffer = toolchain::MemoryBuffer::getMemBuffer(
        toolchain::StringRef(reinterpret_cast<const char*>(binary.data()),
                        binary.size()),
        /*BufferName=*/"", /*RequiresNullTerminator=*/false);
    auto object_file =
        toolchain::object::ObjectFile::createObjectFile(buffer->getMemBufferRef());

    if (!object_file) {
      return absl::InternalError(toolchain::toString(object_file.takeError()));
    }

    auto executable_elf_object_file =
        toolchain::dyn_cast<toolchain::object::ELF64LEObjectFile>(
            object_file.get().get());
