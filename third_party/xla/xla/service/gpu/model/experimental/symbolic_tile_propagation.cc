...
#include "machina/xla/service/gpu/model/experimental/symbolic_tile_propagation.h"
...
std::optional<SymbolicTiles> PropagateTileToInputForConcatenateOp(
  ...
  auto upper_bound = toolchain::dyn_cast<mlir::AffineConstantExpr>(
...
SymbolicTile PropagateTileToInputForSliceImpl(
  ...
  for (const auto& [dim, result_dim_tile] :
       toolchain::enumerate(output_tile.dim_tiles())) {
...
std::optional<SymbolicTiles> PropagateTileToInputForPadOp(
  ...
  for (const auto [result_dim_tile, padding_dim, operand_dim] :
       toolchain::zip(output_tile.dim_tiles(), padding_config.dimensions(),
...
SymbolicTile PropagateTileThroughTransposeOp(
  ...
  for (const auto [dim, permutated_dim] : toolchain::enumerate(permutation)) {
...
SymbolicTiles PropagateTileToInputForDotOp(const TilingSpace& tiling_space,
  ...
  for (auto [output_dim_id, batch_dims] :
       toolchain::enumerate(toolchain::zip(lhs_batch_dims, rhs_batch_dims))) {
  ...
  for (auto [contracting_dim_id, contracting_dims] :
       toolchain::enumerate(toolchain::zip(lhs_contracting_dims, rhs_contracting_dims))) {
...
std::string ToString(const SymbolicTiles& tiles) {
  std::stringstream ss;
  for (const auto& [index, tile] : toolchain::enumerate(tiles)) {
