...
#include "machina/xla/service/gpu/model/experimental/symbolic_tiled_hlo_computation.h"
...
/*static*/ SymbolicTileAnalysisOrError SymbolicTiledComputation::Tile(
  ...
  for (const auto& [root, tile] :
       toolchain::zip(fusion.GetRoots(), tiling_space->tiled_roots())) {
  ...
  while (!worklist.empty()) {
    ...
    for (const auto& [tile, operand] :
         toolchain::zip(*tiled_operands, instruction_adaptor.GetOperands())) {
