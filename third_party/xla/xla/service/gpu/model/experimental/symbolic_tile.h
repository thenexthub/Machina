...
#ifndef MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_EXPERIMENTAL_SYMBOLIC_TILE_H_
#define MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_EXPERIMENTAL_SYMBOLIC_TILE_H_
...
H AbslHashValue(H h, const DimTile& dim_tile) {
  toolchain::hash_code dim_tile_hash = toolchain::hash_combine(
...
class SymbolicTile {
 public:
  SymbolicTile(const TilingSpace& tiling_space,
               toolchain::SmallVector<DimTile> dim_tiles);

  SymbolicTile(const TilingSpace& tiling_space,
               toolchain::ArrayRef<mlir::AffineExpr> offsets,
               toolchain::ArrayRef<mlir::AffineExpr> sizes,
               toolchain::ArrayRef<mlir::AffineExpr> strides,
               toolchain::ArrayRef<mlir::AffineExpr> upper_bounds);

  std::string ToString(bool print_variables = true) const;

  toolchain::SmallVector<mlir::AffineExpr> offsets() const;
  toolchain::SmallVector<mlir::AffineExpr> sizes() const;
  toolchain::SmallVector<mlir::AffineExpr> strides() const;
  toolchain::SmallVector<mlir::AffineExpr> upper_bounds() const;
  toolchain::ArrayRef<DimTile> dim_tiles() const { return dim_tiles_; }
 ...
 private:
  ...
  toolchain::SmallVector<DimTile> dim_tiles_;
};
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_EXPERIMENTAL_SYMBOLIC_TILE_H_
