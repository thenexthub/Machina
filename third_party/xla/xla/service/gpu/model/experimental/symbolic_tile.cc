...
SmallVector<std::string> GetVarNames(int64_t num_vars, toolchain::StringRef prefix) {
...
DimTile GetFullDimTile(int64_t dim_size, MLIRContext* ctx) {
  return DimTile{getAffineConstantExpr(0, ctx),
                 getAffineConstantExpr(toolchain::PowerOf2Ceil(dim_size), ctx),
...
SymbolicTile::SymbolicTile(const TilingSpace& tiling_space,
  ...
  for (auto [offset, size, stride, upper_bound] :
       toolchain::zip(offsets, sizes, strides, upper_bounds)) {
...
std::string SymbolicTile::ToString(bool print_variables) const {
  ...
  // Print offsets.
  ss << " offsets [";
  toolchain::interleaveComma(offsets(), ss, print_expr);
  ss << "] sizes [";
  toolchain::interleaveComma(sizes(), ss, print_expr);
  ss << "] strides [";
  toolchain::interleaveComma(strides(), ss, print_expr);
  ss << "] upper bounds [";
  toolchain::interleaveComma(upper_bounds(), ss, print_expr);
  ss << ']';
  return s;
}
