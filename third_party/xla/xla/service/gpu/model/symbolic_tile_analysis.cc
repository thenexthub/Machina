...
#include "machina/xla/hlo/analysis/indexing_analysis.h"
#include "machina/xla/hlo/analysis/indexing_map.h"
#include "machina/xla/hlo/analysis/indexing_map_serialization.h"
...
#include "machina/xla/service/gpu/model/constraint_expression.h"
...
struct OutputTilingInfo {
  ...
  toolchain::SmallVector<int64_t> num_output_tiles_per_dim;
  ...
  toolchain::SmallVector<int64_t> active_tiling_parameters;
...
toolchain::SmallVector<int64_t> GetNumberOfTilesPerDimension(
  ...
  for (auto [dim_size, tile_size] :
       toolchain::zip(dimensions, tiled_hlo_instr.tile_sizes())) {
...
IndexingMap LinearizeTileOffsets(
  ...
  for (auto [dim_id, tile_expr] :
       toolchain::zip(major_to_minor_active_tiling_parameters,
...
absl::StatusOr<OutputTilingInfo> ComputeOutputTilingInfo(
  ...
  toolchain::SmallVector<int64_t> outer_loop_bounds(num_tiling_parameters, 1);
  std::vector<IndexingMap::Variable> dim_vars(num_tiling_parameters,
                                              ignore_variable);
  toolchain::SmallVector<AffineExpr> tiled_dims(
      num_tiling_parameters, mlir::getAffineConstantExpr(0, mlir_context));
  toolchain::SmallVector<std::pair<mlir::AffineExpr, Interval>> constraints;
...
absl::StatusOr<IndexingMap> ComputeTileOffsetIndexing(
  ...
  toolchain::MapVector<mlir::AffineExpr, Interval> updated_constraints;
...
toolchain::SmallVector<const TiledHloInstruction*> RemoveInstructionByMask(
    const toolchain::SmallVector<const TiledHloInstruction*>& elements,
    const toolchain::SmallBitVector& to_drop) {
  toolchain::SmallVector<const TiledHloInstruction*> result;
  result.reserve(elements.size() - to_drop.count());
  for (const auto& [i, v] : toolchain::enumerate(elements)) {
...
toolchain::SmallVector<const TiledHloInstruction*> MapToTiledInstructions(
...
IndexingMap InsertTilingParameterForContractingDimensions(
  ...
  if (consumer->opcode() == HloOpcode::kDot) {
    ...
    for (auto [parameter_index, contracting_dimension] :
         toolchain::enumerate(contracting_dimensions)) {
    ...
    for (const auto& [i, dim_var] :
         toolchain::enumerate(outermost_fusion_root_to_operand.GetDimVars())) {
...
ComposeIndexingResult ComposeInstructionIndexing(
  ...
  toolchain::SmallBitVector removed = composed_indexing.RemoveUnusedSymbols();
  std::vector<SymbolicTiledHloInstruction*> rt_operands;
  rt_operands.reserve(tiled_hlo_instruction->runtime_variables().size() +
                      operand_indexing.runtime_variables().size());
  for (auto [i, rt] :
       toolchain::enumerate(tiled_hlo_instruction->runtime_variables())) {
  ...
  for (const auto& [i, rt_var] :
       toolchain::enumerate(operand_indexing.runtime_variables())) {
...
/*static*/ SymbolicTileAnalysisOrError SymbolicTileAnalysis::AnalyzeFusionImpl(
  ...
  while (!worklist.empty()) {
    ...
    for (auto [operand_pos, operand_and_indexing_map_set] : toolchain::enumerate(
             toolchain::zip(instruction_adaptor.GetOperands(), operands_indexing))) {
...
absl::StatusOr<bool> IsSafeForBufferSharing(const TiledHloInstruction& output,
  ...
  toolchain::SmallVector<int64_t> num_tiles_per_dim =
  ...
  auto iota = toolchain::seq<int64_t>(0, output.hlo()->shape().dimensions().size());
...
absl::StatusOr<std::vector<const TiledHloInstruction*>> InitializeTiledRoots(
  ...
  for (const auto& tiled_hlo_instr : toolchain::drop_end(tiled_hlo_instructions)) {
  ...
  for (auto [tiled_root, root] : toolchain::zip(tiled_roots, roots)) {
...
absl::StatusOr<TiledHloComputation> ComputeTiledHloInstructionsImpl(
  ...
  for (const std::unique_ptr<SymbolicTiledHloInstruction>& symbolic_tiled_hlo :
    ...
    toolchain::SmallVector<int64_t> tile_strides = EvaluateTileStrides(
  ...
  toolchain::SmallPtrSet<const HloInstruction*, 8> parameters_with_offset_indexing;
  absl::flat_hash_map<const SymbolicTiledHloInstruction*,
  ...
  if (!compute_all_tile_offset_indexing_maps) {
    ...
    for (const std::unique_ptr<SymbolicTiledHloInstruction>& symbolic_tiling :
      ...
      toolchain::SmallVector<int64_t> tile_sizes = EvaluateTileSizes(
  ...
  for (const std::unique_ptr<SymbolicTiledHloInstruction>& symbolic_tiled_hlo :
       analysis.GetSymbolicTiledHloComputation()) {
    toolchain::SmallVector<int64_t> tile_sizes;
    ...
    toolchain::SmallVector<const TiledHloInstruction*> runtime_variables;
    ...
    if (compute_all_tile_offset_indexing_maps ||
      ...
      toolchain::SmallBitVector removed =
    ...
    toolchain::SmallVector<const TiledHloInstruction*> operands =
