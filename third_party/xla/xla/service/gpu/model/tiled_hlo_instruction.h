...
#ifndef MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_TILED_HLO_INSTRUCTION_H_
#define MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_TILED_HLO_INSTRUCTION_H_
...
class TiledHloInstruction {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<TiledHloInstruction>> Create(
      ...
      toolchain::SmallVector<int64_t> tile_strides,
  ...
  const toolchain::SmallVector<const TiledHloInstruction*>& operands() const {
    return operands_;
  }

  const toolchain::SmallVector<const TiledHloInstruction*>& runtime_variables()
      const {
  ...
  // the output shape.
  const toolchain::SmallVector<int64_t>& tile_sizes() const { return tile_sizes_; }
  ...
  // of the output shape.
  const toolchain::SmallVector<int64_t>& tile_strides() const {
 ...
 private:
  ...
  // Operands of the instruction in the tiled computation graph.
  toolchain::SmallVector<const TiledHloInstruction*> operands_;
  toolchain::SmallVector<const TiledHloInstruction*> runtime_variables_;

  // Tile sizes and strides.
  toolchain::SmallVector<int64_t> tile_sizes_;
  toolchain::SmallVector<int64_t> tile_strides_;
...
H AbslHashValue(H h, const TiledHloInstruction& tiled_hlo_instruction) {
  // There is no default hash implementation for toolchain::SmallVector neither in
  // AbslHashValue nor in toolchain::hash_value. We can use the available hash
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_GPU_MODEL_TILED_HLO_INSTRUCTION_H_
