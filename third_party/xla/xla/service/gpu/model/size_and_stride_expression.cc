...
#include "machina/xla/service/gpu/model/size_and_stride_expression.h"
...
using ConjointConstraints = toolchain::SmallVector<Constraint, 2>;
...
std::optional<SizeAndStrideExpression> ExtractSizeAndStrideFromMod(
  ...
  if (toolchain::isa<AffineDimExpr>(lhs)) {
  ...
  AffineExpr dim = toolchain::cast<AffineBinaryOpExpr>(lhs).getLHS();
  AffineExpr den = toolchain::cast<AffineBinaryOpExpr>(lhs).getRHS();
...
void DestructureSummationImpl(AffineExpr expr,
  ...
  switch (expr.getKind()) {
    case AffineExprKind::Add: {
      const auto add = toolchain::cast<AffineBinaryOpExpr>(expr);
...
void SortByStride(std::vector<SizeAndStrideExpression>& sizes_and_strides,
  ...
  absl::c_sort(sizes_and_strides, [&](const SizeAndStrideExpression& sas1,
    ...
    int64_t stride1 =
        std::abs(toolchain::cast<AffineConstantExpr>(sas1.stride).getValue());
    int64_t stride2 =
        std::abs(toolchain::cast<AffineConstantExpr>(sas2.stride).getValue());
    if (reverse) {
...
std::optional<int64_t> TryGetSizeExpressionRangeSize(
  ...
  if (size.getKind() == AffineExprKind::Constant) {
    return toolchain::cast<AffineConstantExpr>(size).getValue();
  }
  CHECK(size.getKind() == AffineExprKind::DimId);
  auto dim_position = toolchain::dyn_cast<AffineDimExpr>(size).getPosition();
...
std::optional<AffineExpr> CombineStrides(
  ...
  for (auto [dim_id, size_and_stride] : toolchain::enumerate(sizes_and_strides)) {
    int64_t stride =
        toolchain::cast<AffineConstantExpr>(size_and_stride.stride).getValue();
    ...
    if (dim_id > 0) {
      ...
      int64_t previous_stride =
          toolchain::cast<AffineConstantExpr>(previous_size_and_stride.stride)
              .getValue();
...
std::optional<SizeAndStrideExpression> ExtractSizeAndStride(
  ...
  switch (strided_indexing.getKind()) {
    ...
    case AffineExprKind::Mul: {
      const auto mul = toolchain::cast<AffineBinaryOpExpr>(strided_indexing);
    ...
    case AffineExprKind::Mod: {
      auto mod = toolchain::cast<AffineBinaryOpExpr>(strided_indexing);
      return ExtractSizeAndStrideFromMod(mod.getLHS(), mod.getRHS());
    }
    case AffineExprKind::FloorDiv: {
      auto floor_div = toolchain::cast<AffineBinaryOpExpr>(strided_indexing);
    ...
    case AffineExprKind::SymbolId: {
      auto symbol = toolchain::cast<AffineSymbolExpr>(strided_indexing);
