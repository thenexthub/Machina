...
#include "machina/xla/service/gpu/model/coalescing_analysis.h"
...
bool EstimateCoalescingViaMemoryTransactionsCount(
  ...
  for (const auto& range : intervals) {
    ...
    memory_transactions += toolchain::divideCeilSigned(num_elements * type_size,
  ...
  int memory_transactions_lower_bound = toolchain::divideCeilSigned(
...
std::optional<GroupedByOpIndexingMap> GetThreadIdToInputMemoryLayoutsMaps(
  ...
  for (const auto& [root_index, hero] :
       toolchain::enumerate(fusion_analysis.fusion_heroes())) {
    for (const auto& [hero_operand_index, hero_operand] :
         toolchain::enumerate(hero.GetOperands())) {
...
void AssignValuesToRTVars(IndexingMap* indexing_map) {
  ...
  toolchain::SmallVector<AffineExpr, 2> symbol_replacements;
