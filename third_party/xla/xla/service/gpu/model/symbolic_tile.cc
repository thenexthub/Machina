...
AffineMap SubstituteAllIndicesAndRangeVarSymbolsWithSameValue(
  ...
  toolchain::DenseMap<AffineExpr, AffineExpr> indices;
...
/*static*/ std::optional<SymbolicTile> SymbolicTile::FromIndexingMap(
  ...
  for (auto [composite_indexing, offset] :
       toolchain::zip(input_affine_map.getResults(), offset_expressions)) {
...
toolchain::SmallVector<int64_t> EvaluateTileOffsets(
...
toolchain::SmallVector<int64_t> EvaluateTileSizes(
    const SymbolicTile& symbolic_tile, absl::Span<int64_t const> parameters) {
  return EvaluateAffineMap(symbolic_tile.size_map(),
                           /*dim_values=*/parameters);
}

toolchain::SmallVector<int64_t> EvaluateTileStrides(
    const SymbolicTile& symbolic_tile, absl::Span<int64_t const> parameters) {
  toolchain::SmallVector<int64_t> clamped_parameters;
  ...
  for (auto [parameter, dim_bounds] :
       toolchain::zip(parameters, symbolic_tile.tile_map().GetDimensionBounds())) {
