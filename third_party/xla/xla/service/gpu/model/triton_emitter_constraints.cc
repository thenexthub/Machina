...
#include "machina/xla/service/gpu/model/affine_map_evaluator.h"
...
toolchain::SmallVector<int64_t> GetPaddedTileSizes(
  ...
  for (int64_t value : tile_sizes) {
    result.push_back(toolchain::PowerOf2Ceil(value));
  }
  return result;
}
...
TritonEmitterConstraints::GetBuilder(
  ...
  return [=](const std::vector<std::unique_ptr<SymbolicTiledHloInstruction>>&
                 instructions,
             const HloFusionAdaptor& fusion_adaptor) {
    toolchain::DenseSet<AffineMap> unique_tile_size_maps;
    toolchain::SmallVector<RootTileInfo, 2> root_infos;
    ...
    toolchain::SmallVector<AffineMap, 4> tile_size_maps(
...
int64_t NumberOfElementsInPaddedTile(
  ...
  for (auto expr : tile_size_map.getResults()) {
    num_elements *= toolchain::PowerOf2Ceil(
...
bool NumberOfBlocksFitsOnDeviceGrid(const Shape& output_shape,
  ...
  if (output_shape.IsArray()) {
    for (auto [dim_size, tile_size] :
         toolchain::zip(output_shape.dimensions(), tiling_parameters)) {
...
absl::StatusOr<bool> TritonEmitterConstraints::ParametersSatisfyConstraints(
  ...
  for (const auto& custom_constraint : custom_constraints_) {
    toolchain::SmallVector<int64_t> transformed_tile_parameters =
  ...
  for (const auto& root : roots_) {
    ...
    for (auto [tile_size, dim_size] :
         toolchain::zip(transformed_tile_parameters, root.dim_sizes)) {
