...
CompileModuleResults InitializeResults(const HloModule* hlo_module,
  ...
  results.llvm_module =
      std::make_unique<toolchain::Module>(module_name, *llvm_context);
  results.llvm_module->setTargetTriple(toolchain::Triple(target_triple));
  results.llvm_module->setDataLayout(data_layout);

  if (split_constants_module) {
    // Constants are emitted into a separate module to avoid caching them.
    results.llvm_module_constants = std::make_unique<toolchain::Module>(
        absl::StrCat(module_name, "_consts"), *llvm_context);
    results.llvm_module_constants->setTargetTriple(toolchain::Triple(target_triple));
...
absl::StatusOr<std::unique_ptr<SequentialThunk>> LowerHlo(
    ...
    toolchain::Module* llvm_module_constants, se::Platform::Id platform_id,
    bool use_cache) {
...
absl::StatusOr<CompileModuleResults> CompileModuleToLlvmIr(
    const HloModule* hlo_module, toolchain::LLVMContext* llvm_context,
