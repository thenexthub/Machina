...
ConvolutionMatch MatchBackwardFilter(HloInstruction* conv) {
  ...
  // similar to third_party/toolchain/toolchain/include/toolchain/IR/PatternMatch.h
  //
...
ConvolutionMatch MatchBackwardInput(HloInstruction* conv) {
  ...
  for (size_t i = 0; i < input_spatial_dims.size(); ++i) {
    ...
    // See the comment in machina/core/kernels/conv_grad_ops.h for how we
...
absl::StatusOr<bool> ConvRewriter::Run(
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(2, "ConvRewriter::Run(), before:\n" + module->ToString());
  bool changed = false;
  ...
  MACHINA_MACHINA_XLA_VLOG_LINES(2, "ConvRewriter::Run(), after:\n" + module->ToString());
  return changed;
}
