...
#include "machina/xla/tools/hlo_extractor.h"
...
using HloInstructionSetVector =
    toolchain::SetVector<HloInstruction*, std::vector<HloInstruction*>,
                    HloInstructionSet>;
...
toolchain::SmallVector<int64_t> GetInversePermutation(
    absl::Span<const int64_t> permutation) {
  toolchain::SmallVector<int64_t> result(permutation.size());
...
toolchain::SmallVector<int64_t> ApplyPermutation(
...
toolchain::SmallVector<int64_t> GetPhysicalDimensions(const Shape& shape) {
...
struct BitcastParams {
  ...
  toolchain::SmallVector<int64_t> new_dims;  // The dims of the broadcast/transpose.
};
...
absl::StatusOr<BitcastParams> CalculateBitcastOfBroadcast(
  ...
  toolchain::SmallVector<bool> is_operand_dim(broadcast_shape.dimensions().size());
  for (const int64_t index : broadcast->dimensions()) {
    is_operand_dim[index] = true;
  }

  // Dimensions of the new broadcast.
  toolchain::SmallVector<int64_t> new_dims;
...
absl::StatusOr<BitcastParams> CalculateBroadcastOfBitcast(
  ...
  toolchain::SmallVector<std::optional<std::pair<int64_t, int64_t>>>
  ...
  toolchain::SmallVector<int64_t> new_shape_dims(new_rank);
...
absl::StatusOr<BitcastParams> CalculateBitcastOfTransposeImpl(
  ...
  // dimensions.
  toolchain::SmallVector<std::pair<int64_t, int64_t>> operand_to_result_range(
      operand_shape.dimensions().size());
  // Maps logical operand dimension index to the physical dimension index.
  toolchain::SmallVector<int64_t> operand_inv_layout =
  ...
  *result.new_shape.mutable_layout() = result_shape.layout();
  ...
  toolchain::SmallVector<int64_t> new_shape_dims(result_shape.dimensions().size());
...
PlanHoistBitcastUpwardsToCallers(const HloInstruction* bitcast) {
  ...
  for (HloInstruction* instruction :
       toolchain::make_range(def_before_use.rbegin(), def_before_use.rend())) {
    auto it = result_shapes.find(instruction);
    if (it == result_shapes.end()) {
...
absl::StatusOr<BlockLevelParameters> FindBlockLevelParameters(
  ...
  auto get_tile_sizes = [&](int64_t rank) {
    ...
    toolchain::SmallVector<int64_t> tile_sizes(rank - 2, 1);
  ...
  toolchain::SmallVector<int64_t> output_tile_sizes = get_tile_sizes(out_rank);
  ...
  do {
    ...
    toolchain::SmallVector<int64_t> mapped_dot_tile_sizes =
