...
#include "machina/xla/hlo/analysis/hlo_dfs_reachability.h"
...
#include "machina/xla/service/gpu/fusion_deduplication_cache.h"
#include "machina/xla/service/gpu/fusion_process_dump.pb.h"
#include "machina/xla/service/gpu/gpu_fusible.h"
...
class PriorityFusionQueue {
 ...
 public:
  ...
  void ComputeAndSetPriorities(
    ...
    for (auto [instruction, priority] : toolchain::zip(instructions, priorities)) {
  ...
  bool DequeueNextProducer() {
    ...
    while (!producer_priority_queue_.empty() && current_consumers_.empty()) {
      ...
      if (HloPredicateIsOp<HloOpcode::kBitcast>(current_producer_)) {
        ...
        // have to do it here.
        toolchain::erase_if(current_consumers_, [&](HloInstruction* consumer) {
