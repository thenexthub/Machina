...
ParallelLoopEmitter::ParallelLoopEmitter(
    ...
    const LaunchDimensions& launch_dimensions, toolchain::IRBuilderBase* b,
...
ParallelLoopEmitter::EmitLinearBaseAndThreadIdx(toolchain::Type* index_type,
                                                toolchain::Value* base_index) {
  toolchain::Value* block_id =
      EmitCallToTargetIntrinsic(TargetIntrinsicID::kBlockIdx, {}, {}, b_);
  llvm_ir::AddRangeMetadata(0, launch_dimensions_.block_counts().x,
                            static_cast<toolchain::Instruction*>(block_id),
  ...
  llvm_ir::AddRangeMetadata(0, launch_dimensions_.thread_counts_per_block().x,
                            static_cast<toolchain::Instruction*>(thread_id_x),
                            b_->GetInsertBlock()->getModule());
  thread_id_x = b_->CreateZExtOrTrunc(thread_id_x, index_type, "thread_id_x");

  toolchain::Value* linear_index_base =
  ...
  if (launch_dimensions_.thread_counts_per_block().y > 1) {
    ...
    llvm_ir::AddRangeMetadata(0, launch_dimensions_.thread_counts_per_block().y,
                              static_cast<toolchain::Instruction*>(thread_id_y),
                              b_->GetInsertBlock()->getModule());
  ...
  // Add an @toolchain.assume(linear_index < threads_per_block * num_blocks).
  //
  ...
  llvm_ir::EmitCallToIntrinsic(
      ...
      {b_->CreateICmpULT(
          linear_index_base,
          toolchain::ConstantInt::get(index_type,
  ...
  if (launch_config_.unroll_factor > 1) {
    linear_index_base = b_->CreateMul(
        linear_index_base,
        toolchain::ConstantInt::get(index_type, launch_config_.unroll_factor),
...
ParallelLoopEmitter::EmitIndexAndSetExitBasicBlock(absl::string_view loop_name,
  ...
  toolchain::Value* linear_index_base = linear_base_and_thread_idx.linear_base;

  std::vector<toolchain::Value*> multidim(shape_.dimensions().size(), nullptr);
  for (int i = 0; i < launch_config_.unroll_factor; ++i) {
    ...
    toolchain::Value* linear_index =
        b_->CreateAdd(linear_index_base, toolchain::ConstantInt::get(index_type, i),
  ...
  auto if_in_bounds = llvm_ir::EmitIfThenElse(
      b_->CreateICmpULT(
          linear_index_base,
          toolchain::ConstantInt::get(index_type, ShapeUtil::ElementsIn(shape_))),
    ...
    if (!check_bounds) {
      ...
      llvm_ir::LlvmIfData if_in_bounds = llvm_ir::EmitIfThenElse(
          b_->CreateICmpULT(array_index.linear(),
                            toolchain::ConstantInt::get(index_type, num_elements)),
...
absl::Status ParallelLoopEmitter::EmitLoop(absl::string_view loop_name,
  ...
  if (total_threads * launch_config_.unroll_factor >= num_elements) {
    ...
    auto constant = [&](int64_t val) {
      return toolchain::ConstantInt::get(index_type, val);
    };
