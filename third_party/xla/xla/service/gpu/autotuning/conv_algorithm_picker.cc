...
#include "machina/xla/service/gpu/autotuning/gpu_autotuning.pb.h"
...
#include "machina/xla/service/gpu/hlo_algorithm_denylist.h"
...
#include "machina/xla/stream_executor/lazy_op_runner.h"
#include "machina/xla/stream_executor/numeric_options.h"
...
absl::StatusOr<bool> CheckRedzones(const se::RedzoneAllocator& allocator,
  ...
  MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER_LEVEL("CudnnConvAlgorithmPicker checking redzones",
                                 2);
...
absl::StatusOr<AutotuneResult> GpuConvAlgorithmPicker::AutotuneOneConvRunner(
  ...
  *result.mutable_run_time() =
    ...
    if (runtime_arguments.canonical_hlo.has_value()) {
      ...
      *entry->mutable_cudnn_version() = GetCudnnVersion(stream_exec);
      ...
      LOG(ERROR) << "To denylist this algorithm for this convolution, "
                    ...
                    "proto to the denylist file pointed by MACHINA_MACHINA_XLA_FLAGS "
  ...
  if (reference_result->has_value()) {
    MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER_LEVEL("BufferComparator::CompareEqual", 2);
    ...
    MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER_LEVEL("Memcpy Reference Result", 2);
...
GpuConvAlgorithmPicker::PickBestAlgorithmNoCacheCuda(
  ...
  MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER(absl::StrCat(
  ...
  if (!best_algo_or.ok()) {
    ...
    if (strict) {
      return Unknown(
          ...
          "MACHINA_MACHINA_XLA_FLAGS=--xla_gpu_strict_conv_algorithm_picker=false.  Please "
