...
TEST_F(GpuKernelTilingTest, UnnestedTransposeWithProperDimensionsTiled) {
  ...
  // Check that a call to toolchain.nvvm.barrier0 is generated.
  //
...
TEST_F(GpuKernelTilingTest, UnnestedTransposeWithSmallDimensionsNotTiled) {
  ...
  // Check that a call to toolchain.nvvm.barrier0 is not generated.  As in
...
TEST_F(GpuKernelTilingTest, TransposedInputWithUserReverseNotTiled) {
  ...
  // Check that a call to toolchain.nvvm.barrier0 is not generated.
  auto hlo_module =
      ParseAndReturnVerifiedModule(kHloString, ConfigWithoutLayoutAssignment())
          .value();
  auto expected_ir = R"(
...
TEST_F(GpuKernelTilingTest, ColumnReductionWithLayoutChangeTiled) {
  ...
  // Check that the kernel is tiled by looking for toolchain.nvvm.atomic.
  auto hlo_module =
      ParseAndReturnVerifiedModule(kHloString, ConfigWithoutLayoutAssignment())
          .value();
