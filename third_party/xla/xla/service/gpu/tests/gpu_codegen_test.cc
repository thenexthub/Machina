  ...
  return absl::StrReplaceAll(
      ...
      {{"KERNEL_ANNOTATION",
       ...
       {"BARRIER()", is_built_with_rocm_
                         ? "@toolchain.amdgcn.s.barrier()"
                         : "@toolchain.nvvm.barrier.cta.sync.aligned.all(i32 0)"},
       {"SHUFFLE", is_built_with_rocm_ ? "i32 @toolchain.amdgcn.ds.swizzle"
                                       : "float @toolchain.nvvm.shfl.sync.down.f32"},
       {"TIDX", is_built_with_rocm_ ? "@toolchain.amdgcn.workitem.id.x"
                                    : "@toolchain.nvvm.read.ptx.sreg.tid.x"},
       {"LCAL", is_built_with_rocm_ ? "%[[LOGICAL_T1:.*]] = call { i1, i64 } "
                                      "@toolchain.amdgcn.if.i64(i1 %[[LOGICAL_T0]])"
                                    : "0"},
       {"EXTV",
        is_built_with_rocm_
