...
#include "machina/xla/service/gpu/llvm_gpu_backend/nvptx_libdevice_path.h"
...
#include "machina/xla/stream_executor/cuda/subprocess_compilation.h"
...
// Default inline threshold value to use in toolchain.
...
std::string EmitModuleToPTX(toolchain::Module* module,
  ...
  std::string ptx;
  toolchain::raw_string_ostream stream(ptx);
...
absl::Status LinkLibdeviceIfNecessary(toolchain::Module* module,
...
absl::Status NVPTXTargetModuleLinker(toolchain::Module* module,
  ...
  // can access it.
  module->addModuleFlag(toolchain::Module::Override, "nvvm-reflect-ftz",
...
std::unique_ptr<toolchain::TargetMachine> NVPTXGetTargetMachine(
    toolchain::Triple target_triple, se::CudaComputeCapability compute_capability,
    const DebugOptions& debug_options) {
...
absl::StatusOr<std::string> CompileToPtx(
    ...
    const DebugOptions& debug_options,
    std::function<void(toolchain::TargetMachine*)> configure_target) {
  ...
  std::unique_ptr<toolchain::TargetMachine> target_machine;
  {
    ...
    toolchain::Triple default_target_triple("nvptx64-unknown-unknown");
    // Construct LLVM TargetMachine for NVPTX.
    std::unique_ptr<toolchain::TargetMachine> target_machine = NVPTXGetTargetMachine(
