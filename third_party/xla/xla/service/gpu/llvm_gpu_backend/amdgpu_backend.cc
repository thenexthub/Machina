...
absl::StatusOr<std::vector<uint8_t>> EmitModuleToHsaco(
    toolchain::Module* module, toolchain::TargetMachine* target_machine,
  ...
  TF_CHECK_OK(tsl::ReadBoolFromEnvVar("TF_ROCM_KEEP_MACHINA_MACHINA_XLA_TEMPFILES",
  ...
  // Dump LLVM IR.
  std::unique_ptr<toolchain::raw_fd_ostream> ir_fs(
      new toolchain::raw_fd_ostream(ir_path, ec, toolchain::sys::fs::OF_None));
  module->print(*ir_fs, nullptr);
  ir_fs->flush();

  // Emit GCN ISA binary.
  toolchain::legacy::PassManager pm;
  pm.add(new toolchain::TargetLibraryInfoWrapperPass(
      toolchain::Triple(module->getTargetTriple())));
  toolchain::SmallVector<char, 0> stream;
  toolchain::raw_svector_ostream pstream(stream);
  std::unique_ptr<toolchain::raw_fd_ostream> isabin_fs(
      new toolchain::raw_fd_ostream(isabin_path, ec, toolchain::sys::fs::OF_Text));
  ...
  if (keep_tempfiles) {
    std::unique_ptr<toolchain::raw_fd_ostream> ir_fs(
        new toolchain::raw_fd_ostream(ir_opt_path, ec, toolchain::sys::fs::OF_None));
    module->print(*ir_fs, nullptr);
    ir_fs->flush();
  }
  ...
  if (debug_options.xla_gpu_use_inprocess_lld()) {
    ...
    lld::Result result;
    {
      ...
      result =
          lld::lldMain(args, toolchain::nulls(), os, {{lld::Gnu, &lld::elf::link}});
    }
    ...
    if (std::getenv("LLVM_PATH")) {
      ...
      lld_path = tsl::io::JoinPath(tsl::RocmRoot(), "toolchain/bin");
    }
    auto lld_program = toolchain::sys::findProgramByName("ld.lld", {lld_path});
    if (!lld_program) {
      return xla::Internal("unable to find ld.lld in PATH: %s",
                           lld_program.getError().message());
    }
    std::vector<toolchain::StringRef> lld_args{
    ...
    std::string error_message;
    int lld_result =
        toolchain::sys::ExecuteAndWait(*lld_program, llvm_ir::AsArrayRef(lld_args),
...
absl::Status LinkROCDLIfNecessary(toolchain::Module* module,
...
absl::Status AMDGPUTargetModuleLinker(
  ...
  if (debug_options.xla_gpu_ftz()) {
    for (toolchain::Function& fn : *module) {
      fn.addFnAttr("denormal-fp-math-f32", "preserve-sign");
    }
  }
  const int32_t kAbiVersion = 500;
  module->addModuleFlag(toolchain::Module::Error, "amdhsa_code_object_version",
                        kAbiVersion);
...
std::unique_ptr<toolchain::TargetMachine> AMDGPUGetTargetMachine(
    toolchain::Triple target_triple, se::GpuComputeCapability gpu_version,
    const DebugOptions& debug_options) {
  auto compute_capability =
      std::get_if<se::RocmComputeCapability>(&gpu_version);
...
void AMDGPUBackendInit(const DebugOptions& debug_options,
  ...
  toolchain::PassRegistry* registry = toolchain::PassRegistry::getPassRegistry();
...
absl::StatusOr<std::vector<uint8_t>> CompileToHsaco(
  ...
  str += module_config_cache_key;
  {
    ...
    MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER("Compile module " + module->getName().str());
    ...
    toolchain::Triple default_target_triple("amdgcn--amdhsa-amdgiz");
