...
#include "machina/xla/backends/gpu/codegen/fusions.h"
...
#include "machina/xla/backends/gpu/runtime/all_gather_thunk.h"
#include "machina/xla/backends/gpu/runtime/all_reduce_thunk.h"
#include "machina/xla/backends/gpu/runtime/all_to_all_thunk.h"
#include "machina/xla/backends/gpu/runtime/cholesky_thunk.h"
#include "machina/xla/backends/gpu/runtime/collective_broadcast_thunk.h"
#include "machina/xla/backends/gpu/runtime/collective_group_thunk.h"
#include "machina/xla/backends/gpu/runtime/collective_permute_thunk.h"
#include "machina/xla/backends/gpu/runtime/collective_thunk.h"
#include "machina/xla/backends/gpu/runtime/command_buffer_cmd.h"
#include "machina/xla/backends/gpu/runtime/command_buffer_cmd_emitter.h"
#include "machina/xla/backends/gpu/runtime/command_buffer_thunk.h"
#include "machina/xla/backends/gpu/runtime/conditional_thunk.h"
#include "machina/xla/backends/gpu/runtime/convolution_thunk.h"
#include "machina/xla/backends/gpu/runtime/copy_thunk.h"
#include "machina/xla/backends/gpu/runtime/cub_sort_thunk.h"
#include "machina/xla/backends/gpu/runtime/cudnn_thunk.h"
#include "machina/xla/backends/gpu/runtime/custom_call_target.h"
#include "machina/xla/backends/gpu/runtime/custom_call_thunk.h"
#include "machina/xla/backends/gpu/runtime/fft_thunk.h"
#include "machina/xla/backends/gpu/runtime/gemm_thunk.h"
#include "machina/xla/backends/gpu/runtime/gpublas_lt_matmul_thunk.h"
#include "machina/xla/backends/gpu/runtime/host_send_recv_thunk.h"
#include "machina/xla/backends/gpu/runtime/infeed_thunk.h"
#include "machina/xla/backends/gpu/runtime/kernel_thunk.h"
#include "machina/xla/backends/gpu/runtime/norm_thunk.h"
#include "machina/xla/backends/gpu/runtime/nvshmem_all_reduce_thunk.h"
#include "machina/xla/backends/gpu/runtime/nvshmem_collective_permute_thunk.h"
#include "machina/xla/backends/gpu/runtime/nvshmem_collective_thunk.h"
#include "machina/xla/backends/gpu/runtime/nvshmem_recv_thunk.h"
#include "machina/xla/backends/gpu/runtime/nvshmem_send_thunk.h"
#include "machina/xla/backends/gpu/runtime/outfeed_thunk.h"
#include "machina/xla/backends/gpu/runtime/p2p_thunk_common.h"
#include "machina/xla/backends/gpu/runtime/ragged_all_to_all_thunk.h"
#include "machina/xla/backends/gpu/runtime/recv_thunk.h"
#include "machina/xla/backends/gpu/runtime/replica_id_thunk.h"
#include "machina/xla/backends/gpu/runtime/send_thunk.h"
#include "machina/xla/backends/gpu/runtime/sequential_thunk.h"
#include "machina/xla/backends/gpu/runtime/thunk.h"
#include "machina/xla/backends/gpu/runtime/topk.h"
#include "machina/xla/backends/gpu/runtime/triangular_solve_thunk.h"
#include "machina/xla/backends/gpu/runtime/wait_for_streams_thunk.h"
#include "machina/xla/backends/gpu/runtime/while_thunk.h"
#include "machina/xla/codegen/emitters/kernel_arguments.h"
#include "machina/xla/ffi/api/c_api.h"
#include "machina/xla/ffi/attribute_map.h"
...
#include "machina/xla/mlir_hlo/transforms/gpu_passes.h"
...
#include "machina/xla/service/custom_call_status.h"
...
#include "machina/xla/service/gpu/gpu_conv_runner.h"
#include "machina/xla/service/gpu/gpu_norm_runner.h"
#include "machina/xla/service/gpu/hlo_fusion_analysis.h"
...
#include "machina/xla/service/gpu/kernels/custom_kernel.h"
...
toolchain::Value* IrEmitterUnnested::CreateLoad(toolchain::Value* address,
                                           toolchain::Type* data_type,
                                           int alignment_bytes) {
  ...
  toolchain::Value* output = toolchain::ConstantInt::get(data_type, 0);
  for (int offset_bytes = 0; offset_bytes < data_bytes;
       offset_bytes += alignment_bytes) {
    toolchain::Value* offset_address = b_.CreateConstInBoundsGEP1_32(
        b_.getInt8Ty(), address, offset_bytes, "offset_address");
    toolchain::Value* partial_value = b_.CreateLoad(b_.getIntNTy(alignment_bitwidth),
                                               offset_address, "partial_value");
    toolchain::Value* zextd =
        b_.CreateZExt(partial_value, output->getType(), "partial_value_zextd");
    toolchain::Value* shifted = b_.CreateShl(
        zextd, toolchain::ConstantInt::get(b_.getInt32Ty(), offset_bytes),
...
void IrEmitterUnnested::CreateStore(toolchain::Value* data, toolchain::Value* address,
                                    int alignment_bytes) {
  ...
  for (int offset_bytes = 0; offset_bytes < data_bytes;
    ...
    toolchain::Value* offset_address = b_.CreateConstInBoundsGEP1_32(
        b_.getInt8Ty(), address, offset_bytes, "offset_address");
    toolchain::Value* shifted_partial = b_.CreateTrunc(
        b_.CreateLShr(data,
                      toolchain::ConstantInt::get(b_.getInt32Ty(), offset_bytes)),
...
absl::Status IrEmitterUnnested::EmitPadToStatic(
  ...
  for (int64_t i = 1; i < output_shapes.size(); ++i) {
    ...
    toolchain::Value* metadata = b_.CreateConstInBoundsGEP1_32(
  ...
  toolchain::Value* dyn_element_total = toolchain::ConstantInt::get(index_ty, 1);
  for (toolchain::Value* dynamic_dim : dynamic_dims) {
    dyn_element_total =
...
absl::Status IrEmitterUnnested::EmitSliceToDynamic(
  ...
  toolchain::Value* dest_buffer = data_array.GetBasePointer();

  // Load dynamic dimensions from memory.
  std::vector<toolchain::Value*> dynamic_dims;
  int alignment = raw_data_size % sizeof(int32_t);
  for (int64_t i = 1; i < instr->operand_count(); ++i) {
    toolchain::Value* source_buffer = ir_arrays[i].GetBasePointer();
    toolchain::Type* source_buffer_pointee_type = ir_arrays[i].GetBasePointeeType();
    toolchain::LoadInst* dyn_dim_size =
...
absl::Status IrEmitterUnnested::EmitTritonCustomCall(
  ...
  auto generate = [this, &instr]() -> absl::StatusOr<KernelReuseCache::Entry> {
    ...
    if (emit_kernels) {
      ...
      toolchain::IRBuilder builder(ir_emitter_context_->llvm_module()->getContext());

      TF_ASSIGN_OR_RETURN(toolchain::Function * kernel,
      ...
      for (auto& arg : prototype_func->args()) {
        ...
        // combination.
        arg.removeAttr(toolchain::Attribute::Alignment);
        arg.removeAttr(toolchain::Attribute::NoAlias);
      }
    }
...
absl::Status IrEmitterUnnested::EmitRngGetAndUpdateState(
  ...
  toolchain::Value* old_state =
      llvm_ir::RngGetAndUpdateState(instr->delta(), module_, &b_);
  toolchain::Value* output_address = ir_arrays[0].EmitArrayElementAddress(
...
absl::Status IrEmitterUnnested::EmitSort(const HloSortInstruction* sort) {
  ...
  auto emit_kernel = [&](absl::Span<const int64_t> xor_masks) {
    ...
    return llvm_ir::EmitSortInPlace(
        ...
        [&](absl::Span<toolchain::Value* const> operands, toolchain::Value* output) {
...
IrEmitterUnnested::BuildKernelThunkForNonFusionOp(
  ...
  for (const auto& [kernel_argument, llvm_arg] :
       toolchain::zip(kernel_arguments.args(), kernel->args())) {
    toolchain::Type* ir_type =
