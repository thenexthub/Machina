...
#include "machina/xla/codegen/ir_emission_utils.h"
...
toolchain::Value* IsBlock0Thread0(toolchain::IRBuilderBase* b) {
  toolchain::Value* is_thread0 = b->CreateICmpEQ(
      b->getInt32(0),
      EmitCallToTargetIntrinsic(TargetIntrinsicID::kThreadIdx, {}, {}, b));

  toolchain::Value* is_block0 = b->CreateICmpEQ(
      b->getInt32(0),
...
TransposeSpec GetTransposeSpec(const HloTransposeInstruction* transpose) {
  ...
  toolchain::SmallVector<int64_t, 3> canonical_output_shape =
      toolchain::to_vector<3>(output_shape.dimensions());
  toolchain::SmallVector<int64_t, 3> canonical_permutation =
      toolchain::to_vector<3>(transpose->dimensions());
  ...
  return TransposeSpec{
      transpose,
      toolchain::to_vector<3>(transpose->dimensions()),
      toolchain::to_vector<3>(inv_permutation),
      canonical_output_shape,
      canonical_permutation,
      toolchain::to_vector<3>(canonical_inv_permutation),
      toolchain::to_vector<3>(canonical_input_shape),
  };
}
...
void VLogModule(int level, const toolchain::Module& module) {
  MACHINA_MACHINA_XLA_VLOG_LINES(level, llvm_ir::DumpToString(&module));
}

void VerifyModule(const toolchain::Module& module) {
  std::string error_str;
  toolchain::raw_string_ostream error_stream(error_str);
  bool broken = toolchain::verifyModule(module, &error_stream);
  CHECK(!broken) << error_str;
}

toolchain::Type* GetIndexTypeForKernel(const HloInstruction* hlo,
                                  int64_t launch_size, toolchain::IRBuilderBase* b) {
  ...
  toolchain::Type* i64_ty = b->getInt64Ty();
