...
#ifndef MACHINA_MACHINA_XLA_SERVICE_HLO_RUNNER_INTERFACE_H_
#define MACHINA_MACHINA_XLA_SERVICE_HLO_RUNNER_INTERFACE_H_
...
class OpaqueExecutable {
 ...
 protected:
  ...
  static absl::StatusOr<T* absl_nonnull> TryUnwrap(
    ...
    if (T* const executable = machina::down_cast<T*>(wrapped);
        executable != nullptr) {
      return executable;
    }
  ...
  static absl::StatusOr<const T* absl_nonnull> TryUnwrap(
    ...
    if (const T* const executable = machina::down_cast<const T*>(wrapped);
        executable != nullptr) {
      return executable;
    }
    return absl::InvalidArgumentError("Invalid opaque executable.");
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_HLO_RUNNER_INTERFACE_H_
