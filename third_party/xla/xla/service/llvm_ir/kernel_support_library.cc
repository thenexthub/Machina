...
absl::Status KernelSupportLibrary::ForWithStatus(
    absl::string_view name, toolchain::Value* start, toolchain::Value* end,
    toolchain::Value* step,
    const std::function<absl::Status(toolchain::Value*, bool)>& for_body_generator) {
  return IfWithStatus(b_->CreateICmpSLT(start, end), [&]() -> absl::Status {
    ...
    return ForWithStatus(
        ...
        [&](toolchain::Value* iv) { return for_body_generator(iv, false); });
  });
}
...
absl::Status KernelSupportLibrary::ForWithStatus(
    absl::string_view name, toolchain::Value* start, toolchain::Value* end,
    toolchain::Value* step,
    const std::function<absl::Status(toolchain::Value*)>& for_body_generator) {
...
absl::Status KernelSupportLibrary::IfWithStatus(
    absl::string_view name, toolchain::Value* condition,
...
void KernelSupportLibrary::EmitAndCallOutlinedKernel(
    const HloModuleConfig& module_config, toolchain::IRBuilderBase* b,
  ...
  std::vector<toolchain::Value*> sanitized_args;
  ...
  if (!function) {
    ...
    std::vector<toolchain::Type*> arg_types;
    std::transform(sanitized_args.begin(), sanitized_args.end(),
                   std::back_inserter(arg_types),
                   [](toolchain::Value* arg) { return arg->getType(); });

    auto* function_type =
        toolchain::FunctionType::get(b->getVoidTy(), arg_types, /*isVarArg=*/false);
    ...
    auto* entry_bb =
        toolchain::BasicBlock::Create(b->getContext(), "entry", function);
    auto* return_inst = toolchain::ReturnInst::Create(b->getContext(),
                                                 /*retVal=*/nullptr, entry_bb);
    // Set the insert point to before return_inst.
    b->SetInsertPoint(return_inst);

    std::vector<toolchain::Value*> arg_values;
    /*
     * clang on OSX doesn't like std::transform or range for loop here.
     * See https://github.com/machina/machina/issues/15196
     */
    for (toolchain::Function::arg_iterator arg = function->arg_begin(),
