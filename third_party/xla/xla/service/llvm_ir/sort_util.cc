...
absl::Status EmitCompareLoopBody(
    ...
    int64_t num_values, toolchain::Value* element_pair_index, int64_t xor_mask,
    toolchain::Type* index_type,
    std::function<toolchain::Value*(int64_t operand, toolchain::Value* index)>
        element_address,
    std::function<toolchain::Type*(int64_t operand, toolchain::Value* index)>
        element_address_pointee_type,
    std::function<void(int64_t operand, toolchain::Value* index, toolchain::Value* value)>
        write_element,
    const EmitCallToNestedComputationCallback& emit_compare_callback,
    toolchain::IRBuilderBase* b, bool needs_bounds_checks = true) {
  auto index_typed_constant = [&](int64_t value) {
    return toolchain::ConstantInt::get(index_type, value);
  };
  ...
  for (int64_t i = 0; i < unroll_factor; ++i) {
    toolchain::Value* current_keys_index;
    ...
    if (block_size == 1) {
      ...
      toolchain::Value* is_within_left_block = b->CreateICmpSLT(
    ...
    toolchain::Value* index_is_inbounds =
        needs_bounds_checks
    ...
    TF_RETURN_IF_ERROR(
        ksl.IfWithStatus("smaller_comparison_index", index_is_inbounds, [&]() {
          std::vector<toolchain::Value*> values_to_compare;
          std::vector<toolchain::Type*> values_to_compare_types;
          ...
          toolchain::Type* pred_type =
              llvm_ir::PrimitiveTypeToIrType(PRED, b->getContext());
          toolchain::Value* compare_return_buffer =
          ...
          toolchain::Value* result = b->CreateLoad(pred_type, compare_return_buffer);

          // Check if the 'compare' function returns true.
          toolchain::Value* is_smaller_than = b->CreateICmpNE(
              result, toolchain::ConstantInt::get(result->getType(), 0),
              "boolean_predicate");
...
absl::Status EmitTiledCompareLoop(
    ...
    const std::vector<toolchain::GlobalVariable*>& param_shmem_buffers,
    int64_t tile_size, int64_t unroll_factor,
    const EmitCallToNestedComputationCallback& emit_compare_callback,
    toolchain::IRBuilderBase* b) {
  KernelSupportLibrary ksl(b);
  toolchain::Value* thread_id = gpu::EmitCallToTargetIntrinsic(
      gpu::TargetIntrinsicID::kThreadIdx, {}, {}, b);
  llvm_ir::AddRangeMetadata(0, num_threads,
                            toolchain::cast<toolchain::Instruction>(thread_id),
                            b->GetInsertBlock()->getModule());
  thread_id = b->CreateIntCast(thread_id, tiled_keys_index.GetType(),
                               /*isSigned=*/true, "thread.id.x");

  auto copy_loop_body =
      [&](std::function<void(toolchain::Value * cache_index, toolchain::Value * index)>
              read_or_write) {
  ...
  std::vector<toolchain::Value*> keys_multi_index = tiled_keys_index.multidim();
  for (int64_t i = 0; i < params.size(); ++i) {
    copy_loop_body([&](toolchain::Value* cache_index, toolchain::Value* index) {
  ...
  auto element_address = [&](int64_t operand, toolchain::Value* index) {
    ...
    return b->CreateAddrSpaceCast(
        ...
        toolchain::PointerType::get(
            toolchain::cast<toolchain::PointerType>(ptr_type)->getContext(),
            /*AddressSpace=*/0));
  };
  auto element_address_pointee_type = [&](int64_t operand, toolchain::Value* index) {
    return toolchain::GetElementPtrInst::getIndexedType(
        param_shmem_buffers[operand]->getValueType(),
        {tiled_keys_index.GetConstantWithIndexType(0), index});
  };
  auto write_element = [&](int64_t operand, toolchain::Value* index,
  ...
  for (int64_t i = 0; i < params.size(); ++i) {
    copy_loop_body([&](toolchain::Value* cache_index, toolchain::Value* index) {
      ...
      auto gep = b->CreateGEP(
          param_shmem_buffers[i]->getValueType(), param_shmem_buffers[i],
          {tiled_keys_index.GetConstantWithIndexType(0), cache_index});
      auto gep_type = toolchain::GetElementPtrInst::getIndexedType(
...
absl::Status EmitSortInPlace(
    ...
    toolchain::IRBuilderBase* b, const gpu::LaunchDimensions& launch_dimensions,
  ...
  std::vector<toolchain::GlobalVariable*> param_shmem_buffers(values_arrays.size(),
                                                         nullptr);
  if (xor_masks.size() > 1) {
    toolchain::Module* module = b->GetInsertBlock()->getParent()->getParent();
    for (int64_t i = 0; i < values_arrays.size(); ++i) {
      toolchain::Type* tile_type = toolchain::ArrayType::get(
  ...
  auto compare_loop_body_emitter =
    ...
    std::vector<toolchain::Value*> keys_multi_index(rank);
    ...
    if (xor_masks.size() > 1) {
      ...
      auto element_address_pointee_type = [&](int64_t operand, toolchain::Value*) {
