...
#include "machina/xla/permutation_util.h"
...
IrArray::Index::Index(absl::Span<toolchain::Value* const> multidim,
                      toolchain::Value* linear, const Shape& shape,
...
void IrArray::Index::Delinearize(std::vector<toolchain::Value*>* multidim,
                                 toolchain::Value* linear, const Shape& shape,
...
void IrArray::Index::Delinearize(std::vector<toolchain::Value*>* multidim,
  ...
  for (int64_t i = 0; i < layout.minor_to_major().size(); ++i) {
    ...
    if (i < layout.minor_to_major().size() - 1) {
      toolchain::Value* casted_dynamic_dim =
...
IrArray::Index::Index(toolchain::Value* linear, const Shape& shape,
...
IrArray::Index::Index(toolchain::Value* linear,
                      absl::Span<toolchain::Value* const> multidim,
...
IrArray::IrArray(toolchain::Value* base_ptr, toolchain::Type* pointee_type, Shape shape)
    : base_ptr_(base_ptr),
  ...
  while (toolchain::ArrayType* array_type =
             toolchain::dyn_cast<toolchain::ArrayType>(element_type_)) {
...
IrArray::Index IrArray::Index::SourceIndexOfReshape(
  ...
  std::vector<toolchain::Value*> source_multidim_index(
      input_shape.dimensions().size(), toolchain::UndefValue::get(index_type_));
  ...
  if (std::optional<ShapeUtil::ShapeEqualityDescriptor> trivial_reshape =
    ...
    for (ssize_t k = common_factors.size() - 2; k >= 0; --k) {
      ...
      toolchain::Value* logical_linear_index =
          Index(absl::Span<toolchain::Value* const>(multidim_).subspan(
                    common_factors[k].second,
                    common_factors[k + 1].second - common_factors[k].second),
...
IrArray::Index IrArray::Index::SourceIndexOfSlice(
    ...
    absl::Span<const int64_t> strides, toolchain::IRBuilderBase* builder) const {
  std::vector<toolchain::Value*> source_multi_index(multidim_.size());
...
IrArray::Index IrArray::Index::SourceIndexOfTranspose(
  ...
  auto operand_multidim_index =
      PermuteInverse<std::vector<toolchain::Value*>>(multidim(), dimension_mapping);
...
IrArray::Index IrArray::Index::SourceIndexOfBitcast(
    const Shape& operand_shape, toolchain::IRBuilderBase* builder) const {
...
IrArray::Index IrArray::Index::SourceIndexOfBroadcast(
  ...
  std::vector<toolchain::Value*> source_index(rank);
  ...
  toolchain::Value* linear = linear_;
  int64_t divisor = 1;
...
toolchain::Value* IrArray::Index::Linearize(absl::Span<const int64_t> dimensions,
                                       toolchain::IRBuilderBase* builder) const {
  ...
  toolchain::Value* logical_linear_index = GetConstantWithIndexType(0);
  int64_t multiplier = 1;
  for (ssize_t i = 0; i < size(); ++i) {
    int64_t dimension = layout_.minor_to_major(i);
    toolchain::Value* addend = builder->CreateMul(
...
toolchain::Value* IrArray::Index::Linearize(
    const std::vector<toolchain::Value*>& dynamic_dims,
    toolchain::IRBuilderBase* builder) const {
  // Each dimension is multiplied by the product of the sizes of all
  // earlier dimensions and added to the accumulator logical_linear_index.
  CHECK_EQ(size(), dynamic_dims.size());
  toolchain::Value* logical_linear_index = GetConstantWithIndexType(0);
  toolchain::Value* multiplier = GetConstantWithIndexType(1);
  for (ssize_t i = 0; i < size(); ++i) {
    int64_t dimension = layout_.minor_to_major(i);
    toolchain::Value* addend = builder->CreateMul((*this)[dimension], multiplier, "",
                                             /*HasNUW=*/true, /*HasNSW=*/true);
    addend = builder->CreateZExtOrTrunc(addend, index_type_);
    logical_linear_index = builder->CreateAdd(logical_linear_index, addend, "",
                                              /*HasNUW=*/true, /*HasNSW=*/true);
...
toolchain::Value* IrArray::EmitArrayElementAddress(const IrArray::Index& index,
                                              toolchain::IRBuilderBase* b,
                                              absl::string_view name,
                                              bool use_linear_index,
                                              toolchain::Value** bit_offset) const {
  ...
  for (int64_t i = 0; i < index.size(); ++i) {
    ...
    actual_index.push_back(
        dim == 1 ? toolchain::ConstantInt::get(index[i]->getType(), 0) : index[i]);
  }
  ...
  std::vector<toolchain::Value*> gep_indices(
      1, toolchain::ConstantInt::get(index[0]->getType(), 0));
  for (int64_t i = 0; i < shape_.dimensions().size(); ++i) {
...
toolchain::Value* IrArray::EmitLinearArrayElementAddress(
    const IrArray::Index& index, toolchain::IRBuilderBase* b, absl::string_view name,
    toolchain::Value** bit_offset) const {
  CHECK(index.LinearValidOnShape(shape_));
  toolchain::Type* type =
      PrimitiveTypeToIrType(shape_.element_type(), b->getContext());
  if (!primitive_util::IsSubByteNonPredType(shape_.element_type())) {
    auto linear_index = toolchain::dyn_cast<toolchain::BinaryOperator>(index.linear());
    if (linear_index && (linear_index->getOpcode() == toolchain::Instruction::Add)) {
      toolchain::Value* index_operand_0 = linear_index->getOperand(0);
      toolchain::Value* index_operand_1 = linear_index->getOperand(1);
      toolchain::Value* ptr_address =
  ...
  toolchain::Type* index_type = index.linear()->getType();
  auto bit_width = primitive_util::BitWidth(shape_.element_type());
  toolchain::Value* elements_per_byte =
      toolchain::ConstantInt::get(index_type, 8 / bit_width);
  toolchain::Value* remainder = b->CreateURem(index.linear(), elements_per_byte);
  toolchain::Value* byte_offset = b->CreateUDiv(index.linear(), elements_per_byte);

  CHECK_NE(bit_offset, nullptr);
  *bit_offset = b->CreateIntCast(
      b->CreateMul(remainder, toolchain::ConstantInt::get(index_type, bit_width)),
...
void IrArray::AnnotateLoadStoreInstructionWithMetadata(
  ...
  CHECK(toolchain::isa<toolchain::LoadInst>(instruction) ||
        toolchain::isa<toolchain::StoreInst>(instruction));
  CHECK(!toolchain::isa<toolchain::StoreInst>(instruction) || !is_invariant_)
...
toolchain::Value* IrArray::EmitReadArrayElement(const Index& index,
                                           toolchain::IRBuilderBase* b,
                                           absl::string_view name,
                                           bool use_linear_index) const {
  toolchain::Value* bit_offset = nullptr;
  toolchain::Value* element_address =
      EmitArrayElementAddress(index, b, name, use_linear_index, &bit_offset);
  toolchain::Type* load_type =
  ...
  toolchain::Value* elem = load;
  if (primitive_util::IsSubByteNonPredType(shape_.element_type())) {
    toolchain::Value* shifted = b->CreateLShr(load, bit_offset);
    elem = b->CreateTrunc(
...
void IrArray::EmitWriteArrayElement(const Index& index, toolchain::Value* value,
                                    toolchain::IRBuilderBase* b,
                                    bool use_linear_index) const {
  toolchain::Value* bit_offset = nullptr;
  toolchain::Value* element_address =
  ...
  if (primitive_util::IsSubByteNonPredType(shape_.element_type())) {
    ...
    // and write it back.
    toolchain::LoadInst* load = b->CreateLoad(b->getInt8Ty(), element_address);
    AnnotateLoadStoreInstructionWithMetadata(load);
    ...
    toolchain::Value* mask = b->getInt8(~LsbMask<uint8_t>(bit_width));
    mask = b->CreateIntrinsic(b->getInt8Ty(), toolchain::Intrinsic::fshl,
                              {mask, mask, bit_offset});
    toolchain::Value* masked_load = b->CreateAnd(load, mask);
    value = b->CreateOr(masked_load, value);
  }
  toolchain::StoreInst* store = b->CreateStore(value, element_address);
...
IrArray IrArray::CastToShape(const Shape& new_shape,
  ...
  toolchain::Type* new_ir_type = llvm_ir::ShapeToIrType(new_shape, b->getContext());
