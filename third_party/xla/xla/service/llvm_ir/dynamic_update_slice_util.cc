...
using IndexGenerator = std::function<absl::StatusOr<toolchain::Value*>(int64_t)>;
...
static absl::Status EmitDynamicUpdateSliceInPlaceImpl(
    ...
    absl::string_view name, toolchain::IRBuilderBase* b) {
  ...
  std::vector<toolchain::Value*> start_multi_index(rank);
  for (int64_t i = 0; i < rank; ++i) {
    TF_ASSIGN_OR_RETURN(start_multi_index[i], start_indices_generator(i));
    toolchain::Value* output_dim_size = toolchain::ConstantInt::get(
        start_multi_index[i]->getType(), output_shape.dimensions(i));
    toolchain::Value* update_dim_size = toolchain::ConstantInt::get(
    ...
    toolchain::Value* max_bound = b->CreateSub(output_dim_size, update_dim_size);
    toolchain::Value* zero =
        toolchain::ConstantInt::get(start_multi_index[i]->getType(), 0);
    start_multi_index[i] =
        b->CreateSelect(b->CreateICmp(is_signed ? toolchain::ICmpInst::ICMP_SGE
                                                : toolchain::ICmpInst::ICMP_UGE,
                                      zero, start_multi_index[i]),
                        zero, start_multi_index[i]);

    start_multi_index[i] =
        b->CreateSelect(b->CreateICmp(is_signed ? toolchain::ICmpInst::ICMP_SLE
  ...
  auto loop_body_emitter =
    ...
    //
    std::vector<toolchain::Value*> output_multi_index(rank);
    for (int64_t i = 0; i < rank; ++i) {
      toolchain::Value* start_index0 = b->CreateIntCast(
    ...
    IrArray::Index output_index(output_multi_index, output_shape,
                                b->getInt64Ty());
    TF_ASSIGN_OR_RETURN(toolchain::Value * update_data,
...
static absl::Status EmitFusedDynamicUpdateSliceInPlaceImpl(
    ...
    const gpu::LaunchDimensions* launch_dimensions, toolchain::IRBuilderBase* b) {
  ...
  for (auto& dus_output_pair : dus_and_output_array) {
    ...
    IndexGenerator start_indices_generator =
        [&](int64_t index) -> absl::StatusOr<toolchain::Value*> {
...
absl::Status EmitFusedDynamicUpdateSliceInPlace(
    ...
    FusedIrEmitter* fused_emitter, toolchain::IRBuilderBase* b) {
