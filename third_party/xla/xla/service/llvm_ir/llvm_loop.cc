...
ForLoop::ForLoop(absl::string_view prefix, absl::string_view suffix,
                 toolchain::Value* start_index, toolchain::Value* end_index,
                 toolchain::Value* step, UnrollMode unroll_mode,
...
/* static */ std::unique_ptr<ForLoop> ForLoop::EmitForLoop(
    absl::string_view prefix, toolchain::Value* start_index, toolchain::Value* end_index,
    toolchain::Value* step, toolchain::IRBuilderBase* b, UnrollMode unroll_mode,
    bool prevent_vectorization) {
...
void ForLoop::Emit(toolchain::IRBuilderBase* b) {
  // The preheader block is the block the builder is currently emitting
  // code into.
  preheader_bb_ = b->GetInsertBlock();

  toolchain::BasicBlock::iterator insert_point = b->GetInsertPoint();
  ...
  toolchain::Function* func = preheader_bb_->getParent();
  b->SetInsertPoint(&func->getEntryBlock(),
                    func->getEntryBlock().getFirstInsertionPt());
  toolchain::Value* indvar_address = b->CreateAlloca(
  ...
  toolchain::Value* exit_cond = b->CreateICmpUGE(indvar_, end_index_);
  ...
  toolchain::Value* step = step_;
  toolchain::Value* indvar = indvar_;

  toolchain::Value* indvar_inc = b->CreateAdd(indvar, step, "invar.inc",
                                         /*HasNUW=*/true, /*HasNSW=*/true);
  b->CreateStore(indvar_inc, indvar_address);
  toolchain::BranchInst* back_branch = b->CreateBr(header_bb_);

  std::vector<toolchain::Metadata*> loop_metadata = GetLoopMetadata(b);
  if (!loop_metadata.empty()) {
    toolchain::LLVMContext* ctx = &start_index_->getContext();
    auto temp_node = toolchain::MDNode::getTemporary(*ctx, {});
    loop_metadata.insert(loop_metadata.begin(), temp_node.get());
    auto loop_id = toolchain::MDNode::get(*ctx, loop_metadata);
    loop_id->replaceOperandWith(0, loop_id);
    back_branch->setMetadata(toolchain::LLVMContext::MD_loop, loop_id);
  }
...
std::vector<toolchain::Metadata*> ForLoop::GetLoopMetadata(toolchain::IRBuilderBase* b) {
  const char* const kLlvmLoopUnrollDisableMDName = "toolchain.loop.unroll.disable";
  const char* const kLlvmLoopUnrollFullMDName = "toolchain.loop.unroll.full";
  const char* const kLlvmLoopVectorizeMDName = "toolchain.loop.vectorize.enable";
  toolchain::LLVMContext* ctx = &start_index_->getContext();

  std::vector<toolchain::Metadata*> result;
  if (unroll_mode_ == xla::llvm_ir::UnrollMode::kNoUnroll) {
    result.push_back(toolchain::MDNode::get(
        *ctx, {toolchain::MDString::get(*ctx, kLlvmLoopUnrollDisableMDName)}));
  }

  if (prevent_vectorization_) {
    result.push_back(toolchain::MDNode::get(
        *ctx, {toolchain::MDString::get(*ctx, kLlvmLoopVectorizeMDName),
               toolchain::ConstantAsMetadata::get(b->getFalse())}));
  }

  if (unroll_mode_ == xla::llvm_ir::UnrollMode::kFullyUnroll) {
    result.push_back(toolchain::MDNode::get(
        *ctx, {toolchain::MDString::get(*ctx, kLlvmLoopUnrollFullMDName)}));
  }
  return result;
}
...
toolchain::BasicBlock* ForLoop::CreateLoopBB(absl::string_view name,
...
std::unique_ptr<ForLoop> ForLoopNest::AddLoop(absl::string_view suffix,
                                              toolchain::Value* start_index,
                                              toolchain::Value* end_index,
...
std::unique_ptr<ForLoop> ForLoopNest::AddLoop(
    absl::string_view suffix, toolchain::Value* start_index, toolchain::Value* end_index,
    toolchain::Value* stride, UnrollMode unroll_mode, bool prevent_vectorization) {
...
std::vector<toolchain::Value*> ForLoopNest::AddLoopsForShapeOnDimensions(
    const Shape& shape, absl::Span<const int64_t> dimensions,
    absl::string_view suffix) {
  std::vector<toolchain::Value*> multi_index(shape.dimensions().size());
...
std::vector<toolchain::Value*> ForLoopNest::EmitOperandArrayLoopNest(
  ...
  // output.
  std::vector<toolchain::Value*> multi_index =
