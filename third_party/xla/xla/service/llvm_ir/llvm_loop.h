...
#ifndef MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LLVM_LOOP_H_
#define MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LLVM_LOOP_H_
...
class ForLoop {
 public:
  ...
  static std::unique_ptr<ForLoop> EmitForLoop(
      absl::string_view prefix, toolchain::Value* start_index,
      toolchain::Value* end_index, toolchain::Value* step, toolchain::IRBuilderBase* b,
  ...
  toolchain::BasicBlock* GetPreheaderBasicBlock() const { return preheader_bb_; }
  toolchain::BasicBlock* GetHeaderBasicBlock() const { return header_bb_; }
  toolchain::BasicBlock* GetBodyBasicBlock() const { return body_bb_; }
  toolchain::BasicBlock* GetExitBasicBlock() const { return exit_bb_; }

  // Return the Value representing the induction variable in the body basic
  // block of the loop.
  toolchain::Value* GetIndVarValue() const { return indvar_; }
 ...
 private:
  ...
  ForLoop(absl::string_view prefix, absl::string_view suffix,
          toolchain::Value* start_index, toolchain::Value* end_index, toolchain::Value* step,
          UnrollMode unroll_mode, bool prevent_vectorization);

  // Emit the loop at the insert point of the builder.
  void Emit(toolchain::IRBuilderBase* b);

  toolchain::BasicBlock* CreateLoopBB(absl::string_view name,
  ...
  // toolchain::Loop for this `ForLoop`.
  std::vector<toolchain::Metadata*> GetLoopMetadata(toolchain::IRBuilderBase* b);

  std::string prefix_;
  std::string suffix_;
  toolchain::Value* start_index_;
  toolchain::Value* end_index_;
  toolchain::Value* step_;
  ...
  toolchain::BasicBlock* insert_before_bb_;

  toolchain::BasicBlock* preheader_bb_;
  toolchain::BasicBlock* header_bb_;
  toolchain::BasicBlock* body_bb_;
  toolchain::BasicBlock* exit_bb_;
  toolchain::Value* indvar_;
...
class ForLoopNest {
 public:
  ForLoopNest(absl::string_view name, toolchain::IRBuilderBase* b,
              toolchain::Type* index_ty = nullptr)
      : name_(name),
  ...
  std::unique_ptr<ForLoop> AddLoop(
      absl::string_view suffix, toolchain::Value* start_index,
      toolchain::Value* end_index, toolchain::Value* stride,
  ...
  std::vector<toolchain::Value*> AddLoopsForShapeOnDimensions(
  ...
  std::vector<toolchain::Value*> EmitOperandArrayLoopNest(
  ...
  // added yet.
  toolchain::BasicBlock* GetOuterLoopPreheaderBasicBlock() {
    return outer_loop_preheader_bb_;
  }
  toolchain::BasicBlock* GetOuterLoopExitBasicBlock() { return outer_loop_exit_bb_; }
  toolchain::BasicBlock* GetInnerLoopBodyBasicBlock() { return inner_loop_body_bb_; }

 private:
  void SetIndexType(toolchain::Type* index_ty) {
  ...
  toolchain::BasicBlock* outer_loop_preheader_bb_;
  toolchain::BasicBlock* outer_loop_exit_bb_;

  // The body basic block of the most-recently added loop, or nullptr if no loop
  // has been added yet.
  toolchain::BasicBlock* inner_loop_body_bb_;
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LLVM_LOOP_H_
