...
#ifndef MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LOOP_EMITTER_H_
#define MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LOOP_EMITTER_H_
...
using ElementGenerator =
    std::function<absl::StatusOr<toolchain::Value*>(const IrArray::Index& index)>;
...
BodyEmitter MakeBodyEmitter(const ElementGenerator& target_element_generator,
                            ...
                            toolchain::IRBuilderBase* b, bool is_tuple);
...
class LoopEmitter {
 public:
  ...
  LoopEmitter(const BodyEmitter& body_emitter, const Shape& shape,
              std::vector<toolchain::Value*> dynamic_dims, toolchain::IRBuilderBase* b);
  ...
  LoopEmitter(const ElementGenerator& target_element_generator,
              const IrArray& target_array, toolchain::IRBuilderBase* b);
  ...
  LoopEmitter(const ElementGenerator& target_element_generator,
              absl::Span<const IrArray> target_arrays, toolchain::IRBuilderBase* b);
  ...
  virtual std::vector<IrArray::Index> EmitIndexAndSetExitBasicBlock(
      absl::string_view loop_name, toolchain::Type* index_type,
  ...
  absl::Status EmitLoop(absl::string_view loop_name = "",
                        toolchain::Type* index_type = nullptr);
 ...
 protected:
  ...
  std::vector<toolchain::Value*> dynamic_dims_;
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_LLVM_IR_LOOP_EMITTER_H_
