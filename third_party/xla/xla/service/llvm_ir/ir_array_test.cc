...
class IrArrayTest : public ::testing::Test {
 public:
  ...
  toolchain::Function* EmitFunctionAndSetInsertPoint(
      toolchain::ArrayRef<toolchain::Type*> params) {
    toolchain::FunctionType* function_type =
        toolchain::FunctionType::get(toolchain::Type::getVoidTy(context_), params,
                                /*isVarArg=*/false);
    ...
    toolchain::BasicBlock* bb = toolchain::BasicBlock::Create(context_, "bb", function);
...
TEST_F(IrArrayTest, EmitArrayElementAddress) {
  toolchain::Function* function = EmitFunctionAndSetInsertPoint(
      {builder_.getPtrTy(), builder_.getInt32Ty()});
  toolchain::Argument* array_ptr = function->getArg(0);
  toolchain::Argument* array_index = function->getArg(1);

  Shape shape = ShapeUtil::MakeShape(F32, {3, 5});
  toolchain::Type* type = llvm_ir::ShapeToIrType(shape, module_.getContext());
...
TEST_F(IrArrayTest, EmitArrayElementAddressInt4NonLinear) {
  toolchain::Function* function = EmitFunctionAndSetInsertPoint(
      {toolchain::PointerType::get(context_, 0), toolchain::Type::getInt32Ty(context_),
       toolchain::Type::getInt32Ty(context_)});
  toolchain::Argument* array_ptr = function->getArg(0);
  toolchain::Argument* array_index0 = function->getArg(1);
  toolchain::Argument* array_index1 = function->getArg(2);
...
TEST_F(IrArrayTest, EmitWriteArrayElementInt4) {
  ...
  toolchain::Argument* array_ptr = function->getArg(0);
  toolchain::Argument* array_index = function->getArg(1);
  toolchain::Argument* val_to_write = function->getArg(2);
  ...
  const char* filecheck_pattern = R"(
    ...
    CHECK: %[[mask:[0-9]+]] = call i8 @toolchain.fshl.i8(i8 -16, i8 -16, i8 %[[trunc]])
