...
void AliasAnalysis::AddAliasingInformationToIrArray(const HloInstruction& hlo,
  ...
  if (module_.config().debug_options().xla_llvm_enable_alias_scope_metadata()) {
    toolchain::MDNode*& alias_scope_md = alias_scope_metadata_[buffer_slice];
  ...
  if (module_.config().debug_options().xla_llvm_enable_noalias_metadata()) {
    toolchain::MDNode*& noalias_md = noalias_metadata_[{buffer_slice, &hlo}];
...
toolchain::MDNode* AliasAnalysis::GetAliasDomain() {
  toolchain::MDBuilder metadata_builder(*context_);
...
toolchain::MDNode* AliasAnalysis::GetAliasScopeMetadataForBuffer(
  ...
  toolchain::MDBuilder metadata_builder(domain->getContext());
  toolchain::MDNode* scope = metadata_builder.createAliasScope(
      "buffer: " + buffer_slice.ToString(), domain);
  toolchain::MDNode* scope_list = toolchain::MDNode::get(domain->getContext(), scope);
  return scope_list;
}

toolchain::MDNode* AliasAnalysis::GetNoaliasMetadataForBuffer(
    const BufferAllocation::Slice& buffer_slice, toolchain::MDNode* domain,
  ...
  toolchain::MDBuilder metadata_builder(domain->getContext());
  std::vector<toolchain::Metadata*> scopes;
  for (const BufferAllocation::Slice noalias_slice : buffers) {
    toolchain::MDNode* scope = metadata_builder.createAliasScope(
        "buffer: " + noalias_slice.ToString(), domain);
    scopes.push_back(scope);
  }
  toolchain::MDNode* noalias_list =
      toolchain::MDNode::get(domain->getContext(), AsArrayRef(scopes));
  return noalias_list;
}
