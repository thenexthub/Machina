...
TEST_F(ElementalIrEmitterExecutionTest, EmitReducePrecisionIR_F16ToF8e5m2) {
  ...
  toolchain::IRBuilder<> builder(llvm_context);
  toolchain::IRBuilderBase* b = &builder;
  toolchain::Type* f16_type = b->getHalfTy();
  ...
  for (auto tc : test_cases) {
    toolchain::Value* c0 = toolchain::ConstantFP::get(f16_type, tc.input);

    absl::StatusOr<toolchain::Value*> f16_reduced_statusor = EmitReducePrecisionIR(
        /*src_ty=*/F16, c0,
    ...
    toolchain::Value* f16_reduced = f16_reduced_statusor.value();
