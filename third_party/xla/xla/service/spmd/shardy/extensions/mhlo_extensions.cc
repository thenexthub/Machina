...
struct RaggedDotShardingRuleOpInterface
  ...
  mlir::sdy::OpShardingRuleAttr getShardingRule(mlir::Operation* op) const {
    mhlo::RaggedDotOp raggedDot = toolchain::cast<mhlo::RaggedDotOp>(op);
    ...
    RaggedDotMode mode;
    if (toolchain::is_contained(lhsContractingDims, lhsRaggedDim)) {
      CHECK(rhsGroupDims.empty());
      mode = RaggedDotMode::kContracting;
    } else if (toolchain::is_contained(lhsBatchingDims, lhsRaggedDim)) {
      CHECK(rhsGroupDims.empty());
    ...
    for (auto [lhsDim, rhsDim] :
         toolchain::zip_equal(lhsBatchingDims, rhsBatchingDims)) {
      builder.addFactor(
          {lhsDim, rhsDim,
    ...
    for (int64_t i = 0; i < lhsType.getRank(); i++) {
      if (!toolchain::is_contained(lhsContractingDims, i) &&
          !toolchain::is_contained(lhsBatchingDims, i)) {
    ...
    for (int64_t i = 0; i < rhsType.getRank(); i++) {
      if (!toolchain::is_contained(rhsContractingDims, i) &&
          !toolchain::is_contained(rhsBatchingDims, i) &&
          !toolchain::is_contained(rhsGroupDims, i)) {
    ...
    for (auto [lhsDim, rhsDim] :
         toolchain::zip_equal(lhsContractingDims, rhsContractingDims)) {
...
struct TopKShardingRuleOpInterface
  ...
  mlir::sdy::OpShardingRuleAttr getShardingRule(mlir::Operation* op) const {
    mhlo::TopKOp topK = toolchain::cast<mhlo::TopKOp>(op);
