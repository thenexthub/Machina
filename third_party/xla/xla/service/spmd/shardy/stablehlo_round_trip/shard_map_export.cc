...
using ManualComputationToParentManualAxes =
    toolchain::SmallDenseMap<ManualComputationOp, SmallVector<StringAttr>>;
...
TensorShardingAttr getFirstSharding(ManualComputationOp op) {
  auto inOutShardings = toolchain::concat<const TensorShardingAttr>(
...
void convertManualComputationOp(
  ...
  for (auto [globalOperand, localArgumentType, inSharding] :
       toolchain::zip_equal(op.getOperands(), op.getBody().getArgumentTypes(),
  ...
  for (auto [terminatorOperand, opResult, outSharding] :
       toolchain::zip_equal(terminator->getOpOperands(), op.getResults(),
