...
#include "machina/xla/mlir_hlo/mhlo/IR/hlo_ops.h"  // for CopyOp
...
mlir::DenseIntElementsAttr getReplicaGroups(
  ...
  int64_t groupSize = 1;
  toolchain::SmallDenseMap<AxisRefAttr, int64_t> axisRefToReductionIndex;
  axisRefToReductionIndex.reserve(reductionAxes.size());
  for (auto [index, axis] : toolchain::enumerate(reductionAxes)) {
  ...
  for (auto [meshIndex, axis] : toolchain::enumerate(meshAxisRefs)) {
...
int64_t convertReduceScatter(sdy::ReduceScatterOp op, int64_t nextChannelId,
  ...
  ManualComputationOp manualComputation = createFullyManualComputation(
      ...
      [&](mlir::BlockArgument arg, OpBuilder& blockBuilder) {
        ...
        SmallVector<int64_t> curShape = toolchain::to_vector(inputType.getShape());
        for (auto [dim, reductionAxes] :
             toolchain::enumerate(op.getReduceScatterAxes())) {
...
void syncInOutUnreducedAxes(mlir::Operation* op) {
  ...
  if (inSharding.anyOfDimShardingOrReplicatedAxis([&](AxisRefAttr inAxis) {
        return toolchain::any_of(outSharding.getUnreducedAxes(),
