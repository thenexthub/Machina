...
#include "machina/xla/service/spmd/shardy/round_trip_common/pipeline_passes.h"
...
SmallVector<SubDimInfo> getOrderedSubDimsFromIotaTileAssignment(
  ...
  for (const int64_t axisSize : toolchain::reverse(axisSizes)) {
...
MeshAxesAndIds findMeshAxesAndIds(
  ...
  toolchain::SmallDenseSet<int64_t> maximalDeviceIdSet;
  ...
  for (auto [axisIndex, axisSize] : toolchain::enumerate(axes)) {
    auto name = StringAttr::get(context, absl::StrCat("_axis_", axisIndex));
    namedAxes.push_back(MeshAxisAttr::get(context, name, axisSize));
  }

  maximalDeviceIds = toolchain::to_vector(maximalDeviceIdSet);
  toolchain::sort(maximalDeviceIds);
  return result;
}
...
TensorShardingAttr convertToSdySharding(
  ...
  for (auto [localAxisIndex, subDimInfo] : toolchain::enumerate(result.subDims)) {
  ...
  for (ArrayRef<int64_t> subDimToLocalAxisIndex : dimToSubDimToLocalAxisIndex) {
    ...
    // major.
    for (int64_t localAxisIndex : toolchain::reverse(subDimToLocalAxisIndex)) {
...
int64_t getRank(mlir::Type type) {
  if (auto tensorType = toolchain::dyn_cast<mlir::ShapedType>(type)) {
...
LogicalResult importShardings(
  ...
  for (auto [argNum, argType] : toolchain::enumerate(funcOp.getArgumentTypes())) {
    if (auto oldSharding =
            funcOp.getArgAttrOfType<StringAttr>(argNum, kXlaShardingAttr)) {
      funcOp.setArgAttr(
  ...
  for (auto [resNum, resType] : toolchain::enumerate(funcOp.getResultTypes())) {
    if (auto oldSharding =
  ...
  funcOp.front().walk([&](mlir::Operation* op) {
    if (auto oldSharding = op->getAttrOfType<StringAttr>(kXlaShardingAttr)) {
      ...
      for (const auto& [resHloSharding, resType] :
           toolchain::zip_equal(flatHloSharding, op->getResultTypes())) {
...
absl::Status addSdyShardingsToEntryComputation(xla::HloModule* module) {
  ...
  auto convertSharding = [mesh](const xla::HloSharding& hloSharding,
    ...
    return convertToSdySharding(hloSharding, mesh,
                                /*deviceIdToMaximalMeshName=*/
                                toolchain::SmallDenseMap<int64_t, mlir::StringRef>(),
                                rank,
