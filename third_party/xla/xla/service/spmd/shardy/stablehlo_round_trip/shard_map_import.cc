...
void insertAxisNamesFromSharding(mlir::MLIRContext* context,
                                 ...
                                 toolchain::SmallDenseSet<StringAttr>& axesSet) {
...
class ShardMapImportPass
 ...
 private:
  void runOnOperation() final {
    ...
    toolchain::SmallDenseMap<StringRef, mlir::Region*> shardMapNameToMovedRegion;
    bool success = true;
    module->walk([&](CallOp op) {
      ...
      if (!inShardings.empty() || !outShardings.empty()) {
        auto inOutShardings =
            toolchain::concat<TensorShardingAttr>(inShardings, outShardings);
        ...
        toolchain::SmallDenseSet<StringAttr> manualAxesSet;
        ...
        toolchain::sort(manualAxes, mesh.getAxisNameComparator());
      }
    ...
    for (auto& [patternOps, callOp, returnOp] : jaxShardMapOpsToBeErased) {
      ...
      for (const auto& [index, resultCustomCalls] :
           toolchain::enumerate(patternOps.resultOps)) {
        if (!resultCustomCalls.shapeTransformOp) {
      ...
      toolchain::SmallDenseSet<Operation*> seenShapeTransformOps;
