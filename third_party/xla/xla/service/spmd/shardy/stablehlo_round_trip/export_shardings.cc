...
class ExportStablehloShardingsPass
 ...
 public:
  ...
  void runOnOperation() final {
    ...
    for (MeshOp meshOp :
         toolchain::make_early_inc_range(moduleOp.getOps<MeshOp>())) {
...
HloSharding getHloShardingForOp(
  ...
  toolchain::transform(shardings, std::back_inserter(newShardings),
  ...
  toolchain::transform(op->getResultTypes(), std::back_inserter(shapes),
...
HloSharding convertToHloSharding(
  ...
  toolchain::SmallDenseMap<AxisRefAttr, int64_t> axisRefToShardedPos;
  ...
  for (auto [index, dimSharding] :
       toolchain::enumerate(sdySharding.getDimShardings())) {
  ...
  for (auto [axisIndex, axisRef] : toolchain::enumerate(meshAxisRefs)) {
