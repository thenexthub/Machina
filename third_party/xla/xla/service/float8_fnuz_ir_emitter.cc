...
absl::StatusOr<const toolchain::fltSemantics*> PrimitiveTypeToAPFloatSemantics(
    PrimitiveType type) {
  switch (type) {
    case F4E2M1FN:
      return &toolchain::APFloat::Float4E2M1FN();
    case F8E3M4:
      return &toolchain::APFloat::Float8E3M4();
    case F8E4M3:
      return &toolchain::APFloat::Float8E4M3();
    case F8E4M3B11FNUZ:
      return &toolchain::APFloat::Float8E4M3B11FNUZ();
    case F8E4M3FN:
      return &toolchain::APFloat::Float8E4M3FN();
    case F8E4M3FNUZ:
      return &toolchain::APFloat::Float8E4M3FNUZ();
    case F8E5M2:
      return &toolchain::APFloat::Float8E5M2();
    case F8E5M2FNUZ:
      return &toolchain::APFloat::Float8E5M2FNUZ();
    case F8E8M0FNU:
      return &toolchain::APFloat::Float8E8M0FNU();
    case BF16:
...
absl::StatusOr<toolchain::Type*> PrimitiveTypeToLLVMType(toolchain::IRBuilderBase* b,
                                                    PrimitiveType type) {
  switch (type) {
    case F4E2M1FN:
...
absl::StatusOr<uint64_t> ComputeMaximumValue(PrimitiveType input_type,
  ...
  // type.
  bool losesInfo;
  toolchain::APFloat largest_output_value =
      toolchain::APFloat::getLargest(*output_semantics);
  largest_output_value.convert(
      *input_semantics, toolchain::RoundingMode::NearestTiesToEven, &losesInfo);

  toolchain::APInt maximum_value = largest_output_value.bitcastToAPInt();
...
absl::StatusOr<toolchain::Value*> IsInputOutsideOutputRange(
    PrimitiveType input_type, toolchain::Value* value, PrimitiveType output_type,
    toolchain::IRBuilderBase* b) {
  const uint64_t shift = BitWidth(input_type) - 1;
  const uint64_t bit_mask = (0x1ull << shift) - 1;

  // Ignore the sign bit.
  toolchain::Value* non_sign_bits = b->CreateAnd(value, bit_mask);

  TF_ASSIGN_OR_RETURN(uint64_t maximum_value,
                      ComputeMaximumValue(input_type, output_type, b));

  // Compare against the maximum value.
  toolchain::Type* uint_type = b->getIntNTy(BitWidth(input_type));
  return b->CreateICmpUGT(non_sign_bits,
                          toolchain::ConstantInt::get(uint_type, maximum_value));
}

toolchain::Value* IsZero(PrimitiveType type, toolchain::Value* value,
                    toolchain::IRBuilderBase* b) {
  ...
  toolchain::Type* uint_type = b->getIntNTy(BitWidth(type));
  return b->CreateICmpEQ(non_sign_bits,
                         toolchain::ConstantInt::get(uint_type, 0x0u));
}

toolchain::Value* IsNormalNumber(PrimitiveType type, toolchain::Value* value,
                            toolchain::IRBuilderBase* b) {
  ...
  toolchain::Value* exponent_bits = b->CreateAnd(value, exponent_bit_mask);
  toolchain::Type* uint_type = b->getIntNTy(BitWidth(type));
  return b->CreateICmpNE(exponent_bits,
                         toolchain::ConstantInt::get(uint_type, 0x0u));
}

toolchain::Value* IsOutputNormal(PrimitiveType input_type, toolchain::Value* exponent,
                            PrimitiveType output_type, toolchain::IRBuilderBase* b) {
  const uint64_t denorm_exponent = UnderflowExponent(output_type) - 1;

  toolchain::Type* uint_type = b->getIntNTy(BitWidth(input_type));
  return b->CreateICmpSGE(exponent,
                          toolchain::ConstantInt::get(uint_type, denorm_exponent));
}

toolchain::Value* Max(toolchain::Type* type, toolchain::Value* x, uint64_t y,
                 toolchain::IRBuilderBase* b) {
  return b->CreateBinaryIntrinsic(toolchain::Intrinsic::smax, x,
                                  toolchain::ConstantInt::get(type, y));
}

toolchain::Value* Min(toolchain::Type* type, toolchain::Value* x, uint64_t y,
                 toolchain::IRBuilderBase* b) {
  return b->CreateBinaryIntrinsic(toolchain::Intrinsic::smin, x,
                                  toolchain::ConstantInt::get(type, y));
}

// Returns the sign bit of the input value shifted down to the least
// significant bit.
toolchain::Value* ExtractSign(PrimitiveType type, toolchain::Value* value,
                         bool preserve_signed_zero, toolchain::IRBuilderBase* b) {
  const uint64_t shift = BitWidth(type) - 1;
  const uint64_t sign_bit_mask = 0x1ull << shift;

  toolchain::Value* sign = b->CreateAnd(value, sign_bit_mask);

  toolchain::Type* uint_type = b->getIntNTy(BitWidth(type));
  sign = b->CreateLShr(sign, toolchain::ConstantInt::get(uint_type, shift));

  if (preserve_signed_zero) {
    return sign;
  }

  toolchain::Value* is_zero_pred = IsZero(type, value, b);
  return b->CreateSelect(is_zero_pred, toolchain::ConstantInt::get(uint_type, 0x0u),
                         sign);
}

// Returns the exponent of the input value shifted down to the least
// significant bits and without any bias.
toolchain::Value* ExtractExponent(PrimitiveType type, toolchain::Value* value,
                             toolchain::IRBuilderBase* b) {
  const uint64_t shift = BitWidth(type) - 1;
  ...
  // Mask out sign bit.
  toolchain::Value* exponent = b->CreateAnd(value, bit_mask);

  // Shift the mantissa bits away, leaving the exponent.
  exponent = b->CreateLShr(
      exponent, toolchain::ConstantInt::get(uint_type, SignificandWidth(type) - 1));

  // Subtract the exponent bias.
  exponent = b->CreateSub(
      exponent, toolchain::ConstantInt::get(uint_type, ExponentBias(type)));

  // If the input number is not a normal number, return the subnormal exponent.
  toolchain::Value* input_normal_pred = IsNormalNumber(type, value, b);
  return b->CreateSelect(
      input_normal_pred, exponent,
      toolchain::ConstantInt::get(uint_type, UnderflowExponent(type) - 1));
}
...
// returned value.
toolchain::Value* ExtractMantissa(PrimitiveType type, toolchain::Value* value,
                             toolchain::IRBuilderBase* b) {
  const uint64_t shift = SignificandWidth(type) - 1;
  const uint64_t mantissa_bit_mask = (0x1ull << shift) - 0x1ull;

  toolchain::Value* mantissa = b->CreateAnd(value, mantissa_bit_mask);

  toolchain::Value* input_normal_pred = IsNormalNumber(type, value, b);
  toolchain::Value* mantissa_normal = b->CreateOr(mantissa, (0x1ull << shift));
...
absl::StatusOr<toolchain::Value*> LastMantissaBit(PrimitiveType input_type,
  ...
  toolchain::Type* int_type = b->getIntNTy(BitWidth(input_type));

  toolchain::Value* exponent = ExtractExponent(input_type, value, b);

  // The index when the input/output is normal.
  toolchain::Value* last_bit_index =
      toolchain::ConstantInt::get(int_type, src_mantissa_bits - dest_mantissa_bits);

  // Increase the index if the output will be denormal given the exponent.
  toolchain::Value* denormal_shift = b->CreateSub(
      toolchain::ConstantInt::get(int_type, UnderflowExponent(output_type) - 1),
      exponent);
  ...
  // 0b0|00000|0110111111
  //             ^-- last mantissa bit
  toolchain::Value* denorm_last_mantissa_bit =
      toolchain::ConstantInt::get(int_type, output_log_minimum - input_log_minimum);

  // Select the last mantissa bit based on whether the input is a normal number.
  toolchain::Value* normal_pred = IsNormalNumber(input_type, value, b);
  ...
  toolchain::Value* last_mantissa_bit =
...
absl::StatusOr<toolchain::Value*> DynamicRoundingBias(PrimitiveType input_type,
                                                 toolchain::Value* value,
                                                 PrimitiveType output_type,
                                                 toolchain::IRBuilderBase* b) {
  toolchain::Type* int_type = b->getIntNTy(BitWidth(input_type));
  ...
  TF_ASSIGN_OR_RETURN(toolchain::Value * shift,
                      LastMantissaBit(input_type, value, output_type, b));

  // Compute the mask to select that bit.
  toolchain::Value* last_mantissa_bit_mask =
      b->CreateShl(toolchain::ConstantInt::get(int_type, 0x1u), shift);

  // Given the mantissa bit mask, compute the rounding bias bits.
  toolchain::Value* base_rounding_bias = b->CreateLShr(last_mantissa_bit_mask, 0x1u);
  base_rounding_bias =
      b->CreateSub(base_rounding_bias, toolchain::ConstantInt::get(int_type, 0x1u));

  // Select the last mantissa bit, and shift it down to the lsb.
  toolchain::Value* mantissa = ExtractMantissa(input_type, value, b);
  toolchain::Value* x_last_mantissa_bit =
...
toolchain::Value* BuildOutputMantissa(PrimitiveType input_type,
                                 toolchain::Value* exponent, toolchain::Value* mantissa,
                                 PrimitiveType output_type,
                                 toolchain::IRBuilderBase* b) {
  toolchain::Type* input_int_type = b->getIntNTy(BitWidth(input_type));

  // Count the number of leading zeros, excluding the bits that would contain
  // the exponent.
  toolchain::Value* zero_count =
      b->CreateBinaryIntrinsic(toolchain::Intrinsic::ctlz, mantissa,
                               toolchain::ConstantInt::get(b->getInt1Ty(), 0x0u));
  zero_count = b->CreateSub(
      zero_count,
      toolchain::ConstantInt::get(input_int_type, ExponentWidth(input_type)));

  // The amount to shift the normal mantissa down.
  toolchain::Value* shift = b->CreateSub(
      toolchain::ConstantInt::get(input_int_type, SignificandWidth(input_type) -
  ...
  shift = b->CreateSub(toolchain::ConstantInt::get(
  ...
  toolchain::Value* mantissa_denorm = b->CreateLShr(mantissa, shift);

  // Test whether the output will be a normal number.
  toolchain::Value* output_normal_pred =
...
// position.
toolchain::Value* BuildOutputExponent(PrimitiveType input_type,
                                 toolchain::Value* exponent, toolchain::Value* mantissa,
                                 PrimitiveType output_type,
                                 toolchain::IRBuilderBase* b) {
  toolchain::Type* input_int_type = b->getIntNTy(BitWidth(input_type));
  ...
  exponent = b->CreateSelect(
      ...
      toolchain::ConstantInt::get(input_int_type, -OverflowExponent(output_type)));
...
// position.
toolchain::Value* BuildOutputSign(toolchain::Value* sign, PrimitiveType output_type,
                             toolchain::IRBuilderBase* b) {
...
absl::StatusOr<uint64_t> GetQNaN(PrimitiveType type) {
  ...
  return toolchain::APFloat::getQNaN(*semantics).bitcastToAPInt().getZExtValue();
}
}  // namespace

absl::StatusOr<toolchain::Value*> EmitFloatingToF8fnuz(PrimitiveType input_type,
                                                  toolchain::Value* input_value,
                                                  PrimitiveType output_type,
                                                  toolchain::IRBuilderBase* b) {
  ...
  toolchain::IntegerType* input_int_type = b->getIntNTy(BitWidth(input_type));
  toolchain::Value* input_uint = b->CreateBitCast(input_value, input_int_type);

  TF_ASSIGN_OR_RETURN(
      toolchain::Value * out_of_range_pred,
  ...
  TF_ASSIGN_OR_RETURN(
      toolchain::Value * input_rounding_bias,
  ...
  toolchain::Value* input_uint_rounded =
  ...
  toolchain::Value* sign =
      ExtractSign(input_type, input_uint, /*preserve_signed_zero=*/false, b);
  toolchain::Value* exponent = ExtractExponent(input_type, input_uint_rounded, b);
  toolchain::Value* mantissa = ExtractMantissa(input_type, input_uint_rounded, b);

  // The component parts of the output value.
  toolchain::Value* output_sign = BuildOutputSign(sign, output_type, b);
  toolchain::Value* output_exponent =
      BuildOutputExponent(input_type, exponent, mantissa, output_type, b);
  toolchain::Value* output_mantissa =
      BuildOutputMantissa(input_type, exponent, mantissa, output_type, b);

  // Bitwise or the output components together.
  toolchain::Value* result = b->CreateOr(output_exponent, output_mantissa);

  // Check for output underflow before adding a sign bit. There's no -0 in
  // fnuz types.
  toolchain::Value* is_zero_pred = IsZero(input_type, result, b);
  output_sign = b->CreateSelect(
      is_zero_pred, toolchain::ConstantInt::get(input_int_type, 0x0u), output_sign);
  ...
  return b->CreateSelect(out_of_range_pred,
                         toolchain::ConstantInt::get(b->getInt8Ty(), output_qnan),
                         result);
}

absl::StatusOr<toolchain::Value*> EmitF8fnuzToFloating(PrimitiveType input_type,
                                                  toolchain::Value* f8_value,
                                                  PrimitiveType output_type,
                                                  toolchain::IRBuilderBase* b,
  ...
  toolchain::IntegerType* output_int_type = b->getIntNTy(output_type_bit_width);

  toolchain::ArrayType* result_lut_array_type =
      toolchain::ArrayType::get(output_int_type, 128);
  ...
  toolchain::Constant* global_result_lut_array = module->getOrInsertGlobal(
      lut_name, result_lut_array_type, [&]() -> toolchain::GlobalVariable* {
        // Since the function range is only 2^8 and symmetric on the sign bit,
        // this is implemented as a table lookup.
        toolchain::Constant* result_lut[128];

        // Populate the table with values computed using toolchain APFloat.
        for (uint8_t i = 0; i < 128; ++i) {
          toolchain::APFloat value(*input_semantics, toolchain::APInt(8, i));

          bool losesInfo;
          value.convert(*output_semantics, toolchain::APFloat::rmNearestTiesToEven,
                        &losesInfo);

          result_lut[i] = toolchain::ConstantInt::get(
              output_int_type, value.bitcastToAPInt().getZExtValue());
        }

        toolchain::Constant* result_lut_array =
            toolchain::ConstantArray::get(result_lut_array_type, result_lut);
  ...
  toolchain::Value* nan_pred = b->CreateICmpEQ(
      f8_value, toolchain::ConstantInt::get(b->getInt8Ty(), input_qnan));

  // Extract the sign, which will be added back to the result of the table
  // lookup.
  toolchain::Value* sign = b->CreateAnd(f8_value, 0x80);

  // The lower 7 bits used s the index for the table lookup.
  toolchain::Value* f8_abs = b->CreateAnd(f8_value, 0x7F);

  // Fetch the value from the lookup table.
  toolchain::Value* result_abs =
  ...
  toolchain::Value* is_output_zero_pred = IsZero(output_type, result_abs, b);
  sign = b->CreateSelect(is_output_zero_pred,
                         toolchain::ConstantInt::get(b->getInt8Ty(), 0x0u), sign);
  ...
  toolchain::Value* result = b->CreateOr(sign, result_abs);
  ...
  return b->CreateBitCast(
      b->CreateSelect(nan_pred,
                      toolchain::ConstantInt::get(output_int_type, output_qnan),
                      result),
      type);
}
