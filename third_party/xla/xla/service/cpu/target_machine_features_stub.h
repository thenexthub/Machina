...
#ifndef MACHINA_XLASERVICE_CPU_TARGET_MACHINE_FEATURES_STUB_H_
#define MACHINA_XLASERVICE_CPU_TARGET_MACHINE_FEATURES_STUB_H_
...
class TargetMachineFeaturesStub : public TargetMachineFeatures {
 public:
  ...
  int vector_register_byte_size(const toolchain::Function& function) const final {
    LOG(FATAL) << "Unexpected call to " << __func__;
  }

  int vector_register_num_elements(const toolchain::Function& function,
                                   PrimitiveType type) const final {
    LOG(FATAL) << "Unexpected call to " << __func__;
  }

  int vector_register_count(const toolchain::Function& function) const final {
    LOG(FATAL) << "Unexpected call to " << __func__;
  }
...
#endif  // MACHINA_XLASERVICE_CPU_TARGET_MACHINE_FEATURES_STUB_H_
