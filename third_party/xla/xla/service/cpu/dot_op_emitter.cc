...
class DotOpEmitter {
 public:
  explicit DotOpEmitter(
      ...
      toolchain::Value* work_group_id, toolchain::Value* executable_run_options_value,
      toolchain::IRBuilderBase* b, const HloModuleConfig& hlo_module_config,
      const TargetMachineFeatures& target_machine_features,
 ...
 private:
  ...
  toolchain::Value* executable_run_options_value_;
...
void DotOpEmitter::EmitTiledLlvmIrGemm() {
  ...
  toolchain::Value* lhs = lhs_array_.GetBasePointer();
  toolchain::Value* rhs = rhs_array_.GetBasePointer();
  toolchain::Value* target = target_array_.GetBasePointer();
...
int64_t DotOpEmitter::EmitTiledLlvmIrGemv() {
  ...
  toolchain::Value* result_op = target_array_.GetBasePointer();
  toolchain::Value* lhs_op =
      swap_operands ? rhs_array_.GetBasePointer() : lhs_array_.GetBasePointer();
  toolchain::Value* rhs_op =
...
void DotOpEmitter::EmitNaiveLlvmIrGemm() {
  ...
  toolchain::Function* func = reduction_loop->GetPreheaderBasicBlock()->getParent();
  SetToFirstInsertPoint(&func->getEntryBlock(), b_);
  toolchain::Type* accum_type = target_array_.GetElementLlvmType();
  toolchain::Value* accum_address =
      b_->CreateAlloca(accum_type, /*ArraySize=*/nullptr, "accum_address");

  // Preheader basic block of reduction loop:
  // - Initialize accumulator to zero.
  toolchain::BasicBlock* preheader_bb = reduction_loop->GetPreheaderBasicBlock();
  b_->SetInsertPoint(preheader_bb->getTerminator());

  b_->CreateStore(toolchain::Constant::getNullValue(accum_type), accum_address);
  ...
  toolchain::Value* lhs_element = lhs_array_.EmitReadArrayElement(lhs_index, b_);
  toolchain::Value* rhs_element = rhs_array_.EmitReadArrayElement(rhs_index, b_);

  toolchain::Value* accum = b_->CreateLoad(accum_type, accum_address);
  toolchain::Value* updated_accum;
  if (ShapeUtil::ElementIsComplex(lhs_shape)) {
    auto real = [&](toolchain::Value* x) { return b_->CreateExtractValue(x, {0}); };
    auto imag = [&](toolchain::Value* x) { return b_->CreateExtractValue(x, {1}); };
    ...
    toolchain::Value* product = b_->CreateMul(lhs_element, rhs_element);
    updated_accum = b_->CreateAdd(accum, product);
  } else if (lhs_shape.element_type() == PRED) {
    toolchain::Value* product = b_->CreateAnd(lhs_element, rhs_element);
    updated_accum = b_->CreateOr(accum, product);
  } else {
    toolchain::Value* product = b_->CreateFMul(lhs_element, rhs_element);
  ...
  toolchain::Value* result = b_->CreateLoad(accum_type, accum_address);
...
absl::Status DotOpEmitter::EmitScalarDot() {
  ...
  toolchain::Type* index_type = b_->getInt64Ty();
...
absl::Status DotOpEmitter::EmitCallToRuntime() {
  ...
  toolchain::Function* function = b_->GetInsertBlock()->getParent();
  toolchain::LLVMContext& context = b_->getContext();
  toolchain::Module* module = function->getParent();
  toolchain::Type* float_type;
  const char* fn_name;
  switch (type) {
    case F16:
  ...
  toolchain::Type* ptr_type = b_->getPtrTy();
  toolchain::Type* int64_type = b_->getInt64Ty();
  toolchain::Type* int32_type = b_->getInt32Ty();
  toolchain::FunctionType* matmul_type = toolchain::FunctionType::get(
      b_->getVoidTy(),
      {ptr_type, ptr_type, ptr_type, ptr_type, int64_type, int64_type,
       int64_type, int32_type, int32_type},
      /*isVarArg=*/false);

  toolchain::FunctionCallee matmul_func =
      module->getOrInsertFunction(fn_name, matmul_type);
  if (auto* fn = toolchain::dyn_cast<toolchain::Function>(matmul_func.getCallee())) {
...
llvm_ir::IrArray CollapseFirstNDims(toolchain::IRBuilderBase* b,
...
llvm_ir::IrArray SliceOutInnerArray(llvm_ir::IrArray outer_array,
                                    toolchain::Value* batch_index,
                                    toolchain::IRBuilderBase* b) {
  Shape inner_shape = DropFirstDim(outer_array.GetShape());
  std::vector<toolchain::Value*> multidim_index(inner_shape.dimensions().size() + 1,
                                           b->getInt64(0));
  multidim_index[0] = batch_index;
  llvm_ir::IrArray::Index slice_index(multidim_index, outer_array.GetShape(),
                                      batch_index->getType());
  toolchain::Value* slice_ptr = outer_array.EmitArrayElementAddress(slice_index, b);
  toolchain::Type* new_ir_type =
...
bool PotentiallyImplementedAsEigenMatmul(
    ...
    toolchain::Value* executable_run_options_value, toolchain::IRBuilderBase* b,
...
absl::StatusOr<DotOpWorkGroupDim> EmitBatchDotOperation(
    const HloInstruction& dot, const llvm_ir::IrArray& target_array,
    const llvm_ir::IrArray& lhs_array, const llvm_ir::IrArray& rhs_array,
    DotOpWorkGroupId work_group_id, toolchain::Value* executable_run_options_value,
    toolchain::IRBuilderBase* b, const HloModuleConfig& hlo_module_config,
    const TargetMachineFeatures& target_machine_features,
    bool allow_runtime_calls, bool allow_parallelism) {
  ...
  if (ShouldUseMultiThreadedEigen(hlo_module_config) &&
    ...
    auto inner_dot = [&](toolchain::Value* batch_index) {
      DotDimensionNumbers adjusted_dim_numbers = dot.dot_dimension_numbers();
      adjusted_dim_numbers.clear_lhs_batch_dimensions();
      adjusted_dim_numbers.clear_rhs_batch_dimensions();
