...
#include "machina/xla/backends/cpu/codegen/emitters/cpu_fusion_emitter.h"
...
#include "machina/xla/shape_partition.h"
...
IrEmitter2::IrEmitter2(const HloModule& hlo_module, toolchain::Module* module,
...
absl::StatusOr<IrEmitter2::KernelInfo> IrEmitter2::EmitPadHostKernel(
  ...
  toolchain::LLVMContext& ctx = module_->getContext();
  toolchain::IRBuilder<> b(ctx);
...
absl::StatusOr<IrEmitter2::KernelInfo> IrEmitter2::EmitFusionHostKernel(
  ...
  toolchain::IRBuilder<> b(module_->getContext());
...
absl::StatusOr<IrEmitter2::KernelInfo> IrEmitter2::EmitSliceToDynamicHostKernel(
  ...
  toolchain::IRBuilder<> ir_builder(module_->getContext());
...
absl::StatusOr<IrEmitter2::ComparatorInfo> IrEmitter2::EmitSortComparator(
  ...
  TF_ASSIGN_OR_RETURN(toolchain::Function * comparator_function,
  ...
  comparator_function->setUWTableKind(toolchain::UWTableKind::Default);
...
IrEmitter2::ParallelPartitionBounds IrEmitter2::EmitParallelPartitionBounds(
    toolchain::IRBuilderBase& b, const KernelPrototype& kernel_prototype,
  ...
  //
  toolchain::ArrayType* dim_bounds_ty = toolchain::ArrayType::get(b.getInt64Ty(), 2);
  toolchain::ArrayType* partition_bounds_ty =
      toolchain::ArrayType::get(dim_bounds_ty, num_parallel_dimensions);
  toolchain::ArrayType* parallel_bounds_ty =
      toolchain::ArrayType::get(partition_bounds_ty, it.GetTotalPartitionCount());

  // Build a nested array of partition bounds from shape partition iterator.
  std::vector<toolchain::Constant*> partition_bounds;
  for (int64_t i = 0; i < it.GetTotalPartitionCount(); ++i) {
    std::vector<toolchain::Constant*> dim_counts;
    for (auto [lower, size] : it.GetPartition(i)) {
      dim_counts.push_back(toolchain::ConstantArray::get(
          dim_bounds_ty, {b.getInt64(lower), b.getInt64(lower + size)}));
    }
    partition_bounds.push_back(
        toolchain::ConstantArray::get(partition_bounds_ty, dim_counts));
  }

  toolchain::Constant* parallel_bounds =
      toolchain::ConstantArray::get(parallel_bounds_ty, partition_bounds);

  toolchain::Module* module = b.GetInsertBlock()->getParent()->getParent();
  toolchain::GlobalVariable* parallel_bounds_global = new toolchain::GlobalVariable(
      /*M=*/*module,
  ...
  for (size_t i = 0; i < num_parallel_dimensions; ++i) {
    toolchain::Value* partition = kernel_prototype.workgroup_id.x;
    toolchain::Value* parallel_dim = b.getInt32(i);

    toolchain::Value* lower_gep = b.CreateInBoundsGEP(
        parallel_bounds_ty, parallel_bounds_global,
        {b.getInt32(0), partition, parallel_dim, b.getInt32(0)},
        absl::StrCat("lo_dim_", i, "_gep"));

    toolchain::Value* upper_gep = b.CreateInBoundsGEP(
        parallel_bounds_ty, parallel_bounds_global,
...
absl::StatusOr<se::ThreadDim> IrEmitter2::EmitElementalLoops(
    toolchain::IRBuilderBase& b, const HloInstruction* instr,
...
void IrEmitter2::AttachInvariantLoadMetadataForLoad(
    toolchain::LoadInst* instr) const {
