...
#include "machina/xla/service/cpu/target_machine_features_stub.h"
...
static std::pair<toolchain::Function*, toolchain::BasicBlock*> CreateFunction(
    toolchain::LLVMContext& context, toolchain::Module* module, toolchain::IRBuilderBase* b) {
  toolchain::PointerType* ptrtype = toolchain::PointerType::getUnqual(context);
  toolchain::FunctionType* ftype = toolchain::FunctionType::get(ptrtype, ptrtype, false);
  ...
  toolchain::BasicBlock* return_block =
      toolchain::BasicBlock::Create(context, "", function);
  b->SetInsertPoint(return_block);
  [[maybe_unused]] toolchain::ReturnInst* ret = b->CreateRet(
...
TEST_F(IrEmitterTest, ComputeFuncStack) {
  ...
  toolchain::IRBuilderBase* b = ir_emitter.b();
  ASSERT_NE(b, nullptr);

  const std::pair<toolchain::Function*, toolchain::BasicBlock*> fb =
      CreateFunction(context, module.get(), b);

  toolchain::Function* function = fb.first;
  toolchain::BasicBlock* return_block = fb.second;

  ASSERT_NE(function, nullptr);
  ASSERT_NE(return_block, nullptr);

  const auto funcname = "func1";
  const auto linkagetype = toolchain::GlobalValue::LinkageTypes::ExternalLinkage;
...
CreateIrEmitterForConstantEmissionTests(HloModule& module,
  ...
  IrCompiler::Options ir_compiler_options{
      /*optimization_level=*/toolchain::CodeGenOptLevel::Default,
...
ENTRY main {
  ...
  auto llvm_module = std::make_unique<toolchain::Module>("test", *llvm_context);
