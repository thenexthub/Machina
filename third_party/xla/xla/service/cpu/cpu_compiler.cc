...
// IWYU pragma: no_include "toolchain/Config/Disassemblers.def.inc"
// IWYU pragma: no_include "toolchain/Config/Targets.def.inc"
...
#include "toolchain/Transforms/Utils/ValueMapper.h"
...
#include "machina/xla/backends/cpu/codegen/emitters/cpu_fusion_emitter_config.h"
...
#include "machina/xla/backends/cpu/constant_allocation.h"
#include "machina/xla/backends/cpu/runtime/function_library.h"
#include "machina/xla/backends/cpu/runtime/thunk.h"
#include "machina/xla/backends/cpu/runtime/thunk.pb.h"
#include "machina/xla/backends/cpu/runtime/thunk_proto_serdes.h"
#include "machina/xla/backends/cpu/transforms/dot_library_rewriter.h"
#include "machina/xla/backends/cpu/transforms/xnn_graph_fusion.h"
#include "machina/xla/backends/cpu/xnn_fusion.h"
#include "machina/xla/cpu_function_runtime.h"
#include "machina/xla/hlo/analysis/alias_info.h"
#include "machina/xla/hlo/analysis/hlo_ordering.h"
#include "machina/xla/hlo/analysis/indexed_array_analysis.h"
#include "machina/xla/hlo/ir/dfs_hlo_visitor_with_default.h"
...
#include "machina/xla/hlo/transforms/expanders/cholesky_expander.h"
...
#include "machina/xla/hlo/transforms/literal_canonicalizer.h"
...
#include "machina/xla/hlo/transforms/simplifiers/batch_dot_simplification.h"
...
#include "machina/xla/hlo/transforms/simplifiers/convolution_group_converter.h"
...
#include "machina/xla/hlo/transforms/simplifiers/hlo_memory_scheduler.h"
...
#include "machina/xla/hlo/transforms/simplifiers/tree_reduction_rewriter.h"
...
#include "machina/xla/literal_pool.h"
...
#include "machina/xla/service/all_to_all_decomposer.h"
...
#include "machina/xla/service/call_graph.h"
#include "machina/xla/service/call_inliner.h"
#include "machina/xla/service/change_op_data_type.h"
#include "machina/xla/service/compiler.h"
#include "machina/xla/service/conditional_simplifier.h"
#include "machina/xla/service/conditional_to_select.h"
#include "machina/xla/service/copy_insertion.h"
#include "machina/xla/service/cpu/buffer_info_util.h"
#include "machina/xla/service/cpu/conv_canonicalization.h"
#include "machina/xla/service/cpu/cpu_aot_compilation_result.h"
#include "machina/xla/service/cpu/cpu_executable.h"
#include "machina/xla/service/cpu/cpu_float_support.h"
#include "machina/xla/service/cpu/cpu_instruction_fusion.h"
#include "machina/xla/service/cpu/cpu_layout_assignment.h"
#include "machina/xla/service/cpu/cpu_multi_output_fusion.h"
...
#include "machina/xla/service/cpu/fusion_wrapper.h"
#include "machina/xla/service/cpu/ir_emitter.h"
#include "machina/xla/service/cpu/ir_emitter2.h"
#include "machina/xla/service/cpu/metrics.h"
#include "machina/xla/service/cpu/parallel_task_assignment.h"
#include "machina/xla/service/cpu/runtime_symbol_generator.h"
#include "machina/xla/service/cpu/small_while_loop_hoisting_pass.h"
#include "machina/xla/service/cpu/thunk_emitter.h"
...
#include "machina/xla/service/sharding_propagation.h"
...
#include "machina/xla/service/spmd/stateful_rng_spmd_partitioner.h"
#include "machina/xla/service/topk_rewriter.h"
#include "machina/xla/service/transpose_folding.h"
#include "machina/xla/service/triangular_solve_expander.h"
#include "machina/xla/service/while_loop_constant_sinking.h"
#include "machina/xla/service/while_loop_invariant_code_motion.h"
...
#include "machina/xla/service/cpu/onednn_contraction_rewriter.h"
#include "machina/xla/service/cpu/onednn_float_support.h"
#include "machina/xla/service/cpu/onednn_ops_rewriter.h"
#endif
...
static tsl::thread::ThreadPool* GetCompilationThreadPool() {
  ...
  static auto* const thread_pool = new tsl::thread::ThreadPool(
      tsl::Env::Default(), thread_options, "xla-cpu-toolchain-codegen",
...
absl::Status CpuCompiler::RunHloPasses(HloModule* module, bool is_aot_compile,
                                       toolchain::TargetMachine* target_machine,
                                       const CompileOptions& compile_options) {
...
toolchain::TargetOptions CompilerTargetOptions(
...
std::pair<LLVMCompiler::ModuleHook, LLVMCompiler::ModuleHook> GetIRModuleHooks(
  ...
  return {[hook](const toolchain::Module& llvm_module) {
            return hook(/*optimized=*/false, llvm_module);
          },
          [hook](const toolchain::Module& llvm_module) {
            return hook(/*optimized=*/true, llvm_module);
          }};
}

absl::Status VerifyLlvmModule(const toolchain::Module& llvm_module) {
  MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER("CpuCompiler - Running LLVM verifier");
...
absl::StatusOr<std::unique_ptr<HloModule>> CpuCompiler::RunHloPasses(
  ...
  TF_ASSIGN_OR_RETURN(
      std::unique_ptr<toolchain::TargetMachine> jit_target_machine,
...
static void DumpModuleToFile(const toolchain::Module& llvm_module,
                             const toolchain::object::ObjectFile& obj_file,
                             const HloModule& hlo_module) {
  absl::string_view id = llvm_module.getModuleIdentifier();
  size_t pos = std::min(id.size(), 1 + kXlaModuleIdentifier.size());
...
static std::function<void(const toolchain::Module&, const toolchain::object::ObjectFile&)>
CreateOrcJITPostCompilationHook(const HloModule* hlo_module,
                                std::vector<ObjFileProto>* obj_files) {
  return [=](const toolchain::Module& llvm_module,
             const toolchain::object::ObjectFile& obj_file) {
    if (obj_files) {
...
static void RemoveUnusedSymbols(toolchain::Module& module) {
  toolchain::SmallVector<toolchain::GlobalVariable*> unused_globals;
  toolchain::SmallVector<toolchain::Function*> unused_functions;
...
static toolchain::orc::ThreadSafeModule CloneAsThreadSafeModule(
    int64_t part, std::unique_ptr<toolchain::Module> module) {
  TraceMe trace([&] {
  ...
  toolchain::SmallString<0> bc;
  toolchain::raw_svector_ostream bcos(bc);
  toolchain::WriteBitcodeToFile(*module, bcos);

  // Parse module back into its own LLVM context.
  auto clone_context = std::make_unique<toolchain::LLVMContext>();
  auto clone_module = toolchain::parseBitcodeFile(
      toolchain::MemoryBufferRef(
          toolchain::StringRef(bc.data(), bc.size()),
          absl::StrFormat("%s_part_%02d", kXlaModuleIdentifier, part)),
      *clone_context);

  return toolchain::orc::ThreadSafeModule(std::move(*clone_module),
...
static CompiledSymbolsPart CollectCompiledSymbolsPart(
    const IrEmitter2& ir_emitter, const toolchain::Module& module) {
  CompiledSymbolsPart syms;

  auto find_kernel =
      [&](toolchain::StringRef name) -> std::optional<IrEmitter2::KernelInfo> {
  ...
  auto find_comparator =
      [&](toolchain::StringRef name) -> std::optional<IrEmitter2::ComparatorInfo> {
...
static bool HasLargeConstants(toolchain::Module& module) {
  ...
  for (auto& g : module.globals()) {
    ...
    toolchain::Constant* initializer = g.getInitializer();
    if (auto* arr = toolchain::dyn_cast<toolchain::ArrayType>(initializer->getType())) {
...
static absl::StatusOr<std::unique_ptr<toolchain::Module>> ExtractKernelsFromModule(
    toolchain::Module* original_module,
    absl::flat_hash_set<toolchain::StringRef> kernels) {
  // Clone into a new module, only keeping definitions of the relevant kernels.
  auto should_clone_definition = [&kernels](const toolchain::GlobalValue* gv) {
    if (auto* func = toolchain::dyn_cast<toolchain::Function>(gv)) {
  ...
  std::unique_ptr<toolchain::Module> module =
      toolchain::CloneModule(*original_module, vmap, should_clone_definition);

  // Erase the cloned symbols from the original module.
  for (const auto& kernel_name : kernels) {
    toolchain::Function* to_be_removed = original_module->getFunction(kernel_name);
...
static void AddXlaBackendExtraOptionsAsModuleFlag(
    toolchain::Module* llvm_module, toolchain::StringRef backend_extra_options) {
  auto* options_mdstring =
      toolchain::MDString::get(llvm_module->getContext(), backend_extra_options);
...
CpuCompiler::CompileCpuExecutable(std::unique_ptr<HloModule> module) {
  ...
  auto llvm_module =
      std::make_unique<toolchain::Module>(kXlaModuleIdentifier, *llvm_context);
  ...
  if (module->config().debug_options().xla_cpu_use_thunk_runtime()) {
    ...
    using BackendOptions = toolchain::StringRef;
    using Kernel = toolchain::StringRef;
    ...
    int dylib_index = 0;
    auto add_jit_module = [&](std::unique_ptr<toolchain::Module> llvm_module_part) {
    ...
    if (num_default_parts > 1) {
      ...
      toolchain::SplitModule(*llvm_module, num_default_parts, add_jit_module,
      ...
      TF_CHECK_OK(jit_compiler.AddModule(toolchain::orc::ThreadSafeModule(
  ...
  TF_ASSIGN_OR_RETURN(toolchain::Function * entry_function,
  ...
  toolchain::orc::ThreadSafeModule thread_safe_module(std::move(llvm_module),
...
absl::StatusOr<std::unique_ptr<Executable>> CpuCompiler::RunBackend(
  ...
  MACHINA_MACHINA_XLA_SCOPED_LOGGING_TIMER(
...
CpuCompiler::CompileAheadOfTime(std::unique_ptr<HloModuleGroup> module_group,
  ...
  // We can pass just one toolchain::TargetOptions when we compile the LLVM module,
  ...
  toolchain::Triple triple(toolchain::Triple::normalize(options.triple()));
  std::string error;
  const toolchain::Target* target =
      toolchain::TargetRegistry::lookupTarget(triple.getTriple(), error);
  if (target == nullptr) {
    return Internal("TargetRegistry::lookupTarget failed: %s", error);
  }

  toolchain::Reloc::Model reloc_model = toolchain::Reloc::Static;
  toolchain::PICLevel::Level pic_level = toolchain::PICLevel::NotPIC;
  toolchain::PIELevel::Level pie_level = toolchain::PIELevel::Default;
  switch (options.relocation_model()) {
    case CpuAotCompilationOptions::RelocationModel::Static:
      reloc_model = toolchain::Reloc::Static;
      pic_level = toolchain::PICLevel::NotPIC;
      pie_level = toolchain::PIELevel::Default;
      break;
    case CpuAotCompilationOptions::RelocationModel::SmallPic:
      reloc_model = toolchain::Reloc::PIC_;
      pic_level = toolchain::PICLevel::SmallPIC;
      pie_level = toolchain::PIELevel::Default;
      break;
    case CpuAotCompilationOptions::RelocationModel::BigPic:
      reloc_model = toolchain::Reloc::PIC_;
      pic_level = toolchain::PICLevel::BigPIC;
      pie_level = toolchain::PIELevel::Default;
      break;
    case CpuAotCompilationOptions::RelocationModel::SmallPie:
      reloc_model = toolchain::Reloc::PIC_;
      pic_level = toolchain::PICLevel::SmallPIC;
      pie_level = toolchain::PIELevel::Small;
      break;
    case CpuAotCompilationOptions::RelocationModel::BigPie:
      reloc_model = toolchain::Reloc::PIC_;
      pic_level = toolchain::PICLevel::BigPIC;
      pie_level = toolchain::PIELevel::Large;
      break;
  }
...
CpuCompiler::CompileAheadOfTimeLegacy(
    ...
    const CpuAotCompilationOptions& aot_options, const toolchain::Triple& triple,
    const toolchain::PICLevel::Level& pic_level,
    const toolchain::PIELevel::Level& pie_level) {
  AliasInfo alias_info;
  TF_ASSIGN_OR_RETURN(
  ...
  if (pic_level != toolchain::PICLevel::NotPIC) {
    llvm_module->setPICLevel(pic_level);
  }
  if (pie_level != toolchain::PIELevel::Default) {
  ...
  TF_ASSIGN_OR_RETURN(
      toolchain::Function * entry_function,
  ...
  auto post_codegen_hook = [&](const toolchain::Module& llvm_module,
                               const toolchain::object::ObjectFile& obj_file) {
    if (!DumpingEnabledForHloModule(*module)) {
      return;
    }
  ...
  std::unique_ptr<toolchain::MemoryBuffer> object_file =
...
CpuCompiler::CompileAheadOfTimeThunks(
  ...
  auto copy_llvm_module_to_local_context =
      [&llvm_context](toolchain::Module& module) {
        ...
        // Parse module back into its own LLVM context.
        auto clone_module = toolchain::parseBitcodeFile(
            toolchain::MemoryBufferRef(toolchain::StringRef(bc.data(), bc.size()),
  ...
  toolchain::Linker linker(*llvm_module);
...
CpuExecutableAotCompilationResult::LoadExecutable(
  ...
  for (auto& obj_file : proto_.object_files()) {
    toolchain::StringRef data(obj_file.contents().data(),
                         obj_file.contents().size());
    TF_RETURN_IF_ERROR(object_loader.AddObjFile(
        toolchain::MemoryBuffer::getMemBuffer(data, obj_file.name())));
  }
...
absl::StatusOr<std::unique_ptr<AotCompilationResult>> CpuCompiler::Export(
  ...
  auto* cpu_executable = machina::down_cast<CpuExecutable*>(executable);
  if (!cpu_executable)
