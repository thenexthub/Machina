...
#include "machina/xla/service/cpu/ir_emitter.h"
...
#include "machina/xla/hlo/ir/collective_device_list.h"
...
#include "machina/xla/service/cpu/cpu_instruction_fusion.h"
#include "machina/xla/service/cpu/cpu_options.h"
#include "machina/xla/service/cpu/cpu_runtime.h"
#include "machina/xla/service/cpu/dot_op_emitter.h"
#include "machina/xla/service/cpu/elemental_ir_emitter.h"
#include "machina/xla/service/cpu/ir_emission_utils.h"
#include "machina/xla/service/cpu/ir_function.h"
#include "machina/xla/service/cpu/onednn_config.pb.h"
#include "machina/xla/service/cpu/parallel_loop_emitter.h"
...
#include "machina/xla/service/llvm_ir/dynamic_update_slice_util.h"
...
#include "machina/xla/tsl/lib/math/math_util.h"
...
IrEmitter::IrEmitter(mlir::MLIRContext* mlir_context,
    ...
    : assignment_(assignment),
      module_(llvm_module),
      arch_type_(toolchain::Triple(llvm_module->getTargetTriple()).getArch()),
...
void IrEmitter::EmitThreadLocalFunctionEpilogue(
  ...
  toolchain::Argument* out_parameter = compute_function()->result_arg();
...
absl::StatusOr<toolchain::Function*> IrEmitter::EmitComputation(
    const HloComputation* computation, absl::string_view function_name_prefix,
    bool is_top_level_computation,
    absl::Span<HloInstruction* const> instruction_order,
    bool allow_reassociation,
    absl::Span<const toolchain::Attribute::AttrKind> function_attributes) {
  ...
  bool use_rdtscp = arch_type_ == toolchain::Triple::ArchType::x86 ||
                    arch_type_ == toolchain::Triple::ArchType::x86_64;
  profiling_state_ = ProfilingState(use_rdtscp);

  tracing_state_.set_enabled(
      computation->parent()->config().cpu_traceme_enabled());

  toolchain::IRBuilderBase::FastMathFlagGuard guard(*b());
  toolchain::FastMathFlags flags = b()->getFastMathFlags();
  ...
  toolchain::Function* ir_function = compute_function()->function();

  for (toolchain::Attribute::AttrKind attr : function_attributes) {
...
void IrEmitter::InitializeIrFunction(const std::string& function_name) {
  ...
  toolchain::Function::LinkageTypes linkage =
      is_top_level_computation_ ? toolchain::GlobalValue::ExternalLinkage
...
toolchain::Constant* IrEmitter::EmitGlobalForLiteral(const Literal& literal) {
  toolchain::Constant* initializer =
      llvm_ir::ConvertLiteralToIrConstant(literal, module_);
  toolchain::GlobalVariable* result_global = new toolchain::GlobalVariable(
      /*Module=*/*module_,
  ...
  result_global->setAlignment(
      toolchain::Align(MinimumAlignmentForShape(literal.shape())));
  result_global->setUnnamedAddr(toolchain::GlobalVariable::UnnamedAddr::Global);
...
absl::Status IrEmitter::EmitConstantGlobals(
  ...
  for (const BufferAllocation& allocation : assignment_.Allocations()) {
    ...
    toolchain::Constant* global_for_const;
...
void IrEmitter::AttachAlignmentMetadataForLoad(toolchain::LoadInst* load,
                                               const Shape& shape) {
...
void IrEmitter::AttachDereferenceableMetadataForLoad(toolchain::LoadInst* load,
                                                     const Shape& shape) {
...
void IrEmitter::AttachInvariantLoadMetadataForLoad(toolchain::LoadInst* load) const {
  AttachInvariantLoadMetadataForLoad(load, hlo_module_config_);
}

/*static*/ void IrEmitter::AttachInvariantLoadMetadataForLoad(
    toolchain::LoadInst* load, const HloModuleConfig& config) {
  if (config.debug_options().xla_llvm_enable_invariant_load_metadata()) {
    load->setMetadata(toolchain::LLVMContext::MD_invariant_load,
                      toolchain::MDNode::get(load->getContext(), /*MDs=*/{}));
  }
}
...
absl::Status IrEmitter::HandleGetTupleElement(
  ...
  toolchain::Value* addr = EmitBufferPointer(slice, get_tuple_element->shape());
...
absl::Status IrEmitter::HandleInfeed(HloInstruction* instruction) {
  ...
  toolchain::Value* data_address = EmitBufferPointer(data_slice, data_shape);
  toolchain::Type* data_type = IrShapeType(data_shape);
  TF_ASSIGN_OR_RETURN(BufferAllocation::Slice token_slice,
                      assignment_.GetUniqueSlice(infeed, {1}));
  toolchain::Value* token_address = EmitBufferPointer(
  ...
  if (data_shape.IsTuple()) {
    ...
    // elements.
    std::vector<toolchain::Value*> tuple_element_addresses;
    for (int i = 0; i < data_shape.tuple_shapes().size(); ++i) {
      ...
      toolchain::Value* tuple_element_address =
...
absl::Status IrEmitter::EmitXfeedTransfer(XfeedKind kind, const Shape& shape,
                                          toolchain::Value* program_buffer_address) {
  ...
  toolchain::Value* acquired_pointer = EmitCallToFunc(
      acquire_func_name,
  ...
  if (kind == XfeedKind::kInfeed) {
    ...
    MemCpy(program_buffer_address, /*DstAlign=*/toolchain::Align(1),
           acquired_pointer,
           /*SrcAlign=*/toolchain::Align(1), length_32);
  } else {
    // Outfeed -- copy from the in-program address to the acquired buffer.
    MemCpy(acquired_pointer, /*DstAlign=*/toolchain::Align(1),
    ...
    if (emit_code_for_msan_) {
      ...
      const toolchain::DataLayout& dl = module_->getDataLayout();
      toolchain::Type* intptr_type = b()->getIntPtrTy(dl);
      EmitCallToFunc(
          "__msan_unpoison",
          {acquired_pointer, toolchain::ConstantInt::get(intptr_type, length)},
          b()->getVoidTy());
    }
  }
...
absl::Status IrEmitter::HandleOutfeed(HloInstruction* outfeed) {
  ...
  toolchain::Value* value = GetEmittedValueFor(operand);
  ...
  for (int i = 0; i < operand_shape.tuple_shapes().size(); ++i) {
    ...
    toolchain::Value* tuple_element = llvm_ir::EmitGetTupleElement(
...
absl::Status IrEmitter::HandleSort(HloInstruction* hlo) {
  ...
  std::vector<toolchain::Value*> destination_addresses(sort->operand_count());
  for (int64_t i = 0; i < sort->operand_count(); ++i) {
    ...
    if (destination_buffer != source_address) {
      ...
      MemCpy(destination_addresses[i],
             /*DstAlign=*/toolchain::Align(primitive_type_size), source_buffer,
             /*SrcAlign=*/toolchain::Align(primitive_type_size), size);
    }
  }
  ...
  toolchain::Value* values = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
      b()->getPtrTy(), b()->getInt32(sort->operand_count()), "cc_values_alloca",
      b());
  toolchain::Value* sizes = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
      b()->getInt32Ty(), b()->getInt32(sort->operand_count()),
      "cc_sizes_alloca", b());
  for (int64_t i = 0; i < sort->operand_count(); ++i) {
    toolchain::Value* slot_in_values_alloca =
        ConstInBoundsGEP1_32(b()->getPtrTy(), values, i);
    Store(destination_addresses[i], slot_in_values_alloca);
    toolchain::Value* slot_in_sizes_alloca =
        ConstInBoundsGEP1_32(b()->getInt32Ty(), sizes, i);
    toolchain::Value* size = b()->getInt32(ShapeUtil::ByteSizeOfPrimitiveType(
...
absl::Status IrEmitter::HandleTuple(HloInstruction* tuple) {
  ...
  toolchain::SmallVector<toolchain::Value*> base_ptrs;
...
absl::Status IrEmitter::HandleConvolution(HloInstruction* convolution) {
  ...
  if (allow_runtime_calls_ && PotentiallyImplementedAsEigenConvolution(
    ...
    if (LayoutUtil::IsMonotonicWithDim0Major(lhs_shape.layout()) &&
      ...
      toolchain::Value* lhs_address = GetEmittedValueFor(lhs);
      toolchain::Value* rhs_address = GetEmittedValueFor(rhs);
...
absl::Status IrEmitter::HandleFft(HloInstruction* fft) {
  ...
  toolchain::Value* operand_address = GetEmittedValueFor(operand);
...
absl::Status IrEmitter::HandleAllReduceSingleReplica(HloInstruction* crs) {
  ...
  std::vector<toolchain::Value*> operand_ptrs;
  for (int64_t i = 0; i < crs->operand_count(); ++i) {
    toolchain::Value* in_ptr = GetEmittedValueFor(crs->operand(i));
    ...
    MemCpy(operand_ptrs.back(), /*DstAlign=*/toolchain::Align(1), in_ptr,
           /*SrcAlign=*/toolchain::Align(1), ShapeUtil::ByteSizeOf(operand_shape));
  }
...
absl::Status IrEmitter::HandleAllReduceMultipleReplica(HloInstruction* crs) {
  ...
  toolchain::Value* replica_groups_v = b()->CreateGlobalStringPtr(replica_groups);

  bool is_tuple = crs->operand_count() > 1;
  std::vector<toolchain::Value*> input_buffer_ptrs;
  std::vector<toolchain::Value*> output_buffer_ptrs;
  if (is_tuple) {
...
absl::Status IrEmitter::HandleReduceScatter(HloInstruction* rs) {
  ...
  toolchain::Value* input_buffer = EmitBufferPointer(input_slice, shape);
  toolchain::Value* output_buffer = EmitBufferPointer(output_slice, shape);
...
absl::Status IrEmitter::HandleAllGather(HloInstruction* instruction) {
  ...
  toolchain::Value* output_buffer = EmitBufferPointer(out_slice, operand_shape);
  toolchain::Value* input_buffer = GetEmittedValueFor(op);
...
absl::Status IrEmitter::HandleCollectivePermute(HloInstruction* crs) {
  ...
  toolchain::Value* source_target_pairs_v =
...
absl::Status IrEmitter::HandlePartitionId(HloInstruction* hlo) {
  ...
  toolchain::Value* output_buffer = EmitBufferPointer(output_slice, hlo->shape());
...
IrEmitter::ReductionGenerator IrEmitter::MatchReductionGenerator(
  ...
  switch (root_instruction->opcode()) {
    ...
    case HloOpcode::kAdd:
      return [root_is_integral](toolchain::IRBuilderBase* b, toolchain::Value* lhs,
                                toolchain::Value* rhs) {
    ...
    case HloOpcode::kAnd:
      return [](toolchain::IRBuilderBase* b, toolchain::Value* lhs, toolchain::Value* rhs) {
    ...
    case HloOpcode::kMaximum:
      return [root_is_floating_point, root_is_signed, this](
                 toolchain::IRBuilderBase* b, toolchain::Value* lhs,
        ...
        return b->CreateSelect(
            b->CreateICmp(root_is_signed ? toolchain::ICmpInst::ICMP_SGE
    ...
    case HloOpcode::kMinimum:
      return [root_is_floating_point, root_is_signed, this](
        ...
        return b->CreateSelect(
            b->CreateICmp(root_is_signed ? toolchain::ICmpInst::ICMP_SLE
...
IrEmitter::ShardedVectorType IrEmitter::CreateShardedVectorType(
  ...
  toolchain::Type* element_ir_type =
  ...
  for (int i = 0, e = 1 + Log2Ceiling(element_count); i < e; i++) {
    ...
    if (current_size_fragment >= vector_register_size_in_elements) {
      auto vector_type = toolchain::VectorType::get(
    ...
    sharded_vector_type.push_back(
        toolchain::VectorType::get(element_ir_type, current_size_fragment, false));
  }
...
IrEmitter::EmitInnerLoopForVectorizedReduction(
    ...
    toolchain::Align element_alignment) {
  ...
  toolchain::Value* init_value_ssa =
      Load(IrShapeType(init_value->shape()), GetEmittedValueFor(init_value));

  for (toolchain::Value* accumulator_shard : accumulator) {
    toolchain::Value* initial_value;
    auto shard_type =
        toolchain::cast<toolchain::AllocaInst>(accumulator_shard)->getAllocatedType();
    if (auto vector_type = toolchain::dyn_cast<toolchain::VectorType>(shard_type)) {
      initial_value =
  ...
  toolchain::Value* input_address =
      arg_array.EmitArrayElementAddress(input_index, b());

  for (int i = 0; i < accumulator.size(); i++) {
    auto alloca = toolchain::cast<toolchain::AllocaInst>(accumulator[i]);
  ...
  for (auto accumulator_shard : accumulator) {
    auto alloca = toolchain::cast<toolchain::AllocaInst>(accumulator_shard);
...
void IrEmitter::EmitShardedVectorStore(
    toolchain::Value* store_address, const std::vector<toolchain::Value*>& value_to_store,
    toolchain::Align alignment, const llvm_ir::IrArray& containing_array) {
...
absl::StatusOr<bool> IrEmitter::EmitVectorizedReduce(
  ...
  toolchain::Align element_alignment(tsl::MathUtil::GCD<unsigned>(
      ShapeUtil::ByteSizeOfPrimitiveType(reduce->shape().element_type()),
  ...
  std::vector<toolchain::Value*> array_multi_index(
  ...
  if (toolchain::BasicBlock* innermost_body_bb =
  ...
  if (innermost_dimension_size >= vectorization_factor) {
    ...
    llvm_ir::IrArray::Index array_index(array_multi_index, reduce->shape(),
                                        b()->getInt64Ty());
    TF_ASSIGN_OR_RETURN(std::vector<toolchain::Value*> accumulator,
    ...
    toolchain::Value* output_address =
...
absl::Status IrEmitter::HandleSlice(HloInstruction* slice) {
  ...
  toolchain::SmallVector<int64_t> outer_dims;
  ...
  std::vector<toolchain::Value*> target_multi_index =
  ...
  std::replace(target_multi_index.begin(), target_multi_index.end(),
               static_cast<toolchain::Value*>(nullptr),
               static_cast<toolchain::Value*>(b()->getInt64(0)));
...
absl::Status IrEmitter::HandlePad(HloInstruction* pad,
  ...
  toolchain::Value* padding_value_addr = padding_value_array.EmitArrayElementAddress(
...
absl::Status IrEmitter::HandleCall(HloInstruction* call) {
  ...
  toolchain::Function* call_ir_function = FindOrDie(
  ...
  if (backend_config_or.ok() &&
    ...
    std::vector<toolchain::Value*> call_args = GetArrayFunctionCallArguments(
        {}, b(), computation->name(),
...
absl::Status IrEmitter::EmitSliceToDynamic(
  ...
  std::vector<toolchain::Value*> dynamic_dims;
  int32_t raw_data_size =
      ShapeUtil::ByteSizeOf(ShapeUtil::MakeStaticShape(hlo->shape()));
  toolchain::Value* dest_buffer = target_array.GetBasePointer();
  for (int64_t i = 1; i < hlo->operand_count(); ++i) {
    const int64_t dim_index = i - 1;
    toolchain::Value* source_buffer = source_arrays[i].GetBasePointer();
    toolchain::LoadInst* dyn_dim_size = Load(IrShapeType(hlo->operand(i)->shape()),
                                        source_buffer, "dyn_dim_size");

    toolchain::Value* metadata = b()->CreateConstInBoundsGEP1_32(
  ...
  auto loop_body_emitter =
    ...
    toolchain::Value* source_element =
        source_arrays[0].EmitReadArrayElement(array_index, b());
    toolchain::Value* linear_index = array_index.Linearize(dynamic_dims, b());
...
absl::Status IrEmitter::HandlePadToStatic(HloInstruction* hlo) {
  ...
  std::vector<toolchain::Value*> dynamic_dims;
  std::vector<toolchain::Value*> tuple_operand_ptrs;
  ...
  toolchain::Value* source_buffer = GetEmittedValueFor(hlo->operand(0));
...
std::vector<StackAlloca> IrEmitter::EmitOneDnnOperandsAlloca(
    HloInstruction* custom_call, toolchain::Value*& args_val, int& arg_indx) {
...
std::pair<toolchain::Value*, StackAlloca> IrEmitter::GetPtrAndAllocaFromBufferSlice(
    const BufferAllocation::Slice& slice, const Shape& shape) {
  toolchain::Value* slice_ptr = EmitBufferPointer(slice, shape);
  toolchain::Type* type = IrShapeType(shape);
...
absl::Status IrEmitter::HandleOneDnnMatMulCalls(
  ...
  toolchain::Type* i64_type = b()->getInt64Ty();
  toolchain::Type* ptr_type = b()->getPtrTy();
  toolchain::ArrayType* ptr_array_type = toolchain::ArrayType::get(ptr_type, nargs);
  toolchain::Value* args_val = toolchain::UndefValue::get(ptr_array_type);

  // Insert nargs.
  toolchain::Value* nargs_val = b()->getInt64(nargs);
  toolchain::Value* nargs_ptr =
  ...
  toolchain::Value* run_opts_val = GetExecutableRunOptionsArgument();
  ...
  toolchain::Value* matmul_config_val =
  ...
  std::vector<toolchain::Value*> fn_call_args;
  ...
  if (use_scratchpad) {
    toolchain::Value* result_slice_ptr;
    toolchain::Value* scratch_slice_ptr;
    ...
    fn_call_args.push_back(result_stack_alloca.value);
    fn_call_args.push_back(toolchain::ConstantPointerNull::get(b()->getPtrTy()));
  }
...
absl::Status IrEmitter::HandleOneDnnConvolution(HloInstruction* custom_call) {
  ...
  toolchain::Value* conv_config_val =
      b()->CreateGlobalStringPtr(llvm_ir::AsStringRef(str_config));
  ...
  toolchain::Value* args_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
...
absl::Status IrEmitter::HandleOneDnnSoftmax(HloInstruction* custom_call) {
  ...
  toolchain::Value* softmax_config_val =
      b()->CreateGlobalStringPtr(llvm_ir::AsStringRef(str_config));
...
absl::Status IrEmitter::HandleCustomCall(HloInstruction* custom_call) {
  ...
  std::vector<toolchain::Value*> operand_values;
  ...
  toolchain::AllocaInst* operands_alloca =
  ...
  for (int64_t i = 0; i < operand_values.size(); ++i) {
    toolchain::Value* slot_in_operands_alloca =
  ...
  std::vector<toolchain::Value*> tuple_ptrs;
  ...
  switch (typed_custom_call->api_version()) {
    ...
    case CustomCallApiVersion::API_VERSION_TYPED_FFI: {
      ...
      toolchain::AllocaInst* results_alloca =
          llvm_ir::EmitAllocaAtFunctionEntryWithCount(
      ...
      if (emit_code_for_msan_) {
        ...
        EmitCallToFunc(
            "__msan_unpoison",
            {results_alloca,
             toolchain::ConstantInt::get(intptr_type,
                                    *results_alloca->getAllocationSize(dl))},
            b()->getVoidTy());
      }
      for (int i = 0; i < buffer_ptrs.size(); ++i) {
        toolchain::Value* tuple_slot_in_results_alloca =
...
absl::Status IrEmitter::HandleWhile(HloInstruction* xla_while) {
  ...
  toolchain::BasicBlock* header_bb = toolchain::BasicBlock::Create(
  ...
  toolchain::Value* while_predicate = ICmpNE(
      Load(IrShapeType(
  ...
  toolchain::BasicBlock* body_bb =
      toolchain::BasicBlock::Create(module_->getContext(), IrName(xla_while, "body"),
                               compute_function()->function());
  toolchain::BasicBlock* exit_bb = toolchain::BasicBlock::Create(
  ...
  toolchain::Function* llvm_fn = compute_function()->function();
...
absl::Status EmitFastConcatenate(
    const HloInstruction* instr,
    absl::Span<const llvm_ir::IrArray> source_arrays,
    const llvm_ir::IrArray& target_array, toolchain::Module* module,
  ...
  absl::c_replace(target_multi_index, static_cast<toolchain::Value*>(nullptr),
                  static_cast<toolchain::Value*>(b.getInt64(0)));
  ...
  toolchain::Value* target_region_begin =
  ...
  for (int64_t i = 0; i < source_arrays.size(); ++i) {
    ...
    llvm_ir::IrArray::Index source_index(target_multi_index, input_shape,
                                         b.getInt64Ty());
    toolchain::Value* copy_source_address =
        source_array.EmitArrayElementAddress(source_index, &b, "src_addr");

    toolchain::Value* copy_target_address =
...
toolchain::Value* IrEmitter::EmitPrintf(absl::string_view fmt,
                                   absl::Span<toolchain::Value* const> arguments) {
  std::vector<toolchain::Value*> call_args;
  ...
  return b()->CreateCall(
      b()->GetInsertBlock()->getParent()->getParent()->getOrInsertFunction(
          "printf",
          toolchain::FunctionType::get(b()->getInt32Ty(), {b()->getPtrTy()},
                                  /*isVarArg=*/true)),
      call_args);
}

toolchain::Value* IrEmitter::EmitPrintfToStderr(
    absl::string_view fmt, absl::Span<toolchain::Value* const> arguments) {
  std::vector<toolchain::Value*> call_args;
  call_args.push_back(b()->CreateGlobalStringPtr(llvm_ir::AsStringRef(fmt)));
  absl::c_copy(arguments, std::back_inserter(call_args));
  return b()->CreateCall(
...
toolchain::Value* IrEmitter::EmitCallToFunc(
    std::string func_name, const std::vector<toolchain::Value*>& arguments,
    toolchain::Type* return_type, bool does_not_throw, bool only_accesses_arg_memory,
    bool only_accesses_inaccessible_mem_or_arg_mem) {
  std::vector<toolchain::Type*> types;
  types.reserve(arguments.size());
  absl::c_transform(arguments, std::back_inserter(types),
                    [&](toolchain::Value* val) { return val->getType(); });
  toolchain::FunctionType* func_type =
      toolchain::FunctionType::get(return_type, types, /*isVarArg=*/false);
  auto func = toolchain::dyn_cast<toolchain::Function>(
      module_->getOrInsertFunction(func_name, func_type).getCallee());
  func->setCallingConv(toolchain::CallingConv::C);
  if (does_not_throw) {
...
static EncodedInfo StoreEncodedTypes(absl::string_view alloca_name,
  ...
  toolchain::AllocaInst* types_alloca = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
  ...
  auto store_type = [&](const Shape& shape, const ShapeIndex& index) {
    ...
    toolchain::Value* slot_in_types_alloca = ir.CreateConstInBoundsGEP1_32(
...
static EncodedInfo StoreEncodedShapes(absl::string_view alloca_name,
  ...
  toolchain::AllocaInst* shapes_alloca = llvm_ir::EmitAllocaAtFunctionEntryWithCount(
  ...
  auto store_shape = [&](const Shape& shape, const ShapeIndex& index) {
    ...
    toolchain::Value* alloca_slot = ir.CreateConstInBoundsGEP1_64(
...
toolchain::Value* IrEmitter::EmitCallToFfi(HloCustomCallInstruction* custom_call,
                                      toolchain::AllocaInst* results_alloca,
                                      toolchain::AllocaInst* operands_alloca) {
...
void IrEmitter::EmitTransferElements(toolchain::Value* target, toolchain::Value* source,
...
void EmitTransferElements(toolchain::Value* target, toolchain::Value* source,
                          int64_t element_count, PrimitiveType primitive_type,
                          const llvm_ir::IrArray& target_array,
                          const llvm_ir::IrArray& source_array,
                          toolchain::Module* module, toolchain::IRBuilderBase& b) {
  unsigned primitive_type_size =
      ShapeUtil::ByteSizeOfPrimitiveType(primitive_type);
  toolchain::Align element_alignment(tsl::MathUtil::GCD<unsigned>(
      primitive_type_size,
      ::xla::cpu::MinimumAlignmentForPrimitiveType(primitive_type)));
  toolchain::Type* primitive_llvm_type =
  ...
  if (element_count == 1) {
    ...
    auto* memcpy_instruction = b.CreateMemCpy(
        target, /*DstAlign=*/toolchain::Align(element_alignment), source,
        /*SrcAlign=*/toolchain::Align(element_alignment),
    ...
    std::map<int, toolchain::MDNode*> merged_metadata =
...
absl::Status IrEmitter::HandleConditional(HloInstruction* conditional) {
  ...
  //     break;
  // }
  toolchain::LoadInst* branch_index_value = Load(
      GetIrArrayFor(branch_index).GetBasePointeeType(),
  ...
  b()->SetInsertPoint(case_block);
  toolchain::SwitchInst* case_inst =
...
absl::Status IrEmitter::HandleRngGetAndUpdateState(HloInstruction* rng_state) {
  ...
  toolchain::Value* old_state = llvm_ir::RngGetAndUpdateState(
      Cast<HloRngGetAndUpdateStateInstruction>(rng_state)->delta(), module_,
      b());

  TF_RETURN_IF_ERROR(EmitTargetAddressForOp(rng_state));
  toolchain::Value* address = GetEmittedValueFor(rng_state);

  // The buffer has an array type while the value has a i128. Cast the
  // buffer to i128 type to store the value.
  toolchain::StoreInst* store = Store(old_state, address);
  store->setAlignment(toolchain::Align(IrEmitter::MinimumAlignmentForPrimitiveType(
...
absl::Status IrEmitter::FinishVisit(HloInstruction* root) {
  ...
  auto record_complete_computation = [&](toolchain::Value* prof_counter) {
    if (prof_counter) {
...
template <typename T>
toolchain::Value* IrEmitter::GetProfileCounterCommon(
    const T& hlo,
...
toolchain::Value* IrEmitter::GetProfileCounterFor(
...
void IrEmitter::ProfilingState::UpdateProfileCounter(toolchain::IRBuilderBase* b,
                                                     toolchain::Value* prof_counter,
                                                     toolchain::Value* cycle_end,
                                                     toolchain::Value* cycle_start) {
  auto* cycle_diff = b->CreateSub(cycle_end, cycle_start);
  toolchain::LoadInst* old_cycle_count = b->CreateLoad(
      toolchain::cast<toolchain::GetElementPtrInst>(prof_counter)->getSourceElementType(),
...
toolchain::Value* IrEmitter::ProfilingState::ReadCycleCounter(
    toolchain::IRBuilderBase* b) {
  toolchain::Module* module = b->GetInsertBlock()->getModule();
  if (!use_rdtscp_) {
    toolchain::Function* func_llvm_readcyclecounter =
        toolchain::Intrinsic::getOrInsertDeclaration(
            module, toolchain::Intrinsic::readcyclecounter);
    return b->CreateCall(func_llvm_readcyclecounter);
  }
  toolchain::Function* func_llvm_x86_rdtscp =
      toolchain::Intrinsic::getOrInsertDeclaration(module,
                                              toolchain::Intrinsic::x86_rdtscp);
  toolchain::Value* rdtscp_call = b->CreateCall(func_llvm_x86_rdtscp);
  return b->CreateExtractValue(rdtscp_call, {0});
}

void IrEmitter::ProfilingState::RecordCycleStart(toolchain::IRBuilderBase* b,
                                                 HloInstruction* hlo) {
...
void IrEmitter::ProfilingState::RecordCycleDelta(toolchain::IRBuilderBase* b,
                                                 HloInstruction* hlo,
                                                 toolchain::Value* prof_counter) {
...
void IrEmitter::ProfilingState::RecordCompleteComputation(
    toolchain::IRBuilderBase* b, toolchain::Value* prof_counter) {
...
void IrEmitter::TracingState::EmitTracingStart(toolchain::IRBuilderBase* b,
                                               HloInstruction* hlo,
                                               toolchain::Value* run_options) {
  if (!enabled_) {
    return;
  }

  toolchain::Type* void_ptr_type = b->getPtrTy();
  toolchain::FunctionType* fn_type = toolchain::FunctionType::get(
      b->getInt64Ty(),
      {void_ptr_type, void_ptr_type, void_ptr_type, b->getInt64Ty()},
      /*isVarArg=*/false);

  toolchain::Function* function = b->GetInsertBlock()->getParent();
  toolchain::Module* module = function->getParent();
  const char* fn_name = runtime::kTracingStartSymbolName;
  toolchain::FunctionCallee trace_func =
      module->getOrInsertFunction(fn_name, fn_type);
  if (auto* fn = toolchain::dyn_cast<toolchain::Function>(trace_func.getCallee())) {
    fn->setCallingConv(toolchain::CallingConv::C);
    fn->setDoesNotThrow();
...
void IrEmitter::TracingState::EmitTracingEnd(toolchain::IRBuilderBase* b,
                                             HloInstruction* hlo,
                                             toolchain::Value* run_options) {
  if (!enabled_) {
    return;
  ...
  toolchain::FunctionType* fn_type =
      toolchain::FunctionType::get(b->getVoidTy(), {b->getPtrTy(), b->getInt64Ty()},
                              /*isVarArg=*/false);
...
llvm_ir::IrArray IrEmitter::GetIrArrayFor(const HloInstruction* hlo) {
  toolchain::Value* value_for_op = GetEmittedValueFor(hlo);

  toolchain::Type* ir_type = IrShapeType(hlo->shape());
...
toolchain::Value* IrEmitter::GetEmittedValueFor(const HloInstruction* hlo) {
...
toolchain::Type* IrEmitter::IrShapeType(const Shape& shape) {
  return llvm_ir::ShapeToIrType(shape, module_->getContext());
}

toolchain::Value* IrEmitter::GetProfileCountersArgument() {
  return compute_function()->profile_counters_arg();
}

toolchain::Value* IrEmitter::GetStatusArgument() {
  return compute_function()->status_arg();
}

toolchain::Value* IrEmitter::GetBufferTableArgument() {
  return compute_function()->buffer_table_arg();
}

toolchain::Value* IrEmitter::GetExecutableRunOptionsArgument() {
  return compute_function()->exec_run_options_arg();
}

toolchain::BasicBlock* IrEmitter::GetReturnBlock() {
...
toolchain::Value* IrEmitter::EmitThreadLocalBufferPointer(
    const BufferAllocation::Slice& slice, const Shape& target_shape) {
  const BufferAllocation& allocation = *slice.allocation();
  toolchain::Value* tempbuf_address = [&]() -> toolchain::Value* {
    auto param_it =
    ...
    if (param_it != computation_parameter_allocations_.end()) {
      ...
      toolchain::Value* params = compute_function()->parameters_arg();
      toolchain::Value* param_address_offset = llvm_ir::EmitBufferIndexingGEP(
          params, b()->getPtrTy(), param_number, b());
      toolchain::LoadInst* param_address_untyped =
    ...
    std::pair<toolchain::Function*, BufferAllocation::Slice> key = {
        compute_function()->function(), slice};
    auto buf_it = thread_local_buffers_.find(key);
    if (buf_it == thread_local_buffers_.end()) {
      toolchain::Value* buffer = llvm_ir::EmitAllocaAtFunctionEntry(
...
toolchain::Value* IrEmitter::EmitGlobalBufferPointer(
    const BufferAllocation::Slice& slice, const Shape& target_shape) {
  const BufferAllocation& allocation = *slice.allocation();
  ...
  toolchain::Value* tempbuf_address_ptr = llvm_ir::EmitBufferIndexingGEP(
      GetBufferTableArgument(), b()->getPtrTy(), index, b());
  toolchain::LoadInst* tempbuf_address_base =
  ...
  toolchain::Value* tempbuf_address_untyped = tempbuf_address_base;
...
toolchain::Value* IrEmitter::EmitBufferPointer(const BufferAllocation::Slice& slice,
...
absl::Status IrEmitter::EmitTargetAddressForOp(const HloInstruction* op) {
  ...
  toolchain::Value* addr = EmitBufferPointer(slice, target_shape);
...
absl::Status IrEmitter::EmitTargetElementLoop(
  ...
  if (target_shape.IsTuple() &&
    ...
    for (int64_t i = 0; i < ShapeUtil::TupleElementCount(target_shape); ++i) {
      ...
      toolchain::Value* op_target_address = EmitBufferPointer(slice, element_shape);
      toolchain::Type* op_target_type = IrShapeType(element_shape);
    ...
    if (ShouldEmitParallelLoopFor(*target_op)) {
      ...
      std::vector<std::pair<toolchain::Value*, toolchain::Value*>> dynamic_loop_bounds =
...
absl::Status IrEmitter::EmitMemcpy(const HloInstruction& source,
  ...
  toolchain::Value* source_value = GetEmittedValueFor(&source);
  toolchain::Value* destination_value = GetEmittedValueFor(&destination);
  int64_t source_size = ByteSizeOf(source.shape());
  // TODO(b/63762267): Be more aggressive about specifying alignment.
  MemCpy(destination_value, /*DstAlign=*/toolchain::Align(1), source_value,
         /*SrcAlign=*/toolchain::Align(1), source_size);
  return absl::OkStatus();
}
...
std::vector<toolchain::Value*> IrEmitter::EmitThreadLocalCall(
  ...
  std::vector<toolchain::Value*> parameter_addrs;
  for (toolchain::Value* parameter : parameters) {
    CHECK(!parameter->getType()->isPointerTy());
    toolchain::Value* parameter_addr = llvm_ir::EmitAllocaAtFunctionEntry(
        parameter->getType(), "arg_addr", b());
    Store(parameter, parameter_addr);
    parameter_addrs.push_back(parameter_addr);
  }

  toolchain::Type* return_value_buffer_type =
      llvm_ir::ShapeToIrType(return_shape, module_->getContext());
  ...
  int retval_alignment =
      is_scalar_return
          ...
          : 0;
  toolchain::AllocaInst* return_value_buffer = llvm_ir::EmitAllocaAtFunctionEntry(
  ...
  toolchain::Value* null_ptr = toolchain::Constant::getNullValue(b()->getPtrTy());
  ...
  for (toolchain::Value* addr : allocas_for_returned_scalars) {
    returned_scalars.push_back(
        Load(toolchain::cast<toolchain::AllocaInst>(addr)->getAllocatedType(), addr));
  }
...
void IrEmitter::EmitGlobalCall(const HloComputation& callee,
  ...
  Call(FindOrDie(emitted_functions_,
       ...
       GetArrayFunctionCallArguments(
           ...
           /*return_value_buffer=*/
           toolchain::Constant::getNullValue(b()->getPtrTy()),
           /*exec_run_options_arg=*/GetExecutableRunOptionsArgument(),
           /*buffer_table_arg=*/GetBufferTableArgument(),
           /*status_arg=*/GetStatusArgument(),
           /*profile_counters_arg=*/GetProfileCountersArgument()));
...
CpuElementalIrEmitter IrEmitter::ElementalIrEmmiterFactory() {
  auto thread_local_call_fn = [this](const HloComputation& callee,
                                     absl::Span<toolchain::Value* const> parameters,
...
absl::StatusOr<toolchain::Function*> IrEmitter::EmitNestedComputation(
  ...
  return EmitComputation(
      ...
      /*function_attributes=*/{toolchain::Attribute::AlwaysInline});
}
