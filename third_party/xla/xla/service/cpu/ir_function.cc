...
static std::vector<toolchain::Type*> GetComputeFunctionParams(
    toolchain::Module* llvm_module, const int64_t num_dynamic_loop_bounds) {
  toolchain::Type* ptr_type =
      toolchain::PointerType::getUnqual(llvm_module->getContext());
  std::vector<toolchain::Type*> compute_function_params(5, ptr_type);
...
IrFunction::IrFunction(const std::string& function_name,
                       ...
                       toolchain::Module* llvm_module, toolchain::IRBuilderBase* b,
...
IrFunction::IrFunction(toolchain::IRBuilderBase* b, toolchain::Module* llvm_module,
                       int64_t num_dynamic_loop_bounds,
...
void IrFunction::Initialize(const std::string& function_name,
  ...
  toolchain::FunctionType* function_type = toolchain::FunctionType::get(
      /*Result=*/toolchain::Type::getVoidTy(llvm_module_->getContext()),
      /*Params=*/
  ...
  toolchain::Function::arg_iterator arg_iter = function_->arg_begin();
  ...
  // disjoint objects.
  toolchain::Argument* retval = result_arg();
  for (toolchain::Argument& argument : function_->args()) {
    // However, the return buffer aliases the temporaries and thus cannot be
    // marked noalias.
    if (&argument == retval) {
      continue;
    }
    function_->addParamAttr(argument.getArgNo(), toolchain::Attribute::NoAlias);
  }

  return_block_ =
      toolchain::BasicBlock::Create(/*Context=*/llvm_module_->getContext(),
                               /*Name=*/"return", /*Parent=*/function_);

  b_->SetInsertPoint(return_block_);
  b_->CreateRetVoid();

  b_->SetInsertPoint(toolchain::BasicBlock::Create(
...
toolchain::Value* EncodeArrayFunctionArguments(
  ...
  toolchain::Value* arguments_buffer;
  if (arguments.empty()) {
    arguments_buffer = toolchain::Constant::getNullValue(b->getPtrTy());
  } else {
    ...
    for (size_t i = 0; i < arguments.size(); i++) {
      toolchain::Value* slot_in_param_addresses =
...
std::vector<toolchain::Value*> GetArrayFunctionCallArguments(
  ...
  toolchain::Value* parameter_addresses_buffer =
      EncodeArrayFunctionArguments(parameter_addresses, name, b);

  return std::vector<toolchain::Value*>{
...
absl::Status EmitCallToParallelForkJoin(
  ...
  std::vector<toolchain::Type*> compute_function_params =
  ...
  compute_function_params.push_back(
      toolchain::PointerType::get(module->getContext(), 0));
  ...
  toolchain::FunctionType* fork_join_type = toolchain::FunctionType::get(
      /*Result=*/toolchain::Type::getVoidTy(module->getContext()),
      /*Params=*/compute_function_params,
      /*isVarArg=*/false);

  toolchain::Function* fork_join_func = toolchain::dyn_cast<toolchain::Function>(
      module
          ->getOrInsertFunction(runtime::kParallelForkJoinSymbolName,
                                fork_join_type)
          .getCallee());
  fork_join_func->setCallingConv(toolchain::CallingConv::C);
  fork_join_func->setDoesNotThrow();

  // Add common compute function arguments.
  std::vector<toolchain::Value*> fork_join_arguments(arguments);
  ...
  // Store dimension partition values as toolchain constants in 'partitions'.
  // See comments in runtime_fork_join.cc for array layout description.
  std::vector<toolchain::Constant*> partitions(partition_array_size);
  ...
  toolchain::ArrayType* partitions_array_type =
      toolchain::ArrayType::get(b->getInt64Ty(), partition_array_size);
  toolchain::Constant* partitions_array =
      toolchain::ConstantArray::get(partitions_array_type, partitions);
  toolchain::GlobalVariable* global_partitions_array = new toolchain::GlobalVariable(
      /*M=*/*module,
