...
std::optional<int64_t> GetMetadataInt(toolchain::Metadata* absl_nullable value) {
  if (value == nullptr) {
    return std::nullopt;
  }
  auto* cam = toolchain::dyn_cast<toolchain::ConstantAsMetadata>(value);
  if (cam == nullptr) {
    return std::nullopt;
  }
  auto* c = toolchain::dyn_cast<toolchain::ConstantInt>(cam->getValue());
  if (c == nullptr) {
    return std::nullopt;
  }
...
std::optional<std::string> GetMetadataString(
    toolchain::Metadata* absl_nullable value) {
  if (value == nullptr) {
    return std::nullopt;
  }
  auto* md_string = toolchain::dyn_cast<toolchain::MDString>(value);
...
std::optional<int64_t> GetXlaDylibIndex(const toolchain::Module& llvm_module) {
  toolchain::Metadata* md = llvm_module.getModuleFlag("xla_dylib_index");
...
TEST_F(CpuCompilerInternalsTest, DylibWithThunks) {
  ...
  int64_t max_seen = -1;
  auto pre_opt_hook = [&](const toolchain::Module& llvm_module) {
