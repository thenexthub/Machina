...
#include "machina/xla/service/cpu/tests/cpu_codegen_test.h"
...
TEST_F(CpuNoAliasTest, Concat) {
  ...
  toolchain::Module ir_module("test", context);
  toolchain::Function* func = toolchain::dyn_cast<toolchain::Function>(
      ir_module.getOrInsertFunction("test_fn", toolchain::Type::getVoidTy(context))
          .getCallee());
  toolchain::BasicBlock* bb = toolchain::BasicBlock::Create(context, "body", func);
  toolchain::IRBuilder<> b(bb);
  auto* zero = toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(context), 0);

  toolchain::ArrayType* array2d_type = toolchain::ArrayType::get(
      toolchain::ArrayType::get(toolchain::Type::getFloatTy(context), 100), 100);

  {
    auto param_x_val = toolchain::cast<toolchain::GlobalVariable>(
  ...
  {
    auto concat1_val = toolchain::cast<toolchain::GlobalVariable>(
  ...
  {
    auto concat2_val = toolchain::cast<toolchain::GlobalVariable>(
