...
ProfilingTestSpec CpuProfilingTestCases[] = {
    ProfilingTestSpec{
        kTriple_x86_64,
        R"(CHECK: [[START:%[^ ]*]] = tail call { i64, i32 } @toolchain.x86.rdtscp
           CHECK: extractvalue { i64, i32 } [[START]], 0
           CHECK: [[STOP:%[^ ]*]] = tail call { i64, i32 } @toolchain.x86.rdtscp
           CHECK: extractvalue { i64, i32 } [[STOP]], 0)"},
    ProfilingTestSpec{kTriple_android_arm,
                      R"(CHECK: call i64 @toolchain.readcyclecounter
                         CHECK: call i64 @toolchain.readcyclecounter)"},
};
