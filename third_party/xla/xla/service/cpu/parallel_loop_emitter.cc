...
ParallelLoopEmitter::ParallelLoopEmitter(
    ...
    const DynamicLoopBounds* dynamic_loop_bounds, toolchain::IRBuilderBase* b)
...
ParallelLoopEmitter::EmitIndexAndSetExitBasicBlock(absl::string_view loop_name,
  ...
  std::vector<toolchain::Value*> array_multi_index(num_dims);
  ...
  for (int i = LayoutUtil::MinorToMajor(shape_).size() - 1; i >= 0; --i) {
    ...
    if (bounds_index < dynamic_loop_bounds_->size()) {
      ...
      toolchain::Value* start_index = (*dynamic_loop_bounds_)[bounds_index].first;
      toolchain::Value* end_index = (*dynamic_loop_bounds_)[bounds_index].second;
