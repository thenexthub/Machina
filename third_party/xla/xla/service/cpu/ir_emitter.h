...
#ifndef MACHINA_MACHINA_XLA_SERVICE_CPU_IR_EMITTER_H_
#define MACHINA_MACHINA_XLA_SERVICE_CPU_IR_EMITTER_H_
...
#include "machina/xla/service/llvm_ir/alias_analysis.h"
...
class IrEmitter : public DfsHloVisitorWithDefault,
 ...
 public:
  ...
  IrEmitter(mlir::MLIRContext* mlir_context, const HloModule& hlo_module,
            const BufferAssignment& assignment, toolchain::Module* llvm_module,
  ...
  absl::StatusOr<toolchain::Function*> EmitComputation(
  ...
  toolchain::IRBuilderBase* b() { return current_builder_; }
  const toolchain::IRBuilderBase* b() const { return current_builder_; }
  // builder() is for IrBuilderMixin.
  toolchain::IRBuilderBase* builder() { return current_builder_; }
  const toolchain::IRBuilderBase* builder() const { return current_builder_; }
  ...
  // Used by IrEmitter2
  void PushComputeFunction(toolchain::IRBuilderBase* b, toolchain::Module* llvm_module,
                           int64_t num_dynamic_loop_bounds,
  ...
  // the runtime.
  std::vector<toolchain::Value*> EmitThreadLocalCall(
  ...
  class IRBuilderGuard {
   public:
    ...
    explicit IRBuilderGuard(IrEmitter* ir_emitter, toolchain::IRBuilderBase* builder)
   ...
   private:
    ...
    toolchain::IRBuilderBase* original_builder_ = nullptr;
  };

  // WithBuilder is a convenience function that creates and returns a
  // IRBuilderGuard for the current IrEmitter.
  [[nodiscard]] IRBuilderGuard WithBuilder(toolchain::IRBuilderBase& builder) {
 ...
 private:
  ...
  std::vector<StackAlloca> EmitOneDnnOperandsAlloca(HloInstruction* custom_call,
                                                    toolchain::Value*& args_val,
                                                    int& arg_indx);
  std::pair<toolchain::Value*, StackAlloca> GetPtrAndAllocaFromBufferSlice(
  ...
  // computation.
  toolchain::Value* GetProfileCounterFor(const HloInstruction& instruction);
  toolchain::Value* GetProfileCounterFor(const HloComputation& computation);

  // Helper function template for the implementation of the above two functions.
  template <typename T>
  toolchain::Value* GetProfileCounterCommon(
      const T& hlo,
  ...
  toolchain::Value* GetEmittedValueFor(const HloInstruction* hlo);
  ...
  toolchain::Type* IrShapeType(const Shape& shape);

  // Get the toolchain::Value* that represents the "prof_counters" argument of the
  // computation function being emitted by this emitter.
  toolchain::Value* GetProfileCountersArgument();

  // Get the toolchain::Value* that represents the "status" argument of the
  // computation function being emitted by this emitter.
  toolchain::Value* GetStatusArgument();
  ...
  toolchain::Value* GetExecutableRunOptionsArgument();

  // Get the toolchain::Value* that represents the "buffer_table" argument of the
  // computation function being emitted by this emitter.
  toolchain::Value* GetBufferTableArgument();

  // Get the toolchain::BasicBlock that contains the return instruction.
  toolchain::BasicBlock* GetReturnBlock();
  ...
  toolchain::Value* EmitGlobalBufferPointer(const BufferAllocation::Slice& slice,
                                       const Shape& target_shape);

  // Helper for EmitBufferPointer.
  toolchain::Value* EmitThreadLocalBufferPointer(
      const BufferAllocation::Slice& slice, const Shape& target_shape);

  // Emits code that computes the address of the given buffer allocation slice.
  toolchain::Value* EmitBufferPointer(const BufferAllocation::Slice& slice,
                                 const Shape& target_shape);

  // Similar to EmitThreadLocal, yet assumes that the function returns a scalar.
  toolchain::Value* EmitScalarReturningThreadLocalCall(
      const HloComputation& callee, absl::Span<toolchain::Value* const> parameters,
  ...
  // its result.
  toolchain::Value* GetBufferForGlobalCallReturnValue(const HloComputation& callee);
  ...
  toolchain::Constant* CreateInitializerForConstantArray(
      const std::vector<toolchain::Constant*>& array_elements, const Shape& shape,
  ...
  // scalar and vector toolchain::Value s.
  //
  ...
  using ShardedVector = std::vector<toolchain::Value*>;

  // A sharded vector type is the element-wise toolchain::Type's of some
  // ShardedVector.
  using ShardedVectorType = std::vector<toolchain::Type*>;
  ...
  // "store_address".
  void EmitShardedVectorStore(toolchain::Value* store_address,
                              const ShardedVector& value_to_store,
                              toolchain::Align alignment,
                              const llvm_ir::IrArray& containing_array);

  using ReductionGenerator = std ::function<toolchain::Value*(
      toolchain::IRBuilderBase*, toolchain::Value*, toolchain::Value*)>;
  ...
  toolchain::Value* EmitCallToFfi(HloCustomCallInstruction* custom_call,
  ...
  toolchain::Triple::ArchType arch_type_;
  ...
  absl::flat_hash_map<ComputationToEmit, toolchain::Function*> emitted_functions_;

  // Map containing all previously emitted thread-local temporary buffers.
  std::map<std::pair<toolchain::Function*, BufferAllocation::Slice>, toolchain::Value*>
      thread_local_buffers_;

  // The following fields track the IR emission state. According to LLVM memory
  // management rules, their memory is owned by the module (Note that IrFunction
  // creates the encapsulated toolchain::Function s.t. it is added to the toolchain
  ...
  absl::flat_hash_map<const HloInstruction*, toolchain::Value*> emitted_value_;
  ...
  class ProfilingState {
   public:
    ...
    void RecordCycleStart(toolchain::IRBuilderBase* b, HloInstruction* hlo);
    // Record the number of cycles it took for an HLO to execute.
    void RecordCycleDelta(toolchain::IRBuilderBase* b, HloInstruction* hlo,
                          toolchain::Value* prof_counter);
    // Record the number of cycles it took for the entire computation to
    // execute.
    void RecordCompleteComputation(toolchain::IRBuilderBase* b,
                                   toolchain::Value* prof_counter);

    // Convenience function to generate a call to an intrinsic which reads the
    // CPU cycle counter.
    toolchain::Value* ReadCycleCounter(toolchain::IRBuilderBase* b);

    // Store the cycle counter delta to the per-HLO profile counter.
    void UpdateProfileCounter(toolchain::IRBuilderBase* b, toolchain::Value* prof_counter,
                              toolchain::Value* cycle_end, toolchain::Value* cycle_start);
   ...
   private:
    ...
    toolchain::Value* first_read_cycle_start_ = nullptr;

    // The last read cycle counter in the program.
    toolchain::Value* last_read_cycle_end_ = nullptr;

    // Maps HLOs to the value the cycle counter contained right before the HLO
    // began to execute.
    absl::flat_hash_map<const HloInstruction*, toolchain::Value*> cycle_starts_;
  };
  ...
  class TracingState {
   public:
    ...
    void EmitTracingStart(toolchain::IRBuilderBase* b, HloInstruction* hlo,
                          toolchain::Value* run_options);
    void EmitTracingEnd(toolchain::IRBuilderBase* b, HloInstruction* hlo,
                        toolchain::Value* run_options);

   private:
    bool enabled_;
    // Maps from HLO to the activity id returned by xprof::TraceMe.
    absl::flat_hash_map<const HloInstruction*, toolchain::Value*> activity_ids_;
  };
  ...
  void AttachAlignmentMetadataForLoad(toolchain::LoadInst* load, const Shape& shape);
  void AttachAlignmentMetadataForLoad(toolchain::LoadInst* load,
                                      int64_t buffer_size);

  // Given a load instruction and a shape or buffer size, annotate the load's
  // result with the dereferenceable bytes required by the shape / buffer size.
  void AttachDereferenceableMetadataForLoad(toolchain::LoadInst* load,
                                            const Shape& shape);
  static void AttachDereferenceableMetadataForLoad(toolchain::LoadInst* load,
                                                   int64_t buffer_size);

  // Given a load instruction, annotate the load's result with the invariant
  // load metadata.
  void AttachInvariantLoadMetadataForLoad(toolchain::LoadInst* load) const;
  static void AttachInvariantLoadMetadataForLoad(toolchain::LoadInst* load,
  ...
  toolchain::Constant* EmitGlobalForLiteral(const Literal& literal);
  ...
  absl::flat_hash_map<LayoutSensitiveLiteralWrapper, toolchain::Constant*>
      emitted_literals_;

  absl::flat_hash_map<BufferAllocation::Index, toolchain::Constant*>
...
#endif  // MACHINA_MACHINA_XLA_SERVICE_CPU_IR_EMITTER_H_
