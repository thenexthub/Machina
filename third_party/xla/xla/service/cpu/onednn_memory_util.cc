...
#include "machina/xla/service/cpu/runtime_lightweight_check.h"
...
StackAlloca GetAllocaAndEmitMemrefInfo(toolchain::IRBuilderBase& builder,
  ...
  // Type of struct
  toolchain::Type* i64_type = builder.getInt64Ty();
  toolchain::Type* ptr_type = builder.getPtrTy();
  toolchain::ArrayType* i64_array_type =
      toolchain::ArrayType::get(builder.getInt64Ty(), kOneDnnMaxNDims);
  toolchain::StructType* memref_info_type = toolchain::StructType::get(
      builder.getContext(),
      {i64_type, i64_type, ptr_type, i64_type, i64_array_type, i64_array_type});

  // Prepare array dims and strides.
  toolchain::Value* dims_val = toolchain::UndefValue::get(i64_array_type);
  toolchain::Value* strides_val = toolchain::UndefValue::get(i64_array_type);
  for (unsigned i = 0; i < rank; ++i) {
    toolchain::Value* dim_val = builder.getInt64(dims[i]);
    toolchain::Value* stride_val = builder.getInt64(strides[i]);
  ...
  // cacheline
  toolchain::Value* dtype_val = builder.getInt64(shape.element_type());
  toolchain::Value* rank_val = builder.getInt64(rank);
  toolchain::Value* pad_val = builder.getInt64(0xff);
  toolchain::Value* data_ptr = ir_array.GetBasePointer();
  toolchain::Value* memref_info_val = toolchain::UndefValue::get(memref_info_type);
  ...
  toolchain::Value* memref_info_ptr = llvm_ir::EmitAllocaAtFunctionEntry(
