...
#include "machina/xla/service/cpu/tiled_dot_emitter.h"
...
class MemoryTile {
 public:
  ...
  MemoryTile(VectorIrBuilder* vsl, toolchain::IRBuilderBase* b, toolchain::Value* matrix,
             int64_t matrix_size_along_minor_dim, toolchain::Value* major_dim_offset,
  ...
  std::vector<toolchain::Value*> LoadTile(toolchain::Value* minor_dim_offset) const {
  ...
  //
  // Note: `major_dim_offset` is a parameter to the constructor.
  void StoreTile(absl::Span<toolchain::Value* const> tile,
                 toolchain::Value* minor_dim_offset) const {
  ...
  //
  // Note: `major_dim_offset` is a parameter to the constructor.
  std::vector<std::vector<toolchain::Value*>> LoadBroadcastTile(
      toolchain::Value* minor_dim_offset, int64_t tile_size_along_middle_dim) const {
    std::vector<std::vector<toolchain::Value*>> result;
 ...
 private:
  ...
  std::vector<toolchain::Value*> pointers_;
};
...
// toolchain::Value of a vector type)
//
// where:
//
...
class ColumnMajorMatrixVectorProductEmitter
 ...
 public:
  ...
  ColumnMajorMatrixVectorProductEmitter(const Config& config, toolchain::Value* lhs,
                                        toolchain::Value* rhs, toolchain::Value* addend,
                                        toolchain::Value* result,
                                        toolchain::Value* work_group_id,
 ...
 private:
  void EmitOuterLoopBody(toolchain::Value* column, int64_t column_count,
                         bool is_first_column);

  MemoryTile GetLhsMemoryTile(toolchain::Value* column_start, int64_t column_count) {
  ...
  std::vector<toolchain::Value*> LoadRhsTile(toolchain::Value* offset, int64_t count) {
    toolchain::Value* base_pointer = vb_.ComputeOffsetPointer(rhs_, offset);
    std::vector<toolchain::Value*> result;
  ...
  void EmitInnerLoopTiled(MemoryTile* lhs_memory_tile,
                          const std::vector<toolchain::Value*>& rhs_tile,
                          int64_t columns, bool is_first_column);

  void EmitInnerLoopEpilogue(toolchain::Value* current_tile_col, int64_t columns,
                             bool is_first_tiled_column);

  Config config_;
  toolchain::Value* lhs_;
  toolchain::Value* rhs_;
  toolchain::Value* addend_;
  toolchain::Value* result_;
  toolchain::Value* work_group_id_;
  toolchain::IRBuilderBase* b_;
...
void ColumnMajorMatrixVectorProductEmitter::EmitOuterLoopBody(
    toolchain::Value* column, int64_t column_count, bool is_first_column) {
  MemoryTile lhs_memory_tile = GetLhsMemoryTile(/*column_start=*/column,
                                                /*column_count=*/column_count);

  std::vector<toolchain::Value*> rhs_tile =
...
void ColumnMajorMatrixVectorProductEmitter::Emit() {
  ...
  ksl_.For("dot.outer.tiled",
           ...
           [&](toolchain::Value* column, bool is_first_column) {
...
void ColumnMajorMatrixVectorProductEmitter::EmitInnerLoopTiled(
    MemoryTile* lhs_memory_tile, const std::vector<toolchain::Value*>& rhs_tile,
    int64_t columns, bool is_first_column) {
  ...
  toolchain::Value* row_start =
      b_->CreateMul(work_group_id_, b_->getInt64(row_per_task));
  toolchain::Value* row_end = b_->CreateAdd(row_start, b_->getInt64(row_per_task));
  toolchain::Value* row_cmp =
      b_->CreateCmp(toolchain::CmpInst::ICMP_SLT, row_end, b_->getInt64(row_limit));
  row_end = b_->CreateSelect(row_cmp, row_end, b_->getInt64(row_limit));

  ksl_.For("dot.inner.tiled", row_start, row_end,
           /*step=*/tile_rows(), [&](toolchain::Value* row) {
             std::vector<toolchain::Value*> lhs_tile =
...
void ColumnMajorMatrixVectorProductEmitter::EmitInnerLoopEpilogue(
    toolchain::Value* current_tile_col, int64_t columns,
    bool is_first_tiled_column) {
  ...
  toolchain::Value* columns_llvm = b_->getInt64(columns);
  ...
  ksl_.For(
      ...
      /*step=*/1, [&](toolchain::Value* col) {
        toolchain::Value* is_first_scalar_col =
            b_->CreateICmpEQ(col, current_tile_col);
        toolchain::Value* rhs_element = vb_.LoadScalar(rhs_, col);
        toolchain::Value* total_offset = b_->CreateMul(col, b_->getInt64(m()));
        toolchain::Value* lhs_base_pointer =
            vb_.ComputeOffsetPointer(lhs_, total_offset);
        ksl_.For(
            "dot.inner.epilg.inner", /*start=*/row_start, /*end=*/m(),
            /*step=*/1, [&](toolchain::Value* scalar_row) {
              toolchain::Value* product = vb_.Mul(
                  vb_.LoadScalar(lhs_base_pointer, scalar_row), rhs_element);
              toolchain::Value* setting_result_first_time = b_->CreateAnd(
...
class RowMajorMatrixVectorProductEmitter
 ...
 public:
  ...
  RowMajorMatrixVectorProductEmitter(const Config& config, toolchain::Value* lhs,
                                     toolchain::Value* rhs, toolchain::Value* addend,
                                     toolchain::Value* result,
                                     toolchain::Value* work_group_id,
                                     toolchain::IRBuilderBase* b)
 ...
 private:
  MemoryTile GetLhsMemoryTile(toolchain::Value* row_start, int64_t row_count) {
    return MemoryTile(&vb_, b_, /*matrix=*/lhs_,
  ...
  void EmitOuterLoopBody(toolchain::Value* row, int64_t row_count);

  void EmitInnerLoopTiled(MemoryTile* lhs_memory_tile, int64_t rows,
                          std::vector<VectorVariable>* vector_accumulators);

  void EmitInnerLoopEpilogue(toolchain::Value* current_tile_row, int64_t rows,
...
void RowMajorMatrixVectorProductEmitter::EmitOuterLoopBody(toolchain::Value* row,
                                                           int64_t row_count) {
  ...
  std::vector<toolchain::Value*> accumulator_values;
  std::transform(
  ...
  std::vector<toolchain::Value*> horizontal_sums;
  ...
  for (int i = 0; i < row_count; i++) {
    toolchain::Value* result_value =
        vb_.Add(horizontal_sums[i], scalar_accumulators[i].Get());
    toolchain::Value* offset = b_->CreateAdd(b_->getInt64(i), row);
...
void RowMajorMatrixVectorProductEmitter::Emit() {
  ...
  ksl_.For("dot.outer.tiled", row_start, row_end, /*step=*/tile_rows(),
           [&](toolchain::Value* row) { EmitOuterLoopBody(row, tile_rows()); });
...
void RowMajorMatrixVectorProductEmitter::EmitInnerLoopTiled(
  ...
  ksl_.For("dot.inner.tiled", /*start=*/0, /*end=*/column_limit,
           /*step=*/tile_cols(), [&](toolchain::Value* col) {
             std::vector<toolchain::Value*> lhs_tile =
                 lhs_memory_tile->LoadTile(/*minor_dim_offset=*/col);
             toolchain::Value* rhs_value = vb_.LoadVector(rhs_, col);
             for (int i = 0; i < rows; i++) {
               toolchain::Value* old_sum = (*vector_accumulators)[i].Get();
...
void RowMajorMatrixVectorProductEmitter::EmitInnerLoopEpilogue(
    toolchain::Value* current_tile_row, int64_t rows,
    std::vector<ScalarVariable>* scalar_accumulators) {
...
class TiledSmallGemmEmitter {
 public:
  ...
  explicit TiledSmallGemmEmitter(Config config, toolchain::Value* lhs,
                                 toolchain::Value* rhs, toolchain::Value* result,
                                 toolchain::IRBuilderBase* b)
      : lhs_(lhs),
        rhs_(rhs),
        result_(result),
 ...
 private:
  ...
  void HandleResiduesOnK(VectorIrBuilder* vsl, toolchain::Value* n_start,
                         toolchain::Value* n_end);
  void HandleResiduesOnM(VectorIrBuilder* vsl, int64_t tile_size_k,
                         toolchain::Value* k_start, toolchain::Value* k_end,
                         toolchain::Value* n_start, toolchain::Value* n_end);
  ...
  void EmitTiledGemm(VectorIrBuilder* vsl, int64_t tile_size_k,
                     toolchain::Value* k_start, toolchain::Value* k_end,
                     toolchain::Value* n_start, toolchain::Value* n_end,
                     int64_t tile_size_m, toolchain::Value* m_start,
                     toolchain::Value* m_end);

  toolchain::Value* GetInt64(int64_t value) { return b_->getInt64(value); }
...
void TiledSmallGemmEmitter::HandleResiduesOnN() {
  ...
  if (n_start != dims().n()) {
    ...
    ksl_.For("epi.n", n_start, dims().n(), 1, [&](toolchain::Value* n_i) {
      toolchain::Value* n_i_next = b_->CreateAdd(n_i, b_->getInt64(1));
...
void TiledSmallGemmEmitter::HandleResiduesOnM(
    VectorIrBuilder* vsl, int64_t tile_size_k, toolchain::Value* k_start,
    toolchain::Value* k_end, toolchain::Value* n_start, toolchain::Value* n_end) {
...
void TiledSmallGemmEmitter::EmitTiledGemm(
    VectorIrBuilder* vsl, int64_t tile_size_k, toolchain::Value* k_start,
    toolchain::Value* k_end, toolchain::Value* n_start, toolchain::Value* n_end,
    int64_t tile_size_m, toolchain::Value* m_start, toolchain::Value* m_end) {
  ksl_.For("dot.m", m_start, m_end, tile_size_m, [&](toolchain::Value* m_i) {
    ...
    MemoryTile lhs_memory_tile(vsl, b_, /*matrix=*/lhs_,
                               ...
                               /*tile_size_along_major_dim=*/tile_size_m);
    ksl_.For(
        "dot.n", n_start, n_end, vsl->vector_size(), [&](toolchain::Value* n_i) {
          TileVariable result_tile_var(vsl, result_memory_tile.LoadTile(n_i));
          ksl_.For("dot.k", k_start, k_end, tile_size_k, [&](toolchain::Value* k_i) {
            MemoryTile rhs_memory_tile(vsl, b_, rhs_, dims().n(), k_i,
                                       tile_size_k);
            std::vector<std::vector<toolchain::Value*>> lhs_tile =
                lhs_memory_tile.LoadBroadcastTile(k_i, tile_size_k);
            std::vector<toolchain::Value*> rhs_tile = rhs_memory_tile.LoadTile(n_i);
            std::vector<toolchain::Value*> result_tile = result_tile_var.Get();
...
void EmitRowMajorGemv(PrimitiveType scalar_type, int64_t num_tasks,
  ...
  KernelSupportLibrary::EmitAndCallOutlinedKernel(
      ...
      work_group_id,
      [&config, b](toolchain::Value* lhs, toolchain::Value* rhs, toolchain::Value* addend,
                   toolchain::Value* result, toolchain::Value* work_group_id) {
...
void EmitSmallGemm(PrimitiveType scalar_type, int64_t m, int64_t k, int64_t n,
  ...
  KernelSupportLibrary::EmitAndCallOutlinedKernel(
      ...
      [&](toolchain::Value* lhs, toolchain::Value* rhs, toolchain::Value* result) {
