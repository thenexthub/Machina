...
#include "machina/xla/codegen/emitters/loop_kernel_emitter.h"
...
absl::Status LoopFusionKernelEmitter::EmitEntryFunction(
  ...
  toolchain::SmallVector<const Shape*> result_shapes;
  ...
  auto body_builder =
      [&](mlir::ImplicitLocOpBuilder& nested_b, mlir::ValueRange symbol_values,
          ...
          mlir::ValueRange output_tensors) -> toolchain::SmallVector<mlir::Value> {
    ...
    for (auto [root_shape, tensor, value] :
         toolchain::zip(result_shapes, output_tensors, result_scalars)) {
      toolchain::SmallVector<mlir::Value> output_indices = emitters::ApplyIndexing(
  ...
  const auto loop_builder = [&](mlir::OpBuilder& builder, mlir::Location loc,
    ...
    for (auto [result, output] : toolchain::zip(loop_results, outputs)) {
