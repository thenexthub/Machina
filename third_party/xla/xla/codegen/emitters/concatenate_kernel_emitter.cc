...
#include "machina/xla/codegen/emitters/concatenate_kernel_emitter.h"
...
#include "machina/xla/runtime/work_item.h"
...
absl::Status ConcatenateFusionKernelEmitter::EmitEntryFunction(
  ...
  toolchain::SmallVector<mlir::Value> input_tensors(
  ...
  toolchain::SmallVector<mlir::Value> result_tensors{output_tensor_args.begin(),
  ...
  const auto forall_body_builder = [&](mlir::OpBuilder& builder,
    ...
    for (auto [operand_index, operand] : toolchain::enumerate(concat->operands())) {
      ...
      auto loop_nest_body_builder = [&, operand_index = operand_index](
        ...
        toolchain::SmallVector<mlir::Value> result_tensors;
        result_tensors.reserve(output_tensor_args.size());
        for (auto [tensor, value] : toolchain::zip(output_tensors, result_scalars)) {
    ...
    for (auto [result, output] : toolchain::zip(result_tensors, outputs)) {
      auto output_tensor = mlir::cast<mlir::RankedTensorType>(output.getType());
      toolchain::SmallVector<mlir::OpFoldResult> offsets(output_tensor.getRank(),
                                                    nested_b.getIndexAttr(0));
      toolchain::SmallVector<mlir::OpFoldResult> sizes =
          mlir::getAsIndexOpFoldResult(context, output_tensor.getShape());
      toolchain::SmallVector<mlir::OpFoldResult> strides(output_tensor.getRank(),
  ...
  toolchain::SmallVector<mlir::OpFoldResult> upper_bounds =
