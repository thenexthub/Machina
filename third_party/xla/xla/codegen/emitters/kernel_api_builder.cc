...
absl::StatusOr<mlir::func::FuncOp> EmitKernelApi(
  ...
  toolchain::SmallVector<mlir::Type> param_types;
  ...
  for (auto [arg_index, param] : toolchain::enumerate(hlo_instruction.operands())) {
    param_types.push_back(
        emitters::TensorShapeToMlirType(param->shape(), builder));
    arg_attrs.push_back(get_arg_attrs(arg_index));
  }

  for (auto [result_index, type] : toolchain::enumerate(result_types)) {
...
IndexingMap GetDefaultWorkItemIndexingMap(const WorkDimensions& work_dimensions,
  ...
  for (auto [idx, dim] : toolchain::enumerate(toolchain::reverse(work_tile_dimensions))) {
...
toolchain::SmallVector<mlir::Value> EmitWorkGroupIds(
