...
struct PackedArgs {
  toolchain::BitVector args_to_erase;
  // replacement_args[i] == i iff !args_to_erase[i].
  toolchain::SmallVector<int> replacement_args;
  ...
  explicit PackedArgs(mlir::func::FuncOp func) {
    ...
    for (auto [idx, operand] : toolchain::enumerate(func.getArguments())) {
  ...
  void Pack(mlir::func::FuncOp op) {
    ...
    assert(toolchain::succeeded(res));
