...
void CollectArgs(AffineExpr expr, AffineExprKind kind,
                 toolchain::SmallVector<AffineExpr>& ret) {
...
Value ExpressionEvaluator::EvaluateAddMul(AffineExpr expr) {
  toolchain::SmallVector<AffineExpr> args;
  ...
  toolchain::stable_sort(args, [&](AffineExpr a, AffineExpr b) {
...
bool IsLoweringSupported(AffineExpr expr, RangeEvaluator& range_evaluator) {
  auto bin_op = toolchain::dyn_cast<AffineBinaryOpExpr>(expr);
  if (!bin_op) {
    return true;
  }
