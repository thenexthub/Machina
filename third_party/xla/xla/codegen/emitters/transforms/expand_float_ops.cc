...
#include "machina/xla/codegen/emitters/implicit_arith_op_builder.h"
#include "machina/xla/codegen/emitters/transforms/passes.h"
#include "machina/xla/mlir_hlo/mhlo/IR/hlo_ops.h"
#include "machina/xla/mlir_hlo/mhlo/transforms/map_mhlo_to_scalar_op.h"
...
#include "machina/xla/codegen/emitters/transforms/passes.h.inc"
...
int GetSignificandBits(mlir::FloatType ty) {
  return toolchain::APFloat::semanticsPrecision(ty.getFloatSemantics()) - 1;
}

int GetExponentBias(mlir::FloatType ty) {
  return 1 - toolchain::APFloat::semanticsMinExponent(ty.getFloatSemantics()) -
         toolchain::isa<mlir::Float8E8M0FNUType>(ty);  // No zero exponent for E8M0.
}

bool IsFNUZ(mlir::FloatType ty) {
  return toolchain::isa<mlir::Float8E4M3B11FNUZType, mlir::Float8E4M3FNUZType,
...
Value IsInf(Value value, mlir::ImplicitLocOpBuilder& b) {
  ...
  if (mlir::LLVM::isCompatibleOuterType(ty)) {
    ...
    Value inf = b.create<ma::ConstantFloatOp>(
        ty, toolchain::APFloat::getInf(ty.getFloatSemantics()));
  ...
  if (toolchain::isa<mlir::Float8E5M2Type>(ty)) {
    Val bits{b.create<ma::BitcastOp>(b.getI8Type(), value), &b};
    return (bits & 0x7F) == 0x7C;
  } else if (toolchain::isa<mlir::Float8E4M3Type>(ty)) {
    Val bits{b.create<ma::BitcastOp>(b.getI8Type(), value), &b};
    return (bits & 0x7F) == 0x78;
  } else if (toolchain::isa<mlir::Float8E3M4Type>(ty)) {
    Val bits{b.create<ma::BitcastOp>(b.getI8Type(), value), &b};
...
Value IsNaN(Value value, mlir::ImplicitLocOpBuilder& b) {
  ...
  if (toolchain::isa<mlir::Float4E2M1FNType>(ty)) {
    return b.create<ma::ConstantIntOp>(b.getI1Type(), false);
  }
...
Value EmitFloatConversion(Value value, mlir::FloatType to_ty,
  ...
  int from_min_exp =
      toolchain::APFloat::semanticsMinExponent(from_ty.getFloatSemantics());
  int from_max_exp =
      toolchain::APFloat::semanticsMaxExponent(from_ty.getFloatSemantics());
  ...
  int to_min_exp =
      toolchain::APFloat::semanticsMinExponent(to_ty.getFloatSemantics());
  int to_max_exp =
      toolchain::APFloat::semanticsMaxExponent(to_ty.getFloatSemantics());
  ...
  if (from_ty.getWidth() <= 8 && to_ty.getWidth() <= 8) {
    ...
    auto may_overflow = [&](mlir::Type a, mlir::Type b) {
      return toolchain::isa<mlir::Float8E8M0FNUType>(a) && b.isF16();
    };
  ...
  Val from_sign_bit;
  if (!toolchain::isa<mlir::Float8E8M0FNUType>(from_ty)) {
    from_sign_bit = from_bits.shrui(from_width - 1) != 0;
    from_bits = from_bits & ((1ULL << (from_width - 1)) - 1);
  }

  auto cst_bits = [&](toolchain::APFloat f) {
    return cst(b.getIntegerType(toolchain::APFloat::getSizeInBits(f.getSemantics())),
  ...
  if (toolchain::isa<mlir::Float4E2M1FNType>(to_ty)) {
    to_zero = cst_bits(toolchain::APFloat::getZero(to_ty.getFloatSemantics()));
    to_nan = to_zero | 0x8;
    to_inf = cst_bits(toolchain::APFloat::getLargest(to_ty.getFloatSemantics()));
  } else if (toolchain::isa<mlir::Float8E8M0FNUType>(to_ty)) {
    to_nan = cst_bits(toolchain::APFloat::getNaN(to_ty.getFloatSemantics()));
    to_inf = to_nan;
    to_zero = Val{to_nan, &b};
  } else {
    to_inf = cst_bits(toolchain::APFloat::getInf(to_ty.getFloatSemantics()));
    to_nan = cst_bits(toolchain::APFloat::getNaN(to_ty.getFloatSemantics()));
    to_zero = cst_bits(toolchain::APFloat::getZero(to_ty.getFloatSemantics()));
  }
  ...
  Value result;
  {
    ...
    int64_t to_highest = toolchain::APFloat::getLargest(to_ty.getFloatSemantics())
                             .bitcastToAPInt()
                             .getZExtValue();
  ...
  if (to_min_exp < from_min_exp) {
    ...
    Val from_has_leading_one = !toolchain::isa<mlir::Float8E8M0FNUType>(from_ty)
                                   ? biased_from_exp != 0
                                   : cst(i32_ty, 1);
  ...
  if (toolchain::isa<mlir::Float8E8M0FNUType>(to_ty)) {
...
struct RewriteF8Cst : public mlir::OpRewritePattern<ma::CmpFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::APFloat rhs_cst(rhs.getType().getFloatSemantics());
...
struct RewriteAbsFPattern : public mlir::OpRewritePattern<mlir::math::AbsFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (!toolchain::APFloat::semanticsHasSignedRepr(
