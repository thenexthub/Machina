...
// CHECK:        func.func private @tensorarg(%[[ARG0:.*]]: !toolchain.ptr
...
// CHECK-DAG:       %[[PTR:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, %[[IDX]]]
// CHECK-DAG:       %[[V2:.*]] = toolchain.load %[[PTR]] invariant
// CHECK:           %[[RET:.*]] = call @add(%[[C2]], %[[V2]])
// CHECK:           return %[[RET]]

// CHECK:        func.func @tensorcall(%[[ARG0:.*]]: !toolchain.ptr
// CHECK-SAME:        {xla.slice_index = 0 : i64}, %[[ARG1:.*]]: index)
// CHECK:           %[[RET:.*]] = call @tensorarg(%[[ARG0]], %[[ARG1]])
// CHECK:           return %[[RET]]

// CHECK:        func.func @stores(
// CHECK-SAME:        %[[ARG0:.*]]: !toolchain.ptr {xla.slice_index = 0 : i64},
// CHECK-SAME:        %[[ARG1:.*]]: !toolchain.ptr {xla.slice_index = 1 : i64})
// CHECK-NEXT:      %[[CST:.*]] = arith.constant 3.000000e+00 : f32
// CHECK-NEXT:      %[[PTR1:.*]] = toolchain.getelementptr inbounds %[[ARG1]][0, 17]
// CHECK-NEXT:      toolchain.store %[[CST]], %[[PTR1]]
// CHECK-NEXT:      %[[PTR2:.*]] = toolchain.getelementptr inbounds %[[ARG1]][0, 23]
// CHECK-NEXT:      toolchain.store %[[CST]], %[[PTR2]]
// CHECK-NEXT:      return
...
// CHECK-SAME:  %[[ARG0:.*]]: !toolchain.ptr, %[[X:.*]]: index) {
// CHECK-DAG:   %[[C0:.*]] = arith.constant 0 : index
// CHECK-DAG:   %[[C1:.*]] = arith.constant 1 : index
// CHECK-DAG:   %[[C2:.*]] = arith.constant 2 : index
// CHECK:       scf.for %[[I:.*]] = %[[C0]] to %[[C2]] step %[[C1]] {
// CHECK:         %[[CAST:.*]] = arith.index_castui %[[I]] : index to i64
// CHECK:         %[[PTR:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, %[[CAST]]]
// CHECK:         toolchain.store {{.*}}, %[[PTR]]
// CHECK:       %[[INBOUNDS:.*]] = arith.cmpi
// CHECK:       scf.if %[[INBOUNDS]] {
// CHECK:         toolchain.store
// CHECK-NEXT:  }
// CHECK-NEXT:  return
...
// CHECK: toolchain.mlir.global private constant @global_cst_0(dense<
// CHECK-SAME: [1.000000e+00, 2.000000e+00]> : tensor<2xf32>) {addr_space = 0 : i32} : !toolchain.array<2 x f32>
// CHECK-LABEL: @extract_from_constant
// CHECK: %[[ADDR_OF:.*]] = toolchain.mlir.addressof @global_cst_0 : !toolchain.ptr
// CHECK: %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ADDR_OF]][0, %{{.*}}] : (!toolchain.ptr, i64) -> !toolchain.ptr, !toolchain.array<2 x f32>
// CHECK: %[[LOAD:.*]] = toolchain.load %[[GEP]] : !toolchain.ptr -> f32
...
// CHECK-SAME: %[[ARG0:.*]]: !toolchain.ptr
// CHECK: %[[C:.*]] = complex.create
// CHECK: %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 1] : (!toolchain.ptr) -> !toolchain.ptr, !toolchain.array<10 x struct<(f32, f32)>>
// CHECK: %[[CAST:.*]] = builtin.unrealized_conversion_cast %[[C]] : complex<f32> to !toolchain.struct<(f32, f32)>
// CHECK: toolchain.store %[[CAST]], %[[GEP]] : !toolchain.struct<(f32, f32)>, !toolchain.ptr
...
// CHECK-SAME: %[[ARG0:.*]]: !toolchain.ptr
// CHECK: %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 1] : (!toolchain.ptr) -> !toolchain.ptr, !toolchain.array<10 x struct<(f32, f32)>>
// CHECK: %[[LOAD:.*]] = toolchain.load %[[GEP]] : !toolchain.ptr -> !toolchain.struct<(f32, f32)>
// CHECK: builtin.unrealized_conversion_cast %[[LOAD]] : !toolchain.struct<(f32, f32)> to complex<f32>
...
// CHECK:      toolchain.mlir.global private @[[SHARED:shared_.*]]()
// CHECK-SAME:     {addr_space = 3 : i32} : !toolchain.array<1024 x f32>
// CHECK:      @transpose_shared
// CHECK:        %[[ADDR:.*]] = toolchain.mlir.addressof @[[SHARED]] : !toolchain.ptr<3>
// CHECK:        %[[CAST:.*]] = toolchain.addrspacecast %[[ADDR]]
// CHECK-SAME:       : !toolchain.ptr<3> to !toolchain.ptr
// CHECK:        scf.for
// CHECK:          %[[ELEM_ADDR:.*]] = toolchain.getelementptr inbounds %[[CAST]]
// CHECK:          toolchain.store {{.*}} %[[ELEM_ADDR]]
// CHECK:        gpu.barrier
// CHECK:        scf.for
// CHECK:          %[[ELEM_ADDR:.*]] = toolchain.getelementptr inbounds %[[CAST]]
// CHECK:          toolchain.load %[[ELEM_ADDR]]
...
// CHECK: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-NEXT: %[[INIT:.*]] = toolchain.load %[[ADDR]]
// CHECK-NEXT: scf.while (%[[VAR:.*]] = %[[INIT]])
// CHECK: %[[RES:.*]] = arith.bitcast %{{.*}} : f32 to i32
// CHECK-NEXT: toolchain.cmpxchg %[[ADDR]], %[[VAR]], %[[RES]]
...
// CHECK: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-NEXT: %[[ADDR_INT:.*]] = toolchain.ptrtoint %[[ADDR]]
// CHECK-NEXT: %[[OFFSET:.*]] = toolchain.and %[[ADDR_INT]], %{{.*}}
// CHECK-NEXT: %[[INDEX:.*]] = toolchain.mul %[[OFFSET]], %{{.*}}
// CHECK-NEXT: %[[BASE:.*]] = toolchain.getelementptr inbounds %[[ADDR]]
// CHECK: %[[INIT:.*]] = toolchain.load %[[BASE]]
// CHECK-NEXT: scf.while (%[[VAR:.*]] = %[[INIT]])
// CHECK-NEXT: %[[VAR_SHIFT:.*]] = toolchain.lshr %[[VAR]], %{{.*}}
// CHECK-NEXT: %[[VAR_TRUNC:.*]] = toolchain.trunc %[[VAR_SHIFT]]
// CHECK-NEXT: arith.bitcast %[[VAR_TRUNC]] : i16 to f16
// CHECK: %[[RES:.*]] = arith.bitcast %{{.*}} : f16 to i16
// CHECK-NEXT: %[[RES_WIDE:.*]] = toolchain.zext %[[RES]]
// CHECK-NEXT: %[[NEW_MASKED:.*]] = toolchain.and %[[VAR]], %{{.*}}
// CHECK-NEXT: %[[RES_SHIFT:.*]] = toolchain.shl %[[RES_WIDE]], %{{.*}}
// CHECK-NEXT: %[[NEW:.*]] = toolchain.or %[[NEW_MASKED]], %[[RES_SHIFT]]
// CHECK-NEXT: toolchain.cmpxchg %[[BASE]], %[[VAR]], %[[NEW]]
...
// CHECK-NEXT: %[[RES_WIDE:.*]] = toolchain.zext %[[RES]]
// CHECK-DAG: %[[RES_SHIFT:.*]] = toolchain.shl %[[RES_WIDE]], %{{.*}}
// CHECK-DAG: %[[NEW_MASKED:.*]] = toolchain.and %[[VAR]], %{{.*}}
// CHECK-NEXT: %[[NEW:.*]] = toolchain.or %[[NEW_MASKED]], %[[RES_SHIFT]]
// CHECK-NEXT: toolchain.cmpxchg %[[BASE]], %[[VAR]], %[[NEW]]
...
// CHECK-SAME:    %[[VAL_0:.*]]: !toolchain.ptr, %[[VAL_1:.*]]: index) {

// CHECK-DAG:  %[[LC1_i4:.*]] = arith.constant 1 : i4
// CHECK-DAG:  %[[C0:.*]] = arith.constant 0 : i64
// CHECK-DAG:  %[[C1:.*]] = arith.constant 1 : i64
// CHECK-DAG:  %[[LC3:.*]] = toolchain.mlir.constant(3 : i64) : i64
// CHECK-DAG:  %[[LCm1:.*]] = toolchain.mlir.constant(-1 : i64) : i64
// CHECK-DAG:  %[[LC8:.*]] = toolchain.mlir.constant(8 : i32) : i32
// CHECK-DAG:  %[[LC0:.*]] = toolchain.mlir.constant(0 : i32) : i32
// CHECK-DAG:  %[[LC4:.*]] = toolchain.mlir.constant(4 : i32) : i32
// CHECK-DAG:  %[[LCm1_i32:.*]] = toolchain.mlir.constant(-1 : i32) : i32
// CHECK-DAG:  %[[LC15:.*]] = toolchain.mlir.constant(15 : i32) : i32
// CHECK-DAG:  %[[LCTRUE:.*]] = toolchain.mlir.constant(true) : i1
...
// CHECK:  %[[VAL_17:.*]] = toolchain.getelementptr inbounds %[[VAL_0]][0, %[[VAL_16]]]
// CHECK-SAME:   : (!toolchain.ptr, i64) -> !toolchain.ptr, !toolchain.array<4 x i8>

// CHECK:  %[[VAL_18:.*]] = toolchain.ptrtoint %[[VAL_17]] : !toolchain.ptr to i64
// CHECK:  %[[VAL_19:.*]] = toolchain.and %[[VAL_18]], %[[LC3]] : i64
// CHECK:  %[[VAL_20:.*]] = toolchain.mul %[[VAL_19]], %[[LCm1]] : i64
// CHECK:  %[[VAL_21:.*]] = toolchain.getelementptr inbounds %[[VAL_17]][%[[VAL_20]]]
// CHECK-SAME:   : (!toolchain.ptr, i64) -> !toolchain.ptr, i8

// CHECK:  %[[VAL_22:.*]] = toolchain.trunc %[[VAL_19]] : i64 to i32
// CHECK:  %[[VAL_23:.*]] = toolchain.mul %[[VAL_22]], %[[LC8]] : i32
// CHECK:  %[[VAL_24:.*]] = toolchain.select %[[VAL_15]], %[[LC0]], %[[LC4]]
// CHECK:  %[[VAL_25:.*]] = toolchain.add %[[VAL_23]], %[[VAL_24]] : i32
// CHECK:  %[[VAL_26:.*]] = toolchain.shl %[[LC15]], %[[VAL_25]] : i32
// CHECK:  %[[VAL_27:.*]] = toolchain.xor %[[LCm1_i32]], %[[VAL_26]] : i32
// CHECK:  %[[LOAD_4BYTES:.*]] = toolchain.load %[[VAL_21]] : !toolchain.ptr -> i32
// CHECK:  scf.while (%[[VAL_30:.*]] = %[[LOAD_4BYTES]]) : (i32) -> i32 {
// CHECK:    %[[VAL_31:.*]] = toolchain.lshr %[[VAL_30]], %[[VAL_25]] : i32
// CHECK:    %[[VAL_32:.*]] = toolchain.trunc %[[VAL_31]] : i32 to i4
// CHECK:    %[[VAL_33:.*]] = arith.addi %[[VAL_32]], %[[LC1_i4]] : i4
// CHECK:    %[[VAL_34:.*]] = toolchain.zext %[[VAL_33]] : i4 to i32
// CHECK:    %[[VAL_35:.*]] = toolchain.and %[[VAL_30]], %[[VAL_27]] : i32
// CHECK:    %[[VAL_36:.*]] = toolchain.shl %[[VAL_34]], %[[VAL_25]] : i32
// CHECK:    %[[VAL_37:.*]] = toolchain.or %[[VAL_35]], %[[VAL_36]] : i32
// CHECK:    toolchain.cmpxchg
...
// CHECK-NEXT: %[[OFFSET:.*]] = toolchain.and %[[ADDR_INT]], %{{.*}}
// CHECK-NEXT: %[[INDEX:.*]] = toolchain.mul %[[OFFSET]], %{{.*}}
// CHECK-NEXT: %[[BASE:.*]] = toolchain.getelementptr inbounds %[[ADDR]][%[[INDEX]]]
// CHECK: %[[INIT:.*]] = toolchain.load %[[BASE]]
// CHECK-NEXT: scf.while (%[[VAR:.*]] = %[[INIT]])
// CHECK-DAG: %[[RES_SHIFT:.*]] = toolchain.shl %{{.*}}, %{{.*}}
// CHECK-DAG: %[[NEW_MASKED:.*]] = toolchain.and %[[VAR]], %{{.*}}
// CHECK-NEXT: %[[NEW:.*]] = toolchain.or %[[NEW_MASKED]], %[[RES_SHIFT]]
// CHECK-NEXT: toolchain.cmpxchg %[[BASE]], %[[VAR]], %[[NEW]]
...
// CHECK: toolchain.mlir.global private @{{.*}}() {addr_space = 3 : i32} : !toolchain.array<10 x struct<(f32, f32)>>
...
// CHECK: toolchain.mlir.global private @{{.*}}() {addr_space = 3 : i32} : !toolchain.array<5 x i8>
...
// CHECK: toolchain.getelementptr
// CHECK-SAME: -> !toolchain.ptr, !toolchain.array<5 x i8>
// CHECK: %[[VALUE_I8:.*]] = arith.extui {{.*}} : i4 to i8
// CHECK: toolchain.getelementptr
// CHECK-SAME: -> !toolchain.ptr, !toolchain.array<10 x i8>
// CHECK: %[[CURRENT_I32:.*]] = toolchain.load
// CHECK-SAME: !toolchain.ptr -> i32
// CHECK: scf.while (%[[INIT:.*]] = %[[CURRENT_I32]])
// CHECK: %[[SHIFTED:.*]] = toolchain.lshr %[[INIT]]
// CHECK: %[[CURRENT:.*]] = toolchain.trunc %[[SHIFTED]]
...
// CHECK: %[[NEW_VALUE_I32:.*]] = toolchain.zext %[[NEW_VALUE]]
// CHECK-DAG: %[[NEW_VALUE_SHIFTED:.*]] = toolchain.shl %[[NEW_VALUE_I32]]
// CHECK-DAG: %[[MASKED_INIT:.*]] = toolchain.and %[[INIT]]
// CHECK: %[[NEW_INIT:.*]] = toolchain.or %[[MASKED_INIT]], %[[NEW_VALUE_SHIFTED]]
// CHECK: toolchain.cmpxchg %{{.*}}, %[[INIT]], %[[NEW_INIT]] monotonic monotonic
// CHECK: scf.condition
...
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.store %[[C2]], %[[ADDR]] atomic unordered {alignment = 4 : i64}
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw add %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw max %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw umax %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw min %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw umin %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[C2:.*]] = arith.constant 2
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] monotonic

// CHECK-VOLTA-LABEL: @direct_atomic_rmw_fadd_f32
// CHECK-VOLTA: %[[C2:.*]] = arith.constant 2
// CHECK-VOLTA: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-VOLTA: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] monotonic

// CHECK-AMPERE-LABEL: @direct_atomic_rmw_fadd_f32
// CHECK-AMPERE: %[[C2:.*]] = arith.constant 2
// CHECK-AMPERE: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-AMPERE: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] monotonic

// CHECK-GFX908-MI100-LABEL: @direct_atomic_rmw_fadd_f32
// CHECK-GFX908-MI100: %[[C2:.*]] = arith.constant 2
// CHECK-GFX908-MI100: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-GFX908-MI100: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] syncscope("agent-one-as") monotonic

// CHECK-GFX90A-MI200-LABEL: @direct_atomic_rmw_fadd_f32
// CHECK-GFX90A-MI200: %[[C2:.*]] = arith.constant 2
// CHECK-GFX90A-MI200: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-GFX90A-MI200: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] syncscope("agent-one-as") monotonic
...
// CHECK-PASCAL-NOT: toolchain.atomicrmw fadd
...
// CHECK-GFX908-MI100: %[[C_NEG4:.*]] = toolchain.mlir.constant(-4 : i64) : i64
// CHECK-GFX908-MI100: %[[C2:.*]] = toolchain.mlir.constant(2 : i32) : i32
// CHECK-GFX908-MI100: %[[C8:.*]] = toolchain.mlir.constant(8 : i32) : i32
// CHECK-GFX908-MI100: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-GFX908-MI100: %[[ADDR_INT:.*]] = toolchain.ptrtoint %[[ADDR]]
// CHECK-GFX908-MI100: %[[ADDR_MASKED:.*]] = toolchain.and %[[ADDR_INT]], %[[C_NEG4]]
// CHECK-GFX908-MI100: %[[ADDR_TRUNC:.*]] = toolchain.trunc %[[ADDR_INT]]
// CHECK-GFX908-MI100: %[[OFFSET:.*]] = toolchain.and %[[ADDR_TRUNC]], %[[C2]]
// CHECK-GFX908-MI100: %[[SHIFT:.*]] = toolchain.mul %[[OFFSET]], %[[C8]]
// CHECK-GFX908-MI100: %[[VAL_INT:.*]] = toolchain.bitcast %[[CST]] : f16 to i16
// CHECK-GFX908-MI100: %[[VAL_WIDE:.*]] = toolchain.zext %[[VAL_INT]] : i16 to i32
// CHECK-GFX908-MI100: %[[VAL_SHIFT:.*]] = toolchain.shl %[[VAL_WIDE]], %[[SHIFT]]
// CHECK-GFX908-MI100: %[[ADDR:.*]] = toolchain.inttoptr %[[ADDR_MASKED]]
// CHECK-GFX908-MI100: %[[VAL:.*]] = toolchain.bitcast %[[VAL_SHIFT]] : i32 to vector<2xf16>
// CHECK-GFX908-MI100: toolchain.atomicrmw fadd %[[ADDR]], %[[VAL]] syncscope("agent-one-as") monotonic


// CHECK-GFX90A-MI200-LABEL: @direct_atomic_rmw_fadd_f16
// CHECK-GFX90A-MI200: %[[CST:.*]] = arith.constant 2
// CHECK-GFX90A-MI200: %[[C_NEG4:.*]] = toolchain.mlir.constant(-4 : i64) : i64
// CHECK-GFX90A-MI200: %[[C2:.*]] = toolchain.mlir.constant(2 : i32) : i32
// CHECK-GFX90A-MI200: %[[C8:.*]] = toolchain.mlir.constant(8 : i32) : i32
// CHECK-GFX90A-MI200: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-GFX90A-MI200: %[[ADDR_INT:.*]] = toolchain.ptrtoint %[[ADDR]]
// CHECK-GFX90A-MI200: %[[ADDR_MASKED:.*]] = toolchain.and %[[ADDR_INT]], %[[C_NEG4]]
// CHECK-GFX90A-MI200: %[[ADDR_TRUNC:.*]] = toolchain.trunc %[[ADDR_INT]]
// CHECK-GFX90A-MI200: %[[OFFSET:.*]] = toolchain.and %[[ADDR_TRUNC]], %[[C2]]
// CHECK-GFX90A-MI200: %[[SHIFT:.*]] = toolchain.mul %[[OFFSET]], %[[C8]]
// CHECK-GFX90A-MI200: %[[VAL_INT:.*]] = toolchain.bitcast %[[CST]] : f16 to i16
// CHECK-GFX90A-MI200: %[[VAL_WIDE:.*]] = toolchain.zext %[[VAL_INT]] : i16 to i32
// CHECK-GFX90A-MI200: %[[VAL_SHIFT:.*]] = toolchain.shl %[[VAL_WIDE]], %[[SHIFT]]
// CHECK-GFX90A-MI200: %[[ADDR:.*]] = toolchain.inttoptr %[[ADDR_MASKED]]
// CHECK-GFX90A-MI200: %[[VAL:.*]] = toolchain.bitcast %[[VAL_SHIFT]] : i32 to vector<2xf16>
// CHECK-GFX90A-MI200: toolchain.atomicrmw fadd %[[ADDR]], %[[VAL]] syncscope("agent-one-as") monotonic
...
// CHECK-HOPPER: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-HOPPER: toolchain.atomicrmw fadd %[[ADDR]], %[[C2]] monotonic
...
// CHECK-PASCAL: %[[NAN:.*]] = toolchain.mlir.constant(0x7FC00000 : f32) : f32
// CHECK-PASCAL: %[[C0:.*]] = toolchain.mlir.constant(0 : i32) : i32
// CHECK-PASCAL: %[[ADDR:.*]] = toolchain.getelementptr
// CHECK-PASCAL: %[[CURRENT:.*]] = toolchain.load %[[ADDR]] : !toolchain.ptr -> f32
// CHECK-PASCAL: %[[CURRENT_IS_NAN:.*]] = toolchain.fcmp "uno" %[[CURRENT]], %[[CURRENT]] : f32
// CHECK-PASCAL: scf.if %[[CURRENT_IS_NAN]] {
// CHECK-PASCAL: } else {
// CHECK-PASCAL:   %[[MODIFIER_IS_NAN:.*]] = toolchain.fcmp "uno" %[[MODIFIER]], %[[MODIFIER]] : f32
// CHECK-PASCAL:   %[[MODIFIER_OR_NAN:.*]] = toolchain.select %[[MODIFIER_IS_NAN]], %[[NAN]], %[[MODIFIER]] : i1, f32
// CHECK-PASCAL:   %[[VAL_13:.*]] = toolchain.fcmp "ult" %[[CURRENT]], %[[MODIFIER_OR_NAN]] : f32
// CHECK-PASCAL:   scf.if %[[VAL_13]] {
// CHECK-PASCAL:     %[[INT_MODIFIER_OR_NAN:.*]] = arith.bitcast %[[MODIFIER_OR_NAN]] : f32 to i32
// CHECK-PASCAL:     %[[IS_POSITIVE:.*]] = toolchain.icmp "sge" %[[INT_MODIFIER_OR_NAN]], %[[C0]] : i32
// CHECK-PASCAL:     scf.if %[[IS_POSITIVE]] {
// CHECK-PASCAL:       toolchain.atomicrmw max %[[ADDR]], %[[INT_MODIFIER_OR_NAN]] monotonic
// CHECK-PASCAL:     } else {
// CHECK-PASCAL:       toolchain.atomicrmw umin %[[ADDR]], %[[INT_MODIFIER_OR_NAN]] monotonic
// CHECK-PASCAL:     }
// CHECK-PASCAL:   }
// CHECK-PASCAL: }
// CHECK-PASCAL: return
...
// CHECK: %[[TMP:.*]] = toolchain.alloca %{{.*}} x i64 : (i32) -> !toolchain.ptr
// CHECK: scf.while (%[[ITER_ARG:.*]] = %{{.*}}) : (i64) -> i64
// CHECK: toolchain.store %[[ITER_ARG]], %[[TMP]]
// CHECK: %[[LD:.*]] = toolchain.load %[[TMP]] : {{.*}} -> !toolchain.struct<(f32, f32)>
...
// CHECK:           %[[PTR1:.*]] = toolchain.getelementptr inbounds %[[BUF:.*]][0, 16]
// CHECK-NEXT:      toolchain.store %[[CST:.*]], %[[PTR1]]
// CHECK-NEXT:      %[[PTR2:.*]] = toolchain.getelementptr inbounds %[[BUF]][0, 22]
// CHECK-NEXT:      toolchain.store %[[CST]], %[[PTR2]]
...
// CHECK:           %[[PTR:.*]] = toolchain.getelementptr inbounds %{{.*}}[0, 16]
// CHECK-NEXT:      toolchain.load %[[PTR]] {alignment = 4 : i64} : !toolchain.ptr -> vector<2xf32>
...
// CHECK-DAG:       %[[PTR1:.*]] = toolchain.getelementptr inbounds %[[BUF:.*]][0, 16]
// CHECK-DAG:       %[[V1_EXT:.*]] = arith.extui %[[V1]]
// CHECK:           toolchain.store %[[V1_EXT]], %[[PTR1]] {alignment = 1 : i64}
// CHECK-DAG:       %[[PTR2:.*]] = toolchain.getelementptr inbounds %[[BUF]][0, 22]
// CHECK-DAG:       %[[V2_EXT:.*]] = arith.extui %[[V2]]
// CHECK:           toolchain.store %[[V2_EXT]], %[[PTR2]] {alignment = 1 : i64}
...
// CHECK-DAG:       %[[PTR:.*]] = toolchain.getelementptr inbounds %{{.*}}[0, 16]
// CHECK:           %[[LOADED:.*]] = toolchain.load %[[PTR]] {alignment = 1 : i64} : !toolchain.ptr
// CHECK:           %[[CAST:.*]] = arith.cmpi ne, %[[LOADED]], %[[C0]]
// CHECK:           return %[[CAST]] : vector<2xi1>

// -----

func.func @transfer_read_alignment(%arg0: tensor<8xi64> {toolchain.align = 32 : index}) -> vector<8xi64> {
  %c0 = arith.constant 0 : index
...
// CHECK-SAME:  %[[ARG0:.*]]: !toolchain.ptr
// CHECK:           %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 0] :
// CHECK-SAME:        !toolchain.array<8 x i64>
// CHECK:           %[[LOADED:.*]] = toolchain.load %[[GEP]] {alignment = 32 : i64} : !toolchain.ptr
// CHECK:           return %[[LOADED]] : vector<8xi64>

// -----

func.func @transfer_read_alignment_non_zero_index(%arg0: tensor<16xi64> {toolchain.align = 32 : index}) -> vector<8xi64> {
...
// CHECK-SAME:  %[[ARG0:.*]]: !toolchain.ptr
// CHECK:           %[[PTR:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 8]
// CHECK-NEXT:      toolchain.load %[[PTR]] {alignment = 8 : i64} : !toolchain.ptr -> vector<8xi64>

// -----

func.func @transfer_write_alignment(%arg0: tensor<8xi64> {toolchain.align = 32 : index}) -> tensor<8xi64> {
  %c0 = arith.constant 0 : index
...
// CHECK:           %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 0] :
// CHECK-SAME:        !toolchain.array<8 x i64>
// CHECK:           toolchain.store %[[C0_I64]], %[[GEP]] {alignment = 32 : i64} : vector<8xi64>, !toolchain.ptr

// -----

func.func @transfer_write_alignment_non_zero_index(%arg0: tensor<8xi64> {toolchain.align = 32 : index}) -> tensor<8xi64> {
  %c8 = arith.constant 8 : index
  %c0_i64 = arith.constant dense<0> : vector<8xi64>
...
// CHECK-SAME:  %[[ARG0:.*]]: !toolchain.ptr
// CHECK-DAG:       %[[C0_I64:.*]] = arith.constant dense<0> : vector<8xi64>
// CHECK:           %[[GEP:.*]] = toolchain.getelementptr inbounds %[[ARG0]][0, 8] :
// CHECK-SAME:        !toolchain.array<8 x i64>
// CHECK:           toolchain.store %[[C0_I64]], %[[GEP]] {alignment = 8 : i64} : vector<8xi64>, !toolchain.ptr
...
// CHECK: toolchain.mlir.global private constant
...
// CHECK-HOPPER-SAME:     %[[ADDR:.*]]: !toolchain.ptr
// CHECK-HOPPER:          toolchain.inline_asm
...
// CHECK: %[[PTR:.*]] = toolchain.getelementptr inbounds %{{.*}}[0, 8] :
// CHECK-SAME: (!toolchain.ptr) -> !toolchain.ptr, !toolchain.array<22 x i8>
// CHECK: toolchain.load %[[PTR]] {alignment = 1 : i64} : !toolchain.ptr -> vector<2xi4>
...
// CHECK: %[[PTR:.*]] = toolchain.getelementptr inbounds %arg0[0, 5] : (!toolchain.ptr) -> !toolchain.ptr, !toolchain.array<22 x i8>
