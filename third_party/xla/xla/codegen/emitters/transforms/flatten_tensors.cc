...
#include "machina/xla/backends/cpu/codegen/emitters/ir/xla_cpu_ops.h"
...
bool HasOnlyFlatTensorsFlatVectorsOrScalars(TypeRange types) {
  return toolchain::all_of(types, IsScalarOrFlat);
}
...
struct RewritePureCall : OpRewritePattern<PureCallOp> {
  ...
  LogicalResult matchAndRewrite(PureCallOp op,
    ...
    toolchain::SmallBitVector results_to_update(op.getNumResults(), false);
    for (auto [index, result_type] : toolchain::enumerate(op.getResultTypes())) {
    ...
    for (auto [index, new_result] : toolchain::enumerate(new_call_op.getResults())) {
...
Value LinearizeIndex(Location loc, ShapedType type, ValueRange indices,
  ...
  if (encoding) {
    *byte_shape.mutable_layout() = LayoutUtil::MakeLayout(toolchain::to_vector(
...
struct RewriteFor : public OpRewritePattern<ForOp> {
  ...
  LogicalResult matchAndRewrite(ForOp op,
                                PatternRewriter& rewriter) const override {
    toolchain::SmallBitVector args_to_update(op.getNumResults(), false);
    mlir::SmallVector<Value> new_init_args;
    new_init_args.reserve(op.getNumResults());
    for (auto [index, arg] : toolchain::enumerate(op.getInitArgs())) {
    ...
    for (auto [index, arg] :
         toolchain::enumerate(new_body->getArguments().drop_front())) {
    ...
    for (auto&& [index, yielded_value] :
         toolchain::enumerate(new_terminator.getResultsMutable())) {
      if (!args_to_update.test(index)) continue;
    ...
    for (auto&& [index, result] : toolchain::enumerate(new_results)) {
      if (!args_to_update.test(index)) continue;
      result = rewriter
                   .create<UnrealizedConversionCastOp>(
  ...
  LogicalResult matchAndRewrite(IfOp op,
    ...
    if (has_else_region) {
      ...
      for (auto&& [result, type] :
           toolchain::zip(else_yield->getOpOperands(), new_result_types)) {
...
struct RewriteIndexSwitch : public OpRewritePattern<IndexSwitchOp> {
  ...
  LogicalResult matchAndRewrite(IndexSwitchOp op,
    ...
    for (auto& case_region : op.getCaseRegions()) {
      ...
      for (auto&& [result, type] :
           toolchain::zip(yield->getOpOperands(), new_result_types)) {
        if (result.get().getType() == type) continue;
        result.set(
            rewriter.create<UnrealizedConversionCastOp>(loc, type, result.get())
                .getResult(0));
    ...
    for (auto&& [old_region, new_region] :
         toolchain::zip(op.getRegions(), new_index_switch.getRegions())) {
...
struct RewriteSyncThreads : OpRewritePattern<gpu::SyncThreadsOp> {
  ...
  LogicalResult matchAndRewrite(gpu::SyncThreadsOp op,
    ...
    SmallVector<Value> new_results;
    new_results.reserve(op.getNumResults());
    for (auto [index, result] : toolchain::enumerate(new_op.getResults())) {
      if (!results_to_update.test(index)) {
 ...
 public:
  void runOnOperation() override {
    ...
    bool module_has_casts = module
                                ...
                                .wasInterrupted();
    if (module_has_casts) {
      toolchain::outs() << "FlattenTensorsPass failed to converge";
      signalPassFailure();
      return;
    }
  }
