...
std::optional<Value> GetVectorBaseIndices(Value index, scf::ForOp loop,
  ...
  for (auto [index, operand] : toolchain::enumerate(apply_indexing.getOperands())) {
  ...
  auto operands = toolchain::to_vector(apply_indexing.getOperands());
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    auto yield_fn = [&](mlir::OpBuilder& yield_b, mlir::Location yield_loc,
                        toolchain::ArrayRef<mlir::BlockArgument> bbarg) {
      auto induction_var =
      ...
      auto insert_op = yield_b.create<mlir::vector::InsertOp>(
          ...
          induction_var);
      return toolchain::SmallVector<Value>{insert_op.getResult()};
    };
