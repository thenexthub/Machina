...
#include "machina/xla/codegen/device_spec.h"
#include "machina/xla/codegen/emitters/transforms/atomic_rmw_utils.h"
...
class RewriteFunctionSignatures : public OpRewritePattern<mlir::func::FuncOp> {
 public:
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!toolchain::any_of(op.getFunctionType().getInputs(), is_tensor)) {
      return rewriter.notifyMatchFailure(op,
                                         "the function has no input tensors");
    }

    bool some_tensor_result =
        toolchain::any_of(op.getFunctionType().getResults(), is_tensor);

    if (!device_spec_.IsCpu()) {
      bool all_tensor_results =
          toolchain::all_of(op.getFunctionType().getResults(), is_tensor);
    ...
    for (auto&& [index, operand] : toolchain::enumerate(new_operands)) {
    ...
    for (auto [arg, arg_type] : toolchain::zip(entry.getArguments(), new_operands)) {
...
struct RewriteFor : public OpRewritePattern<scf::ForOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    toolchain::SmallBitVector inits_to_remove(op.getNumRegionIterArgs(), false);
    SmallVector<Value> new_inits;
    new_inits.reserve(op.getNumResults());
    SmallVector<Value> ptrs;
    ptrs.reserve(op.getNumRegionIterArgs());
    for (auto [index, init] : toolchain::enumerate(op.getInitArgs())) {
    ...
    for (auto [index, arg] : toolchain::enumerate(op.getRegionIterArgs())) {
    ...
    for (auto [index, yielded_value] :
         toolchain::enumerate(new_terminator.getResults())) {
...
ml::GEPOp CreateGep(TypedValue<mlir::RankedTensorType> tensor,
  ...
  auto gep = b.create<ml::GEPOp>(
      ...
      toolchain::SmallVector<mlir::LLVM::GEPArg>{0, linear_index});
...
struct RewriteCall : OpRewritePattern<mlir::func::CallOp> {
  ...
  LogicalResult matchAndRewrite(
    ...
    if (!toolchain::any_of(op->getOperandTypes(), [](Type ty) {
          return mlir::isa<mlir::RankedTensorType>(ty);
        })) {
    ...
    for (const auto&& [index, arg] : toolchain::enumerate(op.getOperands())) {
...
class RewriteAtomicRMW : public OpRewritePattern<AtomicRMWOp> {
 ...
 private:
  toolchain::StringRef determinateScope() const {
    if (device_spec_.IsAmdGpu()) {
      return toolchain::StringRef("agent-one-as");
    }
  ...
  LogicalResult emitNvidiaVectorizedAtomicFAdd(
    ...
    // TODO(https://github.com/thenexthub/Codira/issues/122760): Switch to
  ...
  LogicalResult rewriteAtomicFMaxAsIntAtomics(Location loc, Value modifier_arg,
                                              Value addr,
                                              toolchain::StringRef sync_scope,
                                              OpBuilder& b) const {
    Type element_type = modifier_arg.getType();
