...
#include "machina/xla/codegen/emitters/ir/xla_dialect.cc.inc"
...
std::optional<Interval> GetRange(mlir::Value value) {
  auto attr_to_range = [](mlir::Attribute attr) -> std::optional<Interval> {
    ...
    auto values = toolchain::to_vector(
...
void ApplyIndexingOp::print(OpAsmPrinter& p) {
  ...
  if (num_dimensions > 0) {
    ...
    toolchain::interleaveComma(dimension_operands, p);
    p << ')';
  }
  ...
  if (num_symbols > 0) {
    ...
    toolchain::interleaveComma(symbol_operands, p);
    p << ']';
  }
...
struct FoldApplyIndexingOperands
  ...
  LogicalResult matchAndRewrite(ApplyIndexingOp indexing_op,
    ...
    for (auto [operand, constant_value] :
         toolchain::zip(indexing_op->getOpOperands(), constant_values)) {
...
mlir::LogicalResult ApplyIndexingOp::fold(
    FoldAdaptor adaptor, toolchain::SmallVectorImpl<mlir::OpFoldResult>& results) {
...
void AtomicRMWOp::getAsmResultNames(
    toolchain::function_ref<void(mlir::Value, mlir::StringRef)> setNameFn) {
...
ParseResult LoopOp::parse(OpAsmParser& parser, OperationState& result) {
  ...
  for (auto [index, iter_arg] : toolchain::enumerate(iter_args)) {
...
void LoopOp::print(OpAsmPrinter& p) {
  ...
  toolchain::interleaveComma(
      toolchain::zip(getRegionIterArgs(), getInits()), p,
...
LogicalResult LoopOp::verify() {
  ...
  for (auto [bb_arg, result_type, init] :
       toolchain::zip(getRegionIterArgs(), getResultTypes(), getInits())) {
...
struct FoldConstantDimensions : public mlir::OpRewritePattern<LoopOp> {
  ...
  LogicalResult matchAndRewrite(LoopOp loop_op,
    ...
    for (auto [operand, dim_var] :
         toolchain::zip(loop_op->getOpOperands().take_front(num_dims),
...
#include "machina/xla/codegen/emitters/ir/xla_ops.cc.inc"
