...
mlir::Type TensorShapeToMlirType(const Shape& shape, mlir::OpBuilder& b) {
  ...
  if (!LayoutUtil::IsMonotonicWithDim0Major(shape.layout())) {
    layout = CreateDenseIntElementsAttrFromVector(
        toolchain::to_vector(shape.layout().minor_to_major()), b);
  }
  return mlir::RankedTensorType::get(
      toolchain::to_vector(shape.dimensions()),
      PrimitiveTypeToMlirType(shape.element_type(), b), layout);
}

toolchain::SmallVector<mlir::Type> ShapeToMlirTypes(const Shape& shape,
                                               mlir::OpBuilder& b) {
