...
PartitionedComputation::PartitionedComputation(
  ...
  for (auto [i, instr] : toolchain::enumerate(pre_order)) {
    instr_to_id[instr] = i;
  }
  ...
  for (auto [instr_index, instr] : toolchain::enumerate(pre_order)) {
    ...
    for (auto [operand_instr, operand_maps] :
         toolchain::zip(instr->operands(), operands_indexing)) {
  ...
  for (const auto& [subgraph_data, instr] :
       toolchain::reverse(toolchain::zip(id_to_subgraph_data, pre_order))) {
  ...
  for (auto&& [function_id, instructions] : toolchain::enumerate(functions)) {
...
PartitionedComputation::Subgraph PartitionedComputation::Subgraph::ForEpilogue(
  ...
  visit = [&](const HloInstruction* instruction) {
    ...
    has_no_compute &= HasNoCompute(instruction);
    for (auto [index, operand] : toolchain::enumerate(instruction->operands())) {
      visit(operand);
    }
  };
  ...
  auto create_funcs =
      [&](absl::Span<const PartitionedComputation::Subgraph> subgraphs) {
        for (const auto& subgraph : subgraphs) {
          ...
          func_op->setAttr("toolchain.linkage", mlir::LLVM::LinkageAttr::get(
                                               module->getContext(),
...
mlir::func::FuncOp CreateSubgraphMlirFunction(
  ...
  toolchain::SmallVector<mlir::Type> parameter_types;
  ...
  toolchain::SmallVector<mlir::DictionaryAttr> arg_attrs;
  ...
  auto func_op = b.create<mlir::func::FuncOp>(
      subgraph.name, ty,
      /*attrs=*/toolchain::ArrayRef<mlir::NamedAttribute>{}, arg_attrs);
