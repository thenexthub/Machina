...
toolchain::Type* Type::TypeToIrType(Type type, toolchain::LLVMContext& context) {
  auto* elt_type = llvm_ir::PrimitiveTypeToIrType(type.element_type(), context);
  if (auto width = type.vector_width()) {
    return toolchain::VectorType::get(elt_type, *width, false);
  }
  return elt_type;
}
...
toolchain::Type* Type::to_ir_type(toolchain::LLVMContext& context) const {
