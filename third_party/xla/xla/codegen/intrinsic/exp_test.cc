...
JitRunner CreateJitRunnerWithExpF64(Type type) {
  ...
  toolchain::Function* ldexp_func =
      Ldexp::CreateDefinition(module.get(), type,
          ...
          .value();
  ldexp_func->setLinkage(toolchain::Function::ExternalLinkage);
  toolchain::Function* exp_func = Exp::CreateDefinition(module.get(), type).value();
  exp_func->setLinkage(toolchain::Function::ExternalLinkage);
  EXPECT_FALSE(toolchain::verifyFunction(*exp_func));
