...
toolchain::Value* EmitFastTanh(toolchain::IRBuilderBase* b, toolchain::Value* input,
                          bool with_fma) {
  toolchain::Type* type = input->getType();
  ...
  auto abs_x =
      llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs, {input}, {type}, b);
  auto use_aprox =
      b->CreateFCmpOLT(abs_x, toolchain::ConstantFP::get(type, kCanUseApprox));

  // To simplify the code base until it's an issue, don't have a slow min/max in
  // this approximation.
  toolchain::Value* input_clamped = llvm_ir::EmitFloatMin(
      llvm_ir::EmitFloatMax(input, toolchain::ConstantFP::get(type, minus_clamp), b,
                            /*enable_fast_min_max=*/true),
      toolchain::ConstantFP::get(type, plus_clamp), b, /*enable_fast_min_max=*/true);
  ...
  toolchain::Value* input_squared = b->CreateFMul(input_clamped, input_clamped);
  toolchain::Value* numerator = toolchain::ConstantFP::get(type, numerator_coeffs[0]);
  for (int i = 1; i < numerator_coeffs.size(); i++) {
    numerator = b->CreateFAdd(b->CreateFMul(input_squared, numerator),
                              toolchain::ConstantFP::get(type, numerator_coeffs[i]));
  }

  numerator = b->CreateFMul(input_clamped, numerator);

  toolchain::Value* denominator = toolchain::ConstantFP::get(type, denominator_coeffs[0]);
  for (int i = 1; i < denominator_coeffs.size(); i++) {
    denominator =
        b->CreateFAdd(b->CreateFMul(input_squared, denominator),
                      toolchain::ConstantFP::get(type, denominator_coeffs[i]));
  }
...
toolchain::Value* EmitFastTanhF64(toolchain::IRBuilderBase* b, toolchain::Value* input,
                             bool with_fma) {
  toolchain::Type* type = input->getType();
  ...
  toolchain::Value* plus_clamp = toolchain::ConstantFP::get(type, clamp);
  toolchain::Value* minus_clamp = toolchain::ConstantFP::get(type, -clamp);
  toolchain::Value* x = llvm_ir::EmitFloatMin(
  ...
  toolchain::Value* x2 = b->CreateFMul(x, x);  // x^2.

  // Compute numerator polynomial.
  toolchain::Value* numerator = toolchain::ConstantFP::get(type, numerator_coeffs[0]);
  for (int i = 1; i < numerator_coeffs.size(); ++i) {
    toolchain::Value* alpha = toolchain::ConstantFP::get(type, numerator_coeffs[i]);
  ...
  toolchain::Value* denominator = toolchain::ConstantFP::get(type, denominator_coeffs[0]);
  for (int i = 1; i < denominator_coeffs.size(); i++) {
    toolchain::Value* beta = toolchain::ConstantFP::get(type, denominator_coeffs[i]);
...
absl::StatusOr<toolchain::Function*> Tanh::CreateDefinition(toolchain::Module* module,
                                                       Type type) {
  ...
  toolchain::Function* func = Tanh::GetOrInsertDeclaration(module, type);
  toolchain::Argument* input_x_arg = func->getArg(0);
  input_x_arg->setName("input_x");
  toolchain::BasicBlock* entry_bb =
      toolchain::BasicBlock::Create(module->getContext(), "entry", func);
  ...
  if (type.element_type() == F64) {
    ...
    toolchain::Value* f32_input = builder.CreateFPCast(
        input_x_arg,
