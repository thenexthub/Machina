...
#ifndef MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_INTRINSIC_H_
#define MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_INTRINSIC_H_
...
class Type : public std::variant<Scalar, Vec> {
 public:
  ...
  toolchain::Type* to_ir_type(toolchain::LLVMContext& context) const;
  ...
  static toolchain::Type* TypeToIrType(Type type, toolchain::LLVMContext& context);
  ...
  static Type TypeFromIrType(toolchain::Type* type);
...
// (i.e. toolchain.tanh.* is lowered to libm call), or to the 'compiler-rt' function
...
class Intrinsic {
 public:
  ...
  static absl::StatusOr<toolchain::Function*> CreateDefinition(toolchain::Module* module,
                                                          const Args... types) {
  ...
  template <typename... Args>
  static toolchain::Function* GetOrInsertDeclaration(toolchain::Module* module,
                                                Args... args) {
    static_assert(sizeof...(Args) > 0, "At least one argument is required.");
    static_assert((std::is_convertible_v<Args, Type> && ...),
                  "All arguments must be intrinsic::Type.");
    std::vector<toolchain::Type*> types{
        Type::TypeToIrType(args, module->getContext())...};
    toolchain::Type* return_type = types.front();
    if (Derived::kLastArgIsReturnType) {
      return_type = types.back();
      types.pop_back();
    }
    auto* function_type = toolchain::FunctionType::get(return_type, types, false);
    return toolchain::cast<toolchain::Function>(
...
#endif  // MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_INTRINSIC_H_
