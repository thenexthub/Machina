...
JitRunner CreateJitRunner(Type type) {
  ...
  toolchain::Function* erf_func = Erf::CreateDefinition(module.get(), type).value();
  erf_func->setLinkage(toolchain::Function::ExternalLinkage);
  EXPECT_FALSE(toolchain::verifyFunction(*erf_func));
