...
#ifndef MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_SIMPLE_JIT_RUNNER_H_
#define MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_SIMPLE_JIT_RUNNER_H_
...
class JitRunner {
 public:
  explicit JitRunner(std::unique_ptr<toolchain::Module> module,
  ...
  FnType* GetScalarFn(const std::string& function_name) {
    ...
    if (!function_sym) {
      LOG(FATAL) << "Failed to lookup function: " << function_name << ": "
                 << toolchain::toString(function_sym.takeError());
    }
  ...
  GetVectorizedFn(const std::string& original_function_name,
    ...
    toolchain::Expected<void*> wrapper_ptr_or_err = CreateVectorWrapperWithLoop(
 ...
 private:
  std::unique_ptr<toolchain::orc::LLJIT> jit_;
  std::unique_ptr<toolchain::orc::ThreadSafeContext> tsc_;
  std::unique_ptr<toolchain::orc::RTDyldObjectLinkingLayer> object_layer_;
  toolchain::JITEventListener* perf_listener_ = nullptr;

  toolchain::Expected<void*> CreateVectorWrapperWithLoop(
...
#endif  // MACHINA_MACHINA_XLA_CODEGEN_INTRINSIC_SIMPLE_JIT_RUNNER_H_
