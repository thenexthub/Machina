...
static toolchain::Value* EvaluatePolynomial(toolchain::Type* type, toolchain::Value* x,
  ...
  for (const double c : coefficients) {
    poly = builder.CreateFAdd(builder.CreateFMul(poly, x),
                              toolchain::ConstantFP::get(type, c));
  }
  return poly;
}

absl::StatusOr<toolchain::Function*> Log1p::CreateDefinition(toolchain::Module* module,
                                                        Type intrinsic_type) {
  ...
  auto for_large_x = llvm_ir::EmitCallToIntrinsic(
      toolchain::Intrinsic::log, {builder.CreateFAdd(x, one)}, {type}, &builder);
  ...
  auto abs_x = llvm_ir::EmitCallToIntrinsic(toolchain::Intrinsic::fabs,
                                            {input_value}, {type}, &builder);
  auto x_is_small = builder.CreateFCmpOLT(
      abs_x, toolchain::ConstantFP::get(type, kAntilogarithmIsSmallThreshold));
