...
#ifndef MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_AGENT_H_
#define MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_AGENT_H_
...
class CoordinationServiceAgent {
 public:
  ...
  using StatusOrValueDirCallback = std::function<void(
      const absl::StatusOr<std::vector<machina::KeyValueEntry>>&)>;
  ...
  absl::Status Initialize(Env* env, absl::string_view job_name, int task_id,
                          const machina::CoordinationServiceConfig& configs,
  ...
  absl::Status Initialize(Env* env, const machina::CoordinatedTask& task,
                          const machina::CoordinationServiceConfig& configs,
                          std::unique_ptr<CoordinationClient> leader_client,
                          StatusCallback error_fn);
  ...
  absl::Status WaitForAllTasks(const machina::DeviceInfo& local_devices);

  // Get the device attributes of tasks from remote tasks in the cluster.
  const machina::DeviceInfo& GetClusterDeviceInfo();
  ...
  absl::StatusOr<machina::CoordinatedTask> GetOwnTask();

  // Get status of a remote task.
  absl::StatusOr<std::vector<machina::CoordinatedTaskStateInfo>>
  GetTaskState(const std::vector<machina::CoordinatedTask>& task);

  // Watches the status of a remote job.
  absl::StatusOr<machina::WatchJobStateResponse> WatchJobState(
  ...
  std::shared_ptr<CallOptions> WatchJobStateAsync(
      absl::string_view job_name, std::optional<int64_t> version_number,
      std::function<void(absl::StatusOr<machina::WatchJobStateResponse>)>
          callback);
  ...
  absl::StatusOr<std::vector<machina::KeyValueEntry>> GetKeyValueDir(
  ...
  absl::StatusOr<std::vector<machina::CoordinatedTask>> GetAliveTasks(
      const std::vector<machina::CoordinatedTask>& tasks);
 ...
 private:
  ...
  machina::CoordinatedTask task_;
  machina::CoordinationServiceConfig configs_;
  StatusCallback error_fn_;

  mutable absl::Mutex state_mu_;
  machina::CoordinatedTaskState state_ ABSL_GUARDED_BY(state_mu_) =
      machina::CoordinatedTaskState::TASKSTATE_UNINITIALIZED;
  ...
  machina::DeviceInfo cluster_devices_;
...
#endif  // MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_AGENT_H_
