...
#ifndef MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_H_
#define MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_H_
...
#include "machina/xla/tsl/distributed_runtime/coordination/key_value_store.h"
#include "machina/xla/tsl/lib/gtl/int_type.h"
#include "machina/xla/tsl/platform/env.h"
#include "machina/xla/tsl/platform/status.h"
#include "machina/xla/tsl/protobuf/coordination_config.pb.h"
...
class CoordinationService {
 public:
  ...
  using GetAliveTasksCallback = std::function<void(
      const absl::Status&, const std::vector<machina::CoordinatedTask>&,
  ...
  void SetDeviceAggregationFunction(std::function<machina::DeviceInfo(
                                        const machina::DeviceInfo& devices)>
  ...
  absl::Status RegisterTask(const machina::CoordinatedTask& task,
                            IncarnationId incarnation);
  void RegisterTaskAsync(const machina::CoordinatedTask& task,
  ...
  void WaitForAllTasks(const machina::CoordinatedTask& task,
                       const machina::DeviceInfo& devices,
                       StatusCallback done);
  ...
  void ShutdownTaskAsync(const machina::CoordinatedTask& task,
                         StatusCallback done);
  ...
  //   - InvalidArgument: Unexpected task request.
  //   - FailedPrecondition: task has already disconnected.
  absl::Status ResetTask(const machina::CoordinatedTask& task);

  // Update the heartbeat timestamp of a task. This should only be invoked on
  // the leader of the cluster.
  //   - Internal: Service has shut down.
  absl::Status RecordHeartbeat(const machina::CoordinatedTask& task,
                               IncarnationId incarnation);

  // Set a task in error state permanently.
  absl::Status ReportTaskError(const machina::CoordinatedTask& task,
                               const absl::Status& error);

  // Get the state and the error status of the tasks.
  std::vector<machina::CoordinatedTaskStateInfo> GetTaskState(
      const std::vector<machina::CoordinatedTask>& task);

  // Watches the state and the error status of the job.
  using WatchJobStateCallback = absl::AnyInvocable<void(
      std::vector<machina::CoordinatedTaskStateInfo>, int64_t)>;
  ...
  std::vector<machina::KeyValueEntry> GetKeyValueDir(
  ...
  void BarrierAsync(
      ...
      const machina::CoordinatedTask& task,
      const std::vector<machina::CoordinatedTask>& participating_tasks,
  ...
  void GetAliveTasksAsync(const machina::CoordinatedTask& requesting_task,
                          const std::vector<machina::CoordinatedTask>& tasks,
  ...
  void PollForErrorAsync(const machina::CoordinatedTask& task,
                         StatusCallback done);
 ...
 private:
  ...
  const machina::DeviceInfo& ListClusterDevices()
  ...
  void ConnectTask(const machina::CoordinatedTask& task,
                   IncarnationId incarnation)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  ...
  void PropagateError(
      const absl::Status& error,
      const std::vector<machina::CoordinatedTask>& source_tasks,
  ...
  void SetAllTasksError(const absl::Status& error)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  absl::Status DisconnectTask(const machina::CoordinatedTask& task)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  void DisconnectAllNonRecoverableTasks()
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  std::vector<machina::CoordinatedTask> GetTasksForShutdownBarrier();
  ...
  struct BarrierState {
    ...
    absl::flat_hash_map<machina::CoordinatedTask, bool, CoordinatedTaskHash,
                        CoordinatedTaskEqual>
        tasks_at_barrier;
    absl::flat_hash_set<machina::CoordinatedTask, CoordinatedTaskHash,
                        CoordinatedTaskEqual>
        recoverable_tasks_restarted_during_barrier;
    absl::flat_hash_map<machina::CoordinatedTask, BarrierCallback,
                        CoordinatedTaskHash, CoordinatedTaskEqual>
        done_callbacks;
    // Specifies the task that initiated the barrier (the first task to call the
    // barrier).
    machina::CoordinatedTask initiating_task;
  };
  ...
  bool InitializeBarrier(
      ...
      absl::Duration timeout, const machina::CoordinatedTask& task,
      const std::vector<machina::CoordinatedTask>& participating_tasks,
      BarrierCallback done) ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  ...
  void LeaveOngoingBarriers(const machina::CoordinatedTask& task,
  ...
  bool ValidateTaskArgs(
      BarrierState* barrier, const machina::CoordinatedTask& caller_task,
      const std::vector<machina::CoordinatedTask>& tasks_args)
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  ...
  class ErrorPollingState {
   public:
    ...
    void AddTask(const machina::CoordinatedTask& task,
                 StatusCallback&& done);
    ...
    void RemoveTask(const machina::CoordinatedTask& task,
                    absl::string_view reason);

   private:
    bool responded_ = false;
    absl::Status error_ = absl::OkStatus();
    absl::flat_hash_map<machina::CoordinatedTask, StatusCallback,
                        CoordinatedTaskHash, CoordinatedTaskEqual>
        done_callbacks_;
  ...
  class TaskState {
   public:
    ...
    machina::CoordinatedTaskState GetState() const { return state_; }
    ...
    machina::DeviceInfo GetDeviceInfo() { return devices_; }
    void CollectDeviceInfo(const machina::DeviceInfo& devices) {
      devices_ = devices;
    }
   ...
   private:
    ...
    machina::CoordinatedTaskState state_ =
        machina::CoordinatedTaskState::TASKSTATE_DISCONNECTED;
    absl::Status status_;
  ...
  std::vector<IncarnationId> IncarnationIds(
      absl::Span<const machina::CoordinatedTask> tasks) const
      ABSL_EXCLUSIVE_LOCKS_REQUIRED(state_mu_);
  ...
  static machina::CoordinatedTaskStateInfo CreateTaskStateInfo(
      const machina::CoordinatedTask& task, const TaskState& state);

  // Gets the task states for the provided job.
  std::vector<machina::CoordinatedTaskStateInfo> GetJobState(
  ...
  std::function<machina::DeviceInfo(const machina::DeviceInfo& devices)>
  ...
  std::vector<machina::CoordinatedTask> shutdown_barrier_tasks_
  ...
  machina::DeviceInfo cluster_devices_ ABSL_GUARDED_BY(state_mu_);
...
#endif  // MACHINA_MACHINA_XLA_TSL_DISTRIBUTED_RUNTIME_COORDINATION_COORDINATION_SERVICE_H_
