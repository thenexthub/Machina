...
#include "machina/xla/tsl/distributed_runtime/coordination/coordination_service_error_util.h"
...
namespace tsl {
namespace {
using machina::CoordinatedTask;
using machina::CoordinationServiceError;
using machina::KeyValueEntry;
}  // namespace
...
void CoordinationServiceRpcHandler::RegisterTaskAsync(
    const machina::RegisterTaskRequest* request,
    machina::RegisterTaskResponse* response, StatusCallback done) {
...
void CoordinationServiceRpcHandler::HeartbeatAsync(
    const machina::HeartbeatRequest* request,
    machina::HeartbeatResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::WaitForAllTasksAsync(
    const machina::WaitForAllTasksRequest* request,
    machina::WaitForAllTasksResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::ShutdownTaskAsync(
    const machina::ShutdownTaskRequest* request,
    machina::ShutdownTaskResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::ResetTaskAsync(
    const machina::ResetTaskRequest* request,
    machina::ResetTaskResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::GetTaskStateAsync(
    const machina::GetTaskStateRequest* request,
    machina::GetTaskStateResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::WatchJobStateAsync(
    const machina::WatchJobStateRequest* request,
    machina::WatchJobStateResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
  ...
  service_->WatchJobState(
      ...
      [response, done](std::vector<machina::CoordinatedTaskStateInfo> info,
...
void CoordinationServiceRpcHandler::InsertKeyValueAsync(
    const machina::InsertKeyValueRequest* request,
    machina::InsertKeyValueResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::GetKeyValueAsync(
    const machina::GetKeyValueRequest* request,
    machina::GetKeyValueResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::TryGetKeyValueAsync(
    const machina::TryGetKeyValueRequest* request,
    machina::TryGetKeyValueResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::GetKeyValueDirAsync(
    const machina::GetKeyValueDirRequest* request,
    machina::GetKeyValueDirResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::DeleteKeyValueAsync(
    const machina::DeleteKeyValueRequest* request,
    machina::DeleteKeyValueResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::BarrierAsync(
    const machina::BarrierRequest* request,
...
void CoordinationServiceRpcHandler::CancelBarrierAsync(
    const machina::CancelBarrierRequest* request,
    machina::CancelBarrierResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
...
void CoordinationServiceRpcHandler::GetAliveTasksAsync(
    const machina::GetAliveTasksRequest* request,
    machina::GetAliveTasksResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
  ...
  service_->GetAliveTasksAsync(
      ...
      [done = std::move(done), response](
          ...
          const std::vector<machina::CoordinatedTask>& alive_tasks,
...
void CoordinationServiceRpcHandler::PollForErrorAsync(
    const machina::PollForErrorRequest* request,
    machina::PollForErrorResponse* response, StatusCallback done) {
  absl::ReaderMutexLock l(&mu_);
  if (service_ == nullptr) {
    done(MakeCoordinationError(
        absl::InternalError("Coordination service is not enabled.")));
