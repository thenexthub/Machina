...
using machina::CoordinatedTaskState;
using machina::CoordinatedTaskStateInfo;
...
absl::StatusOr<machina::WatchJobStateResponse>
CoordinationServiceAgent::WatchJobState(absl::string_view job_name,
  ...
  WatchJobStateAsync(
      ...
      [&response, &done](absl::StatusOr<machina::WatchJobStateResponse> r) {
        response = std::move(r);
        done.Notify();
      });
...
absl::StatusOr<std::vector<machina::CoordinatedTask>>
CoordinationServiceAgent::GetAliveTasks(
  ...
  *request->mutable_tasks() = {tasks.begin(), tasks.end()};
  ...
  return std::vector<machina::CoordinatedTask>(
