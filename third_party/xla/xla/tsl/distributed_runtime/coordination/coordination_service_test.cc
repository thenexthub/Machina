...
#include "machina/xla/tsl/distributed_runtime/coordination/test_device.pb.h"
...
using machina::CoordinatedJob;
using machina::CoordinatedTask;
using machina::CoordinationServiceConfig;
using machina::DeviceInfo;
using machina::KeyValueEntry;
using machina::TestDevice;
using machina::TestDeviceList;
...
machina::CoordinatedTaskStateInfo info(
    const machina::CoordinatedTask& task, IncarnationId incarnation_id,
    machina::CoordinatedTaskState state) {
  machina::CoordinatedTaskStateInfo info;
...
TEST_F(CoordinateTwoTasksTest, WatchJobStateSucceeds) {
  ...
  coord_service_->WatchJobState(
      "worker", std::nullopt,
      [&, this](std::vector<machina::CoordinatedTaskStateInfo> got,
                int64_t version_number) {
        using State = machina::CoordinatedTaskState;
        std::vector<machina::CoordinatedTaskStateInfo> want(2);
...
TEST_F(CoordinateTwoTasksTest, WatchJobStateBlocksUntilChange) {
  ...
  coord_service_->WatchJobState(
      "worker", std::nullopt,
      [&](std::vector<machina::CoordinatedTaskStateInfo> got, int64_t v) {
        EXPECT_THAT(v, Ge(0));
        version_number = v;
        done_1.Notify();
      });
