...
#include "machina/xla/tsl/distributed_runtime/rpc/coordination/grpc_coordination_service_impl.h"
...
machina::CoordinatedTask GetTask(int node_id) {
  machina::CoordinatedTask task;
...
class ClientServerTest : public ::testing::Test {
 public:
  CoordinationServiceConfig GetConfig(
    ...
    // Set config.
    machina::CoordinationServiceConfig config;
...
TEST_F(ClientServerTest, GetAliveTasks_Succeed) {
  ...
  auto thread_fn = [&](int node_id) -> absl::Status {
    auto client = GetClient(node_id);
    TF_RETURN_IF_ERROR(client->Connect());
    absl::StatusOr<std::vector<machina::CoordinatedTask>> alive_tasks =
