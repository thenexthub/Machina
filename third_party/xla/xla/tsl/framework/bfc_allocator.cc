...
#include "machina/xla/tsl/profiler/utils/trace_filter_utils.h"
#include "machina/xla/tsl/protobuf/bfc_memory_map.pb.h"
...
MemoryDump BFCAllocator::RecordMemoryMapInternal() {
  ...
  machina::MemAllocatorStats* mas = md.mutable_stats();
  ...
  for (BinNum bin_num = 0; bin_num < kNumBins; bin_num++) {
    ...
    DCHECK_EQ(b->free_chunks.size(),
              bin_info.total_chunks_in_bin - bin_info.total_chunks_in_use);
    machina::BinSummary* bs = md.add_bin_summary();
    bs->set_bin(bin_num);
  ...
  for (const auto& region : region_manager_.regions()) {
    ...
    while (h != kInvalidChunkHandle) {
      const Chunk* c = ChunkFromHandle(h);
      machina::MemChunk* mc = md.add_chunk();
  ...
  for (int i = action_counter_ - history_len; i < action_counter_; ++i) {
    machina::SnapShot* ss = md.add_snap_shot();
