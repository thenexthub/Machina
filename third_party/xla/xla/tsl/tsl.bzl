...
def if_cuda_or_rocm(if_true, if_false = []):
    """Shorthand for select()'ing whether to build for either CUDA or ROCm.
      ...
      dependencies. For eg, see the `//machina/core/kernels:bias_op` target
...
def tsl_copts(
    ...
    return (
        ...
        select({
            ...
            clean_dep("//xla/tsl:no_lgpl_deps"): ["-D__MACHINA_NO_LGPL_DEPS__", "-pthread"],
...
def tf_openmp_copts():
    ...
    return select({
        ...
        # copybara:uncomment_end_and_comment_begin
        clean_dep("//xla/tsl/mkl:build_with_mkl_windows_openmp"): ["/openmp:toolchain"],
        # copybara:comment_end
        "//conditions:default": [],
    })
...
def tsl_gpu_library(deps = None, cuda_deps = None, copts = tsl_copts(), **kwargs):
    ...
    cc_library(
        ...
        copts = (copts + if_cuda(["-DGOOGLE_CUDA=1", "-DNV_CUDNN_DISABLE_EXCEPTION"]) + if_rocm(["-DMACHINA_USE_ROCM=1"]) + if_xla_available(["-DMACHINA_USE_XLA=1"]) + if_mkl(["-DINTEL_MKL=1"]) + if_enable_mkl(["-DENABLE_MKL"]) + if_tensorrt(["-DGOOGLE_TENSORRT=1"])),
        **kwargs
    )
