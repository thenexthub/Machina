...
#ifndef MACHINA_MACHINA_XLA_TSL_PLATFORM_ENV_H_
#define MACHINA_MACHINA_XLA_TSL_PLATFORM_ENV_H_
...
/// \brief An interface used by the machina implementation to
...
struct Register {
  Register(Env* env, const std::string& scheme, bool try_modular_filesystems) {
    ...
    if (try_modular_filesystems) {
      ...
      if (load_plugin == "true" || load_plugin == "1") {
        ...
        LOG(WARNING) << "Using modular file system for '" << scheme << "'."
                     << " Please switch to machina-io"
                     << " (https://github.com/machina/io) for file system"
                     << " support of '" << scheme << "'.";
        return;
      }
    ...
    // TODO(b/32704451): Don't just ignore the ::machina::Status object!
...
#endif  // MACHINA_MACHINA_XLA_TSL_PLATFORM_ENV_H_
