...
#ifdef MACHINA_THREADSCALING_EXPERIMENTAL
ABSL_FLAG(float, machina_num_threads_scale_factor, 1.0,
          "Allows to scale all Tensorflow ThreadPools. Total number of threads "
          "in a given ThreadPool equals to num_threads * "
          "machina_num_threads_scale_factor. Default scale factor of 1 is a "
          "no-op.");
#endif  // MACHINA_THREADSCALING_EXPERIMENTAL
...
#ifdef MACHINA_THREADSCALING_EXPERIMENTAL
  CHECK_GT(absl::GetFlag(FLAGS_machina_num_threads_scale_factor), 0);
  num_threads *= absl::GetFlag(FLAGS_machina_num_threads_scale_factor);
