...
TEST(Status, SaveOKStatusToProto) {
  machina::StatusProto status_proto = StatusToProto(absl::OkStatus());
  EXPECT_EQ(status_proto.code(), error::OK);
  EXPECT_THAT(status_proto.message(), IsEmpty());
}

TEST(Status, SaveErrorStatusToProto) {
  machina::StatusProto status_proto = StatusToProto(errors::Create(
...
TEST(Status, SaveEmptyStatusToProto) {
  machina::StatusProto status_proto = StatusToProto(absl::Status());
  EXPECT_EQ(status_proto.code(), error::OK);
  EXPECT_THAT(status_proto.message(), IsEmpty());
...
TEST(Status, MakeOKStatusFromProto) {
  machina::StatusProto status_proto;
...
TEST(Status, MakeStatusFromEmptyProto) {
  EXPECT_THAT(StatusFromProto(machina::StatusProto()), absl_testing::IsOk());
}
