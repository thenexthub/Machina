...
#ifndef MACHINA_MACHINA_XLA_TSL_PROFILER_UTILS_GROUP_EVENTS_H_
#define MACHINA_MACHINA_XLA_TSL_PROFILER_UTILS_GROUP_EVENTS_H_
...
class EventForest {
 public:
  void AddSpace(
      std::function<XPlaneVisitor(const machina::profiler::XPlane*)>
          visitor_factory,
  ...
  void AddPlanes(
      std::function<XPlaneVisitor(const machina::profiler::XPlane*)>
          visitor_factory,
      const std::vector<machina::profiler::XPlane*>& planes);
 ...
 private:
  ...
  void ConnectIntraThread(machina::profiler::XPlane* plane,
  ...
  std::deque<std::pair<machina::profiler::XPlane*, XPlaneVisitor>> planes_;
...
// TensorFlow.
void GroupTfEvents(machina::profiler::XSpace* space,
                   EventForest* event_forest);
void GroupTfEvents(machina::profiler::XSpace* space);

// Returns true if the given space has TF's loop ops.
bool CheckLoopOp(const machina::profiler::XSpace& space);
...
void GroupHostAndPlanes(
    machina::profiler::XSpace* space,
    const std::vector<machina::profiler::XPlane*>& device_traces,
    EventForest* event_forest);

void GroupXplaneEvents(machina::profiler::XPlane* plane,
...
#endif  // MACHINA_MACHINA_XLA_TSL_PROFILER_UTILS_GROUP_EVENTS_H_
