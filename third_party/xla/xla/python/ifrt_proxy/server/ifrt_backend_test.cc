...
#include "machina/xla/python/ifrt_proxy/server/ifrt_backend.h"
...
#include "machina/xla/python/ifrt/mock.h"
#include "machina/xla/python/ifrt/program.h"
#include "machina/xla/python/ifrt/program_serdes.h"
#include "machina/xla/python/ifrt/serdes.h"
#include "machina/xla/python/ifrt/serdes_version.h"
#include "machina/xla/python/ifrt/shape.h"
#include "machina/xla/python/ifrt/sharding.h"
#include "machina/xla/python/ifrt_proxy/common/array_util.h"
#include "machina/xla/python/ifrt_proxy/common/ifrt_service.pb.h"
#include "machina/xla/python/ifrt_proxy/common/types.pb.h"
#include "machina/xla/python/ifrt_proxy/common/versions.h"
#include "machina/xla/python/ifrt_proxy/server/host_buffer.h"
#include "machina/xla/python/ifrt_proxy/server/host_callback.h"
#include "machina/xla/python/ifrt_proxy/server/version.h"
...
#include "machina/xla/tsl/platform/status_to_from_proto.h"
...
struct TestProgram : toolchain::RTTIExtends<TestProgram, Program> {
  static char ID;  // NOLINT
};

[[maybe_unused]] char TestProgram::ID = 0;  // NOLINT

class TestProgramSerDes : public toolchain::RTTIExtends<TestProgramSerDes, SerDes> {
 public:
  ...
  absl::StatusOr<std::string> Serialize(
    ...
    CHECK(toolchain::isa<TestProgram>(serializable));
    return "";
  }
  ...
  absl::StatusOr<std::unique_ptr<Serializable>> Deserialize(
    ...
    const auto* deserialize_program_options =
        toolchain::cast<DeserializeProgramOptions>(options.get());
...
struct TestCompileOptions
    : toolchain::RTTIExtends<TestCompileOptions, CompileOptions> {
  static char ID;  // NOLINT
};
...
class TestCompileOptionsSerDes
    : public toolchain::RTTIExtends<TestCompileOptionsSerDes, SerDes> {
 public:
  absl::string_view type_name() const override {
  ...
  absl::StatusOr<std::string> Serialize(
      const Serializable& serializable,
      std::unique_ptr<SerializeOptions>) override {
    CHECK(toolchain::isa<TestCompileOptions>(serializable));
    return "";
  }
            ...
            Invoke(
                [&](const std::unique_ptr<xla::ifrt::Program>& program,
                  ...
                  auto* xla_compile_options =
                      toolchain::cast<xla::ifrt::XlaCompileOptions>(options.get());
  ...
  auto host_callback_thread = absl::WrapUnique(tsl::Env::Default()->StartThread(
      tsl::ThreadOptions(), "HostCallback", [&]() {
        ...
        const xla::HostCallback* xla_host_callback =
            &toolchain::cast<RemoteLoadedHostCallback>(loaded_host_callback.get())
                 ->host_callback();
        ASSERT_THAT(
