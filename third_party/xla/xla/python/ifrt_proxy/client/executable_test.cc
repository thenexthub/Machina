...
#include "machina/xla/python/ifrt_proxy/client/executable.h"
...
TEST_F(LoadedExecutableTest, Execute) {
  ...
  EXPECT_EQ(toolchain::cast<Array>(output0.get())
  ...
  EXPECT_EQ(toolchain::cast<Array>(output1.get())
                ->GetHandleUnknownIfBeingDonated()
                ->handle,
            3001);
  ...
  EXPECT_EQ(toolchain::cast<Array>(result.outputs[0].get())
                ->GetHandleUnknownIfBeingDonated()
                ->handle,
            execute_req.result_array_handle()[0]);
  EXPECT_EQ(toolchain::cast<Array>(result.outputs[1].get())
                ->GetHandleUnknownIfBeingDonated()
                ->handle,
