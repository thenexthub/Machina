  ...
  if (auto* xla_options =
    ...
    for (const auto& loaded_host_callback :
      ...
      auto* pjrt_host_callback =
          toolchain::dyn_cast<xla::ifrt::PjRtHostSendAndRecvLoadedHostCallback>(
    ...
    // behavior between OSS MACHINA_MACHINA_XLA_FLAGS and XLA TPU flags. MACHINA_MACHINA_XLA_FLAGS would be
    // captured here and propagated to the server, but XLA TPU flags would not.
    //
    // With the current implementation both MACHINA_MACHINA_XLA_FLAGS and XLA TPU flags should
