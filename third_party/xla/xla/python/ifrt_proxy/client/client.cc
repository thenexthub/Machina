...
#include "machina/xla/python/ifrt_proxy/client/client.h"
...
#include "machina/xla/python/ifrt_proxy/client/array.h"
#include "machina/xla/python/ifrt_proxy/client/device.h"
#include "machina/xla/python/ifrt_proxy/client/memory.h"
...
absl::StatusOr<std::unique_ptr<Client>> Client::Create(
  ...
  for (ifrt::Device* device : all_device_ptrs) {
    machina::down_cast<Device*>(device)->client_ = client.get();
  }
  return client;
}
...
absl::StatusOr<std::vector<xla::ifrt::ArrayRef>> Client::CopyArrays(
  ...
  for (const auto& array : arrays) {
    if (auto* proxy_array =
            toolchain::dyn_cast<xla::ifrt::proxy::Array>(array.get())) {
  ...
  for (int i = 0; i < result_handles.size(); ++i) {
    ...
    auto* proxy_array = toolchain::cast<xla::ifrt::proxy::Array>(arrays[i].get());
...
xla::ifrt::Future<> Client::GetReadyFuture(
  ...
  for (const auto& value : values) {
    ...
    if (auto proxy_array =
            toolchain::dyn_cast<xla::ifrt::proxy::Array>(value.get())) {
...
Client::GetDefaultPjRtLayout(xla::ifrt::DType dtype,
  ...
  LayoutKey key{
      ...
      /*device_summary=*/device_summary(toolchain::dyn_cast<Device>(device))};
