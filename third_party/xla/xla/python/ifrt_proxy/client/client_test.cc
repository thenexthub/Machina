...
TEST_P(ClientTest, CopyArraysDefaultLayoutSuccess) {
  ...
  EXPECT_EQ(toolchain::cast<Array>(copied_arrays[0].get())->custom_layout(),
            nullptr);
  EXPECT_EQ(toolchain::cast<Array>(copied_arrays[1].get())->custom_layout(),
            nullptr);
}
...
TEST_P(ClientTest, CopyArraysCustomLayoutSuccess) {
  ...
  ASSERT_THAT(copied_arrays, SizeIs(2));
  EXPECT_EQ(
      toolchain::cast<Array>(copied_arrays[0].get())->custom_layout()->ToString(),
      layout_1_->ToString());
  EXPECT_EQ(
      toolchain::cast<Array>(copied_arrays[1].get())->custom_layout()->ToString(),
      layout_2_->ToString());
}
