...
#include "machina/xla/python/transfer/pjrt_transfer_server.h"
...
#include "machina/xla/python/transfer/event_loop.h"
#include "machina/xla/python/transfer/socket-server.h"
#include "machina/xla/python/transfer/socket_bulk_transport.h"
#include "machina/xla/python/transfer/streaming.h"
#include "machina/xla/python/transfer/streaming_ifrt.h"
...
absl::StatusOr<xla::PjRtMemorySpace*> GetMemorySpace(
  ...
  if (memory_kind.has_value()) {
    ...
    return machina::down_cast<xla::ifrt::PjRtMemory*>(memory)->pjrt_memory();
  }
  return machina::down_cast<xla::ifrt::PjRtDevice*>(device)
      ->pjrt_device()
...
absl::Status PjRtTransferServer::CrossHostAwaitPull(
  ...
  for (xla::ifrt::ArrayRef& arr : arrays) {
    auto* pjrt_arr = toolchain::dyn_cast_or_null<xla::ifrt::PjRtArray>(arr.get());
    if (pjrt_arr == nullptr) {
