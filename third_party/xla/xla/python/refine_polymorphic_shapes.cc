...
==============================================================================*/
#include "machina/xla/python/refine_polymorphic_shapes.h"
...
constexpr toolchain::StringRef shapeAssertionName = "shape_assertion";
constexpr toolchain::StringRef errorMessageAttrName = "error_message";
// We bound the number of error_message_inputs for using toolchain::formatv
...
// done with the `toolchain::formatv` function
// (https://toolchain.org/docs/ProgrammersManual.html#formatting-strings-the-formatv-function).
//
 ...
 private:
  void runOnOperation() final {
    ...
    func_op.walk([this](mlir::stablehlo::CustomCallOp op) {
      ...
      toolchain::StringRef errorMessage = getErrorMessage(op);
  ...
  mlir::LogicalResult verifyShapeAssertion(mlir::stablehlo::CustomCallOp op) {
    ...
    toolchain::Regex formatSpecifierRE = toolchain::Regex("{([0-9]+)[,:}]");
    do {
      mlir::SmallVector<toolchain::StringRef> formatSpec;
  ...
  toolchain::StringRef getErrorMessage(mlir::stablehlo::CustomCallOp op) const {
    return mlir::cast<mlir::StringAttr>(op->getAttr(errorMessageAttrName))
        .getValue();
  }

  std::string formatErrorMessage(
      toolchain::StringRef errorMessage,
  ...
  Option<bool> enable_shape_assertions{
      *this, "enable-shape-assertions",
      toolchain::cl::desc("Whether shape assertions may generate errors."),
...
absl::Status RefinePolymorphicShapes(toolchain::StringRef module_str,
  ...
  mlir::OwningOpRef<mlir::ModuleOp> module =
      mlir::parseSourceString<mlir::ModuleOp>(
          toolchain::StringRef(module_str.data(), module_str.size()), &context);
  if (!module) {
...
absl::Status ValidateStaticShapes(mlir::ModuleOp module) {
  ...
  module->walk([&](mlir::Operation *op) {
    ...
    opHasDynamicShapes |= toolchain::any_of(op->getResults(), hasDynamicShape);
    for (mlir::Region &region : op->getRegions()) {
      opHasDynamicShapes |=
          toolchain::any_of(region.getArguments(), hasDynamicShape);
    }
