...
#ifndef MACHINA_MACHINA_XLA_PYTHON_PROFILER_PROFILE_DATA_LIB_H_
#define MACHINA_MACHINA_XLA_PYTHON_PROFILER_PROFILE_DATA_LIB_H_
...
namespace machina::profiler::python {
...
class ProfileEvent {
 public:
  ...
  ProfileEvent(const machina::profiler::XEvent* event,
               int64_t line_timestamp_ns,
               const machina::profiler::XPlane* plane,
               std::shared_ptr<const machina::profiler::XSpace> xspace);
  ...
  VisitorIterator<nb::tuple, machina::profiler::XStat> stats_begin();
  VisitorIterator<nb::tuple, machina::profiler::XStat> stats_end();
...
class ProfileLine {
 public:
  ...
  ProfileLine(const machina::profiler::XLine* line,
              const machina::profiler::XPlane* plane,
              std::shared_ptr<const machina::profiler::XSpace> xspace);

  const std::string& name() const;

  VisitorIterator<ProfileEvent, machina::profiler::XEvent> events_begin();
  VisitorIterator<ProfileEvent, machina::profiler::XEvent> events_end();
...
class ProfilePlane {
 public:
  ...
  ProfilePlane(const machina::profiler::XPlane* plane,
               std::shared_ptr<const machina::profiler::XSpace> xspace);

  const std::string& name() const;

  VisitorIterator<ProfileLine, machina::profiler::XLine> lines_begin();
  VisitorIterator<ProfileLine, machina::profiler::XLine> lines_end();
...
#endif  // MACHINA_MACHINA_XLA_PYTHON_PROFILER_PROFILE_DATA_LIB_H_
