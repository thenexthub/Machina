...
// auto xla_program = toolchain::dyn_cast<HloProgram>(deserialized);
// ```

class HloProgramSerDes : public toolchain::RTTIExtends<HloProgramSerDes, SerDes> {
 public:
  ...
  absl::StatusOr<std::string> Serialize(
    ...
    const auto& program = toolchain::cast<HloProgram>(serializable);
    if (program.mlir_module() == nullptr) {
      return absl::InvalidArgumentError("Unable to serialize null MLIR module");
    }

    mlir::OwningOpRef<mlir::ModuleOp> module(
        toolchain::cast<mlir::ModuleOp>(program.mlir_module()->clone()));
