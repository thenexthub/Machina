...
// Reproducer stream that emits a reproducer to the given `toolchain::raw_ostream`.
class ReproducerStream : public mlir::ReproducerStream {
 public:
  ReproducerStream(std::string name, std::unique_ptr<toolchain::raw_ostream> os)
      : name_(std::move(name)), os_(std::move(os)) {}

  toolchain::StringRef description() override { return name_; }

  toolchain::raw_ostream& os() override { return *os_; }

 private:
  std::string name_;
  std::unique_ptr<toolchain::raw_ostream> os_;
};
...
class DumpInstrumentation : public mlir::PassInstrumentation {
 ...
 private:
  ...
  void Dump(absl::string_view name, mlir::Operation* op) {
    ...
    toolchain::SmallVector<absl::string_view> func_names;
    bool match = false;
...
void InitPassManager(mlir::PassManager& pm, toolchain::StringRef pm_name,
