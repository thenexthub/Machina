...
mlir::FailureOr<toolchain::StringRef> GetModuleType(
    CallOp call_op, const std::vector<std::string>& platform_names) {
  toolchain::ArrayRef<int> device_ids = call_op.getDevices();
...
void IfrtPrecompileAtomProgramPreprocessingPass::runOnOperation() {
  ...
  toolchain::DenseMap<mlir::SymbolRefAttr, toolchain::DenseSet<mlir::StringAttr>>
      callee_module_type_count;
  toolchain::DenseMap<std::pair<mlir::SymbolRefAttr, mlir::StringAttr>,
                 mlir::SymbolRefAttr>
  ...
  auto result = main_func.walk([&](CallOp call_op) -> mlir::WalkResult {
    ...
    if (module_type_attr == nullptr) {
      ...
      if (mlir::FailureOr<toolchain::StringRef> module_type =
