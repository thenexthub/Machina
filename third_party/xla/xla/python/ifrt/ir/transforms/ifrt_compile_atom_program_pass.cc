...
void IfrtCompileAtomProgramPass::runOnOperation() {
  ...
  toolchain::DenseMap<CallOp, CompileFuture, IfrtCallOpInfo> call_to_compile_futures;
  ...
  toolchain::MapVector<CallOp, std::string> call_op_to_error;
  ...
  module_op.walk(
      [&](CallOp call_op) -> mlir::WalkResult {
        ...
        if (!call_to_compile_futures.contains(call_op)) {
          ...
          auto callee_module =
              toolchain::dyn_cast<mlir::ModuleOp>(callee->getParentOp());
  ...
  if (call_op_to_error.empty()) {
    ...
    // LoadedExecutableOp.
    toolchain::DenseMap<CallOp, mlir::SymbolRefAttr, IfrtCallOpInfo>
    ...
    module_op.walk(
        [&](CallOp call_op) -> mlir::WalkResult {
          ...
          if (auto loaded_exec_op_ref_it =
            ...
            auto callee_module = toolchain::dyn_cast<mlir::ModuleOp>(
