...
#include "machina/xla/service/compilation_environments.h"
...
// be inferred.
toolchain::SmallVector<bool> GetInputShardingPropagation(
    mlir::func::FuncOp func_op) {
  toolchain::SmallVector<bool> sharding_propagation_to_input;
...
// be inferred.
toolchain::SmallVector<bool> GetOutputShardingPropagation(
    mlir::func::FuncOp func_op) {
  toolchain::SmallVector<bool> sharding_propagation_to_output;
...
MultiThreadedAtomProgramCompiler::GetXlaCompileOptions(
  ...
  if (compile_options_overrides_ != nullptr && compile_options_key != nullptr) {
    if (auto compile_options_override =
      ...
      if (auto xla_options = toolchain::dyn_cast<XlaCompileOptions>(
  ...
  if (!has_compile_options) {
    ...
    toolchain::ArrayRef<int> logical_device_ids = call_op.getDevices();
    if (call_op->hasAttrOfType<mlir::UnitAttr>(kIfrtLocalViewAttrName)) {
      ...
      for (const auto [i, device_id] : toolchain::enumerate(logical_device_ids)) {
