...
#include "machina/xla/python/ifrt/ir/transforms/passes.h"  // IWYU pragma: keep
...
mlir::LogicalResult verifyCallOpAliasesAndDonations(
    T op, toolchain::DenseMap<mlir::Value, mlir::Operation*>& donated_value_to_op) {
  toolchain::DenseSet<int> donated_input_idxs;
  ...
  for (const auto [idx, input] : toolchain::enumerate(op.getInputs())) {
...
void IfrtVerifyDonationPass::runOnOperation() {
  ...
  toolchain::DenseMap<mlir::Value, mlir::Operation*> donated_value_to_op;
        ...
        toolchain::TypeSwitch<mlir::Operation*, mlir::LogicalResult>(op)
            ...
            .Case<mlir::func::ReturnOp>([&](mlir::func::ReturnOp return_op) {
              for (const auto& [idx, result] :
                   toolchain::enumerate(return_op.getOperands())) {
