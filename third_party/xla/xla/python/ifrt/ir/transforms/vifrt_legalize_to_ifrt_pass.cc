...
#include "machina/xla/python/ifrt/ir/constants.h"
#include "machina/xla/python/ifrt/ir/ifrt_dialect.h"
#include "machina/xla/python/ifrt/ir/ifrt_interfaces.h"
#include "machina/xla/python/ifrt/ir/transforms/map_ifrt_to_vifrt.h"
#include "machina/xla/python/ifrt/ir/transforms/passes.h"
#include "machina/xla/python/ifrt/ir/vifrt_dialect.h"
...
#include "machina/xla/python/ifrt/ir/transforms/passes.h.inc"
...
mlir::Attribute convertGeneric(mlir::Attribute vifrt_attr,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Convert generic attribute: " << vifrt_attr
                          << '\n');
  if (!isBuiltinOrVifrtAttr(vifrt_attr)) {
    LLVM_DEBUG(toolchain::dbgs() << "Failed to convert: " << vifrt_attr << '\n');
    return {};
  }
  // Ordered from most constrained to least constrained.
  if (auto attr = toolchain::dyn_cast<VifrtDevicesV1Attr>(vifrt_attr)) {
    return IfrtDevicesAttr::get(attr.getContext(), attr.getIds());
  }
  if (auto attr = toolchain::dyn_cast<VifrtShardingParamV1Attr>(vifrt_attr)) {
    return IfrtShardingParamAttr::get(attr.getContext(), attr.getSharding());
  }
  if (auto attr = toolchain::dyn_cast<VifrtUnspecifiedShardingV1Attr>(vifrt_attr)) {
    return IfrtUnspecifiedShardingAttr::get(attr.getContext());
  }
  if (auto attr = toolchain::dyn_cast<VifrtIntervalV1Attr>(vifrt_attr)) {
    return IfrtIntervalAttr::get(attr.getContext(), attr.getStart(),
                                 attr.getEnd(), attr.getStep());
  }
  if (auto attr = toolchain::dyn_cast<VifrtMappingV1Attr>(vifrt_attr)) {
    auto from_shards = toolchain::dyn_cast_or_null<IfrtIntervalAttr>(
        convertGeneric(attr.getFromShards(), type_converter));
    auto to_shards = toolchain::dyn_cast_or_null<IfrtIntervalAttr>(
        convertGeneric(attr.getToShards(), type_converter));
    if (!from_shards || !to_shards) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to convert: " << attr << '\n');
      return {};
    }
    return IfrtMappingAttr::get(attr.getContext(), from_shards, to_shards);
  }
  if (auto attr = toolchain::dyn_cast<VifrtArrayMappingV1Attr>(vifrt_attr)) {
    toolchain::SmallVector<mlir::Attribute> ifrt_mappings;
    ifrt_mappings.reserve(attr.getMappings().size());
    for (auto mapping : attr.getMappings()) {
      auto ifrt_mapping = toolchain::dyn_cast_or_null<IfrtMappingAttr>(
  ...
  if (auto attr = toolchain::dyn_cast<VifrtTypeV1Attr>(vifrt_attr)) {
  ...
  if (auto attr = toolchain::dyn_cast<mlir::ArrayAttr>(vifrt_attr)) {
    toolchain::SmallVector<mlir::Attribute> ifrt_attrs;
  ...
  if (auto attr = toolchain::dyn_cast<mlir::DenseArrayAttr>(vifrt_attr)) {
  ...
  if (auto attr = toolchain::dyn_cast<mlir::DictionaryAttr>(vifrt_attr)) {
    toolchain::SmallVector<mlir::NamedAttribute> ifrt_attrs;
    ifrt_attrs.reserve(attr.getValue().size());
 ...
 public:
  VifrtToIfrtTypeConverter() : VifrtTypeConverterBuiltin() {
    addConversion([](mlir::Type type) -> mlir::Type {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Invalid type: " << type << '\n');
      return {};
    });
    addConversion([&](VifrtArrayV1Type array) -> mlir::Type {
      auto sharding_attr = toolchain::dyn_cast_or_null<IfrtShardingAttrInterface>(
          convertGeneric(array.getShardingAttr(), this));
      if (!sharding_attr) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to convert sharding: "
                                << array.getShardingAttr() << '\n');
        return {};
      }
      auto devices_attr = toolchain::dyn_cast_or_null<IfrtDevicesAttr>(
          convertGeneric(array.getDevicesAttr(), this));
      if (!devices_attr) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to convert devices: "
                                << array.getDevicesAttr() << '\n');
        return {};
      }
      auto memory_kind_attr =
          toolchain::dyn_cast<mlir::StringAttr>(array.getMemoryKindAttr());
      if (memory_kind_attr && memory_kind_attr.str() == kVifrtDefaultString) {
        // No memory kind was specified.
        memory_kind_attr = nullptr;
      }
      auto layout_attr =
          toolchain::dyn_cast<mlir::StringAttr>(array.getLayoutAttr());
...
mlir::LogicalResult removeDefaultAttrs(
    ...
    toolchain::SmallVector<mlir::NamedAttribute>& vifrt_attrs) {
  absl::flat_hash_set<std::string> to_remove_attrs;
  if constexpr (std::is_same<VifrtOpTy, FuncOpV1>::value) {
    if (auto attr =
            toolchain::dyn_cast<mlir::StringAttr>(vifrt_op.getSymVisibility());
        !attr || attr.str() == kVifrtDefaultString) {
      to_remove_attrs.insert("sym_visibility");
    }
    if (auto attr = toolchain::dyn_cast<mlir::ArrayAttr>(vifrt_op.getArgAttrs());
        !attr || attr.size() == 0) {
      to_remove_attrs.insert("arg_attrs");
      ;
    }
    if (auto attr = toolchain::dyn_cast<mlir::ArrayAttr>(vifrt_op.getResAttrs());
        !attr || attr.size() == 0) {
      to_remove_attrs.insert("res_attrs");
    }
  }
  toolchain::erase_if(vifrt_attrs, [&](mlir::NamedAttribute attr) {
...
mlir::FailureOr<mlir::SymbolRefAttr> getCalleeSymbolRef(CallOpV1 call_op) {
  mlir::StringAttr callee_symbol_ref_str_attr =
      toolchain::dyn_cast_or_null<mlir::StringAttr>(call_op.getCalleeAttr());
  ...
  toolchain::SmallVector<mlir::FlatSymbolRefAttr> symbol_ref_attrs;
  for (const auto [idx, symbol_str] : toolchain::enumerate(symbol_strs)) {
    if (idx == 0) {
...
class VifrtToIfrtOpConverter : public mlir::OpConversionPattern<VifrtOpTy> {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<mlir::Type> ifrt_types;
    if (mlir::failed(this->getTypeConverter()->convertTypes(
            vifrt_op->getResultTypes(), ifrt_types))) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed VIFRT to IFRT type conversion\n");
      return mlir::failure();
    }

    // Convert the IFRT attributes to VIFRT attributes.
    toolchain::SmallVector<mlir::NamedAttribute> ifrt_attrs;
    toolchain::DenseSet<mlir::StringAttr> already_converted_attrs;
    ...
    if constexpr (std::is_same<VifrtOpTy, CallOpV1>::value) {
      ...
      if (mlir::failed(callee_symbol_ref)) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to get callee symbol ref from "
                                << call_op << '\n');
        return mlir::failure();
      }
    ...
    toolchain::SmallVector<mlir::NamedAttribute> vifrt_attrs =
        toolchain::to_vector(vifrt_op->getAttrs());
    if (mlir::failed(removeDefaultAttrs(*this, vifrt_op, vifrt_attrs))) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed to remove default VIFRT attributes\n");
      return mlir::failure();
    }
    for (mlir::NamedAttribute vifrt_attr : vifrt_attrs) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Converting " << vifrt_attr.getName() << ", "
                              << vifrt_attr.getValue() << '\n');
      auto attr_name = getAttrNameFromVifrtToIfrt(vifrt_attr);
      if (mlir::failed(attr_name)) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to convert: " << vifrt_attr.getName()
                                << ", " << vifrt_attr.getValue() << '\n');
        return mlir::failure();
      }
      if (auto ifrt_attr =
    ...
    for (auto [vifrt_region, ifrt_region] :
         toolchain::zip(vifrt_op->getRegions(), ifrt_op->getRegions())) {
...
template <typename TypeOrAttr>
bool allFromIfrt(toolchain::ArrayRef<TypeOrAttr> range) {
  return toolchain::all_of(range, isFromIfrt<TypeOrAttr>);
}
...
struct VifrtLegalizeToIfrtPass
  ...
  void runOnOperation() override {
    if (mlir::failed(
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Failed partial conversion\n");
...
#define GET_OP_LIST
#include "machina/xla/python/ifrt/ir/ifrt_ops.cc.inc"
