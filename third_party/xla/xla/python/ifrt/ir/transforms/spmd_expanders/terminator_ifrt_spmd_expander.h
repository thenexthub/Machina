...
#ifndef MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_TRANSFORMS_SPMD_EXPANDERS_TERMINATOR_IFRT_SPMD_EXPANDER_H_
#define MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_TRANSFORMS_SPMD_EXPANDERS_TERMINATOR_IFRT_SPMD_EXPANDER_H_
...
class TerminatorIfrtSpmdExpander
 ...
 public:
  ...
  mlir::FailureOr<toolchain::DenseMap<int, ShardingParam>> ComputeShardingForward(
      mlir::Operation* op,
      const toolchain::DenseMap<int, ShardingParam>& input_shardings) const;

  mlir::FailureOr<toolchain::DenseMap<int, ShardingParam>> ComputeShardingBackward(
      mlir::Operation* op,
      const toolchain::DenseMap<int, ShardingParam>& output_shardings) const;
};
...
mlir::FailureOr<mlir::Operation*> TerminatorIfrtSpmdExpander<OpT>::SpmdExpand(
  ...
  auto output_types = toolchain::to_vector<8>(op->getOperandTypes());
  if (auto function = toolchain::dyn_cast<mlir::func::FuncOp>(parent_op)) {
...
template <typename OpT>
mlir::FailureOr<toolchain::DenseMap<int, ShardingParam>>
...
#endif  // MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_TRANSFORMS_SPMD_EXPANDERS_TERMINATOR_IFRT_SPMD_EXPANDER_H_
