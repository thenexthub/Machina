...
#include "machina/xla/python/ifrt/ir/transforms/debug.h"
...
class IfrtToDotPass : public impl::IfrtToDotPassBase<IfrtToDotPass> {
 ...
 private:
  ...
  void initMeshColorMapping(mlir::SymbolTableCollection& symbol_table,
    ...
    toolchain::SmallVector<xla::ifrt::IfrtDevicesAttr> devices_attrs;
    ...
    for (auto [idx, devices_attr] : toolchain::enumerate(devices_attrs)) {
  ...
  void emitAttrList(mlir::raw_ostream& os,
    ...
    os << "[";
    toolchain::interleaveComma(map, os, [&](const auto& it) {
  ...
  int emitNodeStmt(mlir::raw_ostream& os, std::string label,
    ...
    os << toolchain::format("v%i ", node_id);
  ...
  void emitEdgeStmt(mlir::raw_ostream& os, int src_node_id, int dst_node_id,
    ...
    edges_.push_back(strFromOs([&](mlir::raw_ostream& os) {
      os << toolchain::format("v%i -> v%i ", src_node_id, dst_node_id);
      emitAttrList(os, attrs);
    }));
  }
  ...
  toolchain::DenseMap<xla::ifrt::IfrtDevicesAttr, std::string>
...
void IfrtToDotPass::runOnOperation() {
  ...
  toolchain::DenseMap<mlir::Value, int> val_to_node_id;
  ...
  mlir::WalkResult result = module_op.walk([&](mlir::Operation* op)
                                               -> mlir::WalkResult {
    auto result =
        toolchain::TypeSwitch<mlir::Operation*, mlir::LogicalResult>(op)
            .Case<xla::ifrt::CallLoadedExecutableOp>([&](auto& op) {
              ...
              for (const auto& input : op.getInputs()) {
                ...
                const auto array_type =
                    toolchain::cast<xla::ifrt::IfrtArrayType>(input.getType());
            ...
            .Case<xla::ifrt::CopyArraysOp>([&](auto& op) {
              for (const auto& [input, output] :
                   toolchain::zip(op.getInputs(), op.getOutputs())) {
