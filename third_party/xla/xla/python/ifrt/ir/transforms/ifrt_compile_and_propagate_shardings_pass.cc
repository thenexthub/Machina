...
#include "machina/xla/python/ifrt/compiler.h"
#include "machina/xla/python/ifrt/ir/atom_program_compiler.h"
...
#include "machina/xla/python/ifrt/ir/transforms/multi_threaded_atom_program_compiler.h"
#include "machina/xla/python/ifrt/ir/transforms/passes.h"
#include "machina/xla/python/ifrt/ir/transforms/utils.h"
#include "machina/xla/python/ifrt/support/sharding_conversions.h"
...
class IfrtCompileAndPropagateShardingsPass
 ...
 public:
  ...
  toolchain::StringRef getArgument() const override {
 ...
 private:
  mlir::LogicalResult WalkCallOp(
      ...
      toolchain::DenseMap<mlir::ModuleOp,
  ...
  mlir::FailureOr<toolchain::SmallVector<ShardingParam>> GetInputShardingParams(
  ...
  // is fetched from the compiled executable. Otherwise, the sharding present in
  // the IFRT IR op is returned.
  mlir::FailureOr<toolchain::SmallVector<ShardingParam>> GetOutputShardingParams(
      CallOp call_op, const AtomProgramCompileResult& compile_result);
  ...
  mlir::FailureOr<CallOp> PropagateShardings(
      ...
      toolchain::ArrayRef<ShardingParam> input_shardings,
      toolchain::ArrayRef<ShardingParam> output_shardings, mlir::OpBuilder& builder);
...
mlir::LogicalResult IfrtCompileAndPropagateShardingsPass::WalkCallOp(
  ...
  auto callee_module = toolchain::dyn_cast<mlir::ModuleOp>(callee->getParentOp());
...
IfrtCompileAndPropagateShardingsPass::WalkCopyArraysOrReshardOp(
  ...
  for (auto [idx, pair] : toolchain::enumerate(toolchain::zip(inputs, outputs))) {
    ...
    if (mlir::isa<IfrtUnspecifiedShardingAttr>(
      ...
      if (toolchain::isa<IfrtUnspecifiedShardingAttr>(
...
mlir::FailureOr<toolchain::SmallVector<ShardingParam>>
IfrtCompileAndPropagateShardingsPass::GetInputShardingParams(
  ...
  toolchain::SmallVector<ShardingParam> in_sharding_params;
  in_sharding_params.reserve(call_op.getInputs().size());
  for (const auto& [idx, input] : toolchain::enumerate(call_op.getInputs())) {
...
IfrtCompileAndPropagateShardingsPass::GetOutputShardingParams(
  ...
  toolchain::SmallVector<ShardingParam> out_sharding_params;
  out_sharding_params.reserve(call_op.getOutputs().size());
  for (const auto& [idx, output] : toolchain::enumerate(call_op.getOutputs())) {
...
IfrtCompileAndPropagateShardingsPass::PropagateShardings(
  ...
  toolchain::SmallVector<mlir::Type, 4> new_call_op_result_types;
  ...
  if (replace_call_op) {
    ...
    for (auto [i, result] : toolchain::enumerate(call_op.getOutputs())) {
