...
bool isIfrtOpOrFunc(mlir::Operation* op) {
  ...
  if (auto func_op = toolchain::dyn_cast_or_null<mlir::func::FuncOp>(op)) {
    return op->hasAttr(kIfrtFunctionAttrName);
  } else if (auto return_op =
                 toolchain::dyn_cast_or_null<mlir::func::ReturnOp>(op)) {
...
mlir::FailureOr<mlir::Attribute> removeAttrsFromOtherDialects(
  ...
  if (auto array_attr = toolchain::dyn_cast<mlir::ArrayAttr>(attr)) {
    toolchain::SmallVector<mlir::Attribute> elements;
  ...
  if (auto dict_attr = toolchain::dyn_cast<mlir::DictionaryAttr>(attr)) {
    toolchain::SmallVector<mlir::NamedAttribute> kept_attrs;
...
mlir::LogicalResult removeAttrsFromOtherDialects(mlir::Operation* op) {
  ...
  if (mlir::succeeded(attr_dict_or)) {
    if (auto attr_dict =
            toolchain::dyn_cast_or_null<mlir::DictionaryAttr>(*attr_dict_or)) {
