...
std::optional<toolchain::SmallVector<mlir::func::FuncOp>> MaybeFindFunctions(
    mlir::Operation* op) {
  ...
  toolchain::SmallVector<mlir::func::FuncOp> called_funcs;
...
mlir::Operation* TopologicalIterator::next() {
  ...
  std::optional<toolchain::SmallVector<mlir::func::FuncOp>> funcs =
...
mlir::LogicalResult UpdateFunctionArgsUsingSharding(
  ...
  for (int i = 0; i < function.getNumArguments(); ++i) {
    ...
    absl::StatusOr<toolchain::SmallVector<int64_t>> arg_local_shape =
...
mlir::LogicalResult UpdateFunctionWithLocalInputShapes(
  ...
  for (auto& operand : function_operands) {
    ...
    toolchain::ArrayRef<int64_t> arg_local_shape = arg_type.getShape();
    mlir::RankedTensorType new_arg_type =
...
mlir::LogicalResult SpmdExpansionPass::spmdExpand(mlir::func::FuncOp func_op) {
  ...
  while (iterator.hasNext()) {
    ...
    if (auto spmd_interface_op =
            toolchain::dyn_cast<xla::ifrt::IfrtSpmdExpandable>(op)) {
