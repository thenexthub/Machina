...
void MergeReshardsIgnoringControlDependencies(
  ...
  // memory kind.
  toolchain::DenseMap<std::tuple<mlir::Operation*, IfrtDevicesAttr, mlir::Attribute,
                            mlir::StringAttr, mlir::StringAttr>,
                 toolchain::SmallVector<ReshardOp>>
  ...
  for (mlir::Value value : source_values) {
    ...
    for (mlir::Operation* user : value.getUsers()) {
      ...
      mlir::Operation* first_reshard_user = *toolchain::min_element(
  ...
  for (auto& [_, reshards] : user_device_donate_tuple_to_reshards) {
    ...
    toolchain::SmallVector<mlir::Value> inputs;
    toolchain::SmallVector<mlir::Type> output_types;
    ...
    for (auto [index, reshard] : toolchain::enumerate(reshards)) {
...
void IfrtMergeReshardsPass::runOnOperation() {
  ...
  {
    toolchain::DenseMap<IfrtDevicesAttr, toolchain::SmallVector<mlir::Value>>
        devices_to_args;
