...
absl::Status IfrtVerifyBoundExternalLoadedExecutablePass::VerifyShardingsEqual(
    ...
    const std::vector<xla::OpSharding>& shardings,
    absl::string_view sharding_type) {
  for (const auto& it : toolchain::enumerate(toolchain::zip(types, shardings))) {
    const auto& [param_type, sharding] = it.value();
    TF_ASSIGN_OR_RETURN(auto hlo_sharding,
                        xla::HloSharding::FromProto(sharding));
    auto array_type = toolchain::dyn_cast<IfrtArrayType>(param_type);
    CHECK(array_type);
    auto array_sharding =
        toolchain::dyn_cast<IfrtShardingParamAttr>(array_type.getShardingAttr());
