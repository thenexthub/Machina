...
mlir::Attribute convertGeneric(mlir::Attribute ifrt_attr,
  ...
  LLVM_DEBUG(toolchain::dbgs() << "Convert generic attribute: " << ifrt_attr
                          << '\n');
  if (!isBuiltinOrIfrtAttr(ifrt_attr)) {
    LLVM_DEBUG(toolchain::dbgs() << "Failed to convert: " << ifrt_attr << '\n');
    return {};
  }
  // Ordered from most constrained to least constrained.
  if (auto attr = toolchain::dyn_cast<IfrtDevicesAttr>(ifrt_attr)) {
    return VifrtDevicesV1Attr::get(attr.getContext(), attr.getIds());
  }
  if (auto attr = toolchain::dyn_cast<IfrtShardingParamAttr>(ifrt_attr)) {
    return VifrtShardingParamV1Attr::get(attr.getContext(), attr.getSharding());
  }
  if (auto attr = toolchain::dyn_cast<IfrtUnspecifiedShardingAttr>(ifrt_attr)) {
    return VifrtUnspecifiedShardingV1Attr::get(attr.getContext());
  }
  if (auto attr = toolchain::dyn_cast<IfrtIntervalAttr>(ifrt_attr)) {
    return VifrtIntervalV1Attr::get(attr.getContext(), attr.getStart(),
                                    attr.getEnd(), attr.getStep());
  }
  if (auto attr = toolchain::dyn_cast<IfrtMappingAttr>(ifrt_attr)) {
    auto from_shards = toolchain::dyn_cast_or_null<VifrtIntervalV1Attr>(
        convertGeneric(attr.getFromShards(), type_converter));
    auto to_shards = toolchain::dyn_cast_or_null<VifrtIntervalV1Attr>(
  ...
  if (auto attr = toolchain::dyn_cast<IfrtArrayMappingAttr>(ifrt_attr)) {
    toolchain::SmallVector<mlir::Attribute> vifrt_mappings;
    vifrt_mappings.reserve(attr.getMappings().size());
    for (auto mapping : attr.getMappings()) {
      auto vifrt_mapping = toolchain::dyn_cast_or_null<VifrtMappingV1Attr>(
  ...
  if (auto attr = toolchain::dyn_cast<mlir::TypeAttr>(ifrt_attr)) {
  ...
  if (auto attr = toolchain::dyn_cast<mlir::ArrayAttr>(ifrt_attr)) {
    toolchain::SmallVector<mlir::Attribute> vifrt_attrs;
  ...
  if (auto attr = toolchain::dyn_cast<mlir::DenseArrayAttr>(ifrt_attr)) {
  ...
  if (auto attr = toolchain::dyn_cast<mlir::DictionaryAttr>(ifrt_attr)) {
 ...
 public:
  IfrtToVifrtTypeConverter() : VifrtTypeConverterBuiltin() {
    ...
    addConversion([&](IfrtArrayType array) -> mlir::Type {
      ...
      auto devices_attr = toolchain::dyn_cast_or_null<VifrtDevicesV1Attr>(
...
class IfrtToVifrtOpConverter : public mlir::OpConversionPattern<IfrtOpTy> {
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<mlir::Type> vifrt_types;
    if (mlir::failed(this->getTypeConverter()->convertTypes(
            ifrt_op->getResultTypes(), vifrt_types))) {
      LLVM_DEBUG(toolchain::dbgs() << "Failed IFRT to VIFRT type conversion\n");
      return mlir::failure();
    }
    ...
    for (mlir::NamedAttribute ifrt_attr : ifrt_op->getAttrs()) {
      ...
      LLVM_DEBUG(toolchain::dbgs() << "Converting " << ifrt_attr.getName() << ", "
                              << ifrt_attr.getValue() << '\n');
      auto attr_name = getAttrNameFromIfrtToVifrt(ifrt_attr);
      if (mlir::failed(attr_name)) {
        LLVM_DEBUG(toolchain::dbgs() << "Failed to convert attribute name: "
                                << ifrt_attr.getName() << ", "
                                << ifrt_attr.getValue() << '\n');
        return mlir::failure();
      }
    ...
    for (auto [ifrt_region, vifrt_region] :
         toolchain::zip(ifrt_op->getRegions(), vifrt_op->getRegions())) {
