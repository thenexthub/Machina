...
mlir::LogicalResult PopulateMetadata(CallOp call_op, mlir::ModuleOp module_op,
  ...
  for (const auto& [i, input] : toolchain::enumerate(call_op.getInputs())) {
    ...
    toolchain::SmallVector<mlir::NamedAttribute, 16> arg_attrs;
  ...
  for (const auto& [i, output] : toolchain::enumerate(call_op.getOutputs())) {
    ...
    toolchain::SmallVector<mlir::NamedAttribute, 16> res_attrs;
...
void IfrtPopulateAtomProgramMetadataPass::runOnOperation() {
  ...
  toolchain::DenseMap<mlir::SymbolRefAttr, toolchain::DenseSet<CallOp, IfrtCallOpInfo>>
      callee_call_count;
  for (CallOp call_op : main_func.getOps<CallOp>()) {
    callee_call_count[call_op.getCallee()].insert(call_op);
  }

  toolchain::DenseMap<CallOp, mlir::SymbolRefAttr, IfrtCallOpInfo> visited_call_ops;
