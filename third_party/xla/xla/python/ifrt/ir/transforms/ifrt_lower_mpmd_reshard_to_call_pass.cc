 ...
 public:
  void runOnOperation() override {
    ...
    auto result = main_func->walk([&](ReshardOp reshard_op) {
      ...
      for (const auto& [idx, pair] : toolchain::enumerate(
               toolchain::zip(reshard_op.getInputs(), reshard_op.getOutputs()))) {
        auto in_array_type =
      ...
      toolchain::SmallVector<int32_t> donated_input_indices;
