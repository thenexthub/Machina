...
void IfrtOutlineAtomProgramToModulePass::runOnOperation() {
  ...
  toolchain::DenseSet<xla::ifrt::CallOp> visited;
  toolchain::SmallVector<mlir::Operation*, 16> to_erase;
  ...
  auto result =
      main_func.walk([&](xla::ifrt::CallOp call_op) -> mlir::WalkResult {
        ...
        if (callee.getSymName() == kCalleeMainFuncName &&
            toolchain::isa<mlir::ModuleOp>(callee->getParentOp())) {
        ...
        {
          // Setup for DFS.
          toolchain::DenseSet<mlir::func::FuncOp> visited_funcs;
          toolchain::SmallVector<mlir::func::FuncOp, 8> func_stack = {callee};
          while (!func_stack.empty()) {
            ...
            mlir::func::FuncOp cloned_func =
                toolchain::cast<mlir::func::FuncOp>(current_func->clone());
            ...
            for (const mlir::SymbolTable::SymbolUse& sym_use : *sym_uses) {
              ...
              auto func = toolchain::dyn_cast<mlir::func::FuncOp>(sym_op);
              if (func == nullptr) {
                return sym_use.getUser()->emitOpError()
                       << "uses a symbol in attributes `"
                       << sym_use.getSymbolRef().getRootReference().str()
        ...
        if (symbol_uses.has_value()) {
          for (const mlir::SymbolTable::SymbolUse symbol_use : *symbol_uses) {
            auto user = toolchain::dyn_cast<xla::ifrt::CallOp>(symbol_use.getUser());
            if (user == nullptr) {
