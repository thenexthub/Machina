...
class ReshardToCopyArraysOpPattern
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::DenseMap<xla::ifrt::IfrtDevicesAttr, toolchain::SmallVector<int>>
        copy_indices;
    // Indices of the input arrays that are resharded.
    toolchain::SmallVector<int> reshard_indices;
    for (const auto& [idx, pair] :
         toolchain::enumerate(toolchain::zip(op.getInputs(), op.getOutputs()))) {
      auto in_array_type =
    ...
    toolchain::SmallVector<mlir::Value> outputs;
    ...
    // that are resharded.
    toolchain::SmallVector<mlir::Value> reshard_input_values;
    toolchain::SmallVector<mlir::Type> reshard_output_types;
    ...
    if (!reshard_input_values.empty()) {
      ...
      for (const auto& [idx, output] :
           toolchain::zip(reshard_indices, reshard_op.getOutputs())) {
        outputs[idx] = output;
      }
    ...
    for (const auto& [devices_attr, indices] : copy_indices) {
      toolchain::SmallVector<mlir::Value> copy_input_values;
      toolchain::SmallVector<mlir::Type> copy_output_types;
      for (int idx : indices) {
      ...
      auto copy_arrays_op = xla::ifrt::CopyArraysOp::create(
          ...
          /*control_inputs=*/op.getControlInputs());
      for (const auto& [idx, output] :
           toolchain::zip(indices, copy_arrays_op.getOutputs())) {
        outputs[idx] = output;
      }
      if (reshard_input_values.empty()) {
