...
#include "machina/xla/python/ifrt/ir/version.h"
...
mlir::FailureOr<Version> validateTargetVersion(toolchain::StringRef version_ref,
                                               mlir::Operation* op) {
...
bool isLegalType(mlir::Type type, const Version& version) {
  ...
  auto ver_type_interface = toolchain::dyn_cast<VifrtVersionedTypeInterface>(type);
  if (!ver_type_interface || !isLegalVersion(ver_type_interface, version)) {
    LLVM_DEBUG(toolchain::dbgs() << "failed to convert type " << type
                            << " to version " << version << '\n');
    return false;
  }
  if (auto array_type = toolchain::dyn_cast<VifrtArrayV1Type>(type)) {
    return isLegalAttribute(array_type.getShardingAttr(), version);
  }
  if (auto func_type = toolchain::dyn_cast<VifrtFunctionV1Type>(type)) {
    auto is_legal_type_fn = [&](mlir::Type type) {
      return isLegalType(type, version);
    };
    return toolchain::all_of(func_type.getInputs(), is_legal_type_fn) &&
           toolchain::all_of(func_type.getOutputs(), is_legal_type_fn);
  }
  return true;
}
...
bool isLegalAttribute(mlir::Attribute attr, const Version& version) {
  ...
  if (auto array_attr = toolchain::dyn_cast<mlir::ArrayAttr>(attr)) {
    return toolchain::all_of(array_attr.getValue(), [&](mlir::Attribute entry) {
      return isLegalAttribute(entry, version);
    });
  }
  if (auto dense_array_attr = toolchain::dyn_cast<mlir::DenseArrayAttr>(attr)) {
    return isLegalType(dense_array_attr.getElementType(), version);
  }
  if (auto dict_attr = toolchain::dyn_cast<mlir::DictionaryAttr>(attr)) {
    return toolchain::all_of(dict_attr.getValue(), [&](const auto& entry) {
  ...
  auto ver_attr_interface = toolchain::dyn_cast<VifrtVersionedAttrInterface>(attr);
  if (!ver_attr_interface || !isLegalVersion(ver_attr_interface, version)) {
    LLVM_DEBUG(toolchain::dbgs() << "failed to convert attribute " << attr
                            << " to version " << version << '\n');
    return false;
  }
  if (auto type_attr = toolchain::dyn_cast<VifrtTypeV1Attr>(attr)) {
    return isLegalType(type_attr.getValue(), version);
  }
  if (auto array_mapping_attr = toolchain::dyn_cast<VifrtArrayMappingV1Attr>(attr)) {
    return toolchain::all_of(array_mapping_attr.getMappings().getValue(),
                        [&](const auto& mapping) {
...
bool isLegalOperation(mlir::Operation* op, const Version& version) {
  auto ver_op_interface = toolchain::dyn_cast<VifrtVersionedOpInterface>(op);
  if (!ver_op_interface || !isLegalVersion(ver_op_interface, version)) {
    LLVM_DEBUG(toolchain::dbgs() << "failed to convert operation " << op
                            << " to version " << version << '\n');
    return false;
  }
  // Verify if all the attributes are legal.
  if (!toolchain::all_of(op->getAttrs(), [&](const mlir::NamedAttribute& attr) {
  ...
  if (!toolchain::all_of(op->getOperandTypes(), is_legal_type_fn) ||
      !toolchain::all_of(op->getResultTypes(), is_legal_type_fn)) {
    return false;
  }
