...
#include "machina/xla/python/ifrt/ir/sharding_param.pb.h"
...
template <typename T>
void PrintDims(toolchain::raw_ostream& os, toolchain::ArrayRef<T> dims) {
  if (dims.empty()) {
...
void PopulateDevices(toolchain::ArrayRef<int> permutation,
                     toolchain::ArrayRef<int> axis_sizes,
                     toolchain::ArrayRef<int> cum_sizes,
                     toolchain::SmallVectorImpl<int>& out_devices, int base = 0) {
...
void PrintInternalV1(toolchain::raw_ostream& os, const ShardingParam& sharding) {
  PrintDims(os, sharding.dim_shards());
  os << " to [";
  toolchain::interleaveComma(
      toolchain::ArrayRef<int>(sharding.minor_to_major().permutation), os);
  os << "] on ";
...
absl::Status ShardingParam::MinorToMajor::verify() const {
  ...
  toolchain::DenseSet<int> permutation_set(permutation.begin(), permutation.end());
...
mlir::LogicalResult ShardingParam::MinorToMajor::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emit_error) const {
  auto status = verify();
  if (status.ok()) {
    return mlir::success();
  }
...
void ShardingParam::MinorToMajor::ToDeviceList(
    toolchain::SmallVectorImpl<int>& out_devices) const {
  toolchain::SmallVector<int, 4> cum_sizes;
  int cum_size = 1;
...
mlir::FailureOr<ShardingParam> ShardingParam::ParseV1(
  ...
  toolchain::SmallVector<int64_t, 4> axis_sizes_64;
  toolchain::SmallVector<int64_t> dim_shards;
  ...
  // toolchain::SmallVector<int64_t>, whereas ShardingParam expects a
...
mlir::LogicalResult ShardingParam::CanApplyTo(
    ...
    mlir::RankedTensorType shape, toolchain::ArrayRef<int> device_ids) const {
...
absl::StatusOr<toolchain::SmallVector<int64_t>>
ShardingParam::GlobalShapeFromLocalShape(
    toolchain::ArrayRef<int64_t> local_shape) const {
  toolchain::SmallVector<int64_t> global_shape;
  if (local_shape.size() != dim_shards().size()) {
    return absl::InvalidArgumentError(
        "Rank of local tensor differs from rank of `dim_shards`.");
  }
  for (auto [idx, dim_shard] : toolchain::enumerate(dim_shards())) {
...
ShardingParam::LocalShapeFromGlobalShape(
    toolchain::ArrayRef<int64_t> global_shape) const {
  auto num_shards = dim_shards();
  toolchain::SmallVector<int64_t> local_shape;
...
toolchain::hash_code hash_value(ShardingParam sharding) {
...
toolchain::raw_ostream& operator<<(toolchain::raw_ostream& os, ShardingParam sharding) {
  // V1 if the current ShardingParam version.
