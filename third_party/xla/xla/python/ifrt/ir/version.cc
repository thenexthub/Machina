...
static int64_t parseNumber(toolchain::StringRef num_ref) {
  int64_t num;
  if (num_ref.getAsInteger(/*radix=*/10, num)) {
    toolchain::report_fatal_error("failed to parse version number");
  }
  return num;
}
...
static mlir::FailureOr<toolchain::SmallVector<int64_t, 3>> extractVersionNumbers(
    toolchain::StringRef version_ref) {
  toolchain::Regex versionRegex("^([0-9]+)\\.([0-9]+)\\.([0-9]+)$");
  toolchain::SmallVector<toolchain::StringRef> matches;
  if (!versionRegex.match(version_ref, &matches)) {
    return mlir::failure();
  }
  return toolchain::SmallVector<int64_t, 3>{parseNumber(matches[1]),
...
mlir::FailureOr<Version> Version::fromString(toolchain::StringRef version_ref) {
...
Version Version::fromCompatibilityRequirement(
  ...
  toolchain::report_fatal_error("Unsupported compatibility requirement");
}

mlir::Diagnostic& operator<<(mlir::Diagnostic& diag, const Version& version) {
  return diag << version.toString();
}
toolchain::raw_ostream& operator<<(toolchain::raw_ostream& os, const Version& version) {
