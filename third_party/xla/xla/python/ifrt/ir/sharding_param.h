...
#ifndef MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_SHARDING_PARAM_H_
#define MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_SHARDING_PARAM_H_
...
#include "machina/xla/python/ifrt/serdes_default_version_accessor.h"
...
class ShardingParam {
 public:
  ...
  struct MinorToMajor {
    ...
    toolchain::SmallVector<int, 4> permutation;
    // The size of mesh dimensions before the permutation.
    toolchain::SmallVector<int, 4> axis_sizes;
    ...
    void ToDeviceList(toolchain::SmallVectorImpl<int>& out_devices) const;
  };
  ...
  absl::StatusOr<toolchain::SmallVector<int64_t>> GlobalShapeFromLocalShape(
      toolchain::ArrayRef<int64_t> local_shape) const;

  absl::StatusOr<toolchain::SmallVector<int64_t>> LocalShapeFromGlobalShape(
      toolchain::ArrayRef<int64_t> global_shape) const;

  // Returns the number of devices the array is sharded over.
  int NumDevices() const;

  toolchain::ArrayRef<int64_t> dim_shards() const { return dim_shards_; }
  ...
  toolchain::hash_code hash_value() const {
    return toolchain::hash_combine(dim_shards(),
                              toolchain::ArrayRef<int>(minor_to_major_.permutation),
                              toolchain::ArrayRef<int>(minor_to_major_.axis_sizes));
  }
...
#endif  // MACHINA_MACHINA_XLA_PYTHON_IFRT_IR_SHARDING_PARAM_H_
