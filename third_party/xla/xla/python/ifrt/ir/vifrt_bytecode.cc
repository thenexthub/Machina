...
#define _EXTRACT_AFTER(a, b) \
  toolchain::StringRef(a).substr(toolchain::StringRef(a).find(b))

#define _LOG_CALL_TO(func)                                              \
  LLVM_DEBUG(toolchain::errs() << "Called: "                                 \
...
#define LOG_NOT_IMPLEMENTED(typeOrAttr)                                  \
  LLVM_DEBUG(toolchain::errs() << "***Not Implemented: " << typeOrAttr << " " \
...
mlir::LogicalResult VifrtBytecodeInterface::writeAttribute(
  ...
  return toolchain::TypeSwitch<mlir::Attribute, mlir::LogicalResult>(attr)
...
VifrtDevicesV1Attr VifrtBytecodeInterface::readDevicesV1Attr(
    mlir::DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<int64_t> device_ids;
  ...
  return VifrtDevicesV1Attr::get(
      getContext(),
      toolchain::SmallVector<int>(device_ids.begin(), device_ids.end()));
}
...
VifrtShardingParamV1Attr VifrtBytecodeInterface::readShardingParamV1Attr(
  ...
  toolchain::SmallVector<int64_t> axis_sizes;
  ...
  minor_to_major.permutation =
      toolchain::SmallVector<int, 4>(permutation.begin(), permutation.end());
  minor_to_major.axis_sizes =
      toolchain::SmallVector<int, 4>(axis_sizes.begin(), axis_sizes.end());
...
VifrtArrayMappingV1Attr VifrtBytecodeInterface::readArrayMappingV1Attr(
  ...
  toolchain::SmallVector<VifrtMappingV1Attr> mappings;
  ...
  mlir::ArrayAttr array_attr = mlir::ArrayAttr::get(
      getContext(),
      toolchain::SmallVector<mlir::Attribute>(mappings.begin(), mappings.end()));
...
mlir::LogicalResult VifrtBytecodeInterface::writeType(
  ...
  return toolchain::TypeSwitch<mlir::Type, mlir::LogicalResult>(type)
...
VifrtFunctionV1Type VifrtBytecodeInterface::readFunctionV1Type(
    mlir::DialectBytecodeReader &reader) const {
  LOG_READ_CALL;
  toolchain::SmallVector<mlir::Type> inputs;
  toolchain::SmallVector<mlir::Type> outputs;
