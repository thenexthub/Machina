...
#include "toolchain/ADT/TypeSwitch.h"  // IWYU pragma: export
...
#include "machina/xla/python/ifrt/ir/vifrt_bytecode.h"
...
template <typename TypeOrAttr>
bool allFromVifrt(toolchain::ArrayRef<TypeOrAttr> range) {
  return toolchain::all_of(range, isFromVifrt<TypeOrAttr>);
}

template <typename TypeOrAttr>
bool allFromBuiltinOrVifrt(toolchain::ArrayRef<TypeOrAttr> range) {
  return toolchain::all_of(range, isFromBuiltinOrVifrt<TypeOrAttr>);
}

// Helper functions for VIFRT printers and parsers.
static void printAttributeArray(mlir::AsmPrinter& os,
                                toolchain::ArrayRef<mlir::Attribute> arrayAttr) {
  os << '[' << arrayAttr << ']';
}

// Parse attributes in brackets: [#virt.attr, #virt.attr]
mlir::ParseResult parseAttributeArray(
    mlir::AsmParser& parser, toolchain::SmallVector<mlir::Attribute>& arrayAttr) {
...
void printFunctionBody(mlir::OpAsmPrinter& p, mlir::Operation*,
  ...
  p.printSymbolName(toolchain::cast<mlir::StringAttr>(name).getValue());
  p << '(';
  toolchain::interleaveComma(region.getArguments(), p,
                        [&](auto arg) { p.printRegionArgument(arg); });
  p << ") -> (";
  auto fn_type = toolchain::cast<xla::ifrt::VifrtFunctionV1Type>(
      toolchain::cast<xla::ifrt::VifrtTypeV1Attr>(func_type).getValue());
  toolchain::interleaveComma(fn_type.getOutputs(), p,
...
mlir::ParseResult parseFunctionBody(mlir::OpAsmParser& parser,
  ...
  toolchain::SmallVector<mlir::OpAsmParser::Argument> args;
  toolchain::SmallVector<mlir::Type> input_types;
  toolchain::SmallVector<mlir::Type> res_types;
...
// Attributes
#include "machina/xla/python/ifrt/ir/vifrt_attr_interfaces.cc.inc"
#define GET_ATTRDEF_CLASSES
#include "machina/xla/python/ifrt/ir/vifrt_attrs.cc.inc"
// Types
#include "machina/xla/python/ifrt/ir/vifrt_type_interfaces.cc.inc"
#define GET_TYPEDEF_CLASSES
#include "machina/xla/python/ifrt/ir/vifrt_types.cc.inc"
// Ops
#include "machina/xla/python/ifrt/ir/vifrt_op_interfaces.cc.inc"
#define GET_OP_CLASSES
#include "machina/xla/python/ifrt/ir/vifrt_ops.cc.inc"
...
void VifrtTypeConverterBuiltin::addBuiltinToVifrtConversions() {
  ...
  addConversion([&](mlir::FunctionType type) -> mlir::Type {
    toolchain::SmallVector<mlir::Type> converted_inputs;
    toolchain::SmallVector<mlir::Type> converted_results;
