...
#include "machina/xla/python/ifrt/ir/ifrt_dialect.cc.inc"
#define GET_TYPEDEF_CLASSES
#include "machina/xla/python/ifrt/ir/ifrt_types.cc.inc"
#define GET_ATTRDEF_CLASSES
#include "machina/xla/python/ifrt/ir/ifrt_attrs.cc.inc"
...
IfrtAsmDialectInterface::AliasResult IfrtAsmDialectInterface::getAlias(
    mlir::Attribute attr, toolchain::raw_ostream& os) const {
  if (auto devices = toolchain::dyn_cast<IfrtDevicesAttr>(attr);
      devices != nullptr && devices.getIds().size() > 4) {
    os << "devices";
    return AliasResult::FinalAlias;
  } else if (auto mapping = toolchain::dyn_cast<IfrtArrayMappingAttr>(attr);
...
mlir::LogicalResult IfrtDialect::verifyOperationAttribute(
  ...
  if (attr.getName() == kIfrtFunctionAttrName) {
    if (!toolchain::isa<mlir::func::FuncOp>(op)) {
...
IfrtUnspecifiedShardingAttr::GlobalShapeFromLocalShape(
  ...
  toolchain::SmallVector<int64_t> global_shape(local_shape.begin(),
                                          local_shape.end());
  return global_shape;
}
...
IfrtUnspecifiedShardingAttr::LocalShapeFromGlobalShape(
    toolchain::ArrayRef<int64_t> global_shape) const {
  // Unspecified sharding does not change the shape.
  toolchain::SmallVector<int64_t> local_shape(global_shape.begin(),
                                         global_shape.end());
  return local_shape;
}
...
toolchain::ArrayRef<int> IfrtArrayType::getDevices() const {
...
IfrtDevicesAttr::operator toolchain::ArrayRef<int>() const { return getIds(); }

mlir::LogicalResult IfrtDevicesAttr::verify(
    toolchain::function_ref<::mlir::InFlightDiagnostic()> emitError,
    toolchain::ArrayRef<int> ids) {
...
mlir::LogicalResult IfrtIntervalAttr::verify(
    toolchain::function_ref<mlir::InFlightDiagnostic()> emitError, int start,
    int end, int step) {
  if (start < 0 || end < 0) {
