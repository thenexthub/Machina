...
#include "machina/xla/python/ifrt/ir/program_interpreter.h"
...
#include "machina/xla/python/ifrt/ir/compiled_ifrt_ir_program.h"
...
#include "machina/xla/python/ifrt/ir/transforms/utils.h"
#include "machina/xla/python/ifrt/remap_plan.h"
#include "machina/xla/python/ifrt/remap_plan.pb.h"
...
absl::StatusOr<toolchain::DenseMap<xla::ifrt::IfrtArrayType, xla::ifrt::ShardingRef>>
PopulateShardingCache(mlir::func::FuncOp main_func, xla::ifrt::Client* client,
                      const xla::ifrt::DeviceListRef& devices) {
  toolchain::DenseMap<xla::ifrt::IfrtArrayType, xla::ifrt::ShardingRef>
      array_type_to_sharding;
  for (const mlir::Operation& op : main_func.getOps()) {
    if (auto copy_arrays_op = toolchain::dyn_cast<xla::ifrt::CopyArraysOp>(&op);
        copy_arrays_op != nullptr) {
      for (const auto [idx, output] :
           toolchain::enumerate(copy_arrays_op.getOutputs())) {
        const auto array_type =
            toolchain::cast<xla::ifrt::IfrtArrayType>(output.getType());
    ...
    } else if (auto remap_op = toolchain::dyn_cast<xla::ifrt::RemapArraysOp>(&op);
               remap_op != nullptr) {
      for (const auto [idx, output] : toolchain::enumerate(remap_op.getOutputs())) {
        const auto array_type =
            toolchain::cast<xla::ifrt::IfrtArrayType>(output.getType());
        TF_RET_CHECK(array_type != nullptr)
            << "Output array #" << idx << " is not of type `IfrtArrayType`. "
...
struct Environment {
  ...
  toolchain::DenseMap<mlir::Value, ArrayState> value_to_array;
...
absl::StatusOr<ExecuteResult> ProgramInterpreter::Execute(
  ...
  for (const auto& [idx, array] : toolchain::enumerate(arrays)) {
    if (array->IsDeleted()) {
      return absl::InvalidArgumentError(absl::StrCat(
  ...
  for (const auto [idx, arg] : toolchain::enumerate(main_func.getArguments())) {
  ...
  for (mlir::Operation& op : main_func.getOps()) {
    auto exec_op_status =
        toolchain::TypeSwitch<const mlir::Operation&, absl::Status>(op)
...
absl::Status ProgramInterpreter::ExecuteOp(
  ...
  toolchain::DenseSet<mlir::Value> array_values_to_gc_from_env;
  for (const auto [idx, input] : toolchain::enumerate(call_loaded_op.getInputs())) {
  ...
  for (const auto [output_array, output] :
       toolchain::zip(result.outputs, call_loaded_op.getOutputs())) {
    if (!output.use_empty()) {
    ...
    if (is_leaf_op) {
      for (mlir::OpOperand& use : output.getUses()) {
        ...
        if (toolchain::dyn_cast<mlir::func::ReturnOp>(use.getOwner()) == nullptr) {
          is_leaf_op = false;
          break;
        }
      }
...
absl::Status ProgramInterpreter::ExecuteOp(xla::ifrt::RemapArraysOp remap_op,
  ...
  for (const auto& array_mapping : remap_op.getMappings()) {
    const auto array_mapping_attr =
        toolchain::cast<xla::ifrt::IfrtArrayMappingAttr>(array_mapping);
    ...
    for (const auto& m : array_mapping_attr.getMappings()) {
      const auto mapping_attr = toolchain::cast<xla::ifrt::IfrtMappingAttr>(m);
  ...
  toolchain::DenseSet<mlir::Value> array_values_to_gc_from_env;
  for (const auto [idx, input] : toolchain::enumerate(remap_op.getInputs())) {
  ...
  for (const auto [output_array, output] :
       toolchain::zip(out_arrays, remap_op.getOutputs())) {
    if (!output.use_empty()) {
      env.AssociateArray(output, ArrayState{/*array=*/std::move(output_array),
                                            /*can_be_donated=*/true});
    }
...
absl::Status ProgramInterpreter::ExecuteOp(
  ...
  toolchain::DenseSet<mlir::Value> array_values_to_gc_from_env;
  for (const auto [idx, input] : toolchain::enumerate(copy_arrays_op.getInputs())) {
  ...
  const auto out_array_type = toolchain::cast<xla::ifrt::IfrtArrayType>(
  ...
  TF_RET_CHECK(copied_arrays.size() == copy_arrays_op.getOutputs().size())
      ...
      << PrettyPrint(copy_arrays_op);
  for (const auto [output_array, output] :
       toolchain::zip(copied_arrays, copy_arrays_op.getOutputs())) {
    if (!output.use_empty()) {
      env.AssociateArray(output, ArrayState{/*array=*/std::move(output_array),
                                            /*can_be_donated=*/true});
    }
...
absl::Status ProgramInterpreter::ExecuteOp(mlir::func::ReturnOp return_op,
  ...
  for (const auto& [idx, result] : toolchain::enumerate(return_op.getOperands())) {
