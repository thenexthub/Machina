...
#include "machina/xla/python/ifrt/ir/ifrt_ops.h"
...
mlir::LogicalResult VerifySameGlobalShape(mlir::Operation* op,
                                          toolchain::StringRef lhs_mnemonic, T lhs,
                                          toolchain::StringRef rhs_mnemonic, U rhs) {
...
mlir::LogicalResult VerifyGlobalLocalShapesEquivalent(
    mlir::Operation* op, toolchain::StringRef call_mnemonic, mlir::Value call_value,
    toolchain::StringRef callee_mnemonic, mlir::Type callee_type) {
...
mlir::LogicalResult VerifyDevicePlacement(
    mlir::Operation* op, toolchain::ArrayRef<int> devices,
    toolchain::ArrayRef<IfrtArrayType> inputs,
    toolchain::ArrayRef<IfrtArrayType> outputs) {
  toolchain::DenseSet<int> device_set;
...
mlir::LogicalResult VerifyElementTypeAndPerShardShapeAreEqual(
  ...
  absl::StatusOr<toolchain::SmallVector<int64_t>> in_per_shard_shape =
  ...
  absl::StatusOr<toolchain::SmallVector<int64_t>> out_per_shard_shape =
  ...
  for (const auto& [in_dim, out_dim] :
       toolchain::zip(*in_per_shard_shape, *out_per_shard_shape)) {
    if (in_dim != out_dim) {
      return op->emitOpError()
             << "can't alias input #" << in_index << " to output #" << out_index
             << " with different per-shard shapes: " << in << " vs " << out;
...
mlir::LogicalResult VerifyIoAliasesAndDonations(
    ...
    toolchain::ArrayRef<int32_t> donated_input_indices,
    toolchain::ArrayRef<IfrtArrayType> inputs,
    toolchain::ArrayRef<IfrtArrayType> outputs) {
  toolchain::SmallSet<int, 4> aliased_or_donated_inputs;
  toolchain::SmallSet<int, 4> aliased_outputs;
  ...
  for (const auto& raw_io_alias :
    ...
    toolchain::ArrayRef<int> io_alias_as_array = raw_io_alias.asArrayRef();
...
bool IsAutoLayout(mlir::Type type) {
  auto array = toolchain::cast_or_null<IfrtArrayType>(type);
...
mlir::LogicalResult ReshardOp::verify() {
  ...
  for (const auto [idx, pair] :
       toolchain::enumerate(toolchain::zip(getInputs(), getOutputs()))) {
...
mlir::LogicalResult AssembleOp::verify() {
  ...
  for (const mlir::Value input : getInputs()) {
    const auto array = toolchain::cast<IfrtArrayType>(input.getType());
  ...
  const toolchain::ArrayRef<int> output_devices = getOutput().getType().getDevices();
...
mlir::LogicalResult DisassembleOp::verify() {
  ...
  for (const mlir::Value output : getOutputs()) {
    const auto array = toolchain::cast<IfrtArrayType>(output.getType());
    if (array.getDevices().size() != 1) {
      return emitOpError()
  ...
  const toolchain::ArrayRef<int> input_devices = getInput().getType().getDevices();
  if (!std::equal(input_devices.begin(), input_devices.end(),
                  output_devices.begin())) {
    return emitOpError() << "requires the same input/output device list. Input "
                         << input_devices << " vs Output " << output_devices;
...
mlir::LogicalResult CopyArraysOp::verify() {
  ...
  for (const auto [idx, pair] :
    ...
    const auto output_array =
        toolchain::cast<IfrtArrayType>(std::get<1>(pair).getType());
...
mlir::LogicalResult RemapArraysOp::verify() {
  ...
  IfrtArrayType first_array =
      toolchain::cast<IfrtArrayType>(getInputs()[0].getType());
  ...
  // shape.
  for (const auto [idx, input] : toolchain::enumerate(getInputs())) {
    const auto array = toolchain::cast<IfrtArrayType>(input.getType());
  ...
  if (num_out_arrays > 0) {
    IfrtArrayType first_out_array =
        toolchain::cast<IfrtArrayType>(getOutputs()[0].getType());
    absl::StatusOr<toolchain::SmallVector<int64_t>> out_per_shard_shape =
    ...
    for (const auto [idx, output] : toolchain::enumerate(getOutputs())) {
      const auto array = toolchain::cast<IfrtArrayType>(output.getType());
      if (array.getShape().getElementType() != dtype) {
  ...
  for (const auto& array_mapping : getMappings()) {
    const auto array_mapping_attr =
        toolchain::cast<IfrtArrayMappingAttr>(array_mapping);
    ...
    for (const auto& mapping : array_mapping_attr.getMappings()) {
      const auto mapping_attr = toolchain::cast<IfrtMappingAttr>(mapping);
...
mlir::LogicalResult CallOp::verifySymbolUses(
  ...
  for (int i = 0; i < callee_type.getNumInputs(); ++i) {
    if (local_view_attr == nullptr) {
      if (mlir::failed(VerifySameGlobalShape(
              *this, toolchain::Twine("Input #").concat(toolchain::Twine(i)).str(),
  ...
  for (int i = 0; i < callee_type.getNumResults(); ++i) {
    if (local_view_attr == nullptr) {
      if (mlir::failed(VerifySameGlobalShape(
              *this, toolchain::Twine("Output #").concat(toolchain::Twine(i)).str(),
...
mlir::LogicalResult CallOp::verify() {
  toolchain::SmallVector<IfrtArrayType, 4> input_arrays;
  input_arrays.reserve(getInputs().size());
  for (const mlir::Value input : getInputs()) {
    input_arrays.push_back(mlir::cast<IfrtArrayType>(input.getType()));
  }

  toolchain::SmallVector<IfrtArrayType, 4> output_arrays;
...
mlir::LogicalResult CallLoadedExecutableOp::verifySymbolUses(
  ...
  toolchain::SmallVector<mlir::Type, 4> output_types;
...
mlir::LogicalResult LoadedExecutableOp::verify() {
  ...
  for (const mlir::Type input : func_type.getInputs()) {
    if (auto input_array = toolchain::dyn_cast<IfrtArrayType>(input)) {
  ...
  for (const mlir::Type output : func_type.getResults()) {
    if (auto output_array = toolchain::dyn_cast<IfrtArrayType>(output)) {
