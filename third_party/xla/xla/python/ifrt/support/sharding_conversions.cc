...
absl::StatusOr<OpSharding> ToOpSharding(const Sharding& sharding) {
  if (auto* sharding_param_sharding =
          toolchain::dyn_cast<xla::ifrt::ShardingParamSharding>(&sharding)) {
...
absl::StatusOr<OpSharding> ToOpSharding(
  ...
  toolchain::SmallVector<int> logical_device_ids;
...
absl::StatusOr<HloSharding> ToHloSharding(const ShardingParam& sharding_param) {
  ...
  toolchain::SmallVector<int64_t> reshape_dims;
  reshape_dims.reserve(axis_sizes.size());
  int device_count = 1;
  for (auto axis_size : toolchain::reverse(axis_sizes)) {
  ...
  for (const int axis_id :
       toolchain::reverse(sharding_param.minor_to_major().permutation)) {
  ...
  if (hlo_sharding.IsTiled()) {
    ...
    if (tile_assignment.iota()->reshape_dims().empty()) {
      ...
      for (auto reshape_dim :
           toolchain::reverse(tile_assignment.iota()->reshape_dims())) {
      ...
      for (int axis_id :
           toolchain::reverse(tile_assignment.iota()->transpose_perm())) {
