...
#include "machina/xla/pjrt/exceptions.h"
#include "machina/xla/pjrt/status_casters.h"
#include "machina/xla/python/aggregate_profile.h"
#include "machina/xla/python/profiler/profile_data_lib.h"
#include "machina/xla/python/profiler_utils.h"
#include "machina/xla/python/xplane_to_profile_instructions.h"
#include "machina/xla/tsl/platform/macros.h"
#include "machina/xla/tsl/profiler/rpc/client/capture_profile.h"
#include "machina/xla/tsl/profiler/rpc/profiler_server.h"
...
machina::ProfileOptions DefaultPythonProfileOptions() {
  machina::ProfileOptions options = tsl::ProfilerSession::DefaultOptions();
...
static std::string GetFdoProfile(const std::string& xspace,
  ...
  machina::profiler::XSpace xspace_proto;
  ...
  machina::profiler::ProfiledInstructionsProto fdo_profile;
...
NB_MODULE(_profiler, m) {
  ...
  profiler_session_class
      ...
      .def("stop_and_get_profile_data",
           [](ProfilerSessionWrapper* sess)
               -> machina::profiler::python::ProfileData {
             auto xspace = std::make_shared<machina::profiler::XSpace>();
             // Disables the ProfilerSession
             xla::ThrowIfError(sess->session->CollectData(xspace.get()));
             return machina::profiler::python::ProfileData(xspace);
           })
      .def("export",
  ...
  nb::class_<machina::ProfileOptions> profile_options_class(
      m, "ProfileOptions");
  profile_options_class
      ...
      .def_prop_rw("include_dataset_ops",
                   &machina::ProfileOptions::include_dataset_ops,
                   &machina::ProfileOptions::set_include_dataset_ops)
      .def_prop_rw("host_tracer_level",
                   &machina::ProfileOptions::host_tracer_level,
                   &machina::ProfileOptions::set_host_tracer_level)
      .def_prop_rw("python_tracer_level",
                   &machina::ProfileOptions::python_tracer_level,
                   &machina::ProfileOptions::set_python_tracer_level)
      .def_prop_rw("enable_hlo_proto",
                   &machina::ProfileOptions::enable_hlo_proto,
                   &machina::ProfileOptions::set_enable_hlo_proto)
      .def_prop_rw("start_timestamp_ns",
                   &machina::ProfileOptions::start_timestamp_ns,
                   &machina::ProfileOptions::set_start_timestamp_ns)
      .def_prop_rw("duration_ms", &machina::ProfileOptions::duration_ms,
                   &machina::ProfileOptions::set_duration_ms)
      .def_prop_rw(
          "raise_error_on_start_failure",
          &machina::ProfileOptions::raise_error_on_start_failure,
          &machina::ProfileOptions::set_raise_error_on_start_failure)
      .def_prop_rw(
          "advanced_configuration",
          &machina::ProfileOptions::advanced_configuration,
          [](machina::ProfileOptions* options, const nb::dict& dict) {
            ...
            for (const auto& item : dict) {
              ...
              machina::ProfileOptions::AdvancedConfigValue config_value;
      ...
      .def_prop_rw(
          "repository_path", &machina::ProfileOptions::repository_path,
          [](machina::ProfileOptions* options, const std::string& path) {
  ...
  m.def(
      ...
      [](std::string tensorboard_dir) -> nb::bytes {
        machina::profiler::ProfiledInstructionsProto profile_proto;
        xla::ThrowIfError(
  ...
  m.def(
      ...
      [](const std::vector<nb::bytes>& profiles, int percentile) -> nb::object {
        std::vector<machina::profiler::ProfiledInstructionsProto>
            fdo_profiles;
        ...
        machina::profiler::ProfiledInstructionsProto result_proto;
