...
#include "machina/xla/pjrt/pjrt_layout.h"
#include "machina/xla/python/ifrt/array.h"
#include "machina/xla/python/ifrt/array_spec.h"
#include "machina/xla/python/ifrt/attribute_map.h"
#include "machina/xla/python/ifrt/basic_device_list.h"
#include "machina/xla/python/ifrt/client.h"
#include "machina/xla/python/ifrt/client_impl_util.h"
...
#include "machina/xla/python/ifrt/sharding.h"
#include "machina/xla/python/ifrt/topology.h"
#include "machina/xla/python/ifrt/tuple.h"
#include "machina/xla/python/ifrt/user_context.h"
#include "machina/xla/python/ifrt/value.h"
#include "machina/xla/python/pjrt_ifrt/basic_string_array.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_array.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_attribute_map_util.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_device.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_dtype.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_memory.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_remap.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_topology.h"
#include "machina/xla/python/pjrt_ifrt/pjrt_tuple.h"
#include "machina/xla/python/pjrt_ifrt/xla_sharding.h"
...
absl::StatusOr<ArrayRef> AssembleStringArrayFromSingleDeviceStringArrays(
  ...
  for (int i = 0; i < arrays.size(); ++i) {
    auto basic_string_array = toolchain::dyn_cast<BasicStringArray>(arrays[i].get());
...
absl::StatusOr<std::unique_ptr<PjRtClient>> PjRtClient::Create(
  ...
  for (Device* ifrt_device : client->addressable_devices_) {
    auto* device = machina::down_cast<PjRtDevice*>(ifrt_device);
...
absl::StatusOr<ArrayRef> PjRtClient::MakeArrayFromHostBuffer(
  ...
  if (!toolchain::isa<const SingleDeviceSharding>(sharding.get()) &&
    ...
    if (sharding->memory_kind().memory_kind().has_value()) {
      ...
      TF_ASSIGN_OR_RETURN(
          buffer, pjrt_client_->BufferFromHostBuffer(
                      ...
                      machina::down_cast<PjRtMemory*>(memory)->pjrt_memory(),
                      /*device_layout=*/nullptr));
    } else {
      TF_ASSIGN_OR_RETURN(xla::PjRtMemorySpace * memory_space,
                          machina::down_cast<PjRtDevice*>(device)
                              ->pjrt_device()
...
absl::StatusOr<ArrayRef> PjRtClient::AssembleArrayFromSingleDeviceArrays(
  ...
  DCHECK(this);
  if (!arrays.empty() &&
      toolchain::isa<const SingleDeviceSharding>(sharding.get())) {
    ...
    return arrays[0];
  } else if (!toolchain::isa<const SingleDeviceSharding, const OpaqueSharding,
  ...
  for (int i = 0; i < arrays.size(); ++i) {
    if (!toolchain::isa<PjRtCompatibleArray>(arrays[i].get())) {
      return InvalidArgument(
...
absl::Status PjRtClient::WatchGlobalProcessInfo(
  ...
  TF_ASSIGN_OR_RETURN(machina::CoordinatedTask task, agent.GetOwnTask());
  ...
  while (true) {
    ...
    VLOG(3) << "Calling WatchJobStateAsync for task "
            << task.ShortDebugString();
    absl::StatusOr<machina::WatchJobStateResponse> response;
    bool done = false;
    std::shared_ptr<tsl::CallOptions> call_opts = agent.WatchJobStateAsync(
        task.job_name(), version_number,
        [this, &response,
         &done](absl::StatusOr<machina::WatchJobStateResponse> r) {
          response = std::move(r);
          absl::MutexLock lock(&shutting_down_mu_);
          done = true;
        });
    ...
    std::vector<machina::CoordinatedTaskStateInfo> state(
        response->task_state().begin(), response->task_state().end());
    absl::c_sort(state,
                 [](const machina::CoordinatedTaskStateInfo& x,
                    const machina::CoordinatedTaskStateInfo& y) -> bool {
