...
absl::Status ValidateArrayCreationInput(
  ...
  for (int i = 0; i < sharding_devices.size(); ++i) {
    PjRtCompatibleDevice* device =
        toolchain::dyn_cast<PjRtCompatibleDevice>(sharding_devices[i]);
    if (!device) {
    ...
    if (devices_equal && (!new_sharding_has_memory_kind || memory_kind_equal)) {
      switch (semantics) {
        case ArrayCopySemantics::kAlwaysCopy: {
          ...
          PjRtMemory* pjrt_memory = toolchain::dyn_cast<PjRtMemory>(memory);
          TF_ASSIGN_OR_RETURN(
              auto copied_buffer,
      ...
      PjRtCompatibleDevice* pjrt_device =
          toolchain::dyn_cast<PjRtCompatibleDevice>(new_sharding_devices[i]);
      if (!pjrt_device) {
        return InvalidArgument(
      ...
      new_client = toolchain::dyn_cast<PjRtCompatibleClient>(pjrt_device->client());
