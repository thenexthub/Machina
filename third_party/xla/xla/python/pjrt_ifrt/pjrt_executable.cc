...
#include "machina/xla/python/pjrt_ifrt/pjrt_executable.h"
...
PjRtLoadedExecutable::Execute(absl::Span<ArrayRef> args,
  ...
  for (int i = 0; i < args.size(); ++i) {
    auto* pjrt_array =
        toolchain::dyn_cast_or_null<PjRtCompatibleArray>(args[i].get());
    if (!pjrt_array) {
      return InvalidArgument(
  ...
  if (platform_id == CpuId() || platform_id == CudaId() ||
    ...
    for (const auto& loaded_host_callback : *all_loaded_host_callbacks_) {
      auto* ffi_loaded_host_callback =
          toolchain::dyn_cast<PjRtFfiLoadedHostCallback>(loaded_host_callback.get());
