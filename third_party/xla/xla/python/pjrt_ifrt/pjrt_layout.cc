...
bool PjRtLayout::operator==(const Layout& other) const {
  ...
  if (const auto* other_pjrt = toolchain::dyn_cast<PjRtLayout>(&other);
      other_pjrt != nullptr) {
...
ToPjRtLayout(DType dtype, const Shape& shard_shape,
  ...
  if (const auto* pjrt_layout = toolchain::dyn_cast<PjRtLayout>(layout.get())) {
    return pjrt_layout->pjrt_layout();
  }
  if (const auto* compact_layout =
          toolchain::dyn_cast<CompactLayout>(layout.get())) {
    xla::Layout layout;
