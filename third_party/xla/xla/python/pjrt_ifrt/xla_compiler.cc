...
#include "machina/xla/python/pjrt_ifrt/xla_compiler.pb.h"
...
class XlaCompileOptionsSerDes
    : public toolchain::RTTIExtends<XlaCompileOptionsSerDes, SerDes> {
 public:
  ...
  absl::StatusOr<std::string> Serialize(
    ...
    const auto& options = toolchain::cast<XlaCompileOptions>(serializable);
...
absl::StatusOr<std::unique_ptr<XlaCompileOptions>> GetXlaCompileOptions(
  ...
  if (!toolchain::isa<XlaCompileOptions>(options.get())) {
...
GetXlaDeserializeExecutableOptions(
  ...
  if (!toolchain::isa<XlaDeserializeExecutableOptions>(options.get())) {
