...
#include "machina/xla/python/logging.h"
#include "machina/xla/python/nb_numpy.h"
#include "machina/xla/python/types.h"
...
PYBIND11_MODULE(_types, py_m) {
  ...
  py_m.def("make_ndarray", &MakeNdarray, py::arg("proto").none(false),
           py::pos_only(), R"pbdoc(
    Converts `machina.compiler.xla.xla_data_pb2.LiteralProto`
  ...
  py_m.def("dtype_to_etype", &DtypeToEtype, py::arg("dtype").none(false),
    ...
    `machina.compiler.xla.xla_data_pb2.PrimitiveType`.
  )pbdoc");

  py_m.def("etype_to_dtype", &EtypeToDtype, py::arg("ptype").none(false),
           py::pos_only(), R"pbdoc(
    Converts `machina.compiler.xla.xla_data_pb2.PrimitiveType` into
    `numpy.dtype`.
  )pbdoc");
