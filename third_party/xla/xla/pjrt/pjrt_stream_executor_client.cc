...
#include "machina/xla/pjrt/abstract_tracked_device_buffer.h"
...
#include "machina/xla/pjrt/metrics.h"
...
AllocateDestinationBuffer(
  ...
  auto* se_client = machina::down_cast<PjRtStreamExecutorClient*>(client);
...
absl::Status PjRtStreamExecutorClient::DmaMap(void* data, size_t buffer_size) {
  ...
  TF_ASSIGN_OR_RETURN(
      LocalDeviceState * local_device,
      machina::down_cast<PjRtStreamExecutorDevice*>(addressable_devices_[0])
          ->GetLocalDeviceState());
...
PjRtStreamExecutorBuffer::PjRtStreamExecutorBuffer(
    ...
    : CommonPjRtBuffer(std::move(device_buffer), memory_space),
      client_(machina::down_cast<PjRtStreamExecutorClient*>(client)),
      on_device_shape_(std::move(on_device_shape)),
      device_(machina::down_cast<PjRtStreamExecutorDevice*>(device)) {}
...
PjRtStreamExecutorBuffer::CopyToDeviceMemorySpace(
  ...
  TF_ASSIGN_OR_RETURN(
      ...
      machina::down_cast<PjRtStreamExecutorDevice*>(dst_device)
          ->GetLocalDeviceState());
...
PjRtStreamExecutorLoadedExecutable::EnqueueExecution(
  ...
  int device_ordinal = machina::down_cast<PjRtStreamExecutorDevice*>(device)
                           ->local_device_state()
                           ->local_device_id()
                           .value();
  LocalDeviceState* device_state = &(client_->device_state(device_ordinal));
  ...
  for (int i = 0; i < argument_handles.size(); ++i) {
    auto* handle =
        machina::down_cast<PjRtStreamExecutorBuffer*>(argument_handles[i]);
...
static absl::Status GetFirstInputError(
  ...
  for (auto* handle : argument_handles) {
    auto* buffer = machina::down_cast<PjRtStreamExecutorBuffer*>(handle);
    PjRtStreamExecutorBuffer::ScopedHold hold =
...
absl::StatusOr<std::string> PjRtStreamExecutorClient::SerializeExecutable(
  ...
  const PjRtStreamExecutorLoadedExecutable* se_executable =
      machina::down_cast<const PjRtStreamExecutorLoadedExecutable*>(
          &executable);
