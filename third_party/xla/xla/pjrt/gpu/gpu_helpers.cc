...
//
// The pool size is controlled by MACHINA_MACHINA_XLA_PJRT_GPU_HOST_MEMORY_LIMIT_GB environment
// variable, which defaults to 64GB.
//
// If MACHINA_MACHINA_XLA_PJRT_GPU_HOST_MEMORY_PREALLOCATE is set to true, the pool will be
// preallocated, and the preallocated size is controlled by
// MACHINA_MACHINA_XLA_PJRT_GPU_HOST_MEMORY_LIMIT_GB environment variable, which defaults to
// 16GB in this case.
absl::StatusOr<std::unique_ptr<tsl::BFCAllocator>> GetGpuHostAllocator(
  ...
  TF_RETURN_IF_ERROR(
      tsl::ReadBoolFromEnvVar("MACHINA_MACHINA_XLA_PJRT_GPU_HOST_MEMORY_PREALLOCATE", false,
  ...
  TF_RETURN_IF_ERROR(
      tsl::ReadInt64FromEnvVar("MACHINA_MACHINA_XLA_PJRT_GPU_HOST_MEMORY_LIMIT_GB",
