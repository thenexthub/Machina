...
#include "machina/xla/pjrt/gpu/se_gpu_pjrt_client.h"
...
#include "machina/xla/backends/gpu/collectives/gpu_cliques.h"
...
#include "machina/xla/core/collectives/collectives.h"
#include "machina/xla/core/collectives/collectives_registry.h"
...
#include "machina/xla/pjrt/distributed/client.h"
#include "machina/xla/pjrt/distributed/in_memory_key_value_store.h"
#include "machina/xla/pjrt/distributed/key_value_store_interface.h"
#include "machina/xla/pjrt/distributed/topology_util.h"
#include "machina/xla/pjrt/event_pool.h"
#include "machina/xla/pjrt/gpu/gpu_helpers.h"
#include "machina/xla/pjrt/gpu/gpu_topology.h"
#include "machina/xla/pjrt/gpu/gpu_topology.pb.h"
#include "machina/xla/pjrt/gpu/se_gpu_topology_description.h"
#include "machina/xla/pjrt/host_memory_spaces.h"
#include "machina/xla/pjrt/local_device_state.h"
#include "machina/xla/pjrt/pjrt_client.h"
#include "machina/xla/pjrt/pjrt_common.h"
#include "machina/xla/pjrt/pjrt_compiler.h"
#include "machina/xla/pjrt/pjrt_device_description.h"
#include "machina/xla/pjrt/pjrt_executable.h"
#include "machina/xla/pjrt/pjrt_future.h"
#include "machina/xla/pjrt/pjrt_stream_executor_client.h"
#include "machina/xla/pjrt/plugin/xla_gpu/xla_gpu_allocator_config.h"
#include "machina/xla/pjrt/plugin/xla_gpu/xla_gpu_client_options.h"
#include "machina/xla/pjrt/stream_executor_executable.h"
#include "machina/xla/pjrt/tracked_device_buffer.h"
#include "machina/xla/pjrt/worker_thread.h"
...
#if defined(GOOGLE_CUDA) || defined(MACHINA_USE_ROCM)
#include "machina/xla/debug_options_flags.h"
#include "machina/xla/pjrt/gpu/gpu_metrics.h"
#include "machina/xla/pjrt/proto/compile_options.pb.h"
#include "machina/xla/pjrt/stream_executor_executable.pb.h"
...
#include "machina/xla/service/gpu/gpu_memory_space_assignment.h"
...
#include "machina/xla/service/gpu/model/gpu_collective_performance_model.h"
#include "machina/xla/stream_executor/gpu/gpu_cudamallocasync_allocator.h"
#elif MACHINA_USE_ROCM
#include "rocm/rocm_config.h"
#endif

#include "machina/xla/service/gpu/gpu_executable_run_options.h"
 ...
 public:
  ...
  Create(absl::Span<const PjRtClient::ShapeSpec> shape_specs,
    ...
    for (int i = 0; i < shape_specs.size(); ++i) {
      ...
      auto* se_buffer =
          machina::down_cast<PjRtStreamExecutorBuffer*>(buffer.get());
      DCHECK(se_buffer);
  ...
  absl::Status TransferLiteralToBuffer(
    ...
    auto* se_client =
        machina::down_cast<PjRtStreamExecutorClient*>(device_->client());
    DCHECK(se_client);
...
GetTargetConfigForDevices(absl::Span<PjRtDevice* const> devices) {
  ...
  for (const PjRtDevice* device : devices) {
    LocalDeviceState* local_device_state =
        machina::down_cast<const PjRtStreamExecutorDevice*>(device)
            ->local_device_state();
...
absl::Status AbortOnFailure(
    absl::Span<const machina::CoordinatedTaskStateInfo> previous_state,
    absl::Span<const machina::CoordinatedTaskStateInfo> current_state) {
  ...
  for (int i = 0; i < previous_state.size(); ++i) {
    const machina::CoordinatedTaskStateInfo& previous = previous_state[i];
    const machina::CoordinatedTaskStateInfo& current = current_state[i];
    ...
    if (previous.state() !=
        machina::CoordinatedTaskState::TASKSTATE_CONNECTED) {
...
StreamExecutorGpuClient::StreamExecutorGpuClient(
  ...
  for (auto* device : addressable_devices()) {
    ...
    machina::down_cast<PjRtStreamExecutorDevice*>(device)->AttachMemorySpace(
...
#if MACHINA_USE_ROCM && defined(TF_ROCM_VERSION)  // rocm
...
#else
  return "<unknown>";
#endif  // MACHINA_USE_ROCM && defined(TF_ROCM_VERSION)
}
...
void StreamExecutorGpuClient::UpdateGlobalProcessInfo(
    absl::Span<machina::CoordinatedTaskStateInfo> infos) {
...
StreamExecutorGpuClient::CreateBuffersForAsyncHostToDevice(
  ...
  auto* stream_executor_device =
      machina::down_cast<PjRtStreamExecutorDevice*>(device);
...
PjRtFuture<> StreamExecutorGpuClient::CopyRawSubBufferToHost(
  ...
  auto* buffer = machina::down_cast<PjRtStreamExecutorBuffer*>(pjrt_buffer);
  DCHECK(buffer);
...
void StreamExecutorGpuClient::CopyToRemoteDevice(
  ...
  absl::StatusOr<LocalDeviceState*> local_device =
      machina::down_cast<PjRtStreamExecutorDevice*>(buffer->device())
          ->GetLocalDeviceState();
  if (!local_device.ok()) {
  ...
  auto* handle = machina::down_cast<PjRtStreamExecutorBuffer*>(buffer);
...
absl::StatusOr<tsl::AllocatorStats> StreamExecutorGpuDevice::GetAllocatorStats()
  ...
  auto* allocator_adapter = dynamic_cast<se::MultiDeviceAdapter*>(
      machina::down_cast<PjRtStreamExecutorClient*>(client())->allocator());
...
StreamExecutorGpuClient::RunAsync(
  ...
  auto* gpu_exec =
      machina::down_cast<xla::gpu::GpuExecutable*>(exec.executable());
