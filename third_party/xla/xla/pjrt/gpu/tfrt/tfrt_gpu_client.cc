...
#include "machina/xla/pjrt/gpu/tfrt/tfrt_gpu_client.h"
...
#include "machina/xla/pjrt/distributed/protocol.pb.h"
...
#include "machina/xla/pjrt/gpu/tfrt/gpu_event.h"
#include "machina/xla/pjrt/gpu/tfrt/host_memory_allocator.h"
#include "machina/xla/pjrt/gpu/tfrt/tracked_gpu_device_buffer.h"
#include "machina/xla/pjrt/host_callback.h"
#include "machina/xla/pjrt/host_memory_spaces.h"
#include "machina/xla/pjrt/layout_mode.h"
#include "machina/xla/pjrt/mlir_to_hlo.h"
...
#include "machina/xla/pjrt/semaphore.h"
#include "machina/xla/pjrt/stream_executor_executable.h"
#include "machina/xla/pjrt/transpose.h"
...
#include "machina/xla/service/generic_transfer_manager.h"
...
std::optional<stream_executor::GpuTargetConfigProto> GetTargetConfigForDevices(
  ...
  for (const PjRtDevice* device : devices) {
    se::StreamExecutor* executor =
        machina::down_cast<const TfrtGpuDevice*>(device)->executor();
    if (executor != nullptr) {
...
std::vector<std::unique_ptr<PjRtMemorySpace>> InitializeMemorySpaces(
  ...
  for (auto* device : addressable_devices) {
    ...
    TfrtGpuDevice* gpu_device = machina::down_cast<TfrtGpuDevice*>(device);
...
absl::StatusOr<std::unique_ptr<PjRtLoadedExecutable>> TfrtGpuClient::Load(
  ...
  auto se_executable = absl::WrapUnique(
      machina::down_cast<StreamExecutorExecutable*>(executable.release()));
...
absl::StatusOr<std::unique_ptr<PjRtBuffer>> TfrtGpuClient::CreateErrorBuffer(
  ...
  TfrtGpuDevice* device =
      machina::down_cast<TfrtGpuDevice*>(memory_space->devices().front());
...
TfrtGpuClient::CreateBuffersForAsyncHostToDevice(
  ...
  auto* tfrt_gpu_device = machina::down_cast<TfrtGpuDevice*>(device);
...
absl::Status TfrtGpuClient::DmaMap(void* data, size_t buffer_size) {
  ...
  se::StreamExecutor* executor =
      machina::down_cast<TfrtGpuDevice*>(addressable_devices_[0])
          ->executor();
  DCHECK(executor);
...
TfrtGpuExecutable::TfrtGpuExecutable(
  ...
  if (device_assignment_ == nullptr) {
    ...
    if ((device_assignment_->replica_count() > 1 ||
      ...
      // "--run=false" option in
      // machina/compiler/xla/tools/multihost_hlo_runner/hlo_runner_main.cc.
  ...
  if (num_addressable_devices == 1 && !ThisThreadIsInsideHostCallback()) {
    ...
    for (int i = 0; i < num_addressable_devices; ++i) {
      ...
      TfrtGpuDevice* gpu_device =
          machina::down_cast<TfrtGpuDevice*>(pjrt_device);
