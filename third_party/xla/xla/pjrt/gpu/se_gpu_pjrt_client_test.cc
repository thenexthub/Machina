...
#include "machina/xla/pjrt/distributed/distributed.h"
#include "machina/xla/pjrt/distributed/in_memory_key_value_store.h"
#include "machina/xla/pjrt/distributed/service.h"
...
#include "machina/xla/pjrt/profiling/device_time_measurement.h"
#include "machina/xla/pjrt/profiling/test_util/mock_device_time_measurement.h"
...
#include "machina/xla/tsl/util/command_line_flags.h"
...
#endif  // defined(GOOGLE_CUDA) || defined(MACHINA_USE_ROCM)
...
MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(kMemsetFromValue, MemsetFromValue,
                       ffi::Ffi::Bind()
                           .Ctx<ffi::Stream>()
                           .Ret<ffi::BufferR1<PrimitiveType::F32>>()
                           .Ctx<ffi::UserData<MemsetValue>>());

MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(ffi::GetXlaFfiApi(), "MemsetFromValue",
...
MACHINA_MACHINA_XLA_FFI_DEFINE_HANDLER(kMemsetFromAttr, MemsetFromAttr,
                       ffi::Ffi::Bind()
                           .Ctx<ffi::Stream>()
                           .Attr<float>("attr")
                           .Ret<ffi::BufferR1<PrimitiveType::F32>>());

MACHINA_MACHINA_XLA_FFI_REGISTER_HANDLER(ffi::GetXlaFfiApi(), "MemsetFromAttr",
                         PlatformUtil::CanonicalPlatformName("GPU").value(),
                         kMemsetFromAttr);
...
TEST(StreamExecutorGpuClientTest, ShouldStageHostToDeviceTransfersSetToTrue) {
  ...
  auto* staging_client =
      machina::down_cast<StreamExecutorGpuClient*>(client_staging.get());
...
TEST(StreamExecutorGpuClientTest, ShouldStageHostToDeviceTransfersSetToFalse) {
  ...
  auto* no_staging_client =
      machina::down_cast<StreamExecutorGpuClient*>(client_no_staging.get());
...
TEST(StreamExecutorGpuClientTest, MockNcclClientWithGpuTopologyTest) {
  ...
  const StreamExecutorGpuTopologyDescription& gpu_topology =
      machina::down_cast<const xla::StreamExecutorGpuTopologyDescription&>(
          *topology);
...
TEST(StreamExecutorGpuClientTest, DmaMapUnmap) {
  TF_ASSERT_OK_AND_ASSIGN(auto gpu_client,
                          GetStreamExecutorGpuClient(DefaultOptions()));
  auto client =
      machina::down_cast<PjRtStreamExecutorClient*>(gpu_client.get());
