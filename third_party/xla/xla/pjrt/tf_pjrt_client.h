...
#ifndef MACHINA_MACHINA_XLA_PJRT_TF_PJRT_CLIENT_H_
#define MACHINA_MACHINA_XLA_PJRT_TF_PJRT_CLIENT_H_
...
class TfPjRtClient : public PjRtClient {
 ...
 private:
  ...
  PjRtBuffer* UnwrapBuffer(PjRtBuffer* buffer) const {
    return machina::down_cast<TfPjRtBuffer*>(buffer)->wrapped();
  }

  // Unwraps a TfPjRtExecutable.
  const PjRtLoadedExecutable& UnwrapExecutable(
      const PjRtLoadedExecutable& executable) const {
    return *machina::down_cast<const TfPjRtExecutable*>(&executable)
                ->wrapped();
  }
...
#endif  // MACHINA_MACHINA_XLA_PJRT_TF_PJRT_CLIENT_H_
