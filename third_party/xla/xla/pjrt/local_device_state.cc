...
LocalDeviceState::LocalDeviceState(se::StreamExecutor* executor,
  ...
  // Setting MACHINA_MACHINA_XLA_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL to false will:
  ...
  absl::Status status =
      tsl::ReadBoolFromEnvVar("MACHINA_MACHINA_XLA_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL", true,
                              &allow_delete_before_fulfill_);
  if (!status.ok()) {
    LOG(ERROR) << "Failed to read MACHINA_MACHINA_XLA_PJRT_GPU_ALLOW_DELETE_BEFORE_FULFILL: "
               << status;
  }
