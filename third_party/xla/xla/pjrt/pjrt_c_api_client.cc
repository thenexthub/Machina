...
#include "machina/xla/pjrt/c/pjrt_c_api_ffi_extension.h"
#include "machina/xla/pjrt/c/pjrt_c_api_helpers.h"
#include "machina/xla/pjrt/c/pjrt_c_api_layouts_extension.h"
#include "machina/xla/pjrt/c/pjrt_c_api_memory_descriptions_extension.h"
#include "machina/xla/pjrt/c/pjrt_c_api_profiler_extension.h"
#include "machina/xla/pjrt/c/pjrt_c_api_stream_extension.h"
#include "machina/xla/pjrt/distributed/key_value_store_interface.h"
#include "machina/xla/pjrt/extensions/cross_host_transfers/pjrt_c_api_cross_host_transfers_extension.h"
...
void PjRtCApiClient::InitDevicesAndMemorySpaces() {
  ...
  for (const auto& device : addressable_devices_) {
    PjRtCApiDevice* cpp_device = machina::down_cast<PjRtCApiDevice*>(device);
  ...
  for (const auto& memory : addressable_memory_spaces_) {
    PjRtCApiMemorySpace* cpp_memory =
        machina::down_cast<PjRtCApiMemorySpace*>(memory);
...
void PjRtCApiClient::UpdateGlobalProcessInfo(
  ...
  auto translate_state = [](machina::CoordinatedTaskState state) {
    switch (state) {
      case machina::CoordinatedTaskState::TASKSTATE_UNSPECIFIED:
        return PJRT_ProcessState_kUnspecified;
      case machina::CoordinatedTaskState::TASKSTATE_UNINITIALIZED:
        return PJRT_ProcessState_kUninitialized;
      case machina::CoordinatedTaskState::TASKSTATE_DISCONNECTED:
        return PJRT_ProcessState_kDisconnected;
      case machina::CoordinatedTaskState::TASKSTATE_CONNECTED:
        return PJRT_ProcessState_kConnected;
      case machina::CoordinatedTaskState::TASKSTATE_ERROR:
  ...
  for (const machina::CoordinatedTaskStateInfo& info : infos) {
...
PjRtCApiClient::CompileAndLoad(mlir::ModuleOp module, CompileOptions options) {
  if (!pjrt_c_api()) toolchain::report_fatal_error("pjrt_c_api is null");
...
PjRtCApiClient::CreateUninitializedBuffer(const Shape& shape,
  ...
  args.memory =
      machina::down_cast<PjRtCApiMemorySpace*>(memory_space)->c_memory();
...
absl::StatusOr<std::uintptr_t> PjRtCApiClient::UnsafeBufferPointer(
  ...
  args.extension_start = nullptr;
  args.buffer =
      machina::down_cast<const PjRtCApiBuffer*>(buffer)->c_buffer();
...
PjRtCApiClient::BufferFromHostBufferInternalImpl(
  ...
  if (std::holds_alternative<PjRtDevice*>(device_or_memory)) {
    args.device = machina::down_cast<PjRtCApiDevice*>(
    ...
    args.device = nullptr;
    args.memory = machina::down_cast<PjRtCApiMemorySpace*>(
...
PjRtCApiClient::MakeCrossHostReceiveBuffers(
  ...
  args.device = machina::down_cast<PjRtCApiDevice*>(device)->c_device();
...
static std::vector<std::vector<PJRT_Buffer*>> Convert2DCppBuffersToCBuffers(
  ...
  for (const auto& cpp_list : cpp_lists) {
    ...
    for (PjRtBuffer* buffer : cpp_list) {
      auto* c_api_argument = machina::down_cast<PjRtCApiBuffer*>(buffer);
...
PjRtCApiLoadedExecutable::ExecuteWithSingleDevice(
  ...
  args.execute_device =
      machina::down_cast<PjRtCApiDevice*>(device)->c_device();
  PJRT_Profiler_Extension profiler_extension =
      pjrt::CreatePjrtProfilerExtension(
          "PJRT_LoadedExecutable_Execute linkage");
  args.extension_start = &profiler_extension.base;
...
absl::StatusOr<std::unique_ptr<PjRtBuffer>> PjRtCApiBuffer::CopyToMemorySpace(
  ...
  if (dst_memory->client() == client_) {
    ...
    args.buffer = buffer_.get();
    args.dst_memory =
        machina::down_cast<PjRtCApiMemorySpace*>(dst_memory)->c_memory();
...
InitializeArgsAndCompileAot(const PJRT_Api* c_api, PjRtClient* client,
  ...
  if (client == nullptr) {
    ...
    args.client =
        machina::down_cast<PjRtCApiClient*>(client)->pjrt_c_client();
  }
  args.topology =
      machina::down_cast<const PjRtCApiTopologyDescription*>(&topology)
          ->c_topology();
  TF_ASSIGN_OR_RETURN(const CompileOptionsProto options_proto,
                      options.ToProto());
  std::string options_str = options_proto.SerializeAsString();
