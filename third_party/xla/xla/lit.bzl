...
def lit_test_suite(
    ...
    See https://toolchain.org/docs/CommandGuide/lit.html for details on lit
    """
...
def lit_script_with_xla_gpu_cuda_data_dir(
    ...
    """Adds a line to the LIT script to set the MACHINA_MACHINA_XLA_FLAGS environment variable."""
    return native.genrule(
        name = name,
        srcs = [input_file],
        outs = [output_file],
        cmd = if_cuda_tools(
            """echo -e '// RUN: export MACHINA_MACHINA_XLA_FLAGS=\"--xla_gpu_cuda_data_dir={}\"' > $@;
...
def lit_test(
    ...
    llvm_symbolizer = "@toolchain-project//toolchain:toolchain-symbolizer"
    ...
    py_binary(
        name = lit_name,
        main = "@toolchain-project//toolchain:utils/lit/lit.py",
        srcs = ["@toolchain-project//toolchain:utils/lit/lit.py"],
        testonly = True,
        deps = [
            "@toolchain-project//toolchain:lit_lib",
            "@pypi_lit//:pkg",
        ],
    )
