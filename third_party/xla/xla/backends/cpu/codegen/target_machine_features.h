...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_FEATURES_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_FEATURES_H_
...
class TargetMachineFeatures {
 public:
  ...
  explicit TargetMachineFeatures(toolchain::TargetMachine* target_machine);
  ...
  const toolchain::TargetMachine* target_machine() const;
  ...
  // "function" since toolchain functions can contain annotations for specializing
  // them to specific micro-architectures (though currently XLA does not use
  // this functionality).
  virtual int32_t vector_register_byte_size(const toolchain::Function& fn) const;

  // Return the number of elements of type `type` that can fit into the largest
  // vector register available.  We need to pass in "function" since toolchain
  ...
  virtual int32_t vector_register_num_elements(const toolchain::Function& fn,
  ...
  // them to specific micro-architectures (though currently XLA does not use
  // this functionality).
  virtual int32_t vector_register_count(const toolchain::Function& fn) const;
 ...
 private:
  toolchain::TargetTransformInfo* GetTargetTransformInfoFor(
      const toolchain::Function& fn) const;

  // A cache of resolved TargetTransformInfo for LLVM functions.
  mutable absl::flat_hash_map<const toolchain::Function*, toolchain::TargetTransformInfo>
      target_transform_info_;

  toolchain::TargetMachine* target_machine_;
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_FEATURES_H_
