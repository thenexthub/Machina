...
ComputationKernelEmitter::EmitKernelDefinition() {
  ...
  toolchain::Value* alloca_size = ir_builder.getInt64(
      kernel_prototype.arguments.size() + kernel_prototype.results.size());
  toolchain::Value* buffer_table = ir_builder.CreateAlloca(
  ...
  for (const auto& [array, slice] : toolchain::zip(
    ...
    toolchain::Value* buffer_table_ptr = llvm_ir::EmitBufferIndexingGEP(
  ...
  for (const auto& [array, slice] :
       toolchain::zip(kernel_prototype.results, kernel_prototype.result_buffers)) {
    int64_t index = buffer_table_index++;
    slice_to_buffer_table_index[slice] = index;
    toolchain::Value* buffer_table_ptr = llvm_ir::EmitBufferIndexingGEP(
        buffer_table, ir_builder.getPtrTy(), index, &ir_builder);
  ...
  TF_ASSIGN_OR_RETURN(
      toolchain::Function * computation_function,
  ...
  toolchain::Value* llvm_nullptr =
      toolchain::Constant::getNullValue(ir_builder.getPtrTy());
  std::vector<toolchain::Value*> args = {llvm_nullptr, llvm_nullptr, llvm_nullptr,
...
absl::StatusOr<toolchain::Function*> ComputationKernelEmitter::EmitNestedComputation(
    toolchain::Function* function, toolchain::BasicBlock* return_block,
    toolchain::IRBuilderBase& builder, toolchain::Module& llvm_module,
