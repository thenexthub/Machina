...
TEST_P(ObjectLoaderTest, Load) {
  ...
  auto object_files_saver =
      [&object_files](const toolchain::Module& /*module*/,
                      const toolchain::object::ObjectFile& object_file) -> void {
  ...
  std::unique_ptr<IrCompiler> ir_compiler = IrCompiler::Create(
      toolchain::TargetOptions(), IrCompiler::Options(), ir_compiler_hooks);
  ...
  toolchain::DataLayout data_layout = compiler.target_machine()->createDataLayout();
