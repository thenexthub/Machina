...
#include "machina/xla/hlo/testlib/hlo_hardware_independent_test_base.h"
...
class KernelApiIrBuilderTestBase : public HloHardwareIndependentTestBase {
 public:
  ...
  toolchain::IRBuilder<> getBuilder() { return toolchain::IRBuilder<>(context_); }
  ...
  void SetKernelFunctionAttributes(toolchain::Function* function) {
    kernel_api_ir_builder_.SetKernelFunctionAttributes(function);
  }

  toolchain::LLVMContext& context() { return context_; }
...
TEST_F(KernelApiIrBuilderTest, BuildKernelPrototype) {
  ...
  toolchain::IRBuilder<> builder = getBuilder();
  builder.SetInsertPoint(prototype.function->getEntryBlock().getTerminator());

  auto* zero = toolchain::ConstantInt::get(toolchain::Type::getInt32Ty(context()), 0);
  ...
  ASSERT_TRUE(*RunFileCheck(DumpToString(),
    ...
    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 0
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_NumWorkGroups, {{.*}} i32 0
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_NumWorkGroups, {{.*}} i32 1
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_NumWorkGroups, {{.*}} i32 2
    CHECK:      load i64
    CHECK:      load i64
    CHECK:      load i64

    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 1
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_WorkGroupId, {{.*}} i32 0
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_WorkGroupId, {{.*}} i32 1
    CHECK:      getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_WorkGroupId, {{.*}} i32 2
    CHECK:      load i64
    CHECK:      load i64
    CHECK:      load i64

    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 3
    CHECK:      load ptr
    CHECK:      getelementptr %MACHINA_MACHINA_XLA_CPU_KernelArg, {{.*}} i32 0, i32 0
    CHECK:      %[[ARG0:.+]] = load ptr, {{.*}}, !invariant.load ![[SCOPE0:.+]], !dereferenceable ![[DEREF_BYTES:.+]], !align ![[ALIGNMENT:.+]]

    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 3
    CHECK:      load ptr
    CHECK:      getelementptr %MACHINA_MACHINA_XLA_CPU_KernelArg, {{.*}} i32 1, i32 0
    CHECK:      %[[ARG1:.+]] = load ptr, {{.*}}, !invariant.load ![[SCOPE0]], !dereferenceable ![[DEREF_BYTES]], !align ![[ALIGNMENT]]

    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 3
    CHECK:      load ptr
    CHECK:      getelementptr %MACHINA_MACHINA_XLA_CPU_KernelArg, {{.*}} i32 2, i32 0
    CHECK:      %[[ARG2:.+]] = load ptr, {{.*}}, !invariant.load ![[SCOPE0]], !dereferenceable ![[DEREF_BYTES]], !align ![[ALIGNMENT]]

    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 3
    CHECK:      load ptr
    CHECK:      getelementptr %MACHINA_MACHINA_XLA_CPU_KernelArg, {{.*}} i32 3, i32 0
...
TEST_F(KernelApiIrBuilderTest, AllInvariantBuffers) {
  ...
  auto module = std::make_unique<toolchain::Module>("test", context);
...
TEST_F(KernelApiIrBuilderTestNoBufferValidation, PartialOverlap) {
  ...
  ASSERT_TRUE(*RunFileCheck(DumpToString(),
    ...
    CHECK-NEXT: getelementptr inbounds nuw %MACHINA_MACHINA_XLA_CPU_KernelCallFrame, {{.*}} i32 3
    CHECK:      load ptr
    CHECK:      getelementptr %MACHINA_MACHINA_XLA_CPU_KernelArg, {{.*}} i32 4, i32 0
...
TEST_F(KernelApiIrBuilderTest, SetKernelFunctionAttributes) {
  toolchain::LLVMContext context;
  auto module = std::make_unique<toolchain::Module>("test", context);
  toolchain::FunctionType* function_ty =
      toolchain::FunctionType::get(toolchain::PointerType::getUnqual(context),
                              toolchain::PointerType::getUnqual(context),
                              /*isVarArg=*/false);
  toolchain::Function* function = toolchain::Function::Create(
      function_ty, toolchain::GlobalValue::ExternalLinkage, "foo", *module);
