...
#include "machina/xla/backends/cpu/codegen/symbol_name_util.h"
...
class MemoryDependencyAnalyzer {
 public:
  MemoryDependencyAnalyzer(
      toolchain::LLVMContext& context, absl::string_view name,
    ...
    toolchain::MDNode* domain = mb_.createAliasScopeDomain(
  ...
  toolchain::MDNode* GetAliasScope(BufferAllocation::Slice slice) {
    auto it = alias_scopes_.find(slice);
    return it == alias_scopes_.end() ? nullptr
                                     : toolchain::MDNode::get(context_, it->second);
  };

  // Construct !noalias metadata for buffer slice.
  toolchain::MDNode* GetNoAlias(BufferAllocation::Slice slice) {
    toolchain::SmallVector<toolchain::Metadata*> scopes;
    for (const auto& [alias_slice, alias_scope] : alias_scopes_) {
      if (!slice.OverlapsWith(alias_slice)) {
        scopes.push_back(alias_scope);
      }
    }
    return scopes.empty() ? nullptr : toolchain::MDNode::get(context_, scopes);
  };
 ...
 private:
  ...
  toolchain::MDBuilder mb_;

  absl::btree_map<BufferAllocation::Slice, toolchain::MDNode*> alias_scopes_;
...
toolchain::StructType* Dim3StructTy(toolchain::LLVMContext& ctx, absl::string_view name) {
  toolchain::IntegerType* i64 = toolchain::IntegerType::getInt64Ty(ctx);
  return toolchain::StructType::create(name, i64, i64, i64);
}

toolchain::StructType* KernelNumWorkGroupsTy(toolchain::LLVMContext& ctx) {
  return Dim3StructTy(ctx, "MACHINA_MACHINA_XLA_CPU_NumWorkGroups");
}

toolchain::StructType* KernelWorkGroupIdTy(toolchain::LLVMContext& ctx) {
  return Dim3StructTy(ctx, "MACHINA_MACHINA_XLA_CPU_WorkGroupId");
}

toolchain::StructType* KernelArgTy(toolchain::LLVMContext& ctx) {
  toolchain::PointerType* ptr = toolchain::PointerType::getUnqual(ctx);
  toolchain::IntegerType* i64 = toolchain::IntegerType::getInt64Ty(ctx);
  return toolchain::StructType::create("MACHINA_MACHINA_XLA_CPU_KernelArg", ptr, i64);
}

toolchain::StructType* KernelCallFrameTy(toolchain::LLVMContext& ctx) {
  toolchain::PointerType* ptr = toolchain::PointerType::getUnqual(ctx);
  toolchain::IntegerType* i64 = toolchain::IntegerType::getInt64Ty(ctx);
  return toolchain::StructType::create("MACHINA_MACHINA_XLA_CPU_KernelCallFrame", ptr, ptr, i64,
                                  ptr);
}

toolchain::FunctionType* KernelFunctionTy(toolchain::LLVMContext& ctx) {
  return toolchain::FunctionType::get(toolchain::PointerType::getUnqual(ctx),
                                 toolchain::PointerType::getUnqual(ctx),
                                 /*isVarArg=*/false);
}
...
void SetModuleMemoryRegionName(toolchain::Module& llvm_module,
                               absl::string_view name) {
  auto& context = llvm_module.getContext();
  toolchain::IRBuilder<> builder(context);

  toolchain::MDString* memory_region_name_md = toolchain::MDString::get(context, name);

  toolchain::MDNode* custom_md_node =
      toolchain::MDNode::get(context, {memory_region_name_md});

  toolchain::NamedMDNode* named_md_node =
...
KernelApiIrBuilder::KernelApiIrBuilder(toolchain::LLVMContext& context,
                                       Options options,
...
auto KernelApiIrBuilder::EmitKernelPrototype(
    toolchain::Module& module, const HloInstruction* instr,
    const BufferAssignment* buffer_assignment,
...
auto KernelApiIrBuilder::EmitKernelPrototype(
    toolchain::Module& module, absl::string_view name,
  ...
  toolchain::IRBuilder<> b(context_);

  // Create a kernel function with HostKernel API.
  toolchain::Function* function = EmitKernelFunction(module, name);

  // Create an entry basic block and set insert point to the end of it.
  b.SetInsertPoint(toolchain::BasicBlock::Create(context_, "", function));

  toolchain::Value* call_frame = function->getArg(0);
  ...
  toolchain::BasicBlock* return_block =
      toolchain::BasicBlock::Create(context_, "return", function);

  b.CreateBr(return_block);

  b.SetInsertPoint(return_block);
  b.CreateRet(
      toolchain::ConstantPointerNull::get(toolchain::PointerType::getUnqual(context_)));
...
std::unique_ptr<toolchain::Module> KernelApiIrBuilder::CreateModule(
    absl::string_view name, toolchain::LLVMContext& context) {
  constexpr absl::string_view kXlaModuleIdentifier = "__compute_module";
  return std::make_unique<toolchain::Module>(
      absl::StrCat(kXlaModuleIdentifier, "_", name), context);
}

auto KernelApiIrBuilder::EmitKernelNumWorkGroups(toolchain::IRBuilderBase& builder,
                                                 toolchain::Value* call_frame)
    -> NumWorkGroups {
  toolchain::Value* num_workgroups_gep = builder.CreateStructGEP(
      call_frame_ty_, call_frame, 0, "num_workgroups_gep");
  toolchain::Value* num_workgroups = builder.CreateLoad(
      builder.getPtrTy(), num_workgroups_gep, "num_workgroups");
  toolchain::Value* x_gep = builder.CreateStructGEP(
      num_workgroups_ty_, num_workgroups, 0, "num_workgroups_x_gep");
  toolchain::Value* y_gep = builder.CreateStructGEP(
      num_workgroups_ty_, num_workgroups, 1, "num_workgroups_y_gep");
  toolchain::Value* z_gep = builder.CreateStructGEP(
...
auto KernelApiIrBuilder::EmitKernelWorkGroupId(toolchain::IRBuilderBase& builder,
                                               toolchain::Value* call_frame)
    -> WorkGroupId {
  toolchain::Value* workgroup_id_gep = builder.CreateStructGEP(
      call_frame_ty_, call_frame, 1, "workgroup_id_gep");
  toolchain::LoadInst* workgroup_id =
      builder.CreateLoad(builder.getPtrTy(), workgroup_id_gep, "workgroup_id");
  toolchain::Value* x_gep = builder.CreateStructGEP(workgroup_id_ty_, workgroup_id,
                                               0, "workgroup_id_x_gep");
  toolchain::Value* y_gep = builder.CreateStructGEP(workgroup_id_ty_, workgroup_id,
                                               1, "workgroup_id_y_gep");
  toolchain::Value* z_gep = builder.CreateStructGEP(workgroup_id_ty_, workgroup_id,
...
llvm_ir::IrArray KernelApiIrBuilder::EmitKernelArgument(
    toolchain::IRBuilderBase& builder, toolchain::Value* call_frame, int64_t index,
    const Shape& shape) {
  toolchain::LLVMContext& ctx = builder.getContext();

  toolchain::Type* ptr = toolchain::PointerType::get(ctx, 0);
  std::string name = absl::StrCat("arg", index);

  toolchain::Value* args_gep =
      builder.CreateStructGEP(call_frame_ty_, call_frame, 3, "args_gep");
  toolchain::LoadInst* args = builder.CreateLoad(ptr, args_gep, "args");
  toolchain::Value* data_gep =
      builder.CreateConstGEP2_32(arg_ty_, args, index, 0, name + "_gep");
  toolchain::LoadInst* data = builder.CreateLoad(ptr, data_gep, name);
  ...
  // dereferenceable.
  const toolchain::Module* llvm_module = builder.GetInsertBlock()->getModule();
  const toolchain::DataLayout& data_layout = llvm_module->getDataLayout();
  ...
  if (options_.enable_invariant_load_metadata) {
    data->setMetadata(toolchain::LLVMContext::MD_invariant_load,
                      toolchain::MDNode::get(data->getContext(), /*MDs=*/{}));
  }

  return llvm_ir::IrArray(data, llvm_ir::ShapeToIrType(shape, ctx), shape);
}

toolchain::Function* KernelApiIrBuilder::EmitKernelFunction(toolchain::Module& module,
                                                       absl::string_view name) {
  toolchain::Function* function = toolchain::Function::Create(
      kernel_function_ty_, toolchain::GlobalValue::ExternalLinkage, name, module);

  SetKernelFunctionAttributes(function);
  return function;
}

void KernelApiIrBuilder::SetKernelFunctionAttributes(toolchain::Function* function) {
