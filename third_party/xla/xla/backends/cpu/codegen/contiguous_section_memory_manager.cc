...
#include "machina/xla/backends/cpu/codegen/contiguous_section_memory_manager.h"
...
class DefaultMemoryMapper final
    : public toolchain::SectionMemoryManager::MemoryMapper {
 public:
  toolchain::sys::MemoryBlock allocateMappedMemory(
      toolchain::SectionMemoryManager::AllocationPurpose purpose, size_t num_bytes,
      const toolchain::sys::MemoryBlock* const near_block, unsigned flags,
      std::error_code& error_code) override {
    return toolchain::sys::Memory::allocateMappedMemory(num_bytes, near_block, flags,
                                                   error_code);
  }

  std::error_code protectMappedMemory(const toolchain::sys::MemoryBlock& block,
                                      unsigned flags) override {
    return toolchain::sys::Memory::protectMappedMemory(block, flags);
  }

  std::error_code releaseMappedMemory(toolchain::sys::MemoryBlock& m) override {
    return toolchain::sys::Memory::releaseMappedMemory(m);
  }
};
...
ContiguousSectionMemoryManager::ContiguousSectionMemoryManager(
    toolchain::SectionMemoryManager::MemoryMapper* mmapper)
...
void ContiguousSectionMemoryManager::reserveAllocationSpace(
    uintptr_t code_size, toolchain::Align code_align, uintptr_t ro_data_size,
    toolchain::Align ro_data_align, uintptr_t rw_data_size,
    toolchain::Align rw_data_align) {
  CHECK_EQ(allocation_.allocatedSize(), 0);

  static const size_t page_size = toolchain::sys::Process::getPageSizeEstimate();
  ...
  allocation_ = mmapper_->allocateMappedMemory(
      toolchain::SectionMemoryManager::AllocationPurpose::Code, total_size, nullptr,
      toolchain::sys::Memory::MF_READ | toolchain::sys::Memory::MF_WRITE, ec);
  if (ec) {
  ...
  code_block_ = code_free_ =
      toolchain::sys::MemoryBlock(reinterpret_cast<void*>(base), code_size);
  base += code_size;
  ro_data_block_ = ro_data_free_ =
      toolchain::sys::MemoryBlock(reinterpret_cast<void*>(base), ro_data_size);
  base += ro_data_size;
  rw_data_block_ = rw_data_free_ =
      toolchain::sys::MemoryBlock(reinterpret_cast<void*>(base), rw_data_size);
}

uint8_t* ContiguousSectionMemoryManager::allocateDataSection(
    uintptr_t size, unsigned alignment, unsigned section_id,
    toolchain::StringRef section_name, bool is_read_only) {
  if (is_read_only) {
...
uint8_t* ContiguousSectionMemoryManager::Allocate(
  ...
  free_block =
      toolchain::sys::MemoryBlock(reinterpret_cast<void*>(base + padded_size),
...
bool ContiguousSectionMemoryManager::finalizeMemory(std::string* err_msg) {
  ...
  ec = mmapper_->protectMappedMemory(
      code_block_, toolchain::sys::Memory::MF_READ | toolchain::sys::Memory::MF_EXEC);
  if (ec) {
    if (err_msg) {
      *err_msg = ec.message();
    }
  ...
  ec =
      mmapper_->protectMappedMemory(ro_data_block_, toolchain::sys::Memory::MF_READ);
  if (ec) {
    if (err_msg) {
      *err_msg = ec.message();
    }
  ...
  toolchain::sys::Memory::InvalidateInstructionCache(code_block_.base(),
