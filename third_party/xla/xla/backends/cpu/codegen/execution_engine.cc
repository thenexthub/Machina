...
#include "machina/xla/service/cpu/orc_jit_memory_mapper.h"

namespace xla::cpu {

static std::unique_ptr<toolchain::orc::RTDyldObjectLinkingLayer>
CreateObjectLinkingLayer(toolchain::orc::ExecutionSession& execution_session) {
  return std::make_unique<toolchain::orc::RTDyldObjectLinkingLayer>(
      execution_session, [](const toolchain::MemoryBuffer& obj) {
...
ExecutionEngine::ExecutionEngine(
  ...
  execution_session_->setErrorReporter([](toolchain::Error err) {
    LOG(ERROR) << "LLVM compilation error: " << toolchain::toString(std::move(err));
  });
}

void ExecutionEngine::RegisterJITEventListeners() {
  gdb_listener_ = toolchain::JITEventListener::createGDBRegistrationListener();
