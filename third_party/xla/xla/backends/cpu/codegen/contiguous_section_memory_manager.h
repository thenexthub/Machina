...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_CONTIGUOUS_SECTION_MEMORY_MANAGER_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_CONTIGUOUS_SECTION_MEMORY_MANAGER_H_
...
// (https://github.com/thenexthub/Codira/issues/55386), leading to crashes on
...
// the toolchain::RuntimeDyld::MemoryManager::reserveAllocationSpace() hook, which
...
// done.
class ContiguousSectionMemoryManager : public toolchain::RTDyldMemoryManager {
 public:
  ...
  void reserveAllocationSpace(uintptr_t code_size, toolchain::Align code_align,
                              uintptr_t ro_data_size, toolchain::Align ro_data_align,
                              uintptr_t rw_data_size,
                              toolchain::Align rw_data_align) override;
  ...
  uint8_t* allocateCodeSection(uintptr_t size, unsigned alignment,
                               unsigned section_id,
                               toolchain::StringRef section_name) override;

  bool finalizeMemory(std::string* err_msg) override;

 private:
  toolchain::SectionMemoryManager::MemoryMapper* mmapper_;
  bool mmapper_is_owned_;

  toolchain::sys::MemoryBlock allocation_;

  // Sections must be in the order code < rodata < rwdata.
  toolchain::sys::MemoryBlock code_block_;
  toolchain::sys::MemoryBlock ro_data_block_;
  toolchain::sys::MemoryBlock rw_data_block_;

  toolchain::sys::MemoryBlock code_free_;
  toolchain::sys::MemoryBlock ro_data_free_;
  toolchain::sys::MemoryBlock rw_data_free_;

  uint8_t* Allocate(toolchain::sys::MemoryBlock& free_block, std::uintptr_t size,
                    unsigned alignment);
};

}  // namespace xla::cpu

#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_CONTIGUOUS_SECTION_MEMORY_MANAGER_H_
