...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_KERNEL_API_IR_BUILDER_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_KERNEL_API_IR_BUILDER_H_
...
class KernelApiIrBuilder {
 public:
  ...
  KernelApiIrBuilder(
      toolchain::LLVMContext& context, Options options,
  ...
  static std::unique_ptr<toolchain::Module> CreateModule(absl::string_view name,
 ...
 private:
  NumWorkGroups EmitKernelNumWorkGroups(toolchain::IRBuilderBase& builder,
                                        toolchain::Value* call_frame);

  WorkGroupId EmitKernelWorkGroupId(toolchain::IRBuilderBase& builder,
                                    toolchain::Value* call_frame);

  llvm_ir::IrArray EmitKernelArgument(toolchain::IRBuilderBase& builder,
                                      toolchain::Value* call_frame, int64_t index,
                                      const Shape& shape);

  toolchain::Function* EmitKernelFunction(toolchain::Module& module,
                                     absl::string_view name);
 ...
 private:
  ...
  toolchain::StructType* num_workgroups_ty_;
  toolchain::StructType* workgroup_id_ty_;
  toolchain::StructType* arg_ty_;
  toolchain::StructType* call_frame_ty_;
  toolchain::FunctionType* kernel_function_ty_;
};
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_KERNEL_API_IR_BUILDER_H_
