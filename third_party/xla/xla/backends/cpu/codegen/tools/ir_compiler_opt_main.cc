...
struct IrCompilerOptConfig {
  ...
  int opt_level = static_cast<int>(toolchain::CodeGenOptLevel::Aggressive);
};
...
absl::StatusOr<std::unique_ptr<toolchain::Module>> ParseLlvmIr(
    const std::string& ir_content, toolchain::LLVMContext& context) {
  toolchain::SMDiagnostic error;
  std::unique_ptr<toolchain::Module> module =
      toolchain::parseIR(toolchain::MemoryBufferRef(ir_content, "input"), error, context);

  if (!module) {
    error.print("ir-compiler-opt", toolchain::errs());
...
absl::StatusOr<std::string> RunIrCompilerPasses(const IrCompilerOptConfig& opts,
  ...
  ir_compiler_options.opt_level =
      static_cast<toolchain::CodeGenOptLevel>(opts.opt_level);
  ...
  TF_ASSIGN_OR_RETURN(std::unique_ptr<toolchain::Module> module,
                      ParseLlvmIr(ir_content, context));

  TF_ASSIGN_OR_RETURN(
      std::unique_ptr<toolchain::TargetMachine> target_machine,
      ir_compiler->InferTargetMachine(
          target_options, static_cast<toolchain::CodeGenOptLevel>(opts.opt_level),
          std::nullopt));

  toolchain::Error error = ir_compiler->RunIrPasses(*module, target_machine.get());
  if (error) {
    return absl::InvalidArgumentError(
        absl::StrCat("Failed to run IR compiler passes: ",
                     toolchain::toString(std::move(error))));
  }
  return toolchain::to_string(*module);
}
