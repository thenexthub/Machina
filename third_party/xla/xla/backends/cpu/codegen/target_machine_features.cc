...
const toolchain::TargetMachine* TargetMachineFeatures::target_machine() const {
...
int32_t TargetMachineFeatures::vectorization_factor_in_bytes() const {
  ...
  // get from toolchain::TargetTransformInfo::getCacheLineSize) of the target
...
int32_t TargetMachineFeatures::vector_register_byte_size(
  ...
  toolchain::TargetTransformInfo* tti = GetTargetTransformInfoFor(fn);
  return tti->getRegisterBitWidth(
             toolchain::TargetTransformInfo::RGK_FixedWidthVector) /
         8;
}

int32_t TargetMachineFeatures::vector_register_num_elements(
    const toolchain::Function& fn, PrimitiveType type) const {
...
toolchain::TargetTransformInfo* TargetMachineFeatures::GetTargetTransformInfoFor(
    const toolchain::Function& fn) const {
...
int64_t TargetMachineFeatures::minimum_alignment_for_allocation(
  ...
  // anyways.
  return std::min<int64_t>(toolchain::PowerOf2Ceil(size_bytes), MinAlign());
}
