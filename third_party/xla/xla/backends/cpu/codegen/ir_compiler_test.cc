...
static absl::StatusOr<std::unique_ptr<toolchain::Module>> ParseModule(
    toolchain::LLVMContext& context, absl::string_view ir, absl::string_view name) {
  toolchain::SMDiagnostic diagnostic;
  toolchain::MemoryBufferRef ir_buffer(ir, name);

  auto m = toolchain::parseAssembly(ir_buffer, diagnostic, context);
  if (m == nullptr) {
...
TEST(IrCompilerTest, OverrideIrCompilerCompileOptions) {
  ...
  std::unique_ptr<IrCompiler> ir_compiler = IrCompiler::Create(
      toolchain::TargetOptions(),
      IrCompiler::Options{/*opt_level=*/toolchain::CodeGenOptLevel::Aggressive},
      compilation_hooks);
...
TEST(IrCompilerTest, TestAdditionalFeatures) {
  ...
  bool has_avx512;
  auto builder =
      [&has_avx512]() -> absl::StatusOr<std::unique_ptr<toolchain::TargetMachine>> {
    ...
    return absl::WrapUnique(target->createTargetMachine(
        toolchain::Triple(toolchain::Twine(triple)), cpu_name, features, target_options,
        /*RM=*/std::nullopt));
  };
