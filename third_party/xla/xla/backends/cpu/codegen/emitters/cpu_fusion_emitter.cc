...
IndexingMap GetDefaultIndexingMap(absl::Span<const int64_t> thread_tile_sizes,
  ...
  for (auto [tile_size, dim_size] : toolchain::zip(thread_tile_sizes, shape)) {
  ...
  for (auto [tile_size, thread_id, index_in_tile, dim] :
       toolchain::zip(thread_tile_sizes, thread_ids, indices_in_tile, shape)) {
...
absl::StatusOr<mlir::func::FuncOp> EmitEntryFunctionApi(
  ...
  for (const auto& [index, arg] : toolchain::enumerate(arguments)) {
  ...
  for (const auto& [index, result] : toolchain::enumerate(results)) {
