...
std::string LlvmModuleToString(const toolchain::Module& module) {
  std::string dump;
  toolchain::raw_string_ostream stream(dump);
  stream << module;
  return dump;
}
...
std::string MlirModuleToString(const mlir::ModuleOp& module) {
  ...
  toolchain::raw_string_ostream mlir_stream(mlir_dump);
...
TEST_F(CpuFusionEmitterTest, ScatterMlir) {
  constexpr absl::string_view kExpected = R"(
    ...
    CHECK:           %[[MACHINA_MACHINA_XLA_LOOP:.+]] = xla.loop
    CHECK:           xla.pure_call
    CHECK:           scf.if
    CHECK:             xla.pure_call
    CHECK:             xla.pure_call
    CHECK:             arith.addf
    CHECK:           return %[[MACHINA_MACHINA_XLA_LOOP]]
  )";
