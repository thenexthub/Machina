...
#include "machina/xla/backends/cpu/codegen/emitters/transforms/xla_cpu_rewrite_patterns.h"
...
#include "machina/xla/backends/cpu/codegen/emitters/ir/xla_cpu_types.h"
...
static mlir::LLVM::LLVMStructType KernelArgType(mlir::MLIRContext* ctx) {
  ...
  auto i64 = mlir::IntegerType::get(ctx, 64);
  return getNewOrExistingStruct(ctx, "MACHINA_MACHINA_XLA_CPU_KernelArg", {ptr, i64});
}

static mlir::LLVM::LLVMStructType KernelCallFrameType(mlir::MLIRContext* ctx) {
  auto ptr = mlir::LLVM::LLVMPointerType::get(ctx);
  auto i64 = mlir::IntegerType::get(ctx, 64);
  return getNewOrExistingStruct(ctx, "MACHINA_MACHINA_XLA_CPU_KernelCallFrame",
                                {ptr, ptr, i64, ptr});
}
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    auto args_gep = b.create<mlir::LLVM::GEPOp>(
        ...
        toolchain::SmallVector<mlir::LLVM::GEPArg, 2>{mlir::LLVM::GEPArg(0),
                                                 mlir::LLVM::GEPArg(3)},
    ...
    auto arg_gep = b.create<mlir::LLVM::GEPOp>(
        ...
        toolchain::SmallVector<mlir::LLVM::GEPArg, 2>{
...
struct RewriteFunctionSignatures : mlir::OpRewritePattern<mlir::func::FuncOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (func_type.getNumInputs() != 1 || func_type.getNumResults() != 1 ||
      ...
      return rewriter.notifyMatchFailure(
          op, "the function signature does not match the MACHINA_MACHINA_XLA_CPU_Kernel type.");
    }

    auto ptr = rewriter.getType<mlir::LLVM::LLVMPointerType>();
    toolchain::SmallVector<mlir::Type> new_operands{ptr};
...
class WrapEntryWithCallFrame
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    toolchain::SmallVector<mlir::Value> call_args;
    mlir::ArrayAttr arg_attrs = op.getArgAttrsAttr();
    for (const auto& [idx, arg] :
         toolchain::enumerate(op.getArguments().drop_back(3))) {
    ...
    op.setPrivate();
    op->setAttr("toolchain.linkage", mlir::LLVM::LinkageAttr::get(
 ...
 private:
  static void AddWorkGroupArguments(mlir::func::FuncOp op) {
    ...
    toolchain::SmallVector<mlir::Type> new_arg_types(op.getArgumentTypes());
    ...
    if (op.getArgAttrs().has_value()) {
      toolchain::SmallVector<mlir::Attribute> arg_attrs;
  ...
  static toolchain::SmallVector<mlir::Value, 3> GetWorkGroupIds(
    ...
    toolchain::SmallVector<mlir::Value, 3> workgroup_ids;
