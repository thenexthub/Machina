...
using ParallelPartitionBounds =
    std::vector<std::pair<toolchain::Value*, toolchain::Value*>>;
...
ElementalKernelEmitter::EmitKernelDefinition() {
  ...
  auto ctx = std::make_unique<toolchain::LLVMContext>();
  ...
  std::unique_ptr<toolchain::Module> llvm_module = KernelApiIrBuilder::CreateModule(
  ...
  toolchain::IRBuilder<> ir_builder(*ctx);
...
ElementalKernelEmitter::ThreadLocalCallbackFactory(toolchain::IRBuilderBase& builder,
  ...
  return [ir_emitter = std::move(ir_emitter), &builder](
             ...
             absl::Span<toolchain::Value* const> parameters, absl::string_view name,
             bool is_reducer) {
    IrEmitter::IRBuilderGuard builder_guard = ir_emitter->WithBuilder(builder);
