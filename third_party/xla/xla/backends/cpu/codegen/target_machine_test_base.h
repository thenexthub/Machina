...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_TEST_BASE_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_TEST_BASE_H_
 ...
 protected:
  ...
  std::unique_ptr<toolchain::TargetMachine> CreateTargetMachine(
    ...
    const toolchain::Target* target =
        toolchain::TargetRegistry::lookupTarget(triple_string, error);
    if (target == nullptr) {
      LOG(ERROR) << "Failed to lookup target: " << error;
    }

    toolchain::Twine triple_twine(triple_string);
    toolchain::Triple triple(triple_twine);
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_TARGET_MACHINE_TEST_BASE_H_
