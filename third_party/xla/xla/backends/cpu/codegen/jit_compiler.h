...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_JIT_COMPILER_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_JIT_COMPILER_H_
...
// has another pre-fabricated ORC JIT stack called `toolchain::orc::LLJIT`.
class JitCompiler {
 public:
  ...
  absl::Status AddModule(toolchain::orc::ThreadSafeModule module,
                         size_t dylib_index = 0);

  // Adds an object file to the dynamic library at `dylib_index`.
  absl::Status AddObjFile(std::unique_ptr<toolchain::MemoryBuffer> obj_file,
                          size_t dylib_index = 0);
  ...
  toolchain::TargetMachine* target_machine() { return target_machine_.get(); }

 private:
  // LLVM ORC task dispatcher that uses `TaskRunner` to run compilation tasks.
  class TaskDispatcher : public toolchain::orc::TaskDispatcher {
   public:
    explicit TaskDispatcher(TaskRunner task_runner);
    ~TaskDispatcher() final;

    void dispatch(std::unique_ptr<toolchain::orc::Task> T) final;
    void shutdown() final;
  ...
  JitCompiler(std::unique_ptr<toolchain::TargetMachine> target_machine,
  ...
  std::unique_ptr<toolchain::TargetMachine> target_machine_;

  TaskDispatcher* task_dispatcher_;  // owned by `execution_session_`

  std::unique_ptr<ExecutionEngine> execution_engine_;
  std::unique_ptr<toolchain::orc::IRCompileLayer> compile_layer_;
};

}  // namespace xla::cpu

#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_CODEGEN_JIT_COMPILER_H_
