...
#include "machina/xla/backends/cpu/nanort/ifrt_client.h"
...
// Base class for all value types. This class doesn't participate in the toolchain
...
class NanoValue : public toolchain::RTTIExtends<Self, Base> {
 public:
...
class NanoExecutable final
    : public toolchain::RTTIExtends<NanoExecutable, ifrt::LoadedExecutable> {
 public:
  ...
  static absl::StatusOr<std::unique_ptr<NanoExecutable>> Create(
    ...
    auto* xla_program = toolchain::dyn_cast<ifrt::HloProgram>(program.get());
 ...
 private:
  ...
  NanoArgumentsFromIfrtArguments(
    ...
    for (int i = 0; i < args.size(); ++i) {
      auto* nano_array = toolchain::dyn_cast_or_null<NanoArray>(args[i].get());
      if (ABSL_PREDICT_FALSE(nano_array == nullptr)) {
        // The input isn't a nano array, so it must be a sharded array.
        auto* sharded_array =
            toolchain::dyn_cast_or_null<ShardedNanoArray>(args[i].get());
...
class NanoCompiler final
    : public toolchain::RTTIExtends<NanoCompiler, ifrt::Compiler> {
 public:
...
// much.
class NanoMemory final : public toolchain::RTTIExtends<NanoMemory, ifrt::Memory> {
 public:
...
class NanoDevice final : public toolchain::RTTIExtends<NanoDevice, ifrt::Device> {
 public:
...
NanoIfrtClient::AssembleArrayFromSingleDeviceArrays(
  ...
  for (const auto& array : arrays) {
    auto* nano_array = toolchain::dyn_cast_or_null<NanoArray>(array.get());
...
absl::StatusOr<std::vector<ifrt::ArrayRef>> NanoIfrtClient::CopyArrays(
  ...
  for (const auto& array : arrays) {
    ...
    TF_ASSIGN_OR_RETURN(auto sharding, array->sharding().WithDeviceAssignment(
                                           devices, memory_kind));
    if (auto nano_array = toolchain::dyn_cast_or_null<NanoArray>(array.get())) {
      copy = tsl::TakeRef(new NanoArray(this, nano_array->dtype(),
                                        nano_array->shape(), nano_array->data(),
                                        std::move(sharding)));
    } else if (auto sharded_nano_array =
                   toolchain::dyn_cast_or_null<ShardedNanoArray>(array.get())) {
