...
LlvmTestKernelEmitter::EmitKernelDefinition() {
  ...
  std::unique_ptr<toolchain::Module> module = toolchain::parseAssembly(
      toolchain::MemoryBufferRef(llvm_ir_, kernel_name_), diagnostic, *context);
  ...
  for (const auto& [arg, allocation] : toolchain::zip(args_, buffer_allocations_)) {
