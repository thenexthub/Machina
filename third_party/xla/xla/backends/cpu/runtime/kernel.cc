...
#include "machina/xla/backends/cpu/runtime/work_queue.h"
...
static absl::InlinedVector<MACHINA_MACHINA_XLA_CPU_KernelArg, 8> ConvertBuffersToKernelArgs(
    absl::Span<const Kernel::DeviceMemoryBase> buffers) {
  absl::InlinedVector<MACHINA_MACHINA_XLA_CPU_KernelArg, 8> args(buffers.size());
...
class Kernel::ParallelTask {
 public:
  ParallelTask(MACHINA_MACHINA_XLA_CPU_Kernel* kernel, NumWorkGroups num_workgroups,
               absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> args);
 ...
 private:
  ...
  MACHINA_MACHINA_XLA_CPU_WorkGroupId Delinearize(uint64_t task_index) const;

  MACHINA_MACHINA_XLA_CPU_Kernel* kernel_;
  MACHINA_MACHINA_XLA_CPU_NumWorkGroups num_workgroups_;
  absl::InlinedVector<MACHINA_MACHINA_XLA_CPU_KernelArg, 8> args_;
...
Kernel::ParallelTask<num_workgroups_x_only>::ParallelTask(
    MACHINA_MACHINA_XLA_CPU_Kernel* kernel, NumWorkGroups num_workgroups,
    absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> args)
    : kernel_(kernel),
...
absl::Status Kernel::ParallelTask<num_workgroups_x_only>::operator()(
  ...
  MACHINA_MACHINA_XLA_CPU_WorkGroupId workgroup_id = Delinearize(task_index);
  MACHINA_MACHINA_XLA_CPU_KernelCallFrame call_frame = {&num_workgroups_, &workgroup_id,
                                        args_.size(), args_.data()};

  MACHINA_MACHINA_XLA_CPU_KernelError* error = (*kernel_)(&call_frame);
...
template <bool num_workgroups_x_only>
MACHINA_MACHINA_XLA_CPU_WorkGroupId Kernel::ParallelTask<num_workgroups_x_only>::Delinearize(
  ...
  if constexpr (num_workgroups_x_only) {
    return MACHINA_MACHINA_XLA_CPU_WorkGroupId{task_index, 0, 0};
  }
  ...
  return MACHINA_MACHINA_XLA_CPU_WorkGroupId{x, y, z};
}

Kernel::Kernel(unsigned arity, MACHINA_MACHINA_XLA_CPU_Kernel* kernel)
...
absl::Status Kernel::Launch(const NumWorkGroups& num_workgroups,
                            absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg> args) const {
  for (uint64_t z = 0; z < num_workgroups.z; ++z) {
    for (uint64_t y = 0; y < num_workgroups.y; ++y) {
      for (uint64_t x = 0; x < num_workgroups.x; ++x) {
        MACHINA_MACHINA_XLA_CPU_NumWorkGroups dim = {num_workgroups.x, num_workgroups.y,
                                     num_workgroups.z};

        MACHINA_MACHINA_XLA_CPU_WorkGroupId id = {x, y, z};

        MACHINA_MACHINA_XLA_CPU_KernelCallFrame call_frame = {&dim, &id, args.size(),
                                              args.data()};
