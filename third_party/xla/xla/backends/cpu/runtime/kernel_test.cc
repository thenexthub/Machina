...
static MACHINA_MACHINA_XLA_CPU_KernelError* AddI32(const MACHINA_MACHINA_XLA_CPU_KernelCallFrame* call_frame) {
  const MACHINA_MACHINA_XLA_CPU_KernelArg& lhs = call_frame->args[0];
  const MACHINA_MACHINA_XLA_CPU_KernelArg& rhs = call_frame->args[1];
  const MACHINA_MACHINA_XLA_CPU_KernelArg& out = call_frame->args[2];
...
TEST(KernelTest, LaunchAsync) {
  ...
  MACHINA_MACHINA_XLA_CPU_KernelArg arg = {&num_tasks, sizeof(num_tasks)};

  auto* no_op = +[](const MACHINA_MACHINA_XLA_CPU_KernelCallFrame* call_frame) {
    auto* n = reinterpret_cast<std::atomic<int32_t>*>(call_frame->args[0].data);
    n->fetch_add(1);
    return static_cast<MACHINA_MACHINA_XLA_CPU_KernelError*>(nullptr);
  };
...
TEST(KernelTest, LaunchAsyncError) {
  ...
  // MACHINA_MACHINA_XLA_CPU_KernelError type is not defined so we simply return a non-nullptr
  // pointer to signal error to the runtime.
  auto* maybe_error = +[](const MACHINA_MACHINA_XLA_CPU_KernelCallFrame* call_frame) {
    if (call_frame->workgroup_id->x == 2 && call_frame->workgroup_id->z == 2) {
      return reinterpret_cast<MACHINA_MACHINA_XLA_CPU_KernelError*>(0xDEADBEEF);
    }
    return static_cast<MACHINA_MACHINA_XLA_CPU_KernelError*>(nullptr);
  };
  ...
  auto event = host_kernel.Launch(
      NumWorkGroups{4, 4, 4}, absl::Span<const MACHINA_MACHINA_XLA_CPU_KernelArg>(), &device);
...
static MACHINA_MACHINA_XLA_CPU_KernelError* NoOp(const MACHINA_MACHINA_XLA_CPU_KernelCallFrame*) {
  return nullptr;
}
