...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
#define MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
...
// trivial layout that can be expressed as toolchain stuct (*).
//
// (*) https://toolchain.org/docs/LangRef.html#structure-types
...
typedef struct MACHINA_MACHINA_XLA_CPU_NumWorkGroups {
  uint64_t x;
  uint64_t y;
  uint64_t z;
} MACHINA_MACHINA_XLA_CPU_NumWorkGrousm;

// Workgroup id for an XLA:CPU kernel invocation.
typedef struct MACHINA_MACHINA_XLA_CPU_WorkGroupId {
  uint64_t x;
  uint64_t y;
  uint64_t z;
} MACHINA_MACHINA_XLA_CPU_WorkGroupId;

// A CPU kernel argument that corresponds to se::DeviceMemoryBase.
typedef struct MACHINA_MACHINA_XLA_CPU_KernelArg {
  void* data;
  size_t size;
} MACHINA_MACHINA_XLA_CPU_KernelArg;

// A CPU kernel call frame.
typedef struct MACHINA_MACHINA_XLA_CPU_KernelCallFrame {
  const MACHINA_MACHINA_XLA_CPU_NumWorkGroups* num_workgroups;
  const MACHINA_MACHINA_XLA_CPU_WorkGroupId* workgroup_id;

  size_t num_args;
  const MACHINA_MACHINA_XLA_CPU_KernelArg* args;
} MACHINA_MACHINA_XLA_CPU_KernelCallFrame;

// Error reporting for host kernels. NULL means success.
typedef struct MACHINA_MACHINA_XLA_CPU_KernelError MACHINA_MACHINA_XLA_CPU_KernelError;

// Host kernel API.
typedef MACHINA_MACHINA_XLA_CPU_KernelError* MACHINA_MACHINA_XLA_CPU_Kernel(
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_CPU_RUNTIME_KERNEL_C_API_H_
