...
#include "machina/xla/backends/gpu/runtime/annotation.h"
...
#include "machina/xla/backends/gpu/runtime/dynamic_slice_thunk.h"
...
#include "machina/xla/runtime/execution_graph.h"
...
#include "machina/xla/service/gpu/buffer_allocations.h"
...
#include "machina/xla/stream_executor/command_buffer.h"
#include "machina/xla/stream_executor/device_memory.h"
#include "machina/xla/stream_executor/dnn.h"
#include "machina/xla/stream_executor/kernel.h"
...
#include "machina/xla/stream_executor/trace_command_buffer_factory.h"
...
DynamicSliceFusionCmd::DynamicSliceFusionCmd(
  ...
  for (auto [arg, offset, orig_shape, sliced_shape, offset_byte_size] :
       toolchain::zip_equal(arguments, offsets, orig_shapes, sliced_shapes,
                       offset_byte_sizes)) {
  ...
  for (auto [argument_idx, slice] : toolchain::enumerate(slices_)) {
...
absl::StatusOr<const se::CommandBuffer::Command*> DynamicSliceFusionCmd::Record(
  ...
  for (auto [argument_idx, slice] : toolchain::enumerate(slices_)) {
    ...
    for (auto [offset_idx, values] : toolchain::enumerate(toolchain::zip(
    ...
    for (auto [offset_idx, values] : toolchain::enumerate(
             toolchain::zip(src_shape.dimensions(), dst_shape.dimensions()))) {
    ...
    for (auto [start, stride] :
         toolchain::zip(slice_starts, *ShapeUtil::ByteStrides(src_shape))) {
