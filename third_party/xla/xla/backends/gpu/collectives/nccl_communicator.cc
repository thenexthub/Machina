...
#include "machina/xla/backends/gpu/collectives/nccl_communicator.h"
...
#include "machina/xla/backends/gpu/collectives/nccl_errors.h"
#include "machina/xla/backends/gpu/collectives/single_threaded_executor.h"
...
#include "machina/xla/stream_executor/gpu/gpu_stream.h"
 ...
 public:
  ...
  absl::Status Unregister() final {
    ...
    auto f = [this]() -> absl::Status {
      ...
      MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclCommDeregister(comm_.comm(), handle_));
...
NcclCommunicator::~NcclCommunicator() {
  auto f = [this]() -> absl::Status {
    ...
    VLOG(1) << "Destroy " << *this;
    return MACHINA_MACHINA_XLA_NCCL_STATUS(ncclCommDestroy(comm_));
  };
...
absl::Status NcclCommunicator::Abort() {
  ...
  return BlockAndGet(Execute([this]() -> absl::Status {
    ...
    return MACHINA_MACHINA_XLA_NCCL_STATUS(ncclCommAbort(comm_));
  }));
}
...
absl::Status NcclCommunicator::HealthCheck() const {
  return BlockAndGet(Execute([this]() -> absl::Status {
    ...
    MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclCommGetAsyncError(comm_, &async_err));
...
absl::StatusOr<size_t> NcclCommunicator::NumRanks() const {
  return BlockAndGet(Execute<size_t>([this]() -> absl::StatusOr<size_t> {
    ...
    int32_t count = 0;
    MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclCommCount(comm_, &count));
    return count;
  }));
}
...
NcclCommunicator::RegisterBuffer(stream_executor::DeviceMemoryBase buffer) {
  ...
  return BlockAndGet(
      Execute<Handle>([&buffer, this]() -> absl::StatusOr<Handle> {
        ...
        void* handle = nullptr;
        MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(
...
absl::Status NcclCommunicator::GroupStart() {
  ...
  MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclGroupStart());
...
absl::Status NcclCommunicator::GroupEnd() {
  ...
  MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclGroupEnd());
  group_nesting_level_--;
  if (group_nesting_level_ > 0) {
...
absl::Status NcclCommunicator::LaunchAllReduce(
  ...
  TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclAllReduce(
...
absl::Status NcclCommunicator::LaunchBroadcast(se::DeviceMemoryBase send_buffer,
  ...
  TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclBroadcast(
      send_buffer.opaque(), recv_buffer.opaque(), ToNcclCount(dtype, count),
...
absl::Status NcclCommunicator::LaunchReduceScatter(
  ...
  TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclReduceScatter(
      send_buffer.opaque(), recv_buffer.opaque(), ToNcclCount(dtype, count),
      nccl_dtype, ToNcclReduction(reduction_kind), comm_,
      se::gpu::AsGpuStreamValue(stream))));
  if (group_nesting_level_ == 0) {
...
absl::Status NcclCommunicator::LaunchAllGather(se::DeviceMemoryBase send_buffer,
  ...
  TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclAllGather(
      send_buffer.opaque(), recv_buffer.opaque(), ToNcclCount(dtype, count),
...
absl::Status NcclCommunicator::LaunchAllToAll(
  ...
  int32_t num_ranks;
  MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclCommCount(comm_, &num_ranks));
...
absl::Status NcclCommunicator::LaunchCollectivePermute(
  ...
  if (source_rank) {
    MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclRecv(
  ...
  for (auto target_rank : target_ranks) {
    MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclSend(
...
absl::Status NcclCommunicator::LaunchSend(se::DeviceMemoryBase send_buffer,
  ...
  TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(
