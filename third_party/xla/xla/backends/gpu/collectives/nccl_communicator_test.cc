...
absl::StatusOr<std::unique_ptr<NcclCommunicator>> CreateCommunicator(
  ...
  auto f = [blocking]() -> absl::StatusOr<ncclComm_t> {
    ...
    ncclUniqueId id;
    TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclGetUniqueId(&id)));
    ...
    if (r == ncclUnhandledCudaError) {
      ...
      // and skip the test.
      LOG(ERROR) << MACHINA_MACHINA_XLA_NCCL_STATUS(r);
      return absl::FailedPreconditionError(kCudaError);
    }
    if (r != ncclSuccess && r != ncclInProgress) {
      return MACHINA_MACHINA_XLA_NCCL_STATUS(r);
    }

    // Wait for the communicator to finish initializing.
    ncclResult_t state = ncclInProgress;
    while (state == ncclInProgress) {
      TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(ncclCommGetAsyncError(comm, &state)));
    }
    TF_RETURN_IF_ERROR(MACHINA_MACHINA_XLA_NCCL_STATUS(state));
    return comm;
  };
