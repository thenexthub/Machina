...
#ifndef MACHINA_MACHINA_XLA_BACKENDS_GPU_COLLECTIVES_NCCL_ERRORS_H_
#define MACHINA_MACHINA_XLA_BACKENDS_GPU_COLLECTIVES_NCCL_ERRORS_H_
...
#include "machina/xla/util.h"  // IWYU pragma: keep
                                                       //
#if MACHINA_USE_ROCM
...
#define MACHINA_MACHINA_XLA_NCCL_STATUS(expr)                                         \
...
#define MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(expr)      \
  do {                                      \
    absl::Status s = MACHINA_MACHINA_XLA_NCCL_STATUS(expr); \
    if (!s.ok()) {                          \
      return s;                             \
    }                                       \
  } while (0)

#define MACHINA_MACHINA_XLA_NCCL_LOG_IF_ERROR(expr)         \
  do {                                      \
    absl::Status s = MACHINA_MACHINA_XLA_NCCL_STATUS(expr); \
    if (!s.ok()) {                          \
...
#define MACHINA_MACHINA_XLA_NCCL_CHECK(expr) CHECK(MACHINA_MACHINA_XLA_NCCL_STATUS(expr).ok())
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_GPU_COLLECTIVES_NCCL_ERRORS_H_
