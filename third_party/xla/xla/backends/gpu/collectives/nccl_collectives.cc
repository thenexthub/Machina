...
#include "machina/xla/backends/gpu/collectives/nccl_collectives.h"
...
absl::StatusOr<CliqueId> NcclCollectives::CreateUniqueCliqueId() const {
  ...
  ncclUniqueId id;
  MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclGetUniqueId(&id));
...
#if !defined(MACHINA_USE_ROCM) || TF_ROCM_VERSION > 50700
...
NcclCollectives::SplitCommunicators(absl::Span<const Communicator* const> comms,
...
#if !defined(MACHINA_USE_ROCM) || TF_ROCM_VERSION >= 60000
  auto make_comm = [&](int i) -> absl::StatusOr<ncclComm_t> {
    VLOG(1) << "Split NCCL communicator " << comms[i] << " with color " << color
            << " and key " << keys[i];
    ncclComm_t split_comm;
    MACHINA_MACHINA_XLA_NCCL_RETURN_IF_ERROR(ncclCommSplit(
  ...
  return absl::UnimplementedError(
      absl::StrFormat("%s:%d: NCCL operation ncclCommSplit not implemented",
                      __FILE__, __LINE__));
#endif  // !defined(MACHINA_USE_ROCM) || TF_ROCM_VERSION >= 60000
}
...
MACHINA_MACHINA_XLA_COLLECTIVES_REGISTER("gpu", "nccl", 1,
