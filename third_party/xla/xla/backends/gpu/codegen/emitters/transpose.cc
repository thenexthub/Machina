...
==============================================================================*/
#include "machina/xla/backends/gpu/codegen/emitters/transpose.h"
...
#include "machina/xla/codegen/emitters/type_util.h"
#include "machina/xla/codegen/emitters/utils.h"
...
TransposeFusion::TransposeFusion(const HloFusionAnalysis& analysis)
  ...
  for (auto [root, hero] :
       toolchain::zip(analysis_.fusion_roots(), analysis_.fusion_heroes())) {
    if (auto transpose =
...
TransposeFusion::WriteResult TransposeFusion::EmitWriteToShMemMlir(
  ...
  auto body_builder = [&](ImplicitLocOpBuilder& nested_b,
    ...
    for (auto [transpose, output] :
         toolchain::zip(shmem_transposes_, output_tensors)) {
    ...
    for (const auto& [value, indices, output] :
         toolchain::zip(side_outputs, side_output_indices,
  ...
  for (auto [index, side_output_result] :
       toolchain::zip(side_output_root_indices_,
...
void TransposeFusion::EmitReadFromShMemMlir(
  ...
  auto result_tensors = emitters::EmitXlaLoopOp(
      ...
      [&](ImplicitLocOpBuilder& nested_b, ValueRange symbol_values,
        ...
        absl::flat_hash_map<const HloInstruction*, toolchain::SmallVector<Value>>
            transpose_values;
        for (auto [transpose, shmem] :
             toolchain::zip(shmem_transposes_, written.shmem_tensors)) {
          transpose_values[transpose].push_back(
              nested_b.create<mt::ExtractOp>(shmem, shmem_indices));
        }
        toolchain::SmallVector<Value> epilogue_indices = thread_and_block_ids;
        ...
        for (auto [root, indexing, root_index] :
             toolchain::zip(shmem_transpose_roots_,
                       computations.epilogues().front().root_indexing,
                       shmem_transpose_root_indices_)) {
          toolchain::SmallVector<Value> indices = ApplyIndexing(
...
toolchain::SmallVector<mlir::AffineExpr, 4> TransposeFusion::GetThreadOffsets(
    bool read, mlir::MLIRContext* ctx) const {
...
IndexingMap TransposeFusion::GetIndexing(bool input, const xla::Shape& shape,
  ...
  toolchain::SmallVector<AffineExpr, 3> offsets;
  for (auto [block_id, block_size, thread] :
       toolchain::zip(block_ids, permuted_block_sizes, thread_offsets)) {
  ...
  for (auto [size, dim] : toolchain::zip(normalized_shape.dimensions(),
...
std::vector<int64_t> GetBlockCounts(absl::Span<const int64_t> shape,
  ...
  for (auto [dim, tile_size] : toolchain::zip(shape, tile)) {
...
PackedTranspose::WriteResult PackedTranspose::EmitWriteToShMemMlir(
  ...
  auto body_builder = [&](ImplicitLocOpBuilder& nested_b,
    ...
    SmallVector<Value> result_tensors;
    for (const auto& [value, indices, output] :
         toolchain::zip(side_outputs, side_output_indices, output_tensors)) {
      result_tensors.push_back(
          nested_b.create<mt::InsertOp>(value, output, indices));
    }
  ...
  result.updated_outputs = output_args;
  for (auto [index, side_output_result] :
       toolchain::zip(side_output_root_indices_, side_output_vector)) {
    result.updated_outputs[index] = side_output_result;
  }
  return result;
}
...
IndexingMap PackedTranspose::GetInputIndexing(MLIRContext* ctx) const {
  ...
  toolchain::SmallVector<AffineExpr, 4> offsets_within_tile(canonical_rank, c0);
  ...
  // Canonical indexing.
  toolchain::SmallVector<AffineExpr, 4> canonical_offsets;
  canonical_offsets.reserve(canonical_rank + 2);
  for (auto [thread_offset, block_index, tile_size] :
       toolchain::zip(offsets_within_tile, block_ids, input_tile_)) {
    canonical_offsets.push_back(block_index * tile_size + thread_offset);
  }
  toolchain::SmallVector<std::pair<AffineExpr, Interval>> constraints{
...
IndexingMap PackedTranspose::GetOutputIndexing(mlir::MLIRContext* ctx) const {
  ...
  canonical_offsets.reserve(canonical_rank + 2);
  for (auto [thread_offset, block_index, tile_size] :
       toolchain::zip(offsets_within_tile, block_ids, output_tile_)) {
    canonical_offsets.push_back(block_index * tile_size + thread_offset);
  }
  toolchain::SmallVector<std::pair<AffineExpr, Interval>> constraints{
      {shmem_col, Interval{0, populated_shmem_cols_ - 1}},
