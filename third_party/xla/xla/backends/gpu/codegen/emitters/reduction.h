...
==============================================================================*/
#ifndef MACHINA_MACHINA_XLA_BACKENDS_GPU_CODEGEN_EMITTERS_REDUCTION_H_
#define MACHINA_MACHINA_XLA_BACKENDS_GPU_CODEGEN_EMITTERS_REDUCTION_H_
...
using HloValueMap =
    absl::flat_hash_map<const HloInstruction*, toolchain::SmallVector<mlir::Value>>;
...
class ReductionFusion : public EmitterBase {
 ...
 protected:
  ...
  toolchain::SmallVector<mlir::Value> EvaluateEpilogue(
      const HloValueMap& results, toolchain::SmallVector<mlir::Value> outputs,
      EmitterState& state, int group_id, mlir::ValueRange symbol_values) const;

  virtual toolchain::SmallVector<mlir::Value> EmitReduction(
      int group_id, EmitterState& state) const = 0;

  // Returns a reduction indexing map with the given results.
  IndexingMap GetIndexingMap(toolchain::ArrayRef<mlir::AffineExpr> results,
...
class RowReductionFusion : public ReductionFusion {
 ...
 protected:
  ...
  toolchain::SmallVector<mlir::Value> EmitReduction(
...
#endif  // MACHINA_MACHINA_XLA_BACKENDS_GPU_CODEGEN_EMITTERS_REDUCTION_H_
