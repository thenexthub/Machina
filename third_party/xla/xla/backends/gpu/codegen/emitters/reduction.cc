...
==============================================================================*/
#include "machina/xla/backends/gpu/codegen/emitters/reduction.h"
...
#include "machina/xla/backends/gpu/codegen/emitters/reduction_base.h"
...
struct ReductionFusion::EmitterState {
  EmitterState(const ReductionFusion& owner, mlir::func::FuncOp entry_function,
    ...
    for (const auto& [root_index, root] :
         toolchain::enumerate(owner.analysis_.fusion_roots())) {
...
PerThreadOutputs ReductionFusion::EmitterState::EmitPerThreadElements(
  ...
  auto body_builder = [&](ImplicitLocOpBuilder& nested_b,
    ...
    toolchain::SmallVector<Value> results = iter_args;
    ...
    toolchain::SmallVector<SideOutput> side_output_values;
    ...
    for (const auto& [side_output, values] :
         toolchain::zip(side_outputs, side_output_values)) {
...
IndexingMap ReductionFusion::GetIndexingMap(
    toolchain::ArrayRef<mlir::AffineExpr> results,
...
std::vector<emitters::EpilogueSpecification> ReductionFusion::GetEpilogues(
  ...
  for (const auto& [heroes, roots] :
       toolchain::zip(reduction_heroes_, reduction_roots_)) {
    epilogues.push_back(
...
absl::Status ReductionFusion::EmitEntryFunction(
  ...
  for (auto [id, region] : toolchain::enumerate(switch_op->getRegions())) {
...
std::optional<IndexingMap> ReductionFusion::ComputeThreadIdToOutputIndexing(
  ...
  for (auto [result, dim_size] : toolchain::zip(
...
SmallVector<Value> ReductionFusion::EvaluateEpilogue(
    const HloValueMap& results, toolchain::SmallVector<Value> outputs,
  ...
  for (auto [index, root] : toolchain::enumerate(epilogue.roots)) {
    auto output_indices =
        emitters::ApplyIndexing(epilogue.root_indexing[index],
                                state.thread_and_block_ids, symbol_values, b);
    for (auto [result_index, result] : toolchain::enumerate(values.at(root))) {
...
IndexingMap ColumnReductionFusion::ComputeReductionInputIndexing(
  ...
  for (auto [result, dim_size] :
       toolchain::zip(results, reduction_dimensions_.dimensions)) {
...
toolchain::SmallVector<mlir::Value> ColumnReductionFusion::EmitReduction(
...
IndexingMap SmallColumnReductionFusion::ComputeReductionInputIndexing(
  ...
  for (auto [result, dim_size] :
       toolchain::zip(map.GetAffineMap().getResults(), input_shape_)) {
    map.AddConstraint(result, {0, dim_size - 1});
  }
  return map;
}
...
toolchain::SmallVector<mlir::Value> SmallColumnReductionFusion::EmitReduction(
    int group_id, EmitterState& state) const {
  HloValueMap inits = GetInits(group_id, state);
  auto per_thread =
      state.EmitPerThreadElements(group_id, inits, state.FusionOutputs());
...
IndexingMap RowReductionFusion::ComputeReductionInputIndexing(
  ...
  for (auto [result, input_dim] : toolchain::zip(indices, input_shape_)) {
...
toolchain::SmallVector<mlir::Value> RowReductionFusion::EmitReduction(
    int group_id, EmitterState& state) const {
  const auto& reductions = reduction_heroes_[group_id];
...
toolchain::SmallVector<mlir::Value> MultiRowReductionFusion::EmitReduction(
    int group_id, EmitterState& state) const {
  HloValueMap inits = GetInits(group_id, state);
  const auto& reductions = reduction_heroes_[group_id];
  auto per_thread =
