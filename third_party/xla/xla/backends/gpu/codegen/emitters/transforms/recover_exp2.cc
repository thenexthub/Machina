...
/// ^^^ must be above others, as toolchain don't strictly follow IWYU
...
class RecoverExp2Pattern : public mlir::OpRewritePattern<mlir::math::ExpOp> {
  ...
  template <typename TupleT, std::size_t Idx = 0>
      std::enable_if_t < Idx<std::tuple_size_v<TupleT>> prefillLogs2From(
                             const toolchain::APFloat& main_log2) {
    using FloatT = std::tuple_element_t<Idx, TupleT>;

    auto f_obj = FloatT{};
    const auto& semantics = f_obj.getFloatSemantics();
    bool lossy{};
    toolchain::APFloat log2_typed{main_log2};
    log2_typed.convert(semantics, toolchain::APFloat::rmNearestTiesToEven, &lossy);
  ...
  void initStaticsIfNeeded() {
    ...
    if (logs2_[0] == 0.) {
      auto log2 = toolchain::APFloat(WidestNarrowType{}.getFloatSemantics());
 ...
 public:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    // in mlir/toolchain now, doing a runtime check for actual operand type to only
    ...
    if (std::any_of(logs2_.begin(), logs2_.end(),
      ...
      // no need to do anything with deletion of other ops, toolchain will delete
