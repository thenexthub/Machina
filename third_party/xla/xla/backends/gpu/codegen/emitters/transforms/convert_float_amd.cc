...
#include "machina/xla/backends/gpu/codegen/emitters/transforms/passes.h.inc"
...
struct Fp8OpRewritePattern : public mlir::OpRewritePattern<SourceOp> {
  ...
  bool isFp8(const mlir::Type& type) const {
    return nativeNanooFp8_ ? toolchain::isa<mlir::Float8E4M3FNUZType>(type)
                           : toolchain::isa<mlir::Float8E4M3FNType>(type);
  }
  bool isBf8(const mlir::Type& type) const {
    return nativeNanooFp8_ ? toolchain::isa<mlir::Float8E5M2FNUZType>(type)
                           : toolchain::isa<mlir::Float8E5M2Type>(type);
  }
...
struct RewriteFp8TruncFPattern : public Fp8OpRewritePattern<arith::TruncFOp> {
  ...
  std::optional<std::tuple<toolchain::SmallVector<mlir::Value, 4>, FixedVectorValue>>
  MatchBuildVector(arith::TruncFOp op, FloatValue value,
    ...
    if (!toolchain::isPowerOf2_64(element_count) || element_count == 1) {
      return std::nullopt;
    }

    toolchain::SmallVector<mlir::Value, 4> inputs(element_count);
    ...
    if (toolchain::any_of(inputs, [](mlir::Value input) { return !input; })) {
      return std::nullopt;
    }
  ...
  mlir::Value EmitVectorizedTruncToF8Intrinsic(
      toolchain::SmallVector<mlir::Value, 4>& inputs, mlir::FixedVectorType to_ty,
    ...
    toolchain::transform(inputs, inputs.begin(), [&](mlir::Value v) -> mlir::Value {
    ...
    mlir::StringAttr cvtIntr = b.getStringAttr(
        isFp8(to_ty.getElementType()) ? "toolchain.amdgcn.cvt.pk.fp8.f32"
                                      : "toolchain.amdgcn.cvt.pk.bf8.f32");
  ...
  mlir::Value EmitTruncToF8Intrinsic(mlir::Value value, mlir::FloatType to_ty,
    ...
    mlir::StringAttr cvtIntr =
        b.getStringAttr(isFp8(to_ty) ? "toolchain.amdgcn.cvt.pk.fp8.f32"
                                     : "toolchain.amdgcn.cvt.pk.bf8.f32");
...
struct RewriteFp8ExtFPattern : public Fp8OpRewritePattern<arith::ExtFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (match) {
      ...
      for (auto [old_value, new_value] :
           toolchain::zip_equal(outputs, new_outputs)) {
  ...
  std::optional<std::tuple<FixedVectorValue, toolchain::SmallVector<mlir::Value, 4>>>
  MatchDecomposeVector(arith::ExtFOp op, FloatValue value,
    ...
    toolchain::SmallVector<mlir::Value, 4> outputs(element_count);
    ...
    if (toolchain::any_of(outputs, [](mlir::Value output) { return !output; })) {
      return std::nullopt;
    }
  ...
  toolchain::SmallVector<mlir::Value, 4> EmitVectorizedExtFromF8Intrinsic(
    ...
    mlir::StringAttr cvtIntr = b.getStringAttr(
        isFp8(value.getType().getElementType()) ? "toolchain.amdgcn.cvt.pk.f32.fp8"
                                                : "toolchain.amdgcn.cvt.pk.f32.bf8");
    ...
    if (to_ty.isF16()) {
      ...
      cvtIntr = b.getStringAttr("toolchain.amdgcn.cvt.pkrtz");
      for (size_t i = 0; i < num_elements / 2; i++) {
        LLVM::CallIntrinsicOp cvtOp = b.create<LLVM::CallIntrinsicOp>(
            result_ty, cvtIntr,
      ...
      toolchain::transform(results, results.begin(),
  ...
  mlir::Value EmitExtFromF8Intrinsic(mlir::Value value, mlir::FloatType to_ty,
    ...
    mlir::Value input = b.create<LLVM::BitcastOp>(
        i32_ty, b.create<LLVM::InsertElementOp>(
                    ...
                    zero_cst));
    mlir::StringAttr cvtIntr =
        b.getStringAttr(isFp8(value.getType()) ? "toolchain.amdgcn.cvt.f32.fp8"
                                               : "toolchain.amdgcn.cvt.f32.bf8");
    LLVM::FastmathFlagsAttr flags =
        LLVM::FastmathFlagsAttr::get(b.getContext(), LLVM::FastmathFlags::ninf);
    LLVM::CallIntrinsicOp cvtOp = b.create<LLVM::CallIntrinsicOp>(
