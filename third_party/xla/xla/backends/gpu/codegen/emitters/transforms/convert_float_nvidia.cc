...
struct RewriteTruncFPattern : public mlir::OpRewritePattern<ma::TruncFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (!toolchain::isa<mlir::Float8E4M3FNType>(dst_ty) &&
        !toolchain::isa<mlir::Float8E5M2Type>(dst_ty)) {
  ...
  Value EmitTruncToF8Intrinsic(Value value, mlir::FloatType to_ty,
    ...
    assert((toolchain::isa<mlir::Float8E4M3FNType, mlir::Float8E5M2Type>(to_ty)));
    ...
    if (value.getType() == b.getF16Type()) {
      ...
      auto cvtIntr = toolchain::isa<mlir::Float8E4M3FNType>(to_ty)
                         ? "toolchain.nvvm.f16x2.to.e4m3x2.rn"
                         : "toolchain.nvvm.f16x2.to.e5m2x2.rn";
      ...
      auto cvtIntr = toolchain::isa<mlir::Float8E4M3FNType>(to_ty)
                         ? "toolchain.nvvm.ff.to.e4m3x2.rn"
                         : "toolchain.nvvm.ff.to.e5m2x2.rn";
      cvtOp = b.create<ml::CallIntrinsicOp>(b.getIntegerType(16),
                                            b.getStringAttr(cvtIntr),
  ...
  static Value FixInfinityConversionValue(Value src, mlir::FloatType src_type,
    ...
    int64_t upper = toolchain::APFloat::getInf(src_type.getFloatSemantics())
                        .bitcastToAPInt()
                        .getZExtValue();
    ...
    int64_t inf_val = toolchain::APFloat::getInf(dst_type.getFloatSemantics())
                          .bitcastToAPInt()
                          .getZExtValue();
    Value sign_dst =
  ...
  static int64_t GetOverflowInputValue(mlir::FloatType src_type,
    ...
    int64_t largest = toolchain::APFloat::getLargest(dst_type.getFloatSemantics())
                          .bitcastToAPInt()
                          .getZExtValue();
...
struct RewriteExtFPattern : public mlir::OpRewritePattern<ma::ExtFOp> {
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    auto src = mlir::cast<FloatValue>(op.getOperand());
    auto dst_ty = mlir::cast<mlir::FloatType>(op.getType());
    if (!toolchain::isa<mlir::Float8E4M3FNType>(src.getType()) &&
        !toolchain::isa<mlir::Float8E5M2Type>(src.getType())) {
      return rewriter.notifyMatchFailure(op, "unsupported float conversion");
    }
  ...
  Value EmitExtFromF8Intrinsic(Value value, mlir::FloatType to_ty,
                               mlir::ImplicitLocOpBuilder& b) const {
    assert((toolchain::isa<mlir::Float8E4M3FNType, mlir::Float8E5M2Type>(
        value.getType())));
    ...
    auto cvtIntr = toolchain::isa<mlir::Float8E4M3FNType>(value.getType())
                       ? "toolchain.nvvm.e4m3x2.to.f16x2.rn"
                       : "toolchain.nvvm.e5m2x2.to.f16x2.rn";
