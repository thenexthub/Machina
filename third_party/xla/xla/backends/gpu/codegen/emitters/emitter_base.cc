...
==============================================================================*/
#include "machina/xla/backends/gpu/codegen/emitters/emitter_base.h"
...
#include "machina/xla/codegen/emitters/ir/xla_dialect.h"
...
#include "machina/xla/codegen/emitters/transforms/pass_pipelines.h"
...
#include "machina/xla/tsl/framework/mlir/status_scoped_diagnostic_handler.h"
...
void AddRanges(toolchain::Function* func, const LaunchDimensions& launch_dims,
               toolchain::Module* module) {
  for (auto& block : *func) {
    for (auto& instr : block) {
      if (auto* call = toolchain::dyn_cast<toolchain::CallInst>(&instr)) {
        if (auto* callee = call->getCalledFunction()) {
          switch (callee->getIntrinsicID()) {
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_tid_x:
            case toolchain::Intrinsic::amdgcn_workitem_id_x:
              llvm_ir::AddRangeMetadata(
                  0, launch_dims.thread_counts_per_block().x, call, module);
              break;
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_tid_y:
            case toolchain::Intrinsic::amdgcn_workitem_id_y:
              llvm_ir::AddRangeMetadata(
                  0, launch_dims.thread_counts_per_block().y, call, module);
              break;
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_tid_z:
            case toolchain::Intrinsic::amdgcn_workitem_id_z:
              llvm_ir::AddRangeMetadata(
                  0, launch_dims.thread_counts_per_block().z, call, module);
              break;
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_ctaid_x:
            case toolchain::Intrinsic::amdgcn_workgroup_id_x:
              llvm_ir::AddRangeMetadata(0, launch_dims.block_counts().x, call,
                                        module);
              break;
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_ctaid_y:
            case toolchain::Intrinsic::amdgcn_workgroup_id_y:
              llvm_ir::AddRangeMetadata(0, launch_dims.block_counts().y, call,
                                        module);
              break;
            case toolchain::Intrinsic::nvvm_read_ptx_sreg_ctaid_z:
            case toolchain::Intrinsic::amdgcn_workgroup_id_z:
...
absl::Status RunPassPipeline(mlir::ModuleOp module, const HloModule& hlo_module,
  ...
  toolchain::raw_string_ostream log_stream(mlir_passes_dump_result);
...
toolchain::SmallVector<Value> EmitterBase::EmitThreadAndBlockIds(
      ...
      ir_emitter_context.kernel_cache().GetWithStatus(
          ...
          [&]() -> absl::StatusOr<KernelReuseCache::Entry> {
            ...
            if (ir_emitter_context.emit_kernels()) {
              ...
              toolchain::IRBuilder<> builder(module->getContext());
              ...
              // both.
              CHECK(!toolchain::Linker::linkModules(
...
absl::StatusOr<std::unique_ptr<toolchain::Module>> EmitterBase::CreateLLVMModule(
    mlir::MLIRContext& mlir_context, toolchain::LLVMContext& llvm_context,
...
emitters::EpilogueSpecification EmitterBase::GetEpilogueForOutputIndexing(
  ...
  absl::flat_hash_map<const HloInstruction*, const HloInstruction*>
      root_to_hero;
  for (auto [root, hero] :
       toolchain::zip(analysis.fusion_roots(), analysis.fusion_heroes())) {
    root_to_hero[&root.instruction()] = &hero.instruction();
  }
  absl::flat_hash_map<const HloInstruction*, int> root_to_index;
  for (auto [index, root] : toolchain::enumerate(analysis.fusion_roots())) {
...
EmitterBase::EmitEpilogue(
    ...
    FuncOp entry_fn,
    const absl::flat_hash_map<const HloInstruction*, toolchain::SmallVector<Value>>&
        injected,
