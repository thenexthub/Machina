...
TEST(CreateTmaDescriptorTest, Valid2DInputReturnCorrectDescriptor) {
  ...
  toolchain::SmallVector<int64_t, 2> global_shape = {256, 128};
  toolchain::SmallVector<int64_t, 2> tile_shape = {64, 32};
  toolchain::SmallVector<int64_t, 2> tile_strides = {1, 1};
  toolchain::SmallVector<int64_t, 2> layout = {1, 0};
...
TEST(CreateTmaDescriptorTest, Valid1DInputReturnCorrectDescriptor) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 2> global_shape = {128};
  toolchain::SmallVector<int64_t, 2> tile_shape = {32};
  toolchain::SmallVector<int64_t, 2> tile_strides = {1};
  toolchain::SmallVector<int64_t, 2> layout = {0};
  int element_byte_size = 4;
...
TEST(CreateTmaDescriptorTest, Valid5DInputReturnCorrectDescriptor) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 5> global_shape = {128, 64, 32, 16, 8};
  toolchain::SmallVector<int64_t, 5> tile_shape = {64, 32, 16, 8, 8};
  toolchain::SmallVector<int64_t, 5> tile_strides = {1, 1, 1, 1, 1};
  toolchain::SmallVector<int64_t, 5> layout = {4, 3, 2, 1, 0};
...
TEST(CreateTmaDescriptorTest, ShapeMismatchFailsGracefully) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 1> global_shape = {128};
  toolchain::SmallVector<int64_t, 2> tile_shape = {128, 128};
  toolchain::SmallVector<int64_t, 2> tile_strides = {1, 1};
  toolchain::SmallVector<int64_t, 2> layout = {1, 0};
  int element_byte_size = 4;
  EXPECT_THAT(
...
TEST(CreateTmaDescriptorTest, EmptyShapeFailsGracefully) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 2> global_shape = {};
  toolchain::SmallVector<int64_t, 2> tile_shape = {};
  toolchain::SmallVector<int64_t, 2> tile_strides = {1, 1};
  toolchain::SmallVector<int64_t, 2> layout = {1, 0};
  int element_byte_size = 4;
  EXPECT_THAT(
...
TEST(CreateTmaDescriptorTest, UnsupportedShapeFailsGracefully) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 6> global_shape = {16, 16, 16, 16, 16, 16};
  toolchain::SmallVector<int64_t, 6> tile_shape = {4, 4, 4, 4, 4, 4};
  toolchain::SmallVector<int64_t, 6> tile_strides = {1, 1, 1, 1, 1, 1};
  toolchain::SmallVector<int64_t, 6> layout = {5, 4, 3, 2, 1, 0};
  int element_byte_size = 4;
  EXPECT_THAT(
      CreateTmaDescriptor(global_shape, tile_shape, tile_strides, layout,
                          element_byte_size, SwizzleMode::k128b),
...
TEST(CreateTmaDescriptorTest, NonUnitTileStridesAreCorrectlyHandled) {
  mlir::MLIRContext mlir_context;
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 2> global_shape = {1024, 1024};
  toolchain::SmallVector<int64_t, 2> tile_shape = {32, 32};
  toolchain::SmallVector<int64_t, 2> tile_strides = {3, 1};
  toolchain::SmallVector<int64_t, 2> layout = {1, 0};
  int element_byte_size = 4;
  SwizzleMode swizzle_mode = SwizzleMode::k128b;
  TF_ASSERT_OK_AND_ASSIGN(
...
TEST(CreateTmaDescriptorTest, BoxDimsAreAdjustedForSwizzleMode) {
  ...
  mlir::Builder b(&mlir_context);
  toolchain::SmallVector<int64_t, 2> global_shape = {1024, 1024};
  toolchain::SmallVector<int64_t, 2> tile_shape = {256, 256};
  toolchain::SmallVector<int64_t, 2> tile_strides = {1, 1};
  toolchain::SmallVector<int64_t, 2> layout = {1, 0};
  int element_byte_size = 4;
