...
#include "machina/xla/autotuning.pb.h"
#include "machina/xla/backends/gpu/codegen/emitters/ir/xla_gpu_ops.h"
#include "machina/xla/backends/gpu/codegen/emitters/transforms/passes.h"
#include "machina/xla/backends/gpu/codegen/triton/compilation_pipeline.h"
#include "machina/xla/backends/gpu/codegen/triton/dot_algorithms.h"
#include "machina/xla/backends/gpu/codegen/triton/emitter_helpers.h"
...
#include "machina/xla/backends/gpu/codegen/triton/tma_utils.h"
#include "machina/xla/backends/gpu/codegen/triton/transforms/passes.h"
#include "machina/xla/codegen/emitter_loc_op_builder.h"
#include "machina/xla/codegen/emitters/elemental_hlo_to_mlir.h"
#include "machina/xla/codegen/emitters/ir/xla_ops.h"
#include "machina/xla/codegen/emitters/transforms/passes.h"
...
#include "machina/xla/hlo/ir/hlo_print_options.h"
...
#include "machina/xla/service/gpu/model/symbolic_tile_analysis.h"
...
#include "machina/xla/service/gpu/model/triton_emitter_constraints.h"
...
#include "machina/xla/tools/hlo_decomposer.h"
...
absl::StatusOr<SmallVector<Value>> ComputeOffsetsForTile(
  ...
  dims.push_back(pid);
  for (const auto& [rt_var, value] : toolchain::zip(rt_vars, runtime_values)) {
    Value clamped_index =
...
absl::StatusOr<TileInfo> TileInfo::Construct(
  ...
  auto minor_to_major_layout = toolchain::to_vector(LayoutUtil::MinorToMajor(shape));
...
ScalarOrTensor BroadcastInDims(EmitterLocOpBuilder& b, ScalarOrTensor value,
  ...
  CHECK(toolchain::is_sorted(dims)) << "broadcast dims must be sorted";
...
absl::StatusOr<ScalarOrTensor> EmitReduce(
  ...
  toolchain::ArrayRef<int64_t> input_shape =
...
Value EmitTiledTranspose(EmitterLocOpBuilder& b, ArrayRef<int64_t> tile_sizes,
  ...
  SmallVector<int32_t> order = toolchain::to_vector_of<int32_t>(dimensions);
  ...
  Value normalized_input =
      trt.IsTranspose1Identity()
          ...
          : EmitTiledTranspose(b, transpose1_tile_sizes,
                               toolchain::to_vector(trt.transpose1_dims), input);
  ...
  return ScalarOrTensor{
      trt.IsTranspose2Identity()
          ...
          : EmitTiledTranspose(b, tiled_bitcast.tile_sizes(),
                               toolchain::to_vector(trt.transpose2_dims),
                               normalized_reshape)};
}
...
absl::StatusOr<Value> MaskDotOperand(EmitterLocOpBuilder& b,
  ...
  toolchain::ArrayRef<int64_t> tile_shape =
...
std::pair<SmallVector<int64_t>, SmallVector<int64_t>> CollapseUnitDims(
  ...
  for (auto [i, size] : toolchain::enumerate(shape)) {
    if (size != 1) {
...
absl::StatusOr<ScalarOrTensor> EmitConcatenate(
  ...
  int64_t limit = 0;
  for (auto [i, operand] : toolchain::enumerate(tiled_concatenate.operands())) {
...
absl::StatusOr<ScalarOrTensor> EmitPad(
  ...
  Value mask;
  for (auto [dim_index, sizes] : toolchain::enumerate(
           toolchain::zip(pad_input_shape, padded_tile_sizes, tile_offsets))) {
...
absl::StatusOr<ScalarOrTensor> EmitTiledHloInstruction(
  ...
  if (hlo->opcode() == HloOpcode::kTranspose) {
    ...
    return ScalarOrTensor(EmitTiledTranspose(
        b, tiled_hlo.tile_sizes(), toolchain::to_vector(transpose->dimensions()),
        values[tiled_hlo.operand(0)].UnwrapUnsafe()));
  }
...
absl::StatusOr<SmallVector<Value>> EmitGeneric(
  ...
  for (auto [root, result, parent_base_ptr] :
       toolchain::zip(tiled_hlo_computation.GetRoots(), results,
...
absl::StatusOr<std::unique_ptr<toolchain::Module>> TranslateLLVMToLLVMIR(
    toolchain::LLVMContext* llvmContext, mlir::ModuleOp module) {
  mlir::DialectRegistry registry;
...
void AppendFuncArgType(absl::Span<const int64_t> dims,
  ...
  if (fusion_kind == kTritonGemmFusionKind) {
    ...
    fn_arg_types.push_back(mlir::RankedTensorType::get(
        toolchain::ArrayRef<int64_t>(dims.data(), dims.size()),
        StorageType(ir_type)));
  }
}
...
absl::StatusOr<stream_executor::gpu::TmaMetadata> ExtractTmaMetadata(
  ...
  for (auto [idx, arg] : toolchain::enumerate(func_ops[0].getArguments())) {
...
absl::StatusOr<TritonWrapperResult> TritonWrapper(
    ...
    const se::DeviceDescription& device_info,
    const BlockLevelParameters& block_level_parameters,
    toolchain::Module* llvm_module, mlir::MLIRContext& mlir_context) {
...
absl::StatusOr<TritonWrapperResult> CompileTritonToLLVM(
    ...
    const se::DeviceDescription& device_info,
    const BlockLevelParameters& block_level_parameters,
    mlir::ModuleOp triton_module, toolchain::Module* llvm_module,
  ...
  std::optional<toolchain::raw_fd_ostream> log_stream;
  if (should_dump_mlir_passes) {
    ...
    if (!outputs_dir.empty()) {
      ...
      std::error_code err;
      log_stream.emplace(path, err, toolchain::sys::fs::OF_None);
      if (err) {
        log_stream.reset();
      ...
      LOG(ERROR)
          ...
          << "flag --xla_dump_to is set, so the toolchain dumps are disabled.";
    }
  }
  ...
  // toolchain::Linker::linkModules() segfaults if we don't strip locations.
  ...
  std::vector<toolchain::Metadata*> captured_nvvm_annotations;
  if (emit_kernel) {
    TF_ASSIGN_OR_RETURN(
        std::unique_ptr<toolchain::Module> ll_triton_module,
        TranslateLLVMToLLVMIR(&llvm_module->getContext(), triton_module));

    MACHINA_MACHINA_XLA_VLOG_LINES(5, llvm_ir::DumpToString(ll_triton_module.get()));
    if (should_verify) {
    ...
    TF_RET_CHECK(
        !toolchain::Linker::linkModules(*llvm_module, std::move(ll_triton_module),
                                   toolchain::Linker::Flags::OverrideFromSrc));

    MACHINA_MACHINA_XLA_VLOG_LINES(5, llvm_ir::DumpToString(llvm_module));
    if (should_verify) {
