...
SmallVector<NamedAttribute> GetExtraAttrs(LLVM::LLVMFuncOp func) {
  toolchain::StringSet<> registered_attr_names{
  ...
  return toolchain::to_vector(
      toolchain::make_filter_range(func->getAttrs(), [&](NamedAttribute attr) {
...
void StripParameterAddressSpaces(RewriterBase& rewriter,
  ...
  SmallVector<Type> generic_func_params(
      toolchain::map_range(func_ty.getParams(), [](Type type) -> Type {
  ...
  SmallVector<DictionaryAttr> arg_attrs(toolchain::map_range(
  ...
  for (auto [arg, type] :
       toolchain::zip(generic_func.getArguments(), func_ty.getParams())) {
    Value converted = arg;
    if (arg.getType() != type) {
      converted =
