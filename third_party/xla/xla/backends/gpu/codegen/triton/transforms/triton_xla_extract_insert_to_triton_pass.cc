...
#include "machina/xla/backends/gpu/codegen/triton/transforms/passes.h.inc"
...
bool AreRankedTensors(ArrayRef<Type> types) {
  return toolchain::all_of(types, [](mlir::Type type) {
...
bool CanUseTMA(::xla::EmitterLocOpBuilder& builder, bool tma_enabled,
  ...
  if (toolchain::any_of(global_shape, [&](uint64_t dim) {
  ...
  if (toolchain::any_of(tile_shape, [&](uint64_t dim) {
...
SmallVector<int32_t> ComputeBoundaryChecks(
  ...
  for (auto [dim_idx, sizes] :
       toolchain::enumerate(toolchain::zip(original_shape, tile_shape))) {
...
SmallVector<Value> ComputeResidualShape(::xla::EmitterLocOpBuilder& builder,
  ...
  for (auto [dim_idx, shape_and_tile_offset] :
       toolchain::enumerate(toolchain::zip(original_shape, tile_offsets))) {
...
Value ComputeLinearOffset(::xla::EmitterLocOpBuilder& builder,
                          ...
                          ValueRange offsets, toolchain::ArrayRef<int64_t> layout) {
...
bool IsNormalizedLayout(ArrayRef<int64_t> layout) {
  for (auto&& [idx, layout_entry] : toolchain::enumerate(layout)) {
...
SmallVector<T> NormalizeImpl(ArrayRef<T> values, ArrayRef<int64_t> layout) {
  if (IsNormalizedLayout(layout)) {
    return toolchain::to_vector(values);
  }

  auto reversed_layout = toolchain::to_vector(layout);
...
SmallVector<Value> Normalize(ValueRange values, ArrayRef<int64_t> layout) {
  SmallVector<Value> values_vec = toolchain::to_vector(values);
...
SmallVector<int32_t> GetInverseLayoutPermutation(ArrayRef<int64_t> layout) {
  ...
  std::reverse(reversed_layout.begin(), reversed_layout.end());
  auto permutation =
      toolchain::to_vector_of<int32_t>(::xla::InversePermutation(reversed_layout));
...
Value CreateMakeTensorPtrOp(::xla::EmitterLocOpBuilder& builder, Value ptr,
  ...
  return builder
      .create<MakeTensorPtrOp>(ptr, residual_shape, strides, zero_offsets,
                               toolchain::to_vector_of<int32_t>(tile_shape),
                               dim_order)
      .getResult();
}
...
class RewriteFuncOp : public mlir::OpRewritePattern<func::FuncOp> {
 ...
 private:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    for (auto&& [index, operand_type] : toolchain::enumerate(new_operand_types)) {
    ...
    for (auto [arg, arg_type] :
         toolchain::zip(entry_block->getArguments(), new_operand_types)) {
 ...
 private:
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (CanUseTMA(builder, tma_enabled_, *device_description_, tile_shape,
                  ...
                  op.getLayout())) {
      SmallVector<int64_t> strides = toolchain::to_vector(op.getStaticStrides());
      if (auto result =
  ...
  mlir::LogicalResult matchAndRewrite(
    ...
    if (CanUseTMA(builder, tma_enabled_, *device_description_, tile_shape,
      ...
      if (!IsNormalizedLayout(op.getLayout())) {
        ...
        auto transpose_order = toolchain::to_vector_of<int32_t>(op.getLayout());
...
class DeviceDescriptionParser
    : public toolchain::cl::parser<stream_executor::DeviceDescription> {
 public:
  using parser::parser;

  bool parse(toolchain::cl::Option& option, StringRef arg_name, StringRef arg_value,
  ...
  Option<stream_executor::DeviceDescription, DeviceDescriptionParser>
      device_description_{
          *this, "gpu_device_info",
          ::toolchain::cl::desc("Serialized stream_executor::GPUDeviceInfo proto")};
  Option<bool> tma_enabled_{*this, "tma_enabled",
                            ::toolchain::cl::desc("Flag to enable/disable TMA"),
