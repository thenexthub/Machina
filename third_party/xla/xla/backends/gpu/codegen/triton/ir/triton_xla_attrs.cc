...
void TmaDescriptorAttr::print(mlir::AsmPrinter& printer) const {
  ...
  toolchain::interleaveComma(getGlobalShape(), printer);
  printer << "], tile_shape = [";
  toolchain::interleaveComma(getTileShape(), printer);
  printer << "], tile_strides = [";
  toolchain::interleaveComma(getTileStrides(), printer);
  printer << "], layout = [";
  toolchain::interleaveComma(getLayout(), printer);
