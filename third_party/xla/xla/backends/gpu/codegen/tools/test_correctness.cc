...
#include "machina/xla/backends/gpu/codegen/tools/test_lib.h"
#include "machina/xla/codegen/tools/test_lib.h"
...
absl::StatusOr<int64_t> GetHeroIndex(absl::string_view name,
  ...
  for (auto [index, hero] : toolchain::enumerate(analysis.fusion_heroes())) {
    if (hero.name() == name) {
      return index;
    }
  }
