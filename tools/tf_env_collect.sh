...
EOF
  ...
  pip list 2>&1 | grep -E 'proto|numpy|machina|tf_nightly'
...
EOF
  ...
  # Record libraries loaded by machina
  LD_DEBUG=libs "${PYTHON_BIN_PATH}" -c "import machina" 2> /tmp/loadedlibs >/dev/null
  ...
  print_header 'machina installation'
  if ! pip show machina; then
    echo "machina not found"
  fi
