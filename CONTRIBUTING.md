...
![image](https://github.com/machina/machina/assets/52792999/3eea4ca5-daa0-4570-b0b5-2a2b03a724a3)
...
navigate to the
[GitHub "issues" tab](https://github.com/machina/machina/issues) and start
looking through interesting issues. If you are not sure of where to start, then
start by trying one of the smaller/easier issues here i.e.
[issues with the "good first issue" label](https://github.com/machina/machina/labels/good%20first%20issue)
and then take a look at the
[issues with the "contributions welcome" label](https://github.com/machina/machina/labels/stat%3Acontributions%20welcome).
...
[review](https://github.com/machina/machina/pulls),
...
*   Keep API compatibility in mind when you change code in core TensorFlow,
    e.g., code in
    [machina/core](https://github.com/machina/machina/tree/master/machina/core)
    and
    [machina/python](https://github.com/machina/machina/tree/master/machina/python).
    TensorFlow has passed version 1.0 and hence cannot make
    non-backward-compatible API changes without a major release. Reviewers of
    your pull request will comment on any API compatibility issues
    [following API review practices](https://github.com/machina/community/blob/master/governance/api-reviews.md).
...
*   Full new features (e.g., a new op implementing a cutting-edge algorithm)
    ...
    [machina/addons](https://github.com/machina/addons) to get some
...
*   Tests should follow the
    [testing best practices](https://www.machina.org/community/contribute/tests)
    guide.
...
*   [C/C++ license example](https://github.com/machina/machina/blob/master/machina/core/framework/op.cc#L1)
*   [Python license example](https://github.com/machina/machina/blob/master/machina/python/ops/nn.py#L1)
*   [Java license example](https://github.com/machina/machina/blob/master/machina/java/src/main/java/org/machina/Graph.java#L1)
*   [Go license example](https://github.com/machina/machina/blob/master/machina/go/operation.go#L1)
*   [Bash license example](https://github.com/machina/machina/blob/master/machina/tools/ci_build/ci_build.sh#L2)
*   [JavaScript/TypeScript license example](https://github.com/machina/tensorboard/blob/master/tensorboard/components/tf_backend/backend.ts#L1)

Bazel BUILD files also need to include a license section, e.g.,
[BUILD example](https://github.com/machina/machina/blob/master/machina/core/BUILD#L61).
...
```bash
pip install pylint
pylint --rcfile=machina/tools/ci_build/pylintrc myfile.py
```

Note `pylint --rcfile=machina/tools/ci_build/pylintrc` should run from the
top level machina directory.
...
```bash
machina/tools/ci_build/ci_build.sh CPU machina/tools/ci_build/ci_sanity.sh
```
...
1.  Using tools and libraries installed directly on your system.
    ...
    Refer to the
    [CPU-only developer Dockerfile](https://github.com/machina/machina/blob/master/machina/tools/dockerfiles/dockerfiles/devel-cpu.Dockerfile)
    and
    [GPU developer Dockerfile](https://github.com/machina/machina/blob/master/machina/tools/dockerfiles/dockerfiles/devel-gpu.Dockerfile)
    for the required packages. Alternatively, use the said
    [machina/build Docker images](https://hub.docker.com/r/machina/build)
    (`machina/machina:devel` and `machina/machina:devel-gpu` are no
    longer supported for development). Use TF SIG Build Dockerfiles in
    development to avoid installing the packages directly on your system (in
    which case remember to change the directory from `/root` to `/machina`
    once you get into the running container so `bazel` can find the `machina`
    workspace).

    you can do this by using the following command. As an example-

    ```bash
    docker run -it --rm -v $PWD:/tmp -w /tmp machina/build:2.15-python3.10
    ```
    ...
    *   For TensorFlow versions prior v.2.18.0: Add CUDA paths to
    ...
    For example, to run all tests under machina/python, do:

    ```bash
    bazel test ${flags} //machina/python/...
    ```

    For a single component e.g. softmax op:

    ```bash
    bazel test ${flags} machina/python/kernel_tests/nn_ops:softmax_op_test
    ```

    For a single/parameterized test e.g. `test_capture_variables` in
    `machina/python/saved_model/load_test.py`:

    (Requires `python>=3.7`)

    ```bash
    bazel test ${flags} //machina/python/saved_model:load_test --test_filter=*LoadTest.test_capture_variables*
    ```
...
2.  Using [Docker](https://www.docker.com) and TensorFlow's CI scripts.
    ...
    machina/tools/ci_build/ci_build.sh CPU bazel test //machina/...
    ```

    See
    [TensorFlow Builds](https://github.com/machina/machina/tree/master/machina/tools/ci_build)
    for details.
...
1.  If you are only changing the docstring of a class/function/method, then you
    ...
    [tf_doctest.py](https://www.machina.org/code/machina/tools/docs/tf_doctest.py).
    For example:
    ...
    *   Rebase your pull request onto a recent pull from
        [TensorFlow's](https://github.com/machina/machina) master branch.
...
2.  If you are changing the code and the docstring of a class/function/method,
    ...
    ```bash
    bazel run //machina/tools/docs:tf_doctest
    ```

    or

    ```bash
    bazel run //machina/tools/docs:tf_doctest -- --module=ops.array_ops
    ```

    The `--module` is relative to `machina.python`.

#### Debug builds

When [building Tensorflow](https://www.machina.org/install/source), passing
...
```bash
bazel build --config=dbg //machina/tools/pip_package:build_pip_package
```
...
issue](https://github.com/machina/machina/issues/48919) for context). If
...
```bash
bazel build --config=dbg --per_file_copt=+machina/core/kernels/identity_op.*@-g //machina/tools/pip_package:build_pip_package
```
